"""Forward Codex app-server notifications into Codify sessions."""
...
from codify.codex_native_elicitation import (
...
# in the bridge dir and posted to Codify as ``external_mcp_startup``; the
...
# Codify control event type sent when a Codex child thread is discovered.
...
class _CodexToolCall:
    ...
    :param call_id: Codex item id reused as the Codify call id, e.g.
        ``"call_abc"``.
    :param name: Codify function-call name, e.g. ``"shell"``.
...
class _CodexForwarderState:
    ...
    :param model: Latest known Codex model for this thread, e.g.
        ``"gpt-5.2-codex"``.
    :param posted_model: Last model already mirrored to Codify via an
    ...
    :param posted_effort: Last reasoning effort already mirrored to Codify
    ...
    :param parent_session_id: Codify parent session id, e.g.
    ...
    :param subagents_by_thread: Maps Codex child thread ids to Codify child
    ...
    :param synced_item_keys: Stable item keys already posted to Codify this
    ...
    :param posted_user_turns: Turn ids whose ``userMessage`` has been
        posted to Codify this connection, e.g. ``{"turn_123"}``. Used to
    ...
    :param prompted_plan_turns: Turn ids that already exposed the
        implementation prompt, either natively or through the Codify bridge.
    ...
    # Last context-compaction status mirrored to Codify
    ...
    def note_resume_response(self, response: CodexMessage) -> None:
        ...
        # Do NOT seed ``posted_model`` here. Codify must learn the session's
    ...
    def session_for_child_thread(self, thread_id: str) -> str | None:
        """
        Return the Codify child session id for a known Codex child thread.

        :param thread_id: Codex child thread id, e.g. ``"thread_child"``.
        :returns: Codify child session id, e.g. ``"conv_child"``, or ``None``
    ...
    def note_child_thread(self, thread_id: str, session_id: str) -> None:
        """
        Record the Codify child session id for a Codex child thread.

        :param thread_id: Codex child thread id, e.g. ``"thread_child"``.
        :param session_id: Codify child session id, e.g. ``"conv_child"``.
        :returns: None.
        """
    ...
    def claim_item_key(self, item_key: str) -> bool:
        """
        Claim a transcript item key for Codify posting.
...
class _CodexTerminalError:
    ...
    ``error`` ThreadItem. Forces the turn's Codify status to ``failed`` and
...
class _CodexTurnStatusEdge:
    """
    Codify session-status edge derived from Codex turn lifecycle state.

    :param status: Codify session status, e.g. ``"running"`` or ``"idle"``.
...
class _OutputTextDeltaCoalescer:
    ...
    Posting each one through Codify as an awaited HTTP request makes the
    ...
    :param client: HTTP client for Codify event posts.
...
class _PendingCodexElicitation:
    """
    Background Codify hook wait for one Codex server-to-client request.
    ...
    :param elicitation_id: Codify elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    """
...
class _CodexElicitationTaskTracker:
    ...
    Codify web/REPL hook does. If the forwarder awaits the Codify hook inline,
    ...
    def start(
        ...
        *,
        ...
        """
        Start one Codify hook bridge in the background.

        :param client: HTTP client for Codify hook posts.
    ...
    async def _post_resolved_once(
        ...
        """
        Post one Codify resolution signal, suppressing duplicates.
...
async def supervise_forwarder(
    *,
    ...
    """
    Mirror Codex app-server notifications into an Codify session.

    :param base_url: Codify server base URL, e.g.
        ``"http://127.0.0.1:6767"``.
    :param headers: Static HTTP headers for Codify requests.
    :param session_id: Codify conversation id, e.g. ``"conv_abc123"``.
    ...
    :param ap_transport: Optional HTTP transport for the Codify client,
    ...
    if client is None:
        client = client_for_transport(app_server_url, client_name="codify-codex-forwarder")
...
async def _maybe_rotate_session_on_thread_started(
    *,
    ...
    """
    Rotate Codify ownership when Codex starts a new native thread.

    Native Codex ``/clear`` starts a fresh app-server thread in the
    existing terminal. The forwarder must move the Codify session binding
    ...
    :param ap_client: Codify HTTP client used for session rotation.
    ...
    # begins. That event must not rotate the parent Codify session — the child
    # is discovered later via a ``collabAgentToolCall`` item and routed to
    # its own Codify child session by ``_handle_event``.
    ...
    _logger.info(
        "Codex forwarder rotated Codify session after native thread switch: "
...
async def _create_thread_replacement_session(
    *,
    ...
    """
    Create and activate the Codify session for a new native Codex thread.

    :param client: Codify HTTP client.
    ...
    :param new_thread_id: Newly started Codex thread id, e.g.
        ``"thread_new"``.
    :returns: New Codify session id, e.g. ``"conv_new"``.
    :raises httpx.HTTPStatusError: If Codify rejects the create, bind,
...
async def _fetch_session_snapshot(client: httpx.AsyncClient, session_id: str) -> dict[str, Any]:
    """
    Fetch an Codify session snapshot for Codex session rotation.

    :param client: Codify HTTP client.
    :param session_id: Codify session id, e.g. ``"conv_abc123"``.
    :returns: Decoded JSON session snapshot.
    :raises httpx.HTTPStatusError: If Codify rejects the request.
...
async def _subscribe_until_ready(
    ...
    *ready_signal* and only retries once the caller observes the thread
    ...
    :param ap_client: Codify HTTP client used for replayed items.
...
async def _post_resume_terminal_status(
    ...
    *,
    ...
    terminal, the forwarder can close the Codify session status even though no
...
def _resume_terminal_status_edge_for_latest_turn(
    ...
    """
    Return the Codify terminal status represented by the latest resume turn.
    ...
    for turn in reversed(turns):
        ...
        if state.active_turn_id is not None and state.active_turn_id != turn_id:
            return None
        status = _codify_status_from_resume_turn(turn)
        if status is None:
            return None
...
def _codify_status_from_resume_turn(turn: dict[str, Any]) -> str | None:
    """
    Convert an explicit Codex resume turn status to Codify session status.
    ...
    :returns: Codify status literal for terminal turns, or ``None`` for active
...
def _resolve_event_session(
    ...
    *,
    ...
    """
    Resolve which Codify session should receive a Codex event.
...
def _refresh_model_from_config(bridge_dir: Path, forwarder_state: _CodexForwarderState) -> None:
    ...
    :func:`~codify.codex_native_bridge.read_codex_config_model` (the
    ...
    it to Codify as ``model_override``. This mirror is a fallback to the codex
...
async def _sync_model_change(
    ...
    *,
    ...
    """
    Mirror a Codex TUI ``/model`` switch to Codify (web picker + cost gate).
    ...
    baseline, POST an
    ``external_model_change`` event so the Codify server persists
...
async def _sync_reasoning_effort_change(
    ...
    *,
    ...
    """
    Mirror Codex's active reasoning effort to Codify session metadata.
...
async def _sync_codex_collaboration_mode_change(
    ...
    *,
    ...
    """
    Mirror Codex's active collaboration mode kind to Codify labels.
    ...
    *,
    ...
    if method == "turn/started":
        ...
        if forwarder_state is not None:
            ...
            # on Codify before this turn's first tool call reaches the cost gate.
            _refresh_model_from_config(bridge_dir, forwarder_state)
            await _sync_model_change(
...
async def _handle_mcp_startup_status(
    ...
    *,
    ...
    text read it) and republishes the full per-server map to Codify so the
...
def _expected_mcp_servers_from_config(bridge_dir: Path) -> list[str]:
    ...
    ``codify`` relay server. Codex-internal servers that are not
    config-declared (e.g. ``codex_apps``) are not visible here and are
    simply absent from the synthesized round.

    :param bridge_dir: Native Codex bridge directory.
    :returns: Sorted enabled server names, e.g. ``["codify", "safe"]``.
...
async def _post_mcp_startup(
    ...
    """
    Post the current per-MCP-server startup map to Codify.
...
async def _handle_codex_elicitation_request(
    ...
    *,
    ...
    """
    Forward one Codex input request to Codify and reply to app-server.

    The Codify hook publishes the web elicitation and blocks until the
    ...
    Codify timed out or saw the upstream disconnect, so the forwarder
...
async def _codex_elicitation_hook_result(
    ...
    *,
    ...
    Empty 2xx responses mean Codify timed out or saw the upstream
...
async def _post_codex_elicitation_request(
    ...
    *,
    ...
    """
    POST a Codex server-to-client request to the Codify hook endpoint,
...
def _note_native_plan_implementation_prompt(
    ...
    the Codify bridge should relay that native request and skip its
...
async def _maybe_handle_plan_implementation_prompt(
    ...
    *,
    ...
    """
    Publish and resolve the Plan-mode implementation prompt in Codify Web.
...
def _turn_started_status_edge(
    ...
    """
    Record a Codex turn start and return the Codify running edge.
...
def _terminal_turn_status_edge(
    ...
    """
    Return the terminal Codify edge for a Codex terminal turn event.
...
def _claim_completed_item(
    ...
    """
    Claim one completed Codex transcript item for Codify posting.
...
async def _handle_completed_item(
    ...
    *,
    ...
    """
    Forward one Codex completed item event when it maps to Codify history.
    ...
    if item_type == "agentMessage":
        ...
        # Codify assigns each mirrored item a position by POST arrival order
...
async def _handle_collab_item(
    ...
    Registers newly discovered child threads and posts Codify status updates
...
def _parent_session_id_from_forwarder_state(
    ...
    """
    Return the parent Codify session id stored on the forwarder state.
...
async def _ensure_child_session(
    ...
    *,
    ...
    """
    Ensure a Codex child thread has an Codify child session row.

    Registers the child via ``_register_child_session`` when unknown,
    then backfills its history at most once per connection.

    :param client: HTTP client for Codify event posts.
    :param parent_session_id: Parent Codify session id, e.g. ``"conv_parent"``.
...
async def _register_child_session(
    ...
    *,
    ...
    :param child_thread_id: Codex child thread id, e.g. ``"thread_child"``.
    :param item: Codex child-spawn item.
    :returns: Codify child session id, or ``None`` on failure.
    """
...
def _extract_child_session_id(
    ...
    :param response: Codify HTTP response.
    :param child_thread_id: Codex child thread id for error logging.
    :returns: Codify child session id, or ``None`` when absent or malformed.
    """
...
async def _backfill_child_thread(
    ...
    *,
    ...
    :param client: HTTP client for Codify event posts.
    :param codex_client: Connected Codex app-server client.
    :param parent_session_id: Parent Codify session id, e.g.
        ``"conv_parent"``.
    :param child_session_id: Codify child session id, e.g.
        ``"conv_child"``.
...
async def _resume_child_thread_or_log(
    ...
    *,
    ...
    :param client: HTTP client for Codify status posts on failure.
    :param codex_client: Connected Codex app-server client.
    :param child_session_id: Codify child session id, e.g. ``"conv_child"``.
    :param child_thread_id: Codex child thread id, e.g.
        ``"thread_child"``.
...
async def _post_collab_agent_statuses(
    ...
    *,
    ...
    """
    Publish Codify status updates from a Codex collab-agent state snapshot.
    ...
    for thread_id, state in states.items():
        ...
        if child_session_id is None:
            continue
        ap_status = _codify_status_from_collab_state(state)
        if ap_status is not None:
            await _post_status(client, child_session_id, ap_status)


def _codify_status_from_collab_state(state: dict[str, Any]) -> str | None:
    """
    Convert a Codex collab-agent state dict to an Codify session status.

    :param state: Codex ``CollabAgentState`` dict, e.g.
        ``{"status": "running"}``.
    :returns: Codify status literal, e.g. ``"running"``, or ``None`` when
...
async def _handle_agent_message_delta(
    ...
    running. Codify normally persists only the completed ``agentMessage`` item,
...
async def _handle_plan_delta(
    ...
    Codify uses the same transient output-text delta channel for both,
...
async def _ensure_user_message_posted(
    ...
    normal claim/post path so it takes an earlier Codify position than the
...
async def _post_tool_item(
    ...
    *,
    ...
    """
    Mirror one completed Codex built-in tool call into Codify history.
...
async def _post_review_mode_marker(
    ...
    """
    Mirror a Codex review-mode enter/exit transition into Codify history.
...
_CODEX_SANDBOX_BYPASS_GUIDANCE = (
    "Codify: Codex's command sandbox could not start because this container "
...
# Codex built-in tool item types this forwarder mirrors into Codify history.
...
async def _post_external_elicitation_resolved(
    ...
    *,
    ...
    :param elicitation_id: Codify elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: ``True`` when Codify accepted the event.
    """
    response = await _post_session_event(
        client,
        session_id,
...
async def _post_compaction_status(
    ...
    *,
    ...
    """
    Mirror a Codex context-compaction edge to Codify (#1255).
...
async def _handle_reasoning_delta(
    ...
    ``item/reasoning/summaryTextDelta`` while it thinks. Codify has no
...
def _session_usage_data_from_params(params: dict[str, Any]) -> dict[str, int] | None:
    """
    Extract Codify session-usage fields from a Codex usage notification.
    ...
    if isinstance(cumulative_input_tokens, int) and cumulative_input_tokens >= 0:
        ...
        # ``response.completed``, so the Codify relay never accounts its cost.
...
class _ForwardHealth:
    """
    Process-level health of Codify session-event forwarding (#1120).
...
def _note_forward_failure(event_type: str) -> None:
    ...
    if (
        ...
        _logger.error(
            "codex-native forward sync degraded: %d consecutive Codify "
...
async def _replay_dead_letters_on_startup(
    ...
    :param ap_client: HTTP client for Codify event posts.
...
async def _post_session_event(
    ...
    *,
    ...
    """
    Post one Codify session event, tracking forward-sync health (#1120).
...
async def _post_session_event_inner(
    ...
    *,
    ...
    """
    Post one Codify session event with bounded transient retries.
...
def _is_final_post_attempt(attempt: int, max_attempts: int) -> bool:
    """
    Return whether an Codify event POST attempt is the final try.
...
def _log_post_transport_failure(event_type: str, exc: httpx.HTTPError, max_attempts: int) -> None:
    """
    Log an exhausted Codify session-event transport failure.
...
def _log_failed_session_event_post(
    ...
    :param response: Final Codify response, or ``None`` after transport
...
def _should_retry_post_status(status_code: int) -> bool:
    """
    Return whether an Codify event POST status is transient.
...
def _post_retry_delay(attempt: int) -> float:
    """
    Return the retry delay for a failed Codify event POST attempt.
...
def _params_with_turn_id(params: dict[str, Any], turn_id: str) -> dict[str, Any]:
    """
    Return params with a top-level ``turnId`` for Codify response ids.
...
def _streaming_message_id(params: dict[str, Any], item_type: str) -> str | None:
    """
    Build a stable Codify live-delta stream id for a Codex item.

    Codify Web uses this id to keep terminal-observed live text in a
...
def _response_id(params: dict[str, Any]) -> str:
    """
    Build a stable Codify response id for a Codex notification.
