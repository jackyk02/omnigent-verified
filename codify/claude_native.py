"""Native Claude Code terminal wrapper for the Codify CLI.

The wrapper deliberately treats Claude Code as a terminal-first
program. It creates or binds an Codify session, launches ``claude``
...
if TYPE_CHECKING:
    from codify.onboarding.provider_config import ProviderEntry
...
from codify import model_catalog
from codify._native_resume_hint import echo_native_resume_hint
from codify._runner_startup import RunnerStartupProgress, runner_startup_progress
from codify._startup_profile import StartupProfiler
from codify._terminal_picker_theme import (
...
from codify.claude_native_forwarder import (
    reset_transcript_forward_state,
    supervise_forwarder,
)
from codify.claude_native_state import (
    read_launch_state,
    redirect_launch_state,
    write_launch_state,
)
from codify.conversation_browser import conversation_url, open_conversation_link_if_enabled
from codify.entities.session_resources import terminal_resource_id
...
from codify.model_fallbacks import static_model_fallback
from codify.native_coding_agents import native_shell_terminal_spec
from codify.native_terminal import (
...
from codify.onboarding.provider_config import SUBSCRIPTION_KIND
from codify.terminals.ws_bridge import (
...
# other sessions inside the TUI.  Codify owns session switching in its own
...
_CLAUDE_STARTUP_PROFILE_ENV_VAR = "CODIFY_CLAUDE_STARTUP_PROFILE"
...
class PreparedClaudeTerminal:
    ...
    :param cold_resumed: ``True`` when we launched a fresh terminal
        against an existing Codify session (i.e. ``--resume <conv>`` with
    ...
    :param tmux_socket: Runner tmux server socket path when the
        ...
        e.g. ``Path("/tmp/codify-501/.../tmux.sock")``. ``None``
...
def resolve_claude_native_model_selection(
    ...
    """Resolve an Codify picker id to a model identifier Claude accepts.

    Claude Code understands its built-in family aliases directly, but the
    extra Sonnet 5 row uses Codify's ``sonnet_5`` id because it occupies
...
def build_native_claude_terminal_env(
    ...
    terminal stays pinned to the session the Codify UI is showing.
...
def run_claude_native(
    *,
    ...
    """
    Launch Claude Code in an Codify terminal and attach locally.

    :param server: Optional remote Codify server URL. ``None`` starts a
        local Codify server using the existing chat server machinery.
    ...
    :param claude_args: Args after ``claude``, e.g.
        ...
        ``-r`` is stripped defensively (Codify owns resume).
    ...
    # login — so `codify claude` honors the provider selection just like
    ...
    with TemporaryDirectory(prefix="codify-claude-native-") as tmpdir:
...
def _resolve_session_id_for_resume(
    *,
    ...
    if not resume_picker:
        return None
    # Deferred — codify_client / repl pull in heavy graphs we don't want at startup.
    from codify_client import CodifyClient

    from codify.repl._resume_picker import pick_conversation_by_wrapper_label_from_sdk

    async def _drive() -> str | None:
        async with CodifyClient(base_url=base_url, headers=headers if headers else None) as client:
...
def _align_working_directory_with_session(
    ...
    *,
    ...
    :mod:`codify.claude_native_state`); this helper reads it back
    ...
    * A user resuming from a different machine has no recorded
    ...
    :param session_id: Resolved Codify conversation id, e.g.
        ``"conv_abc123"``.
    :param base_url: Codify server base URL used to look up Claude's
...
def _fetch_external_session_id_for_redirect(
    *,
    ...
    :param base_url: Codify server base URL, or ``None`` when unavailable.
    :param headers: HTTP headers for the Codify request.
...
def _record_launch_for_fresh_session(session_id: str) -> None:
    ...
    :param session_id: Newly created Codify conversation id, e.g.
        ``"conv_abc123"``.
    :returns: None.
    """
    try:
...
def _strip_resume_from_claude_args(args: tuple[str, ...]) -> tuple[str, ...]:
    ...
    ``codify claude -- --resume <id>``) must not have it reach
    ...
    for arg in args:
        ...
        if arg in ("--resume", "-r"):
            _logger.warning(
                "Stripped stray %s from claude args; use `codify claude --resume`.", arg
            )
            consume_value = True
            continue
...
def _ucode_config_for_profile(
    ...
    *,
    ...
    from codify.onboarding.databricks_config import get_workspace_url_for_profile
    from codify.onboarding.ucode_state import read_ucode_state
    ...
    if agent_state is None:
        raise click.ClickException(
            ...
            "Run `codify setup --internal-beta` to refresh ucode configuration."
        )
    ...
    if refresh_models:
        ...
        try:
            from codify.databricks_model_discovery import (
                discover_databricks_claude_models,
            )
            from codify.runtime.credentials.databricks import (
...
def _provider_config_for_native_claude(entry: ProviderEntry) -> ClaudeNativeUcodeConfig | None:
    ...
    model — so a Claude Code terminal launched by ``codify`` routes
    through the configured provider exactly like the in-process claude-sdk
    harness does (:func:`codify.runtime.workflow.configure_agent_harness_with_provider`).
    ...
    """
    from codify.onboarding.provider_config import ANTHROPIC_FAMILY
...
def resolve_native_claude_config(
    *,
    ...
    ``codify claude`` and the runner's host-spawned auto-create), so the
    native harness honors ``codify setup`` exactly like the in-process
    claude-sdk harness. Precedence mirrors
    :func:`codify.runtime.workflow._resolve_provider_for_build`:
    ...
    2. when spec-less (``codify claude``): an explicit per-family default
    ...
    ``codify setup`` provider config — there is no CLI/env profile
    override.

    :param spec: The agent spec, or ``None`` for the bare ``codify
        claude`` launch.
    :returns: The launch config, or ``None`` to use Claude's own login.
    """
    ...
    from codify.runtime.workflow import _load_global_auth, _resolve_provider_for_build
    ...
    # 2. Spec-less (codify claude): explicit default wins first.
    ...
    _logger.info(
        ...
        "harness, no Databricks profile). Run `codify setup --no-internal-beta` to route "
        "through a provider."
    )
    return None
...
def _materialize_claude_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first session agent spec used by ``codify claude``.
    ...
    raw = {
        ...
        # ``codify/runner/app.py``. Claude Code already operates
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local Codify server, launch Claude, and attach to it.
    ...
    startup_profiler = startup_profiler or StartupProfiler(name="codify claude", enabled=False)
    ...
    try:
        ...
        if resolved_session_id is not None:
            ...
            # independent (writes to ``~/.codify/``), so chdiring
            # now is safe.
...
class _AttachOutcome(Enum):
    ...
    :cvar EXITED: The user quit (stdin EOF / Ctrl-D), the terminal is
        ...
        launcher tears down the runner and Codify terminal resource.
...
def _can_attach_direct_tmux(prepared: PreparedClaudeTerminal) -> bool:
    ...
    :func:`codify.codex_native._can_attach_direct_tmux`.
...
async def _attach_direct_tmux(
    ...
    *,
    ...
    environment so a user who runs ``codify claude`` from inside
    their own tmux can still attach to Codify' server. After the
    ``tmux attach`` child exits, a ``has-session`` probe distinguishes a
    user *detach* (session still alive → keep the Codify terminal resource
    ...
    """
    from codify.terminals.ws_bridge import _check_pane_dead_definitive, _tmux_session_alive
...
async def _attach_with_transcript_forwarder(
    *,
    ...
    :param base_url: Codify server base URL.
    :param headers: Static HTTP auth headers for Codify requests. For
    ...
    :param auth: Optional httpx Auth that mints a fresh bearer token
        ...
        transcript forwarder's HTTP client so Codify posts continue to
    ...
    # offset 0 contains turns Codify already has from the previous run).
    ...
    try:
        ...
        if forwarder is not None:
            ...
            try:
                ...
                # uncaught in the parser) must not skip the Codify terminal
        ...
        # the Codify terminal resource must stay live (the web UI keeps
...
async def _attach_with_reconnect(
    *,
    ...
    :param base_url: Codify server URL for the post-close terminal probe;
    ...
    :param active_session_id_reader: Optional callback that returns
        the latest active Codify session id, e.g. ``"conv_new"``. This is
            ...
            if (
                ...
                async def _terminal_gone_probe(
                    *,
                    ...
                    :returns: ``True`` when the Codify terminal resource
...
async def _is_terminal_resource_gone(
    *,
    ...
    :param base_url: Codify server base URL.
    :param headers: HTTP auth headers for the Codify server. Mutated in
...
async def _sleep(seconds: float) -> None:
    ...
    loop — see ``codify-testing`` skill rule 14 for why globally
...
# (``codify host``) rather than this CLI: ensure the daemon, ask
...
async def _wait_for_claude_terminal_ready(
    ...
    *,
    ...
    :param client: HTTP client pointed at the Codify server.
...
async def _prepare_claude_terminal_via_daemon(
    *,
    ...
    :param base_url: Codify server base URL.
    :param headers: Static HTTP auth headers for Codify requests.
    ...
    """
    from codify.claude_native_bridge import bridge_dir_for_conversation_id
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Claude on a remote Codify server via the connect daemon.
    ...
    :param base_url: Remote Codify server base URL without a trailing
    ...
    :returns: None.
    """
    from codify.chat import _bundle_agent, _remote_headers, _server_auth
    from codify.cli import _ensure_host_daemon
    ...
    try:
        ...
        with runner_startup_progress(initial_message="Preparing Claude...") as progress:
            ...
            try:
                ...
                # TCP connection — the Codify server at this URL isn't reachable.
                # Fail loud with the URL instead of a raw httpx traceback.
                raise click.ClickException(
                    f"Could not reach the codify server at {base_url}. "
...
async def _prepare_claude_terminal(
    *,
    ...
    :param base_url: Codify server base URL.
    :param headers: Static HTTP auth headers for the Codify server.
    :param session_id: Optional existing session id.
...
async def _fetch_claude_session_labels(
    ...
    """
    Fetch labels for an existing Claude-native Codify session.

    :param client: HTTP client for the Codify server.
    :param session_id: Codify conversation id, e.g. ``"conv_abc123"``.
    ...
    if resp.status_code == 404:
        raise click.ClickException(
            ...
            "Run `codify claude` (no --resume) to start a new session.",
        )
    if resp.status_code >= 400:
        raise click.ClickException(
...
async def _resolve_cold_resume_args(
    ...
    if transcript is None:
        ...
        # starting. Launch fresh — the Codify conv survives.
        message = (
...
async def _ensure_local_claude_resume_transcript(
    ...
    *,
    ...
    Cross-machine resume has the Codify conversation and Claude external
    ...
    wrapper always rewrites it from committed Codify items before launch so
    Codify remains the source of truth when a previous local Claude JSONL
    has diverged.

    :param client: HTTP client pointed at the Codify server.
    :param session_id: Codify conversation id, e.g.
        ``"conv_abc123"``.
    ...
    :param workspace: Resolved directory Claude will run in — its
        ...
        ``CODIFY_RUNNER_WORKSPACE``. Pass an already-resolved
    ...
    :returns: Path to the local transcript that was written; ``None`` if
        *external_session_id* is not a safe transcript stem, or if the AP
        ...
        fresh).
    :raises click.ClickException: If Codify history cannot be fetched or
...
async def _resolve_session_item_file_references(
    ...
    *,
    ...
    """
    from codify.inner.native_attachments import has_unresolved_file_id, resolve_file_id_block
...
def _claude_transcript_records_from_session_items(
    ...
    *,
    ...
    """
    Convert Codify session items into Claude Code transcript records.

    :param items: Flat Codify item dicts in chronological order, e.g.
...
def _claude_transcript_record_from_session_item(
    ...
    *,
    ...
    """
    Convert one Codify item into one Claude transcript record.

    :param item: Flat Codify item dict, e.g.
...
def _synthetic_claude_transcript_uuid(
    *,
    ...
    :param external_session_id: Claude-native session id, e.g.
        ``"02857840-6362-408f-b41f-309e396ed7c6"``.
    :param item: Codify item dict. ``id`` is used when present.
    ...
    return str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"codify-claude-resume:{session_id}:{external_session_id}:{stable_item_id}",
        )
    )
...
def _claude_user_content_from_api_blocks(
    ...
    """
    Convert Codify user message blocks into Claude message content.
    ...
    :param content: Codify ``content`` value, e.g.
...
def _claude_attachment_text_blocks_from_api_content(
    ...
    :param content: Codify content array, e.g.
    ...
    """
    from codify.inner.native_attachments import attachment_reference_line
...
def _claude_assistant_content_from_api_blocks(content: object) -> list[dict[str, Any]] | None:
    """
    Convert Codify assistant message blocks into Claude text blocks.
...
def _claude_text_blocks_from_api_content(
    ...
    *,
    ...
    """
    Extract text blocks from an Codify content array.

    :param content: Codify content array, e.g.
        ``[{"type": "input_text", "text": "hello"}]``.
    :param api_type: Codify block type to include, e.g.
...
def _json_object_from_string(value: object) -> dict[str, Any]:
    ...
    :param value: JSON string from an Codify function-call item, e.g.
        ``"{\"file_path\":\"README.md\"}"``.
...
def _strip_unparseable_image_output(output: str) -> str:
    ...
    *looks* like an image but no longer parses as JSON — the shape produced when
    ...
    try:
        ...
        from codify.runtime.prompt import _image_omitted_placeholder
...
def _preflight_local_tools(command: str) -> None:
    ...
    if shutil.which(command) is None:
        raise click.ClickException(
            ...
            "--server selects the Codify server only; Claude still runs locally."
        )
...
async def _create_claude_session(
    ...
    *,
    ...
    ``codify.wrapper = claude-code-native-ui`` label until the
...
async def _launch_claude_terminal(
    ...
    *,
    ...
    :param client: HTTP client pointed at the Codify server. Its
        ``base_url`` and ``headers`` are reused as the
        ``PermissionRequest`` command hook's Codify URL and auth. The hook
...
class _ClaudeTerminalTmux:
    ...
    :param socket: tmux server socket path the runner advertised in
        ...
        ``Path("/tmp/codify-501/.../tmux.sock")``. ``None`` when
        absent.
...
def _claude_terminal_request(
    ...
    *,
    ...
    :param ap_server_url: Codify server base URL passed through to
    ...
    # default. See codify.claude_launcher.
    ...
    spec: dict[str, Any] = {
        ...
        # deployments; ``codify claude`` is a local-only
...
async def attach_local_terminal(
    ...
    *,
    ...
    """
    Attach the local TTY to an Codify terminal WebSocket.
    ...
    :param terminal_gone_probe: Optional async callback returning
        ``True`` once the Codify terminal resource is stopped. When set,
...
async def _close_ws_when_terminal_gone(
    ...
    *,
    ...
    """
    Close the client WebSocket when the Codify terminal resource stops.
...
def _websocket_connect(attach_url: str, *, headers: dict[str, str]) -> Any:
    ...
    from codify.runner.identity import CODIFY_INTERNAL_WS_ORIGIN
    from codify.tls import client_ssl_context
    ...
    handshake_headers = {**headers, "Origin": CODIFY_INTERNAL_WS_ORIGIN}
    ...
    # library default — no TLS). See codify/tls.py and issue #1730.
...
def claude_terminal_resource_id() -> str:
    """
    Return the deterministic terminal id used by ``codify claude``.
