"""TLS trust for codify's own outbound client connections.

codify's uv / python-build-standalone interpreter ships OpenSSL with no
...
verifying client SSL context from it. Use :func:`client_ssl_context` for codify's
own outbound websocket/HTTP clients.

This is distinct from :mod:`codify.inner.egress.ca`, which *manufactures* a
