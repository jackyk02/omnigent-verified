...
from codify.onboarding.gemini_auth import gemini_auth_has_credential
...
# so there is no per-tool / web-routed gate. Codify therefore maps its
# ``permission_mode`` onto this single flag.
_SKIP_PERMISSIONS_FLAG = "--dangerously-skip-permissions"
# Codify permission mode that maps to agy's all-or-nothing bypass.
...
def should_skip_permissions(
    ...
    * the launch is **headless** — no interactive client will attach to answer
    ...
    Codify elicitation by the RPC read driver's interaction bridge (see
    :mod:`codify.antigravity_native_reader` /
    :mod:`codify.antigravity_native_interactions`). agy exposes no firing
    pre-tool hook, so Codify cannot pre-empt a tool before it runs; the
    ...
    .. warning:: The ``headless`` argument is currently derived from the
       ...
       :mod:`codify.antigravity_native`). That is sound only while
    ...
    :param permission_mode: The session's effective Codify permission mode,
...
def build_agy_launch(
    ...
    **Permission mode → agy flag.** ``--dangerously-skip-permissions`` is
    ...
    cannot be routed to the web UI or made per-tool (the genuine Codify policy
