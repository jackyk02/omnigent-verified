...
functions and surfaced through the ``codify_client``
...
(``examples/frontends/terminal.py``, ``codify chat``'s raw-schema
path) keep working without modification.

Used by ``codify chat --tools coding`` and the terminal TUI.
"""
...
from codify_client.tools import build_tool_handler, tool
...
# ``codify chat`` ``_load_tool_handler`` path reach into modules
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    ...
    Used by ``examples/frontends/terminal.py`` and ``codify chat``'s
    raw-schema path. New consumers should construct a
    :class:`~codify_client.tools.ToolHandler` via
    :func:`~codify_client.tools.build_tool_handler` against
