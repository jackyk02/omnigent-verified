"""
Async-dispatch demo tool set for ``codify run --tools async_demo``.
...
Registered via :func:`codify.client_tools.get_tool_set`,
which ``codify run`` calls when ``--tools async_demo`` is
passed.
"""
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    """
    Execute a client-side tool call from ``codify run``.
