...
``CODIFY_DISABLE_TEST_GUARDRAILS`` to a truthy value (``1``, ``true``,
...
# set per-test, not at session configure time). CODIFY_TEST_MODE is
# introduced by this module as the canonical, settable flag.
_TEST_MODE_ENV_VARS = ("CODIFY_TEST_MODE", "CODIFY_ENV")
_TRUTHY_ENV_VALUES = frozenset({"1", "true", "yes", "on"})
_TEST_MODE_ENV_VALUES = _TRUTHY_ENV_VALUES | {"test", "testing"}
_DISABLE_GUARDRAILS_ENV_VAR = "CODIFY_DISABLE_TEST_GUARDRAILS"
...
def looks_like_test_db(db_uri: str) -> bool:
    ...
    path/name token. Everything else (a real ``~/.codify/chat.db``, a Postgres
...
def check_test_environment(
    *,
    ...
    ``CODIFY_DISABLE_TEST_GUARDRAILS`` is truthy, in which case
    ...
    if not looks_like_pytest(env):
        violations.append(
            ...
            "(no PYTEST_CURRENT_TEST / CODIFY_TEST_MODE and pytest not imported)"
        )
