...
Layout (per bridge id):
    ...
    ~/.codify/opencode-native/<sha256(bridge_id)[:32]>/
...
# codex-native ``codify.codex_native.bridge_id`` label.
OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY = "codify.opencode_native.bridge_id"
...
# Token file the shared ``codify.claude_native_bridge serve-mcp`` reads to
...
# Codify builtin tools (``sys_*``/``load_skill``/``web_fetch``) advertised in
...
# the elicitation against the Codify server (mirrors codex-native's
# ``policy_hook.json``; consumed by ``codify.native_cost_popup``).
_COST_POPUP_CONFIG_FILE = "cost_popup.json"
# Filename of the opencode plugin that bridges opencode's lifecycle hooks to the
# Codify policy engine (REQUEST + TOOL_RESULT phases the reactive
# ``permission.asked`` path can't reach).
_POLICY_PLUGIN_FILE = "codify-policy.js"
...
# function exports as plugins (legacy shape). It reads its Codify coordinates
...
// Codify policy bridge for opencode-native (generated; do not edit).
// Forwards opencode lifecycle hooks to the Codify policy engine so
...
const BASE = (process.env.CODIFY_POLICY_URL || "").replace(/\/+$/, "");
const SESSION = process.env.CODIFY_SESSION_ID || "";
const RELAY_FILE = process.env.CODIFY_RELAY_FILE || "";
// Full routing header map baked by the runner for the direct-server fallback.
let POLICY_HEADERS = {};
try {
  POLICY_HEADERS = JSON.parse(process.env.CODIFY_POLICY_HEADERS || "{}") || {};
} catch (e) {
  POLICY_HEADERS = {};
}
...
export const CodifyPolicyPlugin = async () => ({
  ...
  "chat.message": async (_input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      ...
      throw new Error(
        "Codify policy blocked this prompt: " + (verdict.reason || "request denied"),
      );
    }
  },
  ...
  "tool.execute.after": async (input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      output.output = "[Codify policy withheld this tool result: " +
...
def write_opencode_policy_plugin(bridge_dir: Path) -> Path:
    """
    Write the Codify policy-bridge plugin into *bridge_dir* and return its path.

    The runner registers the returned path in the synthesized ``opencode.json``
    ``plugin`` field and stamps ``CODIFY_POLICY_URL`` / ``CODIFY_SESSION_ID``
    / ``CODIFY_POLICY_HEADERS`` on the ``opencode serve`` process so the plugin
...
_STATE_VERSION = 1
_BRIDGE_ROOT = Path.home() / ".codify" / "opencode-native"
_ID_HASH_CHARS = 32
...
def write_relay_bridge_config(bridge_dir: Path) -> None:
    ...
    The shared ``codify.claude_native_bridge serve-mcp`` stdio server (which
...
def write_cost_popup_config(
    ...
    ``codify.native_cost_popup`` subprocess inside a ``tmux display-popup`` on
    the opencode pane; it must POST the verdict to the Codify server but cannot
    ...
    :param bridge_dir: OpenCode-native bridge directory.
    :param ap_server_url: Codify server base URL, e.g. ``"http://127.0.0.1:6767"``.
...
def update_model_override(bridge_dir: Path, model_override: str | None) -> bool:
    """
    Persist a new per-session model override (Codify→opencode model switch).
