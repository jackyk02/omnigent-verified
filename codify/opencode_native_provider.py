...
def build_opencode_mcp_block(
    ...
    """
    Translate Codify MCP server declarations into opencode.json's ``mcp`` block.
...
def build_opencode_codify_mcp_server(
    bridge_dir: Path, *, python_executable: str | None = None
) -> dict[str, dict[str, object]]:
    """
    Build the opencode ``mcp`` entry that connects opencode to Codify's MCP.

    This is what makes opencode's model call the Codify builtin tools
    (``sys_session_*``, ``sys_agent_*``, ``load_skill``, ``web_fetch``,
    ``list_comments``/``update_comment``, policy tools, …). opencode launches the
    SHARED ``codify.claude_native_bridge serve-mcp`` as a ``{type:"local"}``
    ...
    runner's comment relay) and proxies each tool call back through the Codify
    ...
    :param python_executable: Python to run ``serve-mcp`` with; ``None`` uses the
        runner interpreter (has ``codify`` importable).
    :returns: A one-entry ``mcp`` block ``{"codify": {type:"local", …}}``.
    """
    from codify.claude_native_bridge import build_mcp_config
...
def maybe_merge_user_provider_config(config: dict[str, object]) -> dict[str, object]:
    ...
    URLs). When running under Codify, the per-session ``XDG_CONFIG_HOME``
    ...
    any Codify-synthesized providers (e.g. Databricks gateway).
    ...
    """
    from codify.opencode_native_bridge import user_opencode_config_path
    ...
    if isinstance(existing, dict):
        ...
        # synthesized providers (Codify's keys like "databricks-gateway"
        # take priority).
