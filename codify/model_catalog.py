...
precedence as :func:`codify.runtime.workflow._resolve_provider_for_build`
...
(:func:`codify.model_override.normalize_model_for_provider`).
...
from codify._platform import default_shell_argv
from codify.llms.anthropic_model_metadata import parse_anthropic_model_metadata
from codify.model_fallbacks import StaticModelFallback, static_model_fallback
from codify.model_metadata import (
    ModelCapability,
    ModelCostTier,
    ModelIntent,
    ModelMetadata,
...
from codify.model_override import model_family_mismatch
from codify.model_resolver import (
    ModelResolution,
...
from codify.pi_model_compatibility import unsupported_in_pi
from codify.runtime.credentials.databricks import resolve_databricks_workspace

if TYPE_CHECKING:
    from codify.onboarding.providers import ModelInfo
...
# ``codify/llms/adapters/anthropic.py``).
...
# codify-side provider config for it to resolve (or fail), so resolution
...
def is_direct_openai_provider(provider: ResolvedModelProvider) -> bool:
    ...
    if provider.family != OPENAI_FAMILY or not provider.base_url:
        return False
    from codify.onboarding.configure_models import default_base_url_for_family
...
def model_family_token(model_id: str) -> str:
    ...
    :func:`codify.model_override.model_family_mismatch`: Claude ids
...
def catalog_model_entries(provider_name: str) -> tuple[ModelEntry, ...]:
    ...
    :func:`codify.onboarding.providers.get_chat_models`. This adapter keeps
...
def resolve_model_provider(spec: object, harness: str | None) -> ResolvedModelProvider:
    ...
    try:
        ...
        # CodifyError text is this codebase's own (secret-free); anything
        # else is redacted to its type name — raw detail stays at DEBUG.
        reason = str(exc) if isinstance(exc, CodifyError) else type(exc).__name__
...
def _resolve_model_provider_unsafe(spec: object, harness: str | None) -> ResolvedModelProvider:
    ...
    Step 1 reuses :func:`~codify.runtime.workflow._resolve_provider_for_build`
    ...
    from codify.runtime.workflow import _resolve_provider_for_build
...
def _legacy_claude_sdk_provider(spec: AgentSpec) -> ResolvedModelProvider:
    ...
    from codify.spec.types import ApiKeyAuth, DatabricksAuth
...
def list_models_for_worker(
    ...
    *,
    ...
    family rule from :func:`~codify.model_override.model_family_mismatch`
