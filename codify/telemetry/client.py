...
server run — it is NOT the Codify conversation id (which goes in
...
On startup a daemon thread fetches a JSON config from a versioned URL::
    ...
    https://config.codify-telemetry.io/{version}.json          (prod)
    https://config-staging.codify-telemetry.io/{version}.json  (dev/pre-release)

The config shape::

    {
        "codify_version": "0.5.0",
...
_CONFIG_URL_PROD = "https://config.codify-telemetry.io"
_CONFIG_URL_STAGING = "https://config-staging.codify-telemetry.io"
...
def _config_url() -> str:
    ...
    ``CODIFY_TELEMETRY_CONFIG_URL`` overrides for local testing.
    Dev/pre-release versions use the staging URL.
    """
    override = os.environ.get("CODIFY_TELEMETRY_CONFIG_URL", "").strip()
    if override:
...
def _fetch_remote_config() -> TelemetryConfig | None:
    ...
    try:
        ...
        if cfg.get("codify_version") != VERSION:
...
def _config_telemetry_disabled() -> bool:
    ...
    Reads ``~/.codify/config.yaml`` (honouring ``CODIFY_CONFIG_HOME``).
    ...
    try:
        ...
        if config_home:
            ...
            config_path = Path.home() / ".codify" / "config.yaml"
...
def is_disabled() -> bool:
    ...
    Checks (in order):
    1. ``CODIFY_ANALYTICS=0``
    2. ``DISABLE_TELEMETRY=true`` or ``CODIFY_DISABLE_TELEMETRY=true``
    3. ``DO_NOT_TRACK=1``
    4. Any CI environment variable from :data:`_CI_ENV_VARS`
    5. ``telemetry: false`` in ``~/.codify/config.yaml``
...
def _compute_is_disabled() -> bool:
    ...
    if os.environ.get("CODIFY_ANALYTICS", "").strip() == "0":
        return True
    for var in ("DISABLE_TELEMETRY", "CODIFY_DISABLE_TELEMETRY"):
...
def init_client(*, config: Mapping[str, object] | None = None) -> None:
    ...
    try:
        from codify.telemetry.installation_id import get_installation_id
