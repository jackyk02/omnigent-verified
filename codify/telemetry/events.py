...
Each dataclass is passed to :func:`codify.telemetry.emit`.  The client
...
class SessionCreatedEvent:
    ...
    :param installation_id: Server-side installation ID (top-level in wire
        format; see :mod:`codify.telemetry.client`).
    :param session_id: Codify conversation/session identifier (goes into
        ``params``).
    ...
    :param host_installation_id: Installation ID of the host machine
        (``codify host``); ``None`` for CLI sessions.
...
class SessionStoppedEvent:
    ...
    :param session_id: Codify conversation/session identifier.
    :param anon_user_id: First 16 hex chars of ``sha256("<installation_id>:<user_id>")``.
    """
...
class PolicyRegisteredEvent:
    ...
    :param handler: Registered handler path (e.g.
        ``"codify.policies.blast_radius"``) or HTTPS endpoint URL for
        ``type="url"`` policies.
