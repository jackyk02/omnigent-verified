"""CLI-side auth storage for ``codify login``.

Persists per-server auth state in ``~/.codify/auth_tokens.json``
...
- **Databricks Apps pointer records**
  ...
  ``codify login <apps-url>``. These deliberately store NO token:
...
def _token_file_path() -> Path:
    ...
    Uses the shared ``~/.codify`` state directory.

    :returns: Path to ``~/.codify/auth_tokens.json``.
    """
    from codify_ui_sdk.terminal._config import state_dir
...
def _write_tokens_file(path: Path, data: dict[str, dict[str, str | float]]) -> None:
    ...
    :param path: Destination file, i.e. ``~/.codify/auth_tokens.json``.
...
def _store_entry(server_url: str, entry: dict[str, str | float]) -> None:
    ...
    Writes ``~/.codify/auth_tokens.json`` with user-only
...
DATABRICKS_EXTRA_HEADERS_ENV_VAR = "CODIFY_DATABRICKS_EXTRA_HEADERS"
...
def databricks_request_headers(
    ...
    when ``codify login https://<host>/?o=<id>`` recorded a selector, and
