...
from codify.codex_native_bridge import write_policy_hook_config
from codify.codex_native_process_registry import (
...
from codify.inner.codex_executor import (
    _clean_codex_env,
    _codex_cli_version,
...
from codify.inner.databricks_executor import _databricks_gateway_host
...
# Codify policy hook. Codex discovers it as a ``user``-layer hook
...
# might declare.
_POLICY_HOOK_MODULE = "codify.codex_native_hook"
...
def _toml_table_header_name(line: str) -> str | None:
    ...
    :param line: One config line, e.g.
        ``"[mcp_servers.codify] # generated\n"``.
    :returns: The table name, e.g. ``"mcp_servers.codify"``, or
...
def _remove_toml_table(text: str, table_name: str) -> str:
    ...
    Codify MCP server table. This avoids accumulating duplicate
    ``[mcp_servers.codify]`` sections across terminal relaunches.
...
def _codex_mcp_server_config_section(
    ...
    :param python_executable: Python executable for serve-mcp, e.g.
        ...
        :data:`sys.executable`.
    :returns: TOML text for ``[mcp_servers.codify]`` and its
    ...
    return (
        f"[mcp_servers.codify]\n"
        f"command = {json.dumps(python)}\n"
        f"args = [{args_toml}]\n\n"
        "[mcp_servers.codify.tools.sys_session_rename]\n"
...
def _sync_codex_developer_instructions(
    ...
    config_path = codex_home / "config.toml"
    base_path = codex_home / ".codify-developer-instructions-base"
    if config_path.is_symlink():
        target = config_path.resolve()
        config_path.unlink()
        if target.is_file():
    ...
    if base_path.exists():
        ...
        # A previous Codify build may have appended the same framework
...
def _inject_mcp_server_config(
    ...
    """
    Upsert Codify MCP server config into ``config.toml``.

    Writes a ``[mcp_servers.codify]`` section that points Codex
    ...
    if config_path.exists():
        ...
        existing = ""
    updated = _remove_toml_table(existing, "mcp_servers.codify")
...
class CodexAppServerClient:
    ...
    :param client_name: App-server client name for the initialize
        handshake, e.g. ``"codify-codex-native"``.
    """
    ...
    async def respond(self, request_id: int | str, result: CodexParams) -> None:
        ...
        The forwarder handles those requests through Codify and replies
...
async def discover_codex_model_options(*, codex_path: str | None = None) -> list[_JsonObject]:
    ...
    with tempfile.TemporaryDirectory(prefix="codify-codex-model-discovery-") as raw_dir:
        root = Path(raw_dir)
        ...
        try:
            ...
            client = CodexAppServerClient(
                ws_url=listen_url,
                client_name="codify-codex-model-discovery",
            )
...
class CodexNativeAppServer:
    ...
    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.codify/codex-native/<hash>")``. The policy hook
        subprocess is pointed at it via ``--bridge-dir`` and reads the
        session id + Codify coordinates from it.
    :param ap_server_url: Codify server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        and trusts the hook but writes no Codify coordinates, so the hook
        no-ops (no enforcement) until coordinates exist.
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        Codify requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    ...
    :param policy_notice_pending: One-shot flag: ``True`` once a degrade
        ...
        surfaces it to Codify (which posts a single durable banner). Prevents
    ...
    async def start(self) -> None:
        ...
        if codex_version is not None and codex_version < _MIN_POLICY_HOOK_CODEX_VERSION:
            ...
            # Register the Codify policy hook in this private CODEX_HOME
            # *before* launching the app-server so codex discovers it at
            # config load. The Codify coordinates the hook subprocess
    ...
    async def _trust_policy_hooks(self) -> None:
        """
        Mark the registered Codify policy hook as trusted.
        ...
        if self.listen_url and self.listen_url.startswith("ws://"):
            client = CodexAppServerClient(
                ...
                client_name="codify-policy-trust",
            )
        else:
            client = CodexAppServerClient(
                self.socket_path,
    ...
    def _warn_policy_hook_disabled(self) -> None:
        ...
        :attr:`policy_hook_disabled_reason`. When Codify coordinates are
...
def _codex_policy_hook_command(bridge_dir: Path, python_executable: str | None) -> str:
    ...
    :returns: A shell-escaped command string, e.g.
        ``"/path/python -m codify.codex_native_hook evaluate-policy
        --bridge-dir /home/u/.codify/codex-native/abc"``.
    """
    python = python_executable or sys.executable
    return shlex.join(
...
def _merge_user_hooks(policy_payload: _JsonObject, user_hooks_path: Path) -> _JsonObject:
    ...
    appended after Codify's policy hooks for each shared event, and any
    events declared only by the user are added wholesale. This preserves
    all user hooks while keeping the Codify policy hooks in first
...
def _write_codex_policy_hooks_file(
    ...
    alongside Codify's policy hooks. The symlink is replaced by a
    regular merged file.
...
def _our_policy_hooks_from_list(listed: _JsonObject, cwd: str) -> list[_JsonObject]:
    ...
    :param cwd: The cwd whose hook set to read, e.g.
        ``"/home/user/repo"``.
    :returns: The matching Codify hook metadata dicts (possibly
...
def _untrusted_hook_detail(hooks: Sequence[_JsonObject]) -> str:
    ...
    :param hooks: Untrusted Codify hook metadata dicts from
        ``hooks/list``.
...
async def trust_native_policy_hooks(client: CodexAppServerClient, *, cwd: str) -> None:
    """
    Trust the Codify policy hook so codex actually runs it.
    ...
    verifies every Codify hook ended ``trusted``/``managed``.

    :param client: A connected Codex app-server client.
    :param cwd: The session cwd the hooks are scoped to, e.g.
        ``"/home/user/repo"``.
    :returns: None.
    :raises RuntimeError: If no Codify hook is discovered (it was
    ...
    if not ours:
        raise RuntimeError(
            f"Codify policy hook was not discovered for cwd {cwd!r}; "
    ...
    if still_untrusted:
        ...
        raise RuntimeError(
            "Codify policy hook still untrusted after config/batchWrite; "
...
def build_codex_native_server(
    *,
    ...
    :param bridge_dir: Native Codex bridge directory; the policy hook is
        pointed at it and reads the session id + Codify coordinates from it.
    :param ap_server_url: Codify server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        the hook but writes no Codify coordinates (hook no-ops).
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        Codify requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    :param python_executable: Python executable the policy hook command
    ...
    :param extra_config_overrides: Additional ``-c`` config overrides
        ...
        registration for the Codify tool relay.
    :param developer_instructions: Optional framework-owned instructions
    ...
    if not resolved_codex:
        raise ImportError(
            ...
            "nvm-managed bin dir), set CODIFY_CODEX_PATH=/path/to/codex."
        )
    env = _clean_codex_env()
    config_overrides: list[str] = []
...
def codex_session_meta_model_provider(launch: NativeCodexLaunch) -> str:
    ...
    - a Databricks profile launch carries no override here; the provider
      ...
      ``codify_databricks`` id (see ``_databricks_codex_config_overrides``);
...
def _resolve_subscription_launch(
    ...
    """
    from codify.onboarding.ambient import codex_auth_has_credential
...
def resolve_native_codex_launch(*, model: str | None) -> NativeCodexLaunch:
    ...
    (:func:`codify.runtime.workflow._resolve_provider_for_build`) for the
    ``openai`` surface, so ``codify codex`` and a host-spawned native
    Codex session route through ``codify setup``:
    ...
    Credentials are controlled exclusively by ``codify setup``
    ...
    if entry is None:
        ...
        return NativeCodexLaunch(
            ...
            summary=(
                ...
                "screen and never starts a thread; run `codify setup` to route through "
                "a provider"
            ),
        )
...
def client_for_transport(
    ...
    *,
    ...
    :param transport: App-server transport from bridge state, e.g.
        ...
        ``"/home/u/.codify/codex-native/x/app-server.sock"``.
    :param client_name: App-server initialize-handshake client name,
        e.g. ``"codify-codex-native"``.
...
async def preload_codex_thread_for_resume(
    ...
    *,
    ...
    client = client_for_transport(
        transport,
        client_name="codify-codex-native-preload",
    )
    await client.connect()
    try:
        await client.request(
...
def build_codex_remote_args(
    ...
    *separate* process that loads its own config from ``CODEX_HOME`` and
    ...
    through makes the TUI resolve the Codify provider
    ...
    :param remote_url: App-server endpoint the TUI attaches to, e.g.
        ``"unix:///home/user/.codify/codex-native/x/app-server.sock"``
        or ``"ws://127.0.0.1:9876"``.
    :param config_overrides: Codex ``-c`` config override values to apply
        to the TUI, e.g.
        ``('model="databricks-gpt-5-5"', 'model_provider="codify_databricks"')``.
    ...
    :param bypass_hook_trust: When ``True``, emit
        ...
        ``CODEX_HOME`` is provisioned by Codify and there is no terminal
        user to answer the prompt. Default ``False`` for interactive
        ``codify codex`` sessions where the user faces the terminal and
