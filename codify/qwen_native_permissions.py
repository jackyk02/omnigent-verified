...
So to surface those approvals in the Codify web UI (so a user on the Chat tab
...
:mod:`codify.cursor_native_permissions`. See ``docs/QWEN_NATIVE_DESIGN.md`` and
...
from codify.qwen_native_bridge import events_file_path, submit_confirmation
...
def qwen_permission_elicitation_id(session_id: str, request_id: str) -> str:
    """Return the deterministic Codify elicitation id for a qwen control request.
...
def _read_new_control_events(events_file: Path, offset: int) -> tuple[list[_ControlEvent], int]:
    ...
    Mirrors :func:`codify.qwen_native_forwarder._read_new_events`: detects a
