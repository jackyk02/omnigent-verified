...
* **Dev clone** (``git clone … && uv sync`` / ``pip install -e .``): we
  ...
  to ``~/.codify/.update_check.json`` so the (potentially slow)
  ``git fetch`` only runs once per staleness window. The notice points
  the user at ``git pull``.

* **Installed wheel** (``uv tool install codify``,
  ``pip install codify``, ``pipx install``, Homebrew, etc.): no clone
  ...
  old. The index is resolved from ``CODIFY_INDEX_URL`` /
...
_ENV_SKIP = "CODIFY_NO_UPDATE_CHECK"
_CACHE_DIR = Path.home() / ".codify"
...
# explicit ``CODIFY_INDEX_URL`` wins; then uv's and pip's. When none is
...
class _InstalledWheelInfo:
    ...
    :param vcs_url: The git URL recorded in ``direct_url.json``
        ...
        ``"git+https://github.com/codify-ai/codify.git"``.
...
def _resolve_index_url() -> str:
    ...
    1. index env vars (:data:`_INDEX_ENV_VARS`) — explicit
       ``CODIFY_INDEX_URL`` first, then uv's and pip's;
...
def fetch_latest_version(
    ...
    *,
    ...
    """Fetch the latest ``codify`` release from the configured index.
...
def _versions_from_filenames(filenames: Iterable[str]) -> list[Version]:
    ...
    :param filenames: Distribution filenames, e.g.
        ``["codify-0.2.0-py3-none-any.whl", "codify-0.2.0.tar.gz"]``.
    ...
    with contextlib.suppress(OSError, ValueError):
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from codify.update_check import refresh_update_cache as r; r()",
            ],
...
def _find_repo_root() -> Path | None:
    ...
    """
    package_dir = Path(__file__).resolve().parent  # <candidate>/codify/
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    The ``codify/_build_info.py`` module is generated by
    ...
    """
    try:
        from codify import _build_info  # type: ignore[attr-defined]
    except ImportError:
        return None
    try:
...
def _get_distribution() -> importlib.metadata.Distribution | None:
    ...
    :returns: The ``Distribution`` for the ``codify`` package,
...
def _unredact_ssh_userinfo(vcs_url: str) -> str:
    ...
    :param vcs_url: The normalized ``<vcs>+<scheme>://…`` reinstall URL,
        e.g. ``"git+ssh://****@github.com/codify-ai/codify.git"``.
    :returns: The same URL with a redacted SSH user restored to ``git``,
        e.g. ``"git+ssh://git@github.com/codify-ai/codify.git"``;
...
class _UpgradeSuggestion:
    ...
    :param command: The shell command (or prose) we display in the
        nag panel, e.g. ``"uv tool upgrade codify"`` or
        ``"reinstall codify from your original source"``.
        Always populated.
...
def _pip_invocation() -> str:
    ...
    *other* environment's ``pip`` shadows the one running ``omni`` (a
    ...
    copy of codify while the running one stays put. ``<sys.executable>
...
def upgrade_command_for_installed() -> _UpgradeSuggestion | None:
    ...
    upgrade command (``uv tool upgrade codify``, ``pip install -U
    codify``, etc.).
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    ``codify`` and cached its metadata — re-reading in-process returns the
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    The version is read via ``importlib.metadata`` (no ``codify`` import,
    ...
    probe = (
        ...
        "try:\n"
        "    print(m.version('codify'))\n"
        "except Exception:\n"
        "    print('')\n"
        "try:\n"
        "    from codify.update_check import _read_installed_wheel_info as r\n"
        "    i = r()\n"
...
def _remote_git_head(vcs_url: str) -> str | None:
    ...
    :param vcs_url: A normalized VCS URL, e.g.
        ``"git+https://github.com/codify-ai/codify.git"`` or
        ``"git+https://…/codify.git@main"``.
