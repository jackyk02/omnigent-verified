...
RPC read driver (:mod:`codify.antigravity_native_reader`, which polls/streams
trajectory steps to mirror agy's conversation) and the native executor
(:mod:`codify.inner.antigravity_native_executor`, which delivers web/mobile
...
def get_all_cascade_trajectories(port: int) -> dict[str, object]:
    ...
    reader's :func:`~codify.antigravity_native_reader._detect_rotated_cascade`
...
def _list_agy_pids_from_proc() -> list[int]:
    ...
    :func:`codify.antigravity_native_launch.agy_binary_path` resolves to an
...
def resolve_cold_start_agy_rpc_port(
    ...
    ``codify run --server`` multi-session), this scopes the port to THIS
...
async def interrupt_turn(port: int, conversation_id: str) -> bool:
    ...
    DENY/ASK (see :mod:`codify.antigravity_native_audit`). It is **best-effort
