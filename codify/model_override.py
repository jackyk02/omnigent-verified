...
from codify.harness_availability import CODEX_CANONICAL_HARNESSES
from codify.harness_plugins import model_env_keys
...
# in sync with ``_HARNESS_MODEL_ENV_KEY`` in ``codify/runner/app.py``.
...
# in codify/runtime/workflow.py). So unlike the single-vendor harnesses above,
...
def canonical_model_spelling(model: str) -> str:
    ...
    :mod:`codify.cost_plan`) compare in this form.
...
def normalize_model_for_provider(model: str, provider_kind: str | None) -> str:
    ...
    :param provider_kind: The child's resolved provider kind from
        :func:`codify.model_catalog.resolve_model_provider`, e.g.
