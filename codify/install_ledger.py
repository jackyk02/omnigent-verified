...
PROFILE_MARKER_BEGIN = "# >>> Codify installer >>>"
PROFILE_MARKER_END = "# <<< Codify installer <<<"
CONSOLE_SCRIPTS = ["codify", "omni"]
...
def state_dir() -> Path:
    if data_dir := os.environ.get("CODIFY_DATA_DIR"):
...
class LedgerEntries:
    ...
    state_paths: StatePathsEntry = field(
        default_factory=lambda: StatePathsEntry(
            codify_home=str(state_dir()), workspace=str(Path.home() / "codify")
        )
    )
        ...
        entries = LedgerEntries(
            ...
            state_paths=StatePathsEntry(
                **(
                    ...
                    or {
                        "codify_home": str(state_dir()),
                        "workspace": str(Path.home() / "codify"),
                    }
                )
            ),
        )
...
def observed_external_configs(*, deep: bool) -> list[ExternalConfigEntry]:
    ...
    cwd = Path.cwd()
    candidates = [
        (cwd / ".cursor" / "mcp.json", "mcpServers.codify", "json"),
        (cwd / ".kiro" / "settings" / "mcp.json", "mcpServers.codify", "json"),
        (Path.home() / ".claude.json", "mcpServers.codify", "json"),
        (Path.home() / ".codex" / "config.toml", "mcp_servers.codify", "toml"),
    ]
...
def observed_launch_agents(*, deep: bool) -> list[LaunchAgentEntry]:
    ...
    if launchd_dir.is_dir():
        for path in sorted(launchd_dir.glob("*codify*.plist")):
            entries.append(
                LaunchAgentEntry(
                    kind="launchd",
                    path=str(path),
    ...
    if systemd_dir.is_dir():
        for path in sorted(systemd_dir.glob("*codify*.service")):
            entries.append(
                LaunchAgentEntry(
                    kind="systemd_user",
                    path=str(path),
...
def new_ledger(*, source: str, strategy: str, deep: bool) -> InstallLedger:
    ...
    return InstallLedger(
        ...
        generator={
            "name": "codify",
            "version": _version_for(shutil.which("codify")) or "unknown",
            "strategy": strategy,
            "os": platform_name(),
            "wrote_at": now,
        },
...
def write_install_ledger_from_env() -> InstallLedger:
    ...
    if existing and existing.ledger_source == "installer":
        ...
        for name, dep in existing.entries.deps.items():
            if name in ledger.entries.deps and dep.installed_by in {"codify", "preexisting"}:
                current = ledger.entries.deps[name]
                if current.installed_by == "unknown" or dep.installed_by == "codify":
                    current.installed_by = dep.installed_by
                    current.confidence = dep.confidence
    for name in list(ledger.entries.deps):
        env_name = f"CODIFY_LEDGER_DEP_{name.upper()}"
        installed_by = os.environ.get(env_name)
        if installed_by in {"codify", "preexisting", "unknown"}:
    ...
    profile_env = os.environ.get("CODIFY_LEDGER_PROFILE")
    if profile_env:
