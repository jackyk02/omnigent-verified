...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m codify.claude_native_status")
    ...
    # status bar still renders for codify-launched sessions.
...
def _write_context_atomic(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    # event, so the Codify relay's cost accumulation never runs for it).
    ...
    try:
        ...
        print(f"codify claude status: write failed: {exc}", file=sys.stderr)
...
def _chain(command: str, stdin_payload: str) -> None:
    ...
    try:
        ...
        print(f"codify claude status: chain failed: {exc}", file=sys.stderr)
        return
    if proc.stdout:
