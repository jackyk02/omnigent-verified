"""Native Antigravity (agy) TUI wrapper for the Codify CLI.

``codify antigravity`` treats the Antigravity ``agy`` CLI as a
terminal-first program, mirroring ``codify codex`` / ``codify claude``.
It creates or binds an Codify session, launches ``agy`` in a runner-owned
...
* **RPC mirroring (read path) and RPC web-turn delivery (write path).** agy's
  conversation mirrors into the Codify chat view via the RPC read driver
  (:mod:`codify.antigravity_native_reader`), which polls/streams agy's
  ...
  (:mod:`codify.inner.antigravity_native_executor`) over the connect-RPC
...
* **Per-session identity is minted at cold-start, not assigned at launch.** agy
  ...
  :func:`codify.antigravity_native_launch.build_agy_launch`).
...
from codify.antigravity_native_launch import (
    agy_binary_path,
    build_agy_launch,
...
def run_antigravity_native(
    *,
    ...
    """
    Launch the Antigravity (agy) TUI in an Codify terminal and attach.

    :param server: Resolved Codify server URL, e.g.
        ``"http://127.0.0.1:8123"``. ``None`` starts a local Codify
        server using the existing chat-server machinery.
    :param session_id: Optional existing Codify conversation id to
    ...
    :param permission_mode: Optional Codify permission mode, e.g.
        ...
        hang (see
        :func:`codify.antigravity_native_launch.should_skip_permissions`).
    ...
    with TemporaryDirectory(prefix="codify-antigravity-native-") as tmpdir:
...
def _materialize_antigravity_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify antigravity``.
    ...
    raw: dict[str, object] = {
        ...
        # sessions. The Codify MCP relay (wired in #1194 — see
        ...
        # ``_require_os_env`` in ``codify/runner/app.py``). agy already
        ...
        # Declare a default shell terminal so the Codify MCP relay advertises
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local Codify server, launch agy, and attach to it.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch agy on a remote Codify server via a daemon-spawned runner.
    ...
    :returns: None.
    """
    from codify.chat import _bundle_agent, _remote_headers
...
async def _prepare_antigravity_terminal(
    *,
    ...
    :param command: agy executable to run.
    :param model: Optional agy model id.
    :param permission_mode: Optional Codify permission mode threaded into the
        agy argv assembly.
    :param headless: ``True`` when no interactive client will attach (forces
...
async def _launch_and_record(
    ...
    *,
    ...
    :func:`codify.antigravity_native_rpc.resolve_language_server_port`). A pid
    ...
    conversation (:mod:`codify.antigravity_native_reader`) keeps an in-memory
    seen-set only.
...
async def _attach_terminal(
    ...
    **Read/write wiring on the CLI fallback only.** When the runner produced no
    ...
    driver (:func:`codify.antigravity_native_reader.run_reader_with_bridge`,
    which mirrors agy's conversation into the Codify chat view and surfaces
...
async def _agy_cold_start_poll_sleep(seconds: float) -> None:
    ...
    :func:`codify.runner.app._agy_cold_start_poll_sleep`.
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    :func:`codify.runner.app._cold_start_agy_conversation`: once agy is live
    ...
    codex/pi) so a later ``codify antigravity --resume`` reads it back and passes
    ...
    *timeout_s*, ``StartCascade`` erroring, or the PATCH failing) leaves the
    ...
    :param base_url: Codify server base URL for the ``external_session_id``
        PATCH.
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> _AttachOutcome:
    ...
    who runs ``codify antigravity`` from inside their own tmux can still
    attach to Codify's private tmux server. After the attach child
    ...
    """
    from codify.terminals.ws_bridge import _tmux_session_alive
...
async def _create_antigravity_session(
    ...
    *,
    ...
    :returns: New Codify session id, e.g. ``"conv_abc123"``.
...
async def _fetch_antigravity_session(
    ...
    """
    Fetch an existing Codify session snapshot.
...
async def _launch_antigravity_terminal(
    ...
    *,
    ...
    body = {
        ...
        # ``codify/server/routes/sessions.py`` ``is_native_bootstrap``).
        #
...
def _launch_is_headless() -> bool:
    ...
    ``codify antigravity`` attaches the local TTY to the agy tmux terminal so
    ...
    (see :func:`codify.antigravity_native_launch.should_skip_permissions`).
    ...
    .. note:: This TTY signal governs ONLY the human-invoked CLI launch path
       ...
       (:func:`codify.runner.app._auto_create_antigravity_terminal`, the
