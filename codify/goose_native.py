"""Native Goose TUI wrapper for the Codify CLI.

``codify goose`` launches Block's Goose CLI interactive TUI (``goose session``)
inside an Codify-runner-owned tmux terminal and attaches the local TTY — the
goose analog of ``codify cursor`` / ``codify codex`` / ``codify pi``. The
runner spawns the process (see
:func:`codify.runner.app._auto_create_goose_terminal`); this module owns the
...
``~/.config/goose/config.yaml`` + keyring); no Codify-managed key is required.
...
from codify._wrapper_labels import GOOSE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_GOOSE_PATH_ENV = "CODIFY_GOOSE_PATH"
...
def run_goose_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Goose requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_goose_native."
        )
    with TemporaryDirectory(prefix="codify-goose-native-") as tmpdir:
...
def _materialize_goose_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify goose``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Goose on an Codify server via a daemon-spawned runner.
