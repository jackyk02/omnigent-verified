"""Single source of truth for Codify wrapper-session labels.

Wrapper-style sessions (``codify claude`` today; future
``codex`` / ``pi`` wrappers tomorrow) stamp an ``codify.wrapper``
label on the conversation row at creation time. The server reads it
to gate behavior (claude-native message bypass at
``codify/server/routes/sessions.py:182-183``); the chat redirect
...
* keep ``codify.repl._resume_picker`` decoupled from the
  ``codify.claude_native`` import graph (which pulls in tmux /
...
# for the ``codify.*`` namespace; never reused for guardrails /
# policy labels.
WRAPPER_LABEL_KEY = "codify.wrapper"
...
# ``labels["codify.ui"] == "terminal"``). Stamped at creation for the
...
UI_MODE_LABEL_KEY = "codify.ui"
UI_MODE_TERMINAL_VALUE = "terminal"

# Value the ``codify claude`` wrapper writes into
...
# Value the ``codify codex`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
CODEX_NATIVE_WRAPPER_VALUE = "codex-native-ui"

# Value the ``codify pi`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
PI_NATIVE_WRAPPER_VALUE = "pi-native-ui"

# Value the ``codify opencode`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
OPENCODE_NATIVE_WRAPPER_VALUE = "opencode-native-ui"

# Value the ``codify cursor`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
CURSOR_NATIVE_WRAPPER_VALUE = "cursor-native-ui"

# Value the ``codify kiro`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
KIRO_NATIVE_WRAPPER_VALUE = "kiro-native-ui"

# Value the ``codify goose`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
GOOSE_NATIVE_WRAPPER_VALUE = "goose-native-ui"

# Value the ``codify antigravity`` native (agy TUI) wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
ANTIGRAVITY_NATIVE_WRAPPER_VALUE = "antigravity-native-ui"

# Value the ``codify qwen`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
QWEN_NATIVE_WRAPPER_VALUE = "qwen-native-ui"

# Value the ``codify kimi`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
KIMI_NATIVE_WRAPPER_VALUE = "kimi-native-ui"
# Value the ``codify hermes`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
