...
OpenCode's permission surface and Codify's policy/approval model:

1. Normalize a raw permission request into a flat policy-evaluation input.
2. Map an Codify policy verdict (allow / allow-always / deny / ask) onto
   an OpenCode reply.
...
# Codify-side normalized decisions used by the forwarder.
...
def normalize_for_policy(
    ...
    *,
    ...
    """
    Build an Codify policy-evaluation input from a permission request.
    ...
    :param codify_session_id: Owning Codify conversation id.
    ...
    return {
        ...
        "codify_session_id": codify_session_id,
...
def map_verdict_to_decision(verdict: _JsonMapping | None) -> PolicyDecision:
    """
    Map an Codify policy verdict onto a normalized decision.
...
def decision_to_reply(decision: PolicyDecision) -> OpenCodeReply | None:
    ...
    ``permission/index.ts``), which bypasses the Codify policy engine and
