...
Core Codify contributes the built-in harnesses directly. Optional community
packages contribute additional harnesses through the
``codify.community.harness`` entry point group.
"""
...
from codify.harness_capabilities import (
    AuthModel,
    EffortFamily,
    Elicitation,
    ForkHistory,
...
COMMUNITY_ENTRY_POINT_GROUP = "codify.community.harness"
COMMUNITY_MODULE_PREFIX = "codify.community.harness."
...
class NativeHarnessProvider:
    ...
    :mod:`codify.native_dispatch`, so building the registry never imports the
...
# ``codify.<key>_native.bridge_id`` pattern (pinned against the real bridge
...
def _builtin_native_provider(key: str) -> NativeHarnessProvider:
    """Build a built-in provider row from the ``codify.<key>_native`` module.
    ...
    ``codify.runner.native``, and exposes ``build_<key>_native_spawn_env`` in
    ``codify.<key>_native_bridge``. The remaining hooks (interrupt, stop,
    bridge-dir) are still runner-local closures / inline dispatch, so they stay
    ``None`` until those hubs migrate onto the seam.
    """
    module = f"codify.{key}_native"
    return NativeHarnessProvider(
        key=key,
        run_native=f"{module}:run_{key}_native",
        auto_create_terminal=f"codify.runner.native:_launch_{key}",
...
# the codify.<key>_native module layout.
...
# "codify-bench-ok" placeholder the bench token-swaps per allow/deny probe.
_SHELL_PROMPT = "Use your shell/terminal tool to run this exact command: echo codify-bench-ok"
_BASH_PROMPT = "Use the Bash tool to run this exact command: echo codify-bench-ok"
...
_BUILTIN_CAPABILITIES: dict[str, HarnessCapabilities] = {
    ...
    # (codify/inner/open_responses_sdk.py) is concrete: interrupt_session()
...
_BUILTIN_CONTRIBUTION = HarnessContribution(
    ...
    harness_modules={
        "acp": "codify.inner.acp_harness",
        "antigravity": "codify.inner.antigravity_harness",
        "antigravity-native": "codify.inner.antigravity_native_harness",
        "claude-native": "codify.inner.claude_native_harness",
        "claude-sdk": "codify.inner.claude_sdk_harness",
        "codex": "codify.inner.codex_harness",
        "codex-native": "codify.inner.codex_native_harness",
        "copilot": "codify.inner.copilot_harness",
        "cursor": "codify.inner.cursor_harness",
        "cursor-native": "codify.inner.cursor_native_harness",
        "goose": "codify.inner.goose_harness",
        "goose-native": "codify.inner.goose_native_harness",
        "hermes": "codify.inner.hermes_harness",
        "hermes-native": "codify.inner.hermes_native_harness",
        "kimi": "codify.inner.kimi_harness",
        "kimi-native": "codify.inner.kimi_native_harness",
        "kiro-native": "codify.inner.kiro_native_harness",
        "openai-agents": "codify.inner.openai_agents_sdk_harness",
        "opencode-native": "codify.inner.opencode_native_harness",
        "pi": "codify.inner.pi_harness",
        "pi-native": "codify.inner.pi_native_harness",
        "qwen": "codify.inner.qwen_harness",
        "qwen-native": "codify.inner.qwen_native_harness",
    },
    aliases={
        "agy": "antigravity",
        "claude": "claude-sdk",
    ...
    background_title_generators={
        "claude-sdk": BackgroundTitleGeneratorSpec(
            "codify.runner.background_titles.sdk:generate_background_title"
        ),
        "claude-native": BackgroundTitleGeneratorSpec(
            "codify.runner.background_titles.claude_native:generate_background_title",
        ...
        "codex-native": BackgroundTitleGeneratorSpec(
            "codify.runner.background_titles.codex_native:generate_background_title"
        ),
    },
    harness_labels={
...
def harness_catalog() -> list[dict[str, object]]:
    ...
    ``codify setup``; the host reports each step's status in its readiness map.
    """
    ...
    try:
        from codify.onboarding.harness_install import ui_setup_steps
    ...
    try:
        from codify.onboarding.acp_auth import acp_agents
...
def harness_setup_steps_by_spelling() -> dict[str, list[dict[str, str | None]]]:
    ...
    """
    try:
        from codify.onboarding.harness_install import ui_installable_harnesses, ui_setup_steps
    except Exception:  # noqa: BLE001 — a broken onboarding import must not break the catalog
        _logger.debug("setup-step metadata unavailable", exc_info=True)
        return {}
