"""Native Kimi TUI wrapper for the Codify CLI.

``codify kimi`` launches the Kimi CLI's interactive TUI (``kimi``
with no args) inside an Codify-runner-owned tmux terminal and attaches the
local TTY — the kimi analog of ``codify codex`` / ``codify pi``. The runner
spawns the process (see :func:`codify.runner.app._auto_create_kimi_terminal`);
...
from codify._wrapper_labels import KIMI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIMI_PATH_ENV = "CODIFY_KIMI_PATH"
...
def run_kimi_native(
    *,
    ...
    """
    Launch the Kimi TUI in an Codify terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Kimi requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_kimi_native."
        )
    with TemporaryDirectory(prefix="codify-kimi-native-") as tmpdir:
...
def _materialize_kimi_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify kimi``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Kimi on an Codify server via a daemon-spawned runner.
