...
shared :mod:`codify.runtime.harnesses._runner` invokes after
the parent process resolves ``"codex"`` to this module via
:data:`codify.runtime.harnesses._HARNESS_MODULES`.

Internally, instantiates :class:`codify.runtime.harnesses._executor_adapter.ExecutorAdapter`
around a :class:`codify.inner.codex_executor.CodexExecutor`
...
- ``HARNESS_CODEX_CWD``: working directory the executor launches
  the Codex CLI in. ``None`` falls back to ``CODIFY_RUNNER_WORKSPACE``
  if set, then to the subprocess's inherited cwd.
- ``CODIFY_CODEX_PATH``: absolute path to a ``codex`` CLI binary.
...
- ``HARNESS_CODEX_OS_ENV``: JSON-encoded :class:`OSEnvSpec`
  ...
  Codify mode parity with the legacy non-AP path holds for
...
from codify.harness_startup_config import resolve_harness_path
...
_ENV_CODEX_PATH = "CODIFY_CODEX_PATH"
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    Codify serialized via :func:`dataclasses.asdict` on its
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    ``RetryPolicy()`` (defaults) when missing — Codify omits the
...
def _build_codex_executor() -> Executor:
    ...
    :raises ImportError: If the ``codex`` CLI isn't on PATH and
        ``CODIFY_CODEX_PATH`` (legacy ``HARNESS_CODEX_PATH``) isn't set — the inner executor's
    ...
    return CodexExecutor(
        cwd=os.environ.get(_ENV_CWD) or os.environ.get("CODIFY_RUNNER_WORKSPACE") or None,
...
def create_app() -> FastAPI:
    ...
    :data:`codify.runtime.harnesses._HARNESS_MODULES`) and
