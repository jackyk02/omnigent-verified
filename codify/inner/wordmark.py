"""The Codify brand wordmark and Otto lockup for CLI output.

A bold "ANSI-Shadow" block-letter ``codify`` wordmark — the canonical
...
from :mod:`codify.inner.mascots`, which it pairs with 1:1.
...
banner (TTY gating, ``CODIFY_NO_BANNER``) lives one layer up in
:mod:`codify.inner.ui`, which is the only module that should be imported
...
:data:`codify.inner.mascots.MASCOT_ART_COLOR`); the optional gradient
...
#: The rows of the ``codify`` wordmark, as plain (uncolored) text.
...
def render_lockup(
    ...
    *,
    ...
    :param epilogue: Optional aligned label/value rows printed beneath the
        art, e.g. ``[("Version", "0.4.2"), ("Next", "codify setup")]``.
    :returns: None.
    """
...
def render_compact(console: Console, *, subtitle: str | None = None) -> None:
    """
    Print the one-line brandmark: ``✦ codify  <subtitle>``.
    ...
    line.append("codify", style=f"bold {WORDMARK_COLOR}")
    if subtitle:
