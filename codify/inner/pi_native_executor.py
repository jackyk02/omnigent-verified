"""Executor that bridges Codify messages into a native Pi TUI."""
...
class PiNativeExecutor(Executor):
    """
    Harness-side executor for ``codify pi`` web UI turns.

    The native Pi process is already running in the session terminal with
    the Codify Pi extension loaded. Each turn queues the latest user
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from Codify. Ignored for now; native
