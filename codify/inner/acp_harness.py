...
Wraps an :class:`codify.inner.acp_executor.AcpExecutor`, which drives *any*
...
:func:`codify.runtime.workflow._build_acp_spawn_env`), which resolves the
...
Codify stores no credential. Tool approvals surface as web elicitation cards
...
def _build_acp_executor() -> Executor:
    ...
    if not command:
        raise RuntimeError(
            ...
            "Add one via `codify setup` → configure harnesses → Custom ACP agent."
        )
