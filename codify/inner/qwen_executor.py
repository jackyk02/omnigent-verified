...
class QwenExecutor(Executor):
    ...
    def __init__(
        ...
        :param gateway_base_url: OpenAI-compatible base URL of an Codify
        ...
        # ``session/request_permission`` routes through Codify's TOOL_CALL
        ...
        # Adapter-injected tool bridge + the Codify-tool MCP relay it backs.
        # Exposes Codify builtin tools to qwen via session/new.mcpServers (the
        # shared serve-mcp relay); qwen keeps its own built-in tool registry.
        self._tool_executor: _ToolExecutor | None = None
        self._mcp = CodifyAcpMcp(label="qwen")
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        Mirrors :func:`codify.inner.pi_executor._try_sandbox_pi`. When
    ...
    async def _build_spawn_env(self) -> dict[str, str]:
        ...
        families + the spec's ``env_passthrough``, then Codify's
    ...
    async def _respond_to_agent_request(self, request: _AcpJsonObject) -> None:
        ...
        - ``session/request_permission`` — decide via Codify's TOOL_CALL
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here. Executed through the Codify OSEnvironment
          so the spec's sandbox read/write roots are enforced at the Python
          layer and the I/O flows through Codify rather than qwen touching
    ...
    def _accumulate_usage(acc: dict[str, int], update: _AcpJsonObject) -> None:
        ...
        ``MessageEmitter.emitUsageMetadata``). A single Codify turn can drive
    ...
    async def run_turn(
        ...
        """
        # Captured for the Codify MCP relay set up lazily at session/new.
