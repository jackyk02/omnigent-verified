...
around a :class:`codify.inner.claude_sdk_executor.ClaudeSDKExecutor`
...
than per-spec data on the request body. Codify sets these env vars
...
# Set by the Codify workflow layer when executor.auth: {type: api_key, …} is
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing — Codify omits the env var when ``llm.retry`` matches
...
def _build_claude_sdk_executor() -> Executor:
    ...
    return ClaudeSDKExecutor(
        ...
        # CODIFY_RUNNER_WORKSPACE (the folder the user launched in, and
