...
per Codify conversation, created once and reused turn to turn. Each
...
:class:`~codify.inner.executor.ToolSpec` becomes a :class:`copilot.Tool`
...
*inside* the SDK rather than through Codify's bridged-tool dispatch. They are
...
from codify.reasoning_effort import COPILOT_EFFORTS, validate_effort
...
def _build_copilot_prompt(messages: list[Message], *, is_first_turn: bool) -> str:
    ...
    Codify system prompt is delivered separately (``system_message``), so on
...
class CopilotExecutor(Executor):
    ...
    def _make_tools(self, tools: list[ToolSpec]) -> list[Any]:  # type: ignore[explicit-any]
        """Build the SDK ``tools`` list from Codify ToolSpecs.
        ...
        ``skip_permission=True`` because Codify's policy layer (and the
    ...
    def _make_handler(self, tool_name: str) -> Callable[[Any], Awaitable[Any]]:  # type: ignore[explicit-any]
        """Build an async ``handler`` that bridges a Copilot tool call to Codify.
    ...
    async def _on_permission_request(
        ...
        if evaluator is not None:
            ...
            if getattr(verdict, "action", "") == "POLICY_ACTION_DENY":
                ...
                return PermissionDecisionReject(feedback=f"Denied by Codify policy: {reason}")
        ...
        if handler is not None:
            ...
            if not approved:
                return PermissionDecisionReject(feedback="Denied via Codify approval UI")
    ...
    async def _ensure_session(
        ...
        try:
            ...
            from codify.onboarding.copilot_auth import COPILOT_EXTRA
        ...
        try:
            ...
            # its built-in tools) and layers the Codify agent's system prompt
...
def _unwrap_tool_result(raw: Any) -> Any:  # type: ignore[explicit-any]
    ...
    Codify-convention result (e.g. ``{"blocked": True}``) passes through
