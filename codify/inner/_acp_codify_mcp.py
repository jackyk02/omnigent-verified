"""Expose Codify's builtin tools to an ACP agent via ``session/new.mcpServers``.

Shared by the ACP executors (``acp`` generic, ``goose``, ``qwen``). Reuses the
*same* stdio ``serve-mcp`` relay the native harnesses use
(:mod:`codify.claude_native_bridge`): the ACP agent spawns
``python -Im codify.claude_native_bridge serve-mcp --bridge-dir <dir>`` as an
MCP server, which proxies each Codify tool call back through ``tool_executor``
(→ :meth:`TurnContext.dispatch_tool` → the Codify server, where TOOL_CALL /
TOOL_RESULT policy is enforced). The agent keeps its own filesystem/shell tools;
this only *adds* Codify's builtin tools (``sys_session_*``, ``sys_agent_*``,
...
``CODIFY_ACP_MCP=0`` kill switch) yields an empty ``mcpServers`` — the agent
just runs without Codify tools, exactly as before this feature.
"""
...
if TYPE_CHECKING:
    from codify.claude_native_bridge import ClaudeNativeToolRelay, ToolExecutor
...
_ENV_KILL_SWITCH = "CODIFY_ACP_MCP"
...
class CodifyAcpMcp:
    """Lazily-started Codify-tool relay + its ACP ``mcpServers`` entry.
    ...
    def session_new_servers(
        ...
        *,
        ...
        :param tools: Codify tool schemas to advertise (each ``{"name", …}``).
        ...
        try:
            ...
            logger.info(
                "acp[%s] Codify MCP relay ready (%d builtin tools bridged)",
                self._label,
                len(tools),
            )
            return self._acp_servers
            ...
            logger.warning(
                ("acp[%s] Codify MCP bridge setup failed; agent runs without Codify tools: %s"),
                self._label,
                exc,
            )
            self._acp_servers = []
