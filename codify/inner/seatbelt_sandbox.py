...
from the resolved :class:`codify.inner.sandbox.SandboxPolicy`.
...
This is the **macOS parity** for :mod:`codify.inner.bwrap_sandbox`.
...
- The same cwd dotfile/escaping-symlink walker
  (:func:`codify.inner._cwd_scan.scan_cwd_mask_entries`).
- The same egress filtering pipeline: parent-side
  :class:`codify.inner.egress.proxy.EgressProxy`, Unix socket
  bridge in the scratch tmpdir, in-helper TCP→Unix relay via
  :func:`codify.inner.egress.relay.start_relay`, and
...
- The per-helper scratch tmpdir (added by the spawn site via
  :func:`codify.inner.sandbox.with_additional_write_roots`) is
...
- **AF_UNIX path-length cap.** macOS limits ``sun_path`` to ~104
  ...
  ``/var/folders/.../T/codify-osenv-XXXXXX/.egress.sock`` is
...
class SeatbeltSandboxBackend(SandboxBackend):
    ...
    def wrap_launcher_argv(
        ...
        :param argv: The unwrapped helper command, typically
            ``[sys.executable, "-m", "codify.inner.os_env",
        ...
        :param policy: The :class:`SandboxPolicy` produced by
            ...
            :func:`codify.inner.sandbox.with_additional_write_roots`
    ...
    def activate(self, policy: SandboxPolicy) -> None:
        ...
        Unlike :meth:`codify.inner.bwrap_sandbox.BwrapSandboxBackend.activate`,
        ...
        if policy.egress_relay_port is not None and policy.egress_socket_path is not None:
            from codify.inner.egress.relay import start_relay
...
def _build_profile(
    ...
    *,
    ...
    # ``codify.inner.os_env`` even starts. The bwrap backend
...
def _ensure_executable_visible(
    ...
    Parallel to
    :func:`codify.inner.bwrap_sandbox._ensure_executable_visible`,
...
def _scan_extra_roots_mask_entries(
    ...
    *,
    ...
    :func:`codify.inner._cwd_scan.merge_scan_roots` folds the two
...
def _seatbelt_safe_roots(
    ...
    Mirrors :func:`codify.inner.bwrap_sandbox._bwrap_safe_roots`
...
def _interpreter_safe_roots(argv: Sequence[str]) -> list[Path]:
    ...
    Mirrors :func:`codify.inner.bwrap_sandbox._interpreter_safe_roots`:
...
def _per_user_dyld_cache_subpath() -> Path | None:
    ...
    dirs (which live under ``<per-user-folder>/T/codify-osenv-*``,
...
def _scratch_tmpdir(write_roots: list[Path]) -> Path | None:
    ...
    :func:`codify.inner.sandbox.create_private_tmpdir` returns a
    path under ``tempfile.gettempdir()`` (typically ``/var/folders/.../T``
    on macOS). The helper-spawn site adds it to ``write_roots`` via
    :func:`codify.inner.sandbox.with_additional_write_roots`, so
...
def _ensure_profile_tempdir() -> str:
    ...
    ``<system tmpdir>/codify-seatbelt-profiles-XXXX`` with mode
    ...
    if _PROFILE_TEMPDIR is None:
        _PROFILE_TEMPDIR = tempfile.mkdtemp(prefix="codify-seatbelt-profiles-")
