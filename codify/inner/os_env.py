...
from codify._platform import IS_WINDOWS, WINDOWS_ENV_PASSTHROUGH
...
# mirror the pattern in ``codify/sandbox.py`` and ``codify/uc_tools.py``.
...
_DEFAULT_ENV_PASSTHROUGH: tuple[str, ...] = (
    ...
    "PYTHONFAULTHANDLER",
    # Codify session marker: always pass the "inside Codify" marker
    ...
    # the runner via runner.identity.CODIFY_SESSION_ENV_VAR.
    CODIFY_SESSION_ENV_VAR,
...
def build_helper_env(
    ...
    (:data:`~codify.runner.identity.RUNNER_AUTH_SECRET_ENV_VARS`): the
...
def create_os_environment(spec: OSEnvSpec | None) -> OSEnvironment | None:
    ...
    if spec.fork:
        fork_dir = Path(tempfile.mkdtemp(prefix="codify-fork-"))
...
def _project_root() -> Path:
    # File lives at codify/inner/os_env.py; climb two levels to the
...
def _child_shell_env() -> dict[str, str]:
    """
    Environment for agent shell commands, minus codify's own package root.

    The helper prepends its project root to ``PYTHONPATH`` at spawn (see
    ``_HelperProcessClient._start_locked``) purely so ``python -m
    codify.inner.os_env`` can import codify. That entry has no business in
    the commands the agent runs: under a tool install it points at codify's
    ...
    project). Strip only codify's entry — any other ``PYTHONPATH`` the caller
    ...
    if isinstance(token_value, str) and token_value:
        ...
        for env_key in ("HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"):
            ...
            if not parts.hostname:
                continue
            netloc = f"codify:{token_value}@{parts.hostname}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        raise SystemExit("usage: python -m codify.inner.os_env <helper|launch> ...")
    ...
    if command == "helper":
        ...
        if len(args) != 3 or args[1] not in ("--config-fd", "--config-file"):
            raise SystemExit(
                "usage: python -m codify.inner.os_env helper "
    ...
    if command == "launch":
        if len(args) < 3:
            raise SystemExit(
                "usage: python -m codify.inner.os_env launch <sandbox> <target> [args...]"
            )
