...
Wraps a :class:`codify.inner.goose_executor.GooseExecutor`, which drives
...
(:mod:`codify.inner.goose_native_harness`). Goose's mid-turn tool approvals
surface as web elicitation cards (via Codify's TOOL_CALL policy + ``ctx.elicit``
...
``~/.config/goose/config.yaml``); Codify stores no Goose credential. A spec
...
- ``HARNESS_GOOSE_CWD``: working directory for the goose subprocess. ``None``
  falls back to ``CODIFY_RUNNER_WORKSPACE`` then the inherited cwd.
- ``CODIFY_GOOSE_PATH``: absolute path to a ``goose`` CLI binary.
...
from codify.inner.goose_executor import GooseExecutor
...
_ENV_GOOSE_PATH = "CODIFY_GOOSE_PATH"
