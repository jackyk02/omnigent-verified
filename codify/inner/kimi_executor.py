...
- ``CODIFY_KIMI_PATH``: explicit path to the ``kimi`` binary, e.g.
...
and is managed out-of-band via ``kimi provider add`` (Codify-side
...
def _resolve_kimi_binary() -> str:
    ...
    ``CODIFY_KIMI_PATH`` wins (legacy ``HARNESS_KIMI_PATH`` still honored
...
class KimiExecutor(Executor):
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        Mirrors :meth:`codify.inner.qwen_executor.QwenExecutor._sandbox_launch_path`.
    ...
    def _translate_event(self, payload: dict[str, Any]) -> list[ExecutorEvent]:
        """Translate one kimi stream-json line into Codify events.
        ...
        - ``"tool"``: kimi's own tool execution result delivered back to
          its loop. Surfaced as a ``ToolCallComplete`` so the Codify
        ...
        if role == "assistant":
            ...
            # so the Codify UI can render the result. The Session layer
    ...
    async def run_turn(
        ...
        if tools and not self._warned_tools_without_bridge:
            _logger.warning(
                "kimi executor received %d declared tool(s) but Codify has no "
        ...
        if shutil.which(self._binary_path) is None and not Path(self._binary_path).exists():
            yield ExecutorError(
                message=(
                    ...
                    "or set CODIFY_KIMI_PATH (legacy HARNESS_KIMI_PATH) to its"
                    " absolute location."
                ),
                retryable=False,
            )
