...
underlying agent harness.  Codify tools are bridged into the SDK session
...
This executor translates the SDK message stream into Codify ExecutorEvents
...
from codify._platform import resolve_cli_binary, stable_user_id
from codify.inner import _proc
from codify.inner.bundle_skills import ensure_bundle_plugin_manifest
from codify.llms._usage_observer import notify_from_dict as _notify_usage_from_dict
from codify.llms.adapters._content import parse_data_uri as _parse_replay_data_uri
from codify.reasoning_effort import CLAUDE_EFFORTS, validate_effort
...
# ---------------------------------------------------------------------------
# TypeAliases for Codify JSON-shaped boundary values. The SDK exchanges
...
# with the Codify tool executor and the SDK's MCP bridge.
...
# Tool executor callable wired in by ``codify.Session``.
...
_NO_SANDBOX_ENV = "CODIFY_CLAUDE_SDK_NO_SANDBOX"

# Env override for an explicit claude binary, mirroring codex's
# CODIFY_CODEX_PATH. Set this when claude lives on a PATH the host
# daemon doesn't inherit (e.g. an nvm-managed global bin dir).
_CLAUDE_PATH_ENV = "CODIFY_CLAUDE_PATH"
...
# path the Codify producer resolves the model instead (see workflow.py).
_CLAUDE_API_KEY_HELPER_ENV_KEY = "CODIFY_CLAUDE_API_KEY_HELPER"
...
class PreparedClaudeCli:
    """Result of wrapping the Claude CLI in an Codify sandbox.
...
def _build_mcp_tools(
    ...
    """Build SdkMcpTool objects from Codify tool schemas.

    Each tool is backed by a handler that calls the Codify tool_executor
    ...
    for schema in tool_schemas:
        ...
        # itself does not accept ``None``. Codify tool schemas always
...
def _augment_system_prompt_for_codify_mcp_tools(
    system_prompt: str,
    tool_schemas: list[ToolSpec],
) -> str:
    """
    ...
    Codify schemas use bare names such as ``sys_session_send``. The
    Claude SDK exposes tools from our in-process MCP server to the model
    as ``mcp__codify__<bare_name>``. Bundled agent prompts and skills use
    ...
    if examples:
        example_text = "; ".join(
            f"use `mcp__codify__{name}` when instructions say `{name}`" for name in examples
        )
        note = (
            "Claude SDK tool naming: Codify tools are exposed as MCP tools. "
            f"{example_text}. For any other Codify tool, use "
            "`mcp__codify__<tool_name>` rather than the bare name."
        )
    else:
        note = (
            "Claude SDK tool naming: Codify tools are exposed as MCP tools. "
            "When instructions mention a bare Codify tool name, invoke "
            "`mcp__codify__<tool_name>` rather than the bare name."
        )
...
def _find_system_claude() -> str | None:
    ...
    Resolves via the ``CODIFY_CLAUDE_PATH`` override, then ``PATH``, then
...
def _resolve_sandbox_cwd(spec_cwd: str | None) -> pathlib.Path:
    ...
    ``CODIFY_RUNNER_WORKSPACE``. Prefer that workspace as the base so
    ...
    """
    base = os.environ.get("CODIFY_RUNNER_WORKSPACE") or os.getcwd()
    if spec_cwd:
...
def prepare_claude_cli_path(
    ...
    as ``CODIFY_CLAUDE_SDK_NO_SANDBOX``: file/shell access still goes
...
class ClaudeSDKExecutor(Executor):
    ...
    The SDK runs Claude Code's full agent loop internally.  Codify tools
    ...
    is set.  Even without ``os_env``, Codify tries to place the Claude CLI
    ...
    turns by keeping a live Claude SDK client per Codify session.
        ...
        *,
        ...
        Args:
            ...
            os_env: If set, enable built-in OS tools (Bash, Read, Edit, …)
                ...
                spec's sandbox is enabled, Codify wraps the Claude CLI
                in the same sandbox. If omitted, Codify still tries to
        ...
        if not gateway and model is not None and model.startswith("databricks-"):
            raise ValueError(
                ...
                "Databricks provider with `codify setup`, to route through "
        ...
        # the Codify elicitation system rather than silently allowed.
        # ``None`` until the adapter installs it on first use.
        self._elicitation_handler: ElicitationHandler | None = None
        # Live Claude SDK clients keyed by Codify session id.
    ...
    async def _route_options_through_gateway_shim(self, options: SdkOptions) -> None:
        ...
        the :mod:`~codify.inner.claude_gateway_shim` module
    ...
    async def _can_use_tool_for_permission(
        ...
        """
        Route a Claude SDK permission request through the Codify elicitation system.

        Installed as ``options.can_use_tool`` when ``permission_mode`` is
        not ``"bypassPermissions"`` and an elicitation handler has been wired
        in by :class:`codify.runtime.harnesses._executor_adapter.ExecutorAdapter`.
        ...
        if allowed:
            return PermissionResultAllow()
        return PermissionResultDeny(message="Denied via Codify elicitation")
    ...
    async def _evaluate_tool_call_policy(
        ...
        evaluated against Codify TOOL_CALL-phase policies BEFORE they
        ...
        Double-evaluation guard: Codify's OWN tools are exposed as the
        single ``codify`` SDK MCP server (the model sees
        ``mcp__codify__*``). When the model calls one, the SDK wrapper
        routes it back through Codify's dispatch bridge
        ...
        (see ``codify/runner/app.py`` "All tool calls go through AP:/mcp
        ... which enforces TOOL_CALL + TOOL_RESULT policies server-side").
        Spec-declared MCP tools are surfaced through that same
        ``mcp__codify__*`` server, so they are covered there too.
        Evaluating ``mcp__codify__*`` here as well would double-count
        ...
        :returns: :class:`claude_agent_sdk.PermissionResultDeny` when a
            ...
            existing Codify elicitation handler before returning
            ...
            ``mcp__codify__*`` tool already gated on the dispatch path, or
        ...
        if _policy_eval is None:
            return None
        # Codify's own tools are already TOOL_CALL-gated server-side via
        # the dispatch bridge / ProxyMcpManager — don't evaluate them twice.
        if tool_name.startswith("mcp__codify__"):
            return None
        ...
        if _action == "POLICY_ACTION_ASK":
            ...
            reason = _verdict.reason or "Approval required by Codify TOOL_CALL policy"
            if self._elicitation_handler is None:
                logger.warning(
        ...
        if _action == "POLICY_ACTION_DENY":
            ...
            reason = _verdict.reason or "Denied by Codify TOOL_CALL policy"
            logger.info("TOOL_CALL policy denied tool=%s reason=%s", tool_name, reason)
            return PermissionResultDeny(message=reason)
        from claude_agent_sdk import PermissionResultDeny

        reason = f"Unexpected Codify TOOL_CALL policy verdict: {_action!r}"
    ...
    async def run_turn(
        ...
        # Build MCP tools from Codify tool schemas
        mcp_tools = _build_mcp_tools(tools, self._tool_executor) if tools else []

        # Create MCP server config for Codify tools. The SDK's
        ...
        if mcp_tools:
            mcp_servers["codify"] = sdk.create_sdk_mcp_server(
                name="codify",
                version="1.0.0",
                tools=mcp_tools,
            )
            system_prompt = _augment_system_prompt_for_codify_mcp_tools(
                system_prompt,
                tools,
            )

        # Build allowed_tools list.  OS-environment operations route
        # through Codify ``sys_os_*`` MCP tools rather than the
        ...
        # to a ``databricks-*`` model: the Codify producer always resolves a
        ...
        # stays because codify owns conversation persistence
        # via its own conversation store.
        # OS-environment tools are provided via Codify ``sys_os_*``
        ...
        # the CLI subprocess are evaluated against Codify TOOL_CALL
        ...
        # callback, call it with the request data so the Codify server
