"""Cursor preToolUse hook script for Codify policy enforcement.
...
PHASE_TOOL_CALL policy via the Codify server, and returns the
verdict on stdout.
...
def main() -> None:
    ...
    if resp is None:
        ...
        message = f"Tool '{tool_name}' blocked: Codify policy evaluation unavailable"
        if detail:
            message += f" ({detail})"
    ...
    try:
        ...
        json.dump(
            {
                "permission": "deny",
                "agent_message": (f"Tool '{tool_name}' blocked: malformed Codify policy response"),
            },
            sys.stdout,
        )
        return
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["agent_message"] = f"Tool '{tool_name}' denied by Codify policy: {reason}"
