"""Executor that bridges Codify web-chat turns into the native Goose TUI.

It does not launch ``goose`` — the ``codify goose`` wrapper already launched
...
from codify.goose_native_bridge import BRIDGE_DIR_ENV_VAR, inject_user_message
...
class GooseNativeExecutor(Executor):
    """Harness-side executor for ``codify goose`` web-UI turns.
