...
around a :class:`codify.inner.pi_executor.PiExecutor`
...
- ``HARNESS_PI_CWD``: working directory the executor launches
  the Pi CLI in. ``None`` falls back to ``CODIFY_RUNNER_WORKSPACE`` if set,
  then to the subprocess's inherited cwd.
- ``CODIFY_PI_PATH``: absolute path to a ``pi`` CLI binary.
...
from codify.inner.pi_executor import PiExecutor
...
def _build_pi_executor() -> Executor:
    ...
    :raises ImportError: If the ``pi`` CLI isn't on PATH and
        ``CODIFY_PI_PATH`` (legacy ``HARNESS_PI_PATH``) isn't set — the inner executor's
    ...
    return PiExecutor(
        cwd=os.environ.get(_ENV_CWD) or os.environ.get("CODIFY_RUNNER_WORKSPACE"),
