...
:class:`~codify.inner.datamodel.CredentialProxyEntry` the parent
...
class DatabricksProfileTokenProvider:
    ...
    def __init__(
        ...
        *,
        ...
        :raises CodifyError: If the SDK is unavailable or the profile has
            no ``host``.
        """
    ...
    def resolve(self) -> str:
        ...
        :raises CodifyError: If authentication fails.
        """
        with self._lock:
            now = self._clock()
...
def _databricks_config(profile: str) -> object:
    ...
    :raises CodifyError: If the ``databricks`` extra is not installed or
...
def _databricks_authenticate(config: object) -> str:
    ...
    :raises CodifyError: If authentication fails or returns a non-Bearer
        scheme.
    """
    try:
...
def _prepare_databricks_runtime(
    ...
    *,
    ...
    :raises CodifyError: If a profile can't be resolved or two profiles
