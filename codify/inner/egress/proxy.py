...
certificates signed by the CA from :mod:`~codify.inner.egress.ca`.
...
from codify.inner.egress.rules import (
    EgressRule,
    check_host,
    check_request,
    is_dns_safe_host,
...
class EgressProxy:
    ...
    :param upstream_ca_bundle: Optional path to a CA bundle for
        ...
        ``~/.cache/codify-egress`` (never mounted into the sandbox),
    ...
    :param auth_token: When set, every inbound proxy connection MUST
        carry ``Proxy-Authorization: Basic base64("codify:<token>")``
        ...
        :func:`codify.inner.os_env._HelperProcessClient.
    ...
    def _rewrite_authorization(
        ...
        - **Placeholder swap (opt-in).** When the request's
          ...
          :data:`~codify.inner.credential_proxy.SYNTHETIC_CREDENTIAL_PREFIX`
    ...
    async def _send_proxy_auth_required(writer: asyncio.StreamWriter) -> None:
        ...
        realm ``codify`` so any HTTP client following RFC 7235
        ...
        resp = (
            ...
            b'Proxy-Authenticate: Basic realm="codify"\r\n'
            b"Content-Type: text/plain\r\n"
            b"Content-Length: " + str(len(body)).encode() + b"\r\n"
            b"Connection: close\r\n"
            b"\r\n" + body
    ...
    def _check_proxy_auth(self, headers_raw: bytes) -> bool:
        ...
        - When set, the request MUST carry ``Proxy-Authorization:
          Basic <base64(codify:<token>)>``. Compared with
