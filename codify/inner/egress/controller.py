...
1. :class:`codify.inner.os_env._HelperProcessClient` — sandboxed
...
2. :class:`codify.inner.terminal.TerminalInstance` — interactive
...
from codify.inner.credential_proxy import CredentialRewriteRule
...
def start_egress_proxy(
    *,
    ...
    :param rules: Allow-list of HTTP(S) rules as
        ...
        :func:`codify.inner.egress.rules.parse_rules`).
    ...
    proxy = EgressProxy(
        ...
        # (``~/.cache/codify-egress/ca-bundle.pem`` from
...
def apply_egress_env(
    ...
    *,
    ...
    if auth_token is not None:
        proxy_url = f"http://codify:{auth_token}@127.0.0.1:{relay_port}"
    else:
