...
_DEFAULT_CACHE_DIR = os.path.expanduser("~/.cache/codify-egress")
...
def _generate_ca(cert_path: Path, key_path: Path) -> None:
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Codify Egress MITM CA"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "codify"),
        ]
    )
...
def _system_ca_bundle() -> bytes:
    ...
    Delegates cafile resolution to :func:`codify.tls.resolve_ca_file` (single
    ...
    """
    from codify.tls import resolve_ca_file
