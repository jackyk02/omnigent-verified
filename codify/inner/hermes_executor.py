...
turns to maintain conversational context across the Codify session without
re-serialising the full history.

Each turn yields text output as ``TextChunk`` / ``TurnComplete`` events.
Codify policies are enforced on Hermes' native tool calls via Hermes'
...
Requirements:
    ...
    ``CODIFY_HERMES_PATH``; legacy ``HARNESS_HERMES_PATH`` still honored).
...
- ``CODIFY_HERMES_PATH`` — absolute path to the ``hermes`` CLI binary.
...
class HermesExecutor(Executor):
    ...
    history is maintained without Codify re-serializing the full
    message list.
    ...
    ``config.yaml`` that registers an Codify policy hook as a
    ...
    def _setup_hermes_home(self) -> None:
        ...
        When the Codify server URL and conversation ID are available,
        writes a ``config.yaml`` that registers the Codify policy hook as a
        Hermes ``pre_tool_call`` shell hook and an ``mcp_servers.codify``
        entry (``serve-mcp``) exposing Codify builtin tools to the model.
    ...
    def _hermes_session_id(self, session_key: str) -> str | None:
        """Return the stored Hermes session ID for an Codify session key."""
    ...
    def handles_tools_internally(self) -> bool:
        ...
        through Codify's tool dispatch.  Codify policies are enforced
        via Hermes' native ``pre_tool_call`` shell hook that evaluates
        ``PHASE_TOOL_CALL`` against the Codify server before each tool
        execution.
        """
        return True
    ...
    async def run_turn(
        ...
        :param messages: Conversation history from Codify.
    ...
    def _session_key(self, messages: list[Message]) -> str:
        """
        Derive a stable Codify session key from the message list.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        via `hermes --resume` outside Codify.
        """
