"""Core data model types for Codify, which are all exposed to agents and in configs."""
...
# whose concrete types live in other modules (codify.tools,
# codify.policies, ...). Importing them here would create import cycles,
...
class History:
    ...
    def get_context_window(self, max_tokens: int | None = None) -> list[Message]:  # noqa: ARG002
        ...
        :mod:`codify.runtime.compaction`, which operates on the
...
class ExecutorSpec:
    ...
    :param auth: Parsed auth block from the YAML (e.g. api_key +
        base_url). Carried through so the codify spec translator
...
class OSEnvSandboxSpec:
    ...
    # :func:`codify.inner.sandbox._default_sandbox_for_platform`,
    ...
    #   the rest of the tree first (see :mod:`codify.inner._cwd_scan`).
    ...
    # see :data:`codify.inner.os_env._DEFAULT_ENV_PASSTHROUGH`).
    ...
    # ``codify/inner/egress/proxy.py``). Defends against
...
class TerminalEnvSpec:
    ...
    :param terminal_transport: How the web UI attaches to this terminal:
        ...
        ``~/.codify/config.yaml`` opts out to ``pty``. A per-attach
...
class AgentDef:
    ...
    :param instructions: Resolved system-prompt text loaded from
        ...
        See :func:`codify.inner.loader.load_agent_def` for
