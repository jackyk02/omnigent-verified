...
:mod:`codify.runtime.harnesses._runner` invokes after the parent
process resolves ``"kimi"`` to this module via
:data:`codify.runtime.harnesses._HARNESS_MODULES`.

Wraps a :class:`codify.inner.kimi_executor.KimiExecutor` that drives
...
- ``HARNESS_KIMI_CWD`` — working directory the kimi subprocess runs in
  ...
  subprocess ``cwd=``).
- ``CODIFY_KIMI_PATH`` — path to the ``kimi`` binary. Default
...
``~/.kimi/config.toml`` (out-of-band from Codify) — upstream kimi
has no per-spawn ``--config-file`` or env-var provider override.
Codify-side provider injection remains a deferred follow-up.
"""
...
from codify.inner.kimi_executor import KimiExecutor, _resolve_skills_dirs
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    has its own internal sandbox / approval flow, so Codify does not
...
def _build_kimi_executor() -> Executor:
    ...
    return KimiExecutor(
        ...
        # else the runner's CODIFY_RUNNER_WORKSPACE (the cwd the user launched
