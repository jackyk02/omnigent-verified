...
``AsyncAgent`` per Codify conversation, created on a
...
evaluates PHASE_TOOL_CALL and cancels the run on DENY.  Bridged Codify
tools (MCP-wrapped) are already gated server-side via the dispatch bridge
and are skipped to avoid double evaluation.

Crucially, Codify's spec-declared tools (``sys_session_send`` et al.) are
bridged into Cursor **in-process** via the SDK's ``custom_tools``: each
:class:`~codify.inner.executor.ToolSpec` becomes a ``cursor_sdk.CustomTool``
...
# Codify's bridged-tool callback: (tool_name, args) -> awaitable result.
...
def _normalize_cursor_usage(raw: Mapping[str, object], model: str) -> _JsonObject:
    """Map Cursor SDK usage fields to the standard Codify usage dict."""
    ...
    # map to the Codify-standard cache_read_input_tokens /
...
def _build_cursor_prompt(
    ...
    *,
    ...
    first turn the Codify system prompt is prepended (the SDK has no separate
...
def _sdk_message_to_events(message: object) -> list[ExecutorEvent]:
    ...
    if mtype == "tool_call":
        ...
        # Codify tool name + args so the observed events (and any name-keyed
...
def _write_cursor_hooks(cwd: str, hook_script_path: str, server_url: str, session_id: str) -> Path:
    ...
    :param server_url: Codify server URL, e.g. ``"http://127.0.0.1:6767"``.
    ...
    wrapper = hooks_dir / "codify-hook.sh"
...
class _CursorSessionState:
    """Per-Codify-conversation SDK session state."""
...
class CursorExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param permission_mode: Codify permission stance. ``"auto"`` (default)
        ...
        # Codify's session (policy gating, sub-agent dispatch, logging).
    ...
    async def _evaluate_native_tool_policy(self, name: str, args: _JsonObject) -> _PolicyGate:
        ...
        <codify.inner.claude_sdk_executor.ClaudeSDKExecutor>` exposes
    ...
    def _make_execute(
        ...
        """Build a sync ``execute`` that bridges a cursor tool call to Codify.
    ...
    async def _ensure_session(
        ...
        try:
            ...
            from codify.onboarding.cursor_auth import CURSOR_EXTRA
    ...
    async def _close_state(self, state: _CursorSessionState) -> None:
        ...
        if state.hooks_file is not None:
            try:
                ...
                wrapper = state.hooks_file.parent / "codify-hook.sh"
