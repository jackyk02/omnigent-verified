...
output streams into the Codify conversation as chat, and Goose's mid-turn tool
...
ExecutorEvents and routes Goose's permission requests through Codify's
...
class GooseExecutor(Executor):
    ...
    async def _respond_to_agent_request(self, request: _AcpJsonObject) -> None:
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here, executed through the Codify OSEnvironment so
    ...
    def _usage_from_result(result: _AcpJsonObject) -> dict[str, int] | None:
        """Map Goose's final ``result.usage`` to Codify's usage keys.

        Goose reports ``{totalTokens, inputTokens, outputTokens}``; Codify's
