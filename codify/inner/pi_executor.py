...
event stream into Codify ExecutorEvents.

Codify tools are bridged into Pi via a generated JavaScript extension
that registers each tool with ``pi.registerTool()``.  Tool execution is
proxied over a local TCP socket to the Codify Python process, so
policies, history recording, sub-agents, runtime, and all other Codify
...
from codify.inner.agent_env import clean_agent_env
from codify.inner.native_attachments import parse_data_uri
from codify.llms._usage_observer import notify_from_dict as _notify_usage_from_dict
from codify.model_metadata import ModelMetadata, ModelWireAPI
from codify.onboarding.provider_config import CHAT_WIRE_API, RESPONSES_WIRE_API
from codify.pi_model_compatibility import SYSTEM_AI_RESPONSES_KEYWORDS, unsupported_in_pi
from codify.pi_native_credentials import (
    _databricks_workspace_url_for_gateway,
    _is_databricks_ai_gateway_url,
)
from codify.runner.identity import CODIFY_SESSION_ENV_VAR
...
# ---------------------------------------------------------------------------
# TCP tool server — serves Codify tools to the Pi extension
# ---------------------------------------------------------------------------
...
class _ToolServer:
    ...
    async def _evaluate_policy(  # type: ignore[explicit-any]
        ...
        Codify outage can't wedge the agent mid-turn.
        """
...
# ---------------------------------------------------------------------------
# Pi extension generator — bridges Codify tools into Pi
# ---------------------------------------------------------------------------
...
def _generate_extension_js(port: int, tool_schemas: list[ToolSpec], token: str) -> str:
    """Generate a JavaScript Pi extension that registers Codify tools.

    Each tool is forwarded to the TCP tool server at ``127.0.0.1:<port>``.

    :param port: TCP port the Codify tool server is listening on, e.g.
        ``54321``.
    :param tool_schemas: Codify tool schemas to register with Pi.
    ...
    return f"""\
// Auto-generated Codify tool bridge extension for Pi.
// Connects to the Codify TCP tool server on port {port}.
...
/**
 * Ask the Codify tool server for a TOOL_CALL policy verdict on a native
...
module.exports = function(pi) {{
  // Gate native (non-bridged) tool calls through Codify policy. Pi's
  ...
  pi.on("tool_call", async (event) => {{
    ...
    if (verdict && verdict.block) {{
      return {{ block: true, reason: verdict.reason || "blocked by Codify policy" }};
    }}
  }});

  for (const tool of TOOLS) {{
    // Pi passes tool.parameters directly to the LLM as JSON Schema, so we
    // can use the Codify schema as-is without TypeBox conversion.
    pi.registerTool({{
      name: tool.name,
      label: tool.name,
...
_PI_ENV_ALLOW_EXACT: frozenset[str] = frozenset(
    {
        ...
        "SHELL",
        "TZ",
        CODIFY_SESSION_ENV_VAR,  # "inside Codify" marker (CLAUDE_CODE/CODEX analog)
    }
)
...
async def _create_subprocess_exec(*args: Any, **kwargs: Any) -> asyncio.subprocess.Process:  # type: ignore[explicit-any]
    ...
    ``codify.inner.pi_executor.asyncio.create_subprocess_exec``
...
def _clean_pi_env(extra_allowed: Sequence[str] | None = None) -> dict[str, str]:
    ...
    Thin wrapper over :func:`codify.inner.agent_env.clean_agent_env`. The
...
def _build_pi_prompt(messages: list[Message], *, is_first_turn: bool) -> str | list[_JsonObject]:
    ...
    :param messages: Codify conversation history for the
        turn.
...
def _extract_pi_turn_usage(
    ...
    carries the resolved ``model``. This translates those into codify's
    usage schema (see :class:`codify.inner.executor.TurnComplete`).
...
def _aggregate_pi_turn_usage(
    ...
    A single Codify turn drives Pi's full agent loop, which may make
...
class PiExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param gateway_host: Gateway workspace host origin, e.g.
            ...
            ``HARNESS_PI_GATEWAY_HOST`` (written by the Codify workflow layer).
        ...
        # off (they don't route through Codify policies / history and
    ...
    async def _ensure_tool_server(self, tools: list[ToolSpec]) -> int | None:
        """Start the TCP tool server if there are Codify tools to bridge."""
        if not tools:
            return None
    ...
    async def _gate_native_tool(  # type: ignore[explicit-any]
        ...
        """Evaluate a native Pi tool call against Codify TOOL_CALL policy.
        ...
        PreToolUse hooks: the verdict is computed by the Codify server
    ...
    def _build_env_and_dir(
        ...
        :param tools: Codify tool schemas to bridge into Pi.
        :param tool_server_port: TCP port the Codify tool server is
        ...
        """
        env = dict(self._env)
        tmp_dir = tempfile.mkdtemp(prefix="codify_pi_")
        ...
        if self._gateway:
            ...
            from codify.inner.pi_settings import prepare_managed_pi_agent_dir
        ...
        # Generate the Codify tool bridge extension if tools are available.
        if tools and tool_server_port is not None:
            ...
            ext_path = os.path.join(tmp_dir, "codify_tools.js")
            ...
            # an Codify TOOL_CALL policy verdict; see
