...
:func:`codify.inner.sandbox._default_sandbox_for_platform`); spec
...
- The per-helper scratch tmpdir created by
  :func:`codify.inner.sandbox.create_private_tmpdir` is
...
1. :data:`codify.inner._seccomp.BASELINE_DENYLIST_SYSCALLS` — the
...
_HOST_SANDBOX_BACKEND_ENV = "CODIFY_HOST_SANDBOX_BACKEND"
...
def _detect_host_sandbox_backend() -> str | None:
    ...
    Prefers the explicit ``CODIFY_HOST_SANDBOX_BACKEND`` env var (set by
...
class BwrapSandboxBackend(SandboxBackend):
    ...
    def wrap_launcher_argv(
        ...
        :func:`codify.inner.sandbox.run_launcher`.
        """
    ...
    def activate(self, policy: SandboxPolicy) -> None:
        ...
        1. The shared k8s-derived baseline via
           :func:`codify.inner._seccomp.apply_baseline_denylist`.
...
def _bwrap_extra_seccomp_rules() -> list[SeccompRule]:
    ...
    on top of :func:`codify.inner._seccomp.apply_baseline_denylist`.
...
def _resolve_root(cwd: Path, root: str) -> Path:
    ...
    See :data:`codify.inner.seatbelt_sandbox._resolve_root`
...
def _ensure_executable_visible(argv: list[str], cwd: Path) -> list[str]:
    ...
    - ``/home/me/codify/.venv/bin/python`` while the helper
...
def _interpreter_reexpose_after_mask(
    ...
    ``uv tool``-installed codify at
    ``~/.local/share/uv/tools/codify/bin/python``), the dotfile masker
...
def _dotfile_and_symlink_mask_args(
    ...
    Thin emitter over :func:`codify.inner._cwd_scan.scan_cwd_mask_entries`:
    ...
    :func:`codify.inner._cwd_scan.merge_scan_roots` folds both grant
    ...
    See :mod:`codify.inner._cwd_scan` for the masking-decision
