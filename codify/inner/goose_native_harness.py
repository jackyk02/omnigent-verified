...
Wraps a :class:`codify.inner.goose_native_executor.GooseNativeExecutor`, which
injects web-UI messages into the running ``goose session`` TUI (launched by
``codify goose`` in the session terminal) via tmux. The bridge dir is read from
:data:`~codify.goose_native_bridge.BRIDGE_DIR_ENV_VAR` in the spawn env.
...
(``GOOSE_MODE`` / in-terminal prompts), which Codify does not intercept. Treat
...
from codify.inner.goose_native_executor import GooseNativeExecutor
