"""Executor that bridges Codify messages into a native Codex TUI thread."""
...
from codify.inner.codex_goal_command import goal_objective_from_content
...
from codify.inner.native_attachments import (
    materialize_attachment,
    parse_data_uri,
    unresolved_attachment_marker,
)
from codify.reasoning_effort import CODEX_EFFORTS, validate_effort
...
class CodexNativeExecutor(Executor):
    """
    Harness-side executor for ``codify codex`` web UI turns.
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        try:
            if pending:
                ...
                # cancel above already updated what Codify shows, and a
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from Codify. Ignored here;
        ...
        :param config: Per-turn executor config. Its ``model`` and
            ``extra["reasoning_effort"]`` (carrying the Codify web
...
def _model_effort_overrides(config: ExecutorConfig | None) -> dict[str, object]:
    ...
    A model or reasoning-effort change selected in the Codify web UI is
...
def _file_block_to_input_item(
    ...
    if path is not None:
        ...
        # codify/entities/conversation.py. Keep in sync.
