"""Startup banner builder for the Codify REPL and onboarding wizard.

Migrated from the legacy ``codify.inner.cli`` module during the
``--no-sessions-api`` sunset so that the Codify REPL and onboarding wizard can
...
def _display_width(text: str) -> int:
    ...
    :func:`rich.cells.cell_len` (rich >= 14, which codify requires) counts
...
def startup_banner_strings(
    ...
    *,
    ...
    :param info_lines: Explicit dim info rows shown beneath the title,
        ...
        BannerLine("~/codify", dim=True)]``. ``None`` falls
