...
around a :class:`codify.inner.qwen_executor.QwenExecutor`
...
- ``HARNESS_QWEN_CWD``: working directory the executor launches
  the Qwen CLI in. ``None`` falls back to ``CODIFY_RUNNER_WORKSPACE`` if set,
  then to the subprocess's inherited cwd.
- ``CODIFY_QWEN_PATH``: absolute path to a ``qwen`` CLI binary.
...
_ENV_QWEN_PATH = "CODIFY_QWEN_PATH"
...
def _build_qwen_executor() -> Executor:
    ...
    :raises ImportError: If the ``qwen`` CLI isn't on PATH and
        ``CODIFY_QWEN_PATH`` (legacy ``HARNESS_QWEN_PATH``) isn't set — the inner executor's
        constructor surfaces this as a clear ImportError.
    """
    cwd_raw = os.environ.get(_ENV_CWD) or os.environ.get("CODIFY_RUNNER_WORKSPACE")
    cwd = cwd_raw or None
