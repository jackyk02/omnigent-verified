"""Executor that delivers Codify web/mobile turns into a native Antigravity agy.

``codify antigravity`` runs the Antigravity ``agy`` CLI in a runner-owned tmux
terminal and mirrors its transcript into the Codify session via the RPC read
driver (the read path). This executor is the **write path**: when a turn is
submitted from the Codify web/mobile UI it delivers the user's message by
TYPING IT INTO the agy TUI pane over tmux
(:func:`codify.antigravity_native_bridge.inject_user_message_via_tui`), exactly
...
so the agy TUI and the Codify web mirror share ONE conversation in both
...
``GetAvailableModels``. The Codify ``ExecutorConfig.model``/``reasoning_effort``
...
from codify.llms.errors import PermanentLLMError
from codify.reasoning_effort import ANTIGRAVITY_EFFORTS, validate_effort_or_llm_error
...
class AntigravityNativeExecutor(Executor):
    """
    Harness-side executor for ``codify antigravity`` web UI turns.
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        (:func:`codify.antigravity_native_rpc.cancel_cascade_steps`).
        ...
        .. note:: **Scope — RUNNING cascades only (live-verified, C3).**
           ...
           **NO-OP on a step that is WAITING for a user interaction**
           ...
           (:mod:`codify.antigravity_native_interactions`), NOT here — this
    ...
    async def run_turn(
        ...
        (:func:`codify.antigravity_native_rpc.send_user_cascade_message`), which
        ...
        :param tools: Tool schemas from Codify. Ignored; native agy owns its
            own tool surface.
    ...
    async def _deliver(self, text: str) -> str | None:
        ...
        :func:`codify.antigravity_native_bridge.inject_user_message_via_tui`)
        ...
        discovers/binds — see :mod:`codify.antigravity_native_reader`).
...
def _request_session_id_from_env() -> str | None:
    """
    Resolve the Codify session id that requested this harness process.

    :returns: Codify session id, e.g. ``"conv_abc123"``, or ``None``.
    """
...
def _latest_requested_model(steps: list[dict[str, object]]) -> str | None:
    ...
    field :func:`codify.antigravity_native_rpc.send_user_cascade_message` sends
    ...
    :param steps: Trajectory steps as returned by
        :func:`codify.antigravity_native_rpc.get_trajectory_steps`.
...
def _recommended_model(catalog: dict[str, object]) -> str | None:
    ...
    :param catalog: The parsed ``GetAvailableModels`` response as returned by
        :func:`codify.antigravity_native_rpc.get_available_models`.
