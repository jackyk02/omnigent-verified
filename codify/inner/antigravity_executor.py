...
counterpart to :class:`~codify.inner.openai_agents_sdk_executor.OpenAIAgentsSDKExecutor`
and :class:`~codify.inner.claude_sdk_executor.ClaudeSDKExecutor`: a direct
in-process Executor with ``handles_tools_internally() -> True``, per-session
agent reuse, and a streaming :meth:`run_turn` mapping SDK events onto Codify
:class:`~codify.inner.executor.ExecutorEvent` instances.
...
# back through the Session registry, so the in-SDK agent reaches Codify's
...
def _ensure_antigravity_sdk() -> ModuleType:
    ...
    try:
        ...
        from codify.onboarding.antigravity_auth import ANTIGRAVITY_EXTRA
...
def _latest_user_text(messages: list[Message]) -> str:
    ...
    :param messages: The Codify turn message list (role / content dicts),
...
def _render_prior_history(prior: list[Message]) -> str:
    ...
    :param prior: The conversation messages preceding the latest user turn
        (``messages[:-1]``), as Codify role/content dicts.
...
class AntigravityExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # invocations through this to reach Codify's tools under policy.
    ...
    def _session_key(self, messages: list[Message]) -> str:
        ...
        :param messages: The Codify turn message list; the last entry's
    ...
    def _tool_signature(tools: list[ToolSpec]) -> str:
        ...
        :param tools: Codify tool specs for the turn.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        :param session_key: The Codify session id whose agent to release.
        """
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        :param session_key: The Codify session id to interrupt.
    ...
    async def run_turn(
        ...
        :param tools: Codify tool specs exposed to the agent as callables.
    ...
    async def _ensure_agent(
        ...
        *,
        ...
        :param session_key: The Codify session id.
        :param model: The resolved model id to pin, or ``None`` to use the SDK
            default.
        :param system_prompt: The agent's system instructions.
        :param tools: Codify tool specs to expose to the agent.
    ...
    async def _open_agent(
        ...
        *,
        ...
        agent's async context. Codify's tools are exposed as callables
        ...
        :param model: The resolved model id to pin, or ``None`` to use the SDK
            default.
        :param system_prompt: The agent's system instructions.
        :param tools: Codify tool specs to expose as callables.
    ...
    def _build_post_tool_hook(
        ...
        class _CodifyToolCompleteHook(antigravity.hooks.PostToolCallHook):  # type: ignore[misc, name-defined]
        ...
        return _CodifyToolCompleteHook()

    def _build_sdk_tools(self, tools: list[ToolSpec]) -> list[SDKTool]:
        """Build SDK tools (plain callables) from Codify tool specs.
        ...
        the agent reaches Codify's tool registry under policy. Returns ``[]``
        when there are no tools or no executor bridge yet (the agent then runs
        with its native + MCP tools only).

        :param tools: Codify tool specs (``name`` / ``description`` /
            ``parameters``).
    ...
    def _make_tool_callable(self, tool_name: str, description: str) -> _ToolCallable:
        ...
        :param tool_name: The Codify tool name, e.g. ``"sys_shell"``.
    ...
    def _extract_usage(meta: SDKUsage) -> _StrAnyDict | None:
        """Map an SDK ``UsageMetadata`` to Codify's usage dict shape.
