...
def _handle_scan_overflow(
    *,
    ...
    :param logger: The resolved logger to emit the CRITICAL warning on
        ...
        logger), e.g. ``logging.getLogger("codify.inner._cwd_scan")``.
...
def merge_scan_roots(
    ...
    *root_lists: Sequence[Path] | None,
    ...
    roots (see
    :attr:`~codify.inner.sandbox.SandboxPolicy.mask_scan_skip_roots`):
    ...
    - ``recursive=False`` (the default; see
      :attr:`~codify.inner.datamodel.OSEnvSandboxSpec.cwd_hidden_scan_recursive`):
