...
class _CodifyYamlLoader(yaml.SafeLoader):
...
_CodifyYamlLoader.yaml_implicit_resolvers = {
    key: value[:] for key, value in yaml.SafeLoader.yaml_implicit_resolvers.items()
}
for key, resolvers in list(_CodifyYamlLoader.yaml_implicit_resolvers.items()):
    _CodifyYamlLoader.yaml_implicit_resolvers[key] = [
...
_CodifyYamlLoader.add_implicit_resolver(  # type: ignore[no-untyped-call]
    ...
    *,
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        **calls** it. This is the guard for the untrusted
        agent-bundle upload path: ``codify.spec.load`` routes a
        single-file codify YAML bundle here during
        ...
        ``codify run``, operator specs, the CLI) keep working with
    ...
    if isinstance(path_or_dict, (str, Path)):
        ...
        with open(path) as f:
            data = yaml.load(f, Loader=_CodifyYamlLoader)
...
def _read_contained_file(root: Path, value: str) -> str | None:
    ...
    Mirrors :func:`codify.spec.parser._read_contained_file`: resolves
...
def _resolve_instructions(
    ...
    Mirrors :func:`codify.spec.parser._resolve_instructions`
    so codify-flavored YAMLs and native Codify YAMLs treat the
    field identically:
    ...
    - File-path-shaped value that doesn't resolve: silently
      treated as inline text. Matches native Codify behavior so users
...
def _parse_os_env_spec(data: YamlData | str | bool | None) -> OSEnvSpec | None:
    ...
    # Mirror the Codify YAML parser's cross-field validation
    # (codify/spec/parser.py:_parse_os_env). Keeping the two
    ...
    # accepts with weaker semantics than the Codify parser would have
    # allowed).
...
def _effective_terminal_sandbox(
    ...
    :func:`codify.inner.terminal.build_terminal_os_env_spec`: if the
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    if raw_type is None:
        ...
        # (same behavior as the Codify YAML parser, kept in sync so legacy
        # and Codify loaders agree on what an "untyped" sandbox block means).
    ...
    # Mirror the Codify parser's hard reject of ``egress_rules`` paired with
    ...
    # (``codify.spec.parser._parse_credential_proxy``) rather than a
    # second copy so the single-file codify-YAML path and the
...
def load_agent_def_from_path(path_str: str) -> AgentDef:
    ...
    - ``foo.yaml`` (single-file codify YAML) — parsed directly.
    - ``my-agent/`` (codify AGENTSPEC directory with
      ``config.yaml``, or a directory containing exactly one
      codify YAML at its root).
    ...
    from codify.spec import _find_codify_yaml_in_dir

    if path.is_dir():
        resolved_dir = path
        cleanup: tempfile.TemporaryDirectory[str] | None = None
    else:
        from codify.spec.tar_utils import extract_safe

        cleanup = tempfile.TemporaryDirectory(prefix="codify-bundle-")
    ...
    try:
        ...
        if config_yaml.is_file():
            from codify.spec import load as load_agent_spec
        ...
        single = _find_codify_yaml_in_dir(resolved_dir)
        if single is None:
            raise FileNotFoundError(
                f"{path}: directory has neither a config.yaml "
                f"nor a single-file codify YAML at its root"
            )
