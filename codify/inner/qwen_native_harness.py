...
Wraps a :class:`codify.inner.qwen_native_executor.QwenNativeExecutor`, which
appends web-UI messages to the running ``qwen`` TUI's ``--input-file`` (launched
by ``codify qwen`` in the session terminal). The bridge dir is read from
:data:`~codify.qwen_native_bridge.BRIDGE_DIR_ENV_VAR` in the spawn env.
...
Codify does not intercept. qwen *can* delegate approval externally
(``can_use_tool`` control requests on ``--json-file``, answered via
``confirmation_response`` on ``--input-file``) — wiring that through Codify's
...
from codify.inner.qwen_native_executor import QwenNativeExecutor
