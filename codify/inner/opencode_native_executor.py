"""Executor that bridges Codify web turns into a native OpenCode session.

Built on :class:`codify.native_server_harness.NativeServerHarness`: the
runner owns the ``opencode serve`` process + SSE forwarder, and this
executor injects the latest web turn over the
:class:`codify.opencode_http_transport.OpenCodeHttpTransport` using the
...
from codify.native_server_harness import NativeServerHarness
from codify.native_server_transport import NativePrompt
from codify.opencode_http_transport import OpenCodeHttpTransport
...
class OpenCodeNativeExecutor(NativeServerHarness):
    """
    Harness-side executor for ``codify opencode`` web UI turns.
