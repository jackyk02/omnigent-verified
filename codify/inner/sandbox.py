...
from codify.runner.identity import RUNNER_AUTH_SECRET_ENV_VARS
...
_SANDBOX_STRACE_ENV = "CODIFY_SANDBOX_STRACE"
...
_LAUNCHER_WRAPPED_ENV = "CODIFY_LAUNCHER_SPAWN_WRAPPED"
...
_LAUNCHER_PRIVATE_TMPDIR_ENV = "CODIFY_LAUNCHER_SPAWN_PRIVATE_TMPDIR"
...
class SandboxPolicy:
    ...
    :param deny_unix_socket_paths: Absolute paths to AF_UNIX (pathname)
        ...
        ``[Path("/tmp/codify-terminal-ab12/tmux.sock")]``.
    ...
    dropped — see :func:`codify.inner.os_env._HelperProcessClient.
...
class SandboxBackend(ABC):
    ...
    def wrap_launcher_argv(
        ...
        Called by :class:`codify.inner.os_env._HelperProcessClient`
        ...
        :param argv: The unwrapped command argv the parent intends to
            ...
            "codify.inner.os_env", "helper", "<encoded>"]``.
    ...
    def post_spawn(self, policy: SandboxPolicy, pid: int) -> ContainmentHandle | None:
        ...
        Called by :class:`codify.inner.os_env._HelperProcessClient` on
...
def _resolve_grant_root(cwd: Path, root: str) -> Path:
    ...
    :func:`codify.inner.bwrap_sandbox._resolve_root`). Relative entries
...
def create_private_tmpdir() -> Path:
    return Path(tempfile.mkdtemp(prefix="codify-osenv-"))
    ...
    *argv* and returns its exit code.
    ...
    if (
        ...
        try:
            ...
            # ``/tmp/codify-sandbox-*.py`` written by
            ...
            # ``codify.inner.sandbox`` imports succeed even when
            ...
            inline = (
                ...
                "from codify.inner.sandbox import run_launcher; "
            ...
            logger.info(
                "[codify-sandbox] spawn-time wrap re-exec backend=%s wrap_head=%s",
    ...
    logger.info(
        "[codify-sandbox] activating backend=%s active=%s target=%s",
        sandbox.backend_type,
        sandbox.active,
        target_path,
    )
    ...
    if os.environ.get(_SANDBOX_STRACE_ENV):
        ...
        if strace_bin is None:
            ...
            logger.warning(
                "[codify-sandbox] strace active; wrapping target with %s",
                strace_bin,
            )
    try:
        activate_sandbox(sandbox)
        logger.info("[codify-sandbox] activated; spawning target=%s", target_path)
        completed = subprocess.run(target_argv)
        logger.info("[codify-sandbox] target exited rc=%s", completed.returncode)
...
def create_exec_launcher(target_path: str, sandbox: SandboxPolicy) -> str:
    ...
    fd, path = tempfile.mkstemp(prefix="codify-sandbox-", suffix=".py")
    project_root = repr(str(_project_root()))
...
def get_backend(type_name: str) -> SandboxBackend:
    ...
    (:meth:`codify.inner.os_env._HelperProcessClient._start_locked`)
...
def _default_sandbox_for_platform() -> OSEnvSandboxSpec:
    ...
    (e.g. :func:`codify.inner.loader._parse_os_env_sandbox_spec`
    and :func:`codify.spec.parser._parse_os_env_sandbox`) to fill
    ...
    :meth:`codify.inner.bwrap_sandbox.BwrapSandboxBackend.resolve`,
...
def _project_root() -> Path:
    # File lives at codify/inner/sandbox.py; climb two levels to the
    # repo root that hosts `codify/` as a package.
