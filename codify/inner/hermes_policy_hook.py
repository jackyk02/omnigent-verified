"""Hermes ``pre_tool_call`` shell hook for Codify policy enforcement.
...
The hook evaluates ``PHASE_TOOL_CALL`` policy via the Codify server.
...
Environment variables (set by the wrapper shell script):
    ...
    _CODIFY_SERVER_URL  : Base URL of the Codify server
                            (e.g. ``http://127.0.0.1:6767``).
    _CODIFY_SESSION_ID  : Session / conversation ID for policy
                            evaluation.
"""
...
def main() -> None:
    server_url = os.environ.get("_CODIFY_SERVER_URL", "")
    session_id = os.environ.get("_CODIFY_SESSION_ID", "")
    ...
    # Codify relay tools are already gated when the relay dispatches them back
    ...
    if tool_name.startswith(("mcp_codify_", "mcp__codify__")):
        json.dump({}, sys.stdout)
        return
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["reason"] = f"Tool '{tool_name}' denied by Codify policy: {reason}"
        else:
            out["reason"] = f"Tool '{tool_name}' denied by Codify policy"
