...
Wraps a :class:`codify.inner.kimi_native_executor.KimiNativeExecutor`,
which injects web-UI messages into the running ``kimi`` TUI (launched by
``codify kimi`` in the session terminal) via tmux. The bridge dir is read
from :data:`~codify.kimi_native_bridge.BRIDGE_DIR_ENV_VAR` in the spawn env.

Tool policies: kimi-native enforces Codify's tool deny-policy via a
``PreToolUse`` hook (registered in the per-session ``config.toml`` built by
:mod:`codify.kimi_native_credentials`, dispatched to
:mod:`codify.kimi_native_hook`). A ``POLICY_ACTION_DENY`` verdict blocks the
...
answered in the TUI, which Codify cannot intercept). Connector/tool ASK
policies are not enforced (kimi owns the ask); treat the kimi TUI as the
approval surface, with Codify able to hard-deny.
"""
...
from codify.inner.kimi_native_executor import KimiNativeExecutor
