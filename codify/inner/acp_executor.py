...
:class:`~codify.inner.goose_executor.GooseExecutor` /
:class:`~codify.inner.qwen_executor.QwenExecutor`: it spawns whatever command a
...
internally. This executor translates the ACP event stream into Codify
:class:`ExecutorEvent`s and routes the agent's permission requests through
Codify's TOOL_CALL policy + human-consent elicitation.

Vs. the Goose executor this generalizes, it additionally: renders the agent's
tool calls as Codify tool cards (``tool_call`` → ``ToolCallRequest``,
...
from codify.inner._acp_codify_mcp import CodifyAcpMcp
...
class AcpAgentConfig:
    ...
    :param codify_mcp: Expose Codify's builtin tools to the agent via
        ``session/new.mcpServers`` (the shared ``serve-mcp`` relay). On by
        default; the global ``CODIFY_ACP_MCP=0`` kill switch also disables it.
    """
...
class AcpExecutor(Executor):
    ...
    def __init__(
        ...
        # reads/writes back to us (executed through the Codify OSEnvironment,
        ...
        # mid-turn ``session/request_permission`` routes through Codify's
        ...
        # attribute the SDK harnesses use); backs the Codify MCP relay.
        self._tool_executor: _ToolExecutor | None = None

        # Codify-tool MCP bridge — exposes builtin tools to the agent via
        # session/new.mcpServers (lazily started at first session; torn down in
        # :meth:`close`). ``_codify_tools`` is captured each turn for the relay.
        self._mcp = CodifyAcpMcp(label=config.name)
        self._codify_tools: list[ToolSpec] = []
        ...
        try:
            resp = await self._rpc(
                _AGENT_METHOD_INITIALIZE,
                {
                    ...
                    "clientInfo": {"name": "codify", "version": "1.0"},
                    "clientCapabilities": {
                        "fs": {
    ...
    async def _ensure_session(self) -> str:
        ...
        ``mcpServers`` carries Codify's builtin tools (via the shared serve-mcp
        relay) unless disabled — see :class:`CodifyAcpMcp`.
        """
        if self._session_id is not None:
            return self._session_id

        mcp_servers = self._mcp.session_new_servers(
            tools=self._codify_tools,
            tool_executor=getattr(self, "_tool_executor", None),
            loop=asyncio.get_event_loop(),
            enabled=self._config.codify_mcp,
        )
    ...
    async def _respond_to_agent_request(self, request: _AcpJsonObject) -> None:
        ...
        - ``session/request_permission`` — decide via Codify's TOOL_CALL policy
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          advertised, execute through the Codify OSEnvironment so the spec's
        ...
        try:
            if method == _AGENT_REQUEST_REQUEST_PERMISSION:
                ...
                error = {
                    "code": -32601,
                    "message": f"codify: unsupported ACP request method {method!r}",
                }
    ...
    async def _ensure_os_environment(self) -> OSEnvironment:
        ...
        if self._os_environment is None:
            ...
            if env is None:
                raise _AcpRequestError(-32603, "codify: no os_env for fs delegation")
    ...
    def _usage_from_result(result: _AcpJsonObject) -> dict[str, int] | None:
        """Map an agent's final ``result.usage`` to Codify's usage keys.

        ACP does not standardize usage, but agents that report it (Goose) use
        ``{totalTokens, inputTokens, outputTokens}``; Codify's
    ...
    async def run_turn(
        ...
        ``tools`` (Codify's builtin tool schemas) are captured for the Codify
    ...
    async def close(self) -> None:
        ...
        # Tear down the Codify MCP relay HTTP server + its bridge dir first.
