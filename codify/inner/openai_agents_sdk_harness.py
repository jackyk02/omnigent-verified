...
via :data:`codify.runtime.harnesses._HARNESS_MODULES`.

The registry key is ``"openai-agents"`` (matching the Codify
YAML ``executor.harness`` spelling and ``CodifyExecutor``'s
...
around a :class:`codify.inner.openai_agents_sdk_executor.OpenAIAgentsSDKExecutor`
...
- ``HARNESS_OPENAI_AGENTS_DATABRICKS_PROFILE``: Databricks-specific
  ...
  builder
  :func:`codify.runtime.workflow._build_openai_agents_sdk_spawn_env`
