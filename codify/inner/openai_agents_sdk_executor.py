...
Codify as the system of record for tools, policies, and session history.
Codify tools are exposed as SDK FunctionTools whose callbacks route back
through the Session's tool registry.

Conversation state is delegated to the SDK's session layer. Each Codify
Session gets its own SDK SQLiteSession keyed by the Codify session id.
"""
...
from codify.llms.errors import is_context_length_exceeded as _is_context_length_exceeded
from codify.reasoning_effort import OPENAI_AGENTS_EFFORTS, validate_effort
...
# Tool executor callable wired in by ``codify.Session``. The result
# is JSON-ish (dict[str, Any]) but the static type leaks ``Any`` through
# the kwargs dict. Constrained here to match codify.executor.
...
def _normalize_responses_items_for_chat(
    ...
    :func:`~codify.inner.open_responses_sdk._convert_messages_to_responses`
...
def _normalize_content_blocks_for_chat(
    ...
    for block in content:
        ...
        if block_type == "input_file":
            ...
            # Preserve inline images; strip Codify-only metadata.
...
def _copy_known_keys(block: _JsonObject, keys: tuple[str, ...]) -> _JsonObject:
    ...
    Codify content blocks can carry UI/history metadata such as
...
def _patch_openai_sse_keepalive_tolerance() -> None:
    ...
    if decoder is None or getattr(decoder, "_codify_keepalive_patch", False):
        return
    ...
    decoder._codify_keepalive_patch = True
...
def _get_openai_async_client(
    ...
    try:
        ...
        raise ImportError(
            ...
            "the package (`pip install 'codify[databricks]'`) or set "
    ...
    def __init__(
        ...
        *,
        ...
        :param base_url_override: Override the OpenAI-compatible base URL
            ...
            the Codify workflow layer). Required whenever ``gateway_host`` is set.
