"""Executor that bridges Codify web-chat turns into the native Hermes TUI.

It does not launch ``hermes`` — the ``codify hermes`` wrapper already launched
...
:class:`codify.inner.goose_native_executor.GooseNativeExecutor`.
"""
...
class HermesNativeExecutor(Executor):
    """Harness-side executor for ``codify hermes`` web-UI turns.
