"""Shared terminal-output styling for the Codify CLI.
...
that ``codify``'s output reads as one coherent product. See
...
* Machine-readable output (IDs, paths, config dumps, the ``version``
  string) goes to stdout via :data:`console`, so ``codify … | cat``
  stays clean.
...
#: ``CODIFY_NO_SPINNER`` convention in :mod:`codify._runner_startup`.
NO_BANNER_ENV_VAR = "CODIFY_NO_BANNER"
...
console = Console(theme=CODIFY_THEME, highlight=False)
#: Console for stderr — status, warnings, errors, and the brand banner.
err_console = Console(stderr=True, theme=CODIFY_THEME, highlight=False)
...
def show_banner(*, isatty: bool | None = None, env: dict[str, str] | None = None) -> bool:
    ...
    and can be force-disabled with ``CODIFY_NO_BANNER``. Color *within*
...
def print_landing(
    *,
    ...
    banner is suppressed (non-TTY or ``CODIFY_NO_BANNER``).
...
def print_brandmark(subtitle: str | None = None) -> None:
    """
    Print the compact one-line brandmark (``✦ codify``), TTY-gated.
