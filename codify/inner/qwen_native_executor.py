"""Executor that bridges Codify web-chat turns into the native qwen TUI.

It does not launch ``qwen`` — the ``codify qwen`` wrapper already launched the
...
:mod:`codify.qwen_native_forwarder` mirrors the JSON event stream.
...
class QwenNativeExecutor(Executor):
    """Harness-side executor for ``codify qwen`` web-UI turns.
