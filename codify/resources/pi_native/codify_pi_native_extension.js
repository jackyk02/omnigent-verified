// Auto-generated Codify bridge extension for native Pi sessions.
const fs = require("fs");
...
// (codify.native_policy_hook.post_evaluate_with_retry, ~1-day client
...
// _codify_elicitation_id so the server RE-ATTACHES to the existing
...
// codify.policies.types.FAIL_CLOSED_PHASES and the Python native hook's
// fail_closed_hook_output(PreToolUse) → deny.
const _FAIL_CLOSED_REASON =
  "blocked: Codify policy server unreachable — failing closed (PHASE_TOOL_CALL)";
...
function readConfig() {
  const configPath = process.env.CODIFY_PI_NATIVE_CONFIG;
...
/**
 * Evaluate a TOOL_CALL policy for a native Pi tool via the Codify server's
 ...
 * hooks. It does NOT require an active Codify turn context on the harness
 ...
 *       (headersTimeout) we re-POST the SAME _codify_elicitation_id so the
...
async function evalNativePolicyHttp(config, toolName, args) {
  ...
  const body = JSON.stringify({
    ...
    _codify_elicitation_id: elicitationId,
  });
  ...
  while (true) {
    ...
    if (result === "POLICY_ACTION_DENY") {
      return {
        block: true,
        reason: json.reason || "blocked by Codify policy",
      };
    }
 ...
 *
 * Pi expects ``{ content: [{ type: "text", text }], isError }``. The Codify
...
function piResultFromMcpResponse(json) {
  if (json && typeof json === "object" && json.error) {
    const msg =
      (json.error && json.error.message) || "Codify tool call failed";
  ...
  // masquerading as a successful tool result. callCodifyTool detects and
  ...
  if (result && typeof result === "object" && result.resultType === "input_required") {
    return {
      content: [
        {
          type: "text",
          text: "Codify tool call requires approval that was not resolved",
        },
      ],
      isError: true,
    };
 ...
 *
 * The Codify server keys ``inputRequests`` by the server-minted elicitation
      ...
      return {
        piResult: {
          content: [
            {
              type: "text",
              text: `Codify tool call failed: HTTP ${resp.status}`,
            },
          ],
          isError: true,
        },
    ...
    return {
      piResult: {
        content: [
          {
            type: "text",
            text: `Codify tool call failed: ${err && err.message ? err.message : String(err)}`,
          },
        ],
        isError: true,
      },
...
/**
 * Execute an Codify tool by POSTing a JSON-RPC ``tools/call`` request to the
 * server's per-session MCP proxy endpoint
 * (``POST /v1/sessions/{sessionId}/mcp``).
 *
 * This is the SAME endpoint the runner's ``ProxyMcpManager`` uses: the Codify
 ...
 * agent loop.
 */
async function callCodifyTool(config, toolName, args) {
  if (
    !config ||
    !config.serverUrl ||
    !config.sessionId ||
    ...
    return {
      content: [
        { type: "text", text: "Codify tool bridge is not configured" },
      ],
      isError: true,
    };
  }
  ...
  if (!initial.elicitationId || !initial.requestState) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "Codify tool call requires approval but the server sent no resolvable elicitation",
        },
      ],
      isError: true,
    };
  ...
  if (verdict === null) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "Codify tool call requires approval but the policy server was unreachable",
        },
      ],
      isError: true,
    };
  ...
  if (mcpInputRequired(retry.json) !== null) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "Codify tool call still requires approval after one round — not retrying",
        },
      ],
      isError: true,
    };
 ...
 * (codify/inner/pi_executor.py): Pi (``@earendil-works/pi-coding-agent``)
...
async function postEvent(config, body) {
  ...
  try {
    ...
    // Keep Pi responsive even if Codify is temporarily unavailable.
  }
}
...
function setCodifyStatus(config, ctx, state) {
  if (!ctx || !ctx.ui || !config) return;
  const urlLabel = config.conversationUrl
    ? `Codify: ${config.conversationUrl}`
    : "Codify";
  const label = state ? `${urlLabel} · ${state}` : urlLabel;
  try {
    ctx.ui.setTitle(`Codify: ${config.sessionId}`);
    ctx.ui.setStatus("codify", label);
    ctx.ui.setStatus("codify_state", undefined);
  } catch (_err) {}
}
 ...
 * must run here (the Codify server's AP-side compaction would only
    ...
    await postEvent(config, {
      ...
      data: {
        ...
        item_data: {
          ...
          message:
            "Codify: /compact is unavailable for this Pi session. The " +
 ...
 * Codify web UI must be applied here (the ``--model`` launch arg is baked in
 ...
 *     resulting model back to Codify, so the web pill reflects the switch.
 */
async function applyModelChange(pi, config, ctx, modelId) {
  ...
  if (!pi || typeof pi.setModel !== "function" || !listModels) {
    await postModelChangeError(
      config,
      `Codify: could not switch to model "${id}" — this Pi session exposes ` +
  ...
  if (!model) {
    await postModelChangeError(
      config,
      `Codify: model "${id}" is not available in this Pi session.`,
    );
    return false;
  }
  try {
    ...
    if (applied === false) {
      await postModelChangeError(
        config,
        `Codify: could not switch to model "${id}" — no API key is ` +
          "configured for it.",
      );
      return false;
    }
    ...
    await postModelChangeError(
      config,
      `Codify: switching to model "${id}" failed inside Pi.`,
    );
    return false;
  }
}
...
/**
 * Report Pi's live model catalog to Codify for the Web UI model picker.
 *
 * Sourced from Pi's model registry — the models Pi actually loaded for THIS
 * session, whatever their origin: an Codify-configured provider's generated
 ...
 * picker populates in every auth path, including ``/login`` where no Codify
 * models.json exists.
 *
...
function startInboxPoller(pi, config, handleInterrupt, handleCompact, handleModelChange) {
  if (!config || !config.inboxDir || pi.__codifyInboxPoller) return;
  ...
  pi.__codifyInboxPoller = setInterval(() => {
    let files = [];
    try {
      files = fs
          ...
          postEvent(config, {
            ...
            data: {
              ...
              item_data: {
                ...
                message:
                  `Codify: a queued follow-up message (id ${droppedId}) could ` +
...
module.exports = function (pi) {
  ...
  // Names of the Codify tools registered via pi.registerTool below. Bridged
  ...
  if (config && Array.isArray(config.tools)) {
    for (const tool of config.tools) {
      ...
      // Codify schema is usable as-is. execute() round-trips the call to the
      // Codify server's MCP proxy and returns the result to Pi.
      if (typeof pi.registerTool === "function") {
        pi.registerTool({
          ...
          async execute(_toolCallId, params) {
            return callCodifyTool(config, name, params || {});
          },
        });
      }
    }
  ...
  pi.registerCommand("codify", {
    description: "Show the Codify conversation URL",
    async handler(_args, ctx) {
      setCodifyStatus(config, ctx, "linked");
      if (ctx && ctx.ui && config && config.conversationUrl) {
        ctx.ui.notify(`Codify: ${config.conversationUrl}`, "info");
      }
    },
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // ``/login`` session (no Codify ``model_override``, no ``llm_model``)
  ...
  pi.on("model_select", async (event, ctx) => {
    ...
    // Ctrl+P cycling) back to Codify so the web picker reflects it. Skip
  ...
  pi.on("agent_start", async (_event, ctx) => {
    ...
    agentRunning = true;
    setCodifyStatus(config, ctx, "running");
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    clearPendingInterrupt();
    agentRunning = false;
    setCodifyStatus(config, ctx, "idle");
    activeResponseId = null;
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // Bridged Codify tools (registered via pi.registerTool above) are
    ...
    // Evaluate TOOL_CALL policy via the Codify server's session-level HTTP
    ...
    // of whether an Codify turn is currently in flight.
    ...
    if (verdict && verdict.block) {
      return {
        block: true,
        reason: verdict.reason || "blocked by Codify policy",
      };
    }
  });
  ...
  pi.on("message_end", async (event, ctx) => {
    rememberContext(ctx);
    replayPendingInterrupt(ctx);
    setCodifyStatus(config, ctx, undefined);
