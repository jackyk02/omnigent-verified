...
//
// Run with: node codify/resources/pi_native/codify_pi_native_extension.test.js
//
// Manual reproduction of the original bug (for context):
//   1. Start a native Pi session linked to Codify and let it go idle.
...
const EXT_PATH = path.resolve(__dirname, "codify_pi_native_extension.js");
...
(async () => {
  try {
    ...
    for (const h of harnesses) {
      if (h.pi.__codifyInboxPoller) clearInterval(h.pi.__codifyInboxPoller);
      try {
