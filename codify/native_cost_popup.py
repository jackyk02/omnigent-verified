...
verdict to the Codify server's elicitation-resolve endpoint — the **same**
...
:func:`codify.claude_native_bridge.display_cost_approval_popup`) as::

    python -I -m codify.native_cost_popup \
...
def launch_blocked_notice(
    ...
    *,
    ...
    if not clients:
        return
    python = python_executable or sys.executable
    argv = [python, "-I", "-m", "codify.native_cost_popup", "--notice", "--message", message]
    if policy_name:
        argv += ["--policy-name", policy_name]
    inner_cmd = shlex.join(argv)
    for client in clients:
...
def _read_codify_routing(config_file: Path) -> dict[str, object]:
    """
    Read Codify base URL + auth headers from a harness routing-config file.
    ...
    :raises SystemExit: If the file is missing, unreadable, not JSON, or
        ...
        for "where is the Codify server", so this fails loud (the popup just
    ...
    try:
        ...
        raise SystemExit(
            f"cost-approval popup: cannot read Codify config {config_file}: {exc}"
        ) from exc
    if not isinstance(raw, dict) or not isinstance(raw.get("ap_server_url"), str):
        raise SystemExit(
            f"cost-approval popup: Codify config {config_file} has no 'ap_server_url' string"
        )
    return raw
...
def _post_verdict(
    *,
    ...
    """
    POST the verdict to the Codify elicitation-resolve endpoint.

    Hits ``POST /v1/sessions/{id}/elicitations/{eid}/resolve`` with an
    MCP :class:`~codify.server.schemas.ElicitationResult` body — the
    ...
    :param ap_server_url: Codify server base URL, e.g.
        ``"http://127.0.0.1:8787"``.
...
def main(argv: list[str] | None = None) -> int:
    ...
    Reads Codify routing up front (so a missing/invalid config fails loud
    ...
    """
    parser = argparse.ArgumentParser(prog="codify.native_cost_popup")
    parser.add_argument(
        "--notice",
        action="store_true",
    ...
    config = _read_codify_routing(Path(args.config_file))
