...
from codify._platform import IS_WINDOWS, resolve_repo_symlink
from codify.cli_best_of_n import register_best_of_n_commands as _register_best_of_n_commands
from codify.cli_common import (
    RESUME_PICKER_SENTINEL as _RESUME_PICKER_SENTINEL,
)

# Interactive harness/credential configuration lives in codify.cli_config; the
# config/setup Click commands and the first-run plan call these entry points.
from codify.cli_config import (
...
from codify.cli_native import register_native_commands as _register_native_commands
from codify.cli_sandbox import lakebox as _lakebox_alias_group
from codify.cli_sandbox import sandbox as _sandbox_group
from codify.config import (
...
from codify.host.local_server import (
    _DEFAULT_LOCAL_PORT,
    _pid_alive,
    ensure_local_codify_server,
    local_server_status,
    local_server_url_if_healthy,
    server_config_signature,
    stop_local_codify_server,
    stop_untracked_local_server,
)
from codify.inner import _proc, ui
from codify.integration_daemon import IntegrationDaemon
from codify.onboarding.sandboxes import available_providers as _sandbox_providers
from codify.process_logging import LOG_LEVEL_ENV_VAR, LOG_TO_STDERR_ENV_VAR

if TYPE_CHECKING:
    import httpx

    from codify.onboarding.acp_auth import AcpAgentEntry
    from codify.update_check import _InstalledWheelInfo
...
def _build_external_routing_client(
    ...
    from codify.server.smart_routing import _bearer_auth
    ...
    if api_key:
        from codify.spec import expand_env_vars

        auth = _bearer_auth(expand_env_vars({"api_key": api_key})["api_key"])
    elif profile:
        databricks_profile = profile

    from codify.server.smart_routing import ExternalRoutingClient
...
def _build_local_llm_routing_client(
    ...
    if server_llm is None:
        return None
    from codify.runtime.policies.builder import (
    ...
    if policy_client is None:
        return None
    from codify.server.smart_routing import LLMRoutingClient
...
def _server_uvicorn_log_config(
    ...
    *,
    ...
    Uvicorn emits the FastAPI access line itself, so Codify standardizes
    ...
    from codify.process_logging import (
        DEFAULT_LOG_DATEFMT,
        DEFAULT_LOG_FORMAT,
    ...
    log_config["formatters"]["default"] = {
        "()": "codify.process_logging.TerminalLogFormatter",
        "fmt": DEFAULT_LOG_FORMAT,
        "datefmt": DEFAULT_LOG_DATEFMT,
        "use_colors": use_terminal_colors,
    }
    log_config["formatters"]["access"] = {
        "()": "codify.server.performance_metrics.RequestDurationAccessFormatter",
    ...
    if log_path is not None:
        ...
        if mirror:
            log_config["handlers"]["server_terminal"] = {
                "()": "codify.process_logging.terminal_stream_handler",
                "formatter": "default",
                "level": level_name,
            }
...
# Tests may set ``CODIFY_CONFIG_HOME`` to isolate subprocesses from a
# developer's real ``~/.codify/config.yaml``.
_CONFIG_HOME_ENV_VAR = "CODIFY_CONFIG_HOME"
_GLOBAL_CONFIG_PATH: Path = Path.home() / ".codify" / "config.yaml"

# Per-user state directories before / after the omniagents -> codify rename.
...
# directory on first run. ``CODIFY_DATA_DIR`` is the data-isolation override
# a worktree / test sets; when present the user manages their own state and
# migration is skipped.
_STATE_DIR: Path = Path.home() / ".codify"
# Pre-rename state directories, newest first. The name evolved
# ``~/.omniagents`` -> ``~/.codifys`` -> ``~/.codify``; migrate from the
# newest legacy directory that still exists.
_LEGACY_STATE_DIRS: tuple[Path, ...] = (
    Path.home() / ".codifys",
    Path.home() / ".omniagents",
)
_DATA_DIR_ENV_VAR = "CODIFY_DATA_DIR"


def _migrate_legacy_state_dir() -> None:
    """
    One-time relocation of a pre-rename state directory to ``~/.codify``.

    Earlier releases stored all per-user state under ``~/.omniagents`` and then
    ``~/.codifys`` as the name evolved. To avoid silently losing that state,
    move the newest surviving legacy directory to ``~/.codify`` on first run,
    but only when **all** of the following hold:

    - the new ``~/.codify`` does not yet exist (never clobber new state),
    ...
    CLI; a fresh ``~/.codify`` is then created normally and the legacy
    directory is left untouched for the user to migrate by hand. Idempotent:
    once ``~/.codify`` exists this is a no-op.
    ...
    if legacy_pid_file.exists():
        ...
        if legacy_pid is not None and _pid_alive(legacy_pid):
            click.echo(
                ...
                "is still running from it; skipping migration. Run `codify stop` "
                "and re-run to migrate, or move it manually to ~/.codify.",
                err=True,
            )
            return
    ...
    try:
        ...
        click.echo(
            f"Note: could not migrate {legacy_src} to ~/.codify ({exc}); "
            f"starting with fresh state. Your old data is untouched at {legacy_src}.",
            err=True,
        )
        return
    click.echo(f"Migrated per-user state from {legacy_src} to ~/.codify.", err=True)


# Project-level config relative to cwd, analogous to .git/config.
# Resolved at call time so tests can control cwd.
_LOCAL_CONFIG_RELPATH: Path = Path(".codify") / "config.yaml"

# Keys that ``codify config`` accepts.  Mirrors the option names in
...
_GLOBAL_CONFIG_KEYS: frozenset[str] = frozenset(
    {
        # Best-of-N verifier block: proposer roster, selection criteria,
        # and llm_verifier.select tuning (see codify.best_of_n).
        "best_of_n",
        "default_agent",
        "harness",
        "model",
...
_GLOBAL_AGENTS_DIR: Path = Path.home() / ".codify" / "agents"
...
# Overridable via CODIFY_SERVER_SHUTDOWN_TIMEOUT_S for deployments that
...
_SERVER_GRACEFUL_SHUTDOWN_TIMEOUT_S = int(
    os.environ.get(
        "CODIFY_SERVER_SHUTDOWN_TIMEOUT_S",
...
def _effective_global_config_path() -> Path:
    """
    Return the path to the user-level Codify config.

    :returns: ``$CODIFY_CONFIG_HOME/config.yaml`` when the env
...
def _display_path(path: Path) -> str:
    ...
    hardcoded ``~/.codify/...`` literal, this reflects the *actual*
    effective path — so a state dir outside ``$HOME`` (an
    ``CODIFY_CONFIG_HOME`` / ``CODIFY_DATA_DIR`` override) renders as
    its real location rather than a misleading ``~``.

    :param path: The path to display, e.g.
        ``Path("/Users/alice/.codify/logs/server/server-ab12.log")``.
    :returns: ``"~/.codify/..."`` when *path* is under ``$HOME``,
        otherwise ``str(path)``.
    """
    try:
        return f"~/{path.relative_to(Path.home())}"
    except ValueError:
        # Not under $HOME (e.g. an CODIFY_DATA_DIR outside home).
        return str(path)
...
def _display_config_path(path: Path) -> str:
    ...
    :param path: The config path to display, e.g.
        ``Path("/Users/alice/.codify/config.yaml")``.
    :returns: ``"~/.codify/config.yaml"`` when *path* is under
...
def _load_global_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    """
    Load the global codify config from ``~/.codify/config.yaml``.
    ...
    ``codify setup`` and used by the runtime to supply executor
...
def _load_local_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    """
    Load the project-level config from ``.codify/config.yaml`` in cwd.
...
def _load_effective_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    Precedence (highest last): global (``~/.codify/config.yaml``)
    → local (``.codify/config.yaml`` in cwd).  Project config
    always wins so per-repo settings override user defaults.

    The ``harness`` mapping is deep-merged (per-harness sub-keys, local
    winning per-field) via :func:`codify.config._merge_effective_config`
...
def _peek_default_agent_harness(target: str) -> str | None:
    ...
    :param target: The configured ``default_agent`` value, e.g.
        ``"/Users/me/.codify/agents/databricks_coding_agent.yaml"``.
...
def _bundled_example_path(name: str) -> str:
    ...
    Located via the packaged ``codify.resources.examples`` (symlinks to
    ``examples/<name>`` in a dev checkout, real directories in an installed
    wheel), mirroring how the model catalog is located.

    :param name: Bundled example directory name, e.g. ``"best_of_n"``.
    :returns: Absolute path string to the agent directory.
    """
    import importlib.resources

    resource = importlib.resources.files("codify.resources.examples").joinpath(name)
...
def _pick_first_run_harness() -> _FirstRunPlan | None:
    ...
    key / CLI login counts as configured). Claude gets the bundled Best-of-N
    (multi-agent auto-verify) orchestrator as its default agent; Codex / Pi
    launch a bare harness REPL.
    Shared with ``configure harnesses`` via
    :func:`~codify.onboarding.provider_config.default_provider_for_harness`,
    so the two surfaces agree on "what's configured".

    :returns: A :class:`_FirstRunPlan`, or ``None`` when no harness has a usable
        credential.
    """
    from codify.onboarding.detected import effective_config_with_detected
    from codify.onboarding.provider_config import (
    ...
    if default_provider_for_harness(config, "claude-sdk") is not None:
        # Best-of-N (multi-agent auto-verify) is the default experience:
        # proposals from every configured harness, verifier-selected merge.
        return _FirstRunPlan(harness="claude-sdk", agent=_bundled_example_path("best_of_n"))
    ...
    from codify.onboarding.harness_install import KIMI_KEY, harness_cli_installed
...
def _resolve_first_run_plan() -> _FirstRunPlan | None:
    """Resolve the harness + default agent for a bare ``codify run``.
...
def _resolve_default_agent_target(
    ...
    if default_harness is not None:
        click.echo(
            f"codify: default agent '{default_agent}' uses harness "
...
def _parse_config_bool(key: str, value: _ConfigValue) -> bool:
    """
    Parse a boolean value from YAML or ``codify config KEY=VALUE``.
...
def _resolve_auto_open_conversation_setting(cfg: dict[str, Any]) -> bool | None:  # type: ignore[explicit-any]
    ...
    ``codify run`` uses this to default the browser-open ON for
...
def _resolve_auto_open_conversation_from_config(cfg: dict[str, Any]) -> bool:  # type: ignore[explicit-any]
    ...
    ``codify run`` does not use this resolver — it defaults the
...
def _normalize_harness_scalar_on_write(
    ...
    click.echo(
        f"codify: migrated `harness:` to the new mapping form in {path} (behavior unchanged)",
        err=True,
    )
    return True
...
def _save_global_config(  # type: ignore[explicit-any]
    ...
    """
    Merge *settings* into ``~/.codify/config.yaml`` and remove any
    keys listed in *unset_keys*.

    Creates the ``~/.codify/`` directory if it does not exist.
    Values may be plain strings, booleans, or nested mappings (the
    ``auth:`` block written by ``codify setup``, or a ``providers:``
    block written by ``codify setup --no-internal-beta``).
    ...
    *deep_merge_keys*, the incoming mapping is instead merged one level
    ...
    :func:`~codify.onboarding.provider_config.set_default_provider`,
...
def _materialize_bundled_example(name: str) -> Path:
    ...
    user-editable copy under ``~/.codify/agents`` and never overwrite an
    ...
    resource = resources.files("codify.resources.examples").joinpath(name)
    text = resource.read_text(encoding="utf-8")
    executable_placeholder = "__CODIFY_PYTHON_EXECUTABLE__"
    text = text.replace('"${CODIFY_HOME:-$PWD}/.venv/bin/python"', executable_placeholder)
    text = text.replace("${CODIFY_HOME:-$PWD}/.venv/bin/python", executable_placeholder)
...
def _save_local_config(
    ...
    """
    Merge *settings* into ``.codify/config.yaml`` in cwd and remove
    any keys listed in *unset_keys*.

    Creates the ``.codify/`` directory if it does not exist. Mirrors
...
def _default_db_uri() -> str:
    """Default DB URI for ``codify server`` — the machine-global
    ``<data_dir>/chat.db``.

    Resolves to the same path the ``codify run`` daemon spawns its
    local server against (``_local_data_dir()``, honoring
    ``CODIFY_DATA_DIR`` → else ``~/.codify``). Pinning ``server``
    to the same DB as ``run`` means there is **one local DB — and so one
    accounts admin — per machine**, instead of a fresh CWD-relative
    ``codify.db`` (and a fresh admin) for every directory you launch
    from. ``--database-uri`` / the config file still override.

    :returns: e.g. ``"sqlite:////home/alice/.codify/chat.db"``.
    """
    from codify.host.local_server import _local_data_dir

    return f"sqlite:///{_local_data_dir() / 'chat.db'}"


def _default_artifact_location() -> str:
    """Default artifact dir for ``codify server`` — ``<data_dir>/artifacts``.

    Kept in lock-step with :func:`_default_db_uri` so a default-config
    ``codify server`` and ``codify run`` share one coherent
    ...
    :returns: e.g. ``"/home/alice/.codify/artifacts"``.
    """
    from codify.host.local_server import _local_data_dir
...
def _ensure_sqlite_parent_dir(db_uri: str) -> None:
    ...
    ``<data_dir>/chat.db`` (machine-global, honoring ``CODIFY_DATA_DIR``),
    ...
    handles this in ``ensure_local_codify_server``; this is the equivalent for
    the foreground ``codify server`` command.

    No-op for non-SQLite URIs (Postgres etc.) and for in-memory SQLite.

    :param db_uri: The resolved store DB URI, e.g.
        ``"sqlite:////home/alice/.codify/chat.db"`` or
        ``"postgresql://host/db"``.
    :returns: None.
    """
...
def _apply_bind_auth_defaults(host: str) -> None:
    ...
    ``CODIFY_AUTH_PROVIDER`` keeps header/oidc, and
    ``CODIFY_AUTH_ENABLED`` (or its deprecated alias) pins auth on/off.

    Decision matrix for the env-unset default:

    - **Loopback** (``127.0.0.1`` / ``localhost`` / ``::1``) + header
      default → set ``CODIFY_LOCAL_SINGLE_USER=1``: the no-login
      header-mode ``"local"`` fallback. The user's own machine, no proxy
      to inject identity — same posture as the daemon / ``codify run``
      spawn paths.
    - **Non-loopback** (``0.0.0.0``, a network-exposed deploy) + no
      explicit auth → set ``CODIFY_AUTH_ENABLED=1``: accounts (login)
    ...
    :returns: None.
    """
    from codify.server.auth import resolve_auth_source as _resolve_auth_source

    _is_loopback_bind = host in ("127.0.0.1", "localhost", "::1")
    # Compose-style deploys pass CODIFY_AUTH_PROVIDER as an empty
    # string when unset ("${VAR:-}"), so empty and missing both mean
    # "not explicitly pinned".
    _raw_auth_provider = os.environ.get("CODIFY_AUTH_PROVIDER")
    ...
    if _is_loopback_bind and not _auth_provider_explicit and _resolve_auth_source() == "header":
        os.environ.setdefault("CODIFY_LOCAL_SINGLE_USER", "1")

    # Non-loopback + no explicit auth → accounts (login) mode.
    _auth_enabled_explicit = bool(os.environ.get("CODIFY_AUTH_ENABLED", "").strip())
    if not _is_loopback_bind and not _auth_provider_explicit and not _auth_enabled_explicit:
        os.environ.setdefault("CODIFY_AUTH_ENABLED", "1")
        click.echo(
            ...
            "account.\n"
            "    Set CODIFY_AUTH_ENABLED=0 first to override and stay in "
            "single-user mode.",
            err=True,
        )
...
def _create_artifact_store(location: str) -> Any:  # type: ignore[explicit-any]  # returns ArtifactStore protocol (optional deps)
    ...
    if location.startswith("dbfs:/Volumes/"):
        from codify.stores.artifact_store.databricks_volumes import (
...
def _preregister_agent(  # type: ignore[explicit-any]  # agent_store / artifact_store / agent_cache typed Any to avoid import cycle
    ...
    :func:`codify.spec.materialize_bundle`, tars it, validates
    ...
    :param agent_source: Either an agent-image directory containing
        ``config.yaml`` (standard codify shape) or a standalone
        codify YAML file (e.g.
    ...
    from codify.db.utils import generate_agent_id
    from codify.spec import load, materialize_bundle
    ...
    # :func:`codify.inner.cli._codify_register_yaml_bundle` —
    # see designs/RUN_CODIFY_SESSION_RESUMPTION.md. Reusing the
    ...
    # (``ondelete=CASCADE`` in
    # :class:`codify.db.db_models.SqlTask`), wiping every
    ...
    if existing is not None:
        ...
        if existing.bundle_location.rsplit("/", 1)[-1] != bundle_hash:
            ...
            # ``codify/server/routes/agents.py:248``.
...
def _format_version() -> str:
    ...
    ``setup.py`` wrote ``codify/_build_info.py``, the line is
    ...
    :returns: Either ``"codify 0.1.0"`` (no build info), or
        ``"codify 0.1.0 (010cf77c, built 2026-05-21T14:34:45Z)"``.
    """
    import datetime

    from codify.update_check import _read_build_info
    from codify.version import VERSION

    version_str = VERSION
    info = _read_build_info()
    if info is None:
        return f"codify {version_str}"
    epoch, sha = info
    ...
    if sha:
        ...
        return f"codify {version_str} ({sha[:8]}, built {when})"
    # _build_info exists but has no SHA (built without git available).
    return f"codify {version_str} (built {when})"
...
def _print_version_callback(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
    ...
    it ``_read_build_info()`` — during ``codify.cli`` import. The
    successful sub-import would then set ``codify._build_info`` as
    an attribute on the ``codify`` package object. Once that
    attribute exists, ``from codify import _build_info`` short-
...
class _CodifyCLI(click.Group):
    ...
    (``codify run --help``) is untouched.
    """

    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        from codify.inner import ui

        if ui.show_banner():
            from codify.version import VERSION

            epilogue = [("Get started", "codify setup")]
            if VERSION:
                epilogue.insert(0, ("Version", VERSION))
            ui.print_landing(tagline="coding, verified", epilogue=epilogue)
...
@click.option(
    ...
    help="Enable verbose DEBUG logging for Codify processes.",
)
@click.option(
    "--log-to-stderr",
    is_flag=True,
...
def cli() -> None:
    """Codify CLI — best-of-N multi-agent coding, auto-verified."""
...
def _warn_deprecated_harness_path_env_vars() -> None:
    ...
    replaced by ``CODIFY_<NAME>_PATH`` (one var per binary, ``-native`` suffix
    ...
    if not sys.stderr.isatty():
        return
    from codify.harness_startup_config import legacy_harness_path_env_vars_set

    for legacy, canonical in legacy_harness_path_env_vars_set():
        click.echo(
            f"codify: {legacy} is deprecated; set {canonical} instead. "
...
def main() -> None:
    ...
    # dispatch, imports — are all caught. See codify/crash_handler.py.
    from codify.crash_handler import install_crash_handler

    install_crash_handler(app_name="codify", repo="codify-ai/codify")

    cwd = os.getcwd()
    if cwd not in sys.path:
        sys.path.insert(0, cwd)

    # Relocate pre-rename ~/.omniagents state before anything reads ~/.codify
    ...
    # Bare ``codify`` with no args behaves like ``codify run`` on an
    ...
    # Shorthand: ``codify --harness claude [opts]`` →
    ...
    # Shorthand: ``codify myagent.yaml [opts]`` → ``run myagent.yaml [opts]``.
    # Allows ``codify`` to act as a transparent alias for ``codify run``
    ...
    if argv and _is_server_url(argv[0]):
        click.echo(
            ...
            f"Use `codify run --server {argv[0]}`.",
            err=True,
        )
        raise SystemExit(2)
    ...
    if _is_removed_ad_hoc_invocation(argv):
        click.echo(
            ...
            "`codify run <agent.yaml>` or "
            "`codify run --harness <harness>`.",
            err=True,
        )
        raise SystemExit(2)

    # Always-on diagnostics — captures exceptions, lifecycle events,
    # and warnings to ~/.codify/logs/cli/cli-*.log even when --log
    ...
    from codify.cli_diagnostics import (
        log_cli_error_hint,
        log_cli_exception,
        print_setup_hint,
        setup_cli_logging,
    ...
    # ``codify setup`` IS the setup wizard — if it fails, telling the
    # user to "run codify setup" would be circular. ``upgrade`` (and its
    ...
    if not _should_skip_update_check(argv) and sys.stderr.isatty():
        from codify.update_check import maybe_show_update_notice

        maybe_show_update_notice()

    # Terminal-visible deprecation notice for legacy ``HARNESS_*_PATH`` env
    # vars (now ``CODIFY_<NAME>_PATH``). Same gating as the update notice so
    ...
    try:
        ...
        # `codify setup` hint here: genuine crashes are rarely auth issues,
        ...
        from codify.crash_handler import handle_crash
...
def _is_run_shorthand(argv: list[str]) -> bool:
    """Return True when *argv* looks like ``codify <target> [opts]``
    where *target* is an agent YAML/directory rather than a subcommand.

    Used by :func:`main` to transparently redirect
    ``codify myagent.yaml --model m`` to
    ``codify run myagent.yaml --model m``.
...
def _is_removed_ad_hoc_invocation(argv: list[str]) -> bool:
    ...
    - The first non-flag token isn't a known click subcommand and is
      ...
      ``codify "what does this repo do?"``) — the free-text shape
      the removed top-level ad-hoc chat accepted.

    False when the first non-flag token matches a known
    subcommand (``codify run ...``, ``codify attach ...``),
    when the user asks for top-level help/version
    (``codify --help``, ``codify --version``), or when the
    token is a single command-shaped word (e.g. ``codify blah``)
    ...
    # tokens are flags (e.g. ``codify --system-prompt "..."``),
...
_HOST_PID_PATH = Path.home() / ".codify" / "host.pid"
...
# for a daemon that owns a local Codify server. Daemon reuse is keyed on this
...
class _HostDaemonRecord:
    ...
    :param log_path: Daemon log file path, e.g.
        ``"/Users/me/.codify/logs/host/host-abc.log"``.
    :param started_at: Unix epoch seconds when the daemon was spawned,
        e.g. ``1710000000``.
    :param host_id: Local host id advertised to Codify servers, e.g.
...
class _HostHttpResult:
    """
    Decoded Codify management HTTP response.
...
class _DaemonSessionsResult:
    ...
    :param base_url: Codify server base URL, e.g.
...
def _normalize_daemon_target(server_url: str | None) -> str:
    ...
    :param server_url: Requested Codify server URL, e.g.
...
def _daemon_host_online(record: _HostDaemonRecord, *, timeout_s: float = 2.0) -> bool:
    ...
    tunnel to the Codify server is up: the server only reports the host
    ...
    """
    from codify.claude_native_bridge import url_component
...
def _update_daemon_resolved_server_url(target: str, server_url: str) -> None:
    """
    Record the concrete Codify server URL served by a daemon target.
...
def _load_existing_host_id() -> str | None:
    ...
    from codify.host.identity import CONFIG_PATH
...
def _daemon_host_identity_changed(record: _HostDaemonRecord) -> bool:
    ...
    A live daemon can outlast edits to ``~/.codify/config.yaml``. Reusing
...
def _terminate_host_unit(record: _HostDaemonRecord, *, reason: str) -> None:
    """
    Tear down a daemon and, in local mode, the Codify server it owns.

    The ``--local`` daemon spawns its Codify server once and never respawns
...
class _DaemonReuseDecision:
    ...
    :param config_changed: ``True`` when the existing daemon was torn down
        ...
        invocation (e.g. the user flipped ``CODIFY_AUTH_ENABLED``).
...
def _reuse_existing_daemon_record(target: str) -> _DaemonReuseDecision:
    ...
    ``CODIFY_AUTH_ENABLED``) would silently keep its old auth
...
def _foreground_daemon_record(
    *,
    ...
    :param target: Normalized daemon target, e.g.
        ``"https://example.databricksapps.com"`` or ``"local"``.
    :param server_url: Concrete Codify server URL being connected to, e.g.
        ``"http://127.0.0.1:8123"``.
...
def _claim_foreground_daemon_record(
    ...
    if conflict is not None:
        raise click.ClickException(
            ...
            "Run `codify host status` to inspect it or "
            "`codify host stop --server ...` to stop it first."
        )
...
def _load_or_create_host_id() -> str | None:
    ...
    if host_id is not None:
        return host_id
    from codify.host.identity import CONFIG_PATH, load_or_create_host_identity
...
def _ensure_host_daemon(server_url: str | None) -> bool:
    ...
    :param server_url: Codify server URL the daemon connects to, or ``None``
        for local mode — the daemon starts (or reuses) a persistent local
        Codify server and connects to that.
    ...
    args = [sys.executable, "-m", "codify.host._daemon_entry", *mode_args]
...
def _build_host_daemon_env(
    *,
    ...
    Remote daemons connect to an already-running Codify server, so they only
    need process essentials, TLS trust, and Databricks auth. Local daemons
    also start the local Codify server; that server is the user's local runtime
    and must inherit Codify config plus provider credentials such as
    ...
    through :func:`codify.host.connect._build_runner_env`, so these
    local-server credentials do not leak into runner subprocesses.

    :param server_url: Codify server URL for remote mode, e.g.
    ...
    """
    from codify.host.connect import (
...
# free-port respawn cold-boots — see ensure_local_codify_server.
...
def _ensure_databricks_server_auth(server: str, *, non_interactive: bool = False) -> None:
    ...
    TTY we run the same flow ``codify login`` would and continue;
    ...
    :param non_interactive: When ``True``, never run the browser login —
        ...
        TTY. Lets callers (e.g. ``codify host --non-interactive``) keep
    ...
    from codify.chat import _remote_headers
    ...
    if workspace_host is None:
        return
    login_cmd = f"codify login {server}"
    ...
    from codify.cli_auth import load_databricks_org_id

    _databricks_login(server, workspace_host, org_id=load_databricks_org_id(server))


def _ensure_backend(server: str | None) -> str:
    """Ensure the host daemon is running and return the Codify server URL.

    The daemon is the single backend for ``attach`` / ``run`` / ``claude`` /
    ``codex``: it spawns the runner and, in local mode, the Codify server too.
    ...
    :param server: ``--server`` value after config fallback. A non-empty
        ...
        persistent local Codify server and this returns its discovered loopback
        URL.
    ...
    :raises click.ClickException: If local mode's server never becomes
        reachable.
    """
    from codify._runner_startup import (
    ...
    if server:
        ...
        # to its /api/2.0/codify mount, then sign in first when the
        ...
        return server
    # Local mode: the daemon spawns (or reuses) a persistent local Codify server.
...
def _exit_for_auth_mode_change(base_url: str) -> None:
    ...
    The local Codify server bakes its auth posture (header vs accounts, cookie
    secret) at boot, so an ``CODIFY_AUTH_ENABLED`` flip restarts it
    ...
    clear, actionable message and exit 0, so the next ``codify run`` is
    a clean single-mode start. When the new mode is accounts and no admin
    exists yet, point the user at the one-time setup URL.

    :param base_url: The freshly-restarted Codify server URL, e.g.
        ``"http://127.0.0.1:6767"``.
    ...
    if needs_admin_setup:
        ...
        click.echo("  then re-run `codify run` to start.", err=True)
    else:
        click.echo("  Re-run `codify run` to start.", err=True)
    click.echo("", err=True)
    raise SystemExit(0)
...
def _discover_local_server_url(
    ...
    """Poll until the daemon-started local Codify server is reachable.

    In local mode the daemon owns the Codify server; the CLI discovers its URL
    ...
    while time.monotonic() < deadline:
        ...
        if not _host_daemon_alive():
            raise click.ClickException(
                "The local daemon exited before its Codify server became ready. "
                "See logs under ~/.codify/logs/host/ and "
                "~/.codify/logs/server/."
            )
        time.sleep(0.2)
    raise click.ClickException(
        f"Timed out after {timeout:.0f}s waiting for the local Codify server to "
        "start. See ~/.codify/logs/server/ for details."
    )


@dataclass
class _CliRunnerProcess:
    """Runner subprocess metadata for the ``codify server`` command.
...
def _start_cli_runner_process(
    *,
    ...
    ``codify server`` passes its loopback URL; ``run --server``
    passes the remote Codify server URL.

    For remote Databricks-fronted servers, the runner subprocess
    authenticates via the stored ``codify login`` record (or
    ...
    :param runner_id: Optional runner id to advertise. ``None``
        ...
        :func:`codify.runner.identity.get_stable_runner_id`
    ...
    :param log_dir: Optional base log directory to use when
        ...
        ``~/.codify/logs`` location; tests should pass a
    ...
    from codify.runner.identity import (
        RUNNER_ID_ENV_VAR,
    ...
    try:
        with child_logging_popen_kwargs(env) as logging_kwargs:
            runner_proc = subprocess.Popen(
                [sys.executable, "-m", "codify.runner._entry"],
                env=env,
                stdout=log_fh,
                stderr=log_fh,
                **_proc.spawn_kwargs(),
    ...
    if runner_proc.poll() is not None:
        from codify._runner_startup import format_runner_log_tail
...
def _adopt_cli_runner_process(proc: subprocess.Popen[bytes]) -> None:
    ...
    :returns: None.
    """
    from codify.runner.identity import RUNNER_ADOPT_SIGNAL
...
@click.option(
    ...
    help=(
        ...
        "(alternative to CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD). Only "
...
@click.option(
    ...
    help=(
        ...
        "local server recorded in ~/.codify/local_server.pid) instead of "
...
def server(
    ...
    """Start the Codify server, or manage the background server.

    Bare ``codify server`` runs the server in the FOREGROUND (Ctrl-C to
    ...
    :param auto_open: Whether to open the magic-redeem URL in the
        ...
        into the ``CODIFY_ACCOUNTS_AUTO_OPEN`` env var so the
    ...
    :param admin_password: Optional first-run accounts admin password
        ...
        ``CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD`` env var that
    ...
    if background:
        ...
        # reused one too; absent only for a foreground `codify server` whose
    ...
    if admin_password:
        os.environ.setdefault("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD", admin_password)
    ...
    # entrypoint, future `codify run`) that build the app
    # outside this CLI command.
    os.environ["CODIFY_ACCOUNTS_AUTO_OPEN"] = "1" if auto_open else "0"

    # Unified local-server lifecycle — applies ONLY to a *bare* loopback
    # `codify server` (default port + default DB + artifacts), i.e.
    # THE canonical machine-global local server recorded in
    # ~/.codify/local_server.pid:
    ...
    # DEDICATED server here" — the daemon's own spawn (ensure_local_codify_server)
    ...
    from codify.runner.transports.ws_tunnel.limits import (
        RUNNER_TUNNEL_MAX_MESSAGE_BYTES,
        TUNNEL_KEEPALIVE_PING_INTERVAL_S,
        TUNNEL_KEEPALIVE_PING_TIMEOUT_S,
    )
    from codify.server.app import create_app
    from codify.server.auth import create_auth_provider
    from codify.server.server_config import config_str_list
    from codify.stores.agent_store.sqlalchemy_store import SqlAlchemyAgentStore
    from codify.stores.comment_store.sqlalchemy_store import SqlAlchemyCommentStore
    from codify.stores.conversation_store.sqlalchemy_store import (
        SqlAlchemyConversationStore,
    )
    from codify.stores.file_store.sqlalchemy_store import SqlAlchemyFileStore
    ...
    from codify.stores.permission_store.sqlalchemy_store import SqlAlchemyPermissionStore
    from codify.stores.project_store.sqlalchemy_store import SqlAlchemyProjectStore
    from codify.stores.scheduled_task_store.sqlalchemy_store import (
    ...
    from codify.runtime import init as init_runtime
    from codify.runtime.agent_cache import AgentCache
    from codify.runtime.caps import RuntimeCaps
    ...
    from codify.spec import parse_default_policies, parse_server_llm
    ...
    from codify.runtime import telemetry
    ...
    _tunnel_token = os.environ.get("CODIFY_RUNNER_TUNNEL_TOKEN")
    ...
    from codify.stores.host_store import HostStore
    ...
    from codify.server.managed_hosts import parse_sandbox_config
    ...
    # (CODIFY_AUTH_ENABLED=1 without OIDC config, or an explicit
    # CODIFY_AUTH_PROVIDER=accounts), supply sensible defaults
    ...
    # set this explicitly.
    from codify.server.auth import resolve_auth_source

    if resolve_auth_source() == "accounts":
        from codify.server.accounts_secret import load_or_generate_cookie_secret

        os.environ.setdefault(
            "CODIFY_ACCOUNTS_COOKIE_SECRET",
            load_or_generate_cookie_secret(art_loc),
        )
        os.environ.setdefault("CODIFY_ACCOUNTS_BASE_URL", f"http://{host}:{port}")
    ...
    account_store = None
    from codify.server.auth import UnifiedAuthProvider as _UAP

    if isinstance(auth_provider, _UAP) and auth_provider._source == "accounts":
        from codify.server.accounts_store import SqlAlchemyAccountStore
    ...
    server_log_path = configure_process_logging(
        "server",
        logger_names=("codify", "uvicorn", "uvicorn.error", "uvicorn.access"),
    )
    ...
    click.echo(f"Starting codify server on {host}:{port}")
    ...
    # static dir.
    from codify.server.app import _WEB_UI_DIST
    ...
    class _ShutdownSignalingServer(uvicorn.server.Server):
        ...
        async def shutdown(self, sockets=None) -> None:
            ...
            from codify.runtime import session_stream as _session_stream
...
def _stop_local_server_and_daemon(*, force: bool) -> bool:
    """Stop the background Codify server and the local host daemon that owns it.
    ...
    reconnect-flap against a dead server), then the detached Codify server
    recorded in ``~/.codify/local_server.pid``. Best-effort and
...
def server_stop(force: bool) -> None:
    """Stop the background Codify server and the local host daemon.

    Stops the local host daemon first, then the detached server recorded
    in ``~/.codify/local_server.pid`` — its web UI and sessions become
    unreachable. To stop hosting but KEEP the server up, use
    ``codify host stop``; to stop everything, use ``codify stop``.
...
def server_status(json_output: bool) -> None:
    """Show whether the background Codify server is running.

    Reports the recorded pid/port, URL, live-session count, and whether a
    local host daemon is attached. Reads ``~/.codify/local_server.pid``
    and probes ``/health``.
...
def stop(force: bool) -> None:
    """Stop everything Codify is running on this machine.
    ...
    history) up, use ``codify host stop`` instead.
...
def _uninstall_script_path() -> Path:
    ...
    if repo_script.exists():
        return repo_script
    try:
        resource = resources.files("codify.resources.scripts").joinpath("uninstall_oss.sh")
    except ModuleNotFoundError as exc:
        raise click.ClickException("uninstall script is missing from this installation") from exc
    with resources.as_file(resource) as path:
        if path.exists():
            temp_dir = Path(tempfile.mkdtemp(prefix="codify-uninstall-"))
...
def _write_uninstall_manifest(ledger: Any) -> Path:
    ...
    fd, manifest_name = tempfile.mkstemp(prefix="codify-uninstall-ledger-", suffix=".tsv")
...
def _maybe_fast_backfill_install_ledger(argv: Sequence[str]) -> None:
    ...
    with contextlib.suppress(Exception):
        from codify.install_ledger import backfill_install_ledger
...
def _internal_write_ledger(from_env: bool) -> None:
    ...
    del from_env
    from codify.install_ledger import ledger_path, write_install_ledger_from_env
...
def doctor(
    ...
    from codify.install_ledger import backfill_install_ledger, backfill_ledger_path
    ...
    if json_output:
        ...
        click.echo("No Codify install detected; no ledger written.")
    elif apply_changes:
...
@click.option("--purge-workspace", is_flag=True, help="Also remove ~/codify with --purge.")
...
def uninstall(
    ...
    """Uninstall Codify while preserving user data unless --purge is set."""
    from codify.install_ledger import resolve_uninstall_ledger
                ...
                json.dumps(
                    {
                        ...
                        "exit_code": 3,
                        "error": "no Codify install detected",
                    },
                    indent=2,
                )
            )
            raise SystemExit(3)
        click.echo("No Codify install detected; nothing to uninstall.", err=True)
        raise SystemExit(3)
    ...
    for enabled, flag in (
        ...
        if enabled:
            args.append(flag)
    env = os.environ.copy()
    env["CODIFY_UNINSTALL_LEDGER_SOURCE"] = ledger.ledger_source
    manifest = _write_uninstall_manifest(ledger)
    env["CODIFY_UNINSTALL_LEDGER_MANIFEST"] = str(manifest)
    try:
        ...
        if (
            ...
            and script_path.parent.name.startswith("codify-uninstall-")
...
def _upgrade_vcs_install(
    ...
    """
    from codify.update_check import (
    ...
    if remote_sha and current_sha and remote_sha == current_sha:
        click.echo(f"codify is up to date (git {cur_short}, tracking {info.vcs_url}).")
        return
    if known_behind:
        click.echo(
...
def upgrade(check_only: bool, force: bool, pre: bool) -> None:
    """Upgrade the codify CLI to the latest release on PyPI.

    Detects how codify was installed (uv / pip / pipx / poetry), checks
    ...
    if info is None:
        raise click.ClickException(
            "Couldn't determine how codify is installed; upgrade it manually."
        )
    if info.is_editable:
        raise click.ClickException(
    ...
    current = importlib.metadata.version("codify")
    ...
    if latest is None:
        raise click.ClickException(
            ...
            "connection (or CODIFY_INDEX_URL / your configured index) and try again."
        )
    if not _is_newer(latest, current):
        click.echo(f"codify is up to date (v{current}).")
        return
    ...
    if _is_newer(new_version, current):
        ...
        return
    raise click.ClickException(
        f"The upgrade command ran but codify is still v{new_version} (expected "
        ...
        "Reinstall it explicitly — e.g. `uv tool upgrade --reinstall codify` or "
        f"`pip install --force-reinstall 'codify=={latest}'`."
    )
...
def _bundle(source: Path) -> bytes:
    ...
    Codify YAML file, or pass through an existing tarball.
    ...
    :param source: Path to an agent image directory,
        standalone Codify YAML file, or an existing
        ``.tar.gz`` bundle file.
    :returns: The gzipped tarball bytes.
    :raises CodifyError: If a required env var is
    ...
    if source.is_file() and source.suffix.lower() in {".yaml", ".yml"}:
        from codify.spec import materialize_bundle
...
def _resolve_bundle_env_vars(source: Path) -> dict[str, str]:
    ...
    :raises CodifyError: If a ``${VAR}`` reference
...
class _ExecutorDeploy(BaseModel):  # type: ignore[explicit-any]  # auth is a free-form mapping
    ...
    expands (``codify/spec/parser.py`` — ``_parse_executor`` /
...
def _expand_config_env_vars(  # type: ignore[explicit-any]  # raw is parsed YAML (heterogeneous values)
    ...
    :param expand_fn: Callable that expands env var references
        ...
        :func:`codify.spec.expand_env_vars`.
...
@click.option(
    ...
    default=None,
    help=(
        "Remote codify URL. When set, the picker / lookup queries "
        "this server instead of starting a local one. Required when "
        "running ``codify resume`` without a conversation id."
    ),
)
def resume(
    target: str | None,
    ...
    #
    # :param target: Optional Codify conversation id, e.g.
    #     ``"conv_abc123"``. None falls through to the picker.
    # :param server: Remote Codify server URL (optional in id mode;
    #     required in picker mode).
    """Resume an Codify conversation, auto-dispatching by runtime.

    \b
    With CONV_ID: looks up the conversation and dispatches to the
    matching wrapper. claude-native sessions land in
    ``codify claude``; everything else surfaces a clear hint to
    use ``codify run --resume <id> <agent.yaml>``.
    ...
    \b
    Examples:
      codify resume conv_abc123
      codify resume conv_abc123 --server https://<app>.databricksapps.com
      codify resume --server https://<app>.databricksapps.com
    """
    from codify.resume_dispatch import run_resume
...
@click.option(
    ...
    default=None,
    help=(
        "Codify server URL. Defaults to the configured server, an existing "
...
def import_session_command(
    ...
    The source transcript is converted to ordinary Codify items and stored
    ...
    \b
    Examples:
      codify import --harness claude --session <session-id>
      codify import --harness codex --session <session-id>
      codify import --harness opencode --session <session-id>
      codify import --harness qwen --session <session-id>
      codify import --harness claude --last 10
      codify import --harness claude --session <session-id> --force
    """
    import httpx

    from codify.chat import _remote_headers
    from codify.session_import import (
        ImportSource,
        SessionImportNotFoundError,
    )
    from codify.session_import.local import (
    ...
    if base_url is None:
        base_url = ensure_local_codify_server().url
    ...
    for current_source_session_id in source_session_ids:
        ...
        try:
            ...
            raise click.ClickException(f"Could not reach the Codify server: {exc}") from exc
...
def _render_usage(report: dict[str, Any], limit: int) -> None:  # type: ignore[explicit-any]
    ...
    Styling routes through :mod:`codify.inner.ui` — the CLI palette SOT —
    ...
    model — see :class:`codify.server.schemas.SessionUsage`).
...
@click.option(
    ...
    default=None,
    help=(
        "Codify server URL. Defaults to the configured server, or a local server already running."
    ),
)
@click.option(
    "--json",
...
def usage(limit: int, server: str | None, as_json: bool) -> None:
    """Show your Codify LLM cost for today / the last 7 / 30 days.
...
def session(ctx: click.Context) -> None:
    """Manage Codify sessions.

    \b
    Examples:
      codify session export --id conv_abc123
      codify session export --id conv_abc123 --output transcript.jsonl
      codify session export --id conv_abc123 --server https://myserver.com
    """
...
def session_export(session_id: str, output: str | None, server: str | None) -> None:
    ...
    and is independent of ``codify import``, which reads native harness history.

    \b
    Examples:
      codify session export --id conv_abc123
      codify session export --id conv_abc123 --output my_session.jsonl
      codify session export --id conv_abc123 --server https://myserver.com
    """
    import httpx
...
def _import_agent_aliased_types() -> frozenset[str]:
    ...
    """
    from codify.entities.conversation import ITEM_TYPE_TO_DATA_CLS
...
def _import_item_payload(item: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from codify.entities.conversation import parse_item_data
...
@click.option(
    ...
    required=True,
    metavar="FILE",
    help="Path to a JSONL file produced by 'codify session export'.",
)
@click.option(
    "--title",
    default=None,
...
def session_import(input_path: str, title: str | None, server: str | None) -> None:
    ...
    The inverse of ``codify session export``: reads the ``session_meta`` +
    ``item`` lines and recreates the conversation on the target server as a new
    session (a fresh conversation id each time). Distinct from ``codify
    ...
    \b
    Examples:
      codify session import -i my_session.jsonl
      codify session import -i my_session.jsonl --title "Repro of ES-2065116"
      codify session import -i my_session.jsonl --server https://myserver.com
    """
    import httpx

    from codify.chat import _remote_headers
    from codify.db.utils import builtin_agent_id
    from codify.native_coding_agents import native_coding_agent_for_harness
...
# ``codify.chat.run_chat`` for how local-agent options get baked
...
_FROM_OPENCLAW_HELP = (
    "Launch one agent from the OpenClaw/acpx registry without saving it to Codify config."
)
...
_LOG_HELP = "Write a JSON dump of the conversation to ~/.codify/logs/ on exit."


_DEFAULT_HARNESS_PROMPTS = {
    "claude-sdk": (
        "You are Claude Code, running through Codify. "
        "Help the user with software engineering tasks."
    ),
    "codex": (
        "You are Codex, running through Codify. Help the user with software engineering tasks."
    ),
    "cursor": (
        "You are Cursor, running through Codify. Help the user with software engineering tasks."
    ),
    "kimi": (
        "You are Kimi Code, running through Codify. Help the user with software engineering tasks."
    ),
    "qwen": (
        "You are Qwen Code, running through Codify. Help the user with software engineering tasks."
    ),
    "goose": (
        "You are Goose, running through Codify. Help the user with software engineering tasks."
    ),
}
_DEFAULT_HARNESS_PROMPT = "You are a helpful coding agent running through Codify."
...
# operations route through the Codify dispatch path (runner
...
def _validate_harness(harness: str) -> None:
    """
    Fail fast when *harness* is not a supported Codify harness.

    :param harness: Harness id from ``--harness``, e.g.
        ``"claude-sdk"``.
    :raises click.ClickException: If *harness* is unsupported.
    """
    from codify.spec._codify_compat import CODIFY_HARNESSES

    if canonicalize_harness(harness) in CODIFY_HARNESSES:
        return
    allowed = ", ".join(sorted(CODIFY_HARNESSES))
...
def _default_harness_prompt(harness: str) -> str:
    ...
    :returns: Prompt text for the generated Codify YAML.
    """
...
def _resolve_openclaw_run_agent(identifier: str) -> AcpAgentEntry:
    ...
    from codify.onboarding.openclaw_config import (
...
def _materialize_harness_launcher_file(
    *,
    ...
    """
    Create a temporary standalone Codify YAML for no-AGENT ``run``.

    The generated file uses the single-file Codify YAML shape
    (``name`` / ``prompt`` / ``executor``), not native AP
    ``config.yaml``. Passing this file to ``run_chat`` exercises the
    same compat adapter as ``codify run examples/foo.yaml``.

    Harnesses listed in :data:`_OS_ENV_HARNESSES` get an ``os_env``
    block so the workflow injects ``sys_os_*`` tools into the
    request — routing file/shell operations through the Codify
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="codify-harness-launcher-"))
...
def _missing_run_agent_message() -> str:
    ...
    return (
        ...
        "harness directly:\n"
        "  codify run examples/hello_world.yaml\n"
        "  codify run --server http://localhost:6767\n"
        "  codify run --harness claude-sdk\n"
        "  codify run --harness codex"
    )
...
def _dispatch_native_terminal_harness(
    *,
    ...
    Codify turn per message — which persists its own user item — *while*
    ...
    straight to the native wrapper (the same code ``codify cursor`` /
    ``codify claude`` / etc. run), keeping the TUI the single source of
    ...
    if session_id is None and not resume_picker and resume_latest:
        from codify.chat import _remote_headers, _resolve_latest_conversation_id
...
def _reject_agent_with_native_terminal_harness(harness: str) -> None:
    ...
    double-record every message (Codify turn + forwarder mirror). So an
    ...
    raise click.ClickException(
        ...
        f"`codify {native_agent.terminal_name}` (or `run --harness {harness}`)."
    )
...
def _dispatch_run(
    *,
    ...
    """
    Route ``codify run`` to the right impl.

    The click path always drives the Codify server-backed REPL. With
    ``--server <url>``, use that server URL instead of starting a
    local server. (``codify attach`` is a separate attach-only
    ...
    :param server: Server URL from ``--server`` or config. With a local
        target, this is the Codify server used for upload/session setup; with
    ...
    :param resume_parts: Pre-built argument list prefix for the
        ...
        ``["codify", "run", "agent.yaml", "--harness", "codex"]``.
    ...
    if target is not None and _is_server_url(target):
        raise click.ClickException(
            ...
            f"Use `codify run --server {target}` instead."
        )
    ...
    if target is None:
        if server_from_cli and server is not None and harness is None:
            ...
            # URL to its /api/2.0/codify mount and strip any ?o= query. Else
            ...
            if resume_conversation_id is not None and is_interactive_shape:
                from codify.chat import _redirect_native_resume_if_needed, run_attach
            ...
            from codify.chat import run_chat
        ...
        # user message (Codify turn + forwarder mirror). Returns False for
        ...
        # every message (Codify turn + forwarder mirror, same as the no-AGENT
    ...
    if prompt is not None:
        ...
        from codify.chat import run_prompt
...
def _resolve_attach_server(server: str | None, configured_server: str | None) -> str | None:
    """
    Resolve the Codify server URL ``attach`` should join.

    Resolution order: an explicit ``--server`` value, then the configured
    ``server`` default, then a local Codify server already running in the
...
def _require_live_conversation(
    *,
    ...
    :param base_url: Codify server base URL, e.g. ``"http://127.0.0.1:6767"``.
    ...
    if result.status_code != 200:
        raise click.ClickException(
            ...
            f"(server returned {result.status_code}). Run `codify host status` "
            "to list live sessions, or `codify run <agent.yaml>` to start one."
        )
...
@click.option(
    ...
    help=(
        ...
        "overlay, JSONL event log (~/.codify/debug/), and "
...
def attach(
    ...
    ``codify run``; to reopen/restart a stored one use
    ``codify resume``.

    \b
    Examples:
      codify attach conv_abc123
      codify attach conv_abc123 --server https://<app>.databricksapps.com
    """
    cfg = _load_effective_config()
    base_url = _resolve_attach_server(server, cfg.get("server"))
    if base_url is None:
        raise click.ClickException(
            ...
            "server — start one with `codify run`, or point at one with "
            "`--server <url>`."
        )
    if conversation is None:
        raise click.ClickException(
            "Nothing to attach to: `attach` joins a LIVE session by id. "
            f"Run `codify host status` to list sessions on {base_url}, or "
            "`codify run <agent.yaml>` to start a new one."
        )
    ...
    from codify.chat import run_attach
...
# `run` absorbs the legacy ``codify run`` subcommand. With an AGENT
...
# Both paths route through the same Codify server+REPL dispatcher.
@cli.command()
...
@click.option(
    ...
    default=None,
    help=(
        "Remote codify URL. Uploads the local YAML as an ephemeral "
...
@click.option(
    ...
    help=(
        ...
        "(inline equivalent of `codify host`). Requires --server."
    ),
)
def run(
    target: str | None,
    ...
    """Start a session with an Codify agent.
    ...
    Default: codify server+REPL architecture (spawns a local
    ...
    Examples:
      codify run --harness claude-sdk
      codify run --harness codex -p "review the last commit"
      codify run --from-openclaw "Gemini CLI" -p "review the last commit"
      codify run examples/hello_world.yaml
      codify run examples/hello_world.yaml --harness codex --model gpt-5.4-mini
      codify run --server http://localhost:6767
      codify run examples/databricks_coding_agent.yaml --server https://<app>.databricksapps.com
    """
    ...
    if harness is None and not direct_server_cli:
        from codify.harness_startup_config import resolve_harness_config
    ...
    # Interactive ``codify run`` opens the live conversation in the
    ...
    # NOTE: the host daemon + Codify server are ensured inside ``run_chat``'s
...
class _HostGroup(click.Group):
    ...
    ``codify host <url>`` is shorthand for ``codify host
    ...
    def parse_args(self, ctx: click.Context, args: list[str]) -> list[str]:
        ...
        ``codify host <url>`` is shorthand for ``codify host --server
...
def _prompt_stop_local_server() -> None:
    """Ask whether to also stop the detached local Codify server after exit.

    The local-mode host daemon spawns a detached, persistent local AP
    server (:func:`ensure_local_codify_server`) that survives the daemon's exit
...
@click.option("--server", default=None, help="Remote codify server URL.")
@click.option(
    "--non-interactive",
    "non_interactive",
    is_flag=True,
    ...
    help=(
        ...
        "are not logged in, fail with the `codify login` hint instead of "
...
def host(ctx: click.Context, server: str | None, non_interactive: bool) -> None:
    ...
    \b
    Examples:
      codify host https://codify-app.databricksapps.com
      codify host --server https://codify-app.databricksapps.com
      codify host ""   # spawn + connect to a local server

    The server URL may be given positionally (``codify host
    ...
    in, ``host`` runs the same flow ``codify login`` would before
    ...
    :param server: Remote Codify server URL, e.g.
    ...
    :param non_interactive: When ``True``, never launch the browser login
        for an un-authed remote server — fail with the ``codify login``
        hint instead.
    """
    ...
    # reuse) the local Codify server here and connect to it; otherwise connect to
    ...
    # already started by `codify server` or a prior host/run daemon) —
    ...
    try:
        ...
        # a server started by `codify server` or another daemon — killing
...
def _host_group_option(ctx: click.Context, key: str) -> str | None:
    """
    Read a group-level ``codify host`` option for a subcommand.
...
def _daemon_base_url(record: _HostDaemonRecord) -> str | None:
    """
    Resolve the Codify server URL for a daemon record.

    :param record: Daemon registry record to inspect.
    :returns: Codify server URL, e.g. ``"http://127.0.0.1:8123"``, or
...
def _host_http_json(
    *,
    ...
    """
    Send one management request to an Codify server.
...
def _host_error_text(body: _HostJsonObject | str) -> str:
    """
    Extract a concise error string from an Codify response body.
    ...
    *,
    ...
    if base_url is None:
        return _DaemonSessionsResult(
            base_url=None,
            sessions=[],
            error="local Codify server is not reachable",
        )
    host_id = record.host_id or _load_existing_host_id()
    if not host_id:
        return _DaemonSessionsResult(
...
def _add_daemon_host_status(
    ...
    if not isinstance(base_url, str):
        payload["error"] = "local Codify server is not reachable"
        return
...
def _host_truncate(text: _HostJsonValue, *, max_chars: int) -> str:
    ...
    :param text: Value to truncate, e.g. an Codify error message.
...
def _stop_session_on_server(
    *,
    ...
    """
    Stop one Codify session via the server lifecycle event API.
...
def host_stop_session(
    ...
    :param server: Codify server URL that owns the sessions, e.g.
        ``"https://example.databricksapps.com"``. ``None`` falls back
    ...
    if resolved_server is None:
        ...
        if resolved_server is None:
            raise click.ClickException(
                "No server was supplied and no local Codify server is reachable."
            )
...
def version() -> None:
    """Print the installed Codify version."""
    ...
    for item in settings:
        if "=" not in item:
            raise click.ClickException(
                ...
                "Example: codify config set --global default_agent=myagent.yaml"
            )
        ...
        # Resolve ``default_agent`` to an absolute path so ``codify`` works from
...
def _resolve_harness_startup_args(
    ...
    *cli_args* append *after* so a per-invocation flag wins for last-wins CLIs.
    ...
    """
    from codify.harness_startup_config import resolve_harness_args
...
def _print_config_defaults() -> None:
    ...
    The ``KEY=VALUE`` defaults from ``~/.codify/config.yaml`` (user) and
    ``.codify/config.yaml`` in the cwd (project, takes precedence).
    Used by ``codify config list``.
    ...
    if not global_cfg and not local_cfg:
        click.echo(
            "  (none set — `codify config set key=value` for project,\n"
            "   or `codify config set --global key=value` for user-level)"
        )
        return
    ...
    # (``cwd/.codify/config.yaml``) resolves to the SAME file as the
    # user-level path (``~/.codify/config.yaml``). Dedup on the resolved
...
class _ConfigGroup(click.Group):
    ...
    def _legacy_hint(first: str) -> str | None:
        ...
        """
        if first == "--list":
            return "`config --list` is now `codify config list`."
        if first == "--unset":
            return "`config --unset KEY` is now `codify config unset KEY`."
        if first == "--global":
            return (
                "`--global` now goes on the subcommand — "
                "`codify config set --global KEY=VALUE` or "
                "`codify config unset --global KEY`."
            )
        if "=" in first and not first.startswith("-"):
            return f"setting defaults is now `codify config set {first}`."
        return None
...
# Slack socket-mode bot: a separate `codify-slack` package (heavy deps —
# slack_bolt/aiohttp — kept out of the core CLI install). The CLI launches it
# as a subprocess and never imports it.
_SLACK_PACKAGE = "codify_slack"
_SLACK_INSTALL_HINT = (
    "The Slack integration (codify-slack) isn't installed in this "
    "environment. Install it alongside codify with the `slack` extra:\n"
    '  uv pip install "codify[slack]"\n'
...
def _slack_installed() -> bool:
    """Whether the ``codify_slack`` package is importable (not imported)."""
...
def _integration_state_dir() -> Path:
    """Runtime dir for integration daemon records (honors CODIFY_DATA_DIR)."""
    from codify.host.local_server import _local_data_dir
...
def integration(ctx: click.Context) -> None:
    """Run and manage Codify chat integrations.

    \b
    Available integrations:
      slack   The @codify Slack socket-mode bot.
...
def slack(ctx: click.Context, background: bool) -> None:
    """Run the @codify Slack socket-mode bot.
    ...
    Config (Slack tokens, CODIFY_SERVER_URL, …) comes from environment
    ...
    click.echo("Starting the Codify Slack bot (foreground). Press Ctrl-C to stop.")
...
def _start_slack_background() -> None:
    ...
    if not daemon.confirm_alive(record, grace_seconds=2.0):
        ...
        raise click.ClickException(message)
    click.echo(f"Started the Codify Slack bot in the background (pid {record.pid}).")
...
def slack_stop() -> None:
    ...
    if record is None:
        click.echo("Slack bot: not running.")
        return
    click.echo(f"Stopped the Codify Slack bot (pid {record.pid}).")
...
def config_grp() -> None:
    """Get, set, and view Codify defaults and credentials.

    Defaults (auto_open_conversation, default_agent, harness, model,
    server) are used by ``codify run``. Project-level config
    (``.codify/config.yaml`` in the cwd, like ``.git/config``) overrides
    user-level config (``~/.codify/config.yaml``, like ``~/.gitconfig``).
...
def config_list() -> None:
    ...
    merged view of everything ``codify run`` will use (including
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Write to ~/.codify/config.yaml (user-level) instead of the project config.",
)
@click.argument("settings", nargs=-1, required=True, metavar="KEY=VALUE...")
def config_set(is_global: bool, settings: tuple[str, ...]) -> None:
    """Set one or more Codify defaults.

    Without ``--global``, pairs are written to ``.codify/config.yaml``
    in the current directory (project-level, like ``.git/config``); with
    ``--global`` to ``~/.codify/config.yaml`` (user-level, like
    ``~/.gitconfig``). Project values take precedence.

    Supported keys: auto_open_conversation, default_agent, harness,
    model, server.

    :param is_global: When ``True``, write to ``~/.codify/config.yaml``;
        when ``False``, to ``.codify/config.yaml`` in cwd.
    :param settings: ``KEY=VALUE`` pairs to set, e.g.
        ``("default_agent=examples/hello.yaml", "model=gpt-5.4-mini")``.

    \b
    Examples:
      codify config set default_agent=examples/hello_world.yaml
      codify config set --global server=https://<app>.databricksapps.com
    """
    if is_global:
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Remove from ~/.codify/config.yaml (user-level) instead of the project config.",
)
@click.argument("keys", nargs=-1, required=True, metavar="KEY...")
def config_unset(is_global: bool, keys: tuple[str, ...]) -> None:
    """Remove one or more Codify defaults.

    :param is_global: When ``True``, remove from ``~/.codify/config.yaml``;
        when ``False``, from ``.codify/config.yaml`` in cwd.
...
def setup(internal_beta: bool) -> None:
    """
    Launch the Codify first-time setup flow.
    ...
    with ``codify run``. (List configured credentials with
    ``codify config list``.) Pass ``--internal-beta`` to configure
    ...
    if shutil.get_terminal_size(fallback=(80, 24)).lines >= 32:
        ui.print_landing(tagline="coding, verified")
    else:
    ...
    if internal_beta:
        ...
        try:
            import codify.onboarding.internal_beta  # type: ignore[import-not-found]  # noqa: F401
        except ImportError:
            raise click.ClickException(
                "Databricks internal-beta setup is not available in this build. "
                "Run `codify setup` for the standard model/credential setup."
            ) from None
        ...
        from codify.onboarding.databricks_config import (
        ...
        _warn_missing_harness_dependencies()
        from codify.onboarding.internal_beta import _INTERNAL_BETA_DEFAULT_SERVER
        from codify.onboarding.sandboxes.lakebox import (  # type: ignore[import-not-found]
            install_demo_databricks_cli,
        )
        from codify.onboarding.setup import run_onboarding
        ...
        click.echo("Type `codify claude` to get started with Claude Code on codify.")
        return
    ...
    # returns; the user then starts a session with ``codify run``.
    _run_configure_harnesses_interactive()


# ─── sandbox group ────────────────────────────────────────────────
# The provider-agnostic sandbox CLI lives in codify/cli_sandbox.py.
# Provider launcher modules are optional and may be absent from a given
# distribution; hide the group when none are available.
# `codify lakebox` is kept as an alias for `codify sandbox …
...
# ─── debug group ──────────────────────────────────────────────────
#
# Operator-only maintenance commands, grouped under ``codify debug``
# so they stay out of the everyday surface.
#
# ``db-upgrade`` runs manual schema operations on an Codify tracking
...
def debug_db_upgrade(url: str) -> None:
    """
    Upgrade the schema of an Codify tracking database to the
    ...
    from codify.db.utils import _run_migrations
...
def debug_migrate_accounts_to_oidc(
    ...
    against your live DB *before* flipping ``CODIFY_AUTH_PROVIDER``.
    ...
    Examples:
      ...
      codify debug migrate-accounts-to-oidc sqlite:///chat.db --domain example.com
      # Apply it
      codify debug migrate-accounts-to-oidc sqlite:///chat.db --domain example.com --commit
      # Explicit per-user mapping (add --commit to apply)
      codify debug migrate-accounts-to-oidc sqlite:///chat.db --map alice=alice@corp.com
    ...
    from codify.server.identity_migration import build_domain_mapping, remap_identities
    ...
    if not report.committed:
        ...
        click.echo("\nDone. Flip CODIFY_AUTH_PROVIDER=oidc and restart.\n")
...
def debug_logs(
    ...
    \b
    Log locations (relative to ~/.codify or $CODIFY_DATA_DIR):
    ...
    Examples:
      ...
      codify debug logs --list
      # Show runner logs for a specific session (across relaunches)
      codify debug logs --type runner --session conv_abc123
      # List runner log files for a session
      codify debug logs --type runner --session conv_abc123 --list
      # Follow the latest server log in real-time
      codify debug logs --type server --follow
      # Show the full latest CLI diagnostics log
      codify debug logs --type cli -n 0
    """
    import re
    import subprocess
...
def _workspace_mount_probe_matches(candidate: str, probe: httpx.Response) -> bool:
    """Whether a ``/api/2.0/codify`` mount probe answered like codify.

    :param candidate: The probed mount base URL, e.g.
        ``"https://example.databricks.com/api/2.0/codify"``.
    :param probe: The ``GET <candidate>/v1/me`` response.
    :returns: ``True`` when the mount answered 200 (codify itself) or
...
def _cached_workspace_bearer(workspace_host: str) -> str | None:
    ...
    """
    from codify.onboarding.databricks_config import databricks_sdk_installed
...
def _with_default_scheme(server_url: str) -> str:
    ...
    (e.g. ``example.cloud.databricks.com/codify``), so a missing
    ...
    :param server_url: The user-supplied server URL, possibly
        schemeless, e.g. ``"example.cloud.databricks.com/codify"``.
    :returns: The URL with a scheme, e.g.
        ``"https://example.cloud.databricks.com/codify"``.
    """
...
def _workspace_api_server_url(server: str) -> str:
    """Expand a bare Databricks workspace URL to its codify API base.

    ``https://<workspace>`` hosts serve the workspace web app at the
    root; workspace-hosted codify lives at ``/api/2.0/codify``.
    Users naturally paste the bare host, so when a path-less server URL
    answers like a Databricks workspace web app (a non-codify reply
    carrying the ``server: databricks`` header) AND the
    ``/api/2.0/codify`` mount answers like the API proxy, the
    ...
    guide-issued web-UI URL (``https://<ws>/codify``): its bare root
    ...
    :returns: The normalized base URL without a trailing slash, e.g.
        ``"https://example.databricks.com/api/2.0/codify"`` — or the
    ...
    from codify.conversation_browser import (
        WORKSPACE_API_PATH,
        WORKSPACE_UI_PATH,
        display_server_url,
    )
    ...
    # (``https://<ws>/codify``) for browser access; accept it for login
    ...
    # untouched, so a non-workspace server served under ``/codify``
    # still works.
    ...
    try:
        ...
        return server
    # Already something we understand at the root: an codify server
    ...
    if _workspace_mount_probe_matches(candidate, api_probe):
        click.echo(f"Using {display_server_url(candidate)} (Databricks workspace-hosted codify).")
        return candidate
    ...
    if token is not None:
        ...
        if authed_probe is not None and _workspace_mount_probe_matches(candidate, authed_probe):
            click.echo(
                f"Using {display_server_url(candidate)} (Databricks workspace-hosted codify)."
            )
            return candidate
        click.echo(
            f"Note: {server} answers like a Databricks workspace, but "
            f"{candidate} did not answer as an codify server even with "
            f"the cached workspace credentials. Connecting to {server} as "
            "given; if codify is hosted on this workspace, refresh the "
    ...
    click.echo(
        ...
        "unauthenticated requests — if codify is hosted on this "
...
def _databricks_root_is_usable(server: str) -> bool:
    ...
    :returns: True when the host answers as an codify server or a recognized
...
def _resolve_server_url(server: str) -> str:
    """
    Normalize a user-supplied ``--server`` value to the Codify API base.
    ...
    then expand a bare Databricks workspace URL — or the ``/codify``
    web-UI URL the internal user guide hands out — to the
    ``/api/2.0/codify`` mount.
    ...
    :param server: A non-empty ``--server`` value, e.g.
        ``"example.cloud.databricks.com/codify"``.
    :returns: The normalized API base URL without a trailing slash, e.g.
        ``"https://example.cloud.databricks.com/api/2.0/codify"``.
    """
    from codify.conversation_browser import display_server_url
...
def _databricks_workspace_login_target(server: str, probe: httpx.Response) -> str | None:
    ...
    - **Workspace-hosted codify** (e.g.
      ``https://<workspace>/api/2.0/codify``): the workspace API
...
def _databricks_login(server: str, workspace_host: str, org_id: str | None = None) -> None:
    """Log in to a Databricks-fronted Codify server.

    Covers both Databricks Apps deployments and workspace-hosted
    codify (``https://<workspace>/api/2.0/codify``). Reuses an
    ...
    stored in ``~/.codify/auth_tokens.json`` — no profile name is
    ...
    if not databricks_sdk_installed():
        raise click.ClickException(
            ...
            "workspace-hosted codify) requires the `databricks` extra "
    ...
    from codify.cli_auth import store_databricks_auth
...
def _databricks_workspace_token(workspace_host: str) -> str | None:
    ...
    """
    from codify.inner.databricks_executor import (
        DatabricksAuthError,
...
def _remember_default_server(server: str) -> None:
    ...
    A bare ``codify`` (and ``codify host``) fall back to the
    configured ``server`` key when no ``--server`` is passed (see
    :func:`run` and :func:`host`). Without this, a user who runs
    ``codify login <server>`` and then bare ``codify`` is still routed
...
def login(server_url: str) -> None:
    """Authenticate with a remote Codify server.
    ...
    - accounts mode: prompts for username + password (no browser
      ...
      ``~/.codify/auth_tokens.json`` keyed by server URL.
    ...
    - Databricks-fronted (a Databricks App, or codify hosted on
    ...
    Subsequent ``codify run --server <url>`` commands then
    ...
    (the ``server`` key in ``~/.codify/config.yaml``), so a bare
    ``codify`` afterwards targets it instead of whatever default
    ``setup`` baked in.

    \b
    Example:
      codify login http://localhost:6767
      codify login example.cloud.databricks.com/codify  # https:// assumed
      codify          # connects to the server just logged in to
    ...
    :param server_url: The remote server URL, e.g.
        ...
        web-UI URL (``<ws>/codify``) is accepted alongside the bare
        workspace root.
    """
    import httpx as _httpx
    ...
    # hosted codify) means Databricks fronts the server. This
    ...
    if probe.status_code == 401:
        ...
        # 401 with non-JSON body — probably not an Codify server.
    ...
    from codify.cli_auth import store_token
    ...
    try:
        ...
        raise click.ClickException(
            ...
            f"Is the server running with CODIFY_AUTH_PROVIDER=oidc?"
        ) from exc
...
def _accounts_login(server: str) -> None:
    ...
    ``~/.codify/auth_tokens.json`` via the existing
    :func:`codify.cli_auth.store_token`. From there both
    ``codify run`` and ``codify host`` pick it up
...
@click.option(
    ...
    "parent_pane",
    required=True,
    help="Tmux pane id of the parent codify pane (e.g. '%0'). "
    "Forwarded by the wrapped key-binding via #{pane_id}.",
)
def pane_split(direction: str | None, parent_pane: str) -> None:
    """
    Split the parent codify pane and run the chooser in the new pane.

    Internal subcommand invoked by the tmux key-binding wrappers
    installed by ``codify.repl._tmux_pane``. The wrapper fires
    ``run-shell 'codify pane-split -<v|h|w> -p #{pane_id}'`` when
    the user presses their split key while focused on an codify
    pane; tmux substitutes ``#{pane_id}`` to the focused pane's id
    and we exec the right ``tmux split-window`` / ``new-window``
    invocation pointing at ``codify pane-picker``.

    :param direction: One of ``v`` / ``h`` / ``w``. Required.
    :param parent_pane: The codify pane id, e.g. ``%0``. Required.
    """
    import shlex

    from codify.repl._tmux_pane import _resolve_codify_argv

    if direction not in _PANE_SPLIT_DIRECTIONS:
        raise click.ClickException("pane-split requires exactly one of -v, -h, or -w")
    # The new pane runs ``codify pane-picker`` which reads the
    ...
    # include the venv ``bin/`` where ``codify`` lives.
    # ``_resolve_codify_argv`` returns either an absolute
    # path to the binary (preferred) or ``[python, "-m",
    # "codify.cli"]`` as a fallback that always works.
    picker_argv = [
        *_resolve_codify_argv(),
        "pane-picker",
        "--parent-pane",
        parent_pane,
    ]
...
@click.option(
    ...
    "``codify.repl._tmux_pane.register_pane``.",
)
def pane_picker(parent_pane: str) -> None:
    ...
    ``codify pane-split``) execs this command, which:

    1. Reads the parent codify pane's ``@codify-launch-argv``
       and friends.
    ...
    :param parent_pane: The parent codify pane id, e.g. ``%0``.
    """
    import json

    from codify.repl._tmux_pane import (
        OPT_LAUNCH_ARGV,
        read_pane_option,
    )
    ...
    if not launch_argv_json:
        click.echo(
            f"error: parent pane {parent_pane} has no codify context "
...
def _strip_resume_flags(argv: list[str]) -> list[str]:
    ...
    :param argv: Parent's launch argv, e.g.
        ``["python", "-m", "codify.cli", "run", "agent.yaml",
...
def _reject_reserved_kiro_resume_args(kiro_args: tuple[str, ...]) -> None:
    ...
    if any(arg == flag or arg.startswith(f"{flag}=") for arg in kiro_args for flag in reserved):
        raise click.UsageError(
            "Kiro resume flags are reserved for Codify resume handling; use "
            "`codify kiro --resume [CONVERSATION]` instead."
        )
...
# codify.cli_native; register them on the group here, at module bottom, so the
# shared launch helpers their bodies close over are already defined.
_register_native_commands(cli)

# Best-of-N verifier command (`best-of-n` / `bon`) lives in
# codify.cli_best_of_n.
_register_best_of_n_commands(cli)
