...
if TYPE_CHECKING:
    ...
    from codify.codex_native_bridge import CodexNativeBridgeState
    from codify.terminals.registry import TerminalListEntry
...
from codify.harness_aliases import (
    canonicalize_harness,
    is_native_harness,
    native_terminal_name,
)
from codify.harness_plugins import load_object, model_env_keys, spawn_env_builders
from codify.inner.native_attachments import has_unresolved_file_id, resolve_file_id_block
from codify.llms.summarize import (
...
from codify.policies.types import FAIL_CLOSED_PHASES
from codify.runner import native as _native
from codify.runner import pending_approvals
from codify.runner.background_titles import (
...
from codify.runner.background_titles.service import BACKGROUND_TITLE_MAX_PROMPT_CHARS
from codify.runner.codex.goal import CodexGoalRunner
from codify.runner.native import (
...
from codify.runner.native import orchestration as _native_runtime
from codify.runner.native.interrupt import NativeInterruptRunner
from codify.runner.proxy_mcp_manager import ProxyMcpManager
...
from codify.runtime.harnesses.process_manager import HarnessProcessManager, NoLiveHarnessError
from codify.runtime.prompt import (
...
from codify.spec.skill_sources import SkillSourceContext, resolve_harness_skills
from codify.spec.types import AgentSpec, LocalToolInfo, SkillSpec
from codify.terminals.control_bridge import bridge_tmux_control_to_websocket
from codify.terminals.ws_bridge import (
    WS_CLOSE_TERMINAL_NOT_FOUND,
    bridge_tmux_pty_to_websocket,
)
from codify.tools.builtins.load_skill import (
    find_skill_by_name,
...
# ASK gate: policy evaluation (``_evaluate_policy_via_codify``) and sub-agent
...
# delivery signal for the last child of a fan-out, and Codify routinely
...
# marker.
_RUNNER_DISPATCHED_FIELD = "codify_runner_dispatched"
...
async def _evaluate_policy_via_codify(
    *,
    ...
    """
    Proxy a policy evaluation request from the harness to the Codify server.

    Called by the runner's ``proxy_stream`` when it intercepts a
    ``policy_evaluation.requested`` SSE event from the harness. Posts
    the evaluation request to the Codify server's
    ...
    - ``PHASE_LLM_REQUEST`` / ``PHASE_LLM_RESPONSE`` fail OPEN
      (``POLICY_ACTION_ALLOW``) so a transient Codify outage does not
    ...
    :param server_client: HTTP client pointed at the Codify server.
    ...
    verdict_reason: str | None = (
        f"Codify policy evaluation unavailable; failing closed for {phase}."
        if _fail_closed
        else None
    )
...
# Language constant the codify YAML translator stamps on callable-backed
# tools (codify/spec/codify.py:CODIFY_TOOL_LANGUAGE). Duplicated rather
# than imported to avoid pulling the heavy translator module in for one
# string — same rationale as codify/tools/local_callable.py.
_CODIFY_CALLABLE_LANGUAGE = "codify-python-callable"
...
def _looks_like_file_path(path: str) -> bool:
    ...
    source extension (see :func:`codify.spec.parser._discover_local_tools`).
...
def _spec_with_workdir_paths(spec: Any, workdir: Path | None) -> Any:
    ...
    for info in local_tools:
        ...
        if (
            path
            and getattr(info, "language", None) != _CODIFY_CALLABLE_LANGUAGE
...
async def _resolve_forwarded_message_content(
    ...
    *,
    ...
    Remote Codify servers can forward session messages with raw file IDs
...
def register_subagent_work(
    *,
    ...
    :param wrapper_label: Optional child ``codify.wrapper``
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    Codify returns while the parent's runner tunnel is reconnecting), as
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure — the POST may never have reached Codify.
        return True
...
async def _deliver_subagent_wake_post(
    ...
    *,
    ...
    :param server_client: Codify HTTP client for the runner subprocess.
...
def _subagent_delivery_not_confirmed_response(
    ...
    *,
    ...
    was created as a sub-agent. For known sub-agents, Codify must not receive a
...
def create_runner_app(
    *,
    ...
    :param server_client: httpx.AsyncClient pointed at the AP
        ...
        In-process: pointed at the Codify ASGI app.
    ...
    app = FastAPI(title="codify-runner")
    ...
    if terminal_registry is not None:
        from codify.runtime import _globals as _rt_globals
    ...
    async def _resolve_session_claude_launch_config(
        ...
        if task is None:
            from codify.claude_native import resolve_native_claude_config
        ...
        if event.last_output:
            ...
            parts.extend(
                [
                    ...
                    "Codify captured a pane snapshot.",
                ]
            )
        return "\n".join(parts)
    ...
    from codify.runtime.filesystem_registry import (
        FilesystemRegistry,
    ...
    from codify.entities.environment_filesystem import (
        FilesystemEntry,
        ResourceError,
    )
            ...
            if harness_name == "claude-native":
                ...
                async def _claude_build_context(ctx: NativeLaunchContext) -> NativeLaunchContext:
                    ...
                    if bundle_dir is None:
                        bundle_dir = Path(tempfile.mkdtemp(prefix="codify-skill-bundle-"))
                    _logger.info(
            ...
            async with _repl_lock:
                ...
                if not _has_repl_terminal:
                    ...
                    try:
                        ...
                        _logger.exception(
                            "Failed to auto-create codify REPL terminal for %s",
                            session_id,
                        )
                    finally:
    ...
    async def _handle_codex_native_settings_update(
        ...
        from codify.codex_native_app_server import client_for_transport
        ...
        codex_client = client_for_transport(
            state.socket_path,
            client_name="codify-codex-native-runner",
        )
        try:
    ...
    async def _handle_pi_native_model_change(
        ...
        from codify.pi_native_bridge import bridge_dir_for_session_id, enqueue_model_change
    ...
    async def _teardown_session_terminals(conv_id: str) -> None:
        ...
        from codify.runner.tool_dispatch import _publish_terminal_deleted_event
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        from codify.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from codify.opencode_native_client import OpenCodeClientError
    ...
    async def _opencode_native_model_options(conv_id: str) -> list[dict[str, Any]]:
        ...
        from codify.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from codify.opencode_native_client import OpenCodeClient
    ...
    async def _handle_cursor_native_compact(conv_id: str) -> Response:
        from codify.cursor_native_bridge import bridge_dir_for_session_id, inject_user_message
    ...
    async def _handle_pi_native_compact(conv_id: str) -> Response:
        from codify.pi_native_bridge import bridge_dir_for_session_id, enqueue_compact
    ...
    def _inject_codex_compact(socket_path: str, target: str) -> None:
        from codify.claude_native_bridge import _run_tmux
    ...
    async def _handle_qwen_native_compact(conv_id: str) -> Response:
        from codify.qwen_native_bridge import bridge_dir_for_session_id, submit_user_message
    ...
    async def _handle_codex_native_cost_popup(
        ...
        from codify.native_cost_popup import launch_cost_popup
    ...
    async def _handle_opencode_native_blocked_notice(
        ...
        from codify.native_cost_popup import launch_blocked_notice
    ...
    async def _native_cost_popup_config_file(conv_id: str, harness: str) -> Path:
        ...
        from codify.opencode_native_bridge import write_cost_popup_config
        from codify.runner._entry import _make_auth_token_factory

        if harness == "claude-native":
            from codify import claude_native_bridge as _cnb
            ...
            from codify import codex_native_bridge as _cxb
    ...
    async def _repop_pending_cost_popup_on_attach(
        ...
        if harness not in ("claude-native", "codex-native", "opencode-native"):
            return
        from codify.native_cost_popup import launch_cost_popup, wait_for_tmux_client
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if cached_spec is None and spec_resolver is not None:
            ...
            if _aid:
                ...
                except (CodifyError, httpx.HTTPError, RuntimeError):
                    _logger.warning(
        ...
        if cached_spec is not None:
            ...
            from codify.runtime.prompt import build_instructions
        ...
        if conv not in _session_tool_schemas:
            ...
            if cached_spec is not None:
                try:
                    from codify.tools.manager import (
                        ToolManager,
                    )
        ...
        if cached_spec and cached_spec.mcp_servers:
            from codify.runner.mcp_manager import compute_spec_hash
    ...
    async def _stream_message_to_harness(
        ...
        async def proxy_stream() -> AsyncIterator[bytes]:
            ...
            from codify.runner.tool_dispatch import (
            ...
            try:
                async with client.stream(
                    ...
                    _codify_task_id: str | None = body.get("task_id")
                    _buffer = ""
                                        ...
                                        _dispatch_tasks.append(
                                            _asyncio.create_task(
                                                dispatch_tool_locally(
                                                    ...
                                                    conversation_id=conv_id,
                                                    task_id=_codify_task_id or _response_id,
    ...
    async def list_session_resources(
        ...
        from codify.entities.pagination import paginate_in_memory
    ...
    async def create_session_terminal(
        ...
        if declared_terminal is not None:
            from codify.tools.builtins.sys_terminal import (
        ...
        async with lock:
            ...
            if existing is None or not existing.running or not await existing.is_alive():
                ...
                try:
                    ...
                    _logger.exception(
                        "Failed to recreate codify REPL terminal for %s",
                        session_id,
                    )
                    return None
        ...
        async with lock:
            ...
            if existing is None or not existing.running or not await existing.is_alive():
                ...
                try:
                    ...
                    _logger.exception(
                        "Failed to recreate codify qwen terminal for %s",
                        session_id,
                    )
                    return None
        return resolve_terminal_entry_by_resource_id(session_id, terminal_id, registry)
    ...
    async def terminal_resource_attach_ws(
        ...
        if entry is None or not entry.instance.running or not await entry.instance.is_alive():
            if terminal_role == CODIFY_REPL_TERMINAL_ROLE:
        ...
        from codify.inner.terminal import (
    ...
    async def search_environment_files(
        ...
        from codify.runner.environment_filesystem import (
    ...
    async def list_filesystem_changes(
        ...
        from codify.runtime.filesystem_registry import GitStatusUnavailable
    ...
    async def read_environment_file_diff(
        ...
        from codify.entities.environment_filesystem import InvalidPath
        from codify.runner.environment_filesystem import _validate_path
        ...
        from codify.runner.environment_filesystem import CallerProcessFilesystem
    ...
    async def _resolve_session_agent_spec_or_none(session_id: str) -> Any | None:
        ...
        The cursor/opencode/kimi launch arms swallow ``CodifyError`` and
    ...
    async def get_session_models(session_id: str) -> JSONResponse:
        ...
        from codify.model_catalog import catalog_for_spec
    ...
    async def get_session_kiro_model_options(session_id: str) -> JSONResponse:
        if _session_harness_name(session_id) != "kiro-native":
            return JSONResponse(status_code=200, content={"models": []})
        from codify.kiro_native import list_kiro_cli_model_options
    ...
    async def get_session_cursor_model_options(session_id: str) -> JSONResponse:
        if _session_harness_name(session_id) != "cursor-native":
            return JSONResponse(status_code=200, content={"models": []})
        from codify.cursor_native import list_cursor_cli_model_options
    ...
    async def get_session_claude_model_options(session_id: str) -> JSONResponse:
        ...
        from codify.claude_native import claude_native_model_options
    ...
    async def reset_session_agent_cache(session_id: str, request: Request) -> JSONResponse:
        ...
        if not isinstance(agent_id, str) or not agent_id:
            agent_id = _session_agent_ids.get(session_id)
        if not agent_id:
            with contextlib.suppress(CodifyError, httpx.HTTPError, RuntimeError):
                snapshot = await _session_snapshot(session_id)
    ...
    async def mcp_execute(session_id: str, request: Request) -> JSONResponse:
        ...
        if method == "tools/call":
            ...
            from codify.runner.tool_dispatch import execute_tool
            ...
            if "__" in tool_name:
                ...
                try:
                    from codify.tools.mcp import McpElicitationRequired
    ...
    def _resolve_summarize_connection(
        ...
        from codify.spec.types import ApiKeyAuth, DatabricksAuth, ProviderAuth
    ...
    if (
        ...
        from codify.native_cost_popup import _list_tmux_clients
        from codify.runner.tool_dispatch import _publish_terminal_deleted_event
        from codify.terminals.pane_reaper import NativePaneReaper, PaneRef
...
def create_runner_app_from_env() -> FastAPI:
    ...
    minimal :class:`httpx.AsyncClient` for the Codify server, then delegates
    ...
    :class:`~codify.runner.transports.tcp.RunnerTCPSubprocess` and
    :class:`~codify.runner.transports.uds.RunnerSubprocess`.  It is
    intentionally lighter than :func:`codify.runner._entry.create_app`
...
_HARNESS_MODEL_ENV_KEY: dict[str, str] = {
    ...
    # :mod:`codify.inner.kimi_executor`; without this mapping a per-session
...
def _build_spawn_env_from_spec(
    ...
    *,
    ...
    try:
        from codify.runtime.workflow import (
...
async def _evaluate_agent_start_gate(
    ...
    """
    from codify.runner.policy import RunnerToolPolicyGate
...
def _apply_sandbox_override_from_verdict(
    ...
    """
    from codify.inner.datamodel import OSEnvSandboxSpec, OSEnvSpec
