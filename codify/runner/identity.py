...
RUNNER_ID_ENV_VAR = "CODIFY_RUNNER_ID"
RUNNER_PARENT_PID_ENV_VAR = "CODIFY_RUNNER_PARENT_PID"
...
RUNNER_WORKSPACE_ENV_VAR = "CODIFY_RUNNER_WORKSPACE"
RUNNER_TUNNEL_BINDING_TOKEN_ENV_VAR = "CODIFY_RUNNER_TUNNEL_BINDING_TOKEN"
...
RUNNER_INITIAL_AUTH_TOKEN_ENV_VAR = "CODIFY_RUNNER_INITIAL_AUTH_TOKEN"
...
RUNNER_DELEGATED_AUTH_ENV_VAR = "CODIFY_RUNNER_DELEGATED_AUTH"
RUNNER_TUNNEL_TOKEN_HEADER = "X-Codify-Runner-Tunnel-Token"
...
CODIFY_INTERNAL_WS_ORIGIN = "codify://internal"
...
RUNNER_ISOLATE_SESSION_ENV_VAR = "CODIFY_RUNNER_ISOLATE_SESSION"

# Marker env var stamped into every agent-facing environment so any
# process launched inside an Codify agent session can detect it is
# running under Codify. This is the analog of Claude Code's
# ``CLAUDE_CODE`` / ``CLAUDECODE`` and Codex's ``CODEX``. It is set once
# on the runner process (see :mod:`codify.runner._entry`) and inherited
...
# scrub.
CODIFY_SESSION_ENV_VAR = "CODIFY"
CODIFY_SESSION_ENV_VALUE = "1"
...
def get_stable_runner_id() -> str:
    ...
    the id is loaded from ``~/.codify/runners/runner_id`` or
...
def token_bound_runner_id(token: str) -> str:
    ...
    digest = hashlib.sha256(f"codify-runner:{stripped}".encode()).hexdigest()
...
def load_or_create_runner_id(path: Path) -> str:
    ...
    :param path: Path to the runner id cache file, e.g.
        ``Path.home() / ".codify" / "runners" / "runner_id"``.
...
def _default_runner_id_path() -> Path:
    ...
    :returns: ``~/.codify/runners/runner_id``.
    """
    return Path.home() / ".codify" / "runners" / "runner_id"
