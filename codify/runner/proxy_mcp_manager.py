...
Routes all MCP calls through the Codify server's
...
The Codify server holds the live connections in its
:class:`codify.server.mcp_pool.ServerMcpPool` and enforces
...
interface as :class:`codify.runner.mcp_manager.RunnerMcpManager` so
it can be substituted transparently at every dispatch site in
``runner/app.py``.

When to use each implementation:

- **AP mode** (out-of-process runner paired with Codify server):
  ``ProxyMcpManager(session_id, server_client)`` — all tool calls flow
  through the Codify server, policy enforced centrally.
- **No-AP / test mode**:
...
from codify.runner.mcp_manager import McpSchemasResult
from codify.runner.tool_dispatch import MCP_PROXY_CALL_TIMEOUT_S
...
class ProxyMcpManager:
    """Routes runner-side MCP calls through the Codify server MCP proxy endpoint.

    Drop-in substitute for :class:`codify.runner.mcp_manager.RunnerMcpManager`
    used in Codify mode.  The runner creates one instance per session on first MCP use
    and passes it to :func:`codify.runner.tool_dispatch.execute_tool` in place
    of a direct manager.
    ...
    :param ap_client: An :class:`httpx.AsyncClient` pointed at the Codify server.
        Must already carry the runner's service auth (e.g. Databricks bearer
        token) so requests are accepted by the Codify server's auth middleware.
    """
    ...
    def __init__(
        ...
        :param ap_client: HTTP client pointed at the Codify server.
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        """Fetch tool schemas from the Codify server MCP proxy (``tools/list``).

        Sends a ``tools/list`` JSON-RPC 2.0 request to the Codify server's MCP
        proxy endpoint.  Returns tool schemas in the same flat OpenAI
        function-tool format as
        :meth:`codify.runner.mcp_manager.RunnerMcpManager.schemas_for`.

        Tool names are returned with the server namespace prefix applied by
        the Codify server (e.g. ``github__search``).  This ensures the harness
        ...
        try:
            resp = await self._codify_client.post(
                self._mcp_url,
                json=payload,
                timeout=30.0,
            )
        ...
        for tool in tools_list:
            ...
            if not isinstance(name, str) or not name:
                continue
            # The Codify server returns ``inputSchema`` (JSON Schema from MCP).
    ...
    async def call_tool(
        ...
        """Dispatch a tool call via the Codify server MCP proxy (``tools/call``).

        Sends a ``tools/call`` JSON-RPC 2.0 request.  The Codify server enforces
        TOOL_CALL and TOOL_RESULT policies before forwarding to the real MCP
        server.

        **ASK policy / approval flow**: when the Codify server returns an
        ...
        :returns: Tool output string.  On denial the Codify server returns an
        ...
        """
        del spec  # Codify server resolves spec from session context
        ...
        for _attempt in range(2):
            ...
            # ── ASK: Codify server returned InputRequiredResult ───────────────
            ...
            if result.get("resultType") == "input_required":
                ...
                # The Codify server uses the elicitation_id as the key in
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op — the Codify server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op — the Codify server owns and manages MCP connections."""
