...
if TYPE_CHECKING:
    from codify.runtime.filesystem_registry import FilesystemRegistry
...
from codify.model_override import (
    harness_supports_model_override,
    model_family_mismatch,
    normalize_model_for_provider,
    validate_model_override,
)
from codify.native_coding_agents import public_agent_name
from codify.runtime import pending_elicitations
...
from codify.tools import ToolManager
from codify.tools.base import ToolContext
from codify.tools.builtins._arguments import parse_json_object_arguments
from codify.tools.builtins.async_inbox import (
    SysCallAsyncTool,
    SysCancelAsyncTool,
    SysCancelTaskTool,
    SysReadInboxTool,
)
from codify.tools.builtins.download_file import DownloadFileTool
from codify.tools.builtins.list_comments import ListCommentsTool
from codify.tools.builtins.os_env import (
    SysOsEditTool,
    SysOsReadTool,
    SysOsShellTool,
    SysOsWriteTool,
)
from codify.tools.builtins.session_rename import SysSessionRenameTool
from codify.tools.builtins.spawn import (
...
from codify.tools.builtins.timer import (
...
from codify.tools.builtins.update_comment import UpdateCommentTool
from codify.tools.builtins.upload_file import UploadFileTool, safe_resolve
...
_SESSION_WRAPPER_LABEL_KEY = "codify.wrapper"
...
# runner (runner → Codify server → runner). ``sys_os_shell`` accepts caller-provided
...
_TURN_ACTOR_LABEL = "codify.turn_actor"
...
# state via the Codify server's existing REST endpoints (GET /items, GET
...
# (codify.spec.types.SharePolicy) that enable the sys_session_share
...
# these proxy the Codify server's REST endpoints (GET /v1/sessions/{id}/agent,
...
# The runner proxies the Codify server's session policy REST endpoint.
_POLICY_TOOLS = frozenset({"sys_add_policy", "sys_policy_registry"})

# Priority 5l.1: Scheduled-task management — the runner proxies the Codify
...
# Codify desktop renderer claims and executes the action, then POSTs the
...
# none. See codify/tools/builtins/browser.py for the schema-only classes.
...
_BROWSER_TIMEOUT_ERROR = (
    '{"error": "browser action timed out — is the session open in the Codify desktop app?"}'
)
...
# rides through the Codify ``/mcp`` endpoint (comment, session read/write,
...
_NATIVE_RELAY_BUILTIN_TOOLS = (
    ...
    | _TERMINAL_TOOLS
    # ``browser_*`` must ride the native relay: the Codify desktop app
...
def build_native_relay_tool_schemas(spec: AgentSpec | None) -> list[_JsonObject]:
    """Build the flat Codify tool surface for native harness bridges.
    ...
    centralized policy enforcement on the Codify server.
    ...
    """
    from codify.tools.builtins.agents import (
    ...
    if spec is not None:
        ...
        from codify.tools.builtins.policy import SysAddPolicyTool, SysPolicyRegistryTool
    ...
    from codify.inner.os_env import create_os_environment
...
# sys_os_write, e.g. following the ``build-codify`` skill).
_AGENT_CONFIG_SUBDIR = ".codify/agent-configs"
...
async def _execute_uc_function_tool(
    ...
    *,
    ...
    :func:`codify.runner.uc_function.execute_uc_function`.
    ...
    :returns: Tool output as a string, or an error message.
    """
    from codify.runner.uc_function import execute_uc_function
...
def _publish_child_launching_update(
    *,
    ...
    if publish_event is not None:
        ...
        return
    from codify.runtime import session_stream
...
async def _list_child_sessions(
    *,
    ...
    :param server_client: Codify server client.
...
async def _teardown_failed_child(
    ...
    *,
    ...
    """
    from codify.runner import app as _runner_app
...
async def _build_subagent_message_content(
    ...
    *,
    ...
    :param server_client: Authenticated Codify server client.
...
def _normalize_subagent_model(
    ...
    *,
    ...
    :func:`codify.model_override.normalize_model_for_provider` for
    ...
    """
    from codify.model_catalog import resolve_model_provider
...
async def _execute_list_models_tool(*, agent_spec: AgentSpec | None) -> str:
    ...
    cached in :mod:`codify.model_catalog`).
...
async def _execute_subagent_tool(
    ...
    *,
    ...
    :param server_client: httpx client pointed at the Codify server.
    ...
    if existing is not None:
        ...
        # and the requested harness must canonicalize into CODIFY_HARNESSES.
        ...
        # override against the GLOBAL ``CODIFY_HARNESSES`` (plus the codify
        ...
        if harness_override is not None:
            ...
            if canonical not in CODIFY_HARNESSES:
                return (
                    f"Error: sys_session_send 'harness' {harness_override!r} is "
                    f"not a known harness; must be one of {sorted(CODIFY_HARNESSES)}."
                )
        ...
        from codify.onboarding.harness_install import missing_harness_cli
        ...
        if cost_budget is not None:
            policy_body = {
                ...
                "type": "python",
                "handler": "codify.policies.builtins.cost.subagent_cost_budget",
    ...
    from codify.server.schemas import SessionCreatedEvent
    ...
    if created_child:
        ...
        # has no subscribers (they live in the Codify server), so a direct
        # publish here is silently dropped. ``publish_event`` enqueues onto
        # the parent's queue, which the Codify server's relay republishes onto
...
async def _execute_session_create(
    ...
    *,
    ...
    :param server_client: HTTP client pointed at the Codify server; ``None``
...
def _bundle_local_agent_source(source: Path) -> bytes:
    ...
    :param source: Local agent config YAML, agent directory, or
        bundle file, e.g.
        ``Path("/work/.codify/agent-configs/helper.yaml")``.
...
async def _upload_config_bundle(
    ...
    *,
    ...
    :param config_path: Caller-supplied path to the agent config YAML,
        ...
        cwd, e.g. ``".codify/agent-configs/helper.yaml"``.
...
async def _execute_web_fetch_tool(
    ...
    *,
    ...
    """
    from codify.tools.builtins.web_fetch import (
        RESEARCHER_NAME,
...
async def _execute_web_search_tool(
    ...
    *,
    ...
    """
    from codify.tools.base import ToolContext
    from codify.tools.builtins.web_search import WebSearchTool
...
async def _execute_hindsight_tool(
    ...
    *,
    ...
    """
    from codify.tools.base import ToolContext
    from codify.tools.builtins import get_builtin_tool
...
def _has_subagent(
    ...
    dict (codify inner loader) for a matching name.
    ...
    for sa in sub_agents:
        if getattr(sa, "name", None) == sub_agent_name:
            return True
    # Codify inner loader: tools dict with AgentTool entries
    tools = getattr(agent_spec, "tools", None)
...
async def _execute_timer_set(
    ...
    *,
    ...
    """
    from codify.runner import app as _app
...
async def _execute_comment_tool(
    ...
    *,
    ...
    The runner is a separate subprocess from the Codify server and has no
    in-process ``CommentStore``. This handler uses ``server_client`` to
    call the Codify server's REST API (``GET/PATCH
...
async def _execute_browser_tool(
    ...
    *,
    ...
    Does the blocking round-trip that drives the Codify desktop app's
...
async def _execute_scheduled_task_tool(
    ...
    *,
    ...
    Codify server's ``/v1/scheduled-tasks`` REST endpoints over
...
def _project_api_item(item: dict[str, Any]) -> dict[str, str | None]:
    ...
    Mirrors :func:`codify.tools.builtins.spawn._project_activity_item`
...
async def _execute_session_query_tool(
    ...
    *,
    ...
    Codify server's existing REST endpoints over ``server_client``:
    ...
    :mod:`codify.tools.builtins.spawn` so the LLM sees identical
...
def _codify_error_message(resp: httpx.Response) -> str | None:
    """
    Extract the human-readable message from an Codify error response.

    The server renders :class:`codify.errors.CodifyError` as
    ``{"error": {"code": ..., "message": ...}}`` (see the exception
    handler in ``codify/server/app.py``). Return that ``message`` so a
...
async def _session_share_via_rest(
    ...
    if resp.status_code >= 400:
        ...
        detail = _codify_error_message(resp)
...
async def _execute_agent_tool(
    ...
    *,
    ...
    these proxy the Codify server's REST endpoints over ``server_client``:

    - ``sys_agent_get`` → ``GET /v1/sessions/{id}/agent`` (project the
      :class:`~codify.server.schemas.AgentObject`)
...
async def _agent_get_via_rest(
    ...
    Projects the :class:`~codify.server.schemas.AgentObject` fields
...
async def _session_get_history_via_rest(
    ...
    # lives only in the Codify server's pending-elicitations index, replayed
...
async def _session_close_via_rest(
    ...
    On success marks the child with ``codify.closed=true`` and
...
class _PeekMeta:
    ...
    :param pending_elicitations: Outstanding
        ...
        parked on, replayed on the snapshot from the Codify server's
        :mod:`codify.runtime.pending_elicitations` index. Empty list
...
def _clone_os_env_spec(spec: Any) -> Any:
    ...
    Symmetric with :func:`codify.inner.terminal._clone_sandbox_spec`;
...
def _runner_default_os_env_cwd(conversation_id: str | None) -> str:
    ...
    root = Path(
        os.environ.get(
            "CODIFY_RUNNER_OS_ENV_ROOT",
            str(Path(tempfile.gettempdir()) / "codify-runner-os-envs"),
        )
    )
...
def _effective_runner_os_env_spec(
    ...
    - When ``runner_workspace`` is set, it ALWAYS wins — whether
      ...
      passes it via ``CODIFY_RUNNER_WORKSPACE``) and
    ...
    :param runner_workspace: Authoritative runtime cwd for the
        runner, sourced from ``CODIFY_RUNNER_WORKSPACE``.
...
async def _seed_os_env_snapshot(
    ...
    :meth:`~codify.runtime.filesystem_registry.FilesystemRegistry.seed_snapshot`
    ...
    :param os_env: The :class:`~codify.inner.os_env.OSEnvironment` used for
...
async def _execute_os_env_tool(
    ...
    *,
    ...
    """
    from codify.inner.os_env import _DEFAULT_READ_LIMIT, create_os_environment
...
async def _execute_file_tool(
    ...
    *,
    ...
    :param server_client: HTTP client for the Codify server.
    ...
    :param runner_workspace: Authoritative runtime cwd for the runner,
        sourced from ``CODIFY_RUNNER_WORKSPACE``. Combined with
...
async def _execute_terminal_tool(
    ...
    *,
    ...
    # the Codify server's relay republishes onto ``session_stream``.
...
async def _emit_terminal_resource_event(
    *,
    ...
    (:func:`codify.server.routes.sessions._publish_and_persist_resource_event`)
...
async def _publish_terminal_created_event(
    *,
    ...
    if resource_registry is not None:
        ...
        from codify.entities.session_resources import terminal_resource_view
...
def _subagent_tool_result_policy_request(
    ...
    """
    Build the Codify policy-evaluation request for delayed child output.
...
async def _post_subagent_policy_verdict(
    *,
    ...
    """
    POST delayed sub-agent output to Codify policy evaluation.

    :param server_client: HTTP client pointed at Codify server.
...
def _apply_subagent_policy_verdict(
    ...
    """
    Apply an Codify policy verdict to a sub-agent inbox payload.

    :param payload: Original completed sub-agent payload.
    :param verdict: Parsed Codify policy response, e.g.
...
def _inject_orchestrator_skills(
    ...
    """
    Auto-inject built-in platform skills for every codify agent.

    The ``build-codify`` skill teaches the LLM how to author valid
    agent configs. Every agent on the platform should have access to it
    — whether it declares ``tools.agents`` or not — so that any
    ``codify claude`` user can author and launch new agents. The
    skill is injected from the canonical source at
    ``codify/onboarding/agent/skills/build-codify/`` when not
    ...
    if "build-codify" in existing_names:
        return skills
    from codify.spec.parser import _discover_skills
...
def _execute_skill_tool(
    ...
    *,
    ...
    :returns: Tool output string.
    """
    from codify.tools.builtins.load_skill import LoadSkillTool
    from codify.tools.builtins.read_skill_file import ReadSkillFileTool

    bundled_skills = list(getattr(agent_spec, "skills", None) or [])
    skills_filter = getattr(agent_spec, "skills_filter", "all")
    # Auto-inject the build-codify skill for agents that opt into the
