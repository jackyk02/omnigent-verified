...
# Role marker for the embedded Codify REPL terminal auto-created for
# runner-hosted SDK sessions (``codify attach`` in a tmux pane — the
...
CODIFY_REPL_TERMINAL_ROLE = "codify-repl"
...
class SessionResourceRegistry:
    ...
    def set_session_status_publisher(
        ...
        event onto the session's SSE queue (which the Codify server relays
    ...
    def compute_default_env_root(
        ...
        1. ``self._runner_workspace`` (sourced from
           ``CODIFY_RUNNER_WORKSPACE``) — when set, ALWAYS
