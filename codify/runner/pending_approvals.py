...
Lifted out of ``codify.runner.app`` so the gate that lives in
``codify.runner.tool_dispatch`` can register and wait without
...
async def wait_for_user_approval(
    *,
    ...
    Codify server's pending-elicitations index (powers the sidebar
    ...
    :param elicitation_id: Correlation id minted by the Codify server's
    ...
    try:
        ...
        # Signal the Codify server's pending-elicitations index that
