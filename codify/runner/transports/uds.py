...
The CLI integration that wires `codify run` to spawn both a
...
class RunnerSubprocess:
    ...
    Used by tests + Phase 2's `codify run` to ship the runner as
    ...
    :param app_factory_path: Module-and-attribute path uvicorn will
        ...
        ``"codify.runner.app:create_runner_app_from_env"``. Uvicorn's
    ...
    :param startup_timeout_s: Max wait for the subprocess to be
        ...
        factory cold-imports ``codify.runner.app`` (and the whole
    ...
    app_factory_path: str = "codify.runner.app:create_runner_app_from_env"
    ...
    def __enter__(self) -> RunnerSubprocess:
        if self.socket_path is None:
            ...
            self._tmp_dir = tempfile.TemporaryDirectory(prefix="codify-runner-")
        ...
        # (including the codify source) are importable.
        cmd = [
            sys.executable,
            "-m",
            "uvicorn",
