...
# (``codify.chat._wait_for_remote_runner``, currently 60 s).
            ...
            if redirect_url is not None:
                ...
                if not ever_connected and login_redirect_streak >= _LOGIN_REDIRECT_FATAL_ATTEMPTS:
                    raise RuntimeError(
                        ...
                        f"run `codify login {server_url}` or "
                        "`codify setup` to configure credentials"
                    ) from exc
                retry_reason = (
...
async def _serve_tunnel_once(
    ...
    *,
    ...
    :param server_url: HTTP(S) server base URL the tunnel belongs to,
        e.g. ``"https://example.databricks.com/api/2.0/codify"``. Used
