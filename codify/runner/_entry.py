...
- ``RUNNER_SERVER_URL``: Codify server base URL for outbound calls
...
from codify.runner.transports.ws_tunnel.serve import RUNNER_TUNNEL_REJECTION_PREFIX
from codify.version import VERSION

if TYPE_CHECKING:
    from types import TracebackType

    from codify.runner.native import ResolvedSpec
    from codify.runner.transports.ws_tunnel.serve import _ASGIApp

_RUNNER_SERVER_URL_ENV_VAR = "RUNNER_SERVER_URL"
_RUNNER_PREWARM_SPEC_PATH_ENV_VAR = "RUNNER_PREWARM_SPEC_PATH"
# The runner advertises the codify version it is actually running (shared
# with the CLI/server/host) instead of a hard-coded placeholder.
_RUNNER_VERSION = VERSION
_RUNNER_CONFIG_HOME_ENV_VAR = "CODIFY_CONFIG_HOME"
...
def _server_url_from_env() -> str:
    """Return the required Codify server URL from the runner environment.
...
def _runner_config_path() -> Path:
    """Return the global Codify config path visible to the runner.

    Respects :envvar:`CODIFY_CONFIG_HOME` for test isolation and
    subprocess consistency with the CLI/onboarding layer.

    :returns: Config path, e.g. ``Path("~/.codify/config.yaml")``.
    """
    config_home = os.environ.get(_RUNNER_CONFIG_HOME_ENV_VAR)
    if config_home:
        return Path(config_home).expanduser() / "config.yaml"
    return Path.home() / ".codify" / "config.yaml"
...
class _RunnerDatabricksAuth(httpx.Auth):
    ...
    Used by the runner's HTTP client for callbacks to the Codify server
    ...
    def __init__(
        ...
        :param server_url: Codify server URL used to look up the ``?o=``
...
class _InitialAuthTokenFactory:
    ...
    def __init__(self, token: str, server_url: str) -> None:
        ...
        :param server_url: Codify server URL used by the fallback resolver.
        """
...
def _make_auth_token_factory(
    ...
    *,
    ...
    Resolution order:
      ...
      3. Stored OIDC token from ``~/.codify/auth_tokens.json``
         (populated by ``codify login``), keyed by ``server_url``.
      4. Databricks OAuth token (refreshed via the SDK) — host-keyed
         when a Databricks Apps pointer record is stored for
         ``server_url`` (``codify login <apps-url>``), ambient
         otherwise.
    ...
    :param server_url: Server URL to look up the stored OIDC token
        ...
        non-runner callers (e.g. ``codify host``) must pass
    ...
    - :class:`_RunnerDatabricksAuth` for the httpx client
      (refreshed on each HTTP callback to the Codify server).
    - ``codify/host/connect.py`` for the host tunnel's WS upgrade
      headers.
    ...
    def _sdk_token() -> str | None:
        ...
        if not sdk_auth_resolved:
            ...
            # ``codify login <apps-url>``) names the exact workspace
    ...
    def _factory() -> str | None:
        ...
        Checks the stored OIDC token first (from ``codify login``),
...
def _make_managed_mint_factory(
    ...
    :param server_url: Codify server base URL, e.g.
        ``"https://codify.example.com"``.
    ...
    # an Apps OAuth redirect that happens before the request reaches Codify.
...
class _ManagedMintTokenFactory:
    ...
    def __init__(self, mint_url: str, server_url: str, binding_token: str) -> None:
        ...
        :param server_url: Codify server base URL.
...
def _mint_managed_owner_token(
    ...
    :param binding_token: The runner's tunnel binding token, sent as the
        ``X-Codify-Runner-Tunnel-Token`` header to authenticate the mint.
...
def _runner_tunnel_binding_token_from_env() -> str | None:
    ...
    """
    from codify.runner.identity import RUNNER_TUNNEL_BINDING_TOKEN_ENV_VAR
...
def _runner_parent_pid_from_env() -> int | None:
    ...
    """
    from codify.runner.identity import RUNNER_PARENT_PID_ENV_VAR
...
def _runner_workspace_from_env() -> Path | None:
    ...
    """
    from codify.runner.identity import RUNNER_WORKSPACE_ENV_VAR
...
def _runner_isolate_session_from_env() -> bool:
    """Return ``True`` when ``CODIFY_RUNNER_ISOLATE_SESSION`` is ``"1"``.

    See :data:`RUNNER_ISOLATE_SESSION_ENV_VAR` for the contract.
    """
    from codify.runner.identity import RUNNER_ISOLATE_SESSION_ENV_VAR
...
async def _resolve_agent_spec_from_server(
    ...
    """
    Fetch, cache, and parse one agent spec bundle from the Codify server.
...
def create_app(
    ...
    """
    from codify.cli_auth import databricks_request_headers
    from codify.runner.app import create_runner_app
    from codify.runner.identity import (
        CODIFY_INTERNAL_WS_ORIGIN,
        CODIFY_SESSION_ENV_VALUE,
        CODIFY_SESSION_ENV_VAR,
        RUNNER_ID_ENV_VAR,
        RUNNER_TUNNEL_TOKEN_HEADER,
    ...
    # Stamp the Codify session marker into the runner's environment so
    # every process this runner spawns can detect it is running inside an
    # Codify agent session, the way Claude Code sets CLAUDE_CODE and
    ...
    os.environ[CODIFY_SESSION_ENV_VAR] = CODIFY_SESSION_ENV_VALUE

    # Keep the harness manager on its default /tmp/codify root.
    ...
    async def spec_resolver(agent_id: str, session_id: str | None = None) -> ResolvedSpec | None:
        """
        Fetch agent spec from the Codify server, extract under the
    ...
    from codify.inner.terminal import reap_orphaned_terminals
    ...
    async def _start_pm() -> None:
        ...
        if prewarm_path and mcp_manager is not None:
            try:
                from codify.spec import load as _load_spec
...
async def _run_tunnel_from_env() -> None:
    ...
    :returns: None.
    """
    from codify.runner.identity import get_stable_runner_id
    from codify.runner.transports.ws_tunnel.serve import serve_tunnel
...
def _install_crash_logging() -> None:
    ...
    if getattr(previous_hook, "_codify_runner_crash_hook", False):
        return
    ...
    _log_uncaught._codify_runner_crash_hook = True  # type: ignore[attr-defined]
...
def main() -> None:
    ...
    # Agent bundles reference the verifier key as ${DEEPSEEK_API_KEY};
    # spec expansion happens in THIS process, so auto-load the stored key.
