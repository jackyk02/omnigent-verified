...
Pre-refactor the Codify server's :class:`PolicyEngine` enforced
...
``evaluate_policy=True`` to the Codify server, which independently
re-evaluates and parks an elicitation; the runner then awaits the
verdict via :mod:`codify.runner.pending_approvals`. The dual
...
from codify.policies import FunctionPolicy, resolve_function_policy
...
class PolicyVerdict:
    ...
    on ASK (and escalate to the Codify server) rather than treating
    ...
    :param action: ``"allow"`` (tool may proceed), ``"deny"`` (tool
        ...
        the Codify server for a user verdict; ``policy_name`` and
