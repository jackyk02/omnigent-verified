...
from codify.spec.types import AgentSpec, MCPServerConfig, RetryPolicy
from codify.tools.base import is_valid_tool_name
from codify.tools.mcp import McpServerConnection
...
def _build_accept_content(
    ...
    :func:`codify.tools._elicitation_schema.build_accept_content_from_schema`.
...
def _mcp_tool_schema(
    ...
    """
    from codify.tools.mcp import _normalize_input_schema
...
class RunnerMcpManager:
    ...
    def __init__(
        ...
        :param server_client: ``httpx.AsyncClient`` pointed at the
            Codify server. When provided, inline MCP elicitations are
            surfaced to the user via the Codify server's session events
    ...
    def _build_elicitation_callback(
        ...
        async def _elicit(
            ...
            When an Codify server client is available, POSTs a
            ...
            if server_client is None:
                _logger.warning(
                    "MCP elicitation callback: no Codify server client available — declining",
                )
            ...
            try:
                ...
                _logger.warning(
                    "MCP elicitation callback: Codify server POST failed (%s) — declining",
                    exc,
                )
                return ElicitResult(action="decline")
            ...
            if not isinstance(elicitation_id, str) or not elicitation_id:
                _logger.warning(
                    "MCP elicitation callback: Codify server returned no "
            ...
            # won't fire on timeout/cancellation, so the Codify server's
            # sidebar badge may stay stale. Same pattern as
            # proxy_mcp_manager. A future enhancement could POST
            # the resolved event back to the Codify server here.
