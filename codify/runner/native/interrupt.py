...
sixteen closures inside :mod:`codify.runner.app`; this module collapses them
onto one :class:`NativeInterruptRunner` so the dispatch is registry-driven.

Mirrors :class:`codify.runner.codex.goal.CodexGoalRunner`: app-scope state
...
from codify.runner.native.orchestration import (
...
class _UniformInterrupt:
    ...
    :param module: The harness bridge module, e.g. ``"codify.pi_native_bridge"``.
...
class NativeInterruptRunner:
    ...
    async def _claude_interrupt(self, conv_id: str) -> Response:
        from codify.claude_native_bridge import bridge_dir_for_bridge_id, inject_interrupt
    ...
    async def _claude_stop(self, conv_id: str) -> Response:
        from codify.claude_native_bridge import bridge_dir_for_bridge_id, kill_session
