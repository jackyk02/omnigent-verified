...
if TYPE_CHECKING:
    ...
    from codify.claude_native import ClaudeNativeUcodeConfig
    from codify.codex_native_app_server import CodexAppServerClient

import click
import httpx
from fastapi.responses import JSONResponse, Response

from codify._platform import IS_WINDOWS
from codify.entities.session_resources import (
    SessionResourceView,
    session_resource_view_to_dict,
    terminal_resource_id,
)
from codify.harness_plugins import native_provider_for_key
from codify.model_override import validate_model_override
from codify.native_coding_agents import (
    native_coding_agent_for_harness,
    native_coding_agent_for_terminal_name,
)
from codify.native_dispatch import resolve_hook
from codify.runner.resource_registry import (
    ANTIGRAVITY_NATIVE_TERMINAL_ROLE,
    CLAUDE_NATIVE_TERMINAL_ROLE,
    CODEX_NATIVE_TERMINAL_ROLE,
    CODIFY_REPL_TERMINAL_ROLE,
...
from codify.runner.session_init_protocol import (
    RunnerSessionInitEnvelope,
)
from codify.spec.types import AgentSpec

_logger = logging.getLogger("codify.runner.app")
...
def _publish_tmux_target_for_bridge(
    *,
    ...
    from codify.claude_native_bridge import bridge_dir_for_bridge_id, write_tmux_target
...
class _CodexNativeLaunchConfig:
    ...
    :param policy_server_url: Codify server URL for the Codex policy hook and
    ...
    :param fork_source_id: SOURCE conversation id stamped on a forked
        clone (``codify.fork.source_id``), used to locate the
    ...
    :param fork_source_external_id: SOURCE Codex thread id stamped on a
        forked clone (``codify.fork.source_external_session_id``),
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a
        native target (``codify.fork.carry_history``); when no source
        rollout exists to clone (an SDK or cross-family source) the runner
        builds the clone's rollout from the copied Codify items instead (see
        ``_ensure_local_codex_resume_rollout``).
    :param bypass_sandbox: ``True`` when the session opted into Codex's
        DANGEROUS full-bypass stance (``codify.codex_native.bypass_sandbox``
...
class _PiNativeLaunchConfig:
    ...
    :param server_url: Codify server URL for the extension/forwarder.
...
def _codex_session_workspace(session_workspace: str | None) -> Path:
    ...
    runner's ``CODIFY_RUNNER_WORKSPACE``.
    ...
    :raises RuntimeError: If no workspace is available (neither the
        session snapshot nor ``CODIFY_RUNNER_WORKSPACE``).
    """
    raw = session_workspace or _required_runner_env("CODIFY_RUNNER_WORKSPACE")
...
async def _pi_native_launch_config(
    *,
    ...
    :param server_client: Runner Codify server client.
    ...
    from codify.stores.conversation_store import (
...
class _OpenCodeNativeLaunchConfig:
    ...
    :param policy_server_url: Codify server URL for the forwarder.
    ...
    :param fork_carry_history: ``True`` on a forked clone whose prior
        ...
        (``codify.fork.carry_history``); opencode has no native session to
        clone, so the runner rehydrates from the copied Codify transcript.
    """
...
async def _opencode_native_launch_config(
    *,
    ...
    from codify.stores.conversation_store import FORK_CARRY_HISTORY_LABEL_KEY
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    :param server_client: Runner Codify server HTTP client.
    :param ensure_comment_relay: Callback that starts the Codify builtin-tool
        relay for this session's bridge dir (the nested
        ``_ensure_comment_relay_started``). ``None`` skips wiring the Codify
        MCP relay (tests / no server).
    :returns: The created terminal resource view.
    """
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from codify.opencode_native_app_server import (
        OpenCodeNativeServer,
        build_opencode_attach_args,
        opencode_terminal_env,
    )
    from codify.opencode_native_bridge import (
    ...
    from codify.opencode_native_forwarder import OpenCodeNativeForwarder
    ...
    # Codify builtin-tool relay (wired below) can start. Safe to call before
    ...
    from codify.opencode_native_bridge import xdg_config_home_for_bridge_dir
    from codify.opencode_native_provider import (
        build_opencode_codify_mcp_server,
    ...
    # Build opencode's ``mcp`` block: the Codify builtin-tool relay (so the
    # model can call sys_*/load_skill/web_fetch — the real "connects to Codify
    ...
    # through Codify's policy engine via the forwarder's permission gate —
    ...
    if server_client is not None and ensure_comment_relay is not None:
        mcp_block.update(build_opencode_codify_mcp_server(bridge_dir))
    if mcp_block:
        config.setdefault("$schema", "https://opencode.ai/config.json")
        config["mcp"] = mcp_block
        config["permission"] = "ask"

    # Load the Codify policy-bridge plugin so opencode's lifecycle hooks reach
    ...
    # PostToolUse hooks); coordinates come from the CODIFY_* env stamped on
    ...
    if server_client is not None and runner_server_url:
        ...
        policy_env["CODIFY_POLICY_URL"] = runner_server_url
        policy_env["CODIFY_SESSION_ID"] = session_id
        ...
        from codify.claude_native_bridge import _TOOL_RELAY_FILE

        policy_env["CODIFY_RELAY_FILE"] = str(bridge_dir / _TOOL_RELAY_FILE)
        # Bake fallback headers for the first calls before relay starts.
        from codify.runner._entry import _make_auth_token_factory

        _policy_factory = _make_auth_token_factory()
        _policy_token = _policy_factory() if _policy_factory is not None else None
        if _policy_token:
            from codify.cli_auth import databricks_request_headers

            policy_env["CODIFY_POLICY_HEADERS"] = json.dumps(
    ...
    # Start the Codify builtin-tool relay BEFORE opencode boots, so
    ...
    # relay POSTs each call back through the Codify server (policy enforced).
    if server_client is not None and ensure_comment_relay is not None:
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) — we'll rehydrate from the Codify transcript
                    # below instead of silently starting empty.
                    resume_lost_history = True
            if opencode_session_id is None:
                created = await client.create_session({"title": f"codify:{session_id}"})
                ...
                # seed the copied Codify transcript as a noReply preamble.
    ...
    # lifetime; ``server_client`` is the runner's Codify client. The
...
def _build_opencode_policy_evaluator(
    *,
    ...
    :param server_client: Runner's Codify server HTTP client.
    :param conversation_id: Owning Codify session id, e.g. ``"conv_abc"``.
    :returns: An async evaluator returning a verdict mapping, or a deny
        verdict on failure.
    """
    from codify.opencode_native_permissions import OPENCODE_NATIVE_HARNESS
...
def _opencode_native_model_from_spec(agent_spec: Any | None) -> str | None:
    ...
    if agent_spec is None:
        return None
    try:
        from codify.runtime.workflow import _resolve_spec_model
...
def _resolve_opencode_compact_model(
    ...
    opencode's ``/summarize`` requires an explicit model, but Codify
...
def _render_opencode_transcript_text(items: list[Any]) -> str:
    """
    Render committed Codify message items into a plain-text transcript.
...
async def _rehydrate_opencode_session_from_transcript(
    *,
    ...
    persisted opencode session is gone) inject the Codify transcript as a
    ...
    try:
        resp = await server_client.get(
            f"/v1/sessions/{urllib.parse.quote(codify_session_id, safe='')}/items",
    ...
    try:
        ...
        _logger.warning(
            "opencode resume: rehydration seed failed for %s",
            codify_session_id,
            exc_info=True,
        )
        return False
...
def _pi_args_have_session_control(args: list[str]) -> bool:
    ...
    :returns: ``True`` when Codify should not add resume/session flags.
    """
    session_flags = {
        "--session-dir",
        "--session",
...
def _pi_args_have_provider(args: list[str]) -> bool:
    ...
    Codify must not inject the ``codify setup`` provider on top — the
    explicit choice wins.

    :param args: User pass-through Pi CLI args.
    :returns: ``True`` when Codify should not add provider/model args.
    """
...
def _build_pi_native_args(
    *,
    ...
    :param extension_path: Generated Codify Pi extension path.
    :param session_dir: Per-Codify-session Pi session directory.
...
async def _resolve_pi_resume_session(
    *,
    ...
    1. **Cold resume** — the session already carries a captured Pi
       ...
       file from committed Codify items so ``pi --session <id>`` opens with
    ...
    2. **Fork rebuild** — a forked clone bound to a pi-native target with NO
       ...
       session id, build its file from the clone's OWN copied Codify items,
       and patch the server so Codify reflects the clone's session id and a
    ...
    :param session_id: Codify conversation id, e.g. ``"conv_abc123"``.
    ...
    from codify.pi_native_resume import ensure_local_pi_resume_session, mint_pi_session_id
    ...
    model = ""
    try:
        from codify.pi_native_credentials import resolve_pi_native_provider
    ...
    # yet. Build the clone's session from its OWN copied Codify items under a
    ...
    if launch_config.fork_carry_history:
        ...
        if built is not None:
            # Record the minted id so Codify reflects the clone's own Pi
...
async def _auto_create_pi_terminal(
    ...
    *,
    ...
    """
    await _cancel_auto_forwarder_task(session_id)
    from codify.conversation_browser import conversation_url
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from codify.pi_native import resolve_pi_executable
    from codify.pi_native_bridge import (
    ...
    from codify.pi_native_bridge import extension_path as pi_extension_path
    from codify.runner._entry import _make_auth_token_factory
    ...
    # Build the Codify tool surface (sys_* tools) the Pi extension registers
    ...
    # can call Codify tools with centralized server-side policy enforcement
    ...
    try:
        from codify.runner.tool_dispatch import build_native_relay_tool_schemas
    ...
    # Rebuild the local Pi session JSONL from committed Codify items so a
    ...
    from codify.pi_native import pi_supports_approve
    ...
    pi_env = {
        ...
        "CODIFY_PI_NATIVE_BRIDGE_DIR": str(bridge_dir),
    }
    # Route the runner-owned Pi process through the provider configured by
    # ``codify setup`` (Databricks gateway / API key), so a separate
    ...
    if not _pi_args_have_provider(launch_config.terminal_launch_args or []):
        from codify.pi_native_credentials import (
    ...
    if server_client is not None and ensure_comment_relay is not None:
        ...
        from codify.claude_native_bridge import _TOOL_RELAY_FILE, _read_json_file
        from codify.pi_native_bridge import inject_relay_into_config
...
async def _post_pi_native_credential_warning(
    *,
    ...
    :param session_id: Session/conversation identifier.
    :param server_client: Runner Codify server client (``None`` in tests).
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from codify.cursor_native import resolve_cursor_executable
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from codify.cursor_native import is_valid_cursor_chat_id
    from codify.cursor_native_bridge import (
    ...
    from codify.cursor_native_forwarder import clear_cursor_bridge_state, preseed_resume_state
    from codify.cursor_native_status import clear_cursor_status_state
    from codify.cursor_native_usage import clear_cursor_usage_state
    ...
    # Render the copied Codify items once and stash them; the executor prepends
    ...
    if launch_config.fork_carry_history and not preseeded and server_client is not None:
        try:
            from codify.claude_native import _fetch_all_session_items_for_claude_resume
    ...
    # passthrough launch args (``codify cursor -- --model X`` or the joined
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from codify.cursor_native_bridge import write_tmux_target
    ...
    # Mirror the Cursor TUI's conversation back into the Codify session so the
    ...
    from codify.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from codify.cursor_native_forwarder import supervise_cursor_forwarder
    from codify.cursor_native_permissions import (
        cursor_launch_args_enable_yolo,
        supervise_cursor_transcript_elicitations,
    )
    from codify.cursor_native_usage import supervise_cursor_usage_forwarder
    ...
    async def _supervise_cursor_native_bridges() -> None:
        ...
        calls (see :mod:`codify.cursor_native_permissions`) — more reliable
        ...
        :mod:`codify.cursor_native_usage`).
        """
...
async def _auto_create_goose_terminal(
    ...
    *,
    ...
    ``~/.config/goose/config.yaml``), so HOME is inherited and Codify writes no
    ...
    :returns: Created terminal resource view.
    """
    from codify.goose_native import resolve_goose_executable
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from codify.goose_native_bridge import bridge_dir_for_session_id, write_tmux_target
    from codify.goose_native_forwarder import clear_goose_bridge_state
    ...
    # Mirror the Goose TUI's conversation back into the Codify session so the
    ...
    from codify.goose_native_forwarder import supervise_goose_forwarder
    from codify.goose_native_permissions import supervise_goose_approval_mirror
    ...
    async def _supervise_goose_native_bridges() -> None:
        ...
        a web elicitation (see :mod:`codify.goose_native_permissions`).
        """
        await asyncio.gather(
...
async def _auto_create_hermes_terminal(
    ...
    *,
    ...
    ``~/.hermes/config.yaml``), so HOME is inherited and Codify writes no vendor
    ...
    ``cwd`` + ``started_at`` floor (see :mod:`codify.hermes_native_forwarder`).
    ...
    :returns: Created terminal resource view.
    """
    from codify.hermes_native import resolve_hermes_executable
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    # drop the prior terminal's stale forward cursor.
    await _cancel_auto_forwarder_task(session_id)
    from codify.hermes_native_bridge import (
        bridge_dir_for_session_id,
        read_hermes_home,
        write_policy_hook_config,
        write_tmux_target,
    )
    from codify.hermes_native_forwarder import clear_hermes_bridge_state
    from codify.hermes_native_status import clear_hermes_status_state
    ...
    # Write a per-session HERMES_HOME with the Codify policy hook so the
    # native TUI evaluates tool calls against Codify policies.
        ...
        if _source_db is not None and _source_db.is_file():
            ...
            if _target_db is not None:
                try:
                    ...
                    # the forwarder only mirrors NEW messages (Codify already
                    # has the cloned ones from the fork item copy).
                    if _clone_max_id > 0:
                        from codify.hermes_native_forwarder import (
                            _ForwardState,
                            _write_state,
                        )
    ...
    # Mirror the Hermes TUI's conversation back into the Codify session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from codify.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from codify.hermes_native_bridge import read_hermes_home
    from codify.hermes_native_forwarder import supervise_hermes_forwarder
    from codify.hermes_native_permissions import supervise_hermes_approval_mirror
    ...
    if server_client is not None and ensure_comment_relay is not None:
        ...
        from codify.claude_native_bridge import _TOOL_RELAY_FILE, _read_json_file
        from codify.hermes_native_bridge import inject_relay_into_policy_hook
    ...
    async def _supervise_hermes_native_bridges() -> None:
        ...
        elicitation (see :mod:`codify.hermes_native_permissions`).
        """
...
async def _auto_create_kiro_terminal(
    ...
    *,
    ...
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from codify.kiro_native import build_kiro_launch
    from codify.kiro_native_bridge import (
    ...
    bridge_dir = prepare_bridge_dir(session_id)
    # Declare the Codify MCP server in the workspace-scoped kiro config so
    # kiro-cli can call Codify tools. Only when the tool relay will actually
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from codify.kiro_native_bridge import write_tmux_target
    ...
    # Start the Codify builtin-tool relay (writes tool_relay.json into the kiro
    # bridge dir) so the serve-mcp server declared in the workspace mcp.json can
    # route Codify tool calls back through the session's policy/elicitation
    ...
    from codify.kiro_native_permissions import supervise_kiro_permission_mirror
    from codify.kiro_native_session_forwarder import supervise_kiro_session_forwarder
...
async def _persist_qwen_external_session_id(
    ...
    """Record the qwen session id on the Codify session as ``external_session_id``.
    ...
    ``fork_conversation`` stamps as ``codify.fork.source_external_session_id``
    ...
    :param server_client: Runner Codify server client (``None`` skips the write).
    :param session_id: Codify session/conversation id.
...
async def _build_qwen_fork_recording(
    ...
    *,
    ...
    """Synthesize a qwen chat recording for a forked clone from its Codify items.

    A forked clone has its OWN copied Codify items but no qwen recording yet
    ...
    :param server_client: Runner Codify server client.
    :param session_id: The forked clone's Codify conversation id.
    ...
    """
    from codify.pi_native_resume import fetch_all_session_items_for_pi_resume
    from codify.qwen_native_bridge import (
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    env vars or ``~/.qwen`` from ``/auth``), so HOME is inherited and Codify
    ...
    """
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from codify.qwen_native import resolve_qwen_executable
    ...
    from codify.qwen_native_forwarder import clear_qwen_bridge_state
    ...
    # is persisted on the Codify session and read back from the snapshot
    # (``launch_config.external_session_id``), which also lets a fork carry history
    # (``codify.fork.source_external_session_id``). qwen is cleaner than
    ...
    # clone's copied Codify items and force ``--resume``. Gated on a NULL
    ...
    # Expose Codify's builtin tools (sys_*, load_skill, web_fetch, …) to qwen
    ...
    if mcp_enabled:
        try:
            ...
            _logger.warning(
                ...
                "Codify MCP wiring for session %s.",
                session_id,
                exc_info=True,
            )
        else:
    ...
    # the default in-terminal prompt (the embedded pane shows it) — Codify-side
    ...
    # Mirror the qwen TUI's conversation back into the Codify session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from codify.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from codify.qwen_native_bridge import qwen_session_recording_path
    from codify.qwen_native_forwarder import (
        supervise_qwen_compaction_mirror,
        supervise_qwen_forwarder,
    )
    from codify.qwen_native_permissions import supervise_qwen_approval_mirror
    ...
    async def _supervise_qwen_native_bridges() -> None:
        ...
        elicitations (see :mod:`codify.qwen_native_permissions`); the compaction
        ...
        :func:`codify.qwen_native_forwarder.supervise_qwen_compaction_mirror`).
        """
        await asyncio.gather(
            supervise_qwen_forwarder(
                base_url=server_url,
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    :func:`codify.kimi_native_credentials.build_kimi_session_home`) that
    mirrors the user's global ``kimi login`` (symlinked ``oauth`` / providers)
    and adds the Codify tool-policy hooks — a ``PreToolUse`` deny-gate and a
    ``PermissionRequest`` read-only surface dispatched to
    :mod:`codify.kimi_native_hook`. The hook subprocess reads its routing
    from ``hook_config.json`` in the bridge dir.

    A background forwarder (:func:`codify.kimi_native_forwarder.
    supervise_kimi_forwarder`) tails kimi's per-session ``wire.jsonl`` transcript
    and mirrors each user prompt + assistant reply into the Codify chat, so the
    ...
    :param resource_registry: Session resource registry for launching the
        terminal.
    :param publish_event: Runner session event publisher.
    :param server_client: Runner Codify server client (used only for the
    ...
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from codify.kimi_native import resolve_kimi_executable
    from codify.kimi_native_bridge import (
        bridge_dir_for_session_id,
        write_hook_config,
        write_tmux_target,
    )
    from codify.kimi_native_credentials import build_kimi_session_home
    from codify.kimi_native_forwarder import clear_kimi_bridge_state, supervise_kimi_forwarder
    from codify.runner._entry import _make_auth_token_factory
    ...
    # launch args (``codify kimi -- <args>``) are persisted on the session
    # snapshot and threaded here.
    kimi_args = list(launch_config.terminal_launch_args or [])

    # Wire the Codify tool-policy hooks: kimi reads a single
    ...
    # ``codify.kimi_native_hook``. The hook subprocess reads the server URL +
    ...
    # Mirror the kimi TUI transcript into the Codify chat: tail the per-session
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        surface the new terminal on the live stream (the Codify relay
    ...
    :param server_client: Runner's Codify server HTTP client. Used to read
    ...
    from codify.codex_native_app_server import (
    ...
    from codify.codex_native_bridge import (
        clear_bridge_state,
    ...
    # Route across all offerings: a configured provider (codify setup),
    ...
    from codify.inner.codex_executor import _find_codex_cli
    ...
    if (
        ...
        from codify.codex_native import _clone_codex_rollout, _mint_codex_thread_id
        ...
        if cloned_rollout is not None:
            ...
            # Record the assigned thread id now so Codify reflects the clone's
            ...
            if server_client is not None:
                try:
                    ...
                    # unset on the Codify session and a future relaunch of this
    ...
    if (
        ...
        # available: build the clone's rollout from its own copied Codify
        ...
        from codify.codex_native import (
            _ensure_local_codex_resume_rollout,
    ...
    if launch_config.external_session_id is not None and original_external_session_id is not None:
        from codify.codex_native import _ensure_local_codex_resume_rollout
    ...
    # the terminal launch.
    from codify.inner.codex_executor import populate_codex_skills_from_bundle
    ...
    # Codify coordinates for the codex-native policy hook. The hook runs as a
    ...
    event_client = CodexAppServerClient(
        ws_url=codex_ws_url,
        client_name="codify-codex-native-auto",
    )
    if launch_config.external_session_id is None:
        ...
        from codify.codex_native_bridge import CodexNativeBridgeState, write_bridge_state
        ...
        terminal_view = await resource_registry.launch_auxiliary_terminal(
            ...
            spec=TerminalEnvSpec(
                ...
                args=build_codex_remote_args(
                    ...
                    # Codify provider instead of falling back to the
                    ...
                    # Codify provisions the private CODEX_HOME and vets
                ...
                # Match the local ``codify codex`` terminal scrollback.
                scrollback=100_000,
...
async def _codex_discover_thread_and_forward(
    *,
    ...
    """
    Adopt the fresh Codex TUI's thread, then mirror it into the Codify session.
    ...
    :param session_id: Codify session/conversation id, e.g. ``"conv_abc123"``.
    ...
    from codify.codex_native_forwarder import (
        supervise_forwarder,
        wait_for_thread_started,
    )
    from codify.runner._entry import (
    ...
    try:
        ...
        # Mirror the discovered Codex thread id onto the Codify session as its
        ...
        # ``codify.fork.source_external_session_id`` from
        ...
        # would resume fresh. Best-effort: a transient Codify failure here still
...
async def _codex_forward_known_thread(
    *,
    ...
    :returns: None. Runs until cancelled or the app-server connection
        closes.
    """
    from codify.codex_native_forwarder import supervise_forwarder
    from codify.runner._entry import (
        _make_auth_token_factory,
        _RunnerDatabricksAuth,
    )
...
async def _run_antigravity_reader(
    *,
    ...
    :func:`codify.antigravity_native_reader.run_reader_with_bridge` (used by both
    this runner path and the CLI ``codify antigravity`` attach fallback); it
...
async def _auto_create_antigravity_terminal(
    ...
    *,
    ...
    :func:`codify.antigravity_native._launch_antigravity_terminal`, which
    ...
    *attended* (``headless=False``). Auto-bypass comes only from the user's
    ...
    :param ensure_comment_relay: The runner's relay starter
        (``_ensure_comment_relay_started``). When provided, the Codify MCP
    ...
    :raises RuntimeError: If the session snapshot or required runner env is
        unavailable.
    """
    from codify.antigravity_native_bridge import (
    ...
    from codify.antigravity_native_launch import build_agy_launch
    from codify.inner.datamodel import OSEnvSandboxSpec, OSEnvSpec, TerminalEnvSpec
    ...
    # Wire the Codify MCP relay so the wrapped agy gets the sys_* tools
    # (spawn sub-agent sessions, drive Codify terminals, list agents/models,
    ...
    terminal_view = await resource_registry.launch_required_terminal(
        ...
        spec=TerminalEnvSpec(
            ...
            args=list(argv[1:]),
            env=env_overrides,
            # Match the local ``codify antigravity`` terminal scrollback.
            scrollback=100_000,
            ...
            # ``codify antigravity`` path keeps start-on-attach: there a human
            # TTY is the driver.)
            tmux_start_on_attach=False,
        ),
    )
    ...
    # elicitations via the Task 9 hook. The reader owns its own Codify client
...
def _mint_runner_agy_conversation_id() -> str:
    ...
    :func:`codify.antigravity_native._mint_agy_conversation_id`.
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    :func:`codify.antigravity_native_rpc.resolve_cold_start_agy_rpc_port`:
    ...
    The cold-started id is also PATCHed onto the Codify session as
    ...
    *timeout_s*, or ``StartCascade`` erroring) must NOT abort the auto-create:
    ...
    :param server_client: Runner Codify server client used for the
    ...
    from codify.antigravity_native_rpc import (
        AntigravityRpcError,
...
def _terminal_tmux_pane(
    ...
    # instance (see codify.inner.terminal). Guard defensively so a registry
...
async def _session_payload_for_host_spawn_check(
    ...
    :param server_client: The runner's Codify server HTTP client, or
...
async def _codex_session_needs_runner_terminal(
    ...
    - **Sub-agent children** (dispatched server-side via
      ...
      e.g. polly run via ``codify run --server`` — silently never got
    ...
    :param server_client: The runner's Codify server HTTP client, or ``None`` in
...
def _pi_native_model_from_spec(agent_spec: AgentSpec | ResolvedSpec | None) -> str | None:
    ...
    :func:`codify.pi_native_credentials.resolve_pi_native_provider`). The
...
def _cursor_fork_history_preamble(items: list[dict[str, Any]]) -> str:
    ...
    :func:`codify.cursor_native_bridge.wrap_fork_preamble`.

    :param items: Committed Codify items (``GET /v1/sessions/{id}/items``),
        chronological.
...
def _build_claude_native_base_args(
    *,
    ...
    bridge / MCP / hook / Codify wiring. The order is: ``--resume`` for a
    ...
    :param resume_external_session_id: Claude-native session id to
        ...
        :func:`codify.claude_native._clone_claude_transcript`), so
    ...
    """
    from codify.reasoning_effort import CLAUDE_EFFORTS
...
def _publish_terminal_pending(
    ...
    fails. The Codify relay caches the latest value and republishes it, and
...
def _publish_native_terminal_start_error(
    ...
    outside a transcript turn; Codify writes and publishes the turn-scoped
...
def _codex_ensure_response_with_policy_notice(
    ...
    ``policy_hook_disabled_reason`` exactly once so Codify can post a single
...
def _ensure_orchestrator_skills_in_bundle(
    ...
    """
    Link the ``build-codify`` skill into a bundle's ``skills/`` dir.

    Called before native bridge launches so ``--plugin-dir`` (claude) or
    ``CODEX_HOME/skills/`` (codex) picks up the skill. Injects
    unconditionally for every agent — every ``codify claude`` /
    ``codify codex`` user should be able to author new agents. The
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        ...
        republishes it to the web UI) so the Terminal toggle enables
        without a refresh.
    :param server_client: Codify server client used to fetch the session
    ...
    from codify.claude_native_bridge import (
        BRIDGE_ID_LABEL_KEY,
        augment_claude_args,
        ensure_claude_workspace_trusted,
        prepare_bridge_dir,
    )
    from codify.claude_native_forwarder import reset_transcript_forward_state
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec

    workspace = (
        session_init.snapshot.workspace
        if session_init is not None and session_init.snapshot.workspace
        else os.environ.get("CODIFY_RUNNER_WORKSPACE", str(Path.cwd()))
    )
    ...
    # The Codify server URL + auth are needed in two places below: the
    ...
    from codify.claude_launcher import resolve_claude_launch
    from codify.claude_native import (
    ...
    if server_client is not None and session_external_id is not None:
        from codify.claude_native import _ensure_local_claude_resume_transcript
        ...
        from codify.claude_native import _clone_claude_transcript
        ...
        if _cloned is not None:
            ...
            # Record the assigned id now so Codify reflects the clone's own
        ...
        # native transcript from its OWN copied Codify items under a uuid we
        ...
        if _built is not None:
            resume_external_session_id = our_uuid
            # Record the assigned id so Codify reflects the clone's own Claude
    ...
    # (codify setup), a Databricks ucode profile from provider config, or
    ...
    # managed-host path. Identity by default. See codify.claude_launcher.
    ...
    from codify.entities.session_resources import session_resource_view_to_dict
    ...
    # back to the Codify server. Normally the CLI client runs this,
    ...
    # offset 0 already holds every item Codify has. Starting the forwarder at
    ...
    # ``claude_native.py``.
    from codify.claude_native_forwarder import supervise_forwarder
...
async def _auto_create_repl_terminal(
    ...
    *,
    ...
    """
    Auto-create an Codify REPL terminal for a runner-hosted SDK session.
    ...
    terminal hosts the framework's own TUI (``codify attach
    ...
    like the claude-/codex-native terminals — with the Codify REPL as
    the TUI.
    ...
    attach the session is fully live (``codify attach`` fails loud on a
    non-live session) with the REPL sized to the real attached terminal.

    Auth parity with the native terminals: the spawned ``codify
    attach`` resolves credentials for ``--server`` the same way a
    user-launched CLI does (``CODIFY_REMOTE_AUTH_TOKEN`` env → stored
    OIDC token from ``codify login`` → ``~/.databrickscfg``), which
    ...
    :param publish_event: The runner's per-session SSE emitter,
        ...
        without a refresh.
    :param server_client: Codify server client used to stamp the
        ``codify.ui: terminal`` presentation label that makes the web
        UI show the Chat/Terminal toggle.
    :returns: The launched terminal's :class:`SessionResourceView`.
    """
    from codify._wrapper_labels import UI_MODE_LABEL_KEY, UI_MODE_TERMINAL_VALUE
    from codify.inner.datamodel import OSEnvSpec, TerminalEnvSpec

    started_at = time.monotonic()
    workspace = os.environ.get("CODIFY_RUNNER_WORKSPACE", str(Path.cwd()))
    server_url = os.environ.get("RUNNER_SERVER_URL", "http://localhost:6767")
    ...
    env_spec = TerminalEnvSpec(
        ...
        # The runner's interpreter is the venv with codify installed;
        # ``python -m codify`` avoids depending on the console script
        # being on the tmux pane's PATH.
        command=sys.executable,
        args=["-m", "codify", "attach", session_id, "--server", server_url],
        scrollback=50000,
    ...
    terminal_view = await resource_registry.launch_auxiliary_terminal(
        ...
        resource_role=CODIFY_REPL_TERMINAL_ROLE,
    )
    ...
    # show a dead pill. The ``codify.wrapper`` label is deliberately
    ...
    _logger.info(
        "Auto-created codify REPL terminal for session %s: terminal_id=%s "
...
async def _delete_native_bridge_dirs(
    *,
    ...
    ``/tmp/codify-<uid>/<harness>-native/<digest>`` (some use ``~/.codify``)
    ...
    :param server_client: Codify server client used to resolve rotated bridge
    ...
    from codify.goose_native_bridge import (
...
async def _claude_native_bridge_id_for_session(
    *,
    ...
    :param server_client: Codify server client used to fetch the session
        snapshot.
    :param session_id: Codify session/conversation id, e.g.
        ``"conv_abc123"``.
    ...
    :returns: Opaque bridge id from
        ``codify.claude_native.bridge_id`` when present, otherwise
        *session_id* for legacy single-session bridges.
    """
    from codify.claude_native_bridge import BRIDGE_ID_LABEL_KEY
...
async def _resolve_native_spawn_env(
    ...
    *,
    ...
    :param session_id: Codify conversation id.
    :param server_client: Runner's client to the Codify server, for label reads.
...
async def _launch_native_terminal(
    ...
    *,
    ...
    :param resolve_agent_spec: Optional async callback that resolves the session
        ...
        resolver that tolerates ``CodifyError`` must swallow it and return
...
async def _claude_native_terminal_arrives_via_transfer(
    *,
    ...
    :param server_client: Codify client to resolve the bridge id label;
...
async def _antigravity_native_terminal_arrives_via_transfer(
    *,
    ...
    :func:`codify.antigravity_native_reader._rotate_session_for_cascade`) binds the
...
def _is_spec_local_native_python_tool(spec: Any, tool_name: str) -> bool:
    ...
    return any(
        ...
        and getattr(info, "language", None) in ("python", "codify-python-callable")
