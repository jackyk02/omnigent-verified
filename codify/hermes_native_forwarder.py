...
The ``codify hermes`` wrapper launches the real ``hermes`` TUI in a runner-owned
tmux pane, and :mod:`codify.hermes_native_bridge` injects web-UI messages into
it. That covers the web→TUI direction, but the *embedded terminal* is then the
only surface that reflects the agent's work — the Codify conversation view (chat
...
:mod:`codify.goose_native_forwarder`. Hermes stores all sessions in a single
...
PTY-activity watcher (see :mod:`codify.runner.app`) still emits the id-less
...
deduped against a persisted posted-count (:mod:`codify.hermes_native_status`) so
...
from codify import hermes_native_status
...
class _HermesUsageTracker:
    ...
    Follows the :class:`codify.codex_native_forwarder._SessionUsageCoalescer`
...
async def forward_hermes_store_to_session(
    *,
    ...
    # Codify server so we do it at most once per forwarder lifetime.
...
async def supervise_hermes_forwarder(
    *,
    ...
    Mirrors :func:`codify.goose_native_forwarder.supervise_goose_forwarder`:
