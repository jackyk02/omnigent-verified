...
# resolvable (``CODIFY_LAKEBASE_INSTANCE`` is set, or a provider was injected
...
# token-refresh mode.
_LAKEBASE_INSTANCE_ENV = "CODIFY_LAKEBASE_INSTANCE"
...
def set_lakebase_token_provider(provider: LakebaseTokenProvider | None) -> None:
    ...
    *provider* once per new DBAPI connection, and uses the shorter
    ...
    fall back to the ``CODIFY_LAKEBASE_INSTANCE`` env-var path.
...
def _databricks_lakebase_token_provider(instance_name: str) -> str:
    ...
    :param instance_name: The Lakebase database instance name, e.g.
        ``"codify-db"`` (the value of ``CODIFY_LAKEBASE_INSTANCE``).
...
def _create_engine(db_uri: str) -> Engine:
    ...
    multi-process workloads — REPL + Codify server + runner subprocess
    ...
    # ``None`` unless ``CODIFY_LAKEBASE_INSTANCE`` is set or a provider was
    ...
    if db_uri not in _engine_cache:
        with _engine_lock:
            if db_uri not in _engine_cache:
                ...
                from codify.runtime.telemetry import instrument_sqlalchemy_engine
...
def get_or_create_conversation_engine(conv_uri: str) -> Engine:
    ...
    For the common case where AP DB == Codify DB, callers should
...
def _ensure_conversation_tables(engine: Engine) -> None:
    ...
    from codify.db.db_models import ConversationBase
...
def _build_alembic_config(db_uri: str) -> Config:
    ...
    :func:`codify debug db-upgrade` CLI command share the same
...
def _run_migrations(engine: Engine, db_uri: str) -> None:
    ...
    from codify.db.db_models import CodifyBase, ConversationBase
    ...
    for base in (CodifyBase, ConversationBase):
...
def _initialize_or_verify_schema(engine: Engine, db_uri: str) -> None:
    ...
    if current != head:
        _logger.warning(
            "Codify database schema is out of date "
        ...
        if migrated != head:
            raise RuntimeError(
                f"Codify automatic database migration did not reach head "
...
def extract_search_text(item: NewConversationItem) -> str:
    ...
    """
    from codify.entities.conversation import CompactionData
