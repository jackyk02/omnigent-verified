...
This mirrors :data:`codify.server.auth.LEVEL_READ` and friends, the
...
# row (``codify_conversation_metadata.live_status``) so any server replica
...
def encode_session_live_status(name: str) -> int:
    """Encode an ``codify_conversation_metadata.live_status`` name to its int code."""
    return _encode(SESSION_LIVE_STATUS, name, field="codify_conversation_metadata.live_status")


def decode_session_live_status(code: int) -> str:
    """Decode an ``codify_conversation_metadata.live_status`` int code to its name."""
    return _decode(SESSION_LIVE_STATUS, code, field="codify_conversation_metadata.live_status")
