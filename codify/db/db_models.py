"""SQLAlchemy table definitions for the codify database."""
...
from codify.db.compression import CompressedText
...
class CodifyBase(DeclarativeBase):
    """Declarative base for the Codify operational tables.
    ...
    autogenerate can target the Codify side independently of the
    conversation tables.
    """
...
_current_workspace_id: ContextVar[int] = ContextVar(
    "codify_workspace_id", default=DEFAULT_WORKSPACE_ID
)
...
class SqlAgent(CodifyBase):
    ...
    # Enum stored as a stable int code (see codify.db.enum_codecs
...
class SqlAccountToken(CodifyBase):
    """
...
class SqlDeviceGrant(CodifyBase):
    """
    ...
    :param status: ``pending`` / ``approved`` / ``denied`` / ``redeemed`` /
        ``revoked`` (see :data:`codify.db.enum_codecs.DEVICE_GRANT_STATUS`).
    ...
    :param user_id: The Codify identity that approved the grant, set at
...
class SqlSessionPermission(CodifyBase):
    """
...
class SqlConversationMetadata(CodifyBase):
    """
    SQLAlchemy model for the ``codify_conversation_metadata`` table.

    Codify-side operational state for a conversation: runner/host
    ...
    __tablename__ = "codify_conversation_metadata"
...
class SqlProject(CodifyBase):
    ...
    membership lives on ``codify_conversation_metadata.project_id``, not
...
class SqlConversation(ConversationBase):
    ...
    (``agent_id`` + the ``session_overrides`` JSON blob). Codify
...
class SqlConversationLabel(ConversationBase):
    ...
    :param updated_at: Unix epoch seconds of the last write.
        ...
        unchanged (matches codify parity and keeps
...
class SqlPolicy(CodifyBase):
    ...
    # codify.db.enum_codecs POLICY_TYPE: python=1, url=2).
    type: Mapped[int] = mapped_column(SmallInteger)
    ...
    # code (see codify.db.enum_codecs POLICY_SCOPE: default=1, session=2).
...
class SqlUserDailyCost(CodifyBase):
    """
...
class SqlScheduledTask(CodifyBase):
    """
    ...
    :param execution_target: Where a firing runs —
        ...
        (see codify.db.enum_codecs SCHEDULED_TASK_EXECUTION_TARGET); the store
    ...
    :param state: Lifecycle state — ``active``/``paused``/``deleted``.
        ...
        Stored as a stable int code (see codify.db.enum_codecs
    ...
    # Where a firing runs, as a stable int code (see codify.db.enum_codecs
...
class SqlScheduledTaskRun(CodifyBase):
    """
    ...
    :param status: Lifecycle state —
        ...
        as a stable int code (see codify.db.enum_codecs
