"""add project_id to codify_conversation_metadata
...
``codify_conversation_metadata`` — the session→project membership pointer.
...
from codify.db.db_models import Uuid16
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_conversation_metadata_project_id",
        table_name="codify_conversation_metadata",
    )
    with op.batch_alter_table("codify_conversation_metadata") as batch_op:
