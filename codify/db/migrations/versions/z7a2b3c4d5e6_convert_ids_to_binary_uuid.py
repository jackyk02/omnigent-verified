...
``codify_conversation_metadata.runner_id`` and
``conversation_items.response_id`` (polymorphic harness task tokens, not our
uuids), ``codify_conversation_metadata.external_session_id`` (harness-native),
...
``codify_conversation_metadata.host_id`` copies) map to the SAME bytes, so
...
_BINARY_ID_COLUMNS: dict[str, list[str]] = {
    ...
    "codify_conversation_metadata": ["id", "host_id"],
