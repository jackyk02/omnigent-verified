...
``codify_conversation_metadata``. That forced ``list_conversations`` to
pre-fetch every non-archived conversation id from the Codify DB and filter
...
``codify_conversation_metadata`` via a portable correlated subquery, drops
...
def upgrade() -> None:
    ...
    then drop it from ``codify_conversation_metadata``.
    ...
    with op.batch_alter_table("codify_conversation_metadata") as batch_op:
...
def downgrade() -> None:
    ...
    op.execute(
        """
        UPDATE codify_conversation_metadata
        SET archived = COALESCE(
            (SELECT c.archived
             FROM conversations c
             WHERE c.workspace_id = codify_conversation_metadata.workspace_id
               AND c.id = codify_conversation_metadata.id),
            FALSE
        )
        """
    )
