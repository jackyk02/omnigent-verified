"""Split conversations table into conversations + codify_conversation_metadata.

Revision ID: aa1b2c3d4e5f
Revises: z5a2b3c4d5e6
Create Date: 2026-07-10 00:00:00.000000

Splits Codify operational metadata out of the ``conversations`` table into a
new ``codify_conversation_metadata`` table (1-to-1 paired by
...
def upgrade() -> None:
    # 1. Create the new codify_conversation_metadata table.
    op.create_table(
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        SET kind = COALESCE(
            (SELECT m.kind
             FROM codify_conversation_metadata m
    ...
    op.drop_index("ix_conversation_metadata_runner_id", table_name="codify_conversation_metadata")
    op.drop_index("ix_conversation_metadata_kind", table_name="codify_conversation_metadata")
    op.drop_table("codify_conversation_metadata")
