"""Sandbox CLI commands: run an Codify host in a remote sandbox.

``codify sandbox …`` bootstraps an Codify host inside a sandbox
from one of the registered providers (``--provider``) so that sessions
on it are reachable from the server-hosted UI, TUI, and ``codify
...
:mod:`codify.onboarding.sandboxes`.
"""
...
from codify.onboarding.sandboxes import (
    SandboxLauncher,
    available_providers,
    get_launcher,
)
...
def _codify_repo_root() -> Path:
    """
    Locate the codify checkout that hosts the three packages
    we build wheels for (``sdks/python-client``, ``sdks/ui``, ``.``).

    Strategy:

    1. Walk up from the current working directory looking for a parent
       that contains both ``sdks/python-client`` and ``codify``.
       This succeeds when the user runs ``codify sandbox …`` from
       inside a checkout.
    ...
    # ``cli_sandbox.py`` lives at ``<repo>/codify/cli_sandbox.py``;
    # parents[1] is the repo root that hosts ``sdks/`` and
    # ``codify/``. Editable installs (``pip install -e .``) point
    ...
    for candidate in candidates:
        if (candidate / "sdks" / "python-client").is_dir() and (candidate / "codify").is_dir():
            return candidate
    raise click.ClickException(
        "Could not locate the codify repo root from "
...
def _normalize_server_url(server_url: str) -> str:
    ...
    final in-sandbox ``codify login`` step.
...
def _print_ready_banner(provider: str, sandbox_id: str, server_url: str) -> None:
    ...
    click.echo(
        f"  codify sandbox connect --provider {provider} --sandbox-id {sandbox_id} "
        f"--server {server_url}\n"
    )
...
def sandbox() -> None:
    """
    Run an Codify host inside a remote sandbox.

    \b
    Subcommands:
      create   Provision a sandbox + bootstrap Codify into it.
      connect  Register the sandbox as a host with your server (runs
               `codify host` in the sandbox).

    \b
    Provider notes:
      modal    Sandboxes live at most 24 hours (platform cap). Needs
               `pip install 'codify[modal]'` + `modal token new`.
      daytona  No lifetime cap (idle auto-stop is disabled). Needs
               `pip install 'codify[daytona]'` + DAYTONA_API_KEY.
...
@click.option(
    ...
    help=(
        ...
        "inference as `codify login`), and the bootstrap finishes "
        "by logging the sandbox in to it (`codify login` inside the "
...
@click.option(
    ...
    default=None,
    help="Path to the codify checkout.",
)
@click.option(
    "--no-auth",
    "skip_auth",
...
def sandbox_create(
    ...
    """
    Provision a sandbox and ship Codify into it.

    The server's workspace is derived from ``--server`` (the same
    unauthenticated probe ``codify login`` uses), and for lakebox
    ...
    profile. Builds the Codify wheels from your local checkout,
    installs them into the fresh sandbox, and finishes by logging the
    sandbox in to the server (``codify login`` runs inside the
    ...
    After this finishes, run ``codify sandbox connect`` to register
...
@click.option(
    ...
    help=(
        ...
        "`codify login` infers the fronting Databricks workspace "
...
def sandbox_auth(
    ...
    """
    Run the server login inside the sandbox (``codify login``).
    ...
    ``codify sandbox create --sandbox-id`` because it skips wheel
    build / ship / pip install — it only re-authenticates.
    """
    from codify.onboarding.sandboxes import derive_workspace, login_app_oauth_in_sandbox
...
def sandbox_connect(
    ...
    Runs ``codify host --server <url>`` inside the sandbox — the
    host resolves its own credentials (a stored ``codify login``
    ...
    """
    from codify.onboarding.sandboxes import connect_sandbox_host, derive_workspace
