...
then delivers Codify web-UI messages into the *same* pane via
...
When Codify policies are configured the runner writes a per-session
``HERMES_HOME`` with a ``pre_tool_call`` hook (via :func:`write_policy_hook_config`)
that evaluates tool calls against the Codify policy engine — the same hook used
by the headless ``hermes`` harness (:mod:`codify.inner.hermes_executor`). The
...
(:mod:`codify.hermes_native_permissions`), which reads the pane via
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "hermes-native"
_TMUX_FILE = "tmux.json"
_TMUX_READY_TIMEOUT_S = 30.0
_TMUX_SEND_TIMEOUT_S = 10.0
_POLL_INTERVAL_S = 0.2
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "codify-hermes-paste"
...
# starts the Codify MCP server (a heavyweight ``python -m`` subprocess). A
...
# :mod:`codify.runtime.pending_inputs` — the i-th persisted user row drains the
...
def clone_hermes_session(
    ...
    *,
    ...
    try:
        ...
        # messages (Codify already has them from the fork item copy).
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/codify-<uid>/hermes-native/<hash>``."""
...
def build_hermes_native_spawn_env(session_id: str) -> dict[str, str]:
    ...
    :class:`~codify.inner.hermes_native_executor.HermesNativeExecutor` can find
    ...
    :param session_id: The Codify session id (keys the bridge dir).
...
def write_policy_hook_config(
    ...
    """Write per-session ``HERMES_HOME`` with Codify policy hook and MCP server.

    Creates a ``config.yaml`` registering:

    1. A ``pre_tool_call`` shell hook that evaluates tool calls against the
       Codify policy engine (same hook the headless ``hermes`` harness uses).
    2. An ``mcp_servers.codify`` entry that launches the Codify MCP stdio
       server (``serve-mcp --bridge-dir <bridge_dir>``), exposing Codify
    ...
    from codify.native_policy_hook import policy_hook_wrapper_script

    wrapper = hermes_home / "codify-policy-hook.sh"
    ...
    # Register the Codify MCP stdio server so Hermes can call
    # Codify builtin tools (sys_session_*, sys_agent_*, load_skill, etc.).
    config["mcp_servers"] = {
...
def inject_relay_into_policy_hook(
    ...
    hook_script_path = str(Path(__file__).resolve().parent / "inner" / "hermes_policy_hook.py")
    from codify.native_policy_hook import _RELAY_TOKEN_ENV, _RELAY_URL_ENV
    ...
    fd, tmp_name = tempfile.mkstemp(prefix="codify-policy-hook.", dir=str(hermes_home))
    try:
...
def _write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :func:`codify.codex_native_bridge.write_mcp_bridge_config`.
    """
...
def capture_hermes_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`codify.hermes_native_permissions`) to detect Hermes' in-terminal
