...
(``run`` / ``claude`` / ``codex``) asks the Codify server to launch
...
async def wait_for_runner_online(
    ...
    *,
    ...
    if last_error is not None:
        message += f" Last connection error: {last_error!r}."
    message += " Check the runner logs under ~/.codify/logs/runner/."
    raise click.ClickException(message)
...
async def launch_or_reuse_daemon_runner(
    ...
    *,
    ...
    # registry while it (re)connects — e.g. just after `codify host`
