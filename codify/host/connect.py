"""Host process main loop for ``codify host``.
...
from codify._platform import WINDOWS_ENV_PASSTHROUGH
from codify.env_credentials import env_names_with_codify_prefix
from codify.harness_aliases import canonicalize_harness
from codify.harness_availability import HARNESS_BINARY_MISSING, HarnessAvailability
...
from codify.host.identity import HostIdentity, load_or_create_host_identity
from codify.onboarding.harness_auth import (
...
from codify.onboarding.harness_readiness import (
    configured_harness_map,
    harness_is_configured,
)
from codify.onboarding.provider_config import ANTHROPIC_FAMILY, OPENAI_FAMILY
...
from codify.runner.transports.ws_tunnel.frames import (
    PingFrame,
    PongFrame,
    decode_frame,
    encode_frame,
...
def _display_log_path(path: Path) -> str:
    ...
    :param path: Absolute path, typically under the user's state dir, e.g.
        ``Path("/Users/alice/.codify/logs/runner/runner-ab12.log")``.
...
def _read_log_tail(path: Path, max_bytes: int = _LOG_TAIL_MAX_BYTES) -> str:
    ...
    :param path: The runner's captured stdout/stderr log file, e.g.
        ``Path("~/.codify/logs/runner/runner-ab12.log")``.
...
_RUNNER_ENV_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        #   - CODIFY_CONFIG_HOME: where config.yaml / provider config live,
        #     so the runner resolves the same providers the CLI configured.
        #   - CODIFY_DATA_DIR: where the sqlite db + pidfile live, so the
        ...
        # CODIFY_DATABASE_URI is intentionally NOT here — it may embed a
        ...
        # CODIFY_AUTH_PROVIDER and the daemon-spawned local server
        # silently boots in accounts mode while the CLI thinks it's talking
        # to a header-mode server — every CLI request 401s (e.g. the
        # test_run_codify_resumption suite). Not a secret; safe to propagate to
        # any subprocess.
        "CODIFY_AUTH_PROVIDER",
        # Multi-user opt-in switch (create_auth_provider): CODIFY_AUTH_ENABLED
        # turns the env-unset header/local default into accounts (or oidc, when
        # CODIFY_OIDC_* is set); =0 opts back out. Must propagate down the
        # CLI → daemon → local-server chain or `codify run`/`connect` would
        ...
        # ``CODIFY_CLAUDE_SDK_NO_SANDBOX=1 codify run …`` had no
        ...
        "CODIFY_CLAUDE_SDK_NO_SANDBOX",
        # Native-Claude launcher plugin selector: the entry-point NAME of a
        # launcher registered in the ``codify.claude_launcher`` group (e.g.
        # ``isaac``). Read by codify.claude_launcher.resolve_claude_launch in
        ...
        # (and the harness it spawns) never see CODIFY_TELEMETRY_ENABLED, so
        ...
        # telemetry is opt-in. Not a secret (a boolean). The CODIFY_OTEL_*
        # knobs (capture-content, FastAPI toggle) ride the prefix allowlist below.
        "CODIFY_TELEMETRY_ENABLED",
        ...
        # CODIFY_RUNNER_ENV_PASSTHROUGH.
        "CODIFY_DATABRICKS_EXTRA_HEADERS",
    }
...
# both the standard ``OTEL_*`` vars and Codify's ``CODIFY_OTEL_*`` knobs
# (capture-content, FastAPI toggle) so they reach the runner/harness too.
_RUNNER_ENV_ALLOWLIST_PREFIXES: tuple[str, ...] = ("LC_", "MLFLOW_", "OTEL_", "CODIFY_OTEL_")
...
HARNESS_CREDENTIAL_ENV_VARS: frozenset[str] = frozenset(
    ...
    for name in env_names_with_codify_prefix(canonical)
)
...
RUNNER_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_RUNNER_ENV_PASSTHROUGH"
...
class HostConnectError(Exception):
    ...
    instead of backing off, so ``codify host`` exits with an
...
def _build_runner_env(
    ...
    *,
    ...
    :param base_env: Host process environment to filter, e.g.
        ``os.environ``.
    :param server_url: Codify server URL the runner connects back to, e.g.
...
class HostProcess:
    ...
    :param server_url: Codify server URL, e.g.
        ``"https://codify-app.databricksapps.com"``.
    """
    ...
    def __init__(
        ...
        # commands in :mod:`codify.host.git_worktree`) currently in flight.
    ...
    async def _orphan_reaper_loop(self) -> None:
        ...
        (``start_new_session=True`` — ``codify.inner._proc.spawn_kwargs``),
    ...
    def _credentials_fix_hint(self) -> str:
        ...
        :returns: An actionable remedy sentence naming the exact
            command, e.g. ``"Run `codify login <url>` ..."``.
        """
        return (
            f"Run `codify login {self._server_url}` to authenticate (it "
    ...
    def _login_fix_hint(self) -> str:
        """Suggest ``codify login`` as a remedy for an auth rejection.

        The host tunnel's bearer is resolved from a stored ``codify
        login`` record first, then ambient Databricks credentials (see
        :func:`codify.runner._entry._make_auth_token_factory`). When
        the server runs Codify accounts or OIDC auth, a Databricks
        ...
        :returns: A one-sentence remedy naming the exact command, e.g.
            ``"If this server uses Codify accounts or OIDC login, run
            `codify login http://localhost:6767` to authenticate."``.
        """
        return (
            "If this server uses Codify accounts or OIDC login, run "
            f"`codify login {self._server_url}` to authenticate."
        )
    ...
    def _fatal_upgrade_error(self, exc: InvalidURI | InvalidStatus) -> HostConnectError | None:
        ...
        if isinstance(exc, InvalidURI):
            ...
            if self._login_redirect_streak == 1:
                ...
                # `codify host` shows the auth problem and its fix instead
    ...
    async def _handle_launch(
        ...
        try:
            ...
            # session with `codify debug logs --session <id>`. Cap at 32
    ...
    def _handle_install_harness(
        ...
        ``codify setup``) uses, then recomputes readiness so the result frame
    ...
    def _handle_store_secret(self, frame: HostStoreSecretFrame) -> HostStoreSecretResultFrame:
        ...
        :func:`adopt_env_credential`) the ``codify setup`` wizard's
        "add a key / gateway" path uses — the secret goes to the OS keychain
        (else ``~/.codify/secrets.json``) and ``config.yaml`` gets a
    ...
    def _handle_fs_request(self, frame: HostFsRequestFrame) -> HostFsResultFrame:
        ...
        Runs :class:`codify.workspace_fs.WorkspaceReader` against the
        ...
        from codify.workspace_fs import WorkspaceReader, WorkspaceReaderError
    ...
    async def _handle_model_options(
        ...
        if harness == "codex-native":
            try:
                ...
                from codify.model_catalog import (
    ...
    def _dispatch_fs_op(
        ...
        from codify.workspace_fs import WorkspaceReader
    ...
    def _build_connect_headers(self) -> dict[str, str]:
        ...
        """
        from codify.host.identity import HOST_TOKEN_ENV_VAR, MANAGED_HOST_TOKEN_HEADER
        ...
        headers: dict[str, str] = {"Origin": CODIFY_INTERNAL_WS_ORIGIN}
    ...
    def _current_auth_token(self, *, initialize: bool = True) -> str | None:
        ...
        """
        from codify.host.identity import HOST_TOKEN_ENV_VAR
    ...
    async def _serve_frames(self, ws: websockets.asyncio.client.ClientConnection) -> None:
        ...
        _tel_opt_out = False
        try:
            from codify.telemetry.client import is_disabled as _tel_disabled
        ...
        try:
            from codify.telemetry.installation_id import (
                get_installation_id as _get_install_id,
            )
...
def run_host_process(
    ...
    # Tell the user where logs land up front — `codify host` used to run
