"""Host tunnel frame schema for ``codify host``.
...
from codify.harness_availability import HarnessAvailability, is_harness_availability
...
class HostHelloFrame:
    ...
    :param configured_harnesses: Per-harness readiness on this
        ...
        (see ``codify.onboarding.harness_readiness``). Keys
...
class HostInstallHarnessFrame:
    ...
    ``codify setup`` wizard uses. Only allowlisted, npm-installable
...
class HostStoreSecretFrame:
    ...
    ``codify setup`` wizard's "add a key / gateway" path uses: the secret goes
    to the OS keychain (else ``~/.codify/secrets.json``), and ``config.yaml``
...
class HostFsRequestFrame:
    ...
    runs :class:`codify.workspace_fs.WorkspaceReader` against
