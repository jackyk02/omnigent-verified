"""Host identity management for ``codify host``.

Reads or creates the ``host`` section in ``~/.codify/config.yaml``.
The host identity is auto-generated on first ``codify host``
...
HOST_TOKEN_ENV_VAR = "CODIFY_HOST_TOKEN"
HOST_ID_ENV_VAR = "CODIFY_HOST_ID"
HOST_NAME_ENV_VAR = "CODIFY_HOST_NAME"

# WebSocket upgrade header carrying a managed host's launch token.
# Mirrors the runner tunnel's X-Codify-Runner-Tunnel-Token pattern:
...
# auth provider.
MANAGED_HOST_TOKEN_HEADER = "X-Codify-Host-Token"
...
def load_or_create_host_identity(
    ...
    :param path: Path to the config YAML file, e.g.
        ``Path("~/.codify/config.yaml")``. Defaults to
        :data:`CONFIG_PATH`.
