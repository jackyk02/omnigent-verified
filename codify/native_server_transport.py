...
(:class:`codify.opencode_http_transport.OpenCodeHttpTransport`);
:class:`NativeServerTransport` is the seam so the orchestration in
:class:`codify.native_server_harness.NativeServerHarness` stays
protocol-agnostic.
"""
...
class NativeLaunchConfig:
    ...
    :param codify_session_id: Codify conversation id, e.g.
        ``"conv_abc123"``.
...
class NativeServerTransport(Protocol):
    ...
    :class:`~codify.native_server_harness.NativeServerHarness` calls only
    these methods.
    """
