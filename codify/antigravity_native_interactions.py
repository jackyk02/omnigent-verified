...
Codify elicitation, waits for the human's verdict, and delivers it back to agy
...
WAITING step at a HIGHER ``stepIndex``**. Codify elicitations wait on a human
...
* ``deliver`` — defaults to :func:`_deliver_via_rpc`, which offloads the blocking
  :func:`codify.antigravity_native_rpc.handle_user_interaction` to a worker
...
:func:`codify.codex_native_elicitation.codex_elicitation_id`), so a
...
from codify.antigravity_native_steps import PendingInteraction, pending_interaction
from codify.server.routes._antigravity_elicitation import (
...
class Deliver(Protocol):
    ...
    Matches :func:`codify.antigravity_native_rpc.handle_user_interaction`
...
def tui_injector_for(bridge_dir: Path) -> InjectTui:
    ...
    async def _inject(keys: list[str]) -> None:
        ...
        from codify.antigravity_native_bridge import send_interaction_keys_via_tui
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    :func:`codify.antigravity_native_bridge.send_interaction_keys_via_tui`) to a
    ...
    from codify.antigravity_native_bridge import send_interaction_keys_via_tui
    from codify.inner.antigravity_native_executor import _bridge_dir_from_env
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    """
    Build the Codify elicitation id for one agy WAITING interaction.
    ...
    :func:`codify.codex_native_elicitation.codex_elicitation_id`.
...
async def _deliver_via_rpc(
    ...
    *,
    ...
    :func:`codify.antigravity_native_rpc.handle_user_interaction` is synchronous
...
async def bridge_interaction(
    ...
    *,
    ...
    :param deliver: Delivers the built payload to agy; defaults to
        ...
        :func:`codify.antigravity_native_rpc.handle_user_interaction` to a
        worker thread.
