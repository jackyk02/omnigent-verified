"""SSE consumer that mirrors OpenCode events into an Codify session.
...
OpenCode events into Codify session-stream events posted to
...
Design references: the SSE-event → Codify-event translation table in
...
from codify.opencode_native_bridge import update_active_message_id, update_last_event_id
from codify.opencode_native_client import OpenCodeClient, OpenCodeEvent
from codify.opencode_native_permissions import (
...
# Codify session-event types (must match the server's ingestion route;
...
# back to Codify so the web model pill stays in sync (claude-native contract).
...
# ``codify/onboarding/opencode_auth.py``).
...
class OpenCodeNativeForwarder:
    """
    Translate one OpenCode session's SSE stream into Codify events.
    ...
    :param server_client: HTTP client for the Codify server (event posts).
    ...
    :param policy_evaluator: Optional async policy resolver. Production
        ...
        ``codify.runner.app._build_opencode_policy_evaluator``) — the SAME
    ...
    def __init__(
        ...
        *,
        ...
        # Last model mirrored to Codify (provider/id), to dedupe switches.
    ...
    async def handle_event(self, event: OpenCodeEvent) -> None:
        """
        Translate one OpenCode event into Codify session events.
    ...
    async def _post_event(self, event_type: str, data: _JsonObject) -> httpx.Response | None:
        """
        POST one Codify session event with a single retry.

        :param event_type: Codify event type, e.g.
    ...
    async def _on_part_updated(self, event: OpenCodeEvent) -> None:
        ...
        if part_type == "text":
            ...
            # transcript (codify persists no separate user item for these
    ...
    async def _on_compaction_started(self, event: OpenCodeEvent) -> None:
        ...
        server-side. The Codify server maps ``external_compaction_status``
    ...
    async def _on_model_switched(self, event: OpenCodeEvent) -> None:
        ...
        When the user switches model in the opencode TUI, reflect it to Codify
    ...
    async def _on_permission_asked(self, event: OpenCodeEvent) -> None:
        ...
        try:
            await self._opencode.reply_permission(
                request.request_id, reply_body(reply, message="codify-policy")
            )
    ...
    async def _resolve_permission(
        ...
        normalized = normalize_for_policy(
            request_dict,
            codify_session_id=self._session_id,
...
_HANDLERS: dict[str, Callable[[OpenCodeNativeForwarder, OpenCodeEvent], Awaitable[None]]] = {
    ...
    # Mirror a TUI model switch back to Codify (in-harness session-cmd sync).
