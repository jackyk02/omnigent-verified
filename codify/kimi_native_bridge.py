...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "kimi-native"
_TMUX_FILE = "tmux.json"
# Codify routing details the kimi hook subprocess reads to reach the server.
# Mirrors claude-native's ``permission_hook.json`` (server URL + auth headers +
# the active Codify session). Written by the runner at terminal-create time;
# read by :mod:`codify.kimi_native_hook` (PreToolUse deny-gate + the
...
_PASTE_BUFFER = "codify-kimi-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/codify-<uid>/kimi-native/<hash>``."""
...
def write_hook_config(
    ...
    *,
    ...
    """Record the Codify routing details the kimi hook subprocess reads.
    ...
    :func:`codify.claude_native_bridge` ``permission_hook.json`` plumbing.

    :param bridge_dir: The kimi-native bridge dir.
    :param server_url: Codify server base URL, e.g. ``"http://127.0.0.1:8787"``.
    :param headers: Auth headers to replay on the hook's POSTs (may be empty).
    :param session_id: The Codify session the hook events belong to.
    """
    _ensure_dir(bridge_dir)
    payload = {
...
def read_hook_config(bridge_dir: Path) -> dict[str, Any]:
    """Read Codify routing details for the kimi hook subprocess.
...
def read_active_session_id(bridge_dir: Path) -> str | None:
    """Return the Codify session id recorded for the hook subprocess.
...
#: "Approve once" re-prompts each call so Codify governs every one.
APPROVE_KEY = "1"
DENY_KEY = "3"
