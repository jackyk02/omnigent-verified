"""Implementation of the ``codify chat`` command.

The CLI always ends by connecting an Codify client to a server URL. For
...
from codify_client import (
    CodifyError as ClientCodifyError,
)
from codify_client._events import (
    ErrorEvent,
    ResponseCancelled,
    ResponseCompleted,
    ResponseFailed,
...
from codify._wrapper_labels import (
    CLAUDE_NATIVE_WRAPPER_VALUE as _CLAUDE_NATIVE_WRAPPER_LABEL_VALUE,
)
from codify._wrapper_labels import (
    WRAPPER_LABEL_KEY as _CLAUDE_NATIVE_WRAPPER_LABEL_KEY,
)
from codify.conversation_browser import open_conversation_link_if_enabled
from codify.errors import CodifyError
from codify.harness_aliases import canonicalize_harness
from codify.inner import _proc
from codify.inner.databricks_executor import _DatabricksBearerAuth, _read_databrickscfg
from codify.model_catalog import resolve_catalog_model
from codify.model_resolver import ModelResolutionError
from codify.native_coding_agents import native_coding_agent_for_wrapper_label
from codify.native_dispatch import resolve_hook_for_key
...
from codify.spec import load as load_spec
from codify.spec._codify_compat import CODIFY_EXECUTOR_TYPE
from codify.spec.parser import discover_host_skills
from codify.spec.types import AgentSpec, SkillSpec

if TYPE_CHECKING:
    from codify._runner_startup import RunnerStartupProgress
...
# freshly-launched ``codify run`` sessions don't burn a
...
# Optional bearer token for remote codify servers that sit
...
_REMOTE_AUTH_TOKEN_ENV = "CODIFY_REMOTE_AUTH_TOKEN"

# Env-var override name. ``CODIFY_MODEL=foo`` lets a user
...
# ``codify/inner/cli.py:344``.
_CODIFY_MODEL_ENV_VAR = "CODIFY_MODEL"
...
def _default_cli_model() -> str:
    ...
    Reads ``CODIFY_MODEL`` first, then resolves the Databricks OpenAI-family
    ...
    """
    configured = os.environ.get(_CODIFY_MODEL_ENV_VAR)
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "set CODIFY_MODEL, or retry when Databricks catalog discovery is available."
        ) from exc
...
class ChatOverrides:
    """
    CLI overrides from ``codify run`` flags.
    ...
    :param harness: ``--harness`` value, e.g. ``"claude-sdk"``.
        ...
        ``executor.harness`` key for single-file codify YAMLs and to
...
class LocalServer:
    """
    Handle to a locally-launched codify server and its sibling runner.
    ...
    :param log_path: Path to the file that captures the
        ...
        e.g. ``Path("~/.codify/logs/server/server-abc123.log")``.
...
def run_chat(
    ...
    *,
    ...
    """
    Main entry point for ``codify run`` (and the ``attach`` client).
    ...
    :param ephemeral: When ``True``, place the local server's
        ...
        the persistent ``~/.codify`` location. Maps to
...
def run_prompt(
    ...
    *,
    ...
    ``codify run ... -p``. It deliberately bypasses the
...
def run_attach(
    *,
    ...
    :param base_url: Codify server hosting the session, e.g.
        ``"http://127.0.0.1:6767"``.
    ...
    if not info.runner_online:
        raise click.ClickException(
            ...
            "(`codify run` locally, or reconnect it with `codify host`), "
            "then attach again."
        )
...
def _remote_headers(
    ...
    Resolution order:
      1. explicit ``CODIFY_REMOTE_AUTH_TOKEN`` env var
      2. stored OIDC token from ``~/.codify/auth_tokens.json``
         (populated by ``codify login``)
      3. stored Databricks Apps pointer record for ``server_url``
         (populated by ``codify login <apps-url>``) — mints a
    ...
    This lets ``codify run --server <apps-url>`` work against
    Databricks Apps after a one-time ``codify login <apps-url>``,
    ...
    :returns: Headers to pass to httpx / CodifyClient.
    """
    ...
    if token and (token := token.strip()):
        ...
        from codify.cli_auth import load_token

        # 2. Stored OIDC session token from `codify login`.
...
def _stored_databricks_record_token(server_url: str) -> str | None:
    ...
    ``codify login <apps-url>`` stores a pointer record naming the
    ...
    """
    from codify.cli_auth import load_databricks_workspace_host
...
class _DatabricksTokenAuth(httpx.Auth):
    ...
    Resolution order:
      1. static env-var token (``CODIFY_REMOTE_AUTH_TOKEN``)
      2. stored OIDC token (from ``codify login``)
    ...
    def _sdk_token(self) -> str | None:
        ...
        pointer record for the server (from ``codify login
    ...
    def auth_flow(self, request: httpx.Request) -> Generator[httpx.Request, httpx.Response, None]:
        ...
        if self._static_token:
            ...
            yield request
            return
        # Check stored OIDC token from `codify login`.
        if self._server_url:
            from codify.cli_auth import load_token
...
def _server_headers(
    *,
    ...
    """
    Build non-auth HTTP headers for an Codify server client.
    ...
    :param runner_id: Optional runner UUID, e.g.
        ...
        request header.
    :returns: Static headers for ``httpx`` / ``CodifyClient``.
    """
    del runner_id
    return {}
...
def _server_auth(
    ...
    # Check stored `codify login` records: a session JWT or a
    # Databricks Apps pointer record.
    if server_url:
        from codify.cli_auth import load_databricks_workspace_host, load_token
...
def _chat_with_server(
    ...
    *agent_name* is supplied by an upstream setup step such as
    ...
    :param server_log_path: Path to the local server's
        ...
        ``Path("~/.codify/logs/server/server-abc123.log")``. Shown in the
        Ctrl+O debug overview. ``None`` for remote servers.
    :param runner_log_path: Path to the local runner's
        stdout/stderr log file, e.g.
        ``Path("~/.codify/logs/runner/runner-abc123.log")``. Shown in the
    ...
    # ``codify.wrapper`` label on the conversation and re-dispatch
...
def _is_claude_native_conversation(
    *,
    ...
    :param base_url: Codify server base URL.
    :param conversation_id: Codify conversation id.
...
def _redirect_native_resume_if_needed(
    *,
    ...
    """
    Redirect a terminal-native resume before Codify attach liveness runs.

    :param base_url: Codify server base URL, e.g. ``"https://example.com"``.
    :param conversation_id: Codify conversation id, e.g. ``"conv_abc123"``.
    ...
    if run_native is None:
        return False
    # The native TUI owns the turns; resuming through the Codify REPL would run
    # an Codify turn per message *and* let the transcript forwarder mirror the
    # same message from the native store, double-posting each user turn. Redirect
    # to `codify <key> --resume`'s direct tmux attach instead — the wrapper
...
def _finish_native_redirect_progress(
    *,
    ...
    """
    Finish any Codify startup progress and print the native redirect notice.

    :param progress: Optional startup spinner to finish before writing.
    :param conversation_id: Codify conversation id, e.g.
        ``"conv_abc123"``.
    ...
    click.echo(
        (
            ...
            f"session — redirecting to `codify {native_command} --resume`.\n"
        ),
        err=True,
    )
...
def _wrapper_label_for_conversation(
    *,
    ...
    the response's ``labels.codify.wrapper`` field. ``None`` on any
    transport / parse error so a flaky server doesn't silently misroute
    the resume — the caller falls back to the normal Codify REPL path and
...
def _await_accounts_first_run_setup(
    ...
    *,
    ...
    When ``codify run`` (re)spawns the local Codify server in accounts mode on
    ...
    the browser auto-opened (e.g. ``CODIFY_ACCOUNTS_AUTO_OPEN=0``) — and
    ...
    :param base_url: Resolved local Codify server URL, e.g.
        ``"http://127.0.0.1:6767"``.
...
async def _prepare_chat_session_via_daemon(
    *,
    ...
    :param base_url: Codify server base URL, e.g. ``"http://127.0.0.1:8123"``.
    ...
    from codify.host.daemon_launch import (
        launch_or_reuse_daemon_runner,
        wait_for_host_online,
        wait_for_runner_online,
    )
    from codify.native_terminal import bind_session_runner

    async with CodifyClient(base_url=base_url, headers=headers, auth=auth) as sdk:
    ...
    async with httpx.AsyncClient(
        ...
        # clear the ``codify.stopped`` marker on resumed sessions. Must run
...
def _chat_via_daemon(
    ...
    *,
    ...
    :param base_url: Resolved Codify server base URL (the daemon is already
    ...
    :returns: None.
    """
    from codify.host.identity import load_or_create_host_identity
...
def _wait_for_remote_runner(
    ...
    *,
    ...
    :param log_path: Optional path to the captured runner log
        ...
        e.g. ``Path("~/.codify/logs/runner/runner-abcd.log")``.
    ...
    :param show_progress: When ``True`` (default), render a rich
        ...
        non-TTY stderr; see :mod:`codify._runner_startup`.
    :returns: None.
        ...
        try:
            ...
            if resp.status_code in {401, 403}:
                raise click.ClickException(
                    ...
                    "run `codify login <server-url>` or check remote auth credentials."
                    f"{format_runner_log_tail(log_path)}"
                )
        except httpx.HTTPError as exc:
            last_error = exc
...
def _bundle_agent(agent_path: Path) -> bytes:
    ...
    :raises CodifyError: If bundling fails, for example due to
        unresolved environment variables.
    """
    from codify.cli import _bundle
...
def _chat_local(
    ...
    *,
    ...
    :param ephemeral: When ``True``, point the local server at a
        ...
        (default) uses the persistent ``~/.codify``
        location so prior conversations remain reachable —
        see designs/RUN_CODIFY_SESSION_RESUMPTION.md.
    ...
    try:
        ...
        try:
            agent_spec = load_spec(spec_path)
        except (CodifyError, FileNotFoundError) as exc:
...
def _run_headless_prompt(
    ...
    *,
    ...
    try:
        asyncio.run(_main())
    except ClientCodifyError as exc:
...
async def _query_sessions_once(
    *,
    ...
    """
    from codify_client import SessionsChat

    if runner_id is None:
        raise RuntimeError(
            "Sessions API headless prompt requires a registered runner id. "
            "Start through `codify run <agent>` or pass --server so the CLI "
    ...
    # raises ``CodifyError`` on that ``failed`` status, and the
    ...
    # ClientCodifyError`` and print the message to stderr + exit non-zero.
    turn_error = await _persisted_turn_error(client, bound.id)
    if turn_error is not None:
        raise ClientCodifyError(turn_error)
    return None
...
def _response_output_text(output: _ResponseOutput) -> str | None:
    """Extract assistant text from an Codify response ``output`` list."""
    parts: list[str] = []
    for item in output:
...
async def _persisted_turn_text(
    ...
    * A transport-level runner disconnect publishes
      ...
      :meth:`SessionsChat.send` raises ``CodifyError`` on it.
...
def _resolve_resume_target(
    *,
    ...
    ``codify/inner/cli.py:3082-3084`` ("No saved sessions
    to continue.").
...
def _assert_resume_conversation_exists(
    *,
    ...
    async def _lookup() -> None:
        async with CodifyClient(base_url=base_url, headers=headers) as client:
...
def _run_picker(
    *,
    ...
    :raises click.ClickException: When the agent has no prior
        ...
        out of.
    """
    from codify.repl._resume_picker import pick_conversation_from_sdk

    async def _lookup() -> str | None:
        async with CodifyClient(base_url=base_url, headers=headers) as client:
            # Multipart ``codify run <yaml>`` uploads now create a
...
async def _resolve_latest_conversation_id_async(
    *,
    ...
    ``CodifyClient`` connected to a real URL — the path
    used in production by ``_chat_local``.

    :param client: A connected :class:`CodifyClient`.
...
def _materialize_override_bundle(source: Path, overrides: ChatOverrides) -> Path:
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="codify-override-"))
    try:
        if source.is_file():
...
def _cleanup_materialized_override_bundle(materialized: Path) -> None:
    ...
    :param materialized: Path returned by
        ...
        ``Path("/tmp/codify-override-abc/agent.yaml")``.
    :returns: None.
    """
...
def _load_yaml_if_single_file(source: Path) -> _YamlMapping | None:
    ...
    Directories (codify-style with ``config.yaml``) are handled
...
def _effective_openai_auth_harness(
    ...
    if model is None:
        return None
    from codify.llms.routing import infer_harness_from_model
...
def _should_inject_openai_env_auth_for_executor(
    ...
    # CODIFY_CONFIG_HOME, so skipping injection loses nothing. The env
    # bake remains only for users whose SOLE credential is the env key.
    from codify.onboarding.provider_config import (
        default_provider_for_harness,
        load_config,
    )
    from codify.runtime.workflow import _load_global_auth
...
def _apply_overrides_to_raw(raw: _YamlMapping, overrides: ChatOverrides) -> None:
    ...
    if overrides.harness is not None:
        ...
        # harness resolves its provider default — e.g. ``codify run
        ...
        if overrides.model is None:
            ...
            env_model = os.environ.get(_CODIFY_MODEL_ENV_VAR)
...
def _apply_harness_override_to_executor(
    ...
    Single-file codify YAMLs (``name`` + ``prompt``, no
    ...
    silent no-op, which made ``codify run examples/polly
    ...
    :raises click.ClickException: If a ``spec_version`` bundle
        declares a non-codify ``executor.type`` — those executors
        have no ``config.harness``, so the override cannot apply.
    """
    canonical = canonicalize_harness(harness) or harness
    # "spec_version" is the format discriminator (see is_codify_yaml).
    if "spec_version" not in raw:
        executor_block["harness"] = canonical
        return
    etype = str(executor_block.get("type", CODIFY_EXECUTOR_TYPE))
    if etype != CODIFY_EXECUTOR_TYPE:
        raise click.ClickException(
            f"--harness only applies to specs with executor.type "
            f"{CODIFY_EXECUTOR_TYPE!r}; this spec declares executor.type {etype!r}."
        )
    config = executor_block.get("config")
...
def _validate_agent_spec(agent_path: Path) -> None:
    ...
    ``_start_local_server``). Both ``CodifyError`` (parse/
...
def _extract_agent_name(agent_path: Path) -> str:
    ...
    Accepts both agent-image directories and standalone codify
    YAML files. The heavy lifting lives in
    :func:`codify.spec.load`, which dispatches on the source
    ...
    except (CodifyError, FileNotFoundError):
...
def _canonicalize_local_agent_path(agent_path: Path) -> Path:
    ...
    :param agent_path: Existing local path supplied to ``codify run``,
...
def _codify_log_dir() -> Path:
    """
    Resolve the shared Codify process log directory.
    ...
    :returns: ``~/.codify/logs``, created if needed.
    """
    log_dir = logs_root()
...
def _codify_persistent_dir() -> Path:
    """
    Resolve the persistent codify data directory.

    Honors ``CODIFY_DATA_DIR`` (the data-isolation knob a worktree sets
    to avoid sharing ``~/.codify/chat.db``), else lives at
    ``~/.codify`` alongside the native paths ``sessions/`` and ``logs/``
    (see designs/RUN_CODIFY_SESSION_RESUMPTION.md). Created on first access;
    subsequent calls are idempotent.

    Must resolve identically to
    :func:`codify.host.local_server._local_data_dir` — the local server
    writes its DB under that dir while ``codify run`` reads the resume DB
    from here, so a divergence would silently lose history. ``CODIFY_CONFIG_HOME``
    ...
    :returns: The absolute path to the persistent dir,
        ...
        subdir.
    """
    override = os.environ.get("CODIFY_DATA_DIR")
    ap_dir = Path(override).expanduser() if override else Path.home() / ".codify"
...
def _start_local_server(
    ...
    *,
    ...
    """
    Launch a local Codify server.

    Server stdout/stderr are routed to ``server.log`` in a
    per-run directory under ``~/.codify/logs`` so concurrent Codify sessions don't
    ...
    ``~/.codify/{chat.db,artifacts/}`` by default —
    persistent across runs so ``--continue`` / ``--resume``
    can resume prior conversations
    (designs/RUN_CODIFY_SESSION_RESUMPTION.md). Pass
    ...
    :param ephemeral: When ``True``, place the SQLite DB and
        ...
        ``~/.codify`` location. Used for ``--no-session``
    ...
    from codify.cli import _start_cli_runner_process
    from codify.runner.identity import token_bound_runner_id
    ...
    # we spawn below (read by server() via CODIFY_RUNNER_TUNNEL_TOKEN).
    #
    # Accounts opt-in: when the parent shell selects accounts mode
    # (CODIFY_AUTH_PROVIDER=accounts), inject the per-spawn
    # base URL + cookie secret so the spawned server's
    # AccountsConfig.from_env() can satisfy its required-fields check.
    # Mirrors the same logic in cli.py:_ensure_local_codify_server; see
    # the comment block there for the full UX explanation. (Two
    # spawn paths exist because `codify run --server ""` goes
    # through _ensure_local_codify_server while `codify run` with
    # an agent spec and no --server flag goes through here.)
    child_env = {
        **os.environ,
        "CODIFY_RUNNER_TUNNEL_TOKEN": binding_token,
        PROCESS_LOG_FILE_ENV_VAR: str(log_path),
        # Single-user loopback runtime — see ensure_local_codify_server for why
        ...
        "CODIFY_LOCAL_SINGLE_USER": "1",
    }
    # Mirror create_auth_provider's resolution so this spawn path agrees
    # with the daemon path (host/local_server.py::ensure_local_codify_server):
    # header is the env-unset default; CODIFY_AUTH_ENABLED=1 opts into
    # accounts (or oidc when CODIFY_OIDC_* is set). In header/oidc mode we
    ...
    if _accounts_mode:
        if "CODIFY_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            child_env["CODIFY_ACCOUNTS_COOKIE_SECRET"] = secrets.token_hex(32)
        ...
        child_env["CODIFY_ACCOUNTS_BASE_URL"] = f"http://127.0.0.1:{port}"
    ...
    # This ensures the Codify server and its runner subprocess resolve credentials
...
def _stop_local_server(server: LocalServer) -> None:
    ...
    """
    from codify.cli import _stop_cli_runner_process
...
def _spec_used_families(agent_yaml: Path | None) -> list[str]:
    ...
    if agent_yaml.is_dir():
        ...
        return []
    try:
        from codify.onboarding.provider_config import PI_SURFACE, harness_family
        from codify.spec import parse
...
def _run_repl(
    ...
    *,
    ...
    :param auto_open_conversation: When ``True``, open the
        browser conversation URL when the session id becomes known.
    """
    from codify.repl import run_repl
    from codify.repl._session_log import DEFAULT_LOG_DIR
    from codify.repl._tmux_pane import register_pane

    log_dir = DEFAULT_LOG_DIR if log else None

    # Mark this tmux pane as an codify context source and wrap
    ...
    async def _main() -> None:
        ...
        from codify.cli_diagnostics import install_asyncio_exception_handler
...
def _run_one_shot(
    *,
    ...
    try:
        ...
        # raises as an CodifyError. Surface its message as a clean
...
def _load_tool_handler(name: str) -> ToolHandler:
    ...
    """
    try:
        from codify.client_tools import get_tool_set
    ...
    if hasattr(tool_set, "_TOOL_FNS"):
        ...
        from codify_client.tools import build_tool_handler
