"""Native Hermes TUI wrapper for the Codify CLI.

``codify hermes`` launches Nous Research's Hermes Agent interactive TUI (the bare
``hermes`` command) inside an Codify-runner-owned tmux terminal and attaches the
local TTY — the Hermes analog of ``codify goose`` / ``codify cursor``. The
runner spawns the process (see
:func:`codify.runner.app._auto_create_hermes_terminal`); this module owns the
...
``~/.hermes/config.yaml``); no Codify-managed key is required. Like goose there
...
from codify._native_resume_hint import echo_native_cold_resume_hint, echo_native_resume_hint
...
from codify._wrapper_labels import HERMES_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_HERMES_PATH_ENV = "CODIFY_HERMES_PATH"
...
class PreparedHermesTerminal:
    ...
    :param cold_resumed: ``True`` when resuming an existing Codify session whose
...
def run_hermes_native(
    *,
    ...
    :param server: Resolved Codify server URL.
    ...
    if server is None:
        raise click.ClickException(
            "Hermes requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_hermes_native."
        )
    with TemporaryDirectory(prefix="codify-hermes-native-") as tmpdir:
...
def _materialize_hermes_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify hermes``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Hermes on an Codify server via a daemon-spawned runner.
