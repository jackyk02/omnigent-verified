...
Each ``codify <tool>`` command (``claude``, ``codex``, ``pi``, …) launches a
vendor CLI inside an Codify-managed terminal. They were extracted from
:mod:`codify.cli` so that file stops carrying ~1400 lines of near-identical
...
``codify.cli`` after the ``cli`` group and its shared helpers exist. The
command bodies reach those helpers through thin proxies that resolve them on
the ``codify.cli`` module at call time, so this module never needs a
top-level ``import codify.cli`` (no import cycle) and a test's
``monkeypatch`` of an ``codify.cli`` helper still takes effect. Only
decorator-time state (:data:`codify.cli_common.RESUME_PICKER_SENTINEL`,
:data:`codify.cli_common.CLAUDE_STARTUP_PROFILE_ENV_VAR`) is imported at module
scope, from the leaf :mod:`codify.cli_common`.
"""
...
def register_native_commands(cli: click.Group) -> None:
    ...
    Called once from :mod:`codify.cli` at import time. The shared runtime
    helpers are looked up on the ``codify.cli`` module *at call time* (via
    thin proxies below), not bound now — so importing this module never imports
    ``codify.cli`` (no cycle) and tests that ``monkeypatch`` a helper as an
    ``codify.cli`` attribute still take effect inside these commands.

    :param cli: The root ``codify`` Click group to attach commands to.
    """
    import codify.cli as _cli

    # Resolve each shared helper on the live module per call so a test's
    # monkeypatch of ``codify.cli.<helper>`` is honored. Binding the function
    ...
    @click.option(
        ...
        default=None,
        help=(
            "Remote codify URL. Starts a local runner, binds the session, "
    ...
    @click.option(
        ...
        default=None,
        help=(
            "Resume a prior Codify conversation. With a conversation id "
    ...
    @click.option(
        ...
        help=(
            ...
            "``CODIFY_CLAUDE_PATH`` env var or the "
    ...
    def claude(
        ...
        # :param server: Remote Codify server URL, or None for local.
        ...
        """Launch Claude Code in an Codify terminal.

        \b
        Examples:
          codify claude
          codify claude --resume conv_abc123
          codify claude --resume                  # interactive picker
          codify claude --server https://<app>.databricksapps.com
        """
        ...
        # remote otherwise) and resolve the concrete Codify server URL. The daemon
        ...
        from codify.claude_native import run_claude_native
        from codify.harness_startup_config import resolve_harness_command

        startup_profiler.mark("native module imported")

        if claude_command:
            click.echo(
                "codify: `claude --command` is deprecated; set CODIFY_CLAUDE_PATH "
    ...
    @click.option(
        ...
        default=None,
        help=(
            "Remote codify URL. Ensures the host daemon, asks the "
    ...
    def codex(
        ...
        Examples:
          codify codex
          codify codex --resume conv_abc123
          codify codex --resume                  # interactive picker
          codify codex --server https://<app>.databricksapps.com
        """
        ...
        from codify.codex_native import run_codex_native
        from codify.harness_startup_config import resolve_harness_command
        ...
        # Ensure the host daemon (local when ``--server`` is omitted/empty,
        # remote otherwise) and resolve the concrete Codify server URL. Codex follows
    ...
    def opencode(
        ...
        # ``CODIFY_OPENCODE_PATH`` or ``harness.opencode-native.command`` config.
        ...
        """Launch OpenCode TUI in an Codify terminal.

        \b
        Examples:
          codify opencode
          codify opencode --resume conv_abc123
          codify opencode --resume                  # interactive picker
          codify opencode --server https://<app>.databricksapps.com
        """
        from codify.opencode_native import run_opencode_native
    ...
    def pi(
        ...
        """Launch Pi TUI in an Codify terminal.

        \b
        Examples:
          codify pi
          codify pi --resume conv_abc123
          codify pi --resume                    # interactive picker
          codify pi --model local-deepseek/deepseek-v4-flash
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.harness_startup_config import resolve_harness_command
        from codify.pi_native import run_pi_native

        cfg = _load_effective_config()
        # Thread ``harness.pi-native.command`` config into the runner via the
        # canonical ``CODIFY_PI_PATH`` env var (set before ``_ensure_backend``
    ...
    def cursor(
        ...
        """Launch the Cursor TUI in an Codify terminal.

        \b
        Examples:
          codify cursor
          codify cursor --model MODEL_ID
          codify cursor --resume conv_abc123
          codify cursor --resume                 # interactive picker
          codify cursor --mode plan              # start in plan (read-only) mode
          codify cursor --mode ask               # start in ask (Q&A) mode
        """
        ...
        from codify.cursor_native import run_cursor_native
        from codify.harness_startup_config import resolve_harness_command

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.cursor-native.command`` config into the
        # runner via the canonical ``CODIFY_CURSOR_PATH`` env var (set before
        ...
        if _resolved:
            os.environ["CODIFY_CURSOR_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
    ...
    def kiro(
        ...
        """Launch the Kiro TUI in an Codify terminal.

        \b
        Examples:
          codify kiro
          codify kiro --resume conv_abc123
          codify kiro --resume                  # interactive picker
          codify kiro --model auto -p "review this repo"
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.harness_startup_config import resolve_harness_command
        from codify.kiro_native import run_kiro_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.kiro-native.command`` config into the
        # runner via the canonical ``CODIFY_KIRO_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("kiro-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["CODIFY_KIRO_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        if model is None:
            model = cfg.get("model")
    ...
    def goose(
        ...
        """Launch the Goose TUI in an Codify terminal.

        \b
        Examples:
          codify goose
          codify goose --resume conv_abc123
          codify goose --resume                 # interactive picker
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.goose_native import run_goose_native
        from codify.harness_startup_config import resolve_harness_command

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.goose-native.command`` config into the
        # runner via the canonical ``CODIFY_GOOSE_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("goose-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["CODIFY_GOOSE_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def hermes(
        ...
        """Launch the Hermes TUI in an Codify terminal.

        \b
        Examples:
          codify hermes
          codify hermes --resume conv_abc123
          codify hermes --resume                 # interactive picker
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.harness_startup_config import resolve_harness_command
        from codify.hermes_native import run_hermes_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.hermes-native.command`` config into the
        # runner via the canonical ``CODIFY_HERMES_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("hermes-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["CODIFY_HERMES_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    @click.option(
        ...
        default=None,
        help=(
            "Remote codify URL. Ensures the host daemon, binds a runner, "
    ...
    def antigravity(
        ...
        """Launch the Antigravity (agy) TUI in an Codify terminal.

        \b
        Examples:
          codify antigravity
          codify antigravity --resume conv_abc123
          codify antigravity --resume                  # interactive picker
          codify antigravity --server https://<app>.databricksapps.com
        """
        ...
        from codify.antigravity_native import run_antigravity_native
        from codify.harness_startup_config import resolve_harness_command
    ...
    def qwen(
        ...
        """Launch the qwen (Qwen Code) TUI in an Codify terminal.

        \b
        Examples:
          codify qwen
          codify qwen --resume conv_abc123
          codify qwen --resume                  # interactive picker
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.harness_startup_config import resolve_harness_command
        from codify.qwen_native import run_qwen_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.qwen-native.command`` config into the
        # runner via the canonical ``CODIFY_QWEN_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("qwen-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["CODIFY_QWEN_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def kimi(
        ...
        """Launch the Kimi Code TUI in an Codify terminal.

        Boots Moonshot AI's interactive ``kimi`` TUI
        (https://github.com/MoonshotAI/Kimi-Code) in a runner-owned terminal and
        attaches your TTY — the native experience, embedded in the Codify web
        UI. No Codify provider config is needed: kimi authenticates against its
        own backend (``kimi login`` for OAuth, or a Moonshot API key).

        For the headless SDK harness (per-turn ``kimi -p`` behind the Codify
        REPL) use ``codify run --harness kimi`` instead.

        \b
        Examples:
          codify kimi
          codify kimi --resume conv_abc123
          codify kimi --resume                   # interactive picker
        """
        choice = _split_resume_value(resume)
        if session_id is not None and (choice.picker or choice.conversation_id is not None):
            raise click.UsageError(
        ...
        from codify.harness_startup_config import resolve_harness_command
        from codify.kimi_native import run_kimi_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.kimi-native.command`` config into the
        # runner via the canonical ``CODIFY_KIMI_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("kimi-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["CODIFY_KIMI_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
