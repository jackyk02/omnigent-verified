...
Hosts are machines connected via ``codify host``. The store
...
from codify.db.enum_codecs import decode_host_status, encode_host_status
from codify.db.utils import get_or_create_engine, make_managed_session_maker, now_epoch
...
class Host:
    ...
    :param host_id: Stable identifier from the host's local
        ``~/.codify/config.yaml``, e.g. ``"host_a1b2c3d4..."``.
