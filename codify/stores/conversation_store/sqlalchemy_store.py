...
from codify._wrapper_labels import UI_MODE_LABEL_KEY, WRAPPER_LABEL_KEY
from codify.db.converters import sql_agent_to_entity
from codify.db.db_models import (
    LABEL_VALUE_MAX_LEN,
    SqlAgent,
    SqlComment,
    SqlConversation,
...
from codify.db.enum_codecs import (
    decode_item_status,
    decode_item_type,
...
from codify.session_import.models import (
...
def _to_conversation(
    ...
    :param meta: Optional metadata row from
        ``codify_conversation_metadata``. When ``None``, all
        Codify-operational fields default (``kind="default"``,
...
def _new_session_conversation_row(
    ...
    (``session_overrides``) live on this row; Codify operational fields
...
def _new_session_metadata_row(
    ...
    """
    Build the Codify metadata row paired with a new session conversation.
...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def __init__(
        ...
        :param storage_location: SQLAlchemy database URI for the Codify DB,
            e.g. ``"sqlite:///codify.db"`` or
        ...
        # Codify DB: agents, hosts, policies, files, user_daily_costs,
        # session_permissions, comments, codify_conversation_metadata.
        ...
        # Defaults to the Codify DB when not separately configured. Always creates
        # a separate session factory so AP and Codify writes run in independent
        ...
        # Codify) gets the correct lock strategy for each table group.
    ...
    def _get_meta(
        ...
        """
        Fetch the metadata row for a conversation from the Codify DB.
        """
    ...
    def create_conversation(
        ...
        try:
            # Get parent's root from AP, then write AP row and Codify meta separately.
            root_id = new_id
    ...
    def get_session_connectivity(
        ...
        if not conversation_ids:
            return {}
        unique_ids = list(set(conversation_ids))
        # runner_id and host_id are in the Codify DB (metadata).
        with self._session() as session:
    ...
    def set_conversation_project(
        ...
        Sets ``codify_conversation_metadata.project_id``. ``None`` unfiles the
    ...
    def increment_session_usage(
        ...
        from codify.stores.conversation_store import apply_session_usage_delta
    ...
    def list_projects(
        ...
        *non-archived* ``conversation_labels`` row with ``key="omni_project"``
        ...
        """
        from codify.server.auth import LEVEL_OWNER

        # ACL (accessible_by/owned_by) resolves against session_permissions on
        # the Codify DB, so it still needs a pre-fetch; archived now lives on
    ...
    def list_conversations(
        ...
        :param pinned: When ``True``, restrict to sessions ``pinned_owner`` has
            pinned (their per-user ``codify.pinned.<user>`` label — the
        ...
        with self._conv_session() as session:
            ...
            if agent_name is not None:
                # Agents live in the Codify DB — resolve to IDs first, then
            ...
            if project is not None:
                ...
                # projects + metadata are on the Codify DB, so member ids are
            ...
            if pinned:
                ...
                # so match the caller's own key (``codify.pinned.<user>``), not
        ...
        # Fetch metadata from Codify DB and merge.
    ...
    def update_conversation(
        ...
        # binding + per-session override blob) and Codify (metadata).
        with self._conv_session() as ap_sess:
    ...
    def touch_runner_liveness(self, runner_ids: list[str], now: int) -> None:
        ...
        One bulk ``UPDATE`` on ``codify_conversation_metadata``, so
    ...
    def clear_runner_liveness(self, runner_id: str) -> None:
        ...
        Lives on ``codify_conversation_metadata``, so ``conversations.updated_at``
    ...
    def _create_session_with_agent_with_id(
        ...
        *,
        ...
        # Conversation + labels go to AP; agent + metadata go to Codify.
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param resume_source_native_session: When ``True`` (default), a
            ...
            copied Codify items instead.
        :param presentation_labels: When not ``None``, drop the source's
            ``codify.ui`` / ``codify.wrapper`` labels from the clone
    ...
    def _fork_conversation_with_id(
        ...
        *,
        ...
        # from the Codify DB before opening the AP session.
        with self._session() as meta_sess:
        ...
        with self._conv_session() as session:
            ...
            # Cloned agent: the row itself is written to the Codify DB after
            ...
            # Per-user pin keys (``codify.pinned.<user>``) are dynamic-suffix,
        ...
        # Write fork metadata (and cloned agent if any) to the Codify DB.
        with self._session() as meta_sess:
    ...
    def switch_conversation_agent(
        ...
        *,
        ...
        # Codify holds agent+metadata. Read old_agent_id before overwriting it.
        with self._conv_session() as ap_sess:
            row = ap_sess.get(SqlConversation, (current_workspace_id(), conversation_id))
            if row is None:
        ...
        # Update agent + metadata on the Codify side.
        with self._session() as session:
    ...
    async def delete_conversation(self, conversation_id: str) -> bool:
        ...
        # Codify-side rows (metadata/comments/policies/permissions) are cleaned up
        # second. A failure of the second transaction leaves orphaned Codify rows
        ...
        with self._conv_session() as ap_sess:
            ...
            # the Codify transaction below can delete the session-scoped
