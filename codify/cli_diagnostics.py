...
class CliLogContext:
    ...
    :param path: Absolute path to the current invocation's log file,
        e.g. ``~/.codify/logs/cli-20260518-143012-12345-a1b2c3.log``.
...
def _log_dir() -> Path:
    ...
    Uses the shared Codify runtime data dir so ``CODIFY_DATA_DIR``
...
def setup_cli_logging(argv: list[str]) -> CliLogContext:
    ...
    installs the redaction filter, wires up the ``codify`` and
    ``codify_ui_sdk`` logger hierarchies, and prunes old log
    files beyond :data:`MAX_LOG_FILES`.

    Call as early as possible in :func:`codify.cli.main` —
    ...
    if env_truthy(os.environ.get("CODIFY_LOG_TO_STDERR")) and sys.stderr.isatty():
    ...
    # the file handler.
    for name in ("codify", "codify_ui_sdk"):
    ...
    root = logging.getLogger("codify.cli_diagnostics")
...
def install_asyncio_exception_handler(loop: asyncio.AbstractEventLoop) -> None:
    ...
    :param loop: The running event loop (typically from
        ...
        context).
    """
    logger = logging.getLogger("codify.asyncio")
...
def print_setup_hint() -> None:
    ...
    Used by the top-level :func:`codify.cli.main` exception
    ...
    ``codify setup`` keeps the recovery path obvious without
    ...
    print(
        ...
        "`codify setup` to configure a model credential.",
        file=dest,
    )
...
def log_cli_exception(exc: BaseException, *, prefix: str = "CLI error") -> None:
    ...
    if current_cli_log_path() is None:
        return
    logging.getLogger("codify.cli_diagnostics").error(
        "%s: %s",
        prefix,
        exc,
...
def _safe_mtime(path: Path) -> float:
    ...
    ``_prune_old_logs`` runs at the start of every ``codify run``, so two
