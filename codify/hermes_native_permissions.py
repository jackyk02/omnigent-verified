...
lives only in the TUI; to also surface it in the Codify web UI (so a user can
...
terminal). Mirrors :mod:`codify.cursor_native_permissions`. NB: Hermes only
...
from codify.hermes_native_bridge import capture_hermes_pane, send_hermes_pane_keys
...
def hermes_permission_elicitation_id(session_id: str, token: str) -> str:
    """Return the deterministic Codify elicitation id for a Hermes prompt.
