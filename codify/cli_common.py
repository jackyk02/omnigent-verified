"""Shared constants and guards for the CLI, importable without ``codify.cli``.

The native coding-agent subcommands live in :mod:`codify.cli_native`, which
``codify.cli`` imports at module load to register them on the ``cli`` group.
...
that imports nothing from ``codify.cli`` — lets both ``cli`` and ``cli_native``
...
CLAUDE_STARTUP_PROFILE_ENV_VAR = "CODIFY_CLAUDE_STARTUP_PROFILE"


def reject_native_on_windows(harness: str) -> None:
    """Fail a native (tmux/PTY) harness command with an actionable message.

    The ``codify claude`` / ``codex`` / ``cursor`` native wrappers drive a
    ...
    if IS_WINDOWS:
        raise click.ClickException(
            f"`codify {harness}` (native tmux/PTY terminal) is not supported on "
            "Windows. Use an SDK-based harness via `codify run <agent.yaml>` "
            "or the web UI."
        )
