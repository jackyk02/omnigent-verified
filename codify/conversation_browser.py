"""Helpers for opening Codify conversation URLs from CLI frontends."""
...
# Databricks workspace-hosted codify: the API proxy and the web UI are
...
WORKSPACE_API_PATH = "/api/2.0/codify"
WORKSPACE_UI_PATH = "/codify"


def is_workspace_hosted_url(base_url: str) -> bool:
    """
    Whether *base_url* is a Databricks workspace-hosted Codify mount.

    True for the API proxy mount (``https://<ws>/api/2.0/codify``) the
...
def conversation_url(base_url: str, conversation_id: str) -> str:
    """
    Build the browser URL for an Codify conversation.

    For Databricks workspace-hosted servers
    (``https://<ws>/api/2.0/codify``) the web UI lives on the
    workspace SPA mount, so the link becomes
    ``https://<ws>/codify/c/<id>`` — with the ``?o=<org>``
    workspace selector appended when ``codify login`` recorded the
    org id.
