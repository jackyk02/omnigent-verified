...
from codify.inner.native_attachments import UNRESOLVED_ATTACHMENT_MARKER_PATTERN
...
# sync with attachment_reference_line in codify/inner/native_attachments.py
# and codify/inner/codex_native_executor.py.
...
class Conversation:
    ...
    :param host_id: Host that launched (or should launch) the
        ...
        sessions started via ``codify run`` (the CLI
    ...
    :param harness_override: Per-session harness override for the
        ...
        process. Only valid for ``executor.type: codify`` agents;
        the create route validates against ``CODIFY_HARNESSES``.
    ...
    :param terminal_launch_args: Pass-through CLI args for a native
        ...
        harness binary; the command and all bridge / Codify / auth wiring
