...
Per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.2, this rewrite swaps to:
...
- No idle reaper: terminals are explicit-launch only and the LLM is
  expected to ``sys_terminal_close`` when done. Codify shutdown still
  closes everything; per-conversation cleanup runs from the workflow's
  finally block.

The registry is constructed once at Codify startup
(``codify.runtime._globals.init``) and accessed via
``codify.runtime.get_terminal_registry()`` from tools and the
workflow.
...
from codify.inner.terminal import TerminalInstance, create_terminal_instance

logger = logging.getLogger(__name__)

# Per-instance close timeout. Bounds the cleanup window so a wedged
# tmux session can't block Codify shutdown or workflow finalization
...
def conversation_link_for_id(
    ...
    *,
    ...
    :param conversation_id: Conversation/session id, e.g.
        ``"conv_abc123"``.
    :param base_url: Optional Codify server base URL, e.g.
    ...
    # API→UI mount swap (``/api/2.0/codify`` → ``/codify``) and the
...
class TerminalRegistry:
    ...
    def __init__(self, *, conversation_link_base_url: str | None = None) -> None:
        ...
        :param conversation_link_base_url: Optional Codify server base URL
        ...
        # See ``designs/CODIFY_TERMINAL_BRIDGE.md`` §9.1.
    ...
    def native_panes(self) -> list[tuple[str, str, Path]]:
        ...
        (:mod:`codify.terminals.pane_reaper`); the reaper's wiring additionally
        ...
        """
        from codify.terminals.pane_reaper import NATIVE_PANE_TERMINAL_NAMES
    ...
    def transfer(
        ...
        process and tmux pane keep running, but the Codify session that owns
    ...
    async def cleanup_conversation(self, conversation_id: str) -> None:
        ...
        Per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.4, this is **not**
    ...
    async def shutdown(self) -> None:
        ...
        a stuck instance shouldn't block the rest of Codify shutdown.
        """
        with self._lock:
