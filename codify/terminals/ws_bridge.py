...
Lives under :mod:`codify.terminals` (alongside the
...
- :mod:`codify.server.routes.terminal_attach`, when no runner WS
  factory is configured (in-process / test setups, where the tmux
  socket lives on the same filesystem the server can see).
- :mod:`codify.runner.app`, where the runner exposes its own WS
...
# breaks headless hosts (managed sandboxes, ``codify sandbox
