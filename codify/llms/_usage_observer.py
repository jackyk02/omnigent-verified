...
2. **Auto-recorder** that activates whenever
   ``CODIFY_TOKEN_USAGE_JSON=<path>`` is set in the environment.
...
- :meth:`codify.llms.client._ResponsesNamespace.create` (in-process
  LLM client).
...
runner, harness) inherit both ``CODIFY_TOKEN_USAGE_JSON`` and
...
_ENV_VAR = "CODIFY_TOKEN_USAGE_JSON"
...
def _current_test_path() -> Path | None:
    ...
    Derived from the ``CODIFY_TOKEN_USAGE_JSON`` base path plus the
