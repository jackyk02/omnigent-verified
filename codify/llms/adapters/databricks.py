...
back to :func:`~codify.runtime.credentials.databricks.resolve_databricks_workspace`
...
class DatabricksAdapter(OpenAICompatibleAdapter):
    ...
    2. A cached ``databricks.sdk.config.Config`` object whose
       ...
       tracking needed.
    3. :func:`~codify.runtime.credentials.databricks.resolve_databricks_workspace`
       as a final fallback when the SDK is unavailable.

    An :class:`~codify.errors.CodifyError` is raised only when
    all paths fail.
    """
    ...
    async def chat_completions(
        ...
        *,
        ...
        :param connection_params: Optional. When provided, must contain
            ...
            to :func:`~codify.runtime.credentials.databricks.resolve_databricks_workspace`.
        :param timeout: Request timeout in seconds. ``None`` uses
            the module default.
        :returns: Response dict or async iterator of chunk dicts.
        :raises CodifyError: If ``connection_params`` lacks
