...
from codify.llms.adapters.base import BaseAdapter
...
def get_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    :raises CodifyError: If the provider is not supported.
    """
...
def _create_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    if provider in openai_compat_providers:
        ...
        from codify.llms.adapters.openai import OpenAICompatibleAdapter

        return OpenAICompatibleAdapter(base_url=resolved_url)

    if provider == "anthropic":
        from codify.llms.adapters.anthropic import AnthropicAdapter
    ...
    if provider == "bedrock":
        from codify.llms.adapters.bedrock import BedrockAdapter
    ...
    if provider == "databricks":
        from codify.llms.adapters.databricks import DatabricksAdapter
