...
Usage::
    ...
    from codify.llms import Client
...
def __getattr__(name: str) -> Any:
    ...
    Eager top-level imports here pull in ``codify.llms.client``,
    which imports ``codify.reasoning_effort``, which imports
    ``codify.llms.errors`` -- a submodule load that re-triggers
    ...
    raise AttributeError(f"module 'codify.llms' has no attribute {name!r}")
