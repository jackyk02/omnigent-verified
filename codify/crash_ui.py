...
live in :mod:`codify.crash_handler`. Keeping them separate means the
...
def real_stderr() -> TextIO:
    ...
    ``codify.cli_diagnostics.setup_cli_logging`` may replace
...
def _shorten_path(filename: str, *, cwd: str, site_packages: str | None) -> str:
    ...
    * under cwd           → relative (``codify/cli.py``)
    * under $HOME         → ``~/...``
...
def _resolve_top_package(filename: str) -> str | None:
    ...
    for editable installs (``sdks/python-client/codify_client/foo.py`` →
    ``codify_client``) and for wheel installs into site-packages
    (``site-packages/codify_client/foo.py`` → ``codify_client``).
...
# three core packages — ``codify``, ``codify_client``,
# ``codify_ui_sdk`` — plus the ``codify_slack`` integration.
_DEFAULT_FIRST_PARTY_PREFIXES = ("codify",)
...
def _full_traceback_env() -> bool:
    """``CODIFY_FULL_TRACEBACK=1`` disables library-frame collapsing."""
    return os.environ.get("CODIFY_FULL_TRACEBACK", "").strip().lower() in ("1", "true", "yes")
...
def format_traceback(
    ...
    *,
    ...
    1. **Path shortening** — venv ``site-packages`` paths become
       ...
       (``codify/cli.py``), ``$HOME`` becomes ``~``. Long absolute
    ...
    lands on the one line that matters. Set ``CODIFY_FULL_TRACEBACK=1``
...
def _title(name: str) -> str:
    """Capitalize the app name for sentence display (``codify`` → ``Codify``)."""
...
def render_crash_screen(
    *,
    ...
    this static screen — :mod:`codify.crash_handler` owns that, so it
