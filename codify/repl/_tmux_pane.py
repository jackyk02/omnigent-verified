...
1. Marks the pane with custom options so other tooling can identify
   it as an codify pane and recover the launch args.
2. Discovers the user's existing prefix-table ``split-window`` /
   ...
   - When the focused pane has the ``@codify-conv-id`` option set
     (i.e. it's an codify pane), runs ``codify pane-split``
...
non-codify panes (the else branch is the user's untouched
command), so the global mutation of the ``prefix`` table is
behaviorally invisible everywhere except inside an codify pane.
...
# branch is the user's exact original command, so non-codify
# panes keep working.
PANE_INTEGRATION_ENABLED = False

# Pane-option keys we set on the codify pane. Each is read by
# ``codify pane-picker`` to reconstruct the launch context for
# the new pane.
OPT_CONV_ID = "@codify-conv-id"
OPT_AGENT_NAME = "@codify-agent-name"
OPT_AGENT_YAML = "@codify-agent-yaml"
OPT_LAUNCH_ARGV = "@codify-launch-argv"
OPT_SERVER_URL = "@codify-server-url"

# Sentinel value for ``OPT_CONV_ID`` before the first conversation
# id is known. The wrapper's ``#{?#{@codify-conv-id},...}``
# truthiness check fires for any non-empty string, so this
# preserves "yes, this is an codify pane" even when there's no
# real conv id yet.
...
class SplitBinding:
    ...
    :param direction: One of ``'v'`` (vertical split), ``'h'``
        ...
        value passed to ``codify pane-split``.
    :param original_command: The full command string from the
        ...
        wrapper's else branch so non-codify panes get
...
def _resolve_codify_argv() -> list[str]:
    """
    Resolve a callable argv prefix for invoking the running
    codify installation from a context that doesn't share our
    ...
    1. ``sys.argv[0]`` is path-shaped (contains ``/``) — abspath
       it. Covers ``./.venv/bin/codify …`` and absolute-path
       invocations.
    2. ``sys.argv[0]`` is a bare name like ``"codify"`` — try
    ...
    3. Fallback — ``[sys.executable, "-m", "codify.cli"]``.
       Always works because if Python is running this code, the
       ``codify.cli`` module is importable. Trades aesthetics
       (the binding shows ``python -m codify.cli``) for
    ...
    :returns: A list of one or two argv elements. Length-1 means
        ...
        ``[python, "-m", "codify.cli"]``. The caller appends
        the subcommand and its args (e.g. ``["pane-split", "-v",
        ...]``) to this prefix.
    """
    argv0 = sys.argv[0] if sys.argv else ""
    # When the process was launched via ``python -m codify.cli``
    ...
    if argv0.endswith(".py"):
        return [sys.executable, "-m", "codify.cli"]
    ...
    # ``codify.cli`` module is importable since we're running
...
# match is the launcher prefix (the codify binary, ``python
# -m codify.cli``, etc.) and gets stripped during
...
def _user_args_after_launcher(launch_argv: list[str]) -> list[str]:
    ...
    - ``[<codify-binary>, run, …]`` (PATH-resolved console script)
    - ``[<python>, -m, codify.cli, run, …]`` (python-m fallback)
    ...
    :param launch_argv: ``sys.argv`` (or stored ``launch_argv``)
        of the running ``codify`` invocation.
...
def _unset_pane_options(pane_id: str) -> None:
    """
    Remove every ``@codify-*`` pane option this module sets.
    ...
    ``if-shell -F '#{?#{@codify-conv-id},...}'`` wrapper falls
...
def _classify(cmd_tokens: list[str]) -> str | None:
    ...
    routing in codify panes. Acceptable v1 limitation.

    :param cmd_tokens: The command portion of a bind-key line,
        e.g. ``['split-window', '-h', '-c', '#{pane_current_path}']``.
    :returns: Direction code consumed by ``codify pane-split``,
        or ``None``.
    """
    if not cmd_tokens:
        return None
...
_WRAPPER_MARKER_FORMAT = "#{?#{@codify-conv-id},1,0}"


def _unwrap_existing_wrapper(cmd_tokens: list[str]) -> list[str] | None:
    """
    Peel off an existing codify wrapper to recover the user's
...
def _discover_split_bindings() -> list[SplitBinding]:
    ...
    an unfamiliar flag set, are silently skipped. Non-codify
    panes still see those bindings unchanged (they're never
    rewritten); codify panes just don't get chooser routing
    on those keys.
    """
...
def _wrap_binding(binding: SplitBinding, codify_argv: list[str]) -> None:
    """
    Replace one of the user's prefix-table bindings with an
    ``if-shell -F`` wrapper.

    True branch (focused pane is an codify — the
    ``@codify-conv-id`` option is set and non-empty): run
    the resolved ``codify pane-split`` invocation via
    ...
    when the codify branch fires.

    :param binding: One of the user's prefix-table split-window /
        new-window bindings.
    :param codify_argv: Argv prefix for invoking the running
        codify installation, as returned by
        :func:`_resolve_codify_argv`. Length 1 (e.g.
        ``["/venv/bin/codify"]``) for a direct binary
        invocation, or length 3 (``[sys.executable, "-m",
        "codify.cli"]``) for the python-m fallback. Either
...
def register_pane(
    *,
    ...
    """
    Mark the current tmux pane as an codify pane and wrap the
    ...
    :param agent_yaml: Path to the agent spec, or ``None`` when
        ...
        ``OPT_AGENT_YAML``.
    :param launch_argv: ``sys.argv`` of the running ``codify
    ...
    if not PANE_INTEGRATION_ENABLED:
        ...
        # ``#{?#{@codify-conv-id},1,0}`` check returns 0, so the
    ...
    pane_id = _tmux_pane_id()
    if pane_id is None:
        _LOGGER.warning("$TMUX is set but $TMUX_PANE is not — skipping codify pane integration")
        return
    if not _tmux_version_ok():
        _LOGGER.warning(
            "tmux >= %s required for codify pane integration; skipping",
            ".".join(str(n) for n in _MIN_TMUX_VERSION),
        )
        return

    # Resolve a callable ``codify`` argv prefix. tmux's
    ...
    # ``codify`` lives — so a bare ``codify`` exits 127.
    # ``sys.argv[0]`` is the bare name when the user invoked us
    # via ``$PATH`` (e.g. typed ``codify`` after activating
    # a venv), so we can't just pass through ``launch_argv[0]``.
    # See :func:`_resolve_codify_argv` for the resolution
    # ladder (abspath → ``shutil.which`` → ``python -m`` fallback).
    codify_argv = _resolve_codify_argv()
    ...
    # ``codify_argv``. The user's subcommand and args
    # (``run agent.yaml …``) survive untouched.
    #
    # Idempotency under repeat calls: ``_user_args_after_launcher``
    # walks past leading launcher tokens (the codify binary,
    # ``python``, ``-m``, ``codify.cli`` — including doubled
    ...
    # ``-m codify.cli`` markers each time is exactly what this
    # repairs.
    user_args = _user_args_after_launcher(launch_argv)
    normalized_launch_argv = list(codify_argv) + user_args
    ...
    for binding in _discover_split_bindings():
        _wrap_binding(binding, codify_argv)
...
def read_pane_option(pane_id: str, name: str) -> str | None:
    ...
    invocation fails. Used by ``codify pane-picker`` to
