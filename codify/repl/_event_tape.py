...
2. **JSONL Event Log**: same data written to
   ``~/.codify/debug/events-<session_id>.jsonl`` for offline analysis.
...
# Directory under ``~/.codify/`` where event logs are written.
...
def open_event_log(session_id: str) -> pathlib.Path:
    ...
    :returns: Absolute path to the log file, e.g.
        ``~/.codify/debug/events-sess_abc123.jsonl``.
    """
    safe_id = "".join(c if c.isalnum() or c in "-_" else "_" for c in session_id)
    debug_dir = pathlib.Path.home() / ".codify" / _DEBUG_DIR_NAME
