...
Conversations come from Codify' conversation store.
...
# ``codify._wrapper_labels``. Imported here so the picker module
...
from codify.claude_native_state import read_launch_state as _read_claude_launch_state
from codify.codex_native_state import read_launch_state as _read_codex_launch_state
...
# ``❯`` mirrors :meth:`codify_ui_sdk.RichBlockFormatter.\
...
class _ConversationRow(Protocol):
    ...
    Both the SDK's :class:`codify_client.types.Conversation`
    and the store's
    :class:`codify.entities.conversation.Conversation`
    ...
    :param labels: Session-scoped labels (read for the Runtime
        metadata badge — the ``codify.wrapper`` key identifies
...
def _runtime_badge(row: _ConversationRow) -> str:
    ...
    The badge surfaces which Codify wrapper owns the session so a
...
def pick_conversation(
    ...
    *,
    ...
    :param show_runtime: When ``True``, render runtime metadata
        ...
        picker (``codify resume``) where a user is choosing
...
async def pick_conversation_by_wrapper_label_from_sdk(
    ...
    *,
    ...
    """Picker scoped to one wrapper kind (``codify.wrapper=<value>``).
    ...
    (``~/.codify/claude-native/``); sessions created on a
...
async def pick_conversation_cross_agent_from_sdk(
    ...
    *,
    ...
    ``codify resume``'s runtime-dispatch UX."""
...
def _render_workspace_cell(row: _ConversationRow, *, current_cwd: Path) -> Any | None:
    ...
    *current_cwd* (resolved). When state is absent -- legacy session
    ...
    ``~/.codify/claude-native/<hash>/launch.json`` -- fast on
...
async def _collect_previews_async(
    ...
    :param client: The :class:`codify_client.CodifyClient`.
...
def _last_message_preview_from_entities(items: list[Any]) -> _Preview | None:
    ...
    :class:`codify.entities.ConversationItem` objects whose
