"""
JSON dump of an codify conversation — the Codify mode port of the
legacy ``--log`` feature (see designs/RUN_CODIFY_REPL_PARITY.md).

The legacy non-AP path wrote a session-shaped JSON to
``~/.codify/logs/`` at REPL exit (`_write_session_log` /
``_session_log_dict`` in ``codify/inner/cli.py``). That format
was tied to the in-memory :class:`Session` machinery — codify
...
``codify/repl/_repl.py``), so spawn handles are recognized
...
from codify_ui_sdk import state_dir
...
_LOG_FORMAT = "codify-conversation"

# Default log directory. Derives from the shared ``state_dir()``
# (``~/.codify``) so the root is defined in one place. Mirrors
# the legacy non-AP path's ``~/.codify/logs/`` (see
# ``_default_session_log_path`` in ``codify/inner/cli.py``) so
...
def default_log_zip_path(
    ...
    *,
    ...
    Format: ``{output_dir}/codify-logs-{session_slug}-{timestamp}.zip``
    when a session id is available, otherwise
    ``{output_dir}/codify-logs-{timestamp}.zip``. ``None`` uses
    :data:`DEFAULT_LOG_ZIP_DIR`, i.e. ``~/.codify/logs``.
    ...
    if slug is not None:
        return base / f"codify-logs-{slug}-{timestamp}.zip"
    return base / f"codify-logs-{timestamp}.zip"
...
def collect_log_files(log_paths: list[Path]) -> list[tuple[Path, str]]:
    ...
    This helper does **not** walk ``~/.codify/logs`` or
    ``~/.codify/debug`` wholesale; doing so would include unrelated
    sessions.
...
def default_log_path(conversation_id: str, log_dir: Path | None = None) -> Path:
    ...
    treatment in ``codify/inner/cli.py::_default_session_log_path``.
...
async def write_session_log(
    ...
    *,
    ...
    in ``codify/repl/_repl.py`` — same parser the Ctrl+O debug
    ...
    :param client: A connected :class:`CodifyClient`. The
    ...
    :param agent_name: Agent name to include in the dump (for
        ...
        e.g. ``"resume_test"``.
    :param log_dir: Override the default location. ``None`` uses
        :data:`DEFAULT_LOG_DIR` (``~/.codify/logs/``).
...
def _extract_child_conversation_ids(items: list[dict[str, object]]) -> list[str]:
    ...
    from codify.repl._repl import _parse_sub_agent_handle
...
def write_session_log_from_store(
    ...
    *,
    ...
    :class:`codify.stores.conversation_store.ConversationStore`
    directly instead of the SDK.

    Used by the one-shot ``codify run <yaml> -p "…" --log`` path
    where the in-process ASGI app doesn't have a connected
    :class:`CodifyClient` — the run goes through raw httpx +
...
def _fetch_all_items_sync(
    ...
    self-contained — a reader doesn't need any of Codify'
...
async def _fetch_all_items_via_sessions(
    ...
    :param client: The connected :class:`CodifyClient`.
