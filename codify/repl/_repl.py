"""Rich-based REPL for codify — built on the UI SDK framework.
...
# ``from codify_ui_sdk import FormattedItem``.
from codify_ui_sdk.terminal._completer import FileMentionCompleter
from codify_ui_sdk.terminal._formatter import FormattedItem
from codify_ui_sdk.terminal._theme import LIGHT_THEME, get_theme
...
from codify.spec.types import SkillSpec

if TYPE_CHECKING:
    from codify.server.schemas import SessionStatusEvent
...
SlashCommandHandler = Callable[
    [str, Session, CodifyClient, TerminalHost, RichBlockFormatter],
    Awaitable[None],
]
...
# (tests/e2e/codify/_pexpect_harness.py). /quit discoverability is
...
def _load_startup_theme() -> TerminalTheme:
    ...
    On first launch (no persisted theme in ``~/.codify/config.yaml``),
    ...
    from codify.repl._theme_picker import startup_theme_picker
...
def _display_cwd() -> str:
    ...
    :returns: The cwd with ``$HOME`` collapsed to ``~`` (e.g.
        ``"~/codify"``), or the absolute path when it is not
...
def _header_glyph(kind: str) -> str:
    ...
    keeps its :func:`kind_glyph`. CLI surfaces (``codify setup``, the
    ...
    :returns: The kind's glyph (e.g. ``"🔑"``), or ``""`` for the
        subscription kind.
    """
    from codify.onboarding.configure_models import kind_glyph
...
def _build_startup_header(
    ...
    """
    from codify.onboarding.configure_models import (
        credential_label,
        family_label,
    )
...
def _render_startup_banner_ansi(
    ...
    *,
    ...
    """
    from codify.conversation_browser import display_server_url, is_workspace_hosted_url
    from codify.inner.banner import BannerLine, startup_banner_strings

    remote = _is_remote_server_url(server_url)
    # User-facing form of the URL: a Databricks workspace-hosted server is
    # connected to on its ``/api/2.0/codify`` API mount, but the banner
    # should show the recognizable workspace ``/codify`` URL. Non-Databricks
...
async def _fetch_server_version(client: CodifyClient) -> str | None:
    ...
    Routed through the REPL's already-connected :class:`CodifyClient` so
...
def _is_remote_server_url(url: str | None) -> bool:
    ...
    A local ``codify run`` spawns its own Codify server on
...
class _ApprovalState:
    ...
    def remember_always(self, policy_name: str, phase: str) -> None:
        ...
        cache is NEVER persisted to disk; closing ``codify chat``
...
def _build_elicitation_content_from_schema(
    ...
    See :func:`codify.tools._elicitation_schema.build_accept_content_from_schema`
    ...
    """
    from codify.tools._elicitation_schema import build_accept_content_from_schema
...
def _elicitation_resolve_session_id(sdk_event: object, fallback_session_id: str) -> str:
    ...
    :param sdk_event: The translated
        :class:`codify_client._events.ElicitationRequest`; its
...
def _server_event_to_sdk_event(event: object) -> object | None:
    ...
    :mod:`codify_client._events`. Returns ``None`` for variants
    ...
    from codify_client._types import ErrorInfo
    from codify_client._types import Response as SDKResponse
...
class _SessionsChatReplAdapter:
    ...
    def __init__(
        ...
        :param client: The :class:`CodifyClient` used to
        ...
        :param attach_only: When ``True``, run as a pure co-drive client:
            ...
            existing host-bound runner). Used by ``codify attach``.
    ...
    async def _runner_recover_watch(self) -> None:
        ...
        :returns: None.
        """
        _dbg = bool(os.environ.get("CODIFY_SESSIONS_ADAPTER_DEBUG"))
        while True:
            try:
    ...
    async def _ensure_session(self) -> str:
        ...
        Set ``CODIFY_SESSIONS_ADAPTER_DEBUG=1`` to trace
            ...
            if self._session_id is None:
                if self._session_bundle is None:
                    raise RuntimeError(
                        ...
                        "Start the REPL from `codify run <agent.yaml>` so the CLI can "
    ...
    def _emit_runner_recovery_error_once(self, exc: Exception) -> None:
        ...
        :param exc: Failure raised while relaunching or rebinding a
            runner, e.g. an SDK ``CodifyError`` from
        ...
        from codify.server.schemas import ErrorEvent, RetryErrorDetail
    ...
    def _is_terminal_runner_recovery_error(self, exc: Exception) -> bool:
        ...
        :returns: ``True`` for typed server runner-state errors,
            ...
            retrying.
        """
        if not isinstance(exc, CodifyError):
            return False
        code = exc.code or ""
    ...
    async def _stream_pump(self) -> None:
        ...
        """
        from codify.server.schemas import SessionStatusEvent as _StatusEv
    ...
    async def send(
        ...
        *,
        ...
        try:
            ...
            # _on_event.
            from codify_client._events import ResponseCompleted
            from codify_client._types import Response as SDKResponse
    ...
    async def send_skill_slash_command(
        ...
        The Codify server persists a visible ``slash_command`` item and
    ...
    def _spawn_client_tool(
        ...
        async def _run() -> None:
            try:
                ...
                args = {}
            from codify_client._tool_handler import ToolCallInfo
    ...
    async def _handle_elicitation(
        ...
        from codify_client._tool_handler import ElicitationRequestCtx
...
class _TurnProseTracker:
    ...
    (``_flush_relay_text`` in ``codify/server/routes/sessions.py``).
...
def _render_failed_status_error(
    ...
    """
    from codify_client import ErrorBlock
...
async def run_repl(
    ...
    *,
    ...
    :param client: Connected CodifyClient.
    ...
    :param resume_conversation_id: When set, the REPL opens
        ...
        ``--resume <id>`` (see designs/RUN_CODIFY_SESSION_RESUMPTION.md).
    ...
    :param log_dir: When set, write a JSON dump of the active
        ...
        the CLI ``--log`` flag (and the ``~/.codify/logs/``
        default location); see ``codify.repl._session_log`` for
        the schema. The dump runs in the SAME ``async with
        CodifyClient(...)`` scope as the REPL itself, so the
    ...
    :param server_url: Base URL of the Codify server the REPL is
    ...
    # behavior (codify/inner/cli.py:2979 + :2984): when a user
    ...
    from codify.repl._event_tape import EventTape, PipelineCounters, TapeEntry
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, _StatusEv):
            ...
            if event.status == "running":
                from codify_client import BlockContext, ResponseStartBlock
                ...
                from codify_client import TextDone
        ...
        from codify.server.schemas import ElicitationResolvedEvent
        ...
        if isinstance(sdk_ev, _TD):
            from codify_client import TextChunk
        ...
        if isinstance(sdk_ev, _RS):
            from codify_client import BlockContext, ReasoningStartBlock
        ...
        if isinstance(sdk_ev, _RD | _RSD):
            from codify_client import BlockContext
            from codify_client import ReasoningChunk as _RC
        ...
        from codify_client._events import ResponseCompleted as _Completed
        ...
        from codify_client._events import ErrorEvent as _ErrorEvent

        if isinstance(sdk_ev, _ErrorEvent):
            from codify_client import BlockContext, ErrorBlock
        ...
        from codify_client._events import ResponseFailed as _Failed
    ...
    def _maybe_log_tape_entry(entry: TapeEntry) -> None:
        ...
        if _event_log_fh is not None:
            from codify.repl._event_tape import log_entry_jsonl
    ...
    # Paint the composer in the codify-logo green while the line is a "!"
    ...
    async def on_input(text: str, attachments: list[PendingAttachment] | None = None) -> None:
        ...
        try:
            ...
            # was the exact user-reported bug where ``codify chat`` would
    ...
    # usage totals) is codify-specific. The SDK's
    ...
    # ``codify/inner/cli.py:1791-1797``.
    from codify_ui_sdk import OverlayAction
    ...
    if debug_events and _event_tape is not None:
        from codify.repl._event_tape import build_tape_detail, build_tape_targets
        ...
        async def _tape_targets() -> list[OverlayTarget]:
            ...
            """
            from codify.repl._event_tape import _OverlayTargetLike
    ...
    async with host:
        ...
        if debug_events:
            ...
            from codify.repl._event_tape import open_event_log
        ...
        # Codify REPL feels identical at boot. Raw stdout write
        # (matching ``codify/inner/cli.py:2962``) — the banner
        ...
        # streamed yet. See
        # ``designs/RUN_CODIFY_REPL_PARITY.md``.
        import sys as _sys
        ...
        from codify.conversation_browser import is_workspace_hosted_url
        ...
        from codify_ui_sdk import StreamingText
    ...
    # Top-level ``codify resume`` only dispatches claude-native today;
...
async def _maybe_write_session_log(
    ...
    :param client: Connected CodifyClient — REUSED, not opened
...
async def _cmd_help(
    ...
    client: CodifyClient,  # noqa: ARG001
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    from rich.text import Text
...
async def _cmd_theme(
    ...
    client: CodifyClient,  # noqa: ARG001 — dispatch-contract params
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    """
    from codify_ui_sdk.terminal._theme import DARK_THEME, LIGHT_THEME
    from rich.text import Text

    from codify.repl._theme_picker import _build_preview, build_theme_confirmation
...
def _model_readout_harness(active_model: str | None) -> str:
    ...
    :func:`codify.llms.routing.infer_harness_from_model`, falling back
...
def _build_model_readout_lines(
    ...
    it goes through ``codify setup --no-internal-beta`` + a restart. Falls
    ...
    :param config: The parsed effective config mapping (``providers:``
        block), e.g. from
        :func:`codify.onboarding.provider_config.load_config`.
    ...
    if harness and harness_owns_its_credential(harness):
        # The external agent authenticates itself, so there is no Codify
    ...
    if cred is None:
        ...
        if model_override is not None:
            ...
            lines.append("no model configured — run `codify setup --no-internal-beta` to add one")
    ...
    if others:
        ...
        lines.append(
            "  /model <name> changes the model. To switch provider: codify setup (then restart)."
        )
    return lines
...
def _match_configured_provider(config: dict[str, object], token: str) -> str | None:
    ...
    """
    from codify.onboarding.configure_models import provider_display_name
    from codify.onboarding.provider_config import load_providers
...
def _model_validation_warning(model: str) -> str | None:
    ...
    (:func:`codify.onboarding.providers.get_chat_models`). An unknown
    ...
    from codify.llms.routing import parse_model_string
    from codify.onboarding.providers import get_chat_models
...
async def _cmd_model(
    ...
    (it goes through ``codify setup --no-internal-beta`` + a restart).
    ...
    def _emit_model_readout() -> None:
        from codify.onboarding.detected import effective_config_with_detected
        from codify.onboarding.provider_config import load_config
    ...
    from codify.onboarding.configure_models import kind_glyph, provider_display_name
    from codify.onboarding.detected import effective_config_with_detected
    from codify.onboarding.provider_config import (
    ...
    if matched is not None and active_name is not None and matched != active_name:
        ...
        host.output(
            Text.from_markup(
                ...
                f"`codify setup --no-internal-beta` and select it as the "
...
async def _attach_to_conversation(
    ...
    *,
    ...
    - ``run_repl(resume_conversation_id=...)`` on startup
      ...
      designs/RUN_CODIFY_SESSION_RESUMPTION.md) — passes
    ...
    :param client: The :class:`CodifyClient` for fetching
        items.
    ...
    if last_response_id is None:
        ...
        # `codify run` the daemon hands the REPL a freshly-created session
    ...
    if cw:
        ...
        if last_total is not None:
            ...
            from codify.runtime.compaction import count_tokens
...
# Mirrors _DEFAULT_TRIGGER_THRESHOLD from codify.runtime.compaction.
...
async def _refresh_session_metadata(
    ...
    :param client: Codify HTTP client used to fetch the snapshot.
...
async def _update_context_ring_estimate(
    ...
    :param client: Codify HTTP client used to query items.
...
def _build_github_issue_url(
    ...
    body_parts: list[str] = [
        ...
        *info_lines,
        ...
        "     --debug-events for a JSONL event log in ~/.codify/debug/ -->",
    ]

    body = "\n".join(body_parts)
    base = "https://github.com/codify-ai/codify/issues/new"
    return (
        f"{base}"
...
async def _cmd_logs(
    ...
    from codify.cli_diagnostics import current_cli_log_path
    from codify.repl._session_log import write_logs_zip, write_session_log
...
async def _cmd_report(
    arg: str,
    session: Session,
    client: CodifyClient,  # noqa: ARG001 — dispatch-contract param
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
...
async def _collect_terminals_for_conversations(
    ...
    *,
    ...
    :param client: The codify HTTP client.
...
class _TerminalInfo:
    ...
    :param target: Tmux target. Always ``"main"`` per the
        ...
        (``codify/inner/terminal.py:167``); kept on the
...
def _parse_terminal_tool_output(raw: object) -> dict[str, object] | None:
    ...
    string (default executor, codify builtins) and
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    ``codify/inner/cli.py::_open_current_terminal_window``
    so users with muscle memory from the legacy CLI see the
    same behavior under Codify mode.
    ...
    :param client: Codify HTTP client — used to re-walk the
...
def _terminal_attach_command(info: _TerminalInfo) -> str:
    ...
    :meth:`codify.inner.cli._terminal_attach_command`
...
async def _build_terminal_overview(
    ...
    *,
    ...
    :meth:`codify.inner.cli._render_overview_terminal_text`
    output (``cli.py:2232``):
...
def _tmux_session_alive(socket: str, target: str) -> bool:
    ...
    :param socket: Tmux socket path the agent's
        ...
        ``"/tmp/codify-terminal-xyz/tmux.sock"``.
    :param target: Tmux session/target name. Always
        ``"main"`` per
        :class:`codify.inner.terminal.TerminalInstance.tmux_target`.
...
def _parse_sub_agent_handle(raw: str) -> dict[str, object] | None:
    ...
    Native codify builtins (``sys_session_send`` /
...
async def _build_debug_overview(
    ...
    *,
    ...
    The overview intentionally mirrors the ``codify run``
    ...
    2. **Event stream** — all items from the conversation,
       ...
       ``[N] type=...`` shape codify uses. Responses API
       items map onto the codify event vocabulary as
    ...
    :param client: The codify client used for the items fetch.
    ...
    :param cli_log_path: Optional path to the always-on CLI
        diagnostics log (``~/.codify/logs/cli/cli-*.log``).
    ...
    # pair — matching the codify event view. When an output
...
def _render_overview_message_event(
    ...
    """
    Render a ``message`` item as an codify event.

    Splits on role so the header reads ``user_message`` or
    ``assistant_message`` (matching codify' event vocabulary)
...
def _coerce_arguments_dict(raw: object) -> dict[str, object]:
    ...
    The codify API surfaces tool-call arguments as a JSON
...
def _render_message_history_item(
    ...
    if role == "user":
        ...
        return
    if role == "assistant":
        # Skip empty assistant messages. The codify workflow
...
_BANG_TIMEOUT_S: float = float(os.environ.get("CODIFY_BANG_TIMEOUT_S") or 120.0)
_BANG_DISPLAY_MAX: int = int(os.environ.get("CODIFY_BANG_DISPLAY_MAX") or 30_000)
_BANG_CONTEXT_MAX: int = int(os.environ.get("CODIFY_BANG_CONTEXT_MAX") or 16_000)

# The codify-logo green. Marks a "!" shell command consistently: the composer
...
class _BangInputLexer(Lexer):
    ...
    the shell via the passthrough; painting it in the codify-logo green is
...
def _write_bang_overflow(cmd: str, stdout: str, stderr: str) -> str | None:
    ...
    if len(plain_out) + len(plain_err) <= _BANG_CONTEXT_MAX:
        return None
    fd, path = tempfile.mkstemp(prefix="codify-bang-", suffix=".log")
