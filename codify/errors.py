"""Centralized error handling for the codify server.

All user-facing errors should be raised as CodifyError with an
...
New code should prefer CodifyError for consistency.
"""
...
class ErrorCode:
    ...
    :cvar HARNESS_PROTOCOL_VIOLATION: A harness emitted an SSE
        sequence that violates the Codify↔harness contract — e.g.
    ...
    :cvar HARNESS_NOT_CONFIGURED: The session's harness is not
        ...
        running ``codify setup`` on the host machine).
    """
...
_CODE_TO_HTTP_STATUS: dict[str, int] = {
    ...
    # can't satisfy it until the user runs `codify setup` there —
