...
ANTIGRAVITY_NATIVE_BRIDGE_ID_LABEL_KEY = "codify.antigravity_native.bridge_id"
...
_BRIDGE_ROOT = Path.home() / ".codify" / "antigravity-native"
...
# onboarding is a distinct flow Codify does not drive, so it stays ``False``.
...
def ensure_agy_onboarding_complete() -> None:
    ...
    (see :func:`codify.runner.app._auto_create_antigravity_terminal` and the CLI
...
def bridge_root() -> Path:
    ...
    :returns: Absolute root for Antigravity-native bridge directories, e.g.
        ``Path("~/.codify/antigravity-native")``.
    """
    return _BRIDGE_ROOT
...
class AntigravityNativeBridgeState:
    ...
    .. note::
        ...
        driver that superseded it (:mod:`codify.antigravity_native_reader`) keeps
    ...
    .. note::
        ...
        :func:`codify.antigravity_native_rpc.resolve_language_server_port`).
...
# ── Codify MCP relay wiring (sys_* tools) ──────────────────────────────────
#
# agy is otherwise tool-isolated from Codify: unlike claude/codex/cursor it
# wires no MCP relay, so the wrapped agy cannot reach any ``sys_*`` tool (spawn
# sub-agent sessions, drive Codify terminals, list agents/models, ``sys_os_*``).
# This section wires the SAME relay cursor #742 uses — the shared
# ``codify.claude_native_bridge serve-mcp`` stdio server — into agy.
#
...
# Codify's own TOOL_CALL policy + elicitation gate still applies on the server
# side, so auto-approving the agy-side MCP gate only avoids a hidden in-terminal
# prompt blocking the call before Codify ever sees it.
_AGY_ENABLED_TOOLS = [
    "list_comments",
    "sys_add_policy",
    "sys_agent_download",
...
# ``codify.onboarding.gemini_auth``: macOS writes ``oauth_creds.json`` while
...
def agy_gemini_dir(bridge_dir: Path) -> Path:
    ...
    :returns: Absolute ``.gemini`` directory that should receive agy's session
        config/state and the Codify MCP config.
    """
...
def build_mcp_config(
    ...
    *,
    ...
    """Build agy's ``mcp_config.json`` payload for the Codify relay server.

    Mirrors cursor #742's :func:`codify.cursor_native_bridge.build_mcp_config`
    ...
    ``<python> -I -m codify.claude_native_bridge serve-mcp --bridge-dir <dir>``.

    **HOME pinning.** agy spawns this relay as a child. The relay validates its
    ``--bridge-dir`` against ``bridge_root()`` (``$HOME/.codify/antigravity-native``),
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared Codify MCP relay requires.
    ...
    Mirrors :func:`codify.cursor_native_bridge.write_mcp_bridge_config`.
...
def write_mcp_config(
    ...
    *,
    ...
    ``mcp_config.json`` for the Codify relay into the per-session isolated agy
    ...
    config. Mirrors cursor #742's :func:`codify.cursor_native_bridge.write_mcp_config`,
...
def update_conversation_id(
    ...
    Codify session stays the same (e.g. the runner cold-start replacing the
...
_PASTE_BUFFER = "codify-agy-paste"
...
def send_interaction_keys_via_tui(
    ...
    **in-process permission / question prompt**. When agy runs attended (the
    ...
    Codify web UI AND as agy's own numbered TUI prompt ("Do you want to
    proceed?", 1.Yes … 4.No). Delivering the verdict over
    :func:`codify.antigravity_native_rpc.handle_user_interaction` flips the
    ...
    :func:`codify.cursor_native_bridge.send_cursor_pane_keys`.
