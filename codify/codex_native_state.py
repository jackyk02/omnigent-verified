"""Persistent client-side state for ``codify codex`` sessions.

The native Codex wrapper records the cwd used to create a session so a
later ``codify codex --resume <conv_id>`` can launch Codex from the
...
on the shared Codify server.

Layout (per conversation):

    ~/.codify/codex-native/<sha256(conv_id)[:32]>/launch.json
"""
...
_STATE_ROOT_ENV_VAR = "CODIFY_CODEX_NATIVE_STATE_DIR"
...
def _codex_native_state_root() -> Path:
    ...
    setups. Production defaults to ``~/.codify/codex-native``.
    ...
    return Path.home() / ".codify" / "codex-native"
