...
# is generous by default and overridable via CODIFY_GIT_STATUS_TIMEOUT_SECONDS
...
def _git_timeout_seconds() -> float:
    ...
    Reads ``CODIFY_GIT_STATUS_TIMEOUT_SECONDS`` on each call so operators can
    tune it without a restart. Falls back to the default on unset/invalid/
    non-positive values.
    """
    raw = os.environ.get("CODIFY_GIT_STATUS_TIMEOUT_SECONDS")
    if raw is not None:
        try:
            value = float(raw)
        except ValueError:
...
def _untracked_cache_repo_lock(git_root: Path) -> Iterator[None]:
    ...
    if fcntl is None:
        ...
        return
    fd: int | None = None
    lock_path = _git_common_dir(git_root) / "codify-untracked-cache.lock"
    try:
...
class GitFilesystemRegistry(FilesystemRegistry):
    ...
    def start(self) -> None:
        ...
        threading.Thread(
            ...
            name="codify-git-untracked-cache",
            daemon=True,
        ).start()
