...
from codify.env_credentials import expand_envvars_with_codify_prefix
...
from codify.onboarding.ucode_state import UcodeAgentState, read_ucode_state
from codify.runtime import (
    get_artifact_store,
    get_conversation_store,
    get_file_store,
)
from codify.runtime.compaction import (
    CompactionResult,
    SummaryMetadata,
    _CompactionState,
    compact,
...
from codify.runtime.content_resolver import resolve_content_references
...
from codify.spec import AgentSpec
from codify.spec.parser import check_unresolved_env_vars
...
from codify.stores import ConversationStore
...
# Translates Codify spec executor types (underscore) to harness subprocess names
...
def _get_runner_client_for_compaction(
    ...
    profile) instead of the Codify server's.
    ...
    Propagates :class:`CodifyError` when the pinned runner is
    ...
    if conversation_id is None:
        return None
    from codify.runtime import get_runner_router
...
# (``codify/onboarding/provider_config.py`` ``_HARNESS_FAMILY``);
...
_HARNESS_DATABRICKS_PROFILE: dict[AgentHarnessType, str] = {
    ...
    # so Codify cannot thread a Databricks gateway through. Users configure
    # providers via ``kimi provider add`` in ``~/.kimi/config.toml``
    # (Codify-side provider injection is a deferred follow-up).
}
...
def _provider_auth_command(family: FamilyConfig) -> str:
    ...
    :raises CodifyError: If the family carries neither a static
...
def configure_agent_harness_with_provider(
    ...
    *,
    ...
    of ``~/.codify/config.yaml``) and emits the **same** vendor-neutral
    ...
    - ``bedrock`` — rejected (raises): AWS Bedrock mode is wired only into
      the native ``codify claude`` launch, not the in-process / gateway
      harnesses.

    :param env: Mutable spawn-env dict, modified in place.
    :param entry: The resolved provider entry to apply.
    :param harness_type: Canonical harness type, e.g. ``"claude-sdk"``.
    :raises CodifyError: If an inline-family provider lacks the family
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is wired only into the native ``codify claude`` launch
        # (:func:`codify.claude_native._bedrock_config_for_native_claude`),
        ...
        raise CodifyError(
            ...
            f"native 'codify claude' terminal, not the {harness_type!r} harness. "
            "For agents / 'codify run', use a 'gateway' provider "
...
# Maps an codify provider family to the bundled catalog provider name
...
def _catalog_default_model(family_name: str) -> str:
    ...
    :func:`codify.onboarding.providers.default_chat_model` for the rule
    ...
    :param family_name: The codify family, ``"anthropic"`` or
        ``"openai"``.
    :returns: The live catalog's preferred model id.
    :raises CodifyError: If the family is unknown or discovery has no
        compatible model.
    """
...
def _apply_provider_family(
    ...
    :raises CodifyError: If no model is resolvable (neither the spec nor
...
def _optional_provider_family(entry: ProviderEntry, family_name: str) -> FamilyConfig | None:
    ...
    The only :class:`CodifyError` raised at family-access time comes from
...
def _apply_provider_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    :raises CodifyError: If no configured family's credentials resolve,
...
def _apply_cli_config_databricks_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    The gateway-harness pi launch (``codify run`` / agents) and pi-native
    ...
    (:func:`codify.pi_native_credentials._cli_config_pi_provider`) — which
    ...
    :param entry: The resolved ``cli-config`` provider entry (a Databricks
        ...
        reaches here).
    :raises CodifyError: If the cli-config entry cannot be translated into a
    ...
    from codify.pi_native_credentials import _cli_config_pi_provider
...
def _resolve_provider_for_build(
    ...
    *for_launch*) and the readout / cost / native paths (without). Precedence,
    ...
    :raises CodifyError: If a named :class:`ProviderAuth` references a
    ...
    if isinstance(auth, ProviderAuth):
        ...
        if entry is None:
            raise CodifyError(
                ...
                "configured under 'providers:' in ~/.codify/config.yaml. "
                "Run `codify setup --no-internal-beta` to configure one.",
                code=ErrorCode.INVALID_INPUT,
            )
        return entry
...
def _build_claude_sdk_spawn_env(
    ...
    *,
    ...
    vars defined in ``codify/inner/claude_sdk_harness.py``.
    Per the v1 spec-config-flow design (see §Step 5b in the
    design doc), per-spawn env overrides are how Codify threads
    ...
    # ``HARNESS_CLAUDE_SDK_CWD`` in ``codify/inner/claude_sdk_harness.py``.
    if cwd is not None:
    ...
    # 3. Global config ~/.codify/config.yaml auth: — only when spec has
    ...
    # for codify-style specs (the inner CLI auto-creates a
    # ``caller_process`` os_env when ``--os`` is set, and the
    # SDK's bundled CLI exposes the natives unconditionally in
    # some configurations); routing through Codify mode without a
    # similar default leaves Codify mode users staring at a
    ...
    # active and approval requests surface via Codify elicitation. Read from
...
def _apply_harness_path_override(
    ...
    """Thread a config ``harness.<canonical>.command`` into ``CODIFY_<NAME>_PATH``.

    The harness wraps read ``CODIFY_<NAME>_PATH`` to locate their vendor
    CLI (the headless CLI-subprocess family historically read ``HARNESS_*_PATH``;
    both are honored, ``CODIFY_*`` canonical). A user can set that path via
    ...
    from codify.harness_startup_config import (
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    defined in ``codify/inner/codex_harness.py``. Mirrors
    ...
    :func:`codify.inner.executor_factory.create_executor`
    ...
    on the Codify server directly (they propagate to the subprocess
    ...
    # ``HARNESS_CODEX_CWD`` in ``codify/inner/codex_harness.py``.
    if cwd is not None:
...
def _build_pi_spawn_env(
    ...
    *,
    ...
    defined in ``codify/inner/pi_harness.py``. Mirrors
...
def _build_qwen_spawn_env(
    ...
    *,
    ...
    defined in ``codify/inner/qwen_harness.py``. Mirrors
    :func:`_build_claude_sdk_spawn_env` /
    ...
    # harness fall back to CODIFY_RUNNER_WORKSPACE — see HARNESS_QWEN_CWD.
    if cwd is not None:
    ...
    # variants, the qwen wrap (codify/inner/qwen_harness.py) and
...
def _build_goose_spawn_env(
    ...
    *,
    ...
    ``codify/inner/goose_harness.py``. Unlike the SDK harnesses, Goose owns its
    ...
    # harness fall back to CODIFY_RUNNER_WORKSPACE — see HARNESS_GOOSE_CWD.
    if cwd is not None:
...
def _build_acp_spawn_env(
    ...
    *,
    ...
    ``HARNESS_ACP_*`` env vars defined in ``codify/inner/acp_harness.py``.
    ...
    # raises a clear request-time error pointing the user at `codify setup`.

    # Session workspace (selected working folder). ``None`` lets the acp
    # harness fall back to CODIFY_RUNNER_WORKSPACE — see HARNESS_ACP_CWD.
    if cwd is not None:
...
def _load_global_auth() -> ApiKeyAuth | DatabricksAuth | None:
    """
    Load the ``auth:`` block from ``~/.codify/config.yaml``.

    Reads the user-level global config file (respecting
    ``$CODIFY_CONFIG_HOME`` for test isolation) and parses the
    ...
    so the user only configures auth once during ``codify setup``
    ...
    :returns: A :class:`ApiKeyAuth` or :class:`DatabricksAuth`, or
        ...
        file is missing.
    """
    config_home = os.environ.get("CODIFY_CONFIG_HOME")
    path = (
        Path(config_home) / "config.yaml"
        if config_home
        else Path.home() / ".codify" / "config.yaml"
    )
    if not path.exists():
        return None
    try:
    ...
    if auth_type == "api_key":
        ...
        api_key = expand_envvars_with_codify_prefix(api_key)
        check_unresolved_env_vars("auth.api_key", api_key)
        raw_base_url = raw_auth.get("base_url")
        base_url: str | None = None
        if raw_base_url:
            base_url = expand_envvars_with_codify_prefix(str(raw_base_url))
...
def _build_openai_agents_sdk_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    env vars defined in
    ``codify/inner/openai_agents_sdk_harness.py``. Threads
    ...
    3. Global config ``~/.codify/config.yaml`` ``auth:`` block —
    ...
    if isinstance(auth, ApiKeyAuth):
        ...
        if not profile and model and model.startswith(("databricks-", "databricks/")):
            ...
            # `codify setup` provider config, never by shell environment.
            profile = "DEFAULT"
        if profile:
...
def _build_cursor_spawn_env(
    ...
    *,
    ...
    in ``codify/inner/cursor_harness.py``. Unlike the gateway-backed
    ...
    ``CURSOR_API_KEY`` registered once via ``codify setup`` (the dedicated
    ``cursor:`` config block — see :mod:`codify.onboarding.cursor_auth`) is
    ...
    # ``HARNESS_CURSOR_CWD`` in ``codify/inner/cursor_harness.py``.
    if cwd is not None:
    ...
    # ``codify setup`` (the dedicated ``cursor:`` config block), else an
    ...
    if isinstance(spec.executor.auth, ApiKeyAuth):
        ...
        from codify.onboarding.cursor_auth import resolve_cursor_api_key
...
def _build_kimi_spawn_env(
    ...
    *,
    ...
    defined in :mod:`codify.inner.kimi_harness`.
    ...
    :param cwd: Runtime working directory for the kimi subprocess — the
        ...
        When unset, the harness wrap falls back to ``CODIFY_RUNNER_WORKSPACE``.
        Mirrors :func:`_build_pi_spawn_env`'s ``cwd`` handling.
    :returns: A dict of env-var overrides.
    :raises CodifyError: If the spec declares ``executor.auth`` —
    ...
    if spec.executor.auth is not None:
        raise CodifyError(
            ...
            "resulting model id in the agent spec. Codify-side provider "
...
def _build_antigravity_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    dedicated ``antigravity:`` config block from ``codify setup``; (3) an
...
def _build_copilot_spawn_env(
    ...
    *,
    ...
    ``codify/inner/copilot_harness.py``. Like the cursor / antigravity
    ...
    no auth at all, a GitHub token registered once via ``codify setup`` (the
    dedicated ``copilot:`` config block — see
    :mod:`codify.onboarding.copilot_auth`) is used instead, so a user need not
    ...
    # ``HARNESS_COPILOT_CWD`` in ``codify/inner/copilot_harness.py``.
    if cwd is not None:
    ...
    # registered once via ``codify setup`` (the dedicated ``copilot:`` config
...
def _load_initial_history(
    ...
    # the empty synthetic pair — the Codify thinks context is ~0 tokens
...
async def compact_conversation_now(
    *,
    ...
    :returns: The :class:`CompactionResult` returned by
        :func:`codify.runtime.compaction.compact`.
    """
    ...
    if preserve_recent_window is not None:
        ...
        from codify.spec.types import CompactionConfig
    ...
    from codify.llms.context_window import get_model_context_window
...
def _route_bare_model_for_compaction(llm_config: LLMConfig) -> LLMConfig:
    ...
    whose :func:`~codify.llms.routing.parse_model_string` defaults any
...
def _maybe_persist_compaction_item(
    ...
    # session while the Codify thinks context is near-zero and never
...
def _find_spec_by_name(
    ...
    from codify.tools.builtins.web_fetch import RESEARCHER_NAME

    if name == RESEARCHER_NAME:
        owner = _find_web_fetch_owner(spec)
        if owner is not None:
            from codify.tools.builtins.web_fetch import build_researcher_spec
