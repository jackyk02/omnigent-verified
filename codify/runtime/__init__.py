...
from codify.runtime import _globals
from codify.runtime.caps import RuntimeCaps

if TYPE_CHECKING:
    from codify.runner.resource_registry import SessionResourceRegistry
...
def get_terminal_registry() -> TerminalRegistry:
    ...
    ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.2.
...
def set_runner_router(router: RunnerRouter | None) -> None:
    ...
    Production Codify servers use this instead of a process-wide runner
    client so each dispatch resolves the correct runner from
    ``conversations.runner_id`` and the live tunnel registry.

    :param router: :class:`codify.runner.routing.RunnerRouter`,
...
def get_runner_router() -> RunnerRouter | None:
    ...
    :returns: :class:`codify.runner.routing.RunnerRouter` or
        ``None``.
    """
...
def set_harness_process_manager(manager: HarnessProcessManager | None) -> None:
    ...
    ``codify/server/app.py`` after ``HarnessProcessManager.start()``.
...
def get_harness_process_manager() -> HarnessProcessManager:
    ...
    :raises RuntimeError: If the lifespan startup hasn't run yet
        (e.g. workflow invoked before Codify boot completed, or in a
    ...
    if pm is None:
        raise RuntimeError(
            "HarnessProcessManager not initialized — Codify lifespan startup "
