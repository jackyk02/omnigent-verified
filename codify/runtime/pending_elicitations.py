...
Lets the Codify server answer "which sessions have a pending approval
...
lifecycle: when the Codify process dies, both the index and every parked
...
:func:`codify.runtime.session_stream.publish` whenever a
...
* In-memory only; multi-replica Codify deploys would each see their own
...
* Events emitted before the Codify server starts (e.g. between turns,
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> None:
    ...
    * ``response.elicitation_resolved`` — drop the elicitation id
      ...
      Codify server picks it up via ``_relay_runner_stream`` without
...
def counts_for(conversation_ids: list[str]) -> dict[str, int]:
    ...
    :class:`codify.server.schemas.SessionListItem` in one
...
def project_for_peek(event: dict[str, Any]) -> dict[str, Any]:
    ...
    :class:`codify.tools.builtins.spawn.SysSessionGetHistoryTool`
