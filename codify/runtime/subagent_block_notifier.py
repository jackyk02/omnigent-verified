...
``codify/runner/app.py`` leaves the orchestrator idle and unaware. This
...
coroutine (kept free of HTTP/server knowledge) wired up by the Codify server
...
class SubagentBlockNotifier:
    ...
    One instance is constructed and registered per Codify server process.
    Its :meth:`observe` method is registered with
    :func:`codify.runtime.pending_elicitations.set_elicitation_observer`;
    ...
    :param loop: The Codify server's event loop, captured at registration
