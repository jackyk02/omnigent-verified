...
from codify.entities import Policy as StoredPolicy
...
from codify.policies.types import PolicyLLMClient
...
_USER_DAILY_COST_POLICY_PATH = "codify.policies.builtins.cost.user_daily_cost_budget"
...
_SUBAGENT_COST_POLICY_PATH = "codify.policies.builtins.cost.subagent_cost_budget"
...
_ASK_ON_ADD_POLICY_SPEC = FunctionPolicySpec(
    ...
    function=FunctionRef(
        path="codify.policies.builtins.safety.ask_on_add_policy",
        arguments=None,
    ),
)
...
def _load_user_daily_cost(
    ...
    """
    from codify.db.utils import now_epoch, utc_day
...
def build_policy_engine(
    *,
    ...
    :param server_llm: Server-level LLM configuration from
        ...
        :class:`~codify.policies.types.PolicyLLMClient` is
...
def _build_policy_llm_client(
    ...
    The :class:`~codify.llms.client.Client` is instantiated lazily
    ...
    if server_llm is None:
        return None
    from codify.llms.client import Client
...
def _resolve_databricks_connection(profile: str) -> dict[str, str]:
    ...
    Uses
    :func:`~codify.runtime.credentials.databricks.resolve_databricks_workspace`
...
def _load_default_policy_specs(
    ...
    :raises CodifyError: If an enabled policy has an unsupported type.
    """
    if policy_store is None:
        return []
    from codify.db.db_models import current_workspace_id
...
def invalidate_default_policy_specs_cache() -> None:
    ...
    workspace context via :func:`~codify.db.db_models.current_workspace_id`.
    """
    from codify.db.db_models import current_workspace_id
...
def _load_session_policy_specs(
    ...
    raises :class:`CodifyError` rather than being skipped, so a
    ...
    :raises CodifyError: If an enabled policy has an unsupported
        ``type`` (e.g. ``type="url"``).
    """
    if policy_store is None:
        return []
...
def _stored_policy_to_spec(policy: StoredPolicy) -> PolicySpec:
    ...
    For ``type="url"``, raises :class:`CodifyError` (URL policy evaluation
    ...
    :raises CodifyError: If the policy ``type`` cannot be evaluated yet
        (e.g. ``type="url"``).
    """
    if policy.type == "python":
