...
:class:`codify.server.schemas.ElicitationResult` and
...
from codify.policies.types import ElicitationRequest, PolicyResult
...
async def _await_elicitation(
    *,
    ...
    in :func:`codify.runtime.workflow._drive_policy_approval`.
    ...
    :class:`~codify.errors.ElicitationDeclinedError` so callers can
...
def _resolve_elicitation_mode() -> str:
    ...
    Reads ``CODIFY_ELICITATION_MODE`` (``"url"`` or ``"form"``).
    Defaults to ``"url"`` when unset — the standalone approval page
    is the default experience.

    :returns: ``"url"`` or ``"form"``.
    """
    raw = os.environ.get("CODIFY_ELICITATION_MODE", "url").lower().strip()
...
def build_elicitation_request_event(
    ...
    *,
    ...
    :param elicitation_id: Unique id correlating this request to the
        consumer's :class:`codify.server.schemas.ElicitationResult`
...
def _is_explicit_decline(raw: str | None) -> bool:
    ...
    :class:`~codify.errors.ElicitationDeclinedError` instead of
