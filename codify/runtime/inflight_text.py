...
Lets the Codify server replay the text streamed so far when a client
...
* :func:`codify.runtime.session_stream` is pure fan-out with no
...
:func:`codify.runtime.session_stream.publish` (the single SSE
chokepoint, same as :mod:`codify.runtime.pending_elicitations`):
...
the durable transcript. The index lives only in the Codify process, so it
...
``yield``/``await``*. On the single Codify event loop where the relay
...
# message_id is retired, and :func:`codify.runtime.session_stream.publish`
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> bool:
    ...
    * a ``session.status`` event whose status is terminal (see
    ...
    :param event: The event dict as passed to
        :func:`codify.runtime.session_stream.publish`. Reads
    ...
    :returns: ``True`` when the event must be WITHHELD from the live
        ...
        (:func:`codify.runtime.session_stream.publish`) must skip
...
def discard(conversation_id: str) -> None:
    ...
    Called from the Codify relay's teardown (``_relay_runner_stream``'s
