...
Never import this module outside of codify.runtime.
...
# ``codify.runtime``. Conversation-scoped: one entry per
...
# lifespan in ``codify/server/app.py``; accessed via
# ``get_harness_process_manager()`` in ``codify.runtime``.
...
# ``codify.runtime``), the runner routes
...
# in :mod:`codify.server.routes.terminal_attach` produces the
...
def init(
    *,
    ...
    # ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.2 for the design.
