...
from codify.llms.adapters._content import redact_inline_data_uris
...
class _CompactionState:
    ...
    :param conversation_id: Conversation id, e.g.
        ...
        the runner's credentials rather than the Codify server's.
    """
...
async def summarize_history(
    ...
    runner's credentials are used instead of the Codify server's. Falls
...
async def _summarize_via_runner_uncached(
    ...
    so the Codify server needs no LLM auth for this call.
...
def compaction_to_history_items(
    ...
    :param compaction_item: The compaction item from the store,
        ...
        ``data`` of type :class:`~codify.entities.CompactionData`.
    :returns: Two :class:`~codify.entities.ConversationItem`
...
async def compact(
    ...
    *messages* copy passed to the LLM is compacted.
    ...
    :param runner_client: Optional ``httpx.AsyncClient`` pointed at
        ...
        credentials are used instead of the Codify server's. ``None``
