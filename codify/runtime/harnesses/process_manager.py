...
orphan sweep so a previous Codify crash doesn't leave runner processes
behind.

This module knows nothing about the harness API — it spawns a
:mod:`codify.runtime.harnesses._runner` subprocess per
...
:mod:`codify.server.schemas`; everything FastAPI-shaped is in
...
from codify.harness_plugins import missing_install_packages
from codify.inner import _proc
from codify.inner._subprocess_lifecycle import close_subprocess_transport
from codify.runner.identity import strip_runner_auth_secrets
from codify.runtime.harnesses import _HARNESS_MODULES

_logger = logging.getLogger(__name__)

# Per-AP-instance directory holding all per-conversation Unix
# sockets (POSIX) and the AP_PID sentinel file. Each Codify instance gets a
# uuid-named subdir so concurrent Codify processes (zero-downtime restarts,
# multi-tenant single-machine deployments) don't step on each other.
#
# POSIX pins ``/tmp/codify-<uid>`` deliberately: Unix socket paths have a
...
# ``/tmp/codify`` there resolves to ``\tmp\codify`` on the current
# drive — so use the real (already per-user) temp dir.
if IS_WINDOWS:
    _TMP_PARENT = Path(tempfile.gettempdir()) / "codify"
else:
    _TMP_PARENT = Path(f"/tmp/codify-{os.getuid()}")
_TMP_PARENT_ENV_VAR = "CODIFY_HARNESS_TMP_PARENT"
...
# depth.
_HARNESS_AUTH_TOKEN_ENV = "CODIFY_HARNESS_AUTH_TOKEN"

# Sentinel file the Codify instance writes into its subdir on boot. The
# orphan sweep uses it to tell whether a sibling subdir belongs to
# a still-running Codify (leave alone) or a crashed one (kill its
...
_HARNESS_IDLE_TIMEOUT_ENV = "CODIFY_HARNESS_IDLE_TIMEOUT_S"


def _resolve_harness_idle_timeout_s() -> float:
    """Resolve the harness idle-reap window in seconds.

    Honors :envvar:`CODIFY_HARNESS_IDLE_TIMEOUT_S` (``0`` disables reaping);
...
# enough that a wedged subprocess doesn't block Codify shutdown.
...
# or model-thinking gaps. Codify doesn't bound harness-side latency —
...
def _default_tmp_parent() -> Path:
    ...
    Operators can set :envvar:`CODIFY_HARNESS_TMP_PARENT` when
...
def _socket_path(instance_dir: Path, conversation_id: str) -> Path:
    ...
    :param instance_dir: This Codify instance's directory, e.g.
        ``/tmp/codify/ap-abc123``.
...
def _resolve_module_path(harness: str) -> str:
    ...
    raise RuntimeError(
        ...
        f"codify.runtime.harnesses._HARNESS_MODULES)"
    )
    ...
    while True:
        if process.returncode is not None:
            ...
            raise RuntimeError(
                ...
                f"{process.returncode} during spawn (see Codify stderr)"
            )
...
class _SubprocessEntry:
    ...
    :param client: The :class:`httpx.AsyncClient` Codify uses to talk
        to this subprocess over its Unix socket.
    :param socket_path: Absolute Unix socket path the runner
        bound, e.g.
        ``Path("/tmp/codify/ap-abc/conv-xyz.sock")``.
...
class HarnessProcessManager:
    ...
    Use ``start()`` once at Codify boot to create the per-instance
    ...
    terminal state, and ``shutdown()`` at Codify shutdown to release
    ...
    :param reaper_interval_s: Seconds between idle-reaper passes.
        Defaults to 60.
    :param tmp_parent: Override for the parent ``/tmp/codify``
        ...
        :envvar:`CODIFY_HARNESS_TMP_PARENT` to choose a
    ...
    def __init__(
        ...
        *,
        ...
        # CODIFY_HARNESS_IDLE_TIMEOUT_S env var, else the 1-hour default.
    ...
    def instance_dir(self) -> Path:
        """
        This Codify instance's per-instance directory.

        :returns: Path like ``/tmp/codify/ap-<uuid>``.
        """
    ...
    async def forward_cancel(
        ...
        :param conversation_id: The Codify conversation id whose
    ...
    async def release(
        ...
        conversation ids the Codify instance has ever spawned for. If
    ...
    async def shutdown(self) -> None:
        ...
        # rmtree leaves it behind; the next Codify boot's orphan
        # sweep handles it.
    ...
    async def _spawn_entry(
        ...
        # with Codify logs. The runner doesn't currently prefix lines
        ...
        process = await asyncio.create_subprocess_exec(
            sys.executable,
            "-m",
            "codify.runtime.harnesses._runner",
            "--harness",
            harness,
            "--module",
            module_path,
    ...
    async def _idle_reaper_loop(self) -> None:
        ...
        ``CODIFY_HARNESS_IDLE_TIMEOUT_S=0``) disables reaping — each
        pass is a no-op.
        ...
        while True:
            ...
            # (``CODIFY_HARNESS_IDLE_TIMEOUT_S=0``). Without this guard a 0
    ...
    async def _sweep_orphans(self) -> None:
        ...
        for child in self._tmp_parent.iterdir():
            ...
            try:
                ...
                continue
            if _pid_alive(pid):
                # Sibling Codify is still running — leave it alone.
                continue
            _logger.info(
                "sweeping orphaned Codify instance dir %s (pid %d not running)",
                child,
                pid,
            )
