...
Invoked by :class:`codify.runtime.harnesses.process_manager.HarnessProcessManager`
...
- ``--module <python_path>``: importable module that exports
  ...
  :data:`codify.runtime.harnesses._HARNESS_MODULES` before
...
- ``--conversation-id <id>``: the conversation this process serves.
  ...
  doc. Codify allocates the id; the runner does NOT parse it from the
...
_GRACEFUL_SHUTDOWN_TIMEOUT_S = float(os.environ.get("CODIFY_HARNESS_SHUTDOWN_TIMEOUT_S", "5"))
...
_HARD_EXIT_TIMEOUT_S = float(
    os.environ.get(
        "CODIFY_HARNESS_HARD_EXIT_TIMEOUT_S",
...
def _load_harness_app(harness: str, module_path: str, conversation_id: str) -> FastAPI:
    ...
    :param module_path: Fully-qualified Python module to import,
        e.g. ``"codify.inner.claude_sdk_harness"``. Must
    ...
    # (``CODIFY_HARNESS_AUTH_TOKEN``); the scaffold's auth check compares
    ...
    # the gate inert.
    app.state.harness_auth_token = os.environ.get("CODIFY_HARNESS_AUTH_TOKEN")
    return app
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    :param argv: Argument list (without the program name),
        ...
        "codify.inner.claude_sdk_harness", "--socket",
        "/tmp/codify/<id>/conv-abc.sock", "--conversation-id",
    ...
    parser = argparse.ArgumentParser(
        prog="python -m codify.runtime.harnesses._runner",
        description=(
