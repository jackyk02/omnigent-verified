...
- Per-turn translation of Codify :class:`CreateResponseRequest` →
  inner :class:`Message` list + :class:`ExecutorConfig`.
- Per-turn translation of inner :class:`ExecutorEvent`s →
  typed Codify SSE events emitted via :meth:`TurnContext.emit`.
...
from codify.errors import ElicitationDeclinedError
...
from codify.inner.tracing import TracingContext, is_tracing_enabled
from codify.policies.types import FAIL_CLOSED_PHASES
from codify.runtime.harnesses._scaffold import HarnessApp, PolicyVerdictPayload, TurnContext
...
# (e.g. ``mcp__codify__sys_terminal_launch``). Tools whose
...
def _strip_mcp_tool_prefix(name: str) -> str:
    ...
    (e.g. ``mcp__codify__sys_terminal_launch``). The bare
    name (``sys_terminal_launch``) is what the Codify wire shape
    and persisted conversation items carry — kept in sync with
    :func:`codify.runtime.workflow._observed_tool_call_sse_dicts`
    and :func:`_build_observed_tool_items` so the SSE name, the
    store-item name, and the adapter's emitted name all agree.

    Mirrors :func:`codify.runtime.workflow._strip_mcp_tool_prefix`
...
class ExecutorAdapter(HarnessApp):
    ...
    def __init__(
        ...
        # already carried — that's what lets the Codify REPL dedupe
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        ...
        can round-trip spec-declared tools through Codify via the
        ...
        # Codify server (routed through the runner).
        ...
        from codify.runtime.telemetry import current_session_id, session_scope
        ...
        try:
            ...
            if tctx:
                try:
                    from codify.runtime.telemetry import trace_context_for_response
                ...
                async for event in executor.run_turn(
                    ...
                    if ctx.cancelled.is_set():
                        if tctx is not None and agent_span is not None:
                            from codify.runtime.telemetry import record_cancellation
                    ...
                    if tctx is not None:
                        if isinstance(event, ToolCallRequest):
                            ...
                            if event.usage is not None:
                                from codify.runtime.telemetry import record_llm_usage
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        :raises CodifyError: 404 (from the base handler) when no turn is
            in flight.
        """
    ...
    async def _stable_tool_executor(
        ...
        :param tool_name: Tool name from the LLM's call. Carries
            ...
            ``"mcp__codify__sys_terminal_launch"``).
        ...
        # strips the ``mcp__codify__`` prefix before
    ...
    async def _stable_elicitation_handler(
        ...
        Called by :class:`codify.inner.claude_sdk_executor.ClaudeSDKExecutor`
    ...
    async def _stable_policy_evaluator(
        ...
        :returns: The policy verdict from the Codify server.
        """
        ctx = self._current_ctx
        if ctx is None:
            ...
            # the runner's phase-aware default in _evaluate_policy_via_codify —
    ...
    def _translate_event(self, event: ExecutorEvent, ctx: TurnContext) -> None:
        """
        Translate one inner :class:`ExecutorEvent` into Codify SSE
        ...
        if isinstance(event, TextChunk):
            ...
            # Translate inner reasoning to the Codify wire shape so the
            ...
            # (``mcp__codify__sys_terminal_launch``) but the
            # Codify wire shape and persisted conversation items
            # carry the bare form — kept consistent with
            # ``codify/runtime/workflow.py``'s
            ...
            # payload to populate TurnComplete.usage on the Codify side.
            if event.usage is not None:
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        (:data:`codify.runtime.harnesses._client_executor._RETRYABLE_HARNESS_ERROR_CODES`).
        ...
        1. :class:`codify.errors.CodifyError` (incl.
        ...
        :returns: An :class:`ErrorDetail` whose ``code`` matches
            the Codify allowlist for known retryable failures, and is
        ...
        if isinstance(exception, CodifyError):
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    """
    Map an OpenAI SDK exception onto the Codify semantic code allowlist.
    ...
    extending the allowlist would just produce strings Codify ignores.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the Codify allowlist (e.g.
    ...
    from codify.llms.errors import is_context_length_exceeded
...
def _classify_claude_sdk_exception(exception: BaseException) -> str | None:
    """
    Map a :mod:`claude_agent_sdk` exception onto the Codify semantic
    code allowlist.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the Codify allowlist, or ``None``
...
def _classify_httpx_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`httpx` exception onto the Codify semantic code allowlist.
    ...
    # no semantic code, and the Codify retry classifier treats them
    # as permanent.
...
def _classify_anthropic_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`anthropic` SDK exception onto the Codify semantic
    code allowlist.
    ...
    ``[llm] APIConnectionError`` etc., which the Codify retry
...
def classify_inner_exception(exception: BaseException) -> str | None:
    """
    Map any inner-SDK exception onto the Codify semantic code allowlist.
...
async def _bridge_one_dispatch(
    ...
    *,
    ...
    # Try to parse the output as JSON — if Codify serialized the
...
def _extract_role_keyed_messages(
    ...
    """
    Pull role-keyed message items out of an Codify ``input`` list.
    ...
    :param input_value: The Codify ``input`` list.
...
def _serialize_tool_result(event: ToolCallComplete) -> str:
    ...
    - A list of typed content blocks (``{"type": "text", "text":
      ...
      the LLM-rendered tool output reaches Codify intact. Without
    ...
    - Empty string is a last resort — the inner Executor
      ...
      logged so a regression surfaces in the Codify logs.
