...
from codify import _native_forwarder_health as native_forwarder_health
...
# streaming. Matches the existing Codify cadence at
# ``codify/runtime/workflow.py:3732`` — 15 seconds is short
...
# Codify server → runner → harness). Held at one day (86400s) — matching
...
class PolicyVerdictPayload:
    ...
    evaluates the policy on the Codify server and sends the verdict back
...
class TurnContext:
    ...
    :param response_id: Server-allocated id for this turn,
        ...
        ``response.created`` envelope so Codify can correlate
    ...
    def emit(self, event: HarnessStreamEvent) -> None:
        ...
        :param event: A typed event from
            :data:`codify.server.schemas.ServerStreamEvent`,
            e.g.
    ...
    async def dispatch_tool(self, call_id: str, name: str, arguments: str, agent: str) -> str:
        ...
        the parked Future when Codify delivers an event carrying the
        matching ``call_id``. See §How Codify resolves action_required
        tool calls in the design doc.

        :param call_id: Unique identifier for this tool call,
            e.g. ``"call_abc123"``. Codify echoes it back in the
        ...
        :param agent: Agent name that invoked the tool — required
            ...
            :class:`codify.entities.conversation.FunctionCallData`.
        :returns: The tool's output string from
            :class:`codify.server.schemas.ToolResult`.
        ...
        # expects (per codify/entities/conversation.py:FunctionCallData
    ...
    async def evaluate_policy(
        ...
        :returns: The verdict payload from the Codify server.
        :raises asyncio.CancelledError: If the turn is cancelled.
        """
...
class HarnessApp:
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        :class:`codify.runtime.harnesses._executor_adapter.ExecutorAdapter`)
    ...
    def build(self) -> FastAPI:
        ...
        :returns: A configured :class:`FastAPI` instance ready to
            be served by ``codify.runtime.harnesses._runner``.
        """
        app = FastAPI(title="codify-harness", lifespan=self._lifespan)
        ...
        # (Callable[[Request, Exception], ...]) — the CodifyError
        # subclass annotation is what we actually want at runtime, but
        # the static type doesn't accept it. Cast at the boundary.
        app.add_exception_handler(CodifyError, _handle_codify_error)  # type: ignore[arg-type]
    ...
    def _check_conversation_id(self, request: Request, conversation_id: str) -> None:
        ...
        ``codify/runtime/harnesses/_runner.py``). Any
        session-keyed request whose path does not match is
        addressed at the wrong subprocess; failing with 404 is the
        same shape the Codify side uses for unknown conversations
        (per ``codify/server/routes/sessions.py``).
        ...
        :raises CodifyError: 500 if ``app.state.conversation_id``
    ...
    async def _post_session_event(
        ...
        :raises CodifyError: 404 on conversation_id mismatch,
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        :raises CodifyError: 404 if no turn is in flight.
        """
    ...
    async def _start_or_inject_turn(
        ...
        :returns: Either a :class:`StreamingResponse` for the new
            ...
            injection.
        :raises CodifyError: 503 on shutdown.
        """
        if (
    ...
    async def _build_terminal_event(
        ...
        if status_value == "completed":
            ...
            from codify.server.schemas import CancelledEvent
    ...
    async def _resolve_elicitation(
        ...
        :returns: 204 on success.
        :raises CodifyError: 404 if no pending elicitation
...
# subclass — see
# :meth:`codify.runtime.harnesses._executor_adapter.ExecutorAdapter._build_error_detail`.
...
async def _handle_codify_error(_request: Request, exc: CodifyError) -> Response:
    """
    Convert :class:`CodifyError` into a JSON response with the
    correct HTTP status.

    Mirrors AP's exception handler at
    ``codify/server/app.py``.
