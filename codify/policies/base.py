...
the engine (in :mod:`codify.runtime.policies.engine`) does
the filter-gate-dispatch-compose orchestration (see
POLICIES.md §4).

The two concrete subclasses live next to this module:

- :class:`codify.policies.function.FunctionPolicy`
- :class:`codify.policies.prompt.PromptPolicy`
...
from codify.spec.types import PolicySpec
...
class Policy(ABC):
    ...
    async def evaluate(
        ...
        :param context: Read-only context bundle from the
            ...
            with codify' signature (see POLICIES.md §9.1).
    ...
    def reset_turn(self) -> None:  # noqa: B027 — intentional no-op default (see docstring)
        ...
        turn. Mirrors :meth:`codify.runtime.policies.engine.PolicyEngine.reset_turn`.

        The runtime calls this once per "turn" — defined as one
        user prompt → terminal assistant response cycle, which
        in codify corresponds to one ``_run_agent_loop``
        ...
        as :meth:`codify.runtime.policies.engine.PolicyEngine.reset_turn`.
        """
