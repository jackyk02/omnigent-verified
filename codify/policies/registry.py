...
Usage::
    ...
    from codify.policies.registry import load_registry, get_registry
    ...
    schema = get_params_schema("codify.policies.builtins.safety.max_tool_calls_per_turn")
"""
...
class PolicyRegistryEntry:
    ...
    :param handler: Full dotted import path to the callable,
        e.g. ``"codify.policies.builtins.safety.ask_on_os_tools"``.
...
def load_registry(
    ...
    """
    from codify.policies.builtins import BUILTIN_POLICY_MODULES
...
def is_registered_handler(handler: str) -> bool:
    ...
    (:func:`codify.server.bundles.validate_agent_bundle`, which scans
    parsed specs / raw bundle YAML before any handler is resolved or
    called). Trusted spec loading (local ``codify run``, operator
...
def get_params_schema(handler: str) -> dict[str, object] | None:
    ...
    :param handler: Full dotted import path, e.g.
        ``"codify.policies.builtins.safety.max_tool_calls_per_turn"``.
