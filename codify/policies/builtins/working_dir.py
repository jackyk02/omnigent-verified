...
YAML usage::
    ...
    policies:
      no_dir_switch:
        type: function
        function:
          path: codify.policies.builtins.working_dir.block_working_dir_changes
          arguments:
...
from codify.policies.builtins._shell import (
    MAX_SHELL_NESTING,
...
# Includes both the Codify built-in OS shell and Claude Code / Codex
...
def block_working_dir_changes(
    *,
    ...
    :param shell_tools: Names of the shell tools whose ``command`` argument is
        ...
        (Codify built-in + Claude/Codex native). Commands run through
...
POLICY_REGISTRY: list[dict[str, Any]] = [  # type: ignore[explicit-any]
    {
        "handler": "codify.policies.builtins.working_dir.block_working_dir_changes",
        "kind": "factory",
