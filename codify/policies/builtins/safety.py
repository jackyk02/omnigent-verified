...
# PostToolUse hook contract (see ``codify.native_policy_hook``).
# These bypass Codify' ``sys_os_*`` MCP tools and execute directly
...
# (see ``codify.inner.cursor_policy_hook``). Cursor uses ``Shell``
...
# extension hook (see ``codify.inner.pi_executor._gate_native_tool``).
...
# as the Codify ``sys_os_*`` tools (``path`` / ``command``), so the
...
# (see ``codify.inner.hermes_policy_hook``). Hermes uses its own naming
...
def ask_on_os_tools(event: PolicyEvent) -> PolicyResponse:
    ...
    - **Codify built-in OS tools** (``sys_os_read``,
    ...
    if tool in _all_os_tools:
        ...
        if tool in ("sys_os_shell", "Bash", "bash", "Shell", "terminal", "developer__shell"):
            ...
            # Codify tools use ``path``; Claude native tools use ``file_path``.
            preview = (
...
# Codify runner tools that load skills in non-native (SDK) harnesses.
...
def block_skills(blocked: list[str]) -> PolicyCallable:
    ...
    2. **Native ``Skill`` tool** — Claude Code's built-in ``Skill`` tool,
       ...
       the Codify server's ``/policies/evaluate`` endpoint. This is how
    ...
    3. **Slash commands** — ``/skill-name`` commands submitted by the
       user (or UI). The Codify server evaluates these at the ``request``
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        if event_type == "tool_call":
            ...
            # Path 1: Codify runner tools (load_skill / read_skill_file).
            ...
            # Fired via PreToolUse hook → Codify /policies/evaluate.
        ...
        # The Codify server converts ``/skill-name args`` into a synthetic
...
def deny_pii_in_llm_request(
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        - ``request``: user message text, enforced universally on
          the Codify server for every harness (including supervisor,
          native).
...
POLICY_REGISTRY: list[dict[str, object]] = [
    ...
    {
        "handler": "codify.policies.builtins.safety.detect_loop",
        "kind": "factory",
    ...
    },
    {
        ...
        "covers Codify sys_os_* tools, Claude Code native tools "
    ...
    {
        "handler": "codify.policies.builtins.safety.block_skills",
        "kind": "factory",
    ...
    {
        "handler": "codify.policies.builtins.safety.enforce_sandbox",
        "kind": "factory",
    ...
    {
        "handler": "codify.policies.builtins.safety.deny_pii_in_llm_request",
        "kind": "factory",
