...
(``codify/runner/policy.py``) and add no server routes.
"""
...
def spawn_bounds(
    *,
    ...
    hook the runner calls (``codify/runner/policy.py``). This is the v1
...
def worktree_guard(
    *,
    ...
    # Match Codify built-in OS write/edit, Claude/Codex native Write/Edit
    # (surfaced via the PreToolUse hook), and Pi's native lowercase
    # write/edit (surfaced via the pi ``tool_call`` hook). Pi uses the same
    # ``path`` argument key as the Codify tools, so no Pi-specific arg
...
POLICY_REGISTRY: list[dict[str, object]] = [
    {
        "handler": "codify.policies.builtins.orchestration.blast_radius",
        "kind": "factory",
        ...
        "force-push or rm -rf /. Supports Codify, "
        "Claude/Codex, Cursor, Pi, Hermes, and Goose.",
        "params_schema": {
            "type": "object",
            "properties": {
                "gate_pushes": {
    ...
    {
        "handler": "codify.policies.builtins.orchestration.spawn_bounds",
        "kind": "factory",
    ...
    {
        "handler": "codify.policies.builtins.orchestration.headless_subagent_purpose_guard",
        "kind": "factory",
    ...
    {
        "handler": "codify.policies.builtins.orchestration.worktree_guard",
        "kind": "factory",
    ...
    {
        "handler": "codify.policies.builtins.orchestration.read_only_os",
        "kind": "factory",
