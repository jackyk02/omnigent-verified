...
:mod:`codify.inner.loader` — ``handler`` + ``factory_params``)::
...
:mod:`codify.spec.parser` — note the different spelling: a
    ...
    guardrails:
      policies:
        block_shell:
          type: function
          function:
            path: codify.policies.builtins.cel.cel_policy
            arguments:
              expression: 'event.type == "tool_call" && event.data.name == "sys_os_shell"'
              reason: Shell access is blocked.
...
def cel_policy(
    *,
    ...
    :param expression: CEL expression evaluated per policy event.
        ...
        :class:`~codify.policies.schema.PolicyEvent` dict.
