...
class FunctionPolicy(Policy):
    ...
    def reset_turn(self) -> None:
        ...
        Stateful policy callables (notably the legacy codify
        ...
        no-op. Modern codify-native callables that need
...
def resolve_function_policy(spec: FunctionPolicySpec) -> FunctionPolicy:
    ...
    # function.py → _codify_legacy_shim → policies.types →
...
def _coerce_to_policy_result(raw: object, *, spec_name: str) -> PolicyResult:
    ...
    - A foreign PolicyResult-shaped object (any object with at
      least ``.action``, e.g. ``codify.policies.PolicyResult``
      returned by callables ported from the codify examples).
    ...
    if hasattr(raw, "action"):
        ...
        # ``codify.policies.PolicyResult`` that happen to
