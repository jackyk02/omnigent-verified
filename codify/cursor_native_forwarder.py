...
The ``codify cursor`` wrapper launches the real ``cursor-agent`` TUI in a
runner-owned tmux pane, and :mod:`codify.cursor_native_bridge` injects web-UI
messages into it. That covers the web→TUI direction, but the *embedded terminal*
is then the only surface that reflects the agent's work — the Codify
...
:mod:`codify.claude_native_forwarder` (which tails Claude Code's JSONL
transcript) and :mod:`codify.codex_native_forwarder` (which subscribes to the
...
cursor-native (see ``_publish_turn_status`` in :mod:`codify.runner.app`),
...
:mod:`codify.cursor_native_status`) — the SAME server contract
...
from codify import cursor_native_status
from codify._native_post_delivery import post_may_have_been_delivered
from codify.cursor_native_bridge import FORK_HISTORY_CLOSE_TAG, FORK_HISTORY_OPEN_TAG
from codify.inner.native_attachments import ATTACHMENT_MARKER_STRIP_PATTERN
...
#: ``codify.db.db_models.SqlConversationItem``). cursor's content-address blob
...
# first user message, fenced in <codify_fork_history>…</codify_fork_history>
...
# real text — the copied history already lives in the Codify timeline, so
...
def _read_last_used_model(store_path: Path) -> str | None:
    ...
    :func:`codify.cursor_native_bridge.inject_model_command`).
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    un-pinned session falls back to codify's default (e.g. "fable") in the Web
...
async def _patch_external_session_id(
    ...
    """PATCH the Codify session with the cursor chat id for cold-resume.
...
async def _post_external_session_status(
    ...
    (:func:`codify.cursor_native_status.record_turn_end`); this turns that
...
async def _post_external_compaction_status(
    ...
    :func:`codify.claude_native_forwarder._post_external_compaction_status`.
...
async def supervise_cursor_forwarder(
    *,
    ...
    Mirrors :func:`codify.claude_native_forwarder.supervise_forwarder`: the
