...
The ``codify qwen`` wrapper launches the real ``qwen`` TUI in a runner-owned
tmux pane with ``--json-file`` pointed at the bridge dir, and
:mod:`codify.qwen_native_bridge` appends web-UI messages to its ``--input-file``.
That covers the web→TUI direction, but the *embedded terminal* is then the only
surface that reflects the agent's work — the Codify conversation view stays
empty because nothing mirrors the transcript back into the session.

This module is that missing mirror — the qwen analog of
:mod:`codify.goose_native_forwarder`. Where goose has to scrape a SQLite store,
...
- ``{"type":"control_request","request":{"subtype":"can_use_tool",...},
  ...
  (:mod:`codify.qwen_native_permissions`) tails the same stream and surfaces
...
:mod:`codify.runner.app`), exactly as for goose-/cursor-native.
...
from codify.qwen_native_bridge import events_file_path
...
class _ForwardState:
    ...
    :param offset: Byte offset into the ``--json-file`` already consumed. The
        ...
        truncates it (see :func:`~codify.qwen_native_bridge.prepare_bridge_files`),
...
def _event_to_item(event: dict[str, object], agent_name: str) -> _MirrorItem | None:
    ...
    if etype not in ("user", "assistant"):
        ...
        # (codify.qwen_native_permissions) owns them off the same stream.
        return None
    uuid = event.get("uuid")
...
async def supervise_qwen_compaction_mirror(
    *,
    ...
    :param recording_path: qwen's chat recording for this session (see
        :func:`codify.qwen_native_bridge.qwen_session_recording_path`).
    """
    try:
