"""
Fast ``MessageDisplay`` hook for the native Codify Claude wrapper.
...
and :mod:`codify.claude_native_forwarder`.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"codify message-display hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify message-display hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    try:
        ...
        print(f"codify message-display hook: write failed: {exc}", file=sys.stderr)
        return 0
    return 0
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m codify.claude_native_message_display_hook")
