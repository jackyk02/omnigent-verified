...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "kiro-native"
_TMUX_FILE = "tmux.json"
_FORWARDER_READY_FILE = "kiro_session_forwarder_ready.json"
_ACP_RECORD_FILE = "kiro_acp_record.jsonl"
# Shared Codify MCP relay (serve-mcp) registration for kiro.
...
_PASTE_BUFFER = "codify-kiro-paste"
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared Codify MCP bridge requires at boot.

    ``serve-mcp`` (``codify.claude_native_bridge``) reads ``bridge.json`` and
...
def build_kiro_mcp_config(
    ...
    """Build the kiro ``mcpServers`` entry for the Codify relay MCP server.
    ...
    per-server auto-approve field, so Codify MCP tool calls surface through
...
def write_kiro_workspace_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped kiro MCP config declaring the Codify server.
    ...
    ``.cursor/mcp.json``). The Codify entry is *merged* into any existing
...
def inject_interrupt(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    :func:`codify.goose_native_bridge.inject_interrupt`.
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`codify.goose_native_bridge.kill_session`.
