"""Translate the codify-configured model provider into native Pi config.

A native Pi session launches the ``pi`` CLI, which authenticates from its own
config directory (``~/.pi/agent``). Without help, a user who ran ``codify
setup`` would still have to run ``pi`` ``/login`` separately — unlike
claude-native / codex-native, which route through the provider that ``codify
setup`` configured.

This module closes that gap. It resolves the provider configured for the Pi
surface (``~/.codify/config.yaml``) and writes a per-session ``models.json``
...
from codify.model_metadata import ModelWireAPI
from codify.model_override import normalize_model_for_provider
...
from codify.pi_model_compatibility import (
    SYSTEM_AI_RESPONSES_KEYWORDS,
    unsupported_in_pi,
)
from codify.runtime.credentials.databricks import resolve_databricks_workspace
...
if TYPE_CHECKING:
    ...
    from codify.onboarding.ambient import CodexConfigTransport
...
_PI_OPENAI_PROVIDER_ID = "codify-openai"
...
_PI_COMPLETIONS_PROVIDER_ID = "codify-completions"
_PI_MLFLOW_PROVIDER_ID = "codify-mlflow"
...
def _databricks_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    from codify.inner.codex_executor import _databricks_codex_auth_command
    from codify.inner.databricks_executor import _read_databrickscfg_host
...
def _cli_config_databricks_transport(entry: ProviderEntry) -> CodexConfigTransport | None:
    ...
    :class:`~codify.onboarding.ambient.CodexConfigTransport` when so, else
    ...
    from codify.onboarding.ambient import (
        _codex_config_path,
...
def cli_config_pi_provider_capable(entry: ProviderEntry) -> bool:
    ...
    (:mod:`codify.onboarding.provider_config`) consults to decide whether a
...
def _cli_config_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    into ``~/.codex/config.toml`` and ``codify setup`` adopts it as a
...
def resolve_pi_native_provider(
    *,
    ...
    """Resolve the codify-configured provider for a native Pi session.

    Reads the default provider for the Pi surface from
    ``~/.codify/config.yaml`` and translates it into Pi ``models.json``
    ...
    try:
        ...
        # Pi is multi-family; ``codify setup`` marks defaults per family, not
        ...
        if entry is None:
            _LOGGER.info(
                "pi-native: no codify-configured provider for the pi/anthropic/openai "
        ...
        if resolved is None:
            ...
            from codify.onboarding.provider_config import _parse_provider
        ...
        _LOGGER.warning(
            "pi-native: failed to resolve the codify-configured provider; Pi will "
            "use its own login.",
            exc_info=True,
        )
        return None
...
def pi_native_provider_launch(
    ...
    # (GLM, GPT, Llama…) are in additional_providers (codify-openai);
    # Claude models are in the primary provider (codify). Pass the correct
