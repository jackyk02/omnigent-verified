...
:func:`codify.claude_native._claude_terminal_request` (local CLI) and
``_auto_create_claude_terminal`` in :mod:`codify.runner.app` (managed-host
...
:data:`CLAUDE_LAUNCHER_ENTRY_POINT_GROUP` group::
    ...
    [project.entry-points."codify.claude_launcher"]
...
environment the runner runs in -- no Codify code change, no in-tree import
path. At launch time, the ``CODIFY_CLAUDE_LAUNCHER`` environment variable
selects *which* registered launcher to use, by entry-point name (e.g.
``CODIFY_CLAUDE_LAUNCHER=isaac``). Unset -> default launch. The selected
...
the command preserves the Codify bridge unchanged.
...
CLAUDE_LAUNCHER_ENV_VAR = "CODIFY_CLAUDE_LAUNCHER"

#: setuptools entry-point group launcher plugins register themselves in.
CLAUDE_LAUNCHER_ENTRY_POINT_GROUP = "codify.claude_launcher"
...
class ClaudeLauncher(abc.ABC):
    ...
    Codify instantiates the subclass (no-arg constructor) and calls
    ...
    def launch(self, command: str, args: list[str]) -> tuple[str, list[str]]:
        ...
        :param command: Default terminal command Codify would otherwise spawn,
            e.g. ``"claude"``.
        :param args: Fully-augmented Claude CLI args (MCP config, hook settings
            ...
            the Codify bridge.
        :returns: The ``(command, args)`` Codify should spawn instead.
        """
