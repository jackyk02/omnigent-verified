...
Mirrors :mod:`codify.codex_native_app_server` but for OpenCode's HTTP +
...
is runner-internal — the web UI attaches to Codify terminal resources,
...
# try an as-yet-unvalidated 1.18+/v2 release). Mirrors CODIFY_NO_UPDATE_CHECK.
_SKIP_VERSION_CHECK_ENV = "CODIFY_OPENCODE_SKIP_VERSION_CHECK"
...
def list_opencode_cli_model_options(
    *,
    ...
    Codify picker and fall back to the server API if it fails.
...
def filtered_server_env(
    *,
    ...
    :param extra_env: Additional provider env (e.g. from Codify setup).
