"""Per-session ``KIMI_CODE_HOME`` builder that injects Codify hooks.
...
- carries a ``config.toml`` that is the user's config text with two Codify
  ``[[hooks]]`` appended — a ``PreToolUse`` deny-gate and a ``PermissionRequest``
  read-only surface, both dispatched to :mod:`codify.kimi_native_hook`.
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    """Render the two Codify ``[[hooks]]`` entries as TOML text.

    Both hooks dispatch to :mod:`codify.kimi_native_hook` with the bridge
    ...
    :param python_executable: Interpreter to run the hook module; ``None`` uses
        :data:`sys.executable` (the runner's interpreter, which has codify).
    ...
    # ``sys.path[0]``. A workspace that contains its own ``codify/`` directory
    ...
    # from the path, importing only the interpreter's own codify. Mirrors
    # claude-native's ``python -I -m codify.claude_native_hook``.
    base = f"{shlex.quote(python)} -I -m codify.kimi_native_hook"
    ...
    return (
        "\n"
        "# --- Codify native hooks (auto-generated; do not edit) ---\n"
        "[[hooks]]\n"
        'event = "PreToolUse"\n'
        f'command = "{pre}"\n'
        "timeout = 600\n"
...
def build_kimi_session_home(
    ...
    *,
    ...
    """Materialize a session-scoped ``KIMI_CODE_HOME`` with Codify hooks.
    ...
    the user's config plus the Codify hooks. Best-effort and idempotent:
