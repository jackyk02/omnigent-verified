"""
Databricks profile onboarding for Codify.
...
:mod:`codify.onboarding.internal_beta`, which is excluded from the public
...
SKIP_ENV_VAR = "CODIFY_SKIP_ONBOARD"
...
class ProfileSpec:
    """
    One Databricks profile Codify depends on.
...
def _databrickscfg_path() -> Path:
    ...
    a temporary copy (e.g. during ``codify setup``) can redirect all
...
def run_onboarding() -> bool:
    ...
    from codify.onboarding.internal_beta import DEFAULT_PROFILES
    ...
    console.print(
        Panel(
            "[bold]Codify onboarding[/bold]\n\n"
            "Setting up the Databricks profiles Codify needs "
...
def _derive_workspace_profile_name(workspace_url: str, existing: dict[str, str]) -> str:
    ...
    2. A bundled :data:`DEFAULT_PROFILES` spec whose host matches — use its
       ...
       in
       :func:`codify.onboarding.databricks_config.get_workspace_url_for_profile`.
...
def login_databricks_workspace(workspace_url: str, *, console: Console | None = None) -> str:
    ...
    place Codify triggers a Databricks CLI login: it fires solely when
    a user explicitly adds a Databricks provider in
    ``codify setup --no-internal-beta``, never on a bare ``codify run``.
...
def maybe_run_onboarding() -> None:
    """
    Pre-flight onboarding for ``codify run``.
    ...
    Skipped when ``$CODIFY_SKIP_ONBOARD`` is set or stdin isn't a
    ...
    if actions.aliasable:
        ...
        if n:
            console.print(
                f"codify: aliased {n} existing profile(s) "
                f"to Codify names: "
    ...
    console.print(
        f"\n  [yellow]Codify needs Databricks profiles: {', '.join(missing_parts)}[/yellow]"
    )
    try:
    ...
    if answer in ("", "y", "yes"):
        ...
        console.print(
            "  [dim]run `codify setup --internal-beta` when ready "
