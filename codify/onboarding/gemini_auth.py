...
:func:`codify.onboarding.ambient._claude_login_detected` (Claude Code has the
...
:func:`codify.onboarding.harness_install.harness_cli_logged_in`, which runs
...
``~/.gemini/antigravity-cli/settings.json`` because codify's own CLI launch
path creates that file under the real home before agy ever starts
(:func:`codify.antigravity_native_bridge.ensure_agy_feedback_survey_disabled`).
Its presence proves only that codify ran, not that anyone signed in, so
keying on it would leave the credential gate permanently satisfied. Nothing
codify writes can make ``agy models`` exit 0.

Like :func:`codify.onboarding.ambient.codex_auth_has_credential` and
:func:`codify.onboarding.ambient.claude_auth_has_credential`, the file check
...
def gemini_auth_has_credential(creds_path: Path | None = None) -> bool:
    ...
    (:func:`codify.onboarding.harness_install.harness_cli_logged_in`, which
