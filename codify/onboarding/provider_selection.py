"""
Interactive provider and model selection for ``codify create``.
...
def _read_credentials_from_env(provider: str) -> dict[str, str]:
    ...
    if env_var:
        ...
        if resolved is not None:
            ...
            if provider == "openai":
                base_url = getenv_nonempty_with_codify_prefix("OPENAI_BASE_URL")
                if base_url is not None:
                    creds["base_url"] = base_url[1]
            return creds
        raise click.ClickException(
            f"Non-interactive mode requires {env_var} or "
            f"CODIFY_{env_var} for provider {provider!r}."
        )
...
def _collect_env_credentials(
    ...
    for field in fields:
        ...
        resolved = getenv_nonempty_with_codify_prefix(env_name)
        if resolved is not None:
