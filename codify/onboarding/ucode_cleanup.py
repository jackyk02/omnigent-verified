...
:mod:`codify.onboarding.ucode_setup`), which wires the Claude / Codex
...
- **Codex < 0.134.0 (ucode's legacy layout):** ucode deep-merges into the
  ...
  which makes every *bare* ``codex`` run (and Codify' inner codex, which
...
Removing the provider entry from ``~/.codify/config.yaml`` alone leaves
...
def strip_ucode_codex_config(config_path: Path | None = None) -> bool:
    ...
    :raises CodifyError: If the file exists but is not valid TOML —
...
def remove_ucode_wiring() -> UcodeWiringRemoval:
    ...
    :raises CodifyError: If ``~/.codex/config.toml`` exists but is not
