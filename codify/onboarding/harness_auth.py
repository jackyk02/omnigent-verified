...
``codify setup`` wizard. This module is the shared, prompt-free core the host
...
prompts: store the secret (OS keychain, else ``~/.codify/secrets.json``),
...
def _config_writer() -> tuple[Callable[[], dict[str, object]], _ConfigSaver]:
    ...
    Isolated so the daemon writes to the same ``~/.codify/config.yaml`` the
    ...
    matching ``codify setup``'s writer.
    """
    import yaml

    from codify.onboarding.provider_config import _config_path
...
def _pin_defaults_after_write(
    ...
    2. Pi's own default, when Pi still can't resolve a provider. Pi consumes the
       ...
       an codify-managed provider. When the family default is a kind Pi can't
...
def store_harness_credential(
    *,
    ...
    ``codify setup`` "add a key / gateway" path.
...
def adopt_env_credential(*, family: str, env_var: str) -> StoreCredentialResult:
    ...
    codify resolves it at run time.
