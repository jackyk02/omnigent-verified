"""Hermes readiness + config reporting for ``codify setup``.

Like :mod:`codify.onboarding.goose_auth`, Codify manages **no** Hermes
...
provider/model so ``codify setup`` can show Hermes as ready (and which model
...
:func:`codify.hermes_native_bridge._load_user_hermes_config`. A fresh install
...
from codify.onboarding.harness_install import HERMES_KEY, harness_cli_installed
