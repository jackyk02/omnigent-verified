...
:mod:`codify.onboarding.ambient` discovers credentials already on the
...
``providers:`` shape consumed by :mod:`codify.onboarding.provider_config`,
...
from codify.env_credentials import getenv_nonempty_with_codify_prefix
from codify.onboarding.ambient import DetectedProvider, detect_providers
...
def codex_config_provider_dismissed(config: dict[str, object]) -> bool:
    ...
    :returns: ``True`` when ``~/.codex/config.toml`` selects a custom,
        ...
        dismissed in *config*.
    """
    from codify.onboarding.ambient import codex_config_detection
...
def _synthesize_entry(det: DetectedProvider) -> dict[str, object] | None:
    ...
    :returns: A raw provider entry body (config shape), or ``None`` when the
        detection maps to no codify harness surface (a ``family``-less
        ``key`` detection).
    """
    ...
    if det.family is None:
        ...
        # whose provider maps to no codify family).
        return None
    ...
    if det.kind == "key":
        ...
        if vendor is not None:
            ...
            if det.family == OPENAI_FAMILY and env_var in (
                ...
                env_base_url = getenv_nonempty_with_codify_prefix("OPENAI_BASE_URL")
            ...
            # else native Claude routes to the real API or launches model-less.
                ...
                env_base_url = getenv_nonempty_with_codify_prefix("ANTHROPIC_BASE_URL")
                if env_base_url is not None:
                    base_url = env_base_url[1]
                env_model = getenv_nonempty_with_codify_prefix("ANTHROPIC_MODEL")
...
def synthesize_detected_entries(
    ...
    :param detected: Detections from
        :func:`codify.onboarding.ambient.detect_providers`, in priority
        order.
    :returns: Raw provider entries keyed by the detection name, e.g.
        ...
        surface) — see :data:`codify.onboarding.ambient._ENV_KEY_FAMILY`.
        Only detections that map to no codify family at all (a ``family``-less
...
def effective_config_with_detected(
    ...
    :param detected: Detections to merge; defaults to a live
        :func:`codify.onboarding.ambient.detect_providers` call.
