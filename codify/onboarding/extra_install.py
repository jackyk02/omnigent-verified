...
Each SDK harness ships as an optional extra (``codify[cursor]``, etc.).  The
install command depends on *how* codify itself was installed:
...
* **``uv tool``** — the package lives in an isolated tool environment that
  ...
  extra via ``uv tool install --with "codify[extra]" codify --force``; a
  git-source install (``uv tool install git+…``) instead reinstalls from that
  same source (``uv tool install --force "codify[extra] @ git+…"``) so the
  tool is not silently re-pulled from PyPI.
* **``uv`` (non-tool)** — ``uv pip install "codify[extra]"`` targets the
  active virtualenv.
* **``pip`` / fallback** — ``<sys.executable> -m pip install "codify[extra]"``
...
def _is_uv_tool_install() -> bool:
    ...
    :func:`codify.update_check._looks_like_pipx_install`.
...
def _installed_vcs_url() -> str | None:
    """Return the VCS URL codify was installed from, or ``None``.

    Reads the distribution's ``direct_url.json`` (PEP 610) via
    :func:`codify.update_check._read_installed_wheel_info`, which normalizes
    ...
    for plain registry installs, so callers fall back to a bare ``codify``
    target.
    """
    from codify.update_check import _read_installed_wheel_info
...
def extra_install_command(extra: str) -> list[str]:
    ...
    1. ``uv tool`` install, git source → ``uv tool install --force
       "codify[extra] @ git+…"`` (keeps the tool on its original source)
    2. ``uv tool`` install, registry → ``uv tool install --with ...
       codify --force``
    3. ``uv`` on PATH (non-tool) → ``uv pip install "codify[extra]"``
    4. fallback → ``<sys.executable> -m pip install "codify[extra]"``

    :param extra: The pip extra name, e.g. ``"cursor"``.
    :returns: The install argv.
    """
    target = f"codify[{extra}]"

    if _is_uv_tool_install():
        vcs_url = _installed_vcs_url()
        if vcs_url is not None:
            # Git-source tool install: reinstall from the same source with the
            # extra attached, else a bare ``codify`` re-pulls it from PyPI.
            return ["uv", "tool", "install", "--force", f"{target} @ {vcs_url}"]
        return ["uv", "tool", "install", "--with", target, "codify", "--force"]
