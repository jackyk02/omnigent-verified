...
_SOURCE_REPO_URL = "https://github.com/codify-ai/codify.git"
DATABRICKS_EXTRA_INSTALL_HINT = (
    f'uv tool install --force "codify[databricks] @ git+{_SOURCE_REPO_URL}"'
)
...
def databricks_sdk_installed() -> bool:
    ...
    (:mod:`codify.runtime.credentials.databricks`), so onboarding flows
...
def list_databricks_profiles() -> list[str]:
    ...
    Used by ``codify setup --no-internal-beta`` to offer the user a pick-list
...
def get_workspace_url_for_profile(profile: str) -> str | None:
    ...
    :mod:`configparser`, then falls back to Codify' built-in setup
