"""Antigravity Gemini API-key credential storage for ``codify setup``.
...
the codify secret store and is referenced from a dedicated top-level
``antigravity:`` block in ``~/.codify/config.yaml`` (``keychain:`` / ``env:``),
...
:mod:`codify.onboarding.cursor_auth`.
"""
...
def antigravity_sdk_installed() -> bool:
    ...
    :func:`codify.onboarding.databricks_config.databricks_sdk_installed`: uses
...
def install_antigravity_sdk() -> bool:
    ...
    :func:`codify.onboarding.harness_install.install_harness_cli`.
...
def antigravity_api_key_settings(ref: str) -> dict[str, object]:
    ...
    :returns: The settings dict for :func:`codify.cli._save_global_config`.
    """
