---
name: build-codify
description: Patterns and templates for generating valid Codify agent directories. Load when ready to create files.
---
...
that `codify server` uses. If your environment exposes the
...
**`codify-knowledge`** skill if you need the deeper field reference
...
- `guardrails` — runtime policy gates (see `codify-knowledge`).

## Step 2a: Choose an executor

`executor.type` must be one of **`claude_sdk`**, **`agents_sdk`**, or
**`codify`**. There is **no `llm` executor** — do not use it.
...
| A CLI/coding harness, shell + file tools, sub-agents | `codify` + a `config.harness` |

When `executor.type: codify`, **`config.harness` is required** and must
...
only when asked; see `codify-knowledge` for `executor.model` / `auth`.
...
Use the `codify` executor with a coding harness when the agent needs to
...
Sub-agents need the `codify` executor (it provides the spawn tools).
...
- [ ] `executor.type` is one of `claude_sdk`, `agents_sdk`, `codify`
- [ ] If `executor.type: codify`, `executor.config.harness` is set to a
      valid harness
- [ ] `instructions` (or `prompt`) points to a file that exists or is
      inline text
- [ ] When declaring `tools.agents`, the parent uses `executor.type:
      codify`, and each entry has a matching `agents/` subdirectory
