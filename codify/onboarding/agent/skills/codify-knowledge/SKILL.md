...
description: Deep reference on Codify config format, executor types, skill/tool structure, and conventions. Load when you need to look up how the platform works.
---

# Codify Knowledge Base
...
executor:
  # REQUIRED area. type must be one of: claude_sdk | agents_sdk | codify.
  ...
  # type: codify     — subprocess harness; requires config.harness below

  # Only for type: codify — pick the harness that runs the loop.
...
| `codify` | Coding/CLI harnesses, shell + file tools, sub-agents | Spawns a subprocess harness selected by `config.harness` |

**There is no `llm` executor type** — the only valid values are
`claude_sdk`, `agents_sdk`, and `codify`. For **most new simple agents**,
use `claude_sdk` (or `agents_sdk`) — in-process, no extra config. Use
`codify` when the agent needs a specific harness, shell/file access, or
...
prompt; Codify may append framework-owned lifecycle or metadata instructions
...
`executor.type: codify` — that's what provides the spawn tools. Each
sub-agent is a full agent and may use any executor (`claude_sdk`,
`agents_sdk`, or `codify`).
