"""Read/write the kind-typed model-provider config in ``~/.codify/config.yaml``.
...
block in ``~/.codify/config.yaml`` is the source of truth for the
...
from codify.env_credentials import (
    codify_prefixed_env_name,
    expand_envvars_with_codify_prefix,
    getenv_with_codify_prefix,
)
...
# :mod:`codify.onboarding.gemini_auth`, not a ``gemini:`` family here).
...
# (native-``codify claude`` only) can never drive pi. Resolution: an
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # Native CLI harnesses. Specs (and the codify-compat translator)
    ...
    # per-spawn provider override flag, so Codify cannot thread a generic
    ...
    # (file-based, checked in :mod:`codify.onboarding.gemini_auth`) and the
...
class FamilyConfig:
    ...
    :param api_key_ref: A reference to a secret stored outside the config
        ...
        ``"keychain:<name>"`` (read from the codify secret store — see
        :func:`resolve_secret` and :mod:`codify.onboarding.secrets`),
...
class ProviderEntry:
    ...
    def family(self, name: str) -> FamilyConfig | None:
        ...
        :raises CodifyError: If the family references an unset
...
def resolve_secret(ref: str) -> str:
    ...
    - ``"keychain:<name>"`` — read ``<name>`` from the codify secret
      store (OS keychain, else a ``0600`` JSON file). The store is
      populated by ``codify setup --no-internal-beta`` — see
      :mod:`codify.onboarding.secrets`.
    ...
    :raises CodifyError: If an ``env:`` / ``$VAR`` reference names an
    ...
    if ref.startswith("keychain:"):
        ...
        if value is None:
            raise CodifyError(
                ...
                "`codify setup --no-internal-beta` to set it.",
                code=ErrorCode.INVALID_INPUT,
            )
        return value
    if ref.startswith("env:"):
        var = ref[len("env:") :]
        resolved = getenv_with_codify_prefix(var)
        if resolved is None:
            prefixed = codify_prefixed_env_name(var)
    ...
    expanded = expand_envvars_with_codify_prefix(ref)
    check_unresolved_env_vars(ref, expanded)
    return expanded


def _config_path() -> str:
    """Return the path to the global codify config file.

    Respects ``$CODIFY_CONFIG_HOME`` for test isolation (matching the
    rest of the onboarding layer).

    :returns: Path to ``config.yaml``, e.g.
        ``"/home/u/.codify/config.yaml"``.
    """
    config_home = os.environ.get("CODIFY_CONFIG_HOME")
    if config_home:
        return os.path.join(config_home, "config.yaml")
    return os.path.join(os.path.expanduser("~"), ".codify", "config.yaml")
...
def _expand(key: str, value: str) -> str:
    ...
    :raises CodifyError: If a referenced variable is unset.
    """
    expanded = expand_envvars_with_codify_prefix(value)
...
def _expand_family(provider_name: str, family_name: str, family: FamilyConfig) -> FamilyConfig:
    ...
    :returns: A new :class:`FamilyConfig` with ``base_url`` expanded and,
        ...
        ``api_key_ref`` cleared).
    :raises CodifyError: If a referenced environment variable is unset,
...
def _parse_family(provider_name: str, family_name: str, raw: dict[str, object]) -> FamilyConfig:
    ...
    :raises CodifyError: If ``base_url`` is missing, the credential
...
def _parse_default_families(
    ...
    :raises CodifyError: If ``default`` is an unsupported type, or names
...
def _parse_provider(name: str, raw: dict[str, object]) -> ProviderEntry:
    ...
    :raises CodifyError: If ``kind`` is missing/invalid or the
    ...
    # codify/inner/loader.py), so a YAML ``default: true`` arrives as
...
def load_config() -> dict[str, object]:
    """Load the global ``~/.codify/config.yaml`` mapping.
    ...
    ``$CODIFY_CONFIG_HOME`` for test isolation, exactly like the rest
...
def _cli_config_serves_pi(entry: ProviderEntry) -> bool:
    ...
    natively, and :func:`codify.pi_native_credentials._cli_config_pi_provider`
    ...
    The capability check lives in :mod:`codify.pi_native_credentials` (the
    ...
    if entry.kind != CLI_CONFIG_KIND:
        return False
    from codify.pi_native_credentials import cli_config_pi_provider_capable
...
def provider_families(entry: ProviderEntry) -> frozenset[str]:
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is native-``codify claude`` only — the in-process /
...
def get_default_provider(config: dict[str, object], family: str) -> ProviderEntry | None:
    ...
    :raises CodifyError: If any provider is malformed, or more than one
...
def default_provider_for_harness(config: dict[str, object], harness: str) -> ProviderEntry | None:
    ...
    unusable outside its own CLI, and ``bedrock`` is native-``codify
    ...
    :raises CodifyError: If a provider is malformed, or more than one
    ...
    for fam in _PI_FALLBACK_FAMILIES:
        ...
        # native-``codify claude`` only (configure_agent_harness_with_provider
...
def _source_descriptor(family: FamilyConfig) -> str:
    ...
    :raises CodifyError: If the family has no credential source (should
...
def harness_owns_its_credential(harness: str) -> bool:
    """Whether *harness* carries its own auth, so Codify resolves none.

    True for ACP-backed harnesses whose spawn wires no Codify provider
    (``acp``/``acp:<slug>``, goose, and unmapped community ACP plugins):
    the external agent authenticates itself, so describing an Codify
    ...
    """
    from codify.harness_capabilities import AuthModel, IntegrationMode
    from codify.harness_plugins import harness_capabilities
...
def describe_active_credential(
    ...
    :raises CodifyError: If the resolved provider is malformed, or more
...
def set_default_provider(
    ...
    :raises CodifyError: If *name* is absent, an entry is malformed, or
    ...
    if not isinstance(target_raw, dict):
        raise CodifyError(f"provider {name!r} must be a mapping.", code=ErrorCode.INVALID_INPUT)
...
def provider_entry_settings(
    ...
    ``~/.codify/config.yaml`` under ``providers:``. When *make_default*
