"""Cursor API-key credential storage for ``codify setup`` and the runtime.

Cursor is deliberately outside the anthropic/openai provider-family + gateway
machinery (see :func:`codify.runtime.workflow._build_cursor_spawn_env`): the
...
register a ``CURSOR_API_KEY`` once through ``codify setup`` rather than
exporting it in every shell.

This module is that home. The key is stored exactly like the api-key
providers' secrets — in the codify secret store (OS keychain, else a
``0600`` JSON file; see :mod:`codify.onboarding.secrets`) — and referenced
from a dedicated top-level ``cursor:`` block in ``~/.codify/config.yaml``::
...
(:func:`codify.runtime.workflow._load_global_auth`) — a Cursor key parked
...
from codify.onboarding.extra_install import extra_install_command
from codify.onboarding.provider_config import load_config, resolve_secret
...
def cursor_sdk_installed() -> bool:
    ...
    (:mod:`codify.inner.cursor_executor`), so a key can be set with no SDK;
    setup uses this to detect that and offer to install it. Mirrors
    :func:`codify.onboarding.databricks_config.databricks_sdk_installed` /
    :func:`codify.onboarding.antigravity_auth.antigravity_sdk_installed`:
...
def cursor_install_command() -> list[str]:
    ...
    Delegates to :func:`~codify.onboarding.extra_install.extra_install_command`
...
def install_cursor_sdk() -> bool:
    ...
    visible. Mirrors
    :func:`codify.onboarding.antigravity_auth.install_antigravity_sdk`.
...
def cursor_api_key_ref(config: dict[str, object] | None = None) -> str | None:
    ...
    :param config: A pre-loaded config mapping; ``None`` loads
        ``~/.codify/config.yaml`` via :func:`load_config`.
...
def cursor_api_key_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`codify.cli._save_global_config` (a shallow update, so
