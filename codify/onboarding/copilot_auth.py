"""GitHub Copilot token storage for ``codify setup`` and the runtime.

Copilot is deliberately outside the anthropic/openai provider-family + gateway
machinery (see :func:`codify.runtime.workflow._build_copilot_spawn_env`): the
...
to register a Copilot token once through ``codify setup`` rather than
exporting it in every shell.

This module is that home. The token is stored exactly like the api-key
providers' secrets — in the codify secret store (OS keychain, else a ``0600``
JSON file; see :mod:`codify.onboarding.secrets`) — and referenced from a
dedicated top-level ``copilot:`` block in ``~/.codify/config.yaml``::
...
def copilot_sdk_installed() -> bool:
    ...
    (:mod:`codify.inner.copilot_executor`), so a token can be set with no SDK;
    setup uses this to detect that and offer to install it. The
    ``github-copilot-sdk`` package is imported as ``copilot``. Mirrors
    :func:`codify.onboarding.cursor_auth.cursor_sdk_installed` /
...
def install_copilot_sdk() -> bool:
    ...
    visible. Mirrors :func:`codify.onboarding.cursor_auth.install_cursor_sdk`.
...
def copilot_github_token_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`codify.cli._save_global_config` (a shallow update, so it
