"""Setup helpers for invoking ucode from Codify."""
...
def build_ucode_configure_command(
    ...
    *,
    ...
    """Build the ``ucode configure`` command Codify runs.
...
def configure_ucode_for_workspace(
    ...
    *,
    ...
    ``kind: databricks`` provider via ``codify setup --no-internal-beta``.
    ucode writes ``~/.ucode/state.json``, which Codify then reads for
