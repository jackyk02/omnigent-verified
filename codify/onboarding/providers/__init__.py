...
codify-specific and lives here permanently.
"""
...
def _download_provider_catalog(provider: str) -> dict[str, Any] | None:
    ...
    Skipped when ``CODIFY_DISABLE_CATALOG_LOOKUP=1`` (set by the test
    ...
    """
    if os.environ.get("CODIFY_DISABLE_CATALOG_LOOKUP") == "1":
        return None
...
def _fetch_provider_catalog(provider: str) -> dict[str, Any]:
    ...
    dict. Setting ``CODIFY_DISABLE_CATALOG_LOOKUP=1`` bypasses every cache
    tier and network.
