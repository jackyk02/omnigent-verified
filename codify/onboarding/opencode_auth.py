"""OpenCode readiness + credential reporting for ``codify setup``.

Like :mod:`codify.onboarding.goose_auth`, Codify stores **no** OpenCode
...
read-only reporter so ``codify setup`` can show which providers OpenCode can
...
from codify.onboarding.harness_install import OPENCODE_KEY, harness_cli_installed
