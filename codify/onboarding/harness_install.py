...
offer to ``npm install`` it; the first-run ``codify run`` flow uses the
...
:mod:`codify.onboarding.ambient` is file-based and subprocess-free on
...
from codify.harness_install_spec import HarnessInstallSpec, SetupStep
from codify.onboarding.provider_config import ANTHROPIC_FAMILY, GEMINI_FAMILY, OPENAI_FAMILY
from codify.opencode_native_client import (
...
# 0.74 is the oldest Pi whose RPC mode the harness can drive; newer
# launch flags degrade gracefully (see pi_native._supports_* fail-open
# probes), so don't gate readiness on the flag-era 0.79 floor.
_PI_MIN_VERSION = "0.74.0"
...
# ``~/.config/goose/config.yaml``) with no Codify-managed credential, and ships
...
# :func:`codify.onboarding.harness_readiness.harness_is_configured`). The key
...
# Codify-managed credentials). The ``hermes`` binary must be on PATH.
HERMES_KEY = "hermes"
...
_HARNESS_INSTALL: dict[str, HarnessInstallSpec] = {
    ANTHROPIC_FAMILY: HarnessInstallSpec(
        ...
        # The native bridge injects Codify's MCP relay via `--mcp-config`;
...
# (codify stores no key for them), so they are NOT credential-configurable.
...
def ui_credential_configurable_harnesses() -> frozenset[str]:
    ...
    provider credential codify owns (Claude / Codex / Pi families). The
...
#   qwen: env-auth — codify stores no key, so it stays a setup signpost.
_UI_AUTH_STEP_BY_KEY: dict[str, SetupStep] = {
    ...
    QWEN_KEY: SetupStep(
        ...
        # Qwen is env-auth (not UI-authable): codify stores no key for it, so
...
def _all_harness_install() -> dict[str, HarnessInstallSpec]:
    from codify.harness_plugins import install_specs
...
def _all_harness_name_to_key() -> dict[str, str]:
    from codify.harness_plugins import harness_install_keys
...
def _parse_harness_cli_version(text: str) -> str | None:
    ...
    :func:`codify.opencode_native_app_server.parse_opencode_version` but is
