...
:func:`codify.onboarding.provider_config.resolve_secret`). The
``codify setup --no-internal-beta`` command writes a provider's API key here
...
- **OS keychain** (macOS Keychain, GNOME Keyring, Windows Credential
  ...
  actually works and ``CODIFY_DISABLE_KEYRING`` is not set. Secrets
...
- **A ``0600`` JSON file** at ``<config_home>/secrets.json`` (a flat
  ...
  ``CODIFY_DISABLE_KEYRING`` or a keyring call raises
...
:mod:`codify.onboarding.provider_config` — that module imports *this*
one lazily inside :func:`~codify.onboarding.provider_config.resolve_secret`
...
# service groups all codify secrets; the per-secret ``name`` is the
# keychain "username".
_KEYRING_SERVICE = "codify"
...
_DISABLE_KEYRING_ENV = "CODIFY_DISABLE_KEYRING"
...
def _keyring_disabled() -> bool:
    ...
    :func:`codify.runtime.telemetry._env_bool`): ``"true"`` / ``"1"`` /
    ``"yes"`` (case-insensitive) count as set; anything else (including
    unset) does not.

    :returns: ``True`` when ``CODIFY_DISABLE_KEYRING`` is truthy, e.g.
        with ``CODIFY_DISABLE_KEYRING=1`` set.
    """
...
def _use_keyring() -> bool:
    ...
    :returns: ``True`` unless ``CODIFY_DISABLE_KEYRING`` is truthy.
    """
...
def _config_home() -> str:
    """Return the codify config home directory.

    Respects ``$CODIFY_CONFIG_HOME`` for test isolation, matching the
    convention in :func:`codify.onboarding.provider_config._config_path`.

    :returns: The config home path, e.g. ``"/home/u/.codify"`` or the
        value of ``$CODIFY_CONFIG_HOME`` when set.
    """
    config_home = os.environ.get("CODIFY_CONFIG_HOME")
    if config_home:
        return config_home
    return os.path.join(os.path.expanduser("~"), ".codify")
