...
but never ran ``codify setup`` to install it" case.
...
import codify.onboarding.gemini_auth as _gemini_auth
from codify._platform import resolve_cli_binary
from codify.harness_aliases import HARNESS_ALIASES, canonicalize_harness
...
from codify.harness_plugins import harness_install_keys, valid_harnesses
...
# ``codify.onboarding.gemini_auth.gemini_login_detected`` and have the patch
...
# auth backend and no Codify provider family, so readiness is binary presence.
...
# Native Kimi TUI harnesses (``codify kimi``). Like the other native CLIs,
...
def _install_key(canonical: str) -> str:
    ...
    :returns: ``"anthropic"`` / ``"openai"`` for the claude/codex CLIs,
        :data:`~codify.onboarding.harness_install.KIMI_KEY` for kimi,
        :data:`~codify.onboarding.harness_install.OPENCODE_KEY` for
        opencode-native,
        :data:`~codify.onboarding.harness_install.QWEN_KEY` for qwen, or
        :data:`~codify.onboarding.harness_install.PI_KEY` for pi.
    """
...
def _harness_availability_core(harness: str) -> HarnessAvailability:
    ...
    if canonical == CURSOR_KEY:
        ...
        # readiness is whether a key is resolvable — stored by ``codify setup``
        # (the ``cursor:`` block — see :mod:`codify.onboarding.cursor_auth`)
        ...
        # (:mod:`codify.inner.cursor_executor`); gating here would only
    ...
    if canonical == COPILOT_KEY:
        ...
        # resolvable — one stored by ``codify setup`` (the ``copilot:`` config
        # block — see :mod:`codify.onboarding.copilot_auth`) or inherited from
...
# an codify-managed provider). Qwen is absent on purpose: its key lives in the
...
def _family_provider_configured(harness: str) -> bool:
    ...
    Reads the local ``providers:`` config the same way the ``codify setup``
    ...
    (matching the secret-blind ``codify setup`` overview), so a harness can
...
def _cli_family_availability(canonical: str, install_key: str) -> HarnessAvailability:
    ...
    # claude: ready when EITHER an codify-managed provider serves the family
    ...
    from codify.onboarding.harness_install import harness_cli_logged_in
...
def _harness_availability(canonical: str) -> HarnessAvailability:
    ...
    if _is_codex_family_harness(canonical):
        from codify.codex_native import _codex_auth_unavailable_reason
    ...
    if canonical in _PI_HARNESSES:
        # pi has no CLI login — its only credential is an codify-managed
