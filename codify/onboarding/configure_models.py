...
:mod:`codify.cli`; this module owns the look/feel and the
...
from codify.onboarding.interactive import ACCENT, console
...
# see codify.inner.banner._display_width.)
...
# :func:`codify.onboarding.providers.get_all_providers`) to the
# codify family it serves for a ``key`` provider. Anthropic serves the
# Claude (anthropic) surface; OpenAI and OpenAI-compatible vendors serve
# the Codex (openai) surface. Providers absent here have no codify
...
def key_providers() -> list[str]:
    ...
    codify family. Only providers in :data:`_CATALOG_PROVIDER_FAMILY`
...
def add_menu_options() -> list[AddOption]:
    ...
    return [
        ...
        _opt(
            ...
            # `codify[databricks]` command appears on selection.
        ...
        # native ``codify claude`` terminal in Bedrock mode. Listed last so it
        ...
        _opt(
            ...
            "terminal (codify claude). Claude only.",
            BEDROCK_KIND,
        ),
    ]
...
def render_provider_listing_by_harness(
    ...
    several surfaces is listed under each. Used by ``codify config list``.
    ...
    :param providers: Parsed provider entries keyed by name (from
        :func:`~codify.onboarding.provider_config.load_providers`).
    :returns: None. Side effect: writes the listing to the shared console.
    """
    from codify.onboarding.provider_config import provider_families
...
def render_provider_listing(
    ...
    Prints (via the shared :data:`~codify.onboarding.interactive.console`)
    ...
    :param detected: Ambient detections from
        :func:`~codify.onboarding.ambient.detect_providers`; entries
...
def build_bedrock_provider_entry(
    ...
    native ``codify claude`` terminal in AWS Bedrock mode (the in-process /
