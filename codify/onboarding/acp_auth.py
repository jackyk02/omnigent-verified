"""Generic ACP-agent registry for ``codify setup`` and the runtime.

The generic ``acp`` harness (see :func:`codify.runtime.workflow._build_acp_spawn_env`
and :mod:`codify.inner.acp_harness`) drives *any* agent that speaks the Agent
Client Protocol. Which agents are available is pure user config: a list of named
commands in a dedicated top-level ``acp:`` block of ``~/.codify/config.yaml``::
...
here. Auth is each agent's own — Codify stores no credential, so unlike the
...
:mod:`codify.onboarding.cursor_auth`): the CLI orchestrates writes through
:func:`codify.cli._save_global_config` so there is no cli↔onboarding cycle.
"""
...
class AcpAgentEntry:
    ...
    :param model: Optional model id (only honored by agents that accept a model
        in ``session/new``; see :class:`codify.inner.acp_executor.AcpAgentConfig`).
...
def acp_config_summary(config: dict[str, object] | None = None) -> AcpConfigSummary:
    """Summarize the configured ACP agents for ``codify setup``.
