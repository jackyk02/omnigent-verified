...
the SDK is an optional dependency (``pip install 'codify[e2b]'``)
...
- **Templates, not registry images.** Unlike every other launcher, E2B
  ...
  a pre-built E2B *template*. The Codify host image must therefore be
  ...
  built FROM the same host image (codify ``0.1.0`` baked).
...
DEFAULT_E2B_TEMPLATE: str = "codify-host"
...
MAX_LIFETIME_ENV_VAR: str = "CODIFY_E2B_MAX_LIFETIME_S"
...
def _template_build_hint(template: str, exc: Exception) -> click.ClickException:
    ...
    return click.ClickException(
        ...
        "incompatible. Build the Codify host image into an E2B template first "
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'codify[e2b]'`, then set "
...
class E2BSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, template: str | None = None, env: Sequence[str] | None = None) -> None:
        ...
        :param template: Optional E2B template NAME (or id) to provision
            ...
            Codify host image was built into (``e2b template build``),
    ...
    def stream_exec(self, sandbox_id: str, command: str, *, pty: bool = False) -> RemoteProcess:
        ...
        try:
            ...
            # backs exec_foreground, which holds `codify host` open for the
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        FROM the prebaked host image (codify ``0.1.0`` baked).
