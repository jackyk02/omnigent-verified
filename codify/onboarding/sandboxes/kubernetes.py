...
:class:`~codify.onboarding.sandboxes.base.SandboxLauncher` for an
...
dependency (``pip install 'codify[kubernetes]'``) imported lazily, so the
provider can be listed and the module probed without it.

The model is **entrypoint-as-host**: the Pod's container command IS
``codify host``. :meth:`~KubernetesSandboxLauncher.provision` only RESERVES
...
- **Writable HOME.** The host image's WORKDIR is ``/root`` (root-owned), but
  ...
  ``CODIFY_CONFIG_HOME``, the init container receives the same value so its
...
- **PID-1 reaper.** The in-sandbox host re-parents orphaned runner processes to
  ...
  ``codify host``, reaps any children, and forwards SIGTERM for prompt,
...
- **Least privilege.** ``automountServiceAccountToken: false`` keeps the runner
  ...
  filesystem stays writable (the host writes ``/tmp`` and ``~/.codify``).
...
from codify.host.identity import (
    HOST_ID_ENV_VAR,
    HOST_NAME_ENV_VAR,
    HOST_TOKEN_ENV_VAR,
)
...
from codify.onboarding.sandboxes.types import SandboxCapabilities
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_KUBERNETES_HOST_IMAGE"
"""Environment variable overriding
:data:`~codify.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for Kubernetes
sandbox Pods (published multi-arch: amd64 + arm64)."""

NAMESPACE_ENV_VAR: str = "CODIFY_KUBERNETES_NAMESPACE"
...
SANDBOX_SECRET_ENV_VAR: str = "CODIFY_KUBERNETES_SECRET"
...
SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_KUBERNETES_SANDBOX_ENV"
...
SERVICE_ACCOUNT_ENV_VAR: str = "CODIFY_KUBERNETES_SERVICE_ACCOUNT"
...
KUBECONFIG_ENV_VAR: str = "CODIFY_KUBERNETES_KUBECONFIG"
...
_DEFAULT_NAMESPACE: str = "codify-sandboxes"
_DEFAULT_SERVICE_ACCOUNT: str = "codify-runner"
...
# (or a future reconciler) can select codify-managed objects for GC.
_MANAGED_BY_LABEL: str = "app.kubernetes.io/managed-by"
_MANAGED_BY_VALUE: str = "codify"
_ROLE_LABEL: str = "codify.ai/role"
...
# (``codify host …``) as a child, forwards SIGTERM/SIGINT for prompt graceful
...
# in :mod:`codify.server.managed_hosts` (which validate config-sourced names);
...
def _ensure_sdk() -> None:
    ...
    optional dependency — the base ``codify`` install does not pull it in.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'codify[kubernetes]'`."
        ) from exc
...
def _new_pod_name(label: str) -> str:
    ...
    :returns: A Pod name like ``"codify-managed-a1b2c3d4-1a2b3c"``.
    """
    base = re.sub(r"[^a-z0-9-]+", "-", label.lower()).strip("-")
    base = re.sub(r"-+", "-", base) or "host"
    return f"codify-{base[:40]}-{uuid.uuid4().hex[:6]}"
...
def _token_secret_name(pod_name: str) -> str:
    ...
    :returns: The Secret name, e.g. ``"codify-managed-a1b2c3d4-1a2b3c-token"``.
    """
...
def _render_workspace_prep_command(
    ...
    ``$CODIFY_CONFIG_HOME`` or the default ``~/.codify`` when set — all
    ...
    :param workspace: The workspace root to create, e.g. ``"/home/codify/workspace"``.
...
def _render_host_command(server_url: str) -> list[str]:
    """
    Render the main container command that runs ``codify host`` under the
    PID-1 reaper.
    ...
    ``codify host --server <url>`` — as its child. Identity + token reach the
    ...
    script = (
        ...
        f"codify host --server {shlex.quote(server_url)}"
    )
    return ["bash", "-lc", script]
...
def build_pod_manifest(
    *,
    ...
    - An **init container** (:data:`_INIT_CONTAINER_NAME`) creates the workspace
      ...
      (:data:`_CONTAINER_NAME`) runs ``codify host`` under the PID-1 reaper.
    ...
    - Pod + container ``securityContext`` satisfy Pod Security "restricted"
      ...
      root filesystem stays writable (the host writes ``/tmp`` + ``~/.codify``).
    ...
    config_home = env_literals.get("CODIFY_CONFIG_HOME")
    if config_home is not None:
        ...
        # (~/.codify), so only a non-empty override is checked. Resolve
        ...
        # lexical check, so an operator must not aim CODIFY_CONFIG_HOME inside
        ...
        if (
            ...
            raise ValueError(
                f"CODIFY_CONFIG_HOME ({config_home!r}) must resolve under {_HOME_DIR!r} "
        ...
        init_env.append({"name": "CODIFY_CONFIG_HOME", "value": config_home})
...
class KubernetesSandboxLauncher(SandboxHostLauncher):
    ...
    ``codify host``, and :meth:`terminate` deletes both. All transport rides the
    ...
    def __init__(
        ...
        *,
        ...
        :param image: Host image reference — the ``sandbox.kubernetes.image``
            ...
            :data:`~codify.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE`.
    ...
    def provision(self, name: str) -> str:
        ...
        Entrypoint-as-host: the Pod (which boots running ``codify host``) is
    ...
    def start_host(
        ...
        *,
        ...
        :meth:`~codify.onboarding.sandboxes.base.SandboxLauncher.start_host`
        ...
        main container runs ``codify host``, which dials back over the
        ...
        :param host_config: Deployment-supplied ``~/.codify/config.yaml``
    ...
    def _delete_with_retry(self, kind: str, name: str, delete: Callable[[], object]) -> None:
        ...
        click.echo(
            ...
            "and carries the codify managed-by/role labels for GC.",
            err=True,
        )
