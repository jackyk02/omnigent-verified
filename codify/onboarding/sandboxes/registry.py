...
Core Codify contributes built-in sandbox providers directly. Optional
community packages contribute additional providers through the
``codify.sandbox_providers`` entry point group.

The registry mirrors the design of ``codify.harness_plugins``:
contributions are merged into a single plugin state, validation keeps
community provider code in the ``codify.community.sandbox`` namespace, and
...
from codify.onboarding.sandboxes.base import SandboxLifecycle
...
COMMUNITY_ENTRY_POINT_GROUP = "codify.sandbox_providers"
COMMUNITY_MODULE_PREFIX = "codify.community.sandbox."
...
class SandboxProviderContribution:
    ...
    :param name: Package / contributor name, e.g. ``"codify-acme"``. Used
...
def _builtin_contribution() -> SandboxProviderContribution:
    """The built-in sandbox-provider contribution from core Codify."""
    return SandboxProviderContribution(
        name="codify",
        providers={
            "lakebox": SandboxProviderMetadata(
                name="lakebox",
                launcher_class="codify.onboarding.sandboxes.lakebox:LakeboxLauncher",
            ),
            "modal": SandboxProviderMetadata(
                name="modal",
                launcher_class="codify.onboarding.sandboxes.modal:ModalSandboxLauncher",
                managed_token_ttl_s=25 * 3600,
            ),
            "daytona": SandboxProviderMetadata(
                name="daytona",
                launcher_class="codify.onboarding.sandboxes.daytona:DaytonaSandboxLauncher",
                managed_token_ttl_s=7 * 24 * 3600,
            ),
            "boxlite": SandboxProviderMetadata(
                name="boxlite",
                launcher_class="codify.onboarding.sandboxes.boxlite:BoxliteSandboxLauncher",
                managed_token_ttl_s=7 * 24 * 3600,
            ),
            "cwsandbox": SandboxProviderMetadata(
                name="cwsandbox",
                launcher_class="codify.onboarding.sandboxes.cwsandbox:CWSandboxLauncher",
            ),
            "islo": SandboxProviderMetadata(
                name="islo",
                launcher_class="codify.onboarding.sandboxes.islo:IsloSandboxLauncher",
                managed_token_ttl_s=7 * 24 * 3600,
            ),
            "e2b": SandboxProviderMetadata(
                name="e2b",
                launcher_class="codify.onboarding.sandboxes.e2b:E2BSandboxLauncher",
            ),
            "openshell": SandboxProviderMetadata(
                name="openshell",
                launcher_class="codify.onboarding.sandboxes.openshell:OpenShellSandboxLauncher",
                managed_token_ttl_s=7 * 24 * 3600,
            ),
            "kubernetes": SandboxProviderMetadata(
                name="kubernetes",
                launcher_class="codify.onboarding.sandboxes.kubernetes:KubernetesSandboxLauncher",
                managed_token_ttl_s=7 * 24 * 3600,
            ),
        },
    )
