...
dependency (``uv pip install 'codify[openshell]'``) imported lazily, so
...
- **Custom host image.** Codify boots its prebaked host image, which
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_OPENSHELL_HOST_IMAGE"
"""Environment variable overriding :data:`DEFAULT_HOST_IMAGE` for
OpenShell sandboxes."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_OPENSHELL_SANDBOX_ENV"
...
WORKSPACE_ENV_VAR: str = "CODIFY_OPENSHELL_WORKSPACE"
...
_EXEC_TIMEOUT_S = 300
# A foreground host (`codify host`) is held open until Ctrl-C, so its
...
# rather than changing the shared image -- otherwise ``codify host`` resolves
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `uv pip install 'codify[openshell]'`, then select a "
...
class OpenShellSandboxLauncher(SandboxLauncher):
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        Holds ``codify host`` open for ``codify sandbox connect``;
