"""
Provider-agnostic interface for running Codify hosts in remote sandboxes.
...
generic bootstrap flow in :mod:`codify.onboarding.sandboxes.bootstrap`
...
Injected host config uses the loader's ``CODIFY_CONFIG_HOME`` resolution,
...
from codify.host.identity import HOST_ID_ENV_VAR, HOST_NAME_ENV_VAR, HOST_TOKEN_ENV_VAR
from codify.onboarding.sandboxes import types as _sandbox_types
...
DEFAULT_HOST_IMAGE: str = "ghcr.io/codify-ai/codify-host:latest"
"""Default sandbox image across providers: the official prebaked
Codify host image, published by CI from the ``host`` target of
``deploy/docker/Dockerfile`` (``:latest`` tracks main; ``:sha-<short>``
pins a commit). It bakes the full codify install plus git / tmux /
...
def host_image_wheel_install_command(remote_tgz_path: str) -> str:
    ...
    codify at the same ``0.1.0`` version. Without it, pip sees the
...
# run ``codify host``): importing merge logic from the sandbox's installed
# codify package would tie the feature to the IMAGE's package version, and
...
config_dir = config_home if config_home else os.path.join(os.path.expanduser("~"), ".codify")
...
def render_host_config_write_command(host_config: dict[str, object]) -> str:
    ...
    sandbox's config directory before ``codify host`` starts. The directory
    is ``$CODIFY_CONFIG_HOME`` when truthy, otherwise ``~/.codify``, exactly
    matching :func:`codify.onboarding.provider_config._config_path`.
    ...
    ``codify.cli._save_global_config``'s
    ...
    :param host_config: The validated ``sandbox.host_config`` mapping
        (see :func:`codify.server.managed_hosts.parse_sandbox_config`),
...
class SandboxLifecycle(ABC):
    ...
    def capabilities(self) -> _sandbox_types.SandboxCapabilities:
        ...
        :class:`~codify.onboarding.sandboxes.types.SandboxCapabilities`
        object directly.
        """
...
class SandboxExecTransport(SandboxLifecycle):
    ...
    def run_background(
        self, sandbox_id: str, command: str, *, log_path: str = "/tmp/codify-host.log"
    ) -> RemoteCommandResult:
        """
        ...
        ``"ENV=val codify host …"``), and ``nohup`` does NOT honor shell
        ...
        :param command: Shell command to background, e.g.
            ``"ENV=val codify host --server https://…"``.
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        Used to hold ``codify host`` open while the sandbox is
        registered with the App. CLI-bootstrap capability —
        managed-only launchers need not override the raising default.

        :param sandbox_id: Target sandbox.
        :param command: Shell command to execute remotely, e.g.
            ``"codify host --server https://… --profile oss"``.
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        sandbox image (e.g. the Lakebox image bakes codify and its
        ...
        :param remote_tgz_path: Where :func:`~codify.onboarding.
...
class SandboxHostLauncher(SandboxLifecycle):
    ...
    def start_host(
        ...
        *,
        ...
        """
        Start ``codify host`` in the sandbox and return the workspace path.
...
class ExecModelHostLauncher(SandboxHostLauncher, SandboxExecTransport):
    ...
    def start_host(
        ...
        *host_config* into ``~/.codify/config.yaml``, and start the host
        ...
        self.run_background(
            sandbox_id,
            f"{env_prefix} codify host --server {shlex.quote(server_url)}",
        )
        return workspace
