...
optional dependency (``pip install 'codify[boxlite]'``) imported lazily, so
...
- **Local** (no ``endpoint``): ``Boxlite.default()`` — boxes are micro-VMs on
  the codify-server host itself (KVM on Linux / Hypervisor.framework on
...
prebaked host image and starts ``codify host`` over ``run``; it never ships
...
stable, long-lived loop. codify calls launcher methods synchronously off its
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_BOXLITE_HOST_IMAGE"
"""Environment variable overriding
:data:`~codify.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for boxlite
boxes, e.g. an org-internal copy of the host image
(``ghcr.io/<your-org>/codify-host:latest``)."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_BOXLITE_SANDBOX_ENV"
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'codify[boxlite]'`. Local mode also "
    ...
    def __init__(
        ...
        *,
        ...
        :param endpoint: Remote ``boxlite serve`` URL, e.g.
            ...
            (boxes run on the codify-server host via ``Boxlite.default()``).
        ...
        :param image: Registry image reference with codify pre-installed, e.g.
            ``"docker.io/me/codify-host:latest"`` — the server's
