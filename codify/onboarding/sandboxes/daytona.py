...
:class:`~codify.onboarding.sandboxes.base.SandboxLauncher` for
...
(``pip install 'codify[daytona]'``) imported lazily, so the provider
...
bootstrap flow (``codify sandbox create`` / ``connect`` — file
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_DAYTONA_HOST_IMAGE"
"""Environment variable overriding
:data:`~codify.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for
Daytona sandboxes, e.g. an org-internal copy of the host image
(``ghcr.io/<your-org>/codify-host:latest``)."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_DAYTONA_SANDBOX_ENV"
...
# Codify host must survive arbitrary idle gaps between turns, so
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'codify[daytona]'`, "
...
class DaytonaSandboxLauncher(SandboxLauncher):
    ...
    def run(self, sandbox_id: str, command: str, *, check: bool = True) -> RemoteCommandResult:
        ...
        :class:`~codify.onboarding.sandboxes.base.RemoteCommandResult`
    ...
    def terminate(self, sandbox_id: str) -> None:
        ...
        # not an codify dependency — pulling it in for a 3-iteration
