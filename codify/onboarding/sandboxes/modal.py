...
Implements :class:`~codify.onboarding.sandboxes.base.SandboxLauncher`
...
'codify[modal]'``) imported lazily, so the provider can be listed and
...
- **24-hour lifetime.** Modal caps sandbox lifetime at 24 hours;
  ...
  Modal-hosted Codify host must be re-created daily.
...
MODAL_APP_NAME: str = "codify-sandboxes"
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_MODAL_HOST_IMAGE"
"""Environment variable overriding :data:`DEFAULT_HOST_IMAGE`, e.g. for
an org-internal copy of the host image
(``ghcr.io/<your-org>/codify-host:latest``)."""

REGISTRY_SECRET_ENV_VAR: str = "CODIFY_MODAL_REGISTRY_SECRET"
...
SANDBOX_SECRETS_ENV_VAR: str = "CODIFY_MODAL_SANDBOX_SECRETS"
...
# the Codify host's runner + harness processes; 2 vCPU / 4 GiB is
...
def _ensure_sdk() -> None:
    ...
    an optional dependency — the base ``codify`` install does not
    pull it in.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'codify[modal]'`, then "
...
def _build_sandbox_image(image_ref: str | None = None) -> modal.Image:
    ...
    Pulls the prebaked Codify host image — the full codify
    ...
    :param image_ref: Explicit registry image reference, e.g.
        ``"docker.io/me/codify-host:latest"``, or ``None`` to
...
class ModalSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, image: str | None = None, secrets: Sequence[str] | None = None) -> None:
        ...
        :param image: Optional registry image reference to provision
            sandboxes from, e.g. ``"docker.io/me/codify-host:latest"``
        ...
        :param secrets: Optional Modal secret names whose env vars are
            injected into every sandbox, e.g. ``["codify-llm"]`` —
    ...
    def provision(self, name: str) -> str:
        """
        Create a new Modal sandbox under the shared Codify App.
        ...
        :param name: Human-readable label, e.g. ``"codify-host"``.
        ...
        handle.set_tags({"codify-name": name})
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        """
        click.echo(
            f"  → Modal caps sandbox lifetime at 24 hours; re-run `codify "
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        :func:`~codify.onboarding.sandboxes.base.host_image_wheel_install_command`
