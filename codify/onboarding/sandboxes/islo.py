...
Codify's bootstrap needs interactive streaming output.
...
- **Prebaked host image.** Like Modal and Daytona, sandboxes boot from
  the official Codify host image unless overridden. That keeps
...
HOST_IMAGE_ENV_VAR: str = "CODIFY_ISLO_HOST_IMAGE"
"""Environment variable overriding :data:`DEFAULT_HOST_IMAGE` for Islo
sandboxes."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "CODIFY_ISLO_SANDBOX_ENV"
...
for pid in targets:
    try:
        ...
        print(f"could not terminate preserved codify host pid {pid}: {exc}")
...
if targets:
    print(f"stopped preserved codify host daemon(s): {', '.join(map(str, targets))}")
"""
...
def _load_islo_sdk() -> _IsloSDK:
    ...
    The launcher module is importable without ``codify[islo]`` installed;
...
class IsloSandboxLauncher(SandboxLauncher):
    ...
    def _stop_preserved_host_daemon(self, sandbox_id: str) -> None:
        ...
        A paused VM can resume with the old ``codify host`` process still
        ...
        try:
            ...
            click.echo(f"  → warning: could not stop preserved codify host: {exc}", err=True)
