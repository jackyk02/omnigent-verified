"""
Provider-agnostic sandbox bootstrap for Codify hosts.

Composes a :class:`~codify.onboarding.sandboxes.base.SandboxLauncher`
into the full host-bootstrap flow: build the Codify wheels locally,
...
holding ``codify host`` open in it. The end state is a sandbox whose
sessions are reachable from the Codify server's UI, TUI, and
``codify resume``.
...
:mod:`codify.onboarding.sandboxes.lakebox` for the reference
implementation.
"""
...
if TYPE_CHECKING:
    ...
    from codify.onboarding.sandboxes.base import RemoteProcess, SandboxLauncher
...
install: the python client SDK, the UI SDK, and the codify package
...
DEFAULT_SANDBOX_NAME: str = "codify-host"
"""Default label used when ``codify sandbox create`` provisions a
new sandbox."""
...
def build_wheels(
    ...
    *,
    ...
    """
    Build the Codify wheels and pack them into a single tarball.
    ...
    :param repo_root: Path to the codify repo checkout, e.g.
        ``Path("/home/me/codify")``.
    ...
    click.echo(f"▸ Building Codify wheels → {tgz_path}")
    env = os.environ.copy()
...
def ship_wheels(
    ...
    *,
    ...
    """
    Install Codify wheels into a sandbox.
...
def _read_login_url(stream: Iterable[str]) -> str | None:
    ...
    :returns: The authorize URL, or ``None`` when the stream ends
        ...
        ``codify login`` reuses a cached workspace OAuth grant when
...
class DerivedWorkspace:
    ...
    probes of an Codify server URL.
...
def derive_workspace(server_url: str) -> DerivedWorkspace | None:
    ...
    Runs the same unauthenticated detection ``codify login``
    ...
    from codify.cli import _databricks_workspace_login_target
...
def login_app_oauth_in_sandbox(
    ...
    *,
    ...
    """
    Log the sandbox in to *server_url* by running ``codify login``
    **inside the sandbox**, driving the browser step from the local
    ...
    are rejected; only workspace OAuth tokens pass. ``codify
    ...
    verifies), and stores a pointer record so ``codify host`` and
    ...
    1. runs ``codify login <server_url>`` inside the sandbox over a
       PTY;
    ...
    :param server_url: Codify server URL to log in to, e.g.
    ...
    login = launcher.stream_exec(
        sandbox_id,
        f"codify login {shlex.quote(server_url)}",
        pty=True,
    )
    try:
...
def _complete_browser_login(
    ...
    """
    Drive the browser half of an in-sandbox ``codify login``.
    ...
    with launcher.forward_local_port(sandbox_id, port):
        ...
        if returncode != 0:
            raise click.ClickException(
                f"`codify login` inside sandbox '{sandbox_id}' exited with code {returncode}."
            )
...
def set_sandbox_host_name(launcher: SandboxLauncher, sandbox_id: str, host_name: str) -> None:
    """
    Update the sandbox's ``~/.codify/config.yaml`` to use a
    specific host name.
    ...
    ``codify host`` will load that file as-is.
    ...
    """
    click.echo(f"  → setting host name to '{host_name}' in ~/.codify/config.yaml")
    ...
    py = (
        "import os, uuid, yaml; "
        "p=os.path.expanduser('~/.codify/config.yaml'); "
...
def connect_sandbox_host(
    ...
    *,
    ...
    """
    Register the sandbox as a host by running ``codify host`` in it.
    ...
    The remote command is always the bare ``codify host --server
    <url>``: ``codify host`` no longer takes a ``--profile`` flag
    — it resolves credentials itself, via a stored
    ``codify login`` token or the sandbox's ambient Databricks
    ...
    ``~/.codify/config.yaml`` is updated so the host registers
    ...
    :param launcher: The provider's launcher.
    :param sandbox_id: Target sandbox.
    :param server_url: Codify App URL the runner registers with.
    ...
    click.echo("  → running `codify host` in the sandbox (Ctrl-C to detach)")
    returncode = launcher.exec_foreground(sandbox_id, f"codify host --server {server_url}")
    if returncode != 0:
        raise click.ClickException(
            f"`codify host` on sandbox '{sandbox_id}' exited with code {returncode}."
        )
...
def bootstrap_sandbox_host(
    ...
    *,
    ...
    keep-alive → build wheels → ship wheels → ``codify login``
    inside the sandbox.
    ...
    :param sandbox_name: Label for a new sandbox (ignored when
        *sandbox_id* is set).
    :param server_url: Codify server URL the sandbox logs in to.
    ...
    :param repo_root: Path to the codify repo checkout.
