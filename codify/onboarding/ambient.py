...
For the ``codify setup --no-internal-beta`` first-run experience, this module
...
from codify.onboarding.providers import PROVIDER_ENV_VARS
...
class DetectedProvider:
    ...
    :param family: The model family this credential serves
        ...
        credential is detected but maps to no codify harness surface.
...
def _provider_table_has_self_contained_auth(table: dict[str, object]) -> bool:
    ...
    environment variable that codifys would have to thread into the codex
...
def detect_providers() -> list[DetectedProvider]:
    ...
    1. Vendor API keys in the environment, via
       :data:`codify.onboarding.providers.PROVIDER_ENV_VARS`. Only
    ...
    for provider, env_var in PROVIDER_ENV_VARS.items():
        ...
        if provider not in _ENV_KEY_FAMILY:
            continue
        resolved = getenv_nonempty_with_codify_prefix(env_var)
        if resolved is None:
            continue
