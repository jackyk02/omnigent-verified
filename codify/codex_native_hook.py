"""Codex Code hook entrypoint for native Codify policy enforcement.

Registered as the ``PreToolUse`` / ``PostToolUse`` command hook in the
per-session private ``CODEX_HOME`` (see
:mod:`codify.codex_native_app_server`). Codex spawns this module as
...
the Codify policy schema is shared with the Claude-native hook via
:mod:`codify.native_policy_hook`.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"codify codex hook: unknown subcommand {raw_argv[:1]!r}",
        file=sys.stderr,
    )
    return 0
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    :func:`codify.native_policy_hook.hook_payload_to_evaluation_request`,
    ...
    return exit 0 with no output ("no opinion") so non-Codify tool calls
    ...
    :mod:`codify.codex_native_app_server`, not here, because a
    ...
    try:
        ...
        print(f"codify codex evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify codex evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if eval_request is None:
        # Unrecognized hook event or an mcp__codify__* tool (relay-enforced).
        return 0
    ...
    if not resp.content:
        print("codify codex evaluate-policy hook: empty Codify response", file=sys.stderr)
        return _fail_closed()
    ...
    try:
        ...
        print(
            "codify codex evaluate-policy hook: malformed Codify response",
            file=sys.stderr,
        )
        return _fail_closed()
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m codify.codex_native_hook evaluate-policy")
