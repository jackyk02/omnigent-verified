"""Read Codify's user and project configuration."""
...
_GLOBAL_CONFIG_PATH = Path.home() / ".codify" / "config.yaml"
_LOCAL_CONFIG_RELPATH = Path(".codify") / "config.yaml"
...
def _merge_effective_config(
    ...
    :mod:`codify.harness_startup_config` for the ``harness:`` shape.

    :param global_cfg: User-level config (``~/.codify/config.yaml``).
    :param local_cfg: Project-level config (``.codify/config.yaml``).
