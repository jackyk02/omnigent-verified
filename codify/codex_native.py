"""Native Codex TUI wrapper for the Codify CLI."""
...
from codify._wrapper_labels import WRAPPER_LABEL_KEY as _WRAPPER_LABEL_KEY
...
from codify.codex_native_state import read_launch_state, write_launch_state
...
from codify.harness_availability import (
...
def _resolve_codex_auth_source() -> _CodexAuthSource:
    ...
    This is the seam for future managed credentials: once Codify can provide
    ...
    inherited private Codify homes map back to the user's real Codex home.

    :returns: Local Codex auth source to inspect synchronously.
    """
    from codify.inner.codex_executor import _codex_home_config_source_from_env
...
def _find_codex_cli() -> str | None:
    ...
    from codify.onboarding.harness_install import harness_install_spec
    from codify.onboarding.provider_config import OPENAI_FAMILY
...
def run_codex_native(
    *,
    ...
    """
    Launch Codex TUI in an Codify terminal.

    :param server: Resolved Codify server URL, e.g.
        ``"http://127.0.0.1:8123"``.
    :param session_id: Optional existing Codify conversation id,
        e.g. ``"conv_abc123"``.
    ...
    if server is None:
        raise click.ClickException(
            "Codex requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_codex_native."
        )
    with TemporaryDirectory(prefix="codify-codex-native-") as tmpdir:
...
def _materialize_codex_agent_spec(
    ...
    *,
    ...
    """
    Write the terminal-first agent spec used by ``codify codex``.
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local Codify server, launch Codex, and attach to it.

    :param spec_path: Generated Codex wrapper agent spec.
    :param session_id: Optional existing Codify session id.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Codex on an Codify server via a daemon-spawned runner.

    :param base_url: Remote Codify server base URL, e.g.
...
async def _prepare_codex_terminal_via_daemon(
    *,
    ...
    :param base_url: Codify server base URL, e.g.
        ``"https://example.databricks.com"``.
    :param headers: HTTP auth headers for Codify requests.
...
async def _post_initial_prompt(
    *,
    ...
    """
    Send the first Codex prompt through Codify instead of the app-server.
    ...
    :returns: None.
    :raises click.ClickException: If Codify rejects the prompt.
    """
...
async def _prepare_codex_terminal(
    *,
    ...
    async with httpx.AsyncClient(base_url=base_url, headers=headers, timeout=timeout) as client:
        ...
        # so `codify codex` honors the provider selection like the
        ...
        try:
            ...
            if thread_id is None:
                event_client = client_for_transport(
                    codex_ws_url,
                    client_name="codify-codex-native",
                )
            ...
            launched_terminal = await _launch_codex_terminal(
                ...
                # the app-server so it resolves the Codify provider
...
async def _attach_terminal_resource(
    *,
    ...
    """
    Attach the current terminal to the prepared Codify terminal resource.
...
def _active_codex_session_id(bridge_dir: Path) -> str | None:
    """
    Return the active Codify session id for a native Codex bridge.

    :param bridge_dir: Native Codex bridge directory.
    :returns: Codify session id, e.g. ``"conv_abc123"``, or ``None`` when
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> None:
    ...
    environment so users who run ``codify codex`` inside their own
    tmux session can still attach to Codify' private tmux server.
...
async def _fetch_codex_session(client: httpx.AsyncClient, session_id: str) -> dict[str, Any]:
    """
    Fetch an existing Codify session.
...
def _mint_codex_thread_id() -> str:
    ...
    so the clone gets its own Codify ``external_session_id`` — mirroring how
...
def _find_codex_rollout(codex_home: Path, thread_id: str) -> Path | None:
    ...
    :param codex_home: A per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.codify/codex-native/<hash>/codex-home")``.
...
def _clone_codex_rollout(
    ...
    *clone_workspace* (see :func:`_copy_rollout_with_cwd`), preserving the
    ...
    :func:`codify.claude_native._clone_claude_transcript`. See
    ...
    :param clone_codex_home: The clone's per-session private ``CODEX_HOME``,
        e.g. ``Path("~/.codify/codex-native/<hash>/codex-home")``.
...
async def _ensure_local_codex_resume_rollout(
    ...
    *,
    ...
    Cross-machine resume has the Codify conversation and Codex thread id on
    ...
    state, not a cache that Codify should rewrite.
    ...
    :param model_provider: Provider id this session's launch routes through,
        e.g. ``"codify_databricks"`` (see
        :func:`codify.codex_native_app_server.codex_session_meta_model_provider`).
    ...
    :raises click.ClickException: If Codify history cannot be fetched or the
    ...
    if codex_path is not None:
        from codify.inner.codex_executor import _codex_cli_version
...
def _codex_resume_rollout_path(codex_home: Path, external_session_id: str) -> Path:
    ...
    :param codex_home: Per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.codify/codex-native/x/codex-home")``.
...
async def _fetch_all_session_items_for_codex_resume(
    ...
    """
    Fetch committed Codify session items in chronological order.
...
def _codex_rollout_records_from_session_items(
    ...
    *,
    ...
    """
    Convert Codify session items into Codex rollout JSONL records.

    The generated records follow Codex's rollout shape: one
    ``session_meta`` record, a ``turn_context`` before each Codify response
...
def _interrupted_response_ids_from_session_items(items: list[dict[str, Any]]) -> set[str]:
    """
    Return response ids for Codify turns that ended interrupted.

    A Codex interrupted turn is persisted in Codify as visible transcript text
...
def _session_item_response_id(item: dict[str, Any]) -> str | None:
    """
    Extract a non-empty Codify response id from a flat item.
...
def _codex_response_item_from_session_item(item: dict[str, Any]) -> dict[str, Any] | None:
    """
    Convert one Codify item into one Codex ``response_item`` payload.

    :param item: Flat Codify item dict, e.g.
        ``{"type": "function_call", "name": "shell", ...}``.
    :returns: Responses-style item payload, or ``None`` for unsupported
        or empty Codify items.
    """
...
def _is_interrupted_assistant_session_item(item: dict[str, Any]) -> bool:
    """
    Return whether an Codify item is an interrupted assistant partial.

    Codify persists these messages so the web transcript can show the text and
...
def _codex_response_item_payload(item: dict[str, Any]) -> dict[str, Any] | None:
    """
    Convert one supported Codify item into a Codex response payload body.

    :param item: Flat Codify item dict.
    :returns: Payload without the optional item id, or ``None`` for
        unsupported / empty Codify items.
    """
...
def _codex_message_payload_from_session_item(item: dict[str, Any]) -> dict[str, Any] | None:
    """
    Convert an Codify message item into a Codex message payload.

    :param item: Codify message item.
...
def _codex_function_call_payload_from_session_item(
    ...
    """
    Convert an Codify function call item into a Codex function call payload.

    :param item: Codify function call item.
    :returns: Codex function call payload, or ``None`` when optional
        routing fields are absent.
    :raises click.ClickException: If the Codify item violates required tool
        history fields.
    """
    name = item.get("name")
    ...
    if not isinstance(arguments, str):
        item_id = item.get("id")
        raise click.ClickException(
            "Cannot synthesize Codex resume rollout: Codify function_call "
...
def _codex_function_call_output_payload_from_session_item(
    ...
    """
    Convert an Codify function output item into a Codex function output payload.

    :param item: Codify function output item.
    ...
    if not isinstance(output, str):
        item_id = item.get("id")
        raise click.ClickException(
            "Cannot synthesize Codex resume rollout: Codify function_call_output "
...
def _codex_content_blocks_from_api_content(
    ...
    *,
    ...
    """
    Extract text blocks from an Codify content array for Codex rollout items.
...
def _codex_turn_id_for_session_item(
    *,
    ...
    """
    Return a Codex turn id for an Codify item.

    Codex-native forwarder stores Codify ``response_id`` as
    ...
    return (
        ...
        + uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"codify-codex-resume:{session_id}:{external_session_id}:{stable}",
        ).hex
    )
...
async def _patch_external_session_id(
    ...
    """
    Persist the native Codex thread id on the Codify session.
...
async def _wait_for_thread_started(client: CodexAppServerClient) -> str:
    ...
    :func:`codify.codex_native_forwarder.wait_for_thread_started`
    ...
    """
    from codify.codex_native_forwarder import wait_for_thread_started
...
async def _start_initial_turn(app_server_url: str, thread_id: str, prompt: str) -> None:
    ...
    :returns: None.
    """
    client = client_for_transport(app_server_url, client_name="codify-codex-native")
...
async def _launch_codex_terminal(
    ...
    *,
    ...
    :param client: HTTP client pointed at AP.
    :param session_id: Codify session id.
    :param codex_args: Raw Codex CLI args.
    :param command: Codex executable.
...
def _response_error_code(resp: httpx.Response) -> str | None:
    """
    Extract a structured Codify error code from *resp* if present.
...
def _runner_offline_message(message: str) -> bool:
    """
    Return whether *message* is the Codify stale-runner error shape.
...
def _resolve_session_id_for_resume(
    *,
    ...
    async def _drive() -> str | None:
        ...
        :returns: Selected Codify session id, or ``None``.
        """
