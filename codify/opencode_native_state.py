"""Persistent client-side state for ``codify opencode`` sessions.

The native OpenCode wrapper records the cwd used to create a session so a
later ``codify opencode --resume <conv_id>`` can launch OpenCode from
...
the shared Codify server. Mirrors :mod:`codify.codex_native_state`.

Layout (per conversation):

    ~/.codify/opencode-native/<sha256(conv_id)[:32]>/launch.json
"""
...
_STATE_ROOT_ENV_VAR = "CODIFY_OPENCODE_NATIVE_STATE_DIR"
...
def _opencode_native_state_root() -> Path:
    ...
    Production defaults to ``~/.codify/opencode-native``.
    ...
    return Path.home() / ".codify" / "opencode-native"
