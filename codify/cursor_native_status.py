...
:func:`codify.cursor_native_bridge.build_hooks_config`, which registers the
:mod:`codify.cursor_native_usage` recorder). That hook is the ONLY
authoritative "this turn just finished" signal cursor-agent exposes: its SQLite
chat store (tailed by :mod:`codify.cursor_native_forwarder`) carries no
...
:func:`codify.cursor_native_forwarder.forward_cursor_store_to_session` poll
...
def record_turn_end(bridge_dir: Path, payload: object | None = None) -> None:
    ...
    which :func:`codify.cursor_native_usage.record_usage_payload` skips — so the
...
def clear_cursor_status_state(bridge_dir: Path) -> None:
    ...
    Sibling of :func:`codify.cursor_native_usage.clear_cursor_usage_state`: the
