"""Kimi Code hook commands for the native Codify wrapper.

Registered into a per-session ``config.toml`` ``[[hooks]]`` array (see
:mod:`codify.kimi_native_credentials`) so the running ``kimi`` TUI invokes
...
- ``evaluate-policy`` — the ``PreToolUse`` deny-gate. Mirrors
  :func:`codify.claude_native_hook._main_evaluate_policy`: it converts the
  Kimi hook payload into an Codify ``EvaluationRequest`` (the snake-cased
  Kimi fields ``tool_name`` / ``tool_input`` / ``hook_event_name`` line up
  with :func:`codify.native_policy_hook.hook_payload_to_evaluation_request`),
  ...
  runs — Codify enforces its deny-policy without silencing the user's
...
- ``permission-request`` — the interactive web-UI approval. Kimi fires
  ...
  :data:`~codify.kimi_native_bridge.APPROVE_KEY` "Approve once" /
  :data:`~codify.kimi_native_bridge.DENY_KEY` "Reject"). Fail-safe: on no
...
def _read_stdin_payload() -> dict[str, object] | None:
    ...
    try:
        ...
        print(f"codify kimi hook: malformed JSON: {exc}", file=sys.stderr)
        return None
    if not isinstance(payload, dict):
        print("codify kimi hook: expected JSON object", file=sys.stderr)
        return None
    return payload


def _main_evaluate_policy(argv: list[str]) -> int:
    """Evaluate a kimi ``PreToolUse`` hook against Codify policies.
    ...
    CLOSED with a ``deny`` (this hook is the sole Codify enforcement point
    for kimi tool calls).
    ...
    try:
        ...
        print(
            "codify kimi evaluate-policy hook: malformed Codify response",
            file=sys.stderr,
        )
        return _fail_closed()
...
def _main_permission_request(argv: list[str]) -> int:
    ...
    body: dict[str, object] = {
        ...
        "_codify_elicitation_id": f"elicit_kimi_{secrets.token_hex(16)}",
    }
    ...
    try:
        ...
        print(
            f"codify kimi permission-request hook: keystroke inject failed: {exc}",
            file=sys.stderr,
        )
    return 0
...
def _request_web_approval(
    ...
    try:
        ...
        print(
            f"codify kimi permission-request hook: approval request failed: {exc}",
            file=sys.stderr,
        )
        return None
    if not resp.content:
...
def _parse_bridge_dir_args(argv: list[str], prog: str) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog=f"codify.kimi_native_hook {prog}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"codify kimi hook: unknown subcommand {subcommand!r}", file=sys.stderr)
    return 2
