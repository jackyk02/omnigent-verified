"""Native Kiro TUI wrapper for the Codify CLI."""
...
from codify._wrapper_labels import KIRO_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIRO_PATH_ENV = "CODIFY_KIRO_PATH"
...
def run_kiro_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Kiro requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_kiro_native."
        )
    with TemporaryDirectory(prefix="codify-kiro-native-") as tmpdir:
...
def _materialize_kiro_agent_spec(tmpdir: Path, *, model: str | None = None) -> Path:
    """Write the terminal-first agent spec used by ``codify kiro``."""
...
def _run_with_remote_server(
    ...
    *,
    ...
    """Launch Kiro on an Codify server via a daemon-spawned runner."""
