...
The ``codify goose`` wrapper launches the real ``goose session`` TUI in a
runner-owned tmux pane, and :mod:`codify.goose_native_bridge` injects web-UI
...
:mod:`codify.cursor_native_forwarder`. Goose stores sessions in a SQLite
...
runner launches ``goose session --name <codify-session-id>``, discovery is a
...
from codify._native_post_delivery import post_external_session_status
...
def _resolve_goose_session_id(db_path: Path, session_name: str) -> str | None:
    ...
    The runner launches ``goose session --name <codify-session-id>``; the row
...
async def forward_goose_store_to_session(
    *,
    ...
    ``--name <codify-session-id>`` the runner launched with), then polls its
...
async def supervise_goose_forwarder(
    *,
    ...
    Mirrors :func:`codify.cursor_native_forwarder.supervise_cursor_forwarder`:
