...
crash screen (see :mod:`codify.crash_ui`) and lets the user file a
...
from codify.crash_ui import real_stderr, render_crash_screen
from codify.process_logging import data_dir
...
_CONFIG: _CrashConfig = {
    "app_name": "codify",
    "repo": "codify-ai/codify",
    "version": "unknown",
    "crashes_dir": None,
    "keep_reports": 10,
    "first_party_prefixes": ("codify",),
}
...
def install_crash_handler(
    ...
    *,
    ...
    first_party_prefixes: tuple[str, ...] = ("codify",),
) -> None:
    ...
    :param first_party_prefixes: Top-level package prefixes treated as
                      ...
                      ``("codify")``, which covers the three core
                      packages (``codify``, ``codify_client``,
                      ``codify_ui_sdk``) via the ``<prefix>_`` rule.
    """
    ...
    _CONFIG["first_party_prefixes"] = tuple(first_party_prefixes) or ("codify",)
...
def handle_crash(
    ...
    *,
    ...
    try:
        ...
        # library frames); see codify/crash_ui.render_crash_screen.
        ...
        render_crash_screen(
            ...
            stream=stream,
            first_party_prefixes=_CONFIG.get("first_party_prefixes", ("codify",)),
        )
...
def _issue_body(exc: BaseException, tb_text: str) -> str:
    ...
    msg = str(exc).strip() or "(no message)"
    return (
        "This crash was auto-reported by Codify's crash handler.\n\n"
