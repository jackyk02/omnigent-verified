...
``goose configure``), and Codify does not mutate user config in v1.
"""
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "goose-native"
_TMUX_FILE = "tmux.json"
_TMUX_READY_TIMEOUT_S = 30.0
_TMUX_SEND_TIMEOUT_S = 10.0
_POLL_INTERVAL_S = 0.2
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "codify-goose-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/codify-<uid>/goose-native/<hash>``."""
...
def capture_goose_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`codify.goose_native_permissions`) to detect Goose's in-terminal
