"""Shared conversion between native-harness hooks and Codify policy events.
...
entrypoints (:mod:`codify.claude_native_hook`,
:mod:`codify.codex_native_hook`) share one implementation.
...
# ``codify.runner.app._evaluate_policy_via_codify``.
_EVAL_UNAVAILABLE_REASON = (
    "Codify policy evaluation unavailable (could not reach or authenticate to the "
    "Codify server); failing closed for this tool call."
)
_EVAL_UNAVAILABLE_REQUEST_REASON = (
    "Codify policy evaluation unavailable (could not reach or authenticate to the "
    "Codify server); failing closed for this request."
)
...
_AUTH_HEADERS_ENV = "_CODIFY_AUTH_HEADERS"

# Relay env vars for harnesses that deliver auth via env (hermes, cursor).
_RELAY_URL_ENV = "_CODIFY_RELAY_URL"
_RELAY_TOKEN_ENV = "_CODIFY_RELAY_TOKEN"
...
def policy_hook_wrapper_script(server_url: str, session_id: str, hook_script_path: str) -> str:
    ...
    Resolves a one-shot Codify-server token and bakes the auth +
    workspace-routing headers (via
    :func:`codify.cli_auth.databricks_request_headers`) into
    ...
    :param server_url: Codify server base URL the hook posts to.
    ...
    return (
        "#!/bin/sh\n"
        f"export _CODIFY_SERVER_URL={shlex.quote(server_url)}\n"
        f"export _CODIFY_SESSION_ID={shlex.quote(session_id)}\n"
...
class PolicyHookReauth:
    """Callable that re-mints the Codify bearer for a policy hook subprocess.
...
def _is_login_redirect_or_unauthorized(response: httpx.Response) -> bool:
    ...
    Mirrors :func:`codify.runner._entry._is_login_redirect_or_unauthorized`,
...
def hook_payload_to_evaluation_request(
    ...
    ``prompt`` field becomes the request content). Codify MCP tools
    (``mcp__codify__*``) are skipped because they are already
    policy-checked by the relay path (``ProxyMcpManager`` → Codify
    ...
    :returns: An ``EvaluationRequest`` dict suitable for POSTing to
        ...
        policy-relevant (unknown event or an ``mcp__codify__*`` tool).
    """
    ...
    # Codify MCP tools are already policy-checked by the relay path
    # (ProxyMcpManager → Codify /mcp endpoint → _evaluate_tool_call_policy).
    ...
    if isinstance(tool_name, str) and tool_name.startswith("mcp__codify__"):
        return None
...
def fail_closed_hook_output(
    ...
    :data:`codify.policies.types.FAIL_CLOSED_PHASES` (the runner-side
...
def post_evaluate_with_retry(
    ...
    """
    POST to the Codify policy evaluate endpoint, retrying on transient errors.
    ...
    A stable ``_codify_elicitation_id`` is minted once and stamped on
    ...
    :param headers: Auth headers for the Codify server.
    ...
    :param reauth: Optional callable that re-mints fresh auth headers when
        ...
        refresh-capable :class:`~codify.runner._entry._RunnerDatabricksAuth`.
    ...
    request_body = {**eval_request, "_codify_elicitation_id": elicitation_id}
    ...
    while True:
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"codify {hook_label}: Codify auth expired "
            ...
            if status < 500:
                print(
                    f"codify {hook_label}: Codify returned {status}"
                    + (f": {body_preview}" if body_preview else ""),
                    file=sys.stderr,
                )
                return None, last_error
            print(
                f"codify {hook_label}: Codify returned {status}; retrying",
                file=sys.stderr,
            )
        except (httpx.ConnectError, httpx.ConnectTimeout) as exc:
            last_error = f"connection error: {exc}"
            print(
                f"codify {hook_label}: Codify request failed; retrying: {exc}",
                file=sys.stderr,
            )
            ...
            print(
                f"codify {hook_label}: Codify request failed: {exc}",
                file=sys.stderr,
            )
            return None, last_error
        if time.monotonic() + backoff_s >= deadline:
            print(
                f"codify {hook_label}: retry budget exhausted",
                file=sys.stderr,
            )
