...
``~/.pi/plans/codify/harness-startup-command-overrides.md``.

This is a leaf resolver module: it lazy-imports
:func:`codify.harness_aliases.canonicalize_harness` so it can be used
from :mod:`codify.config` (and the CLI) without pulling heavy
...
``command`` —
  ...
  2. ambient env var (``CODIFY_<NAME>_PATH``)
...
# Legacy ``HARNESS_*_PATH`` env vars and their canonical ``CODIFY_<NAME>_PATH``
...
_LEGACY_PATH_VARS: dict[str, str] = {
    "HARNESS_CODEX_PATH": "CODIFY_CODEX_PATH",
    "HARNESS_PI_PATH": "CODIFY_PI_PATH",
    "HARNESS_KIMI_PATH": "CODIFY_KIMI_PATH",
    "HARNESS_GOOSE_PATH": "CODIFY_GOOSE_PATH",
    "HARNESS_QWEN_PATH": "CODIFY_QWEN_PATH",
    "HARNESS_HERMES_PATH": "CODIFY_HERMES_PATH",
}
...
# ``CODIFY_CLAUDE_PATH`` with ``claude-native``. Add entries here only when a
...
def resolve_harness_command(
    ...
    *,
    ...
    # Check both the canonical CODIFY_* and the deprecated HARNESS_* env var
...
def config_harness_path_override(
    ...
    ``CODIFY_<NAME>_PATH`` env var — but only when the user hasn't already
    ...
    ``if v: env["CODIFY_X_PATH"] = v``.

    :param harness: A harness id (canonical or alias), e.g. ``"codex"``.
    :param cfg: Effective config dict.
    :returns: The config command string to set as ``CODIFY_<NAME>_PATH``,
        or ``None`` when config has no override or the ambient env var is set.
    """
    canonical = _canonicalize(harness)
    # Ambient env wins over config — check BOTH the canonical CODIFY_* and
