...
- ``--json-file`` (:func:`events_file_path`): ``qwen`` streams structured JSON
  events here while the TUI renders normally; :mod:`codify.qwen_native_forwarder`
  tails it.
...
#: Fixed namespace for deriving a stable qwen ``--session-id`` from an Codify
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "qwen-native"
...
#: Token config the shared Codify MCP relay (``serve-mcp``) reads from the
#: bridge dir. Mirrors cursor-/claude-native (``cursor_native_bridge.py``).
_BRIDGE_CONFIG_FILE = "bridge.json"
#: Name qwen lists the Codify MCP server under (shows in ``/mcp``).
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/codify-<uid>/qwen-native/<hash>``."""
...
def qwen_session_id_for_conversation(conversation_id: str) -> str:
    """Return the deterministic qwen ``--session-id`` for an Codify conversation.
...
def qwen_session_recording_path(session_id: str, workspace: Path | str) -> Path:
    ...
    ``chat_compression`` marker (see :mod:`codify.qwen_native_forwarder`).
    """
    return (
        Path.home()
        / ".qwen"
...
def _qwen_text_from_api_content(content: object, api_type: str) -> str:
    """Concatenate the text of an Codify content array's blocks of *api_type*.

    :param content: Codify content array, e.g. ``[{"type":"input_text","text":"hi"}]``.
...
def qwen_session_records_from_session_items(
    ...
    *,
    ...
    """Convert Codify session items into qwen chat-recording JSONL records.
...
def write_qwen_session_recording(
    ...
    *,
    ...
    try:
        hostname = socket.gethostname() or "codify"
    except OSError:
        hostname = "codify"
...
def _ensure_secure_bridge_dir(bridge_dir: Path) -> None:
    ...
    an attacker could pre-create ``$TMPDIR/codify-<uid>`` (or a deeper ancestor)
    ...
    """
    from codify.claude_native_bridge import _ensure_secure_dir
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :func:`codify.cursor_native_bridge.write_mcp_bridge_config`.
...
def build_mcp_server_entry(
    ...
    *,
    ...
    """Build qwen's ``mcpServers.codify`` entry for the Codify relay.

    ``trust: true`` auto-approves the qwen-side MCP tool gate so the TUI doesn't
    add a second in-terminal prompt: Codify already gates these calls through
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the per-session Codify MCP config for qwen's ``--mcp-config`` flag.

    Writes ``{"mcpServers": {"codify": ...}}`` to a file *inside the bridge dir*
