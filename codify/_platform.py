...
codify grew up on Linux/macOS and bakes a number of POSIX assumptions into
...
#: Non-sensitive Windows environment variables that a spawned codify
...
def stable_user_id() -> str:
    ...
    ``codify-<id>`` / ``claude-<id>`` under the temp dir). On POSIX this is
...
def resolve_repo_symlink(path: Path) -> Path:
    ...
    *regular file* whose entire content is the link target — e.g. the checked-out
    ``codify/resources/examples/polly`` is a 23-byte file containing
