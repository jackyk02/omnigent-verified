"""Shared process logging setup for Codify entrypoints."""
...
from codify._platform import IS_POSIX

DATA_DIR_ENV_VAR = "CODIFY_DATA_DIR"
LOG_LEVEL_ENV_VAR = "CODIFY_LOG_LEVEL"
LOG_TO_STDERR_ENV_VAR = "CODIFY_LOG_TO_STDERR"
LOG_FORCE_COLOR_ENV_VAR = "CODIFY_LOG_FORCE_COLOR"
PROCESS_LOG_FILE_ENV_VAR = "CODIFY_PROCESS_LOG_FILE"
LOG_TTY_FD_ENV_VAR = "CODIFY_LOG_TTY_FD"
...
class _ProcessLogStreamHandler(logging.StreamHandler[TextIO]):
    _codify_process_log_stderr: bool


class _ProcessLogFileHandler(logging.FileHandler):
    _codify_process_log_path: str
...
def short_logger_name(name: str) -> str:
    ...
    for prefix in ("codify.", "codify_ui_sdk."):
...
def log_record_display_fields(
    ...
    *,
    ...
    """Temporarily add Codify display columns to a log record."""
...
def data_dir() -> Path:
    ...
    return Path(value).expanduser() if value else Path.home() / ".codify"
...
def effective_log_level(default: str = "INFO") -> int:
    """Resolve the effective numeric logging level from ``CODIFY_LOG_LEVEL``."""
...
def terminal_supports_color() -> bool:
    ...
    # Codify-owned mirrors (omnidev panes) may force ANSI; otherwise NO_COLOR wins.
...
def terminal_stream_handler() -> logging.Handler:
    ...
    handler._codify_process_log_stderr = True
    return handler
...
def configure_process_logging(
    ...
    *,
    ...
    logger_names: Sequence[str] = ("codify",),
    root: bool = True,
    force: bool = False,
) -> Path:
    ...
    file_handler._codify_process_log_path = str(path)
...
def _add_handler_once(logger: logging.Logger, handler: logging.Handler) -> None:
    path = getattr(handler, "_codify_process_log_path", None)
    is_stderr = getattr(handler, "_codify_process_log_stderr", False)
    for existing in logger.handlers:
        if path is not None and getattr(existing, "_codify_process_log_path", None) == path:
            handler.close()
            return
        if is_stderr and getattr(existing, "_codify_process_log_stderr", False):
            handler.close()
            return
