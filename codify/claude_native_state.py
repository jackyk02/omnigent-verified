"""Persistent client-side state for ``codify claude`` sessions.
...
Why under ``~/.codify/`` and not the existing bridge dir at
``/tmp/codify-<uid>/claude-native/``: tmpfs gets cleared on
...
the day after creating it still gets the chdir prompt. ``~/.codify/``
is where the persistent Codify server SQLite db and other durable
single-user state already live.

Layout (per conversation):

    ~/.codify/claude-native/<sha256(conv_id)[:32]>/launch.json

The directory is hashed (not the raw conv id) so a malicious server
returning an attacker-chosen conversation id like ``"../../../etc"``
cannot escape ``~/.codify/claude-native/``. A short hash is enough
...
# ``~/.codify/claude-native``. Tests should set this to a per-
# test ``tmp_path`` via monkeypatch.setenv so they never touch the
# user's real home directory.
_STATE_ROOT_ENV_VAR = "CODIFY_CLAUDE_NATIVE_STATE_DIR"
...
# :mod:`codify.claude_native_bridge`.
_ID_HASH_CHARS = 32
...
def _claude_native_state_root() -> Path:
    ...
    directories on disk). Lives under ``~/.codify/claude-native/``
    ...
    return Path.home() / ".codify" / "claude-native"
...
def _state_dir_for_conversation_id(conversation_id: str) -> Path:
    ...
    :param conversation_id: Codify conversation id, bare 32-char hex
...
def read_launch_state(conversation_id: str) -> ClaudeNativeLaunchState | None:
    ...
    * the picker, to render the Workspace column for each row.
    ...
    user who wiped ``~/.codify/claude-native/``). The callers
