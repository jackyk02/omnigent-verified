...
Everything behind ``codify config`` / ``codify setup`` and the first-run
...
Extracted from :mod:`codify.cli` to keep that module under the repo's
per-file size budget; the ``config`` / ``setup`` Click commands stay in
``codify.cli`` and reach these helpers via :func:`register`.

The three config-load helpers these functions need (``_load_global_config``,
``_save_global_config``, ``_load_effective_config``) live in ``codify.cli`` and
are used ~20x each there, so they stay put; this module reaches them through
call-time proxies below, so importing this module never imports ``codify.cli``
(no cycle) and a test's ``monkeypatch`` of ``codify.cli.<helper>`` is still
honoured.
"""
...
from codify.onboarding.ucode_setup import (
...
if TYPE_CHECKING:
    ...
    from codify.onboarding.openclaw_config import OpenClawDiscovery, SourceKind
...
# codify.onboarding.internal_beta (excluded from the OSS build); the
...
def _isolated_databricks_cfg() -> collections.abc.Generator[None, None, None]:
    ...
    the direct configparser writes in ``codify.onboarding.setup``
    ...
    import codify.onboarding.internal_beta as internal_beta  # type: ignore[import-not-found]
    from codify.onboarding.setup import CONFLICTING_ENV_VARS
    ...
    codify_dir = Path.home() / ".codify"
    codify_dir.mkdir(exist_ok=True)
...
def _run_configure_databricks() -> None:
    ...
    Codify reads ``~/.ucode/state.json`` to pick per-harness model
    ...
    click.echo("ucode configuration complete. Codify will use state.json for harness setup.")
...
def _warn_missing_harness_dependencies() -> None:
    ...
    wrapper needs it (Node when a harness CLI runs, tmux when ``codify
    ...
    hard failure would block a valid flow — but ``codify claude`` /
    ``codex`` do need both, hence the prominent notice.

    :returns: None. Side effect: writes a yellow warning block to stderr
        via :mod:`codify.inner.ui` when one or more dependencies are
        missing.
    """
    problems: list[str] = []
...
def _print_credentials_by_harness() -> None:
    ...
    :returns: None. Side effect: writes the listing to the onboarding
        console.
    """
    from codify.onboarding.configure_models import render_provider_listing_by_harness
...
def _existing_key_name_for_ref(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    """
    from codify.onboarding.provider_config import KEY_KIND, load_providers
...
def _family_credential_label(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    """
    from codify.onboarding.provider_config import KEY_KIND
...
def _configure_harness_add(family: str | None = None) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        status. Side effect: writes to ``~/.codify/config.yaml`` and,
        for a pasted API key, the secret store.
    """
    from codify.onboarding import secrets as secret_store
    from codify.onboarding.ambient import detect_providers
    ...
    from codify.onboarding.interactive import console, prompt_text, select
    from codify.onboarding.provider_config import (
        ANTHROPIC_FAMILY,
        BEDROCK_KIND,
        CHAT_WIRE_API,
    ...
    if kind == CLI_CONFIG_KIND:
        ...
        from codify.onboarding.providers import default_chat_model
        ...
        from codify.onboarding.harness_install import harness_install_spec, harness_login
        ...
        # (codify/runtime/credentials/databricks.py), and the SDK is no
        ...
        # the one place Codify triggers a Databricks CLI / ucode login;
        ...
        from codify.onboarding.configure_models import family_label
        from codify.onboarding.databricks_config import normalize_workspace_url
        from codify.onboarding.interactive import clear_screen
        from codify.onboarding.setup import login_databricks_workspace
        from codify.onboarding.ucode_setup import (
...
def _adopt_detected_providers() -> list[str]:
    ...
    """
    from codify.onboarding.detected import (
...
def _promote_global_auth_to_provider() -> str | None:
    ...
    Older ``codify setup`` runs configured Databricks only via the top-level
    ...
    """
    from codify.onboarding.configure_models import build_databricks_provider_entry
    from codify.onboarding.provider_config import (
        load_providers,
        provider_entry_settings,
        provider_families,
...
def _compact_credential_label(det: DetectedProvider) -> str:
    ...
    Unlike :func:`codify.onboarding.configure_models.credential_label`
    ...
    :param det: A credential found by
        :func:`codify.onboarding.ambient.detect_providers`.
    :returns: A short human label, e.g. ``"Anthropic API Key"``,
        ``"Claude Subscription"``, or ``"ChatGPT Subscription"``.
    """
    from codify.onboarding.ambient import SUBSCRIPTION_KIND
    from codify.onboarding.configure_models import credential_label
...
def _announce_auto_configured_credentials(adopted: list[str]) -> None:
    ...
    credentials codify picked up (rather than silently inheriting them).
    ...
    """
    from codify.onboarding.ambient import detect_providers
    from codify.onboarding.interactive import console
    ...
    console.print(
        ...
        f"auto-configured for codify: {', '.join(labels)}[/dim]"
    )
...
def _adopt_ambient_credentials(progress: RunnerStartupProgress | None = None) -> list[str]:
    ...
    The shared front half of both a bare ``codify run``'s first-run path
    ...
    :param progress: Optional spinner handle (from
        :func:`codify._runner_startup.runner_startup_progress`) covering the
...
def _prompt_install_harness(family: str) -> bool:
    ...
    """
    from codify.onboarding.configure_models import family_label
    from codify.onboarding.harness_install import (
        harness_cli_version,
        harness_install_command,
        install_harness_cli,
    )
    from codify.onboarding.interactive import console, select
...
def _manage_harness_providers(family: str) -> None:
    ...
    """
    from codify.onboarding.configure_models import family_label
    from codify.onboarding.harness_install import harness_cli_installed
    from codify.onboarding.interactive import select
...
def _prompt_install_cursor() -> str | None:
    ...
    :func:`codify.onboarding.cursor_auth.cursor_install_command`.
    ...
    from codify.onboarding.cursor_auth import CURSOR_EXTRA, install_cursor_sdk
    from codify.onboarding.extra_install import extra_install_display
    from codify.onboarding.interactive import console, select
...
def _manage_cursor_sdk_harness() -> None:
    ...
    API key stored in the codify secret store, mirroring how the other
    harnesses persist their api keys (the secret in the store, a
    ``keychain:``/``env:`` reference in ``~/.codify/config.yaml``).
    ...
    :returns: None. Side effects: may install the ``cursor`` extra, and may
        write the ``cursor:`` block of ``~/.codify/config.yaml`` and the
        secret store.
    """
    from codify.onboarding import secrets as secret_store
    from codify.onboarding.cursor_auth import (
...
def _set_cursor_api_key() -> str | None:
    ...
    codify secret store under ``keychain:cursor``. The ``crsr_`` prefix is
    ...
    from codify.onboarding.interactive import prompt_text
...
def _manage_cursor_harness() -> None:
    ...
    from codify.onboarding.cursor_auth import cursor_api_key_configured
    from codify.onboarding.harness_install import (
        CURSOR_KEY,
        harness_cli_installed,
        harness_cli_logged_in,
    ...
    while True:
        from codify._platform import resolve_cli_binary
...
def _prompt_install_antigravity() -> str | None:
    ...
    from codify.onboarding.antigravity_auth import ANTIGRAVITY_EXTRA, install_antigravity_sdk
    from codify.onboarding.extra_install import extra_install_display
    from codify.onboarding.interactive import console, select
...
def _manage_antigravity_harness() -> None:
    ...
    """
    from codify.onboarding import secrets as secret_store
    from codify.onboarding.antigravity_auth import (
    ...
    from codify.onboarding.interactive import select
    from codify.onboarding.provider_config import GEMINI_FAMILY
...
def _manage_qwen_harness() -> None:
    ...
    Codify* is deferred (see docs/QWEN_FOLLOWUPS.md, Provider Injection).
...
def _print_goose_auth_help() -> None:
    """Print Goose's configuration options (Codify manages no Goose credential)."""
    from codify.onboarding.interactive import console

    console.print(
        "\n  [bold]Configure Goose[/bold] (Codify stores no Goose credential):\n"
...
def _launch_goose_configure() -> str | None:
    ...
    """
    from codify.onboarding.goose_auth import goose_config_summary
    from codify.onboarding.harness_install import (
        GOOSE_KEY,
        harness_cli_installed,
        harness_install_spec,
...
def _manage_goose_harness() -> None:
    ...
    Goose owns its own auth (keyring / ``~/.config/goose/config.yaml``) — Codify
    ...
    does not store a key through Codify. A missing CLI gates the drill-in
...
def _print_acp_examples() -> None:
    """Print example ACP-agent commands (Codify stores no credential)."""
    from codify.onboarding.interactive import console
    ...
    console.print(
        ...
        "  Codify stores no credential — log into each agent via its own CLI first.\n\n"
...
def _add_acp_agent() -> None:
    ...
    """
    from codify.onboarding.acp_auth import (
        AcpAgentEntry,
        acp_agents,
        acp_agents_settings,
        slugify,
    )
    from codify.onboarding.interactive import console, prompt_text
...
def _choose_openclaw_source() -> OpenClawDiscovery | None:
    ...
    from codify.onboarding.interactive import prompt_text, select
...
def _import_openclaw_agents() -> None:
    ...
    from codify.onboarding.acp_auth import acp_agents_settings, command_binary_on_path
    from codify.onboarding.interactive import console
    from codify.onboarding.openclaw_config import (
    ...
    while True:
        ...
        console.print("  [dim]Codify stores only these launch commands, not credentials.[/dim]")
...
def _manage_acp_agent(slug: str) -> None:
    ...
    A single-shot menu (Remove / Back) — Codify stores no credential, so there
    is nothing else to manage per agent yet.

    :param slug: The agent's slug (see :func:`codify.onboarding.acp_auth.slugify`).
    """
    from codify.onboarding.acp_auth import acp_agents, acp_agents_settings
    from codify.onboarding.interactive import console, select
...
def _manage_hermes_harness() -> None:
    ...
    picker) and is installed via a curl script from Nous Research — Codify
...
def _manage_kiro_harness() -> None:
    ...
    Identity Center) and is installed via Kiro's curl installer — Codify stores
...
def _print_kimi_auth_help() -> None:
    ...
    Kimi authenticates against Moonshot AI's backend rather than an Codify
    ...
    ``~/.kimi/config.toml``. Codify has no per-spawn provider override for
    upstream kimi, so all of this lives in the kimi CLI's own config —
    Codify-side injection remains a deferred follow-up.
    """
    from codify.onboarding.interactive import console
    ...
    console.print(
        ...
        "the agent spec\n"
        "    • Codify stores no kimi credential and cannot thread one per "
...
def _manage_kimi_harness() -> None:
    ...
    :func:`~codify.onboarding.harness_install.harness_cli_logged_in` always
...
def _prompt_install_copilot() -> str | None:
    ...
    index-free — see
    :func:`codify.onboarding.copilot_auth.copilot_install_command`.
    ...
    from codify.onboarding.copilot_auth import COPILOT_EXTRA, install_copilot_sdk
    from codify.onboarding.extra_install import extra_install_display
    from codify.onboarding.interactive import console, select
...
def _manage_copilot_harness() -> None:
    ...
    set / replace / remove a token stored in the codify secret store, mirroring
    ...
    :returns: None. Side effects: may install the ``copilot`` extra, and may
        write the ``copilot:`` block of ``~/.codify/config.yaml`` and the
        secret store.
    """
    from codify.onboarding import secrets as secret_store
    from codify.onboarding.copilot_auth import (
        COPILOT_CONFIG_KEY,
        COPILOT_SECRET_NAME,
...
def _manage_credential(provider: str, family: str) -> str | None:
    ...
    # harness configs outside ~/.codify/config.yaml — so removing it
...
def _remove_subscription(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the level-2 status line, or ``None``
        ...
        harness logout command and writes ``~/.codify/config.yaml``.
    """
    from codify.onboarding.harness_install import harness_install_spec, harness_logout
    from codify.onboarding.interactive import select
...
def _remove_databricks_provider(provider: str) -> str:
    ...
    ``~/.codify/config.yaml`` — most damagingly, for Codex < 0.134.0 it
    ...
    :mod:`codify.onboarding.ucode_cleanup`), so the user's own settings
    ...
    :returns: A confirmation message for the level-2 status line reporting
        ...
        ``claude mcp remove``, and write ``~/.codify/config.yaml``.
    """
    from codify.errors import CodifyError
    from codify.onboarding.ucode_cleanup import remove_ucode_wiring

    cleanup_note = ""
    try:
        removal = remove_ucode_wiring()
    except (CodifyError, OSError) as exc:
...
def _set_harness_default(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.codify/config.yaml``.
    """
    from codify.onboarding.configure_models import family_label
    from codify.onboarding.provider_config import load_providers, set_default_provider
...
def _clear_detection_dismissal(name: str) -> None:
    ...
    :returns: None. Side effect: writes ``~/.codify/config.yaml`` when the
...
def _remove_credential(provider: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.codify/config.yaml`` (and, when the removed entry is
...
def _launch_opencode_auth_login() -> str | None:
    ...
    from codify.onboarding.interactive import console
    from codify.onboarding.opencode_auth import opencode_auth_summary
...
def _run_opencode_auth_list() -> None:
    ...
    from codify.onboarding.harness_install import OPENCODE_KEY, harness_install_spec
...
def _set_opencode_default_model(current: str | None) -> str | None:
    ...
    """
    from codify.onboarding.interactive import console, select
    from codify.onboarding.opencode_auth import reachable_provider_ids
...
def _print_opencode_auth_help() -> None:
    ...
    console.print(
        ...
        "      Codify synthesizes opencode's per-session provider config from it.\n"
        "  Codify stores no OpenCode credential of its own.\n"
...
def _manage_opencode_harness() -> None:
    ...
    Codify. (For the Databricks-gateway path the agent's ``profile`` is
    ...
    # ambient env keys; Codify stores nothing. Report what's reachable and
...
def _run_configure_harnesses_interactive() -> None:
    ...
    Invoked by ``codify setup --no-internal-beta`` and the bare-``run``
    ...
    :returns: None. Side effect: may write ``~/.codify/config.yaml`` via
    ...
    # not an Codify credential), so it dispatches to its own drill-in.
    _QWEN = "\x00qwen"
    # Sentinel marking the OpenCode row — native-server harness with no Codify
    ...
    # Codify credential), so it dispatches to its own drill-in.
    _GOOSE = "\x00goose"
    ...
    def _install_hint(command: str) -> str:
        ...
        # (e.g. ``pip install "codify[cursor]"``) renders literally instead of
    ...
    def build_harness_rows() -> list[tuple[str, str, str, str, str]]:
        ...
        from codify.onboarding.hermes_auth import hermes_config_summary
        from codify.onboarding.opencode_auth import opencode_auth_summary
        ...
        from codify.onboarding.acp_auth import acp_config_summary
        from codify.onboarding.openclaw_config import (
