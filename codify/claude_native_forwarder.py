...
from codify.reasoning_effort import CLAUDE_EFFORTS, EFFORT_CLEAR_VALUES
...
# Claude Code hook event names → Codify session-status values
...
class _ForwardHealth:
    """
    Process-level health of Codify transcript/usage forwarding (#1120).
...
def _note_forward_failure(retry_key: str) -> None:
    ...
    if (
        ...
        _logger.error(
            "claude-native forward sync degraded: %d consecutive Codify "
...
class CompactionForwardState:
    ...
    both reconcile against one durable token so exactly one Codify
...
class SubagentEntry:
    ...
    One of these per Claude-side sub-agent. Tracks the Codify child
    ...
    :param child_conversation_id: Codify child Conversation id minted
...
class SubagentForwardState:
    ...
    :param subagents: Map from Claude-side ``subagent_id`` to the
        ...
        inserted here after the Codify server returns a child
        Conversation id.
    """
...
class DeltaForwardState:
    ...
    Claude process and keeps growing across Codify session rotations, so the
    offset stays monotonic and each new chunk is forwarded to whatever
    Codify session is active when it is read.
...
class _PostRetryEntry:
    """
    In-memory retry state for one outbound Codify event.
...
class _PostRetryDecision:
    """
    Result of recording one outbound Codify post failure.
...
class _PostRetryTracker:
    """
    Track bounded retries and backoff for Codify event posts.
    ...
    backoff so Codify outages do not silently drop transcript data.
...
async def forward_claude_transcript_to_session(
    *,
    ...
    :param headers: Static HTTP headers for Codify requests. Authorization
...
def _parse_json_response(resp: httpx.Response, *, context: str) -> dict[str, object]:
    """
    Parse an Codify JSON response, failing loudly on a non-JSON body.
...
async def _post_external_subagent_start(
    ...
    *,
    ...
    """
    POST ``external_subagent_start`` to the Codify server and return the
    ...
    :returns: The Codify child conversation id, e.g. ``"conv_child456"``.
    :raises httpx.HTTPError: If the Codify request fails or is rejected.
...
async def _forward_available_subagents(
    *,
    ...
    """
    Discover new Claude Task-tool sub-agents on disk, mint Codify child
    ...
    ``subagent_forwarder.json``) holds the Codify child id and byte
...
async def _maybe_rotate_session_on_clear(
    *,
    ...
    """
    Rotate the active Codify session when Claude reports ``/clear``.

    :param client: Codify HTTP client.
    :param session_id: Currently active Codify session id, e.g.
        ``"conv_old"``.
    :param bridge_dir: Native Claude bridge directory.
...
async def _seed_fork_transcript_forward_state(
    *,
    ...
    """
    Seed transcript forwarding after Codify has forked history.

    Claude fork transcripts start with copied source-session records.
    The Codify fork endpoint has already copied those conversation items,
...
async def _create_clear_replacement_session(
    *,
    ...
    """
    Create the fresh Codify session for a Claude ``/clear`` event.
    ...
    :raises httpx.HTTPError: If Codify rejects session creation, new-session
    ...
    clear_resp = await client.patch(
        ...
        json={
            ...
            # ``codify claude --resume``) would put a second forwarder on the
...
async def _maybe_rotate_session_on_fork(
    *,
    ...
    """
    Fork the active Codify session when Claude reports ``/fork``/``/branch``.
...
async def _create_fork_replacement_session(
    *,
    ...
    """
    Create the forked Codify session for a Claude ``/fork``/``/branch``.
    ...
    :raises RuntimeError: If the Codify fork response is malformed.
    """
    old = await _fetch_session_snapshot(client, old_session_id)
    runner_id = old.get("runner_id")
...
def _is_fork_hook_record(record: ClaudeHookRecord) -> bool:
    ...
    completed the Codify fork.

    :param record: Claude hook record read from hooks.jsonl.
    :returns: ``True`` when the active Codify session should be forked.
    """
...
async def _fetch_session_snapshot(
    ...
    """
    Fetch one Codify session snapshot.

    :param client: Codify HTTP client.
    :param session_id: Codify session id, e.g. ``"conv_abc123"``.
    :returns: Parsed JSON snapshot.
    :raises httpx.HTTPError: If Codify returns a non-2xx status.
...
async def _maybe_mirror_external_session_id(
    *,
    ...
    """
    Mirror Claude's native session id onto the Codify conversation row.

    Reads the latest captured Claude-native session id from the
    bridge state file and, if present, PATCHes
    ``external_session_id`` on the Codify conversation. Best-effort: a
    ...
    try:
        ...
        if 400 <= exc.response.status_code < 500:
            ...
            return True
        _logger.warning(
            ("Transient Codify error PATCHing external_session_id (%s); session=%s — will retry"),
            exc.response.status_code,
            session_id,
        )
        return False
...
async def _handle_compact_summary_item(
    ...
    *,
    ...
    persists exactly one Codify ``compaction`` boundary carrying the
...
async def _forward_available_items(
    *,
    ...
    for item in items:
        ...
        # Persist a durable Codify ``compaction`` boundary here (never
    ...
    # statusLine stdin captured by ``codify.claude_native_status``
...
async def _post_clear_supersession(
    ...
    *,
    ...
    :param client: Codify HTTP client (``base_url`` = AP server).
    ...
    *,
    ...
    with (
        ...
        telemetry.span(
            ...
            attributes={"codify.response_id": item.response_id},
        ),
    ):
        resp = await client.post(
...
async def _forward_available_deltas(
    *,
    ...
    :param client: Codify HTTP client.
    :param session_id: Codify session/conversation id deltas are forwarded
...
def _model_alias_for(model: str | None) -> str | None:
    ...
    :data:`codify.claude_native._UCODE_CLAUDE_CUSTOM_TIER`) for the newer
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror model change to Codify session=%s; model pill / "
...
async def _forward_model_from_status(
    ...
    *,
    ...
    wrapper (:mod:`codify.claude_native_status`) persists that model into
...
async def _post_external_compaction_status(
    ...
    *,
    ...
    ``SessionStart`` (``source == "compact"``) hook. The Codify server maps
...
async def _patch_external_session_id(
    ...
    *,
    ...
    """
    PATCH the Codify conversation row with the Claude-native session id.
...
async def _maybe_sync_effort_from_slash_command(
    ...
    *,
    ...
    """
    Mirror an in-pane ``/effort`` change onto the Codify session row.
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror in-pane /effort=%s to Codify session=%s; "
...
def _is_permanent_http_error(exc: httpx.HTTPError) -> bool:
    """
    Return whether ``exc`` is a permanent Codify rejection.

    :param exc: HTTP exception raised while posting an Codify event.
...
def _usage_from_status_state(state: dict[str, object]) -> dict[str, float] | None:
    """
    Convert statusLine ``current_usage`` (+ cost) into the Codify usage shape.
