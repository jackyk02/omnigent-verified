...
[Codify Server API](API.md).

## Codex Goal

Codex-native sessions expose Codex app-server's persisted thread goal as
a live subresource. Goal state is not mirrored into Codify labels or
...
`status` is forwarded exactly as Codex reports it; Codify does not
