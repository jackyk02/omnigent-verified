"""WebSocket ``Origin`` enforcement for the Codify server.
...
Codify server and drive the agent, read session updates, or attach to a
...
- :data:`CODIFY_INTERNAL_WS_ORIGIN` — the sentinel ``Origin`` the
...
from codify.server.auth import local_single_user_enabled
...
# ``codify.runner.identity`` and re-exported here for the server-side
# policy. See :data:`CODIFY_INTERNAL_WS_ORIGIN` there for rationale.
__all__ = [
...
_ALLOWED_ORIGINS_ENV = "CODIFY_WS_ALLOWED_ORIGINS"
...
def parse_allowed_origins() -> frozenset[str]:
    ...
    Reads ``CODIFY_WS_ALLOWED_ORIGINS`` (comma-separated). Whitespace
...
def origin_allowed(
    ...
    *,
    ...
    - The first-party sentinel (:data:`CODIFY_INTERNAL_WS_ORIGIN`) and
    ...
    :param local_mode: Whether the server is a single-user local runtime
        (``CODIFY_LOCAL_SINGLE_USER`` truthy).
    ...
    """
    if origin == CODIFY_INTERNAL_WS_ORIGIN:
        return True
