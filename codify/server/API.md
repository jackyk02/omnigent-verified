...
|---|---|---|
| Agent Management (`/api/agents`) | Codify (ours) | No external reference — this is our own API. |
| Conversations (`/v1/conversations`) | Codify (ours) | No external reference. |
| Sessions (`/v1/sessions`) | Codify (ours) | No external reference. Session-first API for long-running agent interactions. See "Sessions API" section below. |
| Session file resources (`/v1/sessions/{session_id}/resources/files`) | Codify (ours) | No external reference. Files are scoped to their owning session. |
...
|---|---|---|
| Sessions (`/v1/sessions`) | Codify (ours) | No external reference. Purpose-built for agent-native workflows: live tail, queued input, interrupt. |
...
Fields:
  ...
  external_session_id (string or null)
    ...
    Code's session uuid for `codify claude` sessions). Populated
  ...
  pending_inputs (array, default `[]`)
    ...
    writer), so the Codify server holds these in-memory and replays them
  ...
  todos (array, default `[]`)
    Current Claude Code todo list for `codify claude` sessions.
  ...
  terminal_pending (boolean, default `false`)
    ...
    resource exists. Set by two paths: (1) directly by the Codify server at
...
`SessionResponse` shape with `id` set to the new conversation id.
  ...
  host_type (string, "external" | "managed", default "external")
    ...
    caller-managed compute — a host registered via `codify host`
    (pass `host_id`) or a caller-managed runner (no `host_id`).
    `"managed"`: the SERVER provisions a sandbox host from its
    `sandbox:` config (see `codify/server/managed_hosts.py`),
    starts `codify host` inside it, binds the session to it, and
  ...
  terminal_launch_args (array of strings or null)
    ...
    dir, Codify URL, auth), which stay runner-owned. Bounds (count /
...
`codify/server/schemas.py`; the discriminated union
...
> `codify.server.schemas` is the supported pattern.
...
See the per-class docstring in `codify/server/schemas.py` for
