...
from codify.process_logging import log_record_display_fields

_DEFAULT_WINDOWS_SECONDS = (1.0, 10.0, 30.0)
_BYTES_PER_MIB = 1024 * 1024
_OTEL_METER_NAME = "codify.server.performance"
_REQUEST_DURATION_CONTEXT: contextvars.ContextVar[float | None] = contextvars.ContextVar(
    "codify_request_duration_seconds",
    default=None,
)
...
class RequestDurationAccessFormatter(AccessFormatter):
    """
    Uvicorn access formatter that appends Codify request duration.
    ...
    def format(self, record: logging.LogRecord) -> str:
        """Apply Codify's standard source and level display fields."""
...
def _create_request_otel_instruments(meter: MeterLike) -> _RequestOtelInstruments:
    ...
    return _RequestOtelInstruments(
        started=meter.create_counter(
            "codify.server.http.requests.started",
            unit="{request}",
            description="HTTP requests started by the Codify server.",
        ),
        completed=meter.create_counter(
            "codify.server.http.requests.completed",
            unit="{request}",
            description="HTTP requests completed by the Codify server.",
        ),
        failed=meter.create_counter(
            "codify.server.http.requests.failed",
            unit="{request}",
...
def _create_processing_otel_instruments(
    ...
    return _ProcessingOtelInstruments(
        ...
        avg_30s_ms=meter.create_gauge(
            "codify.server.http.request.processing.avg_30s",
            unit="ms",
            description=(
