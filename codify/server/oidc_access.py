...
1. **Domain allowlist.** The email's domain is in the effective
   allowlist — the union of ``CODIFY_OIDC_ALLOWED_DOMAINS`` (env,
   frozen at startup in :class:`~codify.server.oidc.OIDCConfig`)
...
3. **Individual invite.** The email was pre-authorized via an opt-in
   OIDC invite (``CODIFY_OIDC_ALLOW_INVITES`` — see
   :mod:`codify.server.accounts_store` /
...
from codify.server.admin_list import AdminList, MtimeCachedIdentitySet, resolve_data_dir


def resolve_allowed_domains_path() -> Path:
    """Resolve the path to the optional allowed-domains file.

    :returns: ``CODIFY_OIDC_ALLOWED_DOMAINS_PATH`` if set, else
        ``<data_dir>/allowed_domains`` (see
        :func:`codify.server.admin_list.resolve_data_dir`).
    """
    explicit = os.environ.get("CODIFY_OIDC_ALLOWED_DOMAINS_PATH", "").strip()
    if explicit:
...
class InvitedEmailLookup(Protocol):
    ...
    Implemented by
    :class:`~codify.server.accounts_store.SqlAlchemyAccountStore`
...
class OidcAdmissionPolicy:
    ...
    :param env_allowed_domains: The frozen domain allowlist parsed from
        ``CODIFY_OIDC_ALLOWED_DOMAINS`` (``OIDCConfig.allowed_domains``).
