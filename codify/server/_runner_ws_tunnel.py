...
The server's :mod:`codify.server.routes.terminal_attach` route
...
if TYPE_CHECKING:
    ...
    from codify.runner.transports.ws_tunnel.registry import (
        RunnerSession,
        TunnelRegistry,
        WSChannelState,
    )
...
def make_tunnel_ws_factory(
    ...
    Install via :func:`codify.runtime.set_runner_ws_factory`. The
    factory shape (callable ``(runner_path) -> async-context``) is
    what :mod:`codify.server.routes.terminal_attach` already
    expects.
...
class _TunneledWSConn:
    ...
    :func:`codify.server.routes.terminal_attach._shuttle_ws_frames`
