...
The external host flow has a human run ``codify host`` on their own
...
starts ``codify host`` inside it, and waits for that host to
...
:meth:`codify.stores.host_store.HostStore.register_managed_host` and
the managed-token branch in
:mod:`codify.server.routes.host_tunnel`) — the user's own
...
1. **Server YAML** (OSS / self-hosted): :func:`parse_sandbox_config`
   ...
   (``codify server -c`` / ``CODIFY_CONFIG`` /
   ``<data_dir>/config.yaml``)::

       sandbox:
         provider: modal          # lakebox|modal|daytona|boxlite|cwsandbox|islo|e2b|openshell
         server_url: https://codify.example.com
         host_config:             # optional; provider-agnostic. Verbatim
                                  # in-sandbox ~/.codify/config.yaml content,
                                  # installed before `codify host` starts
         ...
         modal:                   # optional block
           image: docker.io/me/codify-host:latest  # default: official image
           secrets: [codify-llm]  # Modal secrets injected as sandbox env
                                     # (harness LLM keys, gateway URLs)
         boxlite:                 # optional block (provider: boxlite)
           image: docker.io/me/codify-host:latest    # shared; default: official
   ...
   (``ghcr.io/codify-ai/codify-host:latest``; see
   :data:`codify.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` and
...
2. **Direct construction** (embedding deployments): build
   ...
   :class:`~codify.onboarding.sandboxes.base.SandboxLauncher`
...
if TYPE_CHECKING:
    from codify.onboarding.sandboxes import SandboxLauncher
...
# operator-overridable (CODIFY_CWSANDBOX_MAX_LIFETIME_S), so the TTL is
...
_HOST_LOG_PATH = "/tmp/codify-host.log"
...
MANAGED_REPO_LABEL_KEY = "codify.sandbox.repo"
...
class ManagedSandboxConfig:
    ...
    :param server_url: Public URL of THIS server that the sandboxed
        host dials back to, e.g. ``"https://codify.example.com"``
    ...
    :param host_config: Verbatim in-sandbox ``~/.codify/config.yaml``
        ...
        :func:`codify.onboarding.sandboxes.base.render_host_config_write_command`.
...
def _modal_launcher_factory(
    ...
    :param image: Registry image reference with codify pre-installed,
        e.g. ``"docker.io/me/codify-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :func:`codify.onboarding.sandboxes.modal._build_sandbox_image`).
    :param secrets: Modal secret names whose env vars (harness LLM
        credentials, gateway URLs) are injected into every sandbox,
        e.g. ``["codify-llm"]``, or ``None`` to resolve from the
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.modal import ModalSandboxLauncher
...
def _parse_host_config(raw: dict[str, object]) -> dict[str, object] | None:
    ...
    if not isinstance(host_config, dict):
        raise ValueError(
            ...
            "in-sandbox ~/.codify/config.yaml content merged in before "
            "'codify host' starts"
        )
    ...
    if "providers" in host_config:
        ...
        from codify.onboarding.provider_config import get_default_provider, load_providers
...
def parse_sandbox_config(raw: object) -> ManagedSandboxConfig | None:
    ...
    :param raw: The raw ``sandbox`` value from the server config YAML,
        ...
        "modal": {"image": "docker.io/me/codify-host:latest"}}``.
    ...
    if provider == "modal":
        ...
        from codify.onboarding.sandboxes.cwsandbox import managed_token_ttl_s

        launcher_factory = _cwsandbox_launcher_factory(
            _parse_cwsandbox_image(raw), _parse_cwsandbox_env(raw)
        )
        # Derived from CODIFY_CWSANDBOX_MAX_LIFETIME_S so the token always
        ...
        from codify.onboarding.sandboxes.e2b import managed_token_ttl_s

        launcher_factory = _e2b_launcher_factory(
            _parse_e2b_template(raw), _parse_provider_env(raw, "e2b")
        )
        # Derived from CODIFY_E2B_MAX_LIFETIME_S so the token always
...
def _parse_modal_image(raw: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with codify pre-installed, e.g. "
            "'docker.io/me/codify-host:latest' (omit it to use the "
            "official image)"
        )
    return image.strip()
...
def _parse_modal_secrets(raw: dict[str, object]) -> list[str] | None:
    ...
    :param raw: The raw ``sandbox`` mapping (provider already known to
        be ``"modal"``).
    :returns: The validated secret names, e.g. ``["codify-llm"]``,
...
def _daytona_launcher_factory(
    ...
    :param image: Registry image reference with codify pre-installed,
        e.g. ``"docker.io/me/codify-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :class:`codify.onboarding.sandboxes.daytona.DaytonaSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.daytona import DaytonaSandboxLauncher
...
def _parse_daytona_image(raw: dict[str, object]) -> str | None:
    ...
    (env-overridable; see
    :mod:`codify.onboarding.sandboxes.daytona`). A
...
def _boxlite_launcher_factory(
    ...
    :param endpoint: Remote ``boxlite serve`` URL (cloud mode), or ``None`` for
        LOCAL mode — boxes run on the codify-server host as embedded micro-VMs
        (no daemon, no ``boxlite serve``).
    :param image: Registry image reference with codify pre-installed, or
        ``None`` to use the official prebaked host image (env-overridable; see
        :class:`codify.onboarding.sandboxes.boxlite.BoxliteSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.boxlite import BoxliteSandboxLauncher
...
def _parse_boxlite_image(section: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with codify pre-installed, e.g. "
            "'docker.io/me/codify-host:latest' (omit it to use the official image)"
        )
    return image.strip()
...
def _cwsandbox_launcher_factory(
    ...
    def _build() -> SandboxLauncher:
        from codify.onboarding.sandboxes.cwsandbox import CWSandboxLauncher
...
def _parse_cwsandbox_image(raw: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with codify pre-installed (omit it to use the official image)"
        )
    return image.strip()
...
def _e2b_launcher_factory(
    ...
    :param template: E2B template NAME the Codify host image was built
        ...
        :class:`codify.onboarding.sandboxes.e2b.E2BSandboxLauncher`).
    :param env: Names of server-process environment variables (harness
        LLM credentials, gateway URLs, ``GIT_TOKEN``) injected into
        every sandbox, e.g. ``["OPENAI_API_KEY", "GIT_TOKEN"]``, or
        ``None`` to resolve from the launcher's env-var fallback /
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.e2b import E2BSandboxLauncher
...
def _parse_e2b_template(raw: dict[str, object]) -> str | None:
    ...
    Codify host image was built into — NOT a registry image reference
    ...
    the launcher resolves :data:`~codify.onboarding.sandboxes.e2b.TEMPLATE_ENV_VAR`
    ...
    if not isinstance(template, str) or not template.strip():
        raise ValueError(
            ...
            "E2B template the codify host image was built into (e.g. "
            "'codify-host'; see deploy/e2b/README.md) — NOT a registry image "
...
def _parse_islo_idle_pause_after_s(raw: dict[str, object]) -> int | None:
    ...
    lifecycle control.
    """
    from codify.onboarding.sandboxes.islo import DEFAULT_IDLE_PAUSE_AFTER_S
...
def _islo_launcher_factory(
    *,
    ...
    :param image: Registry image reference with codify pre-installed,
        e.g. ``"docker.io/me/codify-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :class:`codify.onboarding.sandboxes.islo.IsloSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.islo import IsloSandboxLauncher
...
def _openshell_launcher_factory(
    *,
    ...
    :param image: Registry image reference with codify pre-installed,
        e.g. ``"docker.io/me/codify-host:latest"``, or ``None`` to use
    ...
    :param workspace: OpenShell workspace for sandbox lifecycle, or
        ``None`` to resolve from ``$CODIFY_OPENSHELL_WORKSPACE``
        then ``"default"``.
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.openshell import OpenShellSandboxLauncher
...
# env-var overrides — see codify.onboarding.sandboxes.kubernetes).
...
# shadow the /home/codify mountpoint): the runner's writable-HOME emptyDir
...
# image's codify venv lives at /opt/venv).
...
def _parse_kubernetes_pvc_mounts(raw: dict[str, object]) -> list[dict[str, object]] | None:
    ...
    for i, entry in enumerate(value):
        ...
        # '//home/codify' cannot slip past the reserved-prefix check.
...
def _kubernetes_launcher_factory(
    *,
    ...
    :param image: Registry image with codify pre-installed, or ``None`` for
        the official prebaked host image (env-overridable).
    ...
    def _build() -> SandboxLauncher:
        ...
        from codify.onboarding.sandboxes.kubernetes import KubernetesSandboxLauncher
...
async def launch_managed_host(
    *,
    ...
    → start ``codify host`` inside the sandbox with the token +
...
def host_sandbox_is_running(
    ...
    and fall back to Codify liveness checks.
    """
    launcher = _launcher_for_teardown(host, config)
...
async def resume_managed_host(
    ...
    *,
    ...
    re-execs ``codify host``, and waits for it to re-register. The caller's
