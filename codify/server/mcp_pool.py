...
:class:`codify.runner.mcp_manager.RunnerMcpManager` but used by the
``POST /v1/sessions/{session_id}/mcp`` MCP-proxy endpoint to proxy calls
with policy enforcement.

Reuses :class:`codify.tools.mcp.McpServerConnection` for transport
and :func:`codify.runner.mcp_manager.compute_spec_hash` for stable
pool keying.
"""
...
from codify.runner.mcp_manager import (  # noqa: F401 — McpSchemasResult re-exported for callers
    McpSchemasResult,
    compute_spec_hash,
)
...
class ServerMcpPool:
    ...
    async def call_tool(
        ...
        :returns: Tool result as a string (same shape as
            :meth:`codify.tools.mcp.McpServerConnection.call_tool`).
