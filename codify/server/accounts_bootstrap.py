...
Run once at server startup when ``CODIFY_AUTH_PROVIDER=accounts``.
...
1. **Password flag** — ``--admin-password`` /
   ``CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD``. Headless / CI. Bootstrap
   creates the admin from it directly (this module).
2. **Terminal prompt** — ``codify server`` on a TTY asks for a
...
bootstrap writes a session JWT to ``~/.codify/auth_tokens.json``
(via :mod:`codify.cli_auth`) keyed to this spawn's URL, so the next
``codify run`` is signed in without a prompt. Skipped for
...
from codify.server.auth import _RESERVED_USERS
from codify.server.passwords import hash_password
...
def resolve_admin_username() -> str:
    ...
    1. ``CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME`` env var — explicit
    ...
    """
    explicit = os.environ.get("CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME", "").strip()
    if explicit:
...
class BootstrapResult:
    ...
    :param needs_setup: ``True`` when no admin exists yet and none was
        ...
        ``codify server`` terminal prompt, or a password flag.
    ...
    :param tui_token_written: ``True`` when a CLI session JWT was
        written to ``~/.codify/auth_tokens.json`` (loopback only),
        so the next ``codify run`` is signed in without a prompt.
    """
...
def _mint_loopback_cli_token(
    ...
    *,
    ...
    ``~/.codify/auth_tokens.json`` keyed by the **exact** server
    ...
    per spawn, ``codify run`` 401s against its own loopback
    ...
    try:
        ...
        logger.warning(
            ...
            "`codify run` may need `codify login`",
            exc,
        )
        return False
...
def bootstrap_admin(
    ...
    *,
    ...
    :param account_store: The accounts-specific store backing user
        ...
        for accounts concerns — see ``codify/server/accounts_store.py``
        for the boundary rationale.
    :param init_admin_password: Optional operator-provided
        password from ``CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD``.
    ...
    :param session_ttl_hours: TTL for the CLI session token
        written to ``~/.codify/auth_tokens.json``.
    ...
    if any(u.has_password for u in account_store.list_users()):
        ...
        if init_admin_password:
            ...
            print(
                ...
                "    (--admin-password / CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD).\n"
        ...
        # one-time. Without this, `codify run` 401s against its own
    ...
    #      CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD — headless / CI;
    #   2. the `codify server` terminal prompt (cli.py, on a TTY);
    ...
    # Loopback CLI handoff so `codify run` is signed in without a
