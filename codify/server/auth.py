...
- ``"header"`` (default): reads the ``X-Forwarded-Email`` header
  ...
  (``CODIFY_LOCAL_SINGLE_USER=1``), in which case they fall back
...
_AUTH_ENABLED_ENV = "CODIFY_AUTH_ENABLED"
...
# paths (`codify run` in chat.py, the daemon's
# host/local_server.py) and by the canonical bare loopback
# `codify server` (cli.py) — never by deployed multi-user servers.
...
_LOCAL_SINGLE_USER_ENV = "CODIFY_LOCAL_SINGLE_USER"
...
_AUTH_HEADER_ENV = "CODIFY_AUTH_HEADER"
...
_AUTH_HEADER_STRIP_PREFIX_ENV = "CODIFY_AUTH_HEADER_STRIP_PREFIX"
...
class SharingMode(str, Enum):
    ...
    ``CODIFY_SHARING_MODE`` env var round-trip it directly. Defaults to ``ON``.
    """
...
def local_single_user_enabled() -> bool:
    ...
    Reads ``CODIFY_LOCAL_SINGLE_USER``, the marker the managed
...
def resolve_auth_header() -> str:
    ...
    Reads ``CODIFY_AUTH_HEADER`` and falls back to
    ...
    e.g. ``CODIFY_AUTH_HEADER=Cf-Access-Authenticated-User-Email`` for
...
def resolve_auth_header_strip_prefix() -> str:
    ...
    Reads ``CODIFY_AUTH_HEADER_STRIP_PREFIX`` and returns it
    ...
    ``CODIFY_AUTH_HEADER=X-Goog-Authenticated-User-Email`` at IAP's
    identity header and set
    ``CODIFY_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:`` so the
...
def _auth_enabled() -> bool:
    ...
    Reads ``CODIFY_AUTH_ENABLED``. The explicit-falsy kill-switch
    semantics mean ``CODIFY_AUTH_ENABLED=0`` disables auth even
...
def resolve_auth_source() -> str:
    ...
    :returns: The resolved source string, e.g. ``"accounts"``,
        ...
        ``CODIFY_AUTH_PROVIDER``). The caller is responsible for
        rejecting unknown values.
    """
    raw_source = os.environ.get("CODIFY_AUTH_PROVIDER")
    ...
    if _auth_enabled():
        ...
        if os.environ.get("CODIFY_OIDC_ISSUER", "").strip():
            return "oidc"
        return "accounts"
    return "header"
...
class UnifiedAuthProvider(AuthProvider):
    ...
    ``CODIFY_AUTH_PROVIDER``. OIDC and accounts modes share
    ...
    :param local_single_user: When ``True``, header mode falls back
        ...
        ``CODIFY_LOCAL_SINGLE_USER`` at construction (see
    ...
    :param header_name: The trusted identity header read in header
        ...
        ``CODIFY_AUTH_HEADER`` at construction, falling back to
    ...
    :param header_strip_prefix: A prefix stripped from the identity
        ...
        ``CODIFY_AUTH_HEADER_STRIP_PREFIX`` at construction, falling
    ...
    def mint_runner_token(self, user_id: str, ttl_seconds: int) -> str | None:
        ...
        if cookie_config is None:
            return None
        from codify.server.oidc import mint_session_token
    ...
    def _check_cookie(self, request: HTTPConnection) -> str | None:
        ...
        authenticated via ``codify login``). Both carry the same
        HS256-signed JWT.
        ...
        from codify.server.oidc import hmac_digest
    ...
    def _check_header(self, request: HTTPConnection) -> str | None:
        ...
        by default, overridable via ``CODIFY_AUTH_HEADER`` — e.g.
        ``Cf-Access-Authenticated-User-Email`` for Cloudflare Access).

        When :attr:`_header_strip_prefix` is set (from
        ``CODIFY_AUTH_HEADER_STRIP_PREFIX``), it is removed from the
        ...
        ``CODIFY_LOCAL_SINGLE_USER=1``): there the absent header
...
def create_auth_provider() -> AuthProvider:
    ...
    -----------------------------------
    Set ``CODIFY_AUTH_ENABLED=1`` (or any truthy value) to turn on
    multi-user auth. With no OIDC config present this selects
    ``accounts`` mode — the built-in login flow with
    first-user-is-admin setup. Set the ``CODIFY_OIDC_*`` env vars
    (at minimum ``CODIFY_OIDC_ISSUER``) alongside it and the same
    ...
    (``CODIFY_AUTH_ENABLED`` is the opt-in gate: header is the
    ...
    if source not in ("header", "oidc", "accounts"):
        raise RuntimeError(
            f"Unknown CODIFY_AUTH_PROVIDER={source!r}. Valid: 'header', 'oidc', 'accounts'"
        )

    oidc_config: OIDCConfig | None = None
    accounts_config: AccountsConfig | None = None
    if source == "oidc":
        from codify.server.oidc import OIDCConfig

        oidc_config = OIDCConfig.from_env()
    elif source == "accounts":
        # Reaching here means accounts mode was deliberately selected
        # — either CODIFY_AUTH_PROVIDER=accounts or the
        # CODIFY_AUTH_ENABLED=1 opt-in without OIDC config
