...
def resolve_data_dir() -> Path:
    ...
    1. The parent directory of ``CODIFY_ADMIN_CREDENTIALS_PATH`` if
...
def resolve_admin_list_path() -> Path:
    ...
    :returns: ``CODIFY_ADMIN_LIST_PATH`` if set, else
        ``<data_dir>/admins`` (see :func:`resolve_data_dir`).
    """
    explicit = os.environ.get("CODIFY_ADMIN_LIST_PATH", "").strip()
    if explicit:
...
class AdminFlagStore(Protocol):
    ...
    Both :class:`~codify.stores.permission_store.PermissionStore`
    and :class:`~codify.server.accounts_store.SqlAlchemyAccountStore`
