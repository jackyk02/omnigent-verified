...
A user-set ``CODIFY_ACCOUNTS_COOKIE_SECRET`` always wins —
...
Caller pattern:
    ...
    os.environ.setdefault("CODIFY_ACCOUNTS_COOKIE_SECRET", secret_hex)
...
def load_or_generate_cookie_secret(data_dir: str | os.PathLike[str]) -> str:
    ...
    1. ``CODIFY_ACCOUNTS_COOKIE_SECRET`` env var — operator-set,
    ...
    """
    env_value = os.environ.get("CODIFY_ACCOUNTS_COOKIE_SECRET")
    if env_value:
        return env_value
    ...
    logger.info(
        ...
        "(set CODIFY_ACCOUNTS_COOKIE_SECRET to share across instances)",
        path,
    )
    return fresh
