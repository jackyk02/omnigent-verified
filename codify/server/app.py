"""FastAPI application — main entry point for the codify server."""
...
from codify._platform import resolve_repo_symlink
from codify.db.db_models import InvalidUuidError
...
from codify.resources import examples as _examples_resources
...
from codify.runtime.harnesses.process_manager import HarnessProcessManager
from codify.server import session_live_state
from codify.server.auth import AuthProvider, SharingMode
from codify.server.background_session_titles import (
    BackgroundSessionTitleCoordinator,
    RunnerBackgroundTitleGenerator,
)
from codify.server.managed_hosts import ManagedSandboxConfig
from codify.server.mcp_pool import ServerMcpPool
from codify.server.performance_metrics import (
...
from codify.server.routes.best_of_n import create_best_of_n_router
from codify.server.routes.builtin_agents import create_builtin_agents_router
from codify.server.routes.comments import create_comments_router
from codify.server.routes.default_policies import create_default_policies_router
from codify.server.routes.dictation import create_dictation_router
from codify.server.routes.harnesses import create_harnesses_router
from codify.server.routes.imports import create_imports_router
from codify.server.routes.policy_registry import create_policy_registry_router
from codify.server.routes.projects import create_projects_router
from codify.server.routes.runner_tunnel import create_runner_tunnel_router
from codify.server.routes.scheduled_tasks import create_scheduled_tasks_router
from codify.server.routes.session_mcp_servers import create_session_mcp_servers_router
from codify.server.routes.session_policies import create_session_policies_router
from codify.server.routes.sessions import (
    SessionLiveness,
...
from codify.server.routes.sharing import create_sharing_router
from codify.server.routes.terminal_attach import create_terminal_attach_router
from codify.server.routes.usage import create_usage_router
from codify.server.runner_session_init import RunnerSessionInitializer
from codify.server.scheduled import ScheduledTaskScheduler
from codify.server.ws_origin import WebSocketOriginMiddleware
from codify.stores import (
    AgentStore,
    ArtifactStore,
    ConversationStore,
    FileStore,
)
from codify.stores.comment_store import CommentStore
from codify.stores.conversation_store import SessionConnectivity, runner_seen_is_fresh
from codify.stores.host_store import HostStore
from codify.stores.permission_store import PermissionStore
from codify.stores.policy_store import PolicyStore
...
def _server_version() -> str:
    ...
    Reads :data:`codify.version.VERSION`, the single source of truth shared
...
# CODIFY_WEB_UI_DIST, without rebuilding or repackaging.
_WEB_UI_DIST = Path(
    os.environ.get("CODIFY_WEB_UI_DIST") or (Path(__file__).parent / "static" / "web-ui")
)
...
_BEST_OF_N_AGENT_NAME = "best_of_n"
_UNMATCHED_ROUTE_TEMPLATE = "<unmatched>"
_SESSION_PATH_RE = re.compile(r"/v1/sessions/([^/]+)")
_BEST_OF_N_BUNDLE_SOURCE = resolve_repo_symlink(
    Path(_examples_resources.__file__).parent / "best_of_n"
)
...
def _normalize_tarinfo(tarinfo: tarfile.TarInfo) -> tarfile.TarInfo:
    ...
    ``codify.spec.tar_utils.extract_safe``), so on-disk permissions
    don't depend on it.
...
def _ensure_builtin_agent(
    ...
    *,
    ...
    one. Mirrors the upsert in :func:`codify.cli._register_yaml_bundle`.
...
def _ensure_default_agents(
    ...
    _ensure_default_best_of_n_agent(agent_store, artifact_store, agent_cache)
...
_EXTRA_BUILTIN_AGENTS_ENV = "CODIFY_BUILTIN_AGENT_DIRS"
...
def _build_native_bundle(provider: NativeHarnessProvider) -> bytes:
    ...
    if materialize is None:
        raise CodifyError(f"native provider {provider.key!r} has no materialize_agent_spec hook")
    with tempfile.TemporaryDirectory() as tmpdir:
...
def _ensure_default_native_agents(
    ...
    :param agent_cache: Cache for loaded agent specs.
    """
    from codify.native_coding_agents import NATIVE_CODING_AGENTS
...
def _build_best_of_n_bundle() -> bytes:
    """
    Build a gzipped tarball of the ``examples/best_of_n`` agent bundle.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        bundle_dir = materialize_bundle(_BEST_OF_N_BUNDLE_SOURCE, Path(tmpdir) / "bundle")
        return _tar_gz_dir(bundle_dir)


def _ensure_default_best_of_n_agent(
    agent_store: AgentStore,
    artifact_store: ArtifactStore,
    agent_cache: Any,
) -> None:
    """
    Register the Best-of-N (multi-agent auto-verify) agent if bundled.

    Best-of-N is the default coding mode: it drives the deterministic
    fan-out / verify / merge engine in :mod:`codify.best_of_n`.
    Seeding it lets the Web UI's new-session picker offer it (the
    picker pins it first via ``AGENT_DISPLAY_ORDER``). Skipped when the
    bundle isn't packaged here, mirroring the polly/debby seeders.

    :param agent_store: Store for agent metadata.
    :param artifact_store: Store for agent bundles.
    :param agent_cache: Cache for loaded agent specs.
    """
    if not (_BEST_OF_N_BUNDLE_SOURCE / "config.yaml").is_file():
        _logger.debug(
            "best_of_n bundle not found at %s; skipping seed",
            _BEST_OF_N_BUNDLE_SOURCE,
        )
        return
    ...
    _ensure_builtin_agent(
        ...
        artifact_store,
        agent_cache,
        name=_BEST_OF_N_AGENT_NAME,
        bundle_bytes=_build_best_of_n_bundle(),
    )
...
def create_app(
    ...
    :param allowed_domains: Allowed email domains from the server
        ...
        Union'd with ``CODIFY_OIDC_ALLOWED_DOMAINS`` and the
    ...
    :param sharing_mode: Server policy for creating new session
        ...
        the ``CODIFY_SHARING_MODE`` env var
    ...
    :param public_sharing: Whether public (anyone-with-the-link) read
        ...
        ``CODIFY_PUBLIC_SHARING`` env var (enabled unless explicitly
    ...
    """
    # Auto-load the stored Best-of-N verifier key into this process's env
    # so bundle specs referencing ${DEEPSEEK_API_KEY} expand at seeding.
    ...
    if auth_provider is not None and account_store is not None:
        from codify.server.auth import UnifiedAuthProvider

        if isinstance(auth_provider, UnifiedAuthProvider) and auth_provider._source == "accounts":
            from codify.server.accounts_bootstrap import bootstrap_admin
    ...
    from codify.server.host_registry import HostRegistry, RunnerExitReports
    ...
    async def _lifespan(
        ...
        ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.4.
        ...
        from codify.telemetry import init_client as _init_telemetry

        _init_telemetry(config=server_config)

        # Apply CODIFY_LOG_LEVEL to the codify namespace after
        # uvicorn's dictConfig runs (dictConfig resets existing handlers,
        # making a pre-run basicConfig call ineffective).
        import os as _os

        _log_level_name = _os.environ.get("CODIFY_LOG_LEVEL", "INFO").upper()
        logging.getLogger("codify").setLevel(getattr(logging, _log_level_name, logging.INFO))
        ...
        from codify.runtime import set_resource_registry
        ...
        from codify.server._runner_ws_tunnel import make_tunnel_ws_factory
        ...
        from codify.policies.registry import load_registry
        ...
        # passes CODIFY_ACCOUNTS_AUTO_OPEN=0 for --no-open). Broad
        # try so a missing display / browser never blocks startup.
        if _bootstrap_result is not None and _bootstrap_result.open_url:
            from codify.server.auth import env_var_is_truthy

            if env_var_is_truthy("CODIFY_ACCOUNTS_AUTO_OPEN", default=True):
                import webbrowser
        ...
        if scheduled_task_store is not None:
            from codify.server.scheduled.fire import FireDeps, build_on_fire, build_run_now
        ...
        try:
            ...
            from codify.server.routes.sessions import cancel_managed_launch_tasks
    ...
    app = FastAPI(title="Codify Server", lifespan=_lifespan)
    ...
    # change (no restart).
    from codify.server.admin_list import load_admin_list
    ...
    # endpoint.
    #
    # ``None`` (the OSS default): ``CODIFY_SHARING_MODE`` sets the boot
    ...
    if sharing_mode is None:
        from codify.server.sharing_settings import read_sharing_mode_override

        _sharing_env_default = SharingMode.coerce(os.environ.get("CODIFY_SHARING_MODE"))
    ...
    # above and independent of it. ``None`` reads ``CODIFY_PUBLIC_SHARING``
    ...
    if public_sharing is None:
        from codify.server.sharing_settings import (
    ...
    from codify.server.managed_hosts import ManagedLaunchTracker
    ...
    @app.exception_handler(CodifyError)
    async def _handle_codify_error(
        request: Request,
        exc: CodifyError,
    ) -> JSONResponse:
        """
    ...
    async def health(
        ...
        :class:`~codify.server.routes.sessions.SessionLiveness`.
    ...
    async def version() -> dict[str, str]:
        """
        Return the installed codify package version.
    ...
    async def info() -> dict[str, bool | str | list[str] | None]:
        ...
        Returned at app boot by the frontend (and by ``codify
        ...
        """
        from codify.server.auth import UnifiedAuthProvider, local_single_user_enabled
        ...
        # (CODIFY_LOCAL_SINGLE_USER=1, set by the managed local spawn paths).
        ...
        databricks_features = (
            importlib.util.find_spec("codify.onboarding.sandboxes.lakebox") is not None
        )
        ...
        # server_version is the installed codify package version (same
        ...
        try:
            from codify.runtime._globals import _caps
        ...
        # (CODIFY_HARNESS_INSTALL_ENABLED=1 opts in) while the feature rolls
        # out; when false the SPA keeps the prior "run codify setup" hint.
        ...
        from codify.process_logging import env_truthy
        from codify.server.routes.hosts import HARNESS_INSTALL_ENABLED_ENV
        ...
        from codify.onboarding.harness_install import ui_installable_harnesses
        ...
        from codify.server.dictation import engine_availability
    ...
    # Best-of-N verifier configuration (criteria, selection parameters,
    # proposer harness roster) backing the "Best-of-N Verifier" settings UI.
    app.include_router(
        create_best_of_n_router(
            auth_provider=auth_provider,
            permission_store=permission_store,
        ),
        prefix="/v1",
    ...
    async def _on_runner_exited(runner_id: str, error: str) -> None:
        ...
        from codify.server.schemas import ErrorDetail
    ...
    if host_store is not None:
        from codify.server.routes.host_tunnel import create_host_tunnel_router
        from codify.server.routes.hosts import create_hosts_router
    ...
    if auth_provider is not None and getattr(auth_provider, "login_url", None):
        ...
        if (
            ...
            from codify.server.routes.accounts_auth import (
            ...
            from codify.server.routes.auth import create_auth_router

            # OIDC invites are opt-in (CODIFY_OIDC_ALLOW_INVITES) and
        ...
        # CODIFY_DEVICE_GRANT_ENABLED, and accounts-mode only. OIDC delegates
        ...
        if (
            env_var_is_truthy("CODIFY_DEVICE_GRANT_ENABLED", default=False)
            and isinstance(auth_provider, UnifiedAuthProvider)
            and auth_provider._source == "accounts"
            and permission_store is not None
        ):
            from codify.server.device_grant_store import DeviceGrantStore
            from codify.server.routes.device_auth import create_device_auth_router
            ...
            # into CODIFY_DEVICE_CLIENT_SECRET (which closes initiation to
            # unauthorized callers) rather than leaving it off unknowingly. See
            # designs/DEVICE_AUTH.md § "Device-code phishing".
            if not os.environ.get("CODIFY_DEVICE_CLIENT_SECRET", "").strip():
                _logger.warning(
                    "device-grant: CODIFY_DEVICE_CLIENT_SECRET is not set — the "
                    "/oauth/device/authorize endpoint is PUBLIC (any caller may "
                    "initiate a login flow). Set CODIFY_DEVICE_CLIENT_SECRET on "
    ...
    # built into ``codify/server/static/web-ui/`` by ``web/``'s Vite
