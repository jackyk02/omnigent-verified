...
(:mod:`codify.server.routes.dictation`) with an on-server recognizer
...
(:func:`register_engine`), selected via ``CODIFY_DICTATION_ENGINE``:

- unset (default) — the sherpa-onnx engine. Requires the ``dictation``
  extra (``pip install codify[dictation]``) and a streaming transducer
...
- ``remote`` — relays takes to a dictation worker on another machine
  (``CODIFY_DICTATION_REMOTE_URL``), so a small main server can borrow
...
Env var                                 Default
======================================  ==========================================
``CODIFY_DICTATION_MODEL_DIR``        ``~/.codify/models/dictation/asr``
``CODIFY_DICTATION_PUNCT_DIR``        ``~/.codify/models/dictation/punct``
======================================  ==========================================
...
ENGINE_ENV = "CODIFY_DICTATION_ENGINE"
MODEL_DIR_ENV = "CODIFY_DICTATION_MODEL_DIR"
PUNCT_DIR_ENV = "CODIFY_DICTATION_PUNCT_DIR"
MAX_STREAMS_ENV = "CODIFY_DICTATION_MAX_STREAMS"
#: Worker stream URL for the ``remote`` engine, e.g.
#: ``ws://venus:8100/v1/dictation/stream``.
REMOTE_URL_ENV = "CODIFY_DICTATION_REMOTE_URL"

#: Built-in engine names. The default (empty ``CODIFY_DICTATION_ENGINE``)
...
def register_engine(
    ...
    *,
    ...
    Selected via ``CODIFY_DICTATION_ENGINE=<name>``. This is the whole
...
def _asr_dir() -> Path:
    default = Path.home() / ".codify" / "models" / "dictation" / "asr"
    return Path(os.environ.get(MODEL_DIR_ENV) or default).expanduser()


def _punct_dir() -> Path:
    default = Path.home() / ".codify" / "models" / "dictation" / "punct"
...
class RemoteDictationEngine:
    ...
    protocol — another codify server or the standalone
    ``python -m codify.server.dictation_worker``. Lets a small main
