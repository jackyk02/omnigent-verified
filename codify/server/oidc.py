...
class OIDCConfig:
    ...
    def from_env() -> OIDCConfig:
        ...
        def _require(name: str) -> str:
            ...
            if not val:
                raise RuntimeError(
                    ...
                    f"(CODIFY_AUTH_PROVIDER=oidc requires it)"
                )
            return val

        issuer = _require("CODIFY_OIDC_ISSUER")
        client_id = _require("CODIFY_OIDC_CLIENT_ID")
        client_secret = _require("CODIFY_OIDC_CLIENT_SECRET")
        # Redirect URI: an explicit value wins; otherwise derive it from
        # CODIFY_DOMAIN (the same var the Caddy HTTPS overlay uses) as
        ...
        redirect_uri = os.environ.get("CODIFY_OIDC_REDIRECT_URI", "").strip()
        if not redirect_uri:
            domain = os.environ.get("CODIFY_DOMAIN", "").strip()
            if not domain:
                raise RuntimeError(
                    "Missing required environment variable CODIFY_OIDC_REDIRECT_URI "
                    "(or set CODIFY_DOMAIN to derive https://<domain>/auth/callback)"
                )
            redirect_uri = f"https://{domain}/auth/callback"

        cookie_secret_hex = _require("CODIFY_OIDC_COOKIE_SECRET")
        try:
            cookie_secret = bytes.fromhex(cookie_secret_hex)
        except ValueError as exc:
            raise RuntimeError("CODIFY_OIDC_COOKIE_SECRET must be a valid hex string") from exc
        if len(cookie_secret) < 32:
            raise RuntimeError(
                "CODIFY_OIDC_COOKIE_SECRET must be at least 32 bytes (64 hex chars)"
            )

        session_ttl_hours = int(os.environ.get("CODIFY_OIDC_SESSION_TTL_HOURS", "8"))
        logout_redirect_uri = os.environ.get("CODIFY_OIDC_LOGOUT_REDIRECT_URI", "").strip() or None

        raw_domains = os.environ.get("CODIFY_OIDC_ALLOWED_DOMAINS", "").strip()
        ...
        # email; see codify/server/oidc_access.py). Off by default.
        # Imported lazily to avoid a circular import (auth imports oidc).
        from codify.server.auth import env_var_is_truthy

        allow_invites = env_var_is_truthy("CODIFY_OIDC_ALLOW_INVITES")
        ...
        skip_email_verification = env_var_is_truthy("CODIFY_OIDC_SKIP_EMAIL_VERIFICATION")
        if skip_email_verification:
            _logger.warning(
                "CODIFY_OIDC_SKIP_EMAIL_VERIFICATION is set: the "
        ...
        email_claim = (os.environ.get("CODIFY_OIDC_EMAIL_CLAIM") or "email").strip()
        if email_claim != "email":
            _logger.warning(
                "CODIFY_OIDC_EMAIL_CLAIM is set: the user identity "
            ...
            if not skip_email_verification:
                _logger.warning(
                    ...
                    "CODIFY_OIDC_SKIP_EMAIL_VERIFICATION is also set."
                )
        ...
        if is_github:
            ...
            scopes = (os.environ.get("CODIFY_OIDC_SCOPES") or _GITHUB_SCOPES).strip()
            return OIDCConfig(
        ...
        scopes = (os.environ.get("CODIFY_OIDC_SCOPES") or "openid email profile").strip()
