...
Mirrors the cookie + session bits of :class:`codify.server.oidc.OIDCConfig`
...
- ``BASE_URL`` — required. The user-facing base URL of the
  deployment, e.g. ``"https://codify.example.com"`` or
...
class AccountsConfig:
    ...
    def from_env() -> AccountsConfig:
        """Read and validate every required ``CODIFY_ACCOUNTS_*`` env var.
        ...
        def _require(name: str) -> str:
            ...
            if not val:
                raise RuntimeError(
                    ...
                    f"(CODIFY_AUTH_PROVIDER=accounts requires it)"
                )
            return val

        cookie_secret_hex = _require("CODIFY_ACCOUNTS_COOKIE_SECRET")
        try:
            cookie_secret = bytes.fromhex(cookie_secret_hex)
        except ValueError as exc:
            raise RuntimeError("CODIFY_ACCOUNTS_COOKIE_SECRET must be a valid hex string") from exc
        if len(cookie_secret) < 32:
            raise RuntimeError(
                "CODIFY_ACCOUNTS_COOKIE_SECRET must be at least 32 bytes "
                "(64 hex chars). Generate with `openssl rand -hex 32`."
            )

        base_url = _require("CODIFY_ACCOUNTS_BASE_URL").rstrip("/")
        if not base_url.startswith(("http://", "https://")):
            raise RuntimeError(
                f"CODIFY_ACCOUNTS_BASE_URL must start with http:// or https://; got {base_url!r}"
            )

        session_ttl_hours = int(os.environ.get("CODIFY_ACCOUNTS_SESSION_TTL_HOURS", "8"))
        invite_ttl_seconds = int(os.environ.get("CODIFY_ACCOUNTS_INVITE_TTL_HOURS", "72")) * 3600
        magic_ttl_seconds = int(os.environ.get("CODIFY_ACCOUNTS_MAGIC_TTL_MINUTES", "10")) * 60
        ...
        init_admin = os.environ.get("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD") or None
