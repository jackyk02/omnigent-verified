...
from codify.spec import AgentSpec, ExtractionError, ToolRuntime, load
...
def _is_dotted_callable_path(path: str) -> bool:
    ...
    ``codify.runner.tool_dispatch._resolve_spec_callable``) — the
...
def _reject_uploaded_callable_tools(spec: AgentSpec) -> None:
    ...
    :raises CodifyError: If any (sub-)agent declares a server-runtime tool
...
def _cwd_escapes_workspace(spec_cwd: str) -> bool:
    ...
    ``CODIFY_RUNNER_WORKSPACE`` it becomes the agent environment root and
...
def validate_agent_bundle(
    ...
    *,
    ...
    registration boundary instead (``codify.cli._resolve_bundle_env_vars``).
    ...
    :param enforce_handler_allowlist: When ``True`` (the default),
        ...
        trusted single-user/local server, where ``codify run`` uploads
        ...
        call sites in ``codify/server/routes/sessions.py``, which gate
        this on :func:`codify.server.auth.local_single_user_enabled`.
    :returns: The validated :class:`AgentSpec`.
    :raises CodifyError: If the bundle is invalid, the spec is
    ...
    if enforce_handler_allowlist:
        ...
        # (GHSA-p8rw-8qj3-hf33): on a runner without CODIFY_RUNNER_WORKSPACE
