...
from codify.entities import ConversationItem
...
class AgentObject(BaseModel):
    ...
    :param harness: The agent's harness/kind, e.g. ``"codex"``,
        ...
        ``executor.type: codify`` agents, otherwise the executor
    ...
    :param builtin: Whether this is a server-*seeded* built-in
        ...
        ``codify server --agent``) or a session-scoped upload.
        The Web UI's new-session picker uses this to decide
        whether a same-named ``codify run`` upload may shadow
...
class ConversationObject(BaseModel):
    ...
    :param labels: Session-scoped guardrails labels, mirroring
        ...
        with the legacy ``codify run`` Ctrl+G overview.
    """
...
class ChildSessionSummary(BaseModel):
    ...
    :param title: Sub-agent title, ``"{agent_type}:{session_name}"``
        as written by :func:`codify.tools.builtins.spawn._spawn_one`,
        e.g. ``"researcher:auth"``. ``None`` only for legacy /
        malformed rows; the spawn path always sets it.
    :param tool: UI-facing sub-agent label. For Codify-spawned
    ...
    :param pending_elicitations_count: Number of approval / input
        ...
        server's :mod:`codify.runtime.pending_elicitations`
...
class ElicitationResult(BaseModel):
    ...
    Codify clients deliver this shape inside the session-scoped
...
class SessionCreateRequest(BaseModel):
    ...
    :param host_type: How the session's host is obtained.
        ...
        host they registered via ``codify host`` (pass
    ...
    :param harness_override: Optional per-session brain-harness
        ...
        ``CODIFY_HARNESSES`` and the bound agent must be an
        ``executor.type: codify`` spec. ``None`` (the default) uses
    ...
    def _check_managed_host_fields(self) -> SessionCreateRequest:
        ...
        from codify.server.managed_hosts import is_repo_workspace, parse_repo_workspace
...
class SessionResponse(BaseModel):
    ...
    :param external_session_id: Runtime-native session id this
        ...
        ``codify claude`` sessions. ``None`` for regular
    ...
    :param pending_elicitations: Outstanding approval prompts on
        ...
        the Codify server's in-memory
        :mod:`codify.runtime.pending_elicitations` index.
    :param pending_inputs: Un-consumed web-composer user messages on
        ...
        :mod:`codify.runtime.pending_inputs` index.
    ...
    :param todos: Current Claude Code todo list items for
        ``codify claude`` sessions, as raw dicts from Claude's
        ...
        from the Codify server's in-memory ``_session_todos_cache``.
    ...
    :param terminal_pending: ``True`` while the runner is auto-creating
        ...
        exists. Sourced from the Codify server's in-memory
    ...
    :param sandbox_status: Managed-sandbox launch progress while the
        ...
        Sourced from the Codify server's in-memory
    ...
    # Source: :mod:`codify.runtime.pending_inputs`.
...
class UpdateSessionRequest(BaseModel):
    ...
    :param external_session_id: Runtime-native session id captured
        ...
        ``codify claude`` sessions). Idempotent on same-value
...
class CodexGoalObject(BaseModel):
    ...
    Mirrors Codex app-server's ``ThreadGoal`` shape using Codify's
...
class UpdateCodexGoalStatusRequest(BaseModel):
    ...
    updates. Codify exposes the two user-driven transitions explicitly:
...
class SessionListItem(BaseModel):
    ...
    :param pending_elicitations_count: Number of approval prompts
        ...
        opening each chat. Sourced from the Codify server's in-memory
        :mod:`codify.runtime.pending_elicitations` index,
...
class SessionUsage(BaseModel):
    ...
    :func:`codify.runtime.policies.builder.load_session_usage`. It is the
...
# ─────────────────────────────────────────────────────────────────────
#
# Single source of truth for the codify SSE event stream. Every
...
class _SSEEventBase(BaseModel):
    ...
    - ``sequence_number``: monotonic per-stream sequence number
      ...
      (``_format_sse`` in ``codify/server/routes/sessions.py``).
...
class SessionStatusEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py``) — i.e. while the parent
...
class SessionUsageEvent(_SSEEventBase):
    ...
    out-of-AP runtime (e.g. the ``codify claude`` transcript
...
class SessionModelEvent(_SSEEventBase):
    ...
    ``codify claude`` transcript forwarder when the model is
...
class SessionCollaborationModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``codify.codex_native.collaboration_mode`` on the conversation labels,
...
class SessionTodosEvent(_SSEEventBase):
    ...
    ``codify claude`` transcript forwarder, which captures todo
    updates via ``PostToolUse``/``TodoWrite`` hook events from Claude
    Code and forwards them to the Codify server. Lets web render a
...
class SessionTerminalPendingEvent(_SSEEventBase):
    ...
    1. The Codify server at ``POST /v1/sessions`` for host-launched
    ...
    2. The Codify relay when the runner's ``session.terminal_pending`` frame
...
class SessionSandboxStatusEvent(_SSEEventBase):
    ...
    A managed create returns before its sandbox exists; the Codify
...
class SessionSkillsEvent(_SSEEventBase):
    ...
    ``codify/server/routes/sessions.py``) and serves ``[]`` until
...
class SessionInputConsumedPayload(BaseModel):
    ...
    :param type: The item type discriminator — ``"message"`` for
        ...
        results, etc. Mirrors
        :class:`codify.server.schemas.SessionEventInput`'s
        ``type`` field.
    ...
    :param cleared_pending_id: When this consumed message drains a
        :mod:`codify.runtime.pending_inputs` entry (a native-
...
class SessionInterruptedPayload(BaseModel):
    ...
    ``codify/server/routes/sessions.py``.
...
class SessionInterruptedEvent(_SSEEventBase):
    ...
    ``codify/server/routes/sessions.py`` when a client posts
...
class SessionCreatedEvent(_SSEEventBase):
    ...
    Emitted by ``codify/tools/builtins/spawn.py:_spawn_one``
...
class SessionSupersededEvent(_SSEEventBase):
    ...
    ``codify/server/routes/sessions.py`` when the claude-native
...
class OutputTextDeltaEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py:1352-1356``.
...
class ReasoningStartedEvent(_SSEEventBase):
    ...
    status. Wire shape matches ``codify/runtime/workflow.py:1350``.
...
class ReasoningTextDeltaEvent(_SSEEventBase):
    ...
    shape matches ``codify/runtime/workflow.py:1358-1364``.
...
class ReasoningSummaryTextDeltaEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py:1370-1373``.
...
class OutputItemDoneEvent(_SSEEventBase):
    ...
    :param item: The completed item dict. Heterogeneous and
        ...
        ``codify/entities/conversation.py`` for the
...
class OutputFileDoneEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py`` once per file annotation in
...
class HeartbeatEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py`` (15 seconds at the time of
    writing). Wire shape matches the existing emit at
    ``codify/runtime/workflow.py:4636-4639``.
...
class SessionPresenceEvent(_SSEEventBase):
    ...
    each other's lists. See ``codify/server/presence.py`` and
...
class ElicitationRequestParams(BaseModel):
    ...
    ``codify/runtime/policies/approval.py:175``.
    ...
    :param phase: Codify policy-engine phase the elicitation
        belongs to, e.g. ``"pre_tool_use"``.
    :param policy_name: Codify policy that triggered the
    ...
    # ``elicitation/create`` traversing harness → Codify → client)
...
class ElicitationRequestEvent(_SSEEventBase):
    ...
    Emitted by Codify (or, under the new contract, by a harness)
    ...
    ``type == "approval"`` and
    :class:`codify.server.schemas.ElicitationResult` fields in
    ...
    :param method: MCP method literal — always
        ...
        ``codify/runtime/policies/approval.py``).
...
class BrowserActionRequestEvent(_SSEEventBase):
    ...
    Codify desktop app's embedded browser to act. The event fans out
...
class ElicitationResolvedEvent(_SSEEventBase):
    ...
    Idempotent on the consumer side: the Codify server's index
...
class PolicyDeniedEvent(_SSEEventBase):
    ...
    prompt through Codify's policy engine via the vendor command-hook
...
class CreatedEvent(_SSEEventBase):
    ...
    :class:`codify.server.schemas.ResponseObject` (status will
...
class CompletedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`codify.server.schemas.ResponseObject`.
...
class FailedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`codify.server.schemas.ResponseObject` whose
...
class RetryEvent(_SSEEventBase):
    ...
    Emitted by ``codify/runtime/llm_retry.py`` (LLM calls)
    and ``codify/runtime/tool_retry.py`` (tool calls) before
...
class ErrorEvent(_SSEEventBase):
    ...
    ``codify/runtime/workflow.py`` — terminal LLM failures
...
class CompactionInProgressEvent(_SSEEventBase):
    ...
    Emitted by ``codify/runtime/compaction.py`` while a
...
class CompactionFailedEvent(_SSEEventBase):
    ...
    Emitted by ``codify/server/routes/sessions.py`` when
...
class ClientTaskCancelEvent(_SSEEventBase):
    ...
    Emitted by ``codify/runtime/workflow.py`` when a parent
...
#     from codify.server.schemas import ServerStreamEvent
...
def is_known_event(name: str) -> bool:
    ...
    patch :func:`codify.runtime.session_stream.publish` to
...
# ``type`` on the raw frame, see ``codify/runner/app.py`` proxy_stream)
...
class PolicyEvaluationRequestEvent(_SSEEventBase):
    ...
    policies on the Codify server. The runner intercepts this event in
    ``proxy_stream``, calls the Codify server's
    ...
    :param data: Event data dict passed to the Codify server's
