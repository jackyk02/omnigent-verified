...
The ``codify server`` CLI already takes ``-c/--config`` and reads a
YAML file (see ``codify/cli.py``). The hosted entrypoints —
...
1. ``CODIFY_CONFIG`` env var, if set (explicit path).
2. ``<data_dir>/config.yaml`` if it exists — ``<data_dir>`` is the same
   directory the admin list / credentials use (``/data`` in the Docker
   stack, ``~/.codify`` on a laptop; see
   :func:`codify.server.admin_list.resolve_data_dir`).
...
from codify.server.admin_list import resolve_data_dir
...
def resolve_config_path() -> Path | None:
    ...
    :returns: ``CODIFY_CONFIG`` if set; else ``<data_dir>/config.yaml``
        when that file exists; else ``None``.
    """
    explicit = os.environ.get("CODIFY_CONFIG", "").strip()
    if explicit:
...
def copy_file_count_limit() -> int:
    ...
    :data:`codify.runtime.content_resolver.MAX_COPY_FILES`.
    """
    from codify.runtime.content_resolver import MAX_COPY_FILES
...
def copy_total_bytes_limit() -> int:
    ...
    :data:`codify.runtime.content_resolver.MAX_COPY_TOTAL_BYTES`.
    """
    from codify.runtime.content_resolver import MAX_COPY_TOTAL_BYTES
