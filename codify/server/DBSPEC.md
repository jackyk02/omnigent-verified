...
in `codify/db/db_models.py`, which is the source of truth for the full table
list.
...
`codify/runner/app.py`) and is never written to the DB. Only a coarse mirror
is persisted — `codify_conversation_metadata.live_status` /
...
then cleans up the Codify-side rows: comments, policies, session permissions,
...
Codify rows survive a conversation that no longer exists. Do not remove any
...
in `codify/runner/app.py`; none of it is a DB operation.
