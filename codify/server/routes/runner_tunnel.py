...
from codify.runner.transports.ws_tunnel.registry import RunnerSession, TunnelRegistry
...
def _is_loopback_websocket_client(ws: WebSocket) -> bool:
    ...
    Local ``codify server`` starts an unauthenticated runner that
...
def _resolve_tunnel_owner(
    ...
    *,
    ...
    :param is_loopback: Whether the peer is a loopback client, e.g.
        the unauthenticated local runner started by ``codify server``.
...
def create_runner_tunnel_router(
    ...
    *,
    ...
    def _get_user_id_from_request(request: Request) -> str | None:
        ...
        :raises CodifyError: 401 when the provider rejects the
            request.
        """
    ...
    async def mint_runner_owner_token(request: Request, runner_id: str) -> dict[str, str | int]:
        ...
        (``X-Codify-Runner-Tunnel-Token``) and the server returns a
        ...
        :raises CodifyError: 401 when the binding token is absent,
        ...
        if not token or token_bound_runner_id(token) != runner_id:
            raise CodifyError("unauthenticated", code=ErrorCode.UNAUTHORIZED)
        owner: str | None = None
