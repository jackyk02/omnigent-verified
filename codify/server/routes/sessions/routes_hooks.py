...
def register_hooks_routes(
    ...
    *,
    ...
    async def claude_permission_request_hook(
        ...
        the same Bearer token used for every other Codify request. For
        ...
        :param session_id: Codify conversation id from the URL path.
        :returns: Claude PermissionRequest hookSpecificOutput JSON,
            or ``200`` with empty body on timeout (fail-ask).
        :raises CodifyError: 404 if the session doesn't exist,
    ...
    async def evaluate_policy(
        ...
        command hooks (via ``codify.claude_native_hook``) to
        ...
        raw_elicitation_id = payload.get("_codify_elicitation_id")
        ...
        if raw_elicitation_id is not None:
            if not isinstance(raw_elicitation_id, str) or not (
                ...
                raise CodifyError(
                    "Policy evaluate '_codify_elicitation_id' must match "
    ...
    async def codex_elicitation_request_hook(
        ...
        :raises CodifyError: 404 if the session does not exist,
    ...
    async def antigravity_elicitation_request_hook(
        ...
        :class:`~codify.server.schemas.ElicitationResult` so the bridge
        ...
        :class:`~codify.server.schemas.ElicitationRequestParams` dict.
        ...
        :raises CodifyError: 404 if the session does not exist, 400 if
    ...
    async def cursor_permission_request_hook(
        ...
        (:mod:`codify.cursor_native_permissions`), publishes the standard
        ...
        :returns: An ``ElicitationResult`` (``{"action": …}``) on a web
            ...
            disconnect.
        :raises CodifyError: 404 if the session does not exist, 400 if the
            body is malformed.
        """
        user_id = _get_user_id(request, auth_provider)
        await _require_access(
