...
from codify.runtime.policies.approval import _ELICITATION_MODE
...
from codify.server.permissions import check_session_access
...
from codify.server.routes._content_type import (
    require_json_or_multipart_content_type,
)
from codify.server.routes._errors import session_not_found as _session_not_found
from codify.server.routes._origin import require_trusted_origin
...
from codify.server.routes._sessions.orchestration import *
...
def register_core_routes(
    ...
    *,
    ...
    async def create_session(
        ...
        :raises CodifyError: If metadata, bundle, or agent lookup
        ...
        if launch_host_id is not None and resp.runner_id is None:
            ...
            if host_registry is not None and host_store_inst is not None:
                ...
                from codify.server.routes._host_launch import resolve_host_launch
                ...
                if launch_result.get("status") == "failed":
                    ...
                    # can be stale (the user may have run `codify setup`
    ...
    async def _create_bundled_session_from_multipart(
        ...
        :raises HTTPException: 422 when a required multipart part is
            absent.
        :raises CodifyError: If metadata or bundle validation
    ...
    async def put_read_state(
        ...
        :raises CodifyError: 403 if the caller lacks read access.
        """
        user_id = _require_user(request, auth_provider)
    ...
    async def get_session(
        ...
        :raises CodifyError: 404 if no session exists.
        """
    ...
    async def list_sessions(
        ...
        :param pinned: When ``True``, return only sessions the user
            has pinned (the ``codify.pinned`` label). Lets the
    ...
    async def session_updates(websocket: WebSocket) -> None:
        ...
        async def _emit_deltas() -> None:
            ...
            from codify.server.routes import sessions as _sf
    ...
    from codify.server.routes.codex.sessions import register_codex_session_routes
    ...
    async def automatically_rename_session(
        ...
        conv = await asyncio.to_thread(conversation_store.get_conversation, session_id)
        if conv is None:
            raise CodifyError("Session not found", code=ErrorCode.NOT_FOUND)
    ...
    async def update_session(
        ...
        :raises CodifyError: 400 if the runner is not
        ...
        #   preference (stored as the caller's own ``codify.pinned.<user>``
        ...
        if body.runner_id is not None and permission_store is not None:
            if not check_session_access(
                ...
                raise CodifyError(
                    ...
                    f"To fork this session instead, run: codify run --fork {session_id}",
        ...
        # Pins are per-user. The client writes the canonical ``codify.pinned``
        ...
        # dispatch — Codify server stays harness-agnostic; the runner
        ...
        if live_forward and (model_override is not None or clear_model):
            ...
            # whose history Codify writes. Gate on the wrapper label (NOT
            # codify.ui, which chat-first SDK terminal-view sessions like
        ...
        if set_project:
            ...
            if target_project_id == "":
                ...
                if owned is None:
                    raise CodifyError("Project not found", code=ErrorCode.NOT_FOUND)
    ...
    async def fork_session(
        ...
        :raises CodifyError: 404 if *source_id* does not exist
        ...
        # the copied Codify transcript on its own — mark the fork so the
        ...
        # copied Codify items. Cross-family: the source's native transcript
        # is the wrong format, so ALWAYS rebuild from the copied Codify
        # items (the converters consume Codify's normalized item shape, so
    ...
    async def switch_session_agent(
        ...
        :raises CodifyError: 404 if the session or target agent does
        ...
        try:
            from codify.server.routes import sessions as _sessions_facade
        ...
        # Codify items (a format-agnostic conversion). SDK targets replay
