...
from codify.server.routes.sessions.routes_permissions import (
    _policy_description,
    _policy_type,
    _to_agent_object,
)
...
def register_agent_routes(
    ...
    *,
    ...
    async def get_session_agent(
        ...
        :raises CodifyError: If the session or agent is not found.
        """
    ...
    async def get_session_agent_contents(
        ...
        :raises CodifyError: If the session, agent, or bundle is
            not found.
        """
        user_id = _require_user(request, auth_provider)
        access = await _require_access_and_level(
    ...
    async def update_session_agent(
        ...
        if spec.name is None:
            raise CodifyError("spec missing name", code=ErrorCode.INVALID_INPUT)
    ...
    async def mcp_proxy(
        ...
        - Policy runs on the Codify server where the ConversationStore and
          label state live.
        ...
        if method == "initialize":
            ...
            return _mcp_ok_response(
                rpc_id,
                {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "codify-mcp-proxy", "version": "1.0.0"},
                },
            )
