...
from codify.server.routes._sessions.helpers import _load_agent_spec_for_session
...
def register_resources_routes(
    ...
    *,
    ...
    async def _validate_session(
        ...
        :raises CodifyError: 401/403/404 on auth or access failure.
        """
    ...
    async def _fs_get_with_host_fallback(
        ...
        *,
        ...
        :class:`codify.workspace_fs.WorkspaceReader` and returns the
        ...
        :raises CodifyError: Re-raised runner-offline error when the
    ...
    async def _read_workspace_via_host(
        ...
        """
        from codify.server.routes._host_filesystem import (
            HostFsError,
        ...
        try:
            ...
            if exc.status == 404:
                raise CodifyError(exc.message, code=ErrorCode.NOT_FOUND) from exc
            if exc.status == 400:
    ...
    async def create_session_terminal(
        ...
        or ``bridge_inject_dir`` — the ``codify claude`` / ``codex``
        ...
        :raises CodifyError: 400 when the requested terminal is not
        ...
        if status >= 400:
            ...
            # CodifyError derives http_status from code; pass the runner's code, not a status.
            raise CodifyError(
    ...
    async def copy_session_files(
        ...
        """
        from codify.server.server_config import (
    ...
    async def _proxy_fs_response(
        ...
        *,
        ...
        if status >= 400:
            ...
            if status == 404:
                raise CodifyError(message, code=ErrorCode.NOT_FOUND)
