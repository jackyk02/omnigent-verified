...
from codify.runner.identity import RUNNER_TUNNEL_TOKEN_HEADER, token_bound_runner_id
    ...
    *,
    ...
    async def post_event(
        ...
        - ``"external_output_reasoning_delta"`` publishes a transient
          ...
          observed outside the Codify task runtime, without persisting an
          item or starting / steering a task.
        - ``"external_session_interrupted"`` publishes a
          ``session.interrupted`` event observed outside the Codify task
        ...
        - ``"external_codex_collaboration_mode_change"`` persists the
          ...
          (``codify.codex_native.collaboration_mode``).
        ...
        - ``"message"`` on an ``codify claude`` terminal session
        ...
        if body.type == _COMPACT_TYPE:
            ...
            # The Codify server stays harness-agnostic: it runs its own
            ...
            if runner_client is None and _host_reg is not None:
                ...
                if _host_conn is not None:
                    ...
                    if launch_attempt.error_code == _HARNESS_NOT_CONFIGURED_ERROR_CODE:
                        ...
                        # fix, `codify setup`) the web renders as a
        ...
        if runner_client is None:
            ...
            if body.type == "message" and _is_native_terminal_session(conv):
                ...
                return {"queued": True, "item_id": item_id}
            # Raise so the Codify server doesn't persist an item the
    ...
    async def delete_session(
        ...
        :raises CodifyError: 404 if no session or no access,
        ...
        if runner_client is not None:
            ...
            from codify.runtime import get_terminal_registry
