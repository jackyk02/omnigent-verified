...
from codify.cost_plan import (
    COST_CONTROL_LABEL_NAMESPACE,
    reserved_cost_control_keys,
)
from codify.db.utils import generate_task_id
from codify.entities import (
    Agent,
    Conversation,
    ConversationItem,
    ErrorData,
...
from codify.entities.permission import SessionPermission
...
from codify.policies.types import EvaluationContext
from codify.reasoning_effort import (
    EFFORT_VALUES,
    validate_effort,
)
...
from codify.runner.routing import RunnerRouter
from codify.runner.transports.ws_tunnel.registry import TunnelRegistry
from codify.runtime import (
    get_policy_store,
    inflight_text,
...
from codify.runtime.prompt import model_author_prefix
from codify.runtime.tool_output import cap_tool_output
from codify.server import presence, session_live_state
from codify.server._elicitation_registry import (
    _harness_elicitation_owners,
    _harness_parked_elicitations,
    _harness_pre_resolved_elicitations,
    _PreResolvedHarnessElicitation,
)
from codify.server.auth import (
    LEVEL_OWNER,
    LEVEL_READ,
    RESERVED_USER_PUBLIC,
)
from codify.server.host_registry import HostConnection, HostRegistry, RunnerExitReports
from codify.server.managed_hosts import (
    ManagedHostLaunch,
    ManagedLaunch,
...
from codify.server.routes._auth_helpers import (
    require_access as _require_access,
)
from codify.server.routes._host_worktree import CreatedWorktree
from codify.server.routes._session_create_validation import (
...
from codify.server.routes._sessions.common import *
...
from codify.server.routes._sessions.common import (  # noqa: F401
    _ELICITATION_MODE,
    build_policy_engine,
    get_agent_cache,
    get_caps,
...
from codify.server.schemas import (
    ChildSessionSummary,
    CompletedEvent,
...
from codify.session_lifecycle import (
...
from codify.stores import AgentStore, ConversationStore
...
from codify.stores.host_store import Host, HostStore
...
def _native_ask_gate_lock(conversation_id: str, deciding_policy: str) -> asyncio.Lock:
    ...
    :param conversation_id: Codify conversation id whose ASK gate is being
...
async def _poll_request_disconnect(*args: Any, **kwargs: Any) -> None:
    ...
    from codify.server.routes import sessions as _facade
...
def _publish_and_persist_resource_event(
    ...
    """
    from codify.entities.conversation import ResourceEventData
...
def _signal_terminal_resolved_harness_elicitation_impl(
    ...
    :param session_id: Codify conversation id whose forwarder mirrored the
...
def _client_supplied_hook_elicitation_id(
    ...
    :param payload: Parsed PermissionRequest hook body. Reads the
        optional ``_codify_elicitation_id`` key.
    ...
    :raises CodifyError: 400 when the id is malformed or is
        currently parked by a different session.
    """
    raw = payload.get("_codify_elicitation_id")
    if raw is None:
        return None
    if not isinstance(raw, str) or not _HOOK_ELICITATION_ID_RE.fullmatch(raw):
        raise CodifyError(
            "PermissionRequest hook '_codify_elicitation_id' must match "
...
def _consume_pre_resolved_harness_elicitation(
    ...
    :param session_id: Codify session id, e.g. ``"conv_abc123"``.
...
def _signal_harness_elicitation_resolved_by_id(
    ...
    :param elicitation_id: Harness elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: None.
    :raises CodifyError: If the id is malformed or belongs to a
        different session.
    """
...
def _permission_level_from_grants(
    ...
    Mirrors :func:`codify.server.routes._auth_helpers._get_permission_level_sync`
...
def _owner_from_grants(grants: list[SessionPermission]) -> str | None:
    ...
    Mirrors :func:`codify.server.routes._auth_helpers.get_session_owner_id`
    but operates on grants already held in memory so callers can batch the
    permission-store query across many sessions at once.
...
def _session_status_from_cache(
    ...
    replica persisted (``codify_conversation_metadata.live_status``) — so a replica
...
def _publish_input_consumed(
    ...
    :param cleared_pending_id: When this message drained a
        :mod:`codify.runtime.pending_inputs` entry (native-terminal
...
def _validated_harness_override(value: str | None, agent: Agent) -> str | None:
    ...
    in ``codify/chat.py``): the canonical name must be a known bundle
    harness, and the bound agent must be an ``executor.type: codify``
    ...
    :raises CodifyError: ``invalid_input`` for an unknown harness, a
        non-codify executor type, or an unloadable agent bundle.
    """
    if value is None:
        return None
    from codify.harness_aliases import canonicalize_harness
    from codify.runtime import get_agent_cache
    from codify.spec._codify_compat import (
        CODIFY_EXECUTOR_TYPE,
        CODIFY_HARNESSES,
    )

    canonical = canonicalize_harness(value) or value
    if canonical not in CODIFY_HARNESSES:
        raise CodifyError(
            f"invalid harness_override: must be one of {sorted(CODIFY_HARNESSES)}, got {value!r}",
            code=ErrorCode.INVALID_INPUT,
        )
    try:
        loaded = get_agent_cache().load(
    ...
    if executor_type != CODIFY_EXECUTOR_TYPE:
        raise CodifyError(
            f"harness_override only applies to executor.type "
            f"{CODIFY_EXECUTOR_TYPE!r} agents; agent {agent.name!r} "
...
def _validated_harness_override_executor_type(agent: Agent) -> None:
    """Validate that *agent* is an ``executor.type: codify`` spec.
    ...
    :raises CodifyError: ``invalid_input`` when the agent is not an
        codify executor type or the bundle cannot be loaded.
    """
    from codify.runtime import get_agent_cache
...
def _record_daily_cost(
    ...
    if owner is None:
        return
    from codify.db.utils import now_epoch
...
def _coerce_cumulative_field(
    ...
    *,
    ...
    :raises CodifyError: When present but the wrong type / negative.
    """
    value = data.get(key)
    if value is None:
        return None
...
async def _persist_external_model_change(
    ...
    Codify session: writes ``model_override`` so the value survives reload
    ...
    :raises CodifyError: If ``data.model`` is missing or not a
        non-empty string.
    """
...
def _persist_external_model_options(
    ...
    Codify-configured provider OR the harness's own ``/login``. This is why
    ...
    :raises CodifyError: If the session is not pi-native, or ``data.models``
...
def _validate_external_reasoning_effort(body: SessionEventInput) -> str | None:
    ...
    :raises CodifyError: If the payload is missing or unsupported.
    """
...
async def _persist_external_reasoning_effort_change(
    ...
    Mirrors a native-terminal thinking-level change onto the Codify session.
...
async def _persist_external_codex_collaboration_mode_change(
    ...
    :returns: None.
    :raises CodifyError: If ``data.mode`` is missing or unsupported.
    """
...
def _handle_external_session_todos(
    ...
    :raises CodifyError: When ``data.todos`` is missing or not a list.
    """
...
def _publish_external_output_text_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises CodifyError: If ``data.delta`` is not a string, or any
...
def _publish_external_tool_output_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises CodifyError: If either required value is missing or not a string.
    """
...
def _publish_external_output_reasoning_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises CodifyError: If ``data.delta`` is not a string, or ``data.started``
...
def _parse_external_assistant_message(
    ...
    :raises CodifyError: If required fields are missing or
        malformed.
    """
...
async def _persist_external_assistant_message(
    ...
    """
    Persist and broadcast assistant text produced outside Codify tasks.

    The event is append-only conversation history. It intentionally
    bypasses the legacy persist path so mirroring a
    Claude terminal response does not create or steer an Codify
    agent task.
...
async def _persist_external_subagent_start(
    ...
    never POSTs to Codify to register them. The forwarder watches the
    ...
    codify agent), stamp identifying labels, and publish the
    same ``session.created`` event codify-spawned children fire
    ...
    :raises CodifyError: 400 if the payload is missing any of
    ...
    if parent_conv.agent_id is None:
        ...
        # by ``codify claude`` (the synthetic Claude bundle).
    ...
    # Title format mirrors codify-spawned children
...
def _subagent_delivery_status(
    ...
    A claude-native session running as an Codify sub-agent relabels its
...
def _is_kiro_native_session(conv: Conversation) -> bool:
    ...
    return conv.labels.get("codify.wrapper") == "kiro-native-ui"
...
def _latest_assistant_text_from_store(
    ...
    helper lets Codify forward the durable assistant output with the
...
def _require_external_status_forward(
    ...
    :returns: None.
    :raises CodifyError: If the runner was unavailable or
...
def _require_collaboration_mode_forward(
    ...
    app-server. Persisting the Codify label without a successful runner
    ...
    :param runner_result: HTTP result returned by the runner, or ``None``
        when no runner could be reached.
    :returns: None.
    :raises CodifyError: If no runner was reachable or the runner rejected
...
def _publish_sandbox_status_impl(session_id: str, stage: str, error: str | None = None) -> None:
    ...
    :param stage: The launch stage just entered, e.g.
        ``"provisioning"`` — one of
        :data:`codify.server.schemas.SandboxLaunchStage`.
...
def _publish_session_superseded(session_id: str, target_conversation_id: str) -> None:
    ...
    ``codify/claude_native_forwarder.py``): a client actively viewing
...
async def _get_runner_client_impl(
    ...
    """
    from codify.runtime import get_runner_client
    ...
    if runner_router is not None:
        ...
        except (LookupError, httpx.HTTPError, CodifyError):
            _logger.debug(
...
async def _query_host_runner_status(
    ...
    """
    from codify.host.frames import HostRunnerStatusFrame, encode_host_frame
    from codify.server.routes import sessions as _facade
...
async def _validate_session_workspace(
    *,
    ...
    :mod:`codify.server.routes._workspace_validation` and
    raises :class:`CodifyError` on failure so the route layer
    ...
    :raises CodifyError: With ``ErrorCode.INVALID_INPUT`` on
...
class _HostLaunchAttempt:
    ...
    :param error: Human-readable failure message from the host, e.g.
        ...
        `codify setup` ..."``; ``None`` when there was no error.
    """
...
async def _launch_runner_on_host_impl(
    ...
    """
    from codify.host.frames import HostLaunchRunnerFrame, encode_host_frame
...
async def _provision_managed_sandbox(
    *,
    ...
    """
    from codify.server.managed_hosts import launch_managed_host, relaunch_managed_host
...
async def _await_settled_managed_launch(launch: ManagedLaunch) -> None:
    ...
    :raises CodifyError: 503 when the launch failed or is still
        running at the timeout.
    """
    from codify.server.managed_hosts import MANAGED_LAUNCH_RENDEZVOUS_TIMEOUT_S
...
async def _get_runner_client_for_resource_access_impl(
    ...
    """
    from codify.runtime import get_runner_client, get_runner_router
...
async def _reset_runner_resources_after_switch_impl(session_id: str) -> None:
    ...
    except (httpx.HTTPError, HTTPException, CodifyError, RuntimeError):
        # Best-effort: a runner hiccup must not break the (already-committed)
        # switch. CodifyError covers the session-not-runner-bound / runner-
...
def _native_coding_agent_for_session(conv: Conversation) -> NativeCodingAgent | None:
    ...
    * the ``codify.wrapper`` presentation label — set for the built-in
      terminal-first wrapper sessions (``codify claude`` / ``codify
...
def _native_terminal_name_for_harness(harness: str) -> str:
    ...
    :raises CodifyError: If *harness* is not a supported native
        terminal harness.
    """
...
def _extract_claude_native_runner_failure(resp: httpx.Response) -> str | None:
    ...
    ``response.failed``. Claude-native Codify forwarding must treat that
...
async def _forward_session_change_to_runner_impl(
    ...
    * Explicit ``compact`` — the caller inspects the returned status
      ...
      200) or the Codify server must run its own in-process compaction
    ...
    skipped — the persisted value on the Codify side is the authoritative
    ...
    in the Codify log — otherwise the POST succeeds at the httpx layer
...
async def _stop_session_via_runner_impl(
    ...
    :raises CodifyError: ``RUNNER_UNAVAILABLE`` (HTTP 503) if the
...
async def _stop_session_host_runner(
    ...
    the host's ``codify host`` tunnel is open while the user drives
    ...
    if conn is None:
        ...
        return False
    from codify.host.frames import HostStopRunnerFrame, encode_host_frame
...
def _parse_skill_slash_command(body: SessionEventInput) -> tuple[str, str]:
    ...
    :raises CodifyError: If the payload is not a skill command
...
def _build_skill_slash_command_policy_body(body: SessionEventInput) -> SessionEventInput:
    ...
    :raises CodifyError: If the slash-command payload is invalid.
    """
...
async def _resolve_skill_meta_text_via_runner(
    ...
    :returns: The hidden ``<skill>`` meta text for a single
        ``input_text`` block.
    :raises CodifyError: If the skill is not exposed for the session
...
async def _dispatch_skill_slash_command_to_runner(
    ...
    * a hidden ``message`` item with ``is_meta=True`` containing the
    ...
    :raises CodifyError: If the skill is not exposed for the
...
class _SessionEventDispatchResult:
    ...
    :param pending_id: Id of the :mod:`codify.runtime.pending_inputs`
...
async def _run_compact_locked(
    ...
    async with lock:
        if conv.agent_id is None:
            raise CodifyError("Session has no agent binding", code=ErrorCode.INTERNAL_ERROR)
        ...
        if spec.llm is not None:
            ...
            from codify.spec.types import LLMConfig
        ...
        from codify.runtime.workflow import compact_conversation_now
...
def _agent_provider_family(agent: Agent) -> str | None:
    ...
    """
    from codify.onboarding.provider_config import provider_family_for_harness
...
def _same_provider_family_impl(a: Agent, b: Agent) -> bool:
    ...
    native transcript from Codify items instead).
...
def _agent_is_native_impl(agent: Agent) -> bool:
    ...
    """
    from codify.harness_aliases import is_native_harness
...
def _agent_carries_native_fork_history_impl(agent: Agent) -> bool:
    ...
    session file that the runner rebuilds from the copied Codify items on
...
def _presentation_labels_for_agent_impl(agent: Agent) -> dict[str, str]:
    ...
    main view), gated on ``codify.ui == "terminal"`` plus the matching
    ``codify.wrapper`` value; an SDK agent runs as plain chat (no such
...
def _publish_input_deny_terminal(session_id: str, conv: Conversation, reason: str) -> None:
    ...
    :class:`codify_client.SessionsChat.send`) iterate until a
...
def _validate_terminal_launch_args(value: list[str] | None) -> list[str] | None:
    ...
    a caller to smuggle internal launch wiring (bridge dir, Codify URL,
...
def _validated_cost_control_mode_override(value: str | None) -> str | None:
    ...
    :raises CodifyError: 400 (``invalid_input``) when *value* is
...
def _parse_session_create_metadata(metadata: str) -> SessionCreateMetadata:
    ...
    :raises CodifyError: If the JSON fails the request schema.
    """
    try:
...
def _require_host_conn_for_worktree(host_id: str | None, request: Request) -> HostConnection:
    ...
    :raises CodifyError: ``invalid_input`` when ``host_id`` is
...
async def _create_session_worktree(
    *,
    ...
    :raises CodifyError: ``invalid_input`` for a bad branch name,
        ...
        is configured.
    """
    from codify.host.git_worktree import WorktreeError, validate_branch_name
    from codify.server.routes._host_worktree import (
    ...
    try:
        ...
        raise CodifyError(exc.message, code=ErrorCode.INVALID_INPUT) from exc
    ...
    try:
        ...
        raise CodifyError(exc.message, code=ErrorCode.CONFLICT) from exc
...
def _resolve_subagent_spec(
    *,
    ...
    if agent_cache is None:
        return None
    from codify.runtime.workflow import _find_spec_by_name
...
def _require_declared_subagent(
    *,
    ...
    :raises CodifyError: 404 ``NOT_FOUND`` when the bundle loads and
...
def _spec_config_flag_explicitly_disabled(spec: AgentSpec, key: str) -> bool:
    ...
    ``codify/spec/parser.py`` — ``{str(k): str(v) ...}``), so a YAML
...
def _derive_terminal_launch_args_from_spec(sub_spec: AgentSpec) -> list[str] | None:
    ...
    if harness == _CODEX_NATIVE_HARNESS:
        ...
        # session; the codify executor's later turn/start inherits that
...
def _reject_reserved_cost_control_label_seed(labels: dict[str, str]) -> None:
    ...
    :param labels: The client-supplied initial labels, e.g.
        ``{"team": "ml"}``.
    :raises CodifyError: 400 when any ``cost_control.*`` key is
        present.
    """
...
def _reject_server_reserved_label_seed(labels: dict[str, str] | None) -> None:
    ...
    :raises CodifyError: 400 when any reserved key is present.
    """
    if not labels:
        return
    ...
    # ``codify.pinned`` key (which the route rewrites to the CALLER's per-user
    # key). A suffixed ``codify.pinned.<user>`` is server-derived — accepting
...
def _require_cost_control_label_authority(
    *,
    ...
    :param tunnel_token: Value of the ``X-Codify-Runner-Tunnel-Token``
    ...
    :raises CodifyError: 403 when the caller presents no acceptable
...
def _persist_stored_session_bundle(
    ...
    *,
    ...
    :raises CodifyError: If the agent insert violates integrity
...
async def _authorize_bundled_parent_and_inherit_runner(
    ...
    *,
    ...
    :raises CodifyError: 403/404 when the caller may not access the
        parent session.
    """
...
def _registered_runner_id(
    ...
    *,
    ...
    :raises CodifyError: If the id is empty, the router is
...
def _child_session_summary_from_conversation(
    ...
    :func:`codify.tools.builtins.spawn._spawn_one`, plus the
    ...
    # sidebar rows only), but strip any per-user ``codify.pinned.<user>`` keys
...
async def _handle_advise_models_mcp(
    ...
    *,
    ...
    from codify.model_catalog import spec_harness
    from codify.server.smart_routing import fetch_runner_models
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The Codify server's role here is routing only —
...
async def _load_model_options(
    ...
    import codify.server.routes.sessions as _facade
...
async def _host_model_options_via_registry(host_id: str) -> list[dict[str, Any]] | None:
    ...
    from codify.server.routes.hosts import _proxy_model_options
