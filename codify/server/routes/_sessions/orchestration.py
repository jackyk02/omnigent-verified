...
from codify.db.utils import generate_agent_id, generate_task_id
...
from codify.errors import CodifyError, ElicitationDeclinedError, ErrorCode
from codify.host.frames import (
    HARNESS_NOT_CONFIGURED_ERROR_CODE as _HARNESS_NOT_CONFIGURED_ERROR_CODE,
)
from codify.llms.context_window import resolve_effective_context_window
...
from codify.runner.session_init_protocol import build_runner_session_init_payload
...
from codify.server.bundles import bundle_location, validate_agent_bundle
...
from codify.server.routes._errors import session_not_found as _session_not_found
...
from codify.server.routes._sessions.helpers import *
...
from codify.telemetry import emit as _tel_emit
from codify.telemetry.events import SessionCreatedEvent as _TelSessionCreatedEvent
from codify.telemetry.installation_id import get_installation_id as _get_installation_id
from codify.telemetry.surface import classify_surface as _classify_surface
...
async def _publish_and_wait_for_harness_elicitation(
    ...
    *,
    ...
    Mirrors the ``codify claude`` permission hook contract: the
...
def _labels_for_viewer(labels: dict[str, str], user_id: str | None) -> dict[str, str]:
    ...
    Pins are stored per-user as ``codify.pinned.<user>`` (see
    :func:`pinned_label_key`). On the wire we present a single canonical
    ``codify.pinned`` key — set iff THIS viewer pinned the session — and drop
    every ``codify.pinned.*`` key. That keeps the per-user dimension server-
    ...
    :returns: A copy with all ``codify.pinned.*`` keys removed and the
        canonical ``codify.pinned`` key added when the viewer's own pin
        is present.
    """
...
def _build_session_response(
    ...
    :raises CodifyError: If ``conv.agent_id`` is ``None``.
    """
    ...
    # ``codify.ui = "terminal"`` label. That flag is fully determined by the
    ...
    **only when the turn is priced** (catalog pricing available or a
    ...
    if llm_model:
        if has_provider_cost:
            ...
            from codify.llms.context_window import compute_llm_cost, fetch_model_pricing
...
def _persist_native_cumulative_usage(
    ...
    Unlike the Codify relay path (:func:`_accumulate_session_usage`), which adds
    ...
    :raises CodifyError: When a cumulative field is the wrong type.
    """
...
async def _persist_external_session_usage(
    ...
    :raises CodifyError: On missing / malformed fields.
    """
    ...
    # from the Codify relay's per-response accumulation. Persist this session's
...
async def _persist_model_change_note(
    ...
    ``codify.wrapper`` native label, via ``_is_native_terminal_session``)
    ...
    keys on ``codify.wrapper`` rather than ``codify.ui == "terminal"``
...
async def _persist_external_codex_subagent_start(
    ...
    :raises CodifyError: If ``thread_id`` is missing or parent has
        no bound agent.
    """
...
async def _persist_external_conversation_item(
    ...
    without starting or steering the placeholder Codify agent.
...
def _drive_terminal_resolved_elicitation(session_id: str, persisted: ConversationItem) -> None:
    ...
    :param session_id: Codify conversation id the item was mirrored for,
        e.g. ``"conv_abc123"``.
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    :param tunnel_registry: Runner-tunnel registry used to await the
        ...
        wirings (the rendezvous then settles at frame-send).
    """
    from codify.server.managed_hosts import terminate_managed_host
...
async def _maybe_relaunch_managed_sandbox(
    *,
    ...
    :raises CodifyError: 503 when the relaunch failed or timed out.
    """
...
def _kick_managed_relaunch(
    *,
    ...
    """
    from codify.server.managed_hosts import MANAGED_REPO_LABEL_KEY, parse_repo_workspace
...
async def _run_managed_wake(
    *,
    ...
    """
    from codify.server.managed_hosts import resume_managed_host
    from codify.server.routes import sessions as _facade
...
def _is_native_terminal_session(conv: Conversation) -> bool:
    ...
    True for both a built-in terminal-first wrapper (``codify.wrapper``
...
def _native_terminal_runtime(conv: Conversation) -> tuple[str, str, str]:
    ...
    :raises CodifyError: If the session is not a native terminal harness.
    """
...
async def _persist_host_launch_failure_turn(
    ...
    *,
    ...
    (which names the fix, ``codify setup``), then publishes the same
    ...
    :param host_error: The host's human-readable refusal, e.g.
        ...
        `codify setup` ..."``. ``None`` falls back to a generic
        ``codify setup`` pointer so the banner is never empty.
    ...
    error = ErrorData(
        ...
        message=(
            ...
            # third-party host omits it.
                ...
                "the agent's harness is not configured on the selected host — run `codify setup`"
            )
        ),
    )
    turn_id = generate_task_id()
...
async def _forward_native_subagent_terminal_failure(
    ...
    :returns: None.
    :raises CodifyError: If the parent's runner could not be reached
...
def _build_native_terminal_message_event(
    ...
    :raises CodifyError: If the event is not a user message.
    """
...
async def _forward_native_terminal_message(
    ...
    """
    Forward one Codify web-chat message to the native terminal harness.
    ...
    if (
        ...
        from codify.runtime.content_resolver import (
...
async def _forward_event_to_runner(
    ...
    if conv.harness_override == "auto" and body.type == "message":
        from codify.server.smart_routing import route_session_harness
    ...
    if _should_route:
        ...
        if _user_text:
            if _parent_routing_on:
                ...
                from codify.server.smart_routing import route_turn
...
async def _dispatch_session_event_to_runner_impl(
    ...
    * **transcript-forwarded native + ``type == "message"``**: web-chat user
      ...
      server. The Codify would otherwise persist an AP-side copy AND
    ...
    * **All other cases**: persist the item AP-side (invariant I1:
    ...
    :mod:`codify.runtime.pending_inputs`). It is rolled back if the
    ...
    :param created_by: Authenticated identity of the posting actor,
        ...
        :func:`codify.runtime.pending_inputs.record` and applied
        ...
        async with runner_client.stream(
            ...
            async for chunk in resp.aiter_text():
                ...
                while "\n\n" in buffer:
                    ...
                    # (the Codify subscriber generates its own heartbeats).
...
async def _ensure_runner_relay_ready_impl(
    ...
    The runner stream has no replay buffer. For item events, Codify must
    subscribe to runner output before it forwards the input event; a
    fast harness can otherwise complete before Codify is listening, leaving
    ...
    :returns: The active relay handle, or ``None`` when no runner is
        bound.
    :raises CodifyError: If the relay cannot observe the
...
async def _evaluate_input_policy(
    ...
    *,
    ...
    if has_content_blocks and file_store is not None and artifact_store is not None:
        from codify.runtime.content_resolver import extract_text_attachments
...
async def _wake_parent_for_blocked_child(
    ...
    *outcome* is reported back so the notifier can release its per-block
    ...
    except (httpx.HTTPError, CodifyError):
        _logger.warning(
...
def configure_subagent_block_notifier(
    ...
    :mod:`codify.runtime.pending_elicitations` so a sub-agent that
    ...
    """
    from codify.runtime import pending_elicitations as _pending_elicitations
    from codify.runtime.subagent_block_notifier import SubagentBlockNotifier
...
def _native_subagent_wrapper_labels(
    *,
    ...
    The pill is gated on the conversation's ``codify.wrapper`` +
    ``codify.ui`` labels (see ``web`` ``TerminalFirstContext``), but
...
async def _create_session_from_existing_agent(
    ...
    :raises CodifyError: 404 if no agent matches ``body.agent_id``;
    ...
    # ``agent``/``model`` args. Only applied to codify-executor agents
    ...
    if body.parent_session_id is not None:
        ...
        if (
            ...
            try:
                ...
                # Non-codify agent (e.g. a native wrapper) — can't route
                # harness; leave the orchestrator's choice untouched.
                _force_auto_for_child = False

    # Validated against the loaded spec (known harness + codify
    ...
    try:
        ...
        # NOT git_branch: only a worktree Codify created here may be
    ...
    try:
        ...
        if not _host_opted_out:
            ...
            _client_header = request.headers.get("x-codify-client")
            _surface = (
                _client_header
...
def _create_session_from_bundle(
    ...
    reusing a template by name would make a fresh ``codify run
    ...
    :raises CodifyError: If bundle validation or agent insert
    ...
    # ``codify run`` uploads the operator's own bundle through this same
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    if namespaced_name in ("sys_advise_models", "mcp__codify__sys_advise_models"):
    ...
    # tunnel the runner already opened to the Codify server at startup.
    ...
    try:
        from codify.runner.tool_dispatch import MCP_PROXY_FORWARD_TIMEOUT_S
...
async def _fetch_model_options(
    ...
    * **claude-native** — the provider-neutral aliases from the exact launch
    ...
    if wrapper == _PI_NATIVE_WRAPPER_LABEL_VALUE:
        ...
        # actually loaded regardless of auth path (Codify provider OR pi's
...
async def _get_session_snapshot(
    ...
    :raises CodifyError: 404 if no session exists, 500 if the
