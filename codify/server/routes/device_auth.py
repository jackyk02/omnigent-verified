...
``CODIFY_DEVICE_GRANT_ENABLED`` is set). OIDC deployments delegate login
...
Optionally, setting ``CODIFY_DEVICE_CLIENT_SECRET`` on the server gates
...
# (authorize / token / revoke). When ``CODIFY_DEVICE_CLIENT_SECRET`` is
...
_CLIENT_SECRET_ENV = "CODIFY_DEVICE_CLIENT_SECRET"
_CLIENT_SECRET_HEADER = "X-Codify-Client-Secret"
...
def mint_delegated_token(
    ...
    *,
    ...
    Same HS256 shape as
    :func:`codify.server.oidc.mint_session_token` (so
    ...
    :param user_id: The Codify identity the token acts as (``sub``).
...
def create_device_auth_router(
    ...
    async def device_consent_page(request: Request) -> Response:
        ...
        Codify identity being delegated and the requesting ``client_id``
    ...
    async def device_approve(request: Request) -> Response:
        ...
        requires the browser identity; binds the approving Codify
    ...
    async def revoke(request: Request) -> Response:
        ...
        Backs ``/codify logout``. Accepts a ``refresh_token`` form
    ...
    *,
    ...
    if error:
        ...
        body = (
            ...
            f"<b>{esc(user_id)}</b> on this Codify server.</p>"
    ...
    return (
        ...
        "<title>Authorize access — Codify</title><style>"
