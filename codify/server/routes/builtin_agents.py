...
registered at startup with ``codify server --agent``. They are the
...
from codify.server.routes._auth_helpers import require_user as _require_user
from codify.server.schemas import AgentObject, MCPServerSummary, PaginatedList, SkillSummary
from codify.stores import AgentStore
...
def _to_agent_object(agent: Agent, agent_cache: AgentCache) -> AgentObject:
    ...
    return AgentObject(
        ...
        # by a same-named ``codify run`` upload, but lets a newer
