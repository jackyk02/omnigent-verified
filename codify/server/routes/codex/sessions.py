...
from codify.harness_plugins import CODEX_NATIVE_CODING_AGENT
...
from codify.server.routes._auth_helpers import get_user_id as _get_user_id
from codify.server.routes._auth_helpers import require_access as _require_access
...
def _require_codex_goal_runner_payload(
    ...
    *,
    ...
    back to persisted Codify labels. A missing runner, rejected control
...
async def _start_codex_goal_runner_on_bound_host(
    *,
    ...
    :returns: Runner id expected to connect, or ``None`` if no launch was
        possible.
    :raises CodifyError: If the host reports a non-retryable harness
...
async def _initialize_codex_goal_runner(
    ...
    :param conversation_store: Store used to reload the post-relaunch
        conversation row.
    :returns: None.
    :raises CodifyError: If the session disappeared before init.
    """
...
async def _launch_runner_for_codex_goal(
    *,
    ...
    :returns: Connected runner client, or ``None`` when no runner can be
        launched or reached.
    :raises CodifyError: If the caller lacks edit access or the host
...
async def _require_codex_native_goal_session(
    ...
    :raises CodifyError: 404 when the session is missing, or 400 when it is
    ...
    *,
    ...
    async def get_codex_goal(
        ...
        :returns: Current Codex goal state, or ``goal=None`` when no goal is
            set.
        :raises CodifyError: 400 for non-Codex sessions, 404 for missing
    ...
    async def set_codex_goal(
        ...
        :raises CodifyError: 400 for non-Codex sessions or blank
    ...
    async def update_codex_goal_status(
        ...
        field. The Codify API keeps that distinct from objective/budget
