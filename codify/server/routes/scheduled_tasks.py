...
from codify.entities import ScheduledTask, ScheduledTaskRun
...
from codify.server.routes._host_launch import resolve_host_owner
from codify.server.routes._session_create_validation import (
    validate_existing_host_workspace,
    validate_session_agent,
    validate_session_model_metadata,
)
from codify.server.scheduled.rrule import RRuleValidationError, validate_rrule
from codify.server.scheduled.run_reconciler import force_fail_stale_runs
from codify.stores import AgentStore, ConversationStore, PermissionStore
...
def _validate_rrule_or_400(rrule: str) -> None:
    """Raise a 400 ``CodifyError`` if the RRULE is invalid."""
    try:
        validate_rrule(rrule)
    except RRuleValidationError as exc:
        raise CodifyError(f"invalid rrule: {exc}", code=ErrorCode.INVALID_INPUT) from exc


def _validate_timezone_or_400(timezone: str) -> None:
    """Raise a 400 ``CodifyError`` if *timezone* is not a valid IANA timezone."""
    try:
        ZoneInfo(timezone)
    ...
    *,
    ...
    def _require_owned(scheduled_task_id: str, owner: str | None) -> ScheduledTask:
        ...
        if task is None or task.user_id != owner:
            raise CodifyError("Scheduled task not found", code=ErrorCode.NOT_FOUND)
        return task
