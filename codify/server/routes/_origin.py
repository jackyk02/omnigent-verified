...
The JSON Content-Type guard in :mod:`codify.server.routes._content_type`
...
enforces (:func:`codify.server.ws_origin.origin_allowed`) so HTTP and
...
- the first-party sentinel (:data:`CODIFY_INTERNAL_WS_ORIGIN`,
  ``"codify://internal"``) and any origin in
  ``CODIFY_WS_ALLOWED_ORIGINS`` pass;
...
from codify.server.ws_origin import origin_allowed, parse_allowed_origins
...
def require_trusted_origin(request: Request) -> None:
    ...
    may announce themselves with ``Origin: codify://internal``
    (:data:`CODIFY_INTERNAL_WS_ORIGIN`), but are not required to.
