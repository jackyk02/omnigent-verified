...
from codify.server.host_registry import HostConnection, HostRegistry
from codify.server.routes._auth_helpers import require_user
from codify.server.routes._host_launch import resolve_host_launch
from codify.server.schemas import SessionGitOptions
from codify.stores import AgentStore, ConversationStore
from codify.stores.host_store import HostStore, host_is_live
...
HARNESS_INSTALL_ENABLED_ENV = "CODIFY_HARNESS_INSTALL_ENABLED"
...
def create_hosts_router(
    ...
    *,
    ...
    :param agent_store: Agent store used to resolve a session's agent
        ...
        :func:`codify.server.app.create_app` always supplies it.
    ...
    async def launch_runner(
        ...
        if agent_store is not None and agent_cache is not None:
            from codify.server.routes._workspace_validation import (
        ...
        # CreatedWorktree | None — set ONLY when Codify creates a worktree
        ...
        if result.get("status") == "failed":
            ...
            if result.get("error_code") == HARNESS_NOT_CONFIGURED_ERROR_CODE:
                ...
                # (`codify setup` on the host machine). Surface the
    ...
    async def install_host_harness(
        ...
        ``CODIFY_HARNESS_INSTALL_ENABLED`` (default off): when disabled it
    ...
    async def store_host_harness_credential(
        ...
        ``CODIFY_HARNESS_INSTALL_ENABLED`` exactly like the install route
        (404 when disabled). The host daemon does the write with the same
        non-interactive core the ``codify setup`` wizard uses.
