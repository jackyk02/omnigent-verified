...
from codify.entities import Policy
from codify.errors import CodifyError, ErrorCode
from codify.policies.registry import (
    get_entry,
    is_registered_handler,
    validate_factory_params,
)
from codify.runtime import get_caps
from codify.runtime.policies.builder import invalidate_session_policy_specs_cache
from codify.server.auth import LEVEL_EDIT, LEVEL_READ, AuthProvider
from codify.server.routes._auth_helpers import get_user_id, require_access
from codify.server.routes._errors import session_not_found
from codify.server.schemas import (
    _DOTTED_PATH_RE,
    CreateSessionPolicyRequest,
    UpdateSessionPolicyRequest,
)
from codify.spec.types import FunctionPolicySpec, PolicySpec
...
from codify.telemetry.events import PolicyDeletedEvent as _TelPolicyDeletedEvent
from codify.telemetry.events import PolicyRegisteredEvent as _TelPolicyRegisteredEvent
...
def create_session_policies_router(
    ...
    def _require_session_exists(session_id: str) -> None:
        ...
        :param session_id: The session to check, e.g.
            ``"conv_abc123"``.
        :raises CodifyError: 404 if the session is not found.
        """
    ...
    async def create_policy(
        ...
        :raises CodifyError: 401/403 if the user lacks edit
        ...
        if body.type == "python":
            ...
            if validation_error:
                raise CodifyError(validation_error, code=ErrorCode.INVALID_INPUT)
    ...
    async def list_policies(
        ...
        :raises CodifyError: 401/403 if the user lacks read
    ...
    async def get_policy(
        ...
        if policy is None:
            raise CodifyError("Policy not found", code=ErrorCode.NOT_FOUND)
        return _entity_to_response(policy)
