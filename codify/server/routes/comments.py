...
from codify.db.enum_codecs import COMMENT_STATUS
from codify.entities import Comment
...
def create_comments_router(
    ...
    async def _require_session_access(user_id: str | None, session_id: str, level: int) -> None:
        ...
        :raises CodifyError: 404 when the session does not exist, or the
    ...
    async def _require_comment_author(
        ...
        :raises CodifyError: 404 if the comment is not found in this
            session; 403 if the caller is not the comment's author.
        """
        comment = await asyncio.to_thread(store.get, comment_id, session_id)
        if comment is None:
            raise CodifyError("Comment not found", code=ErrorCode.NOT_FOUND)
    ...
    async def add_comment(
        ...
        :raises CodifyError: 401/403/404 if the user lacks edit permission.
        """
    ...
    async def list_comments(
        ...
        :raises CodifyError: 401/403/404 if the user lacks read permission.
        """
        user_id = get_user_id(request, auth_provider)
    ...
    async def send_to_agent(
        ...
        def _fetch_and_mark() -> list[Comment]:
            ...
            for cid in body.comment_ids:
                ...
                if comment is None:
                    raise CodifyError(f"Comment not found: {cid}", code=ErrorCode.NOT_FOUND)
