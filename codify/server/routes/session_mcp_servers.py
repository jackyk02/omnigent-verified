...
from codify.entities import Agent
...
from codify.server.auth import LEVEL_EDIT, LEVEL_READ, AuthProvider, local_single_user_enabled
...
from codify.spec import AgentSpec, extract_safe
from codify.spec.types import MCPServerConfig
...
def create_session_mcp_servers_router(
    ...
    async def _bound_agent(
        ...
        if conv.agent_id is None:
            raise CodifyError("Session has no agent binding", code=ErrorCode.INVALID_INPUT)
        agent = await asyncio.to_thread(agent_store.get, conv.agent_id)
        if agent is None:
            raise CodifyError("Agent not found", code=ErrorCode.NOT_FOUND)
        return agent
    ...
    async def list_mcp_servers(request: Request, session_id: str) -> dict[str, Any]:
        ...
        if agent_cache is None:
            raise CodifyError("Agent cache not configured", code=ErrorCode.INTERNAL_ERROR)
    ...
    def _mutate_bundle(
        ...
        *,
        ...
        if bundle_bytes is None:
            raise CodifyError("Agent bundle not found", code=ErrorCode.INTERNAL_ERROR)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if mode == "create":
                ...
                if location is None:
                    raise CodifyError("MCP server not found", code=ErrorCode.NOT_FOUND)
...
def _summary_from_spec(spec: AgentSpec, name: str) -> MCPServerSummary:
    ...
    raise CodifyError("MCP server was not saved", code=ErrorCode.INTERNAL_ERROR)
