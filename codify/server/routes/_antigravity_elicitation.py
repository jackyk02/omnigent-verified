...
* :func:`to_elicitation_params` — converts a :class:`PendingInteraction`
  dict (produced by :func:`codify.antigravity_native_steps.pending_interaction`)
  into an :class:`~codify.server.schemas.ElicitationRequestParams` that
  the web UI can render.

* :func:`to_interaction_payload` — converts the user's
  :class:`~codify.server.schemas.ElicitationResult` back into the
...
def to_tui_selection_keys(
    ...
    the pane via
    :func:`codify.antigravity_native_bridge.send_interaction_keys_via_tui`,
