...
Centralizes the construction of common ``CodifyError`` instances so
...
def session_not_found() -> CodifyError:
    ...
    :returns: A fresh :class:`CodifyError` with message
        ``"Session not found"`` and code :attr:`ErrorCode.NOT_FOUND`.
    """
    return CodifyError(_SESSION_NOT_FOUND, code=ErrorCode.NOT_FOUND)
