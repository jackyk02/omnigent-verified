...
- **In-process runner**: the server passes
  :func:`codify.runtime.get_terminal_registry` into
  :func:`codify.runner.create_runner_app`, so the registry is
...
- **Out-of-process runner** over the WebSocket tunnel: the runner
  ...
  (``codify/server/_runner_ws_tunnel.py``), and the runner's
  resource-addressed WS route runs ``tmux attach`` and bridges the
  PTY.

The proxy uses a factory configured via
:func:`codify.runtime.set_runner_ws_factory` in the server
...
from codify.server.auth import LEVEL_OWNER, LEVEL_READ, AuthProvider
from codify.server.routes._auth_helpers import require_access
...
def create_terminal_attach_router(
    *,
    ...
    :func:`codify.server.app.create_app`. The list/CRUD endpoints
