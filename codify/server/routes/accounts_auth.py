...
Mounted at ``/auth`` when ``CODIFY_AUTH_PROVIDER=accounts``.
...
from codify.server.auth import _RESERVED_USERS, RESERVED_USER_LOCAL, UnifiedAuthProvider
...
def create_accounts_auth_router(
    ...
    async def setup(body: SetupRequest) -> Response:
        ...
        that pre-seeds ``CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD`` also
        ...
        # `codify run` needs a CLI token too, not just the browser cookie.
