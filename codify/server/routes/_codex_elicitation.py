...
from codify.codex_native_elicitation import is_codex_request_id
...
class _CodexElicitationAdapter:
    ...
    :param build_response: Converts an Codify web verdict into Codex's
...
class CodexElicitationRequest:
    """
    Validated Codex request expressed in Codify elicitation terms.
...
def parse_codex_elicitation_request(payload: dict[str, Any]) -> CodexElicitationRequest:
    """
    Validate a Codex request envelope and build its Codify adapter object.

    :param payload: Codex JSON-RPC request envelope, e.g.
        ``{"id": 1, "method": "mcpServer/elicitation/request",
        "params": {...}}``.
    :returns: Validated request metadata plus Codify elicitation params.
    :raises CodifyError: If the request shape is unsupported or
...
def _execpolicy_amendment(value: Any) -> list[str] | None:
    ...
    :raises CodifyError: If ``value`` is present but not a
...
def _result_execpolicy_amendment(
    ...
    """
    Extract a user-selected execpolicy amendment from Codify content.
...
def _codex_mcp_elicitation_params(
    ...
    """
    Build Codify params for Codex ``mcpServer/elicitation/request``.
    ...
    :raises CodifyError: If required MCP fields are malformed.
    """
...
def _codex_tool_request_user_input_params(
    ...
    """
    Build Codify params for Codex ``item/tool/requestUserInput``.
    ...
    :returns: AP/MCP-shaped elicitation params.
    :raises CodifyError: If no usable questions are present.
    """
...
def _codex_command_approval_params(
    ...
    """
    Build Codify params for Codex command approval requests.
...
def _codex_file_change_approval_params(
    ...
    """
    Build Codify params for Codex file-change approval requests.
...
def _codex_permissions_approval_params(
    ...
    """
    Build Codify params for Codex permission-profile approval requests.
...
def _codex_apply_patch_approval_params(
    ...
    """
    Build Codify params for legacy Codex patch approval requests.
