...
:mod:`codify.server.auth` and
:mod:`codify.server.permissions` that handle the ``None``
...
from codify.server.permissions import (
...
def require_user(
    ...
    :param auth_provider: The auth provider, or ``None`` to skip auth.
    :returns: User ID string, or ``None`` if no auth provider.
    :raises CodifyError: 401 if the provider returns ``None``.
    """
    if auth_provider is None:
        return None
...
def _require_access_sync(
    ...
    :raises CodifyError: 403 if insufficient level, 404 if no
        access at all.
    """
...
def _require_access_and_level_sync(
    ...
    :raises CodifyError: 401 unauthenticated, 403 insufficient level,
...
async def require_access_and_level(
    ...
    :raises CodifyError: 401 / 403 / 404 as documented on the sync core.
    """
    return await asyncio.to_thread(
