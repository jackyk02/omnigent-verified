...
Hosts (machines running ``codify host``) connect here via
...
from codify.host.identity import MANAGED_HOST_TOKEN_HEADER
...
from codify.server.host_registry import (
    HostConnection,
    HostRegistry,
    RunnerExitReports,
)
...
def create_host_tunnel_router(
    ...
    *,
    ...
    :param local_single_user: When ``True``, allow a host to re-own a
        ...
        ``None`` (the default) resolves from ``CODIFY_LOCAL_SINGLE_USER``
...
async def _ping_loop(
    ...
    (:data:`codify.stores.host_store.HOST_LIVENESS_TTL_S`): when a
