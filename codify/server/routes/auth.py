...
/ ``cli-poll`` pair supports the ``codify login`` CLI command.

See ``designs/OIDC_AUTH.md`` for the complete design.

These routes are only mounted when ``CODIFY_AUTH_PROVIDER=oidc``.
"""
...
from codify.server.admin_list import AdminList, promote_if_listed
...
from codify.server.oidc_access import OidcAdmissionPolicy, resolve_allowed_domains_path
...
def create_auth_router(
    ...
    :param admin_list: File-backed admin roster. Consulted on each
        ...
        :mod:`codify.server.admin_list`). OIDC's only admin signal.
    :param account_store: Invite-token persistence, required only when
        ``CODIFY_OIDC_ALLOW_INVITES`` is on. ``None`` disables the
    ...
    :param allowed_domains: Domains from the server config's
        ...
        ``CODIFY_OIDC_ALLOWED_DOMAINS`` and the runtime-editable file
    ...
    # decides who may sign in — see codify/server/oidc_access.py.
    ...
    async def logout() -> Response:
        ...
        If ``CODIFY_OIDC_LOGOUT_REDIRECT_URI`` is configured,
...
def _resolve_oidc_email(
    ...
    *not* prove the user controls the email address. Without the
    ...
    ``CODIFY_OIDC_SKIP_EMAIL_VERIFICATION``) waives the gate for
    IdPs that omit the claim for directory-managed users (e.g. Okta
    without custom API Access Management).

    ``config.email_claim`` (from ``CODIFY_OIDC_EMAIL_CLAIM``) names
    ...
    if not isinstance(email, str) or not email.strip():
        _logger.warning(
            ...
            "can set CODIFY_OIDC_EMAIL_CLAIM.",
            config.email_claim,
            sorted(claims.keys()),
        )
        return None
    ...
    if config.email_claim != "email":
        if config.skip_email_verification:
            _logger.info(
                ...
                "marker (CODIFY_OIDC_SKIP_EMAIL_VERIFICATION is set)",
                config.email_claim,
                email,
            )
            return email
        _logger.warning(
            ...
            "CODIFY_OIDC_SKIP_EMAIL_VERIFICATION to accept it",
            config.email_claim,
            email,
        )
        return None
    ...
    # operator opted out (CODIFY_OIDC_SKIP_EMAIL_VERIFICATION) for
    ...
    if not _claim_is_verified_true(claims.get("email_verified")):
        if config.skip_email_verification:
            _logger.info(
                ...
                "(CODIFY_OIDC_SKIP_EMAIL_VERIFICATION is set)",
                email,
            )
            return email
        _logger.warning(
