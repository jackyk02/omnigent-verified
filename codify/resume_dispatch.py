"""Top-level ``codify resume`` dispatch.
...
- ``codify resume <conv_id>`` — fetch the conversation, read its
  ``codify.wrapper`` label, and dispatch by runtime.
- ``codify resume`` (no id) — open the cross-agent picker (see
  :func:`codify.repl._resume_picker.pick_conversation_cross_agent_from_sdk`)
...
``codify run --resume`` invocation — the agentless ``run
...
def run_resume(
    *,
    ...
    leak state and collide with any other in-flight ``codify``
    process the user has running.

    :param target: Optional conversation id, e.g. ``"conv_abc123"``.
        ``None`` selects the picker form.
    :param server: Optional remote Codify server URL. Required in the
    ...
    if target is None:
        if server is None:
            raise click.UsageError(
                "`codify resume` (no id) requires `--server <url>`. "
                "Pass a conversation id (`codify resume conv_...`) "
...
def _pick_conversation_for_resume(
    *,
    ...
    from codify.repl._resume_picker import pick_conversation_cross_agent_from_sdk
    ...
    async def _drive() -> str | None:
        ...
        :returns: Selected conversation id, ``None`` on cancel /
            empty list.
        """
        # Deferred import: ``codify_client`` carries the full SDK
...
def _dispatch_by_runtime(
    *,
    ...
    the existing ``codify run --resume`` invocation — the
    ...
    :param server: Optional remote Codify server URL. ``None`` when
    ...
    """
    from codify.db.db_models import InvalidUuidError, uuid_to_bytes
    ...
    if server is not None:
        ...
        raise click.ClickException(
            ...
            f"`codify run --resume {target} <agent.yaml> --server "
    ...
    raise click.ClickException(
        f"Conversation {target!r} is not a terminal-native session "
        f"(wrapper={wrapper!r}). To resume it, run "
        f"`codify run --resume {target} <agent.yaml>`. "
...
def _dispatch_wrapper(
    *,
    ...
    :param wrapper: Value from ``labels.codify.wrapper``.
    :param server: Codify server base URL without trailing slash, or
...
def _read_wrapper_label_local(*, conv_id: str) -> str | None:
    ...
    :param conv_id: Local Codify conversation id, e.g. ``"conv_abc123"``.
    :returns: Value of ``labels.codify.wrapper``, or ``None`` when
        no wrapper label is present.
    :raises click.ClickException: If the conversation id is not found
        in the local persistent store.
    """
    from codify.chat import _codify_persistent_dir
    from codify.stores.conversation_store.sqlalchemy_store import (
        SqlAlchemyConversationStore,
    )

    db_path = _codify_persistent_dir() / "chat.db"
    ...
    if conversation is None:
        raise click.ClickException(
            ...
            "Pass --server if the conversation lives on a remote Codify server.",
        )
...
def _read_wrapper_label_remote(
    *,
    ...
    :param server: Remote Codify server URL.
    :param conv_id: Codify conversation id.
    :returns: Wrapper label string, or ``None`` when no
        ``codify.wrapper`` label is present on the row.
