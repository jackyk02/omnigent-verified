...
(``codify run --server``, ``codify claude --server``)
share two needs:
...
# log collectors.
_NO_SPINNER_ENV_VAR = "CODIFY_NO_SPINNER"
...
def _spinner_enabled(stream_isatty: bool, env: dict[str, str]) -> bool:
    ...
    :param env: Environment snapshot, e.g. ``dict(os.environ)``.
        ``CODIFY_NO_SPINNER`` (any truthy value) force-disables
        the spinner.
...
def runner_startup_progress(
    *,
    ...
    On a TTY (and absent ``CODIFY_NO_SPINNER``) a rich spinner
    ...
    Typical use lives inside :func:`codify.chat._wait_for_remote_runner`,
    ...
    :param enabled: Force the renderer choice. ``None`` (default)
        ...
        ``CODIFY_NO_SPINNER``. ``True`` always renders the
    ...
    if enabled:
        ...
        # one-shot output (``codify run … -p "…"``) stays clean.
    ...
    click.echo(f"codify: {initial_message}", err=True)
    ...
    def _update_plain(msg: str) -> None:
        ...
        :returns: None.
        """
        click.echo(f"codify: {msg}", err=True)
...
def format_runner_log_tail(log_path: Path | None) -> str:
    ...
    so the call sites in ``codify.chat`` and
    ``codify.cli`` keep working unchanged.

    :param log_path: Path to the captured runner log, e.g.
        ``Path("/home/u/.codify/logs/runner/runner-abcd.log")``.
    ...
    :returns: A formatted block ready to append to an exception
        message, e.g.::
            ...
            \nRunner log: /home/u/.codify/logs/runner/runner-abcd.log
