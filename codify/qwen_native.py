"""Native qwen TUI wrapper for the Codify CLI.

``codify qwen`` launches Qwen Code's interactive TUI (``qwen``) inside an
Codify-runner-owned tmux terminal and attaches the local TTY — the qwen analog
of ``codify goose`` / ``codify cursor``. The runner spawns the process (see
:func:`codify.runner.app._auto_create_qwen_terminal`), pointing qwen at the
...
``/auth`` command persisted under ``~/.qwen``); no Codify-managed key is
...
from codify._wrapper_labels import QWEN_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_QWEN_PATH_ENV = "CODIFY_QWEN_PATH"
...
def run_qwen_native(
    *,
    ...
    """
    Launch the qwen TUI in an Codify terminal.
    ...
    if server is None:
        raise click.ClickException(
            "qwen requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_qwen_native."
        )
    with TemporaryDirectory(prefix="codify-qwen-native-") as tmpdir:
...
def _materialize_qwen_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify qwen``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch qwen on an Codify server via a daemon-spawned runner.
