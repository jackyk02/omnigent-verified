"""
In-process callable tools for the codify-style spec adapter.

The codify YAML translator stores user-declared function
tools as :class:`codify.spec.types.LocalToolInfo` entries
with ``language == "codify-python-callable"`` and ``path``
holding a dotted-import string (e.g.
``"examples._shared.tool_functions.calculate"``). These are
distinct from native Codify local tools, which live as
``tools/python/*.py`` files in the agent bundle and execute via
:class:`codify.tools.local.LocalPythonTool` in a subprocess.
...
- :func:`load_local_callable_tools` — helper that
  :class:`codify.tools.manager.ToolManager` calls alongside
  :func:`codify.tools.local.load_local_python_tools` to
  pick up the codify-translated subset.
...
when the codify compat path retires (the legacy executor
...
from codify.spec.types import LocalToolInfo
from codify.tools.base import Tool, ToolContext

_logger = logging.getLogger(__name__)

# Match the language constant used by the codify YAML
# translator (codify/spec/codify.py:CODIFY_TOOL_LANGUAGE).
# Duplicated here because importing from spec/codify.py would
...
class LocalCallableTool(Tool):
    ...
    :attr:`codify.spec.types.LocalToolInfo.path` and is
    ...
    def dotted_path(self) -> str:
        ...
        Used by :func:`codify.runtime.workflow._dispatch_local_callable_tool_async`
    ...
    def description(cls) -> str:
        ...
        :returns: Generic class-level description. Per-instance
            ...
            :meth:`get_schema`.
        """
        return "User-declared function tool from an codify-style YAML."
    ...
    def get_schema(self) -> dict[str, Any]:
        ...
        used (matching what the codify harness advertises
...
def load_local_callable_tools(
    ...
    """
    Build :class:`LocalCallableTool` instances for codify tools.

    Filters *local_tools* to the ones the codify YAML
    translator produced — entries whose ``language`` is
    ``"codify-python-callable"``. Other entries (native AP
    file-based tools with ``language == "python"``) are
    handled by :func:`codify.tools.local.load_local_python_tools`.

    :param local_tools: All :class:`LocalToolInfo` entries from
        the agent spec.
    :returns: One :class:`LocalCallableTool` per codify-style
        entry. Empty list when none are present (which is the
        case for native Codify specs).
    """
    from codify.spec.types import ToolRuntime

    result: list[LocalCallableTool] = []
    for info in local_tools:
        if info.language != _CODIFY_CALLABLE_LANGUAGE:
            continue
...
def _stringify(value: Any) -> str:
    ...
    The legacy codify path stringifies tool results the same
