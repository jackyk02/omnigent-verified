...
from codify.inner.os_env import OSEnvironment
from codify.runtime import get_caps
from codify.spec import AgentSpec
from codify.spec.types import SharePolicy, ToolRuntime
from codify.tools._srt import is_srt_available
from codify.tools.base import Tool, ToolContext, is_valid_tool_name
from codify.tools.builtins import (
    ListCommentsTool,
    LoadSkillTool,
    ReadSkillFileTool,
    SysAdviseModelsTool,
...
from codify.tools.client_specified import ClientSideTool, ClientSideToolSpec
from codify.tools.local import load_local_python_tools
...
class ToolManager:
    ...
    def _register_policy_tools(self) -> None:
        ...
        the Codify server's REST endpoints.
        """
    ...
    def _register_scheduled_task_tools(self) -> None:
        ...
        runner dispatches all four via the Codify server's
    ...
    def _register_timer_tools(self) -> None:
        ...
        match ``codify/inner/datamodel.py::AgentDef.timers``).
    ...
    def _create_builtin(
        ...
        if name == "upload_file":
            from codify.tools.builtins.upload_file import UploadFileTool
    ...
    def _create_web_fetch(self) -> Tool:
        ...
        """
        from codify.tools.builtins.web_fetch import WebFetchTool
    ...
    def _register_task_lifecycle_tools(self) -> None:
        ...
        """
        from codify.tools.builtins.async_inbox import SysCancelTaskTool
    ...
    def _register_browser_tools(self) -> None:
        ...
        dispatch branch (``codify/runner/tool_dispatch.py``), which
        needs the runner's ``server_client`` that ``ToolContext`` does
        not carry.
        """
        from codify.tools.builtins.browser import (
            BrowserClickTool,
            BrowserNavigateTool,
    ...
    def _register_terminal_tools(self) -> None:
        ...
        Per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.3, the five
        ...
        :func:`codify.runtime.get_terminal_registry`) so state
    ...
    def _register_local_tools(self, workdir: Path | None) -> None:
        ...
        for client_info in client_local_tools:
            ...
            # The validator (see ``codify/spec/validator.py``)
        ...
        # Native Codify local tools (``language == "python"``) need a
        # workdir on disk so the subprocess loader can locate the
        # ``tools/python/*.py`` files. Codify-style tools
        # (``language == "codify-python-callable"``) come
        ...
        if workdir is not None:
            for tool in load_local_python_tools(
                ...
                if not is_valid_tool_name(tool.name()):
                    ...
                    continue
                self._tools[tool.name()] = tool
        # Codify-style callable tools — sibling loader for the
        # ``codify-python-callable`` language entries the YAML
        # translator emits. See
        # :mod:`codify.tools.local_callable` for the reasoning;
        ...
        from codify.tools.local_callable import load_local_callable_tools

        for callable_tool in load_local_callable_tools(server_local_tools):
            if not is_valid_tool_name(callable_tool.name()):
                _logger.warning(
                    "Codify callable tool %r has invalid name — skipping",
                    callable_tool.name(),
                )
                continue
            if callable_tool.name() in self._tools:
                ...
                raise ValueError(
                    f"codify callable tool {callable_tool.name()!r} "
                    f"collides with an already-registered tool"
                )
    ...
    def _register_client_tools(
        ...
        Raises :class:`CodifyError` if a tool name violates the
        ...
        :param specs: List of :class:`ClientSideToolSpec` objects to
            register, e.g.
            ``[ClientSideToolSpec(name="get_weather", ...)]``.
        :raises CodifyError: If any tool name is invalid.
        """
        for spec in specs:
