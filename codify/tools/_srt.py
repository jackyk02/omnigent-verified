...
``codify/inner/mcp_tools.py`` (which has never sandboxed
stdio MCPs). Future per-MCP sandboxing — if reintroduced —
should flow through the ``codify/environments/`` primitive
with explicit outbound-host allowlists, not srt-defaults.

The PTY-mode wrap used by :class:`~codify.terminals.shell.Shell`
