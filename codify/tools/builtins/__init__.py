...
from codify.tools.base import Tool
from codify.tools.builtins.advise_models import SysAdviseModelsTool
...
from codify.tools.builtins.list_models import SysListModelsTool
...
from codify.tools.builtins.read_skill_file import (
    ReadSkillFileTool,
)
from codify.tools.builtins.scheduled_tasks import (
...
def _create_search_conversations(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.search_conversations import (
...
def _create_list_files(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.list_files import ListFilesTool
...
def _create_export_agent(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.export_agent import ExportAgentTool
...
def _create_hindsight_retain(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.hindsight import HindsightRetainTool
...
def _create_hindsight_recall(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.hindsight import HindsightRecallTool
...
def _create_hindsight_reflect(config: dict[str, str]) -> Tool:
    ...
    """
    from codify.tools.builtins.hindsight import HindsightReflectTool
...
# per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §3a + §6.2. Their
...
_BUILTIN_REGISTRY: dict[str, _BuiltinFactory | None] = {
    ...
    # via the tool registry — see codify/runtime/policies/approval.py.)
    "web_fetch": None,
    "list_comments": None,
    "update_comment": None,
    ...
    # (``_BROWSER_TOOLS`` in codify/runner/tool_dispatch.py).
