...
``codify/runner/tool_dispatch.py::_execute_web_fetch_tool``) which
...
# spawned — the runner aborts with ``unknown harness 'codify'``.
_UNBOOTABLE_DEFAULT_HARNESS: str = "codify"
...
def _ensure_default_sandbox_runnable() -> None:
    ...
    ``codify.inner.sandbox._default_sandbox_for_platform``) without
    ...
    :raises CodifyError: On Linux when ``bwrap`` is not on ``PATH``,
...
def build_researcher_spec(parent_spec: AgentSpec) -> AgentSpec:
    ...
    - The parent executor's harness (``config``), ``auth``, ``model``, and
      ...
      parent's provider. Without this the child defaults to ``type="codify"``
      with no harness, which the runner rejects as ``unknown harness
      'codify'`` before any model routing (Layer 1), and even past that
    ...
    - An ``os_env`` block — registers ``sys_os_shell`` for one-shot
      ...
      per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §3a in favor of
    ...
    - The parent's ``os_env.sandbox`` (filesystem grants, egress
      ...
      ``codify/inner/os_env.py::create_os_environment``). If the
    ...
    :raises CodifyError: If the parent declares no bootable harness, so the
        researcher would spawn the unspawnable literal harness ``"codify"``;
    ...
    # ``type="codify"`` harness and strips the parent's provider. Drop
    ...
    if child_executor.harness_kind == _UNBOOTABLE_DEFAULT_HARNESS:
        raise CodifyError(
            ...
            f"unknown harness 'codify'. Set executor.config.harness on the parent "
