...
:class:`codify.terminals.TerminalRegistry`. The registry owns
the per-conversation map; tools translate JSON arguments and
forward.

Per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §4.3, these tools register
on AP-side :class:`codify.tools.manager.ToolManager` and flow
...
class _ResolvedInstance:
    ...
    :param lock: The per-instance ``threading.Lock`` from
        ...
        thread's send. See ``designs/CODIFY_TERMINAL_BRIDGE.md`` §9.1.
    """
...
# agents in the wild see an unchanged tool surface under Codify mode.
...
def _check_overrides(
    ...
    per the codify-dev max-40-line-method rule. Returns either
...
class SysTerminalLaunchTool(Tool):
    ...
    def _resolve_cwd(
        ...
        3. ``spec.os_env.cwd`` if it's set to a meaningful path
           (not ``None``, ``""``, ``"."``, or ``"./"``).
        4. ``ctx.workspace`` — the per-task workspace Codify creates
