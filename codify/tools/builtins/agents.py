...
each proxies the Codify server's existing REST endpoints over
``server_client`` (same channel and security posture as the
``sys_session_*`` family in
:mod:`codify.tools.builtins.spawn`). They ship as schema-only
:class:`~codify.tools.base.Tool` subclasses — the base-class
...
class SysAgentGetTool(Tool):
    ...
    :class:`~codify.server.schemas.AgentObject`. Returns
...
class SysAgentListTool(Tool):
    ...
    - **local configs**: agent config YAMLs authored locally with
      ``sys_os_write`` (e.g. following the ``build-codify`` skill) — a
