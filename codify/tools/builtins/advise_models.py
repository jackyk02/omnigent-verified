...
:class:`~codify.tools.manager.ToolManager` so the LLM can see and
call ``sys_advise_models``.  Execution is handled server-side: the
Codify server intercepts the ``tools/call`` in
:func:`~codify.server.routes.sessions._handle_advise_models_mcp`
