...
default (``codify/inner/datamodel.py::AgentDef.async_enabled``),
so agents that don't mention it still see the async surface and
the same YAML produces the same tool list under Codify mode and the
...
class SysCancelTaskTool(Tool):
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "NOTE: this tool only cancels tasks that Codify created via "
...
class SysReadInboxTool(Tool):
    ...
    def is_async(self, arguments: str | None = None) -> bool:
        ...
        routes through :meth:`dispatch_async`, which Codify awaits
