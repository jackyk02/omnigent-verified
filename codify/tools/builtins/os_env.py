...
inner :mod:`codify.inner.session` when the agent's
``os_env`` is set. They wrap a shared
:class:`codify.inner.os_env.OSEnvironment` instance so the
same shell + cwd + sandbox is used across calls.

The Codify path can't reuse those Session tool registrations
directly: its
the harness HTTP executor
talks to the harness subprocess via the new HTTP contract,
and tool dispatch goes through AP's
:class:`codify.tools.manager.ToolManager` (so policies,
...
(:func:`codify.inner.os_env.create_os_environment`,
:class:`OSEnvSpec`) which won't ship in the AP-only deployment
once :mod:`codify.inner` retires. Keeping the dependency
...
from codify.inner.os_env import _DEFAULT_READ_LIMIT, OSEnvironment
...
# ── JSON Schemas ──────────────────────────────────────────────
# Mirror the inner :mod:`codify.inner.session` schemas
...
class _OSEnvBackedTool(Tool):
    ...
    :class:`codify.tools.manager.ToolManager` constructs
...
def build_os_env_tools(os_env: OSEnvironment) -> list[Tool]:
    ...
    Convenience for the
    :class:`codify.tools.manager.ToolManager`'s registration
