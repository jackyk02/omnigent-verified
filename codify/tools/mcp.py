...
The runner's :class:`codify.runner.mcp_manager.RunnerMcpManager`
...
class McpServerConnection:
    ...
    async def _invoke_tool(
        ...
        ``/mcp/execute`` → Codify server) can surface the elicitation
    ...
    async def call_tool_with_elicitation(
        ...
        Called by the Codify server after the user has responded to an
        ...
        # InputRequiredResult on the retry. Raise so the Codify server
    ...
    async def _open_stdio_transport(
        ...
        (``codify/inner/mcp_tools.py``); Codify now matches that
        baseline. Per-MCP sandboxing — if reintroduced — should
        flow through the ``codify/environments/`` primitive
...
async def _sleep(seconds: float) -> None:
    ...
    ``asyncio.sleep`` globally (patching ``codify.tools.mcp.asyncio.sleep``
...
async def _call_tool_with_reconnect(
    ...
    :param session_id: Codify session id forwarded to
