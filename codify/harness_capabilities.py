...
Like :mod:`codify.harness_install_spec`, this type lives outside the
...
Each :class:`~codify.harness_plugins.HarnessContribution` carries a
...
class Elicitation(str, Enum):
    """How a policy ASK / tool-approval is surfaced to the Codify web UI."""

    NONE = "none"
    HOOK = "hook"  # vendor PreToolUse hook posts to Codify
...
class Resume(str, Enum):
    ...
    COLD_ONLY = "cold-only"  # rebuild from Codify transcript / history replay
...
class AuthModel(str, Enum):
    ...
    CODIFY_CREDENTIAL = "codify-credential"  # Codify gateway / provider config
    OWN_AUTH = "own-auth"  # vendor login / API key, not Codify-managed
...
class HarnessCapabilities:
    ...
    :param subagents: Whether the harness can spawn Codify sub-agents.
    ...
    :param shell_tool_prompt: The prompt the bench sends to provoke that tool.
        Must contain the ``codify-bench-ok`` placeholder the probe token-swaps.
