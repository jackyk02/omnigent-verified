...
to ``codify.codex_native._materialize_codex_agent_spec``). The runner
...
This module also hosts the interactive local ``codify opencode`` CLI wrapper
(:func:`run_opencode_native`, the analog of ``codify codex`` / ``codify pi``):
...
the runner's ambient env / ``codify setup`` config (a profile-bound spec routes
...
from codify._wrapper_labels import OPENCODE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from codify.native_terminal import bind_session_runner as _bind_session_runner
from codify.native_terminal import (
    normalize_extra_args as _normalize_extra_args,
)
from codify.native_terminal import url_component
from codify.opencode_native_state import read_launch_state, write_launch_state
...
def run_opencode_native(  # pragma: no cover
    *,
    ...
    """
    Launch the OpenCode TUI in an Codify terminal (the ``codify opencode`` path).

    Mirrors ``codify codex`` / ``codify pi``: ensure a local daemon + runner,
    ...
    :param server: Resolved Codify server URL. ``None`` is an error (the CLI
        must resolve a backend first).
    :param session_id: Optional existing Codify conversation id to resume.
    ...
    if server is None:
        raise click.ClickException(
            "OpenCode requires a resolved Codify server URL. The CLI should resolve "
            "a backend before run_opencode_native."
        )
    with TemporaryDirectory(prefix="codify-opencode-native-") as tmpdir:
...
def _run_with_remote_server(  # pragma: no cover
    ...
    *,
    ...
    """Launch OpenCode on an Codify server via a daemon-spawned runner."""
...
def _resolve_session_id_for_resume(
    *,
    ...
    # e2e, not unit tests (it opens an CodifyClient and an arrow-key picker).
    from codify_client import CodifyClient  # pragma: no cover
