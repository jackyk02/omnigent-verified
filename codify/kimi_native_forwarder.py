"""Mirror a kimi-native TUI session's transcript into the Codify web chat.

The kimi-native harness launches the interactive ``kimi`` TUI in a tmux pane and
injects web-UI turns into it (see :mod:`codify.kimi_native_bridge`). The TUI's
reply renders live in the embedded terminal, but — unlike the SDK ``KimiExecutor``
— nothing flows the assistant's response back into Codify's conversation
transcript (the chat bubbles). This module closes that gap, the kimi analog of
:mod:`codify.cursor_native_forwarder`.
...
``/v1/sessions/{id}/events`` (the same shape :mod:`codify.kimi_native_hook`
