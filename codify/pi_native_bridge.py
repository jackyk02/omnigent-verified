...
PI_NATIVE_CONFIG_ENV_VAR = "CODIFY_PI_NATIVE_CONFIG"

_BRIDGE_ROOT = Path.home() / ".codify" / "pi-native"
_CONFIG_FILE = "config.json"
_EXTENSION_FILE = "codify_pi_native_extension.js"
_EXTENSION_PACKAGE = "codify.resources.pi_native"
_INBOX_DIR = "inbox"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    ...
    :returns: Absolute bridge directory under ``~/.codify/pi-native``.
    """
...
def write_extension_files(
    ...
    *,
    ...
    :param auth_headers: HTTP headers the extension should use when posting
        terminal-originated events back to Codify.
    :param tools: Flat Codify tool schemas
        ...
        runner uses), so the Pi agent can invoke Codify ``sys_*`` tools with
...
def refresh_config_auth_headers(bridge_dir: Path, auth_headers: dict[str, str]) -> bool:
    ...
    so launch-written headers such as ``X-Codify-Runner-Tunnel-Token`` — set
