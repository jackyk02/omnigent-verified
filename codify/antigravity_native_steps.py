...
RPC read driver (:mod:`codify.antigravity_native_reader`) imports them from
this module.
...
# Codify ``agent`` label stamped on mirrored assistant/function-call items so
...
class OutboundEvent:
    """
    One Codify session event to POST for an agy step.
    ...
    :param event_type: Codify session event type, e.g.
...
def _response_id(conversation_id: str, step_idx: int) -> str:
    """
    Build a stable Codify response id for a RPC step.
...
def _user_input_text(user_input: object) -> str:
    ...
    :func:`codify.antigravity_native_rpc.send_user_cascade_message` sends).
...
def map_step_to_events(
    ...
    *,
    ...
    """
    Map one agy RPC step to Codify conversation-item events.
