"""Native Pi TUI wrapper for the Codify CLI."""
...
from codify._wrapper_labels import PI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from codify.pi_native_bridge import bridge_dir_for_session_id
...
def _configured_pi_command(env: Mapping[str, str]) -> str:
    ...
    Reads ``CODIFY_PI_PATH`` (canonical) then the deprecated
    ...
    if legacy:
        from codify.harness_startup_config import _warn_legacy_path
...
def run_pi_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Pi requires a resolved Codify server URL. The CLI should call "
            "_ensure_backend before run_pi_native."
        )
    with TemporaryDirectory(prefix="codify-pi-native-") as tmpdir:
...
def _materialize_pi_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``codify pi``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Pi on an Codify server via a daemon-spawned runner.
...
async def _create_pi_session(
    ...
    *,
    ...
    :param client: HTTP client pointed at Codify.
