"""
Compatibility shim: legacy codify ``(content, phase)`` policy
callables → codify ``(ctx, context)`` convention.

Legacy codify function policies were written like::
...
callable runs under codify it silently short-circuits to
``"allow"`` on the very first phase check (``phase`` is a dict
there, not a string) and any policy written in the old style
becomes a no-op.

This module bridges the gap. The codify→codify
translator in :mod:`codify.spec.codify` routes every
...
every call. Modern callables (codify-native ``(ctx)`` /
...
from codify.spec.types import Phase

# Parameter names that identify a legacy codify policy
# callable — the first two positional parameters must be exactly
# ``content`` then ``phase``. A callable whose first two params
# are ``(foo, bar)`` is treated as codify-native even though
...
# Maps Codify' :class:`Phase` enum to the string literal
...
def build(
    ...
    loading an codify-sourced function policy.

    Imports *target*, optionally unwraps a factory call with
    *factory_kwargs*, then decides whether the resolved callable
    is a legacy codify policy (parameter names
    ``(content, phase)``) or an codify-native one
    ...
    :param configured_phases: The YAML ``on:`` list (verbatim
        ...
        :class:`codify.inner.policies.FunctionPolicy`
        ...
        (:func:`codify.spec.codify._translate_function_policy_yaml`)
        always passes a list.
    :returns: A callable matching Codify'
...
def _coerce_legacy_return(result: object) -> object:
    ...
    by :func:`codify.policies.function._coerce_to_policy_result`.
...
def _wrap_legacy(
    ...
    *,
    ...
    wrapper when present, so the codify ``FunctionPolicy``
    can call it at turn boundaries — preserving the legacy
    codify per-turn-reset contract for stateful policies
    ...
    :param configured_phases: When set, every wrapper invocation
        ...
        codify-native would. ``None`` means the key is
    ...
    # Capture once at wrap time. The codify FunctionPolicy
...
def _v0_event_to_legacy_phase(event: Mapping[str, object]) -> str:
    ...
    Codify' ``Phase`` enum. Provide a direct mapping here.
...
def _convert_args(
    ...
    *,
    ...
    ``(content, phase, context)`` from Codify'
    ...
    - Inner system: the codify :class:`FunctionPolicy` builds an
...
def _legacy_content(ctx: EvaluationContext) -> object:
    ...
    :func:`codify.runtime.workflow._enforce_tool_call_policy`
    and siblings). The legacy codify shape matches those
    normalized shapes almost exactly — this helper bridges the
    per-phase differences:

    - ``INPUT`` / ``OUTPUT``: codify passes the message
      text directly; legacy codify expects the same. Pass
      through.
    - ``TOOL_CALL``: codify passes
      ``{"tool": name, "args": parsed_args_dict}``; legacy
      codify expects the same. Pass through.
    - ``TOOL_RESULT``: codify passes the raw tool output
      string. Legacy codify passes the parsed result *dict*
      via
      :func:`codify.inner.mcp_tools._extract_call_result_payload`
      ...
      structured payload), native codify hands the policy
      ...
      JSON-parse the codify string; on parse failure pass
...
def _maybe_parse_json(value: object) -> object:
    ...
    codify-native's behavior: when an MCP text block is
...
def _legacy_context(
    ...
    *,
    ...
    Legacy codify passed ``{"labels": {...},
    "configured_phases": [...], "tool_name": "sleep"}`` — but
    ``tool_name`` is *only* added on ``TOOL_RESULT`` (see
    :meth:`codify.inner.session.Session._apply_tool_result_policy`,
    which builds ``context = {"tool_name": tool_name}`` for
    that phase). On ``TOOL_CALL``, native codify passes no
    ...
    :class:`codify.inner.policies.FunctionPolicy.evaluate`,
