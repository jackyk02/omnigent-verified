...
# Codify compat: imported surgically from a dedicated module so
# the integration's tech debt is removable in one shot. See
# codify/spec/_codify_compat.py.
from codify.spec._codify_compat import (
    diagnose_yaml_rejection,
    is_codify_yaml,
    load_codify_yaml,
)
from codify.spec.parser import (
    expand_env_vars,
    parse,
    parse_default_policies,
    parse_server_llm,
)
from codify.spec.tar_utils import ExtractionError, extract_safe
...
from codify.spec.validator import ValidationResult, validate
...
def materialize_bundle(source: Path, dest: Path) -> Path:
    ...
    (``config.yaml`` + bundled assets) or a standalone codify
    YAML file. Downstream code (``_preregister_agent``,
    ``_prepare_codify_yaml_bundle``) always wants to operate on a
    ...
    :returns: *dest*, always as a populated directory. For the
        ...
        *source*. For the file case the YAML is placed at the root
        of *dest* so
        :func:`codify.spec._find_codify_yaml_in_dir` picks
...
def _find_codify_yaml_in_dir(root: Path) -> Path | None:
    """
    Return the codify YAML inside *root* when the directory is
    a single-file codify bundle, or ``None`` otherwise.

    A directory qualifies as a single-file codify bundle when
    there is no ``config.yaml`` at the root and exactly one file
    at the root is recognised by :func:`is_codify_yaml`. Any
    other shape (``config.yaml`` present, zero or multiple codify
    YAMLs, YAMLs in subdirectories) returns ``None`` and the caller
    falls through to the standard codify directory-parse path.

    :param root: Path to an extracted bundle directory.
    :returns: The matched codify YAML path, or ``None`` if the
        directory is not a single-file codify bundle.
    """
    if (root / "config.yaml").exists():
        return None
    matches = [p for p in root.iterdir() if p.is_file() and is_codify_yaml(p)]
    if len(matches) == 1:
        return matches[0]
    return None
...
def load(
    ...
    *,
    ...
    If *source* is a file path (tarball or codify YAML) or raw
    bytes, dispatch accordingly.

    :param source: Path to an agent image directory, ``.tar.gz``
        bundle, codify single-file YAML, or raw tarball bytes
    ...
    :param expand_env: Whether to expand ``${VAR}`` references in
        ...
        (local ``codify run``, ``--agent`` preregistration). It
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        :func:`codify.server.bundles.validate_agent_bundle`. It is
        applied for both bundle shapes: the codify single-file YAML
        ...
        so trusted spec loading (local ``codify run``, operator
    ...
    :param prune_invalid_sub_agents: When ``True``, a sub-agent that
        ...
        **execution** paths (runner spec resolution, server
        :class:`~codify.runtime.agent_cache.AgentCache`): a bundle
        ...
        (``codify run``,
        :func:`codify.server.bundles.validate_agent_bundle`) stay
        strict and surface real authoring mistakes to the author.
    :returns: A validated :class:`AgentSpec`.
    :raises CodifyError: If the spec fails validation, if a policy
    ...
    if isinstance(source, bytes):
        ...
        # Codify single-file YAML dispatch — see
        # codify.spec._codify_compat. Tech-debt aside;
        # remove this branch when codify compat ends.
        if is_codify_yaml(source):
            return load_codify_yaml(
                source,
                enforce_handler_allowlist=enforce_handler_allowlist,
                prune_invalid_sub_agents=prune_invalid_sub_agents,
            )
        if source.suffix.lower() in {".yaml", ".yml"}:
            # The path is a YAML file but failed the codify check
            ...
            raise CodifyError(
                f"{source}: not a valid codify YAML — {reason}",
                code=ErrorCode.INVALID_INPUT,
            )
        if dest is None:
            raise CodifyError(
    ...
    # Codify single-file YAML dispatch (extracted-bundle variant)
    # — when a bundle (directory, tarball, or raw bytes) resolves to
    # a root that contains exactly one codify YAML and no
    # ``config.yaml``, route to the codify adapter. This is the
    # shape produced by ``codify server --agent`` when it wraps a
    # YAML into a single-file tarball for the agent store. See
    # codify/spec/_codify_compat.py for the tech-debt
    # ownership; remove when codify compat ends.
    # The codify single-file path (load_codify_yaml) copies
    ...
    candidate = _find_codify_yaml_in_dir(root)
    ...
    if enforce_handler_allowlist:
        ...
        # The single-file codify YAML path is guarded earlier, inside
    ...
    *spec* in place: the failing sub-agent is removed from
    ...
    for sa in spec.sub_agents:
        ...
        _logger.warning(
            ...
            "produced by a newer Codify server with a feature this client does "
...
def _reject_unregistered_spec_policy_handlers(spec: AgentSpec) -> None:
    ...
    :class:`~codify.spec.types.FunctionPolicySpec` entries whose
    ...
    :raises CodifyError: If a function policy names an unregistered
        handler, e.g. ``"subprocess.Popen"``.
    """
    from codify.policies.registry import is_registered_handler
