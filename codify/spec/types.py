...
if TYPE_CHECKING:
    ...
    # codify.policies.types); PhaseSelector.matches takes
...
class ProviderAuth:
    ...
    # Required: provider name from ~/.codify/config.yaml providers:.
    name: str
...
class ExecutorSpec:  # type: ignore[explicit-any]  # config: dict[str, Any] field (see below)
    ...
    :param type: Executor type. ``"codify"`` (default),
    ...
    :param profile: The Databricks workspace profile name from
        ...
        codify-compat sunset this is lifted from raw YAML's
        ``executor.profile`` in the codify path too. ``None``
    ...
    :param config: Executor-type-specific configuration. For
        ``type == "codify"`` this carries ``"harness"`` (e.g.
        ``"claude-sdk"`` or ``"codex"``), optional ``"profile"``
        (e.g. ``"<your-profile>"``), and optional ``"os_env"`` — the
        latter is a nested mapping mirroring the codify
        ...
        🚨 **TECH DEBT — REMOVE WHEN CODIFY COMPAT ENDS.**
        This field exists *solely* to carry harness / profile /
        os_env data for the Codify integration (see
        ``designs/CODIFY_INTEGRATION.md``). A free-form
        ``dict[str, Any]`` on a spec dataclass is the kind of
        bag-of-values escape hatch we'd otherwise reject in
        review — acceptable here only because the Codify
        executor is a temporary bridge with explicit sunset
        criteria. Once Codify is consolidated (phase 6 of the
    ...
    :param auth: Explicit LLM authentication configuration. When set,
        ...
        ``~/.codify/config.yaml``). ``None`` means fall back to
    ...
    # During the codify-compat sunset, lifted from raw YAML's
    # executor.profile in the codify path too. None = resolve
    # via env vars / DEFAULT section. See class docstring.
    # DEPRECATED: use executor.auth: {type: databricks, profile: <name>} instead.
    profile: str | None = None
    # TECH DEBT (codify compat only — see class docstring).
    # Remove when Codify consolidation lands; do NOT extend.
    ...
    def harness_kind(self) -> str:
        ...
        For ``type == "codify"`` the kind lives in
...
class LLMConfig:  # type: ignore[explicit-any]  # extra: dict[str, Any] field (see below)
    ...
    :param fallback_models: Ordered backup models tried, in order,
        ...
        (:class:`~codify.policies.types.PolicyLLMClient`): a call
...
class SandboxConfig:
    ...
    :param container_runtime: The container CLI to use, either
        ...
        via the ``CODIFY_CONTAINER_RUNTIME`` environment variable;
        the constructor argument takes precedence.
    """

    _ENV_VAR = "CODIFY_CONTAINER_RUNTIME"
...
class MCPServerConfig:
    ...
    - ``"stdio"`` — local subprocess, spawned via
      ...
      legacy inner stack at ``codify/inner/mcp_tools.py``
      ...
      sandboxing through the ``codify/environments/``
...
# PolicyEngine live under codify.runtime.policies in later
# phases.
...
class AgentSpec:  # type: ignore[explicit-any]  # params: dict[str, Any] field (see below)
    ...
    :param async_enabled: Whether the LLM-callable async-dispatch
        ...
        ``codify/inner/datamodel.py::AgentDef.async_enabled``
        — the same YAML must produce the same tool surface
        whether the user opts into Codify mode or runs the legacy
    ...
    :param os_env: The agent's OS environment, e.g.
        ...
        :mod:`codify.inner.bwrap_sandbox`), selected when the
        ...
        Native Codify YAML accepts this as a top-level ``os_env:``
        mapping; the codify-compat translator copies
        ``AgentDef.os_env`` into this field directly. The
        ``"inherit"`` string sentinel from codify inline
    ...
    :param terminals: Declared interactive tmux terminals
        ...
        Populated by the codify-compat translator from
        ``AgentDef.terminals``. Native Codify YAML support is
        deferred — see ``designs/CODIFY_TERMINAL_BRIDGE.md`` §3.
    :param timers: Whether the LLM-callable timer builtins
        ...
        ``codify/inner/datamodel.py::AgentDef.timers`` —
        ...
        stack uses in-memory asyncio tasks; Codify supersedes that
