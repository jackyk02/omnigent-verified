...
from codify.spec.parser import _discover_skills, _parse_skill, discover_host_skills
...
def codex_host_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    ``codify.spec``'s module-load path.
    """
    from codify.inner.codex_executor import codex_skill_sources, select_codex_skill_dirs
    ...
    for name, skill_dir in select_codex_skill_dirs(ctx.skills_filter, sources).items():
        ...
        except (CodifyError, OSError):  # best-effort discovery
            continue
...
def pi_host_skills(ctx: SkillSourceContext) -> list[SkillSpec]:
    ...
    sourcing from Pi's internal extension layout. codify can't enumerate
    ...
    codify on harnesses that have *no* host-skill mechanism of their own
    (the unregistered SDK/CLI harnesses — antigravity, qwen — which route a
    typed skill through codify's own resolve+inject), but Pi's competing,
...
# whose skills codify resolves+injects regardless of vendor. Pi is the one
# harness that needs an explicit no-op (see pi_host_skills) because it owns a
# host-skill mechanism codify can't enumerate.
