...
def parse(root: Path, *, expand_env: bool = True) -> AgentSpec:
    ...
    :returns: A fully populated :class:`AgentSpec` (not yet
        validated).
    :raises CodifyError: If ``config.yaml`` is not valid YAML,
    ...
    # ``codify/inner/datamodel.py::AgentDef.async_enabled`` — the
    # same YAML must produce the same tool surface under Codify mode and
    ...
    # ``codify/inner/datamodel.py::AgentDef.timers`` — agents
    ...
    # ``_CODIFY_SYSTEM_PROMPT_KEYS``); ``instructions:`` wins if both set.
...
def _parse_llm(
    ...
    *,
    ...
    :raises CodifyError: If the ``llm:`` block is present but
...
def _parse_sandbox_config(
    ...
    runtime decision, not an agent config decision::
        ...
        sandbox:
          ...
          # optional, defaults to CODIFY_CONTAINER_RUNTIME or docker
...
def _parse_builtin_tools(
    ...
    *,
    ...
    :raises CodifyError: If a dict entry is missing ``name``.
    """
...
def _parse_executor(
    ...
    *,
    ...
    # ``codify`` we ALSO mirror it into ``config["profile"]``
    # so the existing codify executor (which still reads from
    # ``config["profile"]``) keeps working until it is migrated.
    profile_raw = raw.get("profile")
    profile: str | None = None
    if profile_raw is not None:
        profile = str(profile_raw)
    if etype == "codify" and profile is not None and "profile" not in config:
...
def _parse_executor_auth(
    ...
    *,
    ...
    - ``type: provider`` — requires ``name`` (a provider declared in
      the ``providers:`` block of ``~/.codify/config.yaml``).
    ...
    :raises CodifyError: If the ``auth:`` block is present but
...
def _parse_os_env(
    ...
    Native Codify YAML mirrors the codify YAML shape so users
    ...
    :class:`codify.inner.datamodel.OSEnvSpec` for the
    ...
    :raises CodifyError: If *raw* is not a mapping, or
...
def _parse_terminals(
    ...
    Native Codify YAML mirrors the codify-compat ``terminals:`` shape — a
    ...
    ``designs/CODIFY_TERMINAL_BRIDGE.md`` §3 (``_parse_terminals`` parallel to
    ``_parse_os_env``).
    ...
    :raises CodifyError: If ``terminals`` (or any entry) is not a mapping,
...
def _parse_os_env_sandbox(
    ...
    if raw_type is None:
        ...
        from codify.inner.sandbox import _default_sandbox_for_platform
    ...
    if macos_reason is not None:
        raise CodifyError(macos_reason, code=ErrorCode.INVALID_INPUT)
...
def _parse_cwd_allow_hidden(raw: object) -> list[str] | None:
    ...
    :raises CodifyError: If ``raw`` isn't a list, contains a
...
def _parse_cwd_hidden_scan_max_entries(raw: object) -> int:
    ...
    :returns: Validated positive integer, or the dataclass default
        when ``raw`` is ``None``.
    :raises CodifyError: If ``raw`` is not an int or is not
        strictly positive.
    """
    if raw is None:
        return cast(
...
def _parse_cwd_hidden_scan_overflow(raw: object) -> str:
    ...
    :raises CodifyError: If ``raw`` is not one of the supported
        modes.
    """
    if raw is None:
        return cast(
...
def _parse_cwd_hidden_scan_recursive(raw: object) -> bool:
    ...
    :raises CodifyError: If ``raw`` is not a boolean.
    """
    if raw is None:
        return cast(
            bool,
...
def _parse_mask_paths(raw: object) -> list[str] | None:
    ...
    :raises CodifyError: If ``raw`` is not a list, or any entry is
...
def _parse_egress_rules(raw: object) -> list[str] | None:
    ...
    :func:`~codify.inner.egress.rules.parse_rule` so syntax
    ...
    :raises CodifyError: If the value isn't a list or any
        rule fails to parse.
    """
    if raw is None:
        return None
    ...
    if not raw:
        return None
    from codify.inner.egress.rules import parse_rule
...
def _format_validation_error(exc: ValidationError) -> str:
    ...
    :class:`CodifyError` so the CLI / loader surface a single error
...
def _parse_credential_proxy(raw: object) -> CredentialProxySpec | None:
    ...
    is re-raised as an :class:`CodifyError` so callers see one error
    type.
    ...
    :raises CodifyError: If the value isn't a list, an entry fails
...
def _merge_databricks_cli(
    ...
    *,
    ...
    :raises CodifyError: On a duplicate profile across entries or a
...
def _normalize_https_bearer(
    ...
    *,
    ...
    :raises CodifyError: If a host fails DNS-safety validation.
    """
...
def _normalize_gh_basic(
    ...
    *,
    ...
    :raises CodifyError: If an explicit host fails DNS-safety validation.
    """
...
def _parse_credential_proxy_host(raw: str, *, field_path: str) -> str:
    ...
    :raises CodifyError: If the value is empty or the host contains
        characters outside the DNS grammar ``[A-Za-z0-9.-]`` (wildcards
        included — credentials bind to an exact host).
    """
    from codify.inner.egress.rules import is_dns_safe_host
...
def _parse_share_policy(raw: object) -> SharePolicy:
    ...
    :raises CodifyError: When the value is neither ``None`` nor one
...
def _discover_skills(
    ...
    *,
    ...
    for skill_dir in entries:
        ...
        except (CodifyError, yaml.YAMLError) as exc:
            if skipped is None:
                raise
            msg = f"{skill_md}: {exc}"
...
def _parse_skill(skill_md: Path) -> SkillSpec:
    ...
    :raises CodifyError: If the file cannot be read, or the
    ...
    try:
        ...
        # OSError — funnel it through CodifyError too so the lenient
...
def expand_env_vars(
    ...
    Raises :class:`CodifyError` if any value still contains an
    ...
    :raises CodifyError: If a value contains an unresolved
...
def check_unresolved_env_vars(key: str, value: str) -> None:
    ...
    :param value: The expanded value to check, e.g.
        ``"Bearer ${MISSING}"``.
    :raises CodifyError: If *value* contains an unresolved
...
def _discover_mcp_servers(
    ...
    *,
    ...
    :returns: A sorted list of parsed :class:`MCPServerConfig`
        ...
        exist.
    :raises CodifyError: If any YAML file is malformed or
...
def _parse_http_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "http"``.
    :raises CodifyError: If ``url`` is missing or a stdio-only
        field was supplied.
    """
...
def _parse_stdio_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "stdio"``.
    :raises CodifyError: If ``command`` is missing, ``args`` is
...
def _reject_wrong_transport_keys(
    ...
    *,
    ...
    :raises CodifyError: When any *disallowed* key is present
        in *raw*.
    """
...
def _parse_guardrails(
    ...
    *,
    ...
    :returns: A populated :class:`GuardrailsSpec`, or ``None``
        when *raw* is ``None``.
    :raises CodifyError: On any spec-load validation
...
def _parse_guardrails_ask_timeout(raw: object) -> int:
    ...
    :raises CodifyError: On non-integer or non-positive
        values.
    """
...
def _parse_label_defs(
    ...
    :raises CodifyError: On malformed entries — empty
...
def _parse_single_label_def(key: str, entry: object) -> LabelDef:
    ...
    :raises CodifyError: On any malformed value.
    """
...
def _coerce_label_values(key: str, raw: object) -> list[str] | None:
    ...
    :returns: Every element str-coerced; ``None`` when
        *raw* is ``None``.
    :raises CodifyError: When *raw* is a non-list
        non-None value.
    """
    if raw is None:
        return None
...
def _validate_label_def_cross_fields(
    ...
    :raises CodifyError: On any cross-field violation.
    """
...
def _parse_policies(
    ...
    *,
    ...
    :returns: List of policy specs, or ``None`` when *raw*
        is ``None``.
    :raises CodifyError: On any malformed policy entry.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_policy_spec(
    ...
    *,
    ...
    :raises CodifyError: On malformed data or unknown
        policy type.
    """
    ...
    if policy_type == "prompt":
        raise CodifyError(
            ...
            f"'codify.policies.builtins.prompt.prompt_policy' instead.",
            code=ErrorCode.INVALID_INPUT,
        )
...
def _parse_function_policy(
    ...
    :raises CodifyError: On missing ``function:`` field
    ...
    # is the original codify YAML convention.
...
def _parse_on(
    ...
    *,
    ...
    :raises CodifyError: On empty list, unknown phase,
...
def _parse_on_entry(
    ...
    *,
    ...
    :raises CodifyError: On non-string entry, empty
...
def _resolve_phase(
    ...
    *,
    ...
    :raises CodifyError: When *phase_str* is not a
        valid phase.
    """
    try:
...
def _parse_condition(
    ...
    *,
    ...
    stored ``"0"``. The coercion matches codify parity
    for label values.
    ...
    :raises CodifyError: On a non-dict value.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_writable_labels(
    ...
    *,
    ...
    :returns: List of allowed label keys, or ``None`` when
        *raw* is absent.
    :raises CodifyError: When *raw* is not a list.
    """
    if raw is None:
        return None
    if not isinstance(raw, list):
...
def _parse_function_ref(
    ...
    *,
    ...
    :raises CodifyError: On malformed shape — non-string
...
def parse_default_policies(
    ...
    *,
    ...
    :raises CodifyError: On any malformed policy entry — unknown
