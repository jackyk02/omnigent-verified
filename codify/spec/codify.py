"""Bidirectional translator between codify ``AgentSpec`` and codify ``AgentDef``.

Two functions live here:

- :func:`agent_spec_to_agent_def` — forward direction, consumed by
  ``CodifyExecutor`` at executor-construction time.
- :func:`agent_def_to_agent_spec` — reverse direction, consumed by
  :func:`codify.spec.load` when handed an codify YAML so
  ``codify chat foo.yaml`` works transparently.

The bidirectional invariant
``agent_spec_to_agent_def(agent_def_to_agent_spec(d)) == d`` must
hold for every representative codify YAML; the round-trip test
under ``tests/spec/test_codify_roundtrip.py`` enforces it.

Design invariants (see designs/CODIFY_INTEGRATION.md §1 and §2):
...
- Unsupported concepts **fail loud** with an :class:`CodifyError`
  naming the specific field. Currently unsupported (each is an
  codify spec gap to fix, not a translation to paper over):
...
from codify.inner.datamodel import AgentDef, OSEnvSpec, TerminalEnvSpec
from codify.inner.datamodel import ExecutorSpec as OmniExecutorSpec
from codify.inner.tools import (
    AgentTool,
...
from codify.llms.routing import infer_harness_from_model as _infer_harness_from_model
from codify.spec.types import (
    AgentSpec,
    ApiKeyAuth,
    DatabricksAuth,
    ExecutorSpec,
...
# selects ``CodifyExecutor``. Both directions of the translator
# treat this as the discriminator between native codify specs
# and codify-sourced ones.
CODIFY_EXECUTOR_TYPE = "codify"

# Value placed in :attr:`LocalToolInfo.language` for tools that were
# sourced from an codify YAML. Distinguishes them from native
# codify tools (``"python"`` / ``"typescript"``) which live as
# files on disk under ``tools/python/``.
CODIFY_TOOL_LANGUAGE = "codify-python-callable"

# Version stamped on AgentSpecs synthesized from codify YAMLs.
# Codify YAMLs do not declare ``spec_version`` (it's an
# codify concept), so the adapter writes ``1`` — the currently
# valid codify schema version (see codify/spec/validator.py).
...
def agent_spec_to_agent_def(spec: AgentSpec) -> AgentDef:
    """
    Translate an codify ``AgentSpec`` into an codify
    ``AgentDef`` suitable for
    :func:`codify.executor_factory.create_executor`.
    ...
    :param spec: The codify spec to translate. ``spec.llm``
        must be set (the codify harness needs a model name);
        ``spec.executor.type`` must be ``"codify"`` with
        ``executor.config.harness`` populated.
    :returns: A populated :class:`codify.datamodel.AgentDef`
        ...
        rejected upstream.
    :raises CodifyError: If the spec uses an unsupported
    ...
    if spec.executor.model is None:
        raise CodifyError(
            "executor.type='codify' requires a model (set executor.model)",
            code=ErrorCode.INVALID_INPUT,
        )

    # For native Codify v1 specs ``executor.config`` is empty; infer the
    ...
    return AgentDef(
        ...
        tools=_translate_tools_to_codify(spec),
...
def _reject_unsupported_concepts(spec: AgentSpec) -> None:
    ...
    to remove from the spec (or what codify bug to file to
    close the gap).

    :param spec: The codify spec to check.
    :raises CodifyError: On any of:
    ...
    # Policies are enforced by the codify workflow layer
    # (see ``codify.runtime.policies.build_policy_engine``)
    # BEFORE the harness receives a turn — the
    # :class:`CodifyExecutor` is invoked with tool calls
    ...
    # Sandbox declarations (codify ``tools.sandbox.container_image``
    # and the codify OSEnvSandboxSpec) are unsupported. Fail loud
    # if either is populated.
    if spec.tools.sandbox.container_image is not None:
        raise CodifyError(
            "tools.sandbox translation to codify OSEnvSpec is unsupported; "
            "the adapter rejects specs with sandbox rather than silently dropping it",
            code=ErrorCode.INVALID_INPUT,
        )

    # ``cancellable_function`` is an codify concept — Codify'
    ...
    for tool in spec.local_tools:
        ...
        if _is_cancellable_function_path(tool.path):
            raise CodifyError(
                ...
                "codify; cancellation support is an codify spec extension",
                code=ErrorCode.INVALID_INPUT,
            )
...
def _is_cancellable_function_path(path: str) -> bool:
    ...
    explicitly in the codify spec; codify has no spec
...
def _translate_tools_to_codify(spec: AgentSpec) -> dict[str, Tool]:
    """
    Build the ``AgentDef.tools`` dict from Codify' tool model.
    ...
    :class:`codify.tools.FunctionTool`. Filesystem paths
    (``"tools/python/foo.py"``) are not supported — the YAML adapter
    that produces them must encode dotted paths.

    :param spec: The codify spec. ``spec.local_tools`` lists
    ...
    :returns: A ``dict[str, Tool]`` mapping tool name to
        ...
        local tools.
    :raises CodifyError: If a tool path is a filesystem path
    ...
    for tool_info in spec.local_tools:
        ...
        # the codify tool's ``input_schema``. Re-attach it so
    ...
    # so the codify harness surfaces them to the LLM via its
    # own MCP machinery (``codify.inner.mcp_tools``). The
    # forward direction translated ``MCPTool`` →
    # :class:`MCPServerConfig` (stdio or http); this is the inverse.
    # ``CodifyExecutor.from_spec`` calls this reverse path when
    # wrapping an codify spec for an codify harness; if the
...
def _mcp_server_to_mcp_tool(config: MCPServerConfig) -> MCPTool:
    ...
    :returns: An :class:`MCPTool` with the equivalent transport
        shape for the inner codify runtime.
    :raises CodifyError: If the config is missing the
...
def _resolve_dotted_attr(dotted_path: str, tool_name: str) -> Any:
    ...
    :returns: The resolved attribute object (callable or
        runner or other).
    :raises CodifyError: If the path is not a dotted import
    ...
    if "/" in dotted_path or dotted_path.endswith(".py"):
        raise CodifyError(
            ...
            "codify translator requires dotted import paths like "
            "'examples._shared.tool_functions.get_current_time'",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    try:
        ...
        # single CodifyError so the spec author sees a
...
def _resolve_dotted_callable(
    ...
    :raises CodifyError: If the module can't be imported,
...
# ── Policy translation (codify YAML dict → codify
# GuardrailsSpec) ────────────────────────────────────────────────
#
...
# parser (``codify.spec.parser._parse_guardrails``) is invoked
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    codify YAML into an codify :class:`GuardrailsSpec`.

    Codify declares labels/policies at the top level
    (``labels:``, ``label_schema:``, ``policies:``,
    ``ask_timeout:``); codify groups them under a single
    ``guardrails:`` block. This helper transforms the codify
    shape into the codify dict shape and delegates to
    :func:`codify.spec.parser._parse_guardrails` for
    ...
    :param raw_policies: Codify YAML ``policies:`` mapping,
    ...
    :param raw_labels: Codify YAML top-level ``labels:`` map
        ...
        ``initial: None``.
    :param raw_label_schema: Codify YAML top-level
        ``label_schema:`` map of
        ``{label_key: {values: [...], monotonic: "max"|"min"|"none"}}``.
    :param raw_ask_timeout: Codify YAML top-level
        ``ask_timeout:`` (seconds), e.g. ``30``. Passed through
        to ``guardrails.ask_timeout``. ``None`` means use the
        codify default (see
        :data:`codify.spec.types.DEFAULT_ASK_TIMEOUT`).
    ...
    :returns: A validated :class:`GuardrailsSpec`, or ``None``
        ...
        source YAML.
    :raises CodifyError: On malformed entries — same error
        shape the codify YAML parser produces for a native
        ``guardrails:`` block, so YAML authors see consistent
        error messages whether their spec came from codify
        or codify.
    """
    if not raw_policies and not raw_labels and not raw_label_schema and raw_ask_timeout is None:
        return None
    # Build the codify-shaped guardrails dict, then hand it to
    ...
    from codify.spec.parser import _parse_guardrails
    ...
    # ``expand_env`` is False because the upstream codify
...
def _translate_labels_yaml(
    ...
    """
    Merge codify' separate ``labels:`` (initial values) and
    ``label_schema:`` (schemas) into Codify' unified
...
def _translate_policies_yaml(
    ...
    *,
    ...
    """
    Translate the codify ``policies:`` mapping entry-by-entry
    into the codify shape.

    :param raw_policies: Raw ``policies:`` map from the codify
...
def _translate_policy_entry_yaml(
    ...
    *,
    ...
    :raises CodifyError: When the entry declares a policy
    ...
    if policy_type not in _KNOWN_POLICY_TYPES:
        raise CodifyError(
            f"codify policy {policy_name!r}: unknown type "
...
def _translate_function_policy_yaml(
    ...
    """
    Translate an codify ``type: function`` policy to the
    codify shape.
    ...
    - Other fields (``condition``, ``action``, ``set_labels``,
      ...
      longer accept ``on:`` on the codify side; the handler
    ...
    # codify parser side. ``on:`` is intentionally omitted —
    ...
    # If ``function:`` is already present in codify native
    ...
    if callable_path is None:
        # Let the codify parser surface the "function:
        # required" error — it already does so with a clear
        # message referencing the policy name.
        return out
    # Route every codify-sourced function policy through the
    # legacy-compat shim so author callables written with the
    # codify ``(content, phase)`` / ``(content, phase, context)``
    # convention keep working under Codify'
    # ``(ctx, context)`` convention. For codify-native
    # callables the shim is a pass-through (detected by parameter
    # names at policy-build time; see _codify_legacy_shim).
    ...
    out["function"] = {
        "path": "codify.spec._codify_legacy_shim.build",
        "arguments": shim_args,
    }
    return out
...
def _resolve_profile_to_connection(profile: str) -> dict[str, str] | None:
    ...
    Used at spec-translation time to make codify LLMConfig
    self-contained — Codify' LLM adapters take credentials
    via explicit ``connection:`` fields (per the "spec
    self-containment" design principle) and do not read env vars.
    Meanwhile codify declares Databricks creds via the
    ...
    """
    # Import locally to avoid a top-level dependency on codify
    # runtime details from the spec module — only this single
    # translator path needs it.
    from codify.inner.databricks_executor import _read_databrickscfg
...
def _translate_prompt_policy_yaml(
    ...
    *,
    ...
    """
    Translate an codify ``type: prompt`` policy to the
    codify shape.

    Field mapping:

    - ``executor: {model: X}`` → ``llm: {model: X}``. The
      codify prompt policy uses ``executor`` to override
      the classifier LLM; codify uses ``llm``.
    - When *parent_profile* is set AND the policy has no
      ...
      codify classifier actually reaches the Databricks
    ...
    :param parent_profile: Databricks profile from the parent
        ...
        codify LLMConfig so the classifier call at runtime
    ...
    for key in ("on", "condition", "ask_timeout"):
        if key in raw_entry:
            out[key] = raw_entry[key]
    out["function"] = {
        "path": "codify.policies.builtins.prompt.prompt_policy",
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    """
    Translate an codify :class:`AgentDef` into an codify
    :class:`AgentSpec`.

    Reverse direction of :func:`agent_spec_to_agent_def`. Used by
    :func:`codify.spec.load` when handed an codify YAML
    so ``codify chat foo.yaml`` routes the spec through
    ``CodifyExecutor`` at runtime.

    The produced spec sets ``executor.type = "codify"`` so the
    runtime picks ``CodifyExecutor``, with ``harness`` and
    ``profile`` carried in ``executor.config``.

    :param agent_def: Parsed codify agent definition,
        typically from :func:`codify.loader.load_agent_def`.
    ...
    :param raw_yaml: Optional raw YAML dict the ``agent_def`` was
        ...
        Codify' ``guardrails:`` block — the codify
        ...
        (``load_codify_yaml``) read the raw YAML themselves
        and pass it through.
    ...
    :raises CodifyError: When *agent_def* uses an codify
        concept Codify' :class:`AgentSpec` cannot currently
    ...
    # ``codify.inner.loader.load_agent_def``) wins over ``prompt:``
    ...
    # codify gets this "for free" via Session → factory; the
    # codify spawn path runs each sub-agent as an
    ...
    # translation time — the Codify path spawns each child as an
    ...
    # Split codify tools by subtype:
    #   FunctionTool / CancellableFunctionTool
    #       → AgentSpec.local_tools (codify treats every
    ...
    #         Codify' ToolManager, srt-wrapped when
    #         available — see codify/tools/mcp.py).
    ...
    # raw YAML. Policies/labels are lifted from the codify
    # top-level YAML fields into Codify' ``guardrails:``
    # block; enforcement then happens in the codify workflow
    # (outside the codify harness) via the standard
    ...
    if raw_yaml is not None:
        guardrails = _translate_guardrails_yaml(
            ...
            # Baked-in at translation time so Codify'
            # classifier LLM calls honor Databricks routing
            # (codify adapters don't read env vars per the
            # spec-self-containment design principle).
            parent_profile=parent_profile,
        )

    # Per designs/CODIFY_TERMINAL_BRIDGE.md §6.1 step 2 + §3 non-goal
    ...
    # ``terminals`` field in the codify grammar — matches legacy
    # non-AP mode exactly.
    terminals = dict(agent_def.terminals) if agent_def.terminals else None

    # Top-level ``skills:`` from the raw YAML (the codify
    # loader doesn't carry this field on AgentDef — it's
    # codify spec config that controls Claude SDK harness
...
def _translate_skills_filter_from_yaml(
    ...
    codify-format YAML mapping and validate the same shapes
    the codify spec parser accepts (``"all"`` / ``"none"`` /
    list of names).

    The codify loader (``inner.loader.load_agent_def``) does
    NOT read this field — it's codify spec config that
    controls the Claude SDK harness's host-skill loading, not an
    codify runtime concept. So when the YAML is consumed via
    ...
    :raises CodifyError: When the value isn't one of the
...
def _self_agent_tool_to_sub_spec(
    ...
    *,
    ...
    the Codify runtime spawns when the parent's LLM dispatches
...
def _agent_tool_to_sub_spec(
    ...
    *,
    ...
    """
    Translate an codify inline :class:`AgentTool` (sub-agent
    exposed as a tool) into a nested codify :class:`AgentSpec`.
    ...
    :class:`AgentSpec` with ``executor.type == "codify"`` so the
    :class:`CodifyExecutor` runs it when spawned.

    Lossy fields (not modeled on Codify' AgentSpec yet):
    ``max_sessions``, ``pass_history``, ``pass_histories``.
    codify' runtime falls back to its defaults for these on
    the reverse trip.

    :param tool_name: The YAML key under which this AgentTool is
        declared on the parent, e.g. ``"claude_worker"``.
    :param tool: The parsed codify :class:`AgentTool`.
    :param parent_profile: Databricks profile carried on the
        ...
        empty field does.
    :param parent_harness: Codify harness carried on the
        ...
        codify resolves this at ``create_executor`` time;
        codify has to do it at spec-translation time because
    ...
    :param parent_os_env: OS environment on the parent agent.
        ...
        inline AgentTool — legacy codify resolves it from the
        live Session at runtime, but codify runs each child
    ...
    :param raw_executor: The raw ``executor:`` dict for this inline
        ...
        codify' dataclass parsing dropped unknown keys). When set,
        fields the codify :class:`~codify.inner.datamodel.ExecutorSpec`
    ...
    # (see ``codify/tools/manager.py:426`` — "if not
...
def _resolve_inline_agent_tool_os_env(
    ...
    Codify' :class:`AgentTool.os_env` is
    ...
    if isinstance(tool_os_env, str):
        ...
        # strings would be an codify YAML that the loader
...
def _fail_on_unsupported_concepts_def(agent_def: AgentDef) -> None:
    """
    Raise :class:`CodifyError` for every codify concept
    Codify' :class:`AgentSpec` cannot currently represent.

    Each unsupported concept gets its own clear error message
    naming the specific field.

    :param agent_def: Parsed codify agent definition.
    :raises CodifyError: On the first unsupported concept
...
def _fail_on_unsupported_tool(
    ...
    """
    Raise :class:`CodifyError` when *tool* uses an codify
    tool concept codify cannot currently represent.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"glean_search"``.
    :param tool: The parsed codify tool instance.
    :raises CodifyError: On MCP tools or cancellable_function
        tools.
    """
    ...
    if isinstance(tool, MCPTool):
        ...
        # Databricks-managed MCP endpoint that Codify'
        ...
        if tool.databricks_server is not None:
            raise CodifyError(
                f"codify agent {agent_name!r} declares tool "
                f"{tool_name!r} of type `mcp` with "
                f"``databricks_server={tool.databricks_server!r}`` — "
                f"Codify' MCPServerConfig doesn't understand the "
...
def _translate_name_from_def(raw_name: str | None) -> str | None:
    """
    Translate codify ``AgentDef.name`` to :attr:`AgentSpec.name`.

    ``AgentDef.name`` is ``str | None`` on the codify side;
    codify also uses ``None`` for absence. An empty string on
    ...
    :param raw_name: The raw ``name`` field from
        ...
        ``None``.
    :returns: The codify name, or ``None`` when *raw_name*
        is absent or empty.
    """
...
def _translate_prompt_from_def(raw_prompt: str | None) -> str | None:
    """
    Translate codify ``AgentDef.prompt`` to
    :attr:`AgentSpec.instructions`.

    ``AgentDef.prompt`` is ``str | None`` on the codify side;
    codify also uses ``None`` for absence.

    :param raw_prompt: The raw ``prompt`` field from
        :class:`AgentDef`, e.g. ``"You are a friendly
        assistant."``, ``""``, or ``None``.
    :returns: The codify instructions text, or ``None``
...
def _translate_llm_from_def(
    ...
    *,
    ...
    """
    Translate codify ``executor.model`` into an codify
    :class:`LLMConfig`.

    codify stores the model string on its own
    :class:`codify.datamodel.ExecutorSpec`. codify splits
    ...
    ``type = "codify"`` plus the ``harness`` / ``profile``
    config.

    :param oa_executor: The codify
        :class:`~codify.datamodel.ExecutorSpec`, or ``None``
        when the YAML omitted the ``executor:`` block. Example:
        ``OmniExecutorSpec(model="databricks-claude-sonnet-4")``.
    :param raw_executor: Optional raw codify YAML ``executor:``
        ...
        through to :class:`codify.executor.ExecutorConfig.extra`.
        The codify loader drops unknown fields on
        ``ExecutorSpec`` at parse time, so we read them back from
        the raw dict here.
    :returns: An :class:`LLMConfig` with ``model`` populated, or
        ``None`` when codify declared no model.
    """
...
def _translate_executor_from_def(
    ...
    *,
    ...
    """
    Build the codify :class:`ExecutorSpec` for an codify
    agent.

    Returns ``type="codify"`` when a harness can be resolved,
    routing the agent through ``CodifyExecutor``. Raises
    :class:`CodifyError` when a model is declared but no
    ...
    The ``harness`` and ``profile`` fields from codify land in
    ``executor.config`` so :meth:`CodifyExecutor.from_spec` can
    ...
    3. :func:`_infer_harness_from_model` on the resolved model
       string — mirrors pure codify' CLI auto-pick
       (``databricks-claude-*`` → ``claude-sdk``, etc.) so YAMLs
       that declare only a model continue to work under Codify mode.
    ...
    Empty-string coercion mirrors codify' own dataclass
    defaults (``ExecutorSpec.harness: str = ""``,
    ``ExecutorSpec.profile: str = ""``) so the round-trip is
    bit-exact against the codify side.

    :param oa_executor: The codify ``ExecutorSpec`` or
        ``None``.
    ...
    :param raw_executor: Optional raw YAML ``executor:`` mapping.
        ...
        API surface (chat/completions vs. responses). The codify
        loader silently drops unknown fields on its own
        :class:`~codify.inner.datamodel.ExecutorSpec`, so we
        have to recover this field from the raw dict here.
    :returns: An :class:`ExecutorSpec` with ``type="codify"``
        when a harness is known.
    :raises CodifyError: When a model is declared but no
        harness can be inferred from it.
    """
    # Codify' :class:`ExecutorSpec.{harness,profile,model}`
    ...
    # ``use_responses`` and ``acp_agent`` are not fields on the codify inner
    ...
    if oa_executor is not None and oa_executor.auth is not None:
        ...
        from codify.spec.parser import _parse_executor_auth
...
def _translate_mcp_tool_from_def(
    ...
    """
    Translate one codify :class:`MCPTool` into a native
    ...
    - ``tool.command`` populated → ``transport="stdio"`` with
      ...
      subprocess spawns unsandboxed (matching legacy codify,
    ...
    :param tool: The parsed codify :class:`MCPTool` instance.
    :returns: A fully populated :class:`MCPServerConfig`.
    :raises CodifyError: If *tool* has neither ``url`` nor
    ...
    raise CodifyError(
        f"codify MCP tool {tool_name!r} has neither 'url' nor "
...
def _translate_function_tool_from_def(
    ...
    """
    Translate one codify function tool into a
    :class:`LocalToolInfo`.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"get_current_time"``.
    :param tool: The parsed codify tool. Must be a
    ...
    :returns: A :class:`LocalToolInfo` with ``name``, ``path``
        ...
        :data:`CODIFY_TOOL_LANGUAGE`.
    :raises CodifyError: When *tool* is not a
        :class:`FunctionTool` (defensive guard).
    """
    if isinstance(tool, CancellableFunctionTool):
        raise CodifyError(
            f"codify tool {tool_name!r}: "
    ...
    if not isinstance(tool, FunctionTool):
        raise CodifyError(
            f"codify tool {tool_name!r}: expected FunctionTool, got {type(tool).__name__}.",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    # for typed-parameter tools. Without this fallback, codify
...
def _recover_callable_path(
    ...
    codify' loader resolves the YAML's ``callable:`` field
    ...
    :returns: The dotted module path, e.g.
        ``"examples._shared.tool_functions.get_current_time"``.
    :raises CodifyError: When the callable is missing
    ...
    raise CodifyError(
        f"codify tool {tool_name!r}: callable {callable_obj!r} "
