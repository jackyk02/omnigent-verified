...
from codify.spec.types import AgentSpec, ToolRuntime
...
# codify/spec/_codify_compat.py. Placed after the module's
...
from codify.spec._codify_compat import (  # noqa: E402
    CODIFY_EXECUTOR_TYPE,
    validate_codify_executor,
)
...
def _validate_executor_type(
    ...
    elif etype == CODIFY_EXECUTOR_TYPE:
        validate_codify_executor(spec, result)
...
def _validate_local_tools(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    # spec load (the validator runs from ``codify.spec``,
    # which is a lower layer than ``codify.tools``).
    from codify.tools.builtins import BUILTIN_NAMES
...
# allow-list in ``codify/inner/loader.py``. ``none`` is excluded
...
def _validate_os_env(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    The Codify parser already enforces these on the YAML-loaded path,
    but ``AgentSpec`` may also be constructed programmatically —
    by tests, by the Codify compat shim, or by future API
