...
executor then delivers Codify web-UI messages into the *same* pane via
...
if TYPE_CHECKING:
    from codify.cursor_native import CursorModelOption

#: Env var carrying the bridge dir into the harness executor process.
BRIDGE_DIR_ENV_VAR = "HARNESS_CURSOR_NATIVE_BRIDGE_DIR"

_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"codify-{stable_user_id()}" / "cursor-native"
...
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "codify-cursor-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/codify-<uid>/cursor-native/<hash>``."""
...
#: when mirroring the user turn back to Codify so the copied history isn't
#: duplicated in the timeline (see cursor_native_forwarder._unwrap_user_query).
FORK_HISTORY_OPEN_TAG = "<codify_fork_history>"
FORK_HISTORY_CLOSE_TAG = "</codify_fork_history>"
...
def _neutralize_fork_sentinels(text: str) -> str:
    ...
    could literally contain ``<codify_fork_history>`` /
    ``</codify_fork_history>`` (a user typed it, pasted logs, etc.). If left
    ...
    """
    return text.replace(FORK_HISTORY_OPEN_TAG, "[codify_fork_history]").replace(
        FORK_HISTORY_CLOSE_TAG, "[/codify_fork_history]"
    )
...
def wrap_fork_preamble(preamble: str, user_text: str) -> str:
    ...
    duplicated in the Codify timeline.
...
def build_mcp_config(
    ...
    *,
    ...
    """Build Cursor's ``.cursor/mcp.json`` for the Codify relay server.

    Cursor prompts for MCP tool approval before it sends ``tools/call`` to the
    server. Codify tools already route through the Codify ``/mcp`` proxy,
    ...
    hidden in-terminal approval prompt blocking the call before Codify ever
    sees it, while preserving Codify's own policy/elicitation gate.
    """
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config required by the shared Codify MCP bridge."""
    _ensure_dir(bridge_dir)
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped Cursor MCP config for Codify tools."""
...
def build_hooks_config(
    ...
    ``codify.cursor_native_usage record-usage``, which appends the usage to
...
def approve_mcp_server_for_workspace(workspace: Path) -> None:
    """Approve the workspace-scoped Codify MCP server in Cursor's state.
    ...
    ask ``cursor-agent mcp enable codify`` to write the exact approval entry
...
def enable_mcp_for_workspace(workspace: Path) -> None:
    """Ensure Cursor does not keep the Codify MCP disabled for this workspace."""
    disabled_path = (
...
def allow_mcp_tools_in_cli_config() -> None:
    """Allow Codify MCP tool calls in Cursor's CLI permission config."""
...
def capture_cursor_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`codify.cursor_native_permissions`) to detect cursor-agent's native
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`codify.claude_native_bridge.kill_session`.
