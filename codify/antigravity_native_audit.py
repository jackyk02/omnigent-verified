...
.. note::
    **Currently dormant.** This module's only consumer was the transcript
    ...
    it surfaces agy's ``request-review`` prompts as real-time Codify
    elicitations (:mod:`codify.antigravity_native_interactions`) instead of a
...
**cannot intercept a tool before it runs**. The only honest Codify policy
...
* :func:`build_audit_evaluation_request` — wrap a tool call as the proto
  ...
  reusing :func:`codify.native_policy_hook.hook_payload_to_evaluation_request`
...
the (now-deleted) transcript forwarder, which needed the live Codify client and
...
from codify.native_policy_hook import hook_payload_to_evaluation_request
...
def build_audit_evaluation_request(
    *,
    ...
    :func:`codify.native_policy_hook.hook_payload_to_evaluation_request` with a
    ``PreToolUse`` payload so the request lands on the ``PHASE_TOOL_CALL`` phase
    (the phase tool-name / cost / CEL deny policies fire on). ``mcp__codify__*``
    ...
    :returns: An ``EvaluationRequest`` dict to POST to ``/policies/evaluate``, or
        ``None`` when the tool is not policy-relevant (an ``mcp__codify__*``
        tool).
    """
