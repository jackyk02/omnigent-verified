...
into the Codify conversation with ``external_conversation_item`` events.
"""
...
from codify.kiro_native_bridge import write_forwarder_ready
...
def _discover_kiro_session_jsonl(
    *,
    ...
    """Find this Codify session's Kiro JSONL file — only when it's unambiguous.
    ...
    (:func:`codify.cursor_native_forwarder._discover_store`).
...
async def _patch_external_session_id(
    ...
    *,
    ...
    """Persist Kiro's native CLI session id onto the Codify session."""
...
async def forward_kiro_session_to_codify(
    *,
    base_url: str,
...
async def supervise_kiro_session_forwarder(
    *,
    ...
    while True:
        ...
        try:
            await forward_kiro_session_to_codify(
                base_url=base_url,
                headers=headers,
                session_id=session_id,
...
__all__ = [
    "forward_kiro_session_to_codify",
