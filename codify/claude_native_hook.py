"""Claude Code hook recorder for the native Codify wrapper."""
...
from codify.native_policy_hook import (
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        print(
            f"codify hook: ignoring non-integer {name}={raw!r}; using {default}",
            file=sys.stderr,
        )
        return default
...
def _env_float(name: str, default: float) -> float:
    ...
    *default* rather than crashing the hook at import time.
    ...
    if not math.isfinite(value):
        print(
            f"codify hook: ignoring non-finite {name}={raw!r}; using {default}",
            file=sys.stderr,
        )
        return default
    return value
...
_PERMISSION_MAX_CONSECUTIVE_FAILURES = max(1, _env_int("CODIFY_HOOK_MAX_RETRIES", 8))
...
# this via CODIFY_HOOK_HELD_POLL_FLOOR_S so their legitimate slow-human severs
...
_PERMISSION_HELD_POLL_FLOOR_S = max(0.0, _env_float("CODIFY_HOOK_HELD_POLL_FLOOR_S", 10.0))
...
# budget — an unresponsive Codify server would otherwise hang the banner.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"codify claude hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify claude hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if payload.get("hook_event_name") == "SessionStart" and payload.get("source") == "clear":
        ...
        if rotated_session_id:
            payload["codify_clear_rotated_to"] = rotated_session_id
    elif _is_claude_branch_session_start(payload):
        payload["codify_fork_detected"] = True
        rotated_session_id = _rotate_session_on_fork(bridge_dir)
        if rotated_session_id:
            payload["codify_fork_rotated_to"] = rotated_session_id
    try:
        record_hook_event(bridge_dir, payload)
    except Exception as exc:  # noqa: BLE001 - hook must not break Claude Code.
        print(f"codify claude hook: failed to record hook: {exc}", file=sys.stderr)
    ...
    if conversation_url and payload.get("hook_event_name") == "SessionStart":
        print(json.dumps({"systemMessage": (f"Open this session in Codify: {conversation_url}")}))
    return 0
...
def _annotate_resume_session_context(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    if (
        ...
        payload["codify_previous_claude_session_id"] = current_claude_session_id
    payload["codify_claude_session_was_seen"] = new_claude_session_id in seen
...
def _is_claude_branch_session_start(payload: dict[str, object]) -> bool:
    ...
    :param payload: Hook payload read from Claude Code stdin, e.g.
        ``{"hook_event_name": "SessionStart", "source": "resume"}``.
    :returns: ``True`` when the event should fork the active Codify session.
    """
    if payload.get("hook_event_name") != "SessionStart" or payload.get("source") != "resume":
        return False
    new_claude_session_id = payload.get("session_id")
    if not isinstance(new_claude_session_id, str) or not new_claude_session_id:
        return False
    current_claude_session_id = payload.get("codify_previous_claude_session_id")
    if not isinstance(current_claude_session_id, str) or not current_claude_session_id:
        return False
    if payload.get("codify_claude_session_was_seen") is True:
        return False
...
def _rotate_session_on_clear(bridge_dir: Path) -> str | None:
    """
    Rotate Codify sessions synchronously for a Claude ``/clear`` SessionStart.

    The SessionStart hook output is what Claude renders as the welcome
    banner. Rotating here lets the banner point at the new Codify session
    ...
    :param bridge_dir: Native Claude bridge directory.
    :returns: New Codify session id, e.g. ``"conv_new"``, or ``None`` when
    ...
    try:
        ...
        print(f"codify claude clear hook: Codify rotation failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"codify claude clear hook: rotation failed: {exc}", file=sys.stderr)
        return None
...
def _rotate_session_on_fork(bridge_dir: Path) -> str | None:
    """
    Fork Codify sessions synchronously for a Claude ``/fork``/``/branch``.
    ...
    forked Codify session before Claude prints it. Failures return
    ``None`` so the background forwarder can perform the fork from the
    annotated hook record.

    :param bridge_dir: Native Claude bridge directory.
    :returns: New Codify session id, e.g. ``"conv_fork"``, or ``None``
    ...
    try:
        ...
        print(f"codify claude fork hook: Codify fork failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"codify claude fork hook: fork failed: {exc}", file=sys.stderr)
        return None
    return new_session_id
...
def _create_clear_replacement_session(
    ...
    """
    Create and activate the fresh Codify session for ``/clear``.

    :param client: Sync Codify HTTP client.
    :param ap_server_url: Codify server base URL without a trailing slash,
    ...
    :raises httpx.HTTPError: If Codify rejects session creation,
        new-session binding, or terminal transfer.
    :raises RuntimeError: If Codify returns malformed session data.
    """
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "codify claude clear hook: failed to clear old runner binding: "
...
def _create_fork_replacement_session(
    ...
    """
    Create and activate the forked Codify session for Claude ``/fork``.
    ...
    :param old_session_id: Session being forked away from, e.g.
        ``"conv_old"``.
    :param bridge_dir: Native Claude bridge directory.
    :returns: New Codify session id, e.g. ``"conv_fork"``.
    :raises httpx.HTTPError: If Codify rejects session fetch, fork,
        new-session binding, or terminal transfer.
    :raises RuntimeError: If Codify returns malformed session data.
    """
    old_resp = client.get(f"{ap_server_url}/v1/sessions/{url_component(old_session_id)}")
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "codify claude fork hook: failed to clear old runner binding: "
                f"{clear_resp.status_code} {clear_resp.text}"
            ),
            file=sys.stderr,
        )
...
def _conversation_url_for_active_session(
    ...
    """
    Build the web URL for the bridge's current active Codify session.
    ...
    if isinstance(ap_server_url, str) and ap_server_url and session_id:
        ...
        # builder so workspace-hosted servers land on the ``/codify``
...
def _post_hook_with_reattach(
    ...
    ``_codify_elicitation_id`` so the server re-parks the SAME
    ...
    * **Held-poll sever → reset the counter.** An established connection that
    ...
    :param headers: Outbound auth headers for the Codify server.
    ...
    body = {
        **payload,
        "_codify_elicitation_id": f"elicit_claude_{secrets.token_hex(16)}",
    }
    ...
    while True:
        ...
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"codify {hook_label} hook: Codify auth expired "
            ...
            if exc.response.status_code < 500:
                print(
                    f"codify {hook_label} hook: Codify request rejected: {exc}",
                    file=sys.stderr,
                )
                return None
            ...
            print(
                f"codify {hook_label} hook: Codify request failed; retrying: {exc}",
                file=sys.stderr,
            )
        except httpx.HTTPError as exc:
            ...
            print(
                (f"codify {hook_label} hook: Codify request failed ({kind}); retrying: {exc}"),
                file=sys.stderr,
            )
        if is_hard_failure:
        ...
        if consecutive_hard_failures >= _PERMISSION_MAX_CONSECUTIVE_FAILURES:
            print(
                f"codify {hook_label} hook: giving up after "
        ...
        if time.monotonic() >= deadline:
            print(
                f"codify {hook_label} hook: retry budget exhausted "
...
def _main_permission_request(argv: list[str]) -> int:
    """
    Forward one Claude ``PermissionRequest`` hook to the active Codify session.

    :param argv: CLI argv after the ``permission-request`` subcommand,
        e.g. ``["--bridge-dir", "/tmp/x", "--codify-server-url",
        "http://127.0.0.1:8787"]``.
    ...
    try:
        ...
        print(f"codify claude permission hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify claude permission hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("codify claude permission hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.codify_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("codify claude permission hook: Codify server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.codify_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
        if isinstance(raw_headers, dict):
...
def _main_ask_user_question(argv: list[str]) -> int:
    ...
    in all permission modes), POSTs the same payload to the Codify server's
    ...
    try:
        ...
        print(f"codify ask-user-question hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify ask-user-question hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("codify ask-user-question hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.codify_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("codify ask-user-question hook: Codify server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.codify_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
    ...
    if resp is None or not resp.content:
        return 0
    # The Codify server returns a PermissionRequest-shaped response:
    ...
    try:
        ...
        print(f"codify ask-user-question hook: invalid JSON from AP: {exc}", file=sys.stderr)
        return 0
    decision = (
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    ``UserPromptSubmit`` hook against Codify policies.
    ...
    :func:`codify.native_policy_hook.evaluation_response_to_hook_output`.
    ...
    ``mcp__codify__*`` tool already gated on the relay path — still
    return exit 0 with no output ("no opinion") so non-Codify tool
    ...
    try:
        ...
        print(f"codify evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("codify evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
    ...
    if not resp.content:
        print("codify evaluate-policy hook: empty Codify response", file=sys.stderr)
        return _fail_closed()

    try:
        eval_response = resp.json()
    except json.JSONDecodeError:
        print("codify evaluate-policy hook: malformed Codify response", file=sys.stderr)
        return _fail_closed()
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m codify.claude_native_hook")
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    :returns: Parsed argparse namespace.
    """
    parser = argparse.ArgumentParser(prog="python -m codify.claude_native_hook evaluate-policy")
    parser.add_argument("--bridge-dir", required=True)
    return parser.parse_args(argv)
...
def _parse_permission_args(argv: list[str]) -> argparse.Namespace:
    ...
    :returns: Parsed argparse namespace.
    """
    parser = argparse.ArgumentParser(prog="python -m codify.claude_native_hook permission-request")
    parser.add_argument("--bridge-dir", required=True)
    parser.add_argument("--codify-server-url")
    parser.add_argument("--codify-auth-headers-json")
    return parser.parse_args(argv)


def _parse_headers(raw: str | None) -> dict[str, str]:
    """
    Parse serialized Codify auth headers for the permission hook.
    ...
    try:
        ...
        print(f"codify claude permission hook: bad headers JSON: {exc}", file=sys.stderr)
        return {}
