...
CODEX_NATIVE_BRIDGE_ID_LABEL_KEY = "codify.codex_native.bridge_id"
...
# reads to reach the Codify server. Mirrors Claude-native's
...
# prep time (the Codify URL + auth do not change across thread rotations),
# whereas ``state.json`` mutates on every turn/thread change.
_POLICY_HOOK_FILE = "policy_hook.json"
_BRIDGE_ROOT = Path.home() / ".codify" / "codex-native"
...
def codex_mcp_config_overrides(
    ...
    *,
    ...
    """
    Return ``-c`` config overrides that register the Codify MCP server.

    The overrides configure codex to launch ``serve-mcp`` from
    :mod:`codify.claude_native_bridge` as a stdio MCP server.
    ...
    :returns: Codex ``-c`` config override strings, e.g.
        ``['mcp_servers.codify.command="python"', ...]``.
    """
    python = python_executable or sys.executable
    args_toml = json.dumps(
        ["-m", "codify.claude_native_bridge", "serve-mcp", "--bridge-dir", str(bridge_dir)]
    )
    return [
        f'mcp_servers.codify.command="{python}"',
        f"mcp_servers.codify.args={args_toml}",
    ]
...
def write_policy_hook_config(
    ...
    *,
    ...
    """
    Write the Codify coordinates the codex-native policy hook needs.

    The ``PreToolUse`` / ``PostToolUse`` command hook runs as a short
    subprocess that must POST to ``/v1/sessions/{id}/policies/evaluate``
    on the Codify server. It cannot inherit the long-lived forwarder's
    in-memory client, so the Codify base URL and auth headers are persisted
    here and read by :func:`read_policy_hook_config` at hook time.

    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.codify/codex-native/<hash>")``.
    :param ap_server_url: Codify server base URL the hook POSTs to, e.g.
        ``"http://127.0.0.1:8787"``.
    :param ap_auth_headers: Outbound auth headers for Codify requests, e.g.
...
def read_policy_hook_config(bridge_dir: Path) -> dict[str, object] | None:
    """
    Read the Codify coordinates for the codex-native policy hook.

    :param bridge_dir: Native Codex bridge directory.
    :returns: Parsed config, e.g.
        ``{"ap_server_url": "http://127.0.0.1:8787",
        "ap_auth_headers": {"Authorization": "Bearer <token>"}}``, or
        ``None`` when no config has been written (no Codify server
...
def read_codex_config_model(bridge_dir: Path) -> str | None:
    ...
    ``codify.inner.codex_executor``), then seeded with the session's
    launch model by ``_pin_codex_config_model`` in
    ``codify.codex_native_app_server``. An in-TUI ``/model`` writes
...
def settle_pending_mcp_startup(bridge_dir: Path) -> tuple[dict[str, dict[str, str | None]], bool]:
    ...
    connection that owns the thread — never to Codify's observer
