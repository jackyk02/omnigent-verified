...
:class:`codify.native_server_harness.NativeServerHarness` drives it through
...
from codify.native_server_transport import (
    NativeEvent,
    NativeLaunchConfig,
...
from codify.opencode_native_bridge import read_bridge_state
...
def _attachment_to_part(attachment: _JsonMapping) -> _JsonObject | None:
    """
    Convert an Codify attachment block into an OpenCode file part.
        ...
        try:
            if launch.external_session_id:
                ...
                if existing is not None:
                    return existing.id
            created = await client.create_session({"title": f"codify:{launch.codify_session_id}"})
            return created.id
        finally:
            await client.aclose()
