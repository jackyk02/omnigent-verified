...
> Code: `codify/inner/credential_proxy.py`,
> `codify/inner/egress/proxy.py`, `codify/spec/parser.py`.
...
`codify/inner/egress/`). We extend it to attach credentials. The
...
The parser (`codify/spec/parser.py`, `_parse_credential_proxy`)
...
`ValidationError` as an `CodifyError`. It then normalizes the four
...
`codify/inner/credential_proxy.py`:
...
> `CODIFY_CREDENTIAL_PROXY_GIT_HTTPS` env var are gone.

## Proxy rewrite

`codify/inner/egress/proxy.py`: `EgressProxy` takes
...
`codify/inner/egress/controller.py` threads `credential_rewrites`
...
- `codify/inner/os_env.py` — `_start_locked` builds a scoped parent
...
- `codify/inner/sandbox.py` — `credential_proxy` field on
