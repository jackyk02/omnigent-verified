...
with the running Codify web UI.
...
2. Have a workspace with the Codify web UI reachable.
3. Keep two terminals handy: the Codify server logs and (optionally) an
...
## 2a. MCP — Codify builtin relay  [needs web: model must call a sys_* tool]

This is the real "connects to Codify MCP" — opencode's model calling Codify
...
- Steps: in an opencode-native session, ask the model to do something that needs
  ...
  X skill" (`load_skill`).
- Expected:
  - opencode's `opencode.json` `mcp` block has an `codify` `{type:"local"}`
    entry whose command is `… -m codify.claude_native_bridge serve-mcp
  ...
  - The model can call the builtin and gets a real result (proxied through the
    Codify server, so policy applies — a builtin call shows up at the TOOL_CALL
...
- Expected:
  - opencode's per-session `opencode.json` contains the agent servers in the
    ...
    the `codify` relay entry, **and** `permission:{"*":"ask"}`.
  - The MCP tools are visible/callable by the model.
  - Because `permission:ask` is set, the tool call routes through the Codify
...
- Expected:
  - The Codify transcript is rehydrated as a **`noReply` context message**
...
**Codify → opencode (model switch):**
- Steps: change the model from the Codify web UI (model pill) on an
...
**Codify → opencode (clear):**
- Steps: trigger `/clear` from Codify on an opencode-native session.
...
The `codify-policy.js` plugin (loaded via `opencode.json` `plugin:[…]`) bridges
...
- **TOOL_RESULT phase** (`tool.execute.after` → `PHASE_TOOL_RESULT`):
  ...
  - Expected: the model receives `[Codify policy: tool result withheld]`
    instead of the real output (the tool already ran; its result is withheld).
- Fail-open: with the Codify server unreachable, prompts/tools still flow
