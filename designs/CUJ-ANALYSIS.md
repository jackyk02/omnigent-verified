...
Most server logic lives in the (huge) `codify/server/routes/sessions.py` + `stores/conversation_store/`.
...
- **Close / archive** — `PATCH /sessions/{id}` archived=true (owner-only); `is_session_closed()`
  (`session_lifecycle.py:70`) gates input (label `codify.closed` OR legacy title `:closed:` marker);
...
- **SDK harnesses** — in-process agent loop; Codify owns prompt + tool set + turn loop;
  user sees only the Codify WebUI; transcript is 100% Codify. Base `codify/inner/executor.py`.
...
- **Native harnesses** — drive a resident vendor CLI/TUI in a tmux pane and **mirror** its
  ...
  vendor store + mirrored. Base `codify/native_server_harness.py`; dispatch
  `cli.py:5740` (`_dispatch_native_terminal_harness`); metadata `native_coding_agents.py`.
  (in scope: **claude-native**, **codex-native**.)

CUJs:
- **Select harness at session start** — `codify <harness>` or `codify run --harness X`.
...
- **Default model / provider resolution** — chain: CLI `--model` → YAML `executor.model` → env
  (`ANTHROPIC_DEFAULT_MODEL`) → `~/.codify/config.yaml` → per-harness default. `chat.py:600`.
...
### 2.C  Tools, Codify MCP, custom MCP, shells, files, timers ✅

**Codify MCP server (the `sys_*` surface)** — exposed via the `serve-mcp` subcommand;
all tools registered in `codify/tools/manager.py`. Grouped (gating in parens):
...
React app under `web/src/` (note: renamed from `ap-web/` upstream). TUI/REPL under `codify/repl/`.
...
- **Projects (#7)** — `useProjects()` → `GET /v1/sessions/projects`; **implicit** (exist iff ≥1 session); stored as
  reserved label `omni_project`; collapsible (localStorage `codify:collapsed-sidebar-sections`); lazy
  `GET /sessions?project=`. Set at start (NewChatDialog) or kebab → Change project. Design `SESSION_PROJECTS_SIDEBAR.md`.
- **Pin / unpin (#7)** — localStorage `codify:pinned-conversation-ids`; drag-reorder; precedence
...
- **TUI / REPL equivalents** — `codify/repl/_repl.py` (`run_repl`): rich streaming, slash commands, file-mention
...
**OmniBox is *not* a web component** — it's Codify's **OS-level sandbox** (bubblewrap+seccomp / Seatbelt)
...
(credential proxy). Ref: codify-site `docs/omnibox`.
...
- **The executor (its role)** — the heart of the turn loop. `runner/app.py:post_session_events` →
  ...
  persists, forwards. `inner/executor.py:70` ExecutorConfig, `:97` event hierarchy. It translates Codify's abstract
...
- **Custom agent creation / storage (#)** — `codify create` or POST bundle. **Three tiers:** ArtifactStore
...
- **Resume dispatch** — `resume_dispatch.py:39 run_resume` reads the wrapper label → dispatches to the native harness
  (direct-id / picker / remote-server forms). ⚠️ no wrapper label → hint to use `codify run --resume`.

### 2.G  Onboarding, credentials & auth (incl. token refresh) ✅

**First-run setup** — `codify setup` (`cli.py` / `cli_config.py`): provider picker, **ambient detection**
(`onboarding/ambient.py` scans installed CLIs — Claude.app, Codex, LM Studio), saves `~/.codify/config.yaml`.
...
2. **Runner ↔ server** — `runner/_entry.py:_make_auth_token_factory` (`:271`): stored OIDC token
   (`~/.codify/auth_tokens.json`) OR Databricks OAuth via SDK; `_RunnerDatabricksAuth` refreshes per request
...
3. **Client ↔ server** — `server/auth.py:resolve_auth_source` (`:193`), `UnifiedAuthProvider` (`:250`). Three modes:
   ...
   **oidc** (auth-code+PKCE → cookie). Cookie `__Host-ap_session` (HS256, validated every request). CLI: `codify login`
...
- ⚠️ **Policy-hook path (native) — the known bug.** `runner/app.py:1137-1145` snapshots the auth token **once** into
  `policy_hook.json` (`CODIFY_POLICY_AUTH`). The native PreToolUse hook reads it and **never refreshes** → after ~1h
...
**Reasoning-effort source of truth = `codify/reasoning_effort.py`** (in-scope families):
...
- 🔴 **Idle reaper / watchdog kills active turns; native sessions never reaped.** SoT: no writers to
  `_in_flight_response_ids`, no `CODIFY_HARNESS_IDLE_TIMEOUT` knob on `main`. Issues #1414, #1349 (**no PR**),
...
- 🟠 **First-run install: Claude CLI via `npm -g` → EACCES.** Issue #890 · PR #891 (native installer). Also live,
  no PR: #904 (`codify claude` config-json crash), #1023 (`[Errno 8]` macOS arm64).
