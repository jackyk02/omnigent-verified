...
Codify supports **headless / SDK harnesses** as community plugins today (see
`designs/harness-plugin-interface.md`). A package like `codify-foo` declares a
...
`codify.runtime.harnesses._HARNESS_MODULES` and `runner/routing.py`.
...
local-server session, tails its transcript, mirrors output back into Codify,
...
- `codify/harness_plugins.py` — `NativeCodingAgent`, `HarnessContribution`
  (fields `native_harnesses`, `native_agents`), `native_agents()`,
  `native_harnesses()`.
- `codify/native_coding_agents.py` — indexes the registry rows by
  `agent_name` / `harness` / `wrapper_label` / `terminal_name`.
- `codify/_wrapper_labels.py` — the canonical wrapper-label string constants.
- `codify/harness_aliases.py` — canonicalization (`native-pi` → `pi-native`).
...
`import codify.<x>_native`. Grouped by hub:

### 1. The runner — `codify/runner/app.py` (~10.1k lines) + `codify/runner/native/orchestration.py` (~6.5k) — the epicenter

Phase 0 (#3148) moved the native *builders and mirrors* out of `app.py` into
`codify/runner/native/orchestration.py` (re-exported through
`codify/runner/native/__init__.py`), shrinking `app.py` from ~20.1k to
...
### 2. Native launch — `codify/cli.py` (~14.5k lines)
...
importing `from codify.<x>_native import run_<x>_native` and calling
`_reject_native_on_windows("<x>")` with a literal name. No registry indirection
generates these.

### 3. Resume / resume-redirect

- `codify/resume_dispatch.py:216` (`_dispatch_wrapper`) — the canonical
  11-branch `if native_agent.key == "<x>":` chain, each `import
  run_<x>_native`. Used by `codify resume`.
- `codify/chat.py:1057` (`_redirect_native_resume_if_needed`) — a parallel,
...
### 4. Built-in `*-native-ui` agent seeding — `codify/server/app.py`
...
`_materialize_<x>_agent_spec`. `codify/db/utils.py:builtin_agent_id` and
`codify/session_import/local.py` depend on the fixed built-in names.

### 5. Enumerations parallel to the registry (should *derive* from it)

- `codify/spec/_codify_compat.py:88` — `CODIFY_HARNESSES` /
  `CODIFY_HARNESS_ALIASES` frozensets re-list all native ids + `native-*`
  aliases.
- `codify/onboarding/harness_readiness.py` — per-family frozensets gating
  readiness/auth.
- `codify/onboarding/harness_install.py:219` — `_HARNESS_NAME_TO_KEY`.
- `codify/model_override.py` / `codify/model_catalog.py` — `*_FAMILY` /
  `_CURSOR_HARNESSES` frozensets.
- `codify/server/routes/sessions.py` — `_FORK_HISTORY_NATIVE_HARNESSES`,
  `_CURSOR_FORK_HISTORY_HARNESSES`, per-harness wrapper-label/model constants,
  and fork/switch gating.
- `codify/runner/resource_registry.py` — 11 `*_NATIVE_TERMINAL_ROLE`
  constants + the native-role status set.
- `codify/runtime/harnesses/__init__.py:36` — a **dead** `_HARNESS_MODULES`
...
And accessors in `codify/harness_plugins.py`:
...
A tiny resolver (new `codify/native_dispatch.py`) turns a dotted path into a
...
- **`cli.py`** ✅ (#3047) — native subcommand bodies moved into
  `codify/cli_native.py` (they already delegate to `run_<x>_native`); `cli.py`
...
- **`server/routes/sessions.py`** ✅ (#3097) — split into a facade
  ...
  (`codify/server/routes/_sessions/`: `common.py`, `helpers.py`,
...
- **`runner/app.py`** ✅ (#3148) — the native builders and bridge mirrors
  ...
  `codify/runner/native/orchestration.py` (~6.5k lines), re-exported through
  `codify/runner/native/__init__.py`; `app.py` imports them. `app.py` dropped
...
|---|---|---|---|---|---|
| **1.1 Provider model + resolver** | Add `NativeHarnessProvider` (import-path strings), the `native_providers` field + accessors, and `codify/native_dispatch.py` (lazy `importlib` resolver, cached per path). Populate 11 built-in providers pointing at existing `codify.<x>_native` functions. Purely additive — no hub rewired yet. | `harness_plugins.py`, new `native_dispatch.py` | — | Low | 1–2d |
| **1.2 Signature normalization** | Give `run_<x>_native` a uniform `extra_args` spelling with a back-compat `<x>_args` alias (one-release deprecation per CLAUDE.md — name the target release). Decide the `**extra` protocol for the four special-kwarg harnesses (claude/codex/antigravity/opencode). | 11 `codify/<x>_native.py`, `native_dispatch.py` | 1.1 | Low–Med (mechanical ×11) | 2–3d |
...
| **1.8 Derive enumerations** | Add a `fork_history: Literal["none","rebuild","preamble"]` axis to `HarnessCapabilities`; derive the §5 frozensets/dicts from `native_agents()` / capabilities (8 files, ~35 sets); delete the dead `_HARNESS_MODULES` literal. Also add optional `shell_tool_name` / `shell_tool_prompt` fields so the harness bench's tool-call probe can be driven off capabilities instead of its hardcoded `_NATIVE_TOOL_PROVOCATION` table (see "Harness bench compatibility" below). | `harness_capabilities.py`, `_codify_compat.py`, `harness_readiness.py`, `harness_install.py`, `model_override.py`, `model_catalog.py`, `_sessions/common.py`, `resource_registry.py`, `runtime/harnesses/__init__.py`, `tests/harness_bench/native_tui_driver.py`, `tests/test_harness_capabilities.py` | 1.1 | Med | 2–3d |
...
- **Runner extraction is the risk center.** The `_supervise_*_bridges` mirrors
  ...
  (`claude-native-ui:build-codify`, `pi-native-e2e-dev`, etc.).
