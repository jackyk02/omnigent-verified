...
- ✅ **Resume** — text-prefix replay from the Codify transcript (no more silent cross-host amnesia)
...
|---|---|---|
| Connects to Codify MCP | ✗ | was missing → **built**: launches the shared `serve-mcp` relay → `sys_*`/`load_skill`/`web_fetch`/comment/policy tools |
...
| Resume/fork from Codify transcript | ✗ | was missing → **built** (text-prefix replay; fork reuses it) |
...
(`codify/repl/_repl.py` `@_cmd("/compact")`). The web sends it as
...
runs its own AP-side compaction on the Codify conversation store** — which is
...
- *Precedent:* codex/claude-native expose Codify tools via a **relay** — one
  `codify` MCP server (`serve-mcp`) that proxies the active toolset; every
...
- *Native config path:* we synthesize `opencode.json`, so we write **both** the
  ...
  Codify's `TOOL_CALL` policy engine (`opencode_native_permissions.py` +
...
### 1. Compaction — **P0**
- **Current:** nothing. Auto-compaction is invisible to Codify; explicit `/compact` fakes success (see clarification 1).
- **Approach (two parts):**
...
- **Approach:** when `get_session(external_session_id)` returns `None` on a resume that *had* an id, (C) at minimum surface the failure instead of silent amnesia, then (A) rehydrate from the Codify transcript: `GET /v1/sessions/{id}/items` (mirror codex's paginated fetch) → seed a fresh opencode session via `POST /session/{id}/message` and/or the `/sync/history`/`/sync/replay` primitives. Confirm the `/sync/history` body shape against the live server before committing to it.
...
- **Current:** neither direction. Codify `/compact` (and clear/fork/resume) don't reach opencode; TUI-typed `/model`, `/compact`, `/undo` don't mirror back.
- **Approach:** Codify→opencode via `POST /session/{id}/command` (the matrix's "clear/fork/resume/switch"); the `/compact` half is covered by #1. opencode→Codify: handle `command.executed` (+ mirror `/model` to `model_override`, surface `/compact`/`/undo` as `slash_command` items). Overlaps #1/#3/#5; do last.
