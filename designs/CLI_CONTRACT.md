...
So `codify version | cat`, `codify config list | jq`, and piped
...
`codify.inner.mascots.MASCOT_ART_COLOR`, re-exported as `ui.ACCENT`.
The `scripts/install_oss.sh` installer mirrors the same accent
(`\033[38;2;244;59;166m`) so the installer and the tool agree.

## Helper API (`codify.inner.ui`)

Status lines — consistent glyph + color, correct stream:

```python
ui.step("Installing Codify")     # ==>  accent   (stdout)
ui.success("Verified codify")    #  ✓   green    (stdout)
ui.info("Using ~/.codify")       #  ·   dim      (stdout)
...
no-op off a TTY or when `CODIFY_NO_BANNER` is set):

- **Full lockup** — `ui.print_landing(...)` — Otto + wordmark, optional
  gradient / tagline / epilogue. The hero moment, reserved for the few
  landing surfaces:
  - `codify --help` (the top-level group, via `_CodifyCLI.format_help`)
  - `codify setup` (first-run experience)
...
`✦ codify`) exists for opt-in use, but is intentionally **not** wired
onto any command today — add it only if a specific surface clearly wants
light branding.

The bare `codify` invocation on a TTY launches the REPL (its own
...
| `CODIFY_NO_BANNER` truthy  | No banner/brandmark even on a TTY          |
| `CODIFY_NO_SPINNER` truthy | No startup spinner (pre-existing)          |

## Adding a new command — checklist

1. `from codify.inner import ui` (import lazily inside the command body
