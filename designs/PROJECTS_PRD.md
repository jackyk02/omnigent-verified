...
- Status: Draft
- Author: Serena Ruan
- Related: [`designs/SESSION_PROJECTS_SIDEBAR.md`](./SESSION_PROJECTS_SIDEBAR.md) (v1, label-based sidebar grouping), issue [#863](https://github.com/codify-ai/codify/issues/863), PR [#869](https://github.com/codify-ai/codify/pull/869)
...
- **Session = `Conversation`** (`codify/entities/conversation.py`). Key fields:
...
- **Session creation** goes through `POST /v1/sessions`
  (`SessionCreateRequest`, `codify/server/schemas.py`) with `agent_id`,
...
- **Working directory & host**: `workspace` is a path on a `host` (a machine
  running `codify host`, or a server-managed sandbox). Bound at creation,
...
- **Permissions**: per-`(user_id, conversation_id)` rows with levels
  read(1)/edit(2)/manage(3)/owner(4) (`codify/db/db_models.py`,
  `codify/entities/permission.py`). `__public__` sentinel grants public read.
...
migration `z6…`, `codify/db/db_models.py`): `workspace_id` leads the PK, **no
...
(`SqlConversationMetadata`, `codify_conversation_metadata`):
...
| Membership location | `project_id` on `codify_conversation_metadata` | Metadata already holds host/workspace/runner; `list_conversations` can filter it inline. |
...
indexes; `op.add_column("codify_conversation_metadata", project_id)` + its
...
- ✅ **Latency journeys + corpus seeder.** Added `list_projects` (sidebar project
  ...
  latency journeys to `dev/benchmarks/codify`, mirroring the `list_sessions`
