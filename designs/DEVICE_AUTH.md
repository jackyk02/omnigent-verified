...
> **optional** shared secret (`CODIFY_DEVICE_CLIENT_SECRET`) that gates the
> client-facing endpoints when set — see the phishing mitigations below.
>
> Server: `codify/server/routes/device_auth.py` (endpoints + the
> `mint_delegated_token` / `DELEGATED_SCOPE` it owns),
> `codify/server/device_grant_store.py`, `SqlDeviceGrant` +
> `device_grants` migration (`d1e2f3a4b5c6`), and scope + revocation
> enforcement in `codify/server/auth.py` (`delegated_path_allowed`,
> `set_grant_revocation_check`). Wired in `codify/server/app.py`,
> **opt-in and default-off** via `CODIFY_DEVICE_GRANT_ENABLED` (the
...
> Slack: `integrations/slack/src/codify_slack/oauth.py`,
> `tokens.py` (Fernet-encrypted `oauth_tokens`), `auth_manager.py`, plus
> the bearer/refresh wiring in `codify.py` (`ClientAuth`,
> per-`(server,user)` pool). Login is folded into the `/codify` setup
> modal; `/codify logout` revokes + clears.
>
...
process that calls each user's Codify server over HTTP + SSE
(`CodifyClient` / `CodifyClientPool`). Each Slack user's turns must reach
the Codify server **as that user's own authenticated identity** — so the
server can scope permissions and audit who did what — **without the Slack
process ever handling the user's Codify credentials**. An unauthenticated
...
```
  codify server   <->   slack socket server   <->   slack.com   <->   user
...
Codify credential may pass through Slack**. The user authenticates directly
against the Codify server in their own browser, out of band. This is exactly
...
| RFC 8628 role            | Here                                            |
|--------------------------|-------------------------------------------------|
| Authorization Server     | Codify server (`/oauth/device/*`, `/oauth/token`) |
| Resource Server          | Codify server (existing `/v1/**` APIs)        |
...
1. **Public by default, optional client secret.** The baseline boundary is
   ...
   that, setting `CODIFY_DEVICE_CLIENT_SECRET` on the server gates the
   **client-facing** endpoints (authorize / token / revoke) behind a shared
   secret header (`X-Codify-Client-Secret`, constant-time compared), so
   ...
   (`CODIFY_SERVER_URL`), not a user-supplied URL, so the secret only ever
...
```
 ...
 4. User clicks → Codify consent page (verification_uri).
       ...
       permission to act as YOU (alice@example.com) on this Codify server.
 ...
 6. Slack server polls ─ POST /oauth/token ──────────────────▶ Codify
...
knows only the RFC 8628 `client_id` and the Codify identity that
approved it.

## Server-side changes

### Router `codify/server/routes/device_auth.py`

Mounted in `app.py` only when **`CODIFY_DEVICE_GRANT_ENABLED` is truthy**
...
### New store `codify/server/device_grant_store.py`
...
| `user_id`            | bound Codify identity, set at approval           |
...
- **`codify.py`** — attach `Authorization: Bearer` per
  `(server_url, slack_user_id)`; on 401, refresh once and retry; on refresh
  failure, surface a re-login prompt. `CodifyClientPool` keys clients by
...
- **`store.py`** — new `oauth_tokens` table `(team_id, user_id, server_url)` →
  ...
  the DB). `/codify logout` → `POST /oauth/revoke` + local delete.
...
Codify user into approving the verification link, binding the grant to the
...
- **Startup warning.** When the grant is mounted on a multi-user (accounts)
  server with `CODIFY_DEVICE_CLIENT_SECRET` unset, the server logs a loud
...
Setting `CODIFY_DEVICE_CLIENT_SECRET` closes initiation entirely to
unauthorized callers: without the matching `X-Codify-Client-Secret` header,
...
Ordinary Codify session JWTs are stateless and unrevocable today (revocation =
