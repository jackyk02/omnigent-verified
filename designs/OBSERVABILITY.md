# Holistic Distributed Tracing for Codify

**Status:** Proposed
**Scope:** End-to-end visibility into all data flowing between Codify's distributed
...
Codify is a distributed, multi-process system (host daemon, runners/harnesses,
...
Today there is a partial telemetry layer (`codify/runtime/telemetry.py`) built on
...
- **Browser-origin propagation (implemented):** `web/src/lib/telemetry.ts`
  ...
  **opt-in by configuration** — active only when `VITE_OTEL_EXPORTER_OTLP_ENDPOINT` is
  ...
  Service name `omni-web` (override via `VITE_OTEL_SERVICE_NAME`). **CORS:** Codify is a
...
- REST + SSE via `CodifyClient`'s single `httpx.AsyncClient` (`_client.py:89`). Wiring
...
Build on the existing `codify/runtime/telemetry.py` `init()`; it already establishes a
...
2. **Default `FastAPIInstrumentor` on** for both the server and runner apps (currently
   gated behind `CODIFY_OTEL_FASTAPI_INSTRUMENTATION`; the remote-parent span patch in
...
|---|---|---|
| `CODIFY_TELEMETRY_ENABLED` | `true` | **Master opt-in; off by default.** When unset/false, `init()` is a no-op and no instrumentor installs — zero telemetry cost. Required for any row below to take effect. |
...
| `CODIFY_OTEL_FASTAPI_INSTRUMENTATION` | `true` | Server/runner HTTP spans + extract |
| `CODIFY_OTEL_CAPTURE_CONTENT` | `true` (dev only) | Include payloads on spans; **off in prod** (PII) |

Telemetry is **opt-in**: nothing is instrumented and no spans are created unless
`CODIFY_TELEMETRY_ENABLED` is truthy, so a default install is never burdened with
...
tagged with the Codify session (conversation) id (`conv_…`) under the `session.id`
...
`CODIFY_OTEL_CAPTURE_CONTENT=true`. This wires `should_capture_content()` (previously a
dormant flag) into the boundaries Codify controls:

- **Host-tunnel frames** — inbound on the consumer span (`consume_frame_span`) and
  outbound at `encode_host_frame`; recorded as `codify.message.payload`.
...
2. `export CODIFY_TELEMETRY_ENABLED=true` (master opt-in), then
   `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317`,
   `CODIFY_OTEL_FASTAPI_INSTRUMENTATION=true`, `CODIFY_OTEL_CAPTURE_CONTENT=true`.
...
A headless turn (`codify run --harness claude-sdk -p "…"`) against a local Jaeger
...
- **PII / secrets.** `CODIFY_OTEL_CAPTURE_CONTENT` must remain **off** outside dev;
...
- **Browser-side propagation (implemented).** OTel web SDK with fetch/XHR
  ...
  (`:4318`). Codify is same-origin with no `CORSMiddleware`, so same-origin `traceparent`
