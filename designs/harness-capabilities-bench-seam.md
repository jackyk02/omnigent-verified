...
|---|---|---|---|
| **Static** (use this) | `codify.harness_plugins.harness_capabilities()` → `dict[str, HarnessCapabilities]` | *Declared* trait/claim, pre-spawn | Immediately, no subprocess |
| Runtime | `codify.inner.executor.Executor.supports_streaming()` / `interrupt_session()` … | *Actual* in-subprocess behavior | Only after spawn |
...
- Type: `codify/harness_capabilities.py` → `HarnessCapabilities` (frozen
...
| `auth` | `auth` | `CODIFY_CREDENTIAL` / `OWN_AUTH` / `SESSION_SCOPED_CONFIG`. The old free-text ("Anthropic key / Databricks gateway") is richer prose; keep a small enum→string map if you want the exact wording, or simplify. |
...
1. Add the field to `HarnessCapabilities` (+ `as_dict()`), in
   `codify/harness_capabilities.py`.
