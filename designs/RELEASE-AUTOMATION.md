...
| Lockstep bump (3 `pyproject.toml` + `codify/version.py` + `uv.lock`) | `scripts/update_versions.py` (+ `bump-version.yml` wrapper) | ✅ exists, but human-invoked; RELEASING.md still says "hand-edit `uv.lock`" (CI `uv lock` has no proxy problem) |
...
| Secure-repo gates + PyPI publish | manual `gh workflow run codify.yml` ×2–3 (dry-run, [test-pypi], pypi) in `databricks/secure-public-registry-releases-eng` | ❌ manual dispatches |
...
| Homebrew formula bump (`codify-ai/homebrew-tap`) | nobody — tap frozen at **0.2.0** while PyPI is at 0.5.1 | ❌ missing |
...
rc:    dispatch release.yml (version=0.6.0rc1)      # cut/bump/tag — one run
       dispatch secure codify.yml (ref=v0.6.0rc1) # gates → [approve] → publish → validate
final: dispatch release.yml (version=0.6.0)
       dispatch secure codify.yml (ref=v0.6.0)
...
`codify-ai/codify` — weakening the isolation for the sake of one saved
click. Rejected.

## Workflow 1 — `release.yml` (new, codify-ai/codify)
...
1. **plan** (always): validate version shape (reuse `bump-version.yml`'s PEP 440
   ...
   step summary.
2. **execute** (`dry_run == false`): mint the codify-ci App token; create
...
3. **bump-main** (only when the branch was created in this run, i.e. rc1):
   ...
   push to the codify-ci App token (falling back to `GITHUB_TOKEN` where the
...
## Workflow 2 — secure repo `codify.yml` restructure
...
3. **validate**: `needs: publish`. Clean venv; poll the real index until all
   ...
   codify==X codify-client==X codify-ui-sdk==X` (exact rc pins resolve
   without `--pre`); assert `codify --version` == X; import smoke. Replaces
...
PEP 440 prerelease, which default `pip install codify` never resolves — safer
...
1. **checks** (all fail with actionable links):
   ...
   - **docs sweep**: zero open PRs in `codify-site` with base `X.Y-docs` —
...
Current state of `codify-ai/homebrew-tap`: a homebrew-core-style tap that is
already 2/3 automated —

- `Formula/codify.rb`: `Language::Python::Virtualenv` formula; stable
...
`codify-desktop` cask needs nothing: it is `version :latest` /
`sha256 :no_check` against `codify.ai/download/mac`, i.e. evergreen.

New workflow in codify-ai/codify, shaped exactly like
...
- Steps: bounded-poll the PyPI JSON API until the new sdist is visible; on a
  ...
  `brew style`/`brew audit` as a sanity gate; push `bump-codify-<version>`
...
delete restricted to the codify-ci App + admins — even a bypassed workflow
...
- **Delete `release-codify.yml`** — its own header says "to be deleted once
...
- Bot pushes via a **GitHub App token** (`create-github-app-token`), never a
  PAT — same identity pattern as our codify-ci App.
...
- **Neither peer automates post-publish validation** (clean-env install of
  ...
  puts codify ahead of both, not just at parity.
...
- **Gate placement**: the named-required-reviewer GitHub Environment guards
  *only* the VS Code Marketplace publish (highest blast radius); CLI/SDK get a
  ...
  second-person gate on the irreversible step only — for codify, that is the
...
**Considered variant for codify** (from the JetBrains pattern): have
...
- codify's mandatory scan-gates-publish + separate-org publisher is
...
- **Tag ruleset on `v[0-9]*`**: restrict create/update/delete to maintainers +
  the codify-ci App. Today any write-access account can push a version tag
