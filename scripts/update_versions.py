"""
Bump the codify project version across all packages in lockstep.

The four distributions in this repo release together at a single
version:

- ``codify``         — root ``pyproject.toml``
- ``codify-client``  — ``sdks/python-client/pyproject.toml``
- ``codify-ui-sdk``  — ``sdks/ui/pyproject.toml``
- ``codify-slack``   — ``integrations/slack/pyproject.toml``

Each declares its own ``[project].version``. The first three ``==``-pin
their siblings in ``[project].dependencies``; the root ``codify``
package also ``==``-pins ``codify-slack`` in the ``slack`` optional
dependency extra — the lockstep contract that
``.github/workflows/release-codify.yml`` verifies at tag time. This
...
class Package:
    ...
    :param name: Distribution name, e.g. ``"codify"``.
    ...
    :param sibling_pins: Sibling distribution names this package
        ``==``-pins, e.g. ``("codify-client", "codify-ui-sdk")``.
...
def packages(root: Path) -> list[Package]:
    ...
    return [
        Package(
            "codify",
            root / "pyproject.toml",
            ("codify-client", "codify-ui-sdk", "codify-slack"),
        ),
        Package(
            "codify-client",
        ...
        # codify-slack is deliberately decoupled from codify core (it
        # drives the server over HTTP, never imports ``codify``), so it
        # pins no siblings. The root ``codify`` package ``==``-pins it in
...
# ``codify/version.py`` that mirrors the canonical [project].version.
...
def _pin_pattern(name: str) -> re.Pattern[str]:
    ...
    :param name: Distribution name to match, e.g. ``"codify-client"``.
...
def set_version(root: Path, new_version: str) -> list[Path]:
    ...
    Also rewrites the runtime ``VERSION`` constant in ``codify/version.py``
...
def _read_version_constant(root: Path) -> str:
    """
    Return the ``VERSION`` literal from ``codify/version.py``.
...
def check(root: Path, expect: str | None = None) -> str:
    ...
    Also checks the runtime ``VERSION`` constant in ``codify/version.py``
    ...
    for pkg in packages(root):
        ...
        # root ``codify`` package pins ``codify-slack`` in the ``slack``
    ...
    if Version(constant) != Version(resolved):
        raise ValueError(
            f"codify/version.py VERSION {constant!r} != [project].version {resolved!r}"
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="Bump codify package versions in lockstep")
