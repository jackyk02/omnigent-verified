"""Generate and post-process the codify OpenAPI 3.2 document.

The codify server runs on FastAPI 0.135.x, which emits OpenAPI
...
1. Imports :func:`codify.server.app.create_app` and instantiates
...
# (rendered by Scalar on the codify website) is usable for building
...
# Self-hosted base URL. ``codify server`` binds 127.0.0.1:6767 by
# default (see ``_DEFAULT_LOCAL_PORT`` in
# ``codify/host/local_server.py``).
_SERVERS: list[dict[str, str]] = [
    {
        "url": "http://127.0.0.1:6767",
        "description": "Self-hosted Codify server (default local port).",
    },
]
...
Codify is an open-source meta-harness for building and running AI \
agents. This is the REST API exposed by the Codify server: use it to \
...
Codify is self-hosted. The server binds `http://127.0.0.1:6767` by \
default (`codify server`); point the base URL at your own deployment.

## Authentication

There is no API-key or bearer-token scheme. Identity is supplied by the \
deployment's configured auth provider (`CODIFY_AUTH_PROVIDER`):
...
# Auth representations. Codify has no bearer/API-key scheme — identity
# arrives via a trusted-proxy header or a signed session cookie,
# selected by ``CODIFY_AUTH_PROVIDER``. We model both as OpenAPI
