...
(``codify/api/routing/v1/routing.proto``); the runtime imports the generated
``codify.api.routing.v1.routing_pb2`` module. Generated
...
# the generated package path is ``codify/api/routing/v1/routing_pb2`` —
# matching the import the runtime uses (and the proto's ``package`` line).
_REPO_ROOT = Path(__file__).resolve().parent.parent
_PROTO = Path("codify/api/routing/v1/routing.proto")
# The two files protoc emits for the schema, relative to the repo root.
_OUTPUTS = (
    Path("codify/api/routing/v1/routing_pb2.py"),
    Path("codify/api/routing/v1/routing_pb2.pyi"),
)
