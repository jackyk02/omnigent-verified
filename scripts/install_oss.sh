...
# Codify installer.
#
# Installs the published `codify` wheel from PyPI with uv, wires up PATH,
...
init_style() {
  if use_terminal_ui && [ -z "${NO_COLOR:-}" ]; then
    ...
    # palette in codify/inner/ui.py so the installer and the tool agree.
...
# The Otto + "codify" wordmark lockup, printed once at the top of an
# interactive install. Mirrors codify.inner.wordmark.lockup_lines(); the
...
run_with_spinner() {
  ...
  log_file="${TMPDIR:-/tmp}/codify-oss-installer.$$.log"
  status_file="${TMPDIR:-/tmp}/codify-oss-installer.$$.status"
...
ensure_uv() {
  if command -v uv >/dev/null 2>&1; then
    CODIFY_LEDGER_DEP_UV=preexisting
    export CODIFY_LEDGER_DEP_UV
...
check_node() {
  ...
  CODIFY_LEDGER_DEP_NODE=preexisting
  export CODIFY_LEDGER_DEP_NODE
...
check_npm() {
  if command -v npm >/dev/null 2>&1; then
    CODIFY_LEDGER_DEP_NPM=preexisting
    export CODIFY_LEDGER_DEP_NPM
...
# `codify claude` / `codify codex` launch through a local tmux terminal
...
check_tmux() {
  if command -v tmux >/dev/null 2>&1; then
    CODIFY_LEDGER_DEP_TMUX=preexisting
    export CODIFY_LEDGER_DEP_TMUX
  ...
  case "$(uname -s)" in
    Darwin)
      if command -v brew >/dev/null 2>&1; then
        if prompt_yes_no "tmux is missing (needed for \`codify claude\` / \`codify codex\`). Install it with brew?"; then
          run_with_spinner "brew install tmux" brew install tmux || warn "brew install tmux failed — install tmux manually before \`codify claude\`."
          return
        fi
      fi
      warn "tmux not found — \`codify claude\` / \`codify codex\` need it. Install with: brew install tmux"
      ;;
    Linux)
      install_cmd="$(linux_pkg_install_cmd tmux)"
      if [ -n "$install_cmd" ] && prompt_yes_no "tmux is missing (needed for \`codify claude\` / \`codify codex\`). Install it now ($install_cmd)?"; then
        # Run directly (not via run_with_spinner) so sudo can prompt for a password.
        sh -c "$install_cmd" || warn "tmux install failed — run manually: $install_cmd"
        if command -v tmux >/dev/null 2>&1; then
          CODIFY_LEDGER_DEP_TMUX=codify
          export CODIFY_LEDGER_DEP_TMUX
          step "tmux installed"
        fi
        return
      ...
      if [ -n "$install_cmd" ]; then
        warn "tmux not found — \`codify claude\` / \`codify codex\` need it. Install with: $install_cmd"
      else
        warn "tmux not found — \`codify claude\` / \`codify codex\` need it. Install it with your package manager."
      fi
      ;;
  esac
}

# The native `codify claude` / `codify codex` / `pi` harnesses wrap each
...
check_bubblewrap() {
  ...
  if command -v bwrap >/dev/null 2>&1; then
    CODIFY_LEDGER_DEP_BWRAP=preexisting
    export CODIFY_LEDGER_DEP_BWRAP
    step "bubblewrap (bwrap) is available"
    return
  fi

  install_cmd="$(linux_pkg_install_cmd bubblewrap)"
  if [ -n "$install_cmd" ] && prompt_yes_no "bubblewrap is missing (needed to sandbox native \`codify claude\` / \`codify codex\` terminals). Install it now ($install_cmd)?"; then
    run_with_spinner "install bubblewrap" sh -c "$install_cmd" || warn "bubblewrap install failed — run manually: $install_cmd"
    if command -v bwrap >/dev/null 2>&1; then
      CODIFY_LEDGER_DEP_BWRAP=codify
      export CODIFY_LEDGER_DEP_BWRAP
    fi
    return
  fi
  if [ -n "$install_cmd" ]; then
    warn "bubblewrap (bwrap) not found — native \`codify claude\` / \`codify codex\` terminals need it on Linux. Install with: $install_cmd"
  else
    warn "bubblewrap (bwrap) not found — native \`codify claude\` / \`codify codex\` terminals need it on Linux. Install it with your package manager."
  fi
}

install_codify() {
  # Default: the published PyPI wheel (`codify`, optionally `codify==X`).
  ...
  if building_from_source; then
    ...
    # "codify[databricks] @ git+https://...". Without extras, keep the bare
    ...
    step "Installing Codify from source${extras_suffix:+ $extras_suffix} (Python $PYTHON_VERSION)"
  elif [ -n "$VERSION" ]; then
    target="${PACKAGE_NAME}${extras_suffix}==${VERSION}"
    step "Installing Codify $VERSION${extras_suffix:+ $extras_suffix} (Python $PYTHON_VERSION)"
  else
    target="${PACKAGE_NAME}${extras_suffix}"
    step "Installing Codify${extras_suffix:+ $extras_suffix} (Python $PYTHON_VERSION)"
  fi
...
maybe_add_bin_to_path() {
  ...
  begin_marker="# >>> Codify installer >>>"
  end_marker="# <<< Codify installer <<<"
  ...
  if [ -f "$profile" ] && grep -F "$begin_marker" "$profile" >/dev/null 2>&1; then
    ...
    fail "$profile already has an Codify installer block. Update it manually to: $path_line"
  fi
...
write_install_ledger() {
  bin_dir="$1"
  cli_path="$bin_dir/codify"
  if [ ! -x "$cli_path" ]; then
    cli_path="$(command -v codify 2>/dev/null || true)"
  fi
  if [ -z "$cli_path" ]; then
    warn "Could not write install ledger: codify command not found."
    return 0
  fi
  if [ -n "$LEDGER_PROFILE" ]; then
    CODIFY_LEDGER_PROFILE="$LEDGER_PROFILE"
    export CODIFY_LEDGER_PROFILE
  fi
...
verify_codify() {
  ...
  if [ -z "$cli_path" ]; then
    fail "Codify installed, but the codify command was not found."
  fi

  "$cli_path" --help >/dev/null
  step "Verified $cli_path"

  # `omni` is a shorthand alias installed alongside `codify`; check it so a
  ...
  for alias_cmd in omni; do
    if [ ! -x "$bin_dir/$alias_cmd" ] && ! command -v "$alias_cmd" >/dev/null 2>&1; then
      warn "the $alias_cmd alias was not installed (expected a console-script entry point alongside codify)."
    fi
  done
}

# No setup step here by design: the first `codify` run configures a model
...
print_next_steps() {
  ...
  printf '\n%sCodify installed successfully.%s\n\n' "$BOLD" "$RESET"
  ...
  printf '  %scodify claude          # Claude Code\n' "$command_prefix"
  printf '  %scodify codex           # Codex\n\n' "$command_prefix"
  printf 'Manage model credentials any time:\n'
  printf '  %scodify setup\n\n' "$command_prefix"
  printf 'Uninstall the CLI but keep local history and credentials:\n'
  printf '  %scodify uninstall --yes\n\n' "$command_prefix"
  ...
  printf 'and add it via: codify setup -> Databricks.%s\n' "$RESET"
}
