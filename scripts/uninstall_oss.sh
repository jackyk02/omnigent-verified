...
# Codify uninstaller. POSIX sh by design so it still works when the Python
# wheel is wedged or PATH is broken.

set -u

MARKER_BEGIN="# >>> Codify installer >>>"
MARKER_END="# <<< Codify installer <<<"
TAB=$(printf '\t')
TARGETS=""
DRY_RUN=false
YES=false
...
REPORTED=0
EXIT_CODE=0
ACTIONS_FILE="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-actions.XXXXXX")" || exit 1
BACKUPS_FILE="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-backups.XXXXXX")" || exit 1
...
Flags:
  ...
  --purge-workspace          With --purge, also remove ~/codify non-interactively
...
stop_processes() {
  home_dir="$(state_home)"
  pidfiles="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-pidfiles.XXXXXX")" || return 1
  : >"$pidfiles"
  if [ -d "$home_dir" ]; then
  ...
  if command -v tmux >/dev/null 2>&1; then
    sessions_file="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-tmux.XXXXXX")" || return 1
...
unload_launch_agents() {
  [ -n "${CODIFY_UNINSTALL_LEDGER_MANIFEST:-}" ] && [ -f "$CODIFY_UNINSTALL_LEDGER_MANIFEST" ] || return 0
  ...
  done <"$CODIFY_UNINSTALL_LEDGER_MANIFEST"
}
...
has_shell_install_signal() {
  [ -n "${CODIFY_UNINSTALL_LEDGER_SOURCE:-}" ] && [ "$CODIFY_UNINSTALL_LEDGER_SOURCE" != unknown ] && return 0
  [ -f "$(state_home)/installation_id" ] && return 0
  command -v codify >/dev/null 2>&1 && return 0
  command -v omni >/dev/null 2>&1 && return 0
  profiles_file="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-anchor-profiles.XXXXXX")" || return 1
...
remove_profile_block() {
  ...
  if [ -n "$expected_sha" ]; then
    block_file="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-block.XXXXXX")" || return 1
  ...
  backup="$profile.codify.bak.$(date -u +%Y%m%dT%H%M%SZ)"
  tmp="$(mktemp "$profile.codify.tmp.XXXXXX")" || return 1
...
cleanup_profiles() {
  if [ -n "${CODIFY_UNINSTALL_LEDGER_MANIFEST:-}" ] && [ -f "$CODIFY_UNINSTALL_LEDGER_MANIFEST" ]; then
    ...
    profiles_file="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-profiles.XXXXXX")" || return 1
    profile_candidates >"$profiles_file"
    while IFS= read -r profile; do
      [ -n "$profile" ] || continue
      if profile_has_block "$profile"; then
...
remove_delimited_external_config() {
  ...
  backup="$path.codify.bak.$(date -u +%Y%m%dT%H%M%SZ)"
  tmp="$(mktemp "$path.codify.tmp.XXXXXX")" || return 1
...
archive_path_for() {
  ...
  if [ -n "${XDG_STATE_HOME:-}" ]; then
    backup_root="$XDG_STATE_HOME/codify-backups"
  else
    backup_root="$HOME/.codify-backups"
  fi
...
desktop_paths() {
  case "$(uname -s)" in
    Darwin)
      printf '%s\n' \
        "$HOME/Library/Application Support/Codify" \
        "$HOME/Library/Caches/Codify" \
        "$HOME/Library/Logs/Codify"
      ;;
    *)
      printf '%s\n' \
        "${XDG_CONFIG_HOME:-$HOME/.config}/Codify" \
        "${XDG_CACHE_HOME:-$HOME/.cache}/Codify" \
        "${XDG_STATE_HOME:-$HOME/.local/state}/Codify"
      ;;
  esac
}
...
purge_desktop_data() {
  desktop_file="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-desktop.XXXXXX")" || return 1
...
uninstall_wheel() {
  if [ "$DRY_RUN" = true ]; then
    record_action wheel codify remove reported "" "would run uv tool uninstall codify"
    return 0
  fi
  if ! command -v uv >/dev/null 2>&1; then
    if ! command -v codify >/dev/null 2>&1 && ! command -v omni >/dev/null 2>&1; then
      record_action wheel codify remove skipped "" "already absent"
      return 0
    fi
    record_action wheel codify remove failed "" "uv not found; remove the tool manually"
    return 1
  fi
  uv_output="$(mktemp "${TMPDIR:-/tmp}/codify-uninstall-uv.XXXXXX")" || return 1
  if uv tool uninstall codify >"$uv_output" 2>&1; then
    rm -f "$uv_output"
    record_action wheel codify remove done "" "uv tool uninstall codify"
  else
    ...
    case "$output" in
      *'not installed'* | *'No tool'* | *'not found'*)
        ...
        ;;
      *)
        record_action wheel codify remove failed "" "uv tool uninstall failed: $output"
        ;;
    esac
  fi
}
...
emit_json() {
  ...
  printf '  "dry_run": %s,\n' "$DRY_RUN"
  printf '  "ledger_source": "%s",\n' "${CODIFY_UNINSTALL_LEDGER_SOURCE:-unknown}"
  printf '  "actions": [\n'
  first=true
...
if ! has_shell_install_signal; then
  record_action anchor codify detect failed "" "no Codify install detected"
  EXIT_CODE=3
  [ "$JSON" = true ] && emit_json
  exit "$EXIT_CODE"
fi
