"""Sync ``codify/version.py``'s ``VERSION`` to the canonical pyproject version.
...
``codify.version.VERSION`` — a plain constant it can import without touching
...
_VERSION_PY = _REPO_ROOT / "codify" / "version.py"

# The ``VERSION = "..."`` assignment (its own line) in codify/version.py.
...
def _current_constant(text: str) -> str:
    ...
    :param text: Contents of ``codify/version.py``.
