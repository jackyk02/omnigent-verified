...
# so the run never touches your real ~/.codify, ~/.config / ~/Library, or
# package caches.
#
# Usage:
...
runtime_root="$(mktemp -d "${TMPDIR:-/tmp}/codify_backend_XXXXXX")"
...
runtime_db="$runtime_data/codify/chat.db"
server_pid=""
...
mkdir -p \
  ...
  "$(dirname "$runtime_db")" \
  "$runtime_config/codify" "$runtime_data/codify"
...
# UV_/PIP_/CODIFY_ overrides pin the toolchain and app state regardless of
# OS. XDG_* are set so an XDG var already exported in the caller's shell can't
# redirect state back to the real home. CODIFY_SKIP_WEB_UI leaves the server
...
env_vars=(
  ...
  "CODIFY_CONFIG_HOME=$runtime_config/codify"
  "CODIFY_DATA_DIR=$runtime_data/codify"
  "CODIFY_DATABASE_URI=sqlite:///$runtime_db"
...
nohup env "${env_vars[@]}" \
  "$project_venv/bin/codify" server \
    --host 127.0.0.1 \
    --port "$PORT" \
    --database-uri "sqlite:///$runtime_db" \
    --artifact-location "$runtime_artifacts" \
  < /dev/null > "$runtime_logs/codify_server.log" 2>&1 &
server_pid="$!"
...
if [ -z "$ready" ]; then
  ...
  tail -n 40 "$runtime_logs/codify_server.log" >&2 || true
  exit 1
fi
