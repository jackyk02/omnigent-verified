...
When test code writes::
    ...
    with patch("codify.tools.mcp.asyncio.sleep", new_callable=AsyncMock):
        ...

``unittest.mock``'s dotted-path resolver walks:

1. ``import codify.tools.mcp``
2. ``getattr(codify.tools.mcp, "asyncio")`` -> returns the
...
def _attribute_ends_in_asyncio(node: ast.expr) -> bool:
    ...
    e.g. matches ``mcp_mod.asyncio`` and ``codify.tools.mcp.asyncio``.
    """
