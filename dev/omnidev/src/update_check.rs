//! Daily update check for a git-installed codify.
//!
//! Fills a real gap: codify's own update notice only works for PyPI-wheel
...
pub fn check(quiet: bool) -> Result<()> {
    ...
    if prompt_yes_no("Update codify now? [y/N] ") {
        install::update()?;
    } else {
