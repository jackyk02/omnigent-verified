//! Manage the user's git-based codify installation via `uv tool install`.
//!
...
pub const DEFAULT_REPO: &str = "https://github.com/codify-ai/codify.git";
pub const DEFAULT_REF: &str = "main";
pub const DEFAULT_EXTRA: &str = "databricks";
const PYTHON_VERSION: &str = "3.12";

/// Durable record of how the user wants codify installed. Persisted so
...
impl InstallConfig {
    ...
    /// `codify[databricks] @ git+https://github.com/codify-ai/codify.git@main`.
    ...
    pub fn spec(&self) -> String {
        ...
        if self.extras.is_empty() {
            ...
            format!("codify[{}] @ {}", self.extras.join(","), source)
        }
    }
}
...
fn preflight() -> Result<()> {
    ...
    if which("pnpm").is_none() {
        bail!(
            "`pnpm` is not on PATH. Installing codify from git builds the web UI \
...
/// Install codify from git per `config`. `reinstall` forces uv past its cache
...
pub fn install(config: &InstallConfig) -> Result<()> {
    ...
    println!("omnidev: installed codify ({})", config.spec());
    Ok(())
}
...
pub fn update() -> Result<()> {
    ...
    record_installed_sha(&config);
    println!("omnidev: updated codify ({})", config.spec());
    Ok(())
}
...
/// Read the commit the installed codify tool was built from, via its PEP 610
...
pub fn installed_commit() -> Option<String> {
    let dir = uv_tool_dir()?;
    // …/codify/**/codify-*.dist-info/direct_url.json
    let codify_root = dir.join("codify");
    let dist_info = find_dist_info(&codify_root)?;
...
/// Find the `codify-*.dist-info` dir under a uv tool's environment. uv lays
...
fn find_dist_info(root: &std::path::Path) -> Option<PathBuf> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if name.starts_with("codify-") && name.ends_with(".dist-info") {
                return Some(path);
            }
            stack.push(path);
        }
