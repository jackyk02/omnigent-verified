//! omnidev — dev tooling for Codify.
//!
...
//!   keep a git-based codify up to date. These need no checkout and run
//!   anywhere.
//! - **codify passthrough** (`omnidev codify …`): run any codify command
//!   against the current checkout's pod via `uv run codify …`, with the pod's
...
#[derive(Parser, Debug)]
#[command(name = "omnidev", about = "Dev tooling for Codify", version)]
struct Args {
    #[command(subcommand)]
...
/// Top-level subcommands. Install management works anywhere; the `codify`
/// passthrough requires a checkout (like the bare supervisor default).
#[derive(Subcommand, Debug)]
enum Command {
    /// Install codify from git (defaults to the databricks extra, main).
    Install {
    ...
    Update,
    /// Check for an codify update (the shell hook calls this).
    Check {
    ...
    ShellHook,
    /// Run an codify command against this checkout's pod (`uv run codify …`).
    ///
    /// Everything after the subcommand is forwarded verbatim to codify. The
    ...
    /// omnidev's own: `omnidev codify -- --verbose agent run …`.
    Codify {
...
fn main() -> Result<()> {
    ...
    // anywhere — dispatch them before any checkout discovery. `codify …` is
    ...
    match args.command {
        ...
        Some(Command::Codify { args: passthrough }) => run_codify(args.run, passthrough),
        None => run_supervisor(args.run),
    }
}

/// `omnidev codify …` — run an arbitrary codify command against this
/// checkout's pod via `uv run codify …`, with the pod's isolated env (data
...
/// Exits with codify's status code. Acquires no lock — it coexists with a
/// running supervisor (the common case: server up, you run a command).
fn run_codify(args: RunArgs, passthrough: Vec<String>) -> Result<()> {
    ...
    // Read persisted ports so CODIFY_URL points at a running supervisor's
    ...
    let cmd = codify_cmd::build(&pod, &passthrough);
    codify_cmd::run(cmd)
}

/// Default path: the pod supervisor for the current checkout. This is the only
/// path that requires an Codify checkout.
#[tokio::main]
