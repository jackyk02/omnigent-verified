...
pub struct Pod {
    ...
    /// otherwise. Fed to the server as `CODIFY_WS_ALLOWED_ORIGINS`.
    pub trusted_origins: Vec<String>,
}

impl Pod {
    /// Create the pod directory tree (idempotent) and return the pod handle.
    /// Only codify's own state is isolated (DB, artifacts, logs, config); the
    ...
    pub fn create(
        ...
        for sub in ["data/codify", "artifacts", "logs", "config"] {
            let p = dir.join(sub);
    ...
    pub fn db_uri(&self) -> String {
        format!(
            "sqlite:///{}",
            self.dir.join("data/codify/chat.db").display()
        )
    }
    ...
    /// `CODIFY_CONFIG_HOME` so its `config.yaml` is separate from the
    ...
    /// `CODIFY_URL`) stay on `127.0.0.1` so we don't accidentally target IPv6
    ...
    pub fn codify_dir(&self) -> PathBuf {
        self.repo_root.join("codify")
    }
    ...
    /// child. We isolate codify's own state — the DB, data dir, and config
    /// home — so concurrent pods don't share a database, pidfile, or
    /// `config.yaml`. The rest (real `HOME`, credentials, uv/pnpm caches) is
    /// inherited, since the agents codify runs need it. `CODIFY_URL` is the
    /// seam `web/vite.config.ts` reads to point its proxy at this pod's backend;
    /// `CODIFY_CONFIG_HOME` is where the server/host/runner read `config.yaml`.
    pub fn env(&self) -> Vec<(String, String)> {
        let d = |p: &str| self.dir.join(p).display().to_string();
        let mut env = vec![
            ("CODIFY_DATA_DIR".into(), d("data/codify")),
            ("CODIFY_DATABASE_URI".into(), self.db_uri()),
            ("CODIFY_URL".into(), self.server_url()),
            (
                "CODIFY_CONFIG_HOME".into(),
                self.config_dir().display().to_string(),
            ),
        ];
        if let Some(allowed) = self.allowed_origins_env() {
            env.push(("CODIFY_WS_ALLOWED_ORIGINS".into(), allowed));
        }
        env
    }

    /// The `CODIFY_WS_ALLOWED_ORIGINS` value to inject, or `None` to leave it
    ...
    fn allowed_origins_env(&self) -> Option<String> {
        ...
        let inherited = std::env::var("CODIFY_WS_ALLOWED_ORIGINS").unwrap_or_default();
...
/// The developer's real codify `config.yaml` to seed a fresh pod from.
///
/// Honors `CODIFY_CONFIG_HOME` if the parent env sets it (nested/test
/// setups), else `~/.codify/config.yaml` via `HOME`. Returns `None` when the
...
fn real_config_path() -> Option<PathBuf> {
    let home = match std::env::var_os("CODIFY_CONFIG_HOME") {
        Some(h) if !h.is_empty() => PathBuf::from(h),
        _ => PathBuf::from(std::env::var_os("HOME")?).join(".codify"),
    };
...
mod tests {
    ...
    /// Point `CODIFY_CONFIG_HOME` at `home` for the duration of `f`, restoring
    ...
    fn with_config_home<T>(home: &Path, f: impl FnOnce() -> T) -> T {
        ...
        let prev = std::env::var_os("CODIFY_CONFIG_HOME");
        std::env::set_var("CODIFY_CONFIG_HOME", home);
        let out = f();
        match prev {
            Some(v) => std::env::set_var("CODIFY_CONFIG_HOME", v),
            None => std::env::remove_var("CODIFY_CONFIG_HOME"),
        }
        out
    }
    ...
    #[test]
    fn real_config_path_falls_back_to_home_dot_codify() {
        // With no CODIFY_CONFIG_HOME, the real config resolves under
        // `$HOME/.codify/` — the path a normal pod run seeds from.
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let prev_cfg = std::env::var_os("CODIFY_CONFIG_HOME");
        let prev_home = std::env::var_os("HOME");

        let home = tempdir();
        std::fs::create_dir_all(home.join(".codify")).unwrap();
        std::fs::write(home.join(".codify/config.yaml"), "y: 2\n").unwrap();

        std::env::remove_var("CODIFY_CONFIG_HOME");
        ...
        assert_eq!(got, Some(home.join(".codify/config.yaml")));
    }
}
