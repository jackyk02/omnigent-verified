...
/// root. We then require `web/` and `codify/` to be present so we fail early
...
pub fn find_repo_root(start: &Path) -> Result<PathBuf> {
    ...
    while let Some(dir) = cur {
        if dir.join(".jj").is_dir() || dir.join(".git").exists() {
            ...
            if !root.join("codify").is_dir() || !root.join("web").is_dir() {
                bail!(
                    "found a VCS root at {} but it lacks codify/ and web/ — \
                     run omnidev from inside an Codify checkout",
                    root.display()
                );
            }
            return Ok(root);
...
/// `~/.cache/omnidev/codify-check.json` — volatile update-check state.
pub fn check_cache_path() -> Result<PathBuf> {
    Ok(cache_home()?.join("omnidev").join("codify-check.json"))
}
