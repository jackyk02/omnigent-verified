...
/// Start watching `codify_dir` for `*.py` changes. Coalesced bursts become a
/// single `Cmd::Reload(n)` on `cmd_tx`. The returned debouncer must be kept
/// alive for the watch to persist.
///
/// Gitignored files (e.g. the build-time `codify/_build_info.py`) are skipped
...
pub fn spawn(
    ...
    debouncer
        .watch(codify_dir, RecursiveMode::Recursive)
        .with_context(|| format!("watching {}", codify_dir.display()))?;
...
/// contributes no rules. Nested `.gitignore` files under `codify/` are not
...
mod tests {
    ...
    fn plain_python_file_triggers_reload() {
        let ig = ignore_with("codify/_build_info.py");
        assert_eq!(classify(Path::new("/repo/codify/cli.py"), &ig), Ok(()));
    }
    ...
    fn gitignored_python_file_is_skipped() {
        let ig = ignore_with("codify/_build_info.py");
        assert_eq!(
            classify(Path::new("/repo/codify/_build_info.py"), &ig),
            Err("gitignored")
        );
    }
    ...
    fn file_inside_gitignored_dir_is_skipped() {
        ...
        assert_eq!(
            classify(Path::new("/repo/codify/build/foo.py"), &ig),
            Err("gitignored")
        );
    }
    ...
    fn non_python_file_is_skipped() {
        let ig = ignore_with("codify/_build_info.py");
        assert_eq!(
            classify(Path::new("/repo/codify/notes.txt"), &ig),
            Err("non-.py")
        );
    }
    ...
    fn pycache_file_is_skipped() {
        let ig = ignore_with("codify/_build_info.py");
        assert_eq!(
            classify(Path::new("/repo/codify/__pycache__/cli.py"), &ig),
            Err("__pycache__")
        );
    }
}
