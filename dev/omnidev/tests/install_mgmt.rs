...
fn spec_no_extras_is_bare_git_url() {
    let c = InstallConfig {
        repo: "https://github.com/codify-ai/codify.git".into(),
        git_ref: "main".into(),
        extras: vec![],
    };
    assert_eq!(
        c.spec(),
        "git+https://github.com/codify-ai/codify.git@main"
    );
}
...
fn spec_reflects_custom_ref_and_extras() {
    ...
    assert_eq!(
        c.spec(),
        "codify[databricks,kubernetes] @ git+https://example.com/x.git@dev"
    );
}
