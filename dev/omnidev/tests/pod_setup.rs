...
/// A fake checkout (.git + codify/ + web/) is recognized as a root, and a
/// nested subdir resolves up to it.
#[test]
fn finds_repo_root_from_subdir() {
    let tmp = tempdir();
    fs::create_dir_all(tmp.join(".git")).unwrap();
    fs::create_dir_all(tmp.join("codify/server")).unwrap();
    fs::create_dir_all(tmp.join("web/src")).unwrap();

    let root = paths::find_repo_root(&tmp.join("codify/server")).unwrap();
    assert_eq!(root, tmp.canonicalize().unwrap());
}

/// A VCS root without codify/+web/ is rejected.
#[test]
fn rejects_non_codify_project() {
    let tmp = tempdir();
    fs::create_dir_all(tmp.join(".git")).unwrap();
...
const ALLOWED_ORIGINS_ENV: &str = "CODIFY_WS_ALLOWED_ORIGINS";
...
/// Run `body` with `CODIFY_WS_ALLOWED_ORIGINS` set to `value` (or unset when
