...
Dev tooling for Codify, in one binary with three surfaces:

1. A per-repo dev **pod supervisor** (bare `omnidev`) — the default.
2. **Install management** (`omnidev install`/`update`/`check`) — install
   and keep a git-based codify up to date. See
   [Managing your codify install](#managing-your-codify-install). These
   need no checkout and run anywhere.
3. **An codify passthrough** (`omnidev codify …`) — run any codify command
   against the current checkout's pod, with the pod's isolated env applied.
   See [Running codify commands](#running-codify-commands).

## Pod supervisor

A per-repo dev **pod** supervisor, as a single long-running terminal UI. It
replaces the three-terminal local dev flow (`codify server`, `codify host`,
...
- **reloads the backend** (server → host) when you edit `codify/**/*.py`;
  gitignored files under `codify/` (e.g. the build-time `_build_info.py`) are
...
(the `.jj`/`.git` marker) and requires `codify/` and
...
|---|---|---|
| server | `uv run codify --log-to-stderr server --host 127.0.0.1 --port <p> --database-uri … --artifact-location …` | Waited on via `GET /health`. |
| host   | `uv run codify --log-to-stderr host --server http://127.0.0.1:<p>` | Started once the server is healthy. |
| vite   | `pnpm run dev -- --host <host> --port <p> --strictPort` (cwd `web/`) | `CODIFY_URL` points its proxy at the pod's server. |
...
Only Codify's own state is isolated per pod — enough that concurrent pods
never share a database, server pidfile, or `config.yaml` — via
`CODIFY_DATA_DIR`, `CODIFY_DATABASE_URI`, `CODIFY_URL`, and
`CODIFY_CONFIG_HOME`. Everything else (your real `HOME`, credentials, and
uv/pnpm caches) is inherited, because the agents Codify runs need it. This is
...
`CODIFY_CONFIG_HOME`. On first create it's **seeded** from your real
`~/.codify/config.yaml` (if present) so the pod works out of the box — it
keeps your providers — after which the two are independent: server-config edits
inside a pod (via the UI or `codify config`) don't touch your real config.
...
server's `CODIFY_WS_ALLOWED_ORIGINS` allowlist (merged with any value you
...
not auto-trusted (add those to `CODIFY_WS_ALLOWED_ORIGINS` yourself).
...
## Running codify commands

`omnidev codify …` runs any codify command against the current checkout's
pod, via `uv run codify …`, with the pod's isolated env applied
(`CODIFY_DATA_DIR`, `CODIFY_DATABASE_URI`, `CODIFY_CONFIG_HOME`,
`CODIFY_URL`). It resolves the same repo root and pod dir the supervisor
uses, so a command talks to the pod's database and config — and `CODIFY_URL`
points at a running supervisor's server when one is up.

```bash
omnidev codify agent run "fix the flaky test"
omnidev codify config show
omnidev --pod-dir /tmp/x codify agent list   # target a specific pod
```

Everything after the subcommand is forwarded verbatim to codify. It runs in
the foreground (inheriting your stdio) and exits with codify's status code.
...
```bash
omnidev codify -- --verbose agent run …
```

Supervisor-only flags (`--server-port`, `--clean`, `--no-vite`, …) don't apply
to the passthrough; only `--pod-dir` is shared.

## Managing your codify install

For people who *run* codify (installed from git via `uv tool install`) rather
than develop it. This wraps the fiddly PEP 508 install syntax and adds a daily
update check — filling a gap, since codify's own update notice only works for
...
```
omnidev install     # uv tool install codify from git (databricks extra, main)
...
result (`${XDG_CACHE_HOME:-~/.cache}/omnidev/codify-check.json`) and, when
...
and, on a terminal, prompts `Update codify now? [y/N]`; on yes it runs
`omnidev update` in the foreground. Declining suppresses that same commit until
a newer one lands. Set `CODIFY_NO_UPDATE_CHECK` in your environment if you want
to silence codify's own separate notice.
