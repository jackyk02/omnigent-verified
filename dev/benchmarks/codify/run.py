"""Codify user-journey benchmark runner.

Boots a real ``codify server`` against a SQLite DB (no runner, no LLM),
...
Runs in the project venv — it imports ``codify`` and ``tests._helpers`` and
...
project (which triggers a web-UI build that fails in a worktree)::
    ...
    uv run --no-sync dev/benchmarks/codify/run.py
    uv run --no-sync dev/benchmarks/codify/run.py --journeys list_sessions,get_session
    uv run --no-sync dev/benchmarks/codify/run.py --requests 500 --concurrency 25 --runs 3
    uv run --no-sync dev/benchmarks/codify/run.py --output bench.json --max-p50-ms 25
...
from dev.benchmarks.codify.environment import BenchEnvironment
from dev.benchmarks.codify.journeys import (
    ALL_JOURNEYS,
    Journey,
    resolve_journeys,
    run_latency,
...
from dev.benchmarks.codify.measure import (
    RunResult,
    aggregate,
    check_thresholds,
    console,
...
from dev.benchmarks.codify.schema import build_report
