# Codify performance benchmark

Baseline, repeatable latency/throughput numbers for key Codify user
journeys, so we can track them over time and catch regressions. Modeled on
MLflow's `dev/benchmarks/gateway/` workflow.

The harness boots a real `codify server`, drives the selected journeys under
...
uv run --no-sync dev/benchmarks/codify/run.py --max-p50-ms 25 --max-p99-ms 100
```
...
`CODIFY_SKIP_WEB_UI=true uv sync` prepares the venv once, then use
`--no-sync`.)
...
excluded. The mock LLM is zero-latency, so every number is codify
...
- That router lives under `dev/`, which `pyproject.toml` excludes from the wheel
  (`include = ["codify*"]`) — a production install can't even import it.
...
- **Simulated provider latency.** The mock LLM returns at ~zero latency, which
  is what isolates codify overhead. A fixed per-response delay knob would let
