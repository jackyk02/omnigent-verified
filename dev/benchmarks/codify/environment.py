...
Codify ``server`` with no Databricks credentials. Two modes:
...
from codify.host.identity import HOST_ID_ENV_VAR, HOST_NAME_ENV_VAR
from codify.runner.identity import CODIFY_INTERNAL_WS_ORIGIN, token_bound_runner_id
...
_DEBUG_ROUTER_MODULE = "dev.benchmarks.codify.debug_router"
...
class BenchEnvironment:
    ...
    :param with_host: When ``True`` (implies ``with_runner``), additionally
        spawn a real ``codify host`` daemon. Additive over ``with_runner``:
    ...
    def _spawn_server(
        ...
        if self.with_runner:
            ...
            env["CODIFY_RUNNER_TUNNEL_TOKEN"] = binding_token
        ...
        runner_env = apply_runner_env(
            {
                **base_env,
                ...
                "CODIFY_RUNNER_WORKSPACE": str(workspace),
            }
        )
        return subprocess.Popen(
    ...
    def _spawn_host(self, base_env: dict[str, str]) -> subprocess.Popen[bytes]:
        ...
        the daemon never touches the developer's real ``~/.codify`` (nor
    ...
    async def time_to_first_delta(
        ...
        Times codify's streaming-pipeline overhead to first token against an
