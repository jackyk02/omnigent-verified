...
def _criteria_dirs():
    ...
    ``criteria/`` folder bundled inside the package, the ``criteria/``
...
def normalize_criteria(criteria):
    ...
    if isinstance(criteria, dict):
        criteria = [{"name": name, "description": desc} for name, desc in criteria.items()]
    ...
    for i, raw in enumerate(criteria):
        if isinstance(raw, str):
            ...
            raise TypeError(f"criteria[{i}] must be a str or dict, got {type(raw).__name__}")
        if not desc:
