...
class ProgressResult:
    ...
    steps: list[int]
    scores: list[float]
    per_rep_scores: list[list[float | None]] = field(repr=False)
...
def build_progress_prompt(
    ...
    out += [
        ...
        trajectory_text,
        "",
        f"You will score the trajectory at {n} CHECKPOINTS. The score measures exactly ONE thing:",
        "",
...
def _expected_value_from_alts(alts: Sequence[tuple[str, float]]) -> float | None:
...
def extract_progress_scores(text, tokens, position_logprobs, n: int) -> list[float | None]:
            ...
            for end_char, next_pos in positions_after:
                if end_char > target_char:
                    ...
                    if 0 <= answer_pos < len(position_logprobs):
                        v = _expected_value_from_alts(position_logprobs[answer_pos])
                        if v is not None:
                            scores[i - 1] = v
                    break
    ...
    if any(s is None for s in scores):
        bare = [
            ...
            if ln.strip() and len(ln.strip()) == 1 and ln.strip() in LETTER_TO_VALUE
        ]
        if len(bare) == n:
...
def track(
    ...
    *,
    ...
    prompt = build_progress_prompt(
        problem, format_steps(steps), t, checkpoint_steps, n_images=len(imgs)
    )
    ...
    if n_evaluations == 1:
        ...
        with ThreadPoolExecutor(max_workers=min(max_workers, n_evaluations)) as executor:
...
class ProgressTracker:
    ...
    def __init__(
        ...
        *,
        ...
        self._per_step_reps: list[list[float | None]] = []
    ...
    def update(self, step: str, images: Any = None) -> float:
        ...
        if step_imgs:
            markers = " ".join(
                f"[Image {len(self._images) + j + 1} attached]" for j in range(len(step_imgs))
            )
            step = f"{step}\n{markers}"
        ...
        prompt = build_progress_prompt(
            self.problem, format_steps(self._step_texts), k, [k], n_images=len(self._images)
        )

        def one_rep(_):
            text, tokens, lps = call_verifier(self.client, prompt, self.model, images=self._images)
        ...
        if self.n_evaluations == 1:
            ...
            with ThreadPoolExecutor(
                max_workers=min(self.max_workers, self.n_evaluations)
            ) as executor:
                reps = list(executor.map(one_rep, range(self.n_evaluations)))
    ...
    def result(self) -> ProgressResult:
        ...
        per_rep = [
            [self._per_step_reps[i][r] for i in range(len(self.steps))]
