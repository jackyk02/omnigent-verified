...
Slack Socket Mode bot that maps one Slack thread to one Codify session. The
bot talks to **one** Codify server, set by the operator via
`CODIFY_SERVER_URL` — Slack users never enter a URL, so the bot only ever
issues requests to that fixed host. Each user still authenticates as their own
Codify identity against it.
...
2. Add the OAuth scopes and event subscriptions listed under **Required scopes**
   below.
3. Add a slash command `/codify` (Features → Slash Commands). In Socket Mode
  the request URL is ignored, so any placeholder works.
4. Install the app into the workspace.
5. Set the two Slack tokens (`CODIFY_SLACK_BOT_TOKEN`,
   `CODIFY_SLACK_APP_TOKEN`) and your Codify server URL
   (`CODIFY_SERVER_URL`) as **environment variables**. If your server sets
   `CODIFY_DEVICE_CLIENT_SECRET`, set the same value here so the bot is
...
### Bot token scopes (`CODIFY_SLACK_BOT_TOKEN`, `xoxb-…`)
...
| `commands` | Register and receive the `/codify` slash command. |
...
### App-level token scope (`CODIFY_SLACK_APP_TOKEN`, `xapp-…`)
...
All configuration (the two Slack tokens, `CODIFY_SERVER_URL`, and the
optional `CODIFY_DEVICE_CLIENT_SECRET` / `CODIFY_SLACK_TOKEN_ENCRYPTION_KEY`)
...
The bot lives in the separate `codify-slack` package, which must be installed
**in the same environment as** `omni` for the `omni integration slack` commands
to find it. Install it as the `slack` extra of codify:

```bash
uv tool install "codify[slack]"     # or, from a source checkout: uv sync --extra slack
```
...
without having configured, the bot DMs them a **Set up Codify** button and,
...
1. The bot validates connectivity to `CODIFY_SERVER_URL`. If the server has
...
For Codify servers with authentication enabled, each Slack user logs in with
their own Codify identity — no Codify credential ever passes through Slack.
Login happens inside the single `/codify` configuration modal, not a separate
command.

The bot **auto-detects the server's auth mode** (an unauthenticated `GET /v1/me`, exactly as the `codify login` CLI does) and picks the matching flow:
...
token can't reach admin endpoints. **The Codify server must have the device
grant enabled** (`CODIFY_DEVICE_GRANT_ENABLED=1` — it is default-off);
otherwise the `/oauth/*` routes are absent and accounts-mode login can't
complete. If the server sets `CODIFY_DEVICE_CLIENT_SECRET`, set the same
value as the bot's `CODIFY_DEVICE_CLIENT_SECRET` so only this authorized
...
per-user login the auto-detect flow can drive. Two options:
  - **Databricks Apps** (the common case): set
    `CODIFY_SLACK_SERVER_AUTH=databricks` and the bot enrolls each user
...
The bot reads no auth-mode config itself; the Codify server's own
`CODIFY_OIDC_*` / `CODIFY_AUTH_*` env vars decide its mode (see the server's
`[deploy/README.md](../../deploy/README.md#auth)`).

Set `CODIFY_SLACK_TOKEN_ENCRYPTION_KEY` (see `.env.example`) to persist tokens
...
`/codify logout` fully resets you: it revokes your delegated token and clears
all your saved settings (agent, host, workspace, and thread→session mappings).
Run `/codify` afterwards to set up again.
...
When the Codify server is deployed as a **Databricks App**, it runs in header
...
1. On `/codify`, the bot looks up the user's email (`users.info`), generates a
...
5. **Confirm before storing:** the GET stores nothing — it shows a consent page
   naming the exact identities being linked ("your Codify `<server>` account
...
Enabled with `CODIFY_SLACK_SERVER_AUTH=databricks` plus the custom OAuth app's
`CODIFY_SLACK_DATABRICKS_CLIENT_ID` / `CODIFY_SLACK_DATABRICKS_CLIENT_SECRET`
and a `CODIFY_SLACK_DATABRICKS_STATE_SECRET` (see `.env.example`). To deploy
...
Run `/codify` (or `/codify config`) any time to reopen this modal and change
...
If the bot can't reach your server, it replies telling you to run `/codify` to
...
```text
Run this on the machine you want to use, then run /codify:
...
the same Codify session. A channel thread belongs to whoever started it; a
...
This integration is a **separate package** (`codify-slack`) with heavy deps
(slack_bolt, aiohttp) kept out of the core `codify` install. It resolves as an
editable path dep of the root `codify` package via the `slack` extra (see
