...
How the Codify Slack bot is built and the key technical decisions behind it.
...
Codify server. It maps **one Slack thread ↔ one Codify session**, streams
the agent's answer into the thread live, and renders tool-approval /
`AskUserQuestion` prompts as interactive Block Kit cards.

The **guiding principle**: the Codify **web UI is the reference client** for
...
| `codify.py` | HTTP/SSE client (`CodifyClient`), connection pool, the `run_turn` stream loop and turn-end detection, error subclasses. |
...
| `service.py` | `SlackCodifyService` — event acceptance, turn routing, turn lifecycle. Delegates streaming to `streaming.py`, elicitation to `elicitation.py`, messages to `notifications.py`. |
...
Slack bot instead opens **one stream per turn** (`CodifyClient.run_turn`).
...
- **401** → "log in again" (`/codify`).
- **Unreachable** → "reconfigure" (`/codify`).
- **No online host** → the `omni host --server …` command.
- **412 `harness_not_configured`** → the server's *curated* `error.message` (run
  `codify setup` on the host). Server error bodies are otherwise **not** echoed
...
Each Slack user authenticates as their own Codify identity — no Codify
...
or `oidc` cli-login inside the `/codify` modal; `header`/proxy mode is
unsupported. Tokens are encrypted at rest when
`CODIFY_SLACK_TOKEN_ENCRYPTION_KEY` is set, else in-memory only. The 401-retry
...
Unit tests use fakes (`FakeCodifyClient`, `FakeSlackClient`) that mirror the
