#!/usr/bin/env python3
"""Deploy the Codify Slack bot to a Databricks App via Asset Bundles.

Builds a wheel for the ``codify-slack`` package, generates an app-level
...
Usage:
    uv run python integrations/slack/deploy/databricks/deploy.py \\
        --app-name codify-slack --profile <your-profile> \\
...
_BUNDLE_RESOURCE_KEY = "codify-slack"

# Distribution / import names of the package being deployed.
_DIST_NAME = "codify-slack"
_WHEEL_PREFIX = "codify_slack-"
...
def _build_wheel() -> Path:
    """Build the codify-slack wheel into the package's dist/, return its path."""
    root = _slack_root()
    dist = root / "dist"
...
def _write_app_pyproject(wheel: Path, deploy_version: str) -> None:
    ...
    pyproject = (
        "[project]\n"
        'name = "codify-slack-databricks-app"\n'
        'version = "0.0.0"\n'
...
def main() -> None:
    ...
    parser.add_argument(
        "--server-url",
        required=True,
        help="Base URL of the Codify server app the bot talks to.",
    )
    parser.add_argument(
        "--oauth-client-id",
        required=True,
