# Deploying the Codify Slack bot on Databricks Apps

This directory deploys the **Codify Slack bot** to
[Databricks Apps](https://docs.databricks.com/aws/en/dev-tools/databricks-apps/)
via [Asset Bundles](https://docs.databricks.com/aws/en/dev-tools/bundles/).

Deploy the bot here when the Codify **server** it talks to is itself a
...
stateless pure-PyPI package. `deploy.py` builds an `codify_slack` wheel,
...
> would fail with `ModuleNotFoundError: No module named 'codify_slack'`.
...
5. The **target Codify server app** already deployed as a Databricks App (you
...
```bash
databricks secrets create-scope codify-slack

databricks secrets put-secret codify-slack slack_bot_token          # xoxb-…
databricks secrets put-secret codify-slack slack_app_token          # xapp-…
...
databricks secrets put-secret codify-slack token_encryption_key --string-value "$KEY"

# Custom U2M OAuth app secret (see below). The client id is public and passed
# inline via --oauth-client-id, NOT stored here.
databricks secrets put-secret codify-slack databricks_oauth_client_secret # <client secret>

# HMAC key signing the enrollment state — its own secret, kept separate from the
# OAuth client secret. Any long random string:
databricks secrets put-secret codify-slack databricks_state_secret \
...
```bash
databricks secrets put-acl codify-slack <app-service-principal> READ
```

Find the SP with `databricks apps get codify-slack -o json | jq -r .service_principal_client_id`
...
uv run python integrations/slack/deploy/databricks/deploy.py \
    --app-name codify-slack \
    --profile <your-profile> \
    --secret-scope codify-slack \
...
```bash
APP_URL="$(databricks apps get codify-slack -o json | jq -r .url)"
...
`bundle run codify-slack --target prod`. No lockfile is generated: the app
...
2. In Slack, run `/codify`. The modal shows a **Sign in with Databricks**
...
- The app runs the OAuth callback web server (`codify_slack/webauth.py`) and,
...
store lives on ephemeral disk (`CODIFY_DATA_DIR=/tmp/codify-slack`),
...
| ---------------------------------------- | -------------------- | ------------------------------------------------ |
| `CODIFY_SLACK_BOT_TOKEN`               | secret               | Slack bot token (`xoxb-…`)                       |
| `CODIFY_SLACK_APP_TOKEN`               | secret               | Slack app-level token (`xapp-…`)                 |
| `CODIFY_SLACK_TOKEN_ENCRYPTION_KEY`    | secret               | Fernet key for tokens at rest                    |
| `CODIFY_SLACK_DATABRICKS_CLIENT_ID`    | `--oauth-client-id`  | Custom U2M OAuth app client id (public, inline)  |
| `CODIFY_SLACK_DATABRICKS_CLIENT_SECRET`| secret               | Custom U2M OAuth app client secret               |
| `CODIFY_SLACK_DATABRICKS_STATE_SECRET` | secret               | HMAC key signing the enrollment `state`          |
| `CODIFY_SLACK_DATABRICKS_SCOPES`       | inline (optional)    | Requested scopes (default `all-apis`; must be a superset of the server app's scopes; `openid` + `offline_access` forced on) |
| `CODIFY_SLACK_SERVER_AUTH`             | inline               | `databricks` (selects the OAuth mode)            |
| `CODIFY_SERVER_URL`                    | `--server-url`       | Codify server the bot drives                   |
| `CODIFY_SLACK_DATABRICKS_APP_URL`      | `--app-url`          | This app's public URL — link base + redirect URI |
| `CODIFY_DATA_DIR`                      | inline               | Ephemeral SQLite store dir                       |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'codify_slack'` | The wheel/`pyproject.toml` were git-ignored, so `bundle deploy` didn't sync them | Ensure `src/*.whl`, `src/pyproject.toml` are untracked but NOT git-ignored; re-run `deploy.py` (not `--skip-build` on a clean `src/`) |
...
| Sign-in page says the link was already used or expired | The redirect was replayed, or the bot restarted between link-issue and callback (in-memory PKCE verifier lost) | Run `/codify` again for a fresh link |
| Enrolled, but turns fail auth against the server                      | User lacks access to the server app, or the token's scopes don't satisfy the server proxy | Grant the user server-app access; widen `CODIFY_SLACK_DATABRICKS_SCOPES` if the server proxy needs more |
...
| `src/app.py` | App entry point — runs `codify_slack.app.run()`. |
