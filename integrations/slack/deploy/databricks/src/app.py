"""Databricks Apps entry point for the Codify Slack bot.

Runs the Socket-Mode bot and, in Databricks web-auth mode, the enrollment web
server. The ``codify_slack`` package is installed from the wheel ``deploy.py``
...
logger = logging.getLogger("codify-slack-app")

try:
    from codify_slack.app import run

    if __name__ == "__main__":
        logger.info("Starting Codify Slack bot (Databricks App)")
        asyncio.run(run())
except Exception:  # startup catch-all; we want every failure logged to /logz
    logger.error("FATAL: Codify Slack bot failed to start:\n%s", traceback.format_exc())
