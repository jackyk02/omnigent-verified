...
import codify_slack.codify as codify_module
import httpx
import pytest
import respx
...
async def test_client_create_and_submit_request_shapes() -> None:
    create = respx.post("http://codify.test/v1/sessions").mock(
        return_value=httpx.Response(201, json={"id": "conv_1"})
    )
    submit = respx.post("http://codify.test/v1/sessions/conv_1/events").mock(
        return_value=httpx.Response(200, json={})
    )
    client = CodifyClient("http://codify.test")
...
async def test_check_health_probes_health_endpoint() -> None:
    health = respx.get("http://codify.test/health").mock(
...
async def test_validate_returns_agents_and_online_hosts() -> None:
    respx.get("http://codify.test/health").mock(
        return_value=httpx.Response(200, json={"status": "ok"})
    )
    respx.get("http://codify.test/v1/agents").mock(
        return_value=httpx.Response(200, json={"data": [{"id": "ag_1", "name": "Helper"}]})
    )
    respx.get("http://codify.test/v1/hosts").mock(
...
async def test_validate_raises_auth_required_on_401() -> None:
    ...
    respx.get("http://codify.test/v1/agents").mock(return_value=httpx.Response(401))
    client = CodifyClient("http://codify.test")
...
async def test_get_host_home_derives_home_from_filesystem_listing() -> None:
    respx.get("http://codify.test/v1/hosts/host_1/filesystem").mock(
        return_value=httpx.Response(
            200,
            json={
                "object": "list",
...
async def test_client_pool_reuses_client_per_server() -> None:
    ...
    try:
        first = await pool.get("http://codify.test/")
        again = await pool.get("http://codify.test")
...
async def test_launch_runner_on_explicit_host() -> None:
    launch = respx.post("http://codify.test/v1/hosts/host_1/runners").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched"})
    )
    respx.get("http://codify.test/v1/runners/runner_launched/status").mock(
...
async def test_launch_runner_picks_random_online_host_when_unspecified() -> None:
    ...
    launch = respx.post("http://codify.test/v1/hosts/host_online/runners").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched"})
    )
    respx.get("http://codify.test/v1/runners/runner_launched/status").mock(
        return_value=httpx.Response(200, json={"runner_id": "runner_launched", "online": True})
...
async def test_launch_runner_errors_when_no_online_host() -> None:
    ...
    assert "No online Codify hosts" in str(raised)


@respx.mock
async def test_launch_runner_raises_host_unavailable_when_host_offline() -> None:
    respx.post("http://codify.test/v1/hosts/host_1/runners").mock(
...
async def test_launch_runner_raises_host_unavailable_when_runner_never_online() -> None:
    ...
    respx.get("http://codify.test/v1/runners/runner_x/status").mock(
        return_value=httpx.Response(200, json={"online": False})
    )
    client = CodifyClient("http://codify.test", runner_launch_timeout_seconds=0.01)
...
async def test_request_wraps_transport_failure_as_server_unreachable() -> None:
    ...
    client = CodifyClient("http://127.0.0.1:1")
...
async def test_run_turn_streams_across_multiple_responses_until_id_terminal() -> None:
    ...
    respx.get("http://codify.test/v1/sessions/conv_1/stream").mock(
        return_value=httpx.Response(200, text=sse_body)
    )
    respx.post("http://codify.test/v1/sessions/conv_1/events").mock(
        return_value=httpx.Response(200, json={})
    )
    client = CodifyClient("http://codify.test")
...
async def test_run_turn_ignores_idless_idle_flap_on_claude_native_cold_start() -> None:
    ...
    # "Codify completed without returning response text" ~5s after submit, even
...
async def test_launch_runner_412_propagates_harness_not_configured_message() -> None:
        ...
        return_value=httpx.Response(
            ...
            json={
                "error": {
                    ...
                    "message": "launch failed: claude CLI missing; run codify setup",
                }
            },
        )
    )
    ...
    assert "codify setup" in str(raised)
...
async def test_stream_401_raises_auth_required_not_response_not_read() -> None:
    ...
    # prompt "/codify to log in again". The stream response body is unread, so
...
async def test_resolve_elicitation_posts_accept() -> None:
    route = respx.post("http://codify.test/v1/sessions/conv_1/elicitations/elicit_1/resolve").mock(
        return_value=httpx.Response(202, json={"queued": False})
    )
    client = CodifyClient("http://codify.test")
    try:
...
async def test_resolve_elicitation_decline_and_benign_statuses() -> None:
    ...
    respx.post("http://codify.test/v1/sessions/conv_1/elicitations/gone/resolve").mock(
...
async def test_get_session_activity_maps_server_state() -> None:
    ...
    client = CodifyClient("http://codify.test")
    try:
        route = respx.get("http://codify.test/v1/sessions/conv_1")
...
async def test_get_session_activity_unreadable_snapshot_is_not_busy() -> None:
    ...
    respx.get("http://codify.test/v1/sessions/conv_1").mock(return_value=httpx.Response(500))
    client = CodifyClient("http://codify.test")
    try:
        a = await client.get_session_activity("conv_1")
    finally:
...
async def test_stream_session_events_preconnect_failure_is_unreachable() -> None:
    ...
    client = CodifyClient("http://127.0.0.1:1")  # nothing listening
    raised: Exception | None = None
    try:
        async with client.stream_session_events("conv_1") as events:
            async for _ in events:
...
async def test_run_turn_reconnects_on_mid_stream_drop_without_resubmit() -> None:
    ...
    respx.get("http://codify.test/v1/sessions/conv_1").mock(
...
async def test_run_turn_raises_stream_interrupted_when_reconnect_exhausted(
    ...
    monkeypatch.setattr(codify_module, "_STREAM_RECONNECT_BACKOFF_S", 0.0)
    ...
    respx.get("http://codify.test/v1/sessions/conv_1/stream").mock(side_effect=_always_dropping)
    respx.get("http://codify.test/v1/sessions/conv_1").mock(
        return_value=httpx.Response(200, json={"status": "running"})
    )
    respx.post("http://codify.test/v1/sessions/conv_1/events").mock(
...
async def test_run_turn_survives_many_drops_that_each_make_progress(
    ...
    monkeypatch.setattr(codify_module, "_STREAM_RECONNECT_BACKOFF_S", 0.0)
    n_legs = codify_module._STREAM_RECONNECT_MAX_ATTEMPTS + 3
    ...
    respx.get("http://codify.test/v1/sessions/conv_1/stream").mock(side_effect=legs)
    respx.get("http://codify.test/v1/sessions/conv_1").mock(
        return_value=httpx.Response(200, json={"status": "running"})
    )
    respx.post("http://codify.test/v1/sessions/conv_1/events").mock(
