...
from codify_slack.approvals import ElicitationCoordinator, Verdict
from codify_slack.elicitation import (
    ElicitationController,
    ElicitationTurnState,
    PendingElicitation,
)
from codify_slack.events import ElicitationRequest
from codify_slack.models import SlackTurn, ThreadKey

_KEY = ThreadKey(team_id="T1", channel_id="C1", thread_ts="100.1")


class _RecordingCodify:
...
async def test_finish_pending_no_redecline_when_timeout_decline_succeeded() -> None:
    ...
    await controller.finish_pending(codify, _turn(), state)  # type: ignore[arg-type]

    assert codify.resolve_calls == []
