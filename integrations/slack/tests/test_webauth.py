...
from codify_slack.databricks_oauth import DatabricksOAuthError, DatabricksTokens
from codify_slack.enrollment_state import verify_state
from codify_slack.tokens import InMemoryTokenStore
...
def _settings() -> Settings:
    return Settings(
        ...
        CODIFY_SLACK_DATABRICKS_APP_URL="https://slackbot.example.com",
        CODIFY_SLACK_DATABRICKS_CLIENT_ID="client-id",
        CODIFY_SLACK_DATABRICKS_CLIENT_SECRET="client-secret",
        CODIFY_SLACK_DATABRICKS_STATE_SECRET=_STATE_SECRET,
    )
...
async def test_get_shows_consent_and_stores_nothing(harness) -> None:
    ...
    assert "https://codify.example.com" in page  # names the server
    assert _EMAIL in page
    assert '<form method="post"' in page  # a Confirm button that POSTs

    assert await store.get("T1", "U1", "https://codify.example.com") is None
    assert enrolled == []
...
async def test_confirm_stores_refreshable_token(harness) -> None:
    ...
    record = await store.get("T1", "U1", "https://codify.example.com")
    ...
    assert enrolled == [("T1", "U1", "https://codify.example.com")]
...
async def test_callback_email_match_case_insensitive() -> None:
    ...
    try:
        ...
        assert resp.status == 200
        assert await store.get("T1", "U1", "https://codify.example.com") is not None
    finally:
        await client.close()
