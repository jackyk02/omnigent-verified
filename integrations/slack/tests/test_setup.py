...
from codify_slack.setup import (
    ACTION_SETUP_START,
    AGENT_BLOCK,
    CALLBACK_SETUP_INFO,
    HOST_BLOCK,
...
def _flow(store: SQLiteStore, pool: CodifyClientPool, auth: Any = None) -> SetupFlow:
...
def test_select_modal_lists_agents_and_hosts() -> None:
    from codify_slack.codify import ValidatedServer
...
async def test_setup_advances_to_select_modal_with_host_home_workspace(
    ...
    assert view["callback_id"] == "codify_setup_select"
...
async def test_setup_shows_login_in_modal_and_advances_on_approval(tmp_path: Path) -> None:
    ...
    No DM and no re-running /codify — login and config are one flow.
    """
    import asyncio
    ...
    try:
        ...
        assert authorize_body["client_id"] == "Slack-Codify-Acme Corp"
    ...
    assert advanced["view"]["callback_id"] == "codify_setup_select"
...
async def test_logout_revokes_all_and_clears_settings(tmp_path: Path) -> None:
    """`/codify logout` revokes every server token and clears saved data."""
...
def _enroll_flow(
    store: SQLiteStore, pool: CodifyClientPool, auth: Any, enrollment_url: Any
) -> SetupFlow:
    return SetupFlow(
        store=store,
        pool=pool,
...
async def test_post_enrollment_advance_uses_freshly_stored_token(tmp_path: Path) -> None:
    ...
    from codify_slack.codify import ClientAuth
...
async def test_setup_settles_modal_before_first_update(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    import codify_slack.setup as setup_mod
