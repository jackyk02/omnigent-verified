...
import pytest
import respx
from codify_slack.auth_manager import AuthManager, slack_client_id
from codify_slack.tokens import EncryptedTokenStore, TokenStore
from cryptography.fernet import Fernet

_BASE = "http://codify.test"
...
def test_slack_client_id_format() -> None:
    assert slack_client_id("Acme Corp") == "Slack-Codify-Acme Corp"
    # Missing/blank workspace name falls back to the bare label.
    assert slack_client_id("") == "Slack-Codify"
    assert slack_client_id("  ") == "Slack-Codify"
...
async def test_authorize_returns_link_and_await_persists_on_approval(tmp_path: Path) -> None:
    ...
    pending = await mgr.authorize(server_url=_BASE, client_id="Slack-Codify-Test")
...
async def test_rotator_refresh_rotates_via_external_client(tmp_path: Path) -> None:
    ...
    # NOT the Codify server's /oauth/* — and a fresh pair is persisted.
    store = EncryptedTokenStore(tmp_path / "t.sqlite3", Fernet.generate_key().decode())
    await store.initialize()
...
async def test_rotator_transient_failure_keeps_token(tmp_path: Path) -> None:
    ...
    from codify_slack.codify import TokenRefreshTransientError
...
async def test_second_enrollment_poll_supersedes_prior_for_same_key(tmp_path: Path) -> None:
    ...
    # in-flight poll. Otherwise each /codify stacks another poll, several race,
