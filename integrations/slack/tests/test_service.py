...
import codify_slack.service as service_module
import pytest
from codify_slack.approvals import Verdict, parse_action_value
from codify_slack.codify import (
    AuthRequiredError,
    CodifyError,
...
from codify_slack.models import ThreadKey, UserConfig
from codify_slack.service import (
    _ACK_TEXT,
    _SERVER_UNREACHABLE_TEXT,
    _STREAM_INTERRUPTED_TEXT,
    SlackCodifyService,
)
from codify_slack.store import SQLiteStore
...
class FakeCodifyClient:
    ...
    async def get_session_activity(self, session_id: str) -> Any:
        from codify_slack.codify import SessionActivity
    ...
    async def get_session_info(self, session_id: str) -> Any:
        from codify_slack.codify import SessionInfo
...
class FakePool:
    """Returns the same FakeCodifyClient for every server URL, recording URLs."""

    def __init__(self, client: FakeCodifyClient) -> None:
        self._client = client
        self.requested: list[str] = []

    async def get(self, server_url: str, user_id: str = "") -> FakeCodifyClient:
...
def _service(
    store: SQLiteStore,
    codify: FakeCodifyClient,
    *,
    setup: FakeSetup | None = None,
) -> tuple[SlackCodifyService, FakePool, FakeSetup]:
    pool = FakePool(codify)
    setup = setup or FakeSetup()
    service = SlackCodifyService(
        store=store,
        pool=pool,  # type: ignore[arg-type]
        setup=setup,  # type: ignore[arg-type]
        server_url="http://codify.test",
    )
...
async def test_app_mention_creates_session_and_posts_response(tmp_path: Path) -> None:
    ...
    codify = FakeCodifyClient()
    service, _pool, _setup = _service(store, codify)
    ...
    assert codify.created[0][0] == "ag_1"
    # Session title is "Slack: <thread permalink>" (a clickable URL the web UI
    # linkifies), not the old opaque "Slack C…/ts" descriptor.
    assert codify.created[0][1] == "Slack: https://slack.test/archives/C1/p100.1"
    assert codify.bound == ["conv_1"]
    assert codify.turns == [("conv_1", "hello")]
    ...
    assert "/c/conv_1|Open in Codify>" in info_text  # web-UI link
...
async def test_session_title_falls_back_when_permalink_unavailable(tmp_path: Path) -> None:
    ...
    assert len(codify.created) == 1
    assert codify.created[0][1] == "Slack thread C1/100.1"
...
async def test_session_info_omits_missing_fields(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    codify = FakeCodifyClient()
    codify.info_harness = None
    codify.info_agent_name = None
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
    ...
    assert "/c/conv_1|Open in Codify>" in info_text  # link still present
...
async def test_no_ack_when_session_cannot_start_host_unavailable(tmp_path: Path) -> None:
    ...
    assert len(slack.posts) == 1
    assert "omni host --server http://codify.test" in slack.posts[-1]["text"]
...
class StreamingClient(FakeCodifyClient):
...
class NoDeltaIdleClient(FakeCodifyClient):
...
async def test_no_delta_idle_answer_keeps_ack_until_visible(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = NoDeltaIdleClient(final_text="Here is the answer.")
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
class MultiMessageClient(FakeCodifyClient):
...
async def test_back_to_back_messages_get_paragraph_break(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = MultiMessageClient(final_text="")
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_long_answer_streams_in_full(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    long_answer = "x" * 9000
    codify = StreamingClient(final_text=long_answer)
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_turn_error_posts_separate_reply_and_keeps_answer(tmp_path: Path) -> None:
    ...
    class ErroringAfterAnswerClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
    ...
    codify = ErroringAfterAnswerClient(final_text="the real answer")
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_turn_error_without_answer_finalizes_with_generic_message(tmp_path: Path) -> None:
    ...
    class ErroringNoAnswerClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
...
async def test_exhausted_reconnect_shows_non_alarming_text(tmp_path: Path) -> None:
    ...
    class StreamInterruptedClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
    ...
    codify = StreamInterruptedClient()
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_stream_closed_mid_turn_continues_in_new_stream(tmp_path: Path) -> None:
    ...
    codify = StreamingClient(final_text="chunk-a" + "y" * 600)
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_stream_closed_then_error_continues_and_posts_failure(tmp_path: Path) -> None:
    ...
    class ClosedThenErrorClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
...
async def test_direct_message_creates_session(tmp_path: Path) -> None:
    ...
    assert codify.created[0][0] == "ag_1"
    assert codify.bound == ["conv_1"]
    assert codify.turns == [("conv_1", "hello there")]
...
async def test_direct_message_threaded_reply_reuses_existing_session(tmp_path: Path) -> None:
    ...
    assert codify.created == []
    assert codify.bound == []
    assert codify.turns == [("conv_existing", "follow up")]
...
async def test_direct_message_top_level_starts_new_session_per_thread(tmp_path: Path) -> None:
    ...
    assert codify.turns == [("conv_1", "brand new topic")]
...
async def test_message_while_server_busy_is_deflected(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    codify = FakeCodifyClient()
    codify.route_status = "running"  # server is busy at route time
    service, _pool, _setup = _service(store, codify)
...
async def test_second_message_while_local_stream_active_is_deflected(tmp_path: Path) -> None:
    ...
    class BlockingClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
    ...
    codify = BlockingClient()
    codify.route_status = "idle"  # server LOOKS idle (the race window)
    service, _pool, _setup = _service(store, codify)
    ...
    assert codify.turns == [("conv_existing", "first")]
    busy = [e for e in slack.ephemerals if "still working on your previous" in e["text"].lower()]
    assert len(busy) == 1
    release.set()
    await service.shutdown()
...
async def test_message_while_awaiting_action_points_to_pending_request(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    codify = FakeCodifyClient()
    codify.route_status = "waiting"
    codify.route_pending_elicitation = True
    service, _pool, _setup = _service(store, codify)
...
async def test_idle_follow_up_message_runs_in_thread(tmp_path: Path) -> None:
    ...
    assert codify.created == []
    assert codify.turns == [("conv_existing", "follow up while idle")]
...
async def test_unconfigured_user_is_prompted_and_no_turn_runs(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    codify = FakeCodifyClient()
    service, _pool, setup = _service(store, codify)
...
async def test_turn_runs_against_the_fixed_operator_server(tmp_path: Path) -> None:
    ...
    slack = FakeSlackClient()
    codify = FakeCodifyClient()
    service, pool, _setup = _service(store, codify)
    ...
    assert pool.requested == ["http://codify.test"]
    assert codify.created[0][0] == "ag_custom"
    record = await store.get_session(ThreadKey("T1", "C1", "100.1"))
    assert record is not None
    assert record.owner_user_id == "U1"


class ServerUnreachableClient(FakeCodifyClient):
    async def create_session(self, agent_id: str, title: str) -> str:
        raise ServerUnreachableError("boom")


class HostUnavailableClient(FakeCodifyClient):
    async def launch_runner(
        self, session_id: str, *, workspace: str, host_id: str | None = None
    ) -> str:
...
class AuthRequiredClient(FakeCodifyClient):
    async def create_session(self, agent_id: str, title: str) -> str:
        raise AuthRequiredError("401")


class ServerErrorClient(FakeCodifyClient):
    async def create_session(self, agent_id: str, title: str) -> str:
        # Mirrors a 500 from POST /v1/sessions: a bare CodifyError, NOT one of
        # the specifically-handled subclasses.
        raise CodifyError("Codify request failed with 500: internal_error")
...
async def test_unreachable_server_prompts_config_command(tmp_path: Path) -> None:
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = ServerUnreachableClient()
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_auth_required_mid_stream_prompts_relogin(tmp_path: Path) -> None:
    ...
    class AuthMidStreamClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
...
async def test_server_error_creating_session_reports(tmp_path: Path) -> None:
    # A 500 from create_session raises a bare CodifyError (not one of the
    ...
    assert await store.get_session(ThreadKey("T1", "C1", "100.1")) is None
    text = slack.posts[-1]["text"]
    # A GENERIC startup-failure message — the raw CodifyError detail (which can
...
async def test_no_online_host_prompts_omni_host_command(tmp_path: Path) -> None:
    ...
    assert await store.get_session(ThreadKey("T1", "C1", "100.1")) is None
    text = slack.posts[-1]["text"]
    assert "omni host --server http://codify.test" in text
    assert "/codify" in text


class HarnessNotConfiguredClient(FakeCodifyClient):
    async def launch_runner(
        self, session_id: str, *, workspace: str, host_id: str | None = None
    ) -> str:
        raise HarnessNotConfiguredError(
            "host failed to launch runner: claude CLI not found; run codify setup"
        )
...
async def test_harness_not_configured_412_surfaces_server_message(tmp_path: Path) -> None:
    ...
    # server's message must reach the user so they know to run `codify setup`,
    # not a generic "request failed".
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = HarnessNotConfiguredClient()
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
class ApprovalClient(FakeCodifyClient):
...
class PreambleThenCommittedAnswerClient(FakeCodifyClient):
...
async def _wait_for_resolved(codify: "FakeCodifyClient", count: int = 1) -> None:
...
async def test_tool_approval_approve_resumes_turn(tmp_path: Path) -> None:
    ...
    await _wait_for_resolved(codify)
    await service.shutdown()

    assert delivered is True
    # Verdict forwarded to the server as accept, then the turn resumed.
    assert codify.resolved == [("conv_1", "elicit_1", True)]
...
async def test_idle_stream_flushes_buffered_text_before_turn_end(
    ...
    class IdleThenEndClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
    ...
    codify = IdleThenEndClient(final_text="partial answer")
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_tool_approval_deny_forwards_decline(tmp_path: Path) -> None:
    ...
    assert codify.resolved == [("conv_1", "elicit_1", False)]
...
async def test_delivery_failed_releases_server_park_at_turn_end(tmp_path: Path) -> None:
    ...
    assert (eid, True) in codify.resolve_calls
    assert (eid, False) in codify.resolve_calls
...
async def test_denied_approval_does_not_resurrect_prior_answer(tmp_path: Path) -> None:
    ...
    class DeniedNoAnswerClient(FakeCodifyClient):
        async def run_turn(
            self,
            session_id: str,
            text: str,
    ...
    codify.latest_message = "PRIOR TURN SUMMARY — should not be re-sent"
    codify.latest_message_id = "prior-msg"
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_stale_approval_click_is_reported_as_not_delivered(tmp_path: Path) -> None:
    store = await _store(tmp_path)
    service, _pool, _setup = _service(store, FakeCodifyClient())
...
async def test_form_elicitation_forwards_selections_as_content(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = ApprovalClient(elicitation_id="elicit_form", event=_form_elicitation_event())
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
    ...
    assert codify.resolved == [("conv_1", "elicit_form", True)]
    assert codify.resolved_content == [{"store": "Redis"}]
...
async def test_unsupported_typed_input_links_to_web_ui(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = ApprovalClient(elicitation_id="elicit_typed", event=_typed_input_elicitation_event())
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
    ...
    assert "http://codify.test/approve/conv_1/elicit_typed" in links[0]["text"]
    assert codify.resolved == []
    assert not any(
...
async def test_url_mode_binary_renders_approval_card(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = ApprovalClient(elicitation_id="elicit_url", event=_url_binary_elicitation_event())
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
    ...
    assert codify.resolved == [("conv_1", "elicit_url", True)]
...
async def test_post_answer_message_only_committed_is_not_dropped(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = PreambleThenCommittedAnswerClient(_form_elicitation_event())
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
class PreambleThenSilentAfterElicitationClient(FakeCodifyClient):
...
async def test_post_elicitation_answer_recovered_when_stream_silent(tmp_path: Path) -> None:
    ...
    store = await _store(tmp_path)
    slack = FakeSlackClient()
    codify = PreambleThenSilentAfterElicitationClient(_form_elicitation_event())
    ...
    codify.latest_message = "Understood — leaving the file in place."
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
async def test_elicitation_clears_working_placeholder(tmp_path: Path) -> None:
    ...
    codify = ApprovalClient(elicitation_id="elicit_1")
    service, _pool, _setup = _service(store, codify)
    await _configure_user(store, "T1", "U1")
...
class EventScriptClient(FakeCodifyClient):
