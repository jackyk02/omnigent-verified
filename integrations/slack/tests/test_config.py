...
import pytest
from codify_slack.config import ConfigError, Settings, load_settings
...
_REQUIRED = {
    "CODIFY_SLACK_BOT_TOKEN": "xoxb-x",
    "CODIFY_SLACK_APP_TOKEN": "xapp-x",
...
def _set_env(monkeypatch: pytest.MonkeyPatch, **overrides: str) -> None:
    ...
    for key in (
        *_REQUIRED,
        "CODIFY_DEVICE_CLIENT_SECRET",
        "CODIFY_DATA_DIR",
        "CODIFY_SLACK_DATABASE_PATH",
        "CODIFY_SLACK_SERVER_AUTH",
        "CODIFY_SLACK_DATABRICKS_STATE_SECRET",
        "CODIFY_SLACK_DATABRICKS_CLIENT_ID",
        "CODIFY_SLACK_DATABRICKS_CLIENT_SECRET",
        "CODIFY_SLACK_DATABRICKS_SCOPES",
        "CODIFY_SLACK_DATABRICKS_APP_URL",
        "DATABRICKS_HOST",
    ):
...
def test_server_url_rejects_plaintext_http_non_loopback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _set_env(monkeypatch, CODIFY_SERVER_URL="http://codify.example.com")
    with pytest.raises(ValidationError):
        _load()
...
def test_load_settings_reports_only_the_missing_var(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    msg = str(excinfo.value)
    assert "CODIFY_SERVER_URL" in msg
    assert "CODIFY_SLACK_BOT_TOKEN" not in msg  # the ones that ARE set aren't flagged
...
def test_load_settings_invalid_value_raises_friendly_configerror(
    ...
    _set_env(monkeypatch, CODIFY_SERVER_URL="ftp://nope")
...
def test_load_settings_succeeds_with_full_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert settings.server_url == _REQUIRED["CODIFY_SERVER_URL"]


def test_device_client_secret_read_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(monkeypatch, CODIFY_DEVICE_CLIENT_SECRET="sekret")
...
def test_database_path_defaults_under_data_dir(
    ...
    # With CODIFY_DATA_DIR set, the store defaults under it (not the cwd).
    _set_env(monkeypatch, CODIFY_DATA_DIR=str(tmp_path))
    assert _load().database_path == tmp_path / "codify_slack.sqlite3"


def test_database_path_defaults_under_home_when_no_data_dir(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Without CODIFY_DATA_DIR, it falls back to ~/.codify — never the cwd.
    _set_env(monkeypatch)
    assert _load().database_path == Path.home() / ".codify" / "codify_slack.sqlite3"


def test_database_path_env_override_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(monkeypatch, CODIFY_SLACK_DATABASE_PATH="/custom/bot.sqlite3")
...
_DATABRICKS_KNOBS = {
    "CODIFY_SLACK_SERVER_AUTH": "databricks",
    "CODIFY_SLACK_DATABRICKS_CLIENT_ID": "client-id",
    "CODIFY_SLACK_DATABRICKS_CLIENT_SECRET": "client-secret",
    "CODIFY_SLACK_DATABRICKS_STATE_SECRET": "state-secret-0123456789abcdef0123456789",
}
...
def test_databricks_mode_requires_oauth_config(
    ...
    _set_env(monkeypatch, CODIFY_SLACK_SERVER_AUTH="databricks")
    with pytest.raises(ValidationError):
        _load()


@pytest.mark.parametrize("omit", sorted(set(_DATABRICKS_KNOBS) - {"CODIFY_SLACK_SERVER_AUTH"}))
...
def test_databricks_mode_rejects_short_state_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    knobs = {**_DATABRICKS_KNOBS, "CODIFY_SLACK_DATABRICKS_STATE_SECRET": "too-short"}
    _set_env(monkeypatch, **knobs)
    with pytest.raises(ValidationError):
        _load()
...
def test_databricks_scopes_force_openid_and_offline(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(monkeypatch, **_DATABRICKS_KNOBS, CODIFY_SLACK_DATABRICKS_SCOPES="supervisor-agents")
...
def test_databricks_redirect_uri_reuses_callback(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(
        monkeypatch,
        **_DATABRICKS_KNOBS,
        CODIFY_SLACK_DATABRICKS_APP_URL="https://bot.example.com",
    )
...
def test_databricks_rejects_plaintext_app_url(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _set_env(
        monkeypatch,
        **_DATABRICKS_KNOBS,
        CODIFY_SLACK_DATABRICKS_APP_URL="http://bot.example.com",
    )
    with pytest.raises(ValidationError):
        _load()
...
def test_databricks_allows_loopback_app_url(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(
        monkeypatch,
        **_DATABRICKS_KNOBS,
        CODIFY_SLACK_DATABRICKS_APP_URL="http://localhost:8000",
    )
