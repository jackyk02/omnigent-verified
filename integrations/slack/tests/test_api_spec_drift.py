"""Guard the Slack client against Codify API spec drift.

The Slack integration is deliberately decoupled from the ``codify`` server
...
(:data:`CODIFY_ENDPOINTS` in ``fakes.py``, kept next to the fake server) with
that document, so:
...
import pytest
from fakes import CODIFY_ENDPOINTS, CODIFY_RESPONSE_FIELDS
...
_DOCUMENTED = [(m, p) for m, p, documented in CODIFY_ENDPOINTS if documented]
_INTERNAL = [(m, p) for m, p, documented in CODIFY_ENDPOINTS if not documented]
...
def test_documented_endpoint_present_in_openapi(method: str, path: str) -> None:
    ...
    assert path in paths, (
        ...
        f"server drifted. Update CodifyClient and the fakes.py catalog together."
    )
...
def test_internal_endpoint_absent_from_openapi(method: str, path: str) -> None:
    ...
    assert method not in methods, (
        ...
        f"flip it to documented=True in fakes.py's CODIFY_ENDPOINTS."
    )


@_needs_spec
@pytest.mark.parametrize("schema_name", sorted(CODIFY_RESPONSE_FIELDS))
def test_response_fields_present_in_schema(schema_name: str) -> None:
    ...
    assert schema_name in schemas, (
        ...
        f"update the CODIFY_RESPONSE_FIELDS catalog and CodifyClient together."
    )
    properties = schemas[schema_name].get("properties", {})
    for field in CODIFY_RESPONSE_FIELDS[schema_name]:
