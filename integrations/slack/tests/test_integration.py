"""Vertical integration tests: codify-slack ↔ Codify HTTP server.

These drive the REAL ``CodifyClient`` (real ``httpx``) against a fake Codify
server (:class:`FakeCodifyServer`, a ``respx`` router that owns the API
...
the focus on the codify-slack ↔ server logic rather than Slack transport.
"""
...
import respx
from codify_slack.auth_manager import AuthManager
from codify_slack.codify import CodifyClientPool
from codify_slack.models import UserConfig
from codify_slack.service import SlackCodifyService
from codify_slack.setup import SetupFlow
from codify_slack.store import SQLiteStore
from codify_slack.tokens import EncryptedTokenStore
from cryptography.fernet import Fernet
from fakes import FakeCodifyServer, RecordingSlackClient, sse_delta, sse_status

_SERVER = "http://codify.test"
...
async def _wait_for_turns(service: SlackCodifyService, timeout: float = _WAIT_TIMEOUT_S) -> None:
...
# ── Scenario 1: /codify against an auth-required server → login link ─────────


@respx.mock
async def test_codify_command_auth_wall_shows_login_link(tmp_path: Path) -> None:
    """``/codify`` on an auth-enabled server: the bot probes the server, hits the
    auth wall on the agent listing, starts the device grant, and shows the
    verification link in the setup modal — no DM."""
    server = FakeCodifyServer(_SERVER)
...
# ── Scenario 2: /codify happy path → agent/host picker ───────────────────────


@respx.mock
async def test_codify_command_happy_path_shows_picker(tmp_path: Path) -> None:
    """``/codify`` with a valid token: validate() succeeds and the modal advances
    ...
    assert view.get("callback_id") == "codify_setup_select"
...
async def test_app_mention_runs_full_turn_and_streams_answer(tmp_path: Path) -> None:
    ...
    pool = CodifyClientPool()
    service = SlackCodifyService(store=store, pool=pool, setup=_NoopSetup(), server_url=_SERVER)
    client = RecordingSlackClient()
...
async def test_auth_wall_mid_turn_prompts_relogin(tmp_path: Path) -> None:
    ...
    from codify_slack.models import ThreadKey
    ...
    service = SlackCodifyService(store=store, pool=pool, setup=setup, server_url=_SERVER)
    client = RecordingSlackClient()
...
async def test_harness_not_configured_surfaces_curated_message(tmp_path: Path) -> None:
    ...
    server = FakeCodifyServer(_SERVER)
    server.harness_not_configured_message = "Run `codify setup` on host h1 to install claude."
    server.install(respx.mock)
    ...
    assert "codify setup" in surfaced.lower()
