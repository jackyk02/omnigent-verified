...
def test_form_card_renders_inputs_per_question() -> None:
    ...
    inputs = {
        ...
        for b in blocks
        if isinstance(b.get("block_id"), str) and b["block_id"].startswith("codify_q::")
    }
    assert inputs == {
        "codify_q::store": "radio_buttons",
        "codify_q::langs": "checkboxes",
    }
...
def test_parse_form_answers_single_and_multi() -> None:
    ...
    state_values = {
        "codify_q::store": {ACTION_FORM_ANSWER: {"selected_option": {"value": "0"}}},
        "codify_q::langs": {
...
def test_parse_form_answers_omits_unanswered() -> None:
    state_values = {
        "codify_q::store": {ACTION_FORM_ANSWER: {"selected_option": None}},
        "codify_q::langs": {ACTION_FORM_ANSWER: {"selected_options": []}},
    }
