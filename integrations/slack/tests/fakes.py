...
- :class:`RecordingSlackClient` — a Slack Web-API client stand-in that records
  ...
  tests do.
- :class:`FakeCodifyServer` — a ``respx`` router that stands in for the
  Codify HTTP API. It owns the endpoint contract (paths, status codes, body
  shapes drawn from ``CodifyClient``) in ONE place, exposes scenario knobs
...
The point of pairing them: drive a real ``CodifyClient`` (real ``httpx``)
...
try:  # pragma: no cover - import shape only
    from codify_slack.service import _ACK_TEXT
...
# ── Codify API contract the Slack client depends on ─────────────────────────
#
...
#
# Keep in sync with ``CodifyClient`` (integrations/slack/src/codify_slack/
# codify.py) and the login flow (oauth.py / auth_manager.py).
CODIFY_ENDPOINTS: list[tuple[str, str, bool]] = [
    # Setup / validation.
    ("GET", "/health", True),
    ("GET", "/v1/me", True),
    ("GET", "/v1/agents", True),
...
CODIFY_RESPONSE_FIELDS: dict[str, tuple[str, ...]] = {
...
class FakeCodifyServer:
    """A ``respx`` router standing in for the Codify HTTP API.
    ...
    def __init__(self, base_url: str = "http://codify.test") -> None:
    ...
    def install(self, respx_mock: respx.MockRouter) -> FakeCodifyServer:
        b = self.base_url
