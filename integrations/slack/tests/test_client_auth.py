...
import httpx
import respx
from codify_slack.codify import ClientAuth, CodifyClient, CodifyClientPool
...
async def test_bearer_attached_to_requests() -> None:
    ...
    client = CodifyClient(_BASE, auth=auth)
    try:
...
async def test_benign_redirect_does_not_trigger_refresh() -> None:
    ...
    def _record(request: httpx.Request) -> httpx.Response:
        calls.append(request.headers.get("Authorization"))
        return httpx.Response(307, headers={"location": "https://codify.test/v1/agents/"})
...
async def test_pool_keys_by_server_and_user() -> None:
    ...
    pool = CodifyClientPool(auth_resolver=resolver)
    try:
