# Critical User Journeys — Codify Slack bot

The user-facing journeys the bot supports, with the canonical Codify terms and
...
Terms used the way the Codify codebase uses them:

- **Enrollment** — linking a Slack user to their own **Codify identity** on the
  ...
  "login/account creation" — the Codify account already exists; enrollment
  authorizes the bot to act as it.
- **Session** — one Codify conversation. The bot maps **one Slack thread → one
...
Link a Slack user to their Codify identity on the server, so the bot can run
...
server's auth mode (`CODIFY_SLACK_SERVER_AUTH`): `auto` drives the server's
...
- **`/codify` retriggers setup.** Reopens the setup modal any time to
  ...
  change.
- **`/codify logout` unlinks the Slack user.** Revokes the grant on the server
...
- **`@codify` starts a session owned by the mentioner.** A channel
...
- **Only `@`-mention replies reach the server.** Plain channel messages — even
  ...
  **not** forwarded to Codify; only `app_mention` events drive a channel turn
...
`/codify`), no online host (`HostUnavailableError` → how to bring one online),
