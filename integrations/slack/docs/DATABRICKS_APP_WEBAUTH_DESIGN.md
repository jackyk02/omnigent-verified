# Design: codify-slack web-auth page for Databricks-App-hosted servers
...
`codify_slack/databricks_oauth.py` (the OAuth client + PKCE),
`codify_slack/enrollment_state.py` (signed OAuth `state` + nonce),
`codify_slack/webauth.py` (the OAuth redirect callback), and the `databricks`
...
Let a Slack user drive an Codify server that is deployed as a
...
- the Databricks Apps proxy authenticates every request and injects
  ...
  reachable path — see the Codify server deploy's `deploy/databricks/README.md`),
- the Codify server cannot mint its own tokens (`mint_runner_token` returns `None` in
  header mode, `codify/server/auth.py`),
- Slack events arrive over a Socket-Mode websocket with **no** authenticated
  ...
  unauthenticated Codify endpoint to run the existing device flow against.

## Core idea

Register a **custom U2M OAuth app** in the Databricks workspace and have
codify-slack drive its **authorization-code + PKCE** flow through a small web
...
access token as the bearer for that user's requests to the Codify server (Databricks'
...
app requests** (`CODIFY_SLACK_DATABRICKS_SCOPES`; `openid` + `offline_access`
...
the Codify server declares, or its Databricks proxy rejects the token (401
on `/api`, 302→login elsewhere). The Codify server accepts `all-apis`, and we have not
...
rest (Fernet), never logged, and revoked on `/codify logout`; and it is a
...
1. Slack user runs `/codify`; bot has no valid token for
   `(team, user, server)`.
...
4. Databricks redirects back to `GET /auth/callback?code=<single-use>&state=<…>`
   ...
   e. **stashes** the tokens under a single-use confirm id and renders a
      **consent page** naming the exact identities ("You are about to connect
      your Codify `<server>` account `<idp-email>` with Slack user
...
— a credential is never persisted without the user affirming the Codify↔Slack
...
- Calls the Codify server with `Authorization: Bearer <access token>`.
- The Codify server's proxy validates it and injects **`X-Forwarded-Email` for
  the real user** → `server/auth.py` header mode maps it to the Codify user.
- **No Codify server changes.** Identity mapping is entirely the existing
  header path.
...
Identity and transport are independent. The bot talks to the Codify server over
ordinary HTTP request/response + SSE (`codify.py`), which is what it already
...
| Approach | Per-user identity? | Token blast radius | Refresh? | Codify server change? |
|---|---|---|---|---|
...
- **Solves the identity problem with zero Codify server changes** — reuses the
...
- **Confirm-before-store** — the GET exchanges the code but stores nothing; the
  ...
  without the user affirming the exact Codify↔Slack linkage on a named page.

### Weaknesses / risks (current state)

1. **Stored token is `all-apis`-broad today.** We request `all-apis`
   (`CODIFY_SLACK_DATABRICKS_SCOPES`; `openid` + `offline_access` always added),
   ...
   because the token's scopes must be a **superset** of what the Codify server
   declares (or its proxy rejects the token), and we haven't confirmed a narrower
   scope its proxy accepts. **Follow-up:** find the Codify server's minimal accepted
   scope and set it here; until then this is the accepted residual risk.
2. **codify-slack holds refreshable user tokens for many users** — a
...
4. **Consumer vs workspace access** — a Slack user with only consumer access may
   authenticate at the enrollment page but still be rejected by the Codify server
...
- **`state` is unforgeable and single-use.** HMAC-SHA256 (keyed by a dedicated
  `CODIFY_SLACK_DATABRICKS_STATE_SECRET`, separate from the OAuth client secret
...
- **id_token trusted on the strength of TLS.** The `email` claim is read without
  ...
  flow depends on — the workspace host, `CODIFY_SERVER_URL` (carries the bearer),
...
1. **Minimum scopes** the Codify server's proxy will accept (must be a superset of
   the app's declared scopes) — narrow `CODIFY_SLACK_DATABRICKS_SCOPES` from
   the `all-apis` default to the Codify server's exact scope once confirmed.
2. **Pending-verifier durability:** the in-memory PKCE verifier map means a bot
   ...
   `/codify`) and the app must run as a single replica (weakness #3). A
