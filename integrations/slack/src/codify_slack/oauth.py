"""Client side of the Codify browserless-login flows.

A Slack user authorizes this bot to act as their own Codify identity
...
- **oidc** → the server's CLI-login ticket flow (``/auth/cli-login`` +
  ...
  expiry), exactly as the ``codify`` CLI does.
...
# CODIFY_DEVICE_CLIENT_SECRET set only accepts this authorized client.
...
class AuthMode(enum.Enum):
    """The Codify server's auth posture, as probed from ``/v1/me``."""
...
class DeviceGrantUnavailableError(OAuthError):
    ...
    exist (the server has ``CODIFY_DEVICE_GRANT_ENABLED`` off, so the request
...
async def probe_auth_mode(server_url: str, http_timeout: float = 10.0) -> AuthMode:
    """Detect the server's auth mode, mirroring ``codify login``.
    ...
    :param server_url: Base URL of the Codify server.
...
async def _start_device_login(
    ...
    try:
        ...
        # CODIFY_DEVICE_GRANT_ENABLED off), so the request fell through to the
...
class DeviceFlowClient:
    """Talks to a single Codify server's ``/oauth/*`` endpoints.
