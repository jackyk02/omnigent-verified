...
# Auth posture the bot assumes for its Codify server. ``auto`` probes the
...
def _local_data_dir() -> Path:
    ...
    Honors ``CODIFY_DATA_DIR`` (the shared data-isolation knob, so a
    checkout/worktree keeps its own state), else ``~/.codify``. Kept as a
    local copy rather than an import so the standalone ``codify-slack``
    package stays decoupled from codify core.
...
class Settings(BaseSettings):
    ...
    slack_bot_token: str = Field(validation_alias="CODIFY_SLACK_BOT_TOKEN")
    slack_app_token: str = Field(validation_alias="CODIFY_SLACK_APP_TOKEN")

    # The one Codify server this bot talks to. Set by the operator, never
    ...
    server_url: str = Field(validation_alias="CODIFY_SERVER_URL")

    # Optional shared secret proving this socket server is an authorized
    # device-grant client. When the Codify server has
    # CODIFY_DEVICE_CLIENT_SECRET set, this must match; the bot sends it
    # in the X-Codify-Client-Secret header on device authorize/token/
    # revoke. Leave unset when the server doesn't require it.
    device_client_secret: str | None = Field(
        default=None,
        validation_alias="CODIFY_DEVICE_CLIENT_SECRET",
    )

    # Bot SQLite store (thread→session map, user configs, encrypted tokens).
    # Defaults under the runtime data dir (``CODIFY_DATA_DIR`` or
    # ``~/.codify``) so the daemon doesn't depend on its launch cwd — set
    # CODIFY_SLACK_DATABASE_PATH to override.
    database_path: Path = Field(
        default_factory=lambda: _local_data_dir() / "codify_slack.sqlite3",
        validation_alias="CODIFY_SLACK_DATABASE_PATH",
    )
    log_level: str = Field(default="INFO", validation_alias="LOG_LEVEL")

    # Fernet key (urlsafe-base64, 32 bytes) that encrypts the delegated
    # Codify access/refresh tokens at rest in the local SQLite store.
    ...
    token_encryption_key: str | None = Field(
        default=None,
        validation_alias="CODIFY_SLACK_TOKEN_ENCRYPTION_KEY",
    )

    # ── Databricks Apps web-auth (header/proxy-mode servers) ──────────────
    #
    # When the Codify server is deployed as a Databricks App, its proxy
    # asserts identity via a header the bot can't produce from a Socket-Mode
    # event. Set CODIFY_SLACK_SERVER_AUTH=databricks and register a custom U2M
    ...
    server_auth_mode: ServerAuthMode = Field(
        default="auto",
        validation_alias="CODIFY_SLACK_SERVER_AUTH",
    )
    ...
    databricks_oauth_client_id: str | None = Field(
        default=None,
        validation_alias="CODIFY_SLACK_DATABRICKS_CLIENT_ID",
    )
    databricks_oauth_client_secret: str | None = Field(
        default=None,
        validation_alias="CODIFY_SLACK_DATABRICKS_CLIENT_SECRET",
    )
    ...
    databricks_state_secret: str | None = Field(
        default=None,
        validation_alias="CODIFY_SLACK_DATABRICKS_STATE_SECRET",
    )
    ...
    databricks_oauth_scopes: str = Field(
        default="all-apis",
        validation_alias="CODIFY_SLACK_DATABRICKS_SCOPES",
    )
    ...
    databricks_app_url: str | None = Field(
        default=None,
        validation_alias="CODIFY_SLACK_DATABRICKS_APP_URL",
    )
    ...
    def _normalize_server_url(cls, value: str) -> str:
        value = value.strip().rstrip("/")
        if not value.startswith(("http://", "https://")):
            raise ValueError("CODIFY_SERVER_URL must start with http:// or https://")
        # The per-user delegated bearer is sent to this host on every request
        # (see codify.py). Plaintext http:// would transmit that credential in
        ...
        if value.startswith("http://") and not _is_loopback_url(value):
            raise ValueError(
                "CODIFY_SERVER_URL must use https:// (plaintext would leak the "
    ...
    def _check_databricks_config(self) -> Settings:
        ...
        missing = [
            name
            for name, value in (
                ("CODIFY_SLACK_DATABRICKS_CLIENT_ID", self.databricks_oauth_client_id),
                (
                    "CODIFY_SLACK_DATABRICKS_CLIENT_SECRET",
                    self.databricks_oauth_client_secret,
                ),
                ("CODIFY_SLACK_DATABRICKS_STATE_SECRET", self.databricks_state_secret),
        ...
        if missing:
            raise ValueError("CODIFY_SLACK_SERVER_AUTH=databricks requires " + ", ".join(missing))
        ...
        if base.startswith("http://") and not _is_loopback_url(base):
            raise ValueError(
                "CODIFY_SLACK_DATABRICKS_APP_URL must use https:// "
        ...
        if len(self.databricks_state_secret or "") < _MIN_STATE_SECRET_LEN:
            raise ValueError(
                f"CODIFY_SLACK_DATABRICKS_STATE_SECRET must be at least "
...
_REQUIRED_ENV_VARS: dict[str, str] = {
    "CODIFY_SLACK_BOT_TOKEN": "Slack bot token (xoxb-…)",
    "CODIFY_SLACK_APP_TOKEN": "Slack app-level token (xapp-…)",
    "CODIFY_SERVER_URL": "Codify server URL (https://…)",
}
...
def _env_alias_for(field_name: str) -> str:
    ...
    ``CODIFY_SERVER_URL``), not the internal snake_case field (``server_url``).
    """
