...
from codify_slack.codify import OutputFile
from codify_slack.models import ThreadKey
from codify_slack.text import GENERIC_FAILURE_TEXT, truncate_for_slack
...
class SlackNotifier:
    ...
    async def post_session_info(
        ...
        *,
        ...
        lines.append(
            f":globe_with_meridians: <{self._session_web_link(session_id)}|Open in Codify>"
        )
        try:
            await client.chat_postMessage(
                channel=key.channel_id,
    ...
    async def notify_non_owner(
        ...
        await self.post_ephemeral(
            ...
            key,
            user_id,
            "This thread's Codify session belongs to whoever started it, so I "
    ...
    def _session_web_link(self, session_id: str) -> str:
        # Link to the session's conversation page in the Codify web UI, where a
