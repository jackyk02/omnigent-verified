...
from codify_slack.approvals import (
    ClickTarget,
    ElicitationCoordinator,
    Verdict,
)
from codify_slack.auth_manager import pack_user_key
...
from codify_slack.elicitation import ElicitationController, ElicitationTurnState
from codify_slack.models import SlackTurn, ThreadKey, event_is_dm
from codify_slack.notifications import (
    SlackNotifier,
    format_output_file,
    format_policy_denied,
    format_todos,
)
from codify_slack.setup import SetupFlow, host_unavailable_text
from codify_slack.store import SQLiteStore
from codify_slack.streaming import (
    SlackClientProtocol,
    _AnswerReply,
)
from codify_slack.text import GENERIC_FAILURE_TEXT, strip_bot_mention
...
_SERVER_UNREACHABLE_TEXT = (
    ":warning: I couldn't reach your Codify server. If it moved or is "
    "down, run /codify to reconfigure."
)
...
# button instead (see ``SlackCodifyService._notify_auth_expired``), which is
...
def _classify_turn_error(exc: BaseException, server_url: str) -> str | None:
    ...
    if isinstance(exc, HarnessNotConfiguredError):
        ...
        # surface it so the user knows to run `codify setup` on the host.
        return f":warning: {exc}"
    return None


class SlackCodifyService:
    def __init__(
        self,
        *,
        store: SQLiteStore,
        ...
        self._pool = pool
        self._setup = setup
        # The one operator-configured Codify server. Always the routing
        ...
        *,
        ...
        try:
            if not event_is_dm(event):
                # In channels Codify only joins a thread when @-mentioned (which
                ...
                # not be added to the Codify session.
                self._logger.info(
        ...
        if key in self._active_threads:
            ...
            if record is not None and record.owner_user_id != requester:
                ...
                if record is not None:
                    codify = await self._pool.get(
                        self._server_url, pack_user_key(key.team_id, requester)
                    )
                    activity = await codify.get_session_activity(record.session_id)
    ...
    async def _run_turn(self, turn: SlackTurn) -> None:
        ...
        try:
            session_id = await self._ensure_session(turn, codify)
        except _AuthExpired:
        ...
        baseline = await codify.latest_assistant_message(session_id)

        try:
            errored = await self._stream_turn(turn, codify, session_id, reply)
        except _TurnAborted:
        ...
        if reply.needs_fallback_text():
            ...
            latest = await codify.latest_assistant_message(session_id)
            if (
                latest is not None
                and latest != baseline
    ...
    async def _ensure_session(self, turn: SlackTurn, codify: CodifyClient) -> str | None:
        ...
        if record is not None:
            self._logger.info(
                "Using existing Codify session thread=%s session_id=%s",
                turn.key.display(),
                record.session_id,
            )
        ...
        try:
            session_id = await codify.create_session(turn.agent_id, turn.title)
            runner_id = await codify.launch_runner(
            ...
            # Any other startup failure (e.g. a 500 surfaced as CodifyError)
            ...
            self._logger.exception("Failed to start Codify session thread=%s", turn.key.display())
            raise _TurnAborted(
                ":warning: Something went wrong starting your Codify session. Please try "
                "again; if it keeps happening, contact your Codify operator."
            ) from exc
        ...
        self._logger.info(
            "Mapped Slack thread to new Codify session thread=%s session_id=%s runner_id=%s",
            turn.key.display(),
            session_id,
            runner_id,
        )
        ...
        try:
            info = await codify.get_session_info(session_id)
    ...
    async def _stream_turn(
        ...
        try:
            ...
            try:
                while True:
                    ...
                    await self._dispatch_stream_event(
                        event, turn, codify, session_id, reply, state
                    )
            finally:
            ...
            self._logger.exception("Codify turn failed for %s", turn.key.display())
            state.errored = True
        finally:
            ...
            await self._elicitation.finish_pending(codify, turn, state.elicitations)
        if state.aborted:
    ...
    async def _dispatch_stream_event(
        ...
        if elicitation is not None:
            ...
            await self._elicitation.start(codify, turn, elicitation, state.elicitations)
            return
        ...
        if event_error:
            ...
            self._logger.warning(
                "Codify in-band turn error thread=%s: %s", turn.key.display(), event_error
            )
            state.errored = True
...
async def _session_title(
    ...
    """Build the Codify session title: ``Slack: <thread permalink>``.
