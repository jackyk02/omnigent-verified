...
- ``POST /auth/callback`` — submitted by the consent page's Confirm button.
  ...
  persisted without the user affirming the Codify↔Slack account linkage.
...
The state signing/verification lives in :mod:`codify_slack.enrollment_state`;
the OAuth client in :mod:`codify_slack.databricks_oauth`. See
...
from codify_slack.config import Settings
from codify_slack.databricks_oauth import (
...
from codify_slack.enrollment_state import (
    EnrollmentState,
    StateError,
    emails_match,
    new_nonce,
...
from codify_slack.tokens import TokenStore
...
class WebAuthServer:
    ...
    def enrollment_url(
        ...
        redirect URI isn't configured (no ``CODIFY_SLACK_DATABRICKS_APP_URL``),
    ...
    async def _handle_callback(self, request: web.Request) -> web.Response:
        ...
        the exact Codify + Slack identities; the token is persisted to the store
    ...
    async def _handle_confirm(self, request: web.Request) -> web.Response:
        ...
        Codify↔Slack linkage on a page that named both identities.
        """
...
def _identity_summary(
    *, verb: str, server_url: str, idp_email: str, slack_email: str, team_name: str
    ...
    return (
        f"You are {verb} your Codify <b>{html.escape(server_url)}</b> account "
...
def _consent_page(
    *, confirm_id: str, server_url: str, idp_email: str, slack_email: str, team_name: str
    ...
    # Codify + Slack identities being linked and requires an explicit Confirm (a
    ...
    message = (
        ...
        "user act as you and use your Codify account. Close this tab instead."
        "<br><br>"
    ...
    return _page("Confirm your Codify connection", message)


def _success_page(*, server_url: str, idp_email: str, slack_email: str, team_name: str) -> str:
    workspace = f" in Slack workspace <b>{html.escape(team_name)}</b>" if team_name else ""
    summary = (
        f"You connected your Codify <b>{html.escape(server_url)}</b> account "
        f"<b>{html.escape(idp_email)}</b> with Slack user "
        f"<b>{html.escape(slack_email)}</b>{workspace}."
    )
    message = (
        ...
        "this, run <code>/codify logout</code> in Slack."
    )
