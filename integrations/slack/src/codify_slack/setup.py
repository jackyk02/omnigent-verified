...
from codify_slack.auth_manager import AuthManager, pack_user_key, slack_client_id
...
from codify_slack.events import host_id_of
from codify_slack.models import UserConfig
from codify_slack.oauth import DeviceGrantUnavailableError, OAuthError
from codify_slack.store import SQLiteStore
from codify_slack.text import truncate_option
...
ACTION_SETUP_START = "codify_setup_start"
...
CALLBACK_SETUP_INFO = "codify_setup_info"
CALLBACK_SELECT_MODAL = "codify_setup_select"

# Slash command that lets a user (re)configure their Codify setup.
COMMAND_NAME = "/codify"
...
class SetupFlow:
    """Per-user Codify setup for the operator-configured server.

    The bot talks to one fixed Codify server (``server_url``, set by the
    operator — never entered by a user), so setup no longer asks for a URL.
    Opening ``/codify`` validates connectivity against that server,
    ...
    async def _handle_config_command(self, ack: Any, command: dict[str, Any], client: Any) -> None:
        # ``/codify`` (or ``/codify config``) opens setup against the fixed
        ...
        # ``/codify logout`` revokes every server token and clears all saved
    ...
    async def _handle_logout(self, *, team_id: str, user_id: str, client: Any) -> None:
        """Handle ``/codify logout`` — full reset for the user.
        ...
        await self._dm_user(
            ...
            user_id,
            text=(
                f":wave: Logged out{servers}. Your Codify settings were "
                "cleared — run `/codify` to set up again."
            ),
    ...
    async def prompt_unconfigured(
        ...
        *,
        ...
        await self._dm_user(
            client,
            user_id,
            text="Set up Codify to start using me.",
        ...
        if in_channel:
            await client.chat_postEphemeral(
                ...
                user=user_id,
                thread_ts=thread_ts,
                text="Let's get you set up — check your DM with me to configure Codify.",
            )
    ...
    async def prompt_relogin(
        ...
        *,
        ...
        delivered = await self._dm_user(
            client,
            user_id,
            text="Your Codify login has expired. Sign in again to keep going.",
        ...
        if in_channel:
            await client.chat_postEphemeral(
                ...
                user=user_id,
                thread_ts=thread_ts,
                text=("Your Codify login has expired — check your DM with me to sign in again."),
            )
        return True
    ...
    async def _begin_setup(self, client: Any, *, team_id: str, user_id: str, view_id: str) -> None:
        ...
        codify = await self._pool.get(server_url, pack_user_key(team_id, user_id))
        try:
            validated = await codify.validate()
            ...
            # the moment the user approves — no DM, no re-running /codify.
            ...
            await client.views_update(
                view_id=view_id,
                view=login_failed_modal(
                    server_url, "Could not reach the Codify server. Try again shortly."
                ),
            )
            return

        await self._advance_to_select(ack, codify, server_url, validated)
    ...
    async def _advance_to_select(
        ...
        workspace_default = await self._resolve_default_workspace(codify, validated.online_hosts)
        await ack(
    ...
    async def _revalidate_and_advance(
        ...
        *,
        ...
        codify = await self._pool.get(server_url, user_key)
        try:
            validated = await codify.validate()
            await self._advance_to_select(
                _ViewUpdateAck(client, view_id), codify, server_url, validated
            )
        except Exception as exc:
            ...
            await self._fail_modal(
                ...
                "validating it. Ask your Codify operator to confirm the OAuth "
        ...
        *,
        ...
        try:
            ...
            await client.views_update(
                ...
                view=login_failed_modal(
                    server_url,
                    "the Codify server doesn't support Device Authorization Grant. "
                    "Please contact your Codify server administrator.",
                ),
            )
            return
        ...
        if not email:
            await client.views_update(
                ...
                view=login_failed_modal(
                    ...
                    "`users:read.email` scope, then run `/codify` again.",
                ),
            )
            return
    ...
    async def _team_name(self, client: Any, team_id: str) -> str:
        ...
        Used only to label the ``client_id`` sent to the Codify server.
    ...
    async def _resolve_default_workspace(
        self, client: CodifyClient, online_hosts: list[dict[str, Any]]
    ) -> str:
    ...
    async def _handle_select_submit(
        ...
        await ack()
        self._logger.info(
            "Saved Codify setup team=%s user=%s server=%s agent=%s host=%s",
            team_id,
            user_id,
            server_url,
            config.agent_id,
    ...
    return [
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "⚙️ Set up Codify"},
                    "style": "primary",
    ...
    return [
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": (
                    ":lock: *Your Codify login has expired.*\n"
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "🔑 Sign in to Codify"},
                    "style": "primary",
                    "action_id": ACTION_SETUP_START,
                }
            ],
...
def connecting_modal() -> dict[str, Any]:
    ...
    return {
        ...
        "title": {"type": "plain_text", "text": "Set up Codify"},
        "close": {"type": "plain_text", "text": "Cancel"},
        "blocks": [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": "Connecting to Codify…"},
            }
        ],
    }
        ...
        "blocks": [
            {
                ...
                "text": {
                    ...
                    "text": (
                        ...
                        "Add an agent on the server, then run `/codify` again."
                    ),
                },
            }
        ],
