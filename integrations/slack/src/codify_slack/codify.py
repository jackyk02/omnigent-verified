...
# ``from codify_slack.codify import extract_delta`` sites keep working.
from codify_slack.events import (
    CodifyError,
    ElicitationOption,
    ElicitationQuestion,
    ElicitationRequest,
...
class RunnerUnavailableError(CodifyError):
    pass


class AuthRequiredError(CodifyError):
    """The Codify server rejected an unauthenticated request (HTTP 401).
...
class ServerUnreachableError(CodifyError):
    """The Codify server could not be reached at all (transport failure)."""


class TokenRefreshTransientError(CodifyError):
...
class StreamInterruptedError(CodifyError):
...
class HostUnavailableError(CodifyError):
...
class HarnessNotConfiguredError(CodifyError):
    """The selected harness isn't configured on the host (HTTP 412).

    A precondition failure the user resolves by running ``codify setup`` on the
...
class ValidatedServer:
    """Outcome of probing an Codify server during Slack setup."""
...
class CodifyClient:
    ...
    def _unreachable(self, exc: httpx.HTTPError) -> ServerUnreachableError:
        ...
        return ServerUnreachableError(
            f"Could not reach Codify server at {self._client.base_url}: {exc}"
        )
    ...
    async def check_health(self) -> None:
        ...
        self._logger.debug("Probing Codify server health")
    ...
    async def create_session(self, agent_id: str, title: str) -> str:
        ...
        self._logger.info("Creating Codify session agent_id=%s", agent_id)
        ...
        if session_id is None:
            ...
            raise CodifyError("Codify server returned no session id.")
        self._logger.info("Created Codify session session_id=%s", session_id)
        return session_id

    async def submit_message(self, session_id: str, text: str) -> None:
        self._logger.info(
            "Submitting Slack message to Codify session_id=%s chars=%s",
            session_id,
            len(text),
        )
        payload = {
        ...
        await _raise_for_status(response)
        self._logger.debug("Submitted Codify message session_id=%s", session_id)
    ...
    async def resolve_elicitation(
        ...
        *,
        ...
        """
        self._logger.info(
            "Resolving Codify elicitation session_id=%s elicitation_id=%s accepted=%s "
            "has_content=%s",
            session_id,
            elicitation_id,
            accepted,
    ...
    async def launch_runner(
        ...
        *,
        ...
        if not workspace:
            raise CodifyError(
                "A workspace path is required to launch an Codify runner. "
                "Re-run setup and set a workspace."
            )
        target_host = host_id or await self._select_random_online_host()
        self._logger.info(
            "Launching Codify runner session_id=%s host_id=%s workspace=%s",
            session_id,
            target_host,
            workspace,
        )
        ...
        if response.status_code in (404, 409):
            self._logger.warning(
                "Codify host unavailable host=%s status=%s body=%r",
                target_host,
                response.status_code,
                response.text,
            )
            raise HostUnavailableError(f"Codify host {target_host} is not available.")
        await _raise_for_status(response)
        payload = response.json()
        ...
        if runner_id is None:
            ...
            raise CodifyError("Codify server returned no runner id.")

        await self.wait_for_runner_online(runner_id)
        self._logger.info(
            "Launched Codify runner session_id=%s runner_id=%s host_id=%s",
            session_id,
            runner_id,
            target_host,
        )
    ...
    async def list_agents(self) -> list[dict[str, Any]]:
        self._logger.debug("Listing built-in Codify agents")
        agents = await self._get_list("/v1/agents", "data", "agents")
        self._logger.info("Found built-in Codify agents count=%s", len(agents))
        return agents

    async def list_hosts(self) -> list[dict[str, Any]]:
        self._logger.debug("Listing Codify hosts")
        hosts = await self._get_list("/v1/hosts", "hosts", "data")
        self._logger.info("Found Codify hosts count=%s", len(hosts))
        return hosts
        ...
        while True:
            ...
            if asyncio.get_running_loop().time() >= deadline:
                raise HostUnavailableError(
                    f"Timed out waiting for launched Codify runner to come online: {runner_id}"
                )
    ...
    async def _select_random_online_host(self) -> str:
        ...
        if not host_ids:
            raise HostUnavailableError("No online Codify hosts are available to launch a runner.")
        host_id = random.choice(host_ids)
        self._logger.info(
            "Selected random Codify host host_id=%s candidates=%s",
            host_id,
            len(host_ids),
        )
        return host_id
    ...
    async def stream_session_events(
        ...
        try:
            async with self._client.stream(
                ...
                await _raise_for_status(response)
                connected = True
                self._logger.debug("Connected to Codify SSE stream session_id=%s", session_id)
                yield iter_sse_events(response.aiter_lines())
        except httpx.HTTPError as exc:
            if connected:
                raise StreamInterruptedError(
                    f"Codify stream to {self._client.base_url} dropped mid-turn: {exc}"
                ) from exc
            raise self._unreachable(exc) from exc
        ...
        while True:
            ...
            try:
                    ...
                    try:
                        while True:
                            ...
                            if not done:
                                ...
                                pending.cancel()
                                self._logger.info(
                                    "Codify stream silent for %ss (no heartbeat) — "
                            ...
                            self._logger.debug(
                                "Received Codify event session_id=%s type=%s",
                                session_id,
                                event.get("type"),
                            )
                            ...
                            if is_hard_terminal_event(event):
                                self._logger.info(
                                    "Codify turn reached hard-terminal event "
                                    "session_id=%s type=%s",
                                    session_id,
                                    event.get("type"),
                                )
                            ...
                            if status in ("running", "waiting") and response_id is not None:
                                ...
                                if id_bearing_match or id_less_end:
                                    self._logger.info(
                                        "Codify turn ended session_id=%s status=%s response_id=%s",
                                        session_id,
                                        status,
                                        response_id,
                                    )
                ...
                if (
                    ...
                    self._logger.info(
                        "Codify stream dropped and reconnect exhausted "
                ...
                if activity.status in ("idle", "failed"):
                    self._logger.info(
                        "Codify stream dropped; server reports turn ended status=%s session_id=%s",
                        activity.status,
                        session_id,
                    )
                    return
                self._logger.info(
                    "Codify stream dropped mid-turn; reconnecting (attempt %s) session_id=%s: %s",
                    attempt,
                    session_id,
                    exc,
                )
    ...
    async def latest_assistant_message(self, session_id: str) -> tuple[str | None, str] | None:
        ...
        fails).
        """
        self._logger.debug("Fetching latest Codify assistant item session_id=%s", session_id)
...
class CodifyClientPool:
    ...
    def __init__(
        ...
        self._clients: dict[tuple[str, str], CodifyClient] = {}
        self._lock = asyncio.Lock()
    ...
    async def get(self, server_url: str, user_id: str = "") -> CodifyClient:
        ...
        async with self._lock:
            client = self._clients.get(key)
            if client is None:
                client = CodifyClient(key[0], timeout=self._timeout, auth=auth)
...
async def _raise_for_status(response: httpx.Response) -> None:
    try:
        ...
        _logger.warning(
            "Codify request failed status=%s url=%s had_bearer=%s body=%r",
            response.status_code,
        ...
        if response.status_code == 503 and error_code == "runner_unavailable":
            raise RunnerUnavailableError("Codify runner is unavailable.") from exc
        # A 3xx redirect means an auth proxy in front of the server is bouncing
        # an unauthenticated request to its login page — the codify API itself
        ...
        if response.is_redirect:
            raise AuthRequiredError(
                f"Codify server requires authentication for {response.request.url}"
            ) from exc
        if response.status_code == 401:
            raise AuthRequiredError(
                f"Codify server requires authentication for {response.request.url}"
        ...
        if response.status_code == 412 and error_code == "harness_not_configured":
            ...
            # on the host — run `codify setup` there). The server's structured
        ...
        raise CodifyError(f"Codify request failed with status {response.status_code}.") from exc
