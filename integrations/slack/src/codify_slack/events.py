...
class CodifyError(RuntimeError):
    """Base error for the Codify client and its event parsing."""
...
class ElicitationRequest:
    ...
    The Codify server parks a running turn when a tool call trips an approval
...
def session_status(event: dict[str, Any]) -> tuple[str, str | None] | None:
    ...
    this lives in ``CodifyClient._run_turn_once``.
    """
