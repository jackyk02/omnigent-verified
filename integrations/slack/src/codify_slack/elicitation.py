...
Extracted from ``SlackCodifyService`` so that class is left with event
...
from codify_slack.codify import CodifyClient, ElicitationRequest
from codify_slack.models import SlackTurn, ThreadKey

if TYPE_CHECKING:
    from codify_slack.streaming import SlackClientProtocol
...
class ElicitationController:
    ...
    async def start(
        ...
        if not request.is_supported:
            await self._post_reply(
                ...
                key,
                (
                    ":link: Codify needs input I can't collect here "
        ...
        try:
            posted = await client.chat_postMessage(
                ...
                text="Codify needs your input to continue.",
                blocks=elicitation_card_blocks(request, turn.owner_user_id),
            )
        except Exception:
            await self._abandon_unpostable(codify, request, key, reason="post failed")
            return
        ...
        if not isinstance(card_ts, str):
            await self._abandon_unpostable(codify, request, key, reason="no message ts")
            return
        pending = PendingElicitation(request=request, card_ts=card_ts)
        state.pending[request.elicitation_id] = pending
        resolver = asyncio.create_task(self._resolve_verdict(codify, request, pending))
    ...
    async def _abandon_unpostable(
        ...
        *,
        ...
        with contextlib.suppress(Exception):
            await codify.resolve_elicitation(
                request.session_id,
                request.elicitation_id,
                accepted=False,
                content=None,
    ...
    async def finish_pending(
        self, codify: CodifyClient, turn: SlackTurn, state: ElicitationTurnState
    ) -> None:
        ...
        for _eid, pending in list(state.pending.items()):
            ...
            if server_park_released:
                ...
                await self._decline_abandoned(codify, pending)
    ...
    async def _decline_abandoned(self, codify: CodifyClient, pending: PendingElicitation) -> None:
    ...
    def _approve_link(self, session_id: str, elicitation_id: str) -> str:
        # Deep link to the elicitation's approve page in the Codify web UI, so
