...
from codify_slack.auth_manager import AuthManager, pack_user_key
from codify_slack.codify import CodifyClientPool
from codify_slack.config import ConfigError, load_settings
from codify_slack.databricks_oauth import DatabricksOAuthClient
...
from codify_slack.tokens import EncryptedTokenStore, InMemoryTokenStore, TokenStore
from codify_slack.webauth import WebAuthServer
...
async def run() -> None:
    ...
    try:
        ...
        print(f"codify-slack: {exc}", file=sys.stderr)
    ...
    logger.info(
        "Starting Codify Slack bot server=%s database=%s",
        settings.server_url,
    ...
    if settings.token_encryption_key:
        ...
        logger.warning(
            "CODIFY_SLACK_TOKEN_ENCRYPTION_KEY not set — delegated tokens will "
    ...
    # The bot talks to one operator-configured Codify server
    ...
    # hit the workspace, not the Codify server. The web server shares the token
    ...
    try:
        ...
        logger.info("Shutting down Codify Slack bot")
...
def register_handlers(app: AsyncApp, service: SlackCodifyService) -> None:
