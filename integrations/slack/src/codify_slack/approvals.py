...
from codify_slack.codify import ElicitationRequest
from codify_slack.text import truncate_for_slack, truncate_option
...
ACTION_APPROVE = "codify_approve_tool"
ACTION_DENY = "codify_deny_tool"
ACTION_FORM_SUBMIT = "codify_form_submit"
ACTION_FORM_CANCEL = "codify_form_cancel"
...
ACTION_FORM_ANSWER = "codify_form_answer"

# Per-question input blocks are keyed ``codify_q::<question_key>`` so the
# submit handler can map each answer back to its question without extra state.
_QUESTION_BLOCK_PREFIX = "codify_q::"
...
def resolved_card_blocks(
    ...
    if outcome is ElicitationOutcome.TIMED_OUT:
        ...
        text += "\n_I couldn't deliver your answer to Codify. Send your message again to retry._"
...
def parse_form_answers(state_values: dict[str, Any]) -> dict[str, Any]:
    ...
    Reads each ``codify_q::<key>`` input block: a radio yields the single
