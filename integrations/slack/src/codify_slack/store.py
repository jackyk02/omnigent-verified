...
from codify_slack.models import SessionRecord, ThreadKey, UserConfig
...
class SQLiteStore:
        ...
        async with aiosqlite.connect(self._path) as db:
            ...
            await db.execute(
                ...
                CREATE TABLE IF NOT EXISTS thread_sessions (
                    ...
                    codify_session_id TEXT NOT NULL,
                    title TEXT NOT NULL,
                    owner_user_id TEXT,
                    host_id TEXT,
                    workspace TEXT,
    ...
    async def get_session(self, key: ThreadKey) -> SessionRecord | None:
        async with aiosqlite.connect(self._path) as db:
            cursor = await db.execute(
                """
                SELECT codify_session_id, owner_user_id, host_id, workspace
    ...
    async def clear_user_data(self, team_id: str, user_id: str) -> None:
        ...
        Backs ``/codify logout``: after this the user is fully reset —
        their agent/host/workspace choice is gone and their channel/DM
        threads no longer map to any Codify session, so a later message
