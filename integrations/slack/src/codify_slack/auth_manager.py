...
from codify_slack.codify import ClientAuth, TokenRefreshTransientError
from codify_slack.oauth import (
...
class TokenRotator(Protocol):
    ...
    hits ``/oidc/v1/token``, not the Codify server), so the AuthManager takes
...
def slack_client_id(team_name: str) -> str:
    ...
    ``"Slack-Codify-Acme Corp"``). Not the user — the per-user
    distinction lives in the token store key. Falls back to a bare
    ``"Slack-Codify"`` when the workspace name is unavailable.
    """
    team_name = team_name.strip()
    return f"Slack-Codify-{team_name}" if team_name else "Slack-Codify"
...
class AuthManager:
    ...
    def __init__(
        ...
        *,
        ...
        # workspace OAuth app instead of the Codify server's /oauth/* endpoints.
        ...
        # ``/codify`` (or a Slack-redelivered slash command) supersedes the
