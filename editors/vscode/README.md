...
A minimal VS Code extension that opens your **running local Codify server** inside
...
[codify-ai/codify#1219](https://github.com/codify-ai/codify/issues/1219)):
...
- On activation the extension discovers a locally running server via
  `~/.codify/local_server.pid` and a `/health` probe (or uses `codify.serverUrl`
  when set to a localhost URL).
- The Codify activity-bar view offers an **Open Codify** button. The
  **Codify: Open** command (`codify.open`) — also on the editor title bar and in the
...
|---|---|---|
| `codify.serverUrl` | `""` | Manual **localhost** server URL override (e.g. `http://127.0.0.1:6767`); empty = auto-discover. Non-localhost URLs are not supported in this build. |
...
pnpm run package      # @vscode/vsce package -> codify-vscode-<version>.vsix
```
...
├── commands/openPanel.ts  # the codify.open command
