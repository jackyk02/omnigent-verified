/**
 * Iframe host HTML for the Codify editor panel.
 *
 * The webview hosts a single static <iframe> pointed at the running LOCAL
 * Codify server — a local server needs no auth, so no token ever appears in
...
<head>
  ...
  <style nonce="${nonce}">
    ...
    #codify-frame { border: 0; width: 100%; height: 100%; display: block; }
  </style>
</head>
<body>
  <div id="root">
    ...
    -->
    <iframe id="codify-frame" src="${escapeAttr(src)}" allow="clipboard-read; clipboard-write" style="border:0;width:100%;height:100%"></iframe>
  </div>
</body>
</html>`;
}
