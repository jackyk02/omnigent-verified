/**
 * Shared render helper for the Codify editor panel.
 *
 * The Codify UI renders in the editor-beside `WebviewPanel`
...
/** Render the Codify iframe host into a webview. Sets `webview.html`. */
export function renderInto(webview: vscode.Webview, opts: RenderIntoOptions): void {
  ...
  opts.log?.(
    `[codify] iframe rendered (origin=${opts.target.origin}, nonce=${nonce.slice(0, 8)}...)`,
  );
}
