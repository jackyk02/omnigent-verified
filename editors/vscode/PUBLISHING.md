# Publishing the Codify VS Code extension

This directory holds the Codify VS Code extension. It is published
...
[`codify.yml`](https://github.com/databricks/secure-public-registry-releases-eng/blob/main/.github/workflows/codify.yml)
(codify's PyPI release, which already checks out `codify-ai/codify`
...
5. **Smoke-test the `.vsix` locally.** Download the `.vsix` from the published
   ...
   ```bash
   gh release download vscode-v<version> --repo codify-ai/codify --pattern '*.vsix'
   code --install-extension codify-vscode-<version>.vsix
   ```

   Reload VS Code, start a local server (`codify server`), and run the
   **Codify: Open** command — confirm it opens an editor pane showing the
   ...
   marketplaces.
6. **Publish to the marketplaces.** Dispatch `codify-vscode.yml` in the
...
| 5   | Ask DECO to register `codify-vscode` under the `databricks` publisher + add dedicated `OMNI_VSCE_TOKEN` / `OMNI_OVSX_PAT` secrets (and an `codify-vscode-marketplace` environment for the reviewer gate)                                                                                                                                    | Slack `#dev-ecosystem-discuss` ([https://databricks.slack.com/archives/C01KSAWFXG8/p1782971196701749](https://databricks.slack.com/archives/C01KSAWFXG8/p1782971196701749)) | human approval      |
| 6   | Add an `codify-vscode.yml` publish workflow in the secure repo, adapting the existing [`databricks-vscode.yml`](https://github.com/databricks/secure-public-registry-releases-eng/blob/main/.github/workflows/databricks-vscode.yml) (SAML SSO required) — it already does download → scan → `vsce publish` + `ovsx publish` in one workflow | `secure-public-registry-releases-eng`                                                                                                                                       | DECO grant (step 5) |
...
`gh release download -R databricks/databricks-vscode`. The codify variant
must point `-R` at `codify-ai/codify` and relax the filename check (it
hard-codes `databricks-*-${VERSION}.vsix`; ours is `codify-vscode-*.vsix`).
- We author `codify-vscode.yml` ourselves — it's our own pipeline, not a fork
...
(`vscode-extension-release.yml` and `codify-vscode.yml`), and we control
both.
- Cross-org fetch from the public `codify-ai/codify` repo is already an
accepted pattern — `codify.yml` checks out `codify-ai/codify` for the
PyPI release. So reading a public codify release from the hardened runner is
not a new blocker.
...
`codify-vscode-marketplace` environment for an independent reviewer gate.)
