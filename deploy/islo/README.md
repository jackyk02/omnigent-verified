...
running Codify hosts, two ways:

- **CLI-launched**: `codify sandbox create` / `connect` provisions a
...
uses the Islo Python SDK, installed with the optional `codify[islo]`
...
- **No local port forward.** Islo can't forward a sandbox→laptop callback
  port, so the interactive in-sandbox `codify login` / App OAuth step is
...
Install Codify with the Islo extra, install the
...
```bash
pip install 'codify[islo]'                   # or: uv tool install 'codify[islo]'
curl -fsSL https://islo.dev/install.sh | sh   # install the islo CLI
islo login                                     # browser OAuth (one-time)
islo api-key create codify --show            # prints an islo_key_… value
...
> interactive `codify login` browser flow (and the in-sandbox App OAuth
...
Sandboxes boot from `ghcr.io/codify-ai/codify-host:latest`, published
by CI from the `host` target of
[`deploy/docker/Dockerfile`](../docker/Dockerfile) with Codify and its
...
Then point Codify at it — `CODIFY_ISLO_HOST_IMAGE` for the CLI flow,
...
```bash
codify sandbox create --provider islo --server https://your-host
```
...
```bash
codify sandbox connect --provider islo \
...
a new Islo account/key. It assumes your Codify server is reachable from
Islo's cloud at `https://your-host` (for local testing, expose it with a
tunnel and use the public URL).

```bash
islo login
islo api-key create codify-smoke --show
export ISLO_API_KEY=islo_key_...
codify sandbox create --provider islo --server https://your-host
codify sandbox connect --provider islo \
  --sandbox-id <id-printed-by-create> \
...
`connect` registers the host with the Codify server, `islo ls` shows the
...
`CODIFY_ISLO_SANDBOX_ENV` in your shell to a comma-separated list of
...
authentication. The interactive `codify login` browser flow can't run
inside an Islo sandbox (no callback port forward), so inject the keys for
the relevant server instead — name them in `CODIFY_ISLO_SANDBOX_ENV`
before `create`:

```bash
export CODIFY_ISLO_SANDBOX_ENV=DATABRICKS_HOST,DATABRICKS_TOKEN
codify sandbox create --provider islo
```
...
Codify resumes the same sandbox id, mints a fresh launch token, and
restarts `codify host` against the existing workspace. Deleting the
...
- one **runner tunnel** per session (`/v1/runners/<token>/tunnel`), opened
  ...
  *whatever server credential it can resolve* — a proxy-injected identity
  (header / OIDC), or a stored `codify login` token (local hosts only; a
...
- **The built-in `accounts` provider (`CODIFY_AUTH_ENABLED=1`)** — the
...
`accounts` provider is fine for CLI-launched hosts (you `codify login`,
...
sandbox:
  ...
  islo:
    ...
    snapshot_name: codify-host-snapshot          # optional named Islo snapshot
...
[Option B](#option-b--codify-env-injection-your-own-key-or-a-subscription).
...
you configure **no model credential on the Codify side at all**. The
flow:
...
The Codify server only ever holds `ISLO_API_KEY` — the credential it
...
gateway automatically. The only Codify-side knob is which gateway a
...
- **No model secret lives in the Codify server's config or
  environment** — nothing to leak there. Contrast [Option B under managed
  hosts](#option-b--codify-env-injection-your-own-key-or-a-subscription),
...
### Option B — Codify env injection (your own key or a subscription)

Bring your own credential by naming it in `CODIFY_ISLO_SANDBOX_ENV`
...
`CODIFY_RUNNER_ENV_PASSTHROUGH=NAME1,NAME2`.
...
via `CODIFY_ISLO_SANDBOX_ENV` / `sandbox.islo.env`. The host image's git
...
- **The agent terminal is sandboxed away from those secrets.** Native
  ...
  (`~/.ssh`, `~/.aws`, the injected `~/.codify` server token) and pins
...
- **Idle pause.** Server-managed Islo sandboxes pause after 15 idle
  ...
  pause/resume lifecycle because the workspace survives and Codify can
  wake it on the next message. Daytona's 15-minute provider default is
  disabled in Codify instead, because Daytona auto-stop would otherwise
...
- **Claude shows "API Usage Billing" / "both `CLAUDE_CODE_OAUTH_TOKEN` and
  ...
  credential is named in `CODIFY_ISLO_SANDBOX_ENV` / `sandbox.islo.env` (the
...
- **"managed host did not come online within 120s."** Check that
  ...
  in-sandbox host log: `~/.codify/logs/host-runner/*.log`.
- **Agent has no credentials.** Verify the injected var names match the
  forwarded set above (or are named in `CODIFY_RUNNER_ENV_PASSTHROUGH`),
...
| `CODIFY_ISLO_HOST_IMAGE` | CLI machine / server | Override the host image ref (`sandbox.islo.image` takes precedence for managed) |
| `CODIFY_ISLO_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.islo.env` takes precedence for managed) |
| `CODIFY_RUNNER_ENV_PASSTHROUGH` | inside the sandbox (injected) | Extra env var names the host forwards to runners |
