...
Deploy Codify to Render in one click. Render provisions the app and a
managed Postgres database, assigns an HTTPS URL on `*.onrender.com`, and
handles SSL automatically. No local tooling required.

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/codify-ai/codify)

> **Note:** The button points at the public repo `github.com/codify-ai/codify`.
> It goes live once that repo **and** the `ghcr.io/codify-ai/codify-server`
...
- **codify** (Starter web service) — pulls the pre-built image
  `ghcr.io/codify-ai/codify-server:latest` (CI-built; ships the web UI
  bundle), served on `https://codify-<hash>.onrender.com`. While the GHCR
...
- **codify-db** (`basic-256mb` managed Postgres) — `DATABASE_URL` is injected
...
> (e.g. a headless deploy), add `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD` in the
...
1. Go to `github.com/settings/developers` → **New OAuth App**.
   - Homepage URL: `https://codify-<hash>.onrender.com`
   - Authorization callback URL:
     `https://codify-<hash>.onrender.com/auth/callback`
   - Click **Register application**, then **Generate a new client secret**.

2. In the Render dashboard, open the **codify** service → **Environment**
   ...
   | `CODIFY_OIDC_CLIENT_ID` | your GitHub OAuth client ID |
   | `CODIFY_OIDC_CLIENT_SECRET` | your GitHub OAuth client secret |
   | `CODIFY_OIDC_REDIRECT_URI` | `https://codify-<hash>.onrender.com/auth/callback` |

   Also add `CODIFY_OIDC_COOKIE_SECRET` = a 64-hex-char value from
...
| `CODIFY_OIDC_CLIENT_SECRET` | your client secret |
| `CODIFY_OIDC_REDIRECT_URI` | `https://codify-<hash>.onrender.com/auth/callback` |
| `CODIFY_OIDC_ALLOWED_DOMAINS` | `example.com` (critical — see note below) |

> **Important:** Without `CODIFY_OIDC_ALLOWED_DOMAINS`, any Google account
> can log in when the OAuth consent screen is "External." Always restrict to
> your domain.

### Generic OIDC (Okta, Auth0, Keycloak, Entra ID)

Set `CODIFY_OIDC_ISSUER` to your IdP's base URL (the one that publishes
`/.well-known/openid-configuration`). The rest of the variables are the same
as above.

## Custom domain

In the Render dashboard, open the **codify** service → **Settings** →
...
Update `CODIFY_OIDC_REDIRECT_URI` to use the custom domain after DNS
propagates.
...
1. In the Render dashboard, open the **codify** service.
