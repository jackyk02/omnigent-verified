...
- Codify server running locally (e.g. `codify server` or
...
CODIFY_WS_ALLOWED_ORIGINS=https://<machine>.ts.net

# Public base URL — used to build the correct __Host- cookie prefix
# and any invite / magic-link URLs.
CODIFY_ACCOUNTS_BASE_URL=https://<machine>.ts.net
```

Without `CODIFY_WS_ALLOWED_ORIGINS` the browser will get WebSocket close
code `4403` and an HTTP 403 *"Forbidden: this endpoint requires a trusted
Origin header"* on chat and file uploads. Without `CODIFY_ACCOUNTS_BASE_URL`
...
Cloud sandbox providers (Modal, Daytona, E2B, …) run the Codify host
...
| Variable | Purpose |
|---|---|
| `CODIFY_WS_ALLOWED_ORIGINS` | Comma-separated origin allowlist. Set to `https://<machine>.ts.net` to trust the Tailscale origin for WebSocket and multipart routes. |
| `CODIFY_ACCOUNTS_BASE_URL` | Public base URL. Used for session cookie security (`__Host-` prefix) and invite / magic-link URLs. |
| `CODIFY_AUTH_ENABLED` | `1` to require login. Recommended when using Tailscale Funnel (public internet exposure). |
