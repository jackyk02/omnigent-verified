...
micro-VM + OCI runtime ("SQLite for sandboxing"). It runs each Codify
...
"managed"`, starts `codify host` inside it, and removes it when the
session is deleted. (There is no `codify sandbox create` CLI bootstrap
...
- **Local** (default — no `cloud:` block): BoxLite is embedded in the
  Codify server process. **No daemon, no `boxlite serve`, no root.**
...
```bash
pip install 'codify[boxlite]'   # installs the boxlite SDK extra
```
...
Add a `sandbox:` block to your server config (`codify server -c …` /
`CODIFY_CONFIG` / `<data_dir>/config.yaml`).
...
sandbox:
  provider: boxlite
  server_url: https://codify.example.com   # the in-box host dials this back
```
...
sandbox:
  ...
  boxlite:
    image: docker.io/me/codify-host:latest     # optional, shared; default: official
...
sandbox:
  ...
  server_url: https://codify.example.com
  boxlite:
    image: ghcr.io/acme/codify-host:latest   # shared
...
| `CODIFY_BOXLITE_HOST_IMAGE` | Override the host image (alternative to `sandbox.boxlite.image`). |
| `CODIFY_BOXLITE_SANDBOX_ENV` | Comma-separated SERVER env var names to inject into boxes (alternative to `sandbox.boxlite.env`). |
...
2. Network defaults to full egress, so the in-box host can reach
   `server_url`.
3. The server runs `codify host` inside the box (over `box.exec`) with
   ...
   same host/runner machinery every Codify host uses — the agent's
...
the in-box host logs to `/tmp/codify-host.log`.

## Limitations

- **Managed-only.** The `codify sandbox create` / `connect` CLI
