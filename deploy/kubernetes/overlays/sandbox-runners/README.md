...
`host_type: managed` session spawns one **runner Pod** that runs `codify host`
...
then runs `codify host` under a tiny PID-1 reaper. The host re-parents runner
...
| Namespace | Holds |
|---|---|
| `codify` | the server, its DB/PVC, its Secrets, the `codify-server` SA |
| `codify-sandboxes` | runner Pods, the per-Pod token Secrets, the harness-creds Secret, the powerless `codify-runner` SA, the scoped Role + RoleBinding |

The server SA's Pod/Secret rights are a **namespaced Role** bound (cross-namespace)
to `codify-sandboxes` only — so a compromised server can manage runner Pods but
...
1. **A server image built with the `kubernetes` extra.** The overlay's
   `images:` block already points at the official `codify-server-kubernetes`
   variant, which includes it — nothing to build. If you self-build instead,
   keep `kubernetes` in `CODIFY_EXTRAS` (see `deploy/docker`) or
...
2. **Harness credentials.** The runners read their LLM / git credentials from a
   Secret named by `secret_name` (default `codify-creds`); you create it out of
...
kubectl create secret generic codify-creds -n codify-sandboxes \
...
to run as `codify-server` with the config mounted. Step 2 supplies the model /
...
A fresh runner Pod has no model keys. They ride the **`codify-creds` Secret**
...
`codify-creds` Secret. The host image's git credential helper answers HTTPS auth
...
| `host_config` | Optional, top-level under `sandbox:` (provider-agnostic, not inside `kubernetes:`): verbatim in-sandbox `~/.codify/config.yaml` content installed before `codify host` starts — e.g. a `providers:` block routing the `pi` harness through a self-hosted gateway (LiteLLM/vLLM). Server-managed: entries injected by a previous launch are replaced or removed on the next launch/resume; config created inside the sandbox survives. Keep secrets out via `api_key_ref: env:VAR`, resolved inside the runner Pod against the `secret_name` Secret. Validated at server startup. |
| `namespace` | Runner-Pod namespace (defaults to `codify-sandboxes`). |
...
| `kubeconfig` | Optional kubeconfig path for the out-of-cluster fallback (env: `CODIFY_KUBERNETES_KUBECONFIG`). |
...
1. Create the PV/PVC **in the runner namespace** (`codify-sandboxes`) out of
   band — via your GitOps repo, with whatever backend your cluster provides
   (NFS/SMB CSI drivers, SAN, cloud disks). Codify only references the claim;
   it never creates volumes, so the server RBAC stays unchanged.
2. List the claims under `sandbox.kubernetes.pvc_mounts` (see
   `sandbox-config.yaml`). Mount paths may not overlap `/home/codify`, the
...
- **Inspect a stuck launch:** `kubectl describe pod <pod> -n codify-sandboxes`
  and `kubectl logs <pod> -n codify-sandboxes -c host` (or `-c workspace-prep`
  for the clone step).
- **403 on launch:** the server SA is missing the Role — re-apply this overlay
  and confirm the cross-namespace RoleBinding subject namespace is `codify`.
- **Runner Pod stuck in `CreateContainerConfigError`:** the `secret_name` Secret
  (`codify-creds`) doesn't exist in the runner namespace — its `envFrom` is
...
- **401 / "could not load Kubernetes configuration":** out of cluster, the server
  can't find a kubeconfig — set `kubeconfig` (or `CODIFY_KUBERNETES_KUBECONFIG`),
