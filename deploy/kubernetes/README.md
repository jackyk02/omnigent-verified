# Codify on Kubernetes

Deploy Codify to any Kubernetes cluster using Kustomize. The manifests pull
...
- **Deployment** — single-replica pod running
  `ghcr.io/codify-ai/codify-server`, served on port 8000.
...
1. **Edit the secret** — set your real `DATABASE_URL` and generate a cookie
   ...
   DATABASE_URL: "postgresql+psycopg://user:pass@your-db-host:5432/codify"
   CODIFY_ACCOUNTS_COOKIE_SECRET: "$(openssl rand -hex 32)"
   ```

2. **Set your domain** *(skip if you're not using the Ingress)* — replace
   `codify.example.com` in `base/ingress.yaml` with your domain, and make sure
...
1. **Edit secrets** — in `overlays/postgres/secret-patch.yaml`, replace
   ...
   DATABASE_URL: "postgresql+psycopg://codify:<strong-password>@postgres:5432/codify"
   CODIFY_ACCOUNTS_COOKIE_SECRET: "$(openssl rand -hex 32)"
   ```
...
1. **Edit the configmap patch** — set `CODIFY_SANDBOX_SERVER_URL` to the
   ...
   CODIFY_SANDBOX_SERVER_URL: "https://codify.example.com"
...
2. **Edit secrets** — in `overlays/openshell/secret-patch.yaml`, set the
   ...
   ```bash
   DATABASE_URL: "postgresql+psycopg://codify:<password>@your-db-host:5432/codify"
   CODIFY_ACCOUNTS_COOKIE_SECRET: "$(openssl rand -hex 32)"
...
3. **Gateway access** — the server pod needs to reach the OpenShell gateway's
   ...
   `openshell-gateway-config` in the `codify` namespace and the deployment
...
```bash
# from the repo root
docker build -t codify-server:ubi -f deploy/docker/Dockerfile.ubi .
```
...
2. **Set your route hostname** — replace `codify.apps.example.com` in
...
`codify host` as its entrypoint and dials back over the launch-token tunnel. It
...
overlay swaps in the official `codify-server-kubernetes` image variant, which
...
# then create the codify-creds harness Secret (see the overlay README)
```
...
- **Model keys** (`ANTHROPIC_API_KEY` / `CLAUDE_CODE_OAUTH_TOKEN` / `OPENAI_API_KEY`
  / `GIT_TOKEN` / …) ride the `codify-creds` Secret projected into every runner Pod.
...
```bash
kubectl get pods -n codify          # codify (and, with the overlay, postgres) → Running
kubectl rollout status deploy/codify -n codify
kubectl logs -n codify deploy/codify          # server logs

# Port-forward the Service and open the app locally:
kubectl port-forward -n codify svc/codify 8000:80
...
domain that already resolves to localhost — `codify.localtest.me` or
...
```bash
codify login https://codify.example.com          # authenticate the CLI
codify host  --server https://codify.example.com # register this machine
```
...
```bash
kubectl create secret generic codify-oidc -n codify \
  --from-literal=CODIFY_AUTH_PROVIDER=oidc \
  --from-literal=CODIFY_OIDC_ISSUER=https://github.com \
  --from-literal=CODIFY_OIDC_CLIENT_ID=<client-id> \
  --from-literal=CODIFY_OIDC_CLIENT_SECRET=<client-secret> \
  --from-literal=CODIFY_OIDC_REDIRECT_URI=https://codify.example.com/auth/callback \
  --from-literal=CODIFY_OIDC_COOKIE_SECRET=$(openssl rand -hex 32)
```

Then add `envFrom: [{secretRef: {name: codify-oidc}}]` to the Deployment
container spec (or merge the values into `codify-secrets`).
