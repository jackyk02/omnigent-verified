# Codify on Cloudflare (Containers + D1 + R2)

Run the Codify server on **Cloudflare Containers**, with **D1** as the
...
> Cloudflare D1 isn't yet first-class in Codify. It works end to end — it's how
...
browser ───────────────►  Worker (src/index.js)
                              ...
                              ▼
                          Container  ──►  the codify server (port 8000)
                          (1 instance)        │            │
...
- **Worker** — a thin front that proxies every request to **one** container
  instance (Codify keeps an in-memory runner registry, so it's single-replica).
- **Container** — the official `ghcr.io/codify-ai/codify-server` image plus
...
- **R2** is the artifact store. Cloudflare container disk is **ephemeral**, so
  ...
  Codify's native `S3ArtifactStore`, selected with
  `CODIFY_ARTIFACT_URI=s3://<bucket>`. No FUSE mount, no sidecar.
...
```bash
npx wrangler r2 bucket create codify-artifacts
```
...
openssl rand -hex 32 | npx wrangler secret put CODIFY_ACCOUNTS_COOKIE_SECRET
...
```bash
npx wrangler deploy
# -> https://codify.<your-subdomain>.workers.dev
```

The container cold-starts on the first request (~10s), then stays warm:

```bash
curl https://codify.<your-subdomain>.workers.dev/health   # {"status":"ok"}
```
...
```bash
codify login https://codify.<your-subdomain>.workers.dev
codify host  --server https://codify.<your-subdomain>.workers.dev
```
