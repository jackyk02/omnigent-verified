# Codify on NVIDIA OpenShell

[NVIDIA OpenShell](https://github.com/NVIDIA/OpenShell) is a self-hosted sandbox
provider. Codify connects to an OpenShell **gateway** with the official
...
This guide covers the Codify-specific OpenShell setup:

- install the `openshell` extra;
- select a working OpenShell gateway;
- use an OpenShell-compatible Codify host image;
- configure CLI-launched or server-managed sandboxes.

```bash
uv pip install 'codify[openshell]'
```

Codify uses OpenShell two ways:

- **CLI-launched**: `codify sandbox create` / `connect` provisions a sandbox
...
- **gRPC, and a gateway you select — not an API key.** Codify connects through
  ...
  There is no base-URL or token knob in Codify — gateway setup and auth are an
  OpenShell concern.
- **No local port forward.** OpenShell has no sandbox→laptop callback path, so
  the interactive in-sandbox `codify login` / App OAuth step is skipped
...
automatically — Codify needs no extra configuration.
...
Sandboxes boot from `ghcr.io/codify-ai/codify-host:latest`, published by CI
from the `host` target of [`deploy/docker/Dockerfile`](../docker/Dockerfile) with
Codify and its dependencies preinstalled — including the coding-harness CLIs
...
```bash
docker run --rm --entrypoint sh ghcr.io/codify-ai/codify-host:latest \
  -lc 'id sandbox && command -v ip && command -v nft'
```

To use a different image (a fork, or extra tooling baked in), run the build from
an Codify repository checkout on an amd64 Docker-capable machine, then push it
...
Then point Codify at it with `CODIFY_OPENSHELL_HOST_IMAGE`.
...
```bash
codify sandbox create --provider openshell --server https://your-host
```
...
```bash
codify sandbox connect --provider openshell \
  --sandbox-id <id-printed-by-create> \
  --server https://your-host
```

`connect` runs `codify host` inside the sandbox and holds the connection open
...
To inject LLM/git credentials into the sandbox, set `CODIFY_OPENSHELL_SANDBOX_ENV`
...
```bash
export CODIFY_OPENSHELL_SANDBOX_ENV=ANTHROPIC_API_KEY,GIT_TOKEN
codify sandbox create --provider openshell --server https://your-host
```

## Server-managed sandboxes

Add a `sandbox:` section to the server config (`codify server -c config.yaml`,
...
> host connected. Inject `CODIFY_RUNNER_ENV_PASSTHROUGH` naming the proxy vars so
> the host forwards them:
> ```yaml
> sandbox:
>   openshell:
>     env: [CODIFY_RUNNER_ENV_PASSTHROUGH, …]   # value set in the server env:
> # CODIFY_RUNNER_ENV_PASSTHROUGH=https_proxy,http_proxy,HTTPS_PROXY,HTTP_PROXY,NO_PROXY,no_proxy
> ```
...
`CODIFY_OPENSHELL_SANDBOX_ENV`; the launcher copies the value from your
...
export CODIFY_OPENSHELL_SANDBOX_ENV=ANTHROPIC_API_KEY
```
...
standard set, inject `CODIFY_RUNNER_ENV_PASSTHROUGH=NAME1,NAME2`.
...
> Codify). See the [OpenShell policy docs](https://docs.nvidia.com/openshell).

## Git credentials (private repositories)

Inject an HTTPS token as `GIT_TOKEN` (GitLab: add `GIT_USERNAME=oauth2`) via
`CODIFY_OPENSHELL_SANDBOX_ENV`. The host image's git credential helper answers
...
- **Sandbox identity.** OpenShell assigns each sandbox a petname (e.g.
  `touched-urial`); that name is the handle Codify prints and reuses. The
...
- **Agent has no credentials** — verify the injected var names match the forwarded
  set (or are named in `CODIFY_RUNNER_ENV_PASSTHROUGH`), and that each name was
...
- **Host registers but the runner never comes online / runner log shows
  ...
  the sandbox's proxy vars. Forward them with `CODIFY_RUNNER_ENV_PASSTHROUGH`
...
| `CODIFY_OPENSHELL_HOST_IMAGE` | CLI machine | Override the host image ref (default `ghcr.io/codify-ai/codify-host:latest`); `sandbox.openshell.image` is the managed equivalent |
| `CODIFY_OPENSHELL_SANDBOX_ENV` | CLI machine | Comma-separated launcher-side env var names to inject into the sandbox; `sandbox.openshell.env` is the managed equivalent |
...
- **Full server-managed session** — a `host_type:"managed"` session drove the
  server to provision a sandbox on the gateway, start `codify host` in it (held
