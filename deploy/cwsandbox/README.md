...
disposable cloud machines for running Codify hosts, two ways:
...
- **No local port forward.** CoreWeave Sandbox can't forward a sandbox→laptop
  callback port, so the interactive in-sandbox `codify login` / App OAuth step
...
with Codify and its dependencies preinstalled — including the coding-harness
...
Then point Codify at it — `CODIFY_CWSANDBOX_HOST_IMAGE` for the CLI flow, or
...
```bash
codify sandbox create --provider cwsandbox --server https://your-host
```
...
```bash
codify sandbox connect --provider cwsandbox \
...
`CODIFY_CWSANDBOX_SANDBOX_ENV` in your shell to a comma-separated list of
...
`connect` runs `codify host` inside the sandbox, and that host must present
credentials when it dials back to a server that requires authentication. The
interactive `codify login` browser flow can't run inside a sandbox (no callback
port forward), so inject the keys for the relevant server instead — name them in
`CODIFY_CWSANDBOX_SANDBOX_ENV` before `create`:

```bash
export CODIFY_CWSANDBOX_SANDBOX_ENV=DATABRICKS_HOST,DATABRICKS_TOKEN
codify sandbox create --provider cwsandbox --server https://your-host
```
...
- **The built-in `accounts` provider (`CODIFY_AUTH_ENABLED=1`)** — the runner
...
`accounts` provider is fine for CLI-launched hosts (you `codify login`, and
...
`CODIFY_CWSANDBOX_SANDBOX_ENV` (CLI) or `sandbox.cwsandbox.env` (managed); the
...
`CODIFY_CWSANDBOX_SANDBOX_ENV` / `sandbox.cwsandbox.env`. The host image's git
...
- **The launch token's lifetime tracks the sandbox lifetime.** CW Sandbox's
  lifetime is operator-overridable (`CODIFY_CWSANDBOX_MAX_LIFETIME_S`, 24h
...
- Sandboxes are reaped at `max_lifetime_seconds` (24h default; override with
  `CODIFY_CWSANDBOX_MAX_LIFETIME_S`). The managed launch-token TTL is set above
...
- **Agent has no credentials** — verify the injected var names match the
  forwarded set (or are named in `CODIFY_RUNNER_ENV_PASSTHROUGH`), and that each
...
| `CODIFY_CWSANDBOX_HOST_IMAGE` | CLI machine / server | Override the host image ref (`sandbox.cwsandbox.image` takes precedence for managed) |
| `CODIFY_CWSANDBOX_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.cwsandbox.env` takes precedence for managed) |
| `CODIFY_CWSANDBOX_MAX_LIFETIME_S` | CLI machine / server | Sandbox lifetime cap in seconds (default 24h); also derives the managed launch-token TTL |
...
Validate the API primitives directly (no Codify or SDK install needed — stdlib
+ curl only):
