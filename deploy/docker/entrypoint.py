"""OSS Docker entrypoint for the Codify server.
...
harness subprocesses on its own. Users run ``codify run … --server
...
logger = logging.getLogger("codify-docker")
...
def run_migrations(database_url: str) -> None:
    ...
    from codify.db.utils import _run_migrations as _run_alembic_upgrade
...
def _resolve_config() -> _ResolvedConfig:
    ...
    from codify.db.utils import normalize_database_url
    from codify.server.paas_env import detect_base_url, resolve_bind_host
    from codify.server.server_config import load_server_config

    # ── Configuration ────────────────────────────────────────
    # Non-secret settings come from a YAML config file (default
    # <data_dir>/config.yaml, e.g. /data/config.yaml on the volume, or
    # CODIFY_CONFIG) — the same experience a laptop gets from
    # `codify server -c`. Secrets stay in the environment:
    ...
    if not database_url:
        raise RuntimeError(
            ...
            "Accepted forms: "
            "'postgresql+psycopg://user:pw@host:5432/codify' (explicit psycopg3), "
    ...
    artifact_store_uri = cfg.get("artifact_store_uri") or os.environ.get("CODIFY_ARTIFACT_URI")
    if artifact_store_uri and not artifact_store_uri.startswith("s3://"):
        raise RuntimeError(
            "CODIFY_ARTIFACT_URI (or `artifact_store_uri:` in config) must be an "
    ...
    # The framework-wide default (a bare local `codify server`) is
    ...
    # operator supplied CODIFY_OIDC_* config. An operator can still
    # force header/oidc/accounts via CODIFY_AUTH_PROVIDER, or turn
    # auth off with CODIFY_AUTH_ENABLED=0.
    os.environ.setdefault("CODIFY_AUTH_ENABLED", "1")

    # Kill-switch ergonomics: CODIFY_AUTH_ENABLED=0 means "no login,
    ...
    # CODIFY_AUTH_PROVIDER=header deploy declared a header-injecting
    # proxy and must stay strict.
    from codify.server.auth import env_var_is_truthy

    # Compose passes CODIFY_AUTH_PROVIDER as "" when unset
    ...
    if not _auth_provider_explicit and not env_var_is_truthy("CODIFY_AUTH_ENABLED"):
    ...
    if resolve_auth_source() == "accounts":
        ...
        if not os.environ.get("CODIFY_ACCOUNTS_COOKIE_SECRET"):
            os.environ["CODIFY_ACCOUNTS_COOKIE_SECRET"] = load_or_generate_cookie_secret(
                artifact_dir
            )
        if not os.environ.get("CODIFY_ACCOUNTS_BASE_URL"):
            ...
            os.environ["CODIFY_ACCOUNTS_BASE_URL"] = detect_base_url(
...
def _select_artifact_store(resolved_config: _ResolvedConfig) -> ArtifactStore:
    ...
    durable :class:`~codify.stores.artifact_store.s3.S3ArtifactStore` (AWS S3,
    ...
    :returns: The selected
        :class:`~codify.stores.artifact_store.ArtifactStore`.
    """
    from codify.stores.artifact_store.local import LocalArtifactStore

    if resolved_config.artifact_store_uri:
        from codify.stores.artifact_store.s3 import S3ArtifactStore
...
def build_app(resolved_config: _ResolvedConfig | None = None) -> _BuiltApp:
    ...
    if isinstance(routing_cfg, dict) and routing_cfg.get("provider") == "external":
        from codify.server.smart_routing import ExternalRoutingClient, _bearer_auth
...
def main() -> None:
    ...
    try:
        ...
        logger.info("Starting codify server on %s:%d", resolved.host, resolved.port)
        uvicorn.run(
            resolved.app,
            host=resolved.host,
            port=resolved.port,
        ...
        logger.error("FATAL: codify server failed to start:\n%s", traceback.format_exc())
