...
- `codify` — the server image (built from `../Dockerfile`)
...
docker compose up -d
docker compose logs -f codify       # ctrl-c when boot is clean
```
...
.env` and edit `POSTGRES_PASSWORD` (and `CODIFY_OIDC_COOKIE_SECRET`
...
```bash
codify run path/to/agent.yaml --server http://localhost:8000
```
...
CODIFY_ACCOUNTS_BASE_URL=https://codify.example.com

docker compose up -d
docker compose logs codify      # shows the "No admin yet" line with your base URL
```
...
```bash
CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD=<your-strong-password>
```

`CODIFY_ADMIN_CREDENTIALS_PATH` (set to `/data/admin-credentials`
...
2. **Mint a cookie secret.** `./bootstrap.sh` already did this on the
   quickstart path — `CODIFY_OIDC_COOKIE_SECRET` is set in your
   `.env`. If you skipped it, run `openssl rand -hex 32` and paste the
   value yourself.

3. **Edit `.env`:**
   ```bash
   CODIFY_AUTH_PROVIDER=oidc
   CODIFY_OIDC_ISSUER=https://github.com
   CODIFY_OIDC_CLIENT_ID=Iv1.abc123…
   CODIFY_OIDC_CLIENT_SECRET=…
   CODIFY_OIDC_REDIRECT_URI=https://codify.example.com/auth/callback
   # CODIFY_OIDC_COOKIE_SECRET is already set by bootstrap.sh — leave it alone.
   ```
...
4. **Bring it up.**
   ...
   is missing — check `docker compose logs codify` if it doesn't
   come up.
...
```bash
CODIFY_AUTH_PROVIDER=oidc
CODIFY_OIDC_ISSUER=https://accounts.google.com
CODIFY_OIDC_CLIENT_ID=…apps.googleusercontent.com
CODIFY_OIDC_CLIENT_SECRET=…
CODIFY_OIDC_REDIRECT_URI=https://codify.example.com/auth/callback
CODIFY_OIDC_COOKIE_SECRET=<64-hex-chars>
CODIFY_OIDC_ALLOWED_DOMAINS=example.com,subsidiary.example.com
```
...
Set `CODIFY_OIDC_ISSUER` to the base URL; the server fetches
discovery at startup.

```bash
CODIFY_AUTH_PROVIDER=oidc
CODIFY_OIDC_ISSUER=https://your-tenant.okta.com
...
1. **Use the bundled Caddy overlay** (easiest — any VPS / EC2 / home
   ...
   ```bash
   # In .env:
   CODIFY_DOMAIN=codify.example.com
   CODIFY_ACME_EMAIL=you@example.com      # optional, for Let's Encrypt notices
   ...
   codify container stops being directly exposed and only :80 +
...
2. **Behind an existing reverse proxy** — point your proxy at
   `codify:8000` over the docker network (or `127.0.0.1:8000`
...
an identity header, set `CODIFY_AUTH_PROVIDER=header`. The
...
a different header name set `CODIFY_AUTH_HEADER` to point the
...
`accounts.google.com:`; set `CODIFY_AUTH_HEADER_STRIP_PREFIX` to drop
...
| `POSTGRES_USER` / `POSTGRES_DB` | `codify` | DB user + database name. |
| `CODIFY_PORT` | `8000` | Host port the server is published on. |
| `CODIFY_AUTH_ENABLED` | `1` (in compose) | Master auth switch. `1` → accounts (or oidc if `CODIFY_OIDC_ISSUER` is set); `0` → single-user local mode (every request is the shared `local` user — local dev only, never shared deploys). |
| `CODIFY_AUTH_PROVIDER` | unset | Escape hatch to pin a mode explicitly: `header` / `accounts` / `oidc`. Overrides the `AUTH_ENABLED` auto-selection. |
| `CODIFY_AUTH_HEADER` | `X-Forwarded-Email` | Header-mode only: name of the trusted identity header. Set for proxies that use another name, e.g. `Cf-Access-Authenticated-User-Email` (Cloudflare Access). |
| `CODIFY_AUTH_HEADER_STRIP_PREFIX` | unset (strip nothing) | Header-mode only: prefix removed from the identity header value. Set to `accounts.google.com:` for Google IAP's `X-Goog-Authenticated-User-Email`. |
| `CODIFY_OIDC_*` | unset | OIDC config — required in oidc mode (issuer set, or `AUTH_PROVIDER=oidc`). See `.env.example`. |
...
The same Dockerfile publishes a second image: the official Codify
...
the full codify install (all three packages + deps, `python` and
...
- `ghcr.io/codify-ai/codify-host:latest` — tracks main HEAD
  (the default for `codify sandbox create --provider modal`)
- `ghcr.io/codify-ai/codify-host:sha-<short>` — immutable
  per-commit pin
- `ghcr.io/codify-ai/codify-host:vX.Y.Z` — release tags

Build it locally from the repo root:

```bash
docker build -t codify-host:latest --target host \
             -f deploy/docker/Dockerfile .
```

### Using it with the Modal sandbox provider

`codify sandbox create --provider modal` boots sandboxes from
`ghcr.io/codify-ai/codify-host:latest` by default. Your local
...
| Variable | Purpose |
|---|---|
| `CODIFY_MODAL_HOST_IMAGE` | Override the image ref, e.g. an org-internal copy (`ghcr.io/<your-org>/codify-host:latest`) or a `:sha-<short>` pin. |
| `CODIFY_MODAL_REGISTRY_SECRET` | Name of a [Modal secret](https://modal.com/secrets) holding registry credentials for private pulls. Create it with keys `REGISTRY_USERNAME` (your registry username) and `REGISTRY_PASSWORD` (for GHCR: a personal access token with `read:packages`). Unset = anonymous pull. |
...
`codify sandbox create --provider daytona` CLI flow). Daytona ingests
...
`CODIFY_DAYTONA_HOST_IMAGE` or the server config's
