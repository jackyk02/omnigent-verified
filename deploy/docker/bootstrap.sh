...
# required secrets (POSTGRES_PASSWORD, CODIFY_OIDC_COOKIE_SECRET)
...
#   - If CODIFY_OIDC_COOKIE_SECRET is unset OR commented out,
...
cookie_current=$(current_value CODIFY_OIDC_COOKIE_SECRET)
if [[ -z "$cookie_current" || "$cookie_current" == "<64-hex-chars>" ]]; then
  set_or_replace_kv CODIFY_OIDC_COOKIE_SECRET "$(openssl rand -hex 32)"
  echo "→ generated CODIFY_OIDC_COOKIE_SECRET"
else
  echo "→ CODIFY_OIDC_COOKIE_SECRET already set, leaving alone"
fi
...
# the operator can switch modes by editing CODIFY_AUTH_PROVIDER
# without re-running bootstrap.
accounts_cookie_current=$(current_value CODIFY_ACCOUNTS_COOKIE_SECRET)
if [[ -z "$accounts_cookie_current" || "$accounts_cookie_current" == "<64-hex-chars>" ]]; then
  set_or_replace_kv CODIFY_ACCOUNTS_COOKIE_SECRET "$(openssl rand -hex 32)"
  echo "→ generated CODIFY_ACCOUNTS_COOKIE_SECRET"
else
  echo "→ CODIFY_ACCOUNTS_COOKIE_SECRET already set, leaving alone"
fi

echo
echo "✓ deploy/docker/.env is ready. Next:"
echo "    docker compose up -d && docker compose logs codify"
echo
...
echo "    CODIFY_ACCOUNTS_BASE_URL=<your public URL>"
