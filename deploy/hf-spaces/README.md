...
3. In the Space **Settings -> Variables and secrets**, set:
   ...
   | `CODIFY_ACCOUNTS_COOKIE_SECRET` | secret | `openssl rand -hex 32` (pin it: ephemeral disk would otherwise drop sessions on restart) |
...
4. The Space builds + boots. No admin credential is auto-generated: first boot
   ...
   the admin directly instead, add `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD` as a
...
5. **Log in via the direct URL** `https://<user>-<space>.hf.space` in its own
   ...
   visitor. Pre-seed `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD` (step 4) or claim
