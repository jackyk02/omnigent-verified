"""Modal deploy glue for the Codify server.

Runs the standard server image (``ghcr.io/codify-ai/codify-server``)
...
SERVER_IMAGE = "ghcr.io/codify-ai/codify-server:latest"
...
artifacts = modal.Volume.from_name("codify-artifacts", create_if_missing=True)


@app.function(
    image=modal.Image.from_registry(SERVER_IMAGE),
    # DATABASE_URL, CODIFY_ACCOUNTS_COOKIE_SECRET, and
    # CODIFY_ACCOUNTS_BASE_URL — created in the README's step 1.
    secrets=[modal.Secret.from_name("codify-deploy")],
