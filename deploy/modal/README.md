...
[Modal](https://modal.com) plays two distinct roles for Codify:

1. **[Server deploy target](#deploying-the-server)** — run the
   Codify server itself on Modal as a single always-on web server
   (`modal_app.py` in this directory).
2. **[Sandbox provider](#sandboxes-for-runner-hosts)** — disposable
   cloud machines for running Codify *hosts*, so sessions execute in
...
Run the Codify server on Modal as a single always-on web server.
...
#    deterministic: https://<workspace>--codify-server.modal.run
#    (your workspace name is shown by `modal profile current`).
modal secret create codify-deploy \
  DATABASE_URL='postgres://…neon.tech/…' \
  CODIFY_ACCOUNTS_COOKIE_SECRET="$(openssl rand -hex 32)" \
  CODIFY_ACCOUNTS_BASE_URL='https://<workspace>--codify-server.modal.run'
...
> add `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD=<password>` to the
> `codify-deploy` secret before the first deploy.
...
> visitor. Pre-seed `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD`, or complete setup
...
Add the OIDC values to the `codify-deploy` secret (Modal secrets are
...
modal secret create codify-deploy \
  ...
  CODIFY_OIDC_REDIRECT_URI='https://<workspace>--codify-server.modal.run/auth/callback' \
  CODIFY_OIDC_COOKIE_SECRET="$(openssl rand -hex 32)"
```
...
Pass `custom_domains=["codify.example.com"]` to `@modal.web_server`
in `modal_app.py` (requires a paid Modal plan), point your DNS at Modal
per the printed instructions, and update `CODIFY_ACCOUNTS_BASE_URL`
(or the OIDC redirect URI) to match.

### Upgrading

`modal deploy deploy/modal/modal_app.py` again — Modal re-resolves
`ghcr.io/codify-ai/codify-server:latest`, so a redeploy is an
...
Codify hosts — no laptop tethered to a session, no VM to babysit.
...
```bash
pip install 'codify[modal]'   # installs the modal SDK extra
modal token new                  # one-time browser auth with Modal
```

`modal token new` writes `~/.modal.toml`. Anywhere Codify needs to
...
Sandboxes boot from `ghcr.io/codify-ai/codify-host:latest`, an image
published by CI from the `host` target of
[`deploy/docker/Dockerfile`](../docker/Dockerfile) with Codify
...
```bash
codify sandbox create --provider modal
```
...
```bash
codify sandbox connect --provider modal \
  --sandbox-id <id-printed-by-create> \
  --server https://your-host
```

`connect` runs `codify host` inside the sandbox and holds the
...
`connect` runs `codify host` inside the sandbox, and that host must
present credentials when it dials back to a server that requires
authentication. The interactive `codify login` browser flow can't
...
name it in `CODIFY_MODAL_SANDBOX_SECRETS` (comma-separated) before
running `create`:

```bash
modal secret create codify-server-auth \
  DATABRICKS_HOST=https://example.databricks.com \
  DATABRICKS_TOKEN=<your-pat>
export CODIFY_MODAL_SANDBOX_SECRETS=codify-server-auth
codify sandbox create --provider modal
```
...
sandboxes — any secret named in `CODIFY_MODAL_SANDBOX_SECRETS` lands
...
Add a `sandbox:` section to the server config (`codify server -c
...
sandbox:
  ...
  modal:
    image: docker.io/<you>/codify-host:latest   # default: official image
    secrets: [codify-llm]                       # Modal secrets to inject
```

A top-level `sandbox.host_config:` (provider-agnostic) holds verbatim
in-sandbox `~/.codify/config.yaml` content — e.g. a `providers:`
block routing a harness through a self-hosted gateway — installed into
the sandbox before `codify host` starts. The block is server-managed:
...
| Variable | Enables |
|---|---|
| `CODIFY_ANTHROPIC_API_KEY` or `ANTHROPIC_API_KEY` | Claude models on the Anthropic API (claude-sdk, pi, claude-code harnesses). Prefer the `CODIFY_` form for Claude Code so the raw `ANTHROPIC_API_KEY` env var is not present in the CLI process. |
...
- **Claude with an API key** — put `CODIFY_ANTHROPIC_API_KEY` in the secret.
  Codify resolves it into Claude Code's `apiKeyHelper`; do not also set
...
`CODIFY_RUNNER_ENV_PASSTHROUGH=NAME1,NAME2` to the secret — the
...
```bash
modal secret create codify-git GIT_TOKEN=github_pat_…
```
...
| `CODIFY_MODAL_HOST_IMAGE` | CLI machine / server | Override the host image ref (`sandbox.modal.image` takes precedence for managed) |
| `CODIFY_MODAL_REGISTRY_SECRET` | CLI machine / server | Modal secret name with `REGISTRY_USERNAME` / `REGISTRY_PASSWORD` for private image pulls |
| `CODIFY_MODAL_SANDBOX_SECRETS` | CLI machine / server | Comma-separated Modal secret names to inject (`sandbox.modal.secrets` takes precedence for managed) |
| `CODIFY_RUNNER_ENV_PASSTHROUGH` | inside the sandbox (set via a Modal secret) | Extra env var names the host forwards to runners |
...
`CODIFY_HOST_TOKEN`, `CODIFY_HOST_ID`, `CODIFY_HOST_NAME` —
...
- **Managed launch hangs then fails.** The server waits up to two
  ...
  inspect the host log inside the sandbox: `/tmp/codify-host.log`.
- **Image pull failures.** Private image without
  `CODIFY_MODAL_REGISTRY_SECRET` set, or a secret missing
  `REGISTRY_USERNAME` / `REGISTRY_PASSWORD`.
- **Agent has no credentials.** Verify the Modal secret is listed in
  ...
  above (or are named in `CODIFY_RUNNER_ENV_PASSTHROUGH`).
