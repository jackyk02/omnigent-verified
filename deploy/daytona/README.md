...
machines for running Codify hosts, two ways:
...
```bash
pip install 'codify[daytona]'   # installs the daytona SDK extra
```
...
>
> 1. The in-sandbox host's dial-back to your Codify `server_url` is
...
```bash
codify sandbox create --provider daytona
```
...
```bash
codify sandbox connect --provider daytona \
  --sandbox-id <id-printed-by-create> \
  --server https://your-host
```

`connect` runs `codify host` inside the sandbox (over a PTY session)
...
> the egress allowlist or the in-sandbox `codify host` can't dial
...
`CODIFY_DAYTONA_SANDBOX_ENV` in your shell to a comma-separated list
...
`CODIFY_DAYTONA_SANDBOX_ENV` before `create` — and the in-sandbox
...
plain HTTP and WebSocket upgrades — to your real Codify server, so a
...
cd deploy/daytona
wrangler deploy --var UPSTREAM_URL:https://your-codify-server
# → https://codify-daytona-relay.<your-subdomain>.workers.dev
```

Point `sandbox.daytona.server_url` at the printed `*.workers.dev` URL.
For a non-allowlisted model endpoint, deploy a second copy
(`name = "codify-llm-relay"`, `UPSTREAM_URL` = your gateway) and
...
- **All managed sandboxes share one Daytona org + API key.**
  Cross-user isolation between Codify users rides entirely on
...
- **No platform lifetime cap.** Unlike Modal's 24-hour limit, Daytona
  sandboxes run until deleted. Codify disables Daytona's 15-minute
...
| `CODIFY_DAYTONA_HOST_IMAGE` | CLI machine / server | Override the host image ref (`sandbox.daytona.image` takes precedence) |
| `CODIFY_DAYTONA_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.daytona.env` takes precedence for managed) |
