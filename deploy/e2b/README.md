...
> `ghcr.io/codify-ai/codify-host` directly — E2B cannot start an
> arbitrary registry image at create time. You must first build the
> Codify host image into an E2B template (a one-time step, below); the
...
```bash
pip install 'codify[e2b]'   # installs the e2b SDK extra
...
> Hobby**) with no "never expire" option. Codify requests the 24 h
...
> `CODIFY_E2B_MAX_LIFETIME_S` to request a specific lifetime and skip the
...
**Debian-based** and **single-stage**. The Codify host image
...
```bash
mkdir -p codify-e2b && cd codify-e2b
cat > e2b.Dockerfile <<'EOF'
# Single-stage, Debian-based — both E2B requirements. The host image
# already bakes the full codify install plus git / tmux / curl, so
# nothing else is needed here.
FROM ghcr.io/codify-ai/codify-host:latest
EOF

e2b template build --name codify-host --dockerfile e2b.Dockerfile
```

`codify-host` is the default template name the launcher looks for
([`DEFAULT_E2B_TEMPLATE`](../../codify/onboarding/sandboxes/e2b.py)), so
...
with `sandbox.e2b.template` / `CODIFY_E2B_TEMPLATE`.
...
```bash
codify sandbox create --provider e2b
```

This starts a sandbox from the `codify-host` template, builds wheels
...
```bash
codify sandbox connect --provider e2b \
...
`CODIFY_E2B_SANDBOX_ENV` in your shell to a comma-separated list of
...
> `codify login` / App OAuth step is therefore skipped automatically
...
sandbox:
  ...
  e2b:
    template: codify-host          # E2B template NAME (default: codify-host)
...
> the other providers. Omit it to use the default `codify-host`
> template.
...
`CODIFY_RUNNER_ENV_PASSTHROUGH` (as an injected variable) names any
extras.
...
`DATABRICKS_TOKEN`) in `CODIFY_E2B_SANDBOX_ENV` before `create`. See
...
- **All managed sandboxes share one E2B account + API key.** Cross-user
  isolation between Codify users rides on E2B's sandbox boundaries, and
...
- **The launch token's lifetime is ~25 h, derived from the *requested*
  ...
  across reconnects. The TTL is computed from `CODIFY_E2B_MAX_LIFETIME_S`
  ...
  tighten this, **set `CODIFY_E2B_MAX_LIFETIME_S` to your account cap** so
...
- **Sandbox URLs are public by default.** E2B exposes sandbox ports via
  public `*.e2b.app` URLs; Codify never opens one (the host dials *out*
...
- **"E2B sandbox creation failed: template '…' is unavailable"** — the
  ...
  `--name codify-host` (or set `sandbox.e2b.template` to your name).
- **"managed host did not come online within 120s"** — the sandbox
  ...
  `/tmp/codify-host.log` inside the sandbox.
...
- **Hard lifetime cap, no idle-stop disable.** `provision` requests
  `CODIFY_E2B_MAX_LIFETIME_S` (default the 24 h Pro maximum); E2B rejects
...
- **Custom images** require rebuilding the template: `FROM` your image in
  ...
  `sandbox.e2b.template` / `CODIFY_E2B_TEMPLATE`.
...
| `CODIFY_E2B_TEMPLATE` | CLI machine / server | E2B template name to provision from (`sandbox.e2b.template` takes precedence; default `codify-host`) |
| `CODIFY_E2B_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.e2b.env` takes precedence for managed) |
| `CODIFY_E2B_MAX_LIFETIME_S` | CLI machine / server | Requested sandbox lifetime in seconds (default 24 h); creation auto-clamps to the account cap if exceeded |
