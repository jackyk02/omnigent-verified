...
Codify ships several ways to deploy the server, organized by
...
Codify server is live with HTTPS in a few minutes.

| Platform | Button | Docs |
|---|---|---|
| **Render** | [![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/codify-ai/codify) | [`render/README.md`](render/README.md) |
...
after deploy by setting the `CODIFY_OIDC_*` vars (auth stays enabled; the
...
└── docker/            ← common Docker image + compose stack
    ...
    ├── docker-compose.yaml   codify + postgres for any Docker host
    ├── entrypoint.py
    ├── .env.example
    ├── README.md
    └── SKILL.md
...
> [Codify on Databricks](https://docs.databricks.com/aws/en/codify/)
...
> MLflow Tracing. Enable the **Codify** preview in your workspace
...
Codify runs in two pieces that talk to each other over a
WebSocket tunnel:
...
`codify run …  --server <url>` or `codify claude  --server <url>`.
...
```bash
codify host https://your-host
```

Or point a one-off run at the server directly:

```bash
codify run path/to/agent.yaml --server https://your-host
```
...
needed (`pip install 'codify[modal]'`, `'codify[daytona]'`, or
`'codify[e2b]'`; Islo uses the built-in HTTP client), authenticate
(`modal token new`, `DAYTONA_API_KEY`, `ISLO_API_KEY`, or `E2B_API_KEY`), then:

```bash
codify sandbox create --provider modal     # or --provider daytona / islo / e2b
codify sandbox connect --provider modal --sandbox-id <id> --server https://your-host
```

> [!NOTE]
> Modal caps sandbox lifetime at 24 hours. Re-run `create` + `connect` to
> roll the host onto a fresh sandbox. Daytona and Islo have no Codify-imposed
...
config (`codify server -c config.yaml`, or `<data_dir>/config.yaml`):
...
(`ghcr.io/codify-ai/codify-host:latest`, published by CI from the `host`
target of [`docker/Dockerfile`](docker/Dockerfile)), so the host starts in
seconds instead of installing Codify at boot. The image ships the
...
docker build -f docker/Dockerfile --target host \
  -t docker.io/<you>/codify-host:latest .
docker push docker.io/<you>/codify-host:latest
```
...
sandbox:
  ...
  modal:
    image: docker.io/<you>/codify-host:latest
```

For private registries, set `CODIFY_MODAL_REGISTRY_SECRET` on the server
to the name of a Modal secret holding `REGISTRY_USERNAME` /
`REGISTRY_PASSWORD`; for CLI-launched sandboxes, `CODIFY_MODAL_HOST_IMAGE`
(or `CODIFY_DAYTONA_HOST_IMAGE` / `CODIFY_ISLO_HOST_IMAGE`) overrides the
image ref.
...
`OPENAI_BASE_URL`, `GEMINI_API_KEY`, plus their `CODIFY_`-prefixed
aliases) to its runners:

```bash
modal secret create codify-llm \
  CODIFY_ANTHROPIC_API_KEY=sk-ant-… OPENAI_API_KEY=sk-…
```

Prefer `CODIFY_ANTHROPIC_API_KEY` for Claude Code API-key auth. Codify
...
the standard set, add `CODIFY_RUNNER_ENV_PASSTHROUGH=NAME1,NAME2` to the
...
Auth is driven by a single switch, `CODIFY_AUTH_ENABLED`. The framework
default (a bare local `codify server`) leaves it off: single-user
`header` mode, no login. The containerized deploys here (Docker / HF / Render /
Railway / Modal / Fly) set `CODIFY_AUTH_ENABLED=1` by default in their
entrypoints,
since a network-exposed instance should be authenticated. With the switch on,
the mode is chosen by your config: supply the `CODIFY_OIDC_*` vars and you
get `oidc`, otherwise you get the built-in `accounts` flow.
`CODIFY_AUTH_PROVIDER` is an explicit escape hatch that pins the mode and
overrides this auto-selection.

| Mode | When to use | What's needed |
|---|---|---|
| `accounts` (deploy default) | Standalone deploy, no external IdP: built-in username/password with first-user-is-admin bootstrap and UI-based invites. Opt in with `CODIFY_AUTH_ENABLED=1` (and no OIDC vars). | Set `CODIFY_ACCOUNTS_COOKIE_SECRET` (or let `bootstrap.sh` mint it) and `CODIFY_ACCOUNTS_BASE_URL` (public URL). On first boot, set the admin password via the web Create-admin form, the terminal prompt, or `--admin-password` / `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD`. |
| `oidc` | Standalone deploy with your own IdP: server handles the full login flow | Set `CODIFY_AUTH_ENABLED=1` and the `CODIFY_OIDC_*` env vars; the presence of `CODIFY_OIDC_ISSUER` selects OIDC (or pin `CODIFY_AUTH_PROVIDER=oidc`). Requires HTTPS (the session cookie uses the `__Host-` prefix). |
| `header` | Behind an existing SSO proxy (oauth2-proxy, AWS ALB OIDC, Cloudflare Access, Tailscale Funnel, …) that injects an identity header | The default when `CODIFY_AUTH_ENABLED` is off; or pin `CODIFY_AUTH_PROVIDER=header`. Reads `X-Forwarded-Email` by default; set `CODIFY_AUTH_HEADER` for proxies that use another name (e.g. `Cf-Access-Authenticated-User-Email`), and `CODIFY_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:` for Google IAP. Proxy MUST strip any inbound copy of the header from clients. Missing headers are always rejected. |
...
```dotenv
# Auth is already on (CODIFY_AUTH_ENABLED=1) by default in the deploys here.
# Adding an OIDC issuer flips the mode to single sign-on. No extra flag.
CODIFY_OIDC_ISSUER=https://accounts.google.com     # or https://github.com / your Okta / Entra URL
CODIFY_DOMAIN=agents.yourcompany.com               # your server's domain
CODIFY_OIDC_CLIENT_ID=…
CODIFY_OIDC_CLIENT_SECRET=…
```
...
> `CODIFY_OIDC_ALLOW_INVITES=1` and send them a one-time invite link,
...
```bash
codify debug migrate-accounts-to-oidc <database-url> --domain yourcompany.com
```
...
`header` mode (`CODIFY_AUTH_PROVIDER=header`) takes the caller's identity
...
`CODIFY_AUTH_HEADER` to that name instead of standing up an extra hop to
...
```dotenv
CODIFY_AUTH_PROVIDER=header
CODIFY_AUTH_HEADER=Cf-Access-Authenticated-User-Email
```
...
`CODIFY_AUTH_HEADER_STRIP_PREFIX` to drop that prefix and recover the bare
email:

```dotenv
CODIFY_AUTH_PROVIDER=header
CODIFY_AUTH_HEADER=X-Goog-Authenticated-User-Email
CODIFY_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:
```
