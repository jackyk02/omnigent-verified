...
Deploy Codify to Fly.io. Fly pulls the prebuilt image, runs it next to a
...
- **codify** — a machine that pulls `ghcr.io/codify-ai/codify-server`,
...
2. **Create the first admin.** No credentials are auto-generated. First boot
   ...
   `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD` (`fly secrets set …`) before first
...
The shim is `FROM ghcr.io/codify-ai/codify-server` with nothing added, so
...
fly secrets set \
  ...
  CODIFY_OIDC_CLIENT_ID=<client-id> \
  CODIFY_OIDC_CLIENT_SECRET=<client-secret> \
  CODIFY_OIDC_REDIRECT_URI=https://<your-app>.fly.dev/auth/callback \
  CODIFY_OIDC_COOKIE_SECRET=$(openssl rand -hex 32) \
  -a <your-app>
```

For Google Workspace, also set `CODIFY_OIDC_ALLOWED_DOMAINS` to restrict
