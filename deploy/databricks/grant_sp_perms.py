...
Alembic can create and migrate Codify tables on first boot.
"""
...
class _GrantArgs(Protocol):
    ...
    :param app_name: Databricks App name, e.g. ``"codify"``.
    :param lakebase_endpoint: Full Lakebase endpoint resource path, e.g.
        ``"projects/codify/branches/production/endpoints/primary"``.
...
def main() -> int:
    ...
    print("Done. The app can create and migrate Codify tables on first boot.")
    return 0
