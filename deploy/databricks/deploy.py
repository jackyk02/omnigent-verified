#!/usr/bin/env python3
"""Deploy Codify to a Databricks App via Databricks Asset Bundles.
...
Usage example:
    python deploy/databricks/deploy.py \\
        --app-name codify --profile <your-profile> \\
        --lakebase-branch projects/codify/branches/production \\
        --lakebase-database \\
            projects/codify/branches/production/databases/databricks-postgres \\
        --volume-name main.codify.artifacts
...
_BUNDLE_RESOURCE_KEY = "codify"

_WHEEL_PREFIXES = ("codify-", "codify_client-", "codify_ui_sdk-")
...
def _compute_deploy_version(base: str, explicit: str | None) -> str:
    ...
    # bumps via `.dev` clashes with `codify-ui-sdk` declaring
    # `codify-client>=0.1.0`. `.post` is a final release and
...
def set_version_in_pyproject(path: Path, new_version: str) -> str:
    ...
    (e.g. ``"codify-client==0.1.0rc2"``). Stamping only the
    ...
    # The lockstep graph is circular: codify pins both SDKs, the
    # client pins codify back, and the ui-sdk pins the client. All
    # three names must be stamped or the resolver still dead-ends.
    updated = re.sub(
        r'"(codify(?:-client|-ui-sdk)?)==[^"]+"',
        rf'"\1=={new_version}"',
        updated,
    )
...
def _clean_build_artifacts() -> None:
    ...
    Old Vite bundles in ``codify/server/static/web-ui/``
    ...
    root = _repo_root()
    targets = [
        root / "codify" / "server" / "static" / "web-ui",
        root / "dist",
        root / "build",
        root / "codify.egg-info",
    ]
...
class _ClassifiedWheels:
    ...
    :param main: The top-level ``codify`` wheel — always uploaded
...
def _classify_wheels(wheels: Iterable[Path]) -> _ClassifiedWheels:
    main_wheel = next(w for w in wheels if w.name.startswith("codify-"))
    small: list[Path] = []
    oversize: list[Path] = []
    for wheel in wheels:
...
def _wheel_version(wheel: Path, prefix: str) -> str:
    """Extract a deploy version from an Codify wheel filename.

    :param wheel: Built wheel path, e.g.
        ``dist/codify-0.1.0.post123-py3-none-any.whl``.
    :param prefix: Expected wheel filename prefix, e.g. ``"codify-"``.
...
def _derive_deploy_version_from_wheels(wheels: list[Path]) -> str:
    ...
    """
    expected_prefixes = ("codify-", "codify_client-", "codify_ui_sdk-")
    versions = []
...
def _sweep_local_src_wheels(keep: set[str]) -> None:
    """Delete Codify wheels from src/ whose filename is not in `keep`.
...
def _wheel_source_path(wheel: Path) -> str:
    ...
    :param wheel: Built wheel path, e.g.
        ``dist/codify-0.1.0-py3-none-any.whl``.
...
def _uv_source_lines(
    ...
    :param main_wheel: Top-level ``codify`` wheel path, e.g.
        ``dist/codify-0.1.0-py3-none-any.whl``.
    ...
    for package_name, wheel_prefix in (
        ("codify", "codify-"),
        ("codify-client", "codify_client-"),
        ("codify-ui-sdk", "codify_ui_sdk-"),
    ):
...
def build_uv_pyproject(
    ...
    dependencies = [
        f'"codify[databricks]=={deploy_version}"',
        f'"codify-client=={deploy_version}"',
        f'"codify-ui-sdk=={deploy_version}"',
    ]
    return (
        "[project]\n"
        'name = "codify-databricks-app"\n'
        'version = "0.0.0"\n'
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--app-name",
        required=True,
        help="Databricks App name, e.g. 'codify'.",
    )
    parser.add_argument(
        "--lakebase-branch",
        required=True,
        help=("Full Lakebase branch resource path, e.g. 'projects/codify/branches/production'."),
    )
    parser.add_argument(
        "--lakebase-database",
        required=True,
...
def main() -> int:
    ...
    if classified.oversize:
        raise SystemExit(
            "uv-based Databricks Apps deploys require all Codify wheels "
