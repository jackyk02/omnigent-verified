...
# deployment of Codify.
#
# Inputs:
#   SKIP_WEB_UI=1   Skip the web SPA build for API-only deployments.
#
# Outputs:
#   dist/codify-<version>-py3-none-any.whl
#   dist/codify_client-<version>-py3-none-any.whl
#   dist/codify_ui_sdk-<version>-py3-none-any.whl
...
rm -rf codify/server/static/web-ui dist build codify.egg-info

if [[ "${SKIP_WEB_UI:-}" != "1" ]]; then
    echo "==> Building web SPA into codify/server/static/web-ui/"
...
echo "==> Building codify-client wheel"
uv build --wheel --out-dir dist/ sdks/python-client/

echo "==> Building codify-ui-sdk wheel"
uv build --wheel --out-dir dist/ sdks/ui/

echo "==> Building codify wheel"
