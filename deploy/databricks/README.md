# Deploying Codify on Databricks Apps

This directory deploys the Codify server to
...
> Enable the **Codify** preview in your workspace settings and follow
...
wc.postgres.create_project(project=Project(), project_id="codify")

branch = "projects/codify/branches/production"
...
```sql
CREATE SCHEMA IF NOT EXISTS main.codify;
CREATE VOLUME IF NOT EXISTS main.codify.artifacts;
```
...
python deploy/databricks/grant_sp_perms.py \
    --app-name codify \
    --lakebase-endpoint projects/codify/branches/production/endpoints/primary \
...
uv run python deploy/databricks/deploy.py \
    ...
    --lakebase-database projects/codify/branches/production/databases/databricks-postgres \
...
`databricks bundle run codify --target prod`, and polls `/health`
with backoff until 200.

All Codify wheels must fit under the Databricks Apps source
...
request, so the app pins `CODIFY_AUTH_PROVIDER=header` (see
`src/app.py`).
...
- **Remote state is per app.** `targets.<t>.workspace.root_path` ends
  ...
  under `.bundle/codify/X`. A deploy of X cannot mutate app Y.
...
| `bundle deploy` fails: "Resource already managed by Terraform" | App already bound to another bundle directory | Run from that directory, or unbind: `databricks bundle deployment unbind codify` |
| `bundle deploy` fails: "An app with the same name already exists" | App exists but isn't bound to this bundle (or a stale per-target local cache from a *different* app made `deploy.py` skip the bind) | `rm -rf deploy/databricks/.databricks/bundle/<target>`, then bind: `databricks bundle deployment bind codify <app-name> --target <target> --auto-approve --var ...` |
