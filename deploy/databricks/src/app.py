"""Databricks Apps entry point for codify.

Starts codify with Lakebase (managed PostgreSQL) as the
...
logger = logging.getLogger("codify-app")
...
try:
    ...
    # CODIFY_AUTH_ENABLED in the deploy env (an explicit
    ...
    if __name__ == "__main__":
        logger.info("Starting codify on 0.0.0.0:%d", PORT)
        uvicorn.run(app, host="0.0.0.0", port=PORT)

except Exception:  # noqa: BLE001 — startup catch-all; we want every failure logged
    logger.error("FATAL: codify failed to start:\n%s", traceback.format_exc())
