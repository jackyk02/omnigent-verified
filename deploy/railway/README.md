...
Deploy Codify to Railway. Railway pulls the pre-built image, runs it next to
...
- **codify** — web service that pulls `ghcr.io/codify-ai/codify-server`
...
> The cookie secret is auto-minted and `CODIFY_ACCOUNTS_BASE_URL` is
> auto-detected from `RAILWAY_PUBLIC_DOMAIN`, so those don't need setting. To
> pin a known admin password, set `CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD`
> before first boot.
...
2. In your Railway project, open the **codify** service → **Variables**
   and add:
   ...
   | `CODIFY_OIDC_REDIRECT_URI` | `https://<project>.up.railway.app/auth/callback` |
   | `CODIFY_OIDC_COOKIE_SECRET` | output of `openssl rand -hex 32` |
...
1. In the Railway dashboard, open the **codify** service.
...
loaded Codify instance (few concurrent users) typically stays under
...
2. Point it at `github.com/codify-ai/codify`.
