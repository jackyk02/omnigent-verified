...
const CODIFY_URL = process.env.CODIFY_URL ?? "http://localhost:6767";
...
function resolveToken(host: string): string | null {
  ...
  if (process.env.CODIFY_AUTH_TOKEN) {
    cachedToken = process.env.CODIFY_AUTH_TOKEN;
    return cachedToken;
  }
...
function configureProxy(target: string, useAuth: boolean): NonNullable<ProxyOptions["configure"]> {
  ...
  // e.g. CODIFY_URL=https://host.com/api/2.0/codify means the browser's
  // /v1/sessions is rewritten to /api/2.0/codify/v1/sessions before forwarding.
...
const parsed = new URL(CODIFY_URL);
const useAuth =
  !!process.env.CODIFY_AUTH_TOKEN ||
...
if (useAuth) {
  ...
  if (token) {
    console.log(`[dev-proxy] target=${CODIFY_URL} (authenticated)`);
  } else {
    console.error(
      `\n[dev-proxy] ERROR: No auth token for ${parsed.origin}.\n` +
        `  Set CODIFY_AUTH_TOKEN or run:  databricks auth login --host ${parsed.origin}\n`,
    );
    process.exit(1);
  }
} else {
  console.log(`[dev-proxy] target=${CODIFY_URL}`);
}

const proxyConfig = createProxyConfig(CODIFY_URL, useAuth);
...
const PWA_MANIFEST = {
  ...
  name: "Codify",
  short_name: "Codify",
  description: "Codify — a common layer over coding agents.",
  start_url: "/",
  scope: "/",
  display: "standalone",
  orientation: "any",
...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      // backend's --cov=codify, not just files a test happened to import.
      include: ["src/**/*.{ts,tsx}"],
      exclude: [
        "src/**/*.test.{ts,tsx}",
        "src/**/*.d.ts",
  ...
  build: {
    ...
    outDir: path.resolve(__dirname, "../codify/server/static/web-ui"),
    emptyOutDir: true,
    rollupOptions: {
      output: {
