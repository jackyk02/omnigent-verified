// Intermediate library build for the embeddable Codify island.
//
...
// into an ESM entry (`dist-embed/codify-embed.js`) plus a `chunks/` tree of
...
// stylesheet (`dist-embed/codify-embed.css`). The universe monolith then
// ingests this graph into its OWN rspack graph (see
// `webapp/web/js/genai/codify/embed/loadCodifyEmbed.ts` + the
// `@codify/embed` alias in `app.rsbuild.config.ts`) and emits the FINAL
...
// react-router instance shared with the host — no `__CODIFY_SHARED__`, no MF.
...
//
// The CSS is post-processed to prefix every selector with `.codify-app` so
...
const scopePlugin = (): postcss.Plugin => ({
  ...
  // scoped utility like `.codify-app .text-[11px]`. Since every embed rule is
  // already scoped under `.codify-app`, we don't need layers for internal
  // ordering; dropping them makes the embed compete on specificity, which it
  // wins (`.codify-app .util` ≥ host's typical element selectors).
  AtRule(atRule) {
...
function resolveExternalCjsRequire(externals: readonly string[]): Plugin {
  ...
  const importName = (spec: string) => `__codifyExt_${spec.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  return {
    ...
    renderChunk(code) {
      ...
      const replacement =
        ...
        `\tconst __codifyExternals = { ${table} };\n` +
        `\tif (Object.prototype.hasOwnProperty.call(__codifyExternals, id)) return __codifyExternals[id];\n` +
...
function scopeCodifyCss(): Plugin {
  return {
    name: "scope-codify-css",
    enforce: "post",
...
export default defineConfig({
  ...
  // `/assets/...` string). Monaco is codify_ui's own dep, so Vite builds the
  ...
  // content-hash on its own CDN (see the `@codify/embed` wiring in
  // app.rsbuild.config.ts). rspack owns the FINAL hashed worker name.
  base: "./",
  plugins: [react(), tailwindcss(), scopeCodifyCss(), resolveExternalCjsRequire(SHARED_EXTERNALS)],
  resolve: {
    alias: {
  ...
  build: {
    ...
    // codify-embed.js:<line>. Consumed at the host build via source-map-loader.
    sourcemap: true,
    lib: {
    ...
    rollupOptions: {
      ...
      output: {
        ...
        entryFileNames: "codify-embed.js",
        chunkFileNames: "chunks/[name]-[hash].js",
        assetFileNames: (assetInfo) => {
          const name = assetInfo.names?.[0] ?? "";
          return name.endsWith(".css") ? "codify-embed.css" : "assets/[name].[ext]";
        },
      },
    },
  },
