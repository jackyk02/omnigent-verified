...
Thin Kotlin/`WebView` shell for Codify. Like the Electron app and the iOS
...
runtime via `window.codifyNative` — see `web/src/lib/nativeBridge.ts`. This
...
(`CodifyBridgeListener`) **origin-allowlisted to the pinned server** and
...
(measured insets injected as `--codify-android-safe-area-*`, consumed by the
...
bridge, which closes codify-ai/codify#969 (the iOS shell drops these) —
...
- `CODIFY_KEYSTORE_FILE` — path to the upload keystore
- `CODIFY_KEYSTORE_PASSWORD`
- `CODIFY_KEY_ALIAS`
- `CODIFY_KEY_PASSWORD`
...
```sh
keytool -genkeypair -v -keystore codify-upload.jks \
  -keyalg RSA -keysize 2048 -validity 10000 -alias codify-upload
```
...
> Implementation for codify-ai/codify#1604; not yet exercised on a device
