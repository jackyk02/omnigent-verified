...
val storeFilePath = signingValue("storeFile", "CODIFY_KEYSTORE_FILE")

android {
    namespace = "ai.codify.android"
    compileSdk = 36

    defaultConfig {
        applicationId = "ai.codify.android"
        minSdk = 28
        targetSdk = 36
    ...
    signingConfigs {
        if (storeFilePath != null) {
            create("release") {
                ...
                storePassword = signingValue("storePassword", "CODIFY_KEYSTORE_PASSWORD")
                keyAlias = signingValue("keyAlias", "CODIFY_KEY_ALIAS")
                keyPassword = signingValue("keyPassword", "CODIFY_KEY_PASSWORD")
            }
        }
    }
...
tasks.register("runDebug") {
    ...
    doLast {
        ProcessBuilder(
            ...
            "start",
            "-n",
            "ai.codify.android/.MainActivity",
...
/** Backend (codify server) port — Vite proxies /v1 here (CODIFY_URL default
...
/** The backend (codify server) process we started. */
...
// ---------------------------------------------------------------------------
// Backend (codify server) — isolated, in a temp dir, no auth.
//
// `startBackendServer` launches `codify server` with CODIFY_DATA_DIR /
// CODIFY_CONFIG_HOME pointed at a throwaway mktemp dir so it never touches
// the user's real ~/.codify. On loopback it's single-user (no login wall).
// Vite proxies /v1 to this backend (CODIFY_URL default = localhost:6767),
...
tasks.register("startBackendServer") {
    description =
        "Start an isolated codify backend server (temp data dir, no auth) for real SPA data"
    group = "verification"
    doLast {
        ...
        backendTempDir = Files.createTempDirectory("codify-screenshot")
        ...
        val pb =
            ProcessBuilder(
                ...
                *agentArg.toTypedArray(),
                ...
                .redirectErrorStream(true)
        pb.environment()["CODIFY_DATA_DIR"] = dataDir.absolutePath
        pb.environment()["CODIFY_CONFIG_HOME"] = configDir.absolutePath
        pb.environment()["CODIFY_DATABASE_URI"] = dbUri
...
private val screenshotAppId = "ai.codify.android"
        ...
        for ((screen, sid) in screens) {
            ...
            val instrumentArgs =
                mutableListOf(
                    ...
                    "-e",
                    "class",
                    "ai.codify.android.ScreenshotTest",
                    "-e",
                    "screen",
                    screen,
                    "-e",
