...
/**
 * The single WebView host. Mirrors the iOS `WebShellView` + `CodifyWebView`:
 * loads the server-served SPA, installs the `window.codifyNative` bridge, and
...
class MainActivity : AppCompatActivity() {
            ...
            object : OnBackPressedCallback(true) {
                override fun handleOnBackPressed() {
                    ...
                    webView.evaluateJavascript(
                        "!!(window.__codifyNativeHandleBack && window.__codifyNativeHandleBack())",
                    ) { handled ->
                        if (!acted) {
                            acted = true
     ...
     * frames on the pinned origin, and [CodifyBridgeListener] also drops
    ...
    private fun installBridge() {
        ...
        try {
            WebViewCompat.addWebMessageListener(
                webView,
                CodifyBridgeListener.JS_OBJECT_NAME,
                setOf(origin),
                CodifyBridgeListener(
     ...
     * injects the session. Triggered by [CodifyWebViewClient] when the server
     ...
     * Mirrors the iOS `URL.codifyHostLabel` in `URL+Codify.swift`.
     */
    ...
    private fun emitNotificationActivation(path: String?) {
        if (path == null) return
        webView.evaluateJavascript(
            "window.__codifyNativeEmitNotificationActivated && " +
                "window.__codifyNativeEmitNotificationActivated(${jsString(path)});",
            null,
        )
    }
    ...
    private fun emitInsets() {
        ...
        // fold:
        //   1. `--codify-safe-top/bottom` — the app's OWN base inset vars. Every
        //      build already derives `--codify-inset-*` and its layout from
        ...
        //   2. `--codify-android-safe-area-*` — consumed by the shell's own
        //      `[data-android-native]` rules when the server IS up to date (folded
        //      via max() in index.css); a harmless no-op otherwise.
        // We deliberately do NOT call `__codifyNativeEmitInsets` — that feeds the
        // iOS *floating-bar* footprints (--codify-native-*-bar; nativeInsets.ts
        ...
        val js =
            ...
            (() => {
              ...
              s.setProperty('--codify-safe-top', top);
              s.setProperty('--codify-safe-bottom', bottom);
              s.setProperty('--codify-android-safe-area-top', top);
              s.setProperty('--codify-android-safe-area-bottom', bottom);
              s.setProperty('--codify-android-safe-area-left', '${bars.left / d}px');
              s.setProperty('--codify-android-safe-area-right', '${bars.right / d}px');
            })();
            """.trimIndent()
    ...
    /** Back [CodifyWebChromeClient.onShowFileChooser] with a document picker. */
    ...
    /** Back [CodifyWebChromeClient.onPermissionRequest] — grant mic to the pinned origin only. */
    ...
    private fun downloadFile(
        ...
        // (fixes codify-ai/codify#969, which the iOS shell leaves broken).
