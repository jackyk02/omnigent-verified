 ...
 * `window.codifyNative` with `kind: "android"`, mirroring the iOS shell's
 * bridge (`web/ios/Codify/CodifyWebView.swift`). The web layer consumes
 * this through `web/src/lib/nativeBridge.ts` — same object shape, same
 * `__codifyNativeEmit*` callback names — so no web change is needed beyond
 * accepting the `"android"` discriminator.
 *
 * web -> native goes through [CodifyBridgeListener.JS_OBJECT_NAME], the
 ...
 * `evaluateJavascript` into the `window.__codifyNativeEmit*` functions here.
 */
object NativeBridgeScript {
    val source: String =
        """
        (() => {
          if (window.codifyNative && window.codifyNative.kind === "android") return;
          ...
          // feeds --codify-safe-top/bottom (the app's own inset vars), but on a
          ...
          const ensureInsetStyles = () => {
            if (document.getElementById("codify-android-insets")) return;
            const T = "var(--codify-safe-top, 0px)";
            const B = "var(--codify-safe-bottom, 0px)";
            const style = document.createElement("style");
            style.id = "codify-android-insets";
            style.textContent = [
              ".chat-header{top:max(0px, calc(" + T + " - 0.5rem)) !important}",
              ".chat-conversation-content{padding-top:calc(var(--codify-header-height, 3.5rem) + 1.5rem + " + T + ") !important}",
          ...
          const post = (payload) => {
            try {
              const bridge = window.${CodifyBridgeListener.JS_OBJECT_NAME};
          ...
          Object.defineProperty(window, "__codifyNativeEmitNotificationActivated", {
          ...
          let lastInsets = null;
          Object.defineProperty(window, "__codifyNativeEmitInsets", {
            configurable: false, enumerable: false, writable: false,
          ...
          Object.defineProperty(window, "__codifyNativeHandleBack", {
            configurable: false, enumerable: false, writable: false,
            value() {
              try {
