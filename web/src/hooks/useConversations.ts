...
export interface PinnedConversationsResult {
   ...
   * an ordinary (unfiltered) session page whose rows carry no `codify.pinned`
...
/**
 * Fetch every pinned session (the `codify.pinned` label) via
 ...
 * `codify.pinned` label and reporting `filterHonored: false` whenever the
...
/**
 * Pin / unpin a session via `PATCH /v1/sessions/{id}` (the `codify.pinned`
 ...
 * persist a bare `codify.pinned` key that the upgraded server discards on
 * read (it only surfaces the caller's per-user `codify.pinned.<user>` key),
