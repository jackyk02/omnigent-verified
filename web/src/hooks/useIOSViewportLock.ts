 ...
 * to `--codify-viewport-height`; the app-shell sizes to that (see index.css).
...
export function useIOSViewportLock(): void {
  useEffect(() => {
    ...
    const apply = () => {
      frame = 0;
      root.style.setProperty("--codify-viewport-height", `${Math.round(viewport.height)}px`);
      resetPan();
    };
    ...
    return () => {
      ...
      root.style.removeProperty("--codify-viewport-height");
    };
  }, []);
}
