...
/** Install a `window.codifyDesktop` bridge; returns the mock so tests assert
...
function installBridge(overrides: Record<string, unknown> = {}) {
  ...
  (window as unknown as { codifyDesktop?: unknown }).codifyDesktop = bridge;
  return bridge;
}
...
afterEach(() => {
  vi.clearAllMocks();
  (window as unknown as { codifyDesktop?: unknown }).codifyDesktop = undefined;
});
...
describe("useBrowserAgentRelay — result POST resilience", () => {
  ...
  it("does nothing when the shell exposes no bridge at handler time", async () => {
    // isElectronShell() is mocked true (hook registers), but codifyDesktop is
