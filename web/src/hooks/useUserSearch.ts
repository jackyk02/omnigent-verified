 ...
 * `CodifyHostConfig.searchUsers` (see `lib/host.ts`). When no host searcher
...
import { getCodifyUserSearch, type UserSuggestion } from "@/lib/host";
...
export function useUserSearch(query: string): UseUserSearchResult {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["codifyUserSearch", trimmed],
