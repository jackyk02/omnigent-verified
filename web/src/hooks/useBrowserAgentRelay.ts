 ...
 *  dispatches to `window.codifyDesktop.browser*` and POSTs the result back
...
/** Subset of `window.codifyDesktop` the relay calls (typed locally, not via
...
function getBrowserDesktop(): BrowserDesktopBridge | null {
  ...
  const w = window as unknown as { codifyDesktop?: BrowserDesktopBridge };
  return w.codifyDesktop ?? null;
}
