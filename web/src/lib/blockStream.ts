// Mirrors sdks/python-client/codify_client/_stream.py.
//
...
interface ReducerState {
  ...
  // `_translate_codify_event`. Without this, the result panel renders
...
function* processEvent(state: ReducerState, event: StreamEvent): Generator<AnyBlock> {
  switch (event.type) {
    ...
    case "tool_call": {
      ...
      // (codify/runtime/harnesses/_executor_adapter.py) threads
      ...
      // line twice. See designs/RUN_CODIFY_REPL_PARITY.md.
      //
 ...
 *
 * Mirrors `codify_client._stream.BlockStream`. The reducer is a
