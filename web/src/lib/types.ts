...
// sdks/python-client/codify_client/_types.py.
//
...
// mirror `codify/server/schemas.py` (`SessionResponse`,
 ...
 * `codify/server/schemas.py:SessionStatusEvent`.
 */
 ...
 *
 * Mirrors `codify.server.schemas.SessionEventInput`.
 */
 ...
 *
 * `queuedItems` is documented in `codify/server/API.md` but is
 ...
 *
 * Mirrors `codify.server.schemas.SessionResponse`.
 */
/**
 ...
 * `codify.server.schemas.ModelUsage`.
 */
...
export interface Session {
  ...
  title: string | null;
  /**
   * Session-scoped guardrails labels (includes `codify.wrapper` /
   * `codify.ui` markers the picker reads, and
   * `codify.fork.source_id` on an unbound coding clone).
   */
 ...
 * `codify.server.schemas.SandboxStatus`. Drives the provisioning
 ...
 * ``codify.server.schemas.SkillSummary``. Just the name +
