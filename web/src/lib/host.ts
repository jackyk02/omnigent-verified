 ...
 * this module is a no-op until `setCodifyHostConfig` is called by the
...
export interface CodifyHostConfig {
  /**
   ...
   * `workspaceFetch` against `/ajax-api/2.0/codify`). When omitted, the
   ...
   * in the UI (e.g. `"/api/2.0/codify"`). When the host proxies the
   * Codify API behind a path prefix, CLI users need the full URL
   * (`https://host/api/2.0/codify`) — this suffix supplies the
   * non-origin part.
   */
...
let hostConfig: CodifyHostConfig = {};
let embedRoot: HTMLElement | null = null;

export function setCodifyHostConfig(config: CodifyHostConfig): void {
  // Guard: never clobber an already-installed fetcher with an empty config.
  // `CodifyApp` installs config during render and React may re-invoke it with
...
export function getCodifyHostConfig(): CodifyHostConfig {
  return hostConfig;
}
 ...
 */
export function getCodifyUserSearch(): CodifyHostConfig["searchUsers"] {
 ...
 */
export function getCodifyTransformShareLink(): CodifyHostConfig["transformShareLink"] {
 ...
 * lands inside the scoped `.codify-app` subtree and inherits its styles.
 ...
 * {@link CodifyHostConfig.cliServerUrlSuffix}.
 */
