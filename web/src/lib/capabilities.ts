...
export interface ServerInfo {
   ...
   * (``CODIFY_LOCAL_SINGLE_USER=1``). This is the sole signal that
  ...
  /**
   * Installed codify server version (same value as ``/api/version``),
   ...
   * (``CODIFY_HARNESS_INSTALL_ENABLED=1``). Gates the New Chat dialog's
...
export async function resolveServerInfo(): Promise<ServerInfo> {
  ...
  pendingServerInfo = (async () => {
    try {
      ...
      // proxied codify API; standalone `hostFetch` falls back to plain
 ...
 * server's ``single_user`` signal (``CODIFY_LOCAL_SINGLE_USER=1``). This
