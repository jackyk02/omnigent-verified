...
// `--codify-native-top-bar` / `--codify-native-bottom-bar`; index.css folds
// them (together with the web-owned visibility flags and `env(safe-area-*)`)
// into `--codify-inset-top` / `--codify-inset-bottom`, which `<PageScroll>`
...
export function initNativeInsets(): () => void {
  return onNativeInsets(({ topBar, bottomBar }) => {
    ...
    root.setProperty("--codify-native-top-bar", `${topBar}px`);
    root.setProperty("--codify-native-bottom-bar", `${bottomBar}px`);
  });
}
