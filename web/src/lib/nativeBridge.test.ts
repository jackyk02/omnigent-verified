...
// The Electron preload bridge mock, installed on window.codifyDesktop.
...
// The iOS WKWebView bridge mock, installed on window.codifyNative.
...
// The Android WebView bridge mock, installed on window.codifyNative. The MVP
...
function setIOS(on: boolean, withClickRouting = true): void {
  if (on) {
    (window as unknown as Record<string, unknown>).codifyNative = {
      kind: "ios",
...
describe("isNativeShell / isElectronShell", () => {
  ...
  it("ignore a bridge with the wrong discriminator", () => {
    (window as unknown as Record<string, unknown>).codifyDesktop = { kind: "nope" };
    (window as unknown as Record<string, unknown>).codifyNative = { kind: "nope" };
    expect(isElectronShell()).toBe(false);
    expect(isIOSShell()).toBe(false);
    expect(isAndroidShell()).toBe(false);
    expect(isNativeShell()).toBe(false);
...
describe("setThemeSource", () => {
  ...
  it("routes the selected theme through the Android bridge over Electron legacy", () => {
    setElectron(true);
    (
      window as unknown as { codifyNative?: { kind: string; setColorScheme?: unknown } }
    ).codifyNative = undefined;
    setThemeSource("system");
    expect(electronSetColorScheme).toHaveBeenCalledWith("system");
  });
});
...
describe("onNativeSidebarDrag", () => {
  ...
  it("returns a no-op unsubscribe under a shell lacking the gesture hook", () => {
    setIOS(true);
    delete (window as unknown as { codifyNative: Record<string, unknown> }).codifyNative
      .onSidebarDrag;
