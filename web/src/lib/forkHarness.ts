...
// a frontend mirror of the server (codify/server/routes/sessions.py):
//
//   - SDK (non-native) harnesses replay the Codify transcript as LLM
...
//     copied Codify items (a same-family native source clones the source
 ...
 * subset of Python `NATIVE_HARNESSES` (`codify/harness_aliases.py`) that the
 ...
 * Codify items (see `write_qwen_session_recording`).
 */
 ...
 * the copied Codify items, so they carry history on BOTH fork and switch.
 * Mirrors Python `_FORK_HISTORY_NATIVE_HARNESSES`
 * (`codify/server/routes/sessions.py`); both canonical and reversed
