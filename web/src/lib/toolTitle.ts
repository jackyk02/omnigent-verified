 ...
 * the codify sys_* tools plus the native harness CLIs (Claude Code's
...
function stripCodifyPrefix(name: string): string {
  return name.startsWith("mcp__codify__") ? name.slice("mcp__codify__".length) : name;
}
...
function categorizeCall(call: ToolRunCall): RunCategory | "other" {
  const name = stripCodifyPrefix(call.name);
