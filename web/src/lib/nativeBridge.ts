...
//   1. A normal browser tab (served by the Codify server).
...
//
// Design notes:
//   * Detection is feature-based (an injected `window.codifyNative` or the
//     legacy Electron `window.codifyDesktop` object), never a build flag —
 ...
 * `window.codifyDesktop`; newer shells expose `window.codifyNative`.
...
interface NativeShellApi {
   ...
   * clicks an `codify://.../c/<id>` link for a server this window is already
...
export interface HostIdentity {
  /** Whether the `codify` CLI was found and is runnable. */
...
function electronApi(): ElectronDesktopApi | undefined {
  ...
  const api = (window as unknown as { codifyDesktop?: ElectronDesktopApi }).codifyDesktop;
...
function nativeApi(): NativeShellApi | undefined {
  if (typeof window === "undefined") return undefined;
  const api = (window as unknown as { codifyNative?: NativeShellApi }).codifyNative;
 ...
 * Older installed builds expose `codifyDesktop` but predate the `browser*`
 ...
 * exposes `window.codifyDesktop` — never a build flag. In a plain browser
 ...
 * setters below fold it into `--codify-*-bar-visible`; the bars' size comes
 * from the native bridge (see {@link onNativeInsets} / nativeInsets.ts). Both
 * combine in `--codify-inset-*` (index.css). Harmless off-shell — the size
...
export function setNativeServerSwitcherHidden(hidden: boolean): void {
  setInsetVar("--codify-top-bar-visible", hidden ? "0" : "1");
  const native = nativeApi();
...
export function setNativeViewMode(params: NativeViewModeParams): void {
  setInsetVar("--codify-bottom-bar-visible", params.visible ? "1" : "0");
  const native = nativeApi();
