...
// documented in codify/server/routes/dictation.py; availability is gated
...
//   (_REMOTE_READY_TIMEOUT_S in codify/server/dictation.py).
...
registerProcessor("codify-pcm16-downsampler", Pcm16Downsampler);
`;
...
export class DictationSession {
  ...
  static async start(events: DictationSessionEvents): Promise<DictationSession> {
    ...
    try {
      ...
      const node = new AudioWorkletNode(audioContext, "codify-pcm16-downsampler");
