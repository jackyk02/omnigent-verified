...
export const WRAPPER_LABEL_KEY = "codify.wrapper";
export const UI_MODE_LABEL_KEY = "codify.ui";
...
export const NATIVE_CODING_AGENTS = [
  ...
  },
  {
    ...
    // `permission: "ask"` so tools route through the Codify policy engine, so
...
// Mirrors codify.harness_aliases.NATIVE_HARNESSES on the server, which
...
/**
 * Whether a session runs a native terminal harness — by its `codify.wrapper`
