...
// /v1/sessions"). Mirrors `codify/server/routes/sessions.py`.
//
...
/** Returns the client surface label for the X-Codify-Client telemetry header. */
...
/**
 * Wire shape of `ModelUsage` from `codify/server/schemas.py` — one
 ...
 * `codify/server/schemas.py`. Snake-case; converted to the
...
interface SessionResponseWire {
  ...
  items?: SessionItem[];
  // `queued_items` is documented in `codify/server/API.md` but
   ...
   * `codify.server.schemas.SandboxStatus`.
   */
 ...
 * status line.
 *
 * The server's :class:`CodifyError` serializes as
...
export async function createSession(
  ...
  const res = await authenticatedFetch("/v1/sessions", {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Codify-Client": getClientSurface() },
...
export async function createBundledSession(
  ...
  const res = await authenticatedFetch("/v1/sessions", {
    method: "POST",
    headers: { "X-Codify-Client": getClientSurface() },
    body: form,
  });
  if (!res.ok) {
 ...
 * (`codify/server/API.md` §Reconnect Contract), callers should
