...
import { getCodifyTransformShareLink, getCodifyUserSearch } from "@/lib/host";
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Sharing has been disabled for this Codify server.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function AddUserField({ value, onChange }: AddUserFieldProps) {
  ...
  const searchUsers = getCodifyUserSearch();
  if (!searchUsers) {
    return (
      <Input
        id="perm-user"
...
function getShareableLink(sessionId: string, rebasePath: (path: string) => string): string {
  ...
  const transform = getCodifyTransformShareLink();
  return transform ? transform(path) : `${window.location.origin}${path}`;
}

/**
 * The `codify://<host>/c/<session_id>` deep link encoded into the share QR
...
function getDeepLink(sessionId: string, rebasePath: (path: string) => string): string {
  ...
  try {
    ...
    return `codify://${host}/c/${sessionId}`;
  } catch {
    // Unparseable transform output: fall back to the current origin's host.
    return `codify://${window.location.host}/c/${sessionId}`;
  }
}
 ...
 * `codify://<host>/c/<id>` deep link so a user can scan it with their phone
 * to open the session in the Codify app. The code is rendered on a fixed
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Scan with your phone's camera to open this session in the Codify app.
          </DialogDescription>
        </DialogHeader>
        <div className="flex justify-center">
          <div className="rounded-lg bg-white p-3">
            <QRCodeSVG
              ...
              bgColor="#ffffff"
              fgColor="#000000"
              aria-label="QR code to open this session in the Codify app"
            />
          </div>
        </div>
        <DialogFooter>
