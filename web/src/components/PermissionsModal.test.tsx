...
vi.mock("@/lib/host", async (importOriginal) => {
  ...
  return {
    ...actual,
    getCodifyUserSearch: vi.fn(() => undefined),
    getCodifyTransformShareLink: vi.fn(() => undefined),
  };
});
...
const userSearchMock = vi.mocked(host.getCodifyUserSearch);
const transformLinkMock = vi.mocked(host.getCodifyTransformShareLink);
...
describe("PermissionsModal", () => {
  ...
  describe("sharing mode", () => {
    it("off: shows the disabled notice and never fetches grants", async () => {
      ...
      expect(
        await screen.findByText("Sharing has been disabled for this Codify server."),
      ).toBeInTheDocument();
    ...
    it("on: renders the full controls with no disabled/read-only notice", async () => {
      ...
      expect(
        screen.queryByText("Sharing has been disabled for this Codify server."),
  ...
  describe("share QR code", () => {
    // The QR encodes an `codify://<host>/c/<id>` deep link (the same scheme
    ...
    it("encodes the codify:// deep link for the session from the server origin", async () => {
      listMock.mockResolvedValue([]);
      ...
      await withLocation("https://app.example.com", async () => {
        ...
        expect(qr).toHaveAttribute("data-value", "codify://app.example.com/c/conv_abc");
      });
    });

    it("includes the port in the host when non-default", async () => {
      // Mirrors the deep-link convention: `codify://localhost:8000/c/<id>`.
      listMock.mockResolvedValue([]);
      ...
      await withLocation("http://localhost:8000", async () => {
        ...
        fireEvent.click(screen.getByRole("button", { name: /open in mobile app/i }));
        const qr = await screen.findByTestId("share-qr-code");
        expect(qr).toHaveAttribute("data-value", "codify://localhost:8000/c/conv_abc");
      });
    });
    ...
    it("derives the host from the host transformShareLink in the embed", async () => {
      ...
      fireEvent.click(screen.getByRole("button", { name: /open in mobile app/i }));
      const qr = await screen.findByTestId("share-qr-code");
      expect(qr).toHaveAttribute("data-value", "codify://host.example.com/c/conv_xyz");
    });
  });
});
