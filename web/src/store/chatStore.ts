...
export interface ChatState {
   ...
   * (claude-native / codex-native), derived from the `codify.wrapper`
   ...
   * vendor TUI (qwen/goose/cursor/pi/opencode) rather than through an Codify
   * model picker. The composer status line hides its model/effort label for
   * these — Codify's bound `llmModel` is just an unused default (it would
   * otherwise read e.g. "claude-sonnet-4-6" on a Qwen session). claude-/codex-
   * native DO expose an Codify picker, so they keep the label. `false` on
   ...
   * ``codify.codex_native.collaboration_mode`` on bind and updated by the
...
const PICKER_PREF_EFFORT_KEY = "codify.picker.effort";
const PICKER_PREF_MODEL_KEY = "codify.picker.model";
...
function nativeModelFamilyForSession(session: Pick<Session, "labels">): NativeModelFamily | null {
  switch (session.labels?.["codify.wrapper"]) {
...
function sessionBindingPatch(
  ...
  | "mcpStartup"
> {
  const wrapper = session.labels?.["codify.wrapper"];
  return {
    isNativeTerminalSession: isNativeWrapper(wrapper),
    // Native wrapper whose model lives in the vendor TUI (no Codify picker):
...
export function handleSessionEvent(event: StreamEvent): void {
  switch (event.type) {
    ...
    case "slash_command":
      ...
      // → `response.output_item.done`. The Codify server bypasses
...
// Mirrors the server's ErrorCode.RUNNER_UNAVAILABLE (codify/errors.py) —
