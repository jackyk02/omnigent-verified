...
const BestOfNRunPage = lazy(() =>
  import("@/pages/BestOfNRunPage").then((m) => ({ default: m.BestOfNRunPage })),
);

interface AppProps {
  /**
   * Mount prefix when embedded (e.g. `/ml/codify-embed`). The route table
    ...
    <Suspense fallback={null}>
      <Routes>
        ...
        <Route element={<AppShell />}>
          ...
          {/* Best-of-N runs: one prompt fanned out to three harnesses, three
              live panes, verifier-picked winner squash-merged. */}
          <Route path={`${prefix}/best-of-n`} element={<BestOfNRunPage />} />
