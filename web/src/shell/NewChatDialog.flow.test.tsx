...
const PROMPT_HISTORY_KEY = "codify:prompt-history:conv_new";
...
describe("NewChatLandingScreen create flow", () => {
  ...
  it("does not create a session when Enter carries the IME keyCode 229 fallback", async () => {
    ...
    await waitForWorkspaceSeed();
    const input = screen.getByTestId("new-chat-landing-input");
    fireEvent.change(input, { target: { value: "codify" } });
  ...
  it("attaches terminal-wrapper labels when the antigravity-native agent is chosen", async () => {
    ...
    expect(body.labels).toEqual({
      "codify.ui": "terminal",
      "codify.wrapper": "antigravity-native-ui",
    });
  });
  ...
  it("persists the picked permission mode for claude-native so the next session seeds it", async () => {
    ...
    await waitFor(() =>
      expect(
        JSON.parse(localStorage.getItem("codify:last-mode-by-harness") ?? "{}")["claude-native"]
          ?.mode,
      ).toBe("acceptEdits"),
    );
  });
  ...
  it("posts no launch args for opencode-native, even after a codex full-access pick", async () => {
    ...
    const [, init] = vi.mocked(authenticatedFetch).mock.calls[0] as [string, RequestInit];
    const body = JSON.parse(init.body as string);
    expect(body.labels?.["codify.wrapper"]).toBe("opencode-native-ui");
  ...
  it("omits terminal_launch_args when permission mode is left at default for claude-native", async () => {
    ...
    expect(body.labels?.["codify.wrapper"]).toBe("claude-code-native-ui");
  ...
  it("persists a picked model for claude-native, preserving the stored effort", async () => {
    ...
    await waitFor(() => {
      const stored = JSON.parse(localStorage.getItem("codify:last-mode-by-harness") ?? "{}")[
        "claude-native"
      ];
  ...
  it("omits terminal_launch_args when approval mode is left at default for codex-native", async () => {
    ...
    const [, init] = vi.mocked(authenticatedFetch).mock.calls[0] as [string, RequestInit];
    const body = JSON.parse(init.body as string);
    expect(body.labels?.["codify.wrapper"]).toBe("codex-native-ui");
    expect(body.terminal_launch_args).toBeUndefined();
  });

  it("diverts a best_of_n submit to the run screen instead of creating a session", async () => {
    // Best-of-N is not a chat session: submitting hands the composer prompt
    // plus the seeded working directory to /best-of-n via router state, and
    // the run page auto-starts the run there.
    setAgents([
      agent({
        id: "ag_best_of_n",
        name: "best_of_n",
    ...
    renderLanding();
    await waitForWorkspaceSeed();
    typeMessage("fix the flaky retry test");
    fireEvent.click(screen.getByTestId("new-chat-landing-submit"));

    await waitFor(() =>
      expect(navigateMock).toHaveBeenCalledWith("/best-of-n", {
        state: {
          autoStart: { prompt: "fix the flaky retry test", repoPath: SEEDED_WORKSPACE },
        },
      }),
    );
    // The whole point of the divert: no POST /v1/sessions on this path.
    expect(authenticatedFetch).not.toHaveBeenCalled();
  });

  it("shows the spec default un-suffixed on the best_of_n picker pill", async () => {
    setAgents([
      agent({
        id: "ag_best_of_n",
        name: "best_of_n",
    ...
    // default is not suffixed (it lives in the config modal's harness select).
    expect(screen.getByTestId("new-chat-landing-agent-select").textContent).toContain("Best-of-N");
    expect(screen.getByTestId("new-chat-landing-agent-select").textContent).not.toContain(
      "Claude SDK",
    );
  });

  it("still diverts (no session POST) after a brain-harness pick in the config modal", async () => {
    // The override modal remains usable, but submitting with best_of_n never
    // creates a session — the run screen owns the experience.
    setAgents([
      agent({
        id: "ag_best_of_n",
        name: "best_of_n",
    ...
    await waitFor(() =>
      expect(navigateMock).toHaveBeenCalledWith("/best-of-n", {
        state: { autoStart: { prompt: "go", repoPath: SEEDED_WORKSPACE } },
      }),
    );
    expect(authenticatedFetch).not.toHaveBeenCalled();
  });
  ...
  it("remembers the picked agent and preselects it on the next visit", async () => {
    ...
    expect(localStorage.getItem("codify:last-agent-id")).toBe("ag_two");
  ...
  it("falls back to the default agent when the remembered id is no longer listed", async () => {
    ...
    localStorage.setItem("codify:last-agent-id", "ag_gone");
    vi.mocked(authenticatedFetch).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ id: "conv_new" }),
    } as unknown as Response);
