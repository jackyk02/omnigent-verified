...
// (max-md:hidden), and "Back to Codify" does NOT close the sidebar overlay
...
describe("SettingsSidebarBody", () => {
  ...
  it("does NOT close the sidebar when 'Back to Codify' is tapped", () => {
    ...
    fireEvent.click(screen.getByRole("link", { name: /Back to Codify/ }));
    expect(onNavClick).not.toHaveBeenCalled();
  });

  it("'Back to Codify' returns to the conversation the user came from", () => {
    ...
    expect(screen.getByRole("link", { name: /Back to Codify/ })).toHaveAttribute(
      "href",
...
describe("useSettingsRoute", () => {
  ...
  it("matches the settings segment under an embed basename", () => {
    ...
    mocks.accountsEnabled = true;
    expect(routeHook("/ml/codify-embed/settings/members")).toEqual({
      inSettings: true,
      section: "members",
    });
  });
