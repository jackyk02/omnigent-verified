...
describe("Sidebar session list", () => {
  ...
  it("swaps the card content to the settings section nav on /settings", () => {
    ...
    expect(screen.getByRole("link", { name: /Back to Codify/ })).toHaveAttribute("href", "/");
  ...
  it("renders Inbox as its own primary navigation row", () => {
    ...
    // Best-of-N has no nav entry — it's the new-session experience for the
    // best_of_n agent, not a separate destination.
    expect(within(primaryNav).queryByTestId("best-of-n-nav")).toBeNull();
  });
...
describe("Sidebar default section collapse", () => {
  ...
  it("honors a persisted collapse of the Sessions list across remount", () => {
    ...
    localStorage.setItem("codify:collapsed-sidebar-sections", JSON.stringify(["Chats"]));
...
describe("Sidebar auto-expand Pinned on pin", () => {
  it("expands a collapsed Pinned section when a session is newly pinned", () => {
    localStorage.setItem("codify:collapsed-sidebar-sections", JSON.stringify(["Pinned"]));
    ...
    expect(screen.getByRole("button", { name: /Pinned/ })).toHaveAttribute("aria-expanded", "true");
    expect(JSON.parse(localStorage.getItem("codify:collapsed-sidebar-sections")!)).not.toContain(
      "Pinned",
    );
  });
});
