...
describe("sidebar highlight while viewing a sub-agent", () => {
  it("renders the official Codify wordmark instead of styled text", () => {
    mockConversations([]);
    renderAt("/");

    const wordmark = screen.getByTestId("sidebar-wordmark");
    expect(wordmark).toHaveAttribute("alt", "Codify");
    expect(wordmark).toHaveClass("h-[15px]", "dark:invert");
    // Small SVGs are inlined as data: URIs by the bundler, so accept either
    // the hashed asset filename or an inlined SVG payload.
    const src = wordmark.getAttribute("src") ?? "";
    expect(src.includes("codify-wordmark") || src.startsWith("data:image/svg")).toBe(true);
  });
