...
import codifyWordmark from "@/assets/codify-wordmark.svg";
...
function useActiveNavItem(): {
  ...
  // Exclude inbox/tasks/best-of-n: they also have no `:conversationId`, so
  // they would otherwise light up the "New session" button. (/best-of-n has
  // no sidebar entry of its own — it's reached from the new-session flow —
  // but it still must not read as the "New session" page.)
  const isNewChatPage =
    activeConversationId == null && !isInboxPage && !isTasksPage && leaf !== "best-of-n";
 ...
 * `PINNED_CONVERSATION_IDS_STORAGE_KEY`. Now they're an `codify.pinned`
...
export function Sidebar({ open, onClose, dragProgress = null, onOpenSearch }: SidebarProps) {
  ...
  // Remember the pre-settings location so "Back to Codify" returns to the
  ...
  // Pins are stored on the server as an `codify.pinned` session label, so
  ...
  // still-local pins up to the server (as the `codify.pinned` label) the
...
function ConversationList({
  ...
  const sections = useMemo(() => {
    ...
    // by when they were pinned (the `codify.pinned` label's epoch-ms value;
  ...
  // Pinned membership is server-authoritative (the `codify.pinned` label),
...
function ConversationMenuItems({
  ...
  isArchived: boolean;
  isOwner: boolean;
  // Server-wide sharing kill switch (CODIFY_SHARING_MODE=off): disables the
        ...
        (isOwner && !sharingOff ? (
          ...
          <Tooltip>
            ...
            <TooltipContent side="left">
              {sharingOff
                ? "Sharing has been disabled for this Codify server."
...
function ConversationRow({
  ...
  // Server-wide sharing kill switch (CODIFY_SHARING_MODE=off) reported by
