...
import { getCliServerUrl, getCodifyHostConfig } from "@/lib/host";
...
import { CodifyLogo } from "@/components/icons/CodifyLogo";
...
// Hidden from the new-session picker only. `nessie` is superseded.
...
const AGENT_PICKER_DESCRIPTIONS: Record<string, string> = {
  best_of_n: "Multi-agent auto-verify",
};
...
const SKILL_PILL_AGENTS = new Set(["best_of_n"]);
...
// command sandbox) — see codify.stores.conversation_store
...
const CODEX_NATIVE_BYPASS_SANDBOX_LABEL_KEY = "codify.codex_native.bypass_sandbox";
 ...
 *
 * Recognizes the CodifyError shape (``{error: {message}}``) and
 ...
 * login" / "set CODIFY_CODEX_PATH"), so it's gated on {@link isCodexHarness}.
 ...
 *
 * Gated on the setup feature: when OFF, renders the original "run codify
 ...
 * agents like Best-of-N, plus custom user agents) and "Harnesses" (the
...
export function AgentHarnessPicker({
  ...
  const renderBadge = (agent: AvailableAgent) => {
    if (harnessUnconfiguredOnHost(agent.harness, host)) {
      return (
        <Badge
          variant="outline"
    ...
    // Best-of-N is the default mode — flag its verify behavior at a glance.
    if (agent.name === "best_of_n") {
      return (
        <Badge
          variant="outline"
          className="ml-auto self-center text-[11px] text-muted-foreground"
          data-testid={`new-chat-landing-agent-verify-${agent.id}`}
        >
          auto-verify
        </Badge>
      );
  ...
  // Split the agents group: built-in bundle agents (Best-of-N) stay inline
    ...
    >
      ...
      >
        {showMore ? (
          ...
          <>
            {/* Agents group leads — Best-of-N is the default mode, so the
            recommended pick sits at the top; built-in bundle agents inline. */}
            ...
            {/* Harnesses group below — the native terminal CLIs. Ready-to-use
            harnesses list inline; "needs setup" ones fold into a "More" group. */}
...
export function NewChatLandingScreen() {
  ...
  // "Agents" (SDK / bundle agents like Best-of-N plus any custom
  // user-registered agents). This is the isNativeCodingAgent split, NOT the
  // builtins/customs split: Best-of-N is a built-in but belongs under
  ...
  const docsLinks = getCodifyHostConfig().docsLinks;
  ...
  // Per-session brain-harness override for bundle agents (best_of_n).
  ...
  async function handleCreate() {
    ...
    if (!canSubmit) return;
    // Best-of-N is not a chat session: submitting hands the prompt + the
    // picked working directory straight to the run screen, which POSTs
    // /v1/best-of-n/runs and shows the three live panes. Carried via router
    // state (prompts can be long — no query params). No POST /v1/sessions
    // happens on this path.
    if (selectedAgent?.name === "best_of_n") {
      const prompt =
      ...
      landingDraft = null;
      navigate("/best-of-n", {
        state: { autoStart: { prompt, repoPath: workspaceTrimmed } },
      });
      return;
    }
    setCreating(true);
      ...
      if (effectiveAgentId === PENDING_AGENT_ID && pendingAgent) {
        ...
        const res = await authenticatedFetch("/v1/sessions", {
          ...
          body: JSON.stringify({
            ...
            // Native terminal agents open terminal-first: `codify.ui:
            // terminal` tells the UI to render the terminal wrapper, and
            // `codify.wrapper` selects which CLI bridge the runner launches.
      ...
      <div className="flex w-full max-w-[840px] flex-col items-center gap-8 px-4 pt-8 pb-16 md:select-none md:px-10">
        <div className="flex w-full flex-col items-center justify-center gap-3.5 sm:flex-row">
          {selectedProject ? (
            ...
            <CodifyLogo className="h-18 w-auto shrink-0" />
          )}
