...
export function AppShell() {
  ...
  // snapshot carries ``codify.ui``/``codify.wrapper`` — without
  ...
  const terminalFirst = sessionLabels["codify.ui"] === "terminal";
  const isClaudeNative = sessionLabels["codify.wrapper"] === "claude-code-native-ui";
  ...
  // (embedded Codify REPL terminal) have NO wrapper label and must
  // keep regular chat behavior. See TerminalFirstContext.tsx.
  const isNativeWrapper = isNativeWrapperLabel(sessionLabels["codify.wrapper"]);
  ...
  // admin set CODIFY_SHARING_MODE=off reports sharing_mode "off" via
  ...
  const isClaudeNativeSubagent =
    activeSession?.labels?.["codify.wrapper"] === "claude-code-native-ui-subagent" ||
    activeConv?.labels?.["codify.wrapper"] === "claude-code-native-ui-subagent";
  ...
  const setPanelInitialKey = useCallback(
    (key: string | null) => {
      ...
      const storageKey = `codify.web.panel-key:${conversationId}`;
      if (key === null) {
  ...
  useEffect(() => {
    ...
    const stored = sessionStorage.getItem(`codify.web.panel-key:${conversationId}`);
