...
//         no row carries the `codify.pinned` label (pre-feature server).
//   new → honors `?pinned=true`, returns only the caller's pinned rows with the
//         label collapsed to the bare `codify.pinned` key, and PATCH stores /
//         clears the per-user pin.
const server = {
  mode: "old" as "old" | "new",
  // Per-user pins the NEW server stores + surfaces (codify.pinned.<user>).
  perUserPins: new Set<string>(),
  // Bare-key pins an OLD server stores from a PATCH. Faithful to the bug: the
  // new server drops any bare `codify.pinned` key on read (only its per-user
...
const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (server.mode === "old") {
    ...
    // WITHOUT any pin label (a bare-key pin isn't surfaced as codify.pinned
