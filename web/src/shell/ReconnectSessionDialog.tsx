 ...
 *   down. The owner reconnects the host (`codify host`); a
 ...
 *
 * 1. `host_offline` — `codify host` re-registers the host
 ...
 *    (`wrapper === "claude-code-native-ui"`) use `codify claude
 *    --resume <id>`; everything else uses the generic `codify run
...
export function buildReconnectCommand({
  ...
  if (state === "host_offline") {
    return ["codify host \\", `  --server ${serverUrl}`].join("\n");
  }
  if (wrapper === CLAUDE_NATIVE_WRAPPER) {
    return ["codify claude \\", `  --resume ${conversationId} \\`, `  --server ${serverUrl}`].join(
      "\n",
    );
  }
  return [
 ...
 *
 * @param wrapper - The conversation's `codify.wrapper` label
 *   (`"claude-code-native-ui"` for `codify claude` sessions). Picks
