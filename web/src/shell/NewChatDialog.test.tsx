...
import { setCodifyHostConfig } from "@/lib/host";
...
const RECENT_KEY = "codify:recent-workspaces";
...
describe("NewChatLandingScreen", () => {
  ...
  it("orders native built-ins together in the agent picker", () => {
    ...
    const bestOfN = screen.getByTestId("new-chat-landing-agent-a_best_of_n");
    ...
    // The Agents group (Best-of-N) renders above the Harnesses group, so the
    // bundle agent PRECEDES every native harness row in document order.
    expect(bestOfN.compareDocumentPosition(cursor) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  });

  it("renders the Agents section above the Harnesses section", () => {
    mockAgents([
      {
        id: "a1",
        name: "claude-native-ui",
    ...
    const harnessesHeader = screen.getByText("Harnesses");
    expect(
      agentsHeader.compareDocumentPosition(harnessesHeader) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  // Best-of-N is a bundle agent whose brain harness (claude-sdk) is overridable,
  // so its config modal lists every brain harness — each badged when unconfigured.
  function mockBestOfNWithBrainReadiness() {
    mockHosts([
      {
        ...host("online"),
  ...
  it("lists every brain harness in a bundle agent's override select by default", () => {
    ...
    mockBestOfNWithBrainReadiness();
    renderLanding();
    openAgentConfig("a_best_of_n");
  ...
  it("seeds the bypass-sandbox label in the create body when armed", async () => {
    ...
    expect(labels["codify.codex_native.bypass_sandbox"]).toBe("1");
    // The native wrapper labels still ride alongside it.
    expect(labels["codify.wrapper"]).toBe("codex-native-ui");
  });
  ...
  it.each([
    {
      name: "not-configured CodifyError",
      status: 400,
...
// agents (best_of_n): pills surface bundled skills without typing "/",
// and clicking one prefills the composer — it never sends.
describe("NewChatLandingScreen skill pills", () => {
  ...
  /** Best-of-N — allowlisted for pills, carrying two bundled skills. */
  function bestOfNAgent(): AvailableAgent {
    return {
      id: "ag_best_of_n",
      name: "best_of_n",
      display_name: "Best-of-N",
      description: "Multi-agent auto-verify",
      harness: "claude-sdk",
      skills: [
  ...
  it("appears when the user switches the picker to an allowlisted agent", () => {
    // Claude Code ranks first (native agents lead via nativeAgentSortRank),
    // so best_of_n is NOT the default selection — no pills until the user
    // picks it. This is the core interaction: click best_of_n in the picker,
    // its skills appear.
    mockAgents([
      {
        id: "a1",
    ...
    expect(screen.queryByTestId("skill-pills")).toBeNull();
    fireEvent.pointerDown(screen.getByTestId("new-chat-landing-agent-select"), { button: 0 });
    fireEvent.click(screen.getByTestId("new-chat-landing-agent-ag_best_of_n"));
...
describe("NewChatLandingScreen @-file-mention", () => {
  ...
  // Path-aware listing: the workspace root holds an "codify" folder + a
  // README; drilling into "codify" reveals a nested folder + a file. Keyed by
  ...
  function mockFsByPath() {
    useHostFilesystemMock.mockImplementation(((_hostId: string | null, path: string | null) => {
      ...
      if (path === ROOT) entries = [dir(`${ROOT}/codify`), file(`${ROOT}/README.md`)];
      else if (path === `${ROOT}/codify`)
        entries = [dir(`${ROOT}/codify/inner`), file(`${ROOT}/codify/cli.py`)];
      return {
  ...
  it("opens the menu listing workspace files when '@' is typed (native agent)", async () => {
    ...
    expect(screen.getByTitle("Open codify")).toBeInTheDocument();
  ...
  it("does NOT open the menu for a non-native (SDK) agent", () => {
    ...
    renderLanding();
    fireEvent.change(input(), { target: { value: "@", selectionStart: 1 } });
    expect(screen.queryByTitle("Open codify")).not.toBeInTheDocument();
  });
  ...
  it("drills into a folder and delivers the chosen file as a workspace-relative marker", async () => {
    ...
    expect((payload as { text: string }).text).toBe("[Attached: codify/cli.py]\n\nexplain this");
  });
  ...
  it("suppresses stale parent rows while a drilled directory is still loading", async () => {
    ...
    // listing arrives.
    const rootEntries = [dir(`${ROOT}/codify`), file(`${ROOT}/README.md`)];
    useHostFilesystemMock.mockImplementation(((_hostId: string | null, path: string | null) => {
  ...
  it("attaches a whole folder with a trailing-slash marker", async () => {
    ...
    fireEvent.click(screen.getByLabelText("Attach whole folder codify"));
    expect(screen.getByText("@codify/")).toBeInTheDocument();
    ...
    await waitFor(() => expect(setPendingInitialPromptMock).toHaveBeenCalled());
    const [, payload] = setPendingInitialPromptMock.mock.calls[0]!;
    expect((payload as { text: string }).text).toBe("[Attached: codify/]\n\nreview it");
  });
