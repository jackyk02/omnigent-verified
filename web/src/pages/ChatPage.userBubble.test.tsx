...
describe("UserBubble copy button", () => {
  ...
  it("shows a toast confirmation on a mobile viewport", async () => {
    ...
    const onToast = vi.fn();
    window.addEventListener("codify:toast", onToast);
    ...
    try {
      ...
      window.removeEventListener("codify:toast", onToast);
...
describe("UserBubble @-mention attachment chips", () => {
  ...
  it("does not chip an absolute upload marker (already shown via its file block)", () => {
    renderBubble(
      userBubble(
        "[Attached: /var/folders/x/codify-1/claude-native/abc/uploads/image.png]\n\nwhat is this",
      ),
    );
  ...
  it.each([
    ["C:\\Users\\me\\AppData\\Local\\Temp\\codify\\uploads\\image.png", "drive (backslash)"],
    ["C:/Users/me/AppData/Local/Temp/codify/uploads/image.png", "drive (forward slash)"],
    ["\\\\host\\share\\codify\\uploads\\image.png", "UNC"],
  ...
  it("chips a relative @-mention but not an absolute upload in the same message", () => {
    renderBubble(
      userBubble(
        "[Attached: /tmp/codify/claude-native/abc/uploads/image.png]\n" +
