...
beforeEach(() => {
  ...
  delete (window as unknown as Record<string, unknown>).codifyDesktop;
});
afterEach(() => {
  cleanup();
...
function installUpdateBridge(config: UpdateConfig = DEFAULT_UPDATE_CONFIG) {
  ...
  (window as unknown as Record<string, unknown>).codifyDesktop = {
    kind: "electron",
    setBadgeCount: vi.fn(),
    notify: vi.fn(),
    updates: bridge,
...
describe("SettingsPage", () => {
  ...
  it("renders the Terminal theme radiogroup with auto selected by default", () => {
    ...
    expect(localStorage.getItem("codify:terminal-theme")).toBeNull();
  });
  ...
  it("persists dark and light terminal theme choices on card click", () => {
    ...
    expect(localStorage.getItem("codify:terminal-theme")).toBe("dark");
    ...
    expect(localStorage.getItem("codify:terminal-theme")).toBe("light");
  ...
  it("reflects a stored light terminal theme on mount", () => {
    localStorage.setItem("codify:terminal-theme", "light");
    renderPage("/settings/appearance");
    expect(screen.getByTestId("terminal-theme-light")).toHaveAttribute("aria-checked", "true");
    expect(screen.getByTestId("terminal-theme-auto")).toHaveAttribute("aria-checked", "false");
  });

  it("renders the color theme dropdown, defaults to Codify, and applies a palette on change", () => {
    localStorage.clear();
    renderPage("/settings/appearance");

    const select = screen.getByTestId("color-theme-select") as HTMLSelectElement;
    // Nothing stored → the default (Codify) palette is selected and no
    ...
    expect(localStorage.getItem("codify:ui-theme-palette")).toBe(JSON.stringify("github"));
  });
  ...
  it("creates and applies a custom theme when a guided color control changes", () => {
    ...
    expect(localStorage.getItem("codify:ui-theme-palette")).toBe(JSON.stringify("custom"));
    expect(JSON.parse(localStorage.getItem("codify:custom-theme") ?? "null")).toMatchObject({
      basePalette: "github",
      accent: "#2563eb",
    });
  ...
  it("shows the default UI font size and steps it up, persisting the choice", () => {
    ...
    expect(localStorage.getItem("codify:ui-font-size")).toBe("17");
  ...
  it("disables the steppers at the min and max bounds", () => {
    localStorage.setItem("codify:ui-font-size", "20");
    renderPage("/settings/appearance");
    ...
    cleanup();
    localStorage.setItem("codify:ui-font-size", "12");
    renderPage("/settings/appearance");
  ...
  it("shows the empty font family default and applies + persists a typed name", () => {
    ...
    // The choice is persisted so it survives a refresh...
    expect(localStorage.getItem("codify:ui-font-family")).toBe(JSON.stringify("Inter"));
  ...
  it("reset restores the system default font family", () => {
    localStorage.setItem("codify:ui-font-family", JSON.stringify("Georgia"));
    renderPage("/settings/appearance");
    const input = screen.getByTestId("ui-font-family-input") as HTMLInputElement;
    ...
    expect(input.value).toBe("");
    expect(document.documentElement.style.getPropertyValue("--ui-font-family")).toBe("");
    expect(localStorage.getItem("codify:ui-font-family")).toBeNull();
  });
  ...
  it("resets every appearance preference back to product defaults", () => {
    ...
    expect(localStorage.getItem("codify:terminal-theme")).toBe("dark");
    expect(localStorage.getItem("codify:ui-theme-palette")).toBe(JSON.stringify("github"));
    expect(localStorage.getItem("codify:ui-font-size")).toBe("18");
    expect(localStorage.getItem("codify:code-font-size")).toBe("15");
    ...
    expect(document.documentElement.style.getPropertyValue("--ui-font-family")).toBe("");
    expect(localStorage.getItem("codify:ui-font-size")).toBeNull();
    expect(localStorage.getItem("codify:code-font-size")).toBeNull();

    // Color theme is back to Codify.
  ...
  it("lets you clear and retype the font size without clamping mid-edit", () => {
    localStorage.setItem("codify:ui-font-size", "13");
    renderPage("/settings/appearance");
    const input = screen.getByTestId("ui-font-size-input") as HTMLInputElement;
    ...
    expect(localStorage.getItem("codify:ui-font-size")).toBe("13");
  ...
  it("clamps a below-min entry to the minimum on blur", () => {
    localStorage.setItem("codify:ui-font-size", "16");
    renderPage("/settings/appearance");
    const input = screen.getByTestId("ui-font-size-input") as HTMLInputElement;
    ...
    expect(localStorage.getItem("codify:ui-font-size")).toBe("12");
  });

  it("reverts an empty entry to the committed size on blur", () => {
    localStorage.setItem("codify:ui-font-size", "15");
    renderPage("/settings/appearance");
    const input = screen.getByTestId("ui-font-size-input") as HTMLInputElement;
    ...
    expect(localStorage.getItem("codify:ui-font-size")).toBe("15");
  });
  ...
  it("shows the default code font size and steps it up, persisting the choice", () => {
    ...
    expect(localStorage.getItem("codify:code-font-size")).toBe("14");
  });

  it("disables the code font steppers at the min and max bounds", () => {
    localStorage.setItem("codify:code-font-size", "24");
    renderPage("/settings/appearance");
    ...
    cleanup();
    localStorage.setItem("codify:code-font-size", "10");
    renderPage("/settings/appearance");
  ...
  it("lets you clear and retype the code font size, clamping below-min on blur", () => {
    localStorage.setItem("codify:code-font-size", "13");
    renderPage("/settings/appearance");
    const input = screen.getByTestId("code-font-size-input") as HTMLInputElement;
    expect(input.value).toBe("13");
    ...
    fireEvent.change(input, { target: { value: "1" } });
    expect(input.value).toBe("1");
    expect(localStorage.getItem("codify:code-font-size")).toBe("13");

    // Finishing to a valid size applies + persists it.
    fireEvent.change(input, { target: { value: "20" } });
    expect(input.value).toBe("20");
    expect(localStorage.getItem("codify:code-font-size")).toBe("20");

    // A still-out-of-range draft clamps to the minimum on blur.
    fireEvent.change(input, { target: { value: "2" } });
    fireEvent.blur(input);
    expect(input.value).toBe("10");
    expect(localStorage.getItem("codify:code-font-size")).toBe("10");
  });
  ...
  it("shows the empty code font family default and applies + persists a typed name", () => {
    ...
    expect(localStorage.getItem("codify:code-font-family")).toBe(JSON.stringify("Fira Code"));
    expect(screen.getByTestId("code-font-family-reset")).not.toBeDisabled();
  });

  it("reset restores the default code font family", () => {
    localStorage.setItem("codify:code-font-family", JSON.stringify("JetBrains Mono"));
    renderPage("/settings/appearance");
    const input = screen.getByTestId("code-font-family-input") as HTMLInputElement;
    // The control reflects the stored preference on mount.
    ...
    expect(input.value).toBe("");
    expect(localStorage.getItem("codify:code-font-family")).toBeNull();
  });
  ...
  it("shows an empty default base branch by default and persists a typed value", () => {
    ...
    expect(localStorage.getItem("codify:default-base-branch")).toBe("main");
  });

  it("reflects a stored default base branch on mount", () => {
    localStorage.setItem("codify:default-base-branch", "develop");
    renderPage("/settings/git");
    const input = screen.getByTestId("settings-default-base-branch-input") as HTMLInputElement;
  ...
  it("clears the default base branch preference when emptied", () => {
    localStorage.setItem("codify:default-base-branch", "main");
    renderPage("/settings/git");
    const input = screen.getByTestId("settings-default-base-branch-input") as HTMLInputElement;
    expect(input.value).toBe("main");
    ...
    fireEvent.change(input, { target: { value: "" } });
    expect(input.value).toBe("");
    expect(localStorage.getItem("codify:default-base-branch")).toBeNull();
  });
