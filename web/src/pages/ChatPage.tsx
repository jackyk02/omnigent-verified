...
const SESSION_DRAFTS_KEY = "codify.sessionDrafts";
...
export function ChatPage() {
  ...
  // Fallback for session-scoped agents (created by `codify run --server`):
  ...
  // (e.g. a new agent registered by a fresh `codify run` after load),
  ...
  const forkSourceId =
    activeSession?.labels?.["codify.fork.source_id"] ??
    activeConv?.labels?.["codify.fork.source_id"] ??
    null;
  ...
  // sidebar row; the landing page (no active session) stays "Codify".
  ...
  const subAgentTabTitle =
    activeSession?.parentSessionId != null
      ...
      : null;
  useEffect(() => {
    const fallback = urlConvId ? UNTITLED_CONVERSATION_LABEL : "Codify";
  ...
  return (
    <SessionSharedContext.Provider value={isSessionShared}>
      ...
      <ReconnectSessionDialog
        ...
        wrapper={activeConv?.labels?.["codify.wrapper"]}
...
function MainAgentSurface({
  ...
  // session (embedded Codify REPL terminal) runs an in-process harness
...
export function JumpToTopButton({
  ...
  return (
    <div
      ...
      // (see .chat-scroll-fade in index.css), so add --codify-inset-top here
      // too to keep the pill centered on the border. The var is 0px off-shell.
      style={{ top: "calc(50px + var(--codify-inset-top))" }}
      className={cn(
    ...
    if (isIOSShell()) {
      ...
      return nativeBarVisible ? (
        <div
          aria-hidden
          className={cn(
            "codify-native-bottom-spacer",
            terminalFirst.view === "chat" && "codify-native-bottom-spacer--chat",
          )}
        />
      ) : null;
    }
...
interface ComposerProps {
   ...
   * (`showModels`); terminal-first SDK sessions (embedded Codify REPL
   * terminal) keep it.
   */
 ...
 *
 * The ``codify.fork.source_id`` label is *provenance*: it stays on the
 ...
 *
 * @param forkSourceId - The `codify.fork.source_id` label value, or null.
 ...
 * ``codify.closed=true``). The sidebar row is only a fallback.
 *
...
export function readOnlyReasonForSessionLabels(
  ...
  const closed = activeSession?.labels?.["codify.closed"] ?? activeConv?.labels?.["codify.closed"];
  if (closed === "true") return "This sub-agent session is closed";
  const wrapper =
    activeSession?.labels?.["codify.wrapper"] ?? activeConv?.labels?.["codify.wrapper"];
...
export function effortLevelsForConv(
  ...
  switch (conv?.labels?.["codify.wrapper"]) {
 ...
 *
 * Gated on the wrapper label, not `codify.ui === "terminal"`:
...
function useResolvedComposerModel(
  ...
  // default. The other vendor-owns wrappers have no Codify-visible model and
  // stay null.
  ...
  const effectiveModel = nativeVendorOwnsModel
    ? modelPickerKind === "cursor" || modelPickerKind === "kiro"
      ? // cursor mirrors its live TUI model into ``model_override``; kiro sets it
        ...
        // the Codify-visible model is ``model_override``.
        sessionModelOverride
