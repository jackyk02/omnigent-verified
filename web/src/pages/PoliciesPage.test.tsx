...
function policy(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    ...
    name: "block_canada",
    type: "python",
    handler: "codify.policies.block_canada",
    factory_params: null,
    enabled: true,
    created_at: 1,
    updated_at: null,
  ...
  it("renders each policy with its handler, a Disabled badge, and parameters", async () => {
    setPolicies([
      ...
      policy({
        id: "p2",
        name: "rate_limit",
        handler: "codify.policies.rate_limit",
    ...
    expect(screen.getByText("codify.policies.block_canada")).toBeInTheDocument();
...
describe("PoliciesPage actions", () => {
  ...
  it("adds array (multi-select) values via the model combobox as a coerced list", async () => {
    ...
    vi.mocked(policies.usePolicyRegistry).mockReturnValue({
      data: [
        {
          handler: "codify.policies.budget",
          kind: "factory",
          name: "Budget Guard",
    ...
    expect(payload.handler).toBe("codify.policies.budget");
  ...
  it("adds a global policy from the registry via the Add dialog", async () => {
    ...
    expect(addMutate).toHaveBeenCalledWith(
      { name: "block_canada", type: "python", handler: "codify.policies.block_canada" },
      expect.anything(),
    );
  });
