...
describe("effortLevelsForConv", () => {
  it("returns the extended ladder (xhigh, max) for claude-code-native-ui", () => {
    ...
    expect(effortLevelsForConv({ labels: { "codify.wrapper": NATIVE } })).toEqual([
      "low",
      "medium",
      "high",
      "xhigh",
  ...
  it("returns the base three levels for a non-native wrapper", () => {
    ...
    expect(effortLevelsForConv({ labels: { "codify.wrapper": "codex-native" } })).toEqual([
      "low",
      "medium",
      "high",
    ]);
...
describe("shouldShowModelPicker", () => {
  it("shows the picker for the native wrappers that honor a model override", () => {
    ...
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": NATIVE } })).toBe(true);
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "codex-native-ui" } })).toBe(true);
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "cursor-native-ui" } })).toBe(true);
    ...
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "opencode-native-ui" } })).toBe(
      true,
    );
    // kiro applies the picked model as --model at launch (no in-session mirror).
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "kiro-native-ui" } })).toBe(true);
    ...
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "pi-native-ui" } })).toBe(true);
  });
  ...
  it("hides the picker for other wrappers and missing labels (fail closed)", () => {
    ...
    expect(shouldShowModelPicker({ labels: { "codify.wrapper": "codex-native" } })).toBe(false);
...
describe("shouldShowEffortPicker", () => {
  it("shows effort controls only for claude-native sessions", () => {
    ...
    // Web UI effort dial.
    expect(shouldShowEffortPicker({ labels: { "codify.wrapper": NATIVE } })).toBe(true);
  });

  it("hides effort controls for other wrappers and missing labels", () => {
    // WHY: fail-closed — no label / non-native wrapper means no dial.
    expect(shouldShowEffortPicker({ labels: { "codify.wrapper": "codex-native" } })).toBe(false);
  ...
  it("hides effort controls for cursor-native (model switch only, for now)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "codify.wrapper": "cursor-native-ui" } })).toBe(
      false,
    );
  });
  ...
  it("hides effort controls for opencode-native (model indicator only)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "codify.wrapper": "opencode-native-ui" } })).toBe(
      false,
    );
  });
  ...
  it("hides effort controls for kiro-native (model selection only)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "codify.wrapper": "kiro-native-ui" } })).toBe(false);
  });
});
