// Embed entry point.
//
// Exposes `CodifyApp` — a plain React component (app-specific providers +
...
//   - tags a root element with `.codify-app` so the scoped stylesheet
...
import { type CodifyHostConfig, setEmbedRoot, setCodifyHostConfig } from "./lib/host";
...
export type { CodifyHostConfig } from "./lib/host";
...
export { setCodifyHostConfig } from "./lib/host";
...
export interface CodifyAppProps extends CodifyHostConfig {
  /**
   * Router basename, e.g. `/ml/codify-embed`. web's routes + navigation
 ...
 * for styling: renders its own `.codify-app` scope wrapper and registers it
 ...
 * null` guard and the embed renders a permanently blank `.codify-app` div.
 */
...
function CodifyProviders({
  ...
  // not the `.codify-app` scope root — so portaled overlays inherit the
  ...
  return (
    ...
    //   - `.codify-app` (outer) is the scope anchor. The scoped stylesheet
    //     rewrites `:root` → `.codify-app` (light tokens) and `.dark` →
    //     `.codify-app .dark`, so the dark class must be a DESCENDANT of the
    ...
    //     light tokens.
    <div className="codify-app" style={{ height: "100%", width: "100%" }}>
      <div
        ref={scopeRef}
...
/**
 * The Codify app for the SAME-ROOT path: a plain component the host renders
 ...
 */
export function CodifyApp({ basename, routing, isDarkMode, ...hostConfig }: CodifyAppProps = {}) {
  ...
  // The host also sets this eagerly at load (see `loadCodifyApp`); this is a
  // belt-and-suspenders that captures the first non-empty props.
  useState(() => {
    setCodifyHostConfig(hostConfig);
    return null;
  });
  ...
  // The embed owns its QueryClient (bundled react-query); `CodifyProviders`
  // reads it back via `useQueryClient()` under this provider.
  return (
    <QueryClientProvider client={queryClient}>
      <CodifyProviders routing={routingApi} basename={basename} isDarkMode={isDarkMode} />
    </QueryClientProvider>
  );
}
