// Discovery and invocation of the local `codify` CLI for the desktop shell.
//
// The desktop manages servers by shelling out to the same `codify` binary a
...
// test/codify_cli.test.js; the functions that actually spawn a binary are
...
const INSTALL_COMMAND =
  "curl -fsSL https://raw.githubusercontent.com/codify-ai/codify/main/scripts/install_oss.sh | sh";
...
/**
 * The Codify local runtime data dir — `$CODIFY_DATA_DIR` (with `~`
 * expanded) or `~/.codify`. Mirrors `_local_data_dir()` in
 * codify/host/local_server.py. The local-server pidfile lives here.
 *
 * @returns {string}
 */
function localDataDir() {
  const raw = process.env.CODIFY_DATA_DIR;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
  return path.join(os.homedir(), ".codify");
}

/**
 * The Codify config dir — `$CODIFY_CONFIG_HOME` (with `~` expanded) or
 * `~/.codify`. config.yaml (machine identity) lives here; it can differ from
 * the data dir under test env overrides, but is the same by default.
 *
 * @returns {string}
 */
function localConfigDir() {
  const raw = process.env.CODIFY_CONFIG_HOME;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
...
/**
 * The shared Codify state dir, ALWAYS `~/.codify` — it ignores
 * `$CODIFY_DATA_DIR`, mirroring `state_dir()` in
 * sdks/ui/codify_ui_sdk/terminal/_config.py and `_HOST_PID_PATH` in
 * codify/cli.py (both hardcode `Path.home()/".codify"`). The auth-token
 * store and the daemon registry live here — NOT under the data dir. Only the
 * local-server pidfile honors `$CODIFY_DATA_DIR` (see {@link localDataDir}).
 *
 * @returns {string}
 */
function stateDir() {
...
/**
 * This machine's Codify host id (e.g. "host_ab12…"), read from the machine
 * identity in `config.yaml` (`host: host_id:`, written by
 * codify/host/identity.py) — instant, no subprocess. Present once generated,
...
/**
 * Local-server status from the pidfile + a pid-liveness check — no `codify
 ...
 * codify/host/local_server.py. Returns null for a stale pidfile (dead pid, a
 ...
 * point (`codify.cli:main`); `omni` is the short alias. We probe `codify`
 ...
 */
const CLI_NAMES = ["codify", "omni"];

/**
 * Well-known install locations for the CLI binary, in priority order. For each
 * directory we list the `codify` name then the `omni` alias.
 ...
 *
 * @param {string} name e.g. "codify" or "omni"
 * @returns {string | null}
 */
...
/**
 * Resolve the CLI on PATH, trying `codify` then the `omni` alias. Returns the
 * first hit, or null when neither is on PATH.
 *
 * @returns {string | null}
 */
function whichCodify() {
...
/**
 * Locate the `codify` binary. Resolution order: a user-configured path, then
 ...
 *
 * @param {string | null | undefined} configuredPath settings.codify_path
 * @param {{
 *   isExecutableFile?: (p: string) => boolean,
 *   whichCodify?: () => string | null,
...
function resolveCliPath(configuredPath, deps = {}) {
  ...
  const which = deps.whichCodify || whichCodify;
...
/**
 * Run an `codify` subcommand and resolve with its captured output. Never
 ...
 * from `~/.codify/auth_tokens.json` (no subprocess), mirroring
 * codify/cli_auth.py: keyed by the trailing-slash-stripped URL, a record is
 * valid if it's a Databricks pointer (has `workspace_host`) or a non-expired
 * session token. The CLI's `state_dir()` is hardcoded to `~/.codify`.
 *
 * @param {string} serverUrl
 * @returns {boolean}
 */
...
/**
 * Run `codify login <serverUrl>` to authenticate the CLI to a server. It's a
...
async function getCliStatus(configuredPath) {
  ...
  // Must exit cleanly AND identify itself as omni — `codify --version` prints
  // e.g. "codify 0.3.0.dev0 (…)". The exit-code alone isn't enough: an
...
/**
 * `codify server status --json`. Returns the parsed payload, or a synthetic
 ...
 * `_daemon_registry_dir()` in codify/cli.py (`<state_dir>/daemons`).
 *
 * @returns {string}
 */
 ...
 * Mirrors the validation in `_record_from_json()` (codify/cli.py): a usable
...
/**
 * Read every daemon registry record from disk (`~/.codify/daemons/*.json`).
 * This is the fast substitute for `codify host status --json`: it gives the
...
/**
 * The Codify server URL a daemon record talks to, mirroring
 * `_daemon_base_url()` (codify/cli.py): a local-mode daemon's URL lives in
 ...
 * non-expired session token stored by `codify login` in `auth_tokens.json`,
 ...
 * for this URL.
 *
 * Unlike `_remote_headers()` (codify/chat.py), this deliberately does NOT
 * honor `CODIFY_REMOTE_AUTH_TOKEN`. That token is destination-independent — it
 ...
 * `~/.codify/daemons/`. The desktop never needs it: it's not inherited by a
 ...
 * `_add_daemon_host_status()` (codify/cli.py) makes, minus the per-session
...
/**
 * This machine's connection to `serverUrl`, resolved WITHOUT the slow `codify
