// Codify desktop shell — Electron edition.
//
...
// SPA served by the Codify server itself. At startup we read a persisted
...
// which detects the Electron preload on `window.codifyDesktop`).
//
...
const { parseCodifyDeepLink, chooseDeepLinkStrategy } = require("./deepLink");
...
const codifyCli = require("./codify_cli");
...
function updateBadge() {
  ...
  console.log(`[codify] setBadgeCount(${total}) -> ${ok}`);
}
 ...
 * the user connected with (e.g. a Databricks ``…/ml/codifys`` mount), so the
 * window keeps both.
 *
 * @param {BrowserWindow} win
...
function broadcastHostStatus() {
  for (const [win, state] of windows) {
    ...
    try {
      win.webContents.send("codify:host-status-changed");
    } catch {
...
const updater = createDesktopUpdater({
  ...
  // an CODIFY_FORCE_DEV_UPDATE_CONFIG-style env var could otherwise point a
...
// corner window so it shows even against servers running old codify web.
...
/**
 * Resolve the `codify` CLI binary path from the user's configured override
 * (``settings.codify_path``) plus the standard locations, or null when none
...
function resolvedCliPath() {
  const configured = loadSettings().codify_path ?? null;
  if (
    cachedCli &&
    cachedCli.configuredPath === configured &&
    cachedCli.path &&
    codifyCli.isExecutableFile(cachedCli.path)
  ) {
    return cachedCli.path;
  }
  const resolved = codifyCli.resolveCliPath(configured);
...
async function applyCliPath(configuredPath) {
  ...
  const status = await codifyCli.getCliStatus(trimmed || null);
  const accepted = status.installed && status.source === "configured";
  if (accepted) {
    const settings = loadSettings();
    settings.codify_path = trimmed;
    saveSettings(settings);
  } else if (trimmed === "") {
    const settings = loadSettings();
    delete settings.codify_path;
    saveSettings(settings);
  }
...
async function clearCliPath() {
  ...
  delete settings.codify_path;
  saveSettings(settings);
  return codifyCli.getCliStatus(null);
}
 ...
 * may carry a workspace mount (e.g. ``https://host/ml/codifys/``). The path
 ...
 * ``/ml/codifys`` — so we string-concatenate: strip the server URL's trailing
...
function createWindow(targetUrl, opts = {}) {
  ...
  // (``codify host --server``, ``codify login``, ``serverAuthed``) — the
  ...
  // Databricks workspace-hosted Codify renders inside the workspace's
  // top-nav chrome (the SPA is a workspace page). On a dedicated desktop
  // window, hide it by overlaying Codify's own root — see
...
function openFindBar(target) {
  ...
  if (existing && !existing.isDestroyed()) {
    existing.focus();
    existing.webContents.send("codify:find-activate");
    return;
  }
  ...
  const onFound = (_event, result) => {
    ...
    bar.webContents.send("codify:find-result", {
...
async function confirmHostEnrollment(win) {
  ...
  // Codify's own prompt rather than an anonymous system alert; in a packaged
  ...
  const { response } = await dialog.showMessageBox(win, {
    ...
    title: "Codify",
    message: `Allow ${host} to manage Codify on this machine?`,
    detail:
...
function signalForeground() {
  try {
    ...
    console.warn("[codify] signalForeground failed:", err);
  }
}
...
function playSystemSound(name) {
  ...
  try {
    ...
    const child = execFile("afplay", [file], (err) => {
      if (err) console.warn("[codify] afplay failed:", err.message);
    });
    child.unref();
  } catch (err) {
    console.warn("[codify] failed to spawn afplay:", err);
  }
}
...
function buildMenu() {
  ...
  // macOS app menu (About/Services/Hide/Quit), named "Codify" via the
        ...
        try {
          ...
          if (status.state === "none") {
            await dialog.showMessageBox(activeWindow(), {
              ...
              detail: `Codify ${app.getVersion()} is the latest version.`,
              buttons: ["OK"],
            });
          }
        } catch (err) {
...
// ---------------------------------------------------------------------------
// IPC: the preload bridge (window.codifyDesktop) forwards these from the
...
//
// `codify:browser-execute` runs JS via executeJavaScript; exposed to preload
...
function registerIpc() {
  ...
  ipcMain.handle("codify:set-server-url", async (event, url) => {
    ...
    // the Codify UI mount so the user can paste just the workspace host.
  ...
  ipcMain.handle("codify:get-server-url", (event) => {
    if (!isSetupPageSender(event)) {
  ...
  ipcMain.handle("codify:get-recent-servers", (event) => {
    if (!isSetupPageSender(event)) {
  ...
  ipcMain.handle("codify:get-server-picker", (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[codify] get-server-picker from untrusted sender dropped");
      return null;
    }
    const win = BrowserWindow.fromWebContents(event.sender);
    const recents = loadSettings().recent_servers;
  ...
  ipcMain.handle("codify:switch-server", (event, url) => {
    if (!isPinnedOriginSender(event)) {
  ...
  ipcMain.on("codify:open-server-setup", (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[codify] open-server-setup from untrusted sender dropped");
      return;
    }
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) return;
  ...
  ipcMain.on("codify:find-query", (event, params) => {
    if (!isFindBarSender(event)) {
      console.warn("[codify] find-query from untrusted sender dropped");
      return;
    }
    ...
    if (text === "") {
      target.webContents.stopFindInPage("clearSelection");
      event.sender.send("codify:find-result", { active: 0, matches: 0 });
      return;
    }
  ...
  ipcMain.on("codify:find-close", (event) => {
    if (!isFindBarSender(event)) {
      console.warn("[codify] find-close from untrusted sender dropped");
      return;
    }
  ...
  ipcMain.on("codify:set-badge-count", (event, count) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[codify] set-badge-count from untrusted sender dropped");
      return;
    }
    // isPinnedOriginSender guarantees the sender window is tracked.
  ...
  ipcMain.handle("codify:notify", (event, params) => {
    ...
    notification.on("click", () => {
      ...
      if (navigatePath && !event.sender.isDestroyed()) {
        try {
          event.sender.send("codify:notification-activated", navigatePath);
        } catch {
  ...
  // Setup page → is the `codify` CLI installed and runnable? Includes the
  // resolved path, version, and the install one-liner to show when missing.
  ipcMain.handle("codify:get-cli-status", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("get-cli-status is only available to the setup page");
    }
    return codifyCli.getCliStatus(loadSettings().codify_path);
  });

  // Setup page → set an explicit path to the `codify` binary. Persisted only
  // when that exact path validates as a runnable codify (so a typo doesn't
  // silently mask a working PATH lookup). Returns the resulting CLI status plus
  // whether the configured path was accepted.
  ipcMain.handle("codify:set-cli-path", async (event, configuredPath) => {
    if (!isSetupPageSender(event)) {
  ...
  // Setup page → native file picker for the codify binary. Returns the chosen
  // path (the renderer feeds it back through set-cli-path) or null on cancel.
  ipcMain.handle("codify:browse-cli-path", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("browse-cli-path is only available to the setup page");
    }
    const win = BrowserWindow.fromWebContents(event.sender) ?? activeWindow();
    const result = await dialog.showOpenDialog(win ?? undefined, {
      title: "Locate the Codify CLI binary",
      properties: ["openFile"],
    });
  ...
  ipcMain.handle("codify:start-local-server", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("start-local-server is only available to the setup page");
    }
    const cliPath = resolvedCliPath();
    if (!cliPath) {
      return { ok: false, error: "The codify CLI was not found. Install it or set its path." };
    }
    return serverManager.startLocalServer(cliPath);
  });

  // SPA → this machine's identity: is the CLI installed, and its host id. Both
  // come from local config (no `codify host status` subprocess), so this is
  // instant — it lets the new-session picker tag/connect "this machine" without
  // waiting on the slow runner-status check.
  ipcMain.handle("codify:host-get-identity", (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[codify] host-get-identity from untrusted sender dropped");
      return null;
    }
    return { cliInstalled: Boolean(resolvedCliPath()), hostId: codifyCli.localHostId() };
  });
  ...
  ipcMain.handle("codify:cli-get-status", async (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[codify] cli-get-status from untrusted sender dropped");
      return null;
    }
    return codifyCli.getCliStatus(loadSettings().codify_path);
  });
  ...
  ipcMain.handle("codify:cli-reset-path", async (event) => {
    if (!isPinnedOriginSender(event)) {
  ...
  ipcMain.on("codify:set-color-scheme", (event, scheme) => {
  ...
  ipcMain.handle("codify:host-control", async (event, action) => {
    if (!isPinnedOriginSender(event)) {
  ...
  // Embedded browser pane — the `codify:browser-*` surface lives in
...
// ---------------------------------------------------------------------------
// Deep links (`codify://<hostname>/c/<session_id>`)
//
// An OS-clicked `codify://` URL opens the named session on the named server.
...
function sendOpenPath(win, routePath) {
  if (!win || win.isDestroyed()) return;
  console.log(`[codify] deep-link: send open-path ${routePath}`);
  try {
    win.webContents.send("codify:open-path", routePath);
  } catch {
    // Window torn down between the check and the send; ignore.
  }
}
...
async function confirmOpenDeepLink(parent, targetOrigin) {
  ...
  const { response } = await dialog.showMessageBox(parent, {
    ...
    icon: icon.isEmpty() ? undefined : icon,
    title: "Codify",
    message: `Open this Codify link?`,
    detail:
      `This link will connect Codify to ${host} and open a conversation.\n\n` +
...
/**
 * Queue a deep link for handling. Unrecognized links (parseCodifyDeepLink
...
function enqueueDeepLink(raw) {
  if (!parseCodifyDeepLink(raw)) {
    console.log(`[codify] deep-link: ignored unrecognized URL ${String(raw)}`);
    return;
  }
  console.log(`[codify] deep-link: queued ${raw} (ready=${app.isReady()})`);
...
function drainPendingDeepLinks() {
  ...
  void handleDeepLink(next)
    .catch((err) => console.warn("[codify] deep-link handling failed:", err))
    .finally(() => {
...
/**
 * Open an `codify://` deep link on the right window. The window-selection
 ...
 * probe is safe post-consent because it can only append a path (`/ml/codifys`)
 * under the SAME origin — it never changes the origin the user approved.
 *
 * @param {string} raw The raw `codify://...` URL.
 * @returns {Promise<void>}
 */
async function handleDeepLink(raw) {
  const parsed = parseCodifyDeepLink(raw);
  if (!parsed) return;
  ...
  // `https://host/ml/codifys`) so we SKIP the probe entirely. null for an
  ...
  console.log(
    `[codify] deep-link: strategy=${decision.strategy} ` +
...
if (!gotLock) {
  ...
  // codify:// URL as a command-line arg. macOS packaged builds get URLs via
  ...
  // `npm start -- 'codify://...'` exercise the real code path on macOS too.
  // Safe: a packaged macOS launch has no codify:// in argv, so no double-handling.
  for (const arg of process.argv) {
    if (typeof arg === "string" && arg.startsWith("codify://")) enqueueDeepLink(arg);
  }

  // macOS: `open-url` fires for codify:// links, including BEFORE
  ...
  app.on("second-instance", (_event, argv) => {
    ...
    // codify:// URL on its command line; the single-instance lock funnels
    // it here. On Windows/Linux that's the OS dispatch; on macOS it's how a
    // second `npm start -- 'codify://...'` reaches the running DEV instance
    ...
    for (const arg of argv) {
      if (typeof arg === "string" && arg.startsWith("codify://")) {
        enqueueDeepLink(arg);
        handledUrl = true;
      }
    }
  ...
  app.whenReady().then(() => {
    ...
    if (process.platform === "win32") app.setAppUserModelId("ai.codify.desktop");
    applyDockIcon();
    registerPermissions();
    ...
    resolvedCliPath();
    // Register the codify:// scheme so OS clicks route to this app. The
    ...
    app.setAsDefaultProtocolClient("codify");
  ...
  // is a no-op, or shutdown hangs (a stuck `codify server stop`), the app
  ...
  // children are SIGKILL'd at 4s and a normal `codify server stop` is sub-
  ...
  // pidfile that the next launch reuses or `codify server stop` reclaims.
  ...
  app.on("before-quit", (event) => {
    ...
    // SIGKILL'd within 4s and `codify server stop` has its own exec timeout.
    (async () => {
      const cliPath = resolvedCliPath();
