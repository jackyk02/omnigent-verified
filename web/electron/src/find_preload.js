...
contextBridge.exposeInMainWorld("codifyFind", {
  /**
   ...
   */
  query: (text, opts) => {
    ipcRenderer.send("codify:find-query", {
      text: String(text ?? ""),
  ...
  close: () => {
    ipcRenderer.send("codify:find-close");
  },
  /**
   ...
   */
  onResult: (callback) => {
    ipcRenderer.on("codify:find-result", (_event, result) => callback(result));
  },
  /** Fires when Cmd/Ctrl+F re-activates an already-open bar. */
  onActivate: (callback) => {
    ipcRenderer.on("codify:find-activate", () => callback());
  },
});
