...
//
// The overlay is a TRUSTED surface: its `codify:overlay-*` IPC drives the
...
function createUpdateOverlay({
  ...
  function registerIpc() {
    ...
    ipcMain.on("codify:overlay-height", (event, height) => {
    ...
    ipcMain.handle("codify:overlay-get-update-config", (event) => {
      guard(event);
      return updater.getConfig();
    });
    ipcMain.handle("codify:overlay-get-update-status", (event) => {
      guard(event);
      return updater.getStatus();
    });
    ipcMain.handle("codify:overlay-update-check", async (event) => {
      guard(event);
      await updater.checkForUpdates({ manual: true });
    });
    ipcMain.handle("codify:overlay-update-download", async (event) => {
      guard(event);
      await updater.downloadUpdate();
    });
    ipcMain.handle("codify:overlay-update-install", (event) => {
      guard(event);
      if (!updater.installUpdateNow()) {
        throw new Error("No downloaded update is ready to install.");
      }
    });
    ipcMain.handle("codify:overlay-set-update-config", (event, patch) => {
      guard(event);
    ...
    nativeTheme.on("updated", () => {
      ...
      for (const overlay of overlays.values()) {
        if (!overlay.isDestroyed()) {
          overlay.webContents.send("codify:overlay-theme", theme);
        }
      }
    });
  }
