...
// Codify SPA. Kept in its own Electron-free module so the injection logic is
 ...
 * dedicated desktop window that chrome is just noise. We promote Codify's
 * own root — ``.codify-app``, the wrapper web's embed entry sets
 * (``web/src/embed.tsx``) — to a full-viewport overlay so it paints over
 * the workspace bar. Keying on Codify's wrapper (defined in THIS repo)
 ...
 * standalone (non-embed) build there is no ``.codify-app``, so the rule is
 * a harmless no-op.
 */
 ...
 * targets ``.codify-app`` — which exists solely in the workspace-embedded
