// Process lifecycle for desktop-managed Codify servers and host connections.
//
// This is the only place the desktop spawns long-lived processes. It owns:
//   - hostChildren: the foreground `codify host --server <url>` processes this
//     app started. They are torn down when the app quits (the confirmed
//     lifecycle: the desktop owns what it starts).
//   - ownedLocalServer: a local `codify server` we started ourselves (and so
...
// (codify_cli.js), which is the single source of truth. This module only
...
/** The line `codify host` prints once the websocket tunnel is up. */
...
/**
 * Every `codify host` child we have spawned and not yet seen exit — including
 ...
 * from `codify host`'s own messages (HostConnectError: "Authentication
 * failed", "HTTP 401", login-page redirect, or the `codify login` hint). Lets
...
function isAuthError(text) {
  return /authentication failed|http 401|unauthor|login page|codify login/i.test(
    String(text || ""),
  );
}

/**
 * True when `codify host` refused to start because a daemon already serves
...
/**
 * Spawn `codify host --server <url>` and resolve once it reports connected
 ...
 * *adopt* it without spawning a duplicate — `codify host` would otherwise
...
async function ensureHostConnected(cliPath, serverUrl) {
  ...
  // two `codify host` processes for one target.
...
async function connectHost(cliPath, serverUrl, key) {
  ...
  // tunnel, via the fast disk-read + single HTTP probe — not the slow `codify
 ...
 * stored credentials, runs `codify login <url>` (browser/OIDC/Databricks; a
 * no-op when the server needs no auth). Returns ok when already authed, after a
 * successful login, or for a no-auth server; an error (pointing at `codify
...
async function ensureServerAuth(cliPath, serverUrl) {
  ...
  return {
    ok: false,
    error: `Sign-in required — run \`codify login ${serverUrl}\` in a terminal, then try again.`,
  };
}
...
async function startLocalServer(cliPath) {
  ...
  // `codify server status` (a Python cold start). We didn't start it, so no
  // ownership claim.
...
async function shutdown(cliPath) {
  ...
  // during a connect can't leave an orphaned `codify host` process.
  const exits = [];
