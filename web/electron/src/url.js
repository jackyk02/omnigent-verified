...
// `setup/index.html`, where it publishes `window.codifyUrl`). One copy keeps
...
(function (root, factory) {
  ...
  /**
   * Path under a Databricks workspace where the Codify web UI is mounted. A
   ...
   * appended.
   *
   * NOTE: the Python CLI records the UI mount as ``/codify`` in
   * ``codify/conversation_browser.py`` (WORKSPACE_UI_PATH), whereas the
   * desktop deliberately keeps ``/ml/codifys`` for now — that is the path the
   * live workspace serves the embedded SPA on. The two are intentionally
   * divergent pending reconciliation; do not "fix" this to ``/codify``
   * without verifying what the workspace actually serves to the desktop shell.
   */
  const WORKSPACE_UI_PATH = "/ml/codifys";
   ...
   * workspaces and have no ``/ml/codifys`` mount. Skip expansion for these
  ...
  /**
   * Expand a bare Databricks workspace URL to its Codify web-UI mount.
   *
   * Mirrors the omni CLI's behavioral detection
   * (``codify/cli.py:_workspace_api_server_url``): rather than match
   ...
   * untouched WITHOUT a probe, so a user who pastes the full ``…/ml/codifys``
  ...
  async function expandDatabricksWorkspaceUrl(normalized) {
    ...
    // no ``/ml/codifys`` mount, so never expand them.
