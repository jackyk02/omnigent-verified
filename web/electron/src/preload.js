...
// page. Two consumers:
//
//   1. window.codifyDesktop — read by the web app's nativeBridge.ts
//      (badge + notifications). Its `kind: "electron"` field is the
//      feature-detection discriminator.
//   2. window.codifySetup — used only by the bundled setup page to
...
contextBridge.exposeInMainWorld("codifyDesktop", {
  ...
  setBadgeCount: (count) => {
    ipcRenderer.send("codify:set-badge-count", count);
  },
  /**
   ...
   */
  notify: (params) =>
    ipcRenderer.invoke("codify:notify", {
      title: params?.title,
      body: params?.body,
  ...
  onNotificationActivated: (callback) => {
    ...
    ipcRenderer.on("codify:notification-activated", listener);
    return () => ipcRenderer.removeListener("codify:notification-activated", listener);
  },
  /**
   * Subscribe to deep-link navigations. When the user clicks an
   * `codify://.../c/<id>` link for a server this window is already on, the
  ...
  onOpenPath: (callback) => {
    ...
    ipcRenderer.on("codify:open-path", listener);
    return () => ipcRenderer.removeListener("codify:open-path", listener);
  },
  /**
   ...
   */
  getServerPicker: () => ipcRenderer.invoke("codify:get-server-picker"),
  /**
   ...
   */
  switchServer: (url) => ipcRenderer.invoke("codify:switch-server", url),
  /** Return this window to the bundled "connect to server" setup page. */
  openServerSetup: () => {
    ipcRenderer.send("codify:open-server-setup");
  },
  /**
   ...
   */
  getHostIdentity: () => ipcRenderer.invoke("codify:host-get-identity"),
  /**
   * Start / stop / restart this machine's host daemon for the window's server.
   * Resolves a `{ ok, error? }` result.
   * @param {"start" | "stop" | "restart"} action
   */
  controlHost: (action) => ipcRenderer.invoke("codify:host-control", action),
  /**
  ...
  onHostStatusChanged: (callback) => {
    ...
    ipcRenderer.on("codify:host-status-changed", listener);
    return () => ipcRenderer.removeListener("codify:host-status-changed", listener);
  },
  /**
   ...
   * binary is in use.
   */
  getCliStatus: () => ipcRenderer.invoke("codify:cli-get-status"),
  /**
   ...
   */
  resetCliPath: () => ipcRenderer.invoke("codify:cli-reset-path"),
  ...
  updates: {
    getConfig: () => ipcRenderer.invoke("codify:get-update-config"),
    getStatus: () => ipcRenderer.invoke("codify:get-update-status").then(bannerSafe),
    check: () => ipcRenderer.invoke("codify:update-check"),
    download: () => ipcRenderer.invoke("codify:update-download"),
    installNow: () => ipcRenderer.invoke("codify:update-install"),
    setConfig: (patch) => ipcRenderer.invoke("codify:set-update-config", patch),
    onStatus: (callback) => {
   ...
   */
  setColorScheme: (scheme) => ipcRenderer.send("codify:set-color-scheme", scheme),
  ...
  browserOpenOrNavigate: (conversationId, url, bounds, opts) =>
    ipcRenderer.invoke("codify:browser-open-or-navigate", { conversationId, url, bounds, opts }),
  /**
  ...
  browserSetActive: (conversationId) =>
    ipcRenderer.invoke("codify:browser-set-active", { conversationId }),
  /**
  ...
  browserResize: (conversationId, bounds) =>
    ipcRenderer.invoke("codify:browser-resize", { conversationId, bounds }),
  /**
  ...
  browserScreenshot: (conversationId) =>
    ipcRenderer.invoke("codify:browser-screenshot", { conversationId }),
  /**
  ...
  browserExecute: (conversationId, js) =>
    ipcRenderer.invoke("codify:browser-execute", { conversationId, js }),
  /**
  ...
  browserClose: (conversationId, reason) =>
    ipcRenderer.invoke("codify:browser-close", { conversationId, reason }),
  /**
  ...
  browserHasView: (conversationId) =>
    ipcRenderer.invoke("codify:browser-has-view", { conversationId }),
  /**
  ...
  browserGoBack: (conversationId) =>
    ipcRenderer.invoke("codify:browser-go-back", { conversationId }),
  /**
  ...
  browserGoForward: (conversationId) =>
    ipcRenderer.invoke("codify:browser-go-forward", { conversationId }),
  /**
  ...
  browserReload: (conversationId) =>
    ipcRenderer.invoke("codify:browser-reload", { conversationId }),
  /**
  ...
  openBrowserDevTools: (conversationId) =>
    ipcRenderer.invoke("codify:open-browser-devtools", { conversationId }),
  /**
  ...
  browserEnableDesignMode: (conversationId) =>
    ipcRenderer.invoke("codify:browser-enable-design-mode", { conversationId }),
  /**
  ...
  browserDisableDesignMode: (conversationId) =>
    ipcRenderer.invoke("codify:browser-disable-design-mode", { conversationId }),
  /**
  ...
  browserSignalDesignResult: (conversationId, result) =>
    ipcRenderer.invoke("codify:browser-signal-design-result", {
      conversationId,
      id: result?.id,
      ok: result?.ok,
...
contextBridge.exposeInMainWorld("codifySetup", {
  getServerUrl: () => ipcRenderer.invoke("codify:get-server-url"),
  /**
   ...
   * @param {string} url
   */
  setServerUrl: (url) => ipcRenderer.invoke("codify:set-server-url", url),
  /** Recently-connected server URLs, most recent first. */
  getRecentServers: () => ipcRenderer.invoke("codify:get-recent-servers"),
  /**
   * Whether the `codify` CLI is installed/runnable, e.g.
   * `{installed, path, version, source, installCommand}`.
   */
  getCliStatus: () => ipcRenderer.invoke("codify:get-cli-status"),
  /**
   * Set an explicit path to the codify binary. Resolves the CLI status plus
   * `accepted` (whether that exact path validated and was saved).
   * @param {string} path
   */
  setCliPath: (path) => ipcRenderer.invoke("codify:set-cli-path", path),
  /** Native file picker for the codify binary; resolves the path or null. */
  browseCliPath: () => ipcRenderer.invoke("codify:browse-cli-path"),
  /**
   * Start (or reuse) the local server. Resolves `{ok, url?, error?}`; the
   * caller then connects to `url` via setServerUrl.
   */
  startLocalServer: () => ipcRenderer.invoke("codify:start-local-server"),
});
