...
//
// An `codify://<hostname>/c/<session_id>` URL names a server by host (with
...
// workspace mount (`/ml/codifys`) is deliberately NOT in the link: it is
...
/**
 * Parse an `codify://` deep link into a server origin + an in-app path.
 *
 ...
 *
 * @param {string} raw e.g. ``"codify://localhost:8000/c/conv_abc"``.
 * @returns {{ origin: string, path: string } | null} ``null`` for anything
 *   that isn't a valid `codify://.../c/<id>` link (wrong scheme, no host,
 *   non-`/c/` path, unparseable input).
 */
function parseCodifyDeepLink(raw) {
  let url;
  try {
  ...
  if (url.protocol !== "codify:") return null;
  // No host → a bare `codify://` or `codify:`; nothing to connect to.
...
module.exports = { parseCodifyDeepLink, chooseDeepLinkStrategy, DEEP_LINK_PATH_RE };
