// Desktop auto-update orchestration for the Codify Electron shell.
//
...
// menu, the `codify:*update*` IPC surface (`registerIpc()`), and the
...
function createDesktopUpdater({
  ...
  function broadcast(status) {
    ...
    for (const win of BrowserWindow.getAllWindows()) {
      ...
      try {
        win.webContents.send("codify:update-status", status);
      } catch {
  ...
  function init() {
    ...
    autoUpdater.on("error", (err) => {
      ...
      manualCheckInFlight = false;
      console[isSecurity ? "error" : "warn"]("[codify] update error:", msg);
  ...
  async function confirmControl(win, action) {
    ...
    const copy = {
      download: {
        ...
        detail:
          `${host} wants to download a desktop update for this Codify app.\n\n` +
          `Only allow servers you trust.`,
      },
      install: {
        message: "Restart Codify to install an update?",
        detail:
          `${host} wants to restart Codify and install the downloaded desktop update.\n\n` +
          `Only allow servers you trust.`,
      },
      config: {
        message: "Change Codify update settings?",
        detail:
          `${host} wants to change how this Codify app checks for and installs updates.\n\n` +
          `Only allow servers you trust.`,
      },
    }[action];
  ...
  function registerIpc() {
    ipcMain.handle("codify:get-update-config", (event) => {
    ...
    ipcMain.handle("codify:get-update-status", (event) => {
      if (!isPinnedOriginSender(event)) {
    ...
    ipcMain.handle("codify:update-check", async (event) => {
      if (!isPinnedOriginSender(event)) {
    ...
    ipcMain.handle("codify:update-download", async (event) => {
      if (!isPinnedOriginSender(event)) {
    ...
    ipcMain.handle("codify:update-install", async (event) => {
      if (!isPinnedOriginSender(event)) {
    ...
    ipcMain.handle("codify:set-update-config", async (event, patch) => {
      if (!isPinnedOriginSender(event)) {
