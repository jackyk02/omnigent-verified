# Codify Desktop (Electron)

A thin [Electron](https://www.electronjs.org) desktop shell around the
existing Codify web UI. It shows the **same** UI you get in a browser, but
...
- **A foreground attention cue.** macOS (and Windows) suppress the notification
  ...
  the shell plays it, the alert is audible **whether Codify is backgrounded or
...
1. If no server URL is saved yet, it shows the setup page (one input +
   Connect). You enter your Codify server URL (default
   `http://localhost:8000`).
...
Electron shell at runtime (the preload exposes `window.codifyDesktop`
...
electron/
  ...
  src/preload.js           # contextBridge: window.codifyDesktop + codifySetup
  src/find_preload.js      # contextBridge for the find bar: window.codifyFind
  ...
  src/browserIpc.js           # codify:browser-* IPC handlers (extracted from main.js)
...
- **Security posture**: `nodeIntegration: false`, `contextIsolation: true`.
  ...
  **pinned to the one server origin the user explicitly connected it to**,
  ...
  - Instead, every privileged IPC handler verifies its sender frame.
    ...
    (`codifySetup`) only works for the bundled setup page itself, so a
...
sequenceDiagram
    ...
    participant S as Codify server
...
- `src/browserIpc.js` — the whole `ipcMain.handle('codify:browser-*')` surface,
...
- `src/preload.js` — adds `browserOpenOrNavigate/SetActive/Resize/Screenshot/`
  ...
  `onBrowserElementPromptDismiss` to `window.codifyDesktop`, each a thin
...
**JS trust boundary (important):** `codify:browser-execute` runs
...
The shell opens on the bundled setup page. Point it at a running Codify
...
```bash
spctl -a -vv dist/mac-arm64/Codify.app   # → "accepted, source=Notarized Developer ID"
```
...
Any reachable Codify server works. For a quick local target, run the
server from this repo:

```bash
# from the repo root, with the project venv:
.venv/bin/python -m codify.server   # serves on http://localhost:8000
```
...
`codify` CLI to start a server and register this machine as a **host** (a
...
The CLI ships under two names that resolve to the same entry point — `codify`
(canonical) and `omni` (short alias) — and the shell probes **both**:
`settings.codify_path` first, then `PATH` (`codify` then `omni`), then the
...
A configured path is saved to `settings.json` (`codify_path`) only once it
...
**"Start a server on this machine"** runs `codify server --background` (idempotent —
...
dialog** ("Allow _host_ to manage Codify on this machine?") with **Don't Allow**
...
`codify host --server <url>`. **Allow Once** connects this time and re-prompts
...
bridge — `window.codifyDesktop` → `getHostStatus` / `getHostIdentity` /
...
`~/Library/Application Support/Codify/settings.json`.)
...
An `codify://<hostname>/c/<session_id>` URL opens that session on that
server in the desktop app — the way a browser deep link opens a page:

```
codify://localhost:8000/c/conv_abc              → http://localhost:8000/c/conv_abc
codify://my-workspace.cloud.databricks.com/c/x → https://…/ml/codifys/c/x
```
...
pasted URL can never disagree. The Databricks workspace mount (`/ml/codifys`)
...
packaged installs, plus a runtime `app.setAsDefaultProtocolClient("codify")`
...
- **Native bridge detection:** `window.codifyDesktop` (`kind: "electron"`),
