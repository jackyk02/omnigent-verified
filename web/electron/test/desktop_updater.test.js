...
describe("desktop_updater — IPC trust + consent", () => {
  ...
  it("prompts and runs each privileged action when approved", async () => {
    ...
    h.updater.init();
    h.updater.registerIpc();
    await h.ipcHandlers.get("codify:update-download")(h.event);
    assert.equal(h.calls.showMessageBox.length, 1);
    assert.equal(h.calls.showMessageBox[0].options.message, "Download an Codify update?");
    ...
    h.updater.init();
    h.updater.registerIpc();
    await h.ipcHandlers.get("codify:set-update-config")(h.event, { mode: "manual" });
    assert.equal(h.calls.showMessageBox[0].options.message, "Change Codify update settings?");
    assert.equal(h.readSettings().update_mode, "manual");
  });
  ...
  it("does not let a denied consent run the action", async () => {
    ...
    await assert.rejects(h.ipcHandlers.get("codify:update-download")(h.event), /approved/);
    assert.equal(h.calls.downloadUpdate, 0);
  });
});
...
describe("desktop_updater — install handoff", () => {
  it("routes an approved install through before-quit to quitAndInstall", async () => {
    ...
    await h.ipcHandlers.get("codify:update-install")(h.event);
    assert.equal(h.calls.showMessageBox.length, 1);
  ...
  it("install fails cleanly when no update is downloaded", async () => {
    ...
    await assert.rejects(
      h.ipcHandlers.get("codify:update-install")(h.event),
