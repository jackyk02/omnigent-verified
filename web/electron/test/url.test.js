...
// pasted workspace URL (schemeless, /codify suffix from the internal user
...
describe("defaultSchemeFor", () => {
  it("defaults remote hosts to https", () => {
    assert.equal(defaultSchemeFor("dbc-x.cloud.databricks.com/codify"), "https");
...
describe("normalizeUrl", () => {
  it("defaults a schemeless workspace /codify URL to https", () => {
    assert.equal(
      normalizeUrl("dbc-a5d4177a-49dc.cloud.databricks.com/codify"),
  ...
  it("trims surrounding whitespace", () => {
    assert.equal(normalizeUrl("  example.com/codify  "), "https://example.com/codify");
  });
...
describe("isPlainHttpRemote", () => {
  it("does not warn for a bare remote host (now https)", () => {
    ...
    assert.equal(isPlainHttpRemote("dbc-x.cloud.databricks.com/codify"), false);
  });
...
describe("expandDatabricksWorkspaceUrl", () => {
  ...
  it("leaves a URL that already carries a path untouched, without probing", async () => {
    ...
    await withFetch(
      ...
      async () => {
        const url = "https://ws.cloud.databricks.com/codify";
