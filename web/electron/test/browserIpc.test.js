...
describe("browserIpc — trust gate", () => {
  it("registers every browser-* channel", () => {
    ...
    for (const ch of [
      "codify:browser-open-or-navigate",
      "codify:browser-set-active",
      "codify:browser-resize",
      "codify:browser-screenshot",
      "codify:browser-execute",
      "codify:browser-has-view",
      "codify:browser-close",
      "codify:browser-go-back",
      "codify:browser-go-forward",
      "codify:browser-reload",
      "codify:open-browser-devtools",
      "codify:browser-enable-design-mode",
      "codify:browser-disable-design-mode",
      "codify:browser-signal-design-result",
    ]) {
...
describe("browserIpc — history navigation", () => {
  it("go-back issues goBack only when canGoBack is true", async () => {
    ...
    const r = await ipcMain.invoke("codify:browser-go-back", event, { conversationId: "conv_1" });
  ...
  it("go-forward issues goForward when canGoForward is true", async () => {
    ...
    const { ipcMain, event } = setup({ webContents: wc });
    await ipcMain.invoke("codify:browser-go-forward", event, { conversationId: "conv_1" });
  ...
  it("reload calls webContents.reload", async () => {
    const wc = makeWebContents();
    const { ipcMain, event } = setup({ webContents: wc });
    await ipcMain.invoke("codify:browser-reload", event, { conversationId: "conv_1" });
  ...
  it("returns {ok:false} for a missing view", async () => {
    ...
    const r = await ipcMain.invoke("codify:browser-go-back", event, { conversationId: "nope" });
...
describe("browserIpc — devtools toggle", () => {
  it("opens devtools docked bottom when closed, closes when open", async () => {
    const wc = makeWebContents();
    const { ipcMain, event } = setup({ webContents: wc });
    await ipcMain.invoke("codify:open-browser-devtools", event, { conversationId: "conv_1" });
...
describe("browserIpc — url live-tracking", () => {
  it("open-or-navigate wires did-navigate listeners that emit url + nav-state", async () => {
    ...
    await ipcMain.invoke("codify:browser-open-or-navigate", event, {
...
describe("browserIpc — design mode", () => {
  ...
  it("enable injects the picker script and attaches a console-message listener", async () => {
    const wc = makeWebContents();
    const { ipcMain, event } = setup({ webContents: wc });
    const r = await ipcMain.invoke("codify:browser-enable-design-mode", event, {
      conversationId: "conv_1",
    });
    assert.equal(r.ok, true);
  ...
  it("a valid nonced submit marker following a native gesture is forwarded", async () => {
    ...
    await ipcMain.invoke("codify:browser-enable-design-mode", event, {
      conversationId: "conv_1",
    });
  ...
  it("disable detaches the console-message listener", async () => {
    ...
    assert.equal(wc.listenerCount("console-message"), 1);
    const r = await ipcMain.invoke("codify:browser-disable-design-mode", event, {
      conversationId: "conv_1",
    });
    assert.equal(r.ok, true);
  ...
  it("signal-design-result forwards the coerced envelope into the page", async () => {
    const wc = makeWebContents();
    const { ipcMain, event } = setup({ webContents: wc });
    const r = await ipcMain.invoke("codify:browser-signal-design-result", event, {
      conversationId: "conv_1",
      id: 7,
      ok: true,
