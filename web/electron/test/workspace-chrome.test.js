...
describe("applyWorkspaceChromeHideCss", () => {
  it("injects the chrome-hide CSS even when the URL is not under the workspace path", () => {
    const injected = [];
    const webContents = {
      // A path variant the old guard would have skipped (not /ml/codifys).
    ...
    assert.deepEqual(
      ...
      [
        ...
        "URL, but it did not fire for a non-/ml/codifys path. A URL/path guard has likely",
        ...
        "must stay unconditional — the CSS only targets .codify-app (workspace-embedded",
  ...
  it("injects the chrome-hide CSS once when did-finish-load fires", () => {
    ...
    assert.deepEqual(
      ...
      [
        ...
        "stays visible in the desktop window and users can navigate out of Codify.",
      ].join(" "),
    );
  });
});
