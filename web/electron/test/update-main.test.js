...
function loadMainHarness({
  ...
  const userData = fs.mkdtempSync(path.join(os.tmpdir(), "codify-update-test-"));
  ...
  const sandbox = {
    ...
    process: {
      ...
      env: {
        ...process.env,
        // No CODIFY_FORCE_DEV_UPDATE_CONFIG injection: main.js now derives
...
describe("auto-update main-process wiring", () => {
  ...
  it("rejects the frozen IPC handlers from a non-pinned sender", async (t) => {
    ...
    const cases = [
      ["codify:get-update-config", []],
      ["codify:get-update-status", []],
      ["codify:update-check", []],
      ["codify:update-download", []],
      ["codify:update-install", []],
      ["codify:set-update-config", [{ mode: "manual" }]],
    ];
    await Promise.all(
  ...
  it("prompts for every privileged update channel before running it", async (t) => {
    const cases = [
      {
        channel: "codify:update-download",
        args: [],
        message: "Download an Codify update?",
        prepare: () => {},
        assertRan: (harness) => {
      ...
      {
        channel: "codify:update-install",
        args: [],
        message: "Restart Codify to install an update?",
        prepare: (harness) => {
      ...
      {
        channel: "codify:set-update-config",
        args: [{ mode: "manual" }],
        message: "Change Codify update settings?",
        prepare: () => {},
        assertRan: (harness) => {
          assert.equal(harness.readSettings().update_mode, "manual");
        },
    ...
    for (const item of cases) {
      ...
      assert.equal(harness.calls.showMessageBox[0].options.title, "Codify");
  ...
  it("routes approved update-install through before-quit cleanup to quitAndInstall", async (t) => {
    ...
    await harness.ipcHandlers.get("codify:update-install")(harness.events.pinned);
  ...
  it("force-exits if before-quit cleanup hangs past the safety cap", async (t) => {
    // A stuck shutdown (e.g. a hung `codify server stop`, or the known
  ...
  it("does not start the install path when no update is downloaded", async (t) => {
    ...
    await assert.rejects(
      harness.ipcHandlers.get("codify:update-install")(harness.events.pinned),
  ...
  it("surfaces manual check failures without changing the status union", async (t) => {
    ...
    await assert.rejects(
      harness.ipcHandlers.get("codify:update-check")(harness.events.pinned),
      /latest\.yml/,
    );
  ...
  it("blocks manual update paths when the updater feed is unavailable in development", async (t) => {
    ...
    await assert.rejects(
      harness.ipcHandlers.get("codify:update-download")(harness.events.pinned),
      /unavailable in development/,
    );
    await assert.rejects(
      harness.ipcHandlers.get("codify:update-install")(harness.events.pinned),
    ...
    await assert.rejects(
      harness.ipcHandlers.get("codify:set-update-config")(harness.events.pinned, {
        mode: "manual",
      }),
      /unavailable in development/,
    );
