  ...
  it("invokes registerWorkspaceChromeHide(win.webContents) as live code", () => {
    assert.match(
      ...
      [
        ...
        "can navigate out of Codify into other workspace apps. Re-add the call (the wiring",
  ...
  it("does not gate the wiring behind a URL/path check", () => {
    assert.doesNotMatch(
      ...
      [
        ...
        "switcher visible. The CSS targets .codify-app (workspace-embedded build only), so",
...
// (/c/<id>) lives UNDER the server's workspace mount (/ml/codifys), so it
...
describe("deep-link path join wiring (src/main.js)", () => {
  it("joins opts.path onto opts.serverUrl via resolveServerPath as live code", () => {
    assert.match(
      ...
      [
        ...
        "resolveServerPath. A deep link to a workspace server (origin + /ml/codifys",
  ...
  it("stores the clean serverUrl (no conversation path) separately from loadUrl", () => {
    // The window's server IDENTITY (for `codify host --server` etc.) must not
    ...
    assert.match(
      ...
      [
        ...
        "identity — it must keep the /c/<id> path out of `codify host --server`.",
...
describe("deep-link ingestion wiring (src/main.js)", () => {
  ...
  it("scans second-instance argv for codify:// and enqueues as live code", () => {
    assert.match(
      liveCode,
      /app\.on\("second-instance"[\s\S]{0,220}startsWith\("codify:\/\/"\)[\s\S]{0,60}enqueueDeepLink\(/,
      [
        "main.js no longer scans second-instance argv for codify://. Windows/Linux",
  ...
  it("registers the codify:// scheme as live code", () => {
    assert.match(
      liveCode,
      /setAsDefaultProtocolClient\("codify"\)/,
      [
        "main.js no longer calls app.setAsDefaultProtocolClient('codify'), so dev",
        "(`electron .`) clicks on an codify:// link won't route to the running dev",
  ...
  it("routes in-place navigation through the codify:open-path channel", () => {
    assert.match(
      liveCode,
      /send\("codify:open-path"/,
      [
        "main.js no longer sends codify:open-path to the SPA, so reuse-inplace deep",
        "links would focus a window without navigating it. Restore sendOpenPath's",
        "webContents.send('codify:open-path', path).",
      ].join(" "),
    );
  });
