// Tests for the pure helpers in src/codify_cli.js, run with `node --test`
...
// connected to server X?" and "which codify binary do we run?".
...
describe("normalizeServerUrl", () => {
  it("strips trailing slashes and trims", () => {
    ...
    assert.equal(normalizeServerUrl("https://x.com/ml/codifys"), "https://x.com/ml/codifys");
  });
...
describe("candidatePaths", () => {
  it("probes both the codify name and the omni alias in each location", () => {
    const paths = candidatePaths();
    // Every well-known dir contributes an `codify` and an `omni` entry.
    assert.ok(paths.some((p) => p.endsWith("/.local/bin/codify")));
    assert.ok(paths.some((p) => p.endsWith("/.local/bin/omni")));
    assert.ok(paths.includes("/opt/homebrew/bin/codify"));
    assert.ok(paths.includes("/opt/homebrew/bin/omni"));
    assert.ok(paths.includes("/usr/local/bin/omni"));
  });

  it("lists the canonical codify name before the omni alias within a dir", () => {
    const paths = candidatePaths();
    const og = paths.indexOf("/opt/homebrew/bin/codify");
...
describe("resolveCliPath", () => {
  it("resolves the omni alias when only it is executable", () => {
    const got = resolveCliPath(null, {
      ...
      whichCodify: () => null,
      candidatePaths: () => ["/home/me/.local/bin/codify", "/home/me/.local/bin/omni"],
    });
    assert.deepEqual(got, { path: "/home/me/.local/bin/omni", source: "candidate" });
  });

  it("prefers a usable configured path", () => {
    const got = resolveCliPath("/custom/codify", {
      isExecutableFile: (p) => p === "/custom/codify",
      whichCodify: () => "/usr/bin/codify",
      candidatePaths: () => ["/home/me/.local/bin/codify"],
    });
    assert.deepEqual(got, { path: "/custom/codify", source: "configured" });
  });

  it("falls back to PATH when the configured path is unusable", () => {
    const got = resolveCliPath("/bad/path", {
      isExecutableFile: (p) => p === "/usr/bin/codify",
      whichCodify: () => "/usr/bin/codify",
      candidatePaths: () => ["/home/me/.local/bin/codify"],
    });
    assert.deepEqual(got, { path: "/usr/bin/codify", source: "path" });
  });

  it("falls back to a candidate when PATH misses (GUI minimal PATH)", () => {
    const got = resolveCliPath(null, {
      isExecutableFile: (p) => p === "/home/me/.local/bin/codify",
      whichCodify: () => null,
      candidatePaths: () => ["/home/me/.local/bin/codify", "/opt/homebrew/bin/codify"],
    });
    assert.deepEqual(got, { path: "/home/me/.local/bin/codify", source: "candidate" });
  });
...
describe("getHostConnectionFast — probe destination & token handling (S1)", () => {
  afterEach(() => {
    mock.restoreAll();
    delete process.env.CODIFY_REMOTE_AUTH_TOKEN;
  });
  ...
  it("never attaches CODIFY_REMOTE_AUTH_TOKEN to a probe", async () => {
    ...
    process.env.CODIFY_REMOTE_AUTH_TOKEN = "secret-token";
    const record = {
      pid: process.pid,
      target: "http://localhost:6767",
      mode: "server",
