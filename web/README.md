...
The web UI for `codify server --agent <agent>`. SPA built with Vite + React + TypeScript +
Tailwind v4 + shadcn/ui. Talks to the current Codify API surface
(`/v1/agents`, `/v1/sessions`, session-scoped
`/v1/sessions/{id}/resources/files`).

## Develop

In one terminal, start the codify server (default port `6767`). Use
`--agent` to pre-register one or more agents at startup (accepts a YAML file or
an agent-image directory; can be repeated):

```bash
.venv/bin/codify server --agent examples/hello_world.yaml
```
...
`CODIFY_URL` to override the proxy target:

```bash
CODIFY_URL=http://localhost:9000 pnpm run dev
```

Additional `codify server` options:
...
## Build + serve from the Codify server
...
Vite writes the bundle to `../codify/server/static/web-ui/` (configured in
`vite.config.ts`). When that directory exists and contains `index.html`, the
FastAPI app in `codify/server/app.py` mounts it at `/`. After a build:
...
`sdks/python-client/codify_client/_stream.py`. Same for:

| TS file                       | Mirrors                                     |
| ----------------------------- | ------------------------------------------- |
| `src/lib/blocks.ts`           | `codify_client/_blocks.py`                  |
| `src/lib/events.ts`           | `codify_client/_events.py`                  |
| `src/lib/types.ts`            | minimal subset of `codify_client/_types.py` |
| `src/lib/sse.ts`              | `codify_client/_sse.py`                     |
| `src/lib/blockStream.ts`      | `codify_client/_stream.py`                  |
