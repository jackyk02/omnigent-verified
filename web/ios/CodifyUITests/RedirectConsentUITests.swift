...
/// lives behind), driven in the simulator via the DEBUG-only `--codify-open-url`
...
///
/// What this test does verify end-to-end: an `codify://` deep link to an
...
final class RedirectConsentUITests: XCTestCase {
  func testDeepLinkConsentOpensApprovedServer() throws {
    let marker = "CODIFY_DEEPLINK_CONSENT_OK"
    ...
    let deepLink = "codify://localhost:\(server.port)/c/conv_smoke"

    let app = XCUIApplication(bundleIdentifier: "ai.codify.ios")
    app.launchArguments += [
      "--codify-open-url", deepLink,
      "--codify-reset-state",
    ]
    app.launch()
