...
@MainActor
final class CodifyUITests: XCTestCase {
  override func setUpWithError() throws {
    continueAfterFailure = false
  }

  func testLocalServerSnapshot() throws {
    let app = XCUIApplication(bundleIdentifier: "ai.codify.ios")
    setupSnapshot(app)
    let serverURL = try XCTUnwrap(
      ScreenshotConfiguration.serverURL(from: app),
      "Pass --codify-server-url or CODIFY_SCREENSHOT_APP_URL for screenshot tests."
    )
    app.launchArguments += [
      "--codify-server-url",
      serverURL,
    ]
    NSLog("Codify screenshot server URL: \(serverURL)")
    app.launchEnvironment["CODIFY_SCREENSHOT_APP_URL"] = serverURL
    app.launch()

    XCTAssertTrue(
      app.staticTexts["Server URL"].waitForExistence(timeout: 15),
      "Expected Codify to show the server selection screen before connecting."
    )
    ...
    XCTAssertTrue(
      ...
      "Expected Codify to connect to \(serverURL) before taking screenshots."
    )
  ...
  /// DEBUG-only `--codify-open-url` launch argument (see `AppRootView`). The
  ...
  ///
  /// `--codify-reset-state` (also DEBUG-only) wipes persisted server
  ...
  private func launchApp(openURL link: String) -> XCUIApplication {
    let app = XCUIApplication(bundleIdentifier: "ai.codify.ios")
    app.launchArguments += ["--codify-open-url", link, "--codify-reset-state"]
    app.launch()
    return app
  }

  /// A valid `codify://` deep link to a server the user has never connected
  ...
  func testValidDeepLinkShowsConsent() throws {
    ...
    let app = launchApp(
      openURL: "codify://localhost:8000/c/e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9")

    let alert = app.alerts["Open this Codify link?"]
    XCTAssertTrue(
  ...
  func testSmuggledQueryDeepLinkIsRejected() throws {
    ...
    let app = launchApp(
      openURL:
        "codify://localhost:8000/c/e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9%3Fview=terminal")
  ...
  func testSmuggledFragmentDeepLinkIsRejected() throws {
    let app = launchApp(
      openURL: "codify://localhost:8000/c/e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9%23evil")
    assertAppReachedSetup(app)
  ...
  func testBenignNonCanonicalIdIsAccepted() throws {
    ...
    let app = launchApp(openURL: "codify://localhost:8000/c/conv_abc")
  ...
  func testEncodedDotDotDeepLinkIsRejected() throws {
    let app = launchApp(openURL: "codify://localhost:8000/c/%2e%2e")
    assertAppReachedSetup(app)
  ...
  func testControlCharDeepLinkIsRejected() throws {
    let app = launchApp(
      openURL: "codify://localhost:8000/c/e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9%00x")
    assertAppReachedSetup(app)
...
private enum ScreenshotConfiguration {
  static func serverURL(from app: XCUIApplication) -> String? {
    ProcessInfo.processInfo.environment["CODIFY_SCREENSHOT_APP_URL"]?.nonEmpty
      ?? app.launchArguments.codifyServerURL
      ?? fastlaneLaunchArguments().codifyServerURL
  }
  ...
  private static func fastlaneLaunchArguments() -> [String] {
    ...
    return contents.codifyShellTokens
  }
...
extension [String] {
  fileprivate var codifyServerURL: String? {
    codifyArgumentValue(after: "--codify-server-url")
      ?? compactMap { argument -> String? in
        guard argument.hasPrefix("--codify-server-url=") else { return nil }
        return String(argument.dropFirst("--codify-server-url=".count)).nonEmpty
      }.first
      ?? firstWebURL
  }
  ...
  fileprivate func codifyArgumentValue(after argumentName: String) -> String? {
...
extension String {
  ...
  fileprivate var codifyShellTokens: [String] {
    guard
