...
3. **Create the app record** in [App Store Connect](https://appstoreconnect.apple.com)
   for bundle ID `ai.codify.ios` (My Apps → +), if it doesn't exist yet.
...
- **Marketing version** (`CFBundleShortVersionString`, currently `0.1.0`) is set
  ...
  configurations of the **Codify** target in Xcode (or via `fastlane
...
`CODIFY_PROD_BUILD_NUMBER=4` to pin a specific processed build.

## Other commands

- `bundle exec fastlane tests` — run the `CodifyTests` unit suite.
- `bundle exec fastlane screenshots` — build and capture App Store screenshots
  into `fastlane/screenshots` using the `CodifyUITests` snapshot target. The
  lane rebuilds the web UI, starts an isolated local Codify server via `uv` on
