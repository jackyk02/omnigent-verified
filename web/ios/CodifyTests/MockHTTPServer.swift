...
///
/// Shared between the integration tests (`CodifyTests`) and the UI tests
/// (`CodifyUITests`) — compiled into both test targets so neither needs its
...
final class MockHTTPServer {
  ...
  private let queue = DispatchQueue(label: "codify.mock-http-server")
