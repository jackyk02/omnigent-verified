...
final class NativeNotificationManager: NSObject, UNUserNotificationCenterDelegate {
  ...
  func setBadgeCount(_ count: Int) {
    Task {
      ...
      do {
        ...
        NSLog("[codify] failed to set badge count: \(String(describing: error))")
      }
    }
  }
  ...
  func notify(title: String, body: String?, navigatePath: String?) {
    Task {
      ...
      let request = UNNotificationRequest(
        identifier: "codify.\(UUID().uuidString)",
        content: content,
        trigger: nil
      )
      ...
      do {
        ...
        NSLog("[codify] failed to add notification: \(String(describing: error))")
      }
    }
  }
  ...
  private func requestAuthorizationIfNeeded() async -> Bool {
    #if DEBUG
      guard !ProcessInfo.processInfo.isCodifyScreenshotRun else { return false }
    #endif
  ...
  extension ProcessInfo {
    fileprivate var isCodifyScreenshotRun: Bool {
      environment["CODIFY_SCREENSHOT_APP_URL"] != nil
