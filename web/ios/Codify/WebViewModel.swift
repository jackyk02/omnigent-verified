...
final class WebViewModel: ObservableObject {
  ...
  func emitNotificationActivation(_ path: String) {
    ...
    let script =
      "window.__codifyNativeEmitNotificationActivated?.(\(Self.javascriptString(path)));"
  ...
  func emitOpenPath(_ path: String) {
    guard path.starts(with: "/") else { return }
    let script = "window.__codifyNativeEmitOpenPath?.(\(Self.javascriptString(path)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  /// web side folds these into its `--codify-inset-*` variables so page
  ...
  func emitInsets(topBar: CGFloat, bottomBar: CGFloat) {
    let script =
      "window.__codifyNativeEmitInsets?.(\(jsNumber(topBar)), \(jsNumber(bottomBar)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitViewModeChanged(_ mode: WebViewMode) {
    let script =
      "window.__codifyNativeEmitViewModeChanged?.(\(Self.javascriptString(mode.rawValue)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitSidebarDrag(phase: String, progress: Double) {
    ...
    let script =
      "window.__codifyNativeEmitSidebarDrag?.(\(Self.javascriptString(phase)), \(clamped));"
    webView?.evaluateJavaScript(script)
  }
