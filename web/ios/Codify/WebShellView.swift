  ...
  var body: some View {
    GeometryReader { geometry in
      ...
      .overlay(alignment: .bottom) {
        ...
        // layer reserves a fixed footprint for it (`.codify-native-bottom-
  ...
  var body: some View {
    Menu {
      Button {
        ...
        Label(currentURL.codifyHostLabel, systemImage: "checkmark")
      }
      .disabled(true)

      let otherServers = recents.filter {
        URL(string: $0)?.codifyOrigin != currentURL.codifyOrigin
      }
      if !otherServers.isEmpty {
        ...
        ForEach(otherServers, id: \.self) { recent in
          Button {
            ...
            Text(URL(string: recent)?.codifyHostLabel ?? recent)
          }
        }
      }
      ...
      HStack(spacing: 6) {
        Text(currentURL.codifyHostLabel)
          .fontWeight(.medium)
          .lineLimit(1)
