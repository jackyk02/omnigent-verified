...
final class SettingsStore: ObservableObject {
  ...
  init(defaults: UserDefaults = .standard) {
    self.defaults = defaults
    #if DEBUG
      // UI tests pass `--codify-reset-state` to start each case with NO
      ...
      if ProcessInfo.processInfo.arguments.contains("--codify-reset-state") {
        for key in [Keys.serverURL, Keys.recentServers, Keys.allowedProtocols] {
          defaults.removeObject(forKey: key)
        }
      }
      serverURL =
        ProcessInfo.processInfo.codifyArgumentValue(after: "--codify-server-url")
        ?? ProcessInfo.processInfo.environment["CODIFY_SCREENSHOT_APP_URL"]
  ...
  func knownServerURL(forOrigin origin: String) -> URL? {
    ...
    for value in candidates {
      guard let url = URL(string: value), url.codifyOrigin == origin else { continue }
      return url
    }
    return nil
  }
  ...
  private enum Keys {
    static let serverURL = "codify.serverURL"
    static let recentServers = "codify.recentServers"
    static let allowedProtocols = "codify.allowedProtocols"
  }
}
