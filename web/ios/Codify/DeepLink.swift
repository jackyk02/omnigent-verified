...
/// A parsed `codify://<hostname>/c/<session_id>` deep link — the iOS analog
/// of the desktop shell's `parseCodifyDeepLink` (web/electron/src/deepLink.js),
...
/// URL never disagree on scheme. The Databricks workspace mount (`/ml/codifys`)
...
///
/// `codify://` is a **custom URL scheme** registered via `CFBundleURLSchemes`
/// in the Info plist. iOS does NOT verify single ownership of a custom scheme:
/// any other installed app can also declare `codify` and win the launch race,
...
/// the server host and conversation id from a clicked `codify://` link. The
...
struct DeepLink: Equatable {
  ...
  /// `URL.codifyOrigin`, which returns nil for IPv6 hosts) so IPv6 loopback
  ...
  /// Parse an `codify://` URL. Returns nil for anything that isn't a valid
  /// `codify://<host>/c/<id>` link (wrong scheme, no host, non-`/c/` path,
  ...
  static func parse(_ raw: URL) -> DeepLink? {
    guard raw.scheme?.lowercased() == "codify" else { return nil }
    guard let host = raw.host, !host.isEmpty else { return nil }

    // v1 accepts only `/c/<id>`. CRUCIALLY, `URL.path` returns the
    // PERCENT-DECODED path, so a link like `codify://host/c/id%3Fview=terminal`
