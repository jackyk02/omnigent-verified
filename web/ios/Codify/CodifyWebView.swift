...
struct CodifyWebView: UIViewRepresentable {
  let initialURL: URL
  ...
  func makeUIView(context: Context) -> WKWebView {
    ...
    contentController.add(context.coordinator, name: "codifyNative")
  ...
  static func dismantleUIView(_ uiView: WKWebView, coordinator: Coordinator) {
    uiView.configuration.userContentController.removeScriptMessageHandler(forName: "codifyNative")
    coordinator.detach()
  }

  private static let nativeBridgeScript = """
    (() => {
      if (window.codifyNative && window.codifyNative.kind === "ios") return;
      ...
      defineEmit("__codifyNativeEmitNotificationActivated", (path) => {
        if (typeof path !== "string" || !path.startsWith("/")) return;
        for (const callback of callbacks) {
          try { callback(path); } catch {}
        }
      });
      defineEmit("__codifyNativeEmitOpenPath", (path) => {
        if (typeof path !== "string" || !path.startsWith("/")) return;
        for (const callback of openPathCallbacks) {
          try { callback(path); } catch {}
        }
      });
      defineEmit("__codifyNativeEmitViewModeChanged", (mode) => {
      ...
      let lastInsets = null;
      defineEmit("__codifyNativeEmitInsets", (topBar, bottomBar) => {
        const insets = {
      ...
      Object.defineProperty(window, "__codifyNativeEmitSidebarDrag", {
        configurable: false,
        enumerable: false,
        writable: false,
      ...
      window.codifyNative = Object.freeze({
        kind: "ios",
        setColorScheme(scheme) {
          if (scheme !== "light" && scheme !== "dark" && scheme !== "system") return;
          window.webkit.messageHandlers.codifyNative.postMessage({
    ...
    UIGestureRecognizerDelegate
  {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
      ...
      if webView.url?.codifyOrigin == pinnedOrigin, let pinnedURL {
    ...
    func webView(
      ...
      guard Self.isAllowedMediaCaptureType(type),
        origin.codifyOrigin == pinnedOrigin,
        webView.url?.codifyOrigin == pinnedOrigin
      else {
    ...
    private func isTrustedBridgeMessage(_ message: WKScriptMessage) -> Bool {
      ...
      guard message.frameInfo.securityOrigin.codifyOrigin == pinnedOrigin else { return false }
      guard webView?.url?.codifyOrigin == pinnedOrigin else { return false }
    ...
    private func promptForExternalURL(_ url: URL, scheme: String) {
      let onPinnedServer = pinnedOrigin != nil && webView?.url?.codifyOrigin == pinnedOrigin
      ...
      let requester = webView?.url?.codifyOrigin ?? "This page"
    ...
    private func handleLoadFailure(_ webView: WKWebView, error: Error) {
      ...
      guard failedURL.codifyOrigin == pinnedOrigin else { return }
    ...
    private func injectWorkspaceChromeCSS(_ webView: WKWebView) {
      ...
      let script = """
        (() => {
          if (document.querySelector("style[data-codify-workspace-chrome]")) return;
          const style = document.createElement("style");
          style.dataset.codifyWorkspaceChrome = "true";
    ...
    private func topViewController() -> UIViewController? {
      ...
      return root?.codifyTopViewController
    }
  }
}
...
extension UIViewController {
  fileprivate var codifyTopViewController: UIViewController {
    if let presentedViewController {
      return presentedViewController.codifyTopViewController
    }
    if let navigation = self as? UINavigationController,
      let visible = navigation.visibleViewController
    {
      return visible.codifyTopViewController
    }
    if let tab = self as? UITabBarController,
      let selected = tab.selectedViewController
    {
      return selected.codifyTopViewController
    }
    return self
  }
}
