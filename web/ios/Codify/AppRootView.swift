...
struct AppRootView: View {
  ...
  var body: some View {
      ...
      case .web(let serverURL, let path):
        WebShellView(
          ...
          loadFailed: { failedURL, message in
            mode = .setup(
              prefill: failedURL.codifyOrigin ?? failedURL.absoluteString, error: message)
          },
    ...
    // raw link (e.g. `codify://host/c/<id>`), exactly what the system would
    // hand `onOpenURL`.
    #if DEBUG
      .task {
        guard let url = ProcessInfo.processInfo.codifyOpenURLArgument else { return }
    ...
    .alert(
      ...
      if let consentDeepLink {
        Text(
          """
          This link will connect Codify to \(consentDeepLink.origin.hostLabel) and open a conversation.
  ...
  private var shouldAutoOpenSavedServer: Bool {
      ...
      return processInfo.environment["CODIFY_SCREENSHOT_APP_URL"] == nil
  ...
  extension ProcessInfo {
    /// The `--codify-open-url <url>` DEBUG-only launch argument, used by UI
    /// tests to drive a deep link through the real handler (see the `.task` seam
    /// in `AppRootView`). Nil if absent or unparseable.
    fileprivate var codifyOpenURLArgument: URL? {
      guard let index = arguments.firstIndex(of: "--codify-open-url"),
...
extension AppRootView {
  /// Handle an `codify://` URL the system routed to the app. The window
  ...
  private func handleDeepLink(_ url: URL) {
    guard let deepLink = DeepLink.parse(url) else {
      #if DEBUG
        if ProcessInfo.processInfo.environment["CODIFY_DEEPLINK_TRACE"] != nil {
  ...
  /// unchanged by the probe (it only appends `/ml/codifys` under it), so the
  ...
  private var currentWebOrigin: String? {
    if case .web(let serverURL, _) = mode { return serverURL.codifyOrigin }
    return nil
  }

  /// Join a basename-less SPA path (`/c/<id>`) onto a server URL that may carry
  /// a workspace mount (`/ml/codifys`). The path lives UNDER the mount, so it
...
extension String {
  ...
  fileprivate var hostLabel: String {
    URL(string: self)?.codifyHostLabel ?? self
  }
}
