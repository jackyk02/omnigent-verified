...
Thin SwiftUI/WKWebView shell for Codify. Like the Electron app, this target
loads the server-served web UI instead of shipping a duplicate copy of the SPA.

## Development

Open `Codify.xcodeproj` in Xcode 16 or newer and run the `Codify` scheme on
an iOS 18 simulator.
...
An `codify://<hostname>/c/<session_id>` URL opens that session on that server
...
URL never disagree. The Databricks workspace mount (`/ml/codifys`) is **not**
...
test from the simulator with `xcrun simctl openurl booted 'codify://...'`.
