...
Thanks for your interest in improving Codify. Issues and pull requests are
...
```bash
git clone https://github.com/codify-ai/codify.git
cd codify
...
so it can run alongside your normal Codify installation and other worktrees.
...
```bash
cd /path/to/codify-worktree
uv sync --extra all --extra dev
omnidev
```

Open the exact `ui` URL displayed in the header; do not assume the Vite port is
`5173`. Python changes under `codify/` reload the server and host, while
...
omnidev codify agent list
```

Keep `omnidev` in the foreground and quit with `q` or `Ctrl-C` so it tears down
all three processes. An interactive terminal inside an existing Codify
...
uv run codify server

# Terminal 2: register your machine as a host
uv run codify host --server http://localhost:6767
...
`omni` is an alias for `codify`, so `omni host --server ...` works too.
The host URL can also be passed positionally (`codify host
...
starts the server in API-only mode (`CODIFY_SKIP_WEB_UI=true`), waits for
...
- **Fully isolated, disposable:** every artifact -- the toolchain and project
  ...
  the run never touches your real `~/.codify`, `~/.config` / `~/Library`, or
  ...
  / `CODIFY_*` overrides pin the toolchain and app state regardless of OS,
...
A change that alters behaviour under `codify/` should ship with a test, and a
...
| Area changed (`codify/…`) | Test suite (`tests/…`) |
| --- | --- |
| `server/` | `server/` |
| `runner/` | `runner/` |
| `runtime/` | `runtime/` |
