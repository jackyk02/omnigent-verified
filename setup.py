"""Custom setuptools build for codify.

Generates ``codify/_build_info.py`` at wheel build time so the
CLI's update-check (``codify/update_check.py``) can tell the user
...
class _GenerateBuildInfo(build_py):
    ...
    def _bundle_scripts(self) -> None:
        ...
        if not src.is_file():
            return
        dest = Path(self.build_lib) / "codify" / "resources" / "scripts" / src.name
    ...
    def _bundle_examples(self) -> None:
        ...
        ``codify/resources/examples/{polly,debby}`` may exist as symlinks
        ...
        package whose ``codify.resources.examples`` has no ``polly`` /
        ``debby`` subdir, and bare ``codify`` (first-run default → polly)
        ...
        checkout) into
        ``build_lib/codify/resources/examples/<name>`` so every wheel is
        ...
        root = Path(__file__).resolve().parent
        dest_root = Path(self.build_lib) / "codify" / "resources" / "examples"
    ...
    def _build_web_ui(self) -> None:
        ...
        (``codify/server/app.py``); when absent it serves an
        ...
        - Skip if ``CODIFY_SKIP_WEB_UI=true``. The hardened CI
        ...
        - Skip if the bundle already exists, UNLESS
          ``CODIFY_BUILD_WEB_UI=1`` forces a rebuild. This keeps
        ...
        - Otherwise the build MUST succeed: a missing Node.js 22+,
          ...
          actionable error. Codify needs Node 22 LTS + pnpm at
        ...
        root = Path(__file__).resolve().parent
        web_src = root / "web"
        bundle = root / "codify" / "server" / "static" / "web-ui" / "index.html"

        if not (web_src / "package.json").is_file():
            return
        # CI opt-out: exact "true" only — this is set by our own
        # workflows, not user-facing config.
        if os.environ.get("CODIFY_SKIP_WEB_UI") == "true":
            return
        force_raw = os.environ.get("CODIFY_BUILD_WEB_UI")
        ...
        if pnpm_cmd is None:
            raise SystemExit(
                "codify build: pnpm not found on PATH, so the web UI "
                "cannot be built. Codify requires Node.js 22 LTS or "
        ...
        try:
            ...
            raise SystemExit(
                f"codify build: web UI build failed ({exc}). Fix the "
                ...
                "server), set CODIFY_SKIP_WEB_UI=true."
            ) from exc

    def _write_build_info(self) -> None:
        """Write ``codify/_build_info.py`` into the source tree.
        ...
        any later non-build code path that does ``from codify
        import _build_info`` works without re-running the build.
        """
        target = Path(__file__).resolve().parent / "codify" / "_build_info.py"
        commit = _git_sha()
        ...
        target.write_text(
            ...
            "defensively (``try: from codify import _build_info``)\n"
...
def _require_node_22() -> None:
    ...
    if node is None:
        raise SystemExit(
            "codify build: Node.js not found on PATH, so the web UI "
            "cannot be built. Codify requires Node.js 22 LTS or newer "
    ...
    try:
        ...
        raise SystemExit(
            f"codify build: could not determine the Node.js version "
            f"(`node --version` failed: {exc}). Codify requires "
    ...
    if major < 22:
        raise SystemExit(
            f"codify build: Node.js {version_str or 'unknown'} is "
            f"installed, but Codify requires Node.js 22 LTS or newer "
