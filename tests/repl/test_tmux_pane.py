...
codify context source via custom pane options and (b) wrap the
user's prefix-table ``split-window`` / ``new-window`` bindings with
``if-shell -F`` conditionals that route to ``codify pane-split``
when the focused pane has ``@codify-conv-id`` set, and otherwise
...
def test_classify_returns_none_for_non_split_commands(tokens: list[str]) -> None:
    ...
    rewrite them, breaking the user's exotic setups in non-codify
    panes too.
    """
...
def test_wrap_binding_constructs_if_shell_wrapper(
    ...
    either non-codify panes break (lost original) or
    codify panes never see the chooser.
    ...
    _wrap_binding(binding, ["/venv/bin/codify"])
    ...
    assert cmd[5:8] == ["if-shell", "-F", "#{?#{@codify-conv-id},1,0}"], (
        f"if-shell -F format-string regressed; got {cmd[5:8]!r}. The "
        f"format must check truthiness of the @codify-conv-id pane "
    ...
    # ``codify_bin`` path (tmux's run-shell inherits the
    ...
    assert cmd[8] == ("run-shell '/venv/bin/codify pane-split -v -p #{pane_id}'"), (
    ...
    assert cmd[9] == 'split-window -c "#{pane_current_path}"', (
        ...
        f"binding must be preserved exactly so non-codify panes get "
...
def test_wrap_binding_uses_horizontal_direction_code(
    ...
    assert captured[0][8] == ("run-shell '/venv/bin/codify pane-split -h -p #{pane_id}'")
...
def test_wrap_binding_uses_window_direction_code(
    ...
    assert captured[0][8] == ("run-shell '/venv/bin/codify pane-split -w -p #{pane_id}'")
...
def test_wrap_binding_quotes_paths_with_spaces(
    ...
    """
    A venv path like ``/Users/me/My Project/.venv/bin/codify``
    ...
    ``run-shell '/Users/me/My Project/.venv/bin/codify pane-split …'``
    ...
    _wrap_binding(
        ...
        ["/Users/me/My Project/.venv/bin/codify"],
    )
    ...
    assert inner_argv[0] == "/Users/me/My Project/.venv/bin/codify", (
        ...
        f"fires ``cd: /Users/me/My`` instead of running the codify "
        f"binary."
    )
...
def test_wrap_binding_supports_python_m_fallback_argv(
    ...
    """
    When :func:`_resolve_codify_argv` falls back to
    ``[sys.executable, "-m", "codify.cli"]`` (no resolvable
    ...
    assert "/usr/bin/python3 -m codify.cli pane-split -v" in chooser, (
        f"python-m fallback prefix not propagated; got chooser={chooser!r}. "
        f"If ``-m codify.cli`` is missing, the wrapper would invoke "
...
def test_resolve_argv_uses_abspath_when_argv0_is_path_shaped(
    ...
    ``./.venv/bin/codify`` or already absolute), the resolver
    ...
    monkeypatch.setattr("sys.argv", ["/some/abs/path/codify", "run"])
    argv = _resolve_codify_argv()
    assert argv == ["/some/abs/path/codify"], (
...
def test_resolve_argv_uses_which_when_argv0_is_bare_name(
    ...
    """
    When ``sys.argv[0]`` is a bare name like ``"codify"`` (the
    ...
    monkeypatch.setattr("sys.argv", ["codify", "run"])
    monkeypatch.setattr(shutil, "which", lambda name: "/resolved/bin/codify")
    argv = _resolve_codify_argv()
    assert argv == ["/resolved/bin/codify"]
...
def test_resolve_argv_falls_back_to_python_m_when_which_misses(
    ...
    "codify.cli"]`` — bulletproof because if Python is
    running this code, ``codify.cli`` is importable.
    ...
    argv = _resolve_codify_argv()
    assert argv == ["/path/to/python", "-m", "codify.cli"], (
        f"python-m fallback regressed; got {argv!r}. The fallback is "
        f"the only path that works in environments where the codify "
...
def test_discover_unwraps_existing_wrapper_to_recover_original(
    ...
    fake_output = (
        ...
        '"#{?#{@codify-conv-id},1,0}" '
        "\"run-shell 'codify pane-split -v -p #{pane_id}'\" "
    ...
    from codify.repl._tmux_pane import _discover_split_bindings
...
def test_register_pane_strips_existing_python_m_prefix_idempotently(
    ...
    # ALREADY in [python, -m, codify.cli, run, ...] form —
    # what a pane-picker-launched REPL would see at boot.
    register_pane(
        conv_id="conv_x",
        agent_name="a",
        agent_yaml=None,
        launch_argv=["/p/python", "-m", "codify.cli", "run", "/x.yaml", "--codify"],
        server_url=None,
    )
    ...
    assert stored_argv == [
        ...
        f"``-m codify.cli`` would crash with a Python module-import "
...
def test_register_pane_repairs_already_doubled_prefix(
    ...
    ``[python, -m, codify.cli, -m, codify.cli, run, ...]``
    — left there by an earlier register_pane that stripped only
    one ``-m codify.cli`` prefix and re-prepended a fresh
...
def test_discover_split_bindings_picks_user_custom_keys(
    ...
    so muscle memory works inside the codify pane.
...
def _pane_integration_enabled(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    """
    monkeypatch.setattr("codify.repl._tmux_pane.PANE_INTEGRATION_ENABLED", True)
    yield
...
def test_register_pane_unmarks_pane_when_kill_switch_disabled_inside_tmux(
    ...
    <opt>`` invocation per ``@codify-*`` option, targeting the
    ...
    pressing the split key in an codify pane stops opening
    ...
    # Every captured call must be an unset of an ``@codify-*``
    ...
    for cmd in captured:
        assert cmd[:5] == ["tmux", "set-option", "-p", "-t", "%5"], (
            ...
            f"only thing this branch should do is unset @codify-* "
    ...
    assert unset_options == {
        ...
        f"kill-switch must unset all 5 @codify-* options; got "
...
def test_register_pane_no_op_outside_tmux(
    ...
    register_pane(
        ...
        launch_argv=["codify", "run", "/tmp/x.yaml"],
...
def test_register_pane_advertises_options_and_wraps_bindings(
    ...
    Claim: ``@codify-launch-argv`` is JSON, all five options
    ...
    monkeypatch.setattr("sys.argv", ["codify", "run", "/agents/cs.yaml", "--codify"])
    monkeypatch.setattr(shutil, "which", lambda name: "/venv/bin/codify")
    ...
    register_pane(
        ...
        launch_argv=["codify", "run", "/agents/cs.yaml", "--codify"],
