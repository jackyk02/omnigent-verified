"""
Tests for :mod:`codify.repl._resume_picker` — the
...
from codify.entities import ConversationItem, MessageData
from codify.repl._resume_picker import (
...
def test_runtime_badge_claude_native() -> None:
    ...
    """
    from codify.repl._resume_picker import _runtime_badge

    row = _BadgeRow(labels={"codify.wrapper": "claude-code-native-ui"})
...
def test_runtime_badge_codex_native() -> None:
    ...
    row = _BadgeRow(labels={"codify.wrapper": "codex-native-ui"})
...
class _FakeSessionsNamespace:
    """Stub mimicking :class:`codify_client.SessionsNamespace`.
...
class _FakeConversationsNamespace:
    """Stub mimicking :class:`codify_client.ConversationsNamespace`.
...
class _FakeAPClient:
    """Stub :class:`codify_client.CodifyClient` exposing
...
async def test_wrapper_label_picker_filters_and_lists_without_agent_filter(
    ...
    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "state"))
    rows = [
        _BadgeRow(
...
def test_render_workspace_cell_no_state_returns_none(
    ...
    """
    from codify.repl._resume_picker import _render_workspace_cell

    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "state"))
    row = _BadgeRow(id="fdae2ccf4f08f386de6f9dabb02ddf22", labels={"codify.wrapper": "x"})
...
def test_workspace_metadata_appears_in_wrapper_picker_list(
    ...
    """
    from codify.claude_native_state import write_launch_state
    from codify.repl._resume_picker import pick_conversation
...
def test_render_workspace_cell_codex_native_uses_codex_state(
    ...
    monkeypatch.setenv("CODIFY_CODEX_NATIVE_STATE_DIR", str(tmp_path / "codex-state"))
    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "claude-state"))
