"""Tests for codify.repl._repl helpers.
...
def test_parse_sub_agent_handle_returns_raw_handle_dict() -> None:
    """
    Native codify builtins persist the spawn output as a raw
...
class _CapturingHost:
    """
    Minimal :class:`codify_ui_sdk.TerminalHost`-shaped stub that
...
def test_render_history_item_assistant_message_empty_body_is_silently_skipped() -> None:
    ...
    codify workflow alongside every real reply. Rendering them
...
def test_parse_terminal_tool_output_handles_direct_dict() -> None:
    """
    Direct-JSON shape (default executor / codify builtins)
...
def test_reconstruct_terminals_records_owning_conversation() -> None:
    ...
    CODIFY_TERMINAL_BRIDGE.md design) is wired correctly. A future
...
def test_render_startup_banner_contains_agent_name() -> None:
    ...
    visible label — users on a fresh Codify session would have
    ...
    # builder wraps the agent label with it (codify/inner/
...
def test_render_startup_banner_omits_keybinding_hints() -> None:
    """
    The Codify welcome banner does NOT carry the keybinding hint row.
    ...
    for legacy_hint in ("ctrl-g debug", "ctrl-d exit"):
        assert legacy_hint not in ansi, (
            ...
            f"which doesn't correspond to an Codify binding."
        )


def test_render_startup_banner_uses_mascot_accent_color() -> None:
    """
    The Codify mode banner box border is rendered in the Codify
    ...
    assert "\x1b[38;2;244;59;166m" in ansi, (
        f"Banner missing Codify truecolor accent escape "
...
def test_render_startup_banner_fits_under_80_columns() -> None:
    """
    The Codify welcome box stays under 80 columns wide even when
    paired with the longest example agent name.

    What this proves: in an 80-column terminal (the de-facto
    minimum for a usable shell) the Codify banner does not wrap. The
...
def test_render_startup_banner_shows_remote_server_url() -> None:
    ...
    banner. A user running ``codify run`` against a freshly
...
def test_startup_header_box_includes_folder_model_and_credential() -> None:
    ...
    assert "~/codify" in plain  # working-folder row
...
def test_startup_header_shows_server_version_on_url_line() -> None:
    ...
    remote = "https://codify.example.com"
    plain = re.sub(
        r"\x1b\[[0-9;]*m",
        "",
        _render_startup_banner_ansi(
...
def test_startup_header_shows_databricks_workspace_url_not_api_mount() -> None:
    """A Databricks server shows the ``/codify`` SPA URL and NO version.

    What this proves two things for a workspace mount: (1) the header maps
    the internal ``/api/2.0/codify`` proxy mount to the recognizable
    workspace ``/codify`` URL — a regression rendering the raw
    ...
    api_mount = "https://e2-dogfood.staging.cloud.databricks.com/api/2.0/codify"
    plain = re.sub(
        r"\x1b\[[0-9;]*m",
        "",
    ...
    assert "https://e2-dogfood.staging.cloud.databricks.com/codify" in plain
    assert "/api/2.0/codify" not in plain
...
def _fake_version_client(by_path: dict[str, dict]) -> tuple[object, list[str]]:
    """Build a fake ``CodifyClient`` whose ``_http.get`` serves per-path JSON.
    ...
    class _FakeClient:
        _base_url = "https://codify.example.com"
        _http = _FakeHttp()
...
def test_fetch_server_version_parses_info(payload, expected) -> None:
    ...
    assert targets[0] == "https://codify.example.com/v1/info"
...
def test_fetch_server_version_falls_back_to_api_version() -> None:
    ...
    assert targets == [
        "https://codify.example.com/v1/info",
        "https://codify.example.com/api/version",
    ]
...
async def test_list_all_conversation_items_paginates_past_100(
    ...
    """
    from codify.repl._repl import _list_all_conversation_items
...
def test_completer_ranks_prefix_before_substring(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.repl._repl.COMMANDS", fake)
...
def test_completer_matches_name_leaf_after_namespace(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.repl._repl.COMMANDS", _FAKE_COMMANDS)
...
def test_clear_command_registered_in_help() -> None:
    ...
    from codify.repl._repl import COMMANDS
...
async def test_registered_skill_command_uses_structured_slash_command() -> None:
    ...
    from codify.repl import _repl as repl_mod
...
def test_server_event_to_sdk_event_translates_llm_error_event() -> None:
    ...
    """
    from codify_client._events import ErrorEvent as _SDKErrorEvent
...
def test_resume_hint_appends_resume_flag_to_invocation_parts() -> None:
    ...
    assert hint == (
        "codify run examples/databricks_coding_agent.yaml "
        "--server https://codify-app.databricksapps.com "
        "--profile oss "
        "--harness claude-sdk "
        "--resume conv_abc"
    )
...
def test_model_readout_own_auth_acp_harness_reports_agent_not_provider() -> None:
    """
    Own-auth ACP harnesses must not report an Codify provider credential.

    ``acp``/``acp:<slug>`` and ``goose`` spawn without any Codify provider
    ...
    """
    from codify.repl._repl import _build_model_readout_lines
...
def test_describe_active_credential_declines_own_auth_acp_harnesses() -> None:
    ...
    this fix).
    """
    from codify.onboarding.provider_config import describe_active_credential
