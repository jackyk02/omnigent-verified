...
import pytest
from codify_client import CodifyError
from codify_client._sessions import Session as SessionSnapshot
...
async def test_interactive_send_runner_unavailable_surfaces_not_hangs() -> None:
    ...
    client = _ChatClient(post_error=CodifyError("runner unavailable", code="runner_unavailable"))
    session = _make_chat_adapter(client)
    try:
        await session.view_session("conv_child", read_only=True, interactive=True)
