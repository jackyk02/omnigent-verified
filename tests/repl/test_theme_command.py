...
import pytest
from codify_ui_sdk.terminal import RichBlockFormatter, TerminalHost
from codify_ui_sdk.terminal._theme import DARK_THEME, LIGHT_THEME

from codify.repl._repl import COMMANDS, _load_startup_theme, handle_slash_command
...
def test_startup_theme_shows_picker_on_first_launch(
    ...
    monkeypatch.setattr(
        "codify.repl._theme_picker.startup_theme_picker",
...
async def test_theme_light_updates_host_and_formatter(
    ...
    assert (tmp_path / ".codify" / "config.yaml").read_text(encoding="utf-8") == (
        "# Codify user configuration\ntui:\n  theme: light\n"
    )
