"""
Tests for :mod:`codify.repl._session_log` — the JSON dump
helper that ports the legacy ``--log`` flag to Codify mode.
...
mocking CodifyClient + responses correctly is heavier than
...
from codify.repl._session_log import (
    DEFAULT_LOG_DIR,
    default_log_path,
...
def test_default_log_path_uses_default_dir_when_none() -> None:
    """
    ``log_dir=None`` resolves to ``~/.codify/logs/`` — the same
...
def test_default_log_path_strips_path_separators(tmp_path: Path) -> None:
    ...
    ``codify/inner/cli.py::_default_session_log_path``.
    """
...
def test_write_session_log_from_store_dumps_basic_conversation(
    ...
    assert payload["format"] == "codify-conversation"
...
def test_write_session_log_walks_sub_agent_children(
    ...
    assert len(children) == 1, (
        ...
        f"codify/repl/_repl.py — the format the spawn tool "
