...
from codify import claude_native, codex_native, native_terminal
from codify.claude_native_bridge import BRIDGE_ID_LABEL_KEY
from codify.host import daemon_launch
...
async def test_create_claude_session_persists_terminal_launch_args() -> None:
    ...
    ``codify claude --server X <flags>`` must carry the flags to a
...
def _install_daemon_seam_mocks(
    ...
    *,
    ...
    :returns: None.
    """
    monkeypatch.setattr("codify.chat._remote_headers", lambda server_url=None, **k: {})
    monkeypatch.setattr("codify.chat._server_auth", lambda server_url=None, **k: None)
    monkeypatch.setattr("codify.chat._bundle_agent", lambda path: b"bundle")
    monkeypatch.setattr(
        "codify.cli._ensure_host_daemon",
...
def test_run_with_remote_server_detach_prints_resume_hint(
    ...
    assert (
        "Resume with: codify claude --server https://example.com --resume conv_existing"
    ) in err
...
def test_run_with_remote_server_unreachable_server_raises_clean_error(
    ...
    """
    An unreachable Codify server fails with an actionable message, not a
    raw httpx traceback.
    ...
    assert "Could not reach the codify server at https://unreachable.example" in str(
        exc_info.value
    )
