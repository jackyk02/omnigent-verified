...
from codify import hermes_native as hn
...
def test_resolve_hermes_executable_honors_path_override() -> None:
    resolved = hn.resolve_hermes_executable(
        env={"CODIFY_HERMES_PATH": "/opt/hermes"},
...
def test_harness_registry_has_hermes_native() -> None:
    ...
    assert _HARNESS_MODULES["hermes-native"] == "codify.inner.hermes_native_harness"
...
def test_native_coding_agent_resolves() -> None:
    ...
    from codify.harness_plugins import HERMES_NATIVE_CODING_AGENT
...
def test_create_app_builds() -> None:
    from codify.inner.hermes_native_harness import create_app
...
def test_configured_hermes_command_default_and_override() -> None:
    ...
    assert hn._configured_hermes_command({"CODIFY_HERMES_PATH": "/opt/hermes"}) == "/opt/hermes"
...
async def test_fetch_hermes_session_handles_status() -> None:
    payload = {"labels": {"codify.wrapper": "hermes-native-ui"}}
    assert (
