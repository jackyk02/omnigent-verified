...
from codify import claude_native_hook, native_policy_hook
...
def test_session_start_hook_records_transcript_state_without_output(
    ...
    This fails if the ``codify claude`` hook reintroduces
...
def test_session_start_hook_emits_conversation_url_system_message(
    ...
    This fails if ``codify claude`` stops routing the web URL
    through Claude's hook output path, leaving users with no startup
    pointer back to the Codify conversation.
    """
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", tmp_path / "root")
    ...
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in Codify: http://127.0.0.1:8787/c/conv_abc"
    }
    assert captured.err == ""
    assert read_transcript_path(bridge_dir) == transcript_path
...
def test_session_start_hook_maps_workspace_hosted_server_to_ui_mount(
    ...
    ``ap_server_url`` is the API proxy base (``/api/2.0/codify``);
    pointing the "Open this session" message there returns JSON, not
    the web UI. The message must land on the ``/codify`` SPA mount
    ...
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", tmp_path / "root")
    monkeypatch.setattr(
        "codify.cli_auth._token_file_path",
        lambda: tmp_path / "auth_tokens.json",
    )
    server = "https://example.databricks.com/api/2.0/codify"
    ...
    assert json.loads(captured.out) == {
        "systemMessage": (
            "Open this session in Codify: "
            "https://example.databricks.com/codify/c/conv_abc?o=2850744067564480"
        )
    }
...
def test_clear_session_start_hook_rotates_before_printing_conversation_url(
    ...
    """
    ``/clear`` SessionStart prints the URL for the replacement Codify session.
        ...
        def get(self, url: str) -> object:
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "labels": {"codify.claude_native.bridge_id": "bridge_shared"},
                },
    ...
    assert exit_code == 0
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in Codify: http://127.0.0.1:8787/c/conv_new"
    }
    assert captured.err == ""
    assert requests == [
        ...
        (
            ...
            {
                "runner_id": "",
                "labels": {"codify.claude_native.bridge_id": "conv_old-cleared"},
            },
        ),
    ]
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert '"codify_clear_rotated_to":"conv_new"' in recorded
    ...
    # _PERMISSION_TIMEOUT_S (86400) an unresponsive Codify server would hang
...
def test_fork_session_start_hook_forks_before_printing_conversation_url(
    ...
    """
    Claude ``/fork`` SessionStart prints the URL for the forked Codify session.
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Fork the Codify session or transfer the terminal.
    ...
    assert exit_code == 0
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in Codify: http://127.0.0.1:8787/c/conv_fork"
    }
    assert captured.err == ""
    assert requests == [
        ("GET", "http://127.0.0.1:8787/v1/sessions/conv_old", None),
    ...
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert '"codify_previous_claude_session_id":"claude_old"' in recorded
    assert '"codify_claude_session_was_seen":false' in recorded
    assert '"codify_fork_detected":true' in recorded
    assert '"codify_fork_rotated_to":"conv_fork"' in recorded
...
def test_resume_session_start_without_branch_marker_does_not_fork(
    ...
    """
    Ordinary Claude resumes do not create Codify forks.

    This fails if every ``SessionStart source=resume`` starts forking
    Codify sessions, which would break normal Claude resume flows.
    """

    class _FailingHttpxClient:
        """
        HTTP client stub that fails if fork detection makes Codify calls.
    ...
    assert exit_code == 0
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in Codify: http://127.0.0.1:8787/c/conv_old"
    }
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert "codify_fork_detected" not in recorded
    assert "codify_fork_rotated_to" not in recorded
...
def test_non_session_start_hook_does_not_emit_conversation_url_context(
    ...
    supposed to update Codify bridge state.
    """
    bridge_dir = tmp_path / "bridge"
...
def test_permission_request_hook_posts_to_active_session_from_bridge_config(
    ...
    """
    Permission command hook routes to the current active Codify session.

    This fails if the hook bakes in the launch conversation id: after
    Claude ``/clear`` rotates the bridge to a new Codify session, approval
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the outgoing Codify request.
    ...
    assert {k: v for k, v in sent.items() if k != "_codify_elicitation_id"} == payload
    assert re.fullmatch(r"elicit_claude_[0-9a-f]{32}", sent["_codify_elicitation_id"]), (
        f"re-attach id outside the claude-hook namespace: {sent.get('_codify_elicitation_id')!r}"
    )
...
def _prepare_permission_bridge(tmp_path: Path, session_id: str) -> Path:
    ...
    :param session_id: Active Codify session id, e.g. ``"conv_x"``.
...
def test_permission_request_hook_retries_transport_cut_with_same_id(
    ...
    ``_codify_elicitation_id`` — a fresh id per attempt would park a
    ...
    first_id = attempts[0]["_codify_elicitation_id"]
    ...
    assert attempts[1]["_codify_elicitation_id"] == first_id
...
def test_permission_request_hook_does_not_retry_rejections(
    ...
    """
    A 4xx from the Codify server is a deliberate answer — no retry.
...
def test_build_hook_settings_registers_policy_hooks_when_codify_server_url_set(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
...
def test_build_hook_settings_registers_message_display_hook(
    ...
    assert "codify.claude_native_message_display_hook" in command
...
def test_build_hook_settings_omits_policy_hooks_without_codify_server_url(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    ``build_hook_settings`` omits policy hooks when no Codify URL is set.

    Without an Codify server there are no policies to evaluate; registering
    ...
    for entry in hooks.get("PostToolUse", []):
        ...
        assert "evaluate-policy" not in cmd, (
            "Policy evaluation hook should not be registered without Codify URL"
        )
...
def test_evaluate_policy_pre_tool_use_converts_and_returns_deny(
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the outgoing Codify request and return a DENY verdict.
...
def test_ask_user_question_hook_noop_in_non_bypass_mode(
    ...
    This fails if the handler forwards the payload to Codify in non-bypass mode —
    ...
    class _RaisesIfCalled:
        ...
        def post(self, *_args: object, **_kwargs: object) -> object:
            """
            Fail if Codify is called — must not happen in non-bypass mode.
    ...
    for mode in ("default", "acceptEdits", "plan", None):
        ...
        captured = capsys.readouterr()
        # No Codify call, no output — "no opinion" so PermissionRequest takes over.
...
def test_ask_user_question_hook_posts_and_returns_pre_tool_use_output_in_bypass_mode(
    ...
    """
    In bypassPermissions mode the hook posts to Codify and returns PreToolUse output.
    ...
    must POST the payload to the Codify session's permission-request endpoint, then
    ...
    Fails if: Codify is not called in bypass mode, the URL targets the wrong session,
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the Codify request and return a canned PermissionRequest response.
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    # Codify must be called with the active session's URL.
    assert posted["url"] == (
...
def test_evaluate_policy_reauths_on_expired_token_instead_of_failing_closed(
    ...
    monkeypatch.setattr(
        "codify.runner._entry._make_auth_token_factory",
    ...
    build_hook_settings(
        bridge_dir,
        ap_server_url="https://codifys.example.databricksapps.com",
...
def test_reattach_cap_is_env_overridable(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_HOOK_MAX_RETRIES`` tunes the hard-failure cap.
    ...
    monkeypatch.setenv("CODIFY_HOOK_MAX_RETRIES", "3")
...
def test_reattach_bad_env_max_retries_falls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_HOOK_MAX_RETRIES", "banana")
    reloaded = importlib.reload(claude_native_hook)
    try:
...
def test_held_poll_floor_is_env_overridable(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_HOOK_HELD_POLL_FLOOR_S`` tunes the flap-vs-held boundary.
    ...
    monkeypatch.setenv("CODIFY_HOOK_HELD_POLL_FLOOR_S", "3.5")
    reloaded = importlib.reload(claude_native_hook)
    try:
        assert reloaded._PERMISSION_HELD_POLL_FLOOR_S == 3.5
    finally:
        monkeypatch.delenv("CODIFY_HOOK_HELD_POLL_FLOOR_S", raising=False)
        importlib.reload(claude_native_hook)
    ...
    for bad in ("not-a-number", "inf", "nan", "-inf"):
        monkeypatch.setenv("CODIFY_HOOK_HELD_POLL_FLOOR_S", bad)
        reloaded = importlib.reload(claude_native_hook)
        try:
