"""
Unit tests for the D7 migration of ``codify.client_tools.coding``
...
1. Eight ``@tool``-decorated functions are exported via the
   module-level ``_TOOL_FNS`` list (so ``codify chat`` /
...
from codify.client_tools.coding import (
    _TOOL_FNS,
    LSP,
    TOOLS,
    Bash,
...
def test_tool_fns_lists_all_eight_tools() -> None:
    ...
    not appear in ``TOOLS`` and ``codify chat`` consumers would
    miss it.
    """
