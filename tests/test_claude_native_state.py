...
``~/.codify/claude-native/<hash>/launch.json`` so the resume
...
:data:`CODIFY_CLAUDE_NATIVE_STATE_DIR`, so these tests never
...
def test_state_dir_for_conversation_id_is_under_root(
    ...
    miss a ``..`` escape).
    """
    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(tmp_path))
...
def test_read_malformed_json_returns_none(
    ...
    # restore of ``codify`` logger propagation in case a sibling
...
def test_state_root_env_var_redirects_state(
    ...
    """
    ``CODIFY_CLAUDE_NATIVE_STATE_DIR`` redirects the state
    tree.
    ...
    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(redirect))
