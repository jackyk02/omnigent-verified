...
import codify.conversation_browser as browser


@pytest.mark.parametrize(
    "url,expected",
    [
        # Databricks workspace API mount → the recognizable /codify SPA URL.
        (
            "https://e2-dogfood.staging.cloud.databricks.com/api/2.0/codify",
            "https://e2-dogfood.staging.cloud.databricks.com/codify",
        ),
        # A trailing ``?o=<org>`` selector on the API base is dropped.
        (
            "https://ws.databricks.com/api/2.0/codify?o=123",
            "https://ws.databricks.com/codify",
        ),
        # Trailing slash on the API mount still maps cleanly.
        (
            "https://ws.databricks.com/api/2.0/codify/",
            "https://ws.databricks.com/codify",
        ),
        # Non-Databricks URLs pass through unchanged (sans trailing slash).
        ("http://127.0.0.1:6767", "http://127.0.0.1:6767"),
        ("https://codify-02m5.onrender.com/", "https://codify-02m5.onrender.com"),
    ],
)
def test_display_server_url_maps_databricks_api_mount(url: str, expected: str) -> None:
    ...
    What this proves: the startup banner shows the workspace ``/codify``
    URL a user recognizes instead of the internal ``/api/2.0/codify``
...
@pytest.mark.parametrize(
    "url,expected",
    [
        ("https://ws.databricks.com/api/2.0/codify", True),
        ("https://ws.databricks.com/api/2.0/codify/", True),
        ("https://ws.databricks.com/codify", False),  # the SPA URL, not the API mount
        ("http://127.0.0.1:6767", False),
        ("https://codify-02m5.onrender.com", False),
    ],
)
def test_is_workspace_hosted_url(url: str, expected: bool) -> None:
    ...
    server-version row fires for ``/api/2.0/codify`` and nothing else, so
...
def test_conversation_url_maps_workspace_hosted_server_to_ui_mount(tmp_path, monkeypatch) -> None:
    ...
    The server base is the API proxy (``/api/2.0/codify``) — linking
    there returns JSON, not the web UI. The browser URL must land on
    ``/codify`` and carry ``?o=<org>`` recorded by ``codify
    ...
    assert url == ("https://example.databricks.com/codify/c/conv_abc123?o=2850744067564480")
...
def test_conversation_url_workspace_hosted_without_org_record(tmp_path, monkeypatch) -> None:
    ...
    url = conversation_url("https://example.databricks.com/api/2.0/codify", "conv_abc123")

    assert url == "https://example.databricks.com/codify/c/conv_abc123"
