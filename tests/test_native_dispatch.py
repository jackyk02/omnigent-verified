...
def test_resolve_colon_and_dot_paths() -> None:
    # module:attr form
    assert native_dispatch.resolve("codify.harness_plugins:load_object") is hp.load_object
    # module.attr form
    assert native_dispatch.resolve("codify.harness_plugins.load_object") is hp.load_object
...
def test_resolve_reflects_monkeypatched_symbol(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    sentinel = object()
    monkeypatch.setattr("codify.harness_plugins.native_agents", sentinel)
    assert native_dispatch.resolve("codify.harness_plugins:native_agents") is sentinel
