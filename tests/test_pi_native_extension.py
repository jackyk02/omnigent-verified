...
process.env.CODIFY_PI_NATIVE_CONFIG = configPath;
...
fs.writeFileSync(
  configPath,
  JSON.stringify({ serverUrl: "http://codify.test", sessionId: "session-1" }),
);
process.env.CODIFY_PI_NATIVE_CONFIG = configPath;
...
def test_registers_codify_tools_and_execute_round_trips(tmp_path: Path) -> None:
    ...
    tool's ``execute`` POSTs a JSON-RPC ``tools/call`` to the Codify server's
...
global.fetch = async (url, request) => {
  ...
  // Mimic the Codify /mcp proxy success envelope.
  return {
    ok: true,
    async json() {
      return {
...
(async () => {
  ...
  assert.equal(call.url, "http://codify.test/v1/sessions/conv_abc/mcp");
...
def test_bridged_tool_call_skips_hook_policy_eval(tmp_path: Path) -> None:
    """The tool_call hook must NOT re-evaluate policy for bridged Codify tools.
...
def test_mcp_unreachable_fails_closed_without_throwing(tmp_path: Path) -> None:
    """An unreachable Codify MCP server resolves to an error, never a throw.
...
fs.writeFileSync(
  configPath,
  JSON.stringify({ serverUrl: "http://codify.test", sessionId: "session-1", inboxDir }),
);
...
(async () => {
  ...
  assert.match(evalBodies[0]._codify_elicitation_id, /^elicit_evaluate_[0-9a-f]{32}$/);
...
(async () => {
  ...
  assert.equal(
    evalBodies[0]._codify_elicitation_id,
    evalBodies[1]._codify_elicitation_id,
  );
})().catch((error) => {
  console.error(error && error.stack ? error.stack : error);
  process.exit(1);
...
def test_policy_transport_error_fails_closed(tmp_path: Path) -> None:
    ...
    null, matching codify.policies.types.FAIL_CLOSED_PHASES and the Python
...
(async () => {
  ...
  const ids = new Set(evalBodies.map((b) => b._codify_elicitation_id));
...
function finish() {
  if (pi.__codifyInboxPoller) clearInterval(pi.__codifyInboxPoller);
...
def test_inbox_model_change_applies_via_set_model(tmp_path: Path) -> None:
    ...
    The runner queues the payload after the Codify server persisted the
...
def test_model_select_mirrors_to_external_model_change(tmp_path: Path) -> None:
    ...
    A ``set`` / ``cycle`` source must mirror back to Codify; a ``restore``
