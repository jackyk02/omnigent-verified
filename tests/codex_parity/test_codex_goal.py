...
from codify.inner.codex_executor import CodexExecutor
from codify.server import app as app_module
from codify.server.routes.sessions import create_sessions_router
...
@pytest.mark.asyncio
async def test_codify_codex_goal_set_api_forwards_mode_configuration() -> None:
...
@pytest.mark.asyncio
async def test_codify_codex_goal_get_without_live_runner_returns_empty_read_only() -> None:
...
@pytest.mark.asyncio
async def test_codify_codex_goal_put_without_live_runner_returns_503() -> None:
    app = _codex_goal_api_app(None)
    transport = httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_codify_codex_goal_set_api_rejects_boolean_token_budget() -> None:
    runner_client = _CodexGoalRunnerClient()
    app = _codex_goal_api_app(runner_client)
    transport = httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_codify_codex_goal_api_preserves_runner_4xx_error_payload() -> None:
    runner_client = _CodexGoalRunnerClient(
        status_code=400,
...
@pytest.mark.asyncio
async def test_codify_codex_goal_api_maps_malformed_runner_body_to_502() -> None:
...
@pytest.mark.asyncio
async def test_codify_codex_goal_api_maps_partial_goal_to_502() -> None:
    runner_client = _CodexGoalRunnerClient(
        response_body={
            "goal": {
                "thread_id": "thread_goal_test",
...
@pytest.mark.asyncio
async def test_codify_codex_goal_status_api_forwards_pause_resume() -> None:
    runner_client = _CodexGoalRunnerClient()
    app = _codex_goal_api_app(runner_client)
    transport = httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_codify_codex_goal_status_api_rejects_codex_owned_statuses() -> None:
    runner_client = _CodexGoalRunnerClient()
    app = _codex_goal_api_app(runner_client)
    transport = httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_codify_codex_goal_set_api_rejects_codex_owned_statuses() -> None:
    runner_client = _CodexGoalRunnerClient()
    app = _codex_goal_api_app(runner_client)
    transport = httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_codify_codex_goal_api_preserves_codex_owned_response_statuses() -> None:
