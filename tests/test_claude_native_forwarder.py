...
import codify.claude_native_forwarder as forwarder
...
def _handler_factory(
    ...
    class _Handler(BaseHTTPRequestHandler):
        """Request handler for the test Codify endpoint."""
...
async def _get_recorded_request(
    ...
    *,
    ...
    The forwarder mirrors Claude's native session id to Codify via a
...
async def test_clear_hook_rotates_active_session_without_reprocessing(
    ...
    """
    Claude ``/clear`` creates a fresh Codify session and consumes the hook.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Mock the Codify session-rotation endpoints.

        :param request: Incoming request.
        :returns: Canned Codify response.
        """
...
async def test_clear_hook_rotation_survives_old_runner_clear_failure(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Mock Codify rotation endpoints with a failing old-session cleanup.
...
async def test_clear_hook_consumes_hook_rotated_session_without_duplicate_fork(
    ...
    record_hook_event(
        bridge_dir,
        {
            "hook_event_name": "SessionStart",
            "source": "clear",
            "codify_clear_rotated_to": "conv_new",
        },
    )
...
@pytest.mark.asyncio
async def test_fork_hook_creates_codify_fork_and_consumes_hook(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    Claude ``/fork`` creates an Codify fork and consumes the hook.
    ...
    monkeypatch.setattr("codify.claude_native_bridge.time.time", lambda: 1779922393.245)
    record_hook_event(
        bridge_dir,
        {
            "hook_event_name": "SessionStart",
            ...
            "codify_previous_claude_session_id": "claude_old",
            "codify_claude_session_was_seen": False,
        },
    )
    calls: list[tuple[str, str, dict[str, Any] | None]] = []
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Mock the Codify fork-rotation endpoints.
...
async def test_fork_hook_consumes_hook_rotated_session_without_duplicate_fork(
    ...
    record_hook_event(
        bridge_dir,
        {
            ...
            "source": "resume",
            "transcript_path": str(transcript_path),
            "codify_fork_detected": True,
            "codify_fork_rotated_to": "conv_fork",
        },
    )
...
@pytest.mark.asyncio
async def test_resume_seen_claude_fork_does_not_create_second_codify_fork(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    Resuming an already-seen Claude branch does not create another Codify fork.
    ...
    record_hook_event(
        bridge_dir,
        {
            ...
            "transcript_path": str(transcript_path),
            "codify_previous_claude_session_id": "claude_old",
            "codify_claude_session_was_seen": True,
        },
    )

    def handler(request: httpx.Request) -> httpx.Response:
        """
        Fail if the forwarder tries to create another Codify fork.
...
async def test_forwarder_posts_visible_transcript_items(tmp_path: Path) -> None:
    """
    The background forwarder reads Claude JSONL and posts Codify items.
    ...
    but no process tailed that transcript into the Codify session
    stream.
    """
...
async def test_forwarder_posts_web_injected_terminal_transcript_items(tmp_path: Path) -> None:
    ...
    for Codify turns. This fails if a leftover pause/cursor path suppresses
...
async def test_forwarder_posts_compaction_in_progress_on_precompact_hook(
    ...
    or automatic overflow); the Codify server never runs the compaction for
    ...
    Codify can publish the spinner SSE.
    """
    bridge_dir = tmp_path / "bridge"
    transcript_path = tmp_path / "session.jsonl"
    transcript_path.write_text("", encoding="utf-8")
...
async def test_forwarder_start_at_end_uses_byte_offset_for_new_lines(
    ...
    monkeypatch.setattr(
        "codify.claude_native_forwarder.read_transcript_items_since_with_position",
...
async def test_forwarder_waits_for_missing_fresh_transcript_without_warning(
    ...
    caplog.set_level(logging.WARNING, logger="codify.claude_native_forwarder")
...
async def test_forwarder_survives_unhandled_loop_exceptions(
    ...
    caplog.set_level(logging.ERROR, logger="codify.claude_native_forwarder")
...
async def test_forwarder_drops_poison_item_after_bounded_permanent_retries(
    ...
    A malformed transcript item that Codify rejects with a permanent 4xx
    ...
    def _handle_request(request: httpx.Request) -> httpx.Response:
        ...
        :returns: HTTP response for the mock Codify endpoint.
        """
...
async def test_forwarder_mirrors_external_session_id_after_hook_event(
    ...
    """
    Forwarder PATCHes the Codify conversation with Claude's session id.
...
async def test_forwarder_retries_model_post_after_transient_failure(tmp_path: Path) -> None:
    ...
    model mirror against a single transient Codify error.
    """
    bridge_dir = tmp_path / "bridge"
    transcript_path = tmp_path / "session.jsonl"
...
async def test_supervise_forwarder_backoff_grows_on_repeated_crashes(
    ...
    Prevents a fast-failing forwarder from POST-storming the Codify server
...
def _start_recording_server_with_responses(
    ...
    the Codify server's response (rather than just a generic 202 ``{}``)
...
async def test_subagent_watcher_forwards_transcript_items_to_child_session(
    ...
    the Codify child session id (not the parent's).
    """
    bridge_dir = tmp_path / "bridge"
    transcript_path = tmp_path / "session.jsonl"
    transcript_path.write_text("", encoding="utf-8")
...
async def test_subagent_watcher_retry_skips_previously_posted_items(
    ...
    Without that guard Codify live subscribers can see item A synced back
...
# ---------------------------------------------------------------------------
# In-pane /effort → Codify session reasoning_effort mirroring
# ---------------------------------------------------------------------------
...
async def test_forward_available_deltas_posts_each_and_advances_offset(tmp_path: Path) -> None:
    ...
    shape the Codify route expects (delta + message_id + index + final) and
...
async def test_forward_available_deltas_drops_on_http_error(tmp_path: Path) -> None:
    ...
    Codify blip must not raise or wedge the tail. Fails if the error
...
async def test_persist_native_compaction_item_posts_compaction_event(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "codify.claude_native_forwarder.read_claude_session_id",
...
async def test_compaction_completed_triggers_persist(tmp_path: Path) -> None:
    ...
    with patch(
        "codify.claude_native_forwarder._persist_native_compaction_item",
        persist_mock,
    ):
        task = asyncio.create_task(
            forward_claude_transcript_to_session(
...
async def test_missing_compact_session_start_still_persists_from_transcript(
    ...
    with patch("codify.claude_native_forwarder._persist_native_compaction_item", persist):
...
async def test_failed_boundary_post_is_retried_not_consumed(tmp_path: Path) -> None:
    ...
    with patch("codify.claude_native_forwarder._persist_native_compaction_item", failing):
        handled = await _handle_compact_summary_item(
            AsyncMock(),
...
async def test_restart_reattach_does_not_repersist_completed_boundary(tmp_path: Path) -> None:
    ...
    from codify.claude_native_forwarder import _PendingCompaction, _write_compaction_state
...
async def test_ambiguous_boundary_post_marks_persisted(tmp_path: Path) -> None:
    ...
    with (
        patch("codify.claude_native_forwarder._persist_native_compaction_item", ambiguous),
        patch("codify.claude_native_forwarder.post_may_have_been_delivered", return_value=True),
    ):
        handled = await _handle_compact_summary_item(
            AsyncMock(),
...
async def test_standalone_completion_hook_persists_without_pending(tmp_path: Path) -> None:
    ...
    from codify.claude_native_forwarder import _mark_compaction_persisted
...
async def test_precompact_miss_is_counted_and_warned(tmp_path: Path) -> None:
    ...
    from codify.claude_native_forwarder import _write_compaction_state
...
async def test_standalone_hook_persist_failure_holds_cursor_for_retry(
    ...
    async def _run_once(state: forwarder.HookForwardState) -> forwarder.HookForwardState:
        ...
        with patch(
            "codify.claude_native_forwarder._post_external_compaction_status",
    ...
    ok = _persist_mock()
    with patch("codify.claude_native_forwarder._persist_native_compaction_item", ok):
...
def _http_status_error(status_code: int, body: object) -> httpx.HTTPStatusError:
    ...
    request = httpx.Request("POST", "http://codify/v1/sessions/conv_x/events")
