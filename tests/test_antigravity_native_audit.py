...
Pure unit tests for :mod:`codify.antigravity_native_audit` — the
...
from codify.antigravity_native_audit import (
    DEGRADE_NOTICE_TEXT,
    HARNESS_NAME,
...
def test_audit_request_skips_codify_mcp_tools() -> None:
    """``mcp__codify__*`` tools are relay-enforced; the audit returns None.

    Only the Codify MCP tools are double-counted by the relay path, so
    :func:`build_audit_evaluation_request` (delegating to
    ``hook_payload_to_evaluation_request``) skips exactly those.
    """
    request = build_audit_evaluation_request(
        tool_name="mcp__codify__sys_call", tool_input={}, model=None
    )
...
def test_audit_request_evaluates_connector_mcp_tools() -> None:
    ...
    Unlike ``mcp__codify__*`` (relay-enforced), connector MCP tools are not
