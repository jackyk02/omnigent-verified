...
class BenchProfile:
    ...
    :param harness: Harness name as registered in
        ``codify.runtime.harnesses._HARNESS_MODULES`` (or resolvable by
...
def resolve_profile(name: str) -> BenchProfile:
    ...
    3. Any harness registered in the codify registry (in-repo or an
    ...
    raise KeyError(
        f"unknown harness {name!r}: not a registered codify harness, not an "
