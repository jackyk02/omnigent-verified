...
def test_ambient_openai_wins_and_skips_resolver(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.runtime.credentials.databricks.resolve_databricks_workspace", _boom
    )
...
def test_config_profile_used_when_no_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.runtime.credentials.databricks.resolve_databricks_workspace", _resolver
    )
    env = resolve_bench_env(None)
...
def test_profile_from_providers_block(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.config.load_global_config", dict)
    monkeypatch.setattr(
        "codify.config.load_effective_config",
        lambda: {
            "providers": {
...
def test_profile_from_auth_block(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "codify.config.load_global_config",
...
def test_profile_from_config_tolerates_malformed_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.config.load_global_config", _malformed)
    monkeypatch.setattr("codify.config.load_effective_config", _malformed)
    assert _REAL_PROFILE_FROM_CONFIG() is None
