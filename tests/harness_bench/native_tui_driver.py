...
from tests.harness_bench.full_server import spawn_codify_server
from tests.harness_bench.mcp_tools import is_target_codify_mcp_tool
...
_CEL_POLICY_HANDLER = "codify.policies.builtins.cel.cel_policy"
...
class NativeVendor:
    ...
    :param own_auth: ``True`` when the vendor logs in itself (auth is not
        ``CODIFY_CREDENTIAL``), so the bench cannot provision it — runnable
...
_NATIVE_CODIFY_MCP_HARNESSES = frozenset(
    {
        "antigravity-native",
        "claude-native",
        "codex-native",
...
_MCP_TOOL_PROMPT = (
    "You must call the codify MCP tool mcp__codify__sys_session_list exactly once. "
...
def native_vendor(harness: str) -> NativeVendor | None:
    ...
    NATIVE_TUI`` in :func:`codify.harness_plugins.harness_capabilities`
    ...
    return NativeVendor(
        ...
        own_auth=caps.auth is not AuthModel.CODIFY_CREDENTIAL,
...
class NativeTuiDriver:
    ...
    def _provision(self) -> None:
        ...
        # Codify-credential natives resolve their provider from global config.
        if not self._vendor.own_auth:
            base_env["CODIFY_CONFIG_HOME"] = str(self._write_provider_config())
        self._proc = spawn_codify_server(self._tmp, port, base_env, binding_token)
        self._wait_health()
        self._daemon = self._spawn_host_daemon(base_env)
        self._client = httpx.Client(
            base_url=self._base_url,
            timeout=300.0,
            headers={"Origin": CODIFY_INTERNAL_WS_ORIGIN},
        )
    ...
    def _write_provider_config(self) -> Path:
        ...
        config_home = self._tmp / "codify-config"
    ...
    def _drive_tool_turn(self, *, deny: bool) -> TurnResult:
        ...
        prompt = self._vendor.tool_prompt.replace("codify-bench-ok", f"codify-bench-{token}")
    ...
    def _drive_mcp_tool_turn(self, *, timeout: float = _TOOL_TURN_TIMEOUT_S) -> TurnResult:
        """Call the read-only Codify MCP relay tool for this native harness."""
        assert self._vendor is not None
        result = TurnResult()
        if self._vendor.harness not in _NATIVE_CODIFY_MCP_HARNESSES:
            result.error = (
                f"{self._vendor.harness!r} has no Codify MCP bridge; "
        ...
        for prompt in (_MCP_TOOL_PROMPT, _MCP_TOOL_RETRY_PROMPT):
            ...
            if any(is_target_codify_mcp_tool(call.get("name")) for call in result.tool_calls):
                result.completed = True
                break
    ...
    def _drive_policy_turn(self, *, action: str) -> TurnResult:
        ...
        try:
            ...
            reader.start()
            ready.wait(timeout=10.0)
            prompt = self._vendor.tool_prompt.replace("codify-bench-ok", f"codify-bench-{action}")
            self._post_message(prompt)
