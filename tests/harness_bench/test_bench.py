...
def test_resolve_registered_harness_by_name() -> None:
    ...
    """
    from codify.harness_plugins import harness_modules
...
def test_resolve_entry_point_plugin_and_alias() -> None:
    ...
    ``codify-rovo`` registers ``rovo-cli`` (alias ``rovo``) via the
    ``codify.community.harness`` entry point and declares no capabilities
    ...
    """
    from codify.harness_plugins import harness_aliases

    if harness_aliases().get("rovo") != "rovo-cli":
        pytest.skip("codify-rovo plugin not installed")
...
def test_registry_profile_happy_path_no_plugin(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    codify-rovo. Guards the coverage the plugin tests skip-gate away.
    """
...
async def test_run_harness_emits_structured_events_and_linesink_adapts() -> None:
    ...
    mcp = next(e for e in finished if e.probe == "codify_mcp")
