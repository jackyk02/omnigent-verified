...
def _profile_from_config() -> str | None:
    ...
    All imports are lazy so importing this module never drags in ``codify.cli``
    at load time.
    """
    from codify.config import load_effective_config, load_global_config
...
def resolve_bench_env(explicit_profile: str | None) -> BenchRuntimeEnv:
    ...
    2. **Profile.** ``explicit_profile`` (the ``--profile`` flag) wins; else the
       ...
       ``~/.codify/config.yaml``). May be ``None`` (the resolver then uses the
...
def bench_creds_skip_reason(explicit_profile: str | None) -> str | None:
    ...
    ``OPENAI_*`` or a configured ``~/.codify`` profile is enough, matching
    ``omni run``.
    ...
    if not profile:
        return (
            ...
            "~/.codify/config.yaml (like `omni run`), or export OPENAI_API_KEY + "
            "OPENAI_BASE_URL"
        )
