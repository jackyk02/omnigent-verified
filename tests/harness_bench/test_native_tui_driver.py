...
def test_tool_turn_deny_attaches_policy_and_observes_denied_event() -> None:
    ...
    assert attached["handler"] == "codify.policies.builtins.cel.cel_policy"
...
def test_mcp_tool_turn_observes_prefixed_codify_tool() -> None:
    client = _FakeClient(items=[_function_call_item("mcp__codify__sys_session_list")])
    driver = _driver_with_fake("claude-native", client)

    result = driver._drive_mcp_tool_turn()

    assert result.completed
    assert [call["name"] for call in result.tool_calls] == ["mcp__codify__sys_session_list"]
    assert "mcp__codify__sys_session_list" in str(client.posted_events[-1])
...
def test_mcp_tool_turn_advances_baseline_after_wrong_tool(
    ...
    def _poll(baseline: int, result: TurnResult, timeout: float) -> None:
        ...
        name = "Bash" if len(baselines) == 1 else "mcp__codify__sys_session_list"
...
def test_mcp_tool_turn_skips_non_mcp_native_relay() -> None:
    ...
    assert result.error and "no Codify MCP bridge" in result.error
...
async def test_probes_read_native_tool_result_as_supported() -> None:
    ...
    class _Driver:
        ...
        async def run_mcp_tool_turn(self) -> TurnResult:
            return TurnResult(
                completed=True,
                tool_calls=[{"name": "mcp__codify__sys_session_list"}],
            )
    ...
    mcp_result = await CodifyMcpProbe().run(_Driver(), profile)
