...
| **Codify MCP** | A native harness calls the read-only `sys_session_list` relay tool through its generated `codify` MCP server. | P1 |
...
| Codify MCP | Not applicable | Generated `codify` MCP relay when supported by the vendor | Not applicable |
...
Reusable production helpers live in `codify.config` and the existing runtime
