"""Shared full-server infrastructure: spawn a real Codify server + runner.
...
def spawn_codify_server(
    tmp: Path, port: int, base_env: dict[str, str], binding_token: str
) -> subprocess.Popen[bytes]:
    """Spawn an ``codify server`` subprocess writing state under *tmp*.
    ...
    return subprocess.Popen(
        args,
        env={**base_env, "CODIFY_RUNNER_TUNNEL_TOKEN": binding_token},
...
class SharedFullServer:
    ...
    The Codify server is multi-agent/multi-session, and a single runner
