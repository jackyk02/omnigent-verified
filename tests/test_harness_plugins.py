...
import codify.harness_plugins as hp
...
def test_community_harness_contribution_is_merged(monkeypatch: pytest.MonkeyPatch) -> None:
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"foo": "codify.community.harness.foo.inner.foo_harness"},
            aliases={"foo-code": "foo"},
            model_env_keys={"foo": "HARNESS_FOO_MODEL"},
            spawn_env_builders={"foo": "codify.community.harness.foo.plugin:build_spawn_env"},
    ...
    assert hp.harness_modules()["foo-code"] == "codify.community.harness.foo.inner.foo_harness"
    assert hp.model_env_keys()["foo"] == "HARNESS_FOO_MODEL"
    assert hp.spawn_env_builders()["foo"] == "codify.community.harness.foo.plugin:build_spawn_env"
...
def test_community_harness_rejects_non_community_import_path(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="codify-foo",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "codify_foo.inner.foo_harness"},
        )
...
def test_community_harness_rejects_builtin_collision(monkeypatch: pytest.MonkeyPatch) -> None:
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"claude-sdk": "codify.community.harness.evil.inner.evil_harness"},
        )
    ...
    assert hp.harness_modules()["claude-sdk"] == "codify.inner.claude_sdk_harness"
...
def test_community_harness_rejects_alias_collision_with_builtin(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="codify-evil",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "codify.community.harness.evil.inner.foo_harness"},
...
def test_community_harness_rejects_community_collision(
    ...
    def _second() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="codify-bar",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "codify.community.harness.bar.inner.foo_harness"},
        )
    ...
    assert hp.harness_modules()["foo"] == "codify.community.harness.foo.inner.foo_harness"
...
def test_community_harness_rejects_native_terminal_metadata(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"foo-native": "codify.community.harness.foo.inner.foo_harness"},
...
def test_community_harness_readiness_uses_install_metadata(
    ...
    from codify.onboarding import harness_readiness as readiness
...
def test_community_namespace_imports_external_harness_package(
    ...
    package_dir = package_root / "codify" / "community" / "harness" / "foo"
    package_dir.mkdir(parents=True)
    (package_dir / "__init__.py").write_text("VALUE = 'ok'\n", encoding="utf-8")

    monkeypatch.syspath_prepend(str(package_root))

    import codify.community as community
    import codify.community.harness as harnesses

    importlib.reload(community)
    importlib.reload(harnesses)
    sys.modules.pop("codify.community.harness.foo", None)

    module = importlib.import_module("codify.community.harness.foo")
...
def test_community_harness_can_register_background_title_generator(
        ...
        return hp.HarnessContribution(
            ...
            background_title_generators={
                "foo": hp.BackgroundTitleGeneratorSpec(
                    "codify.community.harness.foo.background_titles:generate"
                )
            },
        )
    ...
    assert generator.generator == ("codify.community.harness.foo.background_titles:generate")
...
def test_builtin_native_provider_bridge_id_label_keys_match_constants() -> None:
    ...
    ``codify.<key>_native.bridge_id`` pattern rather than importing the bridge
    ...
    """
    from codify.antigravity_native_bridge import ANTIGRAVITY_NATIVE_BRIDGE_ID_LABEL_KEY
    from codify.codex_native_bridge import CODEX_NATIVE_BRIDGE_ID_LABEL_KEY
    from codify.opencode_native_bridge import OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY
