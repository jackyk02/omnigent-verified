...
from codify.server.auth import LEVEL_OWNER, RESERVED_USER_LOCAL
...
def _membership_counts(engine):
    ...
    with engine.connect() as conn:
        ...
        rows = conn.execute(
            text(
                "SELECT id, project_id FROM codify_conversation_metadata "
...
def test_seed_fast_path_row_counts_and_read_path(tmp_path: Path) -> None:
    ...
    from dev.benchmarks.codify import seed as seed_mod
    ...
    assert _count(engine, "codify_conversation_metadata") == 50
    ...
    with engine.connect() as conn:
        ...
        filed = conn.execute(
            text("SELECT COUNT(*) FROM codify_conversation_metadata WHERE project_id IS NOT NULL")
        ).scalar_one()
        assert filed == 30
        ...
        per_project = conn.execute(
            text(
                "SELECT project_id, COUNT(*) FROM codify_conversation_metadata "
    ...
    with engine.connect() as conn:
        ...
        assert {
            r[0] for r in conn.execute(text("SELECT kind FROM codify_conversation_metadata")).all()
        } == {1}
...
def test_seed_slow_path_non_sqlite() -> None:
    ...
    Skipped unless ``CODIFY_BENCH_NONSQLITE_URI`` points at a real non-SQLite
    DB (e.g. the nightly Postgres benchmark). The fast path is SQLite-only, so
    this guards the fallback the nightly run relies on.
    """
    uri = os.environ.get("CODIFY_BENCH_NONSQLITE_URI")
    if not uri:
        pytest.skip(
            "set CODIFY_BENCH_NONSQLITE_URI to a non-SQLite URI to exercise the slow seed path"
        )
