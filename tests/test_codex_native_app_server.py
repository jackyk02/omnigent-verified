...
from codify.codex_native_hook import _EVALUATE_POLICY_TIMEOUT_S
...
async def test_discover_codex_model_options_strips_secrets_and_stops_process(
    ...
    class _FakeClient:
        def __init__(self, *, ws_url: str, client_name: str) -> None:
            ...
            assert client_name == "codify-codex-model-discovery"
...
_CWD = "/home/user/repo"
_OUR_COMMAND = "/venv/bin/python -m codify.codex_native_hook evaluate-policy --bridge-dir /b"
...
def test_build_codex_native_server_profile_error_names_profile(
    ...
    """
    monkeypatch.setattr(
        "codify.codex_native_app_server._find_codex_cli",
        lambda: sys.executable,
    )
    monkeypatch.setattr(
        "codify.codex_native_app_server._databricks_gateway_host",
...
def test_build_codex_native_server_uses_profile_host_without_static_token(
    ...
    A default Codify install may not include ``databricks-sdk`` in the
...
[mcp_servers.codify] # stale generated table
command = "/old/python"
args = ["old"]

[mcp_servers.codify.env] # stale generated env
OLD = "1"

[mcp_servers.codify.tools.sys_session_rename] # stale generated approval
...
"""
    ...
    assert rendered.count("[mcp_servers.codify]") == 1
    assert "[mcp_servers.codify.env]" not in rendered
    parsed = tomllib.loads(rendered)
    assert parsed["mcp_servers"]["other"]["command"] == "other"
    assert parsed["mcp_servers"]["codify"] == {
        "command": "/new/python",
        "args": [
            "-I",
            "-m",
...
async def test_start_writes_fresh_mcp_config_without_leading_blanks(
    ...
    assert rendered.startswith("[mcp_servers.codify]\n")
    parsed = tomllib.loads(rendered)
    assert parsed["mcp_servers"]["codify"] == {
        "command": "/new/python",
        "args": [
...
async def test_untrusted_hook_is_trusted_via_batchwrite() -> None:
    """
    An untrusted Codify hook is trusted with its currentHash.
...
def test_write_codex_policy_hooks_file_merges_user_hooks(tmp_path: Path) -> None:
    ...
    regular file containing both the Codify policy hooks and the user's
    hooks, so user hooks fire alongside policy enforcement.
    """
    from codify.codex_native_app_server import _write_codex_policy_hooks_file
...
async def test_missing_hook_raises() -> None:
    """
    No discovered Codify hook fails loud (anti fail-open).
...
async def test_user_hooks_are_never_trusted() -> None:
    """
    Only Codify hooks are trusted; user-declared hooks are left alone.
...
def _set_codex_version(
    ...
    monkeypatch.setattr("codify.codex_native_app_server._codex_cli_version", _fake_version)
...
async def test_trust_failure_is_fail_open_with_reason(
    ...
    async def _raise_trust(_self: CodexNativeAppServer) -> None:
        raise RuntimeError("Codify policy hook was not discovered for cwd ...")
...
class TestPinCodexConfigModel:
    ...
    def test_replaces_top_level_model_only(self, tmp_path: Path) -> None:
        ...
        """
        from codify.codex_native_app_server import _pin_codex_config_model
    ...
    def test_read_back_by_forwarder_mirror_source(self, tmp_path: Path) -> None:
        ...
        """
        from codify.codex_native_app_server import _pin_codex_config_model
        from codify.codex_native_bridge import read_codex_config_model
