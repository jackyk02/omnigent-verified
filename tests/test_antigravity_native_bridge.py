...
import codify.antigravity_native_bridge as _mod
...
def test_update_conversation_id_mutates_correctly(bridge_dir: Path) -> None:
    ...
    the Codify session stays the same, the bridge must reflect the new id.
    """
...
def test_update_conversation_id_returns_false_and_warns_when_no_state(
    ...
    """
    with caplog.at_level(logging.WARNING, logger="codify.antigravity_native_bridge"):
...
def test_write_read_tmux_target_roundtrip(tmp_path: Path) -> None:
    ...
    write_tmux_target(
        bridge_dir,
        socket_path=Path("/tmp/codify-x/tmux.sock"),
        tmux_target="main",
    )
    info = read_tmux_info(bridge_dir)
    assert info == {"socket_path": "/tmp/codify-x/tmux.sock", "tmux_target": "main"}
...
# ---------------------------------------------------------------------------
# Codify MCP relay wiring (sys_* tools) — #1194
# ---------------------------------------------------------------------------


def test_build_mcp_config_registers_codify_relay(tmp_path: Path) -> None:
...
def test_build_mcp_config_defaults_python_to_current_interpreter(tmp_path: Path) -> None:
    ...
    server = build_mcp_config(tmp_path)["mcpServers"]["codify"]
