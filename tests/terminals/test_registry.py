"""
Unit tests for :class:`codify.terminals.TerminalRegistry`.
...
from codify.inner.terminal import TerminalCreateResult, TerminalInstance
from codify.terminals import TerminalRegistry
from codify.terminals import registry as registry_mod
from codify.terminals.registry import TerminalListEntry, conversation_link_for_id
...
def test_conversation_link_for_id_uses_relative_path_by_default() -> None:
    """
    Conversation links stay relative when no Codify origin is known.
    ...
    at the conversation route when rendered by the Codify web UI.
    """
...
def test_conversation_link_for_id_maps_workspace_hosted_server_to_ui_mount(
    ...
    (``/api/2.0/codify``) — into the registry. A naive
    ``{base}/c/<id>`` would put the JSON API path in the tmux status
    bar; the link must instead land on the ``/codify`` SPA mount and
    carry the ``?o=<org>`` selector ``codify login`` recorded, exactly
    like the CLI's ``Web UI:`` line. Pins parity with
    :func:`codify.conversation_browser.conversation_url`.
    ...
    assert (
        ...
        == "https://example.databricks.com/codify/c/conv_abc123?o=2850744067564480"
    )
...
def test_transfer_moves_terminal_without_closing_tmux(tmp_path: Path) -> None:
    ...
    moving ``claude/main`` from the old Codify conversation to the fresh
...
async def test_cleanup_conversation_closes_all_owners_terminals(
    ...
    sessions; orphans accumulate across the Codify process lifetime.
    """
    await reg_with_tmux.launch("conv_a", "bash", "s1", bash_spec)
    await reg_with_tmux.launch("conv_a", "bash", "s2", bash_spec)
    inst_b = await reg_with_tmux.launch("conv_b", "bash", "s1", bash_spec)
...
async def test_close_with_timeout_still_returns_true(tmp_path: Path) -> None:
    ...
    import codify.terminals.registry as reg_mod
