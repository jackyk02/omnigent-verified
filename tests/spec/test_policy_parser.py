...
def test_parse_guardrails_rejects_non_mapping() -> None:
    ...
    with pytest.raises(CodifyError, match=r"guardrails: must be a mapping"):
...
def test_parse_guardrails_ask_timeout_zero_rejected() -> None:
    ...
    """
    with pytest.raises(CodifyError, match=r"ask_timeout must be > 0"):
...
def test_parse_guardrails_ask_timeout_non_integer_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"ask_timeout must be an integer"):
...
def test_parse_labels_empty_dict_rejected() -> None:
    ...
    """
    with pytest.raises(CodifyError, match=r"empty dict"):
...
def test_parse_labels_initial_not_in_values_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"initial.*not in declared .values."):
        _parse_guardrails(
            _yaml("""
labels:
  level:
...
def test_parse_labels_values_non_list_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"values. must be a list"):
        _parse_guardrails(
            _yaml("""
labels:
  x:
...
def test_parse_function_policy_handler_alias() -> None:
    ...
    the original codify YAML uses ``function:``. Both must
...
def test_parse_function_policy_missing_function_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"function. policies require"):
        _parse_guardrails(
            _yaml("""
policies:
  broken:
...
def test_parse_function_policy_dict_missing_path_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"function.path. must be a"):
        _parse_guardrails(
            _yaml("""
policies:
  broken:
...
def test_parse_function_policy_arguments_non_dict_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"function.arguments. must be a mapping"):
        _parse_guardrails(
            _yaml("""
policies:
  broken:
...
def test_parse_policy_unknown_type_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"must be 'function'"):
        _parse_guardrails(
            _yaml("""
policies:
  p:
...
def test_parse_policy_missing_type_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing required field .type"):
        _parse_guardrails(
            _yaml("""
policies:
  p:
...
def test_parse_condition_empty_dict_returns_none() -> None:
    ...
    revisions raised :class:`CodifyError` here, breaking
...
def test_parse_condition_non_mapping_rejected() -> None:
    ...
    with pytest.raises(CodifyError, match=r"must be a mapping"):
