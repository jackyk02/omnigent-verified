...
``codify/inner/openai_agents_sdk_executor.py``) and, with no
...
_PACKAGED_DEBBY_DIR = _REPO_ROOT / "codify" / "resources" / "examples" / "debby"
...
def test_packaged_debby_resource_stays_in_sync_with_source_example() -> None:
    ...
    ``codify debby`` launches the packaged resource path, not
