"""
Tests for :mod:`codify.spec._codify_legacy_shim` — the
compatibility layer that lets legacy codify
``(content, phase)`` function-policy callables run under
Codify' ``(ctx, context)`` convention.
...
``tests/e2e/codify/test_run_codify_policy_enforcement.py``.
"""
...
from codify.spec._codify_legacy_shim import (
    _convert_args,
...
def test_legacy_content_input_passes_string_through() -> None:
    """REQUEST: codify puts the raw text in ``ctx.content``; legacy expects the same."""
...
def test_legacy_content_tool_call_passes_dict_through() -> None:
    """
    TOOL_CALL: codify already normalizes to
    ``{"tool": name, "args": parsed_args}`` at the enforcement
    site (see ``_enforce_tool_call_policy``). Legacy codify
...
def test_legacy_content_tool_result_plain_string_passes_through() -> None:
    """
    TOOL_RESULT: codify passes the raw tool output string
...
def test_legacy_content_tool_result_json_text_parses_to_dict() -> None:
    """
    TOOL_RESULT: when Codify' raw string IS a JSON-encoded
    object, the shim parses it into the corresponding Python
    dict. Mirrors codify-native's
...
def test_legacy_context_threads_configured_phases_when_provided() -> None:
    ...
    Why this matters: legacy codify callables that read
    ...
    Google MCP write through the Codify denied with
    ...
    silently denies under Codify mode.
    """
    ...
    # native codify — see ``_legacy_context``'s docstring),
...
def test_legacy_context_omits_tool_name_on_tool_call() -> None:
    """
    Native codify adds ``tool_name`` to the legacy context
    ...
    identically across native and Codify mode.
...
def test_legacy_context_omits_tool_name_on_input_and_output() -> None:
    ...
    on REQUEST/RESPONSE in codify today, and any future change
...
def test_build_wraps_legacy_callable_and_converts_args(
    ...
    """
    Legacy callables get wrapped. The wrapper takes Codify'
    ...
    # {"decision": {"result": ...}} format so the Codify'
...
def test_build_returns_modern_callable_unchanged(ephemeral_module: Any) -> None:
    ...
    codify-native policies and lets mixed YAMLs (one
...
def test_shim_builds_usable_FunctionPolicy_through_spec_factory(
    ...
    :func:`codify.policies.function.resolve_function_policy`
    ...
    otherwise.
    """
    from codify.policies.function import resolve_function_policy
    ...
    from codify.spec.types import PolicyAction
...
def test_build_forwards_reset_turn_attribute_from_legacy_callable() -> None:
    ...
    Codify' :class:`FunctionPolicy.reset_turn` can find
...
def test_rate_limit_factory_reset_turn_propagates_through_shim_and_policy() -> None:
    ...
    Codify mode. The agent runs further than the YAML author
