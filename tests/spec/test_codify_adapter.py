"""
Unit tests for the Codify YAML spec adapter.
...
- Dispatch detection in :func:`codify.spec.load`:
  codify YAMLs route to the adapter; codify YAMLs
  (identified by ``spec_version``) use the existing parser.

These are the phase 2 translation unit tests + fail-loud tests
called out in ``designs/CODIFY_INTEGRATION.md`` under the
phase 2 test scope.

Round-trip tests live in ``test_codify_roundtrip.py`` since
...
from codify.spec.codify import (
    CODIFY_EXECUTOR_TYPE,
    CODIFY_TOOL_LANGUAGE,
    agent_def_to_agent_spec,
)
from codify.spec.types import AgentSpec

if TYPE_CHECKING:
    from codify.inner.datamodel import AgentDef
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal codify YAML: ``name`` + ``prompt`` only.
...
def executor_block_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with an ``executor:`` block declaring
...
def function_tools_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with one function-type tool whose
...
def policies_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML declaring a ``policies:`` block. The adapter
    lifts this into ``AgentSpec.guardrails.policies`` so the
    codify workflow enforces it at the configured phases.
...
def os_env_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML declaring a top-level ``os_env:`` block. The
...
def mcp_tool_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with a stdio MCP-type tool.
    ...
    :class:`~codify.tools.mcp.McpServerConnection`.

    Includes ``executor.harness`` so the synthesized AgentSpec
    passes :mod:`codify.spec.validator` — the adapter runs
    ...
    config = {
        ...
        "tools": {
            "glean": {
                ...
                "args": ["-m", "codify.inner.databricks_mcps.glean"],
            },
        },
    }
...
def mcp_http_tool_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with an HTTP MCP-type tool (``url`` + headers).
...
def mcp_databricks_server_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with the ``databricks_server`` MCP shape —
    codify has no resolver for it, so the adapter rejects.
    """
    config = {
        "name": "mcp_db_example",
...
def cancellable_tool_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML declaring a legacy ``cancellable_function``
...
@pytest.fixture()
def codify_spec_dir(tmp_path: Path) -> Path:
    """
    An codify spec directory (``spec_version: 1`` in
    ``config.yaml``). Routes through the existing parser, not
    the codify adapter. Used by the detection tests.
    """
    config = {
        "spec_version": 1,
        "name": "ap-agent",
        "executor": {"type": "codify", "config": {"harness": "claude-sdk"}},
    }
...
def test_agent_def_to_agent_spec_hello_world(
    ...
    executor.type='codify'.

    What breaks if this fails: the baseline phase 2 dispatch —
    ``codify chat hello_world.yaml`` can't produce a valid spec
    ...
    # spec_version is synthesized to the current codify
    # schema version (no spec_version in codify YAMLs).
    assert spec.spec_version == 1
    # executor.type drives the runtime to pick CodifyExecutor.
    assert spec.executor.type == CODIFY_EXECUTOR_TYPE
...
def test_agent_def_to_agent_spec_accepts_claude_harness_alias(tmp_path: Path) -> None:
    """Codify YAML may use ``harness: claude`` as a spec-level alias."""
...
def test_agent_def_to_agent_spec_executor_block(
    ...
    What breaks if this fails: the CodifyExecutor cannot
    pick a harness or workspace profile at instantiation time,
    so every non-trivial codify YAML routes to the wrong
    harness (or fails).
    """
    from codify.inner.loader import load_agent_def
...
def test_agent_def_to_agent_spec_function_tool(
    ...
    What breaks if this fails: CodifyExecutor cannot resolve
    ...
    # from an codify YAML and must be re-resolved via
    # importlib.import_module rather than read off disk.
    assert tool.language == CODIFY_TOOL_LANGUAGE
...
def test_agent_def_to_agent_spec_translates_catalog_path_tool(tmp_path: Path) -> None:
    ...
    """
    from codify.inner.loader import load_agent_def
    from codify.spec.types import ToolRuntime
...
def test_function_tool_parameters_derived_from_callable_signature(
    ...
    the Codify adapter introspects the resolved Python callable's
    signature and exposes that as the LLM-facing JSON-Schema
    ``parameters`` block. Without this fallback, codify
...
def test_load_codify_yaml_missing_package_raises_with_install_hint(
    hello_world_yaml: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    When the ``codify`` package is not importable (e.g. agent-
    plane pip-installed standalone without the sibling codify
    source on PYTHONPATH), loading an codify YAML must surface
    a friendly :class:`CodifyError` with an install hint —
    not a bare ``ModuleNotFoundError`` from deep in the import
    machinery.

    What breaks if this fails: a user running ``codify chat foo.yaml``
    from an env that only has codify gets a cryptic
    ``ModuleNotFoundError: No module named 'codify'`` from
    ``_codify_compat.py``'s import line, with no clue what
    to install. The rewritten error says what's missing and how
    to fix it.
    """
    # Simulate ``codify`` not being importable by wiping it
    ...
    for mod_name in list(sys.modules):
        if mod_name == "codify" or mod_name.startswith("codify."):
    ...
    class _CodifyBlocker:
        """Meta-path finder that refuses to resolve codify."""
        ...
        def find_spec(
            ...
            del path, target
            if fullname == "codify" or fullname.startswith("codify."):
    ...
    blocker = _CodifyBlocker()
    monkeypatch.setattr(sys, "meta_path", [blocker, *sys.meta_path])

    with pytest.raises(CodifyError) as exc_info:
...
def test_load_policies_yaml_lifts_into_guardrails(policies_yaml: Path) -> None:
    """
    Codify YAMLs with a ``policies:`` block produce an
    AgentSpec whose ``guardrails.policies`` carries the
    translated policy, preserving ``name``, the dotted callable
    path, and the phase. The codify workflow then enforces
    at runtime via the standard :class:`PolicyEngine`.

    What breaks if this fails: policies in an codify YAML
    ...
    """
    from codify.spec.types import FunctionPolicySpec
    ...
    assert policy.function.path == "codify.spec._codify_legacy_shim.build"
...
def test_load_os_env_yaml_carries_through_top_level_field(
    ...
    """
    A top-level ``os_env:`` block on an codify YAML
    ...
    """
    from codify.inner.datamodel import OSEnvSpec
...
def test_load_mcp_stdio_yaml_translates_to_mcp_server(mcp_tool_yaml: Path) -> None:
    """
    Codify YAMLs declaring a subprocess MCP tool translate to
    a native ``MCPServerConfig(transport="stdio", ...)`` entry on
    ``AgentSpec.mcp_servers``. At runtime
    :class:`~codify.tools.mcp.McpServerConnection` spawns the
    ...
    under the Codify integration path.
    """
    ...
    # spawn matches what legacy codify ran.
    assert mcp.command == ".venv/bin/python"
    assert mcp.args == ["-m", "codify.inner.databricks_mcps.glean"]
...
def test_load_mcp_http_yaml_translates_to_mcp_server(mcp_http_tool_yaml: Path) -> None:
    """
    Codify YAMLs with an HTTP MCP (``url`` + headers)
    ...
    codify-legacy to Codify mode get either a translator crash
...
def test_mcp_stdio_yaml_reverse_trip_recovers_mcp_tool(mcp_tool_yaml: Path) -> None:
    ...
    path is what :meth:`CodifyExecutor.from_spec` calls
    when wrapping an codify spec for an codify
    harness; a missing reverse translation drops every MCP
    tool from the AgentDef the harness sees.

    What breaks if this fails: a live Codify mode run with a
    ...
    the live E2E test under tests/e2e/codify/ guards
    against.
    """
    from codify.inner.tools import MCPTool
    from codify.spec.codify import agent_spec_to_agent_def
    ...
    assert tool.args == ["-m", "codify.inner.databricks_mcps.glean"]


def test_load_mcp_databricks_server_yaml_raises(mcp_databricks_server_yaml: Path) -> None:
    """
    Codify MCP tools using the ``databricks_server=<name>``
    shape fail loud — Codify' MCPServerConfig doesn't
    ...
    """
    with pytest.raises(CodifyError, match="databricks_server"):
...
def test_load_cancellable_function_yaml_rejected_post_step_c(
    ...
    """
    Codify YAMLs declaring ``type: cancellable_function``
    are rejected by the Codify adapter with a clear migration hint.
    ...
    inner-stack YAML to Codify mode gets pointed at the new shape.
    ...
    ``TypeError`` instead.
    """
    with pytest.raises(CodifyError, match="cancellable_function"):
        load(cancellable_tool_yaml)


# ── Dispatch detection in codify.spec.load ──────────────


def test_load_codify_yaml_routes_to_adapter(
    executor_block_yaml: Path,
) -> None:
    """
    A ``.yaml`` file with ``name`` + ``prompt`` and no
    ``spec_version`` routes through the codify adapter.
    The produced spec has ``executor.type='codify'``, which
    no codify native spec ever emits.
    ...
    itself doesn't depend on which valid codify YAML we feed.

    What breaks if this fails: ``codify chat foo.yaml`` against an
    codify YAML would either crash (no config.yaml in a
    file-shaped source) or silently parse as an codify
...
def test_load_codify_directory_uses_existing_parser(
    codify_spec_dir: Path,
) -> None:
    """
    An codify spec directory (``spec_version`` declared)
    routes through the existing parser unchanged. The resulting
    spec has ``executor.type='codify'`` (the default)
    and no codify extras.

    What breaks if this fails: all existing codify specs
    would stop loading (regression surfaces in every codify
    test suite, so this is a smoke check).
    """
    spec = load(codify_spec_dir)
    # Default executor type, proving the dispatch routed correctly.
    assert spec.executor.type == "codify"
...
def test_load_yaml_with_spec_version_not_routed_to_adapter(
    ...
    an codify spec. The detection rule's negative check on
    ``spec_version`` prevents misrouting.

    This case currently still fails (codify YAML specs are
    directories, not files), but the dispatch MUST pick the
    non-codify branch so the resulting error is an
    codify "dest is required" / parser error — NOT an
    codify adapter error. The assertion below verifies the
    error shape is NOT an codify-adapter error.

    What breaks if this fails: a future change that starts
    supporting codify YAML-file specs would silently route
    to the wrong adapter.
    """
    config = {
        "spec_version": 1,
        "name": "hybrid",
        "prompt": "Looks codify-y but is codify.",
    }
    path = tmp_path / "hybrid.yaml"
    path.write_text(yaml.dump(config))

    # ``spec_version`` marks this file as an codify spec, but
    # codify specs must live in a directory with a
    # ``config.yaml``, not a single YAML file. ``load()`` rejects with
    # an actionable diagnostic. If detection were wrong, we'd get an
    # codify-adapter error (e.g. "missing system-prompt key")
    # instead — the assertion below pins the codify shape.
    with pytest.raises(CodifyError, match="spec_version"):
        load(path)
...
def test_cancellable_function_parameters_forward_trip_preserves_input_schema() -> None:
    ...
    **What breaks if this fails**: the adapter silently translates
    ...
    from codify.inner.tools import CancellableFunctionTool
...
def test_function_tool_parameters_round_trip_preserves_input_schema(
    ...
    explicit ``input_schema`` round-trips through codify
    translation and back without losing the schema.

    Step (c) made plain callables the only supported function-tool
    shape on the Codify path. Schema preservation matters because the
    ...
    from codify.inner.datamodel import AgentDef
    from codify.inner.datamodel import ExecutorSpec as OmniExecutorSpec
    from codify.inner.tools import FunctionTool
    from codify.spec import codify as spec_omni
    from codify.spec.codify import agent_def_to_agent_spec
...
def test_os_env_round_trips_through_translator() -> None:
    ...
    """
    from codify.inner.datamodel import (
        AgentDef,
        OSEnvSandboxSpec,
        OSEnvSpec,
    )
...
def test_inline_agent_tool_inherit_resolves_to_parent_os_env() -> None:
    ...
    :class:`OSEnvSpec` at translation time — codify spawns
    ...
    legacy codify semantics — silently breaks.
    """
    from codify.inner.datamodel import (
        AgentDef,
        OSEnvSpec,
    ...
    from codify.inner.tools import AgentTool
    from codify.spec.codify import agent_def_to_agent_spec
...
def test_inline_agent_tool_inherit_with_no_parent_os_env_yields_none() -> None:
    ...
    ``None`` — matches legacy codify behavior when the
...
def test_instructions_field_falls_back_to_prompt_when_unset() -> None:
    ...
    codify YAML written before the field existed.
    """
    from codify.inner.datamodel import AgentDef
    from codify.spec.codify import agent_def_to_agent_spec
...
def test_instructions_yaml_loads_through_full_pipeline(tmp_path: Path) -> None:
    """
    End-to-end through ``load_codify_yaml`` (the integration
    path the Codify server hits when registering an codify
    bundle): YAML with ``instructions: AGENTS.md`` produces a
    spec whose ``instructions`` field carries the file's
    contents.
    """
    from codify.spec._codify_compat import load_codify_yaml
    ...
    spec = load_codify_yaml(yaml_path)
    assert spec.instructions == "FROM AGENTS DOT MD"


# ── Terminals threading (CODIFY_TERMINAL_BRIDGE §6.1) ────────────
...
def test_terminals_thread_through_translator() -> None:
    ...
    makes ``terminals:`` declarations in codify YAML reach the
    AP-side ``sys_terminal_*`` tools — the whole feature from
    ``designs/CODIFY_TERMINAL_BRIDGE.md`` collapses if this breaks.

    What breaks if this fails: codify YAMLs that declare
    ``terminals:`` boot under Codify mode with
...
def test_harness_auto_picks_from_model_prefix(
    ...
    """
    When an codify YAML declares a model but no harness,
    the adapter fills in the right harness by matching the
    model prefix against
    :data:`~codify.spec.codify._HARNESS_FOR_MODEL_PREFIX`.

    Mirrors the auto-pick pure codify' CLI does at
    ...
    blocking the entire Codify path.
    """
    from codify.inner.datamodel import AgentDef
    from codify.inner.datamodel import ExecutorSpec as OmniExecutorSpec
    from codify.spec.codify import agent_def_to_agent_spec
...
def test_harness_auto_pick_unknown_model_raises() -> None:
    ...
    :raises CodifyError: With a message explaining that the
    ...
    with pytest.raises(CodifyError, match=r"[Hh]arness"):
...
# :func:`codify.loader.load_agent_def`) because the loader
...
def _build_agent_def_with_raw_yaml(
    ...
    codify loader (here stubbed with an empty ``policies``
    ...
    :param policies: Raw codify ``policies:`` dict.
    :param labels: Raw codify top-level ``labels:`` dict
        (initial values).
    :param label_schema: Raw codify top-level
        ``label_schema:`` dict.
    :param ask_timeout: Raw codify top-level
        ``ask_timeout:`` value.
...
def test_function_policy_routes_callable_through_legacy_shim() -> None:
    ...
    legacy codify ``(content, phase)`` convention keep
    working under Codify' ``(ctx, context)`` convention —
    see ``codify.spec._codify_legacy_shim``. The shim
    is a runtime no-op for codify-native callables, so
...
def test_function_policy_callable_alias_resolves_identically_to_handler() -> None:
    ...
    Old codify YAMLs used ``callable:`` as the key name; current
...
def test_ask_timeout_top_level_propagates_to_guardrails() -> None:
    """
    The codify top-level ``ask_timeout:`` lands on
    :attr:`GuardrailsSpec.ask_timeout`.

    What breaks if this fails: ASK-action policies park forever
    (no timeout) or fall back to the codify default even
...
def test_no_policies_no_guardrails_block() -> None:
    """
    An codify YAML without any policies/labels/ask_timeout
    produces a spec with ``guardrails=None``, matching the
    zero-policy case documented in POLICIES.md §10.

    What breaks if this fails: codify would instantiate a
...
def test_executor_extra_field_propagates_to_llm_config() -> None:
    ...
    ``parallel_tool_calls``, etc.) through the Codify path, even
    though those knobs work fine via legacy codify. Makes
    Codify mode a downgrade rather than a compatible integration.
    """
    agent_def, raw_yaml = _build_agent_def_with_raw_yaml()
    raw_yaml["executor"] = {
        "model": "databricks-gpt-5-mini",
...
def test_executor_extra_absent_yields_empty_llm_extra() -> None:
    """
    When the codify YAML omits ``executor.extra``, the
...
def test_use_responses_false_propagates_to_executor_config() -> None:
    """
    ``use_responses: false`` in an codify YAML executor block lands on
    ``spec.executor.config["use_responses"]`` as ``False`` after
    ``agent_def_to_agent_spec``.

    The inner ``ExecutorSpec`` dataclass (``codify.inner.datamodel``)
    has no ``use_responses`` field, so the codify YAML loader silently
...
def test_use_responses_absent_omits_key_from_executor_config() -> None:
    """
    When the codify YAML omits ``use_responses``, the key is absent
...
def test_unknown_policy_type_rejected_with_clear_message() -> None:
    ...
    with pytest.raises(CodifyError, match="weird") as exc_info:
...
def test_self_clone_string_shorthand_loader_produces_selfagent_tool(
    ...
    """
    from codify.inner.loader import load_agent_def
    from codify.inner.tools import SelfAgentTool
...
def test_agent_def_to_agent_spec_self_clone_propagates_parent_config(
    ...
    """
    Translating an codify YAML with ``tools.subtask: self``
    ...
    assert sub.executor.type == CODIFY_EXECUTOR_TYPE
...
def test_compat_yaml_executor_auth_is_not_dropped(tmp_path: Path) -> None:
    """
    ``executor.auth:`` declared in an codify-compat YAML is preserved
    ...
    Uses ``load_codify_yaml`` — the real production entry point — so
    ...
    """
    from codify.spec._codify_compat import load_codify_yaml
    from codify.spec.types import DatabricksAuth
...
def test_compat_yaml_executor_api_key_auth_is_not_dropped(tmp_path: Path) -> None:
    """
    ``executor.auth: {type: api_key, …}`` in an codify-compat YAML is
    ...
    """
    from codify.spec._codify_compat import load_codify_yaml
    from codify.spec.types import ApiKeyAuth
