...
def test_load_tarball_without_dest_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(CodifyError, match="dest is required"):
        load(tar_path)
...
def test_load_yaml_missing_prompt_raises_actionable_error(tmp_path: Path) -> None:
    ...
    codify-YAML check. Without the diagnostic dispatch in
    ...
    with pytest.raises(CodifyError) as excinfo:
        load(yaml_path)
...
def test_load_yaml_with_spec_version_raises_actionable_error(tmp_path: Path) -> None:
    ...
    codify spec but is sitting alone (no ``config.yaml``
    bundle dir). The codify check rejects it. The error
...
def test_load_yaml_with_parse_error_raises_actionable_error(tmp_path: Path) -> None:
    ...
    colon in a value) falls through ``is_codify_yaml`` and,
...
def test_load_invalid_spec_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(CodifyError, match="invalid agent spec"):
        load(tmp_path)
...
def test_materialize_bundle_copies_directory_recursively(tmp_path: Path) -> None:
    ...
    the path taken by native codify agent-image directories
...
def test_materialize_bundle_wraps_yaml_file_in_dest(tmp_path: Path) -> None:
    ...
    ``_find_codify_yaml_in_dir`` expects — it scans the
    ...
    assert returned == dest
    assert dest.is_dir()
    # Basename preserved — Codify' dispatch uses
    # ``is_codify_yaml`` on the exact file (not a synthesized
...
def test_materialize_bundle_then_load_roundtrip_directory(tmp_path: Path) -> None:
    ...
    (``_preregister_agent``, ``_prepare_codify_yaml_bundle``,
    ``_chat_local``) rely on.
    """
    source = tmp_path / "source"
    source.mkdir()
...
def test_materialize_bundle_then_load_roundtrip_yaml(tmp_path: Path) -> None:
    ...
    a standalone codify YAML must produce a directory
    :func:`load` can dispatch through ``_find_codify_yaml_in_dir``.
    ...
    # Codify-sourced spec — translator sets executor.type.
    assert spec.name == "hello-from-yaml"
    assert spec.executor.type == "codify"


def test_load_codify_yaml_preserves_use_responses_bool(tmp_path: Path) -> None:
    """
    ``use_responses: false`` in an codify YAML must land as Python ``False``
    ...
    How it's guarded: ``_codify_compat.load_codify_yaml`` reads the raw YAML
    with ``_CodifyYamlLoader`` (not ``yaml.safe_load``) so this raw read resolves
    ...
    spec = load_codify_yaml(tmp_path / "kimi-test.yaml")
    assert spec.executor.config.get("use_responses") is False


def test_load_codify_yaml_threads_executor_extra_max_tokens_to_llm_extra(
    tmp_path: Path,
) -> None:
    """
    Codify-compatible YAML carries generation kwargs under
    ``executor.extra``; the Codify compatibility loader must translate
    those into ``spec.llm.extra`` so harness-backed Codify execution can
...
def test_load_codify_yaml_unknown_harness_hints_at_version_skew(
    tmp_path: Path,
) -> None:
    """
    ...
    with pytest.raises(CodifyError) as excinfo:
        load_codify_yaml(tmp_path / "skew-test.yaml")
...
def test_load_codify_yaml_missing_harness_omits_version_skew_hint(
    tmp_path: Path,
) -> None:
    """
    ...
    with pytest.raises(CodifyError) as excinfo:
        load_codify_yaml(tmp_path / "no-harness-test.yaml")
...
def test_load_pruning_logs_warning_for_dropped_sub_agent(
    ...
    with caplog.at_level("WARNING", logger="codify.spec"):
        load(tmp_path, prune_invalid_sub_agents=True)
        ...
        yaml.dump(
            {
                "spec_version": 1,
                "name": "grandchild",
                "executor": {"type": "codify", "config": {"harness": _UNKNOWN_HARNESS}},
            }
        )
    )
