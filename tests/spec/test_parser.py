...
from codify.spec.parser import discover_host_skills, parse
from codify.spec.types import ApiKeyAuth, DatabricksAuth, ProviderAuth, SharePolicy


@pytest.fixture(autouse=True)
def _clean_container_runtime_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Ensure CODIFY_CONTAINER_RUNTIME never leaks from the host environment."""
    monkeypatch.delenv("CODIFY_CONTAINER_RUNTIME", raising=False)
...
def test_parse_non_mapping_config(tmp_path: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"must be a YAML mapping"):
        parse(tmp_path)


def test_parse_missing_spec_version(tmp_path: Path) -> None:
    (tmp_path / "config.yaml").write_text(yaml.dump({"name": "no-version"}))
    with pytest.raises(CodifyError, match=r"missing required field: spec_version"):
        parse(tmp_path)
...
def test_parse_llm_missing_model(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"missing required field: model"):
        parse(tmp_path)
...
def test_parse_llm_connection_unresolved_var_raises(
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"Unresolved environment variable"):
        parse(tmp_path)
...
def test_parse_inline_mcp_tools_non_list_raises(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"'tools' must be a list"):
        parse(tmp_path)
...
def test_parse_expand_env_false_keeps_var_references(
    ...
    Used during scaffolding/validation (e.g. ``codify create``) where
...
def test_parse_skill_missing_frontmatter(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing YAML frontmatter"):
        parse(agent_dir)
...
def test_parse_skill_missing_name(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing required field 'name'"):
        parse(agent_dir)
...
def test_parse_skill_missing_description(agent_dir: Path) -> None:
    ...
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("---\nname: no-desc\n---\nContent.")
    with pytest.raises(CodifyError, match=r"missing required field 'description'"):
        parse(agent_dir)


def test_parse_skill_non_utf8_raises_codify_error(agent_dir: Path) -> None:
    """
    A non-UTF-8 SKILL.md must funnel through CodifyError (not escape as a
    ...
    with pytest.raises(CodifyError, match=r"could not be read"):
        parse(agent_dir)
...
# which broke ``codify --harness codex`` REPL launch before the
...
def test_parse_skill_invalid_yaml_frontmatter_in_bundle_raises(
    ...
    with pytest.raises(CodifyError, match=r"invalid YAML frontmatter"):
        parse(agent_dir)
...
def test_discover_host_skills_skips_invalid_yaml_frontmatter(
    ...
    string that aborted ``codify --harness codex`` REPL launch
    in production.
    ...
    with caplog.at_level("WARNING", logger="codify.spec.parser"):
...
def test_parse_skills_filter_invalid_string_rejects(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"\"all\".*\"none\""):
        parse(agent_dir)
...
def test_parse_skills_filter_non_string_list_item_rejects(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"list items must be strings"):
        parse(agent_dir)
...
def test_parse_mcp_env_unresolved_var_raises(
    ...
    """
    Unresolved ``${VAR}`` in MCP env raises ``CodifyError``
...
def test_parse_mcp_missing_transport(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing required field 'transport'"):
        parse(agent_dir)
...
def test_parse_tools_sandbox_runtime_env_var(
    ...
    """CODIFY_CONTAINER_RUNTIME env var is used when YAML omits container_runtime."""
    monkeypatch.setenv("CODIFY_CONTAINER_RUNTIME", "podman")
    config = {
        "spec_version": 1,
...
def test_parse_inline_mcp_rejects_non_dict_headers(tmp_path: Path) -> None:
    ...
    ``CodifyError`` instead of silently falling back to ``{}``.
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"headers.*must be a mapping"):
        parse(tmp_path)
...
def test_parse_inline_mcp_rejects_non_dict_env(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"env.*must be a mapping"):
        parse(tmp_path)
...
def test_parse_os_env_absent_yields_none(agent_dir: Path) -> None:
    ...
    contract from the codify-compat path.
    """
    spec = parse(agent_dir)
...
def test_parse_os_env_caller_process(tmp_path: Path) -> None:
    ...
    What breaks if this fails: native Codify YAMLs cannot opt into
...
def test_parse_os_env_non_mapping_raises(tmp_path: Path) -> None:
    """A scalar/list under ``os_env:`` raises CodifyError —
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"os_env must be a YAML mapping"):
        parse(tmp_path)


def test_parse_os_env_sandbox_non_mapping_raises(tmp_path: Path) -> None:
    """A scalar/list under ``os_env.sandbox:`` raises
    CodifyError — same fail-loud contract as the parent.
    """
    config = {
        "spec_version": 1,
        "name": "bad-sandbox",
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"os_env.sandbox must be a YAML mapping"):
        parse(tmp_path)
...
def test_parse_os_env_sandbox_cwd_allow_hidden_validation(
    ...
    :class:`CodifyError` at parse time with a message that
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=match_regex):
        parse(tmp_path)
...
def test_parse_os_env_sandbox_cwd_hidden_scan_recursive_validation(
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"must be a boolean"):
        parse(tmp_path)
...
def test_parse_os_env_sandbox_cwd_hidden_scan_overflow_validation(
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"must be one of"):
        parse(tmp_path)
...
def test_mcp_missing_url_raises(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing required field 'url'"):
        parse(agent_dir)
...
def test_parse_llm_profile_survives_consolidation(tmp_path: Path) -> None:
    ...
    (``codify/runtime/policies/builder.py``), so losing it makes those
...
def test_parse_builtins_dict_missing_name(tmp_path: Path) -> None:
    """Dict entry without 'name' raises CodifyError."""
    config = {
        "spec_version": 1,
        "tools": {
            "builtins": [
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"name"):
        parse(tmp_path)
...
def test_parse_executor_config(tmp_path: Path) -> None:
    ...
    # Default type should be "codify" when not specified.
...
def test_parse_executor_defaults(tmp_path: Path) -> None:
    ...
    # Default type is "codify" per ExecutorSpec.
    # Failure means the parser uses a different default.
    assert spec.executor.type == "codify"
...
def test_parse_rejects_boolean_values_for_numeric_config_fields(
    ...
    with pytest.raises(CodifyError, match=match):
        parse(tmp_path)
...
def test_parse_rejects_boolean_terminal_scrollback(tmp_path: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"terminals\.main\.scrollback must be an integer"):
        parse(tmp_path)
...
def test_parse_executor_config_field(tmp_path: Path) -> None:
    ...
    The ``config`` field is executor-type-specific. For the codify
    ...
    # breaking codify harness selection at executor construction.
    assert spec.executor.type == "codify"
...
def test_parse_executor_config_missing_defaults_to_empty(
    ...
    config = {"spec_version": 1, "executor": {"type": "codify"}}
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    spec = parse(tmp_path)
...
def test_parse_rejects_boolean_mcp_timeout(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"MCP server 'slow-service'\.timeout"):
        parse(agent_dir)
...
def test_parse_mcp_stdio_rejects_legacy_sandbox_field(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"sandbox.*was removed"):
        parse(agent_dir)
...
def test_parse_mcp_stdio_missing_command_raises(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"missing required field 'command'"):
        parse(agent_dir)
...
def test_parse_mcp_stdio_rejects_http_fields(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"wrong-transport field"):
        parse(agent_dir)
...
def test_parse_mcp_unknown_transport_raises(agent_dir: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"must be 'http' or 'stdio'"):
        parse(agent_dir)
...
def test_parse_share_invalid_value_fails_loud(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match="agent_session_sharing"):
        parse(tmp_path)
...
def test_parse_os_env_sandbox_env_passthrough_default_none(tmp_path: Path) -> None:
    ...
    :func:`codify.inner.os_env.build_helper_env` is meant to close.
    """
    config = {
        "spec_version": 1,
        "name": "no-passthrough",
...
def test_parse_os_env_sandbox_env_passthrough_validation(
    ...
    """
    Invalid ``env_passthrough`` values raise :class:`CodifyError`
...
def test_parse_os_env_start_in_scratch_with_fork_rejected(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"mutually exclusive"):
        parse(tmp_path)
...
def test_parse_os_env_start_in_scratch_with_sandbox_none_rejected(
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"requires an active sandbox"):
        parse(tmp_path)
...
def test_executor_profile_field_lifted_from_yaml(tmp_path: Path) -> None:
    ...
    # Back-compat mirror into config["profile"] for codify.
    # Without this the legacy codify executor (which still
    # reads config["profile"]) would silently fall back to env
    # vars / DEFAULT section.
    assert spec.executor.config.get("profile") == "dev"


def test_executor_profile_field_lifted_for_non_codify(tmp_path: Path) -> None:
...
def test_codify_and_default_executor_minimal_configs_still_parse(tmp_path: Path) -> None:
    """Both legacy ``codify`` and default minimal YAMLs continue to parse cleanly."""
    omni_config = {
        "spec_version": 1,
        "name": "omni-agent",
        "executor": {
    ...
    assert omni_spec.executor.type == "codify"
    ...
    assert llm_spec.executor.type == "codify"
...
def test_parse_executor_auth_provider_missing_name_raises(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"executor.auth.name is required"):
        parse(tmp_path)
...
def test_parse_executor_auth_unknown_type_raises(tmp_path: Path) -> None:
    """An unknown ``auth.type`` value raises :class:`CodifyError`."""
    config = {
        "spec_version": 1,
        "executor": {"auth": {"type": "magic_token", "token": "abc"}},
    }
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"must be 'api_key', 'databricks', or 'provider'"):
        parse(tmp_path)
...
def test_parse_executor_auth_api_key_missing_key_raises(tmp_path: Path) -> None:
    ...
    :class:`CodifyError` rather than producing an empty key.
    """
    config = {
        "spec_version": 1,
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"api_key is required"):
        parse(tmp_path)
...
def test_parse_executor_auth_databricks_missing_profile_raises(tmp_path: Path) -> None:
    ...
    :class:`CodifyError` rather than silently using an empty profile.
    """
    config = {
        "spec_version": 1,
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"profile is required"):
        parse(tmp_path)
...
def test_parse_credential_proxy_rejects_duplicate_host(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"binds host 'github.com' more than once"):
        parse(tmp_path)
...
def test_parse_credential_proxy_databricks_cli_rejected_on_macos(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"databricks_cli' does not work\s+on macOS"):
        parse(tmp_path)
...
def test_parse_credential_proxy_requires_egress_rules(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"requires os_env.sandbox.egress_rules"):
        parse(tmp_path)
...
def test_parse_credential_proxy_requires_hard_backend(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"credential_proxy requires sandbox.type"):
        parse(tmp_path)
...
def test_parse_credential_proxy_gh_basic_rejected_on_macos(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump(config))
    with pytest.raises(CodifyError, match=r"gh_basic' does not work on macOS"):
        parse(tmp_path)
...
def test_config_loader_does_not_mutate_shared_safeloader_resolvers() -> None:
    ...
    """
    import codify.spec.parser as parser
