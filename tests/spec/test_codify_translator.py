"""Unit tests for ``codify.spec.codify.agent_spec_to_agent_def``.

Phase 1 ships the forward direction only. These tests hand-craft
:class:`AgentSpec` objects and assert the resulting
:class:`codify.datamodel.AgentDef` has the expected shape. The
...
from codify.inner.tools import AgentTool, FunctionTool
from codify.spec import (
    AgentSpec,
    ExecutorSpec,
    FunctionPolicySpec,
    FunctionRef,
...
# SandboxConfig is not re-exported from codify.spec's public
...
from codify.spec.types import SandboxConfig
...
# via ``tests.spec.test_codify_translator.sample_tool_callable``.
...
def basic_spec() -> AgentSpec:
    """
    Minimal ``AgentSpec`` targeting the codify executor.

    :returns: A spec with name, instructions, one ``llm.model``, and
        ``executor.type == "codify"`` carrying harness+profile.
    """
    return AgentSpec(
        spec_version=1,
        name="hello-agent",
...
def test_basic_spec_produces_agent_def_with_name_and_prompt(
    ...
    **What breaks if this fails**: the codify harness can
...
def test_basic_spec_maps_llm_and_executor_config(
    ...
    **What breaks if this fails**: the codify
...
def test_missing_profile_maps_to_none(
    ...
    surfaces ``None`` so the codify
    :class:`~codify.datamodel.ExecutorSpec.profile` field
...
def test_spec_with_local_tool_resolves_dotted_path(
    ...
    **What breaks if this fails**: the harness receives a tool
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="sample",
            path="tests.spec.test_codify_translator.sample_tool_callable",
            language="python",
        ),
    ]
    agent_def = agent_spec_to_agent_def(basic_spec)
...
def test_policies_dropped_from_forward_translation(
    ...
    because codify enforces them upstream of the executor.

    **What breaks if this fails**: two regressions to guard
    against:
    1. The translator starts rejecting again (``CodifyError``
       with ``"policies"``) — the codify executor would then
    ...
    2. The translator starts round-tripping policies INTO the
       AgentDef — meaning both the codify workflow AND the
       codify runtime would enforce them, double-counting
       every DENY.
    """
...
def test_sandbox_block_rejected_with_clear_message(
    ...
    **What breaks if this fails**: the harness runs tools outside
    ...
    with pytest.raises(CodifyError, match=r"sandbox"):
...
def test_tool_with_filesystem_path_rejected(
    ...
    **What breaks if this fails**: the translator silently drops
    ...
    with pytest.raises(CodifyError, match=r"dotted"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_with_unimportable_module_rejected(
    ...
    **What breaks if this fails**: a typo in the tool path
    ...
    with pytest.raises(CodifyError, match=r"no_such_module_xyzzy"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_pointing_at_non_callable_rejected(
    ...
    **What breaks if this fails**: the codify harness
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="stringy",
            path="tests.spec.test_codify_translator.NOT_A_FUNCTION",
            language="python",
        ),
    ]
...
def test_missing_llm_rejected(basic_spec: AgentSpec) -> None:
    """
    A spec with ``executor.type='codify'`` but no ``llm``
    block is rejected — the codify harness needs a model
    name. We fail loud at translation time, not deep inside the
    harness constructor.
    """
    basic_spec.executor.model = None
    with pytest.raises(CodifyError, match=r"executor\.model"):
        agent_spec_to_agent_def(basic_spec)


# ── Harness inference for native Codify v1 specs ────────────────────────────────
...
def test_native_codify_spec_infers_harness_from_model(
    model: str,
    expected_harness: str,
) -> None:
    """
    Native Codify v1 specs use ``executor.type="codify"`` with no harness in
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="codify", model=model, config={}),
    )
...
def test_sub_agent_infers_harness_and_forwards_os_env() -> None:
    """
    When a parent spec's sub-agent uses a native Codify v1 executor (no
    ...
    sub_spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="codify", model="databricks-claude-sonnet-4", config={}),
        os_env=sub_os_env,
    )
    parent_spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="codify", model="databricks-gpt-5-4", config={}),
...
def test_server_runtime_tool_with_no_path_rejected(
    ...
    :func:`codify.spec.validator.validate`, but the translator
    ...
    with pytest.raises(CodifyError, match=r"server-runtime tool has no"):
        agent_spec_to_agent_def(basic_spec)
