"""Tests for codify.spec.validator."""
...
def test_codify_executor_accepts_valid_harness() -> None:
    """
    ``codify`` executor with ``config.harness`` set to one of
...
def test_codify_executor_accepts_antigravity_native_harness() -> None:
    """
    ``codify`` executor with ``config.harness == "antigravity-native"``
    validates cleanly.

    Failure here means the antigravity-native harness is missing from
    ``CODIFY_HARNESSES``, which would cause every spec that targets it
...
def test_codify_executor_rejects_missing_harness() -> None:
    """
    ``codify`` executor without ``config.harness`` is rejected.

    Without the harness selector the codify factory cannot
...
def test_codify_executor_rejects_unknown_harness() -> None:
    """
    ``codify`` executor with a harness not in the allowed set
...
def test_codify_executor_rejects_compaction() -> None:
    """
    ``codify`` executor forbids ``compaction`` — the inner
...
def test_mcp_stdio_valid() -> None:
    ...
    spec/codify.py's _translate_mcp_tool_from_def) would fail
...
# programmatically — by tests, the codify compat shim, or any
