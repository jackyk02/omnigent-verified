"""
Round-trip invariant tests for the Codify ↔ AgentSpec
adapter.
...
:func:`codify.spec.codify.agent_spec_to_agent_def`,
...
codify-testing skill, skipped tests rot invisibly. The
...
from codify.spec.codify import (
    ...
    # ``codify/spec/codify.py`` per the design. Phase 1
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal codify YAML — name + prompt only. Round-trip
...
def function_tool_yaml(tmp_path: Path) -> Path:
    """
    Codify YAML with one function-type tool pointing at a
...
def _roundtrip(yaml_path: Path) -> None:
    """
    Load the YAML via codify' loader, translate to an
    ...
    **Lossy by design.** :class:`AgentSpec` does not model every
    codify :class:`FunctionTool` field — ``description``,
    ...
    :param yaml_path: Path to an codify YAML fixture.
    """
    ...
    # codify' CLI auto-pick) — the round-trip contract is
...
def test_roundtrip_hello_world_is_incomplete_for_codify(
    ...
    model, which Codify' strict spec rejects on the way
    back. This is intentional: the codify validator requires
    a harness when ``executor.type == "codify"``, and that
...
def test_roundtrip_executor_block(executor_block_yaml: Path) -> None:
    ...
    differs between the two directions — CodifyExecutor
