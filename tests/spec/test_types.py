"""
Unit tests for :mod:`codify.spec.types`.
...
from codify.spec.types import ExecutorSpec, RetryPolicy


@pytest.mark.parametrize(
    ("executor", "expected"),
    [
        # codify-type agents carry the kind in config.harness.
        (ExecutorSpec(type="codify", config={"harness": "codex"}), "codex"),
        (
            ExecutorSpec(type="codify", config={"harness": "claude-native"}),
            "claude-native",
        ),
        # Non-codify executors carry their kind in `type` and have no
...
def test_retry_policy_from_json_drops_unknown_keys_for_forwards_compat() -> None:
    ...
    Codify serializes it, an older harness wrap (still on the
...
def test_retry_policy_from_json_uses_defaults_for_missing_fields() -> None:
    ...
    Pin: backwards compat — an older Codify serializing a partial
...
def test_retry_policy_from_json_rejects_non_dict_payload() -> None:
    ...
    Pin: defensive against Codify regressions that might
...
def test_retry_policy_from_json_rejects_non_list_status_codes() -> None:
    ...
    Defensive: if Codify serialized it as a string or dict
