"""End-to-end regression test: ``codify codex --resume`` restores history.
...
guard/confirmation rather than a fix — it pins that ``codify codex
...
def test_codex_native_cli_resume_restores_history(
    ...
    Drives ``codify codex --server …`` (gateway routing via the
