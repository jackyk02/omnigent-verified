...
:data:`codify.runtime.harnesses._HARNESS_MODULES`. For each:
...
- The runner subprocess loads the wrap module
  (e.g. :mod:`codify.inner.claude_sdk_harness` or
  :mod:`codify.inner.codex_harness`).
...
hop, unlike the AP-level :mod:`codify.runtime.session_stream`
pub-sub).
...
async def test_harness_wrap_real_llm_smoke(
    ...
    try:
        ...
        # ``codify/runtime/harnesses/_scaffold.py``. The
