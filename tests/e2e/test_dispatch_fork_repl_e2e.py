"""
PTY-driven e2e: ``codify run --harness <harness>`` returns
...
the REPL's SSE rendering (event consumption from the Codify server
...
def repl_env(mock_llm_server_url: str, tmp_path: Path) -> dict[str, str]:
    ...
    :param tmp_path: Isolated temp directory for CODIFY_CONFIG_HOME.
    :returns: Env mapping for ``pexpect.spawn``.
    """
    from tests.e2e.codify._pexpect_harness import ensure_repl_test_theme_env
...
def test_repl_run_routes_harness_through_new_harness_contract(
    ...
    """
    Drive the full ``codify run --harness <harness>``
    ...
    2. It spawns a local Codify server subprocess.
    3. It uploads the spec via ``/api/agents``.
    4. The Codify server's ``_create_executor`` sees an
       ``executor.type == "codify"`` +
       ``config.harness == <harness>`` spec (after the
       codify-YAML translator runs) and dispatches to
...
def test_repl_pexpect_dependencies_are_present() -> None:
    ...
    # Sanity that ``codify`` is importable from this
