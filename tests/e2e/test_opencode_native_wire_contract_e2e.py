...
The opencode-native harness's HTTP+SSE client (``codify.opencode_native_client``)
is hand-shaped from the pinned OpenCode OpenAPI (vendored at
``codify/opencode/openapi-1.17.7.json``), so the rest of the suite exercises it
only against in-process fakes. This test boots a real ``opencode serve`` via the
PR's own :class:`~codify.opencode_native_app_server.OpenCodeNativeServer` and
...
----------------------------------------------------------
* **Opt-in only**: set ``CODIFY_E2E_OPENCODE_NATIVE=1`` and have a pinned
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        "set CODIFY_E2E_OPENCODE_NATIVE=1 (and `npm i -g opencode-ai@1.17.7`) to run"
    ),
)
...
async def test_opencode_native_wire_contract_against_real_server() -> None:
    ...
    try:
        ...
        try:
            ...
            session = await client.create_session({"title": "codify-e2e"})
