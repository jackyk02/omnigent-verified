"""End-to-end tests: ``codify cursor`` drives the native Cursor TUI.
...
harness: ``codify cursor`` launches the official ``cursor-agent`` TUI in a
runner-owned tmux pane, and each web-UI turn is injected into that pane
(bracketed paste + Enter) by
:class:`codify.inner.cursor_native_executor.CursorNativeExecutor`. The TUI's
own conversation store is tailed by
:mod:`codify.cursor_native_forwarder`, which mirrors ``cursor-agent``'s
replies back onto the Codify conversation as assistant items.

These tests drive the full stack the way a user does — spawn ``codify
...
----------------------------------------------------------
* **Opt-in only**: set ``CODIFY_E2E_CURSOR_NATIVE=1`` to run. Like the other
...
from codify.cursor_native_bridge import bridge_dir_for_session_id, kill_session
from tests.e2e._native_resume_helpers import (
    PtyHandle,
    cli_env,
    codify_console_script,
    inject_user_message,
...
pytestmark = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CURSOR_NATIVE") != "1"
    ...
    reason=(
        ...
        "`tmux` binary; set CODIFY_E2E_CURSOR_NATIVE=1 (and have `cursor-agent` "
...
def test_cursor_native_cli_smoke(
    ...
    Spawns a backgrounded ``codify cursor`` session, waits for its terminal
    ...
    omni = str(codify_console_script())
    ...
    try:
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify cursor` did not return marker {marker!r}. The "
...
def test_cursor_native_cli_runs_in_launch_cwd(
    ...
    """``codify cursor`` launches ``cursor-agent`` in the directory it was run from.

    Spawns a backgrounded ``codify cursor`` whose process cwd is a temp
    ...
    try:
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify cursor` did not return marker {marker!r} from "
...
def test_cursor_native_cli_resume_warns_when_terminal_was_killed(
    ...
    item #2. A second ``codify cursor --resume <conv>`` while the original
...
def test_cursor_native_cli_exposes_codify_mcp_tools(
    resume_test_server: str,
    tmp_path: Path,
    request: pytest.FixtureRequest,
) -> None:
    """``codify cursor`` wires Codify tools into Cursor's native MCP client.
    ...
    workspace-scoped ``codify`` server. This catches the regressions that made
    ...
    try:
        ...
        assert (bridge_dir / "tool_relay.json").is_file(), "Codify tool relay was not started"
        ...
        if cli_config_path.is_file():
            ...
            assert "Mcp(codify:sys_session_list)" in allow
            assert "Mcp(codify:sys_os_read)" in allow
        ...
        assert "codify" in listed.stdout
        assert "ready" in listed.stdout.lower()

        tools = subprocess.run(
            ["cursor-agent", "mcp", "list-tools", "codify"],
            cwd=pwd_dir,
            env=env,
            text=True,
            stdout=subprocess.PIPE,
...
def test_cursor_native_cli_mcp_can_call_sys_tool(
    ...
    """Cursor-native's generated Codify MCP server can call ``sys_*`` tools.

    Launches a real Cursor TUI, then calls the same generated ``.cursor/mcp.json``
    Codify relay that Cursor uses. The tool call is direct JSON-RPC over the
...
def test_cursor_native_cli_tool_approval_surfaced_as_elicitation(
    ...
    Unlike the other tests in this module, this launches ``codify cursor``
...
def test_cursor_native_cli_same_cwd_launch_does_not_duplicate(
    ...
    cursor keeps one chat per working directory, so two ``codify cursor``
