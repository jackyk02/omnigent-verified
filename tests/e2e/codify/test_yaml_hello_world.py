...
- Codify' YAML spec parser regresses on ``tools.*`` entries
...
def test_yaml_agent_with_tools(
    ...
    """
    Running ``codify run agent_with_tools.yaml --harness
    ...
    :param codify_repo_root: Cwd for the subprocess — the
    ...
    yaml_path = codify_repo_root / "tests" / "resources" / "examples" / "agent_with_tools.yaml"
...
def _strip_tool_chatter(stdout: str) -> str:
    ...
    The codify CLI prints ``◦ <tool>`` (queued) and
    ...
    :param stdout: Raw stdout from ``codify run``.
