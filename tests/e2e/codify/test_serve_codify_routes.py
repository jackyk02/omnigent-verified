"""Phase 3 integration-code test -- ``codify server --agent`` routing (mock LLM).
...
- The Codify mode dispatch site at ``_serve_agent`` stops calling into
  codify and falls back to the legacy ``create_app``.
- The shim's ``_codify_register_yaml_bundle`` stops registering
  the synthesized bundle with Codify' ``AgentStore``.
...
def _codify_serve_codify(
    *,
    ...
    """Spawn ``codify server --agent <yaml> --port <port>``."""
...
def _wait_for_health(
    ...
    *,
    ...
    """Poll Codify' ``/health`` until the server responds 200."""
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            ...
            raise AssertionError(
                f"codify server --agent exited early with code "
    ...
    pytest.fail(
        f"codify server --agent did not respond on /health within "
...
def _gather_codify_observations(port: int) -> dict[str, Any]:
    """Capture structural observations proving the server is codify."""
    with httpx.Client(
...
def test_serve_codify_routes_to_codify(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
) -> None:
    """
    ``codify server --agent <yaml>`` boots an codify server
    with the YAML pre-registered. No LLM calls are made.
    """
    port = _find_free_port()
    yaml_path = codify_repo_root.joinpath(*_YAML_RELPATH)
    with _codify_serve_codify(
        ...
        observed = _gather_codify_observations(port)

    diffs = compare_snapshot("test_serve_codify_routes", observed)
    assert diffs == [], (
        "Snapshot mismatch for codify server --agent routing:\n"
        + "\n".join(diffs)
        + f"\n\nObserved: {observed!r}"
    )
