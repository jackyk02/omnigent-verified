...
Runs ``codify run <spec> --harness antigravity ...`` as a real subprocess and
...
does NOT use ``patched_databrickscfg`` / ``codify_credentials_env``'s gateway
URL — it authenticates purely from the configured / ambient Gemini key. This
mirrors :mod:`tests.e2e.codify.test_per_harness_cursor` (the other
...
- ``google.antigravity`` importable in the Codify venv (the ``antigravity``
  extra — ``pip install 'codify[antigravity]'``).
- A Gemini / Antigravity API key configured (a stored ``antigravity:`` config
  block resolvable via
  :func:`codify.onboarding.antigravity_auth.antigravity_api_key_configured`,
...
- ``codify.cli``'s ``run`` one-shot / interactive paths stop persisting the
...
# Subprocess timeout per ``codify run`` invocation. The antigravity SDK boots
...
# ``prompt`` / ``executor``) — the form ``codify run <file>`` accepts without a
...
def _antigravity_skip_reason(codify_python: Path) -> str | None:
    ...
    Both prerequisites are probed in the *Codify venv* interpreter (the one the
    ...
    :param codify_python: Interpreter the ``codify`` subprocess will use.
    ...
    if probe.returncode != 0:
        ...
        return (
            "antigravity prerequisite probe failed in the Codify venv "
    ...
    if not have_sdk:
        return (
            ...
            "not importable in the Codify venv (install the 'antigravity' "
            "extra: pip install 'codify[antigravity]')."
        )
    if not have_key:
        return (
...
def _antigravity_env(base_env: dict[str, str], home: Path) -> dict[str, str]:
    ...
    Starts from the shared ``codify_credentials_env`` (so PATH, the onboarding
    suppression knobs, and the worktree ``PYTHONPATH`` propagate) but isolates
    ``$HOME`` and the Codify state/config roots into the test's temp dir, so
    the persistent conversation store this test reads (``$HOME/.codify/chat.db``)
    ...
    :param base_env: The ``codify_credentials_env`` fixture dict.
...
def _assistant_transcript_texts(db_path: Path) -> list[str]:
    ...
    :mod:`tests.e2e.codify.test_server_remote_codify_autonomous_flows`, but
    filtered to assistant-authored messages so the assertions can't be satisfied
    by the echoed user prompt.

    :param db_path: Path to ``$HOME/.codify/chat.db``.
...
def _run_one_shot(
    *,
    ...
    """Run a one-shot ``codify run <spec> --harness antigravity -p <prompt>``.

    Session-backed (no ``--no-session``) so the turn is persisted to
    ``$HOME/.codify/chat.db`` for transcript inspection and so a later
    ``--continue`` can thread onto it.

    :param codify_python: Interpreter from the ``codify_python`` fixture.
    :param codify_repo_root: Cwd for the subprocess (repo root on sys.path).
...
def _assert_clean_assistant_reply(
    ...
    *,
    ...
    :param result: The completed ``codify run`` process.
...
def test_per_harness_antigravity_smoke(
    ...
    The end-to-end smoke gate: one ``codify run --harness antigravity -p``
    ...
    :param codify_python: Interpreter with codify + the antigravity SDK.
    ...
    """
    skip_reason = _antigravity_skip_reason(codify_python)
    ...
    _assert_clean_assistant_reply(fake_home / ".codify" / "chat.db", result, label="smoke")
...
def test_per_harness_antigravity_model_selection(
    ...
    _assert_clean_assistant_reply(
        fake_home / ".codify" / "chat.db", result, label=f"model={model}"
    )
...
def test_per_harness_antigravity_multi_turn_history_retention(
    ...
    fake_home.mkdir()
    env = _antigravity_env(mock_credentials_env, fake_home)
    db_path = fake_home / ".codify" / "chat.db"
