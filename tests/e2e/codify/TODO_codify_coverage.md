# `--codify` example-YAML coverage: known gaps

Companion to `test_run_codify_example_agents.py` and
`test_run_codify_adapter_rejections.py`. Tracks gaps the e2e audit
...
Pure codify' CLI auto-picks a harness from the model's
prefix: `databricks-claude-*` → `claude-sdk`, `databricks-gpt-*`
→ `openai-agents`, etc. The `--codify` adapter's validator refused
specs without an explicit `harness:` field, so every YAML that
relied on pure-codify auto-selection failed at spec-load:

```
invalid agent spec synthesized from codify YAML:
  executor.config.harness: required when executor.type is
  'codify' — must be one of ['claude-sdk', 'codex',
  'openai-agents', 'pi']
```

Fix: added `_infer_harness_from_model` helper in
`codify.spec.codify` with a `_HARNESS_FOR_MODEL_PREFIX`
lookup. `_translate_executor_from_def` calls it as the final
fallback when no explicit or parent harness is present. The
codify CLI's `_apply_overrides_to_yaml` no longer injects a
...
### Fixed — Gap 7: Stock codify policy callables don't work under Codify

`resolve_function_policy` now detects legacy `(content, phase)` callables at
load time and wraps them in `_codify_legacy_shim` — a one-argument adapter
...
exists to protect any user-written codify-style policy callables that have
...
executor.type is 'codify' — must be one of [...]
```
...
installed. Orthogonal to `--codify`. Needs either an
...
works but it's non-obvious and authors following the codify
...
### Fixed — Gap 6: `--codify` hung when FunctionTools were registered on the top-level agent

**Root cause**: the adapter synthesizes `LocalToolInfo.language =
"codify-python-callable"` for every user FunctionTool /
...
`_build_codify_tool_schemas` but never **registered** on
Codify' `ToolManager`. When the harness's LLM invoked one,
...
dispatching, so the same guardrails Codify' native tool
...
- `examples/agent_with_tools.yaml` (xfail removed, passes
  end-to-end through `--codify` with the claude-sdk harness).
- `tests/e2e/codify/test_run_codify_policy_enforcement.py::test_policy_denies_tool_call_by_name`
  (real `codify run --codify -p "what is 6 + 6?"` subprocess
