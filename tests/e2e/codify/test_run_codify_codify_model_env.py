"""E2E coverage for the ``CODIFY_MODEL`` env-var fallback on ``codify run``.

The fallback fires in ``codify/chat.py:_apply_overrides_to_raw`` when the
...
#    ``codify.spec.validator._validate_executor_llm``; any other prefix
#    rejects the YAML before any FM API call happens.
# 2. ``databricks-gpt-`` routes through ``codify.llms.routing.infer_
...
_BOGUS_MODEL = "databricks-gpt-this-model-does-not-exist-codify-env-test-9f3a"

_PROMPT = "say hi in 5 words"
# Wall-clock budget for the subprocess. ``codify run`` spawns the
...
# ``test_codify_model_env_var_bogus_value_fails_with_named_error``).
...
def _run_codify_with_model_env(
    *,
    model_env_value: str,
    codify_python: Path,
    ...
    """
    Run ``codify run <minimal>.yaml -p "..."`` with ``CODIFY_MODEL`` set.
    ...
    because ``codify run`` spawns the AP server + runner as
    ...
    :param model_env_value: ``CODIFY_MODEL`` value (real or bogus).
    ...
    env["CODIFY_MODEL"] = model_env_value
...
def test_codify_model_env_var_drives_successful_run(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    ...
    """
    Smoke test: a valid model in ``CODIFY_MODEL`` produces a successful turn.
    ...
    :param codify_repo_root: Repo root (subprocess cwd).
    ...
    result = _run_codify_with_model_env(
    ...
    assert result.returncode == 0, (
        f"codify run with CODIFY_MODEL={_VALID_MODEL!r} exited "
...
def test_codify_model_env_var_bogus_value_fails_with_named_error(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Decisive test: a bogus ``CODIFY_MODEL`` fails with the bogus name in stderr.
    ...
    assert result.returncode != 0, (
        f"codify run with CODIFY_MODEL={_BOGUS_MODEL!r} unexpectedly "
