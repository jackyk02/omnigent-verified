...
# tests/e2e/codify/test_example_polly.py -> repo root is 3 parents up.
...
def test_orchestrator_executor(polly_spec: AgentSpec) -> None:
    ...
    configured via ``codify setup --no-internal-beta`` (Anthropic key,
...
def test_function_policies_have_nonempty_arguments(polly_spec: AgentSpec) -> None:
    ...
    (codify/policies/function.py). With empty ``arguments: {}`` the factory
