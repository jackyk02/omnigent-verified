...
def _check_worker_harness_available(harness: str, codify_python: Path) -> None:
    """
    Fail loud if the worker harness's prerequisites are missing.

    :param harness: The worker harness identifier under test.
    :param codify_python: The subprocess interpreter.
    """
...
def test_repl_overview_subagent_visibility(
    ...
    _check_worker_harness_available(harness, codify_python)
    ...
    yaml_path = codify_repo_root / "tests" / "resources" / "examples" / "coding_supervisor.yaml"
