"""E2E coverage for the ``codify host`` Ctrl+C stop-server prompt.

``codify host ""`` (local mode) spawns a *detached* background local
AP server (:func:`codify.host.local_server.ensure_local_codify_server`) that
...
3. When connect *reuses* a server it did not spawn (one already brought up
   by a prior daemon, via the real ``ensure_local_codify_server`` path), Ctrl+C
   shows NO prompt and leaves that server running — connect must never
   offer to stop a server it didn't start.

The detached server is the genuine production object (a real ``codify
...
# raw time.sleep — see codify-testing rule 13).
...
def _connect_env(base_env: Mapping[str, str], home: Path) -> dict[str, str]:
    ...
    db land under the per-test directory (``ensure_local_codify_server`` keys its
    data dir off ``~/.codify`` when ``CODIFY_DATA_DIR`` is unset),
...
def _spawn_connect(
    ...
    """
    Spawn ``codify host ""`` (local mode) under a real PTY.

    The empty positional argument selects local mode — connect spawns (or
    reuses) the detached local Codify server and connects the foreground daemon
    to it. Databricks auth comes from the env (the ``--profile`` flag was
    removed from the codify CLI).
...
def _read_local_server_record(home: Path) -> tuple[int, int]:
    ...
    :returns: ``(pid, port)`` recorded by ``ensure_local_codify_server``.
    :raises AssertionError: If the pidfile is missing or malformed.
    """
    pid_path = home / ".codify" / "local_server.pid"
    try:
...
def _prespawn_persistent_server(
    ...
    Invokes the real :func:`ensure_local_codify_server` in a short-lived
    ...
    code = (
        ...
        "print(ensure_local_codify_server().url)"
    )
    proc = subprocess.run(
        [str(codify_python), "-c", code],
        env=dict(env),
        cwd=str(repo_root),
        capture_output=True,
        text=True,
...
def test_host_ctrl_c_yes_stops_local_server(
    ...
    :param codify_repo_root: Repo root fixture (subprocess cwd).
    ...
    child = _spawn_connect(codify_python, codify_repo_root, env)
    server_pid = -1
    try:
        ...
        # The prompt's success line proves stop_local_codify_server() was invoked.
...
def test_host_ctrl_c_reused_server_shows_no_prompt(
    ...
    A server is brought up first via the real ``ensure_local_codify_server`` path
    ...
    server_pid, port = _prespawn_persistent_server(codify_python, codify_repo_root, env)
