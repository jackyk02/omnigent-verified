...
Runs ``codify run hello_world.yaml --harness pi --model
...
- Codify' ``PiExecutor`` regresses (the ``pi --mode rpc``
  ...
  Codify tools with ``pi.registerTool()``).
...
def test_per_harness_pi_one_shot(
    ...
    """
    ``codify run hello_world.yaml --harness pi -p <prompt>``
