...
``test_example_<name>.py`` file under ``tests/e2e/codify/``.

The set of agent roots scanned here is kept in lock-step with the
resolution order in
``tests/e2e/codify/_example_helpers.py::example_yaml_path`` — the
...
def _is_agent_yaml(path: Path) -> bool:
    ...
    ``codify server --config`` file (only ``policies:``), not an
...
_ALT_COVERED: frozenset[str] = frozenset(
    {
        ...
        # under tests/e2e/codify/test_run_codify_* etc.
        "hello_world",
        ...
        "coder",
        # Covered by tests/e2e/codify/test_run_codify_coding_supervisor.py
        # (seven test functions).
        "coding_supervisor",
        # Covered by tests/e2e/test_openai_coder_*.py.
        "openai-coder",
        # Covered by tests/e2e/codify/test_repl_overview_terminal_visibility.py.
        "terminal_workers",
        ...
        "coding-supervisor",
        "compaction-test",
        # Test-only fixtures added with CODIFY_TERMINAL_BRIDGE (commits
        ...
        # exercised in tests/runtime/policies/test_example_codify_yamls.py.
        "risk_score_agent",
        # info_flow_agent: the built-in gdrive information-flow policy
        # (Bell-LaPadula + Biba) is exercised in
        # tests/runtime/policies/test_example_codify_yamls.py.
        "info_flow_agent",
        ...
        # tests/e2e/codify/test_per_harness_qwen.py (skipped without a
...
def test_every_agent_has_a_dedicated_test_file() -> None:
    ...
    e2e_dir = repo_root / "tests" / "e2e" / "codify"
    ...
    assert missing == set(), (
        ...
        f"Create tests/e2e/codify/test_example_<name>.py for "
