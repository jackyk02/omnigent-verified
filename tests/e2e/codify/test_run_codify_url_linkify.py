...
What can break this without breaking the lower layers:
  ...
  - The Codify mode SSE bridge translates tool-result events into
...
def test_run_codify_url_linkify_emits_osc_8_in_pty(
    codify_python: Path,
    ...
    1. Spawn ``codify run --codify`` via pexpect.
    ...
    :param codify_python: Path to the worktree's
        ``.venv/bin/python``.
    :param codify_repo_root: Repo root used as cwd so the
        subprocess imports this worktree's ``codify``
    ...
    # Short top-level tmpdir so the Codify server's per-run
    # subdirs (incl. tmux socket paths if the agent ever
    # touched one) stay under macOS's 104-char Unix-socket
    # limit. Same shape as test_run_codify_terminal_idle.py.
    ...
    try:
        ...
        # On failure, leave the tmpdir intact so the Codify server
        # log is inspectable.
...
def _assert_url_was_linkified(captured: str) -> None:
    ...
    assert bare_count == 0, (
        ...
        f"``sdks/ui/codify_ui_sdk/terminal/_host.py`` for "
