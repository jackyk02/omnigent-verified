...
``codify`` subprocesses under pexpect and exercise the user-visible
...
import pexpect
import pytest
from codify_client import CodifyClient, SessionsChat
...
def _stop_host_daemon(home: Path) -> None:
    ...
    ``codify run --server`` leaves the daemon alive after the REPL exits
...
class _ServerHandle:
    """
    Live standalone Codify server used by ``--server`` e2e tests.
...
def _repl_env(
    ...
    env["CODIFY_SESSIONS_ADAPTER_DEBUG"] = "1"
    ...
    env["CODIFY_REMOTE_AUTH_TOKEN"] = _LOCAL_REMOTE_AUTH_TOKEN
...
def _spawn_run(
    ...
    *,
    ...
    """
    Spawn ``codify run`` under a real PTY.

    :param codify_python: Python interpreter with Codify installed.
    :param repo_root: Checkout root used as subprocess cwd.
    :param yaml_path: Agent YAML path.
    :param env: Subprocess environment.
    :param server_url: Optional Codify server URL for ``--server`` mode.
...
def _session_runner_id(base_url: str, session_id: str) -> str:
    ...
    :param base_url: Codify server URL.
...
def _drive_turn(
    ...
    * **``--server``/daemon** (``base_url`` set): the session is
    ...
    :param base_url: Codify server URL for the ``--server`` flow;
...
def _runner_pid_from_daemon_log(home: Path, runner_id: str) -> int:
    ...
    when it spawns a runner (codify/host/connect.py). Reading the pid
...
def _server_entrypoint() -> str:
    """
    Return a Python entrypoint for a remote-style Codify server.
...
from codify.cli import _create_artifact_store
...
db_uri = os.environ["CODIFY_E2E_DB_URI"]
artifact_location = Path(os.environ["CODIFY_E2E_ARTIFACT_LOCATION"])
port = int(os.environ["CODIFY_E2E_PORT"])
...
def _running_server(
    ...
    """
    Run a remote-style Codify server for ``--server`` tests.
    ...
    server_env = {
        **env,
        "CODIFY_E2E_DB_URI": f"sqlite:///{db_path}",
        "CODIFY_E2E_ARTIFACT_LOCATION": str(artifacts),
        "CODIFY_E2E_PORT": str(port),
    }
...
def _registered_runner(
    ...
    *,
    ...
    """
    from codify.cli import _start_cli_runner_process, _stop_cli_runner_process
...
def test_repl_full_session_lifecycle(
    ...
    :param codify_python: Python interpreter fixture.
    :param codify_repo_root: Repository root fixture.
...
def test_repl_recover_after_runner_death(
    ...
    try:
        with _running_server(codify_python, codify_repo_root, env, tmp_path) as server:
            child = _spawn_run(
                codify_python,
                codify_repo_root,
                yaml_path,
...
async def test_repl_reasoning_effort_threads_through(
    ...
    with _running_server(codify_python, codify_repo_root, env, tmp_path) as server:
        ...
        with _registered_runner(
            ...
            async with CodifyClient(base_url=server.base_url) as client:
