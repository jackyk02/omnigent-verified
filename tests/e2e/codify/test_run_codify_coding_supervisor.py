"""End-to-end test: ``codify run examples/coding_supervisor.yaml
--codify`` works.

Exercises the full pipeline that was failing in user-reported bugs:

1. Codify YAML with ``async: true``, ``cancellable: true``, and
...
2. Non-interactive ``run -p <prompt>`` path — POSTs to
   ``/v1/responses`` on an in-process codify server.
3. The bidirectional translator: YAML → AgentDef →
   :func:`agent_def_to_agent_spec` → AgentSpec → (registered with
   codify server) → :func:`agent_spec_to_agent_def` (inside
   :class:`CodifyExecutor.from_spec`) → AgentDef → codify
...
- Someone reintroduces a fail-loud for ``AgentTool`` in the
  ...
  sub-agent-as-tool declarations stop loading under Codify mode.
- ``_run_agent_via_codify`` regresses to the "requires a prompt"
  hard-error path — interactive ``codify run <yaml>``
...
- The
  :func:`codify.spec.codify._sub_spec_to_agent_tool`
  reverse translation stops emitting ``AgentTool`` entries —
  :class:`CodifyExecutor.from_spec` reconstructs an
...
from tests.e2e.codify.conftest import configure_mock_llm, reset_mock_llm
...
# Codify mode boot (FastAPI + uvicorn + DBOS + alembic) completes
...
# Cold-boot of ``coding_supervisor.yaml`` under Codify mode —
# spawns the in-process Codify server and registers supervisor +
# two sub-agents. Without Codify mode boot is <10s; with Codify mode
# the in-process FastAPI + uvicorn + DBOS + alembic stack adds
# ~30-60s on a cold DBOS db. 120s keeps the test from flaking
# on cold starts. (Genuine regressions in Codify mode boot would
...
_EXPECTED_ROOT_ENTRIES = ("openapi.json", "codify", "pyproject.toml")


def test_run_codify_coding_supervisor_oneshot(
    codify_python: Path,
    ...
    """
    ``codify run examples/coding_supervisor.yaml -p ...``
    ...
    :param codify_python: Interpreter with both codify and
        codify installed (from the shared conftest).
    :param codify_repo_root: Codify repo root. Used as cwd
    ...
    yaml_path = codify_repo_root / _YAML_PATH_REL
    ...
    result = subprocess.run(
        [
            ...
            # test_run_codify_example_agents.py for the
    ...
    assert result.returncode == 0, (
        f"`codify run --codify` exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1000:]}"
    )
...
def test_run_codify_coding_supervisor_exposes_subagent_tools(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    ``codex_worker``) AND at least one codify task-lifecycle
    builtin (``check_task``) — proves :class:`CodifyExecutor`
    advertises both surfaces to the inner codify harness.
    ...
    :param codify_python: Interpreter with codify +
        codify installed.
    :param codify_repo_root: Codify repo root.
    :param mock_credentials_env: Mock-LLM env vars pointing at the
        mock server.
    :param mock_llm_server_url: Mock server URL for configuring
        response queues.
...
def test_run_codify_coding_supervisor_spawns_codex_worker_to_list_files(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: ``codify run`` on
    coding_supervisor.yaml boots the Codify stack, the mock
    ...
    is that the Codify boot path, sub-agent tool registration,
    ...
    :param codify_python: Shared session fixture pointing at
        the repo's ``.venv`` Python.
    :param codify_repo_root: Codify repo root — used as
    ...
    configure_mock_llm(
        mock_llm_server_url,
        [{"text": "openapi.json codify pyproject.toml"}],
    )
    ...
    for entry in _EXPECTED_ROOT_ENTRIES:
        assert (codify_repo_root / entry).exists(), (
            f"Expected filesystem anchor {entry!r} missing from "
            f"{codify_repo_root}. Update _EXPECTED_ROOT_ENTRIES "
...
def test_run_codify_coding_supervisor_interactive_enters_repl(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
) -> None:
    """
    ``codify run examples/coding_supervisor.yaml`` (no
    ...
    :func:`_run_chat_via_codify` so interactive semantics match the
    legacy ``codify run`` no-prompt behavior.

    :param codify_python: Interpreter with codify +
        codify installed.
    :param codify_repo_root: Cwd for the subprocess.
    :param mock_credentials_env: Mock-LLM env vars pointing at the
        mock server.
    """
    yaml_path = codify_repo_root / _YAML_PATH_REL
    ...
    # Use the shared ``spawn_codify_run`` harness so PTY
    ...
    try:
        ...
        assert index == 0, (
            f"`codify run --codify` (no prompt) did not reach the "
...
@pytest.mark.flaky(reruns=2, reruns_delay=5)
def test_run_codify_coding_supervisor_codex_shell_not_disabled(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: under Codify mode, the codex
    ...
    validates is that the infrastructure plumbing (Codify mode
    ...
    ``shell_tool`` whenever any tools were passed. Under Codify
    mode, :class:`CodifyExecutor` always injects codify
    ...
    :param codify_python: Interpreter with codify +
        codify installed.
    :param codify_repo_root: Codify repo root — also the
    ...
    fixture_path = codify_repo_root / _CODEX_REGRESSION_TODO_NAME
    ...
    assert _CODEX_NONEXISTENT_ERROR not in combined, (
        ...
        f"codify/codex_executor.py.\n"
        f"stdout tail:\n{result.stdout[-2500:]}\n"
        f"stderr tail:\n{result.stderr[-1500:]}"
    )
    assert result.returncode == 0, (
        f"--codify exited {result.returncode}. stderr tail:\n"
