...
flow under ``codify run``.

Reproduces the user-reported scenario verbatim:

1. Spawn ``codify run examples/rate_limited_search_agent.yaml``
...
def test_run_codify_rate_limit_approval_round_trip(
    codify_python: Path,
    ...
    :param codify_python: Path to the worktree's
        ``.venv/bin/python``.
    :param codify_repo_root: Repo root the subprocess uses as
    ...
    try:
        ...
        # :func:`codify.repl._repl._make_elicitation_prompt`'s
