"""Fixtures for Codify e2e tests (mock LLM).
...
# Root of the Codify checkout that ships the ``codify``
# package, the example YAMLs, and (in the main checkout) the
# ``.venv`` with codify + pexpect + openai-agents installed.
#
# Derived from the conftest's own location so git worktrees work
# naturally: this file lives at
# ``<root>/tests/e2e/codify/conftest.py`` (post-unification), so
...
# exec the main-checkout ``codify`` (via the editable install),
# missing any per-worktree edits.
_CODIFY_REPO = Path(__file__).resolve().parents[3]
    ...
    while True:
        ...
        if current.parent == current:
            ...
            raise RuntimeError(
                ...
                f"{_CODIFY_REPO} — worktrees share the main "
...
_CODIFY_VENV_PYTHON = _resolve_venv_python()


@pytest.fixture(scope="session")
def codify_python() -> Path:
    """
    Path to the Python interpreter that has the ``codify``
    package + its harness dependencies installed.

    The Codify repo ships its own ``.venv`` with
    ``codify``, ``pexpect``, ``openai-agents``,
    ``claude-agent-sdk``, etc. pre-installed. Agent-plane's e2e
    tests use that interpreter directly rather than adding
    codify as an codify dep (codify is not
    distributed as a package yet).

    :returns: Absolute path to the Codify ``.venv`` Python
        interpreter, e.g.
        ``"/path/to/codify/.venv/bin/python"``.
    :raises RuntimeError: If the interpreter is not present at
        the expected path — indicates the Codify checkout is
        missing or its .venv hasn't been created.
    """
    if not _CODIFY_VENV_PYTHON.is_file():
        raise RuntimeError(
            f"Codify venv python not found at {_CODIFY_VENV_PYTHON}. "
            f"These e2e tests require the sibling checkout at "
            f"{_CODIFY_REPO} with .venv set up."
        )
    return _CODIFY_VENV_PYTHON


@pytest.fixture(scope="session")
def codify_repo_root() -> Path:
    """
    Root of the Codify checkout used as the subprocess cwd.

    Codify YAMLs reference example tool modules via dotted
    ...
    :returns: Absolute path to the Codify repo root, e.g.
        ``"/path/to/codify"``.
    """
    return _CODIFY_REPO
...
def mock_credentials_env(
    ...
    config_home = tmp_path_factory.mktemp("codify-mock-e2e-config")
    (config_home / "config.yaml").write_text(
        "auth:\n  type: api_key\n",
        encoding="utf-8",
    )
    env["CODIFY_CONFIG_HOME"] = str(config_home)
    repo = str(_CODIFY_REPO)
    codify_path = str(_CODIFY_REPO / "codify")
    existing_pp = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = os.pathsep.join(p for p in (repo, codify_path, existing_pp) if p)
    return env
...
def mock_llm_server_url(
    ...
    proc = subprocess.Popen(
        [
            sys.executable,
            str(_CODIFY_REPO / "tests" / "server" / "integration" / "mock_llm_server.py"),
            str(mock_port),
        ],
        env={**os.environ, "PYTHONPATH": str(_CODIFY_REPO)},
        stdout=log_handle,
