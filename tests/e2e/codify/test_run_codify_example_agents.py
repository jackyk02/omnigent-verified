"""
End-to-end: every example YAML the Codify adapter is
...
The companion file
``test_run_codify_adapter_rejections.py`` covers YAMLs the adapter
...
- The Codify mode CLI shim loses a dispatch for a harness.
...
LLM nondeterminism) are tracked in ``TODO_codify_coverage.md`` in
...
_CASES = [
    ...
    pytest.param(
        ...
        ("pyproject.toml", "README.md", "codify", "examples"),
...
def test_run_codify_example_yaml(
    codify_python: Path,
    ...
    :param success_markers: Any-one-of substrings that MUST
        ...
        reply traversed the full Codify mode stack. The mock
    ...
    args = [
        ...
        # ``~/.codify/chat.db`` and DBOS may attempt to
        ...
        # ``HarnessProcessManager not initialized — Codify lifespan
    ...
    assert result.returncode == 0, (
        f"{yaml_rel}: --codify exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1500:]}"
    )
