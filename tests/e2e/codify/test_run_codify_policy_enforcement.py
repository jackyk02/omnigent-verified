"""
End-to-end proof that policies declared in an codify YAML
are enforced by the codify workflow under Codify mode.

The adapter in :mod:`codify.spec.codify` lifts the
YAML's ``policies:`` block into
:attr:`AgentSpec.guardrails.policies`; the codify runtime
...
the legacy codify ``(content, phase)`` callable signature
(``examples.tool_functions.block_long_sleep`` et al.), which
Codify' :class:`FunctionPolicy` dispatcher can't invoke
(it passes ``(ctx, context)`` where ``ctx`` is an
:class:`EvaluationContext` dataclass, not a dict). This test
uses the codify-shaped
``codify._e2e_policy_callables.block_on_sentinel``
callable — an arity-1 callable matching Codify'
convention — so the test proves the translator + engine
integration works and isn't muddied by a separate callable-
portability gap. That gap is tracked in ``TODO_codify_coverage.md``.
...
- The runtime stops reading ``guardrails`` from specs
  synthesized via the codify adapter.
...
# in ``codify/_e2e_policy_callables.py`` DENYs on. The token
...
# The standard DENY sentinel text the codify workflow
# stamps into the response when a policy returns DENY. See
# :func:`codify.runtime.workflow._build_deny_sentinel` —
...
def policy_enforcement_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an codify-shaped YAML registering one
    ...
    (``block_on_sentinel``) is already on ``codify`` and
    matches the codify FunctionPolicy calling convention.
        ...
        config = {
            ...
            "policies": {
                "block_sentinel_input": {
                    "type": "function",
                    "on": ["request"],
                    "handler": ("codify._e2e_policy_callables.block_on_sentinel"),
                },
            },
        }
...
def test_policy_denies_input_containing_sentinel(
    ...
    :param codify_python: Shared interpreter fixture.
    :param codify_repo_root: Subprocess cwd — the YAML's
    ...
    assert result.returncode == 0, (
        f"--codify exited {result.returncode} before reaching "
...
def tool_ban_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an codify-shaped YAML declaring one
    ...
    - ``CodifyExecutor._make_tool_executor_bridge``
        ...
        config = {
            ...
            "policies": {
                "deny_calculate_tool": {
                    ...
                    "function": {
                        "path": "codify.policies.function.make_fixed_action_callable",
                        "arguments": {
                            "action": "deny",
...
def test_policy_denies_tool_call_by_name(
    ...
    """
    ``codify run <yaml> -p "<arithmetic prompt>"``
    ...
    1. The translator expanded ``on: [tool_call] + match_tools:
       ...
       name.
    2. ``CodifyExecutor._make_tool_executor_bridge``
    ...
    - The CodifyExecutor bridge stopped calling
    ...
    :param codify_python: Shared interpreter fixture.
    :param codify_repo_root: Subprocess cwd — conftest's
