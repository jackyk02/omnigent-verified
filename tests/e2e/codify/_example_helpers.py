...
``codify run`` subprocess. The helpers in this module keep the
...
#   args: [-m, codify.inner.databricks_mcps.<X>, --profile,
#          <profile-name>]
# in their YAMLs. The Databricks MCP subprocess has three auth
# modes (see codify/inner/databricks_mcps/common/workspace.py):
...
#   CODIFY_E2E_MCP_PROFILE=<your-profile> pytest ...
#
# Usage — mode 1 (direct PAT, no profile needed):
#   CODIFY_E2E_MCP_HOST=https://... \
#   CODIFY_E2E_MCP_TOKEN=dapi... \
#   pytest ...
#
# When none of the three vars are set, tests fall back to
# structural validation (spec + AgentDef translation).
ENV_MCP_PROFILE = "CODIFY_E2E_MCP_PROFILE"
ENV_MCP_HOST = "CODIFY_E2E_MCP_HOST"
ENV_MCP_TOKEN = "CODIFY_E2E_MCP_TOKEN"
...
def example_yaml_path(repo_root: Path, name: str) -> Path:
    ...
    :param repo_root: Unified repo root; typically the
        ``codify_repo_root`` fixture value.
...
def run_one_shot(
    *,
    ...
    codify_credentials_env: dict[str, str],
    example_name: str,
    ...
    """
    Invoke ``codify run <yaml> -p <prompt>`` one-shot.

    :param codify_python: Interpreter with codify + required
        SDKs installed. Provided by the ``codify_python`` fixture.
    :param codify_repo_root: Cwd so the YAML's ``callable:``
        dotted paths resolve via repo-root-on-sys.path. Provided by
        the ``codify_repo_root`` fixture.
    :param codify_credentials_env: PAT + BASE_URL + profile env
        populated from ``--llm-api-key``. Provided by the
        ``codify_credentials_env`` fixture.
    ...
    :returns: The completed subprocess. Caller decides which
        fields to assert.
    """
    yaml_path = example_yaml_path(codify_repo_root, example_name)
    return run_one_shot_at_path(
        codify_python=codify_python,
        codify_repo_root=codify_repo_root,
        codify_credentials_env=codify_credentials_env,
        yaml_path=yaml_path,
        prompt=prompt,
        harness=harness,
        model=model,
...
def run_one_shot_at_path(
    *,
    ...
    :param codify_repo_root: Cwd (repo root so module lookups
        land).
    :param codify_credentials_env: Env.
...
def validate_agent_def_structure(
    *,
    ...
    :param codify_python: Interpreter with codify installed.
    :param codify_repo_root: Cwd so module-path resolution
        matches the CLI.
...
import json
import sys
sys.path.insert(0, {str(codify_repo_root)!r})
from codify.inner.loader import load_agent_def_from_path
...
def assert_completed_one_shot(
    ...
    """
    Assert a one-shot ``codify run`` finished cleanly.
    ...
    """
    assert result.returncode == 0, (
        f"{example_name!r}: codify run exited with "
...
def mcp_auth_override() -> McpAuthOverride:
    ...
    If ``CODIFY_E2E_MCP_PROFILE`` is set, returns a
    profile-based override. Else if BOTH ``CODIFY_E2E_MCP_HOST``
    and ``CODIFY_E2E_MCP_TOKEN`` are set, returns a PAT-based
...
def _rewrite_mcp_auth_in_place(yaml_obj: object, override: McpAuthOverride) -> int:
    ...
    every ``-m codify.inner.databricks_mcps.*`` subprocess.
...
def _rewrite_args_list(args: list[object], override: McpAuthOverride) -> int:
    ...
    """
    if not any(isinstance(x, str) and x.startswith("codify.inner.databricks_mcps.") for x in args):
        return 0
...
def materialize_yaml_with_mcp_auth(
    ...
    Returns the path to pass to ``codify run``:
