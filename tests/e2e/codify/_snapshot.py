"""Snapshot loader + comparator for Codify Phase 0 characterization tests.

Snapshots are JSON files under ``tests/e2e/codify/snapshots/``
and capture the *structural* observations a test makes about a
live Codify run. They are the golden-master contract the
Phase 0 design requires: written against current Codify,
...
def load_snapshot(test_name: str) -> dict[str, SnapshotField]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            f"their snapshots captured against current Codify."
        )
