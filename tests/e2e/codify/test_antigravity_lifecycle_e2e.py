...
``test_per_harness_cursor.py``): it drives real ``codify run`` subprocesses
...
1. **Parallel turns / no state bleed** — two concurrent ``codify run``
   ...
   :class:`~codify.inner.antigravity_executor.AntigravityExecutor`) does not
...
2. **No orphaned ``localharness``** — snapshot the live ``localharness`` PIDs
   ...
   native binary).
3. **Session cleanup reaps the runner/harness** — after a single ``codify
...
- ``google-antigravity`` importable in the *codify* venv (the test shells
...
- A Gemini API key resolvable — either a configured ``antigravity:`` block
  (``codify setup``) or an ambient ``GEMINI_API_KEY`` / ``ANTIGRAVITY_API_KEY``.
  ...
  / ``codify_credentials_env`` — a missing key is a clean SKIP so the e2e
...
- The ``codify run`` one-shot path stops tearing down its local server /
...
# After a clean ``codify run`` exit, the local server/runner shutdown that
...
def _antigravity_prereqs_missing(codify_python: Path) -> str | None:
    """Return a skip reason if the antigravity harness can't run, else ``None``.

    Probes the *codify* venv (the interpreter the subprocess uses), not the
    test's own interpreter: the SDK import and the key resolution both have to
    hold for the spawned ``codify run`` to do anything. Booleans only — the
    key is never printed.

    :param codify_python: Interpreter with codify installed, from the
    ...
    probe = subprocess.run(
        ...
        cwd=str(codify_python.parent),
    )
    out = probe.stdout.strip().split()
    if probe.returncode != 0 or len(out) != 2:
        return (
            "could not probe antigravity prerequisites in the codify venv "
    ...
    if not key_ok:
        return (
            ...
            "configure an 'antigravity:' key via 'codify setup' or export "
...
def _write_antigravity_bundle(bundle_dir: Path, *, name: str) -> Path:
    ...
    :returns: The bundle directory path (the argument ``codify run`` takes).
    """
...
def _antigravity_env() -> dict[str, str]:
    """Build the subprocess env for an antigravity ``codify run``.
    ...
    """
    env = dict(os.environ)
    env["CODIFY_SKIP_ONBOARD"] = "1"
    env["CODIFY_NO_UPDATE_CHECK"] = "1"
    return env
...
def _run_antigravity_turn(
    *,
    ...
    """Run one ``codify run <bundle> --harness antigravity -p <prompt>``.
    ...
    :param codify_python: Interpreter with codify installed.
    :param codify_repo_root: Cwd for the subprocess (puts example tool modules
        on sys.path, matching the other run_codify tests).
...
@pytest.fixture
def antigravity_runnable(codify_python: Path) -> None:
    """Skip the whole module's tests when antigravity can't run.

    :param codify_python: Interpreter probed for the SDK + key.
    :raises pytest.skip.Exception: When a prerequisite is missing.
    """
    reason = _antigravity_prereqs_missing(codify_python)
...
def test_antigravity_parallel_turns_no_state_bleed(
    ...
    :param codify_python: Interpreter with codify + google-antigravity.
    :param codify_repo_root: Cwd for the subprocesses.
...
def test_antigravity_session_cleanup_reaps_runner(
    ...
    A single ``codify run`` turn owns a self-contained process tree (local
