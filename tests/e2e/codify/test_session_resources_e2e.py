"""E2E test -- Session Resources API against ``codify server`` (mock LLM).

Boots ``codify server --agent <yaml>`` as a subprocess and
...
def _codify_server(
    *,
    ...
    """Spawn ``codify server --agent <agent_path>`` and a sibling runner.

    :param python: Interpreter with codify installed.
    ...
    runner_proc = subprocess.Popen(
        [str(python), "-m", "codify.runner._entry"],
        env={
            **env,
...
def test_session_resources_e2e(
    ...
    with _codify_server(
        ...
        with httpx.Client(
            ...
            # Codify REPL terminal (``terminal_tui_main``) on bind;
