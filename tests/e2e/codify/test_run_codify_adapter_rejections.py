"""
End-to-end: codify example YAMLs that declare unsupported
concepts MUST fail loud at spec-load time under Codify mode.

The adapter in :mod:`codify.spec.codify` rejects several
concepts it cannot faithfully translate into an codify
:class:`AgentSpec` (see
:func:`_reject_unsupported_concepts_def`):

- ``policies`` — label-based + function-type policies have no
  codify parity yet.
- MCP-type tools — codify' subprocess MCP transport has no
  codify equivalent (codify uses HTTP/SSE MCP only).
...
``codify run <yaml>`` exits non-zero with an error
...
# has shrunk over time as the Codify translator has grown:
#
# - **Policies**: lifted into ``AgentSpec.guardrails.policies``
#   and enforced by the workflow layer (see
#   ``test_run_codify_policy_enforcement.py``). No longer rejected.
...
def test_run_codify_rejects_unsupported_yaml(
    codify_python: Path,
    ...
    """
    ``codify run <yaml> -p ...`` exits non-zero and
    ...
    :param codify_repo_root: Subprocess cwd.
    :param codify_credentials_env: Env with PAT + profile.
    :param yaml_rel: Path under *codify_repo_root* to the
        example YAML to load.
    :param expected_error: Substring the adapter's
        :class:`CodifyError` message MUST contain — the
        specific field name that tripped the rejection.
    """
    yaml_path = codify_repo_root / yaml_rel
    ...
    assert result.returncode != 0, (
        f"--codify on {yaml_rel} exited 0 but should have rejected the "
    ...
    assert expected_error in combined, (
        ...
        f"--codify rejection output. The adapter may have raised a less-"
        f"specific error than the CodifyError in "
