"""End-to-end tests for ``codify run`` conversation resumption.
...
LLM in run #2, proving that the persistent codify store at
``$HOME/.codify/chat.db`` carries history between invocations.

**What breaks if this fails:**

- The persistent store path regresses in
  ``codify.chat._codify_persistent_dir`` or
  ``codify.inner.cli._build_codify_stores`` — e.g. someone
  flips back to ``mkdtemp`` and ``--continue`` silently
  starts a fresh conversation.
- Idempotent agent registration regresses
  (``_codify_register_yaml_bundle``) — the second subprocess
...
``designs/RUN_CODIFY_SESSION_RESUMPTION.md`` feature. If it
...
# Subprocess timeout per ``codify run`` invocation.
# 180s matches the existing run_codify tests' headroom for DBOS
...
def _argv_run_codify(
    *,
    ...
    """
    Build the ``codify run`` argv for a one-shot ``-p`` invocation.

    Use for plant steps (no resumption flags). Resume steps need
    :func:`_argv_run_codify_interactive` instead, since the CLI rejects
    ``--continue`` / ``--resume`` combined with ``-p``.

    :param codify_python: Interpreter from the
        ``codify_python`` fixture.
    :param codify_repo_root: Repo root from the
        ``codify_repo_root`` fixture.
...
def _argv_run_codify_interactive(
    *,
    codify_python: Path,
    codify_repo_root: Path,
    extra_flags: list[str],
    ...
    """
    Build the ``codify run`` argv for the interactive REPL.
    ...
    :param codify_python: Interpreter from the fixture.
    :param codify_repo_root: Repo root from the fixture.
...
def _daemon_log_tails(home: Path, *, tail_chars: int = 3000) -> str:
    ...
    Each ``codify run`` subprocess spawns its own local server, host
    daemon, and runner whose logs land under ``$HOME/.codify/logs/``
...
def _isolated_env(
    ...
    """
    Override ``HOME`` and the explicit Codify state/config roots so
    ...
    developer's real ``~/.codify/chat.db`` and could
    ...
    env = dict(base_env)
    env["HOME"] = str(home)
    env["CODIFY_CONFIG_HOME"] = str(home / ".codify")
    env["CODIFY_DATA_DIR"] = str(home / ".codify")
    return env


def test_run_codify_continue_carries_history_across_invocations(
    codify_python: Path,
    codify_repo_root: Path,
...
def test_run_codify_continue_with_no_prior_conversation_exits_nonzero(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
...
def test_run_codify_continue_works_across_oneshot_and_interactive_paths(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    (``test_run_codify_continue_carries_history_across_invocations``)
    couldn't catch it because both subprocesses there go
    through ``_codify_register_yaml_bundle``.
    ...
    interactive_argv = [
        ...
        "codify",
        "run",
        str(codify_repo_root / "tests" / "resources" / "examples" / "hello_world.yaml"),
        "--model",
        _MODEL,
        "--harness",
        _HARNESS,
...
def test_run_codify_session_id_pins_the_specific_conversation(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    *exact* conversation specified, not the most-recent one.
    ...
    # column is a SMALLINT; see codify.db.enum_codecs.CONVERSATION_KIND).
    with sqlite3.connect(str(persistent_db)) as conn:
        rows = conn.execute(
            "SELECT c.id FROM conversations c "
            "JOIN codify_conversation_metadata m "
...
def test_run_codify_session_id_unknown_exits_nonzero(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
...
def test_run_codify_no_session_does_not_pollute_persistent_store(
    codify_python: Path,
    codify_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
    ...
    the persistent ``$HOME/.codify/chat.db`` must NOT
    ...
    # ``_codify_persistent_dir`` regardless of
    ...
    assert not persistent_db.exists(), (
        ...
        f"Either the ephemeral branch in _build_codify_stores "
        f"is no longer being honored, or _codify_persistent_dir "
