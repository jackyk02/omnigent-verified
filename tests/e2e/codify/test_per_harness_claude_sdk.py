...
Runs ``codify run hello_world.yaml --harness claude-sdk -p
...
- Codify' ``ClaudeSDKExecutor`` regresses (auth, MCP tool
...
- The Claude Agent SDK dependency or the ``claude`` CLI binary
  goes missing from the Codify venv.
...
def claude_sdk_available(codify_python: Path) -> bool:
    ...
    *codify* venv — the test's own venv is irrelevant because
    ...
    :param codify_python: The interpreter the subprocess
...
def test_per_harness_claude_sdk_one_shot(
    ...
    """
    ``codify run hello_world.yaml --harness claude-sdk -p
