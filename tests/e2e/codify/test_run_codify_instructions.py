...
- ``codify/inner/loader.py::_resolve_instructions`` regresses.
- ``codify/spec/codify.py::agent_def_to_agent_spec`` stops
  preferring ``AgentDef.instructions`` over ``AgentDef.prompt``.
- ``codify/spec/_codify_compat.py::is_codify_yaml``
...
def test_instructions_path_field_loaded_via_codify_run_codify(
    codify_python: Path,
    ...
    ``codify run`` and the agent starts successfully.
...
"""
    ...
    assert result.returncode == 0, (
        f"codify run exited {result.returncode}. stdout={result.stdout!r} stderr={result.stderr!r}"
    )
