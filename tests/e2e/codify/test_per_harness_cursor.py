...
Runs ``codify run hello_world.yaml --harness cursor -p "..."`` as a real
...
``patched_databrickscfg`` / ``codify_credentials_env``. Because a Cursor API
...
- The ``cursor-sdk`` API contract changes (``AsyncAgent`` / ``AsyncClient`` /
  ``run.messages()`` shape).
- ``codify.cli`` for the ``-p`` one-shot path stops printing assistant text
...
def test_per_harness_cursor_one_shot(
    ...
    """``codify run hello_world.yaml --harness cursor -p <prompt>`` works.

    :param codify_python: Interpreter with codify installed and importable.
    :param codify_repo_root: Cwd for the subprocess so the YAML spec and
