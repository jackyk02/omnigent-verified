...
Runs ``codify run hello_world.yaml --harness openai-agents
...
- Codify' ``OpenAIAgentsSDKExecutor`` regresses (the Runner
  lifecycle, the Responses-API adapter in
  ``codify.open_responses_sdk``, the MCP tool bridging, or
  the event stream translation to ``ExecutorEvent`` types).
- The ``openai-agents`` Python package (``agents`` module) is
  missing from the codify venv or its public API changes
  incompatibly.
- ``codify.cli._run_agent`` for the ``-p`` one-shot path
...
@pytest.fixture
def openai_agents_available(codify_python: Path) -> bool:
    """
    ...
    *codify* venv (the subprocess interpreter) — the current
    pytest interpreter is irrelevant because the test shells out.

    :param codify_python: Interpreter the subprocess will
...
def test_per_harness_openai_agents_sdk_one_shot(
    ...
    """
    ``codify run hello_world.yaml --harness openai-agents -p
    ...
    :param openai_agents_available: True when the ``agents``
        package is importable in the codify venv. On False
    ...
    if not openai_agents_available:
        pytest.skip(
            ...
            "installed in the Codify venv. Skipping — package absent."
        )
