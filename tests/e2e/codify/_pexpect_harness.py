"""Reusable pexpect helpers for driving the Codify REPL.

The Codify REPL is a prompt-toolkit app that renders a full
...
# every pexpect boot.
_TEST_THEME_CONFIG = "# Codify user configuration\ntui:\n  theme: light\n"
...
def ensure_repl_test_theme_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    ``$HOME/.codify/config.yaml`` has no ``tui.theme`` entry.
    ...
    config_path = home / ".codify" / "config.yaml"
...
def spawn_codify_run(
    ...
    *,
    ...
    """
    Spawn ``codify run`` under a PTY for REPL tests.

    :param codify_python: Python interpreter with codify
        installed, e.g.
        ``Path("/Users/.../codify/.venv/bin/python")``.
    :param yaml_path: Absolute path to the agent YAML, e.g.
        ``examples/hello_world.yaml``. Pass ``None`` to exercise
        ``codify run --harness ...`` without an explicit
        agent argument.
    ...
    :param env: Environment dict for the subprocess — supplied
        by the ``codify_credentials_env`` fixture so PAT
        and base URL propagate.
    :param cwd: Working directory for the subprocess. Must be
        the Codify repo root so YAML ``callable:`` entries
    ...
    if yaml_path is None:
        ...
        # ``python -m codify``. The package ``__main__`` module
        # intentionally routes through the legacy argparse quick-chat
        # CLI, while the installed ``codify`` script invokes the
        ...
        command = str(codify_python.parent / "codify")
    else:
        args = [
            "-m",
            "codify",
        ...
        command = str(codify_python)
    if system_prompt is not None:
        args.extend(["--system-prompt", system_prompt])
    if initial_prompt is not None:
        args.extend(["-p", initial_prompt])
    # NOTE: the codify CLI no longer accepts ``--profile``; Databricks
    # routing for spawned CLIs comes from the ``auth:`` block written into
    # the isolated ``CODIFY_CONFIG_HOME`` by ``codify_credentials_env``.
