"""Live E2E: ``codify run`` calls a stdio MCP tool.

Proves the full path landed by the stdio-MCP work:

    YAML  →  codify Codify translator
          ...
          stdio, through the harness, through the Codify mode reply
...
- The Codify translator silently drops the
  :class:`~codify.inner.tools.MCPTool`: the spec loads, but
  the codify runtime never registers the tool, the LLM
  never calls it, and the ``echo: <probe>`` fingerprint is
  absent. The unit test
  (``test_codify_adapter.py::test_load_mcp_stdio_yaml_
...
# existing Codify mode example-agent tests use 240s; shorter here
...
def _write_stdio_mcp_yaml(tmp_path: Path, repo_root: Path) -> Path:
    """
    Materialize an codify agent YAML that declares the
    ...
    :param repo_root: Codify repo root — used to resolve the
    ...
    agent = {
        "name": "echo_codify_stdio_mcp_test",
        "prompt": (
    ...
    yaml_path = tmp_path / "echo_codify_stdio.yaml"
    yaml_path.write_text(yaml.dump(agent))
    return yaml_path


def test_codify_stdio_mcp_tool_roundtrip(tmp_path: Path) -> None:
    """
    Run ``codify run <yaml>`` with a stdio MCP; the
    ...
    # ``codify_repo_root`` session fixture in conftest.py
    ...
    # same ``openai-agents`` / ``codify`` install the test is
    ...
    # Ditto for Databricks env vars the Codify mode shim might
    ...
    # mistake the ask for one of Codify' auto-registered
    ...
    # Codify path has historically swallowed a few of these
    # (see designs/CODIFY_INTEGRATION.md gap log), so assert
    # up front.
    forbidden = (
    ...
    for marker in forbidden:
        assert marker not in combined, (
            f"Forbidden marker {marker!r} in output — a --codify "
            f"failure mode fired. stderr tail:\n{result.stderr[-2000:]}"
        )

    assert result.returncode == 0, (
        f"--codify exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-2000:]}"
    )
