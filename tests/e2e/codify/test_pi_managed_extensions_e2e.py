...
This test drives a real ``codify run --harness pi`` subprocess with:

- an isolated ``HOME`` carrying ``~/.pi/agent/settings.json`` + a marker
  extension;
- a mock OpenAI provider in ``CODIFY_CONFIG_HOME`` so pi enters gateway mode
...
``tests/e2e/codify/`` pi rows — do not run under xdist against a shared mock.
"""
...
_MARKER_NAME = "codify-pi-ext-marker"
...
def test_pi_gateway_run_loads_global_extensions(
    ...
    """
    ``codify run --harness pi`` in gateway mode loads extensions from the
    user's global Pi agent settings.

    The marker extension writes ``~/codify-pi-ext-marker`` on
