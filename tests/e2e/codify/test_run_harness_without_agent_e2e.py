"""Live REPL e2e for ``codify run --harness`` without AGENT.

Migrated to use the mock LLM server. This test drives the user-facing
launcher shape::

    codify run --harness <harness> -p <prompt>
...
def test_run_harness_without_agent_live_repl_round_trip(
    ...
    """``codify run --harness`` boots and answers via each wrapped harness.
    ...
    :param codify_repo_root: Working directory for the subprocess.
...
def test_run_harness_live_matrix_covers_registered_coding_harnesses() -> None:
    ...
    ``CODIFY_HARNESSES`` also contains ``open-responses`` for the
    ...
    ``codify run --harness <native>``. (``opencode-native`` is a
    ...
    (tmux pane + bridge dir), not ``codify run --harness goose-native``.

    ``antigravity-native`` is excluded for the union of both reasons above: it
    is a terminal-first TUI launched via ``codify antigravity`` (runner-owned
    agy tmux pane + bridge dir), not ``codify run --harness antigravity-native``,
    ...
    not ``codify run --harness qwen-native``. Its coverage is the dedicated
    ...
    ``omni kiro`` (tmux pane + bridge dir), not ``codify run --harness
    ...
    ``omni kimi`` (tmux pane + bridge dir), not ``codify run --harness
    ...
    ``codify run --harness hermes-native``, AND it wraps the ``hermes`` CLI
    binary. Its coverage is the dedicated hermes-native bridge/executor/forwarder/
    approval-mirror unit tests.
    """
    expected_live_harnesses = set(CODIFY_HARNESSES).intersection(_HARNESS_MODULES) - {
        "acp",
        "claude-native",
        "codex-native",
        "pi-native",
