...
def _bare_env(home: Path, codify_repo_root: Path) -> dict[str, str]:
    """
    Build a minimal subprocess env that doesn't need LLM credentials.

    :param home: Directory to use as ``$HOME``.
    :param codify_repo_root: Worktree root, prepended onto
        ``PYTHONPATH``.
    :returns: Env dict suitable for ``subprocess.run(env=...)``.
    """
    existing_pp = os.environ.get("PYTHONPATH", "")
    pythonpath = os.pathsep.join(p for p in (str(codify_repo_root), existing_pp) if p)
    return {
        "HOME": str(home),
...
def _run_codify(
    *,
    ...
    """Spawn ``python -m codify <args>`` with the given env."""
    return subprocess.run(
        [str(codify_python), "-m", "codify", *args],
        env=env,
...
def test_global_config_write_then_list_roundtrips(
    ...
    home = tmp_path / "home"
    home.mkdir()
    env = _bare_env(home, codify_repo_root)
...
def test_global_config_default_agent_drives_bare_codify(
    codify_python: Path,
    codify_repo_root: Path,
    ...
    assert run.returncode == 0, (
        f"bare ``codify run`` with global default_agent failed: "
