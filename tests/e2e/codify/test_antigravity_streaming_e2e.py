...
``/v1/sessions`` flow — register an inline Codify agent
...
:mod:`codify.inner.antigravity_executor`: ``conversation.send`` →
...
from codify.onboarding.antigravity_auth import antigravity_api_key_configured
...
def _antigravity_skip_reason() -> str | None:
    ...
    if importlib.util.find_spec("google.antigravity") is None:
        return (
            ...
            "is not importable. Install it with: pip install 'codify[antigravity]'."
        )
...
def _write_antigravity_agent_yaml(tmp_path: Path, *, prompt: str) -> Path:
    """Write a minimal single-file ``harness: antigravity`` Codify bundle.
...
def _assistant_text_from_items(http_client: httpx.Client, session_id: str) -> str:
    ...
    (see ``codify/server/API.md`` § List Conversation Items).
