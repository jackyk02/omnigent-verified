...
Runs ``codify run hello_world.yaml --harness claude-sdk -p <prompt>``
...
Run with:
    ...
    pytest tests/e2e/codify/test_claude_harness_alias_e2e.py -v --profile oss
"""
...
def _clean_env(profile: str) -> dict[str, str]:
    ...
    # The codify CLI no longer accepts ``--profile``; write the
    # supported replacement — an ``auth:`` block in an isolated
    # ``CODIFY_CONFIG_HOME`` — so the spawned CLI routes the
    # claude-sdk harness through this Databricks profile.
    config_home = Path(tempfile.mkdtemp(prefix="codify-alias-config-"))
...
def test_run_with_claude_alias_produces_output(
    ...
    """``codify run --harness claude-sdk`` exits 0 with assistant text.

    Proves the "claude" alias is canonicalized through the full
    CLI → Codify server → harness spawn → LLM call → output path.
