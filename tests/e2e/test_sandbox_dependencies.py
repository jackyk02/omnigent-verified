...
def test_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from codify')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from codify."},
        ],
        key=model,
    )
    ...
    assert "hello from codify" in combined, (
        f"Expected cowsay ASCII art with 'hello from codify' "
...
def test_uv_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    "PYTHONPATH=./_sandbox_uv_cowsay python3 -c "(
                                        "\"import cowsay; cowsay.cow('hello from codify via uv')\""
                                    )
                                ),
                            }
                        ),
            ...
            {"text": "Here is the cowsay output with hello from codify via uv."},
        ],
        key=model,
    )
    ...
    all_output = _tool_call_output_text(body)
    combined = (text + " " + all_output).lower()
    assert "hello from codify via uv" in combined, (
        f"Expected cowsay output with 'hello from codify via uv' -- proves "
