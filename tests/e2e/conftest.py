...
def _enforce_min_server_version(request: pytest.FixtureRequest) -> None:
    ...
    (``CODIFY_COMPAT_SERVER_VERSION`` set). The latter makes the
    ...
    compat_pinned = os.environ.get("CODIFY_COMPAT_SERVER_VERSION")
...
def _enforce_min_runner_version(request: pytest.FixtureRequest) -> None:
    ...
    ``codify.runner._entry`` / ``codify.host._daemon_entry`` subprocesses to
    an older build and sets ``CODIFY_COMPAT_RUNNER_VERSION``. The runner/host
...
# CODIFY_BUILTIN_AGENT_DIRS hook) so fork-switch e2e tests have a
...
def live_server(
    ...
    """
    Start a real ``codify server`` subprocess and yield its base URL.
    ...
    env = {
        **os.environ,
        ...
        "CODIFY_BUILTIN_AGENT_DIRS": str(builtin_sdk_chat_spec),
    }
    ...
    # happening silently — each e2e run polluted ``codify.db``
    ...
    proc = subprocess.Popen(
        ...
        # Compat mode: neutral CWD so the worktree codify/ doesn't shadow
    ...
    runner_proc = subprocess.Popen(
        [runner_executable(), "-m", "codify.runner._entry"],
        env=runner_env,
...
def register_inline_agent(
    ...
    *,
    ...
    """
    Register a single-file codify agent built in-memory.
...
def _materialize_builtin_sdk_chat_spec(
    ...
    *,
    ...
    ``CODIFY_BUILTIN_AGENT_DIRS``, which reads the spec verbatim — it
...
def sys_terminal_test_agent(
    ...
    """
    Upload the ``sys-terminal-test`` agent (codify-flavored
...
def send_user_message_to_session(
    ...
    *,
    ...
    but runner-native sessions do not create Codify DBOS task rows for
...
def poll_session_until_terminal(
    ...
    *,
    ...
    pollable Codify ``Task`` for ``GET /v1/responses/{response_id}``. This
...
def poll_for_pending_tool_calls(
    ...
    *,
    ...
    runner-native session turns, which do not create Codify DBOS task rows
...
def resume_test_server(
    ...
    """
    Spawn a real ``codify server`` that accepts the CLI's own runner.

    Used by the native-CLI resume e2e tests
    (``test_claude_native_cli_resume_e2e`` / ``test_codex_native_cli_resume_e2e``),
    which drive the real ``codify claude/codex --server`` CLI. It differs
    from :func:`live_server` in two ways, both required for that:

    * **No tunnel-token allow-list.** ``CODIFY_RUNNER_TUNNEL_TOKEN``
    ...
    * **Accounts auth.** The server defaults to accounts mode, which
    ...
    env.pop("CODIFY_RUNNER_TUNNEL_TOKEN", None)
