...
This test proves that fix works against a **real** ``codify server``
...
The only thing simulated is the managed-sandbox *condition* — no ``codify
...
def accounts_server(tmp_path: Path) -> Iterator[tuple[str, str]]:
    """Run a real ``codify server`` subprocess with accounts auth enabled.

    Accounts mode is selected by ``CODIFY_AUTH_PROVIDER=accounts`` plus a
    ...
    env = {**os.environ}
    env["CODIFY_AUTH_PROVIDER"] = "accounts"
    env["CODIFY_ACCOUNTS_COOKIE_SECRET"] = _COOKIE_SECRET_HEX
    env["CODIFY_ACCOUNTS_BASE_URL"] = base_url
    ...
    env.pop("CODIFY_OIDC_ISSUER", None)
...
def test_managed_runner_callback_authenticates_end_to_end(
    ...
    #    its binding token and the server URL — no codify-login token, no
