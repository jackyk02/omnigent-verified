...
starts Claude Code in a private tmux window with the Codify MCP
...
``codify claude`` materialises at runtime — not a custom yaml.
...
# Agent name written into the spec by ``codify claude``.
...
_RUN_GATE_ENV = "CODIFY_E2E_CLAUDE_NATIVE_COMMENTS"
...
def claude_native_ui_agent(
    ...
    The spec is identical to what ``codify claude`` materialises via
...
def _claude_code_session(
    ...
    *,
    ...
    """
    Start Claude Code in a private tmux window with the Codify MCP bridge.
    ...
    codify relay tools so MCP permission dialogs don't block the
    ...
    This mirrors what ``codify claude`` does when a user runs it,
    ...
    tmp = tempfile.mkdtemp(prefix="codify-e2e-")
    tmux_socket = Path(tmp) / "tmux.sock"
    tmux_session = f"cne-{session_id[:8]}"
    tmux_target = f"{tmux_session}:0.0"

    # Build Claude Code args with Codify MCP bridge and hooks injected.
    # --allowedTools pre-authorizes the codify relay tools so Claude does
    # not show an interactive permission dialog when it calls list_comments or
    # update_comment.  The MCP server is always named "codify" (_MCP_SERVER_NAME),
    # so the tool identifiers are stable regardless of session.
    base_args: tuple[str, ...] = (
        "--dangerously-skip-permissions",
        "--allowedTools",
        "mcp__codify__list_comments,mcp__codify__update_comment",
    )
    ...
    # how ``codify claude`` unsets ANTHROPIC_API_KEY on launch.
...
def test_claude_native_agent_addresses_comments_without_tool_guidance(
    ...
    Flow:
    1. Skip unless opted in via ``CODIFY_E2E_CLAUDE_NATIVE_COMMENTS`` and
       the ``claude`` / ``tmux`` binaries are available.
    2. Create a runner-bound session with the ``claude-native-ui`` agent.
    3. Start Claude Code in a private tmux window with the Codify
       MCP bridge (same as ``codify claude`` does).
    ...
    **What breaks if this test fails:**
    ...
    - ``_execute_comment_tool`` call path broken (server_client / session
      scoping), so the Codify server ``PATCH /v1/sessions/{id}/comments`` is
      never hit.
    ...
    # the relay, so comment status changes reach the Codify server even
    ...
    with _claude_code_session(session_id, model=None, launch_env=launch_env):
        ...
        # and hit the Codify server directly, so the comment status changes appear
        ...
        assert post_statuses.get(comment1_id) == "addressed", (
            ...
            f"If 'draft', the update_comment relay call did not reach the Codify server."
        )
