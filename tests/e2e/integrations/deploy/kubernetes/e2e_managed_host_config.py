...
Runs against an EXISTING codify server already configured with
...
config landed at ``/home/codify/.codify/config.yaml`` before the host
...
# codify/onboarding/sandboxes/kubernetes.py): the Pod's fixed HOME, the
# main container and host-id env names, and the labels stamped on every runner Pod.
POD_HOME = "/home/codify"
HOST_CONTAINER = "host"
HOST_ID_ENV_VAR = "CODIFY_HOST_ID"
POD_SELECTOR = "app.kubernetes.io/managed-by=codify,codify.ai/role=sandbox-host"
...
def assert_injected_config(
    ...
    log(f"[4/5] reading {POD_HOME}/.codify/config.yaml from {pod}")
    proc = subprocess.run(
        [
            *shlex.split(kubectl),
            "exec",
...
def main() -> int:
    ...
    parser.add_argument("--server", required=True, help="Codify server base URL")
    parser.add_argument("--agent-id", default=None, help="Agent to bind (default: first)")
    parser.add_argument("--namespace", default="codify-sandboxes", help="Runner-Pod namespace")
    parser.add_argument(
        "--expect",
        action="append",
        default=None,
