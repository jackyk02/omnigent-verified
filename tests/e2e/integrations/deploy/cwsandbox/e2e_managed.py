#!/usr/bin/env python3
"""
End-to-end test: create a managed session against an Codify server, and have
...
Two modes:
  ...
  1. --server <url>: use an EXISTING codify server (already configured with
     ...
     serverless users who can't expose a public service.
         ...
         python tests/e2e/integrations/deploy/cwsandbox/e2e_managed.py \
             --server http://my-codify:6767

  2. (default) spin the server up inside a CW Sandbox with a public service. The
     sandbox runs a prebaked image with this fork's codify + the cwsandbox SDK
     (build/push first; pass --image), and the driver injects the LLM creds.
         ...
         python tests/e2e/integrations/deploy/cwsandbox/e2e_managed.py \
             --image docker.io/<you>/codify-cwsandbox:test
"""
...
def _child_env(wandb_key: str) -> dict[str, str]:
    ...
    CODIFY_RUNNER_ENV_PASSTHROUGH. The config's `sandbox.cwsandbox.env` name
...
def configure_and_start_server(sb: Sandbox, server_url: str, wandb_key: str) -> None:
    """Write config + an openai-agents agent, then launch `codify server`."""
    log(f"[2/6] starting codify server (server_url={server_url})")
    ...
    start = (
        f"CODIFY_CONFIG_HOME={CONFIG_HOME} CODIFY_LOCAL_SINGLE_USER=1 "
        f"setsid nohup codify server --host 0.0.0.0 --port {SERVER_PORT} "
        f"--config {CONFIG_HOME}/config.yaml --no-open --agent /root/e2e-agent "
        "> /tmp/codify-server.log 2>&1 < /dev/null & echo started"
    )
...
def _dump_server_logs(sb: Sandbox | None) -> None:
    ...
    out = sb.exec(
        [
            "bash",
            "-lc",
            "tail -50 ~/.codify/logs/cli/cli-*.log 2>/dev/null; "
            "echo '--- stdout ---'; tail -15 /tmp/codify-server.log",
        ]
    ).result()
    log(out.stdout)


def _codify_children() -> list:
    """List sandboxes the launcher tags 'codify' (managed hosts); [] on error."""
    try:
        return Sandbox.list(tags=["codify"]).result()
...
def dump_child_logs(exclude: set[str]) -> None:
    ...
    # belonging to other runs / deployments that share the 'codify' tag.
    children = [c for c in _codify_children() if c.sandbox_id not in exclude]
    ...
    try:
        out = target.exec(
            [
                "bash",
                "-lc",
                "tail -60 ~/.codify/logs/*.log 2>/dev/null; echo '--- host log ---'; "
                "tail -40 /tmp/codify-host.log 2>/dev/null",
            ]
        ).result()
        log(out.stdout)
    except Exception as exc:
...
def main() -> int:
    ...
    parser.add_argument(
        "--server",
        default=None,
        help="Use an EXISTING codify server at this URL (e.g. http://host:6767) "
    ...
    parser.add_argument(
        "--image",
        default=None,
        help="Prebaked codify+cwsandbox image (required only when spinning up the server).",
    )
    parser.add_argument(
        "--agent-id",
        default=None,
