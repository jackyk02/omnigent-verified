...
Drives the REAL :class:`~codify.onboarding.sandboxes.e2b.E2BSandboxLauncher`
...
pre-built codify host template — it validates the launcher's SDK wiring
in isolation. Pass ``--template codify-host`` (after ``e2b template
build``; see deploy/e2b/README.md) to smoke the real host template too.

    pip install 'codify[e2b]'
...
# codify package) isn't importable rather than a raw traceback.
try:
    ...
    print("Run from the repo root with codify installed.", file=sys.stderr)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "'codify-host' to smoke the real host template once built).",
    )
    ...
    marker_name = "CODIFY_E2B_SMOKE_MARKER"
    ...
    try:
        ...
        payload = b"e2b-codify-smoke\x00\x01binary\n"
