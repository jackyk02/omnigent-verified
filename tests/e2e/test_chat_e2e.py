"""E2E test for ``codify chat`` — local mode with mock LLM.

Verifies that ``codify chat ./agent-dir/`` starts a server, opens the
...
def test_chat_local_starts_server_and_agent_responds(
    ...
    """
    ``codify chat ./agent-dir/`` starts a local server with the agent
    and the agent can respond to messages.

    Tests the server startup and agent registration path used by
    ``codify chat`` in local mode. Since the REPL itself is interactive,
...
def test_chat_local_accepts_codify_yaml_file(
    mock_llm_server_url: str | None,
    tmp_path: Path,
) -> None:
    """
    ``codify chat examples/coding_supervisor.yaml`` (or any
    standalone codify YAML) now starts the local server and
    ...
    round-trips through :func:`codify.spec.load` to a
    ...
    **What breaks if this fails:**
    ...
    - ``materialize_bundle``'s file branch produces the wrong
      dir shape and ``_find_codify_yaml_in_dir`` misses the
      YAML.
    - Agent-plane's spec dispatch stops routing codify YAMLs
      through ``load_codify_yaml``.
    ...
    # Inline fixture: minimal codify YAML with the openai-agents harness
