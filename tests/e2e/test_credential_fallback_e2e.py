...
``codify run`` (server → runner → openai-agents harness) credentials the head
...
def local_server(tmp_path: Path, mock_llm_server_url: str) -> Iterator[str]:
    """
    Spawn a throwaway in-tree ``codify server`` (state only).
...
def _fallback_run_env(mock_llm_server_url: str, config_home: Path) -> dict[str, str]:
    """
    Build the ``codify run`` env: no ambient credentials, and an isolated
    ...
    :param config_home: Isolated ``CODIFY_CONFIG_HOME`` (also used as HOME so
...
def test_runner_fallback_credentials_head_with_nondefault_provider(
    ...
    provider, with no ambient credential — end-to-end via ``codify run``
    ...
    config_home = Path(tempfile.mkdtemp(prefix="codify-fallback-cfg-"))
    ...
    assert result.returncode == 0, (
        f"codify run failed (exit {result.returncode}) — the head was not "
