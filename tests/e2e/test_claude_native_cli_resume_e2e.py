"""End-to-end regression test: ``codify claude --resume`` restores history.

Reproduces the user-reported bug: running

    codify claude --server <url> --resume <conv_id>
...
This is the robust, outcome-based check: it drives the REAL ``codify
...
def test_claude_native_cli_resume_restores_history(
    ...
    """
    Cross-context ``codify claude --resume`` restores conversation history.

    Drives the real ``codify claude --server …`` CLI (gateway routing
    ...
    and must go through Codify' cold-resume *synthesis* (rebuild the
    ...
    that only exercises Claude resuming its own file, not Codify' code, and
