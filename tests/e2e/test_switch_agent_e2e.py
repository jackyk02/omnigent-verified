...
an SDK target replays the Codify transcript as context, so a code word planted
...
def test_switch_agent_in_place_carries_history(
    ...
    new agent can only answer from the replayed Codify transcript — a
...
def _upload_single_file_agent_with_os_env(
    ...
    *,
    ...
    """Upload a single-file codify agent that declares an ``os_env``.
