...
This test runs a REAL Codify server subprocess against a REAL SQLite
...
1. The store is constructed in a separate process from the
   test (matches the user's ``codify run`` setup
   where the Codify server is a subprocess of the REPL).
2. The store's session factory + busy_timeout pragmas come
   from the production code path
   (:func:`codify.db.utils.make_managed_session_maker`),
...
- Spins up an Codify server subprocess (:fixture:`ap_server`).
- Imports the Codify server's actual ``SqlAlchemyConversationStore``
...
def _find_free_port() -> int:
    """Pick a free TCP port for the Codify subprocess to bind."""
    s = socket.socket()
    s.bind(("", 0))
...
def test_concurrent_appends_against_live_codify_server_db_no_collision(
    ap_server_with_shared_db: tuple[str, str],
) -> None:
    """
    With a live Codify server running, fire N concurrent appends
    ...
    - The lock-escalation fix in
      ...
      (the Codify server). On revert, the test surfaces the user's
    ...
    Note: this test does NOT exercise the Codify route layer (the
    ...
    # this gets a separate engine in this process (the Codify server
    ...
    assert errors == [], (
        f"Concurrent appends against the live Codify server's SQLite "
