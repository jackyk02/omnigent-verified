...
``codify run <yaml>`` under pexpect and drives approval CUJs
...
def _build_repl_env(mock_llm_server_url: str, tmp_home: Path) -> dict[str, str]:
    ...
    ``codify run`` subprocess uses mock responses.
    """
...
def _spawn_sessions_repl(
    ...
    *,
    ...
    """Spawn ``codify run`` under a PTY (sessions API is default)."""
...
def _spawn_repl_with_args(
    ...
    *,
    ...
    """Spawn ``codify run`` with caller-supplied CLI args."""
    args = [
        "-m",
        "codify",
        "run",
