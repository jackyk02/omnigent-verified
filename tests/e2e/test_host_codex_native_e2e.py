...
from codify.native_coding_agents import CODEX_NATIVE_AGENT_NAME
...
def _spawn_host_daemon(
    *,
    ...
    """
    Spawn an ``codify host`` daemon for the codex-native test.
...
def test_codex_native_builtin_session_can_be_created(
    ...
    assert labels.get("codify.wrapper") == "codex-native-ui", (
        f"Expected wrapper label 'codex-native-ui', got {labels.get('codify.wrapper')!r}"
    )


@pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1" or shutil.which("codex") is None,
    reason=(
        "codex-native round-trip e2e needs `codex` on PATH and CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
...
@pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1" or shutil.which("codex") is None,
    reason=(
        "codex-native subagent e2e needs `codex` on PATH and CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
def test_codex_native_spawn_creates_child_session(
    ...
    """A real Codex native spawn appears as an Codify child session."""
    ...
    try:
        ...
        assert labels.get("codify.codex_native.subagent_thread_id")
        _poll_for_assistant_marker(
            http_client,
            session_id=child_id,
            marker=child_marker,
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
...
@pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1"
...
def test_codex_native_worktree_session_runs_in_worktree(
    ...
    worked because it reads ``CODIFY_RUNNER_WORKSPACE`` directly.
...
@pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1" or shutil.which("codex") is None,
    reason=(
        "codex-native workspace e2e needs `codex` on PATH and CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
...
@pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1" or shutil.which("codex") is None,
    reason=(
        "codex-native image-routing e2e needs `codex` on PATH and CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
...
def test_codex_native_web_model_effort_override_survives_turn(
    ...
    made in the Codify web picker reaches the runner as
    ...
    *honored*, not rejected.
    ...
    profile; set ``CODIFY_E2E_CODEX_SWITCH_MODEL`` to a different valid
    ...
    try:
        ...
        target_model = os.environ.get("CODIFY_E2E_CODEX_SWITCH_MODEL") or current_model
        assert target_model, (
