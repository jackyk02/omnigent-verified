"""End-to-end test: ``codify codex`` runs the agent in the launch cwd.

The CLI sibling of the host worktree/workspace cwd tests in
``test_host_codex_native_e2e.py``. The ``codify codex`` wrapper launches
...
``CODIFY_RUNNER_WORKSPACE`` (the wrapper's launch cwd), never the
spec-bundle extraction dir.

Environment requirements (why this is opt-in, not pure-CI)
----------------------------------------------------------
* **Opt-in only**: set ``CODIFY_E2E_CODEX_NATIVE=1`` to run. codex-native
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        "CODIFY_E2E_CODEX_NATIVE=1 (and have `codex` installed + logged in) to run"
    ),
)
...
def test_codex_native_cli_runs_in_launch_cwd(
    ...
    """
    ``codify codex`` launches the agent in the directory it was run from.

    Spawns a backgrounded ``codify codex`` session whose process cwd is a
    ...
    i.e. ``CODIFY_RUNNER_WORKSPACE`` / the session workspace, not the
    bundle dir.
    ...
    # ``--profile`` was removed from the codify CLI; cli_env(profile=…)
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify codex` did not return marker {marker!r} from "
