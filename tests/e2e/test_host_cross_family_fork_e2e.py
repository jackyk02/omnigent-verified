...
Claude transcript from the fork's copied Codify items
...
the fork route must SKIP the ``codify.fork.source_external_session_id``
...
SDK→native test needs only the Claude side. Set the matching gates::
    ...
    CODIFY_E2E_CLAUDE_NATIVE=1 CODIFY_E2E_CODEX_NATIVE=1 \\
...
_BOTH_NATIVE_GATE = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CLAUDE_NATIVE") != "1"
    or os.environ.get("CODIFY_E2E_CODEX_NATIVE") != "1"
    ...
    reason=(
        ...
        "set CODIFY_E2E_CLAUDE_NATIVE=1 and CODIFY_E2E_CODEX_NATIVE=1 to run"
    ),
)
_CLAUDE_NATIVE_GATE = pytest.mark.skipif(
    ...
    reason=(
        ...
        "set CODIFY_E2E_CLAUDE_NATIVE=1 (and have `claude` installed + logged in) to run"
    ),
)
...
def _recall_marker_in_clone(
    ...
    *,
    ...
    The recall only succeeds if the source's Codify items were rebuilt
    ...
    assert marker in text, (
        ...
        f"{source_harness} source's Codify items were not rebuilt into the "
...
def test_fork_codex_native_into_claude_native_rebuilds_history(
    ...
    Claude transcript from the copied Codify items, then ``--resume`` it.
...
def test_fork_openai_sdk_source_into_claude_native_rebuilds_history(
    ...
    clone's Claude transcript from the copied Codify items and resume it.
