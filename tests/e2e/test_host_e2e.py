"""End-to-end tests for the Host API (``codify connect``).
...
from codify.process_logging import PROCESS_LOG_FILE_ENV_VAR
...
def _write_smoke_agent_yaml(tmp_path: Path) -> Path:
    """Create a minimal Codify YAML for host e2e tests.
...
def test_host_connect_and_list(
    ...
    """
    Start ``codify connect`` as a subprocess, verify the host
...
@pytest.mark.skipif(
    ...
    or not os.environ.get("CODIFY_E2E_CLAUDE_NATIVE"),
    reason=(
        "claude-native host-restart e2e requires `claude` + `tmux` on PATH "
        "and CODIFY_E2E_CLAUDE_NATIVE=1 (needs real claude CLI with mock auth)"
    ),
)
