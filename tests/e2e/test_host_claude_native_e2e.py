...
``codify.claude_native_bridge._wait_for_claude_prompt_ready``).
...
* **Opt-in only**: set ``CODIFY_E2E_CLAUDE_NATIVE=1`` to run. claude-native
...
* The workspace folder must be trusted in ``~/.claude.json`` or Claude
  ...
  teardown. It does NOT touch ``~/.codify/config.yaml``: the test
...
from codify.native_coding_agents import CLAUDE_NATIVE_AGENT_NAME
...
pytestmark = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_CLAUDE_NATIVE") != "1" or shutil.which("claude") is None,
    reason=(
        "claude-native e2e needs an interactive Claude login; set "
        "CODIFY_E2E_CLAUDE_NATIVE=1 (and have `claude` installed + logged in) to run"
    ),
)
...
def _spawn_host_daemon(
    *,
    ...
    """
    Spawn an ``codify host`` daemon under the real ``$HOME``.
    ...
    *live_server* is a fresh random-port instance with no other host of
    ...
    with open(daemon_log, "w") as log_fh:
        return subprocess.Popen(
            ...
            [runner_executable(), "-m", "codify.host._daemon_entry", "--server", live_server],
...
def _plant_poisoned_codify_package(workspace: Path) -> None:
    """
    Write a booby-trapped ``codify/`` package inside *workspace*.
    ...
    Python prepends that cwd to ``sys.path[0]``, so ``import codify``
    in the hook resolves to whatever ``codify/`` lives in the
    ...
    mode: a git worktree checked out in the workspace whose ``codify``
    ...
    :param workspace: Absolute workspace path the session starts in; the
        package is written as ``workspace/codify/__init__.py``.
    :returns: None.
    """
    pkg_dir = workspace / "codify"
    pkg_dir.mkdir()
    (pkg_dir / "__init__.py").write_text(
        "raise RuntimeError(\n"
        '    "POISONED codify package imported from the session cwd -- "\n'
        '    "python -I should have excluded the workspace from sys.path"\n'
        ")\n"
    )


def test_claude_native_hooks_ignore_workspace_codify_package(
    live_server: str,
    http_client: httpx.Client,
    tmp_path: Path,
) -> None:
    """
    Hooks import the installed ``codify``, not a workspace shadow.

    Regression for the ``python -I`` fix (cwd import shadowing). The
    workspace contains a poisoned ``codify/`` package that raises on
    ...
    workspace.mkdir()
    _plant_poisoned_codify_package(workspace)
