...
the in-tree ``codify.inner.nessie.policies`` module that polly's guardrails
...
``codify run`` subprocess against it. This exercises the parts a structural
...
Why a local server (not bare ``codify run``): polly's guardrail policies
(``codify.inner.nessie.policies`` — the package keeps its historical
name) are resolved SERVER-SIDE when the workflow executes. Bare ``codify
run`` routes to the developer's configured default server (the shared
``codify`` prod app), which may not carry the in-tree policy module, so
the turn 500s at event-execution. We therefore stand up a throwaway local
``codify server`` from this working tree - which DOES carry the polly
...
def _mock_env(mock_llm_server_url: str) -> dict[str, str]:
    ...
    ``CODIFY_CONFIG_HOME`` prevents the spawned process from touching
    the developer's real codify state.
    ...
    config_home = Path(tempfile.mkdtemp(prefix="codify-polly-mock-config-"))
...
def local_polly_server(tmp_path: Path) -> Iterator[str]:
    """
    Start a throwaway local ``codify server`` from this working tree.

    The server carries the in-tree ``codify.inner.nessie.policies`` module
...
def test_polly_orchestrator_boots_and_responds(
    ...
    """
    ``codify run <mock-polly> --server <local> -p <prompt>``
