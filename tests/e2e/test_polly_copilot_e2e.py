...
written by ``codify setup``, or an ambient ``COPILOT_GITHUB_TOKEN`` /
...
in-tree ``codify.inner.nessie.policies`` guardrails, resolved server-side) and
...
def local_polly_server_real(tmp_path: Path) -> Iterator[str]:
    """Boot a throwaway local ``codify server`` from this working tree.
    ...
    proc = subprocess.Popen(
        ...
        cwd=str(_REPO),
        env={**os.environ, "CODIFY_SKIP_ONBOARD": "1", "CODIFY_NO_UPDATE_CHECK": "1"},
