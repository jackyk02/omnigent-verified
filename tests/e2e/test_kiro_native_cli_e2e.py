"""End-to-end smoke test: ``codify kiro`` drives the native Kiro TUI.
...
the Codify conversation.

Run locally with a logged-in Kiro CLI::

    CODIFY_E2E_KIRO_NATIVE=1 \
...
pytestmark = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_KIRO_NATIVE") != "1"
    ...
    reason=(
        ...
        "binary; set CODIFY_E2E_KIRO_NATIVE=1 and have `kiro-cli` logged in"
    ),
)
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify kiro` did not return marker {marker!r}. The "
