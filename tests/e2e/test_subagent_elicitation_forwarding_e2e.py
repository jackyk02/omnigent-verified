...
This drives the REAL stack — a local ``codify server`` booted from
this working tree, a local runner + the native ``claude``/``codex``
CLIs spawned by ``codify run --server``, and a real LLM brain. The
...
``CODIFY_E2E_SUBAGENT_ELICIT=1`` and is not collected by default::

    CODIFY_E2E_SUBAGENT_ELICIT=1 \\
...
def _clean_env(profile: str = _PROFILE) -> dict[str, str]:
    ...
    ``CODIFY_CONFIG_HOME`` here (the supported replacement for the
    ...
    ``PYTHONPATH`` is dropped so the child imports codify from
    ...
    for stale in (
        ...
        env.pop(stale, None)
    config_home = Path(tempfile.mkdtemp(prefix="codify-elicit-config-"))
...
def _kill_tree(pid: int, conv_ids: set[str]) -> None:
    ...
    ``codify run`` spawns a detached daemon → runner → per-conversation
    ...
    :param pid: The ``codify run`` subprocess pid.
    ...
    if conv_ids:
        ...
        for line in ps.splitlines():
            if "codify.runtime.harnesses._runner" in line and any(c in line for c in conv_ids):
                toks = line.split()
...
def _kill_native_terminals(conv_ids: set[str]) -> None:
    ...
    server (``/tmp/codify-terminal-*/tmux.sock``). When the parent's
    one-shot ``codify run`` exits, the detached worker's pane can outlive
...
def local_server(tmp_path_factory: pytest.TempPathFactory) -> Iterator[str]:
    """
    Boot a throwaway ``codify server`` from this working tree.

    A bare ``codify run`` would route to the developer's configured
...
def _force_codex_prompting(config_path: Path) -> str | None:
    ...
    ``CODIFY_E2E_SUBAGENT_ELICIT=1`` (a deliberate, human-invoked opt-in,
...
def agent_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    layout ``codify run`` expects (``config.yaml`` at the root,
    ...
    :returns: Path of the bundle root to pass to ``codify run``.
    """
...
def test_subagent_prompt_surfaces_on_parent_and_resolves_via_child(
    ...
    Drives ``codify run ask-mode-supervisor --server <local> -p "..."``
    ...
    # is opt-in (CODIFY_E2E_SUBAGENT_ELICIT=1), never CI.
    codex_config = (
    ...
    try:
        ...
        # ``codify run`` exits after the parent's one-shot turn, but its
