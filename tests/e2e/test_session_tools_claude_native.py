...
to Claude Code when running in an Codify claude-native session.
...
sends a prompt asking it to list its codify MCP tools, and
...
Usage::
    ...
    CODIFY_E2E_CLAUDE_NATIVE_SESSION_TOOLS=1 \\
...
_RUN_GATE_ENV = "CODIFY_E2E_CLAUDE_NATIVE_SESSION_TOOLS"
...
def claude_native_ui_agent(
    ...
    Mirrors what ``codify claude`` materialises at runtime (harness
...
def _claude_code_session(
    ...
    *,
    ...
    tmp = tempfile.mkdtemp(prefix="codify-e2e-session-tools-")
    ...
    base_args: tuple[str, ...] = (
        ...
        "--allowedTools",
        (
            "mcp__codify__list_comments,"
            "mcp__codify__update_comment,"
            "mcp__codify__sys_session_list,"
            "mcp__codify__sys_session_get_history"
        ),
    )
    if model:
...
def test_claude_native_session_tools_visible(
    ...
    in Claude Code's MCP tool list when running in an Codify
    ...
    5. Send a prompt asking Claude to list its codify MCP tools.
    ...
    **What breaks if this test fails:**
    ...
    with _claude_code_session(session_id, model=None, launch_env=launch_env) as bridge_dir:
        ...
        # ── Step 5: Ask Claude to list its codify MCP tools ──────────
        send_user_message_to_session(
            http_client,
            session_id=session_id,
            content=(
                "List the names of ALL tools available to you from the "
                "'codify' MCP server. Just list the tool names, one per "
...
def test_claude_native_relay_advertises_broadened_orchestrator_surface(
    ...
    **What breaks if this fails:** the relay assembly reverted to the
    ...
    ``request.tools`` on non-native harnesses. (The real ``codify
