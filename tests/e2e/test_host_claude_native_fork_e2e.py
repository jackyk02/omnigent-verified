...
``CODIFY_E2E_CLAUDE_NATIVE=1`` (with ``claude`` installed + logged
in) to run::
...
def _wait_for_external_session_id(client: httpx.Client, *, session_id: str, timeout: float) -> str:
    ...
    recorded — the fork stamps ``codify.fork.source_external_session_id``
...
def _host_daemon(tmp_path: Path, live_server: str) -> Iterator[None]:
    """
    Spawn an ``codify connect`` daemon for the test's duration.
...
def test_fork_sdk_source_into_native_builds_history(
    ...
    with _workspaces_trusted_in_claude_config([workspace]):
        with _host_daemon(tmp_path, live_server):
            ...
            # transcript from the copied Codify items (build-from-items).
            ...
            # if the Codify items were rebuilt into its Claude transcript and
            ...
            assert marker in text, (
                ...
                "source's Codify items were not rebuilt into the clone's Claude "
...
def test_fork_native_source_into_sdk_carries_history(
    ...
    transcript replay — the SDK target serializes the copied Codify
    transcript as context). The clone binds the built-in ``sdk-chat-builtin``
    (a plain claude-sdk chat agent seeded via CODIFY_BUILTIN_AGENT_DIRS —
    ...
    with _workspaces_trusted_in_claude_config([workspace]):
        with _host_daemon(tmp_path, live_server):
            ...
            assert snap.get("labels", {}).get("codify.ui") != "terminal", (
