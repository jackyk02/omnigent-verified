...
- **native → SDK**: the SDK target replays the Codify transcript as context
  (and the source's terminal-first presentation labels must be dropped).
- **SDK → native**: the native CLI ignores the Codify transcript, so the
...
behind ``CODIFY_E2E_CLAUDE_NATIVE=1`` like the native fork e2e. Unlike a
