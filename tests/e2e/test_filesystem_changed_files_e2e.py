...
# ``CODIFY_RUNNER_WORKSPACE``. That leaves the runner with no filesystem
...
# ``_resolve_session_fs_registry`` in ``codify/runner/app.py``. The agent-write
...
def fs_workspace(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    subprocess.run(
        [
            "git",
            "-c",
            "user.email=e2e@codify.test",
            "-c",
            "user.name=codify-e2e",
            "commit",
            "-q",
            "-m",
            "seed",
...
def fs_ws_server(
    ...
    """Spawn an ``codify server`` + runner rooted at the isolated git workspace.

    The runner is given ``CODIFY_RUNNER_WORKSPACE=fs_workspace`` (and the
    ...
    env: dict[str, str] = {
        **os.environ,
        ...
        # codify; only the *workspace* (cwd / CODIFY_RUNNER_WORKSPACE) is
    ...
    proc = subprocess.Popen(
        ...
        env={**env, "CODIFY_RUNNER_TUNNEL_TOKEN": binding_token},
    ...
    runner_proc = subprocess.Popen(
        ...
        env={
            **env,
            "CODIFY_RUNNER_ID": fs_ws_runner_id,
            ...
            "CODIFY_RUNNER_WORKSPACE": str(fs_workspace),
        },
        cwd=str(fs_workspace),
...
def test_filesystem_changes_appear_after_agent_write(
    ...
    shared ``live_server``: the runner needs ``CODIFY_RUNNER_WORKSPACE`` set so
