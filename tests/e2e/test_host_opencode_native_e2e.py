...
bridge dirs (``~/.codify/opencode-native``) and the daemon registry off the
...
from codify.native_coding_agents import OPENCODE_NATIVE_AGENT_NAME
from tests._helpers.compat import apply_runner_env, compat_runner_cwd, runner_executable
from tests.e2e.helpers import POLL_INTERVAL_S

pytestmark = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_OPENCODE_NATIVE") != "1" or shutil.which("opencode") is None,
    reason=(
        "opencode-native host e2e needs a pinned `opencode` binary + LLM creds; "
        "set CODIFY_E2E_OPENCODE_NATIVE=1 (and pass --profile/--llm-api-key) to run"
    ),
)


def _spawn_host_daemon(*, tmp_path: Path, live_server: str) -> subprocess.Popen[bytes]:
    """Spawn an ``codify host`` daemon pointed at the test server."""
...
def test_opencode_native_host_session_auto_creates_terminal(
    ...
    try:
        ...
        # The `codify opencode` CLI launcher attaches this TTY directly to the
