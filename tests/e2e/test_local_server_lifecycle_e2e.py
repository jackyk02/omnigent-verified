...
A foreground ``codify server`` registers itself in the machine-global
pidfile AND must stamp the config-signature sidecar, so a later
``codify host`` / ``codify run`` reuses it instead of stopping
...
under ``<home>/.codify`` and never touch the developer's real
``~/.codify`` or a server on the real :8000 (a busy :8000 just makes
...
# cold `codify server` imports the whole stack and runs SQLite
...
# ensure_local_codify_server, so a host appearing online on the ORIGINAL port
...
_ENV_TO_CLEAR = (
    ...
    # ensure_local_codify_server honors CODIFY_DATABASE_URI over the isolated
    # tmp sqlite db, and the server honors CODIFY_RUNNER_TUNNEL_TOKEN — if
...
def _isolated_env(home: Path) -> dict[str, str]:
    ...
    :param home: The tmp home dir; ``<home>/.codify`` holds the pidfile,
...
def _pidfile_path(home: Path) -> Path:
    ...
    :returns: ``<home>/.codify/local_server.pid``.
    """
    return home / ".codify" / "local_server.pid"
...
def _respawned_server_pids(home: Path) -> set[int]:
    ...
    :func:`ensure_local_codify_server`, which spawns a detached
    ``codify server --database-uri sqlite:///<home>/.codify/chat.db
    --artifact-location <home>/.codify/artifacts`` — so its argv carries
    ...
    :returns: PIDs of live ``codify.cli ... server`` processes whose
    ...
    # Match the home as a directory PREFIX (``<home>/.codify/...`` always
    ...
    for line in out.splitlines():
        if "codify.cli" not in line or home_prefix not in line:
            continue
...
class _Procs:
    ...
    def spawn(
        ...
        :param args: CLI args after the ``python -m codify.cli`` prefix.
        ...
        proc = subprocess.Popen(
            [sys.executable, "-m", "codify.cli", *args],
            env=env,
            cwd=str(cwd),
            stdout=fh,
            stderr=fh,
...
def test_connect_reuses_foreground_server_without_killing_it(
    ...
    # via the pidfile rewrite that `ensure_local_codify_server` happens to do. A
...
def test_foreground_server_reuses_connect_owned_server(
    ...
    second canonical ``codify server`` must reuse it (print "already
    ...
    assert rc == 0, (
        f"second `codify server` exited non-zero (rc={rc}).\n"
