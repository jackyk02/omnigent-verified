...
Spawns ``codify chat examples/agents/ask-demo/`` as a subprocess
...
# Seconds to wait for ``codify run`` to reach an input-ready REPL —
...
# (``_prepare_chat_session_via_daemon`` in codify/chat.py):
#
...
def repl_env(
    ...
    """
    Build the env dict for ``codify chat`` — OPENAI_API_KEY plus
    whatever PYTHONPATH the outer shell already provides (so
    ``codify`` + ``codify_client`` resolve to this
    worktree, not the sibling editable install).

    Redirects ``HOME`` to a temp dir seeded with
    ``.codify/config.yaml`` so the spawned interactive REPL starts
    ...
    - ``tui.theme`` is persisted, so the REPL's first-launch theme
      ...
      which reads ``$HOME/.codify/config.yaml`` — NOT
      ``CODIFY_CONFIG_HOME``) is skipped. Under pexpect's pty stdin
    ...
    - ``auto_open_conversation: false`` stops the interactive REPL from
      ...
      ``run`` flag; config is the supported path). ``CODIFY_CONFIG_HOME``
    ...
    resolve, and ``CODIFY_SKIP_ONBOARD`` guards against any other
...
def _require_codify_cli() -> str:
    """
    Resolve the CLI path. Prefers the framework's own
    ``codify`` binary (via the running pytest interpreter's
    venv) over a sibling ``ap`` binary on PATH — the legacy
    ``codify`` ``ap`` CLI doesn't understand
    Codify-format fixtures.

    :returns: Absolute path to an executable.
    """
    venv_codify = Path(sys.executable).parent / "codify"
    if venv_codify.exists():
        return str(venv_codify)
    path = shutil.which("codify") or shutil.which("ap")
    if path is None:
        pytest.skip("Neither codify nor codify CLI on PATH")
    return path
...
def _wait_for_prompt_ready(
    ...
    ``codify chat <path>`` starts a local server, waits for
