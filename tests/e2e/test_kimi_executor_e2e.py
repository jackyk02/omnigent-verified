"""End-to-end tests for :class:`codify.inner.kimi_executor.KimiExecutor`.

Real-binary tests gated on:

- ``CODIFY_E2E_KIMI=1`` in the environment, and
...
with ``CODIFY_E2E_KIMI=1 uv run pytest tests/e2e/test_kimi_executor_e2e.py``.
...
from codify.inner.executor import TextChunk, TurnComplete
from codify.inner.kimi_executor import KimiExecutor, _resolve_kimi_binary


def _kimi_e2e_enabled() -> bool:
    if os.environ.get("CODIFY_E2E_KIMI") != "1":
        return False
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "Real-binary e2e: requires CODIFY_E2E_KIMI=1 and the ``kimi`` (or "
        "HARNESS_KIMI_PATH) binary on PATH. Install via "
        "`curl -fsSL https://code.kimi.com/kimi-code/install.sh | bash` and "
        "run ``kimi login`` once, then re-run with CODIFY_E2E_KIMI=1."
    ),
)
