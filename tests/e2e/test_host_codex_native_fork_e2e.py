...
they got the copied Codify transcript as context but not Codex's
...
pytestmark = pytest.mark.skipif(
    ...
    reason=("codex-native fork e2e needs `codex` on PATH and CODIFY_E2E_CODEX_NATIVE=1 to run"),
)
...
def _host_daemon(tmp_path: Path, live_server: str) -> Iterator[None]:
    """
    Spawn a codex-native ``codify connect`` daemon for the test.
