"""End-to-end tests: ``codify goose`` drives the native Goose TUI.

The goose-native sibling of ``test_cursor_native_cli_e2e``. ``goose-native`` is a
*terminal-first* harness: ``codify goose`` launches Block's ``goose session``
TUI in a runner-owned tmux pane, and each web-UI turn is injected into that pane
(bracketed paste + Enter) by
:class:`codify.inner.goose_native_executor.GooseNativeExecutor`. The TUI's own
SQLite session store is tailed by :mod:`codify.goose_native_forwarder`, which
mirrors Goose's replies back onto the Codify conversation as assistant items.

These tests drive the full stack the way a user does — spawn ``codify goose``,
...
----------------------------------------------------------
* **Opt-in only**: set ``CODIFY_E2E_GOOSE_NATIVE=1`` to run. Like the other
...
* ``GOOSE_MODE=auto`` is recommended in the test environment so the cwd test's
  file-read tool call is not blocked on an in-terminal approval prompt.
    ...
    CODIFY_E2E_GOOSE_NATIVE=1 GOOSE_MODE=auto \
...
pytestmark = pytest.mark.skipif(
    os.environ.get("CODIFY_E2E_GOOSE_NATIVE") != "1"
    ...
    reason=(
        ...
        "binary; set CODIFY_E2E_GOOSE_NATIVE=1 (and have `goose` installed + "
...
def test_goose_native_cli_smoke(
    ...
    Spawns a backgrounded ``codify goose`` session, waits for its terminal to
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify goose` did not return marker {marker!r}. The "
...
def test_goose_native_cli_runs_in_launch_cwd(
    ...
    """``codify goose`` launches ``goose session`` in the directory it was run from.

    Spawns a backgrounded ``codify goose`` whose process cwd is a temp dir
    ...
    try:
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`codify goose` did not return marker {marker!r} from "
