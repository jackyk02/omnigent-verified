...
Both ``codify claude`` and ``codify codex`` support resuming a prior
...
# Generous PTY width so the CLI's "Codify: <url>/c/<conv_id>" and
# "Resume with: … --resume <conv_id>" lines (which carry the full id) are not
# wrapped/truncated by the terminal.
_PTY_ROWS = 60
_PTY_COLS = 220

# Env vars that, leaked from a parent codify/Claude/Codex process into the
...
_STALE_ENV_VARS = (
    ...
    "TMUX",
    "RUNNER_SERVER_URL",
    "CODIFY_RUNNER_WORKSPACE",
    "CODIFY_RUNNER_ID",
    "CODIFY_RUNNER_TUNNEL_BINDING_TOKEN",
)


def codify_console_script() -> Path:
    """
    Return the ``codify`` console-script path in the active venv.

    Driving the installed console script (rather than ``python -m codify``)
    matches how users invoke the CLI; ``PYTHONPATH`` (set by :func:`cli_env`)
    points it at the worktree's code.

    :returns: Absolute path to the ``codify`` entry point next to the
        running interpreter, e.g.
        ``"/Users/…/codify/.venv/bin/codify"``.
    :raises RuntimeError: If the console script is not found beside
        ``sys.executable``.
    """
    candidate = Path(sys.executable).parent / "codify"
    if not candidate.is_file():
        raise RuntimeError(
            f"`codify` console script not found at {candidate}; the test venv "
            f"must have codify installed."
        )
    return candidate
...
def cli_env(*, profile: str | None = None) -> dict[str, str]:
    """
    Build the subprocess environment for an ``codify <harness>`` PTY run.
    ...
    otherwise leak from this (possibly codify-hosted) process into the
    ...
    :param profile: Databricks profile for the LLM gateway. When set, an
        isolated ``CODIFY_CONFIG_HOME`` is created containing an
    ...
    if profile is not None:
        config_home = Path(tempfile.mkdtemp(prefix="codify-native-config-"))
        (config_home / "config.yaml").write_text(
            f"auth:\n  type: databricks\n  profile: {profile}\n",
            encoding="utf-8",
        )
        env["CODIFY_CONFIG_HOME"] = str(config_home)
...
def run_cli_oneshot(args: list[str], *, env: dict[str, str], timeout: float) -> str:
    """
    Run ``codify <harness> … -p <prompt>`` under a PTY until it exits.
...
class PtyHandle:
    """
    A backgrounded ``codify <harness>`` PTY session kept alive for the test.
...
def spawn_cli_background(
    ...
    """
    Spawn ``codify <harness> …`` under a PTY and keep it running.
    ...
    :param args: Full argv (no ``-p`` — the session stays interactive), e.g.
        ``["/…/codify", "codex", "--server", url, "--resume", "conv_abc"]``.
    :param env: Subprocess environment from :func:`cli_env`.
...
def wait_for_conversation_id(handle: PtyHandle, *, timeout: float) -> str:
    ...
    Both CLIs print ``Codify: <url>/c/conv_<hex>`` shortly after creating
...
def assert_native_cli_resume_restores_history(
    *,
    ...
    Harness-agnostic regression check for ``codify <harness> --resume``:
    ...
    :param profile: Databricks CLI profile for the model gateway, e.g.
        ...
        ``auth:`` block + ``DATABRICKS_CONFIG_PROFILE`` (the codify
    ...
    :param force_cold_resume: When ``True``, delete the harness's local
        ...
        transcript and must instead go through Codify' cold-resume
    ...
    # yields "Not logged in"). The cost is that a *concurrent* ``codify``
    # process on the same machine can thrash the shared host daemon
    # (``~/.codify/host.pid``); run this opt-in test on an otherwise-idle
    # machine.
