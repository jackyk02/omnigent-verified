...
def non_git_workspace() -> Iterator[Path]:
    ...
    checkout.  The ``codify server`` subprocess is started with this
    ...
    """
    tmp = Path(tempfile.mkdtemp(prefix="codify_e2e_ng_"))
    yield tmp
    import shutil
...
def non_git_server(
    ...
    """Spawn a real ``codify server`` whose CWD is a non-git workspace.
    ...
    runner_proc = subprocess.Popen(
        ...
        env={
            **env,
            "CODIFY_RUNNER_ID": non_git_runner_id,
            ...
            "CODIFY_RUNNER_WORKSPACE": str(non_git_workspace),
        },
        cwd=str(non_git_workspace),
