...
:class:`codify.inner.goose_executor.GooseExecutor` spawns the subprocess,
streams ``agent_message_chunk`` updates as chat text, and routes Goose's mid-turn
``session/request_permission`` through Codify's TOOL_CALL policy + human-consent
...
----------------------------------------------------------
* **Opt-in only**: set ``CODIFY_E2E_GOOSE=1`` to run. Needs a configured Goose
