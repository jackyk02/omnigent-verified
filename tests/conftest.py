"""Shared pytest configuration and fixtures for Codify tests."""
...
os.environ.setdefault("CODIFY_DISABLE_CATALOG_LOOKUP", "1")

# Pin header mode for the whole suite. Header is the env-unset default,
# but a developer's shell often has CODIFY_AUTH_ENABLED=1 set (the
...
# the shell also exports CODIFY_OIDC_ISSUER), booting every server in
# multi-user mode and failing loud with "Missing required environment
# variable CODIFY_ACCOUNTS_COOKIE_SECRET" / "Authentication required"
...
# so subprocess-spawning tests (e2e shells out to `codify run`) inherit
# the pin via env.
...
from codify.db.utils import _engine_cache, _engine_lock, get_or_create_engine
from codify.runtime.filesystem_registry import GitFilesystemRegistry
...
def _run_test_environment_guardrails(config: pytest.Config) -> None:
    ...
    or port. Set ``CODIFY_DISABLE_TEST_GUARDRAILS=1`` to temporarily
    downgrade violations to warn-only for deliberate integration runs.
    """
    from codify.testing.guardrails import check_test_environment

    db_uri = os.environ.get("CODIFY_DATABASE_URI", "")
    base_url = config.getoption("--codify-server-url", default=None)
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        default=None,
        help=(
            "Base URL of an externally-managed `codify.cli server` to run "
...
def _isolate_claude_native_state(
    ...
    The ``codify claude`` wrapper writes per-conversation
    launch state (the cwd a session was created in) under
    ``~/.codify/claude-native/<hash>/launch.json``. Any test
    that drives the wrapper -- directly or indirectly via test
    fakes that invoke its helpers -- would otherwise write to the
    developer's real ``~/.codify`` directory and pollute it
    across test runs.

    The state module honors :data:`CODIFY_CLAUDE_NATIVE_STATE_DIR`
    ...
    monkeypatch.setenv("CODIFY_CLAUDE_NATIVE_STATE_DIR", str(state_dir))
...
def _isolate_codex_native_state(
    ...
    The ``codify codex`` wrapper writes per-conversation launch
    state under ``~/.codify/codex-native/<hash>/launch.json``.
    Tests that drive the wrapper should never write to or read from
    the developer's real persistent resume state.

    The state module honors :data:`CODIFY_CODEX_NATIVE_STATE_DIR`
    ...
    monkeypatch.setenv("CODIFY_CODEX_NATIVE_STATE_DIR", str(state_dir))
...
def _worker_db_uri() -> Generator[str, None, None]:
    ...
    When ``CODIFY_TEST_DB_URI`` is set, creates one database per worker
    (``codify_test_w0``, ``codify_test_w1``, …), runs Alembic migrations
    ...
    base_uri = os.environ.get("CODIFY_TEST_DB_URI", "")
    if not base_uri:
        yield ""
        return

    worker = os.environ.get("PYTEST_XDIST_WORKER", "w0")
    db_name = f"codify_test_{worker}"
...
def db_uri(tmp_path: Path, _worker_db_uri: str) -> Generator[str, None, None]:
    ...
    * **Postgres / MySQL** (``CODIFY_TEST_DB_URI`` set): reuses the
...
def lowered_idle_thresholds(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from codify.inner import terminal as terminal_module
