"""Tests for codify.pi_native_credentials (native Pi provider wiring)."""
...
from codify import pi_native_credentials as creds
...
def test_resolves_databricks_default_to_anthropic_gateway(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from codify.inner import databricks_executor
...
def test_databricks_model_list_failure_has_no_warning(
    ...
    from codify.inner import databricks_executor
    from codify.runtime.credentials import databricks as rt_databricks
...
def test_write_models_config_is_owner_only(tmp_path: Path) -> None:
    ...
    assert written["providers"]["codify"]["apiKey"] == "sk-secret"
...
def test_provider_launch_returns_env_and_args(tmp_path: Path) -> None:
    ...
    assert args == ["--provider", "codify", "--model", "claude-sonnet-4-6"]
...
def test_cli_config_databricks_warns_on_unresolvable(
    ...
    with caplog.at_level(logging.INFO, logger="codify.pi_native_credentials"):
        assert (
...
def test_workspace_url_for_dedicated_gateway_uses_profile(
    ...
    from codify.runtime.credentials import databricks as db_creds_mod
...
def test_model_override_beats_databricks_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg = provider.to_models_config()
    model_ids = [m["id"] for m in cfg["providers"]["codify"]["models"]]
...
def test_model_override_beats_inline_family_default() -> None:
    ...
    cfg = provider.to_models_config()
    assert cfg["providers"]["codify"]["models"] == [{"id": "claude-opus-4-7"}]
...
def test_databricks_prefixed_override_normalized_for_inline_openai() -> None:
    ...
    cfg = provider.to_models_config()
    assert cfg["providers"]["codify"]["models"] == [{"id": "gpt-5-4"}]
...
def test_inline_family_passes_non_mechanical_override_through() -> None:
    ...
    assert provider.model == "zai-org/GLM-4.7"
    cfg = provider.to_models_config()
    assert cfg["providers"]["codify"]["models"] == [{"id": "zai-org/GLM-4.7"}]
...
def test_databricks_profile_registers_gpt_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The ``codify-openai`` provider targets ``/serving-endpoints`` so Pi's
    ...
    cfg = provider.to_models_config()
    openai_entry = cfg["providers"].get("codify-openai")
    assert openai_entry is not None, "codify-openai (responses) provider missing from models.json"
    ...
    completions_entry = cfg["providers"].get("codify-completions")
    assert completions_entry is not None, "codify-completions provider missing from models.json"
...
def test_cli_config_databricks_registers_gpt_provider(
    ...
    cfg = provider.to_models_config()
    openai_entry = cfg["providers"].get("codify-openai")
    assert openai_entry is not None, "codify-openai provider missing from models.json"
