...
Ports relevant callables from codify
`examples/tool_functions.py` into a location importable by the
codify parser (dotted path `tests._fixtures.agents.*`).
...
def block_long_sleep(event: PolicyEvent) -> PolicyResponse:
    """
    Ported from codify ``block_long_sleep``.
...
def _extract_args(content: object) -> dict[str, object]:
    ...
    already-parsed dicts or JSON-encoded strings (Codify'
