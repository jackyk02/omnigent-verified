...
from codify import install_ledger


def test_install_ledger_round_trip_and_mode(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("CODIFY_DATA_DIR", str(tmp_path / ".codify"))
...
def test_write_install_ledger_from_env_records_profile_and_dep_provenance(
    ...
    monkeypatch.setenv("CODIFY_DATA_DIR", str(state))
    monkeypatch.setenv("CODIFY_LEDGER_PROFILE", str(profile))
    monkeypatch.setenv("CODIFY_LEDGER_DEP_UV", "codify")
    ...
    assert ledger.entries.deps["uv"].installed_by == "codify"
...
def test_installer_ledger_supersedes_backfill_and_preserves_dep_ownership(
    ...
    assert rewritten.entries.deps["uv"].installed_by == "codify"
...
def test_install_ledger_merges_existing_and_observed_external_configs(
    ...
    old_config = install_ledger.ExternalConfigEntry(
        ...
        marker="mcpServers.codify",
        format="json",
        allowlist=["mcpServers.codify"],
    )
    ...
    observed_config.write_text('{"mcpServers": {"codify": {"command": "python"}}}\n')
...
def test_deep_backfill_observes_external_config_and_launch_agents(
    ...
    cursor_config.write_text('{"mcpServers": {"codify": {"command": "python"}}}\n')
    launch_agent = launch_dir / "ai.codify.local.plist"
    ...
    assert ledger.entries.injected_external_config[0].marker == "mcpServers.codify"
