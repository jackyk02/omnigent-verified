...
from codify import native_policy_hook
...
def test_codify_mcp_tools_are_skipped(hook_event: str) -> None:
    """
    Codify MCP tools return None and are never sent to /policies/evaluate.

    Codify MCP tool calls are already policy-checked by the relay path
    ...
    result = hook_payload_to_evaluation_request(
        hook_event,
        {"tool_name": "mcp__codify__list_comments", "tool_input": {}, "tool_output": "x"},
    )
...
def test_connector_native_mcp_tools_are_evaluated(hook_event: str, expected_type: str) -> None:
    ...
    do not round-trip through Codify's MCP proxy, so this hook is their
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_resp(302, "https://codifys.example.databricksapps.com/.auth/callback"), True),
...
def test_policy_hook_request_headers_merges_baked_auth(
    ...
    the executor bakes them into ``_CODIFY_AUTH_HEADERS``; the reader must
    fold them onto ``Content-Type`` for the policy POST.
    """
    monkeypatch.setenv(
        "_CODIFY_AUTH_HEADERS",
...
def test_policy_hook_request_headers_tolerates_missing_or_bad_env(
    ...
    """
    if raw:
        monkeypatch.setenv("_CODIFY_AUTH_HEADERS", raw)
    else:
        monkeypatch.delenv("_CODIFY_AUTH_HEADERS", raising=False)
...
def test_policy_hook_wrapper_script_bakes_auth_and_routing(
    ...
    """
    import codify.cli_auth as cli_auth
    import codify.runner._entry as entry
    ...
    script = native_policy_hook.policy_hook_wrapper_script(
        "https://acme.databricks.com/api/2.0/codify", "conv_x", "/path/hook.py"
    )

    assert script.startswith("#!/bin/sh\n")
    assert "_CODIFY_SERVER_URL=https://acme.databricks.com/api/2.0/codify" in script
    assert "_CODIFY_SESSION_ID=conv_x" in script
    # The baked headers carry BOTH the bearer and the routing header.
    line = next(ln for ln in script.splitlines() if ln.startswith("export _CODIFY_AUTH_HEADERS="))
...
def test_policy_hook_reauth_remints_and_preserves_routing_header(
    ...
    reauth = native_policy_hook.policy_hook_reauth(
        "https://acme.databricks.com/api/2.0/codify",
