"""Tests for shared Codify config loading."""
...
from codify.config import _merge_effective_config, global_config_path, load_effective_config
...
def test_effective_config_deep_merges_harness_mapping(
    ...
    config_home.mkdir()
    (project / ".codify").mkdir(parents=True)
    (config_home / "config.yaml").write_text(
        "harness:\n  default: claude-sdk\n  claude-sdk:\n    command: /global/claude\n"
        "  codex:\n    args: [--config, k=v]\n"
    )
    (project / ".codify" / "config.yaml").write_text(
        "harness:\n  codex:\n    command: /local/codex\n"
    )
...
def test_effective_config_merges_project_over_user(
    ...
    (project / ".codify" / "config.yaml").write_text("profile: local\n")
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(config_home))
    monkeypatch.chdir(project)
