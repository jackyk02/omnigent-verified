...
``codify.inner.sandbox`` surface, so the tests only verify two
...
import codify.inner.bwrap_sandbox as inner_bwrap
import codify.inner.sandbox as inner_sandbox
from codify import sandbox
from codify.sandbox import bwrap


def test_sandbox_all_symbols_importable() -> None:
    """
    Every name in ``codify.sandbox.__all__`` resolves to an attribute on
    the wrapper module.
...
def test_bwrap_all_symbols_importable() -> None:
    """
    Every name in ``codify.sandbox.bwrap.__all__`` resolves on the
    submodule.
    ...
    for name in bwrap.__all__:
        assert hasattr(bwrap, name), f"codify.sandbox.bwrap missing re-export {name!r}"


def test_bwrap_reexports_are_inner_objects() -> None:
    """
    ``BwrapSandboxBackend`` re-exports the same identity as
    ``codify.inner.bwrap_sandbox``.
...
def test_default_sandbox_for_platform_is_bwrap_on_linux_regardless_of_binary() -> None:
    ...
    **not** depend on whether the ``bwrap`` binary is installed on the
    ...
    from codify.inner.datamodel import OSEnvSandboxSpec

    with (
        patch.object(sys, "platform", "linux"),
        patch("codify.inner.sandbox.shutil.which", return_value=None),
    ):
