...
from codify.cli import _ensure_host_daemon, _host_daemon_alive, cli
...
class _HostRun:
    ...
    :param server_url: Codify server URL the daemon was told to connect
...
def test_host_command_registered() -> None:
    ...
    and users can't run ``codify host``.
    """
    runner = CliRunner()
...
def test_host_no_server_starts_local_backend(
    ...
    """
    Verify that ``host`` with no --server starts a local Codify server.

    Under the daemon model, ``codify host`` (no URL, no config) is
    valid: it starts (or reuses) a persistent local Codify server and connects
    ...
    monkeypatch.setattr("codify.cli._HOST_PID_PATH", tmp_path / "host.pid")
    ...
    with (
        ...
        patch("codify.host.connect.run_host_process", _fake_run),
    ):
        runner = CliRunner()
...
def test_host_reads_server_from_global_config(
    ...
    ``~/.codify/config.yaml`` has a ``server:`` key.
    """
    ...
    with patch("codify.host.connect.run_host_process", _fake_run):
        runner = CliRunner()
        result = runner.invoke(cli, ["host"])
...
def test_host_accepts_option_after_positional_server(
    ...
    URL: ``codify host https://… --non-interactive`` is the scripted/CI
...
def test_host_accepts_empty_positional_as_local_marker(
    ...
    overrides configured remote defaults and starts the local Codify server.
    """
    (tmp_path / "config.yaml").write_text("server: https://from-config.example.com\n")
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path))
    monkeypatch.setattr("codify.cli._HOST_PID_PATH", tmp_path / "host.pid")
...
def test_host_status_subcommand_still_dispatches(
    ...
    with (
        patch("codify.host.connect.run_host_process", _fake_run),
        patch("codify.cli._selected_daemon_records", _fake_selected),
    ):
        runner = CliRunner()
...
def test_host_daemon_alive_returns_false_when_no_pid_file(
    ...
    """
    with patch("codify.cli._HOST_PID_PATH", tmp_path / "host.pid"):
...
def test_host_daemon_alive_returns_false_for_dead_pid(
    ...
    with patch("codify.cli._HOST_PID_PATH", pid_path):
        assert _host_daemon_alive() is False
...
def test_ensure_host_daemon_writes_pid_file(
    ...
    with (
        patch("codify.cli._HOST_PID_PATH", pid_path),
        patch("codify.cli.subprocess.Popen", side_effect=_fake_popen),
    ):
...
def test_ensure_host_daemon_keeps_old_for_different_server(
    ...
    with (
        patch("codify.cli._HOST_PID_PATH", pid_path),
        patch("codify.cli._pid_alive", lambda pid: pid in {4242, 4243}),
        patch("codify.cli.os.kill", lambda pid, sig: killed.append(pid)),
        patch("codify.cli.subprocess.Popen", side_effect=_fake_popen),
    ):
...
def test_ensure_host_daemon_skips_if_alive(
    ...
    with (
        patch("codify.cli._HOST_PID_PATH", pid_path),
        patch("codify.cli.subprocess.Popen", side_effect=_counting_popen),
    ):
        _ensure_host_daemon("http://localhost:8000")
