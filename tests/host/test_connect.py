...
from codify.host.identity import HostIdentity
...
async def test_handle_model_options_uses_codex_provider_catalog(
    ...
    from codify import codex_native_app_server
    from codify.model_catalog import ModelEntry, ModelListing
    ...
    monkeypatch.setattr(
        "codify.model_catalog.list_models_for_worker",
...
async def test_handle_model_options_filters_direct_openai_through_codex_catalog(
    ...
    from codify import codex_native_app_server
    from codify.model_catalog import ModelEntry, ModelListing, ResolvedModelProvider
    ...
    monkeypatch.setattr(
        "codify.model_catalog.resolve_model_provider",
...
async def test_handle_launch_spawns_subprocess(
    ...
    with patch("codify.host.connect.subprocess.Popen", side_effect=_fake_popen):
    ...
    assert spawned_env.get("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN") == "test_token_abc"
    assert spawned_env.get(RUNNER_INITIAL_AUTH_TOKEN_ENV_VAR) == "host-bootstrap-bearer"
    assert spawned_env.get("CODIFY_RUNNER_WORKSPACE") == str(workspace)
...
async def test_handle_launch_refuses_unconfigured_harness(
    ...
    monkeypatch.setattr(
        "codify.host.connect.harness_is_configured",
...
async def test_handle_launch_prints_exact_runner_log_path(
    ...
    tmp (no write to the developer's real ``~/.codify``).
    """
    ...
    runner_log_dir = tmp_path / ".codify" / "logs" / "runner"
    ...
    assert f"log: ~/.codify/logs/runner/{log_files[0].name}" in out
...
async def test_live_host_refreshes_harness_readiness_without_reconnect(
    ...
    monkeypatch.setattr(
        "codify.host.connect.configured_harness_map",
    ...
    monkeypatch.setattr(
        "codify.host.connect.HARNESS_READINESS_REFRESH_INTERVAL_S",
        0.01,
    )
    host = _make_host_process()
...
async def test_live_host_full_refresh_detects_auth_completion(
    ...
    monkeypatch.setattr(
        "codify.host.connect.HARNESS_READINESS_FULL_REFRESH_INTERVAL_S",
        0.01,
    )
    host = _make_host_process()
    tunnel = _ReadinessChangingTunnel()
...
async def test_handle_launch_immediate_exit_reports_exit_code_and_log_tail(
    ...
    assert "~/.codify/logs/runner/runner-" in error
...
async def test_watch_runner_reports_unexpected_exit(
    ...
    """
    monkeypatch.setattr(Path, "home", classmethod(lambda _cls: tmp_path))
    monkeypatch.setattr("codify.host.connect._RUNNER_WATCH_INTERVAL_S", 0.01)
    host = _make_host_process()
    tunnel = _FakeTunnel()
...
async def test_hello_advertises_installed_version() -> None:
    """The ``host.hello`` frame reports the codify version, not a placeholder.
...
def test_reaper_does_not_steal_host_owned_subprocess_exit_code(tmp_path: Path) -> None:
    ...
    :mod:`codify.host.git_worktree`, run via ``subprocess.run`` under
...
def test_install_child_subreaper_is_safe_to_call() -> None:
    ...
    from codify.host.connect import _install_child_subreaper
...
def test_host_spawned_runner_has_parent_pid_env(
    ...
    CODIFY_RUNNER_PARENT_PID set to the host's PID.
    ...
    with patch("codify.host.connect.subprocess.Popen", side_effect=_capture_env):
        import asyncio
    ...
    parent_pid = spawned_env.get("CODIFY_RUNNER_PARENT_PID")
...
def test_build_runner_env_allowlists_host_env_and_strips_secrets() -> None:
    ...
    base = {
        ...
        "SOME_RANDOM_VAR": "x",
        "CODIFY_CLAUDE_SDK_NO_SANDBOX": "1",
        "KUBECONFIG": "/home/alice/.kube/config",
        "CLAUDE_CODE_SKIP_BEDROCK_AUTH": "1",
        "CODIFY_DATABRICKS_EXTRA_HEADERS": '{"x-databricks-route-hint": "instance-abc"}',
        "CODIFY_LOG_LEVEL": "DEBUG",
        "CODIFY_LOG_TO_STDERR": "1",
        "CODIFY_LOG_TTY_FD": "9",
    }
    ...
    # harness, so a bare ``CODIFY_CLAUDE_SDK_NO_SANDBOX=1 codify run …``
    # must reach the runner without also forcing
    # ``CODIFY_RUNNER_ENV_PASSTHROUGH=CODIFY_CLAUDE_SDK_NO_SANDBOX``.
    assert env["CODIFY_CLAUDE_SDK_NO_SANDBOX"] == "1"
    ...
    # (without the operator also listing it in CODIFY_RUNNER_ENV_PASSTHROUGH).
    assert env["CODIFY_DATABRICKS_EXTRA_HEADERS"] == '{"x-databricks-route-hint": "instance-abc"}'
    # Process logging controls forward so host-spawned runners honor --debug
    # and --log-to-stderr.
    assert env["CODIFY_LOG_LEVEL"] == "DEBUG"
    assert env["CODIFY_LOG_TO_STDERR"] == "1"
    assert env["CODIFY_LOG_TTY_FD"] == "9"
...
def test_build_runner_env_forwards_harness_credentials_and_endpoints() -> None:
    ...
    """
    from codify.host.connect import HARNESS_CREDENTIAL_ENV_VARS
...
def test_build_runner_env_forwards_codify_prefixed_harness_credentials() -> None:
    ...
    base = {
        "PATH": "/usr/bin",
        "HOME": "/root",
        "CODIFY_ANTHROPIC_API_KEY": "sk-prefixed",
    }
    ...
    assert "CODIFY_ANTHROPIC_API_KEY" in HARNESS_CREDENTIAL_ENV_VARS
    assert env["CODIFY_ANTHROPIC_API_KEY"] == "sk-prefixed"
    assert "ANTHROPIC_API_KEY" not in env


def test_build_runner_env_passthrough_extends_forwarded_set() -> None:
    """
    CODIFY_RUNNER_ENV_PASSTHROUGH names EXTRA vars to forward (for
    ...
    base = {
        "PATH": "/usr/bin",
        "HOME": "/root",
        "CODIFY_RUNNER_ENV_PASSTHROUGH": "MY_GATEWAY_TOKEN, MY_GATEWAY_URL",
...
def test_build_runner_env_propagates_data_dir_paths_not_db_uri() -> None:
    ...
    Regression guard: ``CODIFY_CONFIG_HOME`` was absent from the
    allowlist, so the daemon/runner used ``~/.codify`` while a CLI run
    under an isolated config home read the local-server pidfile elsewhere —
    discovery then timed out (the e2e ``CODIFY_CONFIG_HOME`` isolation
    ...
    base = {
        "PATH": "/usr/bin:/bin",
        "CODIFY_CONFIG_HOME": "/tmp/iso-home",
        "CODIFY_DATA_DIR": "/tmp/iso-data",
        "CODIFY_DATABASE_URI": "postgresql://user:pw@host/db",
    }
    ...
    assert env["CODIFY_CONFIG_HOME"] == "/tmp/iso-home"
    assert env["CODIFY_DATA_DIR"] == "/tmp/iso-data"
    ...
    assert "CODIFY_DATABASE_URI" not in env


def test_build_runner_env_propagates_disable_keyring() -> None:
    """``CODIFY_DISABLE_KEYRING`` propagates so the runner resolves
    ...
    """
    base = {"PATH": "/usr/bin:/bin", "CODIFY_DISABLE_KEYRING": "1"}
    env = _build_runner_env(
        base,
        server_url="http://server",
        runner_id="runner_abc",
    ...
    assert env["CODIFY_DISABLE_KEYRING"] == "1"
...
def test_handle_install_harness_success_returns_refreshed_readiness(
    ...
    """
    import codify.host.connect as connect
...
def test_handle_store_secret_key_writes_and_returns_readiness(
    ...
    """
    import codify.host.connect as connect
    from codify.onboarding.harness_auth import StoreCredentialResult
...
def test_handle_store_secret_adopt_calls_adopt_core(
    ...
    import codify.host.connect as connect
    from codify.onboarding.harness_auth import DetectedCredential, StoreCredentialResult
...
def test_handle_store_secret_adopt_refuses_undetected_env_var(
    ...
    """
    import codify.host.connect as connect
    from codify.onboarding.harness_auth import DetectedCredential
...
def _patch_connect(monkeypatch: pytest.MonkeyPatch, spy: _ConnectSpy) -> None:
    ...
    import codify.runner._entry as entry_mod
...
def test_build_connect_headers_adds_org_header(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CODIFY_HOST_TOKEN", raising=False)
    monkeypatch.setattr(entry_mod, "_make_auth_token_factory", lambda *, server_url=None: None)
    monkeypatch.setattr("codify.cli_auth.load_databricks_org_id", lambda _url: "2850744067564480")

    headers = _host("https://acme.databricks.com/api/2.0/codify")._build_connect_headers()
...
async def test_run_retries_on_login_redirect(
    ...
    ``codify login`` remediation so the operator can act if the cause
    is persistent.
    """
    monkeypatch.setattr("codify.host.connect._RECONNECT_BASE_S", 0.0)
    spy = _ConnectSpy(
        [
    ...
    with caplog.at_level(logging.WARNING, logger="codify.host.connect"):
        await host.run()
    ...
    # the single remediation message recommending `codify login <url>`.
    assert any("login page" in r.message for r in caplog.records)
    assert any(
        "codify login https://app.example.databricks.com" in r.message for r in caplog.records
    )
...
async def test_login_redirect_prints_warning_to_terminal(
    ...
    not-logged-in ``codify host`` sat completely silent on the terminal
    ...
    assert "codify login https://app.example.databricks.com" in err
...
async def test_fresh_host_fails_loud_after_persistent_login_redirects(
    ...
    assert "codify login https://app.example.databricks.com" in message
...
async def test_auth_rejection_suggests_codify_login(
    monkeypatch: pytest.MonkeyPatch, status: int
) -> None:
    """401/403 rejections point the user at ``codify login``.
    ...
    assert f"codify login {server_url}" in str(excinfo.value)


async def test_non_auth_permanent_4xx_omits_login_hint(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A non-auth permanent 4xx (404) does NOT suggest ``codify login``.
    ...
    assert "codify login" not in message
...
def test_run_host_process_announces_session_log_dir_on_start(
    ...
    """``codify host`` names the session-log dir on start (was silent).

    The reported regression: ``codify host`` ran completely silently, so a
    ...
    out = capsys.readouterr().out
    assert "Session logs: ~/.codify/logs/runner/" in out
    assert "This host's log: ~/.codify/logs/host/host-" in out
