"""Tests for the persistent background local Codify server helpers.

Covers ``codify.host.local_server``: reuse-vs-respawn detection
(:func:`local_server_url_if_healthy`) and the spawn wiring
(:func:`ensure_local_codify_server`). The connect daemon owns this server in
...
from codify.host import local_server
...
def test_wait_for_local_server_bypasses_environment_proxies(
    ...
    local_server._wait_for_local_codify_server(
...
def test_ensure_local_codify_server_reuses_without_spawning(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    invocation's. With the matching sig stamped, ``ensure_local_codify_server``
    ...
    result = local_server.ensure_local_codify_server()
    assert result.url == "http://127.0.0.1:8123"
    # Reused an existing healthy server — did not start a new process.
    assert result.spawned is False


def test_ensure_local_codify_server_respawns_on_config_drift(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    stopped: list[bool] = []
    monkeypatch.setattr(local_server, "stop_local_codify_server", lambda: stopped.append(True))
...
def test_server_config_signature_changes_with_version(
    ...
    the upgraded CLI's, so ``ensure_local_codify_server`` respawns it on
    the new code through the existing config-drift path.
    """
    import importlib.metadata

    from codify.server import auth as auth_mod
...
def test_ensure_local_codify_server_spawns_when_none_healthy(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    launched as ``codify server`` on 127.0.0.1, the pidfile records
    ...
    # ensure_local_codify_server picks the port via pick_local_port (prefers :8000,
...
def test_stop_local_codify_server_waits_for_process_exit(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """``stop_local_codify_server`` blocks until the server process dies.

    The bug: fire-and-forget SIGTERM left the port bound until the OS
    reaped the process, so the next ``ensure_local_codify_server`` or
    ``codify server`` failed to bind. The fix polls ``_pid_alive``
    ...
    local_server.stop_local_codify_server()
...
def test_stop_local_codify_server_escalates_to_sigkill(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
...
def test_local_data_dir_honors_data_dir_not_config_home(
    ...
    """``_local_data_dir`` isolates the runtime DB via ``CODIFY_DATA_DIR`` only.

    Two worktrees sharing ``~/.codify/chat.db`` with divergent Alembic
    heads can't migrate the shared DB, so the daemon-backed server fails to
    boot. ``CODIFY_DATA_DIR`` is the purpose-built data-isolation knob.
    ``CODIFY_CONFIG_HOME`` MUST NOT move the DB — it isolates config only;
    ...
    """
    monkeypatch.delenv("CODIFY_DATA_DIR", raising=False)
    monkeypatch.delenv("CODIFY_CONFIG_HOME", raising=False)
    # Default: ~/.codify.
    assert local_server._local_data_dir() == Path.home() / ".codify"
    # CONFIG_HOME does NOT move the data dir — a failure here means config
    # isolation is leaking back into data-dir selection.
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path / "cfg"))
    assert local_server._local_data_dir() == Path.home() / ".codify"
...
def test_pick_local_port_returns_preferred_when_free() -> None:
    ...
    across ``codify server`` and daemon spawns. We use an
...
def test_register_then_clear_local_server_round_trip(
    ...
    This is the handshake that lets a foreground ``codify server``
...
def test_register_local_server_stamps_matching_sig(
    ...
    pidfile, so a foreground ``codify server`` presented no sig and the
    next ``connect``/``run`` saw ``None != desired`` and stopped + respawned
    it. With the sig stamped, the reuse path in ``ensure_local_codify_server``
    ...
    # Foreground `codify server` advertises itself in the pidfile + sig.
    local_server.register_local_server(8000)
    assert sig_file.read_text().strip() == local_server.server_config_signature()
...
def test_ensure_local_codify_server_spawn_records_and_returns_log_path(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A spawned server returns its captured-log path and records it for status.

    ``codify server --background`` used to be a black box — it printed only the
    ...
    assert result.log_path.parent == tmp_path / ".codify" / "logs" / "server"
...
def test_ensure_local_codify_server_reuse_reads_log_path_sidecar(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    sig_file.write_text(local_server.server_config_signature() + "\n")
    log_ref = tmp_path / "local_server.logpath"
    recorded = tmp_path / ".codify" / "logs" / "server" / "server-cd34.log"
...
def test_register_local_server_clears_stale_log_ref(
    ...
    A foreground ``codify server`` streams logs to its own terminal, not a
...
def test_stop_untracked_local_server_kills_orphan_on_default_port(
    ...
    """A live Codify server on :8000 with no pidfile entry is found and stopped.

    This is the reported bug: the pidfile was lost while the server lived, so
    ``stop_local_codify_server`` (pidfile-scoped) couldn't see it. The sweep must
...
def test_stop_untracked_local_server_noop_when_nothing_listening(
    ...
    if there's no Codify server answering, we must not even look up a PID.
    """
    import httpx
    ...
    monkeypatch.setattr(
        local_server,
        "subprocess",
        _fake_subprocess(raises=AssertionError("lsof consulted despite no Codify server")),
    )
...
def test_stop_untracked_local_server_noop_on_non_codify_listener(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_stop_untracked_local_server_noop_when_lsof_unavailable(
    ...
    """A live Codify server but no resolvable PID (lsof missing) → degrade, no kill.
...
def test_ensure_retries_when_child_dies_and_foreign_owner_holds_port(
    ...
    monkeypatch.setattr(local_server, "_wait_for_local_codify_server", _wait)
...
def test_ensure_startup_failure_without_contention_raises(
    ...
    with pytest.raises(click.ClickException) as excinfo:
        local_server.ensure_local_codify_server()
