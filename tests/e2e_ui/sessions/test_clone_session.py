...
``codify host`` daemon this harness doesn't spawn, so the worktree test
...
def test_clone_dialog_offers_cross_family_native_target_and_forks(
    ...
    assert labels.get("codify.fork.carry_history") == "1", (
        f"cross-family native fork must stamp carry-history, got {labels!r}"
    )
    assert "codify.fork.source_external_session_id" not in labels, (
        f"fork of an SDK source must not stamp a source native session id, got {labels!r}"
    )
    assert labels.get("codify.wrapper") == "claude-code-native-ui", (
...
def test_clone_worktree_source_prefills_repo_and_validates_directory(
    ...
    The harness spawns no ``codify host`` daemon, so the host-side wire is
