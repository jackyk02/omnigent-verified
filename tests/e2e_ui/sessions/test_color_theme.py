...
``Select``) — one option per palette (Codify, Dracula, GitHub, Catppuccin,
...
``localStorage["codify:ui-theme-palette"]``. The default "Codify" palette
...
def _stored_palette(page: Page) -> str | None:
    ...
    return page.evaluate("() => window.localStorage.getItem('codify:ui-theme-palette')")
...
def _stored_custom_theme(page: Page) -> dict[str, object] | None:
    ...
    raw = page.evaluate("() => window.localStorage.getItem('codify:custom-theme')")
...
def test_color_palette_applies_persists_and_resets(
    ...
    Fresh load is the default "Codify" (its name shown, nothing stored, no
    ``data-theme``). Picking GitHub sets ``data-theme="github"`` and persists it —
    and survives a reload (re-applied at boot). Returning to Codify removes the
    attribute and clears the stored key.
    """
    base_url, _session_id = seeded_session
    _open_appearance(page, base_url)

    # Fresh context → default "Codify": the trigger shows it, no override, and
    # nothing persisted.
    expect(_color_theme_select(page)).to_contain_text("Codify")
    ...
    # → back to Codify (the default): the override is removed and the stored
    # key cleared, since the default reverts to the base brand tokens.
    _pick_palette(page, "Codify")
    expect(_color_theme_select(page)).to_contain_text("Codify")
    assert _data_theme(page) is None, "<html> kept data-theme after returning to Codify"
