...
def test_appearance_reset_restores_defaults(page: Page, seeded_session: tuple[str, str]) -> None:
    ...
    stored_font_size = page.evaluate("() => window.localStorage.getItem('codify:ui-font-size')")
    ...
    assert page.evaluate("() => window.localStorage.getItem('codify:ui-font-size')") == "18"
    assert page.evaluate("() => window.localStorage.getItem('codify:terminal-theme')") == "dark"
    ...
    expect(font_size_input).to_have_value("16")
    expect(page.get_by_test_id("terminal-theme-auto")).to_have_attribute("aria-checked", "true")
    assert page.evaluate("() => window.localStorage.getItem('codify:ui-font-size')") is None
    assert page.evaluate("() => window.localStorage.getItem('codify:terminal-theme')") is None
