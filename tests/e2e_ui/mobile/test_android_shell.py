...
``window.codifyNative = {kind: "android", ...}``. The web layer feature-detects
...
OS-measured inset as ``--codify-android-safe-area-*`` and ``index.css`` folds
it into the shared ``--codify-safe-top/bottom`` with ``max()``.
...
end-to-end we inject a minimal ``window.codifyNative`` stub via
...
# Read the *resolved* ``--codify-safe-top`` in pixels. ``getComputedStyle`` on a
# custom property returns its declared text (the ``max()`` expression), so instead
# size a throwaway probe by ``var(--codify-safe-top)`` and read its computed
# height, which resolves the fold.
_READ_SAFE_TOP_PX = """
() => {
  const probe = document.createElement('div');
  probe.style.cssText =
    'position:absolute;visibility:hidden;pointer-events:none;height:var(--codify-safe-top)';
...
def test_android_shell_tags_root_and_folds_os_inset(
    ...
    shell injects as ``--codify-android-safe-area-top`` reaches the shared
    ``--codify-safe-top`` through the ``index.css`` ``max()`` fold (which is 0
    ...
    # --codify-android-safe-area-top; index.css folds it into
    # --codify-safe-top via max(), so the layout reads the real inset.
    page.evaluate(
        "() => document.documentElement.style"
        ".setProperty('--codify-android-safe-area-top', '40px')"
    )
...
def test_no_android_tag_or_fold_in_plain_browser(
    ...
    Without the ``window.codifyNative`` stub, ``isAndroidShell()`` is false, so
    the app-shell must NOT carry ``data-android-native`` and the
    ``--codify-android-safe-area-*`` fold must contribute nothing — the gate
    ...
    # The web app never injects --codify-android-safe-area-*, so with
...
window.__codifyActivations = [];
window.codifyNative = {
  kind: "android",
  setBadgeCount: function () {},
  notify: function () { return Promise.resolve(false); },
  onNotificationActivated: function (cb) {
    window.__codifyActivate = cb;
    return function () { delete window.__codifyActivate; };
  },
  onNativeInsets: function () { return function () {}; },
};
"""
...
def test_badge_notification_activation_navigates_to_target(
    ...
    page.wait_for_function("() => typeof window.__codifyActivate === 'function'")

    # Fire the callback the way the shell does for a single-unread badge tap.
    page.evaluate("path => window.__codifyActivate(path)", f"/c/{session_id}")
