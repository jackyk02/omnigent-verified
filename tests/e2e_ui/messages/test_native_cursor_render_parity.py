...
A native forwarder (:mod:`codify.cursor_native_forwarder`) tails
...
by :class:`codify.inner.cursor_native_executor.CursorNativeExecutor`. This
...
the same Codify-owned half (the forwarder mirroring cursor's transcript OUT as
...
``codify.inner.cursor_harness``), so it cannot be pointed at the mock LLM the
custom-agent suites use; instead this test seeds a cursor chat store and runs the
real :func:`codify.cursor_native_forwarder.forward_cursor_store_to_session`
...
def test_native_cursor_mirror_renders_without_live_agent(
    ...
    login, so it skips on every PR. This covers the SAME Codify-owned path —
