...
exposes ``window.codifyDesktop.kind === "electron"`` *and* the embedded-
...
a minimal ``window.codifyDesktop`` stub via ``add_init_script`` *before any
...
def test_no_browser_tab_in_plain_browser(
    ...
    Without the ``window.codifyDesktop`` stub, ``supportsBrowser()`` is
