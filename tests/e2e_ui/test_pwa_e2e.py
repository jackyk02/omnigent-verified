...
LLM, so this spawns a plain ``codify server`` (no ``--agent``, no runner, no
...
``codify/server/app.py``, which is exactly what the browser checks rely on.
...
def _wait_healthy(proc: subprocess.Popen[bytes], base_url: str, log_path: Path) -> None:
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            ...
            raise RuntimeError(
                f"codify server exited early (code {proc.returncode}).\n{log[:2000]}"
            )
        try:
    ...
    log = log_path.read_text() if log_path.exists() else ""
    raise RuntimeError(
        f"codify server not healthy within {_HEALTH_TIMEOUT_S:.0f}s on "
...
def pwa_server(
    ...
    """Serve the built SPA via a no-agent ``codify server`` and yield its URL.
    ...
    ``codify.db`` / ``artifacts`` are never touched.
    ...
    # PYTHONPATH forces the subprocess to import codify from the worktree, not
