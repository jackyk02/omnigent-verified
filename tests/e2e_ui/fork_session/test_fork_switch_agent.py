...
renders, and the fork's labels route the runner correctly —
  ...
  - native targets that can replay fork history stamp
    ``codify.fork.carry_history`` (the runner must rebuild the native
    transcript; absent → the clone would launch fresh and lose history) and
    every native target stamps the TARGET ``codify.wrapper`` (so the clone
...
_WRAPPER_LABEL_KEY = "codify.wrapper"
_CARRY_HISTORY_LABEL_KEY = "codify.fork.carry_history"
_SOURCE_EXTERNAL_SESSION_LABEL_KEY = "codify.fork.source_external_session_id"
...
def _agent_id_by_name(base_url: str, name: str) -> str:
    ...
    assert agent is not None, (
        ...
        f"targets come from CODIFY_BUILTIN_AGENT_DIRS and the native targets "
...
def test_fork_switch_agent_carries_history(
    ...
    :param expected_wrapper: TARGET ``codify.wrapper`` value, or ``None``
