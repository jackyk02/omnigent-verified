...
the scheme-defaulting this change added means a bare (or ``/codify``)
...
(``web/electron/src/url.js``), loaded here as ``window.codifyUrl``, so the
...
the ``live_server`` codify backend.
"""
...
# web/electron/src/url.js, so window.codifyUrl is the real shared module.
_SETUP_PAGE = Path(__file__).resolve().parents[3] / "web" / "electron" / "setup" / "index.html"

# The setup page expects the Electron preload bridge (window.codifySetup),
...
def test_bare_workspace_url_connects_without_http_warning(page: Page) -> None:
    """A schemeless ``<ws>/codify`` connects on the first click, no warning.
    ...
    page.fill("#url", "dbc-x.cloud.databricks.com/codify")
    page.click("#connect")

    page.wait_for_function("() => window.__connectCalls.length === 1")
    assert page.evaluate("() => window.__connectCalls") == ["dbc-x.cloud.databricks.com/codify"]
...
def test_shared_url_module_defaults_scheme_in_browser(page: Page) -> None:
    ...
    ``window.codifyUrl`` — the exact module the Electron main process uses to
    ...
    # Remote host → https; the guide's /codify suffix is preserved.
    assert (
        page.evaluate("() => window.codifyUrl.normalizeUrl('dbc-x.cloud.databricks.com/codify')")
        == "https://dbc-x.cloud.databricks.com/codify"
    )
    # Loopback stays http for local dev.
    assert (
        page.evaluate("() => window.codifyUrl.normalizeUrl('localhost:6767')")
        == "http://localhost:6767/"
    )
