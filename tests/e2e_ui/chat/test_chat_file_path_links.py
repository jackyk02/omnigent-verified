...
under ``Path.home()``: with no ``CODIFY_RUNNER_WORKSPACE`` set (the e2e
...
def _agent_bundle(cwd: str) -> bytes:
    ...
    through the codify compat adapter, matching the conftest helpers.
...
def linkify_session(
    ...
    ws = Path(tempfile.mkdtemp(prefix="codify-e2e-ui-links-", dir=Path.home()))
