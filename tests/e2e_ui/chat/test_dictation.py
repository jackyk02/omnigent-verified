...
→ the server's fake engine (``CODIFY_DICTATION_ENGINE=fake``, set by the
...
- ``/v1/info`` stopped advertising ``dictation_available`` (capability
  plumbing in ``codify/server/app.py`` or ``web/src/lib/capabilities.ts``).
- The WebSocket route broke (``codify/server/routes/dictation.py``).
...
from codify.server.dictation import FAKE_SCRIPT as _FAKE_SCRIPT
