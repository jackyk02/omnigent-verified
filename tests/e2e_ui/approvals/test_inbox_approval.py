...
codify#927: when a hook retry re-parks the *same* elicitation id after the
...
def _permission_hook_payload(elicitation_id: str) -> dict:
    ...
    Mirrors the ``codify claude`` wrapper's hook subprocess: the
    ``_codify_elicitation_id`` is minted once per prompt and re-sent on
    ...
    return {
        ...
        "_codify_elicitation_id": elicitation_id,
    }
...
def test_reparked_elicitation_reliably_resurfaces_in_inbox(
    ...
    Regression for codify#927. When the user approves an inbox approval, the
    ...
    codify#927 shape of a single prompt whose hook keeps re-parking the same
    ...
    # A single elicitation id, re-parked repeatedly — the codify#927 scenario
