...
The suite spawns a real ``codify server --agent`` subprocess against
...
Local usage::
    ...
    cd web && npm run dev &
    codify server --agent examples/hello_world.yaml &
    CODIFY_E2E_ALLOW_DEV_BASE_URL=1 \
      uv run pytest tests/e2e_ui --ui-base-url http://127.0.0.1:5173

``codify server`` is documented at ``codify/cli.py:server``:
it spins up uvicorn with the Codify app and spawns an out-of-process
...
_ALLOW_DEV_BASE_URL_ENV = "CODIFY_E2E_ALLOW_DEV_BASE_URL"
_CODEX_GOAL_MIN_VERSION = (0, 139, 0)
_PUBLIC_LOOPBACK_HOST = "codify-e2e-public.test"
...
_BUILD_OUTPUT = _REPO_ROOT / "codify" / "server" / "static" / "web-ui"

# ``codify server --agent`` runs the spec through the strict
...
# executor.type is 'codify'``. The model name (gpt-4o-mini) is a plain
...
# codify/runtime/workflow.py) and fails with DatabricksAuthError in CI.
_TEST_AGENT_YAML = """\
name: hello_world
...
def _build_hello_world_bundle() -> bytes:
    ...
    through the codify compat adapter (which translates
    ``executor.harness`` → ``executor.config.harness`` and sets
    ``executor.type: codify``). Using ``config.yaml`` would
...
# startup via ``CODIFY_BUILTIN_AGENT_DIRS``, so ``live_server`` writes
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    parser.addoption(
        ...
        default=False,
        help=(
            "Reuse whatever's already in codify/server/static/web-ui/ "
...
def browser_type_launch_args(
    ...
    if os.environ.get("CODIFY_PW_NO_SANDBOX"):
        launch_args["args"] = [
            *launch_args.get("args", []),
            "--no-sandbox",
...
def _register_agent_yaml(
    ...
    *,
    ...
    ``arcname`` defaults to ``config.yaml`` for native Codify specs. Pass a
    ``*.yaml`` filename for codify-flavored single-file specs; the
    compat loader only routes those through the codify translator when
...
def mock_llm_server_url(
    ...
    fixture points the spawned codify server at this URL so all agent
...
def _assert_pwa_build(build_output: Path) -> None:
    ...
    cache or serve the app shell: Codify is a cloud app, so a stale cached
...
def built_spa(request: pytest.FixtureRequest) -> None:
    """
    Build the web SPA into ``codify/server/static/web-ui/``.
...
def _spawn_runner_against_external_server(
    ...
    ``codify server`` process (and its pre-registered ``hello_world``
    agent), but the runner-bound fixtures still need a runner id this
    process controls. Mirrors :func:`codify.cli._start_cli_runner_process`
    ...
    :func:`codify.runner.identity.token_bound_runner_id`, so we use
    ...
    env = {
        **os.environ,
        ...
        "CODIFY_RUNNER_ID": runner_id,
        "CODIFY_RUNNER_TUNNEL_BINDING_TOKEN": binding_token,
        "CODIFY_RUNNER_PARENT_PID": str(os.getpid()),
...
def live_server(
    ...
    """
    Spawn ``codify server --agent examples/hello_world.yaml`` and
    yield its base URL.
    ...
    runner_id = token_bound_runner_id(binding_token)
    # PYTHONPATH forces the subprocess to import codify from
    ...
    # CODIFY_RUNNER_TUNNEL_TOKEN lets the server accept
    ...
    env = {
        **os.environ,
        ...
        "CODIFY_BUILTIN_AGENT_DIRS": os.pathsep.join(builtin_dirs),
        ...
        "CODIFY_DICTATION_ENGINE": os.environ.get("CODIFY_DICTATION_ENGINE", "fake"),
    }
    ...
    proc = subprocess.Popen(
        [
            ...
            # Mirrors ``python -m codify`` (codify/__main__.py).
            "-c",
            "import codify.server.presence as _p; _p._LEAVE_GRACE_S = 1.0; "
            + "from codify.cli import main; main()",
            "server",
            "--host",
            "127.0.0.1",
            "--port",
    ...
    if not ready:
        ...
        log_text = log_path.read_text() if log_path.exists() else ""
        raise RuntimeError(
            f"`codify server` did not become healthy within "
...
def seeded_session(
    ...
    :param live_server: Spawned server fixture — its
        ``CODIFY_RUNNER_ID`` and pre-registered agent are reused.
...
# codify-flavored single-file YAML (intentionally no
# ``spec_version``): AP-native YAML currently ignores ``terminals:``,
# while the codify-compat translator threads the declaration into
...
def terminal_session(
    ...
    with (
        ...
        # Use the codify shorthand YAML with a non-config.yaml
...
def _two_agent_chat_yaml(verification_code: str, question_code: str) -> str:
    ...
    (Deep Thought) — the codify-flavored shape parsed by
    ``codify/inner/loader.py:_parse_tool``, same as the
...
def two_agent_chat_session(
    ...
    buf = io.BytesIO()
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        # Non-config.yaml arcname routes the bundle through the codify
...
guardrails:
  ...
  policies:
    blast_radius:
      type: function
      function:
        path: codify.inner.nessie.policies.blast_radius
        arguments:
...
def server_pid(live_server: str) -> int:
    """
    PID of the ``codify server`` process spawned by
    :func:`live_server`.
...
# spec ``codify claude`` ships — and yields ``(base_url, session_id)``. The
...
# the same terminal-first spec ``codify codex`` ships — and yields
...
def _create_native_claude_session(
    ...
    *,
    ...
    Reuses the exact terminal-first spec ``codify claude`` ships
    (:func:`codify.claude_native._materialize_claude_agent_spec`) so the
    fixture never drifts from production, and stamps the same wrapper /
    terminal-first labels (``codify.wrapper`` + ``codify.ui = terminal``)
    the CLI writes. The spec carries no ``spec_version``, so it is bundled
    under a ``*.yaml`` arcname to route through the codify compat translator
    ...
    :param terminal_launch_args: Pass-through ``claude`` CLI args persisted on
        ...
        ``_build_claude_native_base_args`` in ``codify/runner/app.py``). Used
    ...
    from codify.claude_native import _materialize_claude_agent_spec
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        data = yaml_text.encode()
        # Non-config.yaml arcname → codify compat translator (the spec has
...
def _create_native_codex_session(base_url: str, runner_id: str) -> str:
    ...
    Reuses the exact terminal-first spec ``codify codex`` ships
    (:func:`codify.codex_native._materialize_codex_agent_spec`) so the
    fixture never drifts from production, and stamps the same wrapper /
    terminal-first labels (``codify.wrapper`` + ``codify.ui = terminal``)
    the CLI writes. The spec carries no ``spec_version``, so it is bundled
    under a ``*.yaml`` arcname to route through the codify compat translator
    ...
    from codify.codex_native import _materialize_codex_agent_spec
    ...
    # ``workspace`` nor CODIFY_RUNNER_WORKSPACE is set. Pin it on THIS
    # session only (via metadata.workspace) rather than exporting
    # CODIFY_RUNNER_WORKSPACE on the shared runner — a runner-wide value
...
def _temp_codify_mock_config(
    mock_llm_server_url: str, harness: str
) -> Generator[None, None, None]:
    """Temporarily write a mock provider config to ~/.codify/config.yaml.
...
def native_claude_mock_session(
    ...
    credentials), the existing ``~/.codify/config.yaml`` is left untouched so
    ...
    ``~/.codify/config.yaml`` and restored on teardown.
    ...
    if use_mock:
        ctx: Any = _temp_codify_mock_config(mock_llm_server_url, "claude")
    else:
...
def native_codex_mock_session(
    ...
    use_mock = not os.environ.get("LLM_API_KEY")
    if use_mock:
        ctx: Any = _temp_codify_mock_config(mock_llm_server_url, "codex")
    else:
        ctx = contextlib.nullcontext()
    with ctx:
        session_id = _create_native_codex_session(live_server, runner_id)
...
def mocked_native_codex_goal_session(
    ...
    the mock ``CODIFY_CONFIG_HOME`` / ``HOME`` / source ``CODEX_HOME`` must
    ...
    shared_env = {
        **os.environ,
        "PYTHONPATH": f"{_REPO_ROOT}{os.pathsep}{os.environ.get('PYTHONPATH', '')}",
        "CODIFY_CONFIG_HOME": str(config_home),
        "CODIFY_CODEX_NATIVE_STATE_DIR": str(state_dir),
...
# session — the same terminal-first spec ``codify cursor`` ships — and yields
# ``(base_url, session_id)``. The runner auto-launches ``cursor-agent`` in the
# session terminal on bind (``_auto_create_cursor_terminal`` in
# ``codify/runner/app.py``), exactly like the claude/codex fixtures.
#
...
def _create_native_cursor_session(
    ...
    Reuses the exact terminal-first spec ``codify cursor`` ships
    (:func:`codify.cursor_native._materialize_cursor_agent_spec`) so the
    fixture never drifts from production, and stamps the same wrapper /
    terminal-first labels (``codify.wrapper`` + ``codify.ui = terminal``)
    the CLI writes. The spec carries no ``spec_version``, so it is bundled
    under a ``*.yaml`` arcname to route through the codify compat translator
    ...
    auto-bootstrap (:func:`codify.runner.app._auto_create_cursor_terminal`):
    ...
    from codify.cursor_native import _materialize_cursor_agent_spec
...
def _create_native_goose_session(base_url: str, runner_id: str) -> str:
    ...
    spec ``codify goose`` ships
    (:func:`codify.goose_native._materialize_goose_agent_spec`) and stamps the
    same wrapper / terminal-first labels. Binding triggers the runner's
    goose-native auto-bootstrap
    (:func:`codify.runner.app._auto_create_goose_terminal`), which launches
    ...
    from codify.goose_native import _materialize_goose_agent_spec
...
def _create_native_kiro_session(base_url: str, runner_id: str) -> str:
    ...
    ``codify kiro`` ships
    (:func:`codify.kiro_native._materialize_kiro_agent_spec`) and stamps the
    same wrapper / terminal-first labels. Binding triggers the runner's
    kiro-native auto-bootstrap
    (:func:`codify.runner.app._auto_create_kiro_terminal`), which launches the
    ...
    from codify.kiro_native import _materialize_kiro_agent_spec
...
def _create_native_hermes_session(base_url: str, runner_id: str) -> str:
    ...
    spec ``codify hermes`` ships
    (:func:`codify.hermes_native._materialize_hermes_agent_spec`) and stamps the
    same wrapper / terminal-first labels. Binding triggers the runner's
    hermes-native auto-bootstrap
    (:func:`codify.runner.app._auto_create_hermes_terminal`), which launches the
    ...
    from codify.hermes_native import _materialize_hermes_agent_spec
...
def native_cursor_approval_session(
    ...
    side mirror (:mod:`codify.cursor_native_permissions`) surfaces those as
