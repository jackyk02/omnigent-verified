"""Shared helper: spawn a dedicated *accounts-mode* Codify server with the
device-authorization grant enabled.

The suite's shared ``live_server`` runs single-user
(``CODIFY_LOCAL_SINGLE_USER=1``, set in ``tests/conftest.py``) with auth
...
checks), the device grant mounted (``CODIFY_DEVICE_GRANT_ENABLED=1``), and a
seeded admin the browser can sign in as.

This spins one up with a pre-seeded admin (``CODIFY_ACCOUNTS_INIT_ADMIN_*``)
...
def spawn_accounts_server(mock_llm_server_url: str, server_tmp) -> Iterator[AccountsServer]:
    ...
    server_env = {
        **os.environ,
        ...
        "CODIFY_ACCOUNTS_COOKIE_SECRET": secrets.token_hex(32),
        "CODIFY_ACCOUNTS_BASE_URL": public_url,
        "CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME": ADMIN_USERNAME,
        "CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD": ADMIN_PASSWORD,
        "CODIFY_ACCOUNTS_AUTO_OPEN": "0",
        "CODIFY_ADMIN_CREDENTIALS_PATH": str(server_tmp / "admin-creds"),
        # Mount the /oauth/* device routes (opt-in, default-off).
        "CODIFY_DEVICE_GRANT_ENABLED": "1",
