...
server's ``CODIFY_SHARING_MODE`` is ``off``.
...
NON-single-user server with ``CODIFY_SHARING_MODE=off`` and drives the real
...
_OFF_REASON = "Sharing has been disabled for this Codify server."
...
def sharing_off_server(
    ...
    """A dedicated NON-single-user server with ``CODIFY_SHARING_MODE=off``.
    ...
    yield from spawn_multi_user_server(
        mock_llm_server_url,
        server_tmp,
        extra_server_env={"CODIFY_SHARING_MODE": "off"},
    )
...
def test_sharing_off_disables_share_button_with_tooltip(
    ...
    """``CODIFY_SHARING_MODE=off`` grays out the header Share button and
