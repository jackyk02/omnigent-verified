"""Shared helper: spawn a dedicated *multi-user* header-auth Codify server.

The suite's shared ``live_server`` runs single-user
(``CODIFY_LOCAL_SINGLE_USER=1``, set in ``tests/conftest.py``), where the
...
declared via ``CODIFY_ADMINS`` so a header-identified browser can manage, and
...
# admin-list file (CODIFY_ADMIN_LIST_PATH) so it resolves as an admin —
...
def spawn_multi_user_server(
    ...
    *,
    ...
    # see codify/server/admin_list.py). There is no admin *env var* — the
    # roster is the config ``admins:`` list or this file — so point
    # CODIFY_ADMIN_LIST_PATH at it. This makes ADMIN_EMAIL resolve as admin
    ...
    server_env = {
        **os.environ,
        ...
        "CODIFY_ADMIN_LIST_PATH": str(admins_path),
