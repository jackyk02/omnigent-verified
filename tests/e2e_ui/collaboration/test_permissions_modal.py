...
def test_single_user_hides_share_button(
    ...
    """The shared e2e server is single-user (CODIFY_LOCAL_SINGLE_USER=1), so
...
def test_share_modal_qr_code_opens_mobile_deep_link(
    ...
    session's ``codify://<host>/c/<id>`` deep link.
    ...
    qr_svg = qr_dialog.locator("[aria-label='QR code to open this session in the Codify app']")
