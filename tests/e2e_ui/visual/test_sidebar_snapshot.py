...
# per-user `codify.pinned.<user>` key back to this bare key on the wire). Its
# value is the epoch-ms pin time; any non-empty value means pinned.
_PINNED_LABEL_KEY = "codify.pinned"
...
def test_populated_sidebar_matches_baseline(
    ...
    :param live_server: Base URL of the spawned ``codify server`` serving the
    ...
    page.add_init_script(
        f'window.localStorage.setItem("codify:recent-workspaces",'
        f' JSON.stringify({{"{_HOST_ID}": ["/work/repo"]}}));'
        f'window.localStorage.setItem("codify:expanded-project-sections",'
