...
# CODIFY_CONTAINER_RUNTIME=podman to use Podman instead of Docker.  It:
...
#
#   --skip-build  Reuse an existing codify/server/static/web-ui build (e.g.
...
NPM_VERSION="11.12.1"
BUILD_OUTPUT="codify/server/static/web-ui"
...
CONTAINER_RUNTIME="${CODIFY_CONTAINER_RUNTIME:-docker}"
...
"$CONTAINER_RUNTIME" run --rm --platform "$PLATFORM" -v "$PWD":/work -w /work \
  ...
  -e CODIFY_PW_NO_SANDBOX=1 \
  -e CODIFY_SKIP_WEB_UI=true \
