...
_BOOT_MODULES = (
    ...
    "codify.runtime",
    "codify.server.app",
    "codify.server.server_config",
    "codify.stores.agent_store.sqlalchemy_store",
    "codify.stores.artifact_store.local",
    "uvicorn",
)
...
# CODIFY_ARTIFACT_URI=s3://… selects the remote S3ArtifactStore (durable on an
...
def _entrypoint_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    # ambient ~/.codify/config.yaml (keeps the test hermetic; CI has none).
    config_file = tmp_path / "config.yaml"
    config_file.write_text("{}\n")
    monkeypatch.setenv("CODIFY_CONFIG", str(config_file))
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@localhost:5432/codify")
    monkeypatch.setenv("ARTIFACT_DIR", str(tmp_path / "artifacts"))
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "0")
    monkeypatch.delenv("CODIFY_ARTIFACT_URI", raising=False)
...
def test_resolve_config_captures_s3_artifact_uri(
    ...
    monkeypatch.setenv("CODIFY_ARTIFACT_URI", "s3://my-bucket/artifacts")
...
def test_resolve_config_rejects_non_s3_artifact_uri(
    ...
    monkeypatch.setenv("CODIFY_ARTIFACT_URI", "gs://my-bucket")
...
def test_select_artifact_store(
    ...
    resolved = _ResolvedConfig(
        cfg={},
        database_url="postgresql://u:p@localhost/codify",
