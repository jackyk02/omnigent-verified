...
``CODIFY_TEST_MODEL_SPREAD=1``, set in e2e CI) and rotates to a
different model on each ``@pytest.mark.llm_flaky`` rerun.
``@pytest.mark.model_pinned`` opts a test out of both. Pools are
overridable via ``CODIFY_TEST_MODEL_POOL_<KEY>`` (comma-separated).
...
_SPREAD_ENV_VAR = "CODIFY_TEST_MODEL_SPREAD"
_POOL_ENV_PREFIX = "CODIFY_TEST_MODEL_POOL_"
...
def spread_enabled() -> bool:
    """Whether hash-spreading is enabled via ``CODIFY_TEST_MODEL_SPREAD``."""
