...
from codify import codex_native, codex_native_app_server, codex_native_forwarder
...
from codify.codex_native_elicitation import codex_elicitation_id
...
def _stub_catalog_default(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "codify.model_catalog.resolve_catalog_model",
...
def test_codex_auth_unavailable_reason_provider_override_available(
    ...
    auth_path = tmp_path / "codex-home" / "auth.json"  # never created
    launch = codex_native_app_server.NativeCodexLaunch(
        config_overrides=['model_provider="codify_databricks"'], model=None, profile=None
    )
    _point_codex_auth_check_at(monkeypatch, auth_path, binary_present=True, launch=launch)
...
def test_preload_codex_thread_for_resume_resumes_and_closes(
    ...
    monkeypatch.setattr(
        "codify.codex_native_app_server.CodexAppServerClient",
        fake_client_factory,
    )
...
def _expected_delta_data(
    ...
    *,
    ...
    """
    Build the Codify event data expected for one Codex native text delta.
...
def _expected_status_data(status: str, turn_id: str) -> dict[str, Any]:
    """
    Build the Codify event data expected for one Codex native status edge.

    :param status: Codify session status, e.g. ``"running"``.
    :param turn_id: Codex turn id, e.g. ``"turn_123"``.
...
def _usage_coalescer(
    ...
    :param session_id: Codify session id, e.g. ``"conv_123"``.
...
def test_materialized_codex_agent_spec_loads_as_valid_codify_yaml(
    tmp_path: Path,
) -> None:
    """
    The generated wrapper spec passes Codify YAML validation.
...
@pytest.mark.parametrize(
    ...
    [
        # Fresh thread over a Unix socket (local ``codify codex``
    ...
    [
        ...
        (
            ...
            [
                ...
                "-c",
                'model_provider="codify_databricks"',
                "--remote",
                "ws://127.0.0.1:9876",
            ],
        ),
...
def test_build_codex_remote_args_bypass_emits_flag_and_strips_conflicts(
    ...
    See :func:`codify.codex_native_app_server._strip_approval_sandbox_flags`.
...
def test_codex_app_server_client_responds_to_server_requests(
    ...
    app-server to the Codify client. After AP/web resolves the
...
def test_wait_for_thread_started_fails_when_stream_ends() -> None:
    ...
    of leaving the Codify session without a bridge state.
    """
...
def test_supervise_forwarder_resumes_when_it_opens_client(
    ...
    monkeypatch.setattr("codify.codex_native_app_server.CodexAppServerClient", fake_client_factory)
...
def test_subscribe_until_ready_replays_completed_turn_status(
    ...
    turn state instead of leaving the Codify session running forever.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture Codify event posts from the forwarder.
...
def test_forwarder_ignores_thread_started_for_current_codex_thread(tmp_path: Path) -> None:
    """
    A duplicate ``thread/started`` notification does not rotate Codify sessions.
...
def test_forwarder_rotates_session_on_new_codex_thread_and_posts_to_new_session(
    ...
    """
    Native Codex thread switches create a replacement Codify session.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve Codify calls made during Codex session rotation.

        :param request: HTTP request from the forwarder.
        :returns: Fake Codify response.
        """
        ...
        if request.method == "GET" and request.url.path == "/v1/sessions/conv_old":
            return httpx.Response(
                ...
                json={
                    ...
                    "labels": {
                        "codify.wrapper": "codex-native-ui",
                        "codify.codex_native.bridge_id": "bridge_shared",
                    },
                },
            )
...
def test_forwarder_rotation_failure_preserves_old_target(
    ...
    If Codify rejects replacement-session creation, the forwarder logs the
    event-handler failure and continues. The old target must remain
    intact; closing its coalescer before the Codify move succeeds would
    ...
    class _FakeCoalescer:
        ...
        :param session_id: Codify session id represented by this fake,
            e.g. ``"conv_old"``.
        """
    ...
    async def fail_create_thread_replacement_session(**_kwargs: object) -> str:
        """
        Simulate Codify rejecting the replacement-session operation.

        :returns: Never returns successfully.
        :raises RuntimeError: Always raised to model Codify failure.
        """
...
def test_forwarder_posts_active_codex_agent_message_delta(tmp_path: Path) -> None:
    """
    Codex assistant deltas are forwarded as transient Codify text deltas.
    ...
    item, so this asserts on the exact Codify event envelope.
    """
    write_bridge_state(
        tmp_path,
        CodexNativeBridgeState(
...
def test_forwarder_persists_interrupted_codex_partial_agent_message(tmp_path: Path) -> None:
    ...
    Without this fallback, Codify Web shows the streamed text live but loses it
...
def test_forwarder_posts_active_codex_plan_delta(tmp_path: Path) -> None:
    """
    Codex plan deltas are forwarded as transient Codify text deltas.

    Plan mode uses ``item/plan/delta`` while rendering the visible
    plan. If this branch is missing, Codify web stays blank even though the
...
def test_forwarder_coalesces_codex_agent_message_deltas(tmp_path: Path) -> None:
    """
    Native Codex streaming does not post one Codify event per tiny delta.

    Breaking the coalescer would recreate the slow-drain failure where
    Codex finishes locally while the Codify SSE stream is still serialized
...
def test_forwarder_posts_codex_usage_live_per_frame(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture Codify event posts from both coalescers.
...
def test_output_text_delta_coalescer_auto_flushes(
    ...
    Codify post directly instead of calling ``flush()``, so removing any
    ...
    :param expected_delta: Coalesced Codify delta payload.
    :returns: None.
    """
    posted: list[dict[str, Any]] = []
    ...
    async def run() -> None:
        """
        Append deltas and wait for the automatic Codify post.
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            """
            Capture Codify event posts from the coalescer.
...
def test_supervise_forwarder_continues_after_event_handler_failure(
    ...
    async def fake_handle_event(
        ...
        *,
        ...
        :param _client: Codify HTTP client.
        :param session_id: Codify session id, e.g. ``"conv_123"``.
...
def test_forwarder_sends_codex_mcp_elicitation_response_to_app_server(
    ...
    """
    Codex MCP elicitation requests are forwarded to Codify and the Codify hook
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture the Codify hook request and return an accepted MCP result.

        :param request: HTTP request sent by the forwarder.
        :returns: Codify hook response.
        """
...
def test_forwarder_keeps_streaming_when_native_tui_answers_codex_elicitation(
    ...
    """
    Native TUI approval must not park the Codify web mirror.

    The Codify hook remains pending when a separate native Codex client
    answers the prompt first. Codex app-server emits
    ``serverRequest/resolved`` with the original request id; the
    forwarder must mirror that exact resolution to Codify and still mirror
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        """
        Hold the hook open and capture subsequent Codify event posts.

        :param request: HTTP request sent by the forwarder.
        :returns: Codify event response for non-hook posts.
        """
...
def test_forwarder_sends_codex_request_user_input_response_to_app_server(
    ...
    """
    Codex requestUserInput frames use the same Codify hook path and relay
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a requestUserInput result from the Codify hook.
...
def test_forwarder_flushes_plan_text_before_codex_request_user_input(
    ...
    """
    Buffered plan deltas reach Codify before the final plan prompt.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture Codify posts in arrival order.

        :param request: HTTP request sent by the forwarder.
        :returns: Codify response appropriate to the endpoint.
        """
...
def test_forwarder_synthesizes_plan_implementation_prompt_after_completed_plan_turn(
    ...
    """
    Completed Plan-mode turns surface the final implementation prompt in Codify Web.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture Codify posts and decline the synthesized prompt.
...
def test_forwarder_starts_default_turn_from_plan_implementation_prompt(
    ...
    the answer back into Codex app-server actions, Codify Web would look
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        :param request: HTTP request sent by the forwarder.
        :returns: Codify hook or event response.
        """
        if request.url.path.endswith("/events"):
            return httpx.Response(202, json={"queued": False})
        return httpx.Response(
...
def test_forwarder_starts_fresh_thread_from_clear_context_plan_prompt(
    ...
    This mirrors the terminal TUI action closely enough for Codify Web:
...
def test_forwarder_sends_codex_command_approval_response_to_app_server(
    ...
    """
    Codex command-approval request frames use the Codify hook path and
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a command approval result from the Codify hook.
...
def test_forwarder_sends_codex_permissions_response_to_app_server(
    ...
    """
    Codex permission-profile request frames are relayed through Codify and
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a permissions approval result from the Codify hook.
...
def test_forwarder_logs_unsupported_codex_server_request(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        """
        raise AssertionError(f"unexpected Codify request: {request.method} {request.url}")
...
def test_forwarder_leaves_codex_elicitation_pending_on_empty_hook_body(
    ...
    """
    Empty Codify hook responses represent timeout/disconnect fallback, not
    ...
    def handler(_request: httpx.Request) -> httpx.Response:
        """
        Return the Codify hook's fail-ask shape.
...
def test_forwarder_posts_codex_user_and_agent_messages(tmp_path: Path) -> None:
    ...
    external conversation items for the Codify session stream.
    """
    posted: list[dict[str, Any]] = []
...
def test_forwarder_recovers_missed_user_message_before_assistant(tmp_path: Path) -> None:
    ...
    BEFORE the reply so Codify assigns it the earlier position.
    """
    posted: list[dict[str, Any]] = []
...
def test_forwarder_posts_completed_codex_plan_item() -> None:
    """
    Completed Codex ``plan`` thread items are mirrored into Codify history.
...
def _capture_handler(posted: list[dict[str, Any]]) -> Callable[[httpx.Request], httpx.Response]:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture one Codify event post from the forwarder.
...
async def _replay_completed_item(
    ...
    :param handler: MockTransport handler capturing the Codify posts.
    :returns: None.
    """
    async with httpx.AsyncClient(
        base_url="http://127.0.0.1:8000",
...
def test_forwarder_posts_codex_command_execution_tool_call() -> None:
    ...
    aggregated output) must be mirrored as the Codify ``function_call`` /
...
def test_forwarder_retries_transient_external_item_rejection(
    ...
    """
    Transient Codify failures do not drop the mirrored Codex item.
...
def test_forwarder_logs_rejected_external_item(
    ...
    """
    Codify 4xx responses are logged so mirror failures are diagnosable.
    """
...
def test_forwarder_marks_codex_skill_user_message_as_meta(tmp_path: Path) -> None:
    ...
    for Codify resume/history replay while tagging it ``is_meta`` so UI
    clients can hide it.
    """
    posted: list[dict[str, Any]] = []
...
def test_local_run_prints_resume_hint_after_attach(
    ...
    """
    Local ``codify codex`` prints a copyable resume hint on exit.
    ...
    monkeypatch.setattr("codify.chat._find_free_port", lambda: 23456)
    monkeypatch.setattr("codify.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("codify.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("codify.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr("codify.chat._bundle_agent", lambda path: b"bundle")
    ...
    resume_hint = "Resume with: codify codex --resume conv_codex_fresh"
...
def test_local_resume_does_not_print_redundant_resume_hint(
    ...
    """
    ``codify codex --resume`` does not echo another resume prompt.
    ...
    monkeypatch.setattr("codify.chat._find_free_port", lambda: 23457)
    monkeypatch.setattr("codify.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("codify.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("codify.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr(codex_native, "_prepare_codex_terminal", fake_prepare)
...
def test_record_launch_for_fresh_session_persists_current_cwd(
    ...
    :returns: None.
    """
    from codify.codex_native_state import read_launch_state

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    monkeypatch.chdir(workspace)
    monkeypatch.setenv("CODIFY_CODEX_NATIVE_STATE_DIR", str(tmp_path / "state"))
...
def test_align_working_directory_with_session_matching_cwd_is_noop(
    ...
    :returns: None.
    """
    from codify.codex_native_state import write_launch_state
...
def test_run_with_remote_server_aligns_cwd_before_daemon_prepare(
    ...
    :returns: None.
    """
    import codify.chat as chat_mod
    import codify.cli as cli_mod
    import codify.host.identity as identity_mod
...
async def test_prepare_codex_terminal_via_daemon_creates_runner_and_ensures_terminal(
    ...
    against an ``httpx.MockTransport`` Codify server. Removing terminal launch arg
    persistence, daemon runner launch, the runner re-bind (which clears
    ``codify.stopped`` on resume), the ``ensure_native_terminal``
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Route Codify requests issued by daemon preparation.

        :param request: Incoming mock HTTP request.
        :returns: Mock Codify response.
        """
    ...
    def client_factory(*args: object, **kwargs: object) -> httpx.AsyncClient:
        """
        Inject the mock Codify transport into clients created by the helper.
    ...
    # Runner re-bind clears codify.stopped on resume.
...
async def test_prepare_codex_terminal_via_daemon_live_resume_skips_config_patch(
    ...
    rollout from Codify history while the app-server may be appending to it.
    ...
    calls: list[tuple[str, str, object]] = []
    thread_id = "019e96aa-0be2-7343-8d3b-6f914d60936b"
    monkeypatch.setattr("codify.codex_native_bridge._BRIDGE_ROOT", tmp_path / "bridges")
    from codify.codex_native_bridge import bridge_dir_for_bridge_id, codex_home_for_bridge_dir
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Route Codify requests for a live resume.
        ...
        if request.method == "GET" and path == "/v1/sessions/conv_live":
            return httpx.Response(
                200,
                json={
                    "labels": {"codify.wrapper": "codex-native-ui"},
...
async def test_prepare_codex_terminal_hot_resume_does_not_rewrite_rollout(
    ...
    rewrite rollout files from Codify history because Codex may be appending
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve a codex-native session, live terminal, and Codify item history.
        ...
        if request.method == "GET" and path == f"/v1/sessions/{session_id}":
            return httpx.Response(
                ...
                json={
                    "labels": {
                        "codify.wrapper": "codex-native-ui",
                        "codify.codex_native.bridge_id": bridge_id,
                    },
                    "external_session_id": thread_id,
                },
            )
...
async def test_find_running_codex_terminal_known_misses_relaunch(
    ...
    :param status_code: HTTP status returned by the Codify resource lookup.
    :param body: Structured Codify error body for the lookup.
    :returns: None.
    """
...
def test_attach_with_forwarder_uses_direct_tmux_when_socket_is_local(
    ...
    monkeypatch.setattr("codify.codex_native.shutil.which", lambda _name: "/usr/bin/tmux")
...
def test_attach_with_forwarder_closes_active_rotated_session_terminal(
    ...
    """
    Wrapper exit closes the terminal on the active rotated Codify session.
    ...
    async def fake_attach_terminal_resource(**_kwargs: object) -> None:
        """
        Simulate ``/clear`` rotating Codify ownership during attach.
...
def test_attach_terminal_resource_runner_owned_missing_socket_fails_loud(
    ...
    the Codify terminal WebSocket would reintroduce CLI-owned terminal IO.
...
def test_attach_with_forwarder_falls_back_when_tmux_socket_is_not_local(
    ...
    """
    Non-local runner sockets keep using the Codify terminal attach bridge.
...
def test_session_usage_data_extracts_cumulative_tokens() -> None:
    ...
    stays 0 (codex produces no ``response.completed`` for the Codify relay).
    """
    params = {
        "threadId": "thread_123",
        "tokenUsage": {
...
def test_usage_coalescer_flush_attaches_model_to_every_post() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture Codify usage posts from the coalescer.
...
def _transcript_posts(
    ...
    """
    Filter Codify posts to the ``external_conversation_item`` events for one session.

    :param posted: All captured Codify posts as ``(path, body)`` tuples.
    :param session_id: Codify session id to filter for.
...
def _registration_posts(
    ...
    """
    Filter Codify posts to the ``external_codex_subagent_start`` events for a parent.

    :param posted: All captured Codify posts as ``(path, body)`` tuples.
    :param parent_session_id: Parent Codify session id to filter for.
...
def _make_codify_handler(
    posted: list[tuple[str, dict[str, Any]]],
    child_session_id: str = "conv_child",
) -> Callable[[httpx.Request], httpx.Response]:
    """
    Build an Codify ``MockTransport`` handler that registers a child session on demand.

    :param posted: Mutable list collecting all captured Codify requests.
    :param child_session_id: Codify child session id to return for
...
def test_forwarder_dedupes_replay_and_live_child_item(
    ...
    async def run() -> None:
        ...
        await codex_native_forwarder.supervise_forwarder(
            ...
            ap_transport=httpx.MockTransport(_make_codify_handler(posted)),
        )
...
def test_forwarder_registers_subagent_activity_before_child_events(
    ...
    assert registrations, "subAgentActivity(kind=started) must create an Codify child session"
...
def test_forwarder_does_not_double_write_stable_id_item_delivered_twice(
    ...
    async def run() -> None:
        ...
        async with httpx.AsyncClient(
            base_url="http://127.0.0.1:8000",
            transport=httpx.MockTransport(_make_codify_handler(posted)),
        ) as client:
...
def test_forwarder_child_thread_started_does_not_rotate_parent_session(
    ...
    rotate the parent Codify session.
    ...
    by the rotation check, otherwise the parent's Codify session would be
    ...
    async def run() -> bool:
        ...
        async with httpx.AsyncClient(
            base_url="http://127.0.0.1:8000",
            transport=httpx.MockTransport(_make_codify_handler(ap_posts)),
        ) as ap_client:
            target = codex_native_forwarder._ForwarderTarget(
                session_id="conv_parent",
                thread_id="thread_parent",
    ...
    # No Codify calls should have been made during rotation detection.
    assert ap_posts == []
...
def test_forwarder_routes_live_child_items_to_child_session(
    ...
    to the child Codify session, not the parent.
...
def test_forwarder_resolves_child_thread_elicitation_on_child_session(
    ...
    published on the child Codify session (``route_session_id``). Codex's
...
@pytest.mark.asyncio
async def test_ensure_local_codex_resume_rollout_synthesizes_codify_history(
    tmp_path: Path,
) -> None:
    """
    Cross-machine Codex resume rebuilds a local rollout from Codify history.

    The server can know the Codify conversation and Codex thread id while the
    current host has no ``$CODEX_HOME/sessions/.../rollout-*-<thread>.jsonl``.
    This helper must fetch committed Codify items, follow pagination, and write
    ...
    assert records[0]["payload"]["model_provider"] == "codify_databricks"
...
async def test_ensure_local_codex_resume_rollout_preserves_existing_rollout(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Fail if Codify history is fetched despite a local rollout.

        :param request: Incoming mock HTTP request.
        :returns: Mock Codify response.
        """
        del request
        raise AssertionError("existing rollout should avoid Codify history fetch")
...
async def test_ensure_local_codex_resume_rollout_rejects_malformed_tool_history(
    ...
    """
    Codex rollout synthesis fails loudly for corrupt Codify tool history.

    Tool call ``arguments`` and tool ``output`` are required Codify string
    fields. Missing values must not be invented as ``{}`` or ``""``,
    because Codex would then resume a tool history that never happened.

    :param tmp_path: Temporary directory for isolated ``CODEX_HOME``.
    :param bad_item: Malformed Codify item to serve.
    :param message: Expected diagnostic substring.
    """

    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve malformed Codify history.

        :param request: Incoming mock HTTP request.
        :returns: Mock Codify item page.
        """
        assert request.url.path == "/v1/sessions/conv_codex/items"
...
async def test_ensure_local_codex_resume_rollout_rejects_unsafe_thread_id(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Fail if Codify history is fetched for an unsafe thread id.

        :param request: Incoming mock HTTP request.
        :returns: Never returns.
        """
        del request
        raise AssertionError("unsafe thread id should be rejected before Codify fetch")
...
def test_resolve_native_codex_launch_no_provider_sets_login_fallback_summary(
    ...
    from codify.onboarding import detected, provider_config
    from codify.runtime import workflow
...
def test_codex_discover_thread_and_forward_writes_routing_summary_on_timeout(
    ...
    from codify import codex_native_forwarder as _fwd
    from codify.codex_native_bridge import read_bridge_startup_error
    from codify.runner.native import orchestration as native_orch
