...
def test_build_codify_mcp_server_points_serve_mcp_at_bridge_dir() -> None:
    block = build_opencode_codify_mcp_server(Path("/tmp/bridge-xyz"))
    assert set(block) == {"codify"}
    entry = block["codify"]
    ...
    assert "codify.claude_native_bridge" in cmd
    assert entry.get("environment", {}).get("PYTHONUNBUFFERED") == "1"


def test_build_codify_mcp_server_honors_python_executable() -> None:
    block = build_opencode_codify_mcp_server(Path("/tmp/b"), python_executable="/custom/python")
    assert block["codify"]["command"][0] == "/custom/python"
...
def test_build_mcp_block_stdio_and_http() -> None:
    ...
    from codify.opencode_native_provider import build_opencode_mcp_block
...
def test_build_mcp_block_http_databricks_injects_bearer(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import codify.opencode_native_provider as prov
