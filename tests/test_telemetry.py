...
from codify.telemetry.surface import classify_surface
...
def _reset_is_disabled_cache():
    ...
    import codify.telemetry.client as _mod
...
def test_is_disabled_codify_analytics_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_ANALYTICS=0`` disables telemetry."""
    monkeypatch.setenv("CODIFY_ANALYTICS", "0")
    monkeypatch.delenv("CI", raising=False)
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    from codify.telemetry.client import is_disabled
...
def test_is_disabled_do_not_track(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CODIFY_ANALYTICS", raising=False)
    monkeypatch.delenv("CODIFY_DISABLE_TELEMETRY", raising=False)
...
def test_is_disabled_codify_disable_telemetry(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_DISABLE_TELEMETRY=1`` disables telemetry."""
    monkeypatch.delenv("CODIFY_ANALYTICS", raising=False)
    monkeypatch.setenv("CODIFY_DISABLE_TELEMETRY", "1")
    monkeypatch.delenv("CI", raising=False)
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    from codify.telemetry.client import is_disabled
...
def test_get_installation_id_creates_uuid(tmp_path: Path) -> None:
    ...
    import codify.telemetry.installation_id as _mod
    ...
    with (
        ...
        patch(
            "codify.telemetry.installation_id._telemetry_file_path",
...
def test_get_installation_id_corrupted_file(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "codify.telemetry.installation_id._write_to_disk",
...
def test_host_hello_frame_roundtrip_with_installation_id() -> None:
    ...
    from codify.host.frames import HostHelloFrame, decode_host_frame, encode_host_frame
    from codify.runtime import telemetry as _telemetry_mod
...
def test_build_record_promotes_host_installation_id() -> None:
    ...
    import codify.telemetry.client as _mod
    from codify.telemetry.events import SessionCreatedEvent
...
def test_resolve_harness_none_when_conv_is_none() -> None:
    ...
    from codify.server.routes.sessions import _resolve_harness
...
def test_resolve_harness_none_when_agent_store_uninitialized() -> None:
    ...
    with patch("codify.runtime._globals._agent_store", None):
...
def test_resolve_harness_none_when_agent_not_in_store() -> None:
    ...
    with patch("codify.runtime._globals._agent_store", mock_store):
        assert _resolve_harness(conv) is None
...
def test_resolve_harness_sdk_via_config_key() -> None:
    ...
    with (
        patch("codify.runtime._globals._agent_store", mock_store),
        patch("codify.runtime.get_agent_cache", return_value=mock_cache),
    ):
        assert _resolve_harness(conv) == "claude-sdk"
