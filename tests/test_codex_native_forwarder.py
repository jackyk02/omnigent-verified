...
(:mod:`codify.codex_native_forwarder`).
...
module) onto ``_CodexForwarderState.model`` and mirrors it to the Codify server
...
model IS mirrored (so Codify learns the session's model even when unchanged);
...
from codify import codex_native_forwarder as fwd
...
from codify.codex_native_forwarder import _persist_codex_compaction_item
...
async def test_sync_model_change_posts_on_change() -> None:
    ...
    The in-TUI ``/model`` switch (gpt-5.5 → gpt-5.4) must mirror to Codify as
...
async def test_sync_model_change_no_post_when_unchanged() -> None:
    ...
    Prevents the spawn/startup model from being echoed back to Codify as a
...
def test_note_resume_response_records_model_without_seeding_baseline() -> None:
    ...
    Codify must learn the session's ACTUAL model — including the spawn default —
...
async def test_sync_after_resume_posts_spawn_model() -> None:
    ...
    spawned on gpt-5.4-mini, the model never "changed", yet Codify must still
...
def test_thread_settings_updated_records_effort_and_collaboration_mode() -> None:
    ...
    no state to mirror, so Codify would keep stale ``reasoning_effort`` and
...
async def test_sync_reasoning_effort_change_posts_and_dedupes() -> None:
    """
    Codex effort changes mirror to Codify exactly once per observed value.
...
async def test_sync_reasoning_effort_change_posts_clear() -> None:
    """
    Codex clearing effort mirrors JSON null to Codify.
    ...
    early on falsey ``None``, Codify would keep a stale explicit effort.
    """
    client = _RecordingClient()
    state = fwd._CodexForwarderState(
        effort=None,
...
async def test_sync_codex_collaboration_mode_change_posts_and_dedupes() -> None:
    """
    Codex collaboration mode changes mirror to Codify labels once.
...
async def test_post_user_message_image_only_posts_empty_content() -> None:
    """
    An image-only ``userMessage`` is posted with EMPTY Codify content.
...
def test_terminal_turn_status_edge_empty_turn_idle_and_warns(
    ...
    with caplog.at_level("WARNING", logger="codify.codex_native_forwarder"):
        edge = fwd._terminal_turn_status_edge(tmp_path, "turn/completed", params)
...
def test_codify_status_from_resume_turn_error_parity() -> None:
    ...
    assert fwd._codify_status_from_resume_turn(turn_with_error) == "failed"
    # Parity check: the clean turn still resolves to idle.
    assert fwd._codify_status_from_resume_turn(turn_clean) == "idle"
...
async def test_post_session_event_records_connectivity_failure_for_watchdog(
    ...
    """
    from codify import _native_forwarder_health as health
...
async def test_mcp_startup_event_records_and_posts(tmp_path: Path) -> None:
    ...
    from codify.codex_native_bridge import read_mcp_startup
...
async def test_seed_mcp_startup_round_posts_config_servers(tmp_path: Path) -> None:
    ...
    try:
        assert read_mcp_startup(tmp_path) == {
            ...
            "codify": {"status": "starting", "error": None},
        }
        assert client.posts == [
            (
                "/v1/sessions/conv_x/events",
...
async def test_seed_mcp_startup_round_skips_when_state_exists(tmp_path: Path) -> None:
    ...
    """
    from codify.codex_native_bridge import read_mcp_startup, update_mcp_server_startup
