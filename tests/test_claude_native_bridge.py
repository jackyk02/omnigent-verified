...
from codify import claude_native_bridge, native_cost_popup
...
def _trust_tmp_bridge_parent(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    :returns: None.
    """
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", tmp_path)
...
def subprocess_bridge_root() -> Iterator[Path]:
    ...
    :yields: Temporary directory path under the production trusted
        ...
        ``python -m codify.claude_native_bridge`` accepts bridge
        writes without inheriting pytest monkeypatches.
    """
    production_root = (
        Path(tempfile.gettempdir())
        / f"codify-{claude_native_bridge.stable_user_id()}"
        / "claude-native"
    )
...
def test_prepare_bridge_dir_preserves_token_and_updates_runtime(
    ...
    If this regresses, reattaching ``codify claude`` can rotate the
    bearer token while the already-running MCP server still expects the
    old token.
    """
    root = tmp_path / "root"
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", root)
...
def test_prepare_bridge_dir_preserves_permission_hook_config(
    ...
    """
    Re-preparing a bridge keeps the permission command hook's Codify URL.

    The claude-native ``PermissionRequest`` command hook reads
    ``permission_hook.json`` at hook time to learn which Codify server to
...
def test_prepare_bridge_dir_restricts_filesystem_permissions(
    ...
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", tmp_path / "claude-native")
...
def test_prepare_bridge_dir_refuses_symlinked_ancestor(
    ...
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", symlink)
...
def test_trusted_parent_accepts_qwen_native_bridge_dir(
    ...
    (``$TMPDIR/codify-<uid>/qwen-native``); if that root is missing from the
    ...
    """
    from codify import qwen_native_bridge
    ...
    qwen_root = tmp_path / "codify-test" / "qwen-native"
...
def test_trusted_parent_accepts_kiro_native_bridge_dir(
    ...
    files under its own root (``$TMPDIR/codify-<uid>/kiro-native``). Without the
    ...
    """
    from codify import kiro_native_bridge
    ...
    kiro_root = tmp_path / "codify-test" / "kiro-native"
...
def test_read_transcript_items_since_ignores_observed_status_records(tmp_path: Path) -> None:
    """
    Non-conversation Claude JSONL records do not become Codify items.
...
def test_augment_claude_args_injects_mcp_and_hooks(tmp_path: Path) -> None:
    """
    Claude receives the Codify MCP server and hook settings in one launch.
    ...
    server = mcp_config["mcpServers"]["codify"]
    ...
    assert "codify.claude_native_hook" in settings["hooks"]["Stop"][0]["hooks"][0]["command"]
    ...
    assert "codify.claude_native_hook" in settings["hooks"]["PreCompact"][0]["hooks"][0]["command"]
...
def test_augment_claude_args_mirrors_launch_overrides_into_settings(
    ...
    global ``~/.claude/settings.json`` and diverging from the Codify
    session pill.
    """
    args = augment_claude_args(
...
def test_augment_claude_args_merges_caller_allowed_tools(tmp_path: Path) -> None:
    ...
    args = augment_claude_args(
        ...
        bridge_dir=tmp_path,
        python_executable="/venv/bin/python",
        allowed_tools=("mcp__codify__sys_session_rename", "Bash"),
    )
...
def test_augment_claude_args_omits_permission_hook_without_codify_server(
    tmp_path: Path,
) -> None:
    """
    No ``PermissionRequest`` hook is registered when the wrapper has
    no Codify server URL to point Claude at.

    This guards the default path: a call site that forgets to plumb
    the Codify server URL must NOT silently fall through to Claude's TUI
...
def test_augment_claude_args_registers_permission_command_hook(
    ...
    active Codify session at hook time.
    ...
    assert "codify.claude_native_hook permission-request" in hook["command"]
    ...
    assert "codify.claude_native_status" in settings["statusLine"]["command"]
...
def test_augment_claude_args_omits_user_prompt_submit_policy_hook_without_server(
    ...
    Policy hooks only make sense when an Codify server is configured to
...
def test_mcp_server_initialize_omits_blocked_channel_capability(
    ...
    """
    monkeypatch.setattr(
        "codify.claude_native_bridge._TRUSTED_PARENT",
        Path(tempfile.gettempdir()),
    )
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", subprocess_bridge_root)
    bridge_dir = prepare_bridge_dir("conv_abc", workspace=tmp_path)
...
def test_inject_user_message_raises_when_tmux_target_never_published(
    ...
    Failing silently would let the Codify turn complete with no user
...
def test_inject_user_message_resends_enter_when_first_submit_swallowed(
    ...
    monkeypatch.setattr("codify.claude_native_bridge._CLAUDE_READY_POLL_INTERVAL_S", 0.01)
    monkeypatch.setattr("codify.claude_native_bridge._SUBMIT_RETRY_INTERVAL_S", 0.02)
    monkeypatch.setattr("codify.claude_native_bridge._PASTE_SETTLE_S", 0.0)
    bridge_dir = tmp_path / "bridge"
    write_tmux_target(
        bridge_dir,
        socket_path=Path("/tmp/example/tmux.sock"),
...
def test_inject_user_message_raises_when_draft_never_submits(
    ...
    dialog), returning success would complete the Codify turn with
    ...
    monkeypatch.setattr("codify.claude_native_bridge._CLAUDE_READY_POLL_INTERVAL_S", 0.01)
    monkeypatch.setattr("codify.claude_native_bridge._SUBMIT_RETRY_INTERVAL_S", 0.02)
    monkeypatch.setattr("codify.claude_native_bridge._SUBMIT_VERIFY_TIMEOUT_S", 0.2)
    monkeypatch.setattr("codify.claude_native_bridge._PASTE_SETTLE_S", 0.0)
    bridge_dir = tmp_path / "bridge"
    write_tmux_target(
        bridge_dir,
...
def test_inject_interrupt_sends_escape_keystroke(
    ...
    keyword changes, Claude won't see a cancel and the Codify stop
...
def test_inject_interrupt_raises_when_tmux_target_never_published(
    ...
    Codify server falls back to the DBOS cancel path. Swallowing this
...
@pytest.mark.asyncio
async def test_channel_server_relays_active_codify_tools(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    subprocess_bridge_root: Path,
) -> None:
    ...
    call the Codify tools made available to the server-side agent.
    """
    monkeypatch.setattr(
        "codify.claude_native_bridge._TRUSTED_PARENT",
        Path(tempfile.gettempdir()),
    ...
    try:
        ...
        relay = start_tool_relay(
            ...
            tools=[
                {
                    "name": "sys_custom",
                    "description": "Test-only active Codify tool.",
                    "parameters": {
                        "type": "object",
...
def test_call_relay_tool_returns_mcp_error_on_read_timeout(
    ...
    assert "failed to call Codify tool relay" in error_text
...
async def test_start_tool_relay_accepts_codex_native_bridge_root(
    ...
    ``~/.codify/codex-native`` instead of Claude's ``/tmp`` bridge
    ...
    :returns: None.
    """
    from codify import codex_native_bridge

    codex_root = tmp_path / ".codify" / "codex-native"
    monkeypatch.setattr("codify.codex_native_bridge._BRIDGE_ROOT", codex_root)
...
async def test_start_tool_relay_accepts_antigravity_native_bridge_root(
    ...
    ``~/.codify/antigravity-native`` (the same ``$HOME/.codify/<harness>``
    ...
    :returns: None.
    """
    from codify import antigravity_native_bridge

    antigravity_root = tmp_path / ".codify" / "antigravity-native"
    monkeypatch.setattr("codify.antigravity_native_bridge._BRIDGE_ROOT", antigravity_root)
            ...
            tools=[
                {
                    ...
                    "description": "Spawn an Codify sub-agent session.",
                    "parameters": {"type": "object", "properties": {}},
                }
            ],
            tool_executor=_executor,
...
async def test_start_tool_relay_accepts_opencode_native_bridge_root(
    ...
    ``~/.codify/opencode-native`` (the same ``$HOME/.codify/<harness>``
    ...
    :returns: None.
    """
    from codify import opencode_native_bridge

    opencode_root = tmp_path / ".codify" / "opencode-native"
    monkeypatch.setattr("codify.opencode_native_bridge._BRIDGE_ROOT", opencode_root)
            ...
            tools=[
                {
                    ...
                    "description": "List Codify sessions.",
                    "parameters": {"type": "object", "properties": {}},
                }
            ],
            tool_executor=_executor,
...
async def test_relay_close_keeps_advertisement_owned_by_newer_relay(
    ...
    """
    monkeypatch.setattr("codify.claude_native_bridge._TRUSTED_PARENT", Path("/tmp"))
    monkeypatch.setattr("codify.claude_native_bridge._BRIDGE_ROOT", subprocess_bridge_root)
...
def test_read_user_status_line_command_returns_string(
    ...
    custom status bar still renders for codify-launched sessions.
    """
    fake_home = tmp_path
...
def test_ensure_trusted_creates_config_when_missing(
    ...
    This is the core workspace-trust case: an ``codify host`` machine that
...
def test_display_cost_approval_popup_builds_detached_tmux_command(
    ...
    client (``-c``), launches :mod:`codify.native_cost_popup`, and
    forwards the session/elicitation/message plus THIS bridge's
    ``permission_hook.json`` (where the popup reads the Codify url/token). A
    ...
    assert "codify.native_cost_popup" in inner
...
def test_compute_transcript_cumulative_cost_dedupes_by_request_id(
    ...
    monkeypatch.setattr("codify.llms.context_window.fetch_model_pricing", lambda model: pricing)
...
def test_compute_transcript_cumulative_cost_none_when_nothing_priceable(
    ...
    claude_native_bridge._TRANSCRIPT_PRICING_CACHE.clear()
    monkeypatch.setattr("codify.llms.context_window.fetch_model_pricing", lambda model: None)
...
def test_format_terminal_failure_tail_caps_length(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    :returns: None.
    """
    monkeypatch.setattr("codify.claude_native_bridge._TERMINAL_FAILURE_TAIL_CHARS", 50)
...
def test_wait_for_claude_prompt_ready_surfaces_terminal_output_on_timeout(
    ...
    monkeypatch.setattr(
        "codify.claude_native_bridge._capture_pane",
...
def test_wait_for_claude_prompt_ready_tail_is_observed_not_recaptured(
    ...
    monkeypatch.setattr("codify.claude_native_bridge._capture_pane", fake_capture)
    with pytest.raises(RuntimeError) as excinfo:
        claude_native_bridge._wait_for_claude_prompt_ready(
            "/tmp/example/tmux.sock",
            "claude:0.0",
