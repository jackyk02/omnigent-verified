...
1. :class:`codify.server.device_grant_store.DeviceGrantStore` — the
...
from codify.server.device_grant_store import DeviceGrantStore, hash_secret
...
def test_approve_binds_identity(store: DeviceGrantStore) -> None:
    """Approval binds the Codify identity and stamps approved_at."""
    g = _new_grant(store)
...
def _build_accounts_app(
    ...
    monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD", "admin-pw-12345")
    ...
    if device_grant_enabled:
        monkeypatch.setenv("CODIFY_DEVICE_GRANT_ENABLED", "1")
    else:
        monkeypatch.delenv("CODIFY_DEVICE_GRANT_ENABLED", raising=False)
...
def secret_app(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[TestClient]:
    """An accounts-mode app with CODIFY_DEVICE_CLIENT_SECRET enforced."""
    monkeypatch.setenv("CODIFY_DEVICE_CLIENT_SECRET", _SECRET)
    yield from _build_accounts_app(tmp_path, monkeypatch)
...
def test_client_secret_required_when_configured(secret_app: TestClient) -> None:
    ...
    hdr = {"X-Codify-Client-Secret": _SECRET}
    ...
    r = secret_app.post(
        ...
        headers={"X-Codify-Client-Secret": "wrong"},
    )
...
def _capture_app_warnings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> list[str]:
    ...
    Attaches a handler directly to the ``codify.server.app`` logger rather
    ...
    logger = logging.getLogger("codify.server.app")
    handler = _Collector()
...
def test_startup_warns_when_client_secret_unset(
    ...
    operator should know to opt into CODIFY_DEVICE_CLIENT_SECRET."""
    monkeypatch.delenv("CODIFY_DEVICE_CLIENT_SECRET", raising=False)
    warnings = _capture_app_warnings(tmp_path, monkeypatch)
    assert any(
        "CODIFY_DEVICE_CLIENT_SECRET is not set" in m and "PUBLIC" in m for m in warnings
    ), warnings
...
def test_startup_silent_when_client_secret_set(
    ...
    monkeypatch.setenv("CODIFY_DEVICE_CLIENT_SECRET", _SECRET)
    warnings = _capture_app_warnings(tmp_path, monkeypatch)
    assert not any("CODIFY_DEVICE_CLIENT_SECRET is not set" in m for m in warnings)
