...
``codify/server/app.py`` rather than in a route sub-module, so
...
from codify.server import app as server_app
...
async def test_version_returns_source_of_truth_version(
    ...
    """GET /api/version returns ``codify.version.VERSION``.
    ...
    assert body["version"] == VERSION, (
        f"Expected version {VERSION!r} from codify.version.VERSION, got {body['version']!r}."
    )


def test_server_version_reads_version_constant() -> None:
    """The server version is the shared ``codify.version.VERSION`` constant."""
    from codify.version import VERSION
...
def _register_live_runner(app: FastAPI, runner_id: str) -> None:
    ...
    """
    from codify.runner.transports.ws_tunnel.frames import HelloFrame
...
async def test_health_batch_reports_strict_runner_and_host_liveness(
    ...
    conversation_store.set_labels(stopped.id, {"codify.stopped": "true"})
...
async def test_health_reports_host_version_from_live_registry(
    ...
    """
    from codify.host.frames import HostHelloFrame
...
async def test_health_unbound_fork_of_coding_session_reads_offline(
    ...
    - a fork carrying ``codify.fork.source_id`` reads offline, so the
    ...
    conversation_store.set_labels(
        coding_fork.id, {"codify.fork.source_id": "e9f8f58523cec9a57d3bdf93be543e8c"}
    )
...
def test_ensure_extra_builtin_agents_skips_bad_path_and_seeds_good(
    ...
    """A bad entry in CODIFY_BUILTIN_AGENT_DIRS is logged + skipped, not fatal.
...
def test_ensure_default_native_agents_seeds_qwen_card(seed_stores: _SeedStores) -> None:
    ...
    seeder Qwen Code only appears after the ``codify qwen`` CLI first registers
...
def test_ensure_default_native_agents_raises_when_provider_missing(
    ...
    with pytest.raises(CodifyError, match="no provider row to seed from"):
        server_app._ensure_default_native_agents(
            seed_stores.agent_store,
            seed_stores.artifact_store,
            seed_stores.agent_cache,
...
def test_build_native_bundle_raises_without_materialize_hook() -> None:
    ...
    from codify.errors import CodifyError
    from codify.harness_plugins import NativeHarnessProvider

    provider = NativeHarnessProvider(
        key="ghost",
        run_native="codify.ghost_native:run_ghost_native",
        auto_create_terminal="codify.runner.native:_launch_ghost",
        materialize_agent_spec=None,
    )
    with pytest.raises(CodifyError, match="no materialize_agent_spec hook"):
...
async def test_api_only_root_serves_html_200_to_any_client(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as c:
        for headers in ({"accept": "text/html"}, {"accept": "application/json"}):
            ...
            assert "CODIFY_SKIP_WEB_UI" in resp.text
...
async def test_health_derives_runner_online_from_fresh_row_stamp(
    ...
    from codify.stores.conversation_store import RUNNER_LIVENESS_TTL_S
...
def test_load_debug_routers_collects_entries() -> None:
    ...
    entries = server_app._load_debug_routers(["dev.benchmarks.codify.debug_router"])
