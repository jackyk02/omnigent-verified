"""Tests for :mod:`codify.server.managed_hosts`."""
...
from codify.onboarding.sandboxes.base import render_host_config_write_command
from codify.onboarding.sandboxes.e2b import managed_token_ttl_s as e2b_managed_token_ttl_s
...
from codify.stores.conversation_store.sqlalchemy_store import SqlAlchemyConversationStore
...
def test_parse_valid_modal_config_builds_image_parameterized_factory(
    ...
    cfg = parse_sandbox_config(
        {
            ...
            # `codify host --server <url>` and double slashes break joins.
    ...
    assert fake.image == "docker.io/me/codify-host:latest"
    ...
    cfg = parse_sandbox_config(
        {
            ...
            "server_url": "https://srv.example.com/",
            "daytona": {
                "image": "docker.io/me/codify-host:latest",
...
def test_parse_valid_e2b_config_builds_parameterized_factory(
    ...
    assert cfg.launcher_factory() is fake
    assert fake.template == "codify-host"
    assert fake.env == ["OPENAI_API_KEY", "GIT_TOKEN"]
...
def test_parse_valid_kubernetes_config_builds_parameterized_factory(
    ...
    cfg = parse_sandbox_config(
        {
            "provider": "kubernetes",
            "server_url": "http://codify.codify.svc.cluster.local/",
            "kubernetes": {
                "image": "ghcr.io/me/codify-host:latest",
                "env": ["OPENAI_API_KEY", "GIT_TOKEN"],
                "namespace": "codify-sandboxes",
                "secret_name": "codify-creds",
                "service_account": "codify-runner",
                "node_selector": {"codify.ai/runner-ready": "true"},
                "in_cluster": True,
    ...
    assert cfg is not None
    assert cfg.server_url == "http://codify.codify.svc.cluster.local"
    ...
    assert cfg.launcher_factory() is fake
    assert fake.image == "ghcr.io/me/codify-host:latest"
    assert fake.env == ["OPENAI_API_KEY", "GIT_TOKEN"]
    assert fake.namespace == "codify-sandboxes"
    assert fake.secret_name == "codify-creds"
    assert fake.service_account == "codify-runner"
    assert fake.node_selector == {"codify.ai/runner-ready": "true"}
...
def test_parse_host_config_threads_verbatim_without_resolving_secrets(
    ...
    monkeypatch.delenv("CODIFY_LITELLM_API_KEY", raising=False)
    host_config = {
        "providers": {
            "litellm": {
                "kind": "gateway",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"node_selector": {"codify.ai/x": "Bad Value"}}, "node_selector"),
...
def test_parse_kubernetes_pvc_mounts_normalizes_and_reaches_launcher(
    ...
    cfg = parse_sandbox_config(
        {
            ...
            "kubernetes": {
                "pvc_mounts": [
                    {"claim_name": "codify-datasets", "mount_path": "/mnt/datasets"},
    ...
    assert fake.pvc_mounts == [
        {"claim_name": "codify-datasets", "mount_path": "/mnt/datasets", "read_only": True},
        {"claim_name": "scratch", "mount_path": "/mnt/scratch", "read_only": False},
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"claim_name": "c", "mount_path": "/home//codify"}], "normalized"),
        ([{"claim_name": "c", "mount_path": "/home/./codify"}], "normalized"),
        # Exactly two leading slashes survive posixpath.normpath (POSIX) but
        # the kernel collapses them, so '//home/codify' would shadow HOME.
        ([{"claim_name": "c", "mount_path": "//home/codify"}], "normalized"),
        ([{"claim_name": "c", "mount_path": "//mnt/x"}], "normalized"),
        ([{"claim_name": "c", "mount_path": "/"}], "reserved"),
        ([{"claim_name": "c", "mount_path": "/home/codify/data"}], "reserved"),
        ([{"claim_name": "c", "mount_path": "/var/run/secrets/x"}], "reserved"),
        # Ancestors of reserved paths: a PVC at /home would mount over the
        # HOME emptyDir's /home/codify mountpoint (likewise /var, /var/run
        ...
        # /opt hosts the image's codify venv (/opt/venv).
...
@pytest.mark.parametrize("mount_path", ["/home/other", "/home/codify-data", "/var/lib", "/runway"])
...
def test_reserved_mount_prefixes_pin_the_launcher_home_dir() -> None:
    ...
    from codify.onboarding.sandboxes.kubernetes import _HOME_DIR
    from codify.server.managed_hosts import _KUBERNETES_RESERVED_MOUNT_PREFIXES
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"secret_name": "s", "mount_path": "/home/codify/data"}], "reserved"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # codify itself uses — inside the sandbox this would degrade
...
async def test_launch_materializes_host_config_before_host_start(db_uri: str) -> None:
    ...
    ``codify host`` starts — the whole point of the injection is that the
    ...
    host_index = next(i for i, cmd in enumerate(fake.commands) if "codify host --server" in cmd)
...
async def test_resume_rematerializes_host_config_before_host_restart(db_uri: str) -> None:
    ...
    host_index = next(i for i, cmd in enumerate(resumed_commands) if "codify host --server" in cmd)
    assert write_index < host_index
...
async def test_launch_online_timeout_terminates_and_deletes_host(
    ...
    # call time.
    monkeypatch.setattr("codify.server.managed_hosts.MANAGED_HOST_ONLINE_TIMEOUT_S", 0.05)
    monkeypatch.setattr("codify.server.managed_hosts._ONLINE_POLL_INTERVAL_S", 0.01)
    host_store = HostStore(db_uri)
...
async def test_launch_with_repo_clones_into_workspace(db_uri: str) -> None:
    ...
    host_start_index = next(i for i, c in enumerate(fake.commands) if "codify host" in c)
...
class _EntrypointFakeLauncher(FakeSandboxLauncher):
    ...
    def provision(self, name: str) -> str:
        ...
        return f"codify-pod-{len(self.provisioned_names)}"
    ...
    def start_host(
        ...
        *,
        ...
        return f"/home/codify/workspace/{repo_name}" if repo_name else "/home/codify/workspace"
...
async def test_launch_entrypoint_provider_arms_token_before_launch_host(db_uri: str) -> None:
    ...
    assert call["sandbox_id"] == "codify-pod-1"
    ...
    assert result.workspace == "/home/codify/workspace/repo"
    host = host_store.get_host(result.host_id)
    assert host is not None
    assert host.status == "online"
    assert host.sandbox_provider == "kubernetes"
    assert host.sandbox_id == "codify-pod-1"
...
async def test_launch_entrypoint_provider_cleans_up_on_launch_failure(db_uri: str) -> None:
    ...
    assert fake.terminated == ["codify-pod-1"]
    assert host_store.list_hosts(_OWNER) == []
...
def test_parse_modal_secrets_thread_to_launcher(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg = parse_sandbox_config(
        {
            "provider": "modal",
            "server_url": "https://s.example.com",
            "modal": {"secrets": ["codify-llm", "gateway-extras"]},
        }
    )
    assert cfg is not None
    fake = FakeSandboxLauncher()
    install_fake_modal_launcher(monkeypatch, fake)
    assert cfg.launcher_factory() is fake
    assert fake.secrets == ["codify-llm", "gateway-extras"]
...
@pytest.mark.parametrize(
    "secrets",
    [
        "codify-llm",  # scalar, not a list
        ["codify-llm", 7],  # non-string entry
        ["  "],  # empty name
    ],
)
