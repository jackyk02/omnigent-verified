...
class HostStartInvocation:
    ...
    Parsed from the ``codify host`` start command the managed-launch
    ...
    :param host_id: Value of the injected ``CODIFY_HOST_ID``.
    :param host_name: Value of the injected ``CODIFY_HOST_NAME``.
    :param token: Value of the injected ``CODIFY_HOST_TOKEN`` — the
        raw launch token.
...
def _parse_host_start(command: str) -> HostStartInvocation:
    ...
    The managed flow injects ``CODIFY_HOST_TOKEN`` / ``_HOST_ID`` /
    ...
    """
    values: dict[str, str] = {}
    for var in ("CODIFY_HOST_TOKEN", "CODIFY_HOST_ID", "CODIFY_HOST_NAME"):
    ...
    return HostStartInvocation(
        host_id=values["CODIFY_HOST_ID"],
        host_name=values["CODIFY_HOST_NAME"],
        token=values["CODIFY_HOST_TOKEN"],
        command=command,
    )
...
def install_fake_modal_launcher(
    ...
    """
    import codify.onboarding.sandboxes.modal as modal_mod
...
def install_fake_daytona_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.daytona as daytona_mod
...
def install_fake_boxlite_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.boxlite as boxlite_mod
...
def install_fake_islo_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.islo as islo_mod
...
def install_fake_e2b_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.e2b as e2b_mod
...
def install_fake_openshell_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.openshell as openshell_mod
...
def install_fake_kubernetes_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import codify.onboarding.sandboxes.kubernetes as kubernetes_mod
...
def build_agent_bundle(
    ...
    :param executor: Optional executor block to write verbatim,
        e.g. ``{"type": "codify", "config": {"harness":
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tf:
        ...
        for sa in sub_agents or []:
            sa_config: dict[str, Any] = {
                ...
                # harness for an codify executor. Default to the same
...
class CapturingRunnerClient:
    ...
    Records every control event the Codify server POSTs to the runner's
    ...
    ``monkeypatch.setattr("codify.runtime._globals._runner_client", c)``;
