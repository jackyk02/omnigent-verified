...
from codify.server import mcp_pool as _mcp_pool_module
from codify.server.mcp_pool import McpToolEntry, ServerMcpPool
...
def patch_mcp_connection(
    ...
    **before** calling pool methods. If a name is missing from the dict a
    ...
    monkeypatch.setattr(
        "codify.server.mcp_pool.McpServerConnection",
...
async def test_shutdown_all_closes_all_entries(
    ...
    Failure means the Codify server shutdown leaks MCP connections, blocking
