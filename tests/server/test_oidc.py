...
Tests mirror the source at ``codify/server/oidc.py`` and
``codify/server/auth.py``.
"""
...
from codify.server.oidc import (
    OIDCConfig,
...
def test_header_source_rejects_missing_header(
    ...
    """
    # The provider resolves CODIFY_LOCAL_SINGLE_USER at
    ...
    monkeypatch.delenv("CODIFY_LOCAL_SINGLE_USER", raising=False)
    provider = UnifiedAuthProvider(source="header")
...
def test_header_source_local_single_user_falls_back_to_local() -> None:
    ...
    ``CODIFY_LOCAL_SINGLE_USER=1`` (the managed local spawn
...
def test_header_source_resolves_single_user_from_env(
    ...
    ``CODIFY_LOCAL_SINGLE_USER=1`` env var, not a constructor
    argument — this pins the env-resolution path they rely on.
    """
    monkeypatch.setenv("CODIFY_LOCAL_SINGLE_USER", "1")
    provider = UnifiedAuthProvider(source="header")
    request = _mock_request()
    assert provider.get_user_id(request) == RESERVED_USER_LOCAL
...
def test_header_source_resolves_header_name_from_env(
    ...
    """``header_name=None`` resolves from ``CODIFY_AUTH_HEADER``.

    The deploy path configures the header name via env var, not a
    constructor argument — this pins the env-resolution path.
    """
    monkeypatch.setenv("CODIFY_AUTH_HEADER", "Cf-Access-Authenticated-User-Email")
    provider = UnifiedAuthProvider(source="header")
    request = _mock_request(
        headers={"Cf-Access-Authenticated-User-Email": "alice@example.com"},
    )
...
def test_resolve_auth_header_defaults_to_x_forwarded_email(
    ...
    """Unset/empty ``CODIFY_AUTH_HEADER`` falls back to the default.
    ...
    """
    monkeypatch.delenv("CODIFY_AUTH_HEADER", raising=False)
    assert resolve_auth_header() == "X-Forwarded-Email"
    monkeypatch.setenv("CODIFY_AUTH_HEADER", "   ")
    assert resolve_auth_header() == "X-Forwarded-Email"
...
def test_header_source_resolves_strip_prefix_from_env(
    ...
    ``CODIFY_AUTH_HEADER_STRIP_PREFIX``, not a constructor argument —
    this pins the env-resolution path IAP deployments rely on.
    """
    monkeypatch.setenv("CODIFY_AUTH_HEADER", _IAP_HEADER)
    monkeypatch.setenv("CODIFY_AUTH_HEADER_STRIP_PREFIX", _IAP_PREFIX)
    provider = UnifiedAuthProvider(source="header")
    request = _mock_request(headers={_IAP_HEADER: f"{_IAP_PREFIX}alice@example.com"})
    assert provider.get_user_id(request) == "alice@example.com"
...
def test_resolve_auth_header_strip_prefix_defaults_to_empty(
    ...
    """Unset/whitespace ``CODIFY_AUTH_HEADER_STRIP_PREFIX`` → ``""``.
    ...
    verbatim.
    """
    monkeypatch.delenv("CODIFY_AUTH_HEADER_STRIP_PREFIX", raising=False)
    assert resolve_auth_header_strip_prefix() == ""
    monkeypatch.setenv("CODIFY_AUTH_HEADER_STRIP_PREFIX", "   ")
    assert resolve_auth_header_strip_prefix() == ""
...
def test_oidc_source_accepts_bearer_token() -> None:
    ...
    This is the code path used by ``codify run --server`` after
    ``codify login`` stores a token. The CLI sends the JWT as
...
def test_factory_returns_header_provider_explicit_zero_config(
    ...
    """Explicit CODIFY_AUTH_PROVIDER=header returns a header provider.
    ...
    explicitly so an ambient ``CODIFY_AUTH_ENABLED=1`` can't
...
def test_factory_rejects_unknown_source(
    ...
    """
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "saml")
    with pytest.raises(RuntimeError, match="Unknown CODIFY_AUTH_PROVIDER"):
        create_auth_provider()
...
def test_factory_oidc_fails_without_config(
    ...
    # Don't set any CODIFY_OIDC_* vars.
    monkeypatch.delenv("CODIFY_OIDC_ISSUER", raising=False)
    monkeypatch.delenv("CODIFY_OIDC_CLIENT_ID", raising=False)
    monkeypatch.delenv("CODIFY_OIDC_CLIENT_SECRET", raising=False)
    monkeypatch.delenv("CODIFY_OIDC_REDIRECT_URI", raising=False)
    monkeypatch.delenv("CODIFY_OIDC_COOKIE_SECRET", raising=False)
...
def test_oidc_config_rejects_short_cookie_secret(
    ...
    """
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "test")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app/callback")
    # 16 bytes = 32 hex chars, but we need 32 bytes = 64 hex chars.
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 16)
...
def test_oidc_config_rejects_invalid_hex_secret(
    ...
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app/callback")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "not-hex-at-all")
...
def test_oidc_config_github_provider(
    ...
    """
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "test-client")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test-secret")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app/callback")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)
...
def test_oidc_config_github_empty_scopes_env_falls_back_to_default(
    ...
    """Empty CODIFY_OIDC_SCOPES → provider default (else GitHub 404s consent)."""
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "test")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app/callback")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)
    monkeypatch.setenv("CODIFY_OIDC_SCOPES", "")
...
def test_oidc_config_allowed_domains_parsing(
    ...
    """CODIFY_OIDC_ALLOWED_DOMAINS is parsed into a frozenset.
    ...
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app/callback")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)
    monkeypatch.setenv("CODIFY_OIDC_ALLOWED_DOMAINS", " Example.COM , test.org ")
...
def test_oidc_redirect_uri_derived_from_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    """With REDIRECT_URI unset, it's derived as https://<CODIFY_DOMAIN>/auth/callback.

    Lets a domain-based deploy set one var (CODIFY_DOMAIN, already
    ...
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.delenv("CODIFY_OIDC_REDIRECT_URI", raising=False)
    monkeypatch.setenv("CODIFY_DOMAIN", "codify.example.com")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)

    config = OIDCConfig.from_env()

    assert config.redirect_uri == "https://codify.example.com/auth/callback"
...
def test_oidc_explicit_redirect_uri_wins_over_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    """An explicit REDIRECT_URI is used verbatim even when CODIFY_DOMAIN is set."""
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "test")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "http://10.0.0.5:8000/auth/callback")
    monkeypatch.setenv("CODIFY_DOMAIN", "codify.example.com")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)
...
def test_oidc_redirect_uri_required_without_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    """Fail loud when neither REDIRECT_URI nor CODIFY_DOMAIN is set."""
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "test")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "test")
    monkeypatch.delenv("CODIFY_OIDC_REDIRECT_URI", raising=False)
    monkeypatch.delenv("CODIFY_DOMAIN", raising=False)
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)

    with pytest.raises(RuntimeError, match="CODIFY_OIDC_REDIRECT_URI"):
        OIDCConfig.from_env()
