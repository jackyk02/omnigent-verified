...
from codify.server.auth import RESERVED_USER_LOCAL, AuthProvider
...
async def test_ws_tunnel_loopback_unauthenticated_registers_as_local() -> None:
    ...
    ``codify server`` starts an unauthenticated runner that connects
...
def _mint_route_app(
    *,
    ...
    """Tunnel-route app with the ``CodifyError`` -> HTTP handler installed.

    The bare :func:`_tunnel_route_app` omits ``create_app``'s exception
    handler, so the mint endpoint's ``CodifyError`` would surface as a
    ...
    async def _handle(request: Request, exc: CodifyError) -> JSONResponse:
...
async def _post_mint_token(
    ...
    *,
    ...
    :param token: Binding token for the ``X-Codify-Runner-Tunnel-Token``
...
async def test_ping_loop_restamps_runner_liveness(
    ...
    :returns: None.
    """
    import codify.server.routes.runner_tunnel as tunnel_mod
