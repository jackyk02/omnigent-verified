...
async def test_fork_coding_session_stamps_fork_source_label(
    ...
    ``codify.fork.source_id`` label (value = source id). That label
    ...
    # wrong source.
    assert fork["labels"].get("codify.fork.source_id") == source["id"], (
        f"Expected fork-source label = {source['id']!r}, got "
        f"{fork['labels'].get('codify.fork.source_id')!r}."
    )
...
async def test_fork_chat_session_has_no_fork_source_label(
    ...
    dot keeps reading it reachable — i.e. the ``codify.fork.source_id``
    ...
    assert "codify.fork.source_id" not in fork["labels"], (
