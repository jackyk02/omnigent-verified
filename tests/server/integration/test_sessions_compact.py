...
the Codify server stays harness-agnostic: it forwards the control to the
...
* other harnesses **204** no-op — the Codify server owns the operation.
* a failed injection (pane not attached) returns **503**.

These tests pin the Codify side of that contract by stubbing the runner's
...
from codify.runtime.compaction import CompactionResult
...
def _fake_runner_returning(compact_status: int) -> tuple[httpx.AsyncClient, list[dict[str, Any]]]:
    ...
    the test can assert the Codify server actually forwarded the control,
...
async def test_compact_skips_codify_compaction_when_runner_handles_it(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    A 200 from the runner (claude-native injected ``/compact``) makes
    the Codify server skip its own compaction.
    ...
    When the runner reports it handled the control (200), the Codify server
    ...
    async def _must_not_run(**_: Any) -> CompactionResult:
        ...
        raise AssertionError(
            ...
            "reported it handled /compact (200). The Codify server fell "
            "through to its own compaction instead of skipping."
        )

    monkeypatch.setattr(
        "codify.runtime.workflow.compact_conversation_now",
        _must_not_run,
    )
    ...
    # handled it, Codify returned without running (or raising from) its own
    # compaction.
    assert resp.status_code == 202, resp.text
    assert resp.json() == {"queued": False}, resp.text
    # Exactly one compact control was forwarded to the runner. 0 = the
    # Codify server never forwarded (it would have run _run_compact_locked
...
async def test_compact_runs_codify_compaction_when_runner_noops(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    A 204 from the runner (in-process harness) makes the Codify server run
    ...
    Codify server it owns the operation, so it must still forward the
    ...
    # Codify server is harness-agnostic and forwards for every harness.
    assert captured == [{"type": "compact"}], (
...
async def test_compact_model_less_sdk_harness_returns_clear_unavailable_message(
    ...
    agent = await create_test_agent(
        ...
        executor={"type": "codify", "config": {"harness": "openai-agents"}},
        include_llm=False,
    )
    sid = await _create_session(client, agent["id"])
...
async def test_compact_errors_when_runner_injection_fails(
    ...
    wrong (summarising the mirror). The Codify server must surface the
    ...
    async def _must_not_run(**_: Any) -> CompactionResult:
        ...
        raise AssertionError(
            "compact_conversation_now must not run when the runner "
            "returned a non-200/204 status — Codify fell through to its "
...
async def test_external_compaction_status_publishes_compaction_sse(
    ...
    post-compaction SessionStart (completed) hooks. Codify must translate it
