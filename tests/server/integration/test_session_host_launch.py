...
_HARNESS_REFUSAL = (
    "harness 'codex' is not configured on host 'laptop' — run `codify setup` on that machine"
)
...
async def test_inline_create_harness_not_configured_stays_lenient(
    ...
    ``codify setup`` since the host last connected), so create never
...
async def test_message_relaunch_harness_not_configured_persists_error_turn(
    ...
    carrying the host's `codify setup` message (the web renders it as
    ...
    assert "codify setup" in error_items[0]["message"]
...
async def test_stopped_host_session_writes_no_label_and_host_stays_online(
    ...
    writes NO persistent marker. With the host's ``codify host``
    ...
    NO ``codify.stopped`` label is persisted. Mutation check: re-add a
    ...
    # sticky label would resurface the retired codify.stopped behavior.
    snap = await client.get(f"/v1/sessions/{session_id}")
    assert "codify.stopped" not in snap.json()["labels"], (
        f"Stop must NOT persist any codify.stopped label; got {snap.json()['labels']!r}"
    )
...
async def test_codex_goal_relaunch_posts_session_init_before_goal_event(
    ...
    assertion.
    """
    from codify._wrapper_labels import CODEX_NATIVE_WRAPPER_VALUE, WRAPPER_LABEL_KEY
    from codify.server.routes.codex import sessions as codex_sessions_module
...
async def _serve_fs_requests(
    ...
    *,
    ...
    Runs the production :class:`codify.workspace_fs.WorkspaceReader`
    ...
    from codify.host.frames import HostFsRequestFrame, HostFsResultFrame
...
async def test_offline_runner_serves_file_content_and_changes_from_host(
    ...
    from codify.runtime import _globals, set_runner_router
    ...
    class _OfflineRunnerRouter:
        def client_for_session_resources(self, session_id: str) -> object:
            del session_id
            raise CodifyError("runner is offline", code=ErrorCode.RUNNER_UNAVAILABLE)
