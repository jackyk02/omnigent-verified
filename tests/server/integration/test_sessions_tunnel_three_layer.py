"""
Tunnel three-layer integration test: Codify → WS tunnel → runner → harness.
...
Layers:
    Codify server ──HTTP──> RunnerRouter ──WSTunnelTransport──>
...
from codify_client._events import ResponseCompleted, ResponseFailed
from codify_client._files import FilesNamespace
from codify_client._sessions import SessionsNamespace
from fastapi import FastAPI

from codify.repl._repl import _SessionsChatReplAdapter
...
from codify.runner.transports.ws_tunnel.serve import dispatch_via_asgi
from codify.runner.transports.ws_tunnel.transport import WSTunnelTransport
...
def _build_harness_agent_bundle() -> bytes:
    ...
    Uses ``executor.type='codify'`` with
...
# ── Fixture: Codify app + tunnel WS + runner app + EchoHarness ────
...
async def tunnel_three_layer_stack(tmp_path: Path) -> AsyncIterator[_TunnelStack]:
    """Wire Codify server + WS-tunneled runner + EchoHarness in-process.

    Lifecycle: build stores, init runtime, override the test harness
    module entry, build Codify app + runner app, open the WS tunnel via
    ...
    # Build the Codify server. ``create_app`` constructs the
...
def _fake_client(ap_client: httpx.AsyncClient) -> object:
    ...
    :param ap_client: ASGI-backed client pointed at the Codify app.
    :returns: Duck-typed client with the namespaces the adapter uses.
    """
    base = str(ap_client.base_url).rstrip("/")

    class _FakeClient:
        """Duck-typed Codify client backed by the tunnel-stack httpx client."""
...
def _new_repl_adapter(
    ...
    *,
    ...
    """Create a sessions REPL adapter over the tunneled Codify stack.
...
async def test_on_runner_connect_restarts_relay_via_router(
    ...
    """
    from codify.runner.routing import RoutedRunner
    from codify.server.routes import sessions as sessions_routes
    from codify.server.routes.sessions import _runner_relay_tasks
    ...
    from codify.runtime import get_conversation_store
