...
from codify.server.auth import LEVEL_EDIT, LEVEL_MANAGE, LEVEL_OWNER, LEVEL_READ
...
def auth_app(
    ...
    return create_app(
        ...
        # create_auth_provider() so ambient CODIFY_* env vars in
...
def local_auth_app(
    ...
    ``CODIFY_LOCAL_SINGLE_USER=1``): requests without
...
async def test_single_user_local_can_access_own_session(
    ...
    — the bare `codify run` / local web UI flow.
    """
