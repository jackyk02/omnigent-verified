...
_CODEX_EXECUTOR: dict[str, Any] = {"type": "codify", "config": {"harness": "codex"}}
...
async def test_add_claude_native_child_applies_wrapper_label(
    ...
    when its conversation carries ``codify.wrapper`` =
    ...
    #  - codify.wrapper → runner routes the session to Claude Code.
    #  - codify.ui=terminal → AppShell renders it terminal-first.
    assert row["labels"].get("codify.wrapper") == "claude-code-native-ui"
    assert row["labels"].get("codify.ui") == "terminal"
...
@pytest.mark.xfail(
    reason=(
        ...
        "codify/server/API.md, not wired in app.py). Flips to XPASS "
