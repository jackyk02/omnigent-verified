...
from codify.entities.conversation import MessageData, NewConversationItem
from codify.server.routes import sessions as sessions_module
from codify.server.routes.sessions import routes_events as routes_events_module
...
def _seed_child(
    *,
    ...
    Mirrors what :func:`codify.tools.builtins.spawn._spawn_one` does,
...
async def test_closed_child_session_display_is_sanitized_and_read_only(
    ...
    ``codify.closed=true`` label for clients, and reject new user
...
async def test_native_subagent_session_stamps_terminal_ui_labels(
    ...
    the Chat/Terminal pill (gated on ``codify.ui == "terminal"``).
    ...
    assert labels.get("codify.wrapper") == expected_wrapper
    assert labels.get("codify.ui") == "terminal"
...
async def test_native_subagent_message_uses_native_terminal_forward(
    ...
    ``claude-native`` or ``codex-native``, Codify must forward the prompt to
    ...
    assert child["labels"].get("codify.wrapper") == expected_wrapper

    forwarded: list[dict[str, Any]] = []

    def _handler(request: httpx.Request) -> httpx.Response:
        """
        Capture the event Codify forwards to the fake runner.
...
async def test_non_native_subagent_session_has_no_terminal_ui_labels(
    ...
    assert resp.status_code == 201, resp.text
    labels = resp.json()["labels"]
    assert "codify.wrapper" not in labels
    assert "codify.ui" not in labels
