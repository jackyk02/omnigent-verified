...
from codify.entities import Conversation
        ...
        sandbox_config=parse_sandbox_config(
            {
                ...
                "modal": {"image": "docker.io/test/codify-host:latest"},
            }
        ),
    )
...
async def _fake_sandbox_host(
    ...
    ``CODIFY_HOST_NAME``; the name must match the pre-registered
    ...
    scope["headers"] = [(b"x-codify-host-token", token.encode("ascii"))]
...
async def test_managed_session_create_end_to_end(
    ...
    monkeypatch.setattr("codify.server.managed_hosts.MANAGED_HOST_ONLINE_TIMEOUT_S", 10)
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions._HOST_RELAUNCH_RUNNER_CONNECT_TIMEOUT_S", 0.2
    )
...
async def test_message_relaunches_dead_managed_sandbox(
    ...
    monkeypatch.setattr("codify.server.routes.sessions._HOST_BOUND_RUNNER_CONNECT_GRACE_S", 0.1)
    loop = asyncio.get_running_loop()
    host_futures: list[asyncio.Future[ApplicationCommunicator]] = []
...
async def test_managed_wake_fails_when_runner_never_reconnects(
    ...
    monkeypatch.setattr("codify.server.managed_hosts.resume_managed_host", _resume_noop)
