...
payload (when ``codify claude`` wraps the native TUI), parks
...
from codify.server.routes import sessions as sessions_route
...
async def test_permission_request_hook_accepts_kimi_namespaced_elicitation_id(
    ...
    """
    A kimi-native hook supplies ``_codify_elicitation_id = elicit_kimi_…`` on
    ...
    payload = await _claude_permission_payload()
    payload["_codify_elicitation_id"] = elicitation_id
...
async def test_cursor_permission_request_hook_allow_round_trip(
    ...
    The runner-side mirror (``codify.cursor_native_permissions``) POSTs a
...
async def test_qwen_permission_request_hook_allow_round_trip(
    ...
    The runner-side mirror (``codify.qwen_native_permissions``) reads a
...
async def test_top_level_elicitations_route_is_not_mounted(
    ...
    """
    Codify no longer exposes ``POST /v1/elicitations/{id}``.
    ...
    - HTTP (``client`` fixture, same app): the live response is
      ...
      web-ui build exists at ``codify/server/static/web-ui/index.html``
...
async def test_permission_request_hook_forwards_cwd_and_permission_mode(
    ...
    :func:`codify.server.routes.sessions._signal_terminal_resolved_harness_elicitation`),
...
async def test_permission_request_hook_timeout_clears_pending_index(
    ...
    TUI instead of the web UI) is the ONLY signal the Codify server
    ...
    """
    from codify.runtime import pending_elicitations, session_stream
...
async def test_permission_hook_repark_within_grace_keeps_card_pending(
    ...
    """
    A hook retry that re-parks the same ``_codify_elicitation_id``
    ...
    payload = {
        **(await _claude_permission_payload()),
        "_codify_elicitation_id": _REATTACH_ELICITATION_ID,
    }
...
async def test_permission_hook_rejects_malformed_reattach_id(
    ...
    """
    A client-supplied ``_codify_elicitation_id`` outside the
    ...
    payload = {
        **(await _claude_permission_payload()),
        "_codify_elicitation_id": "elicit_codex_squat",
    }
    ...
    assert "_codify_elicitation_id" in resp.text
