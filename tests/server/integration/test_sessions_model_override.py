...
async def test_context_window_uses_effective_model(
    ...
    *which* model the snapshot used to size the window.
    """
    from codify.llms import context_window as context_window_mod
...
async def test_context_window_override_bypasses_declared_window(
    ...
    agent = await create_test_agent(
        ...
        executor={"type": "codify", "context_window": 1_000_000},
    )
    session = await _create_session(client, agent["id"])
    sid = session["id"]
...
async def test_silent_patch_skips_claude_native_forward(
    ...
    Updated for the unified-events refactor: Codify server no longer
    ...
    try:
        ...
        await client.patch(
            f"/v1/sessions/{sid}",
            json={"labels": {"codify.wrapper": "claude-code-native-ui"}},
        )
        captured.clear()
    ...
    # happen anymore — its callsite is gone from Codify server.
...
async def test_smart_routing_overrides_orchestrator_model_for_child_session(
    ...
    with patch("codify.server.smart_routing.route_session_harness", _fake_route_session_harness):
        agent = await create_test_agent(client)
