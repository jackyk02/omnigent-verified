...
from codify.server.routes import session_mcp_servers as mcp_routes
...
async def test_create_mcp_server_supports_single_yaml_bundle(client: httpx.AsyncClient) -> None:
    """Single-file codify YAML bundles are updated inline."""
...
def _single_yaml_bundle(yaml_text: str) -> bytes:
    """Build a tar.gz bundle containing one codify YAML file."""
    buf = io.BytesIO()
