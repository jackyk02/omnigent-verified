...
from codify.runtime import get_caps, session_stream
...
from codify.server.auth import LEVEL_EDIT
from codify.spec.types import FunctionPolicySpec, FunctionRef
...
def _patch_default_policies(monkeypatch: pytest.MonkeyPatch, fn_path: str) -> None:
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions.get_caps",
        lambda: patched_caps,
    )
...
def test_mrtr_response_url_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.server.routes.sessions._ELICITATION_MODE", "url")

    from codify.server.routes.sessions import _mcp_input_required_response
...
def test_mrtr_response_form_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.server.routes.sessions._ELICITATION_MODE", "form")
