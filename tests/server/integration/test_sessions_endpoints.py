...
from codify.runtime.tool_output import MAX_TOOL_OUTPUT_BYTES
from codify.server.background_session_titles import BackgroundTitleRequest
...
from codify.tools.builtins.load_skill import format_skill_meta_text
...
async def test_first_message_schedules_background_semantic_title(
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions._get_runner_client",
        get_runner_client,
    )
...
async def test_list_sessions_reflects_relay_status_cache(
    ...
    """
    from codify.server.routes import sessions as sessions_module
...
async def test_session_snapshot_reflects_terminal_pending_cache(
    ...
    This is the reconnect channel: the Codify session stream has no replay
...
async def test_external_session_superseded_publishes_redirect_event(
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions.session_stream.publish",
...
async def test_external_session_superseded_drains_pending_inputs(
    ...
    """
    from codify.runtime import pending_inputs
...
async def test_external_subagent_start_mints_child_session(
    ...
    Task-tool subagents (which never POST to Codify themselves) so the
    ...
    # Title format mirrors codify-spawned children (``tool:name``).
    ...
    assert child["labels"]["codify.claude_native.description"] == "Trace the auth flow"
...
async def test_external_subagent_start_adopts_unlabeled_title_collision(
    ...
    assert child["labels"]["codify.claude_native.subagent_id"] == "a5c7effac5a9a35ab"
    assert child["labels"]["codify.claude_native.description"] == "Trace the auth flow"
...
async def test_skill_slash_command_non_json_resolve_surfaces_controlled_error(
    ...
    by a proxy) surfaces as a controlled ``CodifyError`` (HTTP 500
...
async def test_claude_native_session_discoverable_with_terminal_metadata(
    ...
    assert row["labels"]["codify.wrapper"] == "claude-code-native-ui"
    assert row["labels"]["codify.ui"] == "terminal"
    ...
    assert snap["labels"]["codify.wrapper"] == "claude-code-native-ui"
    assert snap["labels"]["codify.ui"] == "terminal"
...
async def test_post_external_assistant_message_persists_and_streams(
    ...
    This is the path used by ``codify claude`` to mirror real
...
async def test_post_external_function_call_output_caps_oversized_output(
    ...
    assert "[output truncated by codify:" in persisted["data"]["output"]
    ...
    assert "[output truncated by codify:" in fco_events[0]
...
async def test_post_external_session_status_publishes_session_status(
    ...
    updates without going through the Codify task lifecycle.
    A regression here would break the idle indicator for
    ``codify claude`` sessions: Codify would never learn Claude
...
async def test_patch_runner_rebind_clears_stale_failed_status(
    ...
    ``codify codex --resume`` binds a newly launched runner through
...
async def test_post_external_session_status_idle_forwards_persisted_assistant_output(
    ...
    The native forwarder posts transcript items to Codify server, then posts
    ...
    may not have the Codify transcript in local memory.
    """
    from codify.server.routes import sessions as sessions_module
...
async def test_post_external_session_status_propagates_runner_delivery_failure(
    ...
    delivery to the parent's inbox. If Codify returned ``{"queued": false}`` after
    ...
    # message. If absent, Codify flattened the failure and made diagnosis harder.
...
async def test_post_external_session_usage_rejects_empty_payload(
    ...
    otherwise round-trip a no-op event to Codify every poll.
    """
    agent = await create_test_agent(client)
    session = await _create_session(client, agent["id"])
...
async def test_external_session_usage_persists_cumulative_cost(
    ...
    separate process), so the Codify relay's ``_accumulate_session_usage`` never
...
async def test_external_session_usage_cumulative_cost_is_set_not_added(
    ...
    A failure (1.32) would mean the native path wrongly reused the Codify relay's
...
async def test_external_session_usage_codex_tokens_priced(
    ...
    monkeypatch.setattr(
        "codify.llms.context_window.fetch_model_pricing",
...
async def test_accumulate_session_usage_prices_from_usage_model(
    ...
    the spec model.
    """
    from codify.server.routes import sessions as sessions_routes
...
async def test_accumulate_session_usage_codex_unpinned_model_gets_by_model_entry(
    ...
    agent = await create_test_agent(
        client,
        executor={"type": "codify", "config": {"harness": "codex"}},
...
# PreToolUse hook) — see codify/policies/builtins/cost.py. The old
...
_COST_GUARD_GUARDRAILS = {
    "policies": {
        "cost_guard": {
            "type": "function",
            "function": {
                "path": "codify.policies.builtins.cost.cost_budget",
...
async def test_external_session_usage_over_budget_does_not_stop_session(
    ...
    assert conv.labels.get("codify.stopped") != "true"
...
async def test_relay_tool_call_ask_approval_persists_checkpoint(
    ...
    """
    from codify.policies.builtins.cost import _ASK_APPROVED_KEY
...
async def test_post_external_model_change_does_not_forward_to_runner(
    ...
    """
    from codify.runtime import set_runner_client
...
async def test_post_external_model_options_populates_picker_and_publishes(
    ...
    """
    from codify.server.routes import sessions as _mod
    ...
    session = await _create_session(
        client,
        agent["id"],
        labels={"codify.ui": "terminal", "codify.wrapper": "pi-native-ui"},
    )
...
async def test_post_external_codex_collaboration_mode_change_persists_label(
    ...
    snapshot = (await client.get(f"/v1/sessions/{session['id']}")).json()
    assert snapshot["labels"]["codify.codex_native.collaboration_mode"] == "plan"
...
async def test_patch_model_override_records_system_note_for_inprocess_session(
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions._forward_session_change_to_runner",
        _noop_forward,
    )
    agent = await create_test_agent(client)
    session = await _create_session(client, agent["id"])
...
async def test_patch_model_override_skips_note_for_native_session(
    ...
    """
    A native-wrapper session (``codify.wrapper`` set, here alongside
    ``codify.ui == "terminal"``) must NOT get an injected note —
    ...
    label, NOT ``codify.ui`` alone — see
    ``test_patch_model_override_records_note_for_terminal_view_sdk_session``
    for the chat-first SDK session that has ``codify.ui`` but no wrapper
...
async def test_patch_model_override_records_note_for_terminal_view_sdk_session(
    ...
    (``codify.ui == "terminal"`` but NO ``codify.wrapper``) DOES get the
    note.

    This is the polly / debby case: when such an agent is launched via
    ``codify run``, the runner stamps ``codify.ui: terminal`` to enable
    the web Chat/Terminal toggle (runner ``app.py``), but the brain is an
    in-process claude-sdk agent whose history Codify writes — so a web
    ``/model`` switch should land a durable ``[System: ...]`` note. Gating on
    ``codify.ui`` (the pre-fix behavior) wrongly suppressed it; the gate
    must key on the ``codify.wrapper`` native label instead.
    """
    published: list[tuple[str, dict[str, Any]]] = []
    monkeypatch.setattr(
        "codify.server.routes.sessions.session_stream.publish",
    ...
    session = await _create_session(
        ...
        # session launched via `codify run`.
        labels={"codify.ui": "terminal"},
    )
    ...
    assert patch.status_code == 200, patch.text
    # Note IS recorded: ``codify.ui == "terminal"`` alone must not suppress
    # it. An empty list here means the gate regressed to keying on
    # ``codify.ui``, re-breaking the polly/debby web ``/model`` feedback.
    assert _model_change_notes(published) == [
...
async def test_post_external_session_todos_publishes_session_todos(
    ...
    would break the panel for ``codify claude`` sessions: the UI would
...
async def test_external_user_message_seeds_title_on_claude_native_session(
    ...
    With the placeholder carve-out removed, ``codify claude``
...
async def test_interrupt_on_claude_native_session_skips_idle_publish_on_runner_failure(
    ...
    Codify must NOT lie to the UI by publishing idle. The spinner spins
    is the right signal — it tells the user the cancel didn't land.

    After the interrupt-unification refactor the Codify side no longer
    ...
    """
    from codify.runtime import session_stream, set_runner_client
...
async def test_stop_session_forwards_stop_session_event_to_runner(
    ...
    The Codify server stays harness-agnostic: it doesn't kill anything
    itself, it relays a ``{"type": "stop_session"}`` event to the
    runner, whose dispatch decides what to do (hard-kill tmux for
    claude-native, 204 for in-process). This pins that the Codify forward
    ...
    # stop_session type. 0 = the Codify branch didn't forward (no-op stop
...
async def test_stop_session_surfaces_runner_failure_as_error(
    ...
    Codify server swallowed the runner's 503 and returned 202
    ``{queued: false}``, the web UI would close its confirmation
    dialog as if the session stopped — the exact silent-failure the
    review flagged. This pins that the Codify route raises (non-2xx)
    ...
    from codify.server.routes.sessions import _interrupt_fenced_sessions
...
class _ForwardedEffort:
    ...
    :param url: Fully-qualified runner URL the Codify server POSTed to.
    :param body: Parsed JSON body the Codify server sent, or ``None``
...
async def test_patch_collaboration_mode_persists_label_and_forwards_event(
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["labels"]["codify.codex_native.collaboration_mode"] == "plan"
...
async def test_patch_collaboration_mode_requires_live_runner_before_persisting(
    ...
    assert "codify.codex_native.collaboration_mode" not in snapshot["labels"]
    assert published == []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, "high", "high", "high"),
        # (2) Non-native + same level → Codify server is harness-agnostic
        ...
        # Before refactor, Codify would short-circuit and not POST at all.
        (True, "default", None, None),
        # (4) Level in EFFORT_VALUES but not CLAUDE_EFFORTS (``none``,
        # ``minimal``). After refactor Codify no longer filters — it
...
async def test_patch_reasoning_effort_forwards_effort_change_event(
    ...
    runner ``/events`` — harness-agnostic on the Codify side.

    Before the refactor, Codify server made a native-only POST to
    ...
    * Codify server does not check ``_is_native_terminal_session``
    ...
    # Exactly one POST to the unified /events route. 0 = Codify server
    ...
    # happen anymore — its callsite is gone from Codify server. A non-
...
async def test_patch_reasoning_effort_swallows_runner_failure(
    ...
    contract on the Codify side is unchanged.
    """
    from codify.runtime import set_runner_client
    ...
    # not skipped). 0 = Codify server bailed before forwarding (regression
...
async def test_external_codex_subagent_start_mints_child_session(
    ...
    labels = child["labels"]
    assert labels["codify.codex_native.subagent_thread_id"] == "thread_child_alpha"
    assert labels["codify.codex_native.parent_thread_id"] == "thread_parent"
    assert labels["codify.codex_native.collab_tool_call_id"] == "collab_123"
    assert labels["codify.codex_native.agent_nickname"] == "auth-auditor"
    assert labels["codify.codex_native.agent_role"] == "reviewer"
...
async def test_external_codex_subagent_start_adopts_unlabeled_title_collision(
    ...
    # normal label lookup.
    assert children[0]["labels"]["codify.codex_native.subagent_thread_id"] == "thread_child_gamma"
...
async def test_external_codex_subagent_terminal_status_accepted_without_runner(
    ...
    Codify-spawned native sub-agents must forward terminal status to the
    ...
    """
    from codify.server.routes import sessions as sessions_mod
