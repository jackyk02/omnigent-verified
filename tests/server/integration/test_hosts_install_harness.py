...
``CODIFY_HARNESS_INSTALL_ENABLED``; the fixture enables it so the
...
def _enable_install_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The route is invisible (404) unless ``CODIFY_HARNESS_INSTALL_ENABLED``
    is truthy; the happy-path and validation tests need it on.
    """
    monkeypatch.setenv("CODIFY_HARNESS_INSTALL_ENABLED", "1")
...
async def test_install_harness_route_hidden_when_flag_off(
    ...
    """
    monkeypatch.setenv("CODIFY_HARNESS_INSTALL_ENABLED", "0")
