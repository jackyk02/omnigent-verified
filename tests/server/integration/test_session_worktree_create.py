...
from codify.server.host_registry import HostConnection
...
async def test_create_failure_never_removes_existing_worktree(
    ...
    for a *pre-existing* worktree without Codify creating one. The
    create-rollback (``git worktree remove --force`` + ``git branch -D``)
    is gated on Codify having created a worktree, NOT on ``git_branch``
...
async def test_create_failure_rolls_back_codify_created_worktree(
    register_worktree_host: RegisterHost,
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A create_conversation failure DOES clean up an Codify-made worktree.

    The counterpart to the data-loss guard: when Codify creates the
    ...
    rollback guard (gated on Codify having created a worktree) still
    ...
    # Codify created the worktree, so the rollback force-removed it: one
