...
from codify.server.routes._host_launch import HostLaunchTarget, resolve_host_launch
...
async def test_launch_runner_happy_path(
    ...
    async def _respond_to_launch() -> None:
        ...
        from codify.host.frames import HostLaunchRunnerFrame, decode_host_frame
...
async def test_launch_runner_harness_not_configured_returns_412(
    ...
    machine-readable code (and the `codify setup` hint) on the
    ...
    # CodifyError → JSON handler create_app installs (app.py), so the
    # route's raise surfaces exactly as it would in production wiring.
    @app.exception_handler(CodifyError)
    async def _handle(request: object, exc: CodifyError) -> JSONResponse:
        """Convert CodifyError to the production JSON error shape.
                ...
                await comm.send_input(
                    {
                        ...
                        "text": encode_host_frame(
                            HostLaunchRunnerResultFrame(
                                request_id=frame.request_id,
                                status="failed",
                                error=("harness 'codex' is not configured — run `codify setup`"),
    ...
    assert "codify setup" in body["error"]["message"]

    # _rollback_failed_launch ran: the session is fully unbound so a
    # retry after `codify setup` starts clean.
...
def multi_user_app(
    ...
    app.include_router(
        ...
        # default from tests/conftest.py (CODIFY_LOCAL_SINGLE_USER=1),
...
async def test_launch_runner_validates_workspace_boundary(
    ...
    from codify.server.routes import _workspace_validation
...
async def test_runner_exited_report_surfaces_in_runner_status(
    ...
    """
    from codify.host.frames import HostRunnerExitedFrame
    from codify.server.host_registry import RunnerExitReports
