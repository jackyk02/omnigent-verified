"""Tests for the file-backed admin roster (:mod:`codify.server.admin_list`).
...
from codify.server.admin_list import (
    AdminList,
...
from codify.server.routes.accounts_auth import create_accounts_auth_router
...
def test_resolve_admin_list_path_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_ADMIN_LIST_PATH`` wins over the default."""
    monkeypatch.setenv("CODIFY_ADMIN_LIST_PATH", "/etc/codify/admins")
    assert resolve_admin_list_path() == Path("/etc/codify/admins")
...
def test_resolve_data_dir_uses_credentials_parent(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_ADMIN_CREDENTIALS_PATH", "/data/admin-credentials")
    monkeypatch.delenv("CODIFY_ADMIN_LIST_PATH", raising=False)
...
def test_resolve_data_dir_defaults_to_home(monkeypatch: pytest.MonkeyPatch) -> None:
    """With no env, the data dir is ``~/.codify``."""
    monkeypatch.delenv("CODIFY_ADMIN_CREDENTIALS_PATH", raising=False)
    monkeypatch.delenv("CODIFY_ADMIN_LIST_PATH", raising=False)
    assert resolve_data_dir() == Path.home() / ".codify"
    assert resolve_admin_list_path() == Path.home() / ".codify" / "admins"
...
def test_load_admin_list_binds_resolved_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_ADMIN_LIST_PATH", "/tmp/codify-admins-test")
    al = load_admin_list()
    assert al.path == Path("/tmp/codify-admins-test")
...
def test_load_admin_list_passes_extra(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_ADMIN_LIST_PATH", "/tmp/codify-admins-absent")
