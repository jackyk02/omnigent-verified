...
from codify.server.ws_origin import (
...
def _clean_origin_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The middleware reads ``CODIFY_LOCAL_SINGLE_USER`` and
    ``CODIFY_WS_ALLOWED_ORIGINS`` per connection; a value inherited
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (CODIFY_INTERNAL_WS_ORIGIN, True, True),
        (CODIFY_INTERNAL_WS_ORIGIN, False, True),
...
def test_origin_allowed_with_allowlist(
    ...
    """Policy decisions when ``CODIFY_WS_ALLOWED_ORIGINS`` is set.
...
def test_e2e_local_mode_admits_internal_sentinel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with client.websocket_connect("/ws", headers={"origin": CODIFY_INTERNAL_WS_ORIGIN}) as ws:
        ws.send_text("hi")
        assert ws.receive_text() == "echo:hi"
    assert app.state.accepted == [True]
...
def test_e2e_allowlist_denies_unlisted_origin_in_non_local_mode(
    ...
    ``CODIFY_WS_ALLOWED_ORIGINS`` flips the non-local default from
