...
1. ``OIDCConfig`` parsing of ``CODIFY_OIDC_ALLOW_INVITES`` and the
...
from codify.server.admin_list import AdminList
from codify.server.auth import UnifiedAuthProvider
from codify.server.oidc import OIDCConfig, mint_session_cookie
from codify.server.oidc_access import OidcAdmissionPolicy
...
@pytest.mark.parametrize(
    ("value", "expected"),
    # Follows the project env-var convention (codify.server.auth
...
def test_allow_invites_env_parsing(
    ...
    """``CODIFY_OIDC_ALLOW_INVITES`` parses truthy values; default off.

    Built via the GitHub provider branch so no network discovery runs.
    """
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://github.com")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_ID", "cid")
    monkeypatch.setenv("CODIFY_OIDC_CLIENT_SECRET", "secret")
    monkeypatch.setenv("CODIFY_OIDC_REDIRECT_URI", "https://app.example.com/auth/callback")
    monkeypatch.setenv("CODIFY_OIDC_COOKIE_SECRET", "aa" * 32)
    monkeypatch.setenv("CODIFY_OIDC_ALLOW_INVITES", value)
