...
1. **Direct unit tests on the union (Part 1)** — fast, pure-Python
   invariants over :data:`codify.server.schemas.ServerStreamEvent`.
...
def test_emit_sites_referenced_by_grep_are_all_in_the_union() -> None:
    ...
    paths = [
        repo_root / "codify/runtime/workflow.py",
        repo_root / "codify/runtime/compaction.py",
        repo_root / "codify/runtime/policies/approval.py",
        repo_root / "codify/runtime/llm_retry.py",
        repo_root / "codify/runtime/tool_retry.py",
        repo_root / "codify/server/routes/sessions.py",
    ]
    ...
    assert not unknown, (
        ...
        f"codify.server.schemas and include it in the "
...
def test_session_created_event_optional_agent_id() -> None:
    ...
    Production code in
    :func:`codify.tools.builtins.spawn._publish_session_created_on_parent`
...
def test_publish_policy_denied_helper_emits_typed_event() -> None:
    ...
    """
    from codify.runtime import session_stream as cs
    from codify.server.routes.sessions import _publish_policy_denied
...
def test_policy_denied_format_sse_uses_response_prefixed_wire_name() -> None:
    ...
    """
    from codify.server.routes.sessions import _format_sse
...
async def test_stream_overflow_closes_without_done_for_reconnect(
    ...
    from codify.server.routes.sessions import _stream_live_events
...
def test_publish_session_status_helper_uses_waiting_literal() -> None:
    ...
    """
    from codify.runtime import session_stream as cs
    from codify.server.routes.sessions import _publish_status as _publish_session_status
...
def test_session_created_event_payload_shape() -> None:
    ...
    ``codify/runner/tool_dispatch.py::_execute_subagent_tool``).
