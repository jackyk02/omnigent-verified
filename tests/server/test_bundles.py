"""
Tests for uploaded agent bundle validation (``codify/server/bundles.py``).
...
from codify.server.bundles import validate_agent_bundle

_SECRET_ENV_VAR = "CODIFY_W7_BUNDLE_SECRET"
...
def _single_file_yaml_bundle(yaml_text: str) -> bytes:
    ...
    Produces the single-file codify YAML shape (no ``config.yaml``),
    which ``codify.spec.load`` dispatches to the inner loader — the
...
# Minimal codify ``config.yaml`` (AGENTSPEC directory shape).
_MIN_CONFIG = (
    "spec_version: 1\n"
    "name: {name}\n"
    "executor:\n"
...
def test_validate_agent_bundle_does_not_expand_env(
    ...
    server secret ``CODIFY_W7_BUNDLE_SECRET`` would be baked into the
            ...
            "tools/mcp/leaky.yaml": yaml.dump(
                {
                    ...
                    "headers": {"Authorization": "Bearer ${CODIFY_W7_BUNDLE_SECRET}"},
                }
            ),
        }
    )
    ...
    assert header == "Bearer ${CODIFY_W7_BUNDLE_SECRET}"
...
def test_validate_bundle_allows_custom_handler_when_not_enforced() -> None:
    ...
    This is the trusted single-user / local-server path: ``codify
...
def test_validate_bundle_accepts_registered_policy_handler() -> None:
    ...
    spec = validate_agent_bundle(
        _single_file_yaml_bundle(
            ...
            "  ask_os:\n"
            "    type: function\n"
            "    handler: codify.policies.builtins.safety.ask_on_os_tools\n"
        ),
    )
...
def test_validate_bundle_rejects_injection_handler_without_executing(
    ...
    with pytest.raises(CodifyError, match=r"not a registered policy handler"):
    ...
    bundle = _make_bundle_bytes(
        {
            ...
            "agents/good/config.yaml": (
                ...
                + "    ask_os:\n"
                + "      type: function\n"
                + "      function: codify.policies.builtins.safety.ask_on_os_tools\n"
            ),
        }
    )
    spec = validate_agent_bundle(bundle)
...
def test_validate_agent_bundle_rejects_escaping_os_env_cwd(bad_cwd: str) -> None:
    ...
    On a runner without ``CODIFY_RUNNER_WORKSPACE`` such a cwd becomes the
    ...
    """
    with pytest.raises(CodifyError, match=r"os_env\.cwd must be a relative path"):
...
# A codify function tool whose ``callable:`` is a dotted import path the
...
def test_validate_bundle_rejects_server_callable_tool() -> None:
    ...
    with pytest.raises(CodifyError, match=r"may not declare a server-side Python callable tool"):
        validate_agent_bundle(bundle)
...
def test_reject_uploaded_callable_tools_recurses_into_sub_agents() -> None:
    ...
    """
    from codify.server.bundles import _reject_uploaded_callable_tools
    from codify.spec import AgentSpec
    from codify.spec.types import LocalToolInfo, ToolRuntime
    ...
    sub.local_tools = [
        LocalToolInfo(
            ...
            language="codify-python-callable",
