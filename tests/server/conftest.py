...
from codify.server import _elicitation_registry, presence
...
def pytest_runtest_teardown(item: pytest.Item) -> None:
    ...
    raise RuntimeError(
        f"codify.server.routes.sessions.{', '.join(leaked)} left monkeypatched "
...
def runtime_init(
    ...
    monkeypatch.setattr(
        "codify.runtime.workflow._get_llm_client",
        lambda: mock_llm,
    )
    yield
...
def _first_party_origin_on_asgi(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _with_origin(self: httpx.ASGITransport, request: httpx.Request) -> httpx.Response:
        ...
        if "origin" not in request.headers:
            request.headers["origin"] = CODIFY_INTERNAL_WS_ORIGIN
