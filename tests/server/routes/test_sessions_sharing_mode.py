...
- ``create_app(sharing_mode=…)`` wiring — static value, per-request
  callable, and the ``CODIFY_SHARING_MODE`` env-var default.
...
from codify.server import sharing_settings
...
def test_wiring_defaults_to_on_when_env_unset(
    ...
    monkeypatch.delenv("CODIFY_SHARING_MODE", raising=False)
...
def test_wiring_reads_env_var(
    ...
    """``CODIFY_SHARING_MODE`` is the top-level control when no arg is given."""
    monkeypatch.setenv("CODIFY_SHARING_MODE", raw)
    app = _build_app(db_uri, tmp_path)
...
def test_wiring_static_value_overrides_env(
    ...
    monkeypatch.setenv("CODIFY_SHARING_MODE", "off")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "/var/home",
        "/workspaces/codify",  # a project checkout, not a home
        "/srv/work",
        "/tmp/session",
    ],
)
...
def test_override_beats_env_default(
    ...
    monkeypatch.setenv("CODIFY_SHARING_MODE", "on")
...
def test_env_default_used_when_no_override(
    ...
    monkeypatch.setenv("CODIFY_SHARING_MODE", "read_only")
    app = _build_app(db_uri, tmp_path)
    assert app.state.sharing_mode() is SharingMode.READ_ONLY
...
# ── Public-access switch (CODIFY_PUBLIC_SHARING) ───────────────────
...
def test_public_sharing_defaults_enabled(
    ...
    monkeypatch.delenv("CODIFY_PUBLIC_SHARING", raising=False)
    app = _build_app(db_uri, tmp_path)
...
def test_public_sharing_reads_env_var(
    ...
    """``CODIFY_PUBLIC_SHARING`` is the top-level default when no arg is given."""
    monkeypatch.setenv("CODIFY_PUBLIC_SHARING", raw)
    app = _build_app(db_uri, tmp_path)
...
def test_public_override_roundtrip_and_precedence(
    ...
    monkeypatch.setenv("CODIFY_PUBLIC_SHARING", "1")
...
async def test_read_only_grantee_can_pin_shared_session(db_uri: str, tmp_path: Path) -> None:
    ...
    async with _client(app, _GRANTEE) as c:
        resp = await c.patch(
            ...
            json={"labels": {"codify.pinned": "1721760000000"}},
        )
        assert resp.status_code == 200, resp.text
        assert resp.json()["labels"].get("codify.pinned") == "1721760000000"
...
async def test_read_only_grantee_cannot_edit_other_labels(db_uri: str, tmp_path: Path) -> None:
    ...
    async with _client(app, _GRANTEE) as c:
        ...
        resp = await c.patch(
            f"/v1/sessions/{conv.id}",
            json={"labels": {"codify.pinned": "1721760000000", "omni_project": "Moonshot"}},
        )
        assert resp.status_code == 403, resp.text
