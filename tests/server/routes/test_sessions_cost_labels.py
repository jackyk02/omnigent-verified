...
from codify.server.auth import LEVEL_EDIT, LEVEL_OWNER, UnifiedAuthProvider
...
def _install_error_handler(app: FastAPI) -> None:
    """Mirror ``create_app()``'s CodifyError → HTTP translation.

    :param app: The bare test app mounting only the sessions router.
    """

    @app.exception_handler(CodifyError)
    async def _handle_codify_error(request: Request, exc: CodifyError) -> JSONResponse:
        """Translate CodifyError to its HTTP status."""
        del request
        return JSONResponse(
...
def test_create_session_rejects_cost_control_label_seed(
    ...
    resp = TestClient(app).post(
        ...
        headers={"X-Forwarded-Email": ALICE, "Origin": CODIFY_INTERNAL_WS_ORIGIN},
    )
