...
# ── Pinned session label (codify.pinned) ───────────────────────────
...
async def test_patch_session_pins_and_unpins(
    ...
    """PATCH with the canonical ``labels: {"codify.pinned": <pin-time>}`` pins
    the session for the CALLER: the server rewrites it to the per-user key
    ``codify.pinned.<user>`` in storage (so it doesn't pin for others), and an
    ...
    assert "codify.pinned" not in conv.labels
...
async def test_patch_rejects_client_supplied_per_user_pin_key(
    ...
    """A client may only write the bare canonical ``codify.pinned`` key. A
    suffixed ``codify.pinned.<user>`` is server-derived — accepting one would
    ...
    for value in ("1721760000000", ""):
        resp = await client.patch(
            f"/v1/sessions/{session_id}",
            json={"labels": {"codify.pinned.bob@example.com": value}},
            headers={"Content-Type": "application/json"},
        )
        assert resp.status_code == 400
    conv = conv_store.get_conversation(session_id)
    assert conv is not None
    assert "codify.pinned.bob@example.com" not in conv.labels
