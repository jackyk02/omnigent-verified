...
:func:`codify.server.ws_origin.origin_allowed` policy into a FastAPI
...
- it sources ``local_mode`` from ``local_single_user_enabled()`` and
  ...
  proven by flipping ``CODIFY_LOCAL_SINGLE_USER`` /
  ``CODIFY_WS_ALLOWED_ORIGINS`` and watching the same Origin change verdict;
...
_LOCAL_ENV = "CODIFY_LOCAL_SINGLE_USER"
_ALLOWLIST_ENV = "CODIFY_WS_ALLOWED_ORIGINS"
...
def _clean_origin_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``require_trusted_origin`` reads ``CODIFY_LOCAL_SINGLE_USER`` and
    ``CODIFY_WS_ALLOWED_ORIGINS`` per call (the test suite sets the
...
def test_cross_origin_is_allowed_when_not_local_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    # _clean_origin_env left CODIFY_LOCAL_SINGLE_USER unset → non-local.
...
def test_sentinel_origin_is_allowed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``CODIFY_INTERNAL_WS_ORIGIN``; the shared policy always admits it. A
    raise here would block first-party traffic that opts to send the
    sentinel.
    """
    monkeypatch.setenv(_LOCAL_ENV, "1")
    assert require_trusted_origin(_request_with_origin(CODIFY_INTERNAL_WS_ORIGIN)) is None
...
def test_allowlisted_origin_is_allowed_and_unlisted_rejected(
    ...
    """
    ``CODIFY_WS_ALLOWED_ORIGINS`` wiring: listed passes, unlisted 403s.
