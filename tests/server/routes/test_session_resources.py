...
from codify.entities import DEFAULT_ENVIRONMENT_ID, Conversation, ConversationItem, PagedList
from codify.errors import CodifyError, ErrorCode
from codify.runtime import _globals, session_stream, set_runner_client, set_runner_router
from codify.server.routes.sessions import _ancestor_session_ids, create_sessions_router
from codify.server.schemas import SessionEventInput
...
async def test_claude_native_message_forwards_to_runner_without_persisting(
    ...
    """
    Claude-native web-chat input is runner injection, not Codify persistence.

    This fails if the route regresses to the legacy create-or-steer
    path, which would either start a duplicate Codify agent task or make
...
async def test_claude_native_message_surfaces_runner_sse_failure(
    ...
    This fails if the Codify route treats the runner's streaming HTTP 200
...
async def test_create_terminal_native_bootstrap_exempt_from_gate(
    ...
    The ``codify claude`` / ``codex`` wrappers launch the session's
...
async def test_create_terminal_surfaces_runner_error_without_crashing(
    ...
    ``CodifyError(..., http_status=status)``, but ``CodifyError``
...
async def test_transfer_terminal_surfaces_runner_error_without_crashing(
    ...
    its ``status >= 400`` branch built ``CodifyError(..., http_status=status)``,
    but ``CodifyError`` has no ``http_status`` arg (it is a derived
...
def file_store(db_uri: str) -> Any:
    ...
    from codify.stores.file_store.sqlalchemy_store import (
        SqlAlchemyFileStore,
    )
...
def file_app(
    ...
    @test_app.exception_handler(CodifyError)
    async def _handle(
        request: Request,
        exc: CodifyError,
    ) -> JSONResponse:
...
async def test_copy_files_rollback_deletes_blobs_when_row_delete_fails(
    ...
    caplog.set_level(logging.WARNING, logger="codify.server.routes.sessions")
...
async def test_copy_files_rejects_over_count_before_any_blob_read(
    ...
    """
    import codify.server.server_config as server_config
...
async def test_relay_persists_terminal_resource_created_from_runner() -> None:
    ...
    ``session.resource.created`` on its SSE stream. The Codify relay must
    ...
    REST resource path.
    """
    from codify.server.routes.sessions import _relay_runner_stream
...
async def test_relay_persists_failed_status_error_labels_from_runner() -> None:
    ...
    assert (
        store._conversations["79b22ebd2309e48fdeb450c65611d51b"].labels[
            "codify.last_task_error_code"
        ]
        == "required_terminal_exited"
    )
    assert (
        store._conversations["79b22ebd2309e48fdeb450c65611d51b"].labels[
            "codify.last_task_error_message"
        ]
...
async def test_relay_truncates_long_error_message_before_persisting() -> None:
    ...
    """
    from codify.server.routes.sessions import _LABEL_VALUE_MAX_LEN, _relay_runner_stream
...
async def test_native_dispatch_fast_fails_and_consumes_message_on_terminal_error() -> None:
    ...
    The Codify server first asks the runner to ensure the native terminal.
    When the runner returns a structured startup failure, Codify must not
    ...
    """
    from codify.server.routes.sessions import _dispatch_session_event_to_runner
    ...
    # the input pending forever; two would mean the Codify fast-fail path
...
async def test_kiro_external_prompt_matches_pending_and_reports_skipped_input() -> None:
    ...
    from codify.runtime import pending_inputs
    from codify.server.routes.sessions import _persist_external_conversation_item
...
async def test_native_dispatch_reports_malformed_runner_error_body() -> None:
    ...
    instead of the structured native startup error JSON, Codify still fast
...
async def test_native_dispatch_transport_error_does_not_fallback_to_forwarding() -> None:
    ...
    If Codify cannot reach the runner's terminal ensure endpoint, it must
...
async def test_native_dispatch_records_same_error_after_recovery_boundary() -> None:
    ...
    """
    from codify.entities import MessageData, NewConversationItem
    from codify.server.routes.sessions import _dispatch_session_event_to_runner
...
async def test_relay_publishes_inflight_frames_and_discards_on_exit() -> None:
    ...
    * **Leak fix:** when the relay task exits without a terminal event,
    ...
    """
    from codify.runtime import inflight_text, session_stream
    from codify.server.routes.sessions import _relay_runner_stream
...
async def test_relay_flushes_partial_text_on_failed_turn_before_error_item() -> None:
    ...
    """
    from codify.runtime import inflight_text
    from codify.server.routes.sessions import _relay_runner_stream
...
async def test_relay_lets_elicitation_resolved_pass_the_fence(
    ...
    """
    from codify.runtime import inflight_text, pending_elicitations, session_stream
    from codify.server.routes import sessions as sessions_module
    from codify.server.routes.sessions import _relay_runner_stream
...
class _SubagentTerminalStore:
    ...
    def list_items(
        ...
        *,
        ...
        from codify.entities import ConversationItem, MessageData, PagedList
...
def _make_subagent_conv(child_id: str, *, wrapper: str, kind: str = "sub_agent") -> Conversation:
    ...
    :param wrapper: ``codify.wrapper`` label, e.g.
    ...
    return Conversation(
        ...
        agent_id="087b7cb7ac30abf4debfaa578d052ec6",
        kind=kind,
        labels={"codify.ui": "terminal", "codify.wrapper": wrapper},
    )
