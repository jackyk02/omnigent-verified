...
import codify.server.routes.sessions as sessions_routes
from codify.server.auth import LEVEL_OWNER, UnifiedAuthProvider
from codify.server.routes.sessions import SessionLiveness, create_sessions_router
...
def test_watch_set_truncated_at_cap(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.server.routes.sessions"):
        with TestClient(app).websocket_connect(
            "/v1/sessions/updates", headers={"X-Forwarded-Email": ALICE}
        ) as ws:
...
def test_pin_label_collapses_to_canonical_key_on_the_wire(
    ...
    """Pins are stored per-user (``codify.pinned.<user>``), but the watch
    stream collapses the caller's own pin key to the canonical
    ``codify.pinned`` and never emits another user's per-user key. Exercised
    ...
    with TestClient(app).websocket_connect(
        ...
        assert labels.get("codify.pinned") == "1721760000000"
...
def comment_clock(monkeypatch: pytest.MonkeyPatch) -> dict[str, int]:
    ...
    state = {"now": 1_000}
    monkeypatch.setattr(
        "codify.stores.comment_store.sqlalchemy_store.now_epoch_us",
