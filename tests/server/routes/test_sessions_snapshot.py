...
from codify.entities import Conversation, ConversationItem, MessageData, PagedList
from codify.server.routes import sessions as _sessions_mod
...
async def test_session_snapshot_uses_child_spec_metadata(
    ...
    monkeypatch.setattr("codify.runtime.get_runner_client", lambda: None)
    monkeypatch.setattr("codify.runtime.get_runner_router", lambda: None)
...
async def test_session_snapshot_surfaces_status_error_labels_as_last_task_error() -> None:
    ...
    conv = Conversation(
        ...
        agent_id="087b7cb7ac30abf4debfaa578d052ec6",
        labels={
            "codify.last_task_error_code": "required_terminal_exited",
            "codify.last_task_error_message": "Required terminal exited unexpectedly",
        },
    )
    conv_store = _ConversationStore(
        [_message_item("item_1", "hi")],
...
async def test_session_snapshot_queries_runner_on_cache_miss(
    ...
    monkeypatch.setattr(
        "codify.runtime.get_runner_client",
        lambda: fake_client,
    )
...
async def test_session_snapshot_defaults_idle_when_runner_unreachable(
    ...
    monkeypatch.setattr(
        "codify.runtime.get_runner_router",
        lambda: None,
    )
...
async def test_session_snapshot_includes_skills_from_runner(
    ...
    fake_client = _FakeRunnerClient()
    monkeypatch.setattr("codify.runtime.get_runner_client", lambda: fake_client)
    monkeypatch.setattr("codify.runtime.get_runner_router", lambda: None)
...
async def test_session_snapshot_serves_pi_model_options_from_extension_push(
    ...
    authenticated (Codify provider OR pi's own ``/login``). Before any push,
...
async def test_session_snapshot_publishes_skills_event_when_fetch_resolves(
    ...
    monkeypatch.setattr("codify.runtime.get_runner_client", lambda: _FakeRunnerClient())
    monkeypatch.setattr("codify.runtime.get_runner_router", lambda: None)
    ...
    # tests — see codify-testing rule 14.
...
async def test_persist_error_labels_truncates_long_message() -> None:
    ...
    stored = captured["0099dc8be6d82871e2e450424d46d1b7"]["codify.last_task_error_message"]
...
async def test_persist_error_labels_short_message_stored_verbatim() -> None:
    ...
    assert (
        captured["d6e1678fb446a1cf5a892e0df60aaba3"]["codify.last_task_error_message"]
        == "Process exited with code 1"
    )
    assert (
        captured["d6e1678fb446a1cf5a892e0df60aaba3"]["codify.last_task_error_code"]
        == "runner_error"
    )
