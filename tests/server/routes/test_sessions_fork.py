...
from codify.entities import Agent, Conversation, ConversationItem, MessageData, PagedList
...
def _build_app(
    ...
    that ``CodifyError`` is translated into the correct HTTP
    ...
    async def _handle_codify_error(
        ...
        """Translate CodifyError to an HTTP error response."""
        del request
        return JSONResponse(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            True,
            {"codify.ui": "terminal", "codify.wrapper": "claude-code-native-ui"},
        ),
        ...
        # native transcript from the copied Codify items — but the source's
        ...
        (
            ...
            True,
            False,
            {"codify.ui": "terminal", "codify.wrapper": "codex-native-ui"},
        ),
        ...
        (
            ...
            True,
            False,
            {"codify.ui": "terminal", "codify.wrapper": "cursor-native-ui"},
        ),
        # pi-native CAN carry fork history: the runner rebuilds Pi's JSONL
        # session file from the copied Codify items. Cross-family from a
        ...
        (
            ...
            True,
            False,
            {"codify.ui": "terminal", "codify.wrapper": "pi-native-ui"},
        ),
        # qwen-native CAN carry fork history: the runner rebuilds qwen's on-disk
        # chat recording (+ runtime/meta sidecars) from the copied Codify items
        ...
        (
            ...
            True,
            False,
            {"codify.ui": "terminal", "codify.wrapper": "qwen-native-ui"},
        ),
...
async def test_fork_switch_model_and_carry_gating(
    ...
    monkeypatch.setattr(
        "codify.server.routes.sessions.get_agent_cache",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("cursor-native", True),
        # pi rebuilds its JSONL session file from the copied Codify items
...
async def test_fork_cursor_pi_native_carry_gating(
    ...
    JSONL session file from the copied Codify items. Both therefore mark
