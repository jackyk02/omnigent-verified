...
async def test_switch_same_family_native_carries_history(
    ...
    labels = {"codify.ui": "terminal", "codify.wrapper": "claude-code-native-ui"}
...
async def test_switch_cross_family_resets_model_but_carries_history(
    ...
    native transcript from this session's own Codify items, a conversion
    ...
    # rebuilds the transcript from Codify items. False here would mean
...
async def test_switch_publishes_agent_changed_event(
    ...
    # through the shared module singleton) — codify-testing rule 14.
