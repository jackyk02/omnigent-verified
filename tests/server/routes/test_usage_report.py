...
from codify.server.routes.usage import (
    _build_usage_report,
    _day_offset,
    _session_cost,
    _session_models,
...
def test_build_usage_report_summary_from_daily_rollup(
    ...
    # from codify.db.utils, so patch it there.
    monkeypatch.setattr("codify.db.utils.now_epoch", lambda: 1_784_678_400)
...
def test_build_usage_report_sessions_detail(
    ...
    monkeypatch.setattr("codify.db.utils.now_epoch", lambda: now)
    report = _build_usage_report(store, None)
