...
1. ``codify.server.passwords`` — argon2 wrapper (hash / verify
   round-trip, constant-time-equalized failures).
2. ``codify.server.accounts_config.AccountsConfig`` — env-var
   parsing, fail-loud validation, secure-cookie derivation.
3. ``codify.server.auth.UnifiedAuthProvider`` with
   ``source='accounts'`` — cookie validation, reserved-name
   rejection, ``create_auth_provider`` factory gating.
4. ``codify.server.accounts_bootstrap.bootstrap_admin`` matrix
   (auto-gen / pre-seed / idempotent / loopback handoff).
5. ``codify.server.routes.accounts_auth`` — login / logout /
...
from codify.server.accounts_bootstrap import (
    bootstrap_admin,
    resolve_admin_username,
)
from codify.server.accounts_config import AccountsConfig
...
from codify.server.passwords import (
    InvalidPasswordError,
    hash_password,
    needs_rehash,
    verify_password,
)
from codify.stores.permission_store.sqlalchemy_store import (
...
def _clear_ambient_oidc_issuer(monkeypatch: pytest.MonkeyPatch) -> None:
    """Strip an ambient ``CODIFY_OIDC_ISSUER`` for the accounts suite.
    ...
    """
    monkeypatch.delenv("CODIFY_OIDC_ISSUER", raising=False)
...
def _set_required_accounts_env(
    ...
    *,
    base_url: str = "https://codify.example.com",
) -> None:
    """Populate every required env var so from_env() doesn't fail loud."""
    monkeypatch.setenv("CODIFY_ACCOUNTS_COOKIE_SECRET", secrets.token_hex(32))
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", base_url)
...
def test_accounts_config_round_trips_required_env(
    ...
    monkeypatch.setenv("CODIFY_ACCOUNTS_COOKIE_SECRET", secret_hex)
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", "https://codify.example.com")

    cfg = AccountsConfig.from_env()

    assert cfg.cookie_secret == bytes.fromhex(secret_hex)
    assert cfg.base_url == "https://codify.example.com"
...
def test_accounts_config_missing_cookie_secret_fails_loud(
    ...
    monkeypatch.delenv("CODIFY_ACCOUNTS_COOKIE_SECRET", raising=False)
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", "http://localhost:8000")

    with pytest.raises(RuntimeError, match="CODIFY_ACCOUNTS_COOKIE_SECRET"):
...
def test_accounts_config_short_cookie_secret_fails_loud(
    ...
    """
    monkeypatch.setenv("CODIFY_ACCOUNTS_COOKIE_SECRET", "00" * 16)  # only 16 bytes
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", "http://localhost:8000")
...
def test_accounts_config_non_hex_cookie_secret_fails_loud(
    ...
    monkeypatch.setenv("CODIFY_ACCOUNTS_COOKIE_SECRET", "not-hex-at-all")
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", "http://localhost:8000")
...
def test_accounts_config_rejects_non_http_scheme(
    ...
    monkeypatch.setenv("CODIFY_ACCOUNTS_COOKIE_SECRET", secrets.token_hex(32))
    monkeypatch.setenv("CODIFY_ACCOUNTS_BASE_URL", "ftp://codify.example.com")
...
def test_accounts_config_init_admin_empty_string_is_unset(
    ...
    monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD", "")
...
def test_accounts_source_reads_valid_cookie() -> None:
    ...
    from codify.server.oidc import mint_session_cookie
...
def test_accounts_source_accepts_bearer_token_for_cli() -> None:
    ...
    the token up from ~/.codify/auth_tokens.json — the same
...
def test_resolve_auth_source_defaults_to_header(
    ...
    desync them.
    """
    monkeypatch.delenv("CODIFY_AUTH_PROVIDER", raising=False)
    monkeypatch.delenv("CODIFY_AUTH_ENABLED", raising=False)
...
def test_resolve_auth_source_opt_in_selects_accounts(
    ...
    """``CODIFY_AUTH_ENABLED=1`` (no OIDC config) opts into accounts mode."""
    monkeypatch.delenv("CODIFY_AUTH_PROVIDER", raising=False)
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "1")
...
def test_resolve_auth_source_oidc_issuer_selects_oidc(
    ...
    """``CODIFY_AUTH_ENABLED=1`` + an OIDC issuer selects oidc, not accounts.
    ...
    monkeypatch.delenv("CODIFY_AUTH_PROVIDER", raising=False)
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "1")
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://accounts.google.com")
...
def test_resolve_auth_source_explicit_passthrough_lowercased(
    ...
    """
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "OIDC")
...
def test_factory_defaults_to_header_when_env_unset(
    ...
    """Unset CODIFY_AUTH_PROVIDER (+ no enable switch) → header mode.

    The shipped default is single-user, no-login: a bare
    ``codify server`` on a laptop should pop open with no
    multi-user wiring. Multi-user (accounts) is opt-in via
    ``CODIFY_AUTH_ENABLED=1`` (see
...
def test_factory_explicit_header_beats_enable_switch(
    ...
    """Explicit ``CODIFY_AUTH_PROVIDER=header`` wins over the enable switch.

    An explicit provider always wins, so a stale
    ``CODIFY_AUTH_ENABLED=1`` in a shell can't silently turn
    accounts on for a deploy that pinned header (e.g. the internal
    hosted product, which sets header via ``setdefault`` in its
    entrypoint).
    """
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "header")
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "1")
...
def test_factory_accepts_accounts_explicit(
    ...
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "accounts")
    _set_required_accounts_env(monkeypatch)
...
def test_factory_rejects_unknown_source(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "bogus")
...
def test_factory_accounts_enabled_falsy_stays_header(
    ...
    """An explicitly falsy ``CODIFY_AUTH_ENABLED`` → header mode.
    ...
    """
    monkeypatch.delenv("CODIFY_AUTH_PROVIDER", raising=False)
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", disable_value)
...
def test_factory_accounts_enabled_truthy_enables_accounts(
    ...
    """A truthy ``CODIFY_AUTH_ENABLED`` (no OIDC) opts INTO accounts mode.

    This is the multi-user opt-in: with no explicit
    ``CODIFY_AUTH_PROVIDER`` and no OIDC issuer, a truthy enable
    switch turns on the accounts login flow (the inverse of the
    env-unset header default).
    """
    monkeypatch.delenv("CODIFY_AUTH_PROVIDER", raising=False)
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", enable_value)
    _set_required_accounts_env(monkeypatch)
...
def fresh_store(tmp_path: Path) -> SqlAlchemyAccountStore:
    ...
    from codify.db.utils import get_or_create_engine
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    ``~/.codify/auth_tokens.json`` — fine, but noisy. The
    fixture also pins CODIFY_ADMIN_CREDENTIALS_PATH so the
    bootstrap's 0600 file lands inside the tmp dir too.
    """
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("CODIFY_ADMIN_CREDENTIALS_PATH", str(tmp_path / "admin-credentials"))
    ...
    # below.
    monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME", "admin")
    return tmp_path
...
def test_bootstrap_with_password_creates_admin(
    ...
    ``CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD``) is the one
...
def test_bootstrap_remote_no_password_needs_setup_no_token(
    ...
    result = bootstrap_admin(
        fresh_store,
        base_url="https://codify.example.com",
    ...
    assert not (isolated_home / ".codify" / "auth_tokens.json").exists()
...
def test_bootstrap_init_password_loopback_writes_cli_token_no_autoopen(
    ...
    (so ``codify run`` is signed in), but does NOT auto-open the
...
def test_bootstrap_refreshes_cli_token_on_returning_loopback_boot(
    ...
    must re-mint a token for the current URL — otherwise ``codify
    ...
    assert cli_auth.load_token(new_url) is not None, (
        "returning boot must mint a CLI token for the new spawn URL so `codify run` authenticates"
    )
...
def test_resolve_admin_username_uses_env_override(
    ...
    """CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME wins over the OS user.
    ...
    """
    monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME", "operator")
...
def test_resolve_admin_username_falls_back_to_os_user(
    ...
    This is the laptop-UX win — running ``codify server`` as
    ...
    """
    monkeypatch.delenv("CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME", raising=False)
...
def _build_accounts_app(
    ...
    *,
    ...
    if init_admin_password is not None:
        monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD", init_admin_password)
    else:
        monkeypatch.delenv("CODIFY_ACCOUNTS_INIT_ADMIN_PASSWORD", raising=False)
    # Pin the admin username to "admin" so the existing test
    # assertions don't depend on whatever getpass.getuser() returns
    # in CI. The OS-username resolution path is exercised by
    # dedicated tests below.
    monkeypatch.setenv("CODIFY_ACCOUNTS_INIT_ADMIN_USERNAME", "admin")
    monkeypatch.setenv("CODIFY_ADMIN_CREDENTIALS_PATH", str(tmp_path / "admin-creds"))
    # Don't auto-open the browser during tests.
    monkeypatch.setenv("CODIFY_ACCOUNTS_AUTO_OPEN", "0")
    ...
    from codify.stores.agent_store.sqlalchemy_store import (
        SqlAlchemyAgentStore,
    )
    from codify.stores.artifact_store.local import LocalArtifactStore
    from codify.stores.comment_store.sqlalchemy_store import (
...
def header_mode_app(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[TestClient]:
    ...
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "header")
    monkeypatch.delenv("CODIFY_ACCOUNTS_COOKIE_SECRET", raising=False)
    monkeypatch.delenv("CODIFY_ACCOUNTS_BASE_URL", raising=False)
...
def test_admin_list_excludes_legacy_local_and_public_sentinels(
    ...
    """
    from codify.db.db_models import SqlUser
    from codify.db.utils import get_or_create_engine, make_managed_session_maker
...
def test_cli_accounts_login_happy_path_stores_token(
    ...
    # The store_token side effect lands in ~/.codify/auth_tokens.json.
...
def test_setup_writes_loopback_cli_token(
    ...
    The local CUJ: ``codify run`` (re)spawns the local server in
    ...
    the in-flight ``codify run`` is signed in immediately instead of
