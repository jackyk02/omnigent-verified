"""Tests for the shared server-config loader (:mod:`codify.server.server_config`).
...
def _pin_data_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("CODIFY_CONFIG", raising=False)
...
def test_resolve_config_path_env_override(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """``CODIFY_CONFIG`` wins over the data-dir default."""
    p = tmp_path / "custom.yaml"
    p.write_text("{}")
    monkeypatch.setenv("CODIFY_CONFIG", str(p))
