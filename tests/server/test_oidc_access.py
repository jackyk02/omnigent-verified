"""Tests for the OIDC admission policy (:mod:`codify.server.oidc_access`).
...
def test_resolve_allowed_domains_path_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CODIFY_OIDC_ALLOWED_DOMAINS_PATH`` wins over the default."""
    monkeypatch.setenv("CODIFY_OIDC_ALLOWED_DOMAINS_PATH", "/etc/codify/domains")
    assert resolve_allowed_domains_path() == Path("/etc/codify/domains")
...
def test_resolve_allowed_domains_path_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CODIFY_OIDC_ALLOWED_DOMAINS_PATH", raising=False)
