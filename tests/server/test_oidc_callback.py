...
from codify.server.routes.auth import _AUTH_STATE_COOKIE_PLAIN, create_auth_router
...
def _oidc_config(
    ...
    :param skip_email_verification: Waive the ``email_verified`` gate,
        as ``CODIFY_OIDC_SKIP_EMAIL_VERIFICATION`` would.
    :param email_claim: Claim carrying the email identity, as
        ``CODIFY_OIDC_EMAIL_CLAIM`` would set it.
    """
    return OIDCConfig(
        issuer=_ISSUER,
...
def test_callback_skip_verification_flag_admits_unverified(
    ...
    opted out via ``CODIFY_OIDC_SKIP_EMAIL_VERIFICATION``.
    """
    client, keys = callback_client
    token = keys.sign_id_token(claims)
...
def test_callback_custom_email_claim_admits_upn(
    ...
    claim configured via ``CODIFY_OIDC_EMAIL_CLAIM`` and the
...
def test_callback_custom_email_claim_still_requires_verification_optout(
    ...
    ``CODIFY_OIDC_SKIP_EMAIL_VERIFICATION`` both shapes are rejected.
    """
    client, keys = callback_client
    token = keys.sign_id_token(claims)
