"""Tests for the ``codify_conversation_metadata.runner_id`` column.
...
``codify_conversation_metadata`` rather than ``conversations``.
"""
...
def test_migration_adds_runner_id_column_nullable(db_engine: Engine) -> None:
    """The migration creates ``codify_conversation_metadata.runner_id``
    as nullable VARCHAR(64).
    ...
    assert len(runner_id_cols) == 1, (
        f"Expected exactly one 'runner_id' column on codify_conversation_metadata, "
        f"got {len(runner_id_cols)}. "
        f"If 0, the migration didn't include the column."
    )
    runner_id_col = runner_id_cols[0]
    assert runner_id_col["nullable"], "codify_conversation_metadata.runner_id must be NULLABLE"
...
def test_runner_id_round_trip_null_and_value(db_engine: Engine) -> None:
    ...
    with db_engine.connect() as conn:
        ...
        result = conn.execute(
            sa.text("SELECT runner_id FROM codify_conversation_metadata WHERE id = :id"),
            {"id": "0d42a93a625e91d8b607d375fbd860ad"},
        ).scalar_one()
        ...
        conn.execute(
            sa.text(
                "INSERT INTO codify_conversation_metadata (id, kind, runner_id) "
                "VALUES (:id, 1, :rid)"
            ),
...
def test_no_foreign_key_constraint_on_runner_id(db_engine: Engine) -> None:
    """``codify_conversation_metadata.runner_id`` MUST NOT have a foreign key constraint.
    ...
    """
    fks = sa.inspect(db_engine).get_foreign_keys("codify_conversation_metadata")
    runner_fks = [fk for fk in fks if "runner_id" in fk.get("constrained_columns", [])]
    assert runner_fks == [], (
        f"codify_conversation_metadata.runner_id should have no FK; got {runner_fks}. "
