...
from codify.db.utils import _build_alembic_config, clear_engine_cache
...
def test_metadata_split_round_trip_with_host_bound_row(tmp_path: Path) -> None:
    ...
    # Upgrade: operational columns move to codify_conversation_metadata.
        ...
        rows = {
            ...
            for r in conn.execute(
                sa.text(
                    ...
                    " FROM codify_conversation_metadata ORDER BY id"
                )
            )
        }
    ...
    assert "codify_conversation_metadata" not in sa.inspect(raw_engine).get_table_names()
    with raw_engine.begin() as conn:
