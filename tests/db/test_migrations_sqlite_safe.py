...
Codify runs the same migration chain against Postgres/Lakebase (the
...
_VERSIONS_DIR = Path(codify.db.__file__).parent / "migrations" / "versions"
...
def test_full_migration_chain_round_trips_on_sqlite() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        config.set_main_option(
            "script_location", str(Path(codify.db.__file__).parent / "migrations")
        )
