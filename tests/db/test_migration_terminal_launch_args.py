"""Tests for the ``codify_conversation_metadata.terminal_launch_args`` column.
...
(``codify.db.compression``). These tests exercise the schema directly (raw
...
def test_terminal_launch_args_column_present_and_nullable(db_engine: Engine) -> None:
    ...
    ``codify_conversation_metadata.terminal_launch_args``
    ...
    ``codify.db.compression``, whose framed bytes can contain NUL and
    ...
    assert len(matches) == 1, (
        ...
        f"codify_conversation_metadata, got {len(matches)}. "
        f"If 0, the migration didn't apply."
    )
    col = matches[0]
    assert col["nullable"], (
        "codify_conversation_metadata.terminal_launch_args must be NULLABLE — "
...
def test_terminal_launch_args_round_trip_null_and_json(db_engine: Engine) -> None:
    ...
    with db_engine.connect() as conn:
        ...
        result = conn.execute(
            sa.text(
                "SELECT terminal_launch_args FROM codify_conversation_metadata WHERE id = :id"
            ),
            {"id": "8c2ab1547c91fcc61ae8226f37d07400"},
        ).scalar_one()
