"""Tests for the ``codify_conversation_metadata.workspace`` column and its check constraint.
...
These columns now live on ``codify_conversation_metadata`` rather than
...
from codify.db.utils import clear_engine_cache, get_or_create_engine
...
def test_workspace_column_present_and_nullable(db_engine: Engine) -> None:
    """
    Verify the migration creates ``codify_conversation_metadata.workspace``
    ...
    """
    cols = sa.inspect(db_engine).get_columns("codify_conversation_metadata")
    workspace_cols = [c for c in cols if c["name"] == "workspace"]
    assert len(workspace_cols) == 1, (
        f"Expected exactly one 'workspace' column on codify_conversation_metadata, "
        f"got {len(workspace_cols)}. If 0, the migration didn't apply."
    )
    workspace_col = workspace_cols[0]
    assert workspace_col["nullable"], (
        "codify_conversation_metadata.workspace must be NULLABLE — pre-feature rows "
...
def test_workspace_round_trip_null_and_value(db_engine: Engine) -> None:
    ...
    with db_engine.connect() as conn:
        ...
        conn.execute(
            sa.text("INSERT INTO codify_conversation_metadata (id, kind) VALUES (:id, 1)"),
            {"id": "5221a3ae03d21ff063a9255347dde591"},
        )
        result = conn.execute(
            sa.text("SELECT workspace FROM codify_conversation_metadata WHERE id = :id"),
            {"id": "5221a3ae03d21ff063a9255347dde591"},
        ).scalar_one()
        ...
        conn.execute(
            sa.text(
                "INSERT INTO codify_conversation_metadata "
                "(id, kind, workspace) "
                "VALUES (:id, 1, :ws)"
            ),
            {
...
def test_check_constraint_blocks_host_id_without_workspace(
    ...
    ``(host_id NOT NULL, workspace NULL)`` on codify_conversation_metadata.
...
def test_check_constraint_allows_host_id_with_workspace(
    ...
    with db_engine.connect() as conn:
        ...
        result = conn.execute(
            sa.text("SELECT host_id, workspace FROM codify_conversation_metadata WHERE id = :id"),
            {"id": "8ebb129f4017d3388ddf25bd4d2d731c"},
        ).one()
...
def test_runner_id_is_indexed(db_engine: Engine) -> None:
    ...
    codify_conversation_metadata at head.
    ...
    """
    index_names = {
        ix["name"] for ix in sa.inspect(db_engine).get_indexes("codify_conversation_metadata")
    }
    assert "ix_conversation_metadata_runner_id" in index_names, (
        f"Expected ix_conversation_metadata_runner_id on codify_conversation_metadata; "
...
def test_compressed_columns_are_binary_at_head(db_engine: Engine) -> None:
    ...
    These columns are stored zstd-compressed by ``codify.db.compression``;
    ...
    live on ``codify_conversation_metadata``.
    """
    inspector = sa.inspect(db_engine)
    expected = {
        "codify_conversation_metadata": [
            "session_usage",
            "session_state",
