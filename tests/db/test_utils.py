"""Tests for database engine pool configuration (codify/db/utils.py)."""
...
def test_sqlite_engine_skips_server_pool_settings_and_enables_wal(
    ...
    connection so multi-process workloads (REPL + Codify server +
...
def _clear_lakebase_override() -> Any:
    ...
    from codify.db.utils import set_lakebase_token_provider as _set
...
def test_static_postgres_uri_path_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from codify.db import utils

    # No override installed (autouse fixture) and no env var → no token path.
    monkeypatch.delenv("CODIFY_LAKEBASE_INSTANCE", raising=False)
...
def test_resolve_token_provider_env_and_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    The provider resolves from ``CODIFY_LAKEBASE_INSTANCE`` when set, and an
    ...
    monkeypatch.setenv("CODIFY_LAKEBASE_INSTANCE", "codify-db")
...
def test_initialize_or_verify_schema_reports_manual_retry_when_auto_migration_fails(
    ...
    head, DB URL, and manual ``codify debug db-upgrade`` retry command.
    """
    ...
    monkeypatch.setattr("codify.db.utils._run_migrations", _fail_migration)
    ...
    assert "codify debug db-upgrade" in msg, (
...
def test_item_type_id_and_data_registries_cover_the_same_types() -> None:
    ...
    the id map)."""
    from codify.db.utils import _ITEM_TYPES
