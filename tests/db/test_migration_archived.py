...
``codify_conversation_metadata``. Migration ``9d820f91deef`` moves it back
...
def test_archived_column_present_and_not_nullable(db_engine: Engine) -> None:
    ...
    removes it from ``codify_conversation_metadata``.
    ...
    meta_cols = [
        c for c in insp.get_columns("codify_conversation_metadata") if c["name"] == "archived"
    ]
    assert not meta_cols, (
        "archived must no longer exist on codify_conversation_metadata after the move."
    )
...
def test_archived_backfilled_from_metadata(tmp_path: Path) -> None:
    ...
    ``codify_conversation_metadata`` onto ``conversations``.
    ...
    try:
        ...
        with raw_engine.begin() as conn:
            ...
            conn.execute(
                sa.text(
                    "INSERT INTO codify_conversation_metadata (id, kind, archived) "
                    "VALUES (:a, 1, 1), (:b, 1, 0)"
                ),
                {"a": id_a, "b": id_b},
            )
