"""Tests for codify.harness_aliases."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # specs and CODIFY_HARNESSES use "openai-agents".
...
def test_canonicalize_harness(alias: str | None, canonical: str | None) -> None:
    ...
    ``codify run ... --harness openai-agents-sdk`` invocation at
    ``_validate_harness``.
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("kiro-native", True),
        ("native-kiro", True),
        # SDK harnesses are NOT native — they replay the Codify
...
def test_kiro_native_is_valid_codify_harness_but_plain_kiro_is_not() -> None:
    """Kiro's native identity is canonical; plain ``kiro`` is not a generic harness."""
    assert "kiro-native" in CODIFY_HARNESSES
    assert "kiro" not in CODIFY_HARNESSES
