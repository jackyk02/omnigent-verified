...
def _patch_client_deps(
    ...
    monkeypatch.setattr(
        "codify.llms.client.parse_model_string",
        lambda model: routed,
    )
    ...
    monkeypatch.setattr(
        "codify.llms.client.get_adapter",
        lambda provider: mock_adapter,
    )

    # Stub the responses-to-chat conversion helpers
    monkeypatch.setattr(
        "codify.llms.client.responses_input_to_chat_messages",
        lambda input, instructions: [{"role": "user", "content": "test"}],
    )
    monkeypatch.setattr(
        "codify.llms.client.chat_response_to_response",
...
async def test_text_json_schema_translated_to_response_format(
    ...
    """
    from codify.llms.routing import RoutedModel

    captured: list[dict[str, Any]] = []
    adapter = _CapturingAdapter(captured)
    routed = RoutedModel(provider="databricks", model="test-model")

    monkeypatch.setattr("codify.llms.client.parse_model_string", lambda model: routed)
    monkeypatch.setattr("codify.llms.client.get_adapter", lambda provider: adapter)
    monkeypatch.setattr(
        "codify.llms.client.responses_input_to_chat_messages",
        lambda input, instructions: [{"role": "user", "content": "test"}],
    )
