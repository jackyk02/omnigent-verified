...
from codify.llms.adapters.vertex import _build_vertex_url, _resolve_vertex_params


def test_resolve_raises_when_no_params() -> None:
    """
    ``None`` input raises ``CodifyError`` — Vertex requires
    connection_params.
    """
    with pytest.raises(CodifyError, match="requires connection_params"):
        _resolve_vertex_params(None)


def test_resolve_raises_when_empty_params() -> None:
    """
    Empty dict raises ``CodifyError`` — Vertex requires
...
def test_resolve_raises_when_project_missing() -> None:
    """
    CodifyError when ``"location"`` is provided but ``"project"`` is not.
    No env var fallback.
    """
    params = {"location": "us-east1"}
    with pytest.raises(CodifyError, match="requires 'project'"):
        _resolve_vertex_params(params)


def test_resolve_raises_when_location_missing() -> None:
    """
    CodifyError when ``"project"`` is provided but ``"location"`` is not.
    No env var fallback.
    """
    params = {"project": "my-proj"}
    with pytest.raises(CodifyError, match="requires 'location'"):
        _resolve_vertex_params(params)
...
def test_resolve_raises_when_no_recognized_keys() -> None:
    ...
    raise CodifyError — Vertex needs at least one of these.
    """
...
def test_get_base_url_raises() -> None:
    ...
    from codify.llms.adapters.vertex import VertexAdapter

    adapter = VertexAdapter()
    with pytest.raises(CodifyError, match="requires"):
