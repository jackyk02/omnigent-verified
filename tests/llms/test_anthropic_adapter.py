...
from codify.llms.adapters.anthropic import (
    _anthropic_to_chat,
    _chat_to_anthropic,
...
from codify.llms.errors import ContextWindowExceededError, PermanentLLMError
from codify.model_metadata import ModelMetadata, ModelReasoningMetadata, ModelReasoningMode
...
def test_build_headers_with_api_key() -> None:
    ...
    from codify.llms.adapters.anthropic import _build_headers
...
def test_effort_to_budget_low() -> None:
    from codify.llms.adapters.anthropic import _effort_to_budget
...
async def test_model_metadata_lookup_does_not_cache_failures(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.llms.adapters.anthropic"):
        first = await _get_anthropic_model_metadata(
            headers,
            "https://api.anthropic.com/v1",
            "claude-opus-4-8",
...
async def test_stream_to_chat_chunks_text_delta() -> None:
    ...
    from codify.llms.adapters.anthropic import _stream_to_chat_chunks
