...
def test_load_registry_discovers_builtins() -> None:
    ...
    assert "codify.policies.builtins.safety.max_tool_calls_per_session" in handlers
    assert "codify.policies.builtins.safety.ask_on_os_tools" in handlers
...
def test_registry_entries_have_correct_kind() -> None:
    ...
    assert (
        by_handler["codify.policies.builtins.safety.max_tool_calls_per_session"].kind == "factory"
    )
    assert by_handler["codify.policies.builtins.safety.ask_on_os_tools"].kind == "callable"
...
def test_get_params_schema_returns_schema() -> None:
    ...
    load_registry()
    schema = get_params_schema("codify.policies.builtins.safety.max_tool_calls_per_session")
...
def test_blast_radius_schema_explains_configuration() -> None:
    ...
    load_registry()
    schema = get_params_schema("codify.policies.builtins.orchestration.blast_radius")
...
def test_get_params_schema_none_for_direct_callable() -> None:
    ...
    load_registry()
    schema = get_params_schema("codify.policies.builtins.safety.ask_on_os_tools")
...
def test_validate_valid_params() -> None:
    ...
    error = validate_factory_params(
        "codify.policies.builtins.safety.max_tool_calls_per_session",
        {"limit": 5},
    )
...
def test_validate_params_on_no_schema_callable() -> None:
    ...
    load_registry()
    error = validate_factory_params(
        "codify.policies.builtins.safety.ask_on_os_tools",
        {"unexpected": 1},
    )
    assert error is not None
...
def test_is_registered_handler_true_for_builtin() -> None:
    ...
    load_registry()
    assert is_registered_handler("codify.policies.builtins.safety.ask_on_os_tools") is True
...
def test_is_registered_handler_false_for_unregistered_first_party() -> None:
    ...
    the ``codify`` package but is not a browsable registry entry, so the
    ...
    """
    load_registry()
    assert is_registered_handler("codify.policies.function.make_fixed_action_callable") is False
...
def test_is_registered_handler_includes_extra_modules() -> None:
    ...
    load_registry(extra_modules=["codify.policies.builtins.google"])
    assert is_registered_handler("codify.policies.builtins.google.gdrive_policy") is True
