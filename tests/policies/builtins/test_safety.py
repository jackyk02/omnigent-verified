...
(:mod:`codify.policies.builtins.safety`).

Covers:

- ``ask_on_os_tools`` — ASKs approval before file/shell tool calls,
  including Codify ``sys_os_*`` tools, Claude Code native tools
...
from codify.policies.builtins.safety import ask_on_os_tools, block_skills
from codify.policies.schema import PolicyEvent
from tests.policies.builtins.helpers import tool_call_event as tc

# ── ask_on_os_tools: Codify sys_os_* tools ─────────────────────────────
...
def test_ask_on_os_tools_asks_for_sys_os_tools(tool: str) -> None:
    ...
    If any returns ALLOW, the policy is not matching the Codify
...
def test_ask_on_os_tools_asks_for_pi_native_tools(
    ...
    (which match the Codify convention, not Claude's ``file_path``)
...
def _request_event(text: str) -> PolicyEvent:
    ...
    The Codify server evaluates skill slash commands at the REQUEST phase
...
def test_block_skills_denies_slash_command_blocked_skill() -> None:
    ...
    This is the path the Codify server takes when the user types
...
def test_ask_on_add_policy_asks_for_sys_add_policy() -> None:
    ...
    from codify.policies.builtins.safety import ask_on_add_policy
    ...
    result = ask_on_add_policy(
        {
            ...
            "data": {
                ...
                "arguments": {
                    "name": "block_shell",
                    "handler": "codify.policies.builtins.cel.cel_policy",
                },
            },
        }
    )
...
def test_block_skills_denies_native_skill_tool() -> None:
    ...
    ``PreToolUse`` hook fires → Codify server evaluates → this policy denies.
