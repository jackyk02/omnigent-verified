"""
Tests for :mod:`codify.policies.builtins.routing`.
...
from codify.policies.builtins.routing import (
    _CACHE_KEY_PREFIX,
...
class _FakeResponse:
    """
    Minimal stand-in for ``codify.llms.types.Response``.
...
def test_registry_entry_well_formed() -> None:
    ...
    assert "codify.policies.builtins.routing.deny_trivial_to_expensive_model" in handlers
    assert "codify.policies.builtins.routing.intent_based_authorization" in handlers

    trivial_entry = next(
        e
        for e in POLICY_REGISTRY
        if e["handler"] == "codify.policies.builtins.routing.deny_trivial_to_expensive_model"
    )
    ...
    intent_entry = next(
        e
        for e in POLICY_REGISTRY
        if e["handler"] == "codify.policies.builtins.routing.intent_based_authorization"
    )
