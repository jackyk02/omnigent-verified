...
def test_parse_volume_root_with_prefix() -> None:
    ...
    **What breaks if wrong**: prefix is stripped, artifacts land
    ...
    """
    result = _parse_volume_root("dbfs:/Volumes/cat/schema/vol/codify/artifacts")
    assert result == "/Volumes/cat/schema/vol/codify/artifacts"
...
def test_resolve_constructs_full_path(
    ...
    **What breaks if wrong**: SDK receives wrong file path,
    ...
    """
    monkeypatch.setattr(
        "codify.stores.artifact_store.databricks_volumes._ensure_databricks_sdk",
        lambda: None,
    )
