...
Codify DB and AP DB backed by two separate SQLite files, verifying that
...
@pytest.fixture()
def codify_db(tmp_path: Path) -> Path:
    return tmp_path / "codify.db"
...
@pytest.fixture()
def store(codify_db: Path, conv_db: Path) -> SqlAlchemyConversationStore:
...
def test_tables_live_in_correct_db(
    ...
    codify_tables = _tables(codify_db)
    ...
    # Codify tables in codify_db
    for t in ("codify_conversation_metadata", "agents", "hosts", "policies", "comments"):
        assert t in codify_tables, f"{t} missing from codify_db"

    # AP tables must NOT appear in codify_db (no schema migration runs there)
    assert "codify_conversation_metadata" not in conv_tables
...
def test_create_conversation_rows_land_in_correct_db(
    codify_db: Path, conv_db: Path, store: SqlAlchemyConversationStore
) -> None:
    ...
    # Codify DB: operational fields
    assert _count(codify_db, "codify_conversation_metadata") == 1
    assert _col(codify_db, "codify_conversation_metadata", "runner_id") == ["runner_abc"]
    assert _col(codify_db, "codify_conversation_metadata", "workspace") == ["/tmp/proj"]
...
def test_create_sub_agent_conversation(
    ...
    kind_code = _col(codify_db, "codify_conversation_metadata", "kind", f"id=X'{child.id}'")
    assert kind_code == [2]
...
def test_kind_derived_from_parent_nullness_not_metadata(
    ...
    before the Codify metadata row: deleting the metadata row must not flip a
    ...
    with sqlite3.connect(str(codify_db)) as conn:
        conn.execute("DELETE FROM codify_conversation_metadata WHERE id = ?", (child.id,))
...
def test_child_listing_does_not_prefetch_workspace_wide(
    ...
    """The parent-scoped child listing must not open an Codify-pool session to
    ...
    ``self._session`` (the Codify pool) for a kind/archived prefetch. It may
    ...
    parent-scoped page fetch opens the Codify pool at most once (page-metadata
    ...
    # One Codify-pool session for the page-metadata merge; the workspace-wide
    # prefetch (a second, unbounded one) must be gone.
    assert calls["codify_sessions"] <= 1
...
def test_set_runner_id_lands_in_codify_db(
    codify_db: Path, store: SqlAlchemyConversationStore
) -> None:
    conv = store.create_conversation(title="runner")
    store.set_runner_id(conv.id, "runner_xyz")
    runner_ids = _col(codify_db, "codify_conversation_metadata", "runner_id", f"id=X'{conv.id}'")
    assert runner_ids == ["runner_xyz"]


def test_set_session_state_lands_in_codify_db(
    codify_db: Path, store: SqlAlchemyConversationStore
) -> None:
...
def test_set_conversation_project_lands_in_codify_db(
    codify_db: Path, store: SqlAlchemyConversationStore
) -> None:
    """``project_id`` is written to the metadata row in the Codify DB."""
    project_id = "b" * 32
    conv = store.create_conversation(title="filed")
    filed = store.set_conversation_project(conv.id, project_id)
    assert filed is True

    stored = _col(codify_db, "codify_conversation_metadata", "project_id", f"id=X'{conv.id}'")
...
def test_list_conversations_project_name_filter_crosses_dbs(
    ...
    member ids from the Codify DB (``projects`` JOIN ``conversation_metadata``,
    ...
    # projects lives on the Codify DB, so create it against that URI.
    project_store = SqlAlchemyProjectStore(f"sqlite:///{codify_db}")
...
def test_append_and_list_items_land_in_conv_db(
    ...
    from codify.entities import NewConversationItem
    from codify.entities.conversation import MessageData
...
def test_delete_conversation_cleans_both_dbs(
    ...
    assert _count(codify_db, "codify_conversation_metadata") == 0
...
def test_delete_conversation_subtree_cleans_both_dbs(
    ...
    assert _count(codify_db, "codify_conversation_metadata") == 2
...
def test_get_runner_ids_reads_from_codify_db(store: SqlAlchemyConversationStore) -> None:
...
def test_agent_store_resolves_session_id_across_dbs(
    ...
    wired only to the Codify DB would query the wrong database and
    ...
    # Agent row lands in the Codify DB; the binding on the AP DB's
    # conversations.agent_id column.
    assert _count(codify_db, "agents") == 1
...
def test_update_conversation_archives_without_metadata_row(
    ...
    with sqlite3.connect(str(codify_db)) as conn:
        conn.execute(
            "DELETE FROM codify_conversation_metadata WHERE id IN (?, ?)",
...
def test_delete_conversation_deletes_session_scoped_agent(
    ...
    assert _count(codify_db, "agents") == 0
    assert _count(conv_db, "conversations") == 0
...
def test_delete_conversation_keeps_template_agent(
    ...
    assert _col(codify_db, "agents", "id") == ["191cbf904e3223e9e00ac9a1abfe79a5"]
