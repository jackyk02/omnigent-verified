...
from codify.server.auth import RESERVED_USER_LOCAL
...
def test_fork_drops_per_user_pin_labels(
    ...
    from codify.stores.conversation_store import pinned_label_key
...
def test_create_with_existing_caller_supplied_id_raises(db_uri: str) -> None:
    ...
    from codify.stores.conversation_store import ConversationAlreadyExistsError
...
def test_get_conversations_bulk(
    ...
    conversation_store.set_labels(a.id, {"codify.ui": "terminal"})
    ...
    assert result[a.id].labels == {"codify.ui": "terminal"}
...
def test_update_archived_bumps_updated_at(
    ...
    monkeypatch.setattr(
        "codify.stores.conversation_store.sqlalchemy_store.now_epoch",
...
def test_position_not_enforced_by_db(
    ...
    """
    from codify.db.db_models import SqlConversationItem
    from codify.db.enum_codecs import encode_item_status, encode_item_type
    from codify.db.utils import generate_item_id
...
def test_list_items_type_filter_returns_only_matching_type(
    ...
    """
    from codify.entities import CompactionData
...
def test_append_bumps_updated_at(
    ...
    to the current time.
    """
    import codify.stores.conversation_store.sqlalchemy_store as store_mod
...
def test_create_duplicate_title_under_same_parent_raises(
    ...
    from codify.stores.conversation_store import NameAlreadyExistsError
...
def test_list_conversations_filtered_by_agent_id_returns_matching_only(
    ...
    """
    Powers Codify mode ``--continue`` (resume the most-recent
...
def test_list_conversations_by_runner_id_hydrates_labels(
    ...
    conversation_store.set_labels(
        bound.id,
        {
            "codify.fork.carry_history": "1",
            "codify.fork.source_external_session_id": "src-claude-sid",
        },
    )
...
def test_set_host_id_missing_conversation_raises(
    ...
    """
    from codify.stores.conversation_store import ConversationNotFoundError
...
def test_create_session_with_agent_top_level_unchanged(
    ...
    contract (CLI ``codify run`` sessions): kind stays "default",
...
def test_fork_conversation_drops_instance_scoped_labels(
    ...
    conversation_store.set_labels(
        source.id,
        {
            "codify.claude_native.bridge_id": source.id,
            "codify.codex_native.bridge_id": source.id,
            "codify.last_context_tokens": "39903",
            "codify.last_context_window": "1000000",
            # The dangerous bypass opt-in must NOT ride into the fork.
            "codify.codex_native.bypass_sandbox": "1",
    ...
    # usage.
    assert fork.labels == {"codify.wrapper": "claude-code-native-ui"}, (
...
def test_fork_conversation_stamps_source_external_session_id(
    ...
    ``codify.fork.source_external_session_id`` to resume + branch the
    ...
    assert fork.labels.get("codify.fork.source_external_session_id") == "claude-uuid-abc", (
    ...
    # yet. Copying external_session_id would make two Codify sessions claim
...
def test_fork_conversation_no_external_session_id_no_directive(
    ...
    assert "codify.fork.source_external_session_id" not in fork.labels, (
...
def test_fork_conversation_truncated_drops_external_session_directive(
    ...
    If ``codify.fork.source_external_session_id`` were stamped, the
...
def test_fork_conversation_cross_family_drops_external_session_directive(
    ...
    must not stamp ``codify.fork.source_external_session_id``: the
    ...
    the copied Codify items) instead.
    """
    from codify.stores.conversation_store import (
        FORK_CARRY_HISTORY_LABEL_KEY,
        FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY,
...
def test_fork_conversation_up_to_last_response_keeps_external_directive(
    ...
    """
    from codify.stores.conversation_store import FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY
...
def test_instance_scoped_label_keys_match_harness_constants() -> None:
    ...
    from codify.codex_native_bridge import CODEX_NATIVE_BRIDGE_ID_LABEL_KEY
    from codify.stores.conversation_store import _INSTANCE_SCOPED_LABEL_KEYS
...
def test_fork_conversation_carry_history_into_native_stamps_label(
    ...
    The runner reads ``codify.fork.carry_history`` to decide whether a
...
def test_switch_conversation_agent_cross_family_resets_and_relabels(
    ...
    # codify.stopped marker was retired upstream.
    instance_label = "codify.last_context_tokens"
    ...
    assert "codify.codex_native.bypass_sandbox" not in updated.labels, (
...
def test_get_session_connectivity_batches_runner_and_host(
    ...
    fresh" — the retired ``codify.stopped`` marker is no longer a
...
def test_get_session_connectivity_reports_needs_workspace_for_fork(
    ...
    ``codify.fork.source_id`` label (set by ``fork_conversation``).
    ...
    conversation_store.set_labels(
        fork.id, {"codify.fork.source_id": "e9f8f58523cec9a57d3bdf93be543e8c"}
    )
...
def test_get_session_owner_excludes_public_sentinel(
    ...
    from codify.server.auth import RESERVED_USER_PUBLIC
    from codify.stores.permission_store.sqlalchemy_store import (
        SqlAlchemyPermissionStore,
    )
...
def _stored_next_position(
    ...
    from codify.db.db_models import SqlConversation
...
def test_list_conversations_filters_by_pinned_label(
    ...
    ``codify.pinned.<user>`` label, not a shared key (see ``pinned_label_key``).
...
def test_pinned_label_key_fits_column_for_long_user_ids() -> None:
    ...
    from codify.stores.conversation_store import PINNED_LABEL_KEY, pinned_label_key
...
def test_live_state_columns_round_trip_without_bumping_updated_at(
    ...
    """
    from codify.stores.conversation_store import runner_seen_is_fresh
...
def test_live_state_writes_via_chokepoint_land_in_scoped_workspace(
    ...
    from codify.db.db_models import workspace_scope
