...
def test_set_labels_clamps_overlong_value_to_column_width(
    ...
    conversation_store.set_labels(conv.id, {"codify.last_task_error_message": overlong})
    got = conversation_store.get_conversation(conv.id)
    assert got is not None
    stored = got.labels["codify.last_task_error_message"]
...
async def test_delete_conversation_cascades_to_labels(
    ...
    from codify.db.db_models import SqlConversationLabel
