...
from codify.runner.transports.ws_tunnel import serve as serve_module
...
from codify.runner.transports.ws_tunnel.serve import (
...
async def test_serve_tunnel_fails_loud_on_auth_redirect(
    ...
    assert "codify setup" in message
...
async def test_serve_tunnel_once_sends_bearer_header(
    ...
    monkeypatch.setattr("codify.cli_auth.load_databricks_org_id", lambda _server_url: None)
...
async def test_serve_tunnel_once_sends_org_header(
    ...
    monkeypatch.setattr(websockets, "connect", _fake_connect)
    monkeypatch.setattr(
        "codify.cli_auth.load_databricks_org_id", lambda _server_url: "2850744067564480"
    )
...
async def _capture_connect_kwargs(
    ...
    monkeypatch.setattr("codify.cli_auth.databricks_request_headers", lambda *_a, **_k: {})
    await _serve_tunnel_once(
