...
from codify.runner.transports.tcp import (
    RunnerTCPSubprocess,
    create_tcp_client,
)

# ``_entry:create_app`` requires RUNNER_SERVER_URL in the subprocess
# environment. Tests don't need a real Codify server — the env var only
...
def test_runner_tcp_subprocess_with_explicit_port() -> None:
    ...
    from codify.runner.transports.tcp import _pick_free_port
...
def test_tcp_subprocess_with_bad_app_factory_raises() -> None:
    ...
    with pytest.raises(RuntimeError, match="exited prematurely"):
        with RunnerTCPSubprocess(app_factory_path="codify.does.not.exist:app"):
            pass
