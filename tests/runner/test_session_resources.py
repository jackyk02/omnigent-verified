...
from codify.entities import DEFAULT_ENVIRONMENT_ID
...
from codify.inner.os_env import EditEntry, OpResult, OSEnvironment
from codify.inner.terminal import TerminalInstance
from codify.runner import create_runner_app
from codify.runner import resource_registry as resource_registry_mod
...
from codify.terminals import TerminalListEntry, TerminalRegistry
...
async def test_get_terminal_by_id_returns_404_when_tmux_exited(
    ...
    This is the signal ``codify claude`` uses after its attach
...
async def test_transfer_terminal_moves_resource_without_closing(
    ...
    the old conversation, making the new Codify conversation unable to
...
async def test_create_terminal_uses_runner_workspace_as_default_cwd(
    ...
    This is the resource endpoint used by ``codify claude``. The
...
async def test_create_terminal_threads_agent_parent_os_env_through(
    ...
    ``codify claude`` wrapper) could spawn an unsandboxed
    terminal in a session whose YAML declared an egress allow-list.
    """
    from codify.inner.datamodel import AgentDef, OSEnvSandboxSpec, OSEnvSpec
...
async def test_concurrent_resource_reads_share_one_session_snapshot(
    ...
    resolution, instead of each request stampeding the Codify server.
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server that blocks + counts the session snapshot fetch.
    ...
    async def _spec_resolver(resolved_agent_id: str, session_id: str | None = None) -> AgentSpec:
        ...
        return AgentSpec(
            spec_version=1,
            name="burst-agent",
            executor=ExecutorSpec(type="codify", config={"harness": "runner-test-default"}),
        )
...
async def test_failed_session_snapshot_is_not_cached_and_retries(
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: fail the first snapshot, succeed afterward.
    ...
    # CodifyError → the endpoint returned a non-200 error response.
...
async def test_unbound_agent_snapshot_is_not_cached_and_retries(
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: 200 with no agent_id first, then the bound one.
...
async def test_claude_terminal_ensure_concurrent_calls_create_once(
    ...
    * ``create_count == 1`` proves the second request took the early-return
    ...
    monkeypatch.setattr(
        "codify.runner.native.orchestration._auto_create_claude_terminal", fake_auto_create
    )
