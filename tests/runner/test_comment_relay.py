...
from codify.claude_native_bridge import bridge_dir_for_bridge_id, prepare_bridge_dir
from codify.entities.session_resources import SessionResourceView, terminal_resource_view
from codify.inner.datamodel import TerminalEnvSpec
from codify.runner import create_runner_app
from codify.terminals import TerminalListEntry
from tests.runner.helpers import NullServerClient, make_test_terminal_instance

# Matches ``_TOOL_RELAY_FILE`` in ``codify.claude_native_bridge``.
...
def _skip_tools_changed_notification(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.claude_native_bridge.post_tools_changed", _noop)
...
async def relay_env(tmp_path: Path, client: httpx.AsyncClient) -> AsyncIterator[_RelayEnv]:
    ...
    does not create it, so this mirrors what ``codify claude`` does on
...
async def _launch_terminal(
    ...
    *,
    ...
    """
    POST the claude terminal-launch request used by ``codify claude``.
...
@pytest.mark.asyncio
async def test_relay_executor_routes_through_codify_in_codify_mode(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Route relay tool execution through Codify ``/mcp`` for policy enforcement.
    ...
    :class:`~codify.runner.proxy_mcp_manager.ProxyMcpManager` instead of
    ...
    """
    import codify.claude_native_bridge as _bridge_mod

    # Records every POST sent to the fake Codify server.
    ap_mcp_posts: list[dict[str, Any]] = []

    class _FakeApClient:
        """Fake Codify server client that captures /mcp calls and returns a fixed result.

        Appends each POST request body to the outer ``ap_mcp_posts`` list via
        closure so the test can assert on what was sent to the Codify server.
        """
    ...
    try:
        ...
        # In Codify mode the executor must have POSTed a tools/call JSON-RPC to the
        # Codify server's /mcp endpoint, not called the direct comment handler.
        mcp_call = next(
            (
                r
        ...
        assert mcp_call is not None, (
            "No tools/call request reached the Codify /mcp endpoint. "
        ...
        assert mcp_call["json"]["params"]["arguments"] == {"status": "pending"}, (
            "Arguments were not forwarded correctly to Codify /mcp."
        )
        ...
        # The Codify response's text content must be parsed back to a dict.
        assert result == {"items": []}, f"Expected parsed Codify response dict, got {result!r}."
    finally:
        shutil.rmtree(bridge_dir, ignore_errors=True)
...
async def test_relay_policy_evaluate_proxies_to_server_client(
    ...
    from codify.claude_native_bridge import prepare_bridge_dir as _prep
    from codify.claude_native_bridge import start_tool_relay
