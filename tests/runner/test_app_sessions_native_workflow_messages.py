...
from codify.runtime.prompt import SHARED_SESSION_AUTHORSHIP_INSTRUCTION
...
async def test_messages_reach_harness_in_submission_order() -> None:
    ...
    ``post_session_events`` (codify/runner/app.py) the turn-vs-buffer
...
async def test_cold_loaded_shared_history_respects_attribution_flag(
    ...
    env_name = "CODIFY_SHARED_MESSAGE_ATTRIBUTION_ENABLED"
...
async def test_session_creation_does_not_replay_trailing_user_for_codex_native(
    ...
    producing an assistant item, Codify history can end with the user prompt even
    though Codex already consumed it. Generic crash recovery would treat that
    as an unanswered Codify turn and resend the same prompt when ``codify
    codex`` reattaches.
...
async def test_catch_up_scan_skips_codex_native_history_entries(
    ...
    assert server_client.get_calls == [], (
        "Catch-up scan must skip Codex-native sessions before fetching Codify "
