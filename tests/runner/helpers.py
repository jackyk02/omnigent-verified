...
class NullServerClient:
    """Minimal fake Codify server client for tests that do not exercise
    Codify interactions.

    Returns empty/204 responses to all requests so the runner's Codify calls
    (session fetch, label patch, history load, etc.) succeed silently.
    Used wherever ``create_runner_app`` is called in tests that only
    exercise runner-local behavior with no real Codify server.
    """
