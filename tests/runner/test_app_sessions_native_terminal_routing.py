...
from codify.entities.session_resources import SessionResourceView, terminal_resource_id
...
async def test_create_session_terminal_ensure_routes_claude_native(
    ...
    monkeypatch.setattr(
        "codify.runner.native.orchestration._auto_create_claude_terminal", _stub_auto_create
    )
...
async def test_create_session_terminal_ensure_failure_returns_json_without_live_error(
    ...
    ``ensure_native_terminal`` is called by the Codify server while handling a
    user message. Codify owns that failed transcript turn: it persists the
    ...
    monkeypatch.setattr(
        ...
        _failing_auto_create,
    )
    monkeypatch.setattr(
        "codify.runner.app._publish_native_terminal_start_error",
...
async def test_create_session_terminal_ensure_routes_codex_native(
    ...
    monkeypatch.setattr(
        "codify.runner.native.orchestration._auto_create_codex_terminal", _stub_auto_create
    )
    monkeypatch.setattr(SessionResourceRegistry, "get_terminal_resource", _stub_get_terminal)
    monkeypatch.setattr(
        SessionResourceRegistry,
...
async def test_late_status_for_deleted_sub_agent_child_is_not_a_spurious_503() -> None:
    ...
    ``503 subagent_delivery_not_confirmed`` (which Codify then retries) plus an
...
class _RecordedPatch:
    ...
    :param json: JSON body, e.g. ``{"labels": {"codify.ui": "terminal"}}``.
    """
    ...
    class _FakeResourceRegistry:
        ...
        async def launch_auxiliary_terminal(
            ...
            *,
            ...
            assert resource_role == CODIFY_REPL_TERMINAL_ROLE
...
async def test_create_session_repl_terminal_dispatch(
    ...
    monkeypatch.setattr("codify.runner.app._auto_create_repl_terminal", _fake_auto_create_repl)
    ...
    monkeypatch.setattr(
        "codify.runner.app._codex_session_needs_runner_terminal", _fake_codex_needs
    )
