...
from codify.runner.routing import RunnerRouter, runner_dispatch_harness
...
from codify.spec import AgentSpec, ExecutorSpec, LLMConfig
...
def _assert_codify_error(
    excinfo: pytest.ExceptionInfo[CodifyError],
    *,
    code: str,
) -> None:
    """
    Assert a structured Codify error code.
...
async def test_runner_router_requires_existing_runner_binding() -> None:
    ...
    try:
        ...
        _assert_codify_error(excinfo, code=ErrorCode.CONFLICT)
...
async def test_runner_router_requires_pinned_runner_to_be_online() -> None:
    ...
    try:
        ...
        _assert_codify_error(excinfo, code=ErrorCode.RUNNER_UNAVAILABLE)
    finally:
        await router.aclose()
...
async def test_runner_router_fails_when_no_runner_supports_harness() -> None:
    ...
    try:
        ...
        _assert_codify_error(excinfo, code=ErrorCode.RUNNER_CAPABILITY_MISMATCH)
    finally:
        await router.aclose()
