...
from codify.codex_native_app_server import NativeCodexLaunch
from codify.harness_plugins import BackgroundTitleGeneratorSpec
from codify.runner import create_runner_app
from codify.runner.background_titles import BackgroundTitleContext
from codify.runner.background_titles import claude_native as claude_native_titles
from codify.runner.background_titles import codex_native as codex_native_titles
...
async def test_background_title_uses_isolated_codex_process(
    ...
    monkeypatch.setattr(
        "codify.runner.app._resolve_harness_config",
...
async def test_background_title_resolves_synthetic_claude_policy_gate(
    ...
    monkeypatch.setattr(
        "codify.runner.background_titles.sdk.BACKGROUND_TITLE_INFERENCE_TIMEOUT_SECONDS",
        0.05,
    )
    app = create_runner_app(
        process_manager=process_manager,  # type: ignore[arg-type]
...
async def test_claude_native_title_uses_tool_free_print_mode(
    ...
    monkeypatch.setattr(
        "codify.claude_native.resolve_native_claude_config",
        lambda spec=None: None,
    )
    monkeypatch.setattr(
        "codify.claude_launcher.resolve_claude_launch",
...
async def test_codex_native_title_uses_ephemeral_tool_free_exec(
    ...
    monkeypatch.setattr(asyncio, "create_subprocess_exec", create_subprocess_exec)
    monkeypatch.setattr(
        "codify.codex_native_app_server.resolve_native_codex_launch",
    ...
    assert "codify-codex-title-" in captured["kwargs"]["cwd"]
    codex_home = Path(captured["kwargs"]["env"]["CODEX_HOME"])
...
async def test_background_title_dispatches_any_registered_harness(
    ...
    monkeypatch.setattr(
        "codify.runner.background_titles.service.background_title_generators",
        lambda: {
            "community-example": BackgroundTitleGeneratorSpec(
                "codify.community.harness.example.background_titles:generate"
            )
        },
    )
    monkeypatch.setattr(
        "codify.runner.background_titles.service.load_object",
