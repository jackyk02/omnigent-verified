...
async def test_external_session_status_idle_delivers_forwarded_native_output_to_parent_inbox() -> (
    ...
    Native harness transcript items are persisted by Codify server, so the
...
async def test_external_status_idle_without_output_omits_stale_history_preview() -> None:
    ...
    If Codify has no authoritative native transcript text to forward, the parent
...
async def test_events_interrupt_on_native_session_injects_escape_without_marker(
    ...
    # Native spec: executor.type="codify" + config.harness="claude-native"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # shows work as soon as Codify accepts the turn, but must not use the
...
async def test_message_turn_lifecycle_status_suppressed_for_terminal_backed_harnesses(
    ...
    useful immediate signal that Codify accepted the turn, but its ``idle`` edge
    ...
    spec = AgentSpec(
        spec_version=1,
        name="t",
        # executor.type="codify" + config.harness=<harness> is the
