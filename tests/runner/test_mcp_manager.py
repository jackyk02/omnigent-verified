...
from codify.runner import mcp_manager as _mcp_manager_module
from codify.runner.mcp_manager import (
    _POOL_SPEC_CAPACITY,
    McpSchemasResult,
    RunnerMcpManager,
    compute_server_hash,
...
from codify.spec.types import AgentSpec, MCPServerConfig
...
def patch_connection(
    ...
    monkeypatch.setattr(
        "codify.runner.mcp_manager.McpServerConnection",
        _PatchedConn,
    )
...
async def test_invalid_tool_name_is_filtered(
    ...
    """
    # Force the `codify` package logger to propagate so caplog
    # captures warnings. Defensive: ``codify.cli_diagnostics
    # .setup_cli_logging`` sets ``codify.propagate = False`` and
    ...
    # every ``codify.*`` warning.
    logging.getLogger("codify").propagate = True
    patch_connection["__tools_for__"]["jira"] = [
    ...
    # xdist worker may have set ``codify.propagate = False`` (the CLI
    ...
    mcp_logger = logging.getLogger("codify.runner.mcp_manager")
    ...
    try:
        ...
        # caplog's handler lives. Passing ``logger="codify.runner.
...
def test_strip_mcp_tool_prefix_preserves_bare_double_underscore() -> None:
    ...
    through.
    """
    from codify.runtime.workflow import _strip_mcp_tool_prefix
