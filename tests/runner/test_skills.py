...
Skills are resolved on the runner, not the Codify server, because the
...
from codify.runner.app import ResolvedSpec
...
class _ServerClient:
    """
    Fake Codify server client whose session snapshot carries an agent_id and
...
async def test_resolve_session_skill_unknown_returns_404_with_available(tmp_path: Path) -> None:
    ...
    sorted list of available skill names (the error the Codify server
...
async def test_session_skills_cache_ttl_expiry_rediscovers(
    ...
    (home / ".claude" / "skills").mkdir(parents=True)
    monkeypatch.setattr("pathlib.Path.home", lambda: home)
    monkeypatch.setattr("codify.runner.app._SESSION_SKILLS_CACHE_TTL_SECONDS", 0.0)
