...
def _patch_attach_spawn(
    ...
    monkeypatch.setattr("codify.terminals.ws_bridge._fork_exec_pty", fake_fork_exec)
...
def test_runner_resource_attach_selects_control_bridge_on_transport_query(
    ...
    monkeypatch.setattr("codify.runner.app.bridge_tmux_control_to_websocket", fake_control)
    monkeypatch.setattr("codify.runner.app.bridge_tmux_pty_to_websocket", fake_pty)
    # Point config resolution at an empty scratch dir so the no-query case is
    # deterministic regardless of the developer's real ~/.codify/config.yaml.
...
def test_runner_resource_attach_recreates_dead_repl_terminal(
    ...
    ``codify attach`` process exits (user Ctrl+C, crash at deferred
    ...
    async def fake_auto_create(
        ...
        *,
        ...
        :param server_client: Codify server client (unused).
    ...
    monkeypatch.setattr("codify.runner.app._auto_create_repl_terminal", fake_auto_create)
...
def test_runner_resource_attach_recreates_dead_qwen_terminal(
    ...
    monkeypatch.setattr("codify.runner.app._auto_create_qwen_terminal", fake_auto_create)
...
def test_runner_resource_attach_dead_non_repl_terminal_keeps_4404(
    ...
    ``codify-repl`` role stamped, the dead-pane attach must close
    ...
    async def must_not_recreate(*args: object, **kwargs: object) -> None:
        ...
        raise AssertionError(
            ...
            "recreate path must be gated on CODIFY_REPL_TERMINAL_ROLE."
        )

    monkeypatch.setattr("codify.runner.app._auto_create_repl_terminal", must_not_recreate)
...
def test_runner_resource_attach_closes_4404_when_pty_ends(
    ...
    while the browser (or ``codify claude --server``) is attached.
    Without the dedicated close code, the client's reconnect loop in
    ``codify/claude_native.py`` interprets the close as a transient
    ...
    monkeypatch.setattr("codify.terminals.ws_bridge.pty.fork", fake_fork)
    monkeypatch.setattr("codify.terminals.ws_bridge.os.kill", lambda *_args, **_kw: None)
