...
from codify import native_dispatch
...
from codify.runner import tool_dispatch as _tool_dispatch
...
from codify.runner.native import NativeLaunchContext, _resolve_native_spawn_env
from codify.runner.resource_registry import (
    SessionResourceRegistry,
)
from codify.spec.types import AgentSpec, ExecutorSpec, LocalToolInfo
...
async def test_session_labels_for_runner_spawn_timeout_is_quiet(
    ...
    cannot be fetched. A slow Codify session lookup therefore must not emit a
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://ap") as client:
        with caplog.at_level(logging.DEBUG, logger="codify.runner.app"):
...
async def test_resolve_native_spawn_env_bare_builder_takes_session_id_only() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codify.pi_native_bridge.build_pi_native_spawn_env", _fake_build)
...
async def test_resolve_native_spawn_env_label_builder_reads_bridge_id() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codify.codex_native_bridge.build_codex_native_spawn_env", _fake_build)
        async with httpx.AsyncClient(transport=transport, base_url="http://ap") as client:
            env = await _resolve_native_spawn_env(
                "codex-native",
                "conv_codex",
...
async def test_resolve_native_spawn_env_claude_uses_bridge_id_helper() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codify.claude_native_bridge.build_claude_native_spawn_env", _fake_build)
        mp.setattr(
            "codify.runner.native.orchestration._claude_native_bridge_id_with_optional_labels",
            _fake_bridge_id,
        )
        async with httpx.AsyncClient(base_url="http://ap") as client:
            env = await _resolve_native_spawn_env(
...
async def test_resolve_native_spawn_env_hermes_writes_policy_hook_before_build() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codify.hermes_native_bridge.write_policy_hook_config", _fake_write)
        mp.setattr("codify.hermes_native_bridge.build_hermes_native_spawn_env", _fake_build)
        async with httpx.AsyncClient(base_url="http://ap") as client:
            env = await _resolve_native_spawn_env(
                "hermes-native",
                "conv_hermes",
...
async def test_launch_adapters_forward_expected_kwarg_subset(
    ...
    from codify.runner.native import orchestration as orch
...
async def test_launch_native_terminal_creates_when_absent(
    ...
    from codify.runner.native import _launch_native_terminal
    ...
    monkeypatch.setattr("codify.runner.native._launch_pi", _fake_launch_pi)
    locks: dict[str, Any] = {}
...
async def test_launch_native_terminal_skips_when_terminal_exists(
    ...
    monkeypatch.setattr("codify.runner.native._launch_pi", _must_not_call)
    result = await _launch_native_terminal(
        "pi-native",
...
async def test_launch_native_terminal_force_recreate_tears_down_existing(
    ...
    from codify.runner.native import PreLaunchResult, _launch_native_terminal
...
async def test_launch_native_terminal_publishes_start_error_on_failure(
    ...
    monkeypatch.setattr("codify.runner.native._launch_pi", _boom)
...
async def test_ensure_native_terminal_returns_existing_without_creating(
    ...
    from codify.runner.native import _ensure_native_terminal
    ...
    monkeypatch.setattr("codify.runner.native._launch_goose", _fake_launch)
...
async def test_ensure_native_terminal_builder_error_returns_500(
    ...
    monkeypatch.setattr("codify.runner.native._launch_goose", _boom)
...
async def test_ensure_native_terminal_non_owned_closes_and_recreates(
    ...
    monkeypatch.setattr("codify.runner.native._launch_codex", _fake_launch)
...
async def test_runner_session_tool_schemas_use_resolved_bundle_workdir(tmp_path: Path) -> None:
    ...
    (tool_dir / "bundle_tool.py").write_text(
        "from codify_client.tools import tool\n\n"
        "@tool\n"
...
async def test_action_required_marker_round_trips_to_relayed_frame() -> None:
    """The runner stamps ``codify_runner_dispatched`` on action_required frames.

    The Codify executor's ``_runner_dispatches`` predicate reads this marker
    ...
    # Runner dispatched the MCP tool through the Codify server proxy (AP mode).
...
async def test_create_session_preserves_existing_event_queue() -> None:
    ...
    The Codify relay's ``GET /stream`` lazily creates the per-session event
...
async def test_session_stream_emits_heartbeat_on_idle() -> None:
    ...
    import codify.runner.app as _runner_app_module
    ...
    try:
        ...
        assert collected[0] == {"type": "session.heartbeat"}, (
            "The first stream frame must be the ready heartbeat. Codify waits "
...
async def test_native_session_create_seeds_harness_compaction_anchor(
    ...
    """
    import codify.runner.app as runner_app_mod
