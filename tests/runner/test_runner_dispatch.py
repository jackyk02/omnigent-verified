...
``codify/runtime/harnesses/`` machinery — NOT a parallel impl),
...
from codify.runner import create_runner_app
from codify.runner.app import (
    _build_spawn_env_from_spec,
    _forward_harness_response,
    _resolve_harness_config,
)
from codify.runtime.harnesses import _HARNESS_MODULES
from codify.runtime.harnesses.process_manager import HarnessProcessManager
from codify.session_lifecycle import CLOSED_LABEL_KEY, CLOSED_LABEL_VALUE
from codify.spec.types import AgentSpec, ExecutorSpec, SharePolicy
...
def _assume_harness_clis_installed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "codify.onboarding.harness_install.missing_harness_cli",
        lambda harness: None,
    )
...
async def _drain_published_statuses(
    ...
    *,
    ...
    (``codify.runner.app._session_event_queues_ref``) — the same queue
    ...
    """
    from codify.runner.app import _session_event_queues_ref
...
async def test_runner_resolves_agent_from_server_snapshot_when_msg_lacks_agent_id() -> None:
    ...
    def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: the session snapshot carries the agent_id.
    ...
    async def _snapshot_spec_resolver(agent_id: str, session_id: str | None = None) -> AgentSpec:
        ...
        return AgentSpec(
            spec_version=1,
            name="ondemand-agent",
            executor=ExecutorSpec(type="codify", config={"harness": resolved_harness}),
        )
    ...
    try:
        async with _runner_test_client(app) as http:
            response = await http.post(
                ...
                # path the Codify server uses to forward session messages.
...
class _ContentCapturingProcessManager:
    ...
    async def get_client(
        ...
        *,
        ...
        :param conversation_id: Codify conversation id (unused).
...
async def test_runner_reloads_full_history_on_cold_cache_after_restart() -> None:
    ...
    """
    from codify.runner import app as runner_app
    ...
    def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: snapshot + full persisted history on ``/items``.
    ...
    try:
        async with _runner_test_client(app) as http:
            response = await http.post(
                ...
                # the Codify server uses to forward session messages.
                f"/v1/sessions/{conv}/events",
                json={
                    "type": "message",
                    "role": "user",
                    "model": "x",
                    # The store id the Codify server persisted for this turn
...
async def test_runner_cold_cache_appends_message_when_store_lacks_it() -> None:
    ...
    def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: history reload that does NOT include the new message.
...
async def test_runner_cold_cache_keeps_trailing_user_when_no_persisted_id() -> None:
    ...
    def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: history reload ending on a real prior user message.
...
async def test_runner_cold_cache_uses_resolved_message_not_stored_file_id() -> None:
    ...
    def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub Codify server: file resolution, snapshot, and a history reload
...
async def test_runner_post_returns_503_when_spec_resolver_fails(
    ...
    async with _runner_test_client(app) as http:
        with caplog.at_level(logging.WARNING, logger="codify.runner.app"):
            response = await http.post(
...
async def test_runner_background_turn_emits_failed_when_spawn_env_build_raises(
    ...
    ``CodifyError``), the setup phase of ``_run_turn_bg`` failed before
    ...
    (no ``?stream=true`` — the production path the Codify server uses) and
    ...
    def _raising_build(
        ...
        :returns: Never returns.
        :raises CodifyError: Always — mirrors the no-model provider error.
        """
        del spec, workdir
        raise CodifyError(
    ...
    monkeypatch.setattr(
        "codify.runtime.workflow._build_claude_sdk_spawn_env",
        _raising_build,
    )
    ...
    async with _runner_test_client(app) as http:
        response = await http.post(
            ...
            # Codify server uses to forward session messages).
            f"/v1/sessions/{conv}/events",
            json={
                "type": "message",
                "role": "user",
    ...
    # state and cleared. Without the fix, the setup-phase CodifyError is
...
async def test_runner_failed_status_carries_setup_error_message(
    ...
    ``{code, message}`` error so Codify and the REPL can render it.
    ...
    def _raising_build(
        ...
        :param workdir: Bundle workdir (unused).
        :returns: Never returns.
        :raises CodifyError: Always.
        """
        del spec, workdir
        raise CodifyError(raised_message, code=ErrorCode.INVALID_INPUT)
    ...
    # The raised CodifyError message is wrapped as "turn setup
...
async def test_runner_publishes_terminal_failed_when_harness_stream_fails(
    ...
    # out of the real ``~/.codify/codex-native`` tree. The module documents
    # this monkeypatch as the supported test isolation point.
    monkeypatch.setattr("codify.codex_native_bridge._BRIDGE_ROOT", tmp_path)
    ...
    async def _spec_resolver(agent_id: str, session_id: str | None = None) -> AgentSpec:
        ...
        return AgentSpec(
            ...
            executor=ExecutorSpec(type="codify", config={"harness": harness}),
        )
...
async def test_runner_os_env_tools_use_agent_spec_cwd() -> None:
    ...
    from codify.runner.tool_dispatch import _execute_os_env_tool
...
async def test_runner_os_env_placeholder_cwd_uses_cli_workspace(
    ...
    workspace.mkdir()
    monkeypatch.setenv("CODIFY_RUNNER_OS_ENV_ROOT", str(tmp_path / "fallback"))
    spec = AgentSpec(
        spec_version=1,
        os_env=OSEnvSpec(
            type="caller_process",
...
async def test_runner_os_env_tools_default_to_conversation_workspace(monkeypatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        monkeypatch.setenv("CODIFY_RUNNER_OS_ENV_ROOT", td)
        spec = AgentSpec(
            spec_version=1,
            os_env=OSEnvSpec(
                type="caller_process",
...
def test_clone_os_env_spec_preserves_all_sandbox_fields() -> None:
    ...
    :func:`codify.inner.terminal._clone_sandbox_spec`: hand-enumerated
    ...
    from codify.inner.datamodel import OSEnvSandboxSpec, OSEnvSpec
    from codify.runner.tool_dispatch import _clone_os_env_spec
...
def test_effective_runner_os_env_defaults_when_spec_has_no_os_env(monkeypatch) -> None:
    ...
    :returns: None.
    """
    from codify.runner.tool_dispatch import _effective_runner_os_env_spec
    from codify.spec.types import AgentSpec
...
def test_effective_runner_os_env_runner_workspace_overrides_absolute_spec_cwd(
    ...
    with ``CODIFY_RUNNER_WORKSPACE`` set, it always wins —
...
async def test_runner_terminal_dispatch_passes_cli_workspace(
    ...
    :returns: None.
    """
    from codify.inner.datamodel import TerminalEnvSpec
    from codify.runner.tool_dispatch import _execute_terminal_tool
    from codify.terminals import TerminalRegistry
    from codify.tools.base import ToolContext
    from codify.tools.builtins.sys_terminal import SysTerminalLaunchTool
...
class _StubTerminalInstance:
    ...
    def __init__(self, running: bool = True) -> None:
        ...
        self.os_env = None
        self.socket_path = Path("/tmp/codify-test-tmux.sock")
...
async def test_terminal_close_dispatch_emits_resource_deleted(
    ...
    from codify.runner.tool_dispatch import _execute_terminal_tool
    from codify.tools.base import ToolContext
    from codify.tools.builtins.sys_terminal import SysTerminalCloseTool
...
async def test_execute_tool_rejects_non_object_arguments(
    ...
    """
    from codify.runner import tool_dispatch
    from codify.runner.tool_dispatch import execute_tool
...
def test_subagent_read_tools_are_runner_local() -> None:
    ...
    """
    from codify.runner.tool_dispatch import should_dispatch_locally
...
async def test_sys_session_send_reuses_existing_child_session(
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "GET" and request.url.path == "/v1/sessions/conv_parent":
            return httpx.Response(
                200,
                json={"labels": {"codify.turn_actor": "bob@example.com"}},
            )
        if (
            request.method == "GET"
...
def _spec_with_subagent_harness(harness: str) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        sub_agents=[
            SimpleNamespace(
                name="worker",
                executor=SimpleNamespace(type="codify", config={"harness": harness}),
            )
        ]
    )
...
async def test_sys_session_send_blocks_fresh_dispatch_when_harness_cli_missing(
    ...
    :param monkeypatch: Pytest monkeypatch fixture.
    """
    from codify.onboarding.harness_install import HarnessInstallSpec
    from codify.runner import app as runner_app
    from codify.runner.tool_dispatch import execute_tool
...
def _isolate_model_providers(
    ...
    monkeypatch.setattr("codify.onboarding.detected.detect_providers", list)
...
async def test_sys_session_send_by_id_rejects_closed_child(
    ...
    Without checking ``codify.closed=true`` in by-id mode, an
...
async def test_scaffold_subagent_defers_terminal_delivery_while_continuation_buffered() -> None:
    ...
    async def _spec_resolver(agent_id: str, session_id: str | None = None) -> AgentSpec:
        ...
        return AgentSpec(
            ...
            executor=ExecutorSpec(type="codify", config={"harness": _TEST_HARNESS_NAME}),
        )
...
async def test_sys_read_inbox_applies_subagent_tool_result_policy(
    ...
    output must still pass through Codify policy evaluation before the LLM
    sees it in the inbox drain.

    :param status: Terminal sub-agent status being drained.
    :param policy_response: Fake Codify policy verdict body.
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """Capture the Codify policy evaluation request."""
        if (
            request.method == "POST"
...
async def test_sys_read_inbox_requeues_subagent_output_on_transient_policy_failure() -> None:
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        ...
        :param request: Codify policy-evaluation request.
...
def test_session_status_to_task_status_maps_known_values() -> None:
    ...
    """
    from codify.runner.app import _session_status_to_task_status
...
def test_truncate_child_preview_caps_with_ellipsis() -> None:
    ...
    """
    from codify.runner.app import _CHILD_PREVIEW_MAX_CHARS, _truncate_child_preview
...
# Codify tool calls surface as action_required and route through
...
# handler calls the Codify server's existing REST endpoints, so tests use a
...
def _session_query_client(
    ...
    :returns: An ``httpx.AsyncClient`` pointed at a fake Codify server.
    """
...
async def test_session_list_maps_children_and_skips_closed() -> None:
    ...
    """
    from codify.runner.tool_dispatch import _execute_session_query_tool
...
async def test_session_close_patches_tombstoned_title() -> None:
    ...
    fresh child. The ``codify.closed=true`` label is the
...
def test_native_relay_builtin_set_matches_toolmanager_gating(
    ...
    """
    from codify.runner.tool_dispatch import _NATIVE_RELAY_BUILTIN_TOOLS
    from codify.spec.types import ToolsConfig
...
def test_native_relay_advertises_terminal_tools_per_spec_gate(
    ...
    from codify.inner.datamodel import TerminalEnvSpec
    from codify.runner.tool_dispatch import _NATIVE_RELAY_BUILTIN_TOOLS
    from codify.runtime import _globals as rt_globals
    from codify.terminals.registry import TerminalRegistry
    from codify.tools.manager import ToolManager
...
async def test_sys_agent_list_merges_three_sources(tmp_path: Path) -> None:
    ...
    """
    from codify.runner.tool_dispatch import _AGENT_CONFIG_SUBDIR, execute_tool
...
async def test_sys_session_get_info_maps_error_statuses(
    ...
    :param status_code: HTTP status the mocked Codify server returns.
...
async def test_sys_session_share_surfaces_server_message_on_4xx() -> None:
    ...
    # Mirrors the CodifyError envelope the server's exception handler
    # emits (codify/server/app.py) for the public + level>read guard.
...
def test_format_async_task_item_empty_subagent_completion_reads_as_no_output(
    ...
    """
    from codify.runner.tool_dispatch import _format_async_task_item
...
async def test_sys_session_send_rejects_both_session_id_and_named_target() -> None:
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        ...
        :param request: Any Codify request — none should occur on the reject path.
...
async def test_create_session_reinit_preserves_existing_inbox() -> None:
    ...
    The Codify server re-POSTs ``/v1/sessions`` for every bound conversation
...
async def test_approval_event_flattened_for_harness_scaffold() -> None:
    ...
    """
    from codify.runtime.harnesses._scaffold import ApprovalEvent
