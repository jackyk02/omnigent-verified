...
async def test_create_session_threads_workspace_to_pi_cwd(
    ...
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path / "config-home"))
...
async def test_codex_top_level_session_needs_runner_terminal_for_all_session_shapes(
    ...
    host-id gate returns ``False`` here, ``codify codex`` falls back to
    a CLI-owned app-server.
    """
    from codify.runner.app import _codex_session_needs_runner_terminal
...
async def test_auto_create_codex_terminal_uses_persisted_resume_launch_config(
    ...
    ``terminal_launch_args`` and ``external_session_id`` from the Codify snapshot,
    ...
    :returns: None.
    """
    import codify.codex_native_app_server as codex_app_mod
    from codify.runner import app as runner_app_mod
    ...
    monkeypatch.setattr("codify.runner._entry._make_auth_token_factory", lambda: None)
...
async def test_auto_create_codex_terminal_fork_clones_rollout_and_resumes(
    ...
    on the Codify session, and launch ``codex resume <minted_id>`` (not the
    ...
    """
    import codify.codex_native_app_server as codex_app_mod
    from codify import codex_native
    ...
    monkeypatch.setattr(codex_native_bridge, "_BRIDGE_ROOT", tmp_path / "codex-bridge")
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", str(workspace))
    monkeypatch.setenv("RUNNER_SERVER_URL", "http://ap.example")
    monkeypatch.delenv("DATABRICKS_CONFIG_PROFILE", raising=False)
    monkeypatch.setattr("codify.runner._entry._make_auth_token_factory", lambda: None)
...
async def test_auto_create_codex_terminal_fork_builds_rollout_from_items_and_resumes(
    ...
    Codex source whose rollout lives outside Codify's private ``CODEX_HOME``.
    ...
    class _ItemsForkSnapshotClient:
        ...
        plus the copied Codify items the rollout is built from."""
    ...
    # copied Codify items, not a source rollout, seeded the history.
    clone_home = codex_home_for_bridge_dir(bridge_dir_for_bridge_id(session_id))
    ...
    assert codeword in body, (
        ...
        "copied Codify items; missing it means history was not seeded."
    )
...
async def test_auto_create_codex_terminal_uses_worktree_workspace_not_bundle_dir(
    ...
    #   runner_env  — CODIFY_RUNNER_WORKSPACE (claude-native's source)
    ...
    monkeypatch.setattr(codex_native_bridge, "_BRIDGE_ROOT", tmp_path / "codex-bridge")
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", str(runner_env))
    monkeypatch.setenv("RUNNER_SERVER_URL", "http://ap.example")
    monkeypatch.delenv("DATABRICKS_CONFIG_PROFILE", raising=False)
    monkeypatch.setattr("codify.runner._entry._make_auth_token_factory", lambda: None)
    bridge_dir = codex_native_bridge.bridge_dir_for_bridge_id(session_id)
...
async def test_claude_native_first_turn_not_blocked_by_cold_bridge_notify(
    ...
    monkeypatch.setattr(
        "codify.claude_native_bridge.post_tools_changed",
...
async def _run_antigravity_auto_create(
    ...
    *,
    ...
    :param snapshot: The Codify session snapshot the helper should read.
    ...
    """
    import codify.antigravity_native_launch as launch_mod
    import codify.antigravity_native_reader as reader_mod
    import codify.antigravity_native_rpc as rpc_mod
    from codify import antigravity_native_bridge as bridge_mod
    from codify.runner import app as runner_app_mod
    ...
    # real Codify client around this stub — fine, since nothing posts here.
...
async def test_auto_create_antigravity_wires_reader_task_and_interaction_bridge(
    ...
    * the reader is wired with an ``on_pending_interaction`` that, when a WAITING
    ...
    import codify.antigravity_native_rpc as rpc_mod
    from codify import antigravity_native_bridge as bridge_mod
    from codify import antigravity_native_interactions as interactions_mod
    from codify.antigravity_native_interactions import agy_elicitation_id
    from codify.antigravity_native_steps import pending_interaction
    from codify.runner import app as runner_app_mod
    ...
    # Control the reader's Codify client transport: record the elicitation hook
...
@pytest.mark.asyncio
async def test_auto_create_antigravity_wires_codify_mcp_relay(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Auto-create wires the Codify MCP relay so agy gets the sys_* tools (#1194).
    ...
    * the launch args carry ``--gemini_dir=<isolated .gemini>`` while the launch
    ...
    server = payload["mcpServers"]["codify"]
    assert server["args"][:4] == ["-I", "-m", "codify.claude_native_bridge", "serve-mcp"]
...
async def test_codex_subagent_always_needs_runner_terminal(
    ...
    parent (e.g. nessie run via ``codify run --server``) silently never get
    ...
    class _Client:
        async def get(self, url: str, *, timeout: float) -> httpx.Response:
            ...
            :param url: Codify session snapshot URL.
            :param timeout: HTTP timeout in seconds.
            :returns: Fake Codify session response.
            """
            del timeout
...
async def test_codex_discover_thread_and_forward_cleans_up_on_discovery_failure(
    ...
    """
    from codify import codex_native_forwarder
    from codify.runner.app import (
