...
from codify.entities.session_resources import SessionResourceView
from codify.inner.terminal import TerminalInstance
...
from codify.runner.session_init_protocol import RunnerSessionInitEnvelope
...
def test_read_relay_policy_config_returns_coords_from_tool_relay_json(
    ...
    from codify.native_policy_hook import read_relay_policy_config
...
async def test_auto_create_pi_terminal_launches_required_terminal(
    ...
    """
    import codify.pi_native as pi_native
    import codify.pi_native_bridge as pi_native_bridge
    import codify.pi_native_credentials as pi_native_credentials
    ...
    monkeypatch.setattr("codify.runner.app._pi_native_launch_config", _fake_launch_config)
...
async def test_auto_create_kiro_terminal_launches_required_terminal_with_isolated_env(
    ...
    import codify.kiro_native as kiro_native
    ...
    monkeypatch.setattr(
        "codify.kiro_native_session_forwarder.supervise_kiro_session_forwarder",
        _fake_supervise_kiro_session_forwarder,
    )
    monkeypatch.setattr(
        "codify.kiro_native_permissions.supervise_kiro_permission_mirror",
    ...
    monkeypatch.setattr("codify.runner.app._kiro_native_launch_config", _fake_launch_config)
    ...
    # The Codify MCP tool relay is seeded for this session's bridge dir.
    assert relay_calls == [
        {
    ...
    # And the Codify MCP server is declared in the workspace-scoped kiro config.
    ...
    assert "serve-mcp" in mcp_servers["codify"]["args"]
...
async def test_auto_create_kiro_terminal_skips_mcp_wiring_without_relay(
    ...
    """Without a comment-relay callback, the Codify MCP is NOT wired.
...
async def test_auto_create_claude_terminal_passes_session_effort(
    ...
    When the Codify server returns a session with a persisted
    ...
    monkeypatch.setattr(
        "codify.claude_native_forwarder.supervise_forwarder",
        _no_op_forwarder,
    )
    ...
    # Fake Codify server client that returns a session with reasoning_effort.
    ...
    envelope = RunnerSessionInitEnvelope.model_validate(
        {
            ...
            "snapshot": {
                ...
                "labels": {"codify.fork.carry_history": "1"},
            },
        }
    )
...
def test_agent_os_env_from_spec_unwraps_resolved_and_handles_none() -> None:
    ...
    bare = AgentSpec(
        spec_version=1,
        name="agent",
        executor=ExecutorSpec(type="codify", config={}),
        os_env=os_env,
    )
...
async def test_auto_create_claude_terminal_injects_ucode_gateway_config(
    ...
    # global config's ``auth:`` block (written by ``codify setup``),
    ...
    # ``codify.claude_native`` per call, so patch it at the source.
    monkeypatch.setattr(
        "codify.claude_native._ucode_config_for_profile",
...
async def _run_auto_create_cursor_terminal(
    *,
    ...
    """
    from codify.runner import _entry as _runner_entry

    workspace = tmp_path / "ws"
    workspace.mkdir()
    monkeypatch.setattr(cursor_native_bridge, "_BRIDGE_ROOT", tmp_path / "cursor-bridge")
    monkeypatch.setenv("RUNNER_SERVER_URL", "http://127.0.0.1:8000")
    monkeypatch.setattr("codify.cursor_native.resolve_cursor_executable", lambda: "cursor-agent")

    async def _no_op_forwarder(**kwargs: Any) -> None:
        del kwargs

    monkeypatch.setattr(
        "codify.cursor_native_forwarder.supervise_cursor_forwarder",
        _no_op_forwarder,
    )
    monkeypatch.setattr(
        "codify.cursor_native_usage.supervise_cursor_usage_forwarder",
        _no_op_forwarder,
    )
    ...
    monkeypatch.setattr(
        "codify.cursor_native_permissions.supervise_cursor_transcript_elicitations",
...
async def test_auto_create_claude_terminal_forwarder_skips_replayed_transcript_on_resume(
    ...
    transcript file already holds every item Codify has at offset 0. The
    ...
    monkeypatch.setattr(claude_native_bridge, "_BRIDGE_ROOT", tmp_path / "root")
    monkeypatch.setenv("RUNNER_SERVER_URL", "http://127.0.0.1:8000")
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", str(tmp_path / "workspace"))
    ...
    # Transcript synthesis from Codify history has its own coverage; stub it to
    ...
    monkeypatch.setattr(
        "codify.claude_native._ensure_local_claude_resume_transcript",
        _fake_synth,
    )
...
async def test_auto_create_claude_terminal_emits_resource_created_event(
    ...
    # Routed under the session id so the Codify relay forwards it to that
...
def test_publish_terminal_pending_emits_pending_then_clear() -> None:
    """
    ``_publish_terminal_pending`` emits the wire shape the Codify relay
...
def test_publish_native_terminal_start_error_emits_failed_status_only(
    ...
    runner publishes one here, Codify can persist an orphan transcript error
...
def test_terminal_lookup_miss_log_explains_stopped_registered_terminal(
    ...
    try:
        with caplog.at_level(logging.INFO, logger="codify.runner.app"):
            _log_terminal_lookup_miss(
                resource_registry,
...
async def test_auto_create_claude_terminal_resets_stale_bridge_id_label(
    ...
    """
    Auto-create corrects a stale ``bridge_id`` label on the Codify session.
    ...
    # Capture all HTTP requests made to the fake Codify server.
...
async def test_create_session_auto_create_guard_skips_rotation_targets(
    ...
    monkeypatch.setattr(
        "codify.runner.native.orchestration._auto_create_claude_terminal",
    ...
    native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "claude-native"}),
    )
...
async def test_create_session_antigravity_auto_create_guard_skips_rotation_targets(
    ...
    A ``/clear`` rotation binds the runner to a fresh Codify session, then
    ...
    """
    monkeypatch.setattr(
        "codify.antigravity_native_bridge._BRIDGE_ROOT",
    ...
    monkeypatch.setattr(
        "codify.runner.native.orchestration._auto_create_antigravity_terminal",
        _recording_auto_create,
    )
    ...
    native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "antigravity-native"}),
    )
...
async def test_auto_create_claude_terminal_registers_permission_hook(
    ...
    present (web-UI sessions, the ``codify host`` host API). It
    must pass the Codify server URL into ``augment_claude_args`` so
    ...
    # so it must be written with the runner's Codify server URL.
    ...
    from codify.runner._entry import _RunnerDatabricksAuth
