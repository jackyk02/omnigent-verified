...
class _TrackingTerminalRegistry:
    ...
    def __init__(self, *, conversation_link_base_url: str | None = None) -> None:
        ...
        :param conversation_link_base_url: Codify server base URL passed
...
def test_make_auth_token_factory_returns_factory_when_databricks_creds_available(
    ...
    :returns: None.
    """
    from codify.inner.databricks_executor import _DatabricksBearerAuth
    ...
    monkeypatch.setattr(
        "codify.inner.databricks_executor._resolve_databricks_auth",
...
def test_make_auth_token_factory_returns_none_without_databricks_creds(
    ...
    :returns: None.
    """
    from codify.inner.databricks_executor import DatabricksAuthError
...
def test_make_auth_token_factory_uses_managed_mint_when_only_binding_token(
    ...
    monkeypatch.setenv("RUNNER_SERVER_URL", "https://codify.example.com")
    monkeypatch.setenv("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN", "managed-binding-token")
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
    monkeypatch.setattr("codify.inner.databricks_executor._resolve_databricks_auth", _no_sdk)
    monkeypatch.setattr(
        "codify.runner._entry._mint_managed_owner_token",
...
def test_make_auth_token_factory_prefers_host_delegation_over_user_credentials(
    ...
    monkeypatch.setenv("RUNNER_SERVER_URL", "https://codify.example.com")
    monkeypatch.setenv("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN", "host-binding-token")
    monkeypatch.setenv("CODIFY_RUNNER_DELEGATED_AUTH", "1")
    monkeypatch.setattr(
        "codify.inner.databricks_executor._resolve_databricks_auth",
...
def test_initial_host_token_defers_local_auth_until_rejected(
    ...
    monkeypatch.setenv("CODIFY_RUNNER_DELEGATED_AUTH", "1")
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
    monkeypatch.setattr("codify.inner.databricks_executor._resolve_databricks_auth", _resolve)
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _unexpected_mint)
...
def test_delegated_factory_falls_back_when_apps_proxy_redirects_mint(
    ...
    def _apps_redirect(mint_url: str, server_url: str, binding_token: str) -> tuple[str, float]:
        """Model the Apps edge intercepting the mint request before Codify."""
    ...
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _apps_redirect)
...
def test_make_auth_token_factory_none_without_creds_or_binding_token(
    ...
    monkeypatch.setenv("RUNNER_SERVER_URL", "https://codify.example.com")
    monkeypatch.delenv("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN", raising=False)
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
    monkeypatch.setattr("codify.inner.databricks_executor._resolve_databricks_auth", _no_sdk)
...
def test_managed_mint_factory_caches_token_until_refresh_skew(
    ...
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _fake_mint)
...
def test_managed_mint_factory_no_factory_when_server_definitively_refuses(
    ...
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _refuses)
...
def test_managed_mint_factory_installs_for_retry_on_transient_boot_failure(
    ...
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _blip)
...
def test_managed_mint_factory_declines_at_request_time_and_auth_sends_bare(
    ...
    monkeypatch.setattr("codify.runner._entry._mint_managed_owner_token", _boot_blip_then_refuse)
...
def test_mint_managed_owner_token_posts_binding_token_and_parses_response(
    ...
    the tunnel binding token in ``X-Codify-Runner-Tunnel-Token``,
    ...
    monkeypatch.setattr("codify.runner._entry.httpx.Client", _fake_client)
...
def test_runner_databricks_auth_injects_fresh_token_per_request() -> None:
    ...
    Codify server break after 1 hour.
...
@pytest.mark.parametrize(
    "location",
    [
        # Real-world shape captured from the Codify HTTP path: the Apps
...
async def test_runner_databricks_auth_end_to_end_through_mock_transport() -> None:
    ...
    2. Codify front door bounces with ``302 → /oidc/...authorize``.
...
def test_runner_tunnel_binding_token_from_env_rejects_empty_token(
    ...
    :returns: None.
    """
    monkeypatch.setenv("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN", "  ")
...
def test_runner_tunnel_binding_token_from_env_strips_value(
    ...
    :returns: None.
    """
    monkeypatch.setenv("CODIFY_RUNNER_TUNNEL_BINDING_TOKEN", " bind-token ")
...
def test_runner_parent_pid_from_env_returns_none_without_pid(
    ...
    :returns: None.
    """
    monkeypatch.delenv("CODIFY_RUNNER_PARENT_PID", raising=False)
...
def test_runner_parent_pid_from_env_rejects_invalid_pid(
    ...
    :returns: None.
    """
    monkeypatch.setenv("CODIFY_RUNNER_PARENT_PID", value)

    with pytest.raises(RuntimeError, match="CODIFY_RUNNER_PARENT_PID"):
        _runner_parent_pid_from_env()
...
def test_runner_parent_pid_from_env_strips_value(
    ...
    :returns: None.
    """
    monkeypatch.setenv("CODIFY_RUNNER_PARENT_PID", " 12345 ")
...
def test_run_parent_death_killer_logs_reason_before_hard_exit(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.runner._entry"):
        _run_parent_death_killer(
            os.getppid() + 100000,  # already "orphaned"
            lambda: None,
            poll_interval_s=0.01,
...
def test_install_crash_logging_records_uncaught_exception(
    ...
    try:
        ...
        with caplog.at_level(logging.CRITICAL, logger="codify.runner._entry"):
            try:
...
async def test_install_signal_handlers_records_signal_reason() -> None:
    ...
    :returns: None.
    """
    from codify.runner._entry import _install_signal_handlers
...
async def test_runner_shutdown_closes_terminal_registry(
    ...
    and in ``codify run --server`` mode those terminals are owned
    ...
    monkeypatch.setattr(
        "codify.runtime.harnesses.process_manager.HarnessProcessManager",
        _FakeProcessManager,
    )
    monkeypatch.setattr(
        "codify.terminals.TerminalRegistry",
    ...
    monkeypatch.setattr(
        "codify.runner.identity.get_stable_runner_id",
    ...
    # In Codify mode (P1) the entry point passes mcp_manager=None; MCP calls are
    ...
    assert not mcp_managers, (
        "RunnerMcpManager should not be created by create_app() in Codify mode; "
...
def test_runner_workspace_from_env_rejects_empty_value(
    ...
    :returns: None.
    """
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", "  ")
...
def test_runner_workspace_from_env_resolves_value(
    ...
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", f" {workspace} ")
...
def test_main_reports_tunnel_rejection_without_traceback(
    ...
    monkeypatch.setattr(
        "codify.runner._entry._run_tunnel_from_env",
...
def test_main_configures_runner_process_logging(
    ...
    monkeypatch.setattr(
        "codify.process_logging.configure_process_logging",
