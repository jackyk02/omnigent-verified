...
async def test_auto_create_pi_terminal_threads_spec_model_into_models_json(
    ...
    :returns: None.
    """
    import codify.pi_native_bridge as pi_bridge
    import codify.pi_native_credentials as creds
    ...
    monkeypatch.setattr("codify.pi_native.resolve_pi_executable", lambda: "/usr/bin/pi")
    ...
    assert models["providers"]["codify"]["models"] == [{"id": "claude-opus-4-7"}]
...
async def test_auto_create_pi_terminal_no_spec_model_uses_provider_default(
    ...
    agent_dir = Path(launched["env"]["PI_CODING_AGENT_DIR"])
    models = json.loads((agent_dir / "models.json").read_text(encoding="utf-8"))
    assert models["providers"]["codify"]["models"] == [{"id": "claude-sonnet-4-6"}]
