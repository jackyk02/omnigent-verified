...
async def test_terminal_resource_role_moves_on_transfer(
    ...
    Native Codex can rotate ownership between Codify sessions. If the role stays
...
def test_resolve_environment_creates_primary_lazily(
    ...
    os.environ["CODIFY_RUNNER_OS_ENV_ROOT"] = str(tmp_path)
    try:
        reg = SessionResourceRegistry()
        ...
        os.environ.pop("CODIFY_RUNNER_OS_ENV_ROOT", None)
...
def test_resolve_environment_default_pins_none_sandbox_when_no_agent_spec(
    ...
    """
    monkeypatch.setattr(
        "codify.inner.sandbox.shutil.which",
        lambda name: "/usr/bin/bwrap",
    )
    monkeypatch.setenv("CODIFY_RUNNER_OS_ENV_ROOT", str(tmp_path))
...
def test_compute_default_env_root_runner_workspace_overrides_relative_cwd(
    ...
    terminal cwd flows through ``CODIFY_RUNNER_WORKSPACE`` and
