"""Tests for ``_codex_native_launch_config`` in ``codify/runner/app.py``.
...
from codify.runner.app import _codex_native_launch_config
...
async def test_happy_path_parses_full_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    snapshot = {
        ...
        "labels": {
            "codify.fork.source_id": "conv_source",
            "codify.fork.source_external_session_id": "thread_src",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"codify.codex_native.bypass_sandbox": "0"},  # explicit off
        {"codify.codex_native.bypass_sandbox": "true"},  # only "1" arms it
        {"codify.codex_native.bypass_sandbox": ""},  # empty string
    ],
)
