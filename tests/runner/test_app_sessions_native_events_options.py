...
async def test_events_effort_change_on_native_session_skips_inject_for_unsupported_level(
    ...
    Codify server is harness-agnostic — it always forwards the new
    ...
    the Codify side; the next spawn picks up the value via ``--effort``.

    Pins that the validation lives in the runner (where the
    harness-specific knowledge belongs), not in the Codify server.
    """
    from codify.spec.types import ExecutorSpec
...
async def test_events_effort_change_on_native_session_returns_503_when_bridge_not_ready(
    ...
    legacy route returns. Codify server's PATCH swallows this 503 and
...
async def test_events_effort_change_on_non_native_session_is_204_noop(
    ...
    Codify server still POSTs ``effort_change`` to ``/events`` for every
...
async def test_events_compact_on_native_session_types_slash_command(
    ...
    Codify server's own compaction would only summarise the transcript
    ...
    The 200 (not 204) is load-bearing: the Codify server reads it to know
    ...
    the Codify server fall through to ``_run_compact_locked``, which 400s
    ...
    # in-process no-op (wrong harness branch) → Codify falls through to
...
async def test_events_compact_on_codex_native_injects_slash_command(
    ...
    load-bearing: the Codify server reads it to skip its own
    AP-side compaction.
    """
    from codify.runner.app import _session_event_queues_ref
    ...
    codex_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "codex-native"}),
    )
...
async def test_events_compact_on_codex_native_returns_204_when_no_terminal() -> None:
    ...
    has nowhere to go.  204 tells the Codify server to fall back to
...
async def test_events_compact_on_codex_native_returns_503_on_tmux_failure(
    ...
    The 503 tells the Codify server the control was NOT handled, so it
...
async def test_events_compact_on_cursor_native_pastes_summarize_and_raises_spinner(
    ...
    path.  The 200 (not 204) is load-bearing: the Codify server reads it to
    ...
    cursor_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "cursor-native"}),
    )
    ...
    # no-op branch (the original gap) → Codify runs its own compaction and 400s.
    assert resp.status_code == 200, (
...
async def test_events_compact_on_pi_native_enqueues_compact_payload(
    ...
    compaction must run there (the Codify server's AP-side compaction would
    ...
    Pins:
    1. 200 returned (not 204) so the Codify server skips its own AP-side
       compaction.
    ...
    pi_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "pi-native"}),
    )
...
async def test_events_compact_on_pi_native_returns_503_when_inbox_unwritable(
    ...
    request; the Codify server then treats it as not-handled.
    """
    import codify.pi_native_bridge as pi_native_bridge
    from codify.spec.types import ExecutorSpec
...
async def test_events_compact_on_qwen_native_submits_compress_and_raises_spinner(
    ...
    qwen_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "qwen-native"}),
    )
...
class _FakeOpenCodeCompactClient:
    ...
    Stands in for :class:`codify.opencode_native_client.OpenCodeClient` so
...
async def _drive_opencode_native_compact(
    ...
    *,
    ...
    from codify.opencode_native_bridge import OpenCodeNativeBridgeState
    from codify.opencode_native_client import OpenCodeSession
    from codify.runner.app import _AUTO_OPENCODE_SERVERS, _session_event_queues_ref
    from codify.spec.types import ExecutorSpec
    from tests.runner.helpers import make_test_terminal_instance
    ...
    opencode_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "opencode-native"}),
    )
...
def test_resolve_opencode_compact_model_prefers_latest_assistant_message() -> None:
    ...
    """
    from codify.opencode_native_client import OpenCodeSession
    from codify.runner.app import _resolve_opencode_compact_model
...
def test_resolve_opencode_compact_model_returns_none_when_unresolvable() -> None:
    ...
    Covers the live Codify flow: the session is created without a model and
...
async def test_events_compact_on_opencode_native_summarizes_from_assistant_message(
    ...
    ``modelID``) because Codify creates the session without a model. A 200
    return is load-bearing: the Codify server reads it to skip its AP-side
...
async def test_events_compact_on_opencode_native_204_when_model_unresolvable(
    ...
    The 204 tells the Codify server to run its own AP-side compaction.
    """
    resp, client = await _drive_opencode_native_compact(
        monkeypatch,
        tmp_path,
...
async def test_events_compact_on_opencode_native_503_when_summarize_raises(
    ...
    The Codify server must see the failure (rather than a silent fallback)
...
async def test_events_compact_on_non_native_session_is_204_noop(
    ...
    The Codify server forwards ``compact`` to ``/events`` for every harness
    ...
    Codify server to run its own compaction.
    """
    from codify.spec.types import ExecutorSpec
...
@pytest.mark.parametrize(
    ...
    # bridge_id != conv_id, stored on the ``codify.claude_native
...
async def test_events_native_dispatch_resolves_bridge_id_via_label_lookup(
    ...
    ``codify.claude_native.bridge_id``).
    ...
    the assertion fails.
    """
    from codify.runner import app as runner_app_module
    from codify.spec.types import ExecutorSpec
...
async def test_events_model_change_on_kiro_session_types_slash_command(
    ...
    native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="codify", config={"harness": "kiro-native"}),
    )
...
async def test_events_model_change_on_native_session_skips_inject_for_empty_or_null(
    ...
    handler, not in the Codify server.
    """
    from codify.spec.types import ExecutorSpec
...
async def test_events_model_change_on_native_session_returns_503_when_bridge_not_ready(
    ...
    advertised) returns 503 with the same error code shape the
    legacy ``/claude-native-model`` route used. Codify server's PATCH
...
async def test_events_model_change_on_non_native_session_is_204_noop(
    ...
    each turn (or via the per-event override). Codify server is harness-
...
async def test_events_model_change_on_cursor_native_session_returns_503_when_not_ready(
    ...
    code; Codify server swallows it and the next spawn applies ``--model``.
    """
    from codify.spec.types import ExecutorSpec
