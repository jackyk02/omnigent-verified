...
from codify import codex_native_hook, native_policy_hook
...
class _RaisesIfCalled:
    ...
    def post(self, url: str, *, json: dict[str, object]) -> httpx.Response:
        ...
        :param url: Target Codify URL (unused).
        ...
        raise AssertionError(
            "evaluate-policy POSTed to Codify when it should have short-circuited "
...
def bridge_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    touches the real ``~/.codify`` tree, then writes a valid bridge
    state whose ``session_id`` the hook reads to build the Codify URL.

    :param tmp_path: pytest temp directory.
    :param monkeypatch: pytest monkeypatch fixture.
    :returns: Prepared bridge directory.
    """
    monkeypatch.setattr("codify.codex_native_bridge._BRIDGE_ROOT", tmp_path / "codex-native")
...
def test_missing_policy_config_is_fail_open(
    ...
    The session has state but no Codify coordinates were written (e.g. a
    local run with no Codify server), so there is nothing to enforce against.
