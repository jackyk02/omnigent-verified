"""Tests for the ``omni upgrade`` command (codify.cli.upgrade)."""
...
from codify.host.local_server import LocalServerInfo
from codify.update_check import _InstalledWheelInfo


def _uv_registry_info() -> _InstalledWheelInfo:
    """A registry uv-tool install → ``uv tool upgrade codify`` (runnable)."""
...
def _git_install_info() -> _InstalledWheelInfo:
    ...
    return _InstalledWheelInfo(
        install_time_epoch=0.0,
        installer="uv",
        vcs_url="git+https://github.com/codify-ai/codify.git",
        commit_sha="a" * 40,
        is_editable=False,
        package_version="0.1.0",
        detected_installer="uv",
...
def _wheel_install(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.update_check._read_installed_wheel_info", _uv_registry_info)
    # Neutralize the process side effects unless a test opts in to assert them.
    monkeypatch.setattr(
        "codify.cli.local_server_status",
        lambda: LocalServerInfo(running=False, pid=None, port=None, url=None),
    )
    monkeypatch.setattr("codify.cli._stop_local_server_and_daemon", lambda *, force: False)
...
def test_upgrade_up_to_date(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda *_a, **_k: "0.1.0")

    def _must_not_run(*_a: object, **_k: object) -> int:
        raise AssertionError("upgrade command ran while already up to date")

    monkeypatch.setattr("codify.update_check._run_upgrade_command", _must_not_run)
...
def test_upgrade_check_reports_newer_and_exits_nonzero(
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
...
def test_upgrade_runs_installer_and_drains_first(
    ...
    events: list[str] = []
    monkeypatch.setattr(
        "codify.cli._wait_for_local_sessions_to_drain", lambda: events.append("drained")
    )
    ...
    monkeypatch.setattr("codify.cli._stop_local_server_and_daemon", _stop)
    ...
    monkeypatch.setattr("codify.update_check._run_upgrade_command", _run)
    ...
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.2.0", None)
    )
    ...
    assert ran == ["uv tool upgrade codify"]
...
def test_upgrade_force_skips_drain_and_force_stops(
    ...
    monkeypatch.setattr("codify.cli._wait_for_local_sessions_to_drain", _no_drain)

    stop_calls: list[bool] = []
    monkeypatch.setattr(
        "codify.cli._stop_local_server_and_daemon",
        lambda *, force: stop_calls.append(force) or False,
    )
    monkeypatch.setattr("codify.update_check._run_upgrade_command", lambda *_a, **_k: 0)
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.2.0", None)
    )
...
def test_upgrade_install_failure_surfaces(
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
    monkeypatch.setattr("codify.update_check._run_upgrade_command", lambda *_a, **_k: 3)
...
def test_upgrade_index_unreachable(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda *_a, **_k: None)
...
def test_upgrade_rejects_dev_clone(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    ...
    monkeypatch.setattr("codify.update_check._find_repo_root", lambda: Path("/repo"))
...
def test_upgrade_rejects_editable(monkeypatch: pytest.MonkeyPatch, _wheel_install: None) -> None:
    ...
    editable = _InstalledWheelInfo(
        install_time_epoch=0.0,
        installer="uv",
        vcs_url="file:///Users/me/codify",
        commit_sha=None,
        is_editable=True,
        package_version="0.1.0",
        detected_installer="uv",
    )
    monkeypatch.setattr("codify.update_check._read_installed_wheel_info", lambda: editable)
...
def test_upgrade_pre_check_detects_release_candidate(
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", _fetch)
...
def test_count_running_sessions_ignores_idle_connected(
    ...
    from codify.cli import _count_running_sessions, _SessionPagesResult
    ...
    monkeypatch.setattr(
        "codify.cli._fetch_session_pages",
...
def test_drain_returns_immediately_when_only_idle_connected(
    ...
    """
    from codify.cli import _SessionPagesResult, _wait_for_local_sessions_to_drain
...
def test_upgrade_pre_passes_prerelease_flag_to_installer(
    ...
    monkeypatch.setattr(
        "codify.update_check.fetch_latest_version",
        lambda include_prereleases=False, **_kw: "0.1.1rc1",
    )
    monkeypatch.setattr("codify.cli._wait_for_local_sessions_to_drain", lambda: None)
    monkeypatch.setattr("codify.cli._stop_local_server_and_daemon", lambda *, force: False)
    ran: list[str] = []
    monkeypatch.setattr(
        "codify.update_check._run_upgrade_command",
        lambda command, _console: ran.append(command) or 0,
    )
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.1.1rc1", None)
    )

    result = CliRunner().invoke(cli, ["upgrade", "--pre"])

    assert result.exit_code == 0, result.output
    assert ran == ["uv tool upgrade codify --prerelease allow"]
...
def test_update_suppresses_update_check_like_upgrade() -> None:
    ...
    from codify.cli import _should_skip_update_check
...
def test_upgrade_noop_install_reports_failure_not_success(
    ...
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.1.0", None)
    )
...
def test_upgrade_unconfirmed_version_is_honest(
    ...
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda *_a, **_k: "0.2.0")
    monkeypatch.setattr("codify.update_check._run_upgrade_command", lambda *_a, **_k: 0)
    monkeypatch.setattr("codify.update_check._probe_installed_distribution", lambda: (None, None))
...
def test_upgrade_git_install_up_to_date(
    ...
    monkeypatch.setattr("codify.update_check._read_installed_wheel_info", _git_install_info)
    # fetch_latest_version must never be consulted for a git install.
    monkeypatch.setattr(
        "codify.update_check.fetch_latest_version",
        lambda *_a, **_k: pytest.fail("git install must not query PyPI"),
    )
    monkeypatch.setattr("codify.update_check._remote_git_head", lambda _url: "a" * 40)
...
def test_upgrade_git_check_behind_exits_nonzero(
    ...
    monkeypatch.setattr("codify.update_check._read_installed_wheel_info", _git_install_info)
    monkeypatch.setattr("codify.update_check._remote_git_head", lambda _url: "b" * 40)
...
def test_upgrade_git_install_repulls_and_verifies_commit(
    ...
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.1.0", "b" * 40)
    )

    result = CliRunner().invoke(cli, ["upgrade"])

    assert result.exit_code == 0, result.output
    assert ran == ["uv tool install --reinstall git+https://github.com/codify-ai/codify.git"]
...
def test_upgrade_git_install_noop_does_not_claim_update(
    ...
    monkeypatch.setattr("codify.update_check._remote_git_head", lambda _url: None)
    monkeypatch.setattr("codify.update_check._run_upgrade_command", lambda *_a, **_k: 0)
    monkeypatch.setattr(
        "codify.update_check._probe_installed_distribution", lambda: ("0.1.0", "a" * 40)
    )
