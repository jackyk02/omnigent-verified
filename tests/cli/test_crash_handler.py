"""Tests for the friendly crash handler + UI (``codify.crash_handler``).
...
isolated via ``CODIFY_DATA_DIR`` so no reports are written to the
...
from codify import crash_handler as ch
from codify import crash_ui
...
def data_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    d = tmp_path / "codify-data"
    monkeypatch.setenv("CODIFY_DATA_DIR", str(d))
    return d
...
def test_build_report_contains_required_fields(data_dir: Path) -> None:
    ch.install_crash_handler("codify", "codify-ai/codify")
    exc = _make_exc("the frobnicator failed")
    report = ch._build_report(
        exc, "Traceback...\nValueError: the frobnicator failed\n", source="uncaught"
    )
    assert "# Crash Report — codify" in report
    assert "ValueError" in report
    assert f"codify {VERSION}" in report or "codify unknown" in report  # version line
    assert "https://github.com/codify-ai/codify" in report
...
def test_command_line_redacts_tokens_in_argv(
    ...
    monkeypatch.setattr(
        sys, "argv", ["codify", "run", "a.yaml", "--api-key", "sk-supersecret123456"]
    )
...
def test_save_report_writes_and_rotates(data_dir: Path) -> None:
    ch.install_crash_handler("codify", "codify-ai/codify", keep_reports=2)
...
def test_save_report_keeps_every_same_second_report(data_dir: Path) -> None:
    ...
    """
    ch.install_crash_handler("codify", "codify-ai/codify", keep_reports=10)
    paths = [ch._save_report(f"report {i}\n") for i in range(5)]
...
def test_render_non_tty_is_plain_no_box(data_dir: Path) -> None:
    ...
    assert "Codify ran into an issue." in out
...
def test_render_tty_shows_header_and_copyable_path(data_dir: Path) -> None:
    ...
    # Header present.
    assert "Codify ran into an issue." in plain
...
def test_full_traceback_env_disables_collapsing(
    ...
    monkeypatch.setenv("CODIFY_FULL_TRACEBACK", "1")
    exc, tb = _traceback_with_library_frames()
    out = crash_ui.format_traceback(exc, tb, colored=False, unicode_ok=True)
...
def test_first_party_sdk_shown_even_in_site_packages(data_dir: Path, tmp_path: Path) -> None:
    ...
    In a shipped wheel the SDKs (``codify_client``, ``codify_ui_sdk``)
    live under site-packages next to click/yaml. The default first-party
    prefix ``("codify",)`` must keep their frames shown rather than
    ...
    fake = os.path.join(sp, "codify_client")
    os.makedirs(fake, exist_ok=True)
    probe = os.path.join(fake, "_probe.py")
    with open(probe, "w") as f:
        f.write('def boom(): raise RuntimeError("sdk probe")\n')
    spec = importlib.util.spec_from_file_location("codify_client._probe", probe)
    ...
    try:
        ...
        _os.chdir(old)
    assert "codify_client/_probe.py" in out  # shown, not collapsed
    assert "frames hidden in codify_client" not in out
...
def test_interactive_yes_copies_and_opens_browser(
    ...
    assert calls["url"].startswith("https://github.com/codify-ai/codify/issues/new?")
...
def test_issue_url_is_prefilled_title(data_dir: Path) -> None:
    ...
    assert url.startswith("https://github.com/codify-ai/codify/issues/new?")
