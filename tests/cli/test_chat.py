"""Tests for codify.chat — codify chat CLI logic."""
...
import httpx
import pytest
from codify_client import CodifyError as ClientCodifyError
from codify_client import QueryResult

import codify.chat as chat_module
...
from codify.cli import _build_resume_parts
from codify.inner.databricks_executor import DatabricksCredentials
from codify.model_resolver import ModelResolutionError
from codify.spec import load as load_spec
from codify.spec import validate as validate_spec
...
def test_redirect_native_resume_routes_kiro_wrapper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.kiro_native.run_kiro_native", _capture)
...
def test_validate_agent_spec_unresolved_env_var(
    ...
    # proves the underlying CodifyError message reached the user
...
def test_validate_agent_spec_missing_config(tmp_path: Path) -> None:
    ...
    # (not the CodifyError branch) — both must convert.
...
def test_wait_for_server_uses_fast_poll_before_backoff(
    ...
    monkeypatch.setattr("codify.chat.time.monotonic", _fake_monotonic)
    monkeypatch.setattr("codify.chat.time.sleep", _fake_sleep)
    monkeypatch.setattr("codify.chat.httpx.get", _fake_get)
...
def test_raise_server_failed_truncates_log_to_tail(tmp_path: Path) -> None:
    ...
    must be preserved.
    """
    from codify.chat import _SERVER_LOG_TAIL_LINES
    ...
    tail_lines = [
        ...
        "  File codify/server/app.py, line 42, in create_app",
        "RuntimeError: missing required field 'agent'",
    ]
    log.write_text("\n".join(head_lines + tail_lines) + "\n")
    server = SimpleNamespace(
        proc=SimpleNamespace(args=["python", "-m", "codify", "server"]),
        log_path=log,
    )
    ...
    assert "python -m codify server" in msg
...
def test_start_local_server_spawns_runner_as_sibling(
    ...
    via ``CODIFY_RUNNER_TUNNEL_TOKEN`` so it accepts exactly the
    sibling runner's tunnel. The runner is NOT a child of the server.
    """
    from codify.cli import _CliRunnerProcess
    ...
    import codify.runner.identity  # noqa: F401

    monkeypatch.setattr("codify.chat.subprocess.Popen", _fake_popen)
    monkeypatch.setattr("codify.chat._codify_log_dir", lambda: tmp_path / "logs")
    monkeypatch.setattr(
        "codify.chat.load_spec",
        lambda _path: SimpleNamespace(executor=SimpleNamespace(profile=None)),
    )
    monkeypatch.setattr(
        "codify.cli._start_cli_runner_process",
        _fake_start_runner,
    )
    ...
    assert "CODIFY_RUNNER_TUNNEL_TOKEN" in server_popen_calls[0].env

    # Runner was spawned as a sibling via _start_cli_runner_process.
    assert len(runner_calls) == 1
    assert runner_calls[0]["server_url"] == "http://127.0.0.1:8765"
    assert (
        runner_calls[0]["tunnel_token"] == server_popen_calls[0].env["CODIFY_RUNNER_TUNNEL_TOKEN"]
    )
...
def test_wait_for_remote_runner_timeout_surfaces_log_path(
    ...
    monkeypatch.setattr("codify.chat.time.monotonic", _fake_monotonic)
    monkeypatch.setattr("codify.chat.time.sleep", lambda _s: None)
    monkeypatch.setattr(
        "codify.chat.httpx.get",
        lambda *_a, **_k: _Resp(),
    )
...
def test_wait_for_remote_runner_early_exit_surfaces_log_path(
    ...
    monkeypatch.setattr("codify.chat.time.monotonic", lambda: 0.0)
    monkeypatch.setattr("codify.chat.time.sleep", lambda _s: None)
...
def test_run_chat_with_server_url_routes_through_daemon(
    ...
    monkeypatch.setattr("codify.cli._ensure_backend", _fake_ensure_backend)
...
def test_chat_via_daemon_uses_directory_bundle_for_root_config_yaml(
    ...
    monkeypatch.setattr(
        "codify.host.identity.load_or_create_host_identity",
...
def test_run_local_headless_prompt_uses_directory_bundle_for_root_config_yaml(
    ...
    This covers ``codify run bundle/config.yaml -p ...``. Without the
...
class _FakeSdkClient:
    """Async-context-manager stand-in for ``CodifyClient`` in prep tests.
...
def _patch_daemon_launch(monkeypatch: pytest.MonkeyPatch, captured: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("codify.host.daemon_launch.wait_for_host_online", _no_host_wait)
    monkeypatch.setattr("codify.host.daemon_launch.launch_or_reuse_daemon_runner", _fake_launch)
    monkeypatch.setattr("codify.host.daemon_launch.wait_for_runner_online", _no_runner_wait)
    monkeypatch.setattr("codify.native_terminal.bind_session_runner", _fake_bind)
...
def test_prepare_chat_session_via_daemon_binds_runner_to_clear_stopped_marker(
    ...
    paths, neither of which clears the server-side ``codify.stopped`` marker
    ...
    # PATCH chokepoint that clears codify.stopped — same pattern as
    # ``codify claude`` (claude_native.py's bind_session_runner call).
...
def test_default_cli_model_resolves_catalog_when_env_unset(
    ...
    monkeypatch.delenv("CODIFY_MODEL", raising=False)
...
def test_default_cli_model_fails_clearly_without_catalog(
    ...
    with pytest.raises(click.ClickException, match="Pass --model, set CODIFY_MODEL"):
        _default_cli_model()


def test_default_cli_model_honors_codify_model_env_var(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    With ``CODIFY_MODEL=foo`` set, the helper returns
    ``"foo"``.

    What this proves: the env-var override fires without consulting discovery.
    """
    monkeypatch.setenv("CODIFY_MODEL", "databricks-claude-sonnet-4-6")
...
def test_apply_overrides_yaml_model_wins_over_env_and_catalog(
    ...
    monkeypatch.setenv("CODIFY_MODEL", "from-env")
    monkeypatch.setattr(
        chat_module,
...
def test_apply_overrides_explicit_model_wins_over_env_var(
    ...
    assert executor.get("model") == "from-flag", (
        f"--model override must win over CODIFY_MODEL. Got "
...
def test_apply_overrides_writes_nested_config_harness_for_spec_version_bundle() -> None:
    ...
    Regression guard for the polly no-op: ``codify run
...
def test_apply_overrides_flat_harness_creates_no_config_for_single_file_yaml() -> None:
    """
    Single-file codify YAMLs (no ``spec_version``) keep the flat
...
def test_apply_overrides_canonicalizes_alias_into_spec_version_config(
    ...
    assert executor["config"]["harness"] == canonical, (
        ...
        f"A raw alias here would fail CODIFY_HARNESSES validation or "
...
def test_apply_overrides_harness_and_model_together_for_spec_version_bundle() -> None:
    ...
    This is the polly-on-GPT invocation shape: ``codify run
...
def test_apply_overrides_rejects_harness_for_non_codify_executor_type() -> None:
    """
    A spec_version bundle with a non-codify ``executor.type`` fails
...
def test_materialize_override_bundle_bakes_env_var_into_yaml(
    ...
    real YAML file, set ``CODIFY_MODEL=foo``, materialize a
    ...
    on-disk override the codify server reads. If the
...
def test_apply_overrides_skips_default_for_nested_harness(
    ...
    monkeypatch.setenv("CODIFY_MODEL", "databricks-gpt-5-4")
    raw: dict[str, object] = {
        "spec_version": 1,
        "name": "debby",
        "prompt": "hi",
...
def test_materialize_directory_bundle_with_override_keeps_nested_harness_unpinned(
    ...
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path / "config"))
...
def test_materialize_bundle_overrides_brain_harness(
    ...
    """
    A REAL bundled orchestrator (best_of_n), materialized with
    ``--harness``, parses to a valid spec whose brain runs the requested
    harness and whose proposer sub-agents keep their own harnesses.

    End-to-end through the production pipeline: ``copytree`` →
    ``_apply_overrides_to_raw`` → ``yaml.safe_dump`` → ``codify.spec.load``
    → ``validate``. This is the exact path ``codify run examples/best_of_n
    --harness pi`` takes before the bundle reaches a server.
    ...
    still validates (the harness is in CODIFY_HARNESSES); (3) the
    override never leaks into the sub-agents, which would break
    cross-vendor proposal diversity (best_of_n's workers).

    :param bundle_name: Packaged example bundle under
        ``codify.resources.examples``, e.g. ``"best_of_n"``.
    ...
    # The bundle's codex proposer references ${DEEPSEEK_API_KEY}; provide a
    # dummy so env-isolated parsing succeeds.
    monkeypatch.setenv("DEEPSEEK_API_KEY", "test-key-for-bundle-parse")

    # Isolate from the developer's codify config and ambient creds so
    ...
    bundle_dir = Path(
        str(importlib.resources.files("codify.resources.examples").joinpath(bundle_name))
    )
...
def test_remote_headers_prefers_explicit_remote_token_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_REMOTE_AUTH_TOKEN", "env-token")
    monkeypatch.setattr(
        chat_module,
        "_read_databrickscfg",
...
def test_remote_headers_falls_back_to_ambient_databricks_creds(
    ...
    Bottom of the resolution chain: with ``CODIFY_REMOTE_AUTH_TOKEN``
    ...
    """
    monkeypatch.delenv("CODIFY_REMOTE_AUTH_TOKEN", raising=False)
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
...
def test_remote_headers_adds_org_id_header(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli_auth.load_databricks_org_id", lambda _url: "2850744067564480")

    headers = _remote_headers(server_url="https://acme.databricks.com/api/2.0/codify")
...
def test_remote_headers_omits_org_when_no_record(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
    monkeypatch.setattr(chat_module, "_stored_databricks_record_token", lambda _url: "rec-tok")
    monkeypatch.setattr("codify.cli_auth.load_databricks_org_id", lambda _url: None)

    headers = _remote_headers(server_url="https://single.databricks.com/api/2.0/codify")
...
def test_run_repl_auto_opens_conversation_when_session_starts(
    ...
    class _Client:
        """Async context manager stub for :class:`CodifyClient`."""
    ...
    monkeypatch.setattr(chat_module, "CodifyClient", _Client)
    monkeypatch.setattr("codify.repl.run_repl", _fake_run_repl)
    monkeypatch.setattr(chat_module, "open_conversation_link_if_enabled", _fake_open)
    monkeypatch.setattr("codify.repl._tmux_pane.register_pane", lambda **kwargs: None)
...
def _make_run_context(**params: object) -> click.Context:
    ...
    ctx = click.Context(run_cmd, info_name="run", parent=click.Context(cli, info_name="codify"))
    ctx.params = merged
    return ctx
...
def test_build_resume_parts_omits_defaults() -> None:
    ...
    assert parts == ["codify", "run", "agent.yaml"]
...
def _stub_run_repl_deps(
    ...
    *,
    ...
    Replaces ``run_repl`` (the async REPL), ``CodifyClient``
    ...
    # ``from codify.repl import run_repl``, which reads the
    ...
    import codify.repl as _repl_pkg

    monkeypatch.setattr(_repl_pkg, "run_repl", _fake_run_repl)
    monkeypatch.setattr("codify.repl._repl.run_repl", _fake_run_repl)
    monkeypatch.setattr("codify.chat.CodifyClient", _FakeClientCtx)
    monkeypatch.setattr("codify.chat._server_auth", lambda server_url=None: None)
    monkeypatch.setattr(
...
def test_run_repl_passes_resume_parts_to_run_repl(
    ...
    parts = ["codify", "run", "agent.yaml", "--server", "https://example.com"]
...
class _FakeResumeClient:
    """Minimal CodifyClient-like object exposing sessions."""
...
class _FakeClientCtx:
    """Minimal CodifyClient stand-in that works as an async context manager.
...
def test_databricks_token_auth_resolves_sdk_once(
    ...
    :returns: None.
    """
    import codify.inner.databricks_executor as dbx
    ...
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)  # skip OIDC path
    ...
    monkeypatch.setattr("codify.cli_auth.load_databricks_workspace_host", lambda _url: None)
...
def test_databricks_token_auth_sets_org_header(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: None)
    monkeypatch.setattr(
        "codify.cli_auth.databricks_request_headers",
    ...
    auth = chat_module._DatabricksTokenAuth(
        server_url="https://acme.databricks.com/api/2.0/codify"
    )
    flow = auth.auth_flow(
        httpx.Request("GET", "https://acme.databricks.com/api/2.0/codify/v1/sessions")
    )
    request = next(flow)
    flow.close()
...
def test_spec_used_families_multi_vendor_directory_agent(tmp_path) -> None:
    ...
    # (the latter is what `codify run <dir>` threads through).
...
def test_await_accounts_setup_noop_when_token_present(
    ...
    monkeypatch.setattr("codify.cli_auth.load_token", lambda _url: "existing-token")

    def _must_not_probe(*_a: object, **_k: object) -> object:
        raise AssertionError("must not call /v1/info when a token already exists")

    monkeypatch.setattr("codify.chat.httpx.get", _must_not_probe)
...
def test_await_accounts_setup_noop_for_header_mode(
    ...
    monkeypatch.setattr("codify.chat.time.sleep", _must_not_sleep)
...
def test_await_accounts_setup_waits_then_continues(
    ...
    monkeypatch.setattr("codify.cli_auth.load_token", _load)
    monkeypatch.setattr(
        "codify.chat.httpx.get",
...
async def _run_one_shot(
    ...
    :param client: The fake Codify client supplying the transcript.
    :param query_impl: The async ``query`` behavior to install.
    :param monkeypatch: pytest monkeypatch fixture.
    :returns: Whatever ``_query_sessions_once`` returns.
    """
    # chat.py does ``from codify_client import SessionsChat`` inside
    # the function, so patch the attribute on the package (resolved at
    # call time), not a chat-module-local alias.
    monkeypatch.setattr("codify_client.SessionsChat", _fake_sessions_chat_cls(query_impl))
...
async def test_query_sessions_once_reconciles_persisted_text_on_failed_status(
    ...
    This is the exact reported bug: ``codify run -p`` printed
...
async def test_query_sessions_once_reraises_when_no_persisted_text(
    ...
    with pytest.raises(ClientCodifyError, match="auth misconfigured"):
...
async def test_query_sessions_once_surfaces_persisted_error_when_no_text(
    ...
    with pytest.raises(ClientCodifyError, match="Failed to start cursor-sdk agent"):
...
async def test_query_sessions_once_reraises_on_failed_with_only_partial_item(
    ...
    with pytest.raises(ClientCodifyError, match="turn failed"):
...
def test_env_auth_injection_skipped_when_global_auth_configured(
    ...
    With a global ``auth:`` block (written by ``codify setup``), the
    ...
    """
    from codify.chat import _inject_openai_env_auth_if_needed
    ...
    # runner resolves it via CODIFY_CONFIG_HOME. A baked api_key here
...
def test_redirect_native_resume_handles_cursor(monkeypatch: pytest.MonkeyPatch) -> None:
    """A cursor-native resume hands off to ``codify cursor`` (direct attach).

    Regression: without a cursor branch in ``_redirect_native_resume_if_needed``
    the resume fell through to the Codify REPL, which drove an Codify turn
    ...
    """
    from codify._wrapper_labels import CURSOR_NATIVE_WRAPPER_VALUE
    ...
    monkeypatch.setattr("codify.cursor_native.run_cursor_native", _fake_run_cursor_native)
...
def test_redirect_native_resume_covers_every_native_agent(
    ...
    goose/hermes/antigravity/qwen/opencode resume fell through to the Codify
    ...
    """
    from codify._wrapper_labels import GOOSE_NATIVE_WRAPPER_VALUE
...
def test_cursor_native_resume_never_drives_an_codify_turn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    duplicate had two sources: (1) the Codify turn the REPL drives, which
    ...
    def _fail_repl(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("_run_repl drove an Codify turn for a cursor-native resume")

    def _fail_one_shot(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("_run_one_shot drove an Codify turn for a cursor-native resume")
