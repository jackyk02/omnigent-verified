"""Unit tests for CLI OIDC token storage (codify/cli_auth.py).
...
def test_store_and_load_token(token_dir) -> None:
    ...
    This is the happy path: ``codify login`` stores a token,
    ``codify run --server`` loads it.
    """
    from codify.cli_auth import load_token, store_token
...
def test_load_returns_none_when_no_file(token_dir) -> None:
    ...
    The first time a user runs ``codify run --server`` without
    having run ``codify login``, there should be no crash.
    """
...
def test_clear_token(token_dir) -> None:
    ...
    """
    from codify.cli_auth import clear_token, load_token, store_token
...
def test_store_overwrites_existing(token_dir) -> None:
    ...
    Re-running ``codify login`` should update the token, not
    append.
    """
    from codify.cli_auth import load_token, store_token
...
def test_store_and_load_databricks_record(token_dir) -> None:
    ...
    ``codify login <apps-url>`` stores the record; the server-auth
    chain looks up the workspace host to mint fresh tokens.
    """
    from codify.cli_auth import load_databricks_workspace_host, store_databricks_auth
...
def test_databricks_request_headers_org_only(token_dir) -> None:
    ...
    """
    from codify.cli_auth import databricks_request_headers, store_databricks_auth
    ...
    assert databricks_request_headers("https://acme.databricks.com/api/2.0/codify") == {
        "X-Databricks-Org-Id": "2850744067564480"
    }

    store_databricks_auth(
        server_url="https://single.databricks.com/api/2.0/codify",
        workspace_host="https://single.databricks.com",
    )
    assert databricks_request_headers("https://single.databricks.com/api/2.0/codify") == {}
...
def test_databricks_request_headers_pairs_bearer_and_org(token_dir) -> None:
    ...
    recorded = "https://acme.databricks.com/api/2.0/codify"
...
def test_databricks_request_headers_folds_extra_headers(token_dir, monkeypatch) -> None:
    """CODIFY_DATABRICKS_EXTRA_HEADERS rides every request built via the helper.
    ...
    monkeypatch.setenv("CODIFY_DATABRICKS_EXTRA_HEADERS", "not-json")
    assert databricks_request_headers("https://other.example.com", bearer_token="tok") == {
        "Authorization": "Bearer tok",
    }
    # Unset (prod default) is a no-op.
    monkeypatch.delenv("CODIFY_DATABRICKS_EXTRA_HEADERS", raising=False)
    assert databricks_request_headers("https://other.example.com", bearer_token="tok") == {
        "Authorization": "Bearer tok",
    }
...
def test_load_token_returns_none_for_databricks_record(token_dir) -> None:
    ...
    """
    from codify.cli_auth import load_token, store_databricks_auth
...
def test_load_databricks_host_returns_none_for_jwt_record(token_dir) -> None:
    ...
    from codify.cli_auth import load_databricks_workspace_host, store_token
