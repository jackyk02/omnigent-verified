"""Tests for codify.cli — bundle env var resolution."""
...
from codify.errors import CodifyError
from codify.onboarding.ambient import DetectedProvider
...
def _restore_logging_state() -> Iterator[None]:
    ...
    """
    names = ("codify", "codify_ui_sdk", "httpx", "httpcore", "asyncio", "urllib3")
    snapshots = {}
    for name in names:
...
def test_python_module_entrypoint_uses_unified_click_cli() -> None:
    """
    ``python -m codify`` must dispatch through the same click CLI
    as the installed ``codify`` console script.

    This catches ``codify/__main__.py`` pointing at the legacy
    argparse CLI, which bypasses the Codify REPL path. In that broken
    state ``python -m codify run ...`` opens the old ``>``
    prompt and loses AP-only input features such as slash-command
    autocomplete and bracketed-paste abstraction.
    """
    result = subprocess.run(
        [sys.executable, "-m", "codify", "--help"],
        check=True,
        capture_output=True,
        text=True,
        timeout=20,
    ...
    assert "Usage: python -m codify [OPTIONS] COMMAND [ARGS]..." in result.stdout
    assert "Commands:" in result.stdout
    assert "run" in result.stdout and "Attach the REPL to a LIVE session" in result.stdout
    assert "Codify quick chat" not in result.stdout
...
def test_server_uvicorn_log_config_uses_terminal_handler_when_requested(
    ...
    monkeypatch.setattr("codify.process_logging.terminal_supports_color", lambda: True)
...
def test_server_uvicorn_log_config_mirrors_foreground_tty_by_default(
    ...
    """Foreground ``codify server`` keeps uvicorn access logs on stderr."""
    monkeypatch.delenv("CODIFY_LOG_TO_STDERR", raising=False)
...
def test_debug_logs_runner_session_reads_new_and_legacy_dirs(
    ...
    monkeypatch.setenv("CODIFY_DATA_DIR", str(data_dir))
...
def _fake_run_claude_native_capture(
    ...
    Shared by the ``codify claude`` CLI parsing tests below so a
    ...
    def _stub(**kwargs: object) -> None:
        ...
        :param kwargs: Whatever ``codify.claude_native.run_claude_native``
...
def _fake_run_codex_native_capture(
    ...
    Shared by ``codify codex`` parsing tests so wrapper signature
    ...
    def _stub(**kwargs: object) -> None:
        ...
        :param kwargs: Whatever ``codify.codex_native.run_codex_native``
            is called with.
        """
        captured.update(kwargs)
...
def test_claude_command_resume_binds_session_and_passes_unknown_args(
    ...
    """
    ``codify claude --resume <conv_id>`` binds the Codify
    ...
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr(
        "codify.claude_native.run_claude_native",
    ...
    assert result.exit_code == 0, result.output
    # ``--resume conv_abc`` binds the Codify conv id; everything
...
def test_claude_command_short_r_binds_codify_session(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    ``codify claude -r <conv_id>`` is the Codify resume shortcut.

    With the unified ``--resume`` UX, ``-r`` is the Codify alias
    (not Claude's own short flag). Users who need Claude's own
    resume can rely on the wrapper to translate the Codify conv
    id internally — see ``codify.claude_native._resolve_cold_resume_args``
    for the cold-resume injection.
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "codify.claude_native.run_claude_native",
        _fake_run_claude_native_capture(captured),
    )
...
def test_claude_command_profile_startup_threads_profiler(
    ...
    ``codify claude``: the profiler must be created in the Click
...
def test_claude_command_flag_is_deprecated(
    ...
    """``codify claude --command`` emits a DeprecationWarning pointing to env/config."""
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "codify.claude_native.run_claude_native",
    ...
    assert "CODIFY_CLAUDE_PATH" in result.output
...
def test_codex_command_resume_binds_session_and_passes_unknown_args(
    ...
    """
    ``codify codex --resume <conv_id>`` binds the Codify
    session and preserves Codex CLI passthrough args after ``--``.
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr(
        "codify.codex_native.run_codex_native",
...
def test_codex_command_bare_resume_requests_picker(
    ...
    """
    ``codify codex --resume`` requests the codex-native picker.
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
...
def test_codex_command_session_legacy_alias_routes_to_session_id(
    ...
    """
    ``codify codex --session <id>`` routes into ``session_id``.
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
...
def test_codex_command_env_var_passes_command_to_run_codex_native(
    ...
    """``CODIFY_CODEX_PATH`` forwards ``command`` to the runner."""
    captured: dict[str, object] = {}
    monkeypatch.setenv("CODIFY_CODEX_PATH", "/x/y")
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "codify.codex_native.run_codex_native",
...
def test_codex_command_honors_config_command_when_env_absent(
    ...
    captured: dict[str, object] = {}
    monkeypatch.delenv("CODIFY_CODEX_PATH", raising=False)
    monkeypatch.setattr(
        "codify.cli._load_effective_config",
...
def test_codex_command_env_var_overrides_config_command(
    ...
    """``CODIFY_CODEX_PATH`` env var beats ``harness.codex-native.command`` config."""
    captured: dict[str, object] = {}
    monkeypatch.setenv("CODIFY_CODEX_PATH", "/from/env")
    monkeypatch.setattr(
        "codify.cli._load_effective_config",
        lambda: {"harness": {"codex-native": {"command": "/from/config"}}},
    )
...
def test_antigravity_command_env_var_passes_command_to_run_antigravity_native(
    ...
    """``CODIFY_ANTIGRAVITY_PATH`` forwards ``command`` to the runner."""
    captured: dict[str, object] = {}
    monkeypatch.setenv("CODIFY_ANTIGRAVITY_PATH", "/x/y")
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "codify.antigravity_native.run_antigravity_native",
...
def test_antigravity_command_honors_config_command_when_env_absent(
    ...
    captured: dict[str, object] = {}
    monkeypatch.delenv("CODIFY_ANTIGRAVITY_PATH", raising=False)
    monkeypatch.setattr(
        "codify.cli._load_effective_config",
...
def test_pi_config_args_form_base_cli_args_append(
    ...
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
        "codify.pi_native.run_pi_native",
        lambda **kwargs: captured.update(kwargs),
    )
...
def test_kiro_command_is_registered_in_click_help() -> None:
    """``codify kiro`` is a true top-level Click command."""
...
def test_kiro_command_parses_native_options_and_prompt(
    ...
    """``codify kiro`` routes mapped options to the native Kiro runner."""
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", lambda: {"server": "https://cfg"})
    monkeypatch.setattr("codify.cli._ensure_backend", lambda server: server)
    monkeypatch.setattr(
        "codify.kiro_native.run_kiro_native",
...
def test_kiro_command_bare_resume_requests_picker(monkeypatch: pytest.MonkeyPatch) -> None:
    """``codify kiro --resume`` requests the Kiro-native picker."""
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli._ensure_backend", lambda *_: "http://localhost:0")
    monkeypatch.setattr(
...
def test_pi_config_command_threads_to_harness_path_env_var(
    ...
    """``harness.pi-native.command`` config sets ``CODIFY_PI_PATH`` before launch.

    The env-resolver harnesses (pi, cursor, kiro, goose, qwen, kimi, hermes)
    resolve their executable RUNNER-SIDE from ``CODIFY_<NAME>_PATH`` (the
    ...
    inherits it.
    """
    monkeypatch.setenv("CODIFY_PI_PATH", "")  # ensure teardown restores (CLI overwrites it)
    monkeypatch.setattr(
        "codify.cli._load_effective_config",
        lambda: {"harness": {"pi-native": {"command": "/custom/pi"}}},
    )
    monkeypatch.setattr("codify.cli._ensure_backend", lambda s: s or "http://localhost:1")
    monkeypatch.setattr("codify.pi_native.run_pi_native", lambda **kw: None)

    result = CliRunner().invoke(cli, ["pi"])

    assert result.exit_code == 0, result.output
    assert os.environ["CODIFY_PI_PATH"] == "/custom/pi"
...
def test_cli_warns_deprecated_harness_path_env_vars(
    ...
    from codify.cli import _warn_deprecated_harness_path_env_vars

    monkeypatch.setenv("HARNESS_CODEX_PATH", "/usr/local/bin/codex")
    monkeypatch.setenv("HARNESS_PI_PATH", "/custom/pi")
    monkeypatch.delenv("CODIFY_CODEX_PATH", raising=False)
    monkeypatch.delenv("CODIFY_PI_PATH", raising=False)
    ...
    assert "set CODIFY_CODEX_PATH instead" in out
    assert "HARNESS_PI_PATH is deprecated" in out
    assert "set CODIFY_PI_PATH instead" in out
...
# ── bundled-agent shorthands (codify polly / codify debby) ──────────


def test_first_run_plan_default_is_bundled_best_of_n(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Bare ``codify`` (Claude creds) launches the bundled Best-of-N agent.

    Pins the default-mode contract: the first-run plan's default agent is
    the bundled ``best_of_n`` (multi-agent auto-verify) directory.
    """
    monkeypatch.setattr(
        "codify.onboarding.provider_config.default_provider_for_harness",
        _fake_provider_for("claude-sdk"),
    )
    plan = _pick_first_run_harness()
    assert plan is not None
    assert plan.agent == _bundled_example_path("best_of_n")
...
def _write_isolated_provider_config(
    ...
    """Write an isolated ``~/.codify/config.yaml`` with *providers*.

    :param config_home: Directory to use as ``$CODIFY_CONFIG_HOME``.
...
def test_start_cli_runner_process_uses_token_bound_runner_id(
    ...
    """
    captured: dict[str, object] = {}
    monkeypatch.setattr("codify.cli.secrets.token_urlsafe", lambda _size: "bind-token")
    ...
    monkeypatch.setattr("codify.cli.subprocess.Popen", _Proc)
    ...
    assert "CODIFY_RUNNER_TUNNEL_TOKEN" not in env
...
def test_start_cli_runner_process_binds_stable_local_runner_to_generated_token(
    ...
    """
    captured: dict[str, object] = {}
    monkeypatch.delenv("CODIFY_RUNNER_TUNNEL_TOKEN", raising=False)
    monkeypatch.setattr(
        "codify.cli.secrets.token_urlsafe",
...
def test_start_cli_runner_process_reports_captured_log_path(
    ...
    monkeypatch.setattr("codify.cli.subprocess.Popen", _ExitedProc)
...
def test_server_command_reads_tunnel_token_and_does_not_spawn_runner(
    ...
    """``codify server`` is a pure state server — no embedded runner.

    The server reads ``CODIFY_RUNNER_TUNNEL_TOKEN`` from the
    ...
    from codify.server import app as app_module
    ...
    monkeypatch.setenv("CODIFY_RUNNER_TUNNEL_TOKEN", "test-tunnel-token-abc")

    # On a loopback bind the `server` command reuses an already-running
    # local server (and registers itself in ~/.codify/local_server.pid).
    ...
    from codify.host import local_server as _local_server_mod
...
def test_server_with_explicit_db_does_not_reuse_canonical_server(
    ...
    canonical server via ~/.codify/local_server.pid) must apply ONLY
    to a *bare* loopback ``codify server``. The daemon and the e2e
    ...
    # A healthy canonical server EXISTS. A bare `codify server` would
...
def test_server_with_explicit_port_does_not_check_canonical_server(
    ...
    ``codify server --port <new>`` reuse and exit. Explicit port
    ...
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "0")
    monkeypatch.setenv("CODIFY_DATA_DIR", str(tmp_path / "data"))
    ...
    assert "Starting codify server on 127.0.0.1:44770" in result.output
...
def test_server_command_explicit_occupied_port_fails() -> None:
    ...
    then asks ``codify server`` for that exact port. The command
    ...
    assert "Starting codify server" not in result.output
...
def test_server_command_explicit_port_uses_bind_probe_not_connect_probe(
    ...
    assert f"Starting codify server on 127.0.0.1:{port}" in result.output
...
def test_expand_config_expands_executor_connection(
    ...
    just ``llm.connection``) or local ``codify run`` specs using the
...
def test_expand_config_unresolved_var_raises(
    ...
    """
    ``_expand_config_env_vars`` raises ``CodifyError``
    ...
    with pytest.raises(CodifyError, match="MISSING_KEY_12345"):
...
def test_resolve_bundle_missing_env_var_raises(
    ...
    """
    ``_resolve_bundle_env_vars`` raises ``CodifyError``
    ...
    with pytest.raises(CodifyError, match="NONEXISTENT_DEPLOY_KEY"):
...
def test_bundle_materializes_standalone_codify_yaml(tmp_path: Path) -> None:
    """
    ``_bundle`` wraps a standalone codify YAML file in a tarball.

    ``codify run <yaml> --server`` uploads the returned bytes
...
def test_bundle_missing_env_var_raises(
    ...
    """
    ``_bundle`` raises ``CodifyError`` when the agent
    ...
    with pytest.raises(CodifyError, match="NONEXISTENT_BUNDLE_KEY"):
        _bundle(tmp_path)
...
def test_preregister_agent_accepts_directory(tmp_path: Path) -> None:
    ...
    What breaks if this fails: ``codify server --agent my-agent/``
    regresses — the standard case every codify user exercises.
    """
...
def test_preregister_agent_accepts_codify_yaml_file(tmp_path: Path) -> None:
    """
    A standalone codify YAML file registers identically — the
    spec's ``name`` field becomes the agent name, and the tarball
    stored in the artifact store round-trips through
    ``_find_codify_yaml_in_dir`` to the same spec.

    What breaks if this fails: ``codify server --agent coding_supervisor.yaml``
    ...
    # The YAML's ``name`` field flows through the codify
...
def test_materialize_harness_launcher_file_writes_codify_yaml() -> None:
    """No-AGENT run materialization writes a standalone Codify YAML file."""
...
def test_run_from_openclaw_dispatches_ephemeral_acp_agent(
    ...
    from codify.runtime.workflow import _build_acp_spawn_env
    from codify.spec import load
    ...
    monkeypatch.setattr(
        "codify.onboarding.openclaw_config.discover_openclaw_agents",
        lambda: discovery,
    )
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    config_home = tmp_path / "codify-config"
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(config_home))
    run_chat = Mock()
    monkeypatch.setattr("codify.chat.run_chat", run_chat)
...
def test_run_from_openclaw_agent_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
    from codify.onboarding.openclaw_config import OpenClawDiscovery
...
def test_run_without_agent_drops_into_configure_when_unconfigured(
    ...
    mutates the real ``~/.codify`` config, and never launches a daemon.
    """
    # Empty config + no detectable provider before/after configure, so the
    # first-run plan resolves to "nothing configured".
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path))
    monkeypatch.setattr("codify.cli._load_effective_config", dict)
    monkeypatch.setattr("codify.cli_config._promote_global_auth_to_provider", Mock())
    monkeypatch.setattr("codify.cli_config._adopt_detected_providers", Mock(return_value=[]))
    monkeypatch.setattr(
        "codify.onboarding.provider_config.default_provider_for_harness",
    ...
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_installed",
        lambda _key: False,
    )
    # The configure picker would block on a real terminal; stub it.
    configure = Mock()
    monkeypatch.setattr("codify.cli._run_configure_harnesses_interactive", configure)
...
def test_run_without_agent_claude_alias_dispatches_generated_yaml_headlessly(
    ...
    """
    # Isolate from any real ~/.codify/config.yaml on the developer's machine
    # (config defaults and ambient creds must not leak into the generated YAML
    # or the dispatch kwargs asserted below).
    monkeypatch.setattr("codify.cli._load_global_config", dict)
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path))
    monkeypatch.setenv("CODIFY_DISABLE_KEYRING", "1")
    ...
    run_prompt = Mock()
    monkeypatch.setattr("codify.chat.run_chat", run_chat)
    monkeypatch.setattr("codify.chat.run_prompt", run_prompt)
...
@pytest.mark.parametrize("flag", ["--codify", "--no-sessions-api"])
...
def test_attach_without_server_errors_loud(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: None)
...
def test_attach_forwards_live_conversation_to_run_attach(
    ...
    # Isolate from the developer's real ~/.codify config (a configured
    ...
    monkeypatch.setattr("codify.cli._require_live_conversation", lambda **_kw: None)
    run_attach = Mock()
    monkeypatch.setattr("codify.chat.run_attach", run_attach)
...
def test_run_server_resume_by_id_forwards_to_run_attach(
    ...
    """
    # Isolate from the developer's real ~/.codify config so a configured
    ...
    monkeypatch.setattr("codify.cli._require_live_conversation", lambda **_kw: None)
    monkeypatch.setattr("codify.chat._redirect_native_resume_if_needed", lambda **_kw: False)
    run_attach = Mock()
    run_chat = Mock()
    monkeypatch.setattr("codify.chat.run_attach", run_attach)
    monkeypatch.setattr("codify.chat.run_chat", run_chat)
...
def test_run_server_resume_native_redirects_before_attach_preflight(
    ...
    The pre-attach liveness check is for Codify REPL co-drive. Native-wrapper
    sessions need to hand off to ``codify claude`` / ``codify codex``
    ...
    def _must_not_attach(**_kwargs: object) -> None:
        """Fail if direct-server resume reaches Codify attach after native redirect."""
        raise AssertionError("native resume should not call run_attach after redirect")

    monkeypatch.setattr("codify.chat._redirect_native_resume_if_needed", _redirect)
    monkeypatch.setattr("codify.cli._require_live_conversation", _must_not_preflight)
    monkeypatch.setattr("codify.chat.run_attach", _must_not_attach)
...
def test_load_global_config_uses_env_override(
    ...
    """
    ``CODIFY_CONFIG_HOME`` redirects the user config path.
...
def test_load_global_config_returns_empty_when_missing(
    ...
    """
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", tmp_path / "config.yaml")
...
def test_save_and_load_global_config_round_trips(
    ...
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", config_path)
...
def test_config_list_empty(
    ...
    """
    ``codify config list`` prints a no-defaults message when neither
    global nor project-level config files exist.

    :param monkeypatch: Pytest monkeypatch fixture.
    :param tmp_path: Temporary directory standing in for ~/.codify and cwd.
    """
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", tmp_path / "config.yaml")
    monkeypatch.setattr("codify.cli._load_local_config", dict)
    ...
    monkeypatch.setattr("codify.cli._print_credentials_by_harness", lambda: None)
...
def test_config_set_global_writes_file(
    ...
    """
    ``codify config set --global key=value`` persists the value so that
    ``_load_global_config`` returns it.

    :param monkeypatch: Pytest monkeypatch fixture.
    :param tmp_path: Temporary directory standing in for ~/.codify.
    """
    config_path = tmp_path / "config.yaml"
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", config_path)
...
def test_config_set_global_reports_effective_config_home(
    ...
    """
    ``CODIFY_CONFIG_HOME`` redirects both the write and the reported path.
...
def test_config_list_shows_saved_values(
    ...
    """
    ``codify config list`` prints all defaults that were previously
...
def test_config_list_dedups_when_cwd_is_config_home(
    ...
    path (``cwd/.codify/config.yaml``) resolves to the SAME file as the
    user-level path (``~/.codify/config.yaml``).  The defaults section
    ...
    """
    # Global config lives at ``<home>/.codify/config.yaml``; chdir to
    # that same home dir so the local loader reads the identical file.
    config_dir = tmp_path / ".codify"
    config_dir.mkdir()
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", config_dir / "config.yaml")
    ...
    # ``# ~/.codify/config.yaml`` literal and once under the resolved
...
def test_save_local_config_migrates_scalar_harness(
    ...
    local_path = tmp_path / ".codify" / "config.yaml"
    local_path.parent.mkdir(parents=True)
    local_path.write_text("harness: codex\n", encoding="utf-8")

    _save_local_config({"model": "x"})

    from codify.cli import _load_local_config
...
def test_config_unset_removes_key(
    ...
    """
    ``codify config unset --global server`` removes the key from
...
def test_config_unknown_key_raises_error(
    ...
    """
    ``codify config set --global unknown=value`` rejects keys that are
...
def test_config_set_profile_rejected_as_unknown_key(
    ...
    """
    ``codify config set profile=...`` fails with the unknown-key error.
...
def test_config_set_local_writes_project_config(
    ...
    """
    ``codify config set key=value`` without ``--global`` writes to
    ``.codify/config.yaml`` in the current directory.

    :param monkeypatch: Pytest monkeypatch fixture.
    :param tmp_path: Temporary directory used as a stand-in project root.
    """
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", tmp_path / "global.yaml")
...
# ---------------------------------------------------------------------------
# `codify run` picks up global config defaults
# ---------------------------------------------------------------------------
...
def test_run_applies_global_config_agent_default(
    ...
    """
    ``codify run`` (no AGENT arg) uses the ``default_agent`` key from
    ...
    monkeypatch.setattr("codify.cli._dispatch_run", fake_dispatch)
    monkeypatch.setattr("codify.cli._build_resume_parts", lambda: None)
    monkeypatch.setattr(
        "codify.cli._split_resume_value",
...
def test_run_cli_arg_overrides_global_config(
    ...
    """
    An explicit CLI arg on ``codify run`` takes precedence over the
...
def test_run_applies_auto_open_conversation_config(
    ...
    """
    ``codify run`` forwards the persisted browser-open setting.
...
def _capture_run_dispatch(
    ...
    """
    Wire ``codify run`` to capture dispatch kwargs without launching.

    Points the global config at an empty *tmp_path* file (so the test is
    isolated from the developer's real ``~/.codify/config.yaml``) and
...
def test_run_interactive_defaults_browser_open_on(
    ...
    """
    Interactive ``codify run`` opens the browser by default.
...
def test_run_headless_prompt_defaults_browser_open_off(
    ...
    """
    Headless ``codify run -p`` stays quiet by default.
...
def test_attach_applies_auto_open_conversation_config(
    ...
    """
    ``codify attach`` reads browser-open from config and forwards it to run_attach.
...
def test_claude_applies_auto_open_conversation_config(
    ...
    """
    ``codify claude`` forwards the persisted browser-open setting.
...
def test_codex_applies_auto_open_conversation_config(
    ...
    """
    ``codify codex`` forwards the persisted browser-open setting.
...
def test_run_bare_codify_with_harness_only_config(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """
    Bare ``codify`` with only ``harness`` in global config dispatches
...
def test_bare_codify_harness_flag_dispatches_to_run(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``codify --harness ...`` is shorthand for ``codify run --harness ...``."""
    ...
    monkeypatch.setattr(sys, "argv", ["codify", "--harness", "claude"])
...
def test_bare_codify_non_tty_shows_help(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Bare ``codify`` in a non-interactive shell (no TTY) shows help.
    ...
    monkeypatch.setattr(sys, "argv", ["codify"])
...
def test_bare_codify_tty_dispatches_to_run(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Bare ``codify`` on an interactive terminal behaves like ``codify run``.
    ...
    """
    from codify import cli as cli_module
...
def test_bare_codify_rejects_positional_server_url(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["codify", "http://localhost:8000"])
    ...
    assert "codify run --server http://localhost:8000" in terminal.err
...
def test_setup_command_replaces_wizard(monkeypatch: pytest.MonkeyPatch) -> None:
    """``codify setup`` is the visible standard setup flow command."""
    configure_flow = Mock()
    configure_databricks = Mock()
    run_onboarding = Mock(return_value=True)
    monkeypatch.setattr(
        "codify.cli._run_configure_harnesses_interactive",
        configure_flow,
    )
    monkeypatch.setattr("codify.cli._run_configure_databricks", configure_databricks)
    monkeypatch.setattr("codify.onboarding.setup.run_onboarding", run_onboarding)
...
def test_setup_uses_compact_branding_on_short_terminals(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._run_configure_harnesses_interactive", configure_flow)
    monkeypatch.setattr("codify.inner.ui.print_landing", print_landing)
    monkeypatch.setattr("codify.inner.ui.print_brandmark", print_brandmark)
    monkeypatch.setattr(
        "codify.cli.shutil.get_terminal_size",
...
def test_setup_keeps_full_landing_on_tall_terminals(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 0, result.output
    print_landing.assert_called_once_with(tagline="coding, verified")
...
def test_node_dependency_problem_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.cli.shutil.which", lambda _: None)
...
def test_node_dependency_problem_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli.shutil.which", lambda _: "/usr/bin/node")
    monkeypatch.setattr(
        "codify.cli.subprocess.run",
...
def test_node_dependency_problem_probe_inconclusive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli.subprocess.run", _boom)
...
def test_warn_missing_harness_dependencies_silent_when_present(
    ...
    monkeypatch.setattr("codify.cli.shutil.which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr(
        "codify.cli.subprocess.run",
        _fake_node_run("v22.14.0", probe_returncode=0),
    )
...
def test_warn_missing_harness_dependencies_lists_all_gaps(
    ...
    monkeypatch.setattr("codify.cli.shutil.which", _which)
    monkeypatch.setattr(
        "codify.cli.subprocess.run",
        _fake_node_run("v20.12.2", probe_returncode=1),
    )
...
def test_click_subcommands_allowlist_covers_registered_commands() -> None:
    ...
    entrypoint — exactly the bug where ``codify configure`` errored with
    ...
    """
    from codify.cli import _CLICK_SUBCOMMANDS, cli
...
# ── first-run smart defaults (codify run) ──────────
...
def test_pick_first_run_prefers_claude_with_best_of_n(monkeypatch: pytest.MonkeyPatch) -> None:
    """Claude configured → claude-sdk + the bundled Best-of-N agent.

    Claude wins the priority order and is the only family that gets a default
    *example* agent (best_of_n). A regression that dropped it or picked the
    ...
    assert plan.agent is not None and plan.agent.endswith("best_of_n")
...
def test_resolve_first_run_plan_does_not_persist_derived_default(
    ...
    current creds (and promote them to Best-of-N). Asserts the resolved plan
    is Claude→best_of_n yet no global ``harness`` / ``default_agent`` was
    written.
    """
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path))
    monkeypatch.setattr("codify.cli_config._promote_global_auth_to_provider", Mock())
...
def test_resolve_first_run_plan_re_derives_when_creds_change(
    ...
    """Adding Claude promotes a Codex-only user to Best-of-N on the next bare run.
    ...
    *current* creds: Codex-only → a bare codex REPL; after Claude is added →
    claude-sdk + best_of_n (our primary). A regression that re-persisted the
    first pick would pin the user to codex and fail the second half.
    """
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path))
    monkeypatch.setattr("codify.cli_config._promote_global_auth_to_provider", Mock())
    monkeypatch.setattr("codify.cli_config._adopt_detected_providers", Mock(return_value=[]))
    ...
    # 2) Claude added (now both configured) → promoted to claude-sdk +
    #    best_of_n, NOT pinned to the earlier codex pick.
    monkeypatch.setattr(
        "codify.onboarding.provider_config.default_provider_for_harness",
    ...
    assert second.agent is not None and second.agent.endswith("best_of_n")
...
def test_announce_auto_configured_credentials_names_creds_compactly(
    ...
    monkeypatch.setattr("codify.onboarding.ambient.detect_providers", lambda: detected)
...
def test_adopt_ambient_credentials_announces_only_what_was_adopted(
    ...
    monkeypatch.setattr("codify.cli_config._promote_global_auth_to_provider", Mock())
    monkeypatch.setattr(
        "codify.cli_config._adopt_detected_providers", Mock(return_value=["anthropic"])
    )
    monkeypatch.setattr(
        "codify.onboarding.ambient.detect_providers",
        lambda: [
            DetectedProvider(
                name="anthropic", kind="key", family="anthropic", source="$ANTHROPIC_API_KEY"
            )
...
def test_ensure_sqlite_parent_dir_noop_for_memory_and_non_sqlite(tmp_path: Path) -> None:
    ...
    _ensure_sqlite_parent_dir("postgresql://user:pw@db.example.com:5432/codify")
...
def test_native_terminal_dispatch_specs_cover_registered_native_agents() -> None:
    from codify.harness_plugins import native_agents
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "opencode-native",
            "codify.opencode_native.run_opencode_native",
            {"extra_args": (), "model": "native-model"},
        ),
        (
            "cursor-native",
            "codify.cursor_native.run_cursor_native",
            {"extra_args": ("--model", "native-model")},
        ),
        (
            "kimi-native",
            "codify.kimi_native.run_kimi_native",
            {"extra_args": ("--model", "native-model")},
        ),
        (
            "kiro-native",
        ...
        (
            "goose-native",
            "codify.goose_native.run_goose_native",
            {"extra_args": ("--model", "native-model")},
        ),
        (
            "antigravity-native",
        ...
        (
            "qwen-native",
            "codify.qwen_native.run_qwen_native",
            {"extra_args": ("--model", "native-model")},
        ),
        (
            "hermes-native",
            "codify.hermes_native.run_hermes_native",
            {"extra_args": ("--model", "native-model")},
        ),
    ],
)
def test_dispatch_native_terminal_harness_launches_registered_wrapper(
    ...
    monkeypatch.setattr("codify.cli._ensure_backend", lambda _s: "http://localhost:0")
    captured: dict[str, object] = {}
...
def test_dispatch_native_terminal_harness_cursor_launches_wrapper(
    ...
    drove an Codify turn (which persists its own user item) on top of the
...
@pytest.mark.parametrize(
    ...
    [
        ("goose-native", "codify.goose_native.run_goose_native", "extra_args"),
        ("qwen-native", "codify.qwen_native.run_qwen_native", "extra_args"),
        ("hermes-native", "codify.hermes_native.run_hermes_native", "extra_args"),
    ],
)
@pytest.mark.parametrize(
...
def test_dispatch_native_terminal_harness_ignores_non_native(
    ...
    monkeypatch.setattr("codify.cli._ensure_backend", _must_not_run)
...
def test_dispatch_native_terminal_harness_continue_resumes_latest(
    ...
    monkeypatch.setattr("codify.chat._resolve_latest_conversation_id", _fake_latest)
    monkeypatch.setattr("codify.chat._remote_headers", lambda **_kw: {})
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "codify.cursor_native.run_cursor_native",
        lambda **kwargs: captured.update(kwargs),
...
def test_dispatch_native_terminal_harness_continue_with_no_prior_fails_loud(
    ...
    """
    monkeypatch.setattr("codify.cli._ensure_backend", lambda _s: "http://localhost:0")
    monkeypatch.setattr("codify.chat._resolve_latest_conversation_id", lambda **_kw: None)
    monkeypatch.setattr("codify.chat._remote_headers", lambda **_kw: {})

    def _must_not_launch(**_kw: object) -> None:
        raise AssertionError("wrapper launched despite no conversation to continue")

    monkeypatch.setattr("codify.cursor_native.run_cursor_native", _must_not_launch)
...
def test_dispatch_native_terminal_harness_explicit_id_skips_latest_lookup(
    ...
    monkeypatch.setattr("codify.chat._resolve_latest_conversation_id", _must_not_lookup)
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "codify.cursor_native.run_cursor_native",
        lambda **kwargs: captured.update(kwargs),
...
# ── codify setup: Qwen Code drill-in (_manage_qwen_harness) ────────────
...
def test_manage_qwen_harness_declines_install_returns(
    ...
    """
    import codify.onboarding.harness_install as hi
    import codify.onboarding.interactive as it
    ...
    launch = Mock()
    monkeypatch.setattr("codify.cli_config._launch_qwen_auth", launch)
...
def test_manage_qwen_harness_back_does_not_launch(
    ...
    monkeypatch.setattr("codify.cli_config._qwen_auth_configured", lambda: False)
    monkeypatch.setattr(it, "console", Mock())
...
# ── codify setup: Hermes drill-in (_manage_hermes_harness) ─────────────
...
# ── codify setup: Goose drill-in (_manage_goose_harness) ───────────────
...
def test_manage_goose_harness_missing_cli_shows_hint_returns(
    ...
    monkeypatch.setattr(it, "console", Mock())
    launch = Mock()
    monkeypatch.setattr("codify.cli_config._launch_goose_configure", launch)
...
def test_manage_goose_harness_back_does_not_launch(
    ...
    import codify.onboarding.goose_auth as ga
    import codify.onboarding.harness_install as hi
    import codify.onboarding.interactive as it
...
# ── codify setup: Kimi Code drill-in (_manage_kimi_harness) ────────────
