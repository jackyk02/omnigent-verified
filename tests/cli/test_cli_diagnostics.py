...
from codify import cli_diagnostics
...
def _capture_logger_snapshots() -> dict[str, _LoggerSnapshot]:
    ...
    for name in ("", "codify", "codify_ui_sdk", "databricks.sdk"):
...
def test_setup_cli_logging_honors_debug_level(
    ...
    """``CODIFY_LOG_LEVEL=DEBUG`` makes debug records reach cli logs."""
    del isolated_cli_diagnostics
    monkeypatch.setenv("CODIFY_LOG_LEVEL", "DEBUG")
    ctx = cli_diagnostics.setup_cli_logging(["run", "agent.yaml"])

    logging.getLogger("codify.test").debug("debug-visible")
...
def test_main_logs_click_exceptions(
    ...
    # raises before any daemon/network work. (A bare `codify run` no longer
    # errors — it drops into first-run `configure harnesses` — so it can't be
    # the trigger here.)
    monkeypatch.setattr(sys, "argv", ["codify", "run", "--harness", "not-a-real-harness"])
...
async def test_slash_command_exceptions_reach_cli_log(
    ...
    from codify.repl._repl import handle_slash_command
...
def test_prune_old_logs_survives_file_vanishing_mid_sort(
    ...
    """Concurrent ``codify run`` launches race to prune the same logs; a file
