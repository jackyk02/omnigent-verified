...
from codify.cli import _build_external_routing_client, _build_local_llm_routing_client
from codify.server.smart_routing import ExternalRoutingClient, LLMRoutingClient
...
def test_external_api_key_expands_env(monkeypatch: Any) -> None:
    ...
    monkeypatch.setenv("CODIFY_TEST_ROUTING_KEY", "sekret")
    cfg = {
        "provider": "external",
        "base_url": "https://host/v1",
        "router_name": "task_v0",
        "api_key": "${CODIFY_TEST_ROUTING_KEY}",
    }
    client = _build_external_routing_client(cfg)
    assert isinstance(client, ExternalRoutingClient)
...
def test_llm_builds_client() -> None:
    ...
    with (
        patch(
            "codify.runtime.policies.builder._resolve_server_llm_connection",
            return_value={"base_url": "b", "api_key": "k"},
        ),
        patch(
            "codify.runtime.policies.builder._build_policy_llm_client",
