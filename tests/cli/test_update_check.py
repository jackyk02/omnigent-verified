"""Tests for :mod:`codify.update_check`."""
...
def test_find_repo_root_no_git_integration(
    ...
    fake_file = tmp_path / "codify" / "update_check.py"
    fake_file.parent.mkdir(parents=True)
    fake_file.write_text("")

    import codify.update_check as mod
...
def test_find_repo_root_ignores_unrelated_ancestor_git(
    ...
    codify was installed under ``~/.local/share/uv/tools/`` —
...
def test_find_repo_root_requires_pyproject_alongside_git(
    ...
    """``.git/`` next to ``codify/`` without ``pyproject.toml`` → None.

    Defense in depth against a hypothetical layout where someone
    has a directory tree like ``some_repo/codify/`` (e.g. a
    ...
    happens to contain a folder called ``codify/``.
    """
    repo_like = tmp_path
    (repo_like / ".git").mkdir()
    # Deliberately NO pyproject.toml here.
    pkg_dir = repo_like / "codify"
    pkg_dir.mkdir()
    fake_file = pkg_dir / "update_check.py"
    fake_file.write_text("")
...
def test_find_repo_root_accepts_git_plus_pyproject(
    ...
    """``.git/`` AND ``pyproject.toml`` directly above ``codify/`` → repo root."""
    repo = tmp_path / "codify"
    repo.mkdir()
    (repo / ".git").mkdir()
    ...
    monkeypatch.setattr(mod, "__file__", str(fake_file))
    # Returns exactly the repo root — the parent of codify/.
...
def test_read_cache_returns_none_when_missing(
    ...
    monkeypatch.setattr("codify.update_check._CACHE_FILE", tmp_path / "nope.json")
...
def test_read_cache_returns_none_on_corrupt_json(
    ...
    monkeypatch.setattr("codify.update_check._CACHE_FILE", cache_file)
    assert _read_cache() is None
...
def test_write_then_read_cache_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.update_check._CACHE_DIR", tmp_path)
    monkeypatch.setattr("codify.update_check._CACHE_FILE", cache_file)
...
def test_fetch_and_count_git_not_found(tmp_path: Path) -> None:
    ...
    with patch("codify.update_check.subprocess.run", side_effect=FileNotFoundError):
...
def test_fetch_and_count_fetch_fails(tmp_path: Path) -> None:
    ...
    with patch(
        "codify.update_check.subprocess.run",
...
def test_fetch_and_count_success(tmp_path: Path) -> None:
    ...
    with patch("codify.update_check.subprocess.run", side_effect=fake_run):
...
def test_skipped_when_env_set(
    ...
    """No-op when ``CODIFY_NO_UPDATE_CHECK`` is set."""
    monkeypatch.setenv("CODIFY_NO_UPDATE_CHECK", "1")
    maybe_show_update_notice()
...
def test_no_repo_root_routes_to_wheel_check(
    ...
    """
    monkeypatch.delenv("CODIFY_NO_UPDATE_CHECK", raising=False)
    wheel_called = False
    ...
    monkeypatch.setattr("codify.update_check._run_installed_wheel_check", _stub_wheel_check)
    with patch("codify.update_check._find_repo_root", return_value=None):
        maybe_show_update_notice()
...
def test_fresh_cache_shows_notice(
    ...
    with patch("codify.update_check._find_repo_root", return_value=tmp_path):
        maybe_show_update_notice()
...
def test_fresh_cache_clears_after_pull(
    ...
    with (
        patch("codify.update_check._find_repo_root", return_value=tmp_path),
        patch("codify.update_check._get_head_sha", return_value="new_sha"),
        patch("codify.update_check._local_rev_list_count", return_value=0),
    ):
        maybe_show_update_notice()
...
def test_stale_cache_triggers_check(
    ...
    with (
        patch("codify.update_check._find_repo_root", return_value=tmp_path),
        patch("codify.update_check.subprocess.run", side_effect=fake_run),
    ):
        maybe_show_update_notice()
...
from codify.update_check import (  # noqa: E402
...
def _block_build_info_import(monkeypatch: pytest.MonkeyPatch) -> None:
    """Make ``from codify import _build_info`` fail in every test.
    ...
    1. ``sys.modules["codify._build_info"] = None`` — Python's
       documented "this import raises ImportError" sentinel.
    2. ``delattr(codify, "_build_info")`` — once a previous test
       has done ``from codify import _build_info`` successfully
       (via its own ``sys.modules`` override with a fake module),
       Python *also* sets ``_build_info`` as an attribute on the
       ``codify`` package. Subsequent ``from codify import
    ...
    codify.update_check import _read_build_info`` inside a test
    body would resolve to the stubbed module attribute).
    """
    monkeypatch.setitem(sys.modules, "codify._build_info", None)
    ...
    monkeypatch.delattr("codify._build_info", raising=False)
...
def _no_real_background_refresh(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.update_check._spawn_background_refresh", lambda: None)
...
_FAKE_GIT_URL = "git+https://github.com/example-org/codify.git"
...
def _write_fake_dist_info(
    ...
    *,
    ...
    """
    dist_info = tmp_path / "codify-0.1.0.dist-info"
    dist_info.mkdir()
...
def test_read_wheel_info_uv_git_install(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.update_check._get_distribution", lambda: dist)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "git+ssh://****@github.com/codify-ai/codify.git",
            "git+ssh://git@github.com/codify-ai/codify.git",
        ),
        # Same redaction, but the URL was stored without the ``git+``
        # VCS prefix (the shape uv wrote on the machine in the report).
        (
            "ssh://****@github.com/codify-ai/codify.git",
            "ssh://git@github.com/codify-ai/codify.git",
        ),
...
def test_read_wheel_info_repairs_redacted_ssh_user(
    ...
    """
    redacted_url = "ssh://****@github.com/codify-ai/codify.git"
    dist = _write_fake_dist_info(
        tmp_path,
        installer="uv",
        direct_url={
    ...
    assert info.vcs_url == "git+ssh://git@github.com/codify-ai/codify.git"
    ...
    assert (
        suggestion.command
        == "uv tool install --reinstall git+ssh://git@github.com/codify-ai/codify.git"
    )
...
def test_read_wheel_info_pip_registry_install(
    ...
    """``pip install codify`` from PyPI: no direct_url, mtime fallback."""
...
def test_read_wheel_info_returns_none_when_not_installed(
    ...
    monkeypatch.setattr("codify.update_check._get_distribution", lambda: None)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("uv", None, "uv tool upgrade codify", True),
        ...
        ("custom_tool", _FAKE_GIT_URL, f"reinstall codify from {_FAKE_GIT_URL}", False),
        # Unknown installer with no source URL — honest fallback.
        # Must also be runnable=False.
        (None, None, "reinstall codify from your original source", False),
    ],
)
def test_build_upgrade_suggestion_matrix(
    ...
    try to ``subprocess.run`` the literal string "reinstall codify
...
def test_pip_upgrade_suggestions_use_running_interpreter(
    ...
    assert (
        ...
        == "/opt/venv/bin/python -m pip install -U codify"
    )
    assert (
...
def _point_cache_at(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.update_check._CACHE_DIR", tmp_path)
    monkeypatch.setattr("codify.update_check._CACHE_FILE", tmp_path / ".update_check.json")
...
def test_wheel_check_nags_when_newer_release_available(
    ...
    assert "codify 0.2.0 is out" in err
...
def test_wheel_check_bails_for_editable_install(
    ...
    spawned: list[bool] = []
    monkeypatch.setattr(
        "codify.update_check._spawn_background_refresh", lambda: spawned.append(True)
    )
    dist = _write_fake_dist_info(
        tmp_path,
        installer="uv",
...
def test_wheel_check_env_var_disables(
    ...
    """``CODIFY_NO_UPDATE_CHECK`` skips the wheel check entirely.
...
def test_is_newer_pep440_ordering() -> None:
    ...
    from codify.update_check import _is_newer
...
_INDEX_ENV_VARS = ("CODIFY_INDEX_URL", "UV_DEFAULT_INDEX", "UV_INDEX_URL", "PIP_INDEX_URL")
...
def _clear_index_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.update_check._index_from_uv_config", lambda: "")
    monkeypatch.setattr("codify.update_check._index_from_pip_config", lambda: "")
...
def test_fetch_latest_version_pep691_json(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from codify.update_check import fetch_latest_version
    ...
    assert captured["url"] == "https://pypi.org/simple/codify/"
...
def test_fetch_latest_version_from_files_when_no_versions_key(
    ...
    body = {
        "files": [
            {"filename": "codify-0.1.0-py3-none-any.whl"},
            {"filename": "codify-0.2.0.tar.gz"},
            {"filename": "codify-0.3.0rc1-py3-none-any.whl"},  # prerelease → excluded
...
def test_fetch_latest_version_html_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    html = (
        ...
        '<a href="/p/codify-0.1.0-py3-none-any.whl#sha256=a">'
        "codify-0.1.0-py3-none-any.whl</a>"
        '<a href="/p/codify-0.2.0.tar.gz#sha256=b">codify-0.2.0.tar.gz</a>'
        "</body></html>"
    )
    monkeypatch.setattr(
...
def test_build_upgrade_suggestion_allow_prerelease() -> None:
    ...
    assert (
        ...
        == f"{_pip_invocation()} install -U codify --pre"
    )
    # VCS install carries the flag too.
    assert (
        _build_upgrade_suggestion(
            _info("uv", vcs_url="git+https://x/codify.git"), allow_prerelease=True
        ).command
        == "uv tool install --reinstall git+https://x/codify.git --prerelease allow"
    )
...
def test_resolve_index_url_precedence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from codify.update_check import _resolve_index_url
    ...
    monkeypatch.setenv("CODIFY_INDEX_URL", "https://override.example/simple")
    assert _resolve_index_url() == "https://override.example/simple"

    # Multiple whitespace/comma-separated URLs → the first (primary) index.
    monkeypatch.setenv("CODIFY_INDEX_URL", "https://a.example/simple, https://b.example/simple")
...
def test_refresh_update_cache_writes_latest_and_preserves_notified(
    ...
    dist = _write_fake_dist_info(tmp_path, installer="uv")
    monkeypatch.setattr("codify.update_check._get_distribution", lambda: dist)
    monkeypatch.setattr("codify.update_check._find_repo_root", lambda: None)
    monkeypatch.setattr("codify.update_check.fetch_latest_version", lambda: "0.2.0")

    from codify.update_check import refresh_update_cache
...
def test_refresh_update_cache_noop_for_clone(
    ...
    monkeypatch.delenv("CODIFY_NO_UPDATE_CHECK", raising=False)
    _point_cache_at(tmp_path, monkeypatch)
    monkeypatch.setattr("codify.update_check._find_repo_root", lambda: tmp_path)

    def _must_not_fetch() -> str:
        raise AssertionError("PyPI fetch attempted in a dev clone")

    monkeypatch.setattr("codify.update_check.fetch_latest_version", _must_not_fetch)
...
def test_wheel_info_prefers_build_info_over_uv_cache(
    ...
    monkeypatch.setattr("codify.update_check._get_distribution", lambda: dist)
    monkeypatch.setattr(
        "codify.update_check._read_build_info",
...
def test_wheel_info_falls_back_to_uv_cache_when_build_info_missing(
    ...
    monkeypatch.setattr("codify.update_check._read_build_info", lambda: None)
...
def test_read_build_info_returns_none_when_module_missing() -> None:
    ...
    ``sys.modules["codify._build_info"]`` by setting it to
...
def test_read_build_info_returns_values_when_module_present(
    ...
    monkeypatch.setitem(sys.modules, "codify._build_info", fake_module)
...
def test_legacy_cache_without_kind_field_defaults_to_clone(
    ...
    Backward-compat for users whose ``~/.codify/.update_check.json``
...
def test_run_upgrade_command_invokes_subprocess(
    ...
    from codify.update_check import _run_upgrade_command
...
def test_run_upgrade_command_returns_minus_one_when_binary_missing(
    ...
    console = Console(stderr=True)
    code = _run_upgrade_command("uv tool upgrade codify", console)
...
def test_format_version_falls_back_to_bare_version_when_build_info_missing() -> None:
    """Without ``_build_info``, ``--version`` prints ``codify <ver>``.

    Source checkouts (and any wheel built without our setup.py hook)
    hit this path. The line must remain stable across releases —
    scripts that grep for "codify X.Y.Z" must keep working.
    """
    # The autouse fixture has already blocked sys.modules['codify._build_info']
    # via the None sentinel, so _read_build_info returns None.
    from codify.cli import _format_version

    out = _format_version()
    # Exact prefix match — if the format ever gains extra content
    # in the no-build-info case, scripts that look for "codify
    # X.Y.Z" at the start of the line still work.
    assert out.startswith("codify ")
...
def test_split_vcs_url_strips_prefix_and_separates_revision() -> None:
    ...
    from codify.update_check import _split_vcs_url

    assert _split_vcs_url("git+https://github.com/o/codify.git") == (
        "https://github.com/o/codify.git",
        None,
    )
    assert _split_vcs_url("git+https://github.com/o/codify.git@main") == (
        "https://github.com/o/codify.git",
        "main",
    )
...
def test_split_vcs_url_strips_pip_fragment() -> None:
    ...
    assert _split_vcs_url("git+https://github.com/o/codify.git#egg=codify") == (
        "https://github.com/o/codify.git",
        None,
    )
    assert _split_vcs_url("git+https://github.com/o/codify.git@main#subdirectory=pkg") == (
        "https://github.com/o/codify.git",
        "main",
    )
...
def test_split_vcs_url_ssh_userinfo_is_not_a_revision() -> None:
    ...
    assert _split_vcs_url("git+ssh://git@github.com/o/codify.git") == (
        "ssh://git@github.com/o/codify.git",
        None,
    )
    # …but a real trailing revision still parses, even with SSH userinfo.
    assert _split_vcs_url("git+ssh://git@github.com/o/codify.git@v1") == (
        "ssh://git@github.com/o/codify.git",
        "v1",
    )
