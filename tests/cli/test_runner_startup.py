"""Tests for ``codify._runner_startup`` (startup UX helpers).
...
def test_spinner_disabled_by_env_var(value: str) -> None:
    """Any truthy ``CODIFY_NO_SPINNER`` value disables the spinner.
...
def test_spinner_not_disabled_by_falsy_env_var() -> None:
    """``CODIFY_NO_SPINNER=0`` (or empty) leaves the spinner on.
...
def test_runner_startup_progress_plain_mode_prints_to_stderr(
    ...
    assert "codify: Starting local runner" in captured.err
    assert "codify: Waiting for runner to register" in captured.err
...
def test_runner_startup_progress_plain_mode_does_not_swallow_exceptions(
    ...
    # suppress it.
    assert "codify: Starting" in capsys.readouterr().err
...
def test_startup_phase_labels_avoid_internal_jargon(label: str) -> None:
    ...
    # vocabulary (it is the ``codify server`` they may run directly).
    jargon = {
        "daemon",
        "host",
        "runner",
