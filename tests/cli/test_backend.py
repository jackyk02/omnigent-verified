...
a daemon-started local Codify server. Covers ``_ensure_host_daemon`` (local vs
...
# ``codify.host.connect`` lazily, and the daemon-spawn tests below patch
...
import codify.host.connect  # noqa: F401
...
from codify.host.local_server import LocalServerStartup
...
def test_ensure_host_daemon_local_inherits_data_dir_and_db_uri(
    ...
    In local mode the daemon owns the local Codify server, so it must resolve the
    ...
    captured: dict[str, object] = {}
    _patch_daemon_spawn(monkeypatch, tmp_path, captured)
    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path / "iso"))
    monkeypatch.setenv("CODIFY_DATABASE_URI", "postgresql://u:pw@h/db")

    _ensure_host_daemon(None)

    env = captured["env"]
    assert isinstance(env, dict)
    assert env["CODIFY_CONFIG_HOME"] == str(tmp_path / "iso")
    assert env["CODIFY_DATABASE_URI"] == "postgresql://u:pw@h/db"


def test_build_host_daemon_env_local_preserves_server_credentials(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Local daemon env carries credentials needed by its Codify server.

    The daemon's local server is the process that performs LLM calls, so
    stripping ``OPENAI_*`` here makes default persistent ``codify run``
...
def test_ensure_host_daemon_respawns_on_config_drift(
    ...
    flipped ``CODIFY_AUTH_ENABLED``), the unit is torn down and a
...
def test_foreground_connect_registers_status_record(
    ...
    monkeypatch.setattr("codify.host.connect.run_host_process", _fake_run_host_process)
...
def test_foreground_connect_refuses_duplicate_live_daemon(
    ...
    monkeypatch.setattr(
        "codify.host.connect.run_host_process",
...
def _patch_foreground_host_local(
    ...
    *,
    ...
    :param run_host_process: Stub for ``run_host_process`` controlling how
        ...
        ``SystemExit``).
    :param spawned: Whether ``ensure_local_codify_server`` reports it spawned a
    ...
    monkeypatch.setattr("codify.host.connect.run_host_process", run_host_process)
...
def test_foreground_connect_local_prompts_and_stops_server_on_yes(
    ...
    stopped: list[bool] = []
    monkeypatch.setattr(cli, "stop_local_codify_server", lambda: stopped.append(True))
...
def test_foreground_connect_reused_server_omits_prompt(
    ...
    """Reusing a server we didn't spawn (e.g. ``codify server``) skips the prompt.
...
def test_host_stop_stops_sessions_before_daemon(
    ...
    def _fake_http_json(**kwargs: object) -> cli._HostHttpResult:
        """Record lifecycle requests and return minimal Codify responses."""
...
def test_ensure_backend_defaults_scheme_https(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = _ensure_backend("dbc-x.cloud.databricks.com/codify")

    # Scheme defaulted to https before the workspace expansion ran.
    assert seen == ["https://dbc-x.cloud.databricks.com/codify"]
    assert result == _expand_marker("https://dbc-x.cloud.databricks.com/codify")
...
def test_claude_command_routes_server_through_ensure_backend(
    ...
    """``codify claude --server ""`` resolves via ``_ensure_backend``.
...
def _capture_run_chat(monkeypatch: pytest.MonkeyPatch) -> dict[str, object]:
    ...
    monkeypatch.setattr("codify.chat.run_chat", _stub)
    return captured
...
def test_ensure_backend_databricks_preflight_hints_headless(
    ...
    assert "codify login https://myapp-1234.aws.databricksapps.com" in str(exc.value)
...
def test_databricks_preflight_non_interactive_overrides_tty(
    ...
    assert f"codify login {_HOST_DATABRICKS_SERVER}" in str(exc.value)
...
def test_host_non_interactive_flag_positional_shorthand(
    ...
    """``codify host <url> --non-interactive`` parses the flag with the shorthand."""
    preflight = _capture_preflight(monkeypatch)
    _patch_foreground_host(monkeypatch, tmp_path)
...
def test_host_remote_preflight_hints_headless(
    ...
    assert result.exit_code != 0
    assert f"codify login {_HOST_DATABRICKS_SERVER}" in result.output
...
# its ``/api/2.0/codify`` mount via ``_ensure_backend`` (covered above);
...
def _expand_marker(server: str) -> str:
    ...
    """
    return f"{server.rstrip('/')}/api/2.0/codify"
...
def test_resolve_attach_server_expands_explicit_workspace_url(
    ...
    hit the workspace web app and 404'd instead of the codify API.
    """
    seen: list[str] = []
    monkeypatch.setattr(cli, "_workspace_api_server_url", _recording_expander(seen))
    ...
    assert result == "https://ws.example.net/api/2.0/codify"
...
def test_resolve_attach_server_defaults_scheme_https(monkeypatch: pytest.MonkeyPatch) -> None:
    """``attach --server <ws>/codify`` (no scheme) is defaulted to https."""
    seen: list[str] = []
    monkeypatch.setattr(cli, "_workspace_api_server_url", _recording_expander(seen))

    result = _resolve_attach_server("dbc-x.cloud.databricks.com/codify", configured_server=None)
...
def test_resolve_host_server_expands_explicit_workspace_url(
    ...
    The daemon is registered under the expanded ``/api/2.0/codify`` URL,
...
def test_resolve_host_server_reads_config_and_expands(
    ...
    assert _resolve_host_server(None) == "https://ws.example.net/api/2.0/codify"
...
def test_resolve_host_server_defaults_scheme_and_accepts_codify(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``host`` subcommands accept a schemeless ``/codify`` workspace URL.

    The internal user guide's web URL omits the scheme and ends in
    ``/codify``; host must default it to https before expansion, just
    like ``codify login``.
    """
    seen: list[str] = []
    monkeypatch.setattr(cli, "_workspace_api_server_url", _recording_expander(seen))

    result = _resolve_host_server("dbc-x.cloud.databricks.com/codify")
...
def test_host_command_defaults_scheme_and_accepts_codify_web_url(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """``codify host --server <ws>/codify`` (no scheme) normalizes before connect.

    Pasting the guide's web URL (schemeless, ``/codify`` suffix) must
    ...
    result = CliRunner().invoke(
        cli_group, ["host", "--server", "dbc-x.cloud.databricks.com/codify"]
    )
    ...
    assert observed == [_expand_marker("https://dbc-x.cloud.databricks.com/codify")]


def test_resume_command_expands_server_url(monkeypatch: pytest.MonkeyPatch) -> None:
    """``codify resume <id> --server <workspace>`` expands before dispatch.
    ...
    captured: dict[str, object] = {}
    monkeypatch.setattr(
        "codify.resume_dispatch.run_resume",
    ...
    assert captured == {
        "target": "conv_abc123",
        "server": "https://ws.example.net/api/2.0/codify",
    }
...
def test_resume_command_defaults_scheme_https(monkeypatch: pytest.MonkeyPatch) -> None:
    """``codify resume --server <ws>/codify`` (no scheme) is defaulted to https."""
    seen: list[str] = []
    monkeypatch.setattr(cli, "_workspace_api_server_url", _recording_expander(seen))
    captured: dict[str, object] = {}
    monkeypatch.setattr(
    ...
    result = CliRunner().invoke(
        cli_group,
        ["resume", "conv_abc123", "--server", "dbc-x.cloud.databricks.com/codify"],
    )

    assert result.exit_code == 0, result.output
    assert seen == ["https://dbc-x.cloud.databricks.com/codify"]
    assert captured["server"] == _expand_marker("https://dbc-x.cloud.databricks.com/codify")
