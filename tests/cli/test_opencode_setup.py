...
import codify.cli_config as cli_config
from codify.cli import _load_global_config


@pytest.fixture
def _isolated_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """Point the global config at a tmp file so saves don't touch ``~/.codify``."""
    path = tmp_path / "config.yaml"
    monkeypatch.setattr("codify.cli._GLOBAL_CONFIG_PATH", path)
    return path
...
def test_list_models_parses_nonblank_lines(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_install_spec", lambda _key: _fake_spec()
    )
    monkeypatch.setattr(
...
def test_list_models_empty_when_cli_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_install_spec", lambda _key: None
    )
...
def test_set_default_model_persists_choice(
    ...
    monkeypatch.setattr("codify.onboarding.interactive.select", lambda *a, **k: 0)
...
def test_set_default_model_clear_unsets(
    ...
    monkeypatch.setattr("codify.onboarding.interactive.select", lambda *a, **k: 1)
...
def test_set_default_model_no_models_short_circuits(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.onboarding.interactive.select", _select)
    status = cli_config._set_opencode_default_model(current=None)
