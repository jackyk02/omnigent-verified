...
- explicit ``CODIFY_AUTH_PROVIDER`` → no implicit change;
- explicit ``CODIFY_AUTH_ENABLED=0`` → no implicit re-enable.
"""
...
from codify.cli import _apply_bind_auth_defaults
...
def test_loopback_respects_explicit_single_user_off(
    ...
    """An explicit CODIFY_LOCAL_SINGLE_USER=0 wins on loopback.

    setdefault must not clobber an operator's explicit "off".
    """
    monkeypatch.setenv("CODIFY_LOCAL_SINGLE_USER", "0")
    _apply_bind_auth_defaults("127.0.0.1")
    assert os.environ.get("CODIFY_LOCAL_SINGLE_USER") == "0"
    assert _stderr(capsys) == ""
...
def test_non_loopback_enables_accounts(host: str, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert os.environ.get("CODIFY_AUTH_ENABLED") == "1"
    # The single-user marker must NOT be set on a non-loopback bind.
    assert os.environ.get("CODIFY_LOCAL_SINGLE_USER") is None
...
def test_non_loopback_respects_explicit_auth_provider(
    ...
    """An explicit CODIFY_AUTH_PROVIDER prevents the auto-enable.
...
def test_non_loopback_respects_explicit_auth_enabled_zero(
    ...
    """An explicit CODIFY_AUTH_ENABLED=0 disables auth — no re-enable.
    ...
    assert os.environ.get("CODIFY_AUTH_ENABLED") == "0"
    assert _stderr(capsys) == ""
...
def test_non_loopback_with_existing_auth_enabled_no_double_warning(
    ...
    """CODIFY_AUTH_ENABLED=1 already set → no warning (it's not auto-enabled).
...
def test_non_loopback_empty_auth_provider_string_is_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    """An empty CODIFY_AUTH_PROVIDER ('') is treated as unset.
    ...
    """
    monkeypatch.setenv("CODIFY_AUTH_PROVIDER", "")
    _apply_bind_auth_defaults("0.0.0.0")
...
def test_non_loopback_with_oidc_resolves_to_oidc(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setenv("CODIFY_AUTH_ENABLED", "1")
    monkeypatch.setenv("CODIFY_OIDC_ISSUER", "https://idp.example.com")
