...
from codify.integration_daemon import DaemonRecord, IntegrationDaemon


@pytest.fixture
def data_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolate daemon state under a temp CODIFY_DATA_DIR."""
    monkeypatch.setenv("CODIFY_DATA_DIR", str(tmp_path))
    return tmp_path
...
def test_start_writes_record_detached(tmp_path: Path) -> None:
    d = IntegrationDaemon("slack", tmp_path)
    with mock.patch("codify.integration_daemon.subprocess.Popen") as popen:
        popen.return_value.pid = 777
        record = d.start(["python", "-m", "codify_slack"], {"A": "b"})
...
def test_slack_background_hint_when_not_installed(data_dir: Path) -> None:
    runner = CliRunner()
    with mock.patch("codify.cli._slack_installed", return_value=False):
        result = runner.invoke(cli, ["integration", "slack", "--background"])
    assert result.exit_code != 0
    assert "isn't installed" in result.output
    assert "codify-slack" in result.output
...
def test_slack_background_status_stop_lifecycle(data_dir: Path) -> None:
    runner = CliRunner()
    with (
        mock.patch("codify.cli._slack_installed", return_value=True),
        mock.patch("codify.integration_daemon.subprocess.Popen") as popen,
...
def test_slack_foreground_runs_subprocess(data_dir: Path) -> None:
    ...
    with (
        mock.patch("codify.cli._slack_installed", return_value=True),
        mock.patch("codify.cli.subprocess.run") as run,
    ):
