...
from codify.cli import _HostDaemonRecord, _SessionPagesResult, cli
from codify.host.local_server import LocalServerInfo, LocalServerStartup
...
def test_server_status_not_running(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._find_daemon_record", lambda target: None)
...
def test_server_status_running_reports_details(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._find_daemon_record", lambda target: _record())
    monkeypatch.setattr(
...
def test_server_status_json(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: LocalServerInfo(
            ...
            log_path=Path("/tmp/.codify/logs/server/server-ab12.log"),
        ),
    )
    monkeypatch.setattr("codify.cli._find_daemon_record", lambda target: None)
    monkeypatch.setattr(
    ...
    assert json.loads(result.output) == {
        ...
        "port": 8123,
        "url": "http://127.0.0.1:8123",
        "log_path": "/tmp/.codify/logs/server/server-ab12.log",
        "live_sessions": 0,
...
def test_server_status_text_reports_log_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: LocalServerInfo(
            ...
            port=8123,
            url="http://127.0.0.1:8123",
            log_path=Path.home() / ".codify" / "logs" / "server" / "server-ab12.log",
        ),
    )
    monkeypatch.setattr("codify.cli._find_daemon_record", lambda target: None)
    monkeypatch.setattr(
    ...
    assert "log: ~/.codify/logs/server/server-ab12.log" in result.output
...
def test_server_status_session_count_failure_is_graceful(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._fetch_session_pages", _boom)
...
def test_server_background_spawns(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.cli.ensure_local_codify_server",
...
def test_server_background_reuses(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: LocalServerStartup(
            url="http://127.0.0.1:8123",
            spawned=False,
            log_path=Path.home() / ".codify" / "logs" / "server" / "server-cd34.log",
        ),
    )
    ...
    assert "log: ~/.codify/logs/server/server-cd34.log" in result.output
...
def test_server_background_omits_log_when_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    A foreground ``codify server`` streams logs to its terminal, so a
...
def test_server_stop_stops_server_and_local_daemon(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: "http://127.0.0.1:8123")
    local_record = _record()
    monkeypatch.setattr(
        "codify.cli._find_daemon_record",
        lambda target: local_record if target == "local" else None,
    )
    terminated: list[_HostDaemonRecord] = []
    monkeypatch.setattr(
        "codify.cli._terminate_daemon",
        lambda record, *, force: terminated.append(record),
    )
    stop_server = Mock()
    monkeypatch.setattr("codify.cli.stop_local_codify_server", stop_server)
    monkeypatch.setattr("codify.cli.stop_untracked_local_server", lambda: None)
...
def test_stop_terminates_all_daemons_and_server(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._list_daemon_records", lambda: records)
    terminated: list[_HostDaemonRecord] = []
    monkeypatch.setattr(
        "codify.cli._terminate_daemon",
        lambda record, *, force: terminated.append(record),
...
def test_stop_nothing_running(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._list_daemon_records", list)
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: None)
    monkeypatch.setattr("codify.cli.stop_local_codify_server", Mock())
    monkeypatch.setattr("codify.cli.stop_untracked_local_server", lambda: None)
...
def test_stop_surfaces_failures_and_suggests_force(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli._terminate_daemon", _terminate)
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: None)
    monkeypatch.setattr("codify.cli.stop_local_codify_server", Mock())
    monkeypatch.setattr("codify.cli.stop_untracked_local_server", lambda: None)
...
def test_stop_clears_stale_legacy_host_pid(
    ...
    monkeypatch.setattr("codify.cli._HOST_PID_PATH", host_pid)
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: None)
    monkeypatch.setattr("codify.cli.stop_local_codify_server", Mock())
    monkeypatch.setattr("codify.cli.stop_untracked_local_server", lambda: None)
...
def test_stop_reports_untracked_orphan_server(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.cli.local_server_url_if_healthy", lambda: None)
    monkeypatch.setattr("codify.cli.stop_local_codify_server", Mock())
    monkeypatch.setattr("codify.cli.stop_untracked_local_server", lambda: 93359)
