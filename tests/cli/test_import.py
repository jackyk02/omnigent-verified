"""Tests for the top-level ``codify import`` command."""
...
from codify.session_import.models import SessionImportNotFoundError
...
def test_import_command_loads_local_session_and_posts_normalized_items(tmp_path: Path) -> None:
    """The CLI reads local history and submits only Codify item shapes."""
    ...
    with patch("codify.cli._resolve_attach_server", return_value=_BASE):
...
def test_import_command_accepts_opencode_export() -> None:
    ...
    with (
        patch("codify.cli._resolve_attach_server", return_value=_BASE),
        patch("codify.session_import.local._run_opencode_json", return_value=export),
    ):
        result = CliRunner().invoke(
            cli,
...
def test_import_command_reports_opencode_discovery_failure() -> None:
    ...
    with patch(
        "codify.session_import.local._run_opencode_json",
