"""Tests for ``codify setup --no-internal-beta`` (CLI model-provider config).
...
tmp ``~/.codify/config.yaml`` (isolated via ``CODIFY_CONFIG_HOME``)
and the secret store (forced to the file backend via
``CODIFY_DISABLE_KEYRING``). Each test asserts on the persisted YAML
...
from codify.onboarding import providers as provider_catalog
from codify.onboarding import secrets
...
def isolated_config(tmp_path, monkeypatch):
    ...
    Sets ``CODIFY_CONFIG_HOME`` so config and secrets land under
    *tmp_path*, ``CODIFY_DISABLE_KEYRING`` so the secret store uses the
    ...
    monkeypatch.setattr("codify.onboarding.ambient._ollama_reachable", lambda: False)
    monkeypatch.setattr("codify.onboarding.ambient._claude_login_detected", lambda: False)
    return tmp_path
...
def _harnesses_installed(monkeypatch):
    ...
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_login",
        lambda family: True,
    )
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_logout",
        lambda family: True,
    )
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_logged_in",
        lambda family: True,
    )
...
def test_configure_models_list_groups_configured_providers(isolated_config) -> None:
    """``codify config list`` renders each configured provider grouped by harness.
...
def test_add_menu_databricks_option_gated_on_extra(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.onboarding.configure_models.databricks_sdk_installed",
        lambda: False,
    )
    options = add_menu_options()
...
def test_configure_models_add_databricks_aborts_without_extra(
    ...
    monkeypatch.setattr(
        "codify.onboarding.databricks_config.databricks_sdk_installed",
        lambda: False,
    )
    ...
    monkeypatch.setattr(
        "codify.onboarding.setup.login_databricks_workspace",
        _login_must_not_run,
    )
...
def test_kind_glyph_uniform_display_width(kind: str) -> None:
    ...
    """
    from codify.inner.banner import _display_width
...
def test_add_subscription_aborts_when_login_fails(isolated_config, monkeypatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.onboarding.harness_install.harness_login", lambda family: False)
    stdin = "\n".join(["1", "1", "2", "q", "q"]) + "\n"  # Claude → +Add → subscription
    result = CliRunner().invoke(cli, ["setup", "--no-internal-beta"], input=stdin)
    assert result.exit_code == 0, result.output
    cfg = _config_yaml(isolated_config)
...
def test_remove_subscription_declined_keeps_it_and_login(isolated_config, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codify.onboarding.harness_install.harness_logout", _no_logout)
...
def test_render_listing_excludes_configured_subscription_clis(
    ...
    from codify.onboarding.configure_models import render_provider_listing
...
def test_promote_global_auth_backfills_databricks_for_existing_configs(isolated_config) -> None:
    ...
    """
    from codify.cli import _save_global_config
    from codify.cli_config import _promote_global_auth_to_provider
...
def _databricks_add_menu_index() -> int:
    ...
    """
    from codify.onboarding.configure_models import add_menu_options_for_family
    from codify.onboarding.provider_config import ANTHROPIC_FAMILY, DATABRICKS_KIND
...
def test_configure_harnesses_add_databricks_normalizes_url_and_persists(
    ...
    monkeypatch.setattr("codify.onboarding.setup.login_databricks_workspace", _fake_login)
    monkeypatch.setattr(
        "codify.onboarding.ucode_setup.configure_ucode_for_workspace", _fake_configure_ucode
    )
    monkeypatch.setattr("codify.onboarding.ucode_setup.ucode_workspace_exists", _fake_exists)
...
def test_configure_harnesses_add_databricks_fails_loud_when_ucode_records_no_state(
    ...
    monkeypatch.setattr(
        "codify.onboarding.ucode_setup.configure_ucode_for_workspace",
        lambda url, *, agents=None: None,
    )
    # ucode "succeeded" but left no state for this workspace.
    monkeypatch.setattr("codify.onboarding.ucode_setup.ucode_workspace_exists", lambda url: False)
...
def test_configure_harnesses_add_databricks_under_codex_scopes_to_codex(
    ...
    """
    from codify.onboarding.configure_models import add_menu_options_for_family
    from codify.onboarding.provider_config import DATABRICKS_KIND, OPENAI_FAMILY
    ...
    monkeypatch.setattr("codify.onboarding.ucode_setup.ucode_workspace_exists", lambda url: True)
...
def test_uninstalled_harness_shows_x_and_not_installed(isolated_config, monkeypatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_installed", lambda family: False
    )
...
def _capture_setup_overview(
    ...
    monkeypatch.setattr("codify.onboarding.interactive.select", _capture_select)
...
def test_overview_lists_all_harnesses_in_priority_order(isolated_config, monkeypatch) -> None:
    ...
    top-level rendering.
    """
    from codify.onboarding import interactive
...
def test_overview_lists_kiro_row(isolated_config, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_installed", lambda family: True
    )
    options, selectable, descriptions, _, _max_visible = _capture_setup_overview(monkeypatch)
    names = _overview_row_names(options, selectable)
    kiro = names.index("Kiro")
...
def test_overview_hermes_row_reflects_configured_model(isolated_config, monkeypatch) -> None:
    ...
    * a finished ``hermes model`` run (a concrete ``provider`` + ``default``
    ...
    monkeypatch.setattr("codify.onboarding.hermes_auth.hermes_cli_installed", lambda: True)
...
def test_overview_truncates_long_status_for_narrow_terminal(isolated_config, monkeypatch) -> None:
    ...
    from codify.onboarding import interactive
    from codify.onboarding.opencode_auth import OpenCodeAuthSummary

    monkeypatch.setattr(
        "codify.cli.shutil.get_terminal_size", lambda fallback: os.terminal_size((40, 24))
    )
    monkeypatch.setattr(
        "codify.onboarding.opencode_auth.opencode_auth_summary",
...
def test_overview_dispatches_to_correct_manager(
    ...
    called: list[str] = []
    monkeypatch.setattr(
        f"codify.cli_config.{manager_attr}", lambda *a, **k: called.append(manager_attr)
    )
...
def test_overview_status_color_distinguishes_missing_from_unconfigured(
    ...
    monkeypatch.setattr("codify._platform.resolve_cli_binary", lambda _name: None)
    options, selectable, _descriptions, _compact, _max_visible = _capture_setup_overview(
        monkeypatch
    )
    codex = options[_overview_row_names(options, selectable).index("Codex")]
...
def test_overview_descriptions_map_to_their_rows(isolated_config, monkeypatch) -> None:
    ...
    from codify.onboarding.goose_auth import GooseConfigSummary
    from codify.onboarding.hermes_auth import HermesConfigSummary
    from codify.onboarding.opencode_auth import OpenCodeAuthSummary

    monkeypatch.setattr("codify.onboarding.cursor_auth.cursor_sdk_installed", lambda: True)
    monkeypatch.setattr(
        "codify.onboarding.antigravity_auth.antigravity_sdk_installed", lambda: True
    )
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_installed",
        lambda family: family != GEMINI_FAMILY,
    )
    monkeypatch.setattr("codify.onboarding.copilot_auth.copilot_sdk_installed", lambda: True)
    monkeypatch.setattr(
        "codify.onboarding.opencode_auth.opencode_auth_summary",
        lambda: OpenCodeAuthSummary(installed=True, stored_providers=(), env_providers=()),
    )
    monkeypatch.setattr(
        "codify.onboarding.goose_auth.goose_config_summary",
    ...
    monkeypatch.setattr(
        "codify.onboarding.hermes_auth.hermes_config_summary",
...
def test_drill_into_uninstalled_installs_then_proceeds(isolated_config, monkeypatch) -> None:
    ...
    installed: list[str] = []
    monkeypatch.setattr(
        "codify.onboarding.harness_install.install_harness_cli",
...
def test_decline_install_returns_without_installing(isolated_config, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codify.onboarding.harness_install.install_harness_cli", _must_not_install)
...
def test_pi_add_menu_offers_keys_gateway_databricks_but_no_subscription() -> None:
    ...
    """
    from codify.onboarding.provider_config import PI_SURFACE
...
def test_configure_harnesses_pi_page_sets_explicit_pi_default(isolated_config) -> None:
    ...
    """
    from codify.onboarding.provider_config import default_provider_for_harness
...
def test_build_cli_config_provider_entry_shapes() -> None:
    ...
    """
    from codify.onboarding.configure_models import build_cli_config_provider_entry
...
def _cursor_sdk_present(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "codify.onboarding.cursor_auth.cursor_sdk_installed",
        lambda: True,
    )
...
def test_cursor_install_now_invokes_runner_without_index(
    ...
    """Choosing "install it now" shells the install with ``codify[cursor]``.
    ...
    monkeypatch.setattr("codify.onboarding.extra_install._is_uv_tool_install", lambda: False)
    monkeypatch.setattr("codify.onboarding.extra_install.shutil.which", lambda name: None)
    monkeypatch.setattr("codify.onboarding.cursor_auth.subprocess.run", _run)
...
def _antigravity_sdk_present(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "codify.onboarding.antigravity_auth.antigravity_sdk_installed",
        lambda: True,
    )
...
def test_antigravity_sign_in_runs_agy_auth_service(
    ...
    monkeypatch.setattr("codify.onboarding.harness_install.harness_login", login)
...
def test_antigravity_sign_in_skips_sdk_install_prompt(isolated_config, monkeypatch) -> None:
    ...
    prompt = Mock()
    monkeypatch.setattr("codify.cli_config._prompt_install_antigravity", prompt)
    login = Mock(return_value=True)
    monkeypatch.setattr("codify.onboarding.harness_install.harness_login", login)
...
def test_antigravity_overview_install_command_is_selection_only(
    ...
    assert "codify[antigravity]" in Text.from_markup(descriptions[antigravity]).plain
    assert "codify[antigravity]" not in Text.from_markup(options[antigravity]).plain
...
def _copilot_sdk_absent(monkeypatch):
    ...
    for var in ("COPILOT_GITHUB_TOKEN", "GH_TOKEN", "GITHUB_TOKEN"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr("codify.onboarding.copilot_auth.copilot_sdk_installed", lambda: False)
...
def test_copilot_overview_install_command_is_selection_only(
    ...
    soft-SDK-extra harness: the ``codify[copilot]`` install command is the
    ...
    assert "codify[copilot]" in Text.from_markup(descriptions[copilot]).plain
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "10",
            "codify.onboarding.copilot_auth.copilot_sdk_installed",
...
def test_antigravity_drillin_offers_install_when_sdk_missing(
    ...
    out = result.output
    assert "isn't installed" in out
    assert "codify[antigravity]" in out
...
def test_antigravity_install_now_invokes_runner_without_index(
    ...
    """Choosing "install it now" shells the install with ``codify[antigravity]``.
    ...
    monkeypatch.setattr("codify.onboarding.extra_install._is_uv_tool_install", lambda: False)
    monkeypatch.setattr("codify.onboarding.extra_install.shutil.which", lambda name: None)
    monkeypatch.setattr("codify.onboarding.antigravity_auth.subprocess.run", _run)
    ...
    assert len(calls) == 1, f"expected exactly one install invocation, got {calls}"
    argv = calls[0]
    assert "codify[antigravity]" in argv
    assert "install" in argv
    # No index URL / proxy is baked into committed code.
    assert not any("index" in part or "://" in part for part in argv)
...
def test_configure_harnesses_add_other_key_no_remaining_providers_aborts_cleanly(
    ...
    one option`` out of ``codify setup``. The add branch must detect the empty
    ...
    monkeypatch.setattr("codify.onboarding.configure_models.other_key_providers", list)
...
def test_credential_label_bedrock_not_duplicated() -> None:
    ...
    """
    from codify.onboarding.configure_models import credential_label
    from codify.onboarding.provider_config import BEDROCK_KIND
