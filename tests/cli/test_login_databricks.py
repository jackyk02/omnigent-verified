"""Tests for ``codify login`` against Databricks-fronted servers.
...
- workspace-hosted codify (the API proxy answers 401 with a
...
_WORKSPACE_API_URL = f"{_WORKSPACE}/api/2.0/codify"
...
def token_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    the user-level default (config.yaml, via CODIFY_CONFIG_HOME). Both
...
def test_login_workspace_hosted_401_uses_url_host(
    ...
    Hosted codify lives at ``https://<workspace>/api/2.0/codify`` —
    ...
    assert result.exit_code == 0, result.output
    # Keyed by the full server URL (with the /api/2.0/codify path) and
...
def test_login_apps_fails_loud_without_databricks_extra(
    ...
    assert "codify[databricks]" in result.output
...
def test_login_accounts_mode_not_hijacked_by_databricks_detection(
    ...
    codify accounts server has no such header and must keep its
...
def test_login_threads_org_id_through_workspace_login_and_verify(
    ...
    """
    from codify.cli_auth import load_databricks_org_id, load_databricks_workspace_host
...
def test_login_sets_default_server(monkeypatch: pytest.MonkeyPatch, token_dir: Path) -> None:
    ...
    Without this, a freshly-logged-in user who runs a bare ``codify`` is
...
def test_login_header_mode_sets_default_server(
    ...
    ``codify login <url>`` against a header-mode server needs no
    credentials (a proxy injects identity), but the user's intent — "make
    this my server" — is the same, so a bare ``codify`` afterwards
...
def test_login_accounts_mode_sets_default_server(
    ...
    fake = _FakeHttpx(responses=[_response(401, body={"login_url": "/login"})])
    _patch_login_env(monkeypatch, fake_httpx=fake)
    monkeypatch.setattr("codify.cli._accounts_login", lambda server: None)
...
def test_login_failure_leaves_default_server_unchanged(
    ...
    reach would strand every later bare ``codify`` on that dead URL.
    """
...
def test_azure_vanity_url_falls_back_to_probed_canonical_host(
    ...
    probed = _scripted_normalizer_httpx(
        monkeypatch,
        {
            ...
            f"{canonical_root}/api/2.0/codify/v1/me": _response(
                401, headers={"www-authenticate": 'DatabricksRealm realm="codify"'}
            ),
        },
    )
    ...
    assert result == f"{canonical_root}/api/2.0/codify"
...
def test_workspace_url_expands_bare_workspace_host(
    ...
    """A bare workspace URL expands to its /api/2.0/codify mount.
...
def test_workspace_url_leaves_apps_edge_alone(
    ...
    workspace, so a regression here would bolt /api/2.0/codify onto
    every app URL.
    """
...
def test_workspace_url_expands_when_mount_hidden_from_anonymous_probe(
    ...
    Azure workspace edges answer the anonymous /api/2.0/codify probe
    ...
    fake = _FakeHttpx(
        responses=[
            ...
            _response(404, headers={"server": "databricks"}),
            # Mount, authenticated: codify answers.
...
def test_workspace_url_hints_when_authed_probe_also_misses(
    ...
    A 404 with valid credentials means codify is genuinely not
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "dbc-a5d4177a-49dc.cloud.databricks.com/codify",
            "https://dbc-a5d4177a-49dc.cloud.databricks.com/codify",
        ),
...
def test_with_default_scheme(raw: str, expected: str) -> None:
    ...
    (``<ws>/codify``); defaulting to https lets it be pasted verbatim,
...
def test_resolve_server_url_defaults_scheme_and_expands(
    ...
    ``/codify`` web URL both reach the ``/api/2.0/codify`` mount over
    https.
    """
    ...
    assert cli_mod._resolve_server_url("example.databricks.com/codify") == _WORKSPACE_API_URL
...
def test_resolve_server_url_strips_query_and_expands(
    ...
    expansion — ``codify run --server https://<ws>/?o=<id>`` has to reach
    ``/api/2.0/codify`` (the selector rides via the login record /
...
def test_resolve_server_url_strips_query_on_full_mount(
    ...
    """A full ``/api/2.0/codify?o=`` URL returns the clean mount with no probe.
...
def test_workspace_url_expands_web_ui_path_to_api_mount(
    ...
    """The guide's web-UI URL (``<ws>/codify``) expands to the API mount.

    The internal user guide hands out ``<ws>/codify`` for the browser;
    a user who pastes it into ``codify login`` must reach the API mount
    (``/api/2.0/codify``), not 404 the UI path's own /v1/me probe.
    """
    ...
    assert cli_mod._workspace_api_server_url(f"{_WORKSPACE}/codify") == _WORKSPACE_API_URL
    # The bare root and its API mount were probed — never the UI path itself.
    assert f"{_WORKSPACE}/codify/v1/me" not in probed


def test_workspace_url_web_ui_path_left_alone_off_workspace(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A ``/codify`` URL on a non-workspace host is returned untouched.
    ...
    served under ``/codify`` still works.
    """
    ...
    assert (
        cli_mod._workspace_api_server_url("https://omni.example.com/codify")
        == "https://omni.example.com/codify"
    )
...
def test_login_defaults_scheme_to_https(monkeypatch: pytest.MonkeyPatch, token_dir: Path) -> None:
    ...
    result = CliRunner().invoke(cli_group, ["login", "example.databricks.com/api/2.0/codify"])
