"""Tests for the native Antigravity (``codify antigravity``) launcher.
...
import codify.antigravity_native as _mod
import codify.antigravity_native_bridge as bridge_mod
from codify._wrapper_labels import ANTIGRAVITY_NATIVE_WRAPPER_VALUE, WRAPPER_LABEL_KEY
from codify.antigravity_native import antigravity_terminal_resource_id
...
def _stub_agy_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (its own module), plus the re-export in :mod:`codify.antigravity_native` used
    ...
    """
    monkeypatch.setattr("codify.antigravity_native_launch.agy_binary_path", lambda: "agy")
...
async def test_local_fresh_launch_reattaches_to_runner_autocreated_terminal(
    ...
    ``codify antigravity`` path: the local server spawns a CLI runner, and
...
async def test_cli_cold_start_scopes_to_pane_agy(
    ...
    """
    import codify.antigravity_native_rpc as rpc
