"""Tests for codify.tools.manager (ToolManager)."""
...
from codify.tools.mcp import clear_discovery_cache
...
def test_schemas_isolate_a_failing_tool(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.tools.manager"):
        schemas = mgr.get_tool_schemas()
...
def test_advise_models_hidden_when_routing_disabled() -> None:
    ...
    with patch("codify.runtime._globals._caps", new=caps):
...
def test_client_tool_shadows_skill_tool(
    ...
    from codify.tools.client_specified import ClientSideTool
...
def test_client_tool_invalid_name_raises(
    ...
    ``CodifyError`` at registration time.
    """
    spec = _make_spec()
    with pytest.raises(CodifyError, match="Invalid client tool name"):
        ToolManager(
            spec,
