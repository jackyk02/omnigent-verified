...
from codify.tools.builtins.web_search_keenable import (
    _resolve_max_results as _resolve_max_results_keenable,
)
from codify.tools.builtins.web_search_nimble import _resolve_max_results
from codify.tools.builtins.web_search_tavily import (
...
def test_google_backend_via_spec_config(tool_ctx: ToolContext) -> None:
    ...
    with patch("codify.tools.builtins.web_search_google.httpx.get") as mock_get:
...
def test_perplexity_backend_via_spec_config(tool_ctx: ToolContext) -> None:
    ...
    with patch("codify.tools.builtins.web_search_perplexity.httpx.post") as mock_post:
...
def test_nimble_backend_via_spec_config(tool_ctx: ToolContext) -> None:
    ...
    with patch("codify.tools.builtins.web_search_nimble.httpx.post") as mock_post:
        mock_post.return_value = fake_response
...
def test_nimble_sends_x_client_source_header(tool_ctx: ToolContext) -> None:
    """Every request carries the ``X-Client-Source`` header identifying Codify."""
    fake_response = MagicMock()
    fake_response.json.return_value = {"results": []}
    ...
    headers = mock_post.call_args.kwargs["headers"]
    assert headers["X-Client-Source"] == "codify", (
        f"Expected X-Client-Source 'codify', got {headers.get('X-Client-Source')!r}"
    )
...
def test_tavily_backend_via_spec_config(tool_ctx: ToolContext) -> None:
    ...
    with patch("codify.tools.builtins.web_search_tavily.httpx.post") as mock_post:
        mock_post.return_value = fake_response
...
def test_no_search_provider_fails_loudly(
    ...
    must not be invoked.
    """
    import codify.tools.builtins.web_search_duckduckgo as ddg
...
def test_keenable_backend_via_spec_config(tool_ctx: ToolContext) -> None:
    ...
    with patch("codify.tools.builtins.web_search_keenable.httpx.post") as mock_post:
        mock_post.return_value = fake_response
...
def test_keenable_sends_x_keenable_title_header(tool_ctx: ToolContext) -> None:
    ...
    headers = mock_post.call_args.kwargs["headers"]
    assert headers["X-Keenable-Title"] == "Codify"
...
def test_openai_mode_is_not_async() -> None:
    """OpenAI passthrough mode should not enter Codify async dispatch."""
    tool = WebSearchTool(llm_provider="openai")
    assert tool.is_async() is False
