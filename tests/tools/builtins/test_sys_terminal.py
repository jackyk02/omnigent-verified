...
Per ``designs/CODIFY_TERMINAL_BRIDGE.md`` §8.2, these tests use the
...
async def _invoke(tool: Tool, payload: dict[str, object], ctx: ToolContext) -> dict:
    ...
    Mirrors production dispatch: ``codify/runtime/workflow.py`` calls
...
def test_cwd_resolution_uses_workspace_when_spec_cwd_is_dot(
    ...
    This is the load-bearing fix — under Codify mode, AP's process
    ...
    # rare cases the codify-testing skill rule-14 ("no
