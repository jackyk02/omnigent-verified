...
def _make_parent_spec(
    ...
    :param executor_type: Executor type override, or ``None``
        for default (codify executor on the claude-sdk harness).
    :returns: An AgentSpec suitable for constructing WebFetchTool.
    """
    # A real bootable ``type="codify"`` agent always carries a harness in
    # ``executor.config`` — without one ``harness_kind`` is the unspawnable
    # literal "codify". Default the helper to a real harness so fixtures build
...
def test_no_os_env_parent_fails_at_build_when_bwrap_missing(
    ...
    parent = _make_parent_spec()
    assert parent.os_env is None
    with pytest.raises(CodifyError, match="bubblewrap") as excinfo:
...
def test_no_os_env_parent_fails_at_build_when_sandbox_exec_missing(
    ...
    monkeypatch.setattr(shutil, "which", lambda cmd: None)
    with pytest.raises(CodifyError, match="sandbox-exec") as excinfo:
...
def test_web_fetch_is_runner_dispatched() -> None:
    ...
    ``codify/runner/tool_dispatch.py``. If a future change
...
def test_runner_handler_validates_query_required() -> None:
    ...
    from codify.runner.tool_dispatch import _execute_web_fetch_tool
...
def test_researcher_build_fails_loud_when_parent_has_no_harness() -> None:
    ...
    spec. A child carrying the literal ``"codify"`` would crash the runner
    with ``unknown harness 'codify'`` (the original Layer-1 failure). The
...
def test_researcher_inherits_parent_harness_auth_and_model() -> None:
    ...
    ``type`` to ``"codify"`` with an empty ``config``, so:

    - Layer 1 (active): ``executor.harness_kind`` resolves to the literal
      ``"codify"`` (no ``config["harness"]``), and the runner aborts the
      researcher spawn with ``RuntimeError: unknown harness 'codify'``
    ...
    # Layer 1: the child must NOT be the bare "unknown harness 'codify'"
    ...
    assert researcher.executor.harness_kind != "codify"
