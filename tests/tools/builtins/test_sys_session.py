...
monkeypatch the ``codify.runtime`` accessors so the tools see them.
...
def session_fixture(
    ...
    ``codify.runtime.get_conversation_store`` so the tool's
    ...
    # The tools' invoke() does ``from codify.runtime import
    # get_conversation_store`` per call (lazy bind), so patching the
    # runtime module's attribute is sufficient.
    monkeypatch.setattr("codify.runtime.get_conversation_store", lambda: conv_store)
...
def test_close_marks_closed_and_tombstones_internal_title(session_fixture: _Fixture) -> None:
    ...
    The explicit ``codify.closed=true`` label is the behavioral
