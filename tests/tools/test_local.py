"""Tests for codify.tools.local (LocalPythonTool subprocess execution)."""
...
from codify.spec.types import LocalToolInfo, SandboxConfig
from codify.tools.base import ToolContext
from codify.tools.local import (
    LocalPythonTool,
    LocalToolLoadError,
...
def test_sandbox_config_env_var_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """CODIFY_CONTAINER_RUNTIME env var overrides the built-in default."""
...
def test_sandbox_config_env_var_invalid(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_CONTAINER_RUNTIME", "rkt")
    with pytest.raises(ValueError, match="container_runtime"):
        SandboxConfig()
...
def test_build_command_container_env_var(
    ...
    """CODIFY_CONTAINER_RUNTIME env var is picked up by the tool command builder."""
    monkeypatch.setenv("CODIFY_CONTAINER_RUNTIME", "podman")
    tool = _make_tool(tmp_path, container_image="python:3.11")
    cmd = tool._build_command(state_root=None)
    assert cmd[0] == "podman"
...
def _run_runner_with_request(tool_path: Path, tool_name: str, arguments: dict) -> dict:
    ...
    """
    runner = Path(__file__).parent.parent.parent / "codify" / "tools" / "_runner.py"
    request = json.dumps(
        {
