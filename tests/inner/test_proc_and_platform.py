...
Covers codify._platform, codify.inner._proc, the harness IPC endpoint
...
def test_default_shell_argv_runs_an_echo() -> None:
    argv = _platform.default_shell_argv("echo codify-shell-ok")
    out = subprocess.run(argv, capture_output=True, text=True, check=True)
    assert "codify-shell-ok" in out.stdout
...
def test_endpoint_uds_variant_shape() -> None:
    from codify.runtime.harnesses.process_manager import _HarnessEndpoint
...
def test_windows_jobobject_is_platform_default() -> None:
    from codify.inner import sandbox
...
def test_windows_jobobject_kill_on_close_terminates_tree() -> None:
    ...
    from codify.inner.windows_jobobject_sandbox import WindowsJobObjectSandboxBackend
...
def test_parent_death_watchdog_does_not_false_fire_on_windows() -> None:
    ...
    from codify.runner._entry import _parent_is_orphaned
...
def test_host_runner_env_lets_child_import_asyncio_and_resolve_home() -> None:
    ...
    from codify.host.connect import _build_runner_env
...
def test_resolve_cli_binary_prefers_path(monkeypatch):
    monkeypatch.delenv("CODIFY_TESTCLI_PATH", raising=False)
    monkeypatch.setattr(
    ...
    assert _platform.resolve_cli_binary("tool", env_var="CODIFY_TESTCLI_PATH") == "/usr/bin/tool"
...
def test_resolve_cli_binary_env_override_wins(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CODIFY_TESTCLI_PATH", str(override))
    # PATH would resolve elsewhere, but the override takes precedence.
    monkeypatch.setattr(
        _platform.shutil, "which", lambda name: "/usr/bin/tool" if name == "tool" else None
    )
    assert _platform.resolve_cli_binary("tool", env_var="CODIFY_TESTCLI_PATH") == str(override)
...
def test_resolve_cli_binary_falls_back_to_global_dir(monkeypatch, tmp_path):
    ...
    assert _platform.resolve_cli_binary("tool", env_var="CODIFY_TESTCLI_PATH") == str(tool)
...
def test_resolve_cli_binary_returns_none_when_absent(monkeypatch, tmp_path):
    ...
    assert _platform.resolve_cli_binary("tool", env_var="CODIFY_TESTCLI_PATH") is None
...
def test_resolve_cli_binary_warns_on_bad_override(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setenv("CODIFY_TESTCLI_PATH", str(tmp_path / "does-not-exist"))
    monkeypatch.setattr(
        _platform.shutil, "which", lambda name: "/usr/bin/tool" if name == "tool" else None
    )
    monkeypatch.setattr(_platform, "_cli_fallback_dirs", lambda: ())
    with caplog.at_level("WARNING"):
        resolved = _platform.resolve_cli_binary("tool", env_var="CODIFY_TESTCLI_PATH")
    assert resolved == "/usr/bin/tool"
    assert "CODIFY_TESTCLI_PATH" in caplog.text
