"""
Tests for the OTel GenAI semantic-convention attributes on codify's
...
from codify.inner.tracing import TracingContext, enable_tracing
...
def test_content_capture_off_by_default_drops_input_and_output(
    ...
    """
    With CODIFY_OTEL_CAPTURE_CONTENT off (the default),
    input.value / output.value are NOT set on agent + tool spans.
    Metadata attrs (agent.name, tool.name, gen_ai.*) still are.
    """
    monkeypatch.setattr("codify.runtime.telemetry._capture_content", False)
...
def test_content_capture_on_includes_input_and_output(
    ...
    """
    With CODIFY_OTEL_CAPTURE_CONTENT on, input.value and output.value
    appear on agent + tool spans.
    """
    monkeypatch.setattr("codify.runtime.telemetry._capture_content", True)
