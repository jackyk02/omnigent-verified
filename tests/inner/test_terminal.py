"""Unit tests for :mod:`codify.inner.terminal`."""
...
import codify.inner.terminal as terminal_mod
...
def _write_transport_config(config_home: Path, value: str | None) -> None:
    ...
    :param config_home: Directory used as ``CODIFY_CONFIG_HOME``.
...
async def test_launch_disables_tmux_pane_and_window_creation_controls(
    ...
    create extra panes, windows, or sessions outside Codify' terminal
    registry.
...
async def test_launch_strips_env_unset_keys_from_inherited_environment(
    ...
    # ``CODIFY_TMUX_SOCK`` for this any more: the sandbox hardening
    # stopped ``launch`` from advertising the control-socket path to the pane.)
    monkeypatch.setenv("CODIFY_BENIGN_SENTINEL", "keep-me")
    # Seed an inherited CODIFY_TMUX_SOCK so the negative assertion
    ...
    monkeypatch.setenv("CODIFY_TMUX_SOCK", "/leaked/from/parent.sock")
    ...
    assert spawned_env.get("CODIFY_BENIGN_SENTINEL") == "keep-me", (
    ...
    assert "CODIFY_TMUX_SOCK" not in spawned_env, (
        "CODIFY_TMUX_SOCK leaked into the tmux child env — the pane "
...
def _write_instance_dir(root: Path, name: str, owner_pid: int | None) -> Path:
    ...
    :param name: Directory name, e.g. ``"codify-terminal-dead1"``.
...
def test_reap_orphaned_terminals_reaps_only_dead_owner_dirs(
    ...
    dead_dir = _write_instance_dir(tmp_path, "codify-terminal-dead1", _dead_pid())
    live_dir = _write_instance_dir(tmp_path, "codify-terminal-live1", os.getpid())
    unmarked_dir = _write_instance_dir(tmp_path, "codify-terminal-old1", None)
...
def test_reap_orphaned_terminals_kills_server_for_dead_owner_socket(
    ...
    dead_dir = _write_instance_dir(tmp_path, "codify-terminal-dead2", _dead_pid())
