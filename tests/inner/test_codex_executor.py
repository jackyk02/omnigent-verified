...
class TestCodexExecutor(unittest.TestCase):
    def test_databricks_codex_config_overrides(self):
        ...
        self.assertIn('model_provider="codify_databricks"', overrides)
    ...
    def test_codex_config_overrides_neutralize_toml_breakout(self):
        ...
        provider = parsed["model_providers"]["codify_databricks"]
        self.assertEqual(provider["auth"]["args"], ["-c", real_auth])

    def test_constructor_databricks_flag_with_profile(self):
        with (
            patch("codify.inner.codex_executor._find_codex_cli", return_value="/usr/bin/codex"),
            patch(
                "codify.inner.codex_executor._databricks_gateway_host",
        ...
        self.assertIn('model_provider="codify_databricks"', executor._codex_config_overrides[1])
    ...
    def test_constructor_databricks_flag_with_host_override_skips_profile_lookup(self):
        with (
            patch("codify.inner.codex_executor._find_codex_cli", return_value="/usr/bin/codex"),
            patch.dict("os.environ", {}, clear=True),
            patch("codify.inner.codex_executor._databricks_gateway_host") as gateway_host,
        ):
            executor = CodexExecutor(
                gateway=True,
    ...
    def test_constructor_databricks_flag_no_creds_raises(self):
        with (
            patch("codify.inner.codex_executor._find_codex_cli", return_value="/usr/bin/codex"),
            patch.dict("os.environ", {}, clear=True),
            patch("codify.inner.codex_executor._databricks_gateway_host", return_value=None),
        ):
    ...
    def test_close_uses_process_tree_termination(self):
        async def _t():
            ...
            with patch("codify.inner.codex_executor._terminate_process_tree") as terminate_tree:
                await session.close()
    ...
    def test_app_server_uses_workspace_cwd(self):
        async def _t():
            ...
            with tempfile.TemporaryDirectory() as workspace:
                ...
                with patch(
                    "codify.inner.codex_executor._create_subprocess_exec",
        ...
        async def _t():
            ...
            with tempfile.TemporaryDirectory() as workspace:
                ...
                with patch(
                    ...
                    self.assertTrue(codex_home.name.startswith("codify-codex-home-"))
    ...
    def test_app_server_run_turn_warns_and_keeps_waiting_when_events_are_slow(self):
        async def _t():
            ...
            import codify.inner.codex_executor as codex_executor_module
            ...
            try:
                ...
                with self.assertLogs("codify.inner.codex_executor", level="WARNING") as cm:
                    events = [
                        event
                        async for event in session.run_turn(
                            messages=[{"role": "user", "content": "hi"}],
    ...
    def test_app_server_run_turn_notifies_usage_observer(self):
        ...
        from codify.llms import _usage_observer
    ...
    def test_app_server_run_turn_reasoning_deltas_yield_reasoning_chunks(self):
        ...
        think phases (regression guard for codify-ai/codify#738)."""
...
def test_format_codex_error_params_extracts_provider_error_from_nested_error() -> None:
    ...
    """
    from codify.inner.codex_executor import _format_codex_error_params
...
def test_extract_codex_last_turn_usage_splits_cached_out_of_input() -> None:
    ...
    """
    from codify.inner.codex_executor import _extract_codex_last_turn_usage
...
def test_populate_codex_skills_all(tmp_path: Path) -> None:
    ...
    """
    from codify.inner.codex_executor import _populate_codex_skills
...
def test_populate_codex_skills_none(tmp_path: Path) -> None:
    ...
    hermetic-agent regression-pin: Codify must produce no skill
...
def test_populate_codex_home_config_symlinks_auth_and_config(tmp_path: Path) -> None:
    ...
    """
    from codify.inner.codex_executor import _populate_codex_home_config
...
def test_app_server_start_uses_real_home_for_private_inherited_codex_home(
    ...
    Codify itself can be launched from a Codex-managed environment
    ...
    inherited = home / ".codify" / "codex-native" / "abc123" / "codex-home"
...
def test_app_server_start_preserves_custom_home_from_inherited_private_symlink(
    ...
    ``CODEX_HOME`` into an Codify private home. A nested launch inherits
...
def test_clean_codex_env_excludes_openai_api_key(monkeypatch) -> None:
    ...
    """
    from codify.inner.codex_executor import _clean_codex_env
...
def test_clean_codex_env_includes_codify_session_marker(monkeypatch) -> None:
    """The ``CODIFY`` session marker survives the codex env scrub.

    The marker (set once on the runner) must reach the codex CLI so the
    shell commands codex runs can detect they are inside an Codify
    ...
    monkeypatch.setenv(CODIFY_SESSION_ENV_VAR, CODIFY_SESSION_ENV_VALUE)

    env = _clean_codex_env()

    assert env.get(CODIFY_SESSION_ENV_VAR) == CODIFY_SESSION_ENV_VALUE
...
async def test_codex_cli_version_parses_output(
    ...
    monkeypatch.setattr("codify.inner.codex_executor._create_subprocess_exec", _fake_exec)
...
async def test_codex_cli_version_returns_none_on_oserror(
    ...
    monkeypatch.setattr("codify.inner.codex_executor._create_subprocess_exec", _boom)
...
async def test_codex_cli_version_times_out_and_kills_proc(
    ...
    monkeypatch.setattr("codify.inner.codex_executor._CODEX_VERSION_PROBE_TIMEOUT_SECONDS", 0.05)
    monkeypatch.setattr("codify.inner.codex_executor._create_subprocess_exec", _fake_exec)
...
def test_model_provider_override_appends_pin() -> None:
    ...
    """
    with patch("codify.inner.codex_executor._find_codex_cli", return_value="/usr/bin/codex"):
...
def test_select_codex_skill_dirs_all_first_source_wins(tmp_path: Path) -> None:
    from codify.inner.codex_executor import select_codex_skill_dirs
...
def test_codex_skill_sources_order_bundle_then_host(tmp_path: Path) -> None:
    ...
    from codify.inner.codex_executor import codex_skill_sources
...
def test_declared_passthrough_reads_sandbox_env_passthrough():
    ...
    from codify.inner.agent_env import declared_passthrough as _declared_passthrough
...
def test_find_codex_cli_delegates_to_shared_resolver(monkeypatch):
    ...
    CODIFY_CODEX_PATH override. (The resolver's own PATH/override/fallback
    behavior is covered in tests/inner/test_proc_and_platform.py.)"""
    from codify.inner import codex_executor as ce
    ...
    assert captured == {"name": "codex", "env_var": "CODIFY_CODEX_PATH"}
...
class TestCodexAppServerSessionReadOnlyCwd(unittest.TestCase):
    ...
    def _run_start_and_capture_mkdtemp_dir(self, cwd: str) -> str:
        ...
        async def _t():
            ...
            with (
                patch("codify.inner.codex_executor.populate_codex_skills_from_bundle"),
                patch("codify.inner.codex_executor._populate_codex_home_config"),
                patch(
                    "codify.inner.codex_executor._codex_home_config_source_from_env",
                    return_value=None,
                ),
                patch(
                    "codify.inner.codex_executor._create_subprocess_exec",
