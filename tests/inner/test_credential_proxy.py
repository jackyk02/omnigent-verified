"""Unit tests for codify.inner.credential_proxy.
...
def test_databricks_provider_requires_host() -> None:
    ...
    with pytest.raises(CodifyError, match="no 'host'"):
...
def test_databricks_provider_missing_sdk_fails_loud() -> None:
    ...
    try:
        ...
        with pytest.raises(CodifyError, match="requires the Databricks SDK"):
...
def test_databricks_runtime_rejects_same_host_profiles() -> None:
    ...
    with pytest.raises(CodifyError, match="resolve to workspace host"):
