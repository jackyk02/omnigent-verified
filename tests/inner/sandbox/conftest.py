...
- how to build an :class:`OSEnvSandboxSpec` for that backend with the
  ...
  ``import codify.*`` from a tempdir cwd
...
from codify.inner.datamodel import CredentialProxySpec, OSEnvSandboxSpec
...
def _repo_root_for_pythonpath() -> str:
    ...
    reach the ``codify`` package via ``PYTHONPATH``.

    Helper subprocesses run with ``cwd`` set to a throwaway tempdir
    in these tests, so they can't ``import codify`` unless the
...
def active_sandbox_spec_factory(
    ...
    helper subprocess can ``import codify.*`` from a tempdir
...
def sandbox_pythonpath_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bootstrap runs ``import codify.inner.os_env`` in the helper.
