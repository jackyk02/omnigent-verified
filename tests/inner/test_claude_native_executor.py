...
from codify.claude_native_bridge import REQUEST_SESSION_ID_ENV_VAR
from codify.inner import claude_native_executor
from codify.inner.claude_native_executor import ClaudeNativeExecutor
...
async def test_run_turn_injects_user_message_without_streaming_transcript(
    ...
    produce visible Codify chat items. This fails if the executor
...
@pytest.mark.asyncio
async def test_run_turn_does_not_advertise_active_codify_tools(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """
    ...
    from Claude's transcript rather than from a transient Codify turn.
    """
    bridge_dir = tmp_path / "bridge"
    sent_messages: list[dict[str, Any]] = []
