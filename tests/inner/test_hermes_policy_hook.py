...
Codify relay tools (surfaced into Hermes as ``mcp_codify_*``) are gated when
...
from codify.inner import hermes_policy_hook
...
def _run(monkeypatch: pytest.MonkeyPatch, tool_name: str) -> tuple[dict, bool]:
    ...
    monkeypatch.setattr("codify.native_policy_hook.post_evaluate_with_retry", _spy, raising=True)
    monkeypatch.setattr(
        "sys.stdin",
...
@pytest.mark.parametrize(
    "tool_name",
    [
        "mcp_codify_sys_session_get_info",  # hermes single-underscore form
        "mcp_codify_sys_os_write",
        "mcp__codify__list_comments",  # native double-underscore form
    ],
)
...
def test_native_and_other_mcp_tools_are_still_gated(
    ...
    # Hermes' own tools (and non-Codify MCP servers) do NOT round-trip the
