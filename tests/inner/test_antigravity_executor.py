"""
Unit tests for :class:`codify.inner.antigravity_executor.AntigravityExecutor`.
...
from codify.inner import antigravity_executor as ag
from codify.inner.antigravity_executor import AntigravityExecutor, _latest_user_text
...
from codify.llms._usage_observer import add_observer
...
async def test_streaming_maps_text_reasoning_and_usage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Usage maps the SDK's UsageMetadata field names onto Codify's keys and
...
async def test_sys_tools_exposed_as_callables_routing_through_executor(
    ...
    """Codify tools become callable SDK tools whose calls hit ``_tool_executor``.

    This is what lets an Antigravity agent drive Codify's sys / sub-agent
