...
from codify.inner import codex_harness
...
def test_harness_module_registered_in_module_registry() -> None:
    ...
    """
    assert _HARNESS_MODULES.get("codex") == "codify.inner.codex_harness"
...
def test_executor_factory_reads_env_vars(
    ...
    with patch(
        "codify.inner.codex_harness.CodexExecutor.__init__",
        _fake_init,
    ):
...
def test_executor_factory_cwd_falls_back_to_runner_workspace(
    ...
    to ``CODIFY_RUNNER_WORKSPACE``. Mirrors the claude-sdk / kimi / pi /
    hermes harnesses.
    """
...
def test_executor_factory_decodes_os_env_json(
    ...
    Codify serializes ``spec.os_env`` via :func:`dataclasses.asdict`
