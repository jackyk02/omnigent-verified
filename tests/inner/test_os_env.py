"""Unit tests for :mod:`codify.inner.os_env` helper-env construction."""
...
from codify.inner.os_env import (
    _child_shell_env,
    _project_root,
    _read_impl,
    _shell_impl,
...
def test_build_helper_env_active_passes_codify_session_marker() -> None:
    """The ``CODIFY`` session marker survives the active allowlist.
    ...
    Codify session, the way ``CLAUDE_CODE`` / ``CODEX`` are visible in
    ...
    parent = {
        "PATH": "/usr/bin",
        CODIFY_SESSION_ENV_VAR: CODIFY_SESSION_ENV_VALUE,
    }

    env = build_helper_env(parent, _active_policy())

    assert env[CODIFY_SESSION_ENV_VAR] == CODIFY_SESSION_ENV_VALUE
...
# ---------------------------------------------------------------------------
# _child_shell_env — codify's own package root must not leak onto the
...
def test_child_shell_env_strips_project_root_entry(
    ...
    """codify's project root is removed; a project entry is preserved.

    The helper prepends its project root to ``PYTHONPATH`` so it can import
    codify at startup. Commands the agent runs must not inherit that entry,
    or codify's ``site-packages`` shadows the project venv's own packages.
...
def test_child_shell_env_drops_var_when_only_project_root(
    ...
    """When the sole entry is codify's root, ``PYTHONPATH`` is unset.
...
# ---------------------------------------------------------------------------
# End-to-end: the real helper must not leak codify's package root into a
# sys_os_shell command's PYTHONPATH. Guards the wiring in _shell_impl, not
# just _child_shell_env in isolation.
# ---------------------------------------------------------------------------


def test_shell_command_does_not_see_codify_project_root(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A shell command's ``PYTHONPATH`` drops codify's root, keeps the rest.

    Spawns a real ``caller_process`` helper (``sandbox: none`` so it runs on
    every platform) with codify's root pre-seeded on ``PYTHONPATH`` — the
    same shape the helper spawn produces — and asserts the agent's command
    sees the sibling project entry but not codify's, so project subprocesses
