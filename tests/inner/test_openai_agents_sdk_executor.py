...
from codify.inner.openai_agents_sdk_executor import (
...
class TestOpenAIAgentsSDKExecutor(unittest.TestCase):
    ...
    def test_streams_text_and_tool_events(self):
        async def _t():
            ...
            executor = OpenAIAgentsSDKExecutor(client=object())
            with patch(
                "codify.inner.openai_agents_sdk_executor._ensure_agents_sdk",
...
def test_get_openai_client_profile_uses_callback_auth(monkeypatch):
    ...
    from codify.inner.openai_agents_sdk_executor import _get_openai_async_client
...
def test_normalize_content_blocks_strips_filename_from_input_image() -> None:
    """``filename`` is Codify metadata and must not reach OpenAI."""
    blocks = [
        {
            "type": "input_image",
