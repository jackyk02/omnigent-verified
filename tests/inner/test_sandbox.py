...
from codify.inner.sandbox import (
    SandboxPolicy,
...
def test_run_launcher_emits_logger_checkpoints(caplog) -> None:
    ...
    """
    with caplog.at_level(logging.INFO, logger="codify.inner.sandbox"):
        rc = run_launcher(_noop_policy_arg(), sys.executable, ["-c", "pass"])
    assert rc == 0

    messages = [record.getMessage() for record in caplog.records]
    # Absent: wrapper never entered the body.
    assert any("[codify-sandbox] activating backend=none active=False" in m for m in messages), (
        messages
    )
    # Absent on an active policy: activation hung.
    assert any("[codify-sandbox] activated; spawning target=" in m for m in messages), messages
    # Absent: spawned target hung (the claude-sdk symptom).
    assert any("[codify-sandbox] target exited rc=0" in m for m in messages), messages
...
def test_run_launcher_propagates_target_returncode(caplog) -> None:
    ...
    assert rc == 7
    messages = [record.getMessage() for record in caplog.records]
    assert any("[codify-sandbox] target exited rc=7" in m for m in messages), messages
...
def test_exec_launcher_wrapper_subprocess_emits_markers_to_stderr() -> None:
    ...
    assert "[codify-sandbox] activating backend=none active=False" in result.stderr, result.stderr
    assert "[codify-sandbox] activated; spawning target=" in result.stderr, result.stderr
    assert "[codify-sandbox] target exited rc=0" in result.stderr, result.stderr


def test_run_launcher_wraps_target_with_strace_when_env_set(monkeypatch, caplog) -> None:
    """
    ``CODIFY_SANDBOX_STRACE=1`` must prepend ``strace -f -y -e
    trace=file`` to the spawned target's argv so the wrapper's stderr
    captures file-syscall denials (EACCES) from the sandbox.
    """
    from codify.inner import sandbox as sb
    ...
    monkeypatch.setenv("CODIFY_SANDBOX_STRACE", "1")

    with caplog.at_level(logging.WARNING, logger="codify.inner.sandbox"):
...
def test_run_launcher_skips_strace_when_binary_missing(monkeypatch, caplog) -> None:
    """If ``CODIFY_SANDBOX_STRACE`` is set but strace is not on
...
def test_with_additional_write_roots_records_mask_scan_skip_roots() -> None:
    ...
    policy = _noop_policy()
    scratch = Path("/tmp/codify-helper-ab12")
...
def test_with_denied_unix_sockets_resolves_dedupes_and_is_pure() -> None:
    ...
    from codify.inner.sandbox import with_denied_unix_sockets
...
def test_with_spawn_env_allowlist_sets_sorted_deduped_copy() -> None:
    ...
    """
    from codify.inner.sandbox import with_spawn_env_allowlist
