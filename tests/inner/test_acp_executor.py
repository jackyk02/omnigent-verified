"""Tests for the generic ACP executor (:mod:`codify.inner.acp_executor`).
...
from codify.inner._acp_codify_mcp import CodifyAcpMcp, _to_acp_mcp_servers
from codify.inner.acp_executor import AcpAgentConfig, AcpExecutor
...
def test_harness_wrap_requires_command(monkeypatch: pytest.MonkeyPatch) -> None:
    from codify.inner import acp_harness
...
# ---------------------------------------------------------------------------
# Codify MCP bridge (session/new.mcpServers via the shared serve-mcp relay)
# ---------------------------------------------------------------------------
...
def test_mcp_to_acp_servers_flattens_env_to_array() -> None:
    ...
    out = _to_acp_mcp_servers(
        {"mcpServers": {"codify": {"command": "/py", "args": ["-Im", "x"], "env": {"A": "1"}}}}
    )
    assert out == [
        {
            "name": "codify",
...
def test_mcp_kill_switch(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CODIFY_ACP_MCP", "0")
    m = CodifyAcpMcp("t")
    assert (
...
async def test_mcp_relay_starts_and_builds_serve_mcp_entry() -> None:
    ...
    try:
        ...
        entry = servers[0]
        assert entry["name"] == "codify"
        assert "serve-mcp" in entry["args"]
        assert "codify.claude_native_bridge" in entry["args"]
...
async def test_acp_session_new_carries_mcp_servers() -> None:
    ...
    ex._codify_tools = [{"name": "sys_agent_list"}]
    sentinel = [{"name": "codify", "command": "/py", "args": [], "env": []}]
...
@pytest.mark.asyncio
async def test_acp_session_new_codify_mcp_disabled_per_agent() -> None:
    """`codify_mcp=False` on the agent config → no mcpServers in session/new."""
    ex = AcpExecutor(AcpAgentConfig(command="x", codify_mcp=False))
    ex._tool_executor = lambda n, a: None  # type: ignore[assignment]
    ex._codify_tools = [{"name": "sys_agent_list"}]
    captured: dict = {}
