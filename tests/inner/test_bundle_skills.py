"""
Tests for ``codify.inner.bundle_skills`` — the shared helpers that
...
from codify.inner.bundle_skills import (
...
def test_ensure_bundle_plugin_manifest_writes_when_missing(tmp_path: Path) -> None:
    ...
    (e.g. ``codify-ap-chat-x9p606iz/bundle:researcher``).
    """
...
def test_claude_native_skill_args_no_bundle_is_empty() -> None:
    """
    With no bundle (the ``codify claude`` CLI path), no plugin args are
