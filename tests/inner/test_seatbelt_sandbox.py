...
from codify.inner.sandbox import SandboxPolicy, with_denied_unix_sockets
from codify.inner.seatbelt_sandbox import (
...
def test_resolve_raises_on_non_darwin() -> None:
    ...
    with patch("codify.inner.seatbelt_sandbox.sys.platform", "linux"):
...
def test_resolve_raises_when_sandbox_exec_missing() -> None:
    ...
    with patch("codify.inner.seatbelt_sandbox.sys.platform", "darwin"):
        with patch("codify.inner.seatbelt_sandbox.shutil.which", return_value=None):
...
def _safe_helper_argv(tmp_path: Path) -> list[str]:
    ...
    return [str(interpreter), "-m", "codify.inner.os_env", "helper", "X"]
...
def test_profile_scratch_tmpdir_gets_read_and_write_allows(tmp_path: Path) -> None:
    ...
    scratch = sys_tmp / "codify-test-scratch"
...
def test_profile_network_section_for_active_egress_emits_narrow_allows(
    ...
    sys_tmp = Path(tempfile.gettempdir())
    scratch = sys_tmp / "codify-egress-test-scratch"
...
def test_ensure_executable_visible_does_not_widen_when_venv_is_under_cwd() -> None:
    ...
    argv = [str(venv_python), "-m", "codify.inner.os_env", "helper"]
...
def test_helper_boots_when_cwd_lives_under_home_regression() -> None:
    ...
    home = Path.home()
    cwd = home / f".codify-realpath-regression-{uuid.uuid4().hex}"
...
def test_helper_boots_when_interpreter_lives_under_home_uv_layout(
    ...
    home = Path.home()
    fake_install_root = home / f".codify-uv-layout-regression-{uuid.uuid4().hex}"
...
def test_ensure_executable_visible_two_hop_proxy_finds_cpython_install_root(
    ...
    ``uv tool install codify`` creates:
      ~/.local/share/uv/tools/codify/bin/python  →  (proxy symlink)
          ~/.local/share/uv/python/cpython-3.12.X-.../bin/python3.12

    The LITERAL path grandparent (``tools/codify/``) has no CPython
    ...
    cpython_root = home / f".codify-test-cpython-{uuid.uuid4().hex}"
    ...
    tool_root = home / f".codify-test-tool-{uuid.uuid4().hex}"
...
def test_ensure_executable_visible_falls_back_to_install_root_for_uv_layout(
    ...
    with patch(
        "codify.inner.seatbelt_sandbox._UNSAFE_WIDEN_ANCESTORS",
        frozenset({topmost_str}),
    ):
        with caplog.at_level("WARNING", logger="codify.inner.seatbelt_sandbox"):
            extras = _ensure_executable_visible(argv, cwd)
...
def test_framework_write_root_dotfiles_not_masked(tmp_path: Path) -> None:
    ...
    scanned.
    """
    from codify.inner.sandbox import with_additional_write_roots
...
def _uv_style_layout(base: Path) -> tuple[Path, Path, Path]:
    ...
    Mirrors what ``uv tool install codify`` produces: the tool
    ...
    tool_root = base / ".local" / "share" / "uv" / "tools" / "codify"
    (tool_root / "bin").mkdir(parents=True)
...
def test_wrap_launcher_argv_grants_uv_versionless_hop_literal(tmp_path: Path) -> None:
    ...
    argv = backend.wrap_launcher_argv(
        [str(tool_exe), "-m", "codify.inner.os_env", "helper", "X"], policy, cwd
    )
...
def test_run_launcher_spawn_wrap_private_tmpdir_boots_under_seatbelt(
    ...
    from codify.inner.sandbox import _project_root, create_exec_launcher
    ...
    # can import ``codify.inner`` — in production that import is
