...
from codify.inner import cursor_native_executor as cne
from codify.inner.cursor_native_executor import (
    CursorNativeExecutor,
    _content_to_text,
    _latest_user_text,
)
from codify.inner.executor import ExecutorError
...
class TestForkPreamble:
    ...
    def test_wrap_defangs_sentinels_inside_preamble(self) -> None:
        ...
        assert "[/codify_fork_history]" in wrapped
...
class TestBridge:
    ...
    def test_build_mcp_config_registers_codify_relay(self, tmp_path: Path) -> None:
        config = build_mcp_config(tmp_path, python_executable="python-test")
        server = config["mcpServers"]["codify"]
    ...
    def test_write_mcp_config_is_workspace_scoped(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codify.cursor_native_bridge.approve_mcp_server_for_workspace",
        ...
        assert payload["mcpServers"]["codify"]["command"] == "python-test"
    ...
    def test_enable_mcp_for_workspace_removes_disabled_entry(
        ...
        disabled_path.write_text('["codify", "other"]\n', encoding="utf-8")
    ...
    def test_allow_mcp_tools_in_cli_config_adds_specific_allow_rules(
        ...
        config_path.write_text(
            '{"permissions": {"allow": ["Shell(ls)", "Mcp(codify:sys_os_read)"]}}\n',
            encoding="utf-8",
        )
        ...
        assert allow.count("Mcp(codify:sys_os_read)") == 1
        assert "Mcp(codify:sys_session_send)" in allow
    ...
    def test_approve_mcp_server_for_workspace_uses_cursor_cli(
        ...
        monkeypatch.setattr(
            "codify.cursor_native.resolve_cursor_executable",
        ...
        assert calls == [
            {
                "args": (["/bin/cursor-agent-test", "mcp", "enable", "codify"],),
                "kwargs": {
                    "cwd": tmp_path,
...
class TestRegistration:
    def test_harness_is_registered(self) -> None:
        ...
        assert _HARNESS_MODULES["cursor-native"] == "codify.inner.cursor_native_harness"

    def test_harness_is_allowlisted(self) -> None:
        from codify.spec._codify_compat import CODIFY_HARNESSES

        assert "cursor-native" in CODIFY_HARNESSES

    def test_cursor_native_is_terminal_native(self) -> None:
        # cursor-native launches the cursor-agent TUI in an codify terminal
