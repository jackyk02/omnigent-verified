...
from codify.inner.claude_sdk_executor import _to_anthropic_content_blocks
from codify.inner.executor import (
    ExecutorError,
    TextChunk,
    ToolCallComplete,
    ToolCallRequest,
...
class TestPromptExtraction(unittest.TestCase):
    def _make_executor(self):
        from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
...
class TestConstructor(unittest.TestCase):
    def test_default_values(self):
        from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
        from codify.spec.types import RetryPolicy
    ...
    def test_os_env_spec_wraps_cli_and_enables_native_tools(self):
        from codify.inner.claude_sdk_executor import ClaudeSDKExecutor, PreparedClaudeCli
        from codify.inner.datamodel import OSEnvSpec

        spec = OSEnvSpec(type="caller_process", cwd="/tmp/work")
        with (
            patch(
                "codify.inner.claude_sdk_executor._find_system_claude",
                return_value="/usr/bin/claude",
            ),
            patch(
                "codify.inner.claude_sdk_executor.prepare_claude_cli_path",
                return_value=PreparedClaudeCli(
                    cli_path="/tmp/codify-claude-wrapper",
        ...
        self.assertEqual(executor._cli_path, "/tmp/codify-claude-wrapper")
        self.assertEqual(executor._cwd, "/tmp/work")

    def test_prepare_claude_cli_path_adds_internal_roots_to_read_allowlist(self):
        from codify.inner.claude_sdk_executor import prepare_claude_cli_path
        from codify.inner.datamodel import OSEnvSandboxSpec, OSEnvSpec
        from codify.inner.sandbox import SandboxPolicy
        ...
        with (
            patch(
                "codify.inner.claude_sdk_executor.resolve_sandbox",
            ...
            patch(
                "codify.inner.claude_sdk_executor._claude_internal_write_roots",
                return_value=[Path("/home/test/.claude/sessions")],
            ),
            patch(
                "codify.inner.claude_sdk_executor._claude_internal_write_files",
                return_value=[],
            ),
            patch(
                "codify.inner.claude_sdk_executor.create_exec_launcher",
    ...
    def test_default_process_sandbox_wraps_cli_without_enabling_native_tools(self):
        ...
        with (
            ...
            patch(
                "codify.inner.claude_sdk_executor.prepare_tight_cli_process_path",
                return_value="/tmp/codify-claude-tight-wrapper",
            ),
        ):
            executor = ClaudeSDKExecutor()
        ...
        self.assertEqual(
            executor._cli_path,
            "/tmp/codify-claude-tight-wrapper",
        )
    ...
    def test_databricks_flag_with_profile(self):
        ...
        with (
            ...
            patch(
                "codify.inner.databricks_executor._read_databrickscfg",
            ...
            self.assertIn(
                ...
                executor._extra_env["CODIFY_CLAUDE_API_KEY_HELPER"],
            )
    ...
    def test_databricks_explicit_profile_selects_by_profile(self):
        ...
        helper = executor._extra_env["CODIFY_CLAUDE_API_KEY_HELPER"]
    ...
    def test_databricks_flag_no_creds_raises(self):
        ...
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("codify.inner.claude_sdk_executor._resolve_gateway_env", return_value={}),
        ):
    ...
    def test_databricks_flag_with_host_override(self):
        ...
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("codify.inner.databricks_executor._read_databrickscfg") as read_cfg,
        ):
            executor = ClaudeSDKExecutor(
                gateway=True,
    ...
    def test_neutral_gateway_no_model_does_not_inject_databricks_default(self):
        ...
        ``databricks-*`` model: the Codify producer resolves a concrete model
    ...
    def test_force_close_client_uses_process_tree_termination(self):
        ...
        async def _t():
            with patch(
                "codify.inner.claude_sdk_executor._terminate_process_tree"
            ) as terminate_tree:
    ...
    def test_claude_internal_write_files_omits_missing_config(self):
        from codify.inner.claude_sdk_executor import _claude_internal_write_files

        with tempfile.TemporaryDirectory() as td:
            home = Path(td)
            config_path = home / ".claude.json"
            self.assertFalse(config_path.exists())
            with patch("codify.inner.claude_sdk_executor.pathlib.Path.home", return_value=home):
...
class TestBuildMcpTools(unittest.TestCase):
    def test_builds_tools_from_schemas(self):
        from codify.inner.claude_sdk_executor import _build_mcp_tools
...
class TestResolveGatewayEnv(unittest.TestCase):
    def test_from_profile(self):
        from codify.inner.claude_sdk_executor import _resolve_gateway_env
        from codify.inner.databricks_executor import DatabricksCredentials
        ...
        with (
            ...
            self.assertIn(
                ...
                env["CODIFY_CLAUDE_API_KEY_HELPER"],
            )
    ...
    def test_no_creds_returns_empty(self):
        ...
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("codify.inner.databricks_executor._read_databrickscfg", return_value=None),
        ):
            env = _resolve_gateway_env()
    ...
    def test_host_override_skips_profile_lookup(self):
        ...
        with patch("codify.inner.databricks_executor._read_databrickscfg") as read_cfg:
            env = _resolve_gateway_env(
        ...
        self.assertEqual(env["CODIFY_CLAUDE_API_KEY_HELPER"], "printf token")
...
class TestSystemMessages(unittest.TestCase):
    def test_databricks_auth_uses_api_key_helper_settings(self):
        ...
        def _resolve_gateway_env(
            ...
            *,
            ...
            return {
                ...
                "CODIFY_CLAUDE_API_KEY_HELPER": "databricks auth token --host https://host",
        ...
        async def _t():
            ...
            with (
                patch(
                    "codify.inner.claude_sdk_executor._resolve_gateway_env",
                    _resolve_gateway_env,
                ),
                patch("codify.inner.claude_sdk_executor._ensure_sdk", return_value=_FakeSDK),
            ):
                events = [
                    e
        ...
        self.assertNotIn("CODIFY_CLAUDE_API_KEY_HELPER", captured_options[0].env)
    ...
    def test_auth_retry_surfaces_executor_error(self):
        ...
        async def _t():
            executor = ClaudeSDKExecutor()
            with patch("codify.inner.claude_sdk_executor._ensure_sdk", return_value=_FakeSDK):
                events = [
                    e
                    async for e in executor.run_turn(
                        [{"role": "user", "content": "hello"}],
        ...
        async def _t():
            ...
            with patch(
                ...
                return_value={
                    ...
                    "CLAUDE_CODE_API_KEY_HELPER_TTL_MS": "900000",
                    "CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS": "1",
                    "CODIFY_CLAUDE_API_KEY_HELPER": "databricks auth token ...",
                },
            ):
                executor = ClaudeSDKExecutor(gateway=True)
            with patch("codify.inner.claude_sdk_executor._ensure_sdk", return_value=_FakeSDK):
...
class TestSkillsFilterTranslation(unittest.TestCase):
    ...
    def test_all_lets_sdk_default_setting_sources(self) -> None:
        ...
        future SDK changes.
        """
        from codify.inner.claude_sdk_executor import _resolve_skills_option
...
class TestStreamEventStreaming(unittest.TestCase):
    def test_live_clients_are_reused_per_codify_session(self):
        from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
    ...
    def test_os_env_spec_exposes_only_explicit_native_tools(self):
        ...
        async def _t():
            ...
            self.assertIn("mcp__codify__sleep", captured_options["allowed_tools"])
    ...
    def test_mcp_only_session_disables_native_tool_base_set(self):
        ...
        async def _t():
            ...
            # SKILL.md content.
            self.assertEqual(captured_options["tools"], ["Skill"])
            self.assertEqual(captured_options["allowed_tools"], ["mcp__codify__sleep"])
            self.assertIsInstance(events[-1], TurnComplete)
    ...
    def test_session_send_tool_is_exposed_via_mcp(self):
        ...
        async def _t():
            ...
            self.assertIn("mcp__codify__sys_session_send", captured_options["allowed_tools"])
            self.assertIn(
                ("use `mcp__codify__sys_session_send` when instructions say `sys_session_send`"),
    ...
    def test_session_rename_tool_uses_exact_sdk_mcp_name(self):
        ...
        async def _t():
            ...
            self.assertIn(
                "mcp__codify__sys_session_rename",
                captured_options["allowed_tools"],
            )
            self.assertIn(
                "use `mcp__codify__sys_session_rename` when instructions say `sys_session_rename`",
                captured_options["system_prompt"],
            )
            self.assertIsInstance(events[-1], TurnComplete)
    ...
    def test_close_session_disconnects_live_client(self):
        from codify.inner.claude_sdk_executor import ClaudeSDKExecutor, _ClaudeClientState
...
def test_unset_env_var_removes_and_restores(monkeypatch):
    ...
    from codify.inner.claude_sdk_executor import _unset_env_var
...
async def test_get_or_create_client_surfaces_cli_stderr_on_connect_timeout(monkeypatch):
    ...
    doing while it hung.
    """
    from codify.inner import claude_sdk_executor as cse
    from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
...
def test_resolve_sandbox_cwd_roots_relative_at_runner_workspace(monkeypatch) -> None:
    ...
    against ``CODIFY_RUNNER_WORKSPACE`` — not the daemon's process cwd
    ...
    from codify.inner.claude_sdk_executor import _resolve_sandbox_cwd

    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", "/home/bobby/code/agents")
    ...
    monkeypatch.delenv("CODIFY_RUNNER_WORKSPACE", raising=False)
...
def test_prepare_claude_cli_path_bypasses_wrapper_when_env_set(
    ...
    """
    ``CODIFY_CLAUDE_SDK_NO_SANDBOX`` (any truthy value) must skip
    ...
    monkeypatch.setenv("CODIFY_CLAUDE_SDK_NO_SANDBOX", env_value)
    ...
    with caplog.at_level(logging.WARNING, logger="codify.inner.claude_sdk_executor"):
        prepared = prepare_claude_cli_path("/usr/bin/claude", spec)
...
def test_prepare_tight_cli_process_path_bypasses_wrapper_when_env_set(monkeypatch) -> None:
    ...
    from codify.inner.claude_sdk_executor import prepare_tight_cli_process_path

    monkeypatch.setenv("CODIFY_CLAUDE_SDK_NO_SANDBOX", "1")
...
def test_prepare_claude_cli_path_degrades_when_resolve_sandbox_fails(monkeypatch, caplog) -> None:
    ...
    shape as the ``CODIFY_CLAUDE_SDK_NO_SANDBOX`` bypass — and warns.
    """
    from codify.inner.claude_sdk_executor import prepare_claude_cli_path
    ...
    monkeypatch.setattr("codify.inner.claude_sdk_executor.resolve_sandbox", _raise_resolve)

    def _fail_if_called(*args, **kwargs) -> str:
        raise AssertionError("create_exec_launcher must not run when resolve failed")

    monkeypatch.setattr("codify.inner.claude_sdk_executor.create_exec_launcher", _fail_if_called)
...
def test_prepare_claude_cli_path_degrades_when_wrap_probe_fails(monkeypatch, caplog) -> None:
    ...
    monkeypatch.setattr(
        "codify.inner.claude_sdk_executor.get_backend",
...
async def test_result_message_is_error_yields_executor_error() -> None:
    ...
    from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
    from codify.inner.executor import ExecutorError, TurnComplete
...
class TestToolCallPolicyGate(unittest.TestCase):
    ...
    def _verdict(action, reason=None):
        from codify.runtime.harnesses._scaffold import PolicyVerdictPayload
    ...
    def test_ask_verdict_prompts_even_under_bypass(self):
        """A raw ASK verdict is supported by routing to Codify
    ...
    def test_unexpected_verdict_fails_closed(self):
        ...
        async def _t():
            ...
            self.assertIsInstance(result, PermissionResultDeny)
            self.assertIn("Unexpected Codify TOOL_CALL policy verdict", result.message)
    ...
    def test_codify_own_tool_skips_evaluation(self):
        """``mcp__codify__*`` tools are already TOOL_CALL-gated server-side
...
def test_precompact_hook_emits_compaction_complete_with_session_messages() -> None:
    ...
    from codify.inner.claude_sdk_executor import ClaudeSDKExecutor
    from codify.inner.executor import CompactionComplete
    ...
    async def _t():
        ...
        with (
            patch(
                "codify.inner.claude_sdk_executor._ensure_sdk",
...
def test_find_system_claude_delegates_to_shared_resolver(monkeypatch) -> None:
    ...
    CODIFY_CLAUDE_PATH override, so an nvm/npm-installed claude off the host
    ...
    assert captured == {"name": "claude", "env_var": "CODIFY_CLAUDE_PATH"}
