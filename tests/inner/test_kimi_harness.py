...
from codify.inner import kimi_executor, kimi_harness
...
from codify.inner.kimi_executor import (
    _SESSION_RESUME_RE,
    KimiExecutor,
    _latest_user_text,
    _parse_truthy,
...
from codify.spec._codify_compat import CODIFY_HARNESS_ALIASES, CODIFY_HARNESSES
...
def test_kimi_in_module_registry() -> None:
    assert _HARNESS_MODULES.get("kimi") == "codify.inner.kimi_harness"
    assert _HARNESS_MODULES.get("kimi-code") == "codify.inner.kimi_harness"


def test_kimi_in_codify_harnesses_allowlist() -> None:
    assert "kimi" in CODIFY_HARNESSES
    assert "kimi-code" in CODIFY_HARNESS_ALIASES
...
def test_executor_factory_reads_env_vars(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CODIFY_KIMI_PATH", raising=False)
    ...
    with patch(
        "codify.inner.kimi_harness.KimiExecutor.__init__",
        _fake_init,
    ):
...
def test_executor_factory_falls_back_to_runner_workspace_cwd(
    ...
    """With no HARNESS_KIMI_CWD, kimi runs in CODIFY_RUNNER_WORKSPACE — the
    ...
    monkeypatch.setenv("CODIFY_RUNNER_WORKSPACE", "/home/me/project")
...
def test_resolve_kimi_binary_explicit_override(monkeypatch: pytest.MonkeyPatch) -> None:
    # Canonical CODIFY_KIMI_PATH wins.
    monkeypatch.setenv("CODIFY_KIMI_PATH", "/opt/bin/kimi")
...
def test_latest_user_text_drops_image_blocks_with_warning(
    ...
    caplog.set_level(logging.WARNING, logger="codify.inner.kimi_executor")
    messages = [
        {
            "role": "user",
            "content": [
...
def test_translate_event_meta_captures_session_id() -> None:
    ...
    assert events == []  # meta events yield no Codify-visible events
