...
from codify.inner import bwrap_sandbox
from codify.inner.bwrap_sandbox import (
...
def _run_helper_probe(
    ...
    *,
    ...
    call :func:`codify.inner.sandbox.activate_sandbox` with the
    ...
    the probe can ``import codify.*`` inside the sandbox even
    ...
    if policy is None:
        spec = OSEnvSpec(
            ...
            sandbox=OSEnvSandboxSpec(
                ...
                # the codify package during tests run from a
                # tempdir cwd.
...
def _repo_root() -> Path:
    ...
    :mod:`codify` from a clean ``$PYTHONPATH``.

    :returns: The directory containing the ``codify`` package.
    """
...
def test_resolve_raises_on_non_linux() -> None:
    ...
    with patch("codify.inner.bwrap_sandbox.sys.platform", "darwin"):
...
def test_resolve_raises_when_bwrap_missing() -> None:
    ...
    with patch("codify.inner.bwrap_sandbox.shutil.which", return_value=None):
...
def test_wrap_launcher_argv_starts_with_bwrap_and_ends_with_command(
    ...
    argv = backend.wrap_launcher_argv(
        [sys.executable, "-m", "codify.inner.os_env", "helper", "X"],
        policy,
        tmp_path,
    )
    assert argv[0] == "bwrap"
...
def test_wrap_launcher_argv_reexposes_interpreter_under_masked_dotdir(
    ...
    ``~/.local/share/uv/tools/codify/bin/python`` with cwd=``$HOME``),
    ...
    """
    interp_bin = tmp_path / ".local" / "share" / "uv" / "tools" / "codify" / "bin"
...
def test_detect_host_backend_env_declaration_is_normalised(
    ...
    """
    ``CODIFY_HOST_SANDBOX_BACKEND`` is the explicit, authoritative
...
def test_dotfile_masking_skips_target_that_vanished_after_scan(
    ...
    """
    from codify.inner import bwrap_sandbox
    from codify.inner._cwd_scan import MaskedEntry
...
from codify.inner.sandbox import SandboxPolicy, activate_sandbox
...
def test_interpreter_under_masked_dotdir_still_spawns(tmp_path: Path) -> None:
    ...
    Mirrors the reported failure — a ``uv tool``-installed codify at
    ``~/.local/share/uv/tools/codify/bin/python`` with the sandbox
...
def test_seccomp_extra_rules_block_clone3_outright() -> None:
    ...
    from codify.inner._seccomp import scmp_act_errno
...
def test_seccomp_extra_rules_socket_allowlist() -> None:
    ...
    """
    from codify.inner._seccomp import SCMP_CMP_EQ, SCMP_CMP_GE
...
def test_run_launcher_spawn_wrap_private_tmpdir_boots_under_bwrap(tmp_path: Path) -> None:
    ...
    # inline re-exec can import ``codify.inner``.
    policy = _make_policy(
        cwd,
        allow_hidden=[".venv"],
