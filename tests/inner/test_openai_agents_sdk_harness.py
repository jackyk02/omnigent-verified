...
from codify.inner import openai_agents_sdk_harness
...
def test_harness_module_registered_in_module_registry() -> None:
    ...
    matches the Codify YAML spelling (no ``-sdk`` suffix); the
    ...
    """
    assert _HARNESS_MODULES.get("openai-agents") == "codify.inner.openai_agents_sdk_harness"
...
def test_executor_factory_reads_databricks_profile_env(
    ...
    with patch(
        "codify.inner.openai_agents_sdk_harness.OpenAIAgentsSDKExecutor.__init__",
        _fake_init,
    ):
