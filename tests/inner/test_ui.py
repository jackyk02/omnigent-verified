"""Tests for the shared CLI styling layer (``codify.inner.ui``)."""
...
def test_accent_is_brand_magenta() -> None:
    """The shared accent is the Codify brand magenta."""
...
def test_show_banner_respects_no_banner_env(value: str) -> None:
    """``CODIFY_NO_BANNER`` force-disables the banner even on a TTY."""
...
def test_status_lines_go_to_stdout(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    ui.step("Installing Codify")
    ui.success("Verified codify")
    ui.info("Using ~/.codify")
    captured = capsys.readouterr()
    assert "Installing Codify" in captured.out
    assert "✓ Verified codify" in captured.out
    assert captured.err == ""
