...
from codify.inner.pi_executor import (
    PiExecutor,
    _build_models_json,
...
from codify.model_catalog import ModelEntry
...
def test_sanitize_real_sys_session_send_args_collapses_to_object() -> None:
    ...
    """
    from codify.tools.builtins.spawn import _build_sys_session_send_schema
    ...
    async def _run_generated_bridge_tool(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            extension_path = tmp_path / "codify_tools.js"
...
class TestPiExecutorConstructor(unittest.TestCase):
    def test_constructor_finds_pi(self):
        with patch("codify.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"):
            executor = PiExecutor()
        self.assertEqual(executor._pi_path, "/usr/bin/pi")

    def test_constructor_raises_when_pi_not_found(self):
        with patch("codify.inner.pi_executor._find_pi_cli", return_value=None):
            with self.assertRaises(ImportError):
                PiExecutor()

    def test_constructor_databricks_with_env(self):
        with (
            patch("codify.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"),
            patch(
                "codify.inner.pi_executor._read_databrickscfg",
    ...
    def test_constructor_databricks_with_host_override_requires_auth_command(self):
        with (
            patch("codify.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"),
            patch("codify.inner.pi_executor._read_databrickscfg") as read_cfg,
        ):
    ...
    def test_constructor_databricks_with_auth_command(self):
        with (
            ...
            patch("codify.inner.pi_executor._read_databrickscfg") as read_cfg,
            patch(
                "codify.inner.pi_executor._fetch_shell_command_token",
    ...
    def test_constructor_databricks_no_creds_raises(self):
        with (
            ...
            patch("codify.inner.pi_executor._read_databrickscfg", return_value=None),
        ):
...
class TestGateNativeTool(unittest.TestCase):
    ...
    Codify policy engine must become ``block=True`` so Pi refuses the
...
def test_gateway_seeds_managed_settings_from_global_agent(
    ...
    monkeypatch.setattr(
        "codify.inner.pi_settings.DEFAULT_PI_AGENT_DIR",
        global_agent,
    )
    with (
        patch("codify.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"),
...
class TestSessionManagement(unittest.TestCase):
    ...
    def test_close_session(self):
        async def _test():
            ...
            from codify.inner.pi_executor import _PiSessionState
...
def test_resolve_pi_skill_args_all(tmp_path: Path) -> None:
    ...
    """
    from codify.inner.pi_executor import _resolve_pi_skill_args
...
def test_gateway_wire_catalog_fetches_once_and_indexes_aliases() -> None:
    ...
    with (
        ...
        patch(
            "codify.model_catalog.fetch_databricks_model_service_entries",
            return_value=entries,
        ) as fetch,
        patch(
            "codify.model_catalog.catalog_model_entries",
            return_value=(
                ModelEntry(
...
def test_dedicated_gateway_fetches_wire_catalog_from_workspace_host() -> None:
    ...
    with (
        ...
        patch(
            "codify.pi_native_credentials.resolve_databricks_workspace",
        ...
        patch("codify.model_catalog.catalog_model_entries", return_value=()),
    ):
        executor = PiExecutor(
            gateway=True,
...
def test_generic_anthropic_gateway_skips_databricks_wire_catalog() -> None:
    ...
    with (
        ...
        patch("codify.model_catalog.fetch_databricks_model_service_entries") as fetch,
    ):
        executor = PiExecutor(
            gateway=True,
...
def test_clean_pi_env_excludes_host_secrets(monkeypatch) -> None:
    ...
    """
    from codify.inner.pi_executor import _clean_pi_env
...
def test_clean_pi_env_includes_codify_session_marker(monkeypatch) -> None:
    """The ``CODIFY`` session marker survives the Pi env scrub.

    The marker (set once on the runner) must reach the Pi CLI so the
    shell commands Pi runs can detect they are inside an Codify
...
def test_rpc_start_spawns_with_exact_env(monkeypatch) -> None:
    ...
    :param monkeypatch: Pytest monkeypatch fixture.
    """
    from codify.inner import pi_executor as pi_mod
...
def test_rpc_start_log_does_not_leak_system_prompt(monkeypatch, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG, logger="codify.inner.pi_executor"):
        _run(_test())
...
def test_pi_sandbox_launcher_policy_carries_spawn_env_allowlist(monkeypatch, tmp_path) -> None:
    ...
    """
    from codify.inner import sandbox as sandbox_mod
    from codify.inner.datamodel import OSEnvSandboxSpec, OSEnvSpec
...
# those onto codify's usage schema so pi sub-agent cost is priced the same
    ...
    *,
    ...
    msg: dict[str, object] = {
        ...
        "usage": {
            ...
            # ignores it (codify prices from token counts), but include
...
def test_pi_usage_captured_from_message_end() -> None:
    ...
    the codify schema key. Asserts the actual numbers so a swapped or
    ...
    async def _test() -> None:
        ...
        # Each value proves a specific pi-field → codify-key mapping:
