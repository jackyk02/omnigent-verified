...
These exercise the real :mod:`codify.inner.nessie.policies` evaluator
...
from codify.inner.nessie.policies import (
    blast_radius,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Pi native write/edit (lowercase) use ``path`` (Codify convention).
...
def test_worktree_guard_gates_native_write_edit(
    ...
    :param path_key: The argument key carrying the file path (``"file_path"``
        for Claude native, ``"path"`` for Codify built-in).
