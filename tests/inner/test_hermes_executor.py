...
from codify.inner.hermes_executor import (
    HermesExecutor,
    _build_hermes_args,
...
class TestSetupHermesHome:
    ...
    def test_config_registers_policy_hook(self, setup) -> None:
        ...
        assert len(hooks) == 1
        assert "codify-policy-hook.sh" in hooks[0]["command"]

    def test_config_registers_codify_mcp_server(self, setup) -> None:
        """config.yaml carries mcp_servers.codify (serve-mcp) pointed at the bridge
        dir — the parity gap. Fails on the previous setup, which wrote no mcp_servers
        key: a headless Hermes agent had zero Codify tools."""
        home, bridge_dir = setup
        codify_mcp = json.loads((home / "config.yaml").read_text())["mcp_servers"]["codify"]
        assert codify_mcp["args"][:2] == ["-m", "codify.claude_native_bridge"]
        assert "serve-mcp" in codify_mcp["args"]
        assert str(bridge_dir) in codify_mcp["args"]  # serve-mcp --bridge-dir <bridge_dir>
    ...
    def test_credentials_stay_off_the_predictable_bridge_path(self, setup, tmp_path) -> None:
        ...
        assert (home / "codify-policy-hook.sh").is_file()
