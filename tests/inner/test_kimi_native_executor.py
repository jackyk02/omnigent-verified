...
from codify import kimi_native_bridge
from codify.inner.kimi_native_executor import (
    KimiNativeExecutor,
    _content_to_text,
    _latest_user_text,
)
...
class TestSettlePaneReadiness:
    ...
    def test_marker_matches_live_kimi_footer(self) -> None:
        ...
        footer = (
            " K2.7 Code thinking  ~/codify  pr521-kimi-native [+61 -8]"
...
class TestRegistration:
    def test_harness_is_registered(self) -> None:
        ...
        assert _HARNESS_MODULES["kimi-native"] == "codify.inner.kimi_native_harness"

    def test_harness_is_allowlisted(self) -> None:
        from codify.spec._codify_compat import CODIFY_HARNESSES

        assert "kimi-native" in CODIFY_HARNESSES

    def test_kimi_native_is_terminal_native(self) -> None:
        # kimi-native launches the kimi TUI in an codify terminal (like
