...
from codify.inner.executor import ExecutorError, TextChunk, TurnComplete
from codify.inner.qwen_executor import QwenExecutor
...
async def test_run_turn_emits_usage_on_turn_complete(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    notified: list[dict] = []
    monkeypatch.setattr(
        "codify.inner.qwen_executor._notify_usage_from_dict",
...
async def test_run_turn_approval_does_not_count_against_idle_timeout(
    ...
    """
    monkeypatch.setattr("codify.inner.qwen_executor._PROMPT_TIMEOUT_SECONDS", 0.05)
...
def test_qwen_in_harness_allowlist() -> None:
    """'qwen' must be in CODIFY_HARNESSES."""
    from codify.spec._codify_compat import CODIFY_HARNESSES

    assert "qwen" in CODIFY_HARNESSES
...
def test_qwen_code_in_harness_aliases() -> None:
    """'qwen-code' must be in CODIFY_HARNESS_ALIASES."""
    from codify.spec._codify_compat import CODIFY_HARNESS_ALIASES

    assert "qwen-code" in CODIFY_HARNESS_ALIASES
...
def test_qwen_harness_creates_fastapi_app() -> None:
    ...
    from codify.inner.qwen_harness import create_app
...
def test_qwen_harness_module_importable() -> None:
    ...
    from codify.inner import qwen_harness
...
def test_parse_image_data_uri_edge_cases() -> None:
    ...
    from codify.inner.qwen_executor import _parse_image_data_uri
