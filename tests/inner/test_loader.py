...
from codify.inner.datamodel import ExecutorSpec, OSEnvSandboxSpec, OSEnvSpec
from codify.inner.loader import load_agent_def
from codify.inner.policies import FunctionPolicy, PromptPolicy
...
class TestInstructionsField(unittest.TestCase):
    """
    ``instructions:`` field handling in codify-flavored YAML.

    Native Codify YAMLs have always supported ``instructions: <path>``
    (path relative to the bundle dir, falling through to inline
    text if not a file). Codify-flavored YAMLs silently
    ...
    def test_instructions_inline_text_when_no_matching_file(self):
        ...
        Matches the native Codify behavior — silent fall-through to
    ...
    def test_instructions_resolves_relative_to_yaml_dir_not_cwd(self):
        ...
        A user can run ``codify run /elsewhere/agent.yaml`` from
...
class TestLoaderOsEnvValidation(unittest.TestCase):
    """Validate that ``inner.loader`` mirrors Codify parser sandbox checks.

    The legacy ``load_agent_def`` is what the CLI ``codify run``
    actually invokes (via the codify-compat shim). If the legacy
...
"""
    ...
    def test_load_agent_def_parses_credential_proxy(self):
        """Single-file codify YAML must parse ``credential_proxy``.

        Regression: this loader (the path ``codify run agent.yaml``
...
"""
    ...
    def test_load_agent_def_rejects_gh_basic_on_macos(self):
        ...
        ``codify run agent.yaml`` path) must fail loud at load time with the
...
def test_load_agent_def_allows_custom_handler_by_default() -> None:
    ...
    This is the operator/local ``codify run`` path — the custom
