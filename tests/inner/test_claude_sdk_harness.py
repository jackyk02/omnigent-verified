...
from codify.inner import claude_sdk_harness
...
def test_harness_module_registered_in_module_registry() -> None:
    ...
    """
    assert _HARNESS_MODULES.get("claude-sdk") == "codify.inner.claude_sdk_harness"
    assert _HARNESS_MODULES.get("claude") == "codify.inner.claude_sdk_harness"
...
def test_executor_factory_reads_env_vars(
    ...
    with patch(
        "codify.inner.claude_sdk_harness.ClaudeSDKExecutor.__init__",
        _fake_init,
    ):
...
def test_executor_factory_cwd_falls_back_to_runner_workspace(
    ...
    runner's ``CODIFY_RUNNER_WORKSPACE`` (the folder the user launched
...
def test_executor_factory_explicit_cwd_wins_over_workspace(
    ...
    ``CODIFY_RUNNER_WORKSPACE`` fallback."""
...
def test_executor_factory_decodes_os_env_json(
    ...
    Codify serializes ``spec.executor.config["os_env"]`` via
...
def test_skills_filter_env_var_decodes(
    ...
    The env-var bridge between the Codify runtime and the
