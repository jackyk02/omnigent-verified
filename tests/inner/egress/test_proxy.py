"""Tests for codify.inner.egress.proxy — MITM proxy with rule enforcement."""
...
from codify.inner.credential_proxy import (
    SYNTHETIC_CREDENTIAL_PREFIX,
    CredentialRewriteRule,
)
from codify.inner.egress.ca import ensure_ca, ensure_ca_bundle
from codify.inner.egress.certs import HostCertCache
from codify.inner.egress.proxy import EgressProxy
from codify.inner.egress.rules import parse_rules
...
def _basic_auth_header_bytes(token: str) -> bytes:
    ...
    when the proxy URL is ``http://codify:<token>@host:port``.
    """
    import base64

    return b"Basic " + base64.b64encode(f"codify:{token}".encode())
...
async def test_s4_proxy_returns_407_without_proxy_authorization(
    ...
    try:
        ...
        assert b'Proxy-Authenticate: Basic realm="codify"' in response
    finally:
        await proxy.stop()
...
def test_s4_check_proxy_auth_is_case_insensitive_on_header_name() -> None:
    ...
    proxy._expected_auth_value = b"Basic " + base64.b64encode(b"codify:tok")

    expected = b"Basic " + base64.b64encode(b"codify:tok")
