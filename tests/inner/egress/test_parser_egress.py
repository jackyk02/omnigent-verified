"""Tests for egress_rules parser validation in codify.spec.parser."""

from __future__ import annotations

import pytest

from codify.spec.parser import _parse_egress_rules, _parse_os_env_sandbox
...
def test_parse_egress_rules_not_a_list() -> None:
    """Non-list input raises CodifyError."""
    from codify.errors import CodifyError

    with pytest.raises(CodifyError, match="must be a list"):
        _parse_egress_rules("GET api.github.com/**")


def test_parse_egress_rules_non_string_entry() -> None:
    """Non-string entry raises CodifyError."""
    from codify.errors import CodifyError

    with pytest.raises(CodifyError, match="must be strings"):
        _parse_egress_rules([123])


def test_parse_egress_rules_invalid_syntax() -> None:
    """Invalid rule syntax raises CodifyError."""
    from codify.errors import CodifyError

    with pytest.raises(CodifyError, match="is invalid"):
...
def test_sandbox_egress_rules_rejected_for_non_filtering_backends() -> None:
    ...
    with pytest.raises(CodifyError, match=r"linux_bwrap.*darwin_seatbelt") as excinfo:
