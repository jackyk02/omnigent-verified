...
The walker in :mod:`codify.inner._cwd_scan` is consumed by every
...
from codify.inner._cwd_scan import MaskedEntry, merge_scan_roots, scan_cwd_mask_entries
...
def test_overflow_warn_returns_partial_mask_and_logs(
    ...
    for i in range(50):
        (tmp_path / f"file_{i}.txt").write_text("x")
    caplog.set_level("WARNING", logger="codify.inner._cwd_scan")
...
def test_overflow_warn_message_distinguishes_partial_and_bounds_list(
    ...
    caplog.set_level("CRITICAL", logger="codify.inner._cwd_scan")
