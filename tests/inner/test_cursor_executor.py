"""Tests for :class:`codify.inner.cursor_executor.CursorExecutor`.
...
from codify.inner.cursor_executor import (
    CursorExecutor,
...
def test_resolve_model_warns_when_dropping_a_pinned_model(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.inner.cursor_executor"):
...
async def test_custom_tool_execute_bridges_to_tool_executor() -> None:
    ...
    the main loop and resolve Codify's async ``_tool_executor``."""
    executor = CursorExecutor(api_key="crsr_x")
    seen: dict[str, Any] = {}
...
async def test_custom_tool_execute_times_out_to_iserror(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.inner.cursor_executor._TOOL_CALL_TIMEOUT_S", 0.05)
...
async def test_run_turn_captures_usage_from_turn_ended_update(
    ...
    monkeypatch.setattr(
        "codify.inner.cursor_executor._notify_usage_from_dict",
...
def test_cursor_policy_hook_allow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("_CODIFY_SERVER_URL", "http://localhost:6767")
    monkeypatch.setenv("_CODIFY_SESSION_ID", "conv_test")

    stdin_data = json.dumps({"tool_name": "Bash", "tool_input": {"command": "ls"}})

    from codify.inner import cursor_policy_hook
    ...
    with (
        ...
        patch(
            "codify.native_policy_hook.post_evaluate_with_retry",
...
def test_cursor_policy_hook_no_env_fails_open(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("_CODIFY_SERVER_URL", raising=False)
    monkeypatch.delenv("_CODIFY_SESSION_ID", raising=False)
...
def test_cursor_policy_hook_uses_long_read_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        patch.object(sys, "stdin", io.StringIO(stdin_data)),
        patch.object(sys, "stdout", stdout),
        patch("codify.native_policy_hook.post_evaluate_with_retry", mock_fn),
    ):
        cursor_policy_hook.main()
