"""
Tests for :mod:`codify.resume_dispatch` — the top-level
``codify resume`` dispatcher.
...
properties under test are (a) we always preserve the Codify
...
from codify import resume_dispatch
...
def test_run_resume_picker_form_requires_server() -> None:
    """
    ``codify resume`` (no conv id, no --server) must fail loud.

    Without ``target`` we'd open the cross-agent picker; without
    ``--server`` we have no Codify endpoint to query. Starting an
    empty local server just for the picker would race with any
    other ``codify`` process the user has running, so we
...
def test_dispatch_by_runtime_claude_native_remote_routes_to_wrapper(
    ...
    The Codify conv id MUST be preserved as ``session_id`` (the
    ...
    monkeypatch.setattr("codify.claude_native.run_claude_native", _capture)
    ...
    # session_id preserves the Codify conv id end-to-end.
...
def test_dispatch_by_runtime_opencode_native_routes_to_wrapper(
    ...
    ``opencode``, so an opencode-native resume fell through to the Codify
    ...
    monkeypatch.setattr("codify.opencode_native.run_opencode_native", _capture)
...
def test_dispatch_by_runtime_codex_native_remote_routes_to_wrapper(
    ...
    The Codify conv id must be preserved exactly like the
    ...
    monkeypatch.setattr("codify.codex_native.run_codex_native", _capture)
...
def test_dispatch_by_runtime_antigravity_native_remote_routes_to_wrapper(
    ...
    The Codify conv id must be preserved exactly like the codex/claude
    ...
    monkeypatch.setattr("codify.antigravity_native.run_antigravity_native", _capture)
...
def test_dispatch_by_runtime_non_wrapper_local_raises_with_hint(
    ...
    """
    Local non-wrapper conv surfaces the ``codify run --resume`` hint.
    ...
    assert "codify run --resume" in msg
...
def test_read_wrapper_label_local_reads_persistent_store(
    ...
    """
    Local dispatch classifies sessions from ``~/.codify/chat.db``.

    :param monkeypatch: Pytest monkeypatch fixture.
    :param tmp_path: Temporary persistent Codify directory.
    :returns: None.
    """
    ...
    monkeypatch.setattr(chat_mod, "_codify_persistent_dir", lambda: tmp_path)
...
def test_dispatch_by_runtime_non_claude_native_remote_raises_with_hint(
    ...
    copy-pasteable ``codify run --resume`` hint.
    ...
    monkeypatch.setattr("codify.claude_native.run_claude_native", _fail_if_called)
...
def test_read_wrapper_label_remote_returns_none_when_label_missing(
    ...
    """
    A conv with no ``codify.wrapper`` label returns ``None``, which
