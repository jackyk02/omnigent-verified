"""
Tests for ``_build_cursor_spawn_env`` in ``codify/runtime/workflow.py``.
...
from codify.runtime.workflow import _build_cursor_spawn_env
...
def _isolate_global_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Point CODIFY_CONFIG_HOME at an empty temp dir so the developer's real
    ``~/.codify/config.yaml`` can't leak in, and clear any ambient
...
def _write_cursor_config(tmp_path: Path, ref: str) -> None:
    ...
    :param tmp_path: The isolated ``CODIFY_CONFIG_HOME`` (see the autouse
        fixture).
...
def test_stored_cursor_key_used_when_spec_has_no_auth(
    ...
    """A CURSOR_API_KEY registered via ``codify setup`` flows when the spec
...
def test_stored_cursor_key_wins_over_ambient_when_both_set(
    ...
    """With no spec auth, the stored ``cursor:`` key (registered via ``codify
