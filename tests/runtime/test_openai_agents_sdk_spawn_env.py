...
from codify.runtime.workflow import _build_openai_agents_sdk_spawn_env, _load_global_auth
...
def _isolate_global_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """
    Point CODIFY_CONFIG_HOME at an empty temp dir for every test in
    this file so tests that don't explicitly set up a global config are
    not affected by the developer's real ``~/.codify/config.yaml``.

    Tests that need a specific global config write their own config.yaml
    into a separate temp dir and set CODIFY_CONFIG_HOME themselves —
...
def test_use_responses_config_value_is_interpreted_as_boolean(
    ...
    monkeypatch.setattr(
        "codify.runtime.workflow._resolve_provider_for_build",
...
def test_databricks_model_ignores_env_profile(
    ...
    routing — credentials are controlled by the spec or by ``codify
...
def test_profile_injects_ucode_state(
    ...
    monkeypatch.setattr(
        "codify.runtime.workflow.get_workspace_url_for_profile",
        lambda profile: "https://example.databricks.com",
    )
    monkeypatch.setattr(
        "codify.runtime.workflow.read_ucode_state",
...
def test_spec_auth_takes_precedence_over_global_config(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        monkeypatch.setenv("CODIFY_CONFIG_HOME", td)
...
def test_global_config_auth_not_applied_when_spec_has_legacy_profile(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Spec declares profile via the legacy config dict (codify compat path).
...
def test_load_global_auth_api_key_with_base_url(
    ...
    ``~/.codify/config.yaml`` via an env-var reference has the
