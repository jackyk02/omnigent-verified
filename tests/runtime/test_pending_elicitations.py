"""
Unit tests for :mod:`codify.runtime.pending_elicitations`.
...
The wire-up between :func:`codify.runtime.session_stream.publish`
...
def test_resolve_removes_outstanding_id() -> None:
    ...
    the Codify server's approval dispatch calls
...
def test_record_publish_clears_index_on_elicitation_resolved_event() -> None:
    ...
    own approval wait — that's the only signal the Codify server gets
...
def test_record_publish_handles_resolved_event_for_unknown_id() -> None:
    ...
    Codify server's view of what's currently tracked.
    """
    pending_elicitations.record_publish(
        "conv_a",
        {
...
def test_set_elicitation_observer_none_clears_registration() -> None:
    ...
    # Codify server lifespans (the lifespan callsite calls clear at
    # shutdown).
    assert received == ["conv_a"]
