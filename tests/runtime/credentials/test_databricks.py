"""Tests for codify.runtime.credentials.databricks."""
...
def test_databricks_config_profile_env_var_typo_raises_not_falls_back_to_default(
    ...
    # Simulates: `codify run foo.yaml --profile typo-profile --model databricks/m`
...
def test_oauth_profile_error_recommends_extra_when_sdk_absent(
    ...
    monkeypatch.setattr(
        "codify.runtime.credentials.databricks._databricks_sdk_importable",
        lambda: False,
    )
    ...
    assert "codify[databricks]" in msg
...
def test_sdk_value_error_does_not_emit_warning(
    ...
    with caplog.at_level(logging.DEBUG, logger="codify.runtime.credentials.databricks"):
        resolve_databricks_workspace(profile="dev")

    module_records = [
        r for r in caplog.records if r.name == "codify.runtime.credentials.databricks"
    ]
