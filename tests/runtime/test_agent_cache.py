"""Tests for codify.runtime.agent_cache."""
...
def test_load_invalid_spec_raises_codify_error(
    agent_cache: AgentCache,
    artifact_store: LocalArtifactStore,
) -> None:
    """
    ``load()`` raises ``CodifyError`` when the extracted spec
    is invalid.
    ...
    "tools/mcp/leaky.yaml": yaml.dump(
        {
            ...
            "headers": {"Authorization": "Bearer ${CODIFY_W7_TEST_SECRET}"},
        }
    ),
}
...
def _mcp_auth_header(loaded_spec_servers: list[object]) -> str:
    ...
    :returns: The header value, e.g. ``"Bearer ${CODIFY_W7_TEST_SECRET}"``
        when unexpanded.
    """
...
def test_load_does_not_expand_env_by_default(
    ...
    assert header == "Bearer ${CODIFY_W7_TEST_SECRET}"
