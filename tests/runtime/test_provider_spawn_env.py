...
builders in ``codify/runtime/workflow.py``.

Chunk 1b wires the kind-typed provider config
(``codify/onboarding/provider_config.py``) into the four
...
def _clear_ambient_keys(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "codify.runtime.workflow._resolve_catalog_default_model",
...
def config_home(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """
    Point ``$CODIFY_CONFIG_HOME`` at an isolated temp dir.
...
def _write_config(config_home: Path, config: dict[str, object]) -> None:
    ...
    :param config_home: The ``$CODIFY_CONFIG_HOME`` directory.
...
def _make_spec(
    *,
    ...
    return AgentSpec(
        ...
        executor=ExecutorSpec(type="codify", config=config, model=model, auth=auth),
...
def test_global_databricks_auth_beats_ambient_key(
    ...
    Regression guard for the databricks/ucode user: ``codify setup``
...
def test_codex_falls_back_to_first_available_openai_credential(
    ...
    ``codify setup`` (a Databricks workspace, or any key/gateway) but never
...
def test_catalog_default_fails_clearly_when_discovery_is_unavailable(
    ...
    monkeypatch.setattr("codify.onboarding.providers.get_chat_models", lambda _provider: [])
...
def test_databricks_kind_default_routes_through_profile(
    ...
    import codify.runtime.workflow as workflow_mod
...
def test_openai_agents_cli_config_default_fails_loud(config_home: Path) -> None:
    ...
    with pytest.raises(CodifyError, match=r"cli-config.*codex"):
...
def _isolate_home_with_codex_config(config_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    :param config_home: The isolated ``CODIFY_CONFIG_HOME`` directory,
...
def test_kimi_no_provider_emits_no_gateway_vars(config_home: Path) -> None:
    ...
    provider config from ``~/.kimi/config.toml``; Codify never injects."""
    _write_config(config_home, {"providers": {}})
    spec = _make_spec(harness="kimi")
...
def test_kimi_declared_auth_raises(
    ...
    instead. Regression guard for the originally-dead ``CodifyError``."""
    from codify.errors import CodifyError

    _write_config(config_home, {"providers": {}})
    spec = _make_spec(harness="kimi", auth=auth)

    with pytest.raises(CodifyError, match=r"kimi.*does not support"):
...
# ---------------------------------------------------------------------------
# harness.<canonical>.command → CODIFY_<NAME>_PATH (spawn-env builders)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ("codex", _build_codex_spawn_env, "CODIFY_CODEX_PATH"),
        ("pi", _build_pi_spawn_env, "CODIFY_PI_PATH"),
        ("kimi", _build_kimi_spawn_env, "CODIFY_KIMI_PATH"),
        ("goose", _build_goose_spawn_env, "CODIFY_GOOSE_PATH"),
        ("qwen", _build_qwen_spawn_env, "CODIFY_QWEN_PATH"),
    ],
)
def test_spawn_env_threads_config_command_to_path(
    ...
    """A config ``harness.<canonical>.command`` lands as ``CODIFY_<NAME>_PATH``."""
...
def test_spawn_env_ambient_env_wins_over_config_command(
    ...
    """The ambient ``CODIFY_<NAME>_PATH`` env var wins over config ``command``."""
    cfg = _openai_default_config()
...
def test_spawn_env_no_command_emits_no_path(
    ...
    # Neither the canonical CODIFY_* nor the legacy HARNESS_* is emitted.
    assert f"CODIFY_{suffix}_PATH" not in env
...
def test_spawn_env_legacy_env_wins_over_config_command(
    ...
    """
    from codify.harness_startup_config import _LEGACY_PATH_WARNED

    legacy_var = f"HARNESS_{harness.upper()}_PATH"
    _LEGACY_PATH_WARNED.discard(legacy_var)
    monkeypatch.delenv(f"CODIFY_{harness.upper()}_PATH", raising=False)
    ...
    # The builder must not set CODIFY_* from config when the legacy env wins.
    assert f"CODIFY_{harness.upper()}_PATH" not in env
