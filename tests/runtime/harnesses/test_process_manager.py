"""
Tests for :class:`codify.runtime.harnesses.process_manager.HarnessProcessManager`.
...
Tests intentionally use a short tmp parent dir (``/tmp/codify-tests-...``)
...
from codify.runtime.harnesses.process_manager import (
    _AP_PID_FILE,
    _TMP_PARENT_ENV_VAR,
...
async def test_start_creates_instance_dir_with_sentinel(
    ...
    a subsequent Codify boot can't tell live instances from dead.
    """
    await manager.start()
    try:
        ...
        # a sibling Codify boot would correctly identify us as alive.
...
def test_default_tmp_parent_is_per_uid_on_posix(
    ...
    A shared ``/tmp/codify`` breaks multi-user hosts: the first
...
async def test_idle_reaper_disabled_when_timeout_zero(
    ...
    """A non-positive idle window disables reaping (``CODIFY_HARNESS_IDLE_TIMEOUT_S=0``).
...
async def test_orphan_sweep_removes_dead_codify_dirs(
    short_tmp_parent: Path,
) -> None:
    """A sibling Codify dir with a non-running PID gets cleaned.
...
async def test_orphan_sweep_preserves_live_codify_dirs(
    short_tmp_parent: Path,
) -> None:
    """A sibling Codify dir with a live PID is left alone.
    ...
    *this test process* (which is live by definition). The
    ...
    await fresh.start()
    try:
        # If sweep removed the sibling, a concurrent Codify would
...
async def test_get_client_env_override_propagates_to_subprocess(
    ...
    Verifies the v1 spec-config flow: Codify passes per-spec env vars
    via ``env`` to ``get_client``, and the spawned subprocess
    sees them in its own ``os.environ``. Without this propagation,
    Codify would have to mutate its own ``os.environ`` (which races
...
async def test_runner_subprocess_exits_on_sigterm(
    ...
    ``codify.runtime.harnesses._runner`` processes alive because
...
async def test_runner_subprocess_hard_exits_when_sigterm_shutdown_wedges(
    ...
    ``pkill -f codify.runtime.harnesses._runner``: the runner
    should not remain alive forever just because graceful shutdown is
    stuck.
    """
    monkeypatch.setenv("CODIFY_HARNESS_SHUTDOWN_TIMEOUT_S", "0.2")
    monkeypatch.setenv("CODIFY_HARNESS_HARD_EXIT_TIMEOUT_S", "0.5")
    await manager.start()
    try:
...
async def test_orphan_sweep_escalates_to_sigkill(
    ...
    from codify.runtime.harnesses import process_manager as pm_mod
