...
from codify.runtime.harnesses import _runner
...
def test_load_harness_app_import_error_exits(
    ...
    with pytest.raises(SystemExit) as excinfo:
        _runner._load_harness_app("missing", "codify.does_not_exist", "conv_x")
    ...
    assert "'codify.does_not_exist'" in err
...
def test_load_harness_app_module_without_create_app_exits(
    ...
    (``codify.errors``) reproduces the failure mode.
    """
    with pytest.raises(SystemExit) as excinfo:
        _runner._load_harness_app("broken", "codify.errors", "conv_x")
    assert excinfo.value.code == 2
    err = capsys.readouterr().err
