"""
Tests for :class:`codify.runtime.harnesses._executor_adapter.ExecutorAdapter`.

End-to-end through real subprocesses spawned via the same
:class:`HarnessProcessManager` used in production. Uses
:class:`codify.inner.executor.MockExecutor` as the inner
...
def test_build_error_detail_uses_codify_error_code() -> None:
    """
    :class:`CodifyError` (and its
    ...
    """
    from codify.llms.errors import LLMErrorDetail, RetryableLLMError
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
    from codify.runtime.harnesses._scaffold import HarnessApp
    ...
    # Code preserved verbatim — the Codify allowlist matches this and
    # marks the failure retryable. Class-name fallback (which the
    # base HarnessApp implementation would have used) gives
    # ``"RetryableLLMError"`` instead, which Codify would NOT match.
...
def test_classify_openai_exception_maps_known_types() -> None:
    ...
    from codify.runtime.harnesses._executor_adapter import (
...
def test_translate_input_to_messages_string_input_fallback() -> None:
    ...
    the original shape (a bare string is the Codify API's shorthand
...
def test_translate_input_to_messages_legacy_content_blocks_fallback() -> None:
    ...
    The pre-history-fix wire format. Codify clients that haven't
...
def test_translate_event_mcp_tool_call_request_emits_observed_with_bare_name() -> None:
    ...
    name (no ``mcp__codify__`` prefix), inline.

    What this proves: the user sees ``⏵ sys_terminal_launch`` in
    the REPL, not ``⏵ mcp__codify__sys_terminal_launch`` — the
    Codify wire shape and persisted store items carry the bare name
    (per ``codify/runtime/workflow.py``'s
    ...
    emission consistent with the rest of the Codify wire path.
    """
    from codify.inner.executor import ToolCallRequest
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
    ...
    event = ToolCallRequest(
        name="mcp__codify__sys_terminal_launch",
    ...
    assert item["name"] == "sys_terminal_launch", (
        ...
        f"``mcp__codify__`` prefix); got {item['name']!r}. "
        f"If the prefix leaked through, the REPL would render "
        f"``⏵ mcp__codify__sys_terminal_launch`` instead of "
...
def test_tool_call_complete_suppressed_for_dispatched_executor() -> None:
    ...
    from codify.inner.executor import ToolCallComplete, ToolCallStatus
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
...
def test_translate_event_non_mcp_request_queues_tool_use_id() -> None:
    ...
    observed event's call_id and the Codify REPL can dedupe.
    ...
    uuid in :func:`_bridge_one_dispatch`, the Codify client saw
    ...
    assert list(adapter._pending_mcp_call_ids) == ["call_openai_xyz"], (
        ...
        f"this, the Codify client receives two function_call events "
...
async def test_stable_tool_executor_pops_queue_for_bare_tool_name() -> None:
    ...
    server wrapper strips the ``mcp__codify__`` prefix
    before invoking the tool callback. So
    ``_stable_tool_executor`` receives ``"sys_terminal_launch"``,
    NOT ``"mcp__codify__sys_terminal_launch"``. An earlier
...
async def test_observed_and_dispatched_call_ids_match_for_openai_agents() -> None:
    ...
    - Codify client sees A != B → no dedup → REPL renders ⏵ twice
    ...
    the dispatch reuses A → Codify client dedupes → single ⏵ render.
    ...
    assert captured_dispatched_call_ids == [sdk_call_id], (
        ...
        f"the Codify client can dedupe. Observed call_id was "
...
async def test_executor_adapter_builds_config_from_request() -> None:
    ...
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
    from codify.runtime.harnesses._scaffold import TurnContext
    from codify.server.schemas import CreateResponseRequest
...
async def test_watch_injections_emits_consumed_marker_on_accept() -> None:
    ...
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
    from codify.server.schemas import CreateResponseRequest, InjectionConsumedEvent
...
def test_internal_errored_tool_complete_emits_output_with_real_call_id() -> None:
    ...
    """
    from codify.inner.executor import ToolCallComplete, ToolCallRequest, ToolCallStatus
    from codify.runtime.harnesses._executor_adapter import ExecutorAdapter
