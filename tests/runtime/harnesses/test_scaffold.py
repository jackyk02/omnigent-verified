"""
Tests for :class:`codify.runtime.harnesses._scaffold.HarnessApp`.
...
from codify.runtime.harnesses._scaffold import HarnessApp, TurnContext
...
_TRUNCATION_MARKER = "[output truncated by codify:"
...
async def test_session_events_404s_on_conversation_id_mismatch(
    ...
    body = resp.json()
    # Error envelope shape matches CodifyError's serialization
    # (see _handle_codify_error). Without this, AP-side error
...
async def test_session_interrupt_event_404s_when_no_turn_in_flight(
    ...
    than silently no-op so a stray interrupt from Codify after a turn
...
async def test_idle_watchdog_attaches_recent_forwarder_post_failure(
    ...
    from codify.runtime.harnesses import _scaffold
