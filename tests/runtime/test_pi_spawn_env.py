"""
Tests for ``_build_pi_spawn_env`` in ``codify/runtime/workflow.py``.
...
from codify.runtime.workflow import _build_pi_spawn_env
from codify.spec.types import AgentSpec, ExecutorSpec, LLMConfig
...
def _isolate_global_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    this file so the developer's real ``~/.codify/config.yaml`` (e.g.
...
def _make_spec(*, model: str | None = None, profile: str | None = None) -> AgentSpec:
    ...
    return AgentSpec(
        ...
        executor=ExecutorSpec(type="codify", config=config, model=model),
