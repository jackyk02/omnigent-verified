...
Ports these codify ``test_labels_and_policies.py`` cases:
...
from codify.policies.function import FunctionPolicy
...
def test_elicitation_request_event_url_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("codify.runtime.policies.approval._ELICITATION_MODE", "url")
    req = ElicitationRequest(
...
def test_elicitation_request_event_form_mode_explicit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    provided.
    """
    monkeypatch.setattr("codify.runtime.policies.approval._ELICITATION_MODE", "form")
    req = ElicitationRequest(
        message="approve?",
        phase="tool_call",
        policy_names=["gate"],
...
async def test_timeout_does_not_apply_labels(
    ...
    """Ports codify ``test_ask_timeout``. Park raises
...
async def test_missing_verdict_row_denies(
    ...
    """Ports codify ``test_no_handler_denies``. Park
