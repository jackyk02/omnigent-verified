...
Ports and extends these codify cases:
...
from codify.policies.types import EvaluationContext, PolicyResult
...
async def test_sync_callable_allow() -> None:
    """Ports codify ``test_sync_callable_allow``. A sync
...
async def test_sync_callable_block() -> None:
    """Ports codify ``test_sync_callable_block``. A sync
...
async def test_async_callable() -> None:
    """Ports codify ``test_async_callable``. An async
...
async def test_callable_returns_dict_allow() -> None:
    """Ports codify ``test_callable_returns_dict``. A
...
async def test_callable_returns_dict_deny_with_reason() -> None:
    """Ports codify ``test_deny_action_from_dict``. A
...
async def test_callable_returns_foreign_policy_result_shape() -> None:
    ...
    ``PolicyResult`` from ``codify.policies`` — a different
    module than the engine's ``codify.policies.types`` so
    ``isinstance`` fails.

    Uses a local stand-in dataclass so the test doesn't
    depend on whether codify is installed in this
    ...
    class _ForeignAction(enum.Enum):
        """
        Mimics ``codify.policies.PolicyAction`` — wire
        values match Codify', but the enum class is
    ...
    class _ForeignPolicyResult:
        """
        Mimics ``codify.policies.PolicyResult`` — same
...
async def test_two_arg_callable_reads_config_for_decision(
    ...
    """Ports codify ``test_three_arg_callable_reads_labels_for_decision``.
...
async def test_async_two_arg_callable() -> None:
    """Ports codify ``test_three_arg_async_callable``.
...
async def test_rate_limit_closure_counts() -> None:
    """Ports codify ``test_tool_call_rate_limit``. A
...
def test_resolve_function_policy_short_form(tmp_path: Path) -> None:
    ...
    _install_module(
        ...
        "probe",
        """
        from codify.policies.types import PolicyResult
...
async def test_factory_closure_counter_isolated_per_build(
    ...
    """Ports codify ``test_rate_limit_counter_isolated``.
...
def test_function_policy_reset_turn_invokes_callable_attribute(
    ...
    codify policies like ``max_tool_calls_per_turn`` clear
    per-turn accumulators between turns — see
    :meth:`codify.runtime.policies.engine.PolicyEngine.reset_turn`
