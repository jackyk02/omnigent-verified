...
codify-format example YAMLs under ``examples/*.yaml``.

Complements :mod:`test_enforcement_integration`, which loads
pre-translated codify-native fixtures. The fixtures there
are hand-maintained ports; any bug in the codify → codify
adapter layer (e.g. ``condition: {}`` parse rejection,
``match_tools`` → ``on:`` expansion) slips past those tests. This module goes through
:func:`codify.spec.load` — the same path ``codify run``
...
from codify.runtime.policies import (
    _enforce_policy,
    build_policy_engine,
)
from codify.runtime.policies.engine import PolicyEngine
from codify.spec import load
from codify.spec.types import Phase, PolicyAction
...
def _load_engine_from_yaml(
    ...
    """
    Parse an codify-format example YAML and build a real
    :class:`PolicyEngine` bound to a fresh conversation.

    Goes through :func:`codify.spec.load`, so the
    ``_codify_compat`` adapter runs on every call. A bug
...
async def test_agent_with_policies_allows_short_sleep(
    ...
    codify YAML ends at ALLOW for an in-bounds duration.
    """
...
async def test_agent_with_policies_denies_long_sleep(
    ...
    Codify' engine: it DOES. The engine calls 2-arg
    ...
    Claim: the codify YAML + its legacy-style example
    callable actually works through Codify' engine. If
...
# composition is already covered at the codify-native
...
async def test_secure_research_clean_shell_allows(
    ...
    codify ``labels:`` block and the enforcement chain
...
# which collides with an codify reserved builtin name. The
