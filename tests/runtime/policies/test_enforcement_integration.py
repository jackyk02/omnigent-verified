...
Loads agent fixtures ported from codify examples, builds
...
Fixture parity with codify example YAMLs:

- ``tests/_fixtures/agents/policies-demo/`` ↔
  ``codify/examples/agent_with_policies.yaml``
- ``tests/_fixtures/agents/rate-limited-search/`` ↔
  ``codify/examples/rate_limited_search_agent.yaml``
- ``tests/_fixtures/agents/secure-research/`` ↔
  ``codify/examples/secure_research_agent.yaml``

Corresponding codify test cases ported:
...
async def test_policies_demo_allows_short_sleep(
    ...
    FunctionPolicy. Mirrors the codify "Allowed" usage
...
async def test_policies_demo_denies_long_sleep(
    ...
    codify "Blocked tool call" example at the top of
...
async def test_rate_limited_search_first_three_allowed(
    ...
    """Ports codify ``test_first_db_query_allowed_but_escalates``
...
async def test_rate_limited_search_fourth_asks(
    ...
    """Ports codify ``test_second_db_query_requires_ask``
...
async def test_secure_research_clean_agent_allows_shell(
    ...
    """Ports codify ``test_clean_agent_calls_freely``. An
...
async def test_secure_research_web_then_shell_asks(
    ...
    of codify ``test_full_pipeline_happy_path``."""
    engine = _load_engine("secure-research", conversation_store)
