...
- :func:`codify_client._sse._parse_event` —
...
# The editable-install of ``codify_client`` points at the
...
_SDK_ROOT = Path(__file__).resolve().parents[2].parent / "sdks" / "python-client" / "codify_client"


def _load_sdk_module(name: str) -> Any:
    """
    Load ``codify_client.<name>`` from this worktree
    regardless of which ``codify_client`` is resolved
...
def _load_repl_module() -> Any:
    """
    Reload ``codify.repl._repl`` so these tests see the
    edited source. Multiple tests in this file touch the
    module; a stale import cache would silently test the old
    API.

    :returns: The freshly-reloaded ``codify.repl._repl``
        module.
    """
    import importlib

    import codify.repl._repl as repl_mod
