"""Unit tests for codify.runtime.compaction."""
...
from codify.llms.errors import RetryableLLMError
from codify.llms.types import MessageOutput, OutputText, Response
...
from codify.runtime.content_resolver import _resolve_file_id_block
from codify.runtime.workflow import _route_bare_model_for_compaction
from codify.spec.types import CompactionConfig, LLMConfig
...
async def test_no_compaction_under_threshold(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.runtime.compaction.count_tokens", lambda msgs, model: 50)
...
async def test_layer2_triggers_when_layer1_insufficient(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.runtime.compaction.count_tokens", mock_count_tokens)
    ...
    monkeypatch.setattr(
        "codify.runtime.compaction.summarize_history",
        _stub_summarize,
    )
...
async def test_layer3_truncation_preserves_tool_call_pairs(
    ...
    monkeypatch.setattr(
        "codify.runtime.compaction.count_tokens",
        mock_count_tokens,
    )
...
async def test_declared_window_keeps_large_fill_under_budget(
    ...
    """
    monkeypatch.setattr("codify.runtime.compaction.count_tokens", lambda msgs, model: 197_000)
    messages = [_user_msg_dict("hi"), _assistant_msg_dict("hello")]
    history = [_user_msg("msg_001", "hi"), _assistant_msg("msg_002", "hello")]
