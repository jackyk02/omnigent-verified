"""Tests for :mod:`codify.runtime.filesystem_registry`.
...
def test_git_list_changed_files_raises_on_timeout(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codify.runtime.filesystem_registry.subprocess.run", _raise_timeout)
...
def test_git_list_changed_files_raises_on_nonzero_exit(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codify.runtime.filesystem_registry.subprocess.run", _nonzero)
...
def test_git_timeout_seconds_default_and_env_override(monkeypatch) -> None:
    ...
    """
    monkeypatch.delenv("CODIFY_GIT_STATUS_TIMEOUT_SECONDS", raising=False)
    assert _git_timeout_seconds() == pytest.approx(30.0)

    monkeypatch.setenv("CODIFY_GIT_STATUS_TIMEOUT_SECONDS", "90")
    assert _git_timeout_seconds() == pytest.approx(90.0)

    for bad in ("not-a-number", "0", "-5", ""):
        monkeypatch.setenv("CODIFY_GIT_STATUS_TIMEOUT_SECONDS", bad)
        assert _git_timeout_seconds() == pytest.approx(30.0), (
...
def test_git_list_changed_files_honors_env_timeout(tmp_path: Path, monkeypatch) -> None:
    ...
    env = _git_env()
    subprocess.run(["git", "init"], cwd=tmp_path, check=True, capture_output=True, env=env)
    monkeypatch.setenv("CODIFY_GIT_STATUS_TIMEOUT_SECONDS", "42")
    ...
    monkeypatch.setattr("codify.runtime.filesystem_registry.subprocess.run", _capture)
...
def test_untracked_cache_failure_does_not_break_init(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codify.runtime.filesystem_registry.subprocess.run", _raise_oserror)
...
def test_untracked_cache_config_written_once_per_root(tmp_path: Path, monkeypatch) -> None:
    ...
    """
    from codify.runtime import filesystem_registry as fsr
...
def test_git_common_dir_resolves_linked_worktree(tmp_path: Path) -> None:
    ...
    from codify.runtime.filesystem_registry import _git_common_dir
...
def test_git_line_counts_numstat_failure_degrades_but_status_intact(
    ...
    monkeypatch.setattr("codify.runtime.filesystem_registry.subprocess.run", _fail_numstat)
