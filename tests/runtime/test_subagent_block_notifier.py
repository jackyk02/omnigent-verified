"""
Unit tests for :mod:`codify.runtime.subagent_block_notifier`.
...
from codify.entities.conversation import Conversation
from codify.runtime import pending_elicitations, subagent_block_notifier
from codify.runtime.subagent_block_notifier import (
...
async def _instant_sleep(_seconds: float) -> None:
    ...
    Patched in over :func:`codify.runtime.subagent_block_notifier._sleep`
...
async def conv_store(tmp_path: Path) -> AsyncIterator[SqlAlchemyConversationStore]:
    ...
    same ``get_conversation`` path the Codify server uses; a test-only
...
async def test_observe_retries_then_releases_arm_when_dispatch_raises(
    ...
    with caplog.at_level(logging.WARNING, logger="codify.runtime.subagent_block_notifier"):
        notifier.observe(child.id, _request_event("elicit_fail"))
