...
from codify import cursor_native_bridge as cnb
from codify import cursor_native_permissions as cnp
...
async def test_run_one_approval_posts_then_sends_verdict_keystroke(
    ...
    prompt = CursorApprovalPrompt(
        ...
        preview="echo codify_probe > out.txt",
        accept_key="y",
        decline_key="Escape",
    )
...
def test_iter_embedded_json_recovers_from_enclosing_garbage() -> None:
    ...
    assert "codify-list_comments" in names
...
async def test_supervise_transcript_yolo_auto_accepts_without_card(
    ...
    long enough for Codify to otherwise mirror an ApprovalCard and stall a
