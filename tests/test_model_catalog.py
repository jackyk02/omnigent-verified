"""Unit tests for ``codify/model_catalog.py``.
...
import codify.model_catalog as model_catalog
...
from codify.model_resolver import ModelResolutionError, ModelResolutionSource
from codify.onboarding.providers import ModelInfo
from codify.runtime.credentials.databricks import WorkspaceCreds
from codify.spec.types import AgentSpec, ApiKeyAuth, DatabricksAuth, ExecutorSpec
...
def _worker_spec(harness: str, **executor_kwargs: object) -> AgentSpec:
    ...
    return AgentSpec(
        spec_version=1,
        name="worker",
        executor=ExecutorSpec(type="codify", config={"harness": harness}, **executor_kwargs),  # type: ignore[arg-type]
    )
...
def test_resolve_provider_cursor_is_cli_login(
    ...
    cursor-agent has no codify-side provider config — it always brings
...
def test_resolve_provider_legacy_profile(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(
            type="codify", config={"harness": "codex-native", "profile": "legacy-prof"}
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            SimpleNamespace(executor=SimpleNamespace(type="codify", config={})),
            "claude-native",
            id="structural-stub",
        ),
    ],
...
def test_subscription_listing_is_static_and_unverified(
    ...
    assert payload["static_fallback"] == {
        ...
        "provenance": "Codify's release-curated Claude Code alias catalog",
...
def test_cursor_listing_uses_live_cli_base_models(
    ...
    :param tmp_path: Per-test temp dir.
    """
    from codify import cursor_native
...
def test_failed_auth_command_note_never_leaks_the_command(
    ...
    parent = AgentSpec(
        spec_version=1,
        name="orchestrator",
        executor=ExecutorSpec(type="codify", config={"harness": "pi"}),
...
def test_bundled_catalog_entries_normalize_capabilities_context_and_cost(
    ...
    monkeypatch.setattr("codify.onboarding.providers.get_chat_models", lambda _provider: models)
...
def test_resolve_catalog_default_preserves_provider_policy(
    ...
    monkeypatch.setattr("codify.onboarding.providers.get_chat_models", lambda provider: models)
...
def test_resolve_catalog_default_preserves_provider_tier_policy(
    ...
    monkeypatch.setattr(
        "codify.onboarding.providers.get_chat_models",
        lambda _provider: [
...
def test_resolve_catalog_model_fails_when_discovery_is_empty(
    ...
    monkeypatch.setattr("codify.onboarding.providers.get_chat_models", lambda provider: [])
