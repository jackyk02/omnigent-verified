"""Tests for the OpenCode SSE -> Codify event forwarder translation."""
...
import codify.opencode_native_forwarder as fwd_mod
from codify.opencode_native_client import OpenCodeEvent

_SESSION = "ses_1"


class _RecordingServerClient:
    """httpx-shaped stub recording Codify event POSTs."""
...
async def test_user_text_part_is_mirrored_before_the_assistant() -> None:
    ...
    For native-server harnesses codify persists no separate user item, so the
...
async def test_permission_asked_rejects_when_no_policy_wired() -> None:
    ...
    assert opencode.replies == [("per_1", {"reply": "reject", "message": "codify-policy"})]
...
async def test_permission_asked_passes_normalized_input_to_evaluator() -> None:
    ...
    assert seen[0]["codify_session_id"] == "conv_1"
...
async def test_model_switched_mirrors_to_codify_and_dedupes() -> None:
