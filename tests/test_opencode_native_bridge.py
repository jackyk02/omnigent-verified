...
from codify import opencode_native_bridge as bridge
...
def test_write_opencode_policy_plugin(bridge_dir: Path) -> None:
    ...
    assert path.name == "codify-policy.js"
    ...
    assert "CODIFY_POLICY_URL" in src and "CODIFY_SESSION_ID" in src
    assert "/policies/evaluate" in src
    # A function export so opencode's Object.values(mod) loader picks it up.
    assert "export const CodifyPolicyPlugin" in src
...
def test_policy_plugin_merges_routing_headers(bridge_dir: Path) -> None:
    """The generated policy plugin merges CODIFY_POLICY_HEADERS into its
    ...
    assert "CODIFY_POLICY_HEADERS" in src
    ...
    assert "CODIFY_POLICY_AUTH" not in src
