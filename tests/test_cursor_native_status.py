...
1. :mod:`codify.cursor_native_status` — the turn-end marker store + poster
   state (record/count markers, read/write/clear the posted-count).
2. :func:`codify.cursor_native_usage._cli_record_usage` — the cursor ``stop``
...
3. :func:`codify.cursor_native_forwarder.forward_cursor_store_to_session` —
...
from codify import cursor_native_status as status
...
def test_cli_record_usage_records_turn_end(tmp_path: Path, monkeypatch) -> None:
    ...
    from codify import cursor_native_usage
