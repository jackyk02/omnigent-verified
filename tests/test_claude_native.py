...
def test_claude_terminal_request_pins_launch_cwd(tmp_path, monkeypatch) -> None:
    ...
    body = claude_native._claude_terminal_request(
        ...
        command="claude",
        bridge_dir=Path("/tmp/codify-test-bridge"),
    )
    ...
    assert mcp_config["mcpServers"]["codify"]["args"] == [
        "-I",
        "-m",
        "codify.claude_native_bridge",
        "serve-mcp",
        "--bridge-dir",
        "/tmp/codify-test-bridge",
    ]
...
def test_claude_terminal_request_default_launch_is_unwrapped(tmp_path, monkeypatch) -> None:
    """Without ``CODIFY_CLAUDE_LAUNCHER`` the command/args are unchanged."""
    monkeypatch.delenv("CODIFY_CLAUDE_LAUNCHER", raising=False)
    monkeypatch.chdir(tmp_path)
    body = claude_native._claude_terminal_request(
        ("--resume", "s"),
        command="claude",
...
def test_claude_terminal_request_launcher_plugin_wraps(tmp_path, monkeypatch) -> None:
    ...
    (here ``isaac -- <augmented args>``) while the Codify bridge
    (``--mcp-config`` / ``--settings``) survives intact in the passed-through
    argv.
    """

    from codify.claude_launcher import ClaudeLauncher
    ...
    monkeypatch.setenv("CODIFY_CLAUDE_LAUNCHER", "isaac")
    monkeypatch.chdir(tmp_path)
    body = claude_native._claude_terminal_request(
        ("--resume", "s"),
        command="claude",
...
def test_claude_terminal_request_injects_claude_config() -> None:
    ...
    This test pins the native ``codify claude`` launch boundary:
...
def test_ucode_config_for_profile_reads_allowlisted_claude_state(
    ...
    """
    from codify.onboarding.ucode_state import UcodeAgentState, UcodeWorkspaceState
    ...
    monkeypatch.setattr(
        "codify.onboarding.databricks_config.get_workspace_url_for_profile",
        lambda profile: "https://example.databricks.com",
    )
    monkeypatch.setattr(
        "codify.onboarding.ucode_state.read_ucode_state",
...
def test_ucode_config_refreshes_live_models_and_builds_picker_options(
    ...
    monkeypatch.setattr(
        "codify.runtime.credentials.databricks.resolve_databricks_workspace",
    ...
    monkeypatch.setattr(
        "codify.databricks_model_discovery.discover_databricks_claude_models",
        _discover,
    )
...
def test_materialized_session_spec_is_valid_terminal_metadata(
    ...
    """
    The generated bundled agent spec validates for Codify session creation.
    ...
    # observed (see ``codify.claude_native_status``).
    ...
    # to 404 (see _require_os_env in codify/runner/app.py).
    assert raw["os_env"] == {
        "type": "caller_process",
        "cwd": ".",
        "sandbox": {"type": "none"},
    }
    spec = load_codify_yaml(path)
...
def test_local_run_preflights_local_claude_binary(
    ...
    The Codify server and web UI are local in this mode, but Claude is
...
def test_local_run_persists_launch_state_on_fresh_session(
    ...
    test (and vice versa).
    """
    from codify.claude_native_state import read_launch_state
    ...
    monkeypatch.setattr("codify.chat._find_free_port", lambda: 12345)
    ...
    resume_hint = "Resume with: codify claude --resume conv_local_fresh"
...
def test_local_resume_does_not_print_redundant_resume_hint(
    ...
    """
    ``codify claude --resume`` does not echo another resume prompt.
    ...
    monkeypatch.setattr("codify.chat._find_free_port", lambda: 12346)
    monkeypatch.setattr("codify.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("codify.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("codify.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr(claude_native, "_prepare_claude_terminal", fake_prepare)
...
def test_remote_daemon_run_attaches_without_cli_forwarder(
    ...
    """
    Daemon-routed ``codify claude`` leaves forwarding to the runner.
...
async def test_attach_marks_terminal_stopped_on_exit_when_launched(
    ...
    async def fake_close(
        *, base_url: str, headers: dict[str, str], session_id: str, terminal_id: str
        ...
        :param base_url: Codify base URL passed to the cleanup helper.
...
async def test_attach_runs_cleanup_even_when_forwarder_raises(
    ...
    async def fake_close(
        *, base_url: str, headers: dict[str, str], session_id: str, terminal_id: str
        ...
        :param base_url: Codify base URL.
        :param headers: Auth headers.
        :param session_id: Session id being cleaned up.
        :param terminal_id: Terminal resource id being closed.
        :returns: None.
...
async def test_prepare_reattaches_existing_claude_terminal(
    ...
    If this regresses, a second ``codify claude --session`` can
    ...
    async def fake_fetch_labels(_client: object, _session_id: str) -> dict[str, str]:
        ...
        """
        return {"codify.claude_native.bridge_id": "bridge_abc"}
...
async def test_read_claude_terminal_tmux_parses_metadata() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                ...
                "metadata": {
                    "running": True,
                    "tmux_socket": "/tmp/codify-501/claude/tmux.sock",
                    "tmux_target": "main",
                },
            },
        )
    ...
    assert result.socket == Path("/tmp/codify-501/claude/tmux.sock")
...
async def test_ensure_local_claude_resume_transcript_uses_workspace_dir(
    ...
    its ``CODIFY_RUNNER_WORKSPACE`` (not the runner process's actual
...
async def test_ensure_local_claude_resume_transcript_returns_none_when_no_records(
    ...
    """
    Empty Codify history → ``None`` and no transcript file written.
...
async def test_ensure_local_claude_resume_transcript_rematerializes_image_blocks(
    ...
    """
    from codify import claude_native_bridge
...
async def test_create_claude_session_omits_title_for_generic_seed_path() -> None:
    ...
    ``codify.wrapper = claude-code-native-ui`` label
...
# Reconnect tests
#
# These tests cover the reconnect loop that lets ``codify claude``
...
async def test_attach_with_reconnect_passes_terminal_gone_probe_to_attach(
    ...
    The production ``codify claude`` path passes
...
async def test_attach_with_reconnect_exits_when_probe_says_terminal_is_gone(
    ...
    async def _gone_probe(**kwargs: Any) -> bool:
        """Pretend the Codify reports the terminal stopped."""
        del kwargs
        return True
...
async def test_attach_with_reconnect_reconnects_when_probe_says_terminal_alive(
    ...
    async def _alive_probe(**kwargs: Any) -> bool:
        """Pretend the Codify reports the terminal still running."""
        del kwargs
        return False
...
class _FakeTerminalServer:
    """
    Minimal echo WebSocket server stand-in for the Codify terminal-attach
...
async def test_attach_reconnects_through_real_websocket_bounce(
    ...
    ``codify claude`` exited after the first WebSocket close, so
...
def test_websocket_connect_sets_short_close_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured == {
        ...
        "additional_headers": {
            ...
            "Origin": CODIFY_INTERNAL_WS_ORIGIN,
        },
...
def test_strip_resume_from_claude_args_removes_recognized_forms(
    ...
    case fails, upstream Claude will see the Codify conv id and
...
def _conversation_response_body(
    *,
    ...
    """
    Build a minimal Codify ``GET /v1/sessions/{id}`` response body.
    ...
    :param labels: ``labels`` field for the response payload, e.g.
        ``{"codify.wrapper": "claude-code-native-ui"}``.
...
def _items_response_body(
    ...
    *,
    ...
    """
    Build a minimal Codify item-list response body.
...
async def test_resolve_cold_resume_args_injects_external_session_id(
    ...
    would keep the Codify conv id but lose claude-side context.
    ...
    client = await _httpx_client_with_canned_response(
        _conversation_response_body(
            labels={"codify.wrapper": "claude-code-native-ui"},
...
async def test_resolve_cold_resume_args_declines_resume_when_no_history(
    ...
    """
    Empty Codify history → ``()`` (launch fresh), not ``("--resume", sid)``.
...
async def test_resolve_cold_resume_args_bootstraps_missing_local_claude_transcript(
    ...
    """
    Cross-machine cold resume downloads Codify history into Claude JSONL.

    This is the regression case behind the feature: the server knows
    the Codify conversation and Claude external session id, but the local
    machine has no ``~/.claude/projects/<cwd>/<sid>.jsonl``. The
    helper must fetch committed Codify items and write a transcript before
...
async def test_resolve_cold_resume_args_replaces_existing_local_claude_transcript(
    ...
    """
    Cold resume treats Codify history as source of truth over local JSONL.

    Claude can leave a local ``~/.claude/projects/<cwd>/<sid>.jsonl``
    that diverges from the Codify transcript we have persisted. The resume
    path must still fetch Codify items and overwrite that stale file before
    ...
    item_from_ap = {
        ...
        "status": "completed",
        "role": "user",
        "content": [{"type": "input_text", "text": "fresh Codify text"}],
    }
    item_requests = 0
    ...
    assert args == ("--resume", "claude-uuid-abc")
    assert item_requests == 1, "cold resume must fetch Codify items even when local JSONL exists"
    records = [
        json.loads(line)
        for line in transcript_path.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    assert [record["message"]["content"] for record in records] == ["fresh Codify text"]
...
async def test_resolve_cold_resume_args_warns_when_external_session_id_missing(
    ...
    The Codify conv id still survives — the new terminal binds
...
async def test_resolve_cold_resume_args_rejects_non_claude_native_conv() -> None:
    ...
    ``codify claude --resume <run-conv-id>`` programmer error.
    ...
    assert "codify run --resume conv_abc" in excinfo.value.message
...
async def test_prepare_claude_terminal_cold_resume_injects_external_session_id(
    ...
    Load-bearing assertion: the conv id stays the SAME Codify
    ...
    the Codify conv id but Claude starts fresh. Tests
    ...
    async def _fake_launch_claude_terminal(
        ...
        *,
        ...
        :param session_id: Codify conversation id — captured
    ...
    # Codify conv id survives end-to-end. If this assertion
...
async def test_attach_passes_start_at_end_true_on_cold_resume(
    ...
    walk the prior transcript from offset 0 and Codify would broadcast
    ...
    assert captured.get("start_at_end") is True, (
        ...
        f"re-POSTed to Codify on resume and broadcast to live clients."
    )
...
def test_is_claude_native_conversation_returns_true_on_matching_label(
    ...
    """
    200 + ``codify.wrapper=claude-code-native-ui`` → True.
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {"codify.wrapper": "some-other-wrapper"},
        {"unrelated": "x"},
    ],
)
...
def test_is_claude_native_conversation_logs_warning_on_non_200(
    ...
    warning a misrouted resume (auth failure → silent Codify REPL on
...
def test_is_claude_native_conversation_returns_false_on_transport_error(
    ...
    The caller falls back to the Codify REPL path, which surfaces its
...
# touch the developer's real ``~/.codify/``.
...
def test_align_working_directory_matching_cwd_silent_skip(
    ...
    """
    from codify.claude_native_state import write_launch_state
...
def test_align_working_directory_switch_action_chdirs(
    ...
    ``codify claude --resume`` invoked from a different
...
def test_fetch_external_session_id_for_redirect_uses_session_endpoint(
    ...
    class _FakeHttpxClient:
        ...
        def get(self, url: str) -> httpx.Response:
            ...
            :param url: Relative Codify session path, e.g.
...
def test_align_working_directory_redirect_moves_transcript_and_updates_state(
    ...
    and update Codify launch state so future resumes treat the
    current cwd as the session home.
    """
    from codify.claude_native_state import read_launch_state, write_launch_state
...
def _isolated_provider_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    for var in (
        "ANTHROPIC_API_KEY",
        "CODIFY_ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "CODIFY_OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
        "CODIFY_OPENROUTER_API_KEY",
    ):
...
def _no_auth_claude_spec() -> Any:
    ...
    from codify.spec.types import AgentSpec, ExecutorSpec

    return AgentSpec(
        spec_version=1,
        name="t",
        instructions="t",
        executor=ExecutorSpec(type="codify", config={"harness": "claude-sdk"}),
    )
...
def test_resolve_native_claude_config_global_databricks_auth_uses_ucode(
    ...
    ``auth:`` block via ``codify setup`` still routes a bare
    ``codify claude`` launch through ucode, keyed on the auth block's
...
def test_resolve_native_claude_config_ambient_key(
    ...
    First run without configure: a native `codify claude` launch still
...
def test_resolve_native_claude_config_ambient_prefixed_key(
    ...
    monkeypatch.setenv("CODIFY_ANTHROPIC_API_KEY", "sk-ant-prefixed")
