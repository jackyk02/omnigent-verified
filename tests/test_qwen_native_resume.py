...
Covers converting Codify items into qwen chat-recording records and writing
...
@pytest.mark.skipif(
    shutil.which("qwen") is None or os.environ.get("CODIFY_QWEN_E2E") != "1",
    reason="needs the qwen CLI + configured auth; opt in with CODIFY_QWEN_E2E=1",
)
def test_synthesized_recording_loads_on_resume(tmp_path: Path) -> None:
    ...
    Network + auth dependent — skipped unless CODIFY_QWEN_E2E=1 and qwen is on
