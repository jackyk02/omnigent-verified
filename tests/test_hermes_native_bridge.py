...
from codify import hermes_native_bridge as b
...
def test_write_policy_hook_config_creates_expected_files(tmp_path) -> None:
    ...
    assert "_CODIFY_SERVER_URL=http://localhost:6767" in wrapper_text
    assert "_CODIFY_SESSION_ID=session-123" in wrapper_text
...
def test_inject_relay_into_policy_hook_rewrites_wrapper(tmp_path: Path) -> None:
    """inject_relay_into_policy_hook rewrites codify-policy-hook.sh with relay env vars."""
    from codify.hermes_native_bridge import inject_relay_into_policy_hook

    hermes_home = tmp_path / "hermes_home"
    hermes_home.mkdir(mode=0o700)
    wrapper = hermes_home / "codify-policy-hook.sh"
    wrapper.write_text("#!/bin/sh\nexport _CODIFY_AUTH_HEADERS=old\nexec python hook.py\n")
    wrapper.chmod(0o700)

    bridge_dir = tmp_path
    # put hermes_home under bridge_dir/_HERMES_HOME_SUBDIR
    import codify.hermes_native_bridge as _b

    (bridge_dir / _b._HERMES_HOME_SUBDIR).mkdir(parents=True, exist_ok=True)
    real_wrapper = bridge_dir / _b._HERMES_HOME_SUBDIR / "codify-policy-hook.sh"
