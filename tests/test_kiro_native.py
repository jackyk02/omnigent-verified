...
from codify._wrapper_labels import KIRO_NATIVE_WRAPPER_VALUE, WRAPPER_LABEL_KEY
from codify.kiro_native import (
    _KIRO_PATH_ENV,
...
def test_materialized_kiro_agent_spec_passes_current_validator(tmp_path: Path) -> None:
    """``codify kiro`` must not be rejected as an unknown harness at upload."""
...
def test_session_labels_use_kiro_wrapper_value() -> None:
    ...
    from codify.kiro_native import _SESSION_LABELS
...
def test_resolve_kiro_executable_honors_path_override() -> None:
    """``CODIFY_KIRO_PATH`` selects the executable to resolve."""
    seen: list[str] = []
...
def test_tmux_attach_env_filters_to_allowlist(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_UNLISTED_VAR", "present")

    env = _tmux_attach_env()

    assert env["TERM"] == "xterm-256color"
    assert "CODIFY_UNLISTED_VAR" not in env
...
def test_run_kiro_native_requires_server(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codify.kiro_native._preflight_local_tools", lambda: None)

    with pytest.raises(ClickException, match="resolved Codify server URL"):
...
def test_run_kiro_native_materializes_spec_and_delegates(
    ...
    monkeypatch.setattr("codify.kiro_native._run_with_remote_server", _fake_remote)
