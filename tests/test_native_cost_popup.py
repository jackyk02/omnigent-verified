...
Covers :mod:`codify.native_cost_popup` — the program that runs inside
a ``tmux display-popup`` on a native harness pane, reads an
approve/decline answer, and POSTs the verdict to the Codify elicitation-
resolve endpoint (the same endpoint the web ApprovalCard uses).

The tests drive the public :func:`codify.native_cost_popup.main`
...
from codify import native_cost_popup
...
def _install_popup_harness(
    ...
    *,
    ...
    Writes a real AP-routing config file (so ``_read_codify_routing`` exercises
...
def test_main_missing_codify_server_url_fails_loud(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """
    ...
    There is no safe default for "where is the Codify server", so the script
