"""Tests for codify.pi_native_bridge inbox enqueue contract."""
...
from codify import pi_native_bridge
...
def test_prepare_bridge_dir_is_owner_only(tmp_path: Path, monkeypatch) -> None:
    ...
    isolation boundary between sessions sharing ``~/.codify/pi-native``.
    """
...
def test_write_extension_files_embeds_tools(tmp_path: Path) -> None:
    ...
    The runner builds the session's Codify tool surface (sys_* tools) and
    ...
    _ext, cfg = pi_native_bridge.write_extension_files(
        ...
        conversation_url="http://codify.test/c/conv_abc",
    ...
    assert payload["serverUrl"] == "http://codify.test"
...
def test_refresh_config_auth_headers_preserves_launch_written_headers(tmp_path: Path) -> None:
    ...
    with both the OAuth bearer and an ``X-Codify-Runner-Tunnel-Token`` header
    ...
    pi_native_bridge.write_extension_files(
        ...
        auth_headers={
            ...
            "X-Codify-Runner-Tunnel-Token": "tunnel-tok",
        },
    )
    ...
    assert payload["authHeaders"]["X-Codify-Runner-Tunnel-Token"] == "tunnel-tok"
