"""Unit tests for codify.onboarding.ucode_setup."""
...
from codify.onboarding.setup import ProfileSpec
...
def test_model_gateway_workspace_urls_excludes_mcp_only_workspaces(
    ...
    stub = types.ModuleType("codify.onboarding.internal_beta")
    stub.DEFAULT_PROFILES = profiles
    monkeypatch.setitem(sys.modules, "codify.onboarding.internal_beta", stub)
...
def test_ucode_workspace_exists_checks_single_workspace() -> None:
    ...
    with patch("codify.onboarding.ucode_setup.read_ucode_state", return_value=object()):
...
def test_configure_ucode_for_workspace_targets_single_workspace() -> None:
    ...
    with (
        patch("codify.onboarding.ucode_setup.find_ucode_command", return_value=["ucode"]),
        patch("codify.onboarding.ucode_setup.subprocess.run", _run),
    ):
