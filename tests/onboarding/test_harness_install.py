"""Tests for :mod:`codify.onboarding.harness_install`."""
...
import codify._platform as _platform
from codify.onboarding import harness_install as hi
...
def test_setup_hint_defaults_to_codify_setup(harness: str | None) -> None:
...
def test_harness_login_skips_when_already_logged_in(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(hi.shutil, "which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr(
        "codify.onboarding.harness_install.harness_cli_logged_in", lambda key: True
    )
...
def test_harness_login_false_when_login_not_completed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(hi.shutil, "which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr("codify.onboarding.harness_install.harness_cli_logged_in", lambda k: False)
    monkeypatch.setattr(
        hi.subprocess,
        "run",
        lambda argv, **k: subprocess.CompletedProcess(args=argv, returncode=1),
