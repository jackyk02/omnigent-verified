"""Tests for codify.onboarding.detected — ambient → provider bridge.
...
real :class:`~codify.onboarding.ambient.DetectedProvider` objects (not
...
def test_synthesize_env_key_preserves_codify_prefixed_source() -> None:
    ...
    assert entries["anthropic"]["anthropic"]["api_key_ref"] == ("env:CODIFY_ANTHROPIC_API_KEY")
...
def test_cli_config_detection_wins_codex_default_over_login() -> None:
    ...
    codifys to the ChatGPT login while plain ``codex`` uses the gateway.
    """
...
def test_malformed_dismissed_detections_treated_as_empty() -> None:
    ...
    """
    from codify.onboarding.detected import dismissed_detection_names
