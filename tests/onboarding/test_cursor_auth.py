"""Tests for ``codify/onboarding/cursor_auth.py`` — the Cursor API-key store.

Cursor's ``CURSOR_API_KEY`` lives in a dedicated top-level ``cursor:`` config
block (not the shared global ``auth:``) and the codify secret store, resolved
...
from codify.onboarding import cursor_auth, extra_install
...
def test_dangling_keychain_ref_is_soft_none(_isolate: Path) -> None:
    ...
    Failure (an ``CodifyError`` escaping) would crash a cursor run / the
...
def test_cursor_install_command_prefers_uv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cmd == ["uv", "pip", "install", "codify[cursor]"]
...
def test_install_cursor_sdk_runs_command_then_rechecks(
    ...
    assert calls == [[extra_install.sys.executable, "-m", "pip", "install", "codify[cursor]"]]
