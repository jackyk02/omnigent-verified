"""Tests for codify.onboarding.providers — catalog loading and queries."""
...
from codify.onboarding import providers as _providers_mod
from codify.onboarding.providers import (
    ModelInfo,
    ProviderConfig,
    default_chat_model,
    find_catalog_models,
...
def real_catalog_loader(
    ...
    monkeypatch.delenv("CODIFY_DISABLE_CATALOG_LOOKUP", raising=False)
...
def test_get_all_providers_popular_first() -> None:
    ...
    from codify.onboarding.providers import COMMON_PROVIDERS
...
def test_disabled_catalog_lookup_bypasses_memory_disk_and_network(
    ...
    _REAL_FETCH_PROVIDER_CATALOG("anthropic")
    assert (real_catalog_loader / "anthropic.json").is_file()
    monkeypatch.setenv("CODIFY_DISABLE_CATALOG_LOOKUP", "1")
...
def test_get_chat_models_sorted_newest_first() -> None:
    ...
    from codify.onboarding.providers import _extract_model_version
...
def test_default_chat_model_dynamic_skips_specialty_variants() -> None:
    ...
    from codify.onboarding.providers import _SPECIALTY_MODEL_TOKENS
