...
from codify.harness_availability import HARNESS_VERSION_TOO_LOW
...
def _isolate_cursor_credential(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    # an CODIFY_CODEX_PATH override and probes on-disk global install dirs.
    ...
    import codify._platform as platform
...
def test_auth_aware_native_harness_needs_auth_when_installed_not_signed_in(
    ...
    monkeypatch.setattr(
        "codify.onboarding.harness_readiness._family_provider_configured", lambda _h: False
    )
    monkeypatch.setattr(hi, "harness_cli_logged_in", lambda key: False)
    # opencode: no stored/env provider.
    import codify.onboarding.opencode_auth as oc
...
def test_claude_ready_via_configured_provider_without_cli_login(
    ...
    """Claude with an codify provider (API key) but NO CLI login reads ready.
    ...
    _all_clis_installed(monkeypatch)
    monkeypatch.setattr(
        "codify.onboarding.harness_readiness._family_provider_configured", lambda _h: True
    )
...
def test_family_provider_configured_excludes_subscription(
    ...
    """
    import codify.onboarding.harness_readiness as hrmod
    from codify.onboarding.provider_config import KEY_KIND, SUBSCRIPTION_KIND
    ...
    monkeypatch.setattr(
        "codify.onboarding.harness_readiness.default_provider_for_harness",
...
def test_configured_harness_map_covers_all_spellings(
    ...
    expected_keys = {
        ...
        "kimi",
        "kimi-code",
        # Native Kimi (``codify kimi``) — gates on the kimi CLI.
        "kimi-native",
        "native-kimi",
...
def test_configured_harness_map_all_true_with_clis(
    ...
    """
    import codify.onboarding.gemini_auth as _ga

    _all_clis_installed(monkeypatch)
    monkeypatch.setattr(
        "codify.codex_native._codex_auth_unavailable_reason",
        lambda: None,
    )
    ...
    monkeypatch.setattr("codify.onboarding.acp_auth.acp_agents", lambda config=None: [object()])
    result = configured_harness_map()
