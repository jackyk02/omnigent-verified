"""Tests for ``codify/onboarding/antigravity_auth.py`` — the Gemini key store.
...
from codify.onboarding import antigravity_auth, extra_install
...
def test_dangling_keychain_ref_is_soft_none(_isolate: Path) -> None:
    ...
    Failure (an ``CodifyError`` escaping) would crash an antigravity run / the
...
def test_install_antigravity_sdk_runs_command_then_rechecks(
    ...
    assert calls == [[extra_install.sys.executable, "-m", "pip", "install", "codify[antigravity]"]]
