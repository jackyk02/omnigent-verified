"""Tests for :mod:`codify.onboarding.ucode_cleanup`.
...
from codify.onboarding.ucode_cleanup import (
    UcodeWiringRemoval,
...
def test_strip_malformed_toml_raises_and_leaves_file(tmp_path: Path) -> None:
    ...
    with pytest.raises(CodifyError, match="not valid TOML"):
...
def test_remove_web_search_removes_ucode_owned_entry(
    ...
    cli_calls: list[bool] = []
    monkeypatch.setattr(
        "codify.onboarding.ucode_cleanup._remove_web_search_via_claude_cli",
