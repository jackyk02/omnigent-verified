"""Tests for codify.onboarding.provider_config."""
...
def test_resolve_secret_env_ref_accepts_codify_prefixed_alias(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``env:ANTHROPIC_API_KEY`` falls back to ``CODIFY_ANTHROPIC_API_KEY``."""
...
def test_gemini_only_gateway_local_rejected_at_parse(kind: str) -> None:
    ...
    with pytest.raises(CodifyError, match="Gemini surface"):
...
def test_gemini_auth_command_rejected_at_parse() -> None:
    ...
    with pytest.raises(CodifyError, match="auth_command is not allowed on a 'gemini' family"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Bedrock mode is native-`codify claude` only — pi cannot use it.
        (
...
def test_parse_cli_config_entry() -> None:
    ...
    """
    from codify.onboarding.provider_config import load_providers, provider_families
...
def test_parse_cli_config_entry_invalid(body: dict[str, object], message_fragment: str) -> None:
    ...
    with pytest.raises(CodifyError, match=r"cli-config|model_provider|cli"):
...
def test_bedrock_kind_rejected_for_non_native_harnesses() -> None:
    """`kind: bedrock` is native-`codify claude` only; in-process harnesses fail loud.
    ...
    """
    from codify.errors import ErrorCode
    from codify.runtime.workflow import configure_agent_harness_with_provider
    ...
    for harness in ("claude-sdk", "pi"):
        env: dict[str, str] = {}
        with pytest.raises(CodifyError) as exc:
