"""Tests for ``codify/onboarding/copilot_auth.py`` — the Copilot token store.

Copilot's GitHub token lives in a dedicated top-level ``copilot:`` config block
(not the shared global ``auth:``) and the codify secret store, resolved with
...
from codify.onboarding import copilot_auth, extra_install
...
def test_copilot_install_command_prefers_uv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cmd = copilot_install_command()
    assert cmd == ["uv", "pip", "install", "codify[copilot]"]
...
def test_install_copilot_sdk_runs_command_then_rechecks(
    ...
    assert calls == [[extra_install.sys.executable, "-m", "pip", "install", "codify[copilot]"]]
