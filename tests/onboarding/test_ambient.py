"""Tests for codify.onboarding.ambient — machine credential detection.
...
both :func:`codify.onboarding.ambient._ollama_reachable` and
:func:`codify.onboarding.harness_install.harness_cli_logged_in` so no real
...
from codify.onboarding import ambient
...
_PROVIDER_ENV_VARS = [
    ...
    "OPENAI_BASE_URL",
    "CODIFY_OPENAI_BASE_URL",
    "OPENROUTER_API_KEY",
    "CODIFY_OPENROUTER_API_KEY",
    "GEMINI_API_KEY",
    "CODIFY_GEMINI_API_KEY",
...
def clean_env(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    """
    from codify.onboarding import harness_install
...
def test_env_key_detection_accepts_codify_prefixed_alias(
    clean_env, monkeypatch: pytest.MonkeyPatch
) -> None:
    """An ``CODIFY_``-prefixed key is detected without setting the raw name."""
    monkeypatch.setenv("CODIFY_ANTHROPIC_API_KEY", "some-secret-value")
    ...
    assert detected == [
        DetectedProvider(
            ...
            kind="key",
            family="anthropic",
            source="$CODIFY_ANTHROPIC_API_KEY",
        )
    ]
...
def test_codex_config_detected_before_codex_login(clean_env) -> None:
    ...
    (it beats auth.json) — codifys must match. Failure means the
...
def test_vertex_claude_detected_with_all_env_vars(
    ...
    Codify recognises the credential and routes through the CLI's own
    Vertex auth (GCP ADC).
    """
