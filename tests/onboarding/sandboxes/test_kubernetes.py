...
import codify.onboarding.sandboxes.kubernetes as k8s
...
from codify.onboarding.sandboxes.kubernetes import (
...
_MANIFEST_KW = {
    "pod_name": "codify-managed-abc-1a2b3c",
    "namespace": "codify-sandboxes",
    "image": "ghcr.io/codify-ai/codify-host:latest",
    "service_account": "codify-runner",
    "host_id": "host_abcdef",
    "host_name": "managed-abcdef",
    "server_url": "http://srv.example.com",
    "token_secret_name": "codify-managed-abc-1a2b3c-token",
    "harness_secret": "codify-creds",
    "env_literals": {},
    "node_selector": None,
    "workspace": "/home/codify/workspace",
}
...
def test_build_pod_manifest_runs_host_under_reaper_as_container_command() -> None:
    ...
    script = command[2]
    # exec the reaper (so it is PID 1) which then runs `codify host`.
    assert "exec python3 -c" in script
    assert "codify host --server http://srv.example.com" in script
...
def test_build_pod_manifest_init_container_prepares_and_clones_workspace() -> None:
    ...
    manifest = build_pod_manifest(
        **{**_MANIFEST_KW, "clone_dir": "/home/codify/workspace/repo"},
    ...
    assert "mkdir -p /home/codify/workspace" in script
    assert "git clone --branch main --single-branch -- " in script
    assert "https://github.com/org/repo.git /home/codify/workspace/repo" in script
...
def test_build_pod_manifest_forwards_config_home_to_init_container() -> None:
    ...
    manifest = build_pod_manifest(
        **{
            **_MANIFEST_KW,
            "env_literals": {
                "CODIFY_CONFIG_HOME": "/home/codify/custom-config",
    ...
    assert init_env == [
        {"name": "HOME", "value": "/home/codify"},
        {
            "name": "CODIFY_CONFIG_HOME",
            "value": "/home/codify/custom-config",
        },
    ]
...
@pytest.mark.parametrize(
    "config_home",
    ["/tmp/elsewhere", "/home/codify-other", "/home/codify/../tmp", "../etc"],
)
def test_build_pod_manifest_rejects_config_home_outside_home_dir(config_home: str) -> None:
    ...
    """
    with pytest.raises(ValueError, match=r"CODIFY_CONFIG_HOME.*must resolve under"):
        build_pod_manifest(
            **{**_MANIFEST_KW, "env_literals": {"CODIFY_CONFIG_HOME": config_home}},
            host_config=_HOST_CONFIG,
        )
...
def test_build_pod_manifest_accepts_config_home_at_or_under_home_dir(config_home: str) -> None:
    ...
    assert {"name": "CODIFY_CONFIG_HOME", "value": config_home} in manifest["spec"][
        "initContainers"
    ][0]["env"]
...
def test_build_pod_manifest_config_home_outside_home_dir_ok_without_host_config() -> None:
    ...
    manifest = build_pod_manifest(
        **{**_MANIFEST_KW, "env_literals": {"CODIFY_CONFIG_HOME": "/tmp/elsewhere"}},
    )
    init_env = manifest["spec"]["initContainers"][0]["env"]
    assert {"name": "CODIFY_CONFIG_HOME", "value": "/tmp/elsewhere"} in init_env
...
def test_build_pod_manifest_token_rides_secret_ref_not_the_spec() -> None:
    ...
    assert token_entry["valueFrom"]["secretKeyRef"] == {
        "name": "codify-managed-abc-1a2b3c-token",
...
def test_build_token_secret_manifest_carries_token_in_stringdata() -> None:
    ...
    secret = build_token_secret_manifest(
        secret_name="codify-pod-token", namespace="codify-sandboxes", token=_TOKEN
    )
    assert secret["stringData"] == {HOST_TOKEN_ENV_VAR: _TOKEN}
    assert secret["metadata"]["labels"]["app.kubernetes.io/managed-by"] == "codify"
...
def test_build_pod_manifest_harness_secret_projects_into_both_containers() -> None:
    ...
    assert init["envFrom"] == [{"secretRef": {"name": "codify-creds"}}]
    assert host["envFrom"] == [{"secretRef": {"name": "codify-creds"}}]
...
def test_build_pod_manifest_pvc_mounts_land_on_host_container_only() -> None:
    ...
    assert spec["initContainers"][0]["volumeMounts"] == [
        {"name": "home", "mountPath": "/home/codify"}
    ]
...
def test_new_pod_name_and_token_secret_name() -> None:
    ...
    assert name.startswith("codify-managed-abc-123-")
...
def _launcher() -> KubernetesSandboxLauncher:
    ...
    return KubernetesSandboxLauncher(
        in_cluster=True, namespace="codify-sandboxes", secret_name="codify-creds", env=()
    )
...
def test_launch_host_creates_secret_then_pod_and_returns_workspace(
    ...
    assert workspace == "/home/codify/workspace"
    ...
    assert fake_core.created_pods[0]["metadata"]["name"] == "codify-pod-1"
...
def test_launch_host_threads_pvc_mounts_into_the_pod(fake_core: _FakeCore) -> None:
    ...
    assert {
        "name": "pvc-0",
        "persistentVolumeClaim": {"claimName": "codify-datasets", "readOnly": True},
...
def test_launch_host_with_repo_returns_clone_dir(fake_core: _FakeCore) -> None:
    ...
    assert workspace == "/home/codify/workspace/repo"
...
def test_launch_host_cleans_up_on_create_failure(fake_core: _FakeCore) -> None:
    ...
    assert "codify-pod-3-token" in fake_core.deleted_secrets
    assert "codify-pod-3" in fake_core.deleted_pods
...
def test_launch_host_invalid_config_home_fails_before_creating_secret(
    ...
    """
    monkeypatch.setenv("CODIFY_CONFIG_HOME", "/tmp/outside")
    launcher = KubernetesSandboxLauncher(
        in_cluster=True,
        namespace="codify-sandboxes",
        secret_name="codify-creds",
        env=["CODIFY_CONFIG_HOME"],
    )
    with pytest.raises(ValueError, match=r"CODIFY_CONFIG_HOME.*must resolve under"):
        launcher.start_host(
            "codify-pod-x",
...
def test_launch_host_fast_fails_on_clone_failure_with_log_tail(
    ...
    assert "codify-pod-4" in fake_core.deleted_pods
    assert "codify-pod-4-token" in fake_core.deleted_secrets
...
def test_terminate_deletes_pod_and_secret(fake_core: _FakeCore) -> None:
    ...
    _launcher().terminate("codify-pod-6")
    assert fake_core.deleted_pods == ["codify-pod-6"]
    assert fake_core.deleted_secrets == ["codify-pod-6-token"]
...
def test_terminate_is_idempotent_on_404(fake_core: _FakeCore) -> None:
    ...
    _launcher().terminate("codify-pod-7")  # must not raise
    assert fake_core.deleted_secrets == ["codify-pod-7-token"]
...
def test_terminate_retries_transient_then_gives_up_best_effort(
    ...
    _launcher().terminate("codify-pod-8")  # best-effort: must not raise
    assert "could not delete Kubernetes pod 'codify-pod-8'" in capsys.readouterr().err
    # The Secret delete still runs after the Pod gives up.
    assert fake_core.deleted_secrets == ["codify-pod-8-token"]
...
def test_provision_reserves_pod_name_and_no_exec_transport() -> None:
    ...
    assert name.startswith("codify-managed-abc-")
