"""Tests for :mod:`codify.onboarding.sandboxes.islo`."""
...
from codify.onboarding.sandboxes.islo import (
    API_KEY_ENV_VAR,
    HOST_IMAGE_ENV_VAR,
...
def test_prepare_reports_missing_optional_sdk(monkeypatch: pytest.MonkeyPatch) -> None:
    """Selecting Islo without ``codify[islo]`` gives a clear install hint."""
    monkeypatch.setenv(API_KEY_ENV_VAR, "ak-test")
    monkeypatch.setattr(
        islo_mod,
        "_load_islo_sdk",
        lambda: (_ for _ in ()).throw(click.ClickException("install codify[islo]")),
    )

    with pytest.raises(click.ClickException, match=r"codify\[islo\]"):
        IsloSandboxLauncher().prepare()
...
def test_provision_builds_islo_create_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "codify-fixed")
    fake = _FakeIsloAPI()
    ...
    assert sandbox_id == "codify-fixed"
...
def test_provision_uses_image_and_env_var_fallbacks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "codify-env")
    fake = _FakeIsloAPI()
    launcher = IsloSandboxLauncher()
    monkeypatch.setattr(launcher, "_islo", lambda: fake)
...
def test_provision_can_disable_idle_pause_lifecycle(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "codify-manual")
    fake = _FakeIsloAPI()
...
def test_provision_clears_seeded_helper_when_user_injects_claude_cred(
    ...
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "codify-byo")
    fake = _FakeIsloAPI()
    ...
    assert strip_calls[0].sandbox_id == "codify-byo"
...
def test_provision_keeps_seeded_helper_without_user_claude_cred(
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "codify-gw")
    fake = _FakeIsloAPI()
    launcher = IsloSandboxLauncher(env=["OPENAI_API_KEY"])
    monkeypatch.setattr(launcher, "_islo", lambda: fake)
...
def test_start_host_stops_preserved_daemon_before_launch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cleanup_index = next(i for i, cmd in enumerate(commands) if "preserved codify host" in cmd)
    launch_index = next(i for i, cmd in enumerate(commands) if "CODIFY_HOST_TOKEN" in cmd)
