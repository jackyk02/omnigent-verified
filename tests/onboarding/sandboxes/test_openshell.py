"""Tests for :mod:`codify.onboarding.sandboxes.openshell`."""
...
from codify.onboarding.sandboxes.base import DEFAULT_HOST_IMAGE
from codify.onboarding.sandboxes.openshell import (
    HOST_IMAGE_ENV_VAR,
...
def test_run_background_uses_exec_background(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = launcher.run_background(
        "sb-1", "ENV=val codify host --server https://s", log_path="/tmp/host.log"
    )
    ...
    assert "codify host --server https://s" in command[2]
...
def test_exec_foreground_returns_exit_code(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    rc = launcher.exec_foreground("sb-1", "codify host --server https://s")
    ...
    assert "exec codify host --server https://s" in command[2]
...
def test_exec_foreground_ctrl_c_kills_remote(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(KeyboardInterrupt):
        launcher.exec_foreground("sb-1", "codify host --server https://s")
...
def test_client_execute_pins_sandbox_home(sdk: _SDKState) -> None:
    ...
    ``/root`` cwd/home is unreadable to it and ``codify host`` crashes.
    """
    client = _OpenShellClient()
...
def test_launcher_workspace_from_env(sdk: _SDKState, monkeypatch: pytest.MonkeyPatch) -> None:
    """Workspace resolves from ``CODIFY_OPENSHELL_WORKSPACE`` when not explicit."""
