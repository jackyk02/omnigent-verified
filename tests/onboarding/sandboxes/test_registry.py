"""Tests for :mod:`codify.onboarding.sandboxes.registry`."""
...
from codify.onboarding.sandboxes import get_launcher
...
def _set_entry_points(
    ...
    *eps: _FakeEntryPoint,
    ...
    monkeypatch.setattr(
        "codify.onboarding.sandboxes.registry._entry_points",
        lambda: eps,
    )


def test_plugin_state_loads_builtins() -> None:
    """The cached plugin state includes core Codify built-in providers."""
    reset_plugin_state_for_tests()
    state = plugin_state()
...
def test_get_provider_metadata_known_provider() -> None:
    ...
    assert "codify.onboarding.sandboxes.modal:ModalSandboxLauncher" in meta.launcher_class
...
def test_instantiate_rejects_non_launcher_class(
    ...
    monkeypatch.setattr(
        "codify.onboarding.sandboxes.registry._load_object",
