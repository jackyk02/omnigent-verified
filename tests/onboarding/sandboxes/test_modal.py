"""Tests for :mod:`codify.onboarding.sandboxes.modal`."""
...
from codify.onboarding.sandboxes.base import SandboxCapabilityError
from codify.onboarding.sandboxes.modal import (
    DEFAULT_HOST_IMAGE,
    HOST_IMAGE_ENV_VAR,
...
class _FakeImage:
    ...
    :param tag: The registry image reference, e.g.
        ``"ghcr.io/codify-ai/codify-host:latest"``.
...
def test_prepare_raises_with_install_hint_when_sdk_missing(
    ...
    assert "codify[modal]" in str(exc.value)
...
def test_provision_creates_max_lifetime_sandbox_under_shared_app(
    ...
    # Image: the official prebaked host image (codify + git/tmux
    ...
    assert state.sandboxes[sandbox_id].tags == {"codify-name": "my-host"}


def test_provision_honors_image_override_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    CODIFY_MODAL_HOST_IMAGE must replace the default image ref —
    it's the escape hatch for org-internal copies of the host image.
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(HOST_IMAGE_ENV_VAR, "ghcr.io/acme/codify-host:sha-abc1234")
    monkeypatch.delenv(REGISTRY_SECRET_ENV_VAR, raising=False)
    ModalSandboxLauncher().provision("my-host")

    assert state.create_calls[0].image.tag == "ghcr.io/acme/codify-host:sha-abc1234"
...
def test_provision_passes_registry_secret_for_private_pulls(
    ...
    """
    CODIFY_MODAL_REGISTRY_SECRET must thread the named Modal secret
...
def test_exec_foreground_records_pid_and_streams_output(
    ...
    returncode = ModalSandboxLauncher().exec_foreground("sb-1", "codify host --server u")
    ...
    assert "exec codify host --server u" in remote
...
def test_exec_foreground_kills_remote_on_interrupt(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(KeyboardInterrupt):
        ModalSandboxLauncher().exec_foreground("sb-1", "codify host --server u")
...
def test_wheel_install_command_overlays_baked_install() -> None:
    """
    The prebaked host image carries codify at the same 0.1.0
...
def test_provision_explicit_image_overrides_env_and_gets_secret(
    ...
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(HOST_IMAGE_ENV_VAR, "ghcr.io/acme/codify-host:env")
    monkeypatch.setenv(REGISTRY_SECRET_ENV_VAR, "ghcr-pull")
...
def test_provision_injects_configured_sandbox_secrets(
    ...
    ModalSandboxLauncher(secrets=["codify-llm", "gateway-extras"]).provision("my-host")

    assert state.create_calls[0].secrets == [
        _FakeSecret(name="codify-llm"),
...
def test_provision_resolves_sandbox_secrets_env_var(
    ...
    """
    Without constructor names, CODIFY_MODAL_SANDBOX_SECRETS
    (comma-separated, whitespace tolerated) supplies the workload
    secrets — the CLI flow's path to the same feature.
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(SANDBOX_SECRETS_ENV_VAR, "codify-llm, extra-creds")
    ModalSandboxLauncher().provision("my-host")
