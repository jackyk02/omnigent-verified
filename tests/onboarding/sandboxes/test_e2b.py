"""Tests for :mod:`codify.onboarding.sandboxes.e2b`."""
...
from codify.onboarding.sandboxes.e2b import (
...
def test_prepare_raises_install_hint_when_sdk_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(click.ClickException, match=r"pip install 'codify\[e2b\]'"):
        E2BSandboxLauncher().prepare()
...
def test_provision_uses_default_template_and_max_timeout(sdk: _State) -> None:
    ...
    assert sdk.create_kwargs["metadata"] == {"codify-name": "managed-x"}
...
def test_provision_missing_template_points_at_build(sdk: _State) -> None:
    ...
    sdk.create_raises = _SandboxException("404: template 'codify-host' not found")
    with pytest.raises(click.ClickException, match="e2b template build"):
        E2BSandboxLauncher().provision("x")
...
def test_provision_env_override_skips_retry(sdk: _State, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODIFY_E2B_MAX_LIFETIME_S", "3600")
...
@pytest.mark.parametrize(
    ("message", "expected"),
    [
        ("404: template 'codify-host' not found", True),
...
def test_resolve_max_lifetime_rejects_non_numeric(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CODIFY_E2B_MAX_LIFETIME_S", "soon")
...
def test_exec_foreground_echoes_and_returns_exit_code(
    ...
    code = E2BSandboxLauncher().exec_foreground("sb-e2b-1", "codify host")
    assert code == 0
    ...
    assert "TERM=xterm-256color exec codify host" in foreground_cmd
...
def test_stream_exec_disables_per_command_timeout(sdk: _State) -> None:
    ...
    E2BSandboxLauncher().stream_exec("sb-e2b-1", "codify host")
...
def test_exec_foreground_kills_on_keyboard_interrupt(
    ...
    with pytest.raises(KeyboardInterrupt):
        E2BSandboxLauncher().exec_foreground("sb-e2b-1", "codify host")
    assert closed == [True]
