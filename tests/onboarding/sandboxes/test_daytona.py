"""Tests for :mod:`codify.onboarding.sandboxes.daytona`."""
...
from codify.onboarding.sandboxes.daytona import (
    HOST_IMAGE_ENV_VAR,
...
def test_provision_defaults_official_image_and_disables_autostop(
    ...
    assert create.params.labels == {"codify-name": "managed-abc"}
...
def test_provision_env_passthrough_env_var_fallback(
    ...
    ``CODIFY_DAYTONA_SANDBOX_ENV`` names apply (whitespace around
    commas tolerated).
    """
...
def test_terminate_retries_state_change_conflicts(
    ...
    """
    monkeypatch.setattr("codify.onboarding.sandboxes.daytona._TERMINATE_CONFLICT_BACKOFF_S", 0.0)
    launcher = DaytonaSandboxLauncher()
    sandbox_id = launcher.provision("a")
...
def test_wheel_install_command_overlays_baked_install(fake_daytona: _FakeDaytonaState) -> None:
    ...
    image bakes codify at the same version, so plain pip would
...
def test_exec_foreground_runs_command_over_pty(
    ...
    returncode = launcher.exec_foreground(sandbox_id, "codify host --server u")
    ...
    assert process.pty_handle.sent_inputs == ["TERM=xterm-256color exec codify host --server u\n"]
...
def test_exec_foreground_kills_remote_on_interrupt(fake_daytona: _FakeDaytonaState) -> None:
    ...
    re-raises — otherwise a detached `codify host` would keep the
    ...
    with pytest.raises(KeyboardInterrupt):
        launcher.exec_foreground(sandbox_id, "codify host --server u")
