"""Tests for :mod:`codify.onboarding.sandboxes.bootstrap`."""
...
from codify.onboarding.sandboxes import bootstrap as bootstrap_mod
...
def test_read_login_url_returns_none_when_no_url_printed() -> None:
    ...
    an exception: ``codify login`` legitimately completes without a
...
def test_login_runs_in_sandbox_and_forwards_callback_port(
    ...
    """
    Golden path: run ``codify login`` inside the box with a forced
    ...
    # `codify login <server>` (which infers the fronting workspace
    # itself), NOT a raw `databricks auth login` with profile flags.
    assert launcher.stream_calls == [
        _StreamCall(
            command="codify login https://app.example.com",
            pty=True,
        )
    ]
    ...
    assert launcher.log.index(f"run:{launcher.run_commands[0]}") < launcher.log.index(
        "stream:codify login https://app.example.com"
    )
...
def test_login_completes_without_browser_when_no_url_printed(
    ...
    """
    ``codify login`` reuses a cached workspace grant when one
...
def test_ship_wheels_puts_installs_and_persists_path(tmp_path: Path) -> None:
    ...
    # PATH persistence makes `codify` resolvable for later
...
def test_connect_runs_bare_host_command() -> None:
    """
    The foreground command must be the bare ``codify host --server
    <url>`` — ``codify host`` no longer takes ``--profile``, so any
    ...
    assert launcher.foreground_commands == ["codify host --server https://app.example.com"]


def test_connect_sets_host_name_before_connecting() -> None:
    """
    When ``host_name`` is set, connect must (a) edit the sandbox's
    ``~/.codify/config.yaml`` to use that name, and (b) THEN run
    ``codify host``. Order matters — the host reads config.yaml at
    startup.
    """
    launcher = _FakeLauncher()
    connect_sandbox_host(
