"""Tests for :mod:`codify.onboarding.sandboxes.cwsandbox`."""
...
from codify.onboarding.sandboxes.cwsandbox import (
    HOST_IMAGE_ENV_VAR,
...
def test_provision_requests_host_image_and_egress(sdk: _State) -> None:
    ...
    assert sdk.run_kwargs["tags"] == ["codify", "managed-x"]
...
def test_exec_foreground_records_pid_and_streams_output(sdk: _State) -> None:
    ...
    returncode = CWSandboxLauncher().exec_foreground("sb-1", "codify host --server u")
...
def test_exec_foreground_kills_remote_on_interrupt(sdk: _State) -> None:
    ...
    with pytest.raises(KeyboardInterrupt):
        CWSandboxLauncher().exec_foreground("sb-1", "codify host --server u")
