...
def test_run_background_wraps_command_in_sh_c() -> None:
    ...
    in-sandbox ``codify host`` launch died on its ``CODIFY_HOST_TOKEN=…``
    prefix.
    """
    launcher = _RecordingLauncher()

    launcher.run_background("sb-1", "FOO=bar codify host --server https://srv")

    [cmd] = launcher.commands
    assert cmd == (
        "setsid nohup sh -c 'FOO=bar codify host --server https://srv' "
        "> /tmp/codify-host.log 2>&1 < /dev/null & echo launched"
    )
...
def test_start_host_env_prefix_is_honored_by_a_real_shell() -> None:
    ...
    apply its ``CODIFY_HOST_*`` assignments when re-parsed by a shell — the
    exact thing the ``sh -c`` wrapper restores. Run the raw command through a
    real ``sh -c`` (the inner shell of the wrapper) with ``codify host``
    ...
    probe = raw.replace(
        "codify host --server https://srv",
        'sh -c \'printf %s:%s:%s "$CODIFY_HOST_TOKEN" "$CODIFY_HOST_ID" "$CODIFY_HOST_NAME"\'',
    )
    out = subprocess.run(
...
def test_materialize_workspace_override_resolves_local_checkout_without_cloning() -> None:
    ...
    [raw] = launcher.backgrounded
    assert raw.endswith("codify host --server https://srv")
...
def _run_write_command(
    ...
    *,
    ...
    env.pop("CODIFY_CONFIG_HOME", None)
...
def _materialize(
    ...
    config_dir = config_home if config_home is not None else home / ".codify"
    with open(config_dir / "config.yaml") as f:
        return yaml.safe_load(f)


def test_render_host_config_write_command_creates_config_from_scratch(tmp_path: Path) -> None:
    """A fresh sandbox (no ~/.codify at all) gets the injected config verbatim."""
    written = _materialize(render_host_config_write_command(_GATEWAY_HOST_CONFIG), tmp_path)
    assert written == _GATEWAY_HOST_CONFIG


def test_render_host_config_write_command_honors_codify_config_home(
    tmp_path: Path,
) -> None:
    """The writer uses CODIFY_CONFIG_HOME as the config directory itself."""
    home = tmp_path / "home"
    home.mkdir()
...
def test_render_host_config_write_command_merges_providers_and_replaces_other_keys(
    ...
    """
    (tmp_path / ".codify").mkdir()
    (tmp_path / ".codify" / "config.yaml").write_text(
        yaml.safe_dump(
            {
                "providers": {
                    "anthropic": {"kind": "key"},
...
def test_materialized_config_routes_pi_to_the_gateway(
    ...
    from codify.onboarding.provider_config import default_provider_for_harness, load_config

    monkeypatch.setenv("CODIFY_CONFIG_HOME", str(tmp_path / ".codify"))
...
def test_start_host_writes_host_config_before_launching_the_host() -> None:
    ...
    host_index = next(
        i for i, cmd in enumerate(launcher.commands) if "codify host --server" in cmd
    )
...
def _read_marker(home: Path) -> dict[str, object] | None:
    marker = home / ".codify" / ".injected_host_config.json"
...
def test_render_host_config_write_command_preserves_user_created_entries(
    ...
    config_path = tmp_path / ".codify" / "config.yaml"
...
def test_render_host_config_write_command_empty_payload_without_marker_is_noop(
    ...
    assert not (tmp_path / ".codify" / "config.yaml").exists()
    assert _read_marker(tmp_path) is None
...
def test_render_host_config_write_command_corrupt_marker_degrades_to_additive(
    ...
    (tmp_path / ".codify" / ".injected_host_config.json").write_text("{not json")
