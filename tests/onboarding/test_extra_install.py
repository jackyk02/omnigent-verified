"""Tests for ``codify/onboarding/extra_install.py``."""
...
from codify.onboarding import extra_install
...
from codify.onboarding.extra_install import (
    _installed_vcs_url,
    _is_uv_tool_install,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/home/user/.local/share/uv/tools/codify/bin/python", True),
        # Windows layout (forward-slash normalized)
        ("C:/Users/user/AppData/Local/uv/tools/codify/Scripts/python", True),
        # Regular virtualenv — not a uv tool install
        ("/home/user/repos/codify/.venv", False),
        # System Python
        ("/usr", False),
        # pipx venv (should NOT be detected as uv tool)
        ("/home/user/.local/pipx/venvs/codify/bin/python", False),
    ],
...
def test_installed_vcs_url_git_source(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    url = "git+https://github.com/codify-ai/codify.git"
    monkeypatch.setattr(
        "codify.update_check._read_installed_wheel_info",
...
def test_installed_vcs_url_none(monkeypatch: pytest.MonkeyPatch, info: object) -> None:
    ...
    monkeypatch.setattr("codify.update_check._read_installed_wheel_info", lambda: info)
...
def test_extra_install_command_uv_tool_git_source(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cmd = extra_install_command("cursor")
    assert cmd == ["uv", "tool", "install", "--force", f"codify[cursor] @ {url}"]
...
def test_extra_install_command_uv_on_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cmd == ["uv", "pip", "install", "codify[antigravity]"]
...
def test_extra_install_display_matches_command(
    ...
    assert "codify[cursor]" in display
...
def test_harness_extra_is_a_real_pyproject_extra(extra: str) -> None:
    ...
    The install command interpolates the constant into ``codify[<extra>]``; a
