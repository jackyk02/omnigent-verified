"""Tests for :mod:`codify.onboarding.gemini_auth`.
...
from codify.onboarding import gemini_auth as ga
...
def test_codify_written_settings_json_is_not_a_login(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A settings.json codify wrote itself must not read as a login.

    ``ensure_agy_feedback_survey_disabled`` creates
    ``~/.gemini/antigravity-cli/settings.json`` under the real home before agy
    starts, so its existence proves only that codify ran. Treating it as a
