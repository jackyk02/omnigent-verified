...
import codify.onboarding.harness_auth as ha
...
def _isolate_config_and_secrets(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    Keeps every write off the developer's real ``~/.codify`` and OS keychain,
...
def test_store_key_makes_pi_ready_despite_subscription_default(
    ...
    depends on an codify-managed provider. With a pre-existing subscription
