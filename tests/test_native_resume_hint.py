...
from codify._native_resume_hint import (
...
def test_format_native_resume_command_includes_remote_context() -> None:
    ...
    The command must carry the wrapper name, Codify server, and
    Codify conversation id. If any of those fields are dropped, a
    ...
    assert command == ("codify claude --server https://example.databricks.com --resume conv_abc")
