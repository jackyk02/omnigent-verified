"""Tests for the RPC read driver (:mod:`codify.antigravity_native_reader`).

The reader replaces the transcript-tail forwarder's read loop: it polls agy's
connect-RPC for trajectory steps, maps each new step to Codify conversation
...
from codify import antigravity_native_reader as reader
from codify.antigravity_native_bridge import read_bridge_state
from codify.antigravity_native_rpc import AntigravityRpcError
from codify.antigravity_native_steps import PendingInteraction
...
async def test_planner_done_emits_session_usage(
    ...
    The event data must map agy's string-int fields onto the Codify shape:
...
async def test_run_reader_with_bridge_rebinds_after_rotation(
    ...
    monkeypatch.setattr(
        "codify.antigravity_native_interactions.bridge_interaction",
        lambda *a, **k: None,
    )
...
async def test_run_reader_with_bridge_adopts_first_cascade_in_place(
    ...
    adopt the new cascade in the SAME Codify session (rewrite bridge state, NO
