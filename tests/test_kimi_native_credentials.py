...
from codify.kimi_native_credentials import (
...
def test_render_hooks_toml_is_valid_and_complete() -> None:
    ...
    for hook in parsed["hooks"]:
        assert "codify.kimi_native_hook" in hook["command"]
        ...
        # ``codify/`` shadows the install and the hook dies on ImportError
        # before publishing the approval card.
        assert " -I -m codify.kimi_native_hook" in hook["command"]
