...
The wrapper is pure re-exports of ``codify.inner.os_env`` and
the ``OSEnvSpec`` / ``OSEnvSandboxSpec`` dataclasses from
``codify.inner.datamodel``. Tests only verify two properties:
...
import codify.inner.datamodel as inner_dm
import codify.inner.os_env as inner_os_env
from codify import environments


def test_environments_all_symbols_importable() -> None:
    """
    Every name in ``codify.environments.__all__`` resolves on the
    wrapper module.
    ...
    for name in environments.__all__:
        assert hasattr(environments, name), f"codify.environments missing re-export {name!r}"
...
def test_environments_reexports_inner_os_env_objects() -> None:
    ...
    objects as ``codify.inner.os_env``.
