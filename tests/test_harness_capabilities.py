...
from codify import harness_plugins as hp
...
def test_subagents_matches_native_wrapper_label() -> None:
    ...
    # can spawn Codify native sub-agents.
...
def test_native_tui_harnesses_declare_shell_tool_provocation() -> None:
    ...
    a prompt must carry the ``codify-bench-ok`` placeholder the probe
    ...
    for harness, capability in caps.items():
        ...
        assert "codify-bench-ok" in capability.shell_tool_prompt, harness
