...
from codify import codex_native_process_registry as registry
...
def test_registry_add_remove_round_trip(tmp_path: Path) -> None:
    ...
    registry.register_codex_native_process(
        pid=123,
        pgid=456,
        tmux_session_name="codify-codex-123",
        session_tag="tag-123",
...
def test_reconciliation_reaps_alive_tagged_process(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        registry,
        "_process_cmdline",
        lambda _pid: "codex codify_crash_teardown_tag=tag-123 app-server",
    )
...
def test_tmux_session_reaped_only_when_recorded_name_exists(tmp_path: Path, monkeypatch) -> None:
    ...
    registry.register_codex_native_process(
        pid=123,
        pgid=456,
        tmux_session_name="codify-codex-live",
    ...
    registry.register_codex_native_process(
        pid=124,
        pgid=457,
        tmux_session_name="codify-codex-missing",
    ...
    monkeypatch.setattr(
        ...
        "_process_cmdline",
        lambda pid: (
            f"codex codify_crash_teardown_tag=tag-{'live' if pid == 123 else 'missing'} app-server"
        ),
    )
