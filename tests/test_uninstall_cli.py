...
from codify.install_ledger import InstallLedger, new_ledger
...
def test_uninstall_cli_resolves_ledger_and_forwards_flags(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("codify.install_ledger.resolve_uninstall_ledger", _ledger)

    def _run(args, *, env, check):
        calls.append((list(args), env.get("CODIFY_UNINSTALL_LEDGER_SOURCE")))
...
def test_uninstall_cli_refuses_without_install_signal(monkeypatch) -> None:
    runner = CliRunner()
    monkeypatch.setattr("codify.install_ledger.resolve_uninstall_ledger", lambda: None)

    result = runner.invoke(cli_module.cli, ["uninstall", "--json"])

    assert result.exit_code == 3
    assert "no Codify install detected" in result.output
...
def test_uninstall_cli_defaults_to_dry_run_without_destructive_flags(
    ...
    monkeypatch.setattr(cli_module, "_uninstall_script_path", lambda: script)
    monkeypatch.setattr("codify.install_ledger.resolve_uninstall_ledger", lambda: ledger)
...
def test_uninstall_cli_uses_exclusive_manifest_and_cleans_temp_script(
    ...
    runner = CliRunner()
    temp_script_dir = Path(tempfile.gettempdir()) / "codify-uninstall-test-cleanup"
    ...
    def _run(args, *, env, check):
        del args, check
        manifest_paths.append(Path(env["CODIFY_UNINSTALL_LEDGER_MANIFEST"]))
        assert manifest_paths[-1].name.startswith("codify-uninstall-ledger-")
