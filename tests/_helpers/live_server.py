"""
Shared fixtures for tests that need a live ``codify.cli server`` subprocess.

Lifted out of ``tests/e2e/conftest.py`` so the inner test suite
(``tests/inner/test_integration.py`` running with Codify mode) can
...
- :func:`make_live_server_fixture` — factory that builds a
  ...
  ``codify.cli server`` subprocess. The subprocess inherits
...
def start_live_server(
    *,
    ...
    """
    Spawn an ``codify.cli server`` subprocess and wait for
    health.
    ...
    # column" errors. In compat mode (CODIFY_COMPAT_SERVER_PYTHON set) this
    ...
    proc = subprocess.Popen(
        ...
        env=env,
        # Compat mode: neutral CWD so the worktree's codify/ on sys.path[0]
