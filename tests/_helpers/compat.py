...
1. **Server redirect (Config 1)** — pin the ``codify.cli server`` subprocess
   to an older build (``CODIFY_COMPAT_SERVER_PYTHON``) while the client,
   runner, host, and tests stay on main. Skip newer-than-server features with
   ``@pytest.mark.min_server_version(...)``.
2. **Runner/host redirect (Config 2)** — pin the ``codify.runner._entry`` and
   ``codify.host._daemon_entry`` subprocesses to an older build
   (``CODIFY_COMPAT_RUNNER_PYTHON``) while the server, client, and tests stay
...
COMPAT_RUNNER_VERSION_ENV = "CODIFY_COMPAT_RUNNER_VERSION"
...
def _compat_cwd(env_var: str, label: str) -> str | None:
    ...
    ``python -m codify...`` puts the CWD on ``sys.path[0]``, so a subprocess
    launched from the repo checkout would import the worktree's ``codify/``
    ...
    if label not in _compat_cwds:
        _compat_cwds[label] = tempfile.mkdtemp(prefix=f"codify-compat-{label}-cwd-")
...
def compat_server_python() -> str | None:
    ...
    :returns: The value of ``CODIFY_COMPAT_SERVER_PYTHON`` (a venv python
...
def server_executable() -> str:
    """
    Interpreter to launch ``codify.cli server`` with.
...
def compat_server_cwd() -> str | None:
    ...
    empty directory so the worktree's ``codify/`` doesn't shadow the pinned
...
def compat_runner_python() -> str | None:
    ...
    :returns: The value of ``CODIFY_COMPAT_RUNNER_PYTHON`` (a venv python
...
def runner_executable() -> str:
    """
    Interpreter to launch ``codify.runner._entry`` / ``codify.host._daemon_entry``.
...
def pinned_runner_version() -> str | None:
    ...
    workflow-set ``CODIFY_COMPAT_RUNNER_VERSION`` is authoritative. ``None``
...
def reconcile_server_version(
    ...
    *,
    ...
    :param reported: Version from ``GET /api/version``, or ``None`` if it
        couldn't be read.
    :param override: ``CODIFY_COMPAT_SERVER_VERSION`` value, or ``None``.
...
def resolve_server_version(base_url: str) -> str:
    ...
    ``CODIFY_COMPAT_SERVER_VERSION`` covers an unreadable endpoint and
