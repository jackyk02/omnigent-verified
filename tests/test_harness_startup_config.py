"""Unit tests for :mod:`codify.harness_startup_config`."""
...
def test_command_explicit_flag_wins(
    ...
    monkeypatch.setenv("CODIFY_CODEX_PATH", "/env/codex")
...
def test_command_canonical_id_for_native_harness(
    ...
    # ``codex-native`` strips the ``-native`` suffix → ``CODIFY_CODEX_PATH``
    # (shared with the headless ``codex`` harness — one var per binary).
    monkeypatch.setenv("CODIFY_CODEX_PATH", "/env/codex-native")
    assert (
...
def test_command_alias_resolves_to_canonical_env_var(
    ...
    # env var is ``CODIFY_CLAUDE_PATH`` (the binary's var, not the id's var).
    monkeypatch.delenv("CODIFY_CLAUDE_SDK_PATH", raising=False)
    monkeypatch.setenv("CODIFY_CLAUDE_PATH", "/env/claude")
    assert (
...
def test_resolve_harness_path_canonical_env_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CODIFY_CODEX_PATH", "/canonical/codex")
...
def test_resolve_harness_path_ignores_non_registry_legacy_var(
    ...
    name, so it's ignored (only the canonical ``CODIFY_CURSOR_PATH`` works).
    """
    monkeypatch.delenv("CODIFY_CURSOR_PATH", raising=False)
...
def test_resolve_harness_path_strips_native_suffix() -> None:
    """pi-native and pi share CODIFY_PI_PATH."""
    import codify.harness_startup_config as m

    assert m._harness_path_env_var("pi-native") == "CODIFY_PI_PATH"
    assert m._harness_path_env_var("pi") == "CODIFY_PI_PATH"
