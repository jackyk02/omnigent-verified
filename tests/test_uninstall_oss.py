...
from codify.install_ledger import sha256_text
...
def test_uninstall_script_removes_profile_block_and_runs_wheel_last(tmp_path: Path) -> None:
    ...
    assert "Codify installer" not in profile.read_text()
    assert profile.read_text() == "keep\nkeep2\n"
    assert uv_log.read_text().strip() == "tool uninstall codify"
    assert list(home.glob(".zshrc.codify.bak.*"))
...
def test_uninstall_script_refuses_tampered_profile_and_skips_wheel(tmp_path: Path) -> None:
    ...
    env["CODIFY_UNINSTALL_LEDGER_SOURCE"] = "backfill"
    env["HOME"] = str(home)
    env["CODIFY_DATA_DIR"] = str(home / ".codify")
...
def test_uninstall_script_external_config_requires_gate_then_removes_json_key(
    ...
    config.write_text('{"mcp_servers": {"codify": {"url": "x"}, "other": {}}}\n')
    manifest = tmp_path / "manifest.tsv"
    manifest.write_text(
        "\t".join(
            [
    ...
    assert "codify" in config.read_text()
    assert any(
    ...
    assert "codify" not in payload["mcp_servers"]
...
def test_uninstall_script_toml_config_and_launch_agent_reporting(tmp_path: Path) -> None:
    ...
    launch_agent = tmp_path / "ai.codify.plist"
    ...
    manifest.write_text(
        ...
        + "\n"
        + "\t".join(
            ["launch_agent", "launchd", str(launch_agent), "ai.codify", "observed", "high"]
        )
        + "\n"
    )
    env = os.environ.copy()
    ...
    assert removed.returncode == 0, removed.stderr
    assert "mcp_servers.codify" not in config.read_text()
...
def test_uninstall_script_unloads_launch_agent_before_stopping_host_pid(
    ...
    manifest = tmp_path / "manifest.tsv"
    manifest.write_text(
        "\t".join(["launch_agent", "launchd", str(launch_agent), "ai.codify", "observed", "high"])
        + "\n"
    )
    ...
    try:
        result = _run_uninstall(
            ...
            env_updates={
                "CODIFY_UNINSTALL_LEDGER_MANIFEST": str(manifest),
                "CODIFY_UNINSTALL_LEDGER_SOURCE": "installer",
            },
        )
...
def test_uninstall_script_external_json_preserves_key_order(tmp_path: Path) -> None:
    ...
    home.mkdir()
    config = tmp_path / "harness.json"
    config.write_text('{"z": 1, "mcp_servers": {"other": {}, "codify": {}}, "a": 2}\n')
    manifest = tmp_path / "manifest.tsv"
    manifest.write_text(
        "\t".join(
            [
    ...
    result = _run_uninstall(
        ...
        env_updates={
            "CODIFY_UNINSTALL_LEDGER_MANIFEST": str(manifest),
            "CODIFY_UNINSTALL_LEDGER_SOURCE": "backfill",
        },
    )
...
def test_uninstall_script_toml_removes_nested_subtables(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    text = config.read_text()
    assert "mcp_servers.codify" not in text
...
def test_uninstall_script_removes_fish_profile_blocks(tmp_path: Path) -> None:
    ...
    fish_confd = home / ".config" / "fish" / "conf.d" / "codify.fish"
...
def test_uninstall_script_purge_namespaces_xdg_state_backups(tmp_path: Path) -> None:
    ...
    payload = json.loads(result.stdout)
    assert payload["backups"]
    assert Path(payload["backups"][0]).parent == state_home / "codify-backups"
