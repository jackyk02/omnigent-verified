...
Covers :func:`codify.inner.codex_executor._provider_codex_config_overrides`
and :func:`codify.codex_native_app_server.resolve_native_codex_launch` —
...
from codify.codex_native_app_server import resolve_native_codex_launch
from codify.inner.codex_executor import _provider_codex_config_overrides
...
def test_provider_codex_overrides_coerce_chat_wire_to_responses() -> None:
    ...
    assert 'model_provider="codify_provider"' in joined
...
def test_resolve_native_codex_launch_subscription_ignores_private_inherited_home(
    ...
    Nested Codify runs can inherit a per-session private Codex home from
    ...
    _write_codex_login(_isolated, logged_in=True)
    inherited = _isolated / ".codify" / "codex-native" / "abc123" / "codex-home"
