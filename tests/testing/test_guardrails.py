...
from codify.testing.guardrails import (
    DEV_PORTS,
    TestGuardrailError,
    base_url_violation,
...
def _guardrail_warnings(records: list[logging.LogRecord]) -> list[str]:
    ...
    return [
        r.getMessage()
        for r in records
        if r.name == "codify.testing.guardrails" and "TEST GUARDRAIL:" in r.getMessage()
    ]
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "sqlite:////home/alice/.codify/chat.db",
        "sqlite:///testing.db",
        "sqlite:///test123.db",
        "sqlite:///contest.db",
        "sqlite:///latest.db",
        "postgresql://prod-host:5432/codify",
...
def test_looks_like_test_db_rejects_symlink_to_real_db(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    link.symlink_to("/opt/codify/prod.db")
...
def test_looks_like_pytest_via_flag() -> None:
    assert looks_like_pytest({"CODIFY_TEST_MODE": "1"}) is True
...
def test_non_test_process_warns(
    ...
    """
    from codify.testing import guardrails
...
def test_real_db_warns(caplog: pytest.LogCaptureFixture) -> None:
    with caplog.at_level(logging.WARNING):
        check_test_environment(
            env=_TEST_ENV,
            db_uri="sqlite:////home/alice/.codify/chat.db",
            warn_only=True,
        )
    warnings = _guardrail_warnings(caplog.records)
...
def test_escape_hatch_downgrades_hard_fail_to_warning(
    ...
    env = {**_TEST_ENV, "CODIFY_DISABLE_TEST_GUARDRAILS": "yes"}
    with caplog.at_level(logging.WARNING):
        violations = check_test_environment(
            env=env,
            db_uri="sqlite:////home/alice/.codify/chat.db",
