"""Unit tests for :class:`codify_client._sessions.SessionsNamespace`.
...
* ``test_*_404``: that the namespace propagates :class:`CodifyError`
...
from codify_client._sessions import (
    Session,
    SessionsNamespace,
)
...
@pytest.mark.asyncio
async def test_create_404_raises_codify_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            404,
...
@pytest.mark.asyncio
async def test_bind_runner_400_raises_codify_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            400,
...
async def test_fork_404_raises() -> None:
    """``fork()`` raises ``CodifyError`` when the source session is missing.
