...
from codify_ui_sdk.terminal import (
    DEFAULT_USER_CONFIG,
    UserConfig,
    UserConfigError,
    load_user_config,
...
def test_user_config_path_uses_shared_state_dir(tmp_path, monkeypatch) -> None:
    ...
    assert state_dir() == tmp_path / ".codify"
    assert user_config_path() == tmp_path / ".codify" / "config.yaml"
...
def test_save_default_user_config_round_trips_without_tui_table(tmp_path) -> None:
    ...
    assert path.read_text(encoding="utf-8") == "# Codify user configuration\n"
