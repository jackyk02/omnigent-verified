"""Unit tests for :class:`codify_client._sessions_chat.SessionsChat`.
...
import pytest
from codify_client._query import QueryResult, QueryStream
from codify_client._sessions import Session, SessionsNamespace
from codify_client._sessions_chat import (
    SessionsChat,
    SessionToolCallInfo,
)
from codify_client._tool_handler import StreamHooks
from codify_client._types import File
...
async def test_send_raises_on_failed_status_with_error_message() -> None:
    ...
    :class:`CodifyError` with the carried message, instead of
    blocking until the stream closes and returning empty text.
    """
    from codify_client._errors import CodifyError
...
async def test_query_uses_final_message_item_when_deltas_absent() -> None:
    ...
    ``query()`` must surface that text so headless ``codify run
