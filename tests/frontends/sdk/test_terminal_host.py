"""
Unit tests for :class:`codify_ui_sdk.terminal._host.TerminalHost`.
...
import pytest
from codify_client import child_summary_busy
from codify_ui_sdk.terminal._formatter import StreamingText
from codify_ui_sdk.terminal._host import TerminalHost
...
async def test_aenter_logs_stderr_redirect_failures(
    ...
    monkeypatch.setattr(
        "codify.cli_diagnostics.redirect_stderr_to_log",
        _raise_redirect_failure,
    )
    host = TerminalHost(model_name="test")

    with caplog.at_level(logging.ERROR, logger="codify_ui_sdk.terminal._host"):
        async with host:
            pass
...
async def test_aexit_logs_stderr_restore_failures(
    ...
    monkeypatch.setattr(
        "codify.cli_diagnostics.restore_stderr",
...
def _patch_create_output(monkeypatch: pytest.MonkeyPatch, output: object) -> None:
    ...
    """
    monkeypatch.setattr(
        "codify_ui_sdk.terminal._host.create_output",
        lambda: output,
    )
...
async def test_window_title_set_on_aenter_and_cleared_on_aexit(
    ...
    What this proves: a user running ``codify run agent.yaml``
...
def test_default_history_file_matches_legacy_codify_path() -> None:
    """
    With no ``history_file`` override, the host's prompt session
    persists input history to ``~/.codify_history`` — the
    same path the legacy ``codify run`` CLI uses
    (``codify/inner/cli.py:_cli_history_file_path``).

    What this proves: a user who flips between
    ``codify run agent.yaml`` (legacy) and
    ``codify run agent.yaml`` sees the same ↑ / Ctrl+R
    recall in both, instead of two divergent histories. If the
    default drifts (e.g. back to ``~/.codify-history``,
    or to a fresh location), this test fails loud at the SDK
    boundary so the divergence doesn't ship silently. The
    history file's location is part of the Codify mode-vs-legacy
...
async def test_window_title_swallows_set_title_failure(
    ...
    won't break ``codify run`` — the REPL still boots,
...
def test_output_dispatches_stream_replace_to_replace_live_region(
    ...
    """
    from codify_ui_sdk.terminal._formatter import StreamReplace
...
def test_output_wraps_urls_in_osc_8_hyperlink(
    ...
    :func:`codify_ui_sdk.terminal._linkify.linkify_ansi`.
...
def test_compute_sidebar_scroll_offset_no_change_when_selection_in_view() -> None:
    ...
    """
    from codify_ui_sdk.terminal._host import _compute_sidebar_scroll_offset
...
def test_output_stream_live_replaces_live_region(
    ...
    """
    from codify_ui_sdk.terminal._formatter import StreamLive
...
def test_stream_live_then_replace_commits(
    ...
    """
    from codify_ui_sdk.terminal._formatter import StreamLive, StreamReplace
...
def test_live_region_capped_to_viewport_ceiling(
    ...
    monkeypatch.setattr(
        "codify_ui_sdk.terminal._host._term_height",
        lambda: 10,
    )
...
def test_subagent_badge_spinner_animates() -> None:
    ...
    from codify_ui_sdk.terminal._host import _SPINNER_FRAMES
...
def test_subagent_menu_caps_visible_rows_at_five() -> None:
    ...
    from codify_ui_sdk.terminal._host import _SUBAGENT_MENU_MAX_ROWS
