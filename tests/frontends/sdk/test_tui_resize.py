...
import pytest
from codify_ui_sdk.terminal._formatter import StreamingText, StreamLive
from codify_ui_sdk.terminal._host import (
...
def test_resize_width_affects_rich_render_columns(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 200)
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 40)
    host.output(Text("A" * 100))
...
def test_resize_width_affects_streaming_text_wrap(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 30)
...
def test_resize_height_affects_viewport_ceiling(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: 10)
...
def test_resize_height_large_allows_more_streaming(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: 50)
...
def test_resize_mid_stream_changes_ceiling(
    ...
    height = 20
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: height)
    for i in range(10):
        host.output(StreamingText(text=f"line {i}\n"))
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: 8)
...
def test_resize_build_prompt_bar_width(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 50)
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 100)
...
def test_resize_build_toolbar_right_padding(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 60)
    toolbar_60 = host.build_toolbar()
    bar_60 = _extract_toolbar_bar(toolbar_60)

    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_width", lambda: 120)
...
def test_resize_live_region_cap_changes_with_height(
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: 15)
    ...
    monkeypatch.setattr("codify_ui_sdk.terminal._host._term_height", lambda: 40)
