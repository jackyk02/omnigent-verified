...
from codify.opencode_native import (
...
def test_preflight_local_tools_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    import codify.opencode_native as on
...
def test_update_startup_progress_handles_none_and_active() -> None:
    ...
    from codify.opencode_native import _update_startup_progress
...
def test_record_launch_for_fresh_session_persists_current_cwd(
    ...
    """
    import codify.opencode_native as on
    from codify.opencode_native_state import read_launch_state

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    monkeypatch.chdir(workspace)
    monkeypatch.setenv("CODIFY_OPENCODE_NATIVE_STATE_DIR", str(tmp_path / "state"))
...
def test_align_matching_cwd_is_noop(
    ...
    """
    import codify.opencode_native as on
    from codify.opencode_native_state import write_launch_state
