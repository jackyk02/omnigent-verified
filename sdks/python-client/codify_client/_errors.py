"""Typed exceptions for the codify client."""
...
class CodifyError(Exception):
    """Base exception for all codify client errors."""
...
def raise_for_status(status_code: int, body: dict[str, object] | str) -> None:
    ...
    _CODE_MAP: dict[str, type[CodifyError]] = {
        "not_found": CodifyError,
    ...
    if status_code == 404:
        ...
        # CodifyError or the specific subclass at the call site.
        raise CodifyError(message, status_code, code)
...
def require_json_object(resp: httpx.Response, endpoint: str) -> dict[str, object]:
    ...
    protocol/base-URL/auth problem, so raise :class:`CodifyError`
    ...
    :raises CodifyError: If the body is not a JSON object.
    """
    try:
        data = resp.json()
