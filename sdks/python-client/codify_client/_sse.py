...
from codify.server import schemas as _srv_events
...
_log = logging.getLogger("codify_client.sse")
...
def _wire_type(cls: type) -> str:
    ...
    Each :mod:`codify.server.schemas` event class pins its
    ...
    :param cls: A subclass of the server's ``_SSEEventBase``,
        e.g. :class:`codify.server.schemas.OutputTextDeltaEvent`.
...
def _parse_event(event_type: str, data: dict[str, Any]) -> StreamEvent | None:
    ...
    :param event_type: Wire name of the SSE ``event:`` field, e.g.
        ...
        :mod:`codify.server.schemas`) to dispatch.
