"""LocalServer — start and manage a local codify server."""
...
if TYPE_CHECKING:
    from ._client import CodifyClient
...
class LocalServer:
    """Context manager that starts a local codify server.
    ...
    def __init__(
        ...
        *,
        ...
        self._client: CodifyClient | None = None
    ...
    async def __aenter__(self) -> LocalServer:
        ...
        self._client = CodifyClient(base_url=self.base_url)
        return self
    ...
    def _start(self) -> None:
        ...
        self._tmpdir = tempfile.mkdtemp(prefix="codify-client-")
        db_uri = f"sqlite:///{self._tmpdir}/chat.db"
        art_loc = f"{self._tmpdir}/artifacts"

        # Resolve the codify project root so Alembic can find
        # its migrations directory. Walk up from the agent path or
        # from this file's location looking for codify/cli.py.
