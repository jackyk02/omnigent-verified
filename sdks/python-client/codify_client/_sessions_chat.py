...
A higher-level wrapper over :class:`codify_client._sessions.SessionsNamespace`
that mirrors enough of :class:`codify_client._session.Session`'s
public surface for downstream consumers (terminal REPL, ``codify
...
:class:`codify_client._session.Session` synthesizes a multi-turn
...
``codify/server/API.md`` ("Sessions API"). Per the same spec
...
:data:`codify.server.schemas.ServerStreamEvent` envelopes
...
:class:`codify_client._session.Session` and renaming would break
...
from ._errors import CodifyError
...
# :class:`codify.server.schemas.SessionEventInput`. Kept as a
...
class SessionToolCallInfo:
    ...
    :class:`codify_client._tool_handler.ToolCallInfo` — that
...
# terminal set listed in ``codify/server/schemas.py`` (and
# the ``_TERMINAL_STATUSES`` set in
# :mod:`codify_client._session`).
...
class SessionsChat:
    ...
    Usage mirrors :class:`codify_client._session.Session` for
    ...
    session is an HTTP call. Use :meth:`CodifyClient.sessions_chat`
    ...
    :param namespace: The :class:`SessionsNamespace` this helper
        ...
        :class:`CodifyClient`; the chat helper does not own
    ...
    async def create(
        ...
        *,
        ...
        :returns: A :class:`SessionsChat` bound to the newly created
            session.
        :raises CodifyError: If session creation fails.
        """
    ...
    async def refresh(self) -> Session:
        ...
        :raises CodifyError: If the session no longer exists
    ...
    async def tree_busy(self, *, max_depth: int = 3) -> bool:
        ...
        :raises CodifyError: On non-2xx status.
        """
    ...
    async def send(
        ...
        *,
        ...
        :raises CodifyError: If the underlying ``post_event`` or
        ...
        try:
            ...
            while True:
                ...
                if isinstance(event, SessionStatusEvent) and event.status == "failed":
                    ...
                    raise CodifyError(message, code=code)
                try:
        ...
        * a key in ``tool_callables`` that does not name a
        ...
        if self._tool_callables and self._agent_tools_getter is None:
            raise RuntimeError(
                ...
                "CodifyClient.sessions_chat which wires it "
    ...
    async def cancel(self) -> None:
        ...
        :raises CodifyError: If the session does not exist
            (404).
        """
    ...
    async def query(
        ...
        *,
        ...
        :raises CodifyError: If the underlying HTTP calls fail.
    ...
    async def await_turn(self, *, timeout: float | None = 1200.0) -> QueryResult:
        ...
        :raises CodifyError: Propagated from :meth:`stream` if the
    ...
    async def _build_content(
        ...
        :meth:`codify_client._session.Session._build_input_with_files`
...
class _FilesUploader(Protocol):
    ...
    :meth:`codify_client._files.FilesNamespace.upload`. Defined
...
class _FilesGetter(Protocol):
    ...
    Matches the signature of
    :meth:`codify_client._files.FilesNamespace.get`.
    """
...
def _build_tool_call_info(item: dict[str, Any]) -> SessionToolCallInfo:
    ...
    ``codify/server/schemas.py:OutputItemDoneEvent``) —
