...
from codify_client import TERMINAL_TASK_STATUSES, child_session_busy
...
_OVERLAY_REQUEST_SENTINEL: str = "\x00__codify_ui_sdk.overlay_trigger__\x00"
...
_SUBAGENT_SELECT_SENTINEL: str = "\x00__codify_ui_sdk.subagent_select__\x00"
...
# that codify' cli.py uses so the two REPLs look identical
...
class _SubagentNode:
    ...
    # labels / title via :func:`codify.session_lifecycle.is_session_closed`.
...
class Overlay:
    ...
    * **Sidebar + content** (``targets_builder`` is supplied):
    ...
    :param builder: Async callable invoked each time the overlay
        ...
        See ``codify/repl/_repl.py::_build_debug_overview``
    ...
    :param sidebar_width: Column width (in characters) for the
        ...
        — matches the codify debug panel and fits the
...
def _install_csi_u_sequences() -> None:
    ...
    ``codify/inner/cli.py:1186-1239`` so behavior is
...
class TerminalHost:
    ...
    :param history_file: Path for persistent input history.
        Defaults to ``"~/.codify_history"`` to match the
        legacy ``codify run`` CLI's location
        (``codify/inner/cli.py:_cli_history_file_path``) so
        users who flip between legacy and Codify mode see the same
    ...
    def __init__(
        ...
        *,
        ...
        history_file: str = "~/.codify_history",
    ...
    async def __aenter__(self) -> TerminalHost:
        ...
        try:
            from codify.cli_diagnostics import redirect_stderr_to_log
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        try:
            from codify.cli_diagnostics import restore_stderr
    ...
    async def _show_overlay(self, overlay: Overlay) -> None:
        ...
        async def _refresh_loop() -> None:
            ...
            hammer the server. Matches codify' overview polling
            strategy (see ``codify/cli.py::_refresh_loop``),
            with the interval bumped from 50 ms → 500 ms because
            Codify' builder crosses a real HTTP boundary
            while codify' builder just reads in-process state.
            """
            try:
                while True:
                    await asyncio.sleep(0.5)
        ...
        def _get_sidebar() -> ANSI:
            ...
            even with variable-length labels. Matches the codify
    ...
    def upsert_subagent(
        ...
        *,
        ...
        # the ``codify`` server package on the path.
        if not node.closed:
            from codify.session_lifecycle import is_session_closed
    ...
    def build_toolbar(self) -> FormattedText:
        ...
        codify' main REPL shows at the bottom-right. A
