...
under the ``tui:`` table of the shared Codify YAML config file
(``$HOME/.codify/config.yaml``). Today that means the persisted
light/dark theme selection.

The same file is also written by the ``codify`` CLI (top-level keys
...
def state_dir() -> pathlib.Path:
    """Return the shared Codify per-user state directory.

    The directory is currently ``$HOME/.codify``. Callers that only need
...
def user_config_path(root: str | pathlib.Path | None = None) -> pathlib.Path:
    ...
    :param root: Optional explicit state directory. Defaults to
        :func:`state_dir`, i.e. ``$HOME/.codify``.
    :returns: The config file path, e.g.
        ``Path.home() / ".codify" / "config.yaml"``.
    """
...
def save_user_config(
    ...
    Sibling top-level keys written by the ``codify`` CLI (e.g.
...
def _dump_user_config(data: Mapping[str, Any]) -> str:
    ...
    header = "# Codify user configuration\n"
    if not data:
        return header
