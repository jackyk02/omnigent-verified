...
description: Reference guide for building new Codify harness integrations — covers SDK/subprocess harnesses and native harnesses as separate tracks, each with their own feature matrix, implementation patterns, and prioritized checklist.
---

# Harness integration guide

This skill describes the **feature matrix** every Codify harness must
consider. Use it when planning, reviewing, or implementing a new harness.

Codify has two distinct harness tracks with different architectures and
feature sets:
...
- **Native harnesses** — wrap a vendor's own TUI or server and mirror its
  output into Codify. They observe and relay, rather than drive.

---

## Part 1 — SDK / subprocess harnesses

These harnesses run the vendor model directly and bridge Codify tools into
...
|---|---|
| **Connects to Codify MCP** | Harness exposes/consumes tools via the MCP protocol (in-proc SDK MCP server) |
...
| **Streaming** | Harness forwards token-level or delta-level streaming to the Codify forwarder |
| **Codify policies** | Harness enforces Codify-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the harness surfaces the approval request in the Codify web UI so the user can approve or deny |
...
| **Tool-boundary steer** | Codify can inject steering text at tool-call boundaries |
| **Resume/fork from Codify transcript** | Rebuild a conversation from a stored Codify transcript (replay history, seed prompt, or vendor session ID) |
...
| **Cost tracking** | Harness reports token usage and cost data back to Codify for each turn |

### MCP connectivity

The harness must bridge Codify's builtin MCP tools so the model can call
...
The harness must support the Codify policy engine's three verdicts at two
checkpoints:
...
or tool result in the Codify web UI as an approval card, then relay the
...
| Vendor session ID | Relies on the vendor's own session persistence (no Codify-side rebuild) |
...
- [ ] Connects to Codify MCP (in-proc SDK MCP server or vendor-specific bridge)
- [ ] Model override works (or document vendor lock-in)
- [ ] Auth is configured and documented (setup flow in `omni setup`)
- [ ] Streaming forwards to the Codify forwarder
- [ ] Codify policies enforce tool-use rules
...
- [ ] Resume/fork rebuilds conversation from Codify transcript
...
Codify. They relay the vendor's conversation into the Codify session.
...
| **Connects to Codify MCP** | Whether the native harness connects to the Codify MCP server |
...
| **Codify policies** | Whether the native harness enforces Codify-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the native harness surfaces the approval request in the Codify web UI so the user can approve or deny |
| **Interrupt** | User can abort a running turn |
| **Bidirectional sync (TUI->Omni)** | TUI output mirrors into the Codify conversation |
| **In-harness session-cmd sync** | Supports `clear`, `fork`, `resume`, `switch` commands from Codify |
| **Resume/fork from Codify transcript** | Can rebuild conversation from Codify transcript (native rebuild, or fresh launch) |
...
| **Cost tracking** | Native harness reports token usage and cost data back to Codify for each turn |
...
- [ ] Connects to Codify MCP
- [ ] Auth configured (vendor login / config)
- [ ] Streaming forwarder works (deltas preferred; complete-only acceptable)
- [ ] Codify policies enforce tool-use rules (ALLOW / ASK / DENY at both tool call and tool result)
- [ ] Native elicitation surfaces tool-approval requests to web UI
- [ ] Interrupt aborts the running turn
- [ ] Bidirectional sync mirrors TUI output into Codify conversation
- [ ] Cost tracking reports token usage and cost per turn
...
- [ ] Session commands (clear, fork, resume) work from Codify
- [ ] Resume/fork rebuilds from Codify transcript
- [ ] Reasoning tokens are forwarded
