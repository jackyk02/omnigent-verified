...
description: Spin up a live local Codify server and exercise the Antigravity (Gemini) SDK harness end-to-end — build antigravity agents, run real turns, smoke-test, and bug-bash. Load when developing, testing, or debugging the antigravity harness (codify/inner/antigravity_executor.py, antigravity_harness.py, codify/onboarding/antigravity_auth.py) or its auth / model / tool-bridge behavior.
---
...
Codify's `sys_*` tools into the SDK as `custom_tools`. It is **Gemini-native**:
...
2. **A Gemini API key is configured.** The SDK *requires* one (`AIza…`); there
   ...
   ```bash
   .venv/bin/python -c "from codify.onboarding.antigravity_auth import antigravity_api_key_configured as c; import os; print('config:', c(), 'env:', bool(os.environ.get('GEMINI_API_KEY') or os.environ.get('ANTIGRAVITY_API_KEY')))"
   ```
...
3. **`google-antigravity` is installed** (the `antigravity` extra —
   `pip install "codify[antigravity]"`):
...
server on a fixed port with `codify server --port 7777 --no-open`.)
...
1. **`config.yaml`'s `server:` defaults to a *remote* server.** Omitting
   ...
   for local testing. (That allowlist is `codify/spec/_codify_compat.py`; if
...
- **Executor (SDK driver):** `codify/inner/antigravity_executor.py`
- **Wrap (HARNESS_ANTIGRAVITY_* env → executor):** `codify/inner/antigravity_harness.py`
- **Auth / key resolution:** `codify/onboarding/antigravity_auth.py`
- **Spawn env:** `_build_antigravity_spawn_env` in `codify/runtime/workflow.py`
...
`tests/e2e/codify/test_run_harness_without_agent_e2e.py`, because that matrix
