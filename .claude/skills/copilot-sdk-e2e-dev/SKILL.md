...
description: Spin up a live local Codify server and exercise the GitHub Copilot SDK harness end-to-end — build copilot agents, run real turns, smoke-test, and bug-bash. Load when developing, testing, or debugging the copilot harness (codify/inner/copilot_executor.py, copilot_harness.py, codify/onboarding/copilot_auth.py) or its auth / model / tool-bridge behavior.
---
...
Codify conversation — and bridges Codify's `sys_*` tools into Copilot as SDK
...
3. **A GitHub token with Copilot access is configured.** Copilot needs a
   ...
   ```bash
   .venv/bin/python -c "from codify.onboarding.copilot_auth import copilot_github_token_configured; import os; print('config:', copilot_github_token_configured(), 'env:', bool(os.environ.get('GH_TOKEN') or os.environ.get('COPILOT_GITHUB_TOKEN')))"
   ```
...
# --cuj fanout  …  and  --cuj review-pr --repo codify-ai/codify --pr <n>  …
...
- **Executor (SDK bridge):** `codify/inner/copilot_executor.py`
- **Wrap (HARNESS_COPILOT_* env → executor):** `codify/inner/copilot_harness.py`
- **Auth / token resolution:** `codify/onboarding/copilot_auth.py`
- **Spawn env:** `_build_copilot_spawn_env` in `codify/runtime/workflow.py`
