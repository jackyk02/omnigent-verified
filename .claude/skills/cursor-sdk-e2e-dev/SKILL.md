...
description: Spin up a live local Codify server and exercise the Cursor SDK harness end-to-end — build cursor agents, run real turns, smoke-test, and bug-bash. Load when developing, testing, or debugging the cursor harness (codify/inner/cursor_executor.py, cursor_harness.py, cursor_auth.py) or its auth / model / tool-bridge behavior.
---

# Cursor SDK harness: end-to-end dev & testing

The `cursor` harness drives the **Cursor Python SDK** (`cursor_sdk`, an
`AsyncAgent` over a local bridge) and bridges Codify's `sys_*` tools into
...
2. **A Cursor API key is configured.** The SDK *requires* an API key
   ...
   never print the key):
   ```bash
   .venv/bin/python -c "from codify.onboarding.cursor_auth import cursor_api_key_configured; import os; print('config:', cursor_api_key_configured(), 'env:', bool(os.environ.get('CURSOR_API_KEY')))"
   ```
...
1. **`config.yaml`'s `server:` defaults to a *remote* server** (e.g. a
   ...
   `codify/spec/_codify_compat.py`; if a *local* server rejects `cursor`,
...
- **Executor (SDK bridge):** `codify/inner/cursor_executor.py`
- **Wrap (HARNESS_CURSOR_* env → executor):** `codify/inner/cursor_harness.py`
- **Auth / key resolution:** `codify/onboarding/cursor_auth.py`
- **Spawn env:** `_build_cursor_spawn_env` in `codify/runtime/workflow.py`
...
uv run --frozen --extra dev python -m pytest tests/e2e/codify/test_per_harness_cursor.py -q
```
