#!/usr/bin/env python3
"""Drive the Codify CLI through a PTY in a throwaway sandbox and verify it.
...
1. Builds an **isolated config/data sandbox** so nothing the CLI writes ever
   lands in the real ``~/.codify`` — it sets the purpose-built
   ``CODIFY_CONFIG_HOME`` / ``CODIFY_DATA_DIR`` knobs (``codify/cli.py``
...
2. Drives the real ``codify`` binary through ``pexpect`` (a real PTY with a
...
5. Proves it left the real ``~/.codify`` byte-for-byte unchanged.
...
try:
    ...
    sys.stderr.write(
        "verify_cli.py needs `pexpect`. Run it with the codify project's "
        "venv python (it bundles pexpect), e.g.\n"
        "  <repo>/.venv/bin/python verify_cli.py ...\n"
    )
    raise

# --- PTY constants (mirrors tests/e2e/codify/_pexpect_harness.py) ---------
...
# Stable onboarding anchors (codify/cli.py:520, :10064, :10300).
...
def build_sandbox(
    *,
    ...
    """Create an isolated sandbox env that cannot touch the real ``~/.codify``.
    ...
    ".codify"`` (``codify_ui_sdk/terminal/_config.py``) and ignores
    ``CODIFY_CONFIG_HOME`` / ``CODIFY_DATA_DIR``. So redirecting ``HOME`` is
    ...
    :param inherit_home: Opt OUT of home isolation — use the real ``HOME`` (and
        ...
        ``~/.codify/logs`` (the broadened fingerprint catches this).
    :param strip_path: Reduce ``PATH`` to just the codify binary's dir + an
        empty dir, so node/npm/tmux/claude/codex read as "not installed" — i.e.
        a brand-new machine.
    :param codify_bin: Path to the ``codify`` console script being driven.
    :returns: A :class:`Sandbox`.
    """
    root = Path(mkdtemp(prefix="codify-verify-"))
    (root / "config").mkdir()
    (root / "data").mkdir()
    ...
    env["CODIFY_CONFIG_HOME"] = str(root / "config")
    env["CODIFY_DATA_DIR"] = str(root / "data")
    env["CODIFY_NO_UPDATE_CHECK"] = "1"  # keep the update nag out of frames
    env["TERM"] = TERM
...
def fingerprint_real_config() -> dict[str, str]:
    ...
    * the set of ``logs/cli-*.log`` diagnostic files — what *any* non-help CLI
      invocation writes via the hardcoded ``Path.home()/.codify`` state dir.
      A new ``cli-*.log`` basename after the run means we wrote into the real
      home (the precise violation that slips through ``CODIFY_CONFIG_HOME`` /
      ``CODIFY_DATA_DIR``). With home isolation on (the default) none appear;
    ...
    running codify daemons churn them → false alarms). The single ``logs/``
...
def scenario_check_isolation(args, sandbox: Sandbox, result: Result) -> None:
    ...
    Runs ``codify config list`` inside the sandbox (no PTY needed) and
    asserts (a) it executed, (b) the sandbox config home is now used, (c) the
    real ``~/.codify`` fingerprint is unchanged. This is the first thing to
    run to trust every other scenario.
    """
    proc = subprocess.run(
        [str(args.codify), "config", "list"],
        env=sandbox.env,
        cwd=str(args.repo),
        capture_output=True,
        text=True,
    ...
    result.add(
        ...
        Path(sandbox.env["CODIFY_CONFIG_HOME"]).exists(),
        sandbox.env["CODIFY_CONFIG_HOME"],
    )
...
def scenario_help_snapshot(args, sandbox: Sandbox, result: Result) -> None:
    """Render ``codify [SUBCOMMAND] --help`` and lint it for known UX issues.
...
def resolve_codify(repo: Path, explicit: str | None) -> Path:
    """Find the ``codify`` console script to drive."""
    if explicit:
        return Path(explicit).resolve()
    venv = repo / ".venv" / "bin" / "codify"
    if venv.exists():
        return venv.resolve()
    found = shutil.which("codify")
    if found:
        return Path(found).resolve()
    sys.exit("Could not find an `codify` binary; pass --codify <path>.")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--codify",
        help="Path to the codify binary (default: <repo>/.venv/bin/codify or PATH).",
    )
    p.add_argument(
        "--label",
        default="run",
    ...
    p.add_argument(
        ...
        "~/.codify/logs. Use only to reach a real credentialed REPL.",
    )
    p.add_argument(
        "--strip-path",
        action="store_true",
...
def main(argv: Sequence[str]) -> int:
    ...
    result.add(
        ...
        untouched,
        "~/.codify unchanged" if untouched else "REAL CONFIG MUTATED — investigate",
    )
