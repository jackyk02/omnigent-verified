...
description: Verify the Codify CLI's setup/onboarding flow, terminal UI/UX, and critical user journeys in a completely isolated, reproducible loop. Drives the real `codify` binary through a PTY (pexpect) inside a throwaway CODIFY_CONFIG_HOME / CODIFY_DATA_DIR sandbox that never touches the user's real ~/.codify, captures ANSI-stripped frames for UX inspection, and proves a change is verifiable via a before→fix→after baseline diff. Load when developing or reviewing a CLI setup/onboarding/REPL/picker change (codify/cli.py, codify/onboarding/*, codify/repl/*, scripts/install_oss.sh), reproducing a cold-start/first-run UX bug, or confirming a fix actually lands. Several agents can run it concurrently on separate worktrees.
---

# Verifying the Codify CLI setup & UX in a closed loop

The Codify CLI's first impression is: `curl | sh` → run `codify` → pick a
...
`codify` binary through a pseudo-terminal (`pexpect`) inside a throwaway
...
The real `~/.codify` here can be **many GB** (chat DB, runner logs, native
...
- **`HOME` is redirected into the sandbox by default.** This is the load-bearing
  one. `CODIFY_CONFIG_HOME` / `CODIFY_DATA_DIR` (`codify/cli.py`
  ...
  `cli-*.log` under `state_dir()`, hardcoded to `Path.home()/.codify/logs`
  (`codify_ui_sdk/terminal/_config.py`). So only redirecting `HOME` keeps a
...
Every run **fingerprints the real `~/.codify` before and after** (stat-only,
...
- You're in the **worktree whose code you want to test** (each parallel agent
  on its own worktree). The driver runs `codify` from `--repo`'s checkout.
...
- An `codify` binary: the driver auto-finds `<repo>/.venv/bin/codify`, or
  pass `--codify <path>`.
...
|---|---|---|---|
| `check-isolation` | `codify config list` in the sandbox (no PTY) | `config_list_ran`, `sandbox_config_home_used`, `real_config_untouched` | safety gate for everything |
| `cold-start` | `codify setup` via PTY on a simulated fresh machine | `onboarding_rendered`, `harness_menu_present`; note `guided_default_affordance` | cold-start dead-end; missing "recommended start here" |
| `setup-snapshot` | `codify setup`, optional `--nav-down N` arrow steps | `menu_rendered`; saves a frame per step | picker markers/footer/alignment; narrow-terminal at 80×24 |
| `help-snapshot` | `codify [--subcommand] --help` (no PTY) | `help_rendered`, `no_param_leak`, `no_update_dup`; note `top_level_command_count` | `:param` leak, duplicate `update`/`upgrade`, command sprawl |
| `repl-commands` | `codify run <agent>` REPL, sends `/help` + `/quit` | `help_lists_commands`; note `quit_advertised` | REPL discoverability (`/help`, `/quit`) |
...
- **Reusable PTY helpers** live in `tests/e2e/codify/_pexpect_harness.py`
  (`spawn_codify_run`, `wait_for_ready`, `submit_prompt`, `await_turn_complete`,
  `clean_exit`) and the snapshot comparator in `tests/e2e/codify/_snapshot.py`
...
- The driver force-kills the PTY child and its descendants, and runs
  `codify server stop` against the sandbox to reap any spawned background
  server. After a run, confirm nothing leaked:
  `pgrep -af "codify.*(server|runner|host._daemon)"` — anything bound to your
...
- Always drive the CLI through the driver (which redirects `HOME` + the
  config/data knobs), never a bare `codify setup` — that would write to the
  real `~/.codify`. If you pass `--inherit-home`, expect `cli-*.log` writes to
  the real `~/.codify/logs` and a `real_config_untouched: false` — that's the
...
- First-run dispatch / no-arg routing: `codify/cli.py` (`run`, the first-run
  plan, `_run_configure_harnesses_interactive`).
- Onboarding: `codify/onboarding/*` (`setup.py`, `interactive.py`,
  `configure_models.py`, `provider_selection.py`, `detected.py`).
- TUI / REPL & pickers: `codify/repl/*` (`_repl.py`, `_theme_picker.py`,
  `_resume_picker.py`), `codify/_terminal_picker_theme.py`.
