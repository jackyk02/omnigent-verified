...
description: Spin up a live local Codify server + runner and exercise the native Pi TUI harness (pi-native) end-to-end — launch the real `pi` CLI via `codify pi`, drive turns through the web/bridge, smoke-test, and bug-bash. Load when developing, testing, or debugging the pi-native harness (codify/inner/pi_native_executor.py, pi_native_harness.py, codify/pi_native.py, pi_native_bridge.py, pi_native_credentials.py) or its bridge / extension / auth / model behavior.
---
...
(cursor / copilot / antigravity), it does **not** run in-process: `codify pi`
ensures a host daemon, the daemon spawns a **runner** that launches `pi` inside a
runner-owned **tmux** terminal, and your TTY attaches to it. Codify's web-UI
...
```
your TTY ── (attach / pexpect) ──► codify pi (CLI, local)
                                        │ ensures
                                        ▼
...
2. **The `pi` CLI is on PATH** — the harness can't launch without it:
   ...
   # or point at an explicit binary:  export CODIFY_PI_PATH=/path/to/pi
   .venv/bin/python -c "from codify.onboarding.harness_readiness import harness_is_configured; print('pi-native ready:', harness_is_configured('pi-native'))"
   ```
...
5. **Auth is resolvable (booleans/ids only — never print keys).** Native Pi
   normally logs in from its own `~/.pi/agent`. Codify bridges the provider you
   set with `codify setup` instead, writing a managed per-session `models.json`
   and passing `--provider codify --model <resolved>`. Verify what it will use:
   ```bash
   .venv/bin/python -c "from codify.pi_native_credentials import resolve_pi_native_provider as r; p=r(); print('provider:', getattr(p,'provider_id',None), '| api:', getattr(p,'api',None), '| model:', getattr(p,'model',None))"
   ```
   `None` → no codify provider configured; Pi falls back to its own `/login`
   (run `codify setup`, or log into `pi` directly). A Databricks default
...
(`codify pi --server ""` also auto-spawns a persistent local server and uses
...
`codify pi` **attaches an interactive TUI**, so run it where you can hold it
open. Two patterns:
...
```bash
.venv/bin/codify pi --server "$SERVER" 2>&1   # attaches the Pi TUI; leave it running
```
...
spawn `codify pi --server <url>` in a PTY with `cwd=<checkout>`, capture the
...
`terminal_launch_args`), e.g. `codify pi --server "$SERVER" -- --model <id>`;
codify still injects `--provider codify --model <resolved>` when a provider
...
```bash
.venv/bin/python -c "from codify.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))"
# ~/.codify/pi-native/<sha256(conv)[:32]>/
...
#   codify_pi_native_extension.js
ls -la "$(.venv/bin/python -c "from codify.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))")/inbox"
```
...
`HARNESS_PI_NATIVE_REQUEST_SESSION_ID`, `CODIFY_PI_NATIVE_CONFIG`,
`CODIFY_PI_PATH` (legacy `HARNESS_PI_PATH`), `PI_CODING_AGENT_DIR`.
...
| Resume | stop the TUI, `codify pi --server "$SERVER" --resume "$CONV"` — reattaches; `--resume` (no value) opens the pi-native picker |
...
1. **It's a TUI, not `omni run`.** Use `codify pi`. There is no
...
2. **`config.yaml`'s `server:` defaults to a remote server.** Always pass
   ...
   checkout (allowlist: `codify/spec/_codify_compat.py`).
...
- **Executor (bridge enqueue):** `codify/inner/pi_native_executor.py`
- **Harness wrap (`harness: pi-native`):** `codify/inner/pi_native_harness.py`
- **CLI launch / daemon-runner / tmux attach:** `codify/pi_native.py`
  (`run_pi_native`); CLI command `pi(...)` in `codify/cli.py`
- **Bridge (inbox, extension/config writers):** `codify/pi_native_bridge.py`
- **Auth/model → Pi `models.json`:** `codify/pi_native_credentials.py`
- **Extension (JS, polls inbox, posts events/policies):**
  `codify/resources/pi_native/codify_pi_native_extension.js`
- **Readiness gate:** `codify/onboarding/harness_readiness.py`
...
# JS unit tests: node codify/resources/pi_native/codify_pi_native_extension.test.js
```
...
- **History on a rebuilt session** depends on Pi's own `--session-dir` state under
  the bridge dir, not on Codify re-injecting transcript.
...
# rm -rf "$(.venv/bin/python -c "from codify.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))")"
```
