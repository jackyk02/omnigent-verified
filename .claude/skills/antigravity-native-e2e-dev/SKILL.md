...
description: Spin up a live local Codify server + runner and exercise the native Antigravity (agy) TUI harness (antigravity-native) end-to-end — launch the real `agy` CLI via `codify antigravity`, drive turns through the web UI, smoke-test, and bug-bash. Load when developing, testing, or debugging the antigravity-native harness (codify/inner/antigravity_native_executor.py, codify/antigravity_native.py, antigravity_native_bridge.py, antigravity_native_rpc.py, antigravity_native_reader.py, antigravity_native_launch.py) or its agy launch / RPC mirror / tmux delivery / OAuth / MCP-relay behavior. NOT the in-process `antigravity` Gemini SDK harness.
---
...
`agy` CLI, installed from `antigravity.google/cli/install.sh`). `codify
...
```
your TTY ── (attach / pexpect) ──► codify antigravity (CLI, local)
                                        │ ensures
                                        ▼
                                  host daemon ──► local Codify server (AP)
                                        │ spawns                      ▲
                                        ▼                  connect-RPC │ HTTP
...
2. **Read path = RPC.** `antigravity_native_reader` polls/streams agy's connect-RPC
   trajectory steps and mirrors them into the Codify session.
3. **Control = RPC.** Interrupt is `CancelCascadeSteps`; a tool/permission prompt
   is answered via `HandleCascadeUserInteraction` (surfaced as an Codify
   elicitation).

## Prerequisites (check these first)

1. **You're on the branch you want to test**, running from that checkout
   (`.venv/bin/codify` / `.venv/bin/python` from this repo).
2. **The `agy` CLI is on PATH** (or at `~/.local/bin/agy`) — the harness can't
   ...
   .venv/bin/python -c "from codify.onboarding.harness_readiness import harness_is_configured; print('antigravity-native ready:', harness_is_configured('antigravity-native'))"
   ```
3. **`agy` is signed in (OAuth).** agy is **OAuth-only** — it has no `agy login`;
   ...
   ```bash
   .venv/bin/python -c "from codify.onboarding.gemini_auth import gemini_login_detected; print('agy oauth token present:', gemini_login_detected())"
...
(`codify antigravity --server ""` also auto-spawns a persistent local server and
...
`codify antigravity` **attaches an interactive TUI**, so run it where you can
...
```bash
.venv/bin/codify antigravity --server "$SERVER" 2>&1   # attaches the agy TUI; leave it running
...
`claude-native-e2e-test` skill's `cuj_driver.py`: spawn `codify antigravity
...
defaults to the Codify conversation id):

```bash
.venv/bin/python -c "from codify.antigravity_native_bridge import bridge_dir_for_bridge_id as d; print(d('$CONV'))"
# ~/.codify/antigravity-native/<sha256(bridge_id)[:32]>/
...
#   bridge.json    <- token for the Codify MCP relay (sys_* tools)
...
- The **isolated HOME** (`agy-home/`) is why your real `~/.gemini` is never
  ...
  agy's `/mcp` panel should show `✓ codify` with the `sys_*` tools.
...
| Codify MCP relay (`sys_*`) | in the agy TUI run `/mcp` → expect `✓ codify`; prompt agy to `sys_session_list` / spawn a sub-agent |
| Permission elicitation | with a tool that needs approval, agy's `request-review` surfaces as an **Codify elicitation** (interaction bridge); answer it in the web UI and confirm the tool runs |
...
| Resume | stop, `codify antigravity --server "$SERVER" --resume "$CONV"`; `--resume` (no value) opens the antigravity-native picker |
| Concurrency / leaks | drive several sessions; sweep for orphaned `agy` / tmux after teardown |

## Gotchas (these cost real time)

1. **It's a TUI, not `omni run`.** Use `codify antigravity`. The executor only
...
2. **`config.yaml`'s `server:` defaults to a remote server.** Always pass
   ...
   (allowlist: `codify/spec/_codify_compat.py`).
...
- **Executor (write path — types into the TUI):** `codify/inner/antigravity_native_executor.py`
- **Harness wrap (`harness: antigravity-native`):** `codify/inner/antigravity_native_harness.py`
- **CLI launch / daemon-runner / tmux attach:** `codify/antigravity_native.py`
  (`run_antigravity_native`); CLI command `antigravity(...)` in `codify/cli.py`
- **agy argv / auth-mode / permission flag:** `codify/antigravity_native_launch.py`
- **Bridge (state, tmux delivery, isolated HOME, MCP relay):** `codify/antigravity_native_bridge.py`
- **connect-RPC client (port discovery, send/cancel/interaction):** `codify/antigravity_native_rpc.py`
- **RPC read driver (trajectory mirror):** `codify/antigravity_native_reader.py`
- **Steps / interactions / audit:** `codify/antigravity_native_steps.py`,
  `codify/antigravity_native_interactions.py`, `codify/antigravity_native_audit.py`
- **OAuth detection:** `codify/onboarding/gemini_auth.py`
...
- **Permission gating is all-or-nothing + post-hoc.** agy honors only
  ...
  auto-bypasses and the genuine Codify gate is the elicitation + post-hoc audit
...
# rm -rf "$(.venv/bin/python -c "from codify.antigravity_native_bridge import bridge_dir_for_bridge_id as d; print(d('$CONV'))")"
```
