...
**Date:** 2026-06-25
**Harness:** `cursor-native` (the `codify cursor` interactive TUI wrapper)
...
cursor-agent TUI  ── stop hook (per turn, JSON on stdin) ──▶
  python -m codify.cursor_native_usage record-usage --bridge-dir <dir>
...
| File | Change |
|---|---|
| `codify/cursor_native_usage.py` | **New.** Hook recorder (`record-usage`, stdlib-only) + cumulative accumulator + runner-owned poller/supervisor + `clear_cursor_usage_state`. |
| `codify/cursor_native_bridge.py` | `build_hooks_config` / `write_hooks_config` — write `<workspace>/.cursor/hooks.json` registering the `stop` hook (sibling of `write_mcp_config`). |
| `codify/runner/app.py` | In the cursor terminal setup: `write_hooks_config(...)`, `clear_cursor_usage_state(...)`, and `supervise_cursor_usage_forwarder(...)` added to the existing `_supervise_cursor_native_bridges` gather. |
...
- **Unit/logic (done, offline):** drove the real `record-usage` CLI with two
  ...
  skip-empty.
- **e2e (pending):** launch `codify cursor`, run a couple of turns, confirm
