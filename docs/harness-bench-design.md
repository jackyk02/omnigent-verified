...
2. **The `Executor` capability flags** — `supports_streaming()`,
   ...
   `codify/inner/executor.py:541`.
3. **`codify/model_override.py`** — already encodes per-harness facts
...
harness name to module path in `codify/runtime/harnesses/__init__.py:34`.
...
> `codify.community.harness` entry-point group, and the bench derives
...
as `codify.config`, `find_free_port`, and the harness registry rather than
...
| Codify MCP (P1, native only) | call read-only `sys_session_list` through the generated `codify` MCP relay and require a matching function-call item |
...
- **Six P1 probes:** Fork replay, Reasoning, Codify MCP, Policy ALLOW, Policy ASK, and Cost tracking. P1 verdicts
...
| Codify MCP | Not applicable | Not applicable | Generated `codify` MCP relay when supported by the vendor |
...
The seam is **one level down, in the codify server**. A native harness is
...
- **Declarative native tool-relay mechanism** — extend the harness capability
  ...
  the Codify MCP probe's applicability from that declaration instead of the
  bench's temporary `_NATIVE_CODIFY_MCP_HARNESSES` list.
