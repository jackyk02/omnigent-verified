...
**Tracking issue:** codify-ai/codify#1032 (cursor-native tool-call elicitations not surfaced in web UI)

## Goal / behavior

Surface an Codify **elicitation card whenever cursor's native TUI shows an approval
...
> **Branch note:** the working branch `cursor-native-codify-mcp-e2e` carries an in-progress
...
- `codify/cursor_native_bridge.py`: `read_tmux_info`, `_capture_pane`,
  `_run_tmux(..., "send-keys", ...)`, `_session_alive`, `_settle_pane` — the tmux detection +
  answer primitives.
- `codify/cursor_native_forwarder.py`: `supervise_cursor_forwarder` — the backoff / lifecycle
  supervisor pattern to mirror, already wired into `runner/app.py` `_auto_create_cursor_terminal`.
- `codify/server/routes/sessions.py`: `_publish_and_wait_for_harness_elicitation` (publishes
...
1. **Approval scraper supervisor** — a new runner task (new module
   `codify/cursor_native_permissions.py`) modeled on `supervise_cursor_forwarder`: poll
...
3. **Server route** — new `POST /sessions/{id}/hooks/cursor-permission-request` in
   `codify/server/routes/sessions.py`: build `ElicitationRequestParams` from the scraped
   operation, call the existing `_publish_and_wait_for_harness_elicitation`, return
   accept/decline to the parked runner POST.

4. **Runner wiring** (`codify/runner/app.py` `_auto_create_cursor_terminal`): start the
...
5. **Executor coordination** (`codify/cursor_native_bridge.py`): teach `_settle_pane` to treat
