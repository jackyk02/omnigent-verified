...
**Code:** `codify/kiro_native_permissions.py`, `codify/kiro_native_bridge.py`, runner wiring in `codify/runner/app.py`.

## Behavior

`codify kiro` still runs Kiro's own terminal UI. When Kiro shows a tool approval prompt in the embedded Terminal, Codify also mirrors supported one-time approvals into Chat as an approval card. The Terminal prompt remains authoritative and answerable; the Chat card is additive.
...
The usable permission signal is Kiro's TUI ACP recorder. The runner sets `KIRO_ACP_RECORD_PATH` to a per-session file under the Kiro bridge directory, then `codify/kiro_native_permissions.py` tails that JSONL file. The observed record wrapper is:
...
- Kiro remains authenticated by Kiro's own CLI login and does not use Codify Databricks, OpenAI, or Anthropic provider credentials.
