# Running codify on Databricks

A production deployment guide for running codify agents on Databricks
infrastructure. Covers the four canonical integration points:

1. **Databricks Apps** as the managed runtime for the codify server
...
codify's fine standalone with any OTLP backend and any LLM
...
> (Beta) is a fully managed service: Databricks operates the codify
> server for you, already wired to workspace identity, Foundation
> Models, AI Gateway, and MLflow Tracing. You enable the **Codify**
...
> the codify server yourself on Databricks Apps. Reach for it when the
...
This guide assumes you're new to both codify and Databricks. Each
...
When codify runs on Databricks with the four integration points
wired:
...
- **Managed runtime with workspace identity.** The codify server
...
The integration is layered. The codify server runs on Databricks
Apps. It exports OpenTelemetry traces (via the work landed in [PR
#1050](https://github.com/codify-ai/codify/pull/1050)) to MLflow
...
![codify on Databricks architecture](images/databricks/architecture.png)

The boundary stays sharp. codify itself remains a standalone Apache
...
3. **Python 3.11+** locally with `uv` installed (the codify project
   standard).
...
guide, this is the fastest path. It runs codify locally (not on
Apps) but wires up Foundation Models + Gateway + tracing to the
workspace.

```bash
pip install 'codify[tracing]' openai

export DATABRICKS_HOST=https://<your-workspace>.cloud.databricks.com
export DATABRICKS_TOKEN=<personal-access-token>

# Point codify at Mosaic AI Foundation Models as the LLM provider
export OPENAI_BASE_URL=$DATABRICKS_HOST/serving-endpoints
export OPENAI_API_KEY=$DATABRICKS_TOKEN

# Point codify's OTel exporter at the MLflow OTLP receiver in UC
...
# Start the local codify server with tracing
omni server
```
...
Databricks workspace. codify's server is an HTTP app, so it fits
...
The `deploy/databricks/` directory in the codify repository contains
a complete Databricks Asset Bundle for deploying the codify server
to Apps backed by Lakebase. Use it as-is for a first deploy:

```bash
git clone https://github.com/codify-ai/codify
cd codify
...
#   uv run python deploy/databricks/deploy.py --app-name codify --profile <profile> ...
...
Self-hosting the codify server (e.g., on a VM or in a container)
...
codify's harnesses (`claude_sdk`, `openai_agents_sdk`, `pi`, and
...
Then any codify agent spec that points at a `databricks-` model
...
codify doesn't need to know it's calling a Gateway. The Gateway
endpoint speaks the OpenAI API, so the same `OPENAI_BASE_URL` knob
that points codify at Foundation Models points it at the Gateway.
...
The OpenAI-style chat completions URL (what codify uses) is:
...
### Point codify at the Gateway endpoint
...
Then in the codify agent spec, use the Gateway endpoint name as the
model:
...
then called it via the same OpenAI SDK pattern codify uses. The
...
resp = client.chat.completions.create(
    model='codify-docs-test-gateway',   # the Gateway endpoint, not the backing model
...
(`codify-docs-test-gateway`) is what codify calls. Swapping the
...
| Claude Max / ChatGPT Plus / Cursor Pro (OAuth subscription) | No | codify session metadata only; LLM calls invisible | No — billed to the individual's consumer account |
...
If centralized governance is a hard requirement, the codify host
...
| Capability | Raw provider call from codify | Through AI Gateway |
|---|---|---|
...
codify emits OpenTelemetry spans for every agent turn, LLM call, and
...
in [PR #1050](https://github.com/codify-ai/codify/pull/1050). Any
...
Three env vars on the codify server:
...
codify's `telemetry.init()` auto-detects the OTLP endpoint and wires
...
`mlflow.start_span` API codify's `TracingContext` wraps) against the
...
Experiment:   id=3163592711242134 path=/Users/.../codify-databricks-docs-verification
...
codify does not capture message bodies into traces by default. Set:

```bash
export CODIFY_OTEL_CAPTURE_CONTENT=true
```
...
| `CODIFY_OTEL_CAPTURE_CONTENT` | `true` to include message bodies in traces | Apps env (default off) |

---

## Roadmap

The four sections above cover the V1 integration. The codify +
Databricks story extends further. Planned follow-ups, each as a
separate PR:

- **Unity Catalog functions as agent tools.** A `databricks-tools`
  optional extra in codify that exposes UC functions as
...
- **Mosaic AI Agent Evaluation integration.** Sample production
  traces from codify for the managed Mosaic AI Agent Evaluation
  pipeline.
...
Track these on the [codify issues
list](https://github.com/codify-ai/codify/issues) under the
`databricks` label.
...
fresh Lakebase project per codify app rather than sharing one.
...
observability series shipped in PRs [#1050](https://github.com/codify-ai/codify/pull/1050),
[#1068](https://github.com/codify-ai/codify/pull/1068),
[#1070](https://github.com/codify-ai/codify/pull/1070),
[#1071](https://github.com/codify-ai/codify/pull/1071),
[#1072](https://github.com/codify-ai/codify/pull/1072), and
[#1083](https://github.com/codify-ai/codify/pull/1083).
...
- External Model (Gateway) endpoint created, called, and torn down:
  `codify-docs-test-gateway` proxied to `databricks-claude-sonnet-4`
...
ping @debu-sinha on the codify Slack channel.
