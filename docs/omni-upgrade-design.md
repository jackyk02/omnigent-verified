...
How Codify keeps a user's CLI — and the local processes it spawns — current
...
- The CLI ships as `codify` / `omni` → `codify.cli:main()`; the installed
  version comes from `importlib.metadata.version("codify")`.
- Releases publish three lockstep packages (`codify`, `codify-client`,
  `codify-ui-sdk`) to PyPI via `.github/workflows/release-codify.yml`.
  **No GitHub Releases are cut**, so the source of truth for "latest version"
  is the PyPI JSON API: `https://pypi.org/pypi/codify/json` → `info.version`.
- The local server is a detached process on `:6767`, tracked by
  `~/.codify/local_server.pid` plus a config-signature sidecar
  (`local_server.sig`). `ensure_local_codify_server()` reuses a healthy
  server **iff its recorded signature matches**; otherwise it stops it and
  respawns. All durable state lives in sqlite (`~/.codify/chat.db`), not in
...
- PR #172 removed an earlier startup check that nagged on *install age* (it
  ...
  `git fetch` on the hot path. The module (`codify/update_check.py`) stayed in
...
`server_config_signature()` (`codify/host/local_server.py`) now folds the
...
`codify/cli.py`, command `upgrade`. Flow:
...
5. Run the installer-appropriate command (`_build_upgrade_suggestion` +
   `_run_upgrade_command`): `uv tool upgrade codify`, `pip install -U
   codify`, `pipx upgrade codify`, `--reinstall <vcs_url>`, etc.
...
`codify/update_check.py`, installed-wheel path; wired into `main()` behind
...
- **Configured-index aware**: `fetch_latest_version` queries the resolved
  ...
  `CODIFY_INDEX_URL` / `UV_INDEX_URL` / `PIP_INDEX_URL`, then the uv/pip
...
- **Quiet + opt-out**: TTY-only, skipped for `--help` / `version` / internal TUI
  commands / `upgrade` itself, and silenced by `CODIFY_NO_UPDATE_CHECK`.
...
- **Source of truth**: the configured package index's Simple Repository API
  (default `pypi.org/simple`; honors `CODIFY_INDEX_URL` / `UV_INDEX_URL` /
...
- Project-local `uv.toml` / `pyproject [tool.uv]` index URLs (only the user/
  ...
  resolves a global tool; use `CODIFY_INDEX_URL` for anything else).
- A `/api/version` drift warning when attaching to a server you didn't spawn.
- A `config` toggle mirroring `CODIFY_NO_UPDATE_CHECK`.
