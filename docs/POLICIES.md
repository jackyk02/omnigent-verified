...
policies:
  session_budget:
    type: function
    handler: codify.policies.builtins.cost.cost_budget
    factory_params:
      max_cost_usd: 10.00
...
```bash
codify server --config server_config.yaml
```
...
policies:
  ...
  github_access:
    type: function
    handler: codify.policies.builtins.github.github_policy
    factory_params:
      write_repos:
        - myorg/my-repo
      write_branches:
  ...
  google_policy:
    type: function
    handler: codify.policies.builtins.google.gdrive_policy
    factory_params:
      read_all: true
      allow_create: true
```
...
Codify session marker) plus the harness's own variable family, and nothing
...
- **A generic ACP agent** (`type: acp`) gets no vendor family at all, because
  Codify cannot know which vendor an arbitrary agent belongs to. Gemini via
  ACP needs `env_passthrough: ["GEMINI_API_KEY"]`.
- **Goose** owns `GOOSE_*` only. Goose configured through Codify's
...
daily_budget:
  type: function
  handler: codify.policies.builtins.cost.user_daily_cost_budget
  factory_params:
    max_cost_usd: 25.00
...
policies:
  deny_trivial_opus:
    type: function
    handler: codify.policies.builtins.routing.deny_trivial_to_expensive_model
    factory_params:
      expensive_models:
...
```python
from codify.policies.schema import PolicyEvent, PolicyResponse
