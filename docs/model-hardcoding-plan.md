...
Claude and Codex records in `codify/model_fallbacks.py`; each carries an owner,
...
- Unavoidable static aliases pass only when Python AST analysis proves they are
  ...
  `codify/model_fallbacks.py`, including non-empty owner, provenance, and
...
The first migration building block lives in `codify/model_metadata.py` and
`codify/model_resolver.py`:
...
Codify source update and snapshots do not block on the CLI process.
...
- `CODIFY_CI_ANTHROPIC_MODEL`
- `CODIFY_CI_FAST_ANTHROPIC_MODEL`
- `CODIFY_CI_OPENAI_MODEL`
- `CODIFY_CI_E2E_JUDGE_MODEL`
- `CODIFY_CI_E2E_MODEL_POOL_GPT`
- `CODIFY_CI_IMAGE_MODEL`
...
materialization. `--model` and `CODIFY_MODEL` remain higher-precedence explicit
...
entries are ignored. `CODIFY_DISABLE_CATALOG_LOOKUP=1` bypasses in-memory,
...
`codify/model_fallbacks.py`. Each fallback records its adapter owner, catalog
...
`--model` or session override, Codify omits `HARNESS_KIMI_MODEL` and lets the
