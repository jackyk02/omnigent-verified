...
that embeds the **live interactive `qwen` TUI** in the Codify web UI, instead of
...
This lets Codify inject turns atomically *and* recover two things the other
...
```mermaid
flowchart TD
    subgraph CLI["codify qwen (CLI wrapper)"]
      W[qwen_native.py<br/>daemon bind · terminal-ready poll · tmux attach]
    end

    subgraph Runner["Codify Runner"]
...
```
   ...
   └──────────────────────────────────────────────────────────────────┘
            ...
            │
   ┌──────────────────────────────────────────────────────────────────┐
   │   codify qwen  (CLI wrapper, qwen_native.py)                     │
...
- **Control** — instead of racy `tmux send-keys`, Codify writes JSONL to the **IN
...
| **Tool permission gating** | **surrendered to vendor** | **Codify gates** via `can_use_tool` → policy → `confirmation_response` |
...
- **Permission gating works** — the headline win. `can_use_tool` control requests
  let Codify run its TOOL_CALL policy + human elicitation and answer with
...
| Still needs tmux for display | **Inherent** | Any *embedded live terminal* needs a real terminal — unavoidable for the native-TUI goal. Upside: because we already parse the full JSON event stream, a future **pure-web rendering mode** (Codify renders qwen with no tmux) becomes possible as a separate option — but that is explicitly out of scope here. |

## Files to add (mirrors goose-native; bridge is file-based, not send-keys)

| New file | Role |
|---|---|
| `codify/inner/qwen_native_executor.py` | `submit` via input-file; `supports_streaming=False`, `supports_live_message_queue=True` |
| `codify/inner/qwen_native_harness.py` | `create_app()` factory |
| `codify/qwen_native_bridge.py` | input-file append (`submit` / `confirmation_response`), tmux.json, `inject_interrupt` (Escape), `kill_session`, `build_qwen_native_spawn_env` |
| `codify/qwen_native_forwarder.py` | tail `--json-file`, mirror transcript, drive the permission gate |
| `codify/qwen_native.py` | `codify qwen` wrapper (clone `goose_native.py`) |

## Registration touch-points (one-liners, beside the goose entries)

- `codify/runtime/harnesses/__init__.py` → `"qwen-native": "codify.inner.qwen_native_harness"`
- `codify/harness_aliases.py` → add `qwen-native` + `native-qwen` to `NATIVE_HARNESSES`
- `codify/native_coding_agents.py` + `codify/_wrapper_labels.py` → `QWEN_NATIVE_*`
- `codify/onboarding/harness_install.py` → `_HARNESS_NAME_TO_KEY` → existing `QWEN_KEY`
- `codify/runner/app.py` → `_auto_create_qwen_terminal` + the ~7 goose-native dispatch sites
- `codify/cli.py` → `codify qwen` command
...
- **Persisted id (the convention).** The qwen session id is recorded on the
  Codify session via `external_session_id` (`_persist_qwen_external_session_id`
  → `PATCH /v1/sessions/{id}`), read back from the snapshot on the next launch
  (`launch_config.external_session_id`), and stamped as
  `codify.fork.source_external_session_id` so a fork carries history — exactly
