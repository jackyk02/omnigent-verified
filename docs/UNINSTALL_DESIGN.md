...
This document specifies how Codify should be uninstalled. It is written to be
...
2. `codify uninstall` - the discoverable CLI entry. It performs graceful
...
uninstall and only point the user back at `codify uninstall --purge` for
`~/.codify`. Shared runtimes (uv/Node/tmux/bwrap) are report-only in this
...
- Path: `~/.codify/install_ledger.json`
...
| `installation_id` | string \| null | Copied from `~/.codify/installation_id`; the anchor proving an install exists. |
...
`path`, `marker_begin` (`# >>> Codify installer >>>`), `marker_end`
(`# <<< Codify installer <<<`), `line_range` [int,int] (1-indexed inclusive,
...
`injected_external_config` (array) - entries Codify wrote into third-party
files: `path`, `marker` (logical key, e.g. `mcp_servers.codify`), `format`
...
`path` (\| null), `version` (\| null), `installed_by` (`codify` - only ever set
...
```json
{
  ...
  "generator": { "name": "codify", "version": "1.42.0", "strategy": "install", "os": "macos", "wrote_at": "2026-07-14T18:03:22Z" },
  "entries": {
    "profiles": [
      { "path": "~/.zshrc", "marker_begin": "# >>> Codify installer >>>", "marker_end": "# <<< Codify installer <<<",
    ...
    "injected_external_config": [
      { "path": "~/.config/harness/hermes.json", "marker": "mcp_servers.codify", "format": "json",
        "allowlist": ["mcp_servers.codify"], "block_sha256": null, "source": "recorded", "confidence": "certain" }
    ],
    "deps": {
      "uv":   { "present": true, "path": "~/.local/bin/uv", "version": "0.5.11", "installed_by": "codify",    "confidence": "high" },
      "node": { "present": true, "path": "/usr/bin/node",   "version": "22.3.0", "installed_by": "preexisting", "confidence": "high" }
    },
    "wheel": { "installed": true, "uv_tool_dir": "~/.local/share/uv/tools/codify", "bin_dir": "~/.local/bin",
               "console_scripts": ["codify","omni"], "source": "recorded", "confidence": "certain" },
    "launch_agents": [
      { "kind": "launchd", "path": "~/Library/LaunchAgents/dev.codify.daemon.plist", "label": "dev.codify.daemon",
        "source": "recorded", "confidence": "certain" }
    ],
    "state_paths": { "codify_home": "~/.codify", "workspace": "~/codify", "desktop_data": [] }
  }
}
```
...
- [x] `codify _internal write-ledger --from-env` hidden subcommand
...
`codify _internal write-ledger --from-env` (reuses the schema serializer, gets
...
`installed_by: codify` + version, deps found already present get
...
3. Never downgrade `installed_by` (`uv: codify` stays even if uv is now found
   pre-present).
4. Atomic write.
...
one genuine install signal: `~/.codify/installation_id` exists, OR the wheel
is installed (`uv tool list` shows `codify`), OR a known profile contains the
exact marker pair. If none, write nothing and report "no Codify install
detected."
...
- Fast (startup, target <100ms, no package-manager subprocesses): stat the
  ...
  `grep`), stat known `~/.codify` subdirs, existence checks for Electron
...
- Deep (uninstall / doctor, no budget): fast steps plus `uv tool list`/
  `uv tool dir`, `command -v codify omni uv node tmux bwrap`, version
...
| `~/.codify`, `installation_id` | observed | high |
...
- Backfill writes to `~/.codify/install_ledger.backfill.json`, not directly
...
`codify doctor --migrate-ledger [--deep]` which prints a JSON diff and writes
only with `--apply`.
...
- [x] `codify doctor --migrate-ledger [--deep] [--apply]`
- [x] Read-only-except-the-ledger guarantee (tested)

## 5. codify uninstall CLI

`codify uninstall [targets...] [flags...]` (execs `scripts/uninstall_oss.sh`
...
- `state` - remove user data under `~/.codify` and `~/codify` (backup by
  default).
...
- `--purge-workspace` - the only way to clear `~/codify` (your working files)
  non-interactively. Without it, `--purge --yes` still removes `~/.codify`
  (credentials/history) but leaves `~/codify` untouched and prints a notice.
...
|---|---|---|---|
| Wheel (`uv tool uninstall codify`) | dry-run preview | auto-remove | none |
...
| `~/.codify` state root | reported, skipped | removed only with `--purge` | `--purge` |
| `~/codify` workspace | reported, skipped | kept unless `--purge-workspace` | `--purge` AND (`--purge-workspace` or interactive confirm) |
...
- [x] Python `codify uninstall` subcommand that execs the shell script
...
`codify uninstall` performs graceful shutdown + state/JSON in Python, then
execs the shell script for removal. Sequence:

1. Resolve ledger (section 4 resolution order).
2. Stop processes first. Read pidfiles under `~/.codify/run/` (+ `daemons/`,
   `runners/`, `local_server/`): SIGTERM -> wait 5s -> under `--force` SIGKILL.
   Kill only `codify:*` tmux sessions. Unload ledger-recorded LaunchAgents/
...
6. Optional state / desktop-data (only with `--purge` / target). For `--purge`:
   archive to a backup tarball OUTSIDE the target under `~/.codify-backups/`
   ...
   restore command, then delete. Never back up into `~/.codify`. Clearing
   `~/codify` non-interactively requires `--purge-workspace` (see section 5);
...
7. `uv tool uninstall codify` - LAST (so earlier Python-driven steps still
   have the wheel available).

Checklist:

- [x] Process-shutdown protocol (pidfiles, SIGTERM->5s->`--force` SIGKILL,
      `codify:*` tmux, ledger LaunchAgents, abort-if-won't-stop)
...
- [x] `--purge` archives OUTSIDE the target (`.tar.zst`, gzip fallback; fail
      ...
      deletes; `~/codify` gated behind `--purge-workspace` (or confirm)
- [x] `uv tool uninstall codify` runs last
...
```json
{
  ...
  "actions": [
    { "artifact": "profile_block", "path": "~/.zshrc", "planned": "remove",
      "status": "done", "gate": null, "detail": "block removed, backup at ~/.zshrc.codify.bak" },
    { "artifact": "external_config", "path": "~/.config/harness/hermes.json", "marker": "mcp_servers.codify",
  ...
  "backups": ["~/.codify-backups/2026-07-14T18-40-02Z.tar.zst"],
...
| # | Scenario | Expect |
|---|---|---|
| 1 | fish profiles (`config.fish` + `conf.d/codify.fish`) | block removed from both; other lines intact |
...
| 8 | `--purge` with backup | archive written OUTSIDE `~/.codify`; restore command printed; then delete |
| 9 | `--purge --no-backup` | delete without archive; `~/codify` kept unless `--purge-workspace` |
...
| 16 | `--purge --yes` without `--purge-workspace` | `~/.codify` removed; `~/codify` kept + notice |
...
- [x] PR 1 - Ledger schema + serializer. Schema, atomic-write + `0600` writer,
      `codify _internal write-ledger` hidden subcommand, round-trip unit
...
- [x] PR 4 - `uninstall_oss.sh` core. Process shutdown, profile block removal
      ...
      `--dry-run`/`--json`.
- [x] PR 5 - `codify uninstall` subcommand + gates. Python front, targets/
...
Codify is a houseguest.

- Installing = the guest moves in: hangs a coat by the door (the PATH line in
  your shell profile), keeps a box of their stuff in a closet (`~/.codify` -
  settings, logins, chat history) and a desk they work at (`~/codify`).
...
```
        ...
        |   - data destruction (~/.codify, ~/codify)     |
        ...
        +----------------------+---------------------------+
           ...
           |    OUTSIDE target (~/.codify-backups/),  |
           |    then delete state; keep ~/codify       |
           |    unless --purge-workspace or confirm      |
           | 6. uv tool uninstall codify  (LAST)       |
           +--------------------+----------------------+
                                |
                                v
             +--------------------------------------+
   ...
   OS/package-manager uninstall owns package files. The Codify
