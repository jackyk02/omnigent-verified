# Codify bot identities & attribution setup

This doc explains how commits and automated PR reviews in the
`codify-ai/codify` repo are attributed, and how the supporting GitHub App
was set up. There are **two deliberately distinct identities** — do not
conflate them:

| Identity | Used for | Why this identity |
| --- | --- | --- |
| `codify <noreply@codify.ai>` | Co-author trailer on commits authored by **polly's coding sub-agents** | These commits are produced by `git commit` in a worker's local worktree — they are **not** GitHub Actions runs, so a plain org co-author is the honest attribution. No GitHub App user is involved. |
| `codify-ci[bot]` (GitHub App) | **CI automation**: lockfile-regen commits/PRs **and** automated PR-review comments | These actions genuinely run inside GitHub Actions, where the App's private key lives and a short-lived installation token is minted per run. The App is an org-owned, least-privilege identity. |

> **Why two identities and not one?** An earlier draft of this work tried to use
> `codify-ci[bot]` everywhere, including the sub-agent commit trailer. That was
...
## The GitHub App: `codify-ci[bot]`

> **Naming note.** The App was registered as **`codify-ci`** (the bare
> `codify` name was unavailable), so GitHub renders the actor as
> **`codify-ci[bot]`**.

| Field | Value |
| --- | --- |
| App name | `codify-ci` |
| Bot actor | `codify-ci[bot]` |
| App ID | `4082516` |
| Bot numeric user ID | `294685417` |
| CI git author email | `294685417+codify-ci[bot]@users.noreply.github.com` |
...
Created at `https://github.com/organizations/codify-ai/settings/apps/new`:

- **GitHub App name:** `codify-ci` → actor `codify-ci[bot]`.
...
- **Where can this App be installed?** Only on `codify-ai`.
- Installed into `codify-ai`, scoped to the `codify` repo.
...
```bash
gh api users/codify-ci%5Bbot%5D --jq '.id'
# -> 294685417
```
...
In **`codify-ai/codify` → Settings → Secrets and variables → Actions**:

- **Variable** `CODIFY_BOT_APP_ID` = `4082516`
- **Secret** `CODIFY_BOT_APP_KEY` = the App's `.pem` private key

> The workflows gate on `vars.CODIFY_BOT_APP_ID != ''`. If the variable is
...
> **`codify-ci[bot]` replaced the old OSS regen bot.** The previous
...
```
Co-authored-by: codify <noreply@codify.ai>
```
...
- name: Mint App token
  id: app-token
  if: vars.CODIFY_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.CODIFY_BOT_APP_ID }}
    private-key: ${{ secrets.CODIFY_BOT_APP_KEY }}
```

```bash
git config user.name  "codify-ci[bot]"
git config user.email "294685417+codify-ci[bot]@users.noreply.github.com"
```
...
### Automated PR review posted as `codify-ci[bot]`
...
token and posts the review **as `codify-ci[bot]`**:

```yaml
- name: Mint App token
  id: app-token
  if: steps.trigger.outputs.skip != 'true' && steps.creds.outputs.available == 'true' && vars.CODIFY_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.CODIFY_BOT_APP_ID }}
    private-key: ${{ secrets.CODIFY_BOT_APP_KEY }}
...
| --- | --- | --- |
| polly sub-agent commits | `codify <noreply@codify.ai>` (co-author trailer) | `examples/polly/agents/*/config.yaml` |
| Lockfile-regen commits/PRs | `codify-ci[bot]` | `oss-regenerate-and-smoke.yml`, `oss-regen-on-comment.yml` |
| Automated PR review comments | `codify-ci[bot]` (fallback `github-actions[bot]`) | `polly-review.yml` |

**Config:** variable `CODIFY_BOT_APP_ID` = `4082516`, secret
`CODIFY_BOT_APP_KEY` = App private key. The old `OSS_REGEN_APP_*` config and
App are retired.
