...
Codify can run an agent from a single YAML file:

```bash
codify run path/to/agent.yaml
```
...
Codify may append framework-owned lifecycle or metadata instructions at
...
`codify kiro`. It requires `kiro-cli` on `PATH` and Kiro's own login/auth; it
does not use Databricks, OpenAI, or Anthropic provider credentials. Plain
`harness: kiro` is not a generic Codify harness id. Kiro's TUI remains the
...
(`pip install "codify[antigravity]"`). It defaults to **Gemini 3.5 Flash**
...
(`pip install "codify[copilot]"`). The SDK bundles the Copilot CLI it drives
...
CLI. Resolution: spec `auth.api_key` → a token registered via `codify setup`
...
`~/.codify/config.yaml` and reference it (`auth: {type: provider, name: …}`),
...
By default Kimi authenticates against Moonshot AI's backend — Codify
...
shell, declare a key/gateway provider in `~/.codify/config.yaml`, or use
`executor.auth: {type: databricks, profile: …}` and let Codify resolve
the workspace.
...
`executor.auth` block or your `codify setup` provider config — there is
no profile flag.
...
You usually don't need to choose a `sandbox.type` — omit it and Codify picks
...
egress rules, see the `sandbox:` examples below and the sandbox source under `codify/inner/`.
...
The runtime can also be set globally via the `CODIFY_CONTAINER_RUNTIME`
...
tools:
  sandbox:
    ...
    container_runtime: podman  # optional; defaults to docker (or CODIFY_CONTAINER_RUNTIME)
```
...
- Run the YAML before publishing it:
  ...
  ```bash
  codify run path/to/agent.yaml -p "Say hello"
  ```
