...
- `codify run --harness qwen` / `executor.harness: qwen` (alias `qwen-code`).
...
- `codify setup` → **Qwen Code** row: installs the CLI and guides auth
...
- **File I/O delegation (`fs/*`).** When an `os_env` is configured, the executor
  ...
  handlers execute the I/O through the Codify `OSEnvironment`, so the spec's
  sandbox read/write roots are enforced at the Python layer — and the bytes flow
  through Codify rather than qwen touching disk directly. Disabled (qwen uses
...
- [x] **Native TUI variant (`qwen-native` / `native-qwen`).** Implemented — the
  ...
  by `codify qwen`. Unlike the goose/cursor `tmux send-keys` native harnesses,
...
- [x] **Tool-approval elicitation card (TUI → web).** Implemented — qwen's
  ...
  wrong).
  - **Mirror:** `codify/qwen_native_permissions.py` —
...
- [ ] **Composer status line: real model + context ring (Web UI).** For
  ...
  env / qwen's own `/model`), so Codify's `llmModel` was a misleading default.
  ...
  metadata. The forwarder (`codify/qwen_native_forwarder.py`) could parse it
  ...
  - **Context ring + cost tracking also missing**, same root cause: native-qwen
    ...
    `codify/qwen_native_forwarder.py`, split `cache_read_input_tokens` out of
...
- [x] **Restore qwen's TUI history on resume.** `omni qwen --resume <conv_id>`
  ...
  `_auto_create_qwen_terminal` persists the qwen session id on the Codify
  ...
  launch, and it's stamped as `codify.fork.source_external_session_id` for fork
...
- [x] **Carry history into qwen on fork / switch-agent (incl. cross-harness).**
  ...
  `codify.fork.carry_history` and clear `external_session_id` on the clone. On
  the clone's first launch, `_auto_create_qwen_terminal` calls
  `_build_qwen_fork_recording`, which fetches the clone's copied Codify items
  ...
  Codify items (not the source's vendor transcript), it works **cross-harness**
...
- [x] **Codify tools.** Qwen-native now exposes the shared Codify MCP relay
  (`codify.claude_native_bridge serve-mcp`, `mcpServers.codify`,
  ...
  model can call Codify's builtin tools (`sys_*`, `load_skill`, `web_fetch`, …).
...
- [ ] **File I/O recording / content policy.** Codify now *executes* delegated
  file reads/writes through the `OSEnvironment` (see "File I/O delegation" in
  What works today), so the bytes flow through Codify and the sandbox roots are
  enforced. Still missing on top of that: (a) emitting the I/O into Codify's
