...
**Architecture:** agy keeps running in a runner-owned tmux terminal (terminal-first UX). A new RPC client (extending `antigravity_native_rpc.py`) drives: read via `GetCascadeTrajectorySteps`/`StreamAgentStateUpdates`, interactions via `HandleCascadeUserInteraction` (surfaced as codify elicitations), and interrupt via `CancelCascadeSteps`. A pure step→item mapper replaces `step_to_events`. Turn-send stays on tmux send-keys (pending a confirmed user-turn RPC). The transcript forwarder + durable cursor are retired.

**Tech Stack:** Python 3.13+ (uv), httpx (connect-RPC over HTTP/2, JSON), pytest, existing codify server elicitation registry + SSE, agy 1.0.10.
...
- `codify/antigravity_native_rpc.py` (modify) — add interaction/step/cancel RPC methods alongside the existing discovery helpers.
- `codify/antigravity_native_steps.py` (create) — pure step→item mapper (replaces the mapping half of `step_to_events`) + the `WAITING`-interaction extractor.
- `codify/antigravity_native_reader.py` (create) — read driver: poll/stream trajectory steps → post items; owns dedup + the interaction-bridge loop.
- `codify/antigravity_native_interactions.py` (create) — interaction bridge: detect→elicitation→deliver, with the timeout/re-read loop.
- `codify/server/routes/_antigravity_elicitation.py` (create) — parse an agy `WAITING` interaction → `ElicitationRequestParams`, and map an `ElicitationResult` back to an `interaction` payload (mirrors `_codex_elicitation.py`).
- `codify/inner/antigravity_native_executor.py` (modify) — `interrupt_session` → `CancelCascadeSteps`; (turns unchanged pending Task 1).
- `codify/runner/app.py` (modify) — auto-create the RPC reader instead of (or alongside, during cutover) the transcript forwarder.
- `codify/antigravity_native_forwarder.py` (delete at cutover, Task 12).
...
- [ ] **Step 1:** Stand up a live agy on the running codify (`:6767`, HOST_ID `host_1b3116a52bf74d668d3179652c54cdc7`, `HOME=/Users/bryanli`) per memory `agy-rpc-interaction-bridge.md`; discover the RPC port via `discover_language_server_port`.
...
**Files:**
- Modify: `codify/antigravity_native_rpc.py`
...
**Files:** Modify `codify/antigravity_native_rpc.py`; Test `tests/test_antigravity_native_rpc.py`
...
**Files:** Create `codify/antigravity_native_steps.py`; Test `tests/test_antigravity_native_steps.py`
...
**Files:** Modify `codify/antigravity_native_steps.py`; Test `tests/test_antigravity_native_steps.py`
...
**Files:** Create `codify/antigravity_native_reader.py`; Test `tests/test_antigravity_native_reader.py`
...
**Files:** Create `codify/server/routes/_antigravity_elicitation.py`; Test `tests/server/test_antigravity_elicitation.py`
...
**Files:** Create `codify/antigravity_native_interactions.py`; Test `tests/test_antigravity_native_interactions.py`
...
**Files:** Modify `codify/server/routes/sessions.py`; Test `tests/server/test_app.py`
...
**Files:** Modify `codify/inner/antigravity_native_executor.py`; Test `tests/inner/test_antigravity_native_executor.py`
...
**Files:** Modify `codify/runner/app.py`; Test `tests/runtime/.../test_*`
...
**Files:** Delete `codify/antigravity_native_forwarder.py`; Modify `codify/antigravity_native_bridge.py` (drop `forwarded_steps`/cursor), `codify/inner/antigravity_native_executor.py` (drop send-keys-only-for-interactions notes), references; relocate `OutboundEvent`/`_ToolCallIdAllocator` into `antigravity_native_steps.py`; delete `tests/test_antigravity_native_forwarder.py`.
