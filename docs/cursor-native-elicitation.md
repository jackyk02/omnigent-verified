...
**Code:** `codify/cursor_native_permissions.py`, the `cursor-permission-request` hook in
`codify/server/routes/sessions.py`, runner wiring in
`codify/runner/native/orchestration.py`, `web/.../ApprovalCard.tsx`.

## Goal / behavior

Surface an Codify **elicitation card whenever the `cursor-agent` TUI gates a tool call or
asks a question**, answerable from the web **or** the embedded TUI. Cursor's own native gate
stays the source of truth — Codify never modifies cursor's JS bundle and never suppresses
...
- Because a gate answered this way is never seen by a human, the accept logs the tool name and
  an argument preview at INFO: that line is the only record Codify approved the call.
