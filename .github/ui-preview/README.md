...
Deploy a live, per-PR preview of the Codify web UI as a
...
2. The [UI Preview workflow](../workflows/ui-preview.yml) builds the SPA + the
   Codify wheels and deploys them to an ephemeral Databricks App
   (`codify-ui-preview-pr-<N>`).
...
Unlike Codify's production Databricks deploy (`deploy/databricks/`, backed by
...
There is **no LLM or runner baked into the preview** -- Codify runs agent
turns on a runner the user connects from their own machine or sandbox
(`codify run … --server <preview-url>`), where the model credentials live. So
