...
const FIXTURE = {
  areas: [
    { key: "inner", label: "comp:harnesses", paths: ["codify/inner/"], owners: ["ownerA", "ownerB", "ownerC"] },
...
async function runOrch(canned, opts) {
  ...
  const context = { repo: { owner: "codify-ai", repo: "codify" } };
...
(async () => {
  ...
  assert("parseAreas: rules preserve prefixes", rules.some((r) => r.prefix === "codify/inner/") && rules.some((r) => r.prefix === "web/"), JSON.stringify(rules));
  ...
  const srMembers = script.pickSecondReviewer({
    files: ["codify/inner/foo.py"], rules, pool, load: new Map(),
  ...
  const srLoad = script.pickSecondReviewer({
    files: ["codify/inner/foo.py"], rules, pool,
  ...
  let s = await runOrch({
    ...
    files: [{ filename: "codify/inner/foo.py" }],
  });
  ...
  s = await runOrch({
    ...
    files: [{ filename: "codify/inner/foo.py" }],
    comments: [{ user: { login: "codify-ci" }, body: script.MARKER + "\nearlier nudge", created_at: daysAgoIso(2) }],
  });
