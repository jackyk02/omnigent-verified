...
async function run({
        ...
        pullRequest: {
          closingIssuesReferences: {
            nodes: linkedIssues.map((li) => ({
              number: li.number,
              repository: { nameWithOwner: li.repo || "codify-ai/codify" },
  ...
  const context = {
    repo: { owner: "codify-ai", repo: "codify" },
    payload: { pull_request: {
      number: PR_NUMBER, draft: false,
      user: { login: author },
      // precise fork detection compares head vs base full_name
      head: { repo: { full_name: fork ? "external-contributor/codify" : "codify-ai/codify" } },
      base: { repo: { full_name: "codify-ai/codify" } },
...
(async () => {
  ...
  r = await run({ files: ["codify/db/x.py"], load: { SabhyaC26: 1 } });
  ...
  r = await run({
    files: ["codify/inner/a.py", "codify/tools/b.py"],
  ...
  r = await run({ files: ["codify/inner/foo.py"], fork: false });
  ...
  r = await run({ files: ["codify/inner/foo.py"], author: "dhruv0811" });
