...
An example Codify agent that answers questions over **governed AWS data** through
the official [AWS Labs MCP servers](https://github.com/awslabs/mcp) — no custom
connector code required. It shows how any AWS Labs MCP server plugs into Codify as
a `type: mcp` tool.
...
```bash
AWS_PROFILE=my-profile AWS_REGION=us-east-1 codify run examples/aws_analyst
```
