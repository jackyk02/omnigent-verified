...
codify ships **three PyPI packages that version-lock together**:

| Package | What it is |
| --- | --- |
| `codify` | core wheel (bundles the `web` web UI) |
| `codify-client` | Python client SDK |
| `codify-ui-sdk` | terminal UI SDK |

`pip install codify==X` must resolve `codify-client==X` and
`codify-ui-sdk==X`. The pins are **lockstep** (the three packages co-version and
...
- **Source of truth** (versions, tags, GitHub Releases): **`codify-ai/codify`**
...
- **Publishing to PyPI**: the central **secure-release repo**
  **`databricks/secure-public-registry-releases-eng`**, `codify` workflow —
  ...
  dependency scan**. This is why we don't publish from `codify-ai/codify`,
...
The legacy `.github/workflows/release-codify.yml` in this repo is a
...
- Every release ships as an **rc first** (`0.6.0rc1` → … → `0.6.0`). rcs go to
  **real PyPI** as PEP 440 pre-releases — a default `pip install codify`
...
branch** on `codify-site` instead of `main`:
...
Both derive the branch name from `codify/version.py` (`0.6.0.dev0` → `0.6-docs`)
...
```bash
gh workflow run release.yml --repo codify-ai/codify \
...
- commits `release: v0.6.0rc1`, tags, and pushes branch + tag with the
  codify-ci App token, which fires the downstream automation:
...
gh workflow run codify.yml --repo databricks/secure-public-registry-releases-eng \
...
python -m venv /tmp/omni-rc && /tmp/omni-rc/bin/pip install \
  ...
  codify==0.6.0rc1 codify-client==0.6.0rc1 codify-ui-sdk==0.6.0rc1
/tmp/omni-rc/bin/codify --version    # expect 0.6.0rc1
```
...
4. **Finalize — dispatch `Finalize release` (`finalize-release.yml`)**:
   ...
   ```bash
   gh workflow run finalize-release.yml --repo codify-ai/codify -f tag=v0.6.0
   ```

   It verifies PyPI serves all three packages, the CHANGELOG PR isn't open,
   and the **docs sweep**: no open PRs against `0.6-docs` on `codify-site`
...
- **`publish-release` environment** on `codify-ai/codify` with required
  reviewers = the release managers. Without it the finalize publish job runs
  ungated.
- **codify-ci App** installed on `codify-ai/homebrew-tap` (it already
  covers `codify` and `codify-site`).
- **Tag ruleset** (recommended): restrict `v[0-9]*` create/update/delete to
  the codify-ci App + admins, so no write-access account can start the
...
```bash
curl -fsS https://pypi.org/pypi/codify/0.0.1rc2/json   # expect 404
```
...
1. **Plan (read-only)** — dry run is the default:
   ...
   ```bash
   gh workflow run release.yml --repo codify-ai/codify -f version=0.0.1rc2
   ```
...
```bash
gh api -X DELETE 'repos/codify-ai/codify/git/refs/heads/release/v0.0.0'
```
