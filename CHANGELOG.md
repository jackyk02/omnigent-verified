...
- [UI / Feature] Voice dictation in the composer now works in Electron, Firefox, and Chromium via optional server-side transcription (`codify[dictation]`) — local models, live streaming partials, audio never leaves your server. (#2093)
...
- [Feature] Set `CODIFY_CONTAINER_RUNTIME=podman` to use Podman (or another supported runtime) globally instead of Docker, without editing every agent's YAML. (#2949)
...
- [Feature / Test/CI] `codify` benchmark harness gains `--network-delay-ms` and per-journey HTTP request counts (#2977)
...
- [Feature] Import local Qwen, Kiro, Pi, and Kimi coding chats into Codify (#3032)
...
- [Feature] Import existing OpenCode chats, including files and tool activity, with `codify import` (#3046)
...
- [Bug fix / Feature] The Slack bot can now run against an Codify server deployed on Databricks Apps, (#3051)
...
- [Bug fix] Databricks OAuth CLI profiles no longer fail with a misleading "malformed profile" error; the message now explains the real fix (install `codify[databricks]` or refresh the OAuth session). (#3059)
...
- [Bug fix] `codify server --host 0.0.0.0` now enables accounts (login) mode automatically instead of silently 401-ing every request (#3107)
...
- [Feature] `omnidev codify <args…>` runs an codify command against the current checkout's pod via `uv run codify`, with the pod's isolated env applied (#3110)
...
- [Feature] `codify session import` loads a `session export` JSONL back into a server as a new session (#3141)
...
- [Feature] `codify claude` keeps tool search enabled when launched with `CLAUDE_CODE_USE_GATEWAY=1`. (#3161)
- [Test/CI] Fix `test_session_stream_emits_heartbeat_on_idle` after `_session_labels_for_runner_spawn` was extracted into `codify.runner.native.orchestration`; patch the heartbeat cadence on `codify.runner.app` where it is defined and consumed. (#3163)
...
- [Chore / Breaking] Slack-on-Databricks deploy: renamed `CODIFY_SLACK_WEBAUTH_BASE_URL` to `CODIFY_SLACK_DATABRICKS_APP_URL` (`--app-url`), removed the `WEBAUTH_PORT` / `DATABRICKS_WORKSPACE_HOST` overrides, and dropped deploy-time `uv lock` in favor of in-container `uv run`. (#3206)
...
- [Feature] Smart routing now activates automatically when a server `llm:` block or an external `routing:` block is configured — no `CODIFY_SMART_ROUTING` env var needed (#3215)
...
- [Feature] Add a top-level `justfile` with recipes for launching the iOS Simulator, running the Android debug build, starting the codify dev pod, and running pre-commit/lockfile normalization. (#3310)
...
- [Feature] `codify server` now tells users they can set `CODIFY_AUTH_ENABLED=0` to override automatic multi-user mode when binding to a non-loopback interface. (#3320)
- [Chore / Breaking] [Breaking] The deprecated `CODIFY_ACCOUNTS_ENABLED` environment variable has been removed; use `CODIFY_AUTH_ENABLED` instead. (#3322)
...
- [Bug fix] `sys_os_shell` commands no longer inherit codify's own `PYTHONPATH` entry, so project subprocesses resolve their own installed packages instead of codify's. (#1861)
...
- [Bug fix] Fixed the headless Hermes harness registering no Codify MCP server (agent had no builtin tools). (#2216)
...
- [Bug fix] `codify server` now accepts documented boolean server-config values like `sandbox.kubernetes.in_cluster: false` instead of rejecting them at startup. (#2314)
...
- [Bug fix / Feature] `codify run --harness <name>-native` now launches every registered native harness with matching prompt and model behavior. (#2379)
- [Feature / Docs / Test/CI] Harness bench now reports Codify MCP relay support separately from vendor-native tool calling. (#2380)
...
- [Bug fix] Pi no longer shows a blocking "Trust project folder?" dialog when launched via Codify in a project with `.pi/` settings or extensions (requires Pi 0.79+) (#2529)
...
- [Feature / Docs / Test/CI] Add `codify uninstall` for safely removing the CLI, installer-managed shell/config edits, and optionally local Codify state. (#2550)
...
- [UI / Bug fix] The file editor now follows Codify's selected web and desktop color theme instead of the operating-system theme. (#2594)
...
- [Chore / Breaking] `codify[memory]` is renamed to `codify[hindsight]`; the Hindsight memory extra is now the canonical name. (#2605)
- [Bug fix] `codify host status` now hides the session table by default (use `--sessions` to show it), making the command significantly faster. (#2606)
- [Feature] `codify://<hostname>/c/<session_id>` links open that session in the desktop app, reusing an open window on that server in-place (#2607)
...
- [Feature] Session creation requests from the web UI now send an explicit `X-Codify-Client` header, enabling the server to record the exact client surface (web/desktop/ios/android) in telemetry without relying on User-Agent parsing. (#2615)
...
- [Bug fix] Clicking an Codify session URL printed inside the embedded terminal now stays in the web app instead of opening a duplicate browser tab/window. (#2639)
...
- [Feature] `codify://<hostname>/c/<session_id>` links open that session in the iOS app, reusing the open window on that server in-place (#2661)
...
- [Feature] `codify setup` can now install Hermes directly before configuring its model provider. (#2751)
...
- [Bug fix] Native Codex and Claude now resolve their CLI from common global install dirs and an `CODIFY_CODEX_PATH` / `CODIFY_CLAUDE_PATH` override when it isn't on the host daemon's PATH (#2788)
...
- [Feature / Breaking] Per-harness startup command/args overrides via a polymorphic `harness:` config key + `CODIFY_<NAME>_PATH` env-var standardization (legacy `HARNESS_*_PATH` deprecated, slated for v0.8.0 removal) (#2933)
- [Bug fix] `codify[memory]` is restored as a backwards-compat alias for `codify[hindsight]` (the `memory` extra was dropped by #2605); it will be removed in 0.70. (#2938)
- [Feature] `CODIFY_SESSION_RENAME=on` re-enables the automatic first-turn session rename (disabled by default) (#2944)
...
- [Feature] `CODIFY_OIDC_SKIP_EMAIL_VERIFICATION=1` lets OIDC logins through when the IdP omits the `email_verified` claim (e.g. standard-tier Okta with directory-provisioned users) (#1859)
...
- [UI / Bug fix] "Back to Codify" from Settings now returns you to the conversation you were viewing instead of the home page (#1905)
...
- [Bug fix / Chore] Fixed harness workers (claude, codex, etc.) failing to start when codify is launched from a macOS or Linux GUI client due to a stripped PATH. Fix now lives in the Electron launcher (web/electron/src/main.js) per reviewer guidance. (#1935)
...
- [Feature / Test/CI] New official `codify-server-kubernetes` image ships the kubernetes sandbox provider SDK — the `sandbox-runners` overlay now works against published images, no custom build needed. (#2124)
...
- [UI / Feature] Pick a color theme (Codify, Dracula, GitHub, Catppuccin, or Gruvbox) in Appearance settings, independent of light/dark mode. (#2147)
...
- [Feature / Test/CI] Add a performance-benchmark harness for HTTP user journeys, with a seeded corpus, a SQLite+Postgres backend matrix, and a nightly workflow (`uv run dev/benchmarks/codify/run.py`) (#2159)
...
- [Feature / Test/CI] Add a performance-benchmark harness for HTTP + full-turn user journeys (`uv run dev/benchmarks/codify/run.py`), with a seeded corpus and SQLite+Postgres backend matrix (#2202)
- [UI / Bug fix] The new-session picker now remembers the host you last picked instead of resetting to the default. (#2218)
- [Bug fix] Fixed the Hermes `pre_tool_call` hook double-gating Codify relay tools, which parked a (#2220)
...
- [Feature] `codify debug logs` tails runner, server, or CLI diagnostic logs; `--session` scopes runner logs to a specific session across relaunches (#2273)
...
- [Feature] omnidev dev pods now get their own isolated `config.yaml` (seeded from `~/.codify/config.yaml`), so server-config edits while testing in a pod no longer touch your real config (#2360)
...
Highlights and full notes: <https://github.com/codify-ai/codify/releases/tag/v0.4.0>

## [v0.3.0] — 2026-06-26

Highlights and full notes: <https://github.com/codify-ai/codify/releases/tag/v0.3.0>

## [v0.2.0] — 2026-06-19

Highlights and full notes: <https://github.com/codify-ai/codify/releases/tag/v0.2.0>
